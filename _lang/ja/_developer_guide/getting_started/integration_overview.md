...
* [発見と計画](#discovery)：チームと協力してスコープを調整し、データとCampaignの構造を計画し、適切なワークスペース構造を作成します。
...
次に、CRMチームは計画したCampaignを実施するために必要なデータを定義し、データ要件を作成します。
...
| データが正しく保存されていることを確認するためにテストを実行します。 | ユーザーのSegmentを作成します。 |
| セッションデータがBraze内の意図したワークスペースに正しく帰属していることを確認します。 | CampaignとCanvasを正常に起動します。 |
| セッションの開始と終了が記録されていることを確認します。 | 正しいCampaignが正しいユーザーSegmentに表示されていることを確認します。 |
...
| 匿名ユーザープロファイルを作成します。 | プッシュCampaignがデバイスに正しく送信され、エンゲージメントが記録されているかテストします。 |
