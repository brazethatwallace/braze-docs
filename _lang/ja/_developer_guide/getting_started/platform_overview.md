...
![Brazeダッシュボードにある「最近の購入者」というSegmentと、「リンダへのおすすめ」というメールが表示された電話画面が並んでいる。]({% image_buster /assets/img/getting-started/getting-started-segment.png %}){: style="max-width:80%"}
...
| [プッシュ]({{site.baseurl}}/user_guide/channels/push) | iOS用のApple Push Notification Service（APNs）またはAndroid用のFirebase Cloud Messaging（FCM）を使用して、メッセージングCampaignsまたはニュースアイテムから自動的にプッシュ通知をトリガーします。 |
