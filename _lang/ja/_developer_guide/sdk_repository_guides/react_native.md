...
- ルートの`build.gradle`にまだ含まれていない場合は、**Kotlin Gradleプラグイン**を追加します（バージョンはReact Nativeのバージョンに依存します）。
...
開発中はBrazeのログを**ネイティブ**設定で有効にして、SDKがシステムコンソール（Xcode / Android Logcat）に書き込むようにしてください。これにより、初期化、ユーザーの変更、イベント配信を確認できます。
