...
> **信頼できる情報源：** ステップごとの画面、Gradle/CocoaPodsの変更、およびAndroid XMLキーの完全なリストは、[Braze React Nativeデベロッパーガイド](https://www.braze.com/docs/developer_guide/sdk_integration/?sdktab=react%20native)に記載されています。以下のスニペットは最小限の例です。
...
オプションの**SDK認証**：ダッシュボードで有効にしている場合、`changeUser`の第2引数として署名を渡すか、`Braze.setSdkAuthenticationSignature(signature)`を呼び出します。
...
**`enableSDK()`** — **`disableSDK()`**の後にSDKを再度有効にします。**iOS**では、再有効化が**次回のアプリ起動時**まで適用されない場合があります。即時の再有効化に依存する前に、Braze Swift/iOSのドキュメントで確認してください。
