...
プッシュプライマーCampaignsは、アプリのデバイスでプッシュを有効にするようユーザーに促します。ユーザーのデバイスに直接メッセージを送信する許可を得るのは複雑な場合がありますが、当社のガイドが役立ちます。このガイドでは、開発者がプッシュプライミングを統合するために行う必要のあるステップを示します。
