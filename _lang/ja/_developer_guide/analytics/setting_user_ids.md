...
article_title: ユーザー IDを設定する
page_order: 1.1
description: "Braze SDKでユーザー IDを設定する方法について説明します。"
---

# ユーザー IDを設定する {#set-user-ids}

> Braze SDKでユーザー IDを設定する方法について説明します。これは、デバイスやプラットフォームを超えてユーザーを追跡し、[ユーザーデータAPI]({{site.baseurl}}/api/endpoints/user_data)を通じてユーザーデータをインポートし、[メッセージングAPI]({{site.baseurl}}/api/endpoints/messaging)を通じてターゲットメッセージを送信するための一意の識別子です。ユーザーに固有のIDを割り当てない場合、Brazeは代わりに匿名IDを割り当てますが、割り当てるまでこれらの機能を使用することはできません。
...
### 匿名ユーザートラッキングの防止 {#preventing-anonymous-user-tracking}
...
ユーザー IDを設定するには、ユーザーが最初にログインした後に `changeUser()` メソッドを呼び出します。IDは一意である必要があり、[命名のベストプラクティス](#naming-best-practices)に従う必要があります。
...
![Brazeアクションタグの設定を示すダイアログボックス。設定には「tag type」と「external user ID」が含まれています。]({% image_buster /assets/img/web-gtm/gtm-change-user.png %})
{% endtab %}
...
### `changeUser()` の動作 {#how-changeuser-works}
...
- 匿名ユーザーが**新しい**ユーザー ID（Brazeにまだ存在しないもの）で `changeUser()` を呼び出すと、匿名プロファイルのデータが新しい識別済みプロファイルに統合されます。
- 匿名ユーザーが**既存の**ユーザー IDで `changeUser()` を呼び出すと、匿名プロファイルのデータは識別済みプロファイルに統合されません。
