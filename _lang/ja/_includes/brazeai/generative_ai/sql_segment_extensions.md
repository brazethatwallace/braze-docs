...
### ステップ1: エディターを選ぶ {#step-1-choose-an-editor}
...
{% alert tip %}
いずれかのSQLエディターで作成されたすべてのSQLセグメントを手動で完全更新できます。
{% endalert %}
...
2. エディターの上のフィールドから**オペレーター**、**回数**、および**期間**を選択して、スキーマの詳細を追加します。クエリでは、集計列の合計が{% raw %}`{{operator}}`および`{{number of times}}`{% endraw %}プレースホルダーで指定された特定の条件を満たすかどうかが確認されます。これは、従来のセグメントエクステンションを作成するワークフローと同様に機能します。<br><br>
   - **オペレーター：** イベントの発生回数が、指定した回数よりも多いか、少ないか、等しいかを示します。<br>
   ![オペレーターフィールドで「より大きい」が選択されている。]({% image_buster /assets/img_archive/sql_segments_operator.png %})<br><br>
   - **回数：** オペレーターに対してイベントを評価したい回数。<br>
...
インクリメンタルSQLセグメントエクステンションの場合、プレビューにはオペレーター、回数、および期間フィールドからの追加条件は含まれません。

### ステップ4: SQLを反転させる必要があるかどうかを判断する {#step-4-determine-if-you-need-to-invert-sql}
