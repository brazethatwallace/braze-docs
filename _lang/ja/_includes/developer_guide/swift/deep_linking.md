...
### ステップ1:スキームを登録する {#register-a-scheme}
...
### ステップ2:スキームの許可リストを追加する {#step-2-add-a-scheme-allowlist}

`LSApplicationQueriesSchemes`キーをアプリのInfo.plistファイルに追加して、`canOpenURL(_:)`に渡すURLスキームを宣言する必要があります。この許可リストに含まれないスキームを呼び出そうとすると、デバイスのログにエラーが記録され、ディープリンクは開かれません。以下はこのエラーの例です。
...
Xcode 27以降でビルドされたアプリは[`UIScene`ライフサイクル](https://developer.apple.com/documentation/technotes/tn3187-migrating-to-the-uikit-scene-based-life-cycle)の採用が必須となるため、iOSはカスタムスキームURLを`AppDelegate`ではなく`SceneDelegate`の[`scene:openURLContexts:`](https://developer.apple.com/documentation/uikit/uiscenedelegate/scene(_:openurlcontexts:))を通じて配信します。重要な引数は[NSURL](https://developer.apple.com/library/ios/DOCUMENTATION/Cocoa/Reference/Foundation/Classes/NSURL_Class/Reference/Reference.html#//apple_ref/doc/c_ref/NSURL)オブジェクトです。
...
{% alert note %}
アプリがまだ`UIScene`ライフサイクルを採用していない場合、iOSは代わりに`AppDelegate`の[`application:openURL:options:`](https://developer.apple.com/reference/uikit/uiapplicationdelegate/1623112-application?language=objc)を呼び出します。
...
{% tab 完全に無効化 %}
ATSを完全に無効にできます。ただし、セキュリティ保護が失われることと、将来のiOSとの互換性の両方を考慮して、この方法は推奨されません。ATSを無効にするには、アプリの`Info.plist`ファイルに以下を挿入します。
...
エンコードされたリンクをデコードするには、`String`プロパティ[`removingPercentEncoding`](https://developer.apple.com/documentation/swift/stringprotocol/removingpercentencoding)を使用します。また、`BrazeDelegate.braze(_:shouldOpenURL:)`で`true`を返す必要があります。アプリによるURLの処理をトリガーするには、アクションの呼び出しが必要です。たとえば、ステップ3の[`scene:openURLContexts:`](#step-3-implement-a-handler)ハンドラで以下のようにします。
...
3. カスタムスキームベースのディープリンクを使用している場合は、`scene:openURLContexts:`ハンドラに次のコードを追加します。
...
有効にすると、Brazeは`UIScene`ライフサイクルを採用しているアプリ（Xcode 27以降でビルドされたアプリでは必須）の場合は[`scene:continue:`](https://developer.apple.com/documentation/uikit/uiscenedelegate/scene(_:continue:))メソッドを介して`SceneDelegate`に、それ以外の場合は[`application:continueUserActivity:restorationHandler:`](https://developer.apple.com/documentation/uikit/uiapplicationdelegate/1623072-application)を介して`AppDelegate`にユニバーサルリンクを転送します。
...
シミュレータービルドのサポートを追加するには、アプリケーションの`.entitlements`ファイルを_Copy Bundle Resources_ビルドフェーズに追加します。詳細については、[`forwardUniversalLinks`](https://braze-inc.github.io/braze-swift-sdk/documentation/brazekit/braze/configuration-swift.class/forwarduniversallinks)のドキュメントを参照してください。
{% endalert %}
