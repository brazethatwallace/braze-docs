...
[`roRegistry.Delete()`](https://developer.roku.com/docs/references/brightscript/components/roregistry.md)を使用して各Brazeセクションを削除し、`Flush()`を呼び出して変更を永続化します。
...
再度[Braze SDKを初期化]({{site.baseurl}}/developer_guide/sdk_integration/?sdktab=roku)すると、SDKは欠落しているレジストリデータを適切に処理します。
