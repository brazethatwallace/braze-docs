...
テストプッシュ通知またはアプリ内メッセージを送信するには、以前に作成したテストセグメントをターゲットにする必要があります。まずキャンペーンを作成し、通常のステップに従います。**ターゲットオーディエンス**ステップに到達したら、ドロップダウンメニューからテストセグメントを選択します。
...
{% alert note %}
1つのキャンペーンを使用して自分自身にテストメッセージを複数回送信する場合は、キャンペーンコンポーザーの**配信をスケジュール**部分で**ユーザーがキャンペーンを再度受信できるようにする**を選択してください。
{% endalert %}
{% endsubtab %}
...
| プレースホルダー | 説明 |
|---------------------|-----------------------------------------------------------|
| `BRAZE_API_KEY` | 認証に使用するBraze APIキー。Brazeで、**設定** > **APIキー**に移動してキーを見つけます。 |
| `EXTERNAL_USER_ID` | 特定のユーザーにメッセージを送信するために使用する外部ユーザーID。Brazeで、**オーディエンス** > **ユーザーを検索**に移動し、ユーザーを検索します。 |
