...
CampaignまたはCanvasに表示されるターゲット層のサイズは、[Segmentの到達可能なオーディエンスサイズ]({{site.baseurl}}/user_guide/engagement_tools/segments/measuring_segment_size/#segment-membership-calculation)とは異なる場合があります。これは、そのSegmentを追加のフィルターなしでCampaignまたはCanvasに直接追加している場合でも同様です。
これにはいくつかの理由が考えられます。

- グローバルコントロールグループがCampaignまたはCanvasに適用されている場合、そのグローバルコントロールグループ内のユーザーは到達可能なユーザー数から除外されます。
- CampaignまたはCanvasのターゲット層のサイズは、さまざまなメッセージチャネルを通じて連絡できないユーザーを除外します。この動作はチャネルごとに異なります。たとえば、CampaignまたはCanvasの到達可能なオーディエンスでは、配信停止済み、スパムとしてマークされた（メールの場合）、またはハードバウンスした（メールの場合）ユーザーが除外されます。ただし、Segment自体は、到達可能なメールユーザーの推定数を表示する際にオプトアウトのみを除外します。
- Brazeは選択したサブスクリプショングループ内のユーザーにのみSMSメッセージを送信するため、CampaignまたはCanvasのSMSターゲット層からは、選択したサブスクリプショングループに含まれないユーザーも除外されます。
...
![「このエクステンションはアクティブなCampaign、Canvas、またはSegmentで使用されていないため、スケジュールされた更新がオフになりました。セグメントエクステンションは2025年2月23日午前12:00に無効化されました。」という通知。]({% image_buster /assets/img/segment/segment_extension_disabled.png %})
...
メッセージが受信、開封、またはクリックされると、Brazeはインタラクションを記録したプロファイルと同じチャネル識別子を共有するすべてのプロファイルのデータを更新します（例えば、メールの場合は同じメールアドレス、SMSやWhatsAppの場合は同じ電話番号）。メッセージを受信、開封、またはクリックした人と識別子を共有するユーザーは、元々Campaignに含まれていなかったり、メッセージが直接送信されていなかったりしても、このフィルターに一致する場合があります。
