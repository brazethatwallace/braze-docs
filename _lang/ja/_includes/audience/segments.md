...
**更新間隔設定**パネルでは、このセグメントエクステンションが更新される頻度を、毎時、毎日、毎週、または毎月から選択できます。また、更新が行われる特定の時刻（会社のタイムゾーン）を選択する必要があります。例えば以下のようになります。
...
- [分析トラッキング]({{site.baseurl}}/user_guide/analytics/tracking/segment_analytics_tracking)が有効になっているセグメントで使用されていない
- 7日以上変更されていない
...
{% if include.section == "キャンバス variant archived segment" %}

### アーカイブされたセグメントが原因でキャンバスのバリアントを削除できない {#cant-delete-a-canvas-variant-because-of-an-archived-segment}

セグメントフィルターがまだそのバリアントを参照しているために、Brazeがキャンバスのバリアントの削除をブロックしている場合は、その参照を使用しているセグメント（アーカイブされたセグメントを含む）を開き、フィルターからバリアントを削除してください。セグメントを保存した後、キャンバスに戻ってバリアントの削除を再試行してください。

キャンバスを参照しているセグメントを見つけるには、キャンバスを開いてオーディエンスフィルターを確認するか、各セグメントの[メッセージング使用状況]({{site.baseurl}}/user_guide/audience/segments/managing_segments#messaging-use)セクションでリンクされているキャンバスを確認してください。
