...
いいえ、メディアライブラリ内のアセットにストレージ制限はありません。ただし、アセットにはサイズ制限があります（最大5 MB）。
...
メディアライブラリアセットのバニティURLはサポートされていません。カスタムURLを使用するとCDN配信が中断されるためです。キャンペーンがすでにそのURLを参照している場合は、既存のURLで画像を置き換えることができます。詳細については、[ファイルを置き換える]({{site.baseurl}}/user_guide/messaging/design_and_edit/media_library#replace-a-file)を参照してください。
