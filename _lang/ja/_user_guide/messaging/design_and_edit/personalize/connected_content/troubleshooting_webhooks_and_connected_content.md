...
以下の表で症状を確認し、該当するセクションに移動してください。

| 症状 | 移動先 |
| --- | --- |
| メッセージアクティビティログの `4XX` クライアントエラー | [4XX エラー](#4xx-errors) |
| `5XX` サーバーエラーまたはタイムアウト | [5XX エラー](#5xx-errors) |
| `598 Host Unhealthy` またはリクエストの一時停止 | [異常なホストの検出]({{site.baseurl}}/support_contact) |
...
3. `5XX` エラーの場合、エンドポイントの稼働状況、レート制限、およびBrazeがホストを異常と判定していないかを確認します。
...
| **598 Host Unhealthy**        | エンドポイントのホストが一時的に異常と判定されたため、Brazeがレスポンスをシミュレートしました。詳細については、[異常ホスト検出](#unhealthy-host-detection)を参照してください。 |
...
- **リダイレクト応答（`3xx`）:** コネクテッドコンテンツはリダイレクトに従いません。`2xx`応答のみが成功として扱われるため、`301`や`302`は同じURLがPostmanで動作する場合でも空白として表示されることがあります。最終的なリダイレクト先URLを使用するか、Brazeが呼び出すURLで`2xx`応答（通常は`200`）を返すようにエンドポイントを設定してください。[エンドポイントがリダイレクトを返す場合にコネクテッドコンテンツが失敗するのはなぜですか？]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/connected_content/making_an_api_call#why-does-connected-content-fail-when-my-endpoint-returns-a-redirect-301-or-302)を参照してください。
- **HTTPエラーと空のボディ:** `2xx`範囲外のステータスコードやブロックされたホストの場合、コネクテッドコンテンツは空の文字列をレンダリングすることがあります。[API呼び出しの実行]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/connected_content/making_an_api_call)を参照し、**メッセージアクティビティログ**で失敗を確認してください。
