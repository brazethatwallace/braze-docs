...
{% alert important %}
永続的なエントリプロパティは、元のCanvasエディターのアーティファクトであるため、Canvasエントリプロパティなどの用語への非推奨の参照が歴史的な参考として残っています。現在の更新されたCanvasエディターについては、[コンテキストとイベントプロパティ]({{site.baseurl}}/user_guide/messaging/canvas/create_a_canvas/context_and_event_properties/)を参照してください。
{% endalert %}
