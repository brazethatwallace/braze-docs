...
{% alert note %}
実験の結果が不十分で完了した場合、パーソナライゼーションが最もパフォーマンスの高い単一のパスに送るよりも優れた結果をもたらさないとモデルが判断したため、**初期実験**タブのみが表示されます。詳細については、[分析](#analytics)を参照してください。
{% endalert %}
...
1つの実験パスコンポーネントには最大4つのパスを含めることができます。ただし、1回限りの送信キャンバスでは、パーソナライズ済みパスがオンの場合、最大3つのパスを追加できます。4番目のパスは、Brazeが自動的に実験に追加する遅延グループ用に予約する必要があります。
...
最初のユーザーが実験に入ったら、キャンバスを確認して分析データが入ってくるのを確認し、[実験のパフォーマンスを追跡]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/experiment_step#tracking-performance)できます。
...
実験の結果が不十分で完了した場合（例えば、モデルの予測リフトが0.5%のしきい値未満である場合や、意味のあるユーザーセグメントが特定されない場合）、パーソナライゼーションが最もパフォーマンスの高い単一のパスに送るよりも優れた結果をもたらさないとモデルが判断したため、**初期実験**タブのみが表示されます。この場合、設定したフォールバック動作が適用され、パーソナライズ済みパスの分析は利用できません。
