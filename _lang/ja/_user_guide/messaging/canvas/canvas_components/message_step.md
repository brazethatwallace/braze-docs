...
配信バリデーションは、メッセージ送信時にオーディエンスが配信基準を満たしていることを確認するための追加チェックを提供します。この設定は、サイレント時間、インテリジェントタイミング、またはレート制限が有効になっている場合に推奨されます。**メッセージ送信時にオーディエンスを検証**を選択し、セグメントまたは追加のフィルターを追加します。ユーザーがバリデーションを満たさない場合、Canvasを終了するか次のステップに進むかを選択します。
