...
コンテンツオプティマイザーエージェントは、このバリアントで指定された送信設定（メールドメインや返信先アドレスなど）を使用してすべてのメッセージを送信します。新しいデザインから始めることも、このメッセージ用に既存のテンプレートを選択することもできます。このステップでは、メッセージのどのコンポーネントを最適化したいかを検討してください。これらは[ステップ4](#step-4)で定義します。
...
コンテンツオプティマイザーエージェントは、このバリアントで指定された選択済みのプッシュプラットフォームを使用してすべてのメッセージを送信します。新しいデザインから始めることも、このメッセージ用に既存のテンプレートを選択することもできます。このステップでは、メッセージのどのコンポーネントを最適化したいかを検討してください。これらは[ステップ4](#step-4)で定義します。
...
コンテンツオプティマイザーエージェントは、このバリアントで指定された**Content**と**Message**の詳細を使用してすべてのメッセージを送信します。新しいデザインから始めることも、このメッセージ用に既存のテンプレートを選択することもできます。このステップでは、メッセージのどのコンポーネントを最適化したいかを検討してください。これらは[ステップ4](#step-4)で定義します。
...
| 状態 | 意味 |
| --- | --- |
| 学習中 | エージェントはまだコンテンツバリアント全体のパフォーマンスデータを収集しており、一貫した信頼性のある勝者をまだ見つけていません。 |
| 最適化中 | エージェントは他のバリアントを一貫して上回るバリアントを見つけ、勝利する組み合わせに配信をシフトしています。 |
| アクション推奨 | 明確な勝者が現れないままステップがしばらく実行されています。エージェントが勝者を見つけられるよう、ステップの設定を見直してください。 |
...
- 可能であれば、キャンバスに入るユーザー数を増やしてください。送信数が多いほど、エージェントが学習するためのデータが増えます。
- 一般的に、少ない組み合わせよりも多くの組み合わせをテストしてください（[ベストプラクティス](#best-practices)を参照）。これにより、エージェントは何が勝っているかについてより明確なシグナルを得られます。オーディエンスのボリュームが少ない場合（1日あたり平均約3,000送信未満）、組み合わせが多すぎるとボリュームに対して学習が遅くなる可能性があるため、バリアント数をやや減らすことを検討してください。
