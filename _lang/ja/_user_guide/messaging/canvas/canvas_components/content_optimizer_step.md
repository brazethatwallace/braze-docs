...
Canvasが起動された後、Canvasエディターでステップを開いて、実行中のコンテンツオプティマイザーステップを更新できます。以下のことが可能です：
...
変更を公開すると、オプティマイザーはリセットされ、すべてのアクティブなバリアントと組み合わせにわたってトラフィックの再割り当てをゼロから開始します。編集前の履歴データは保持され、**Content Analytics**タブで確認できます。
...
- 早すぎる更新は避けてください。変更を公開するたびに、オプティマイザーはゼロからリセットされます。ステップが何が効果的かを特定する前に更新すると、その発見を活用することができなくなり、ステップは最もパフォーマンスの高い組み合わせに意味のあるトラフィックをシフトする機会を得られません。
- 更新の準備ができたら、パフォーマンスの低いバリアントを無効化し、トップパフォーマーの特徴を活かした新しいバリアントを追加するのが良いアプローチです。
