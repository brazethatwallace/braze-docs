...
[オブジェクト配列]({{site.baseurl}}/user_guide/data/activation/attributes/array_of_objects)は、ユーザーのプロファイルに保存されるデータが豊富なカスタム属性です。ユーザーとブランドとのインタラクション履歴を作成したり、購入履歴やLTV or 生涯価値の合計などの計算フィールドに基づいてセグメントを作成したりできます。
