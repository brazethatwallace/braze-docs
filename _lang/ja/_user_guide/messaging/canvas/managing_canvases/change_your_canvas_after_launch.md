...
- 既存のレート制限により、ユーザーがメッセージを受け取れない
  - アクティブなキャンバスのレート制限を更新すると、新しいレート制限は、キャンバスにすでにいるユーザーを含め、今後のすべてのメッセージ送信に適用されます。ただし、内部キャッシュ（最大30秒）により、新しいレート制限が完全に適用されるまでに短い遅延が発生する場合があります。Brazeはユーザーが現在いるメッセージステップのキューにユーザーを追加するため、各ステップのメッセージが実際に送信される時点で有効なレート制限が適用されます。
