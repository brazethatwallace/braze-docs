...
{% alert important %}
スケジュールの評価とオーディエンスの評価は、スケジュールエントリと即時送信エントリで異なる場合があります。たとえば、ステップが送信時刻に近いタイミングで評価するオプションを使用している場合などです。スケジュールされたエントリまたは送信ウィンドウの数分以内にオーディエンス、スケジュール、または配信設定を編集する場合は、次のステップのキューにすでに入っているユーザーがその変更を反映するかどうかを確認してください（[起動後のCanvasの変更]({{site.baseurl}}/user_guide/messaging/canvas/managing_canvases/change_your_canvas_after_launch/)を参照）。不明な場合は、停止してから複製し、再起動してクリーンな再評価を行ってください。
{% endalert %}
