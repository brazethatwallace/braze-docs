...
- **メッセージ:** キャンペーンのメッセージです。
- **配信:** 配信タイプであり、ユーザーがキャンペーンを受信するタイミングを決定します。
- **ターゲットオーディエンス:** キャンペーンを受信するユーザーを決定します。
- **コンバージョンイベント:** エンゲージメントとレポート目的で追跡する指標です。
{% endtab %}
...
- **コンバージョンイベント:** エンゲージメントとレポート目的で追跡する指標です。
- **エントリスケジュール:** エントリスケジュールのタイプと、ユーザーがキャンバスに入るタイミングを含みます。
- **ターゲットオーディエンス:** このキャンバスに入るユーザーを決定します。
- **送信設定:** キャンバス内のすべてのステップの送信オプションです。
- **キャンバスの構築:** キャンバスのユーザージャーニーです。
{% endtab %}
{% endtabs %}
...
- **レビューサマリー**ステップでセクションの**承認**をクリックすると、その承認は即座に保存されます。
- **保存**ボタンは、キャンバスのコンテンツと設定の変更を保存するものであり、承認ステータスを保存するものではありません。

承認を失わないようにするには:

1. 必要なキャンバスの編集を行い、**保存**をクリックします。
2. キャンバスの保存が完了した後、**レビューサマリー**ステップで該当するセクションを承認します。
3. 承認後にキャンバスをさらに変更した場合のみ、再度**保存**をクリックします。キャンバスを変更して保存すると、すべての承認ステータスが**承認待ち**にリセットされます。
{% endalert %}
{% endtab %}
{% endtabs %}
