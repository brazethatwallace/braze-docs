...
キャンペーンの設定時に最適化として**勝者バリアント**を選択した場合、キャンペーン分析の**AB テスト結果**という追加タブにアクセスできます。勝者バリアントがテストの残りのユーザーに送信された後、このタブにはその送信の結果が表示されます。
...
AB テスト送信を含むキャンペーン全体での勝者バリアントのパフォーマンスを確認するには、**キャンペーン分析**ページを確認してください。

### パーソナライズ済みバリアント {#personalized-variant}

キャンペーンの設定時に最適化として**パーソナライズ済みバリアント**を選択した場合、**AB テスト結果**は**初期テスト**と**パーソナライズ済みバリアント**の2つのタブに分かれています。
...
![ユニーク開封数に最適化されたキャンペーンのパーソナライズ済みバリアントタブ。カードには、予測リフト、全体的なユニーク開封数（パーソナライズ済みバリアント使用時）、予測ユニーク開封数（勝者バリアント使用時）が表示されます。]({% image_buster /assets/img_archive/ab_analytics_pv_1.png %})
...
AB テストまたは多変量分割を含むアプリ内メッセージキャンペーンでは、設定した割合は割り当てターゲットです。レポートされるインプレッションがこれらの割合と正確に一致することはほとんどありません。これは、トリガーアクションを実行したユーザーのみがインプレッションを記録し、コントロールグループのユーザーはトリガーを実行するとメッセージを表示しなくてもインプレッションが記録されるためです。
