...
ランダムオーディエンスの再エントリは、[ABテスト]({{site.baseurl}}/user_guide/messaging/ab_testing#what-are-multivariate-and-ab-testing)やキャンペーンで特定のユーザーグループをターゲットにする場合に役立ちます。ランダムバケット番号を使用してランダムオーディエンスの再エントリを行うには、以下の手順に従ってください。
...
3. キャンペーンまたはキャンバスの**Target Audiences**セクションで、ランダムバケットの設定を指定します。これにより、Brazeは定義されたパーセンテージに基づいてユーザーを適切なバケットに自動的に割り当てます。
