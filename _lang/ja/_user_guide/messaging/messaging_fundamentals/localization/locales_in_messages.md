...
| 機能 | 必要なユーザー権限 |
| --- | --- |
| テンプレート | ロケールと翻訳を追加したいテンプレートタイプに応じて、以下の権限が必要です。<br><br> {::nomarkdown} <ul><li>Edit Email Templates</li><li>Edit IAM Templates</li><li>Edit Content Block Templates</li></ul>{:/} |
{: .reset-td-br-1 .reset-td-br-2 aria-label="前提条件" }
...
{% alert tip %}
翻訳したいテキストをハイライトし、キーボードショートカット**Cmd + Alt + L**（macOS）または**Ctrl + Alt + L**（Windows）を使用して翻訳タグで囲みます。<br><br>このショートカットは、メールおよびContent Blocksのドラッグ＆ドロップエディターを除く、多言語メッセージングをサポートするすべてのチャネルで機能します。それらのエディターでは、**パーソナライゼーションを追加**ボタンを使用して翻訳タグを追加してください。
{% endalert %}
...
![保存された翻訳を持つContent Blocksのリストが表示された言語を管理セクション。]({% image_buster /assets/img/multi-language_support/content_blocks_translations.png %}){: style="max-width:70%;"}

{% alert important %}
各Content Blockに、メッセージに追加したすべてのロケールの翻訳が含まれていることを確認してください。Content Blockに追加したロケールの翻訳が欠けている場合、そのロケールのユーザーには元の言語で表示されます。
{% endalert %}
...
**テンプレートをダウンロード**を選択して、選択した翻訳IDとロケールのマトリクスを含むCSVをダウンロードします。各ロケールの翻訳を入力します。完成したファイルをアップロードすると、翻訳がメッセージに適用されます。
...
保存された翻訳はContent Blockに関連付けられたままになります。ブロックがメッセージに追加されると、その翻訳も自動的に含まれます。
...
多言語メールテンプレートのダウンロード時にエラーが発生する場合、翻訳タグがBrazeのメール本文処理と競合するHTML属性やCSSスタイリングを囲んでいる可能性があります。
...
- [HTML属性と構造](#html-attributes-and-structure)で説明されているように、人間が読むテキストコンテンツのみを翻訳タグで囲んでください。
...
いいえ。ベストプラクティスとして、人間が読むテキストやローカライズが必要なコンテンツのみを囲むべきです。これにより、フォーマット、リンク、その他のテキスト以外の要素が壊れるのを防ぐことができます。

さらに、正確な翻訳を作成し、パフォーマンスやサイズの制限を回避するために、意味的に関連する小さなテキストの単位で囲むことを検討してください。
...
| 翻訳タグに完全なURLまたはLiquidで生成されたURLが含まれている。 | リンク切れやリンクトラッキングの問題が発生した場合に備えて、URLを含む翻訳タグが識別されます。 |
| 翻訳タグにクエリパラメーターが含まれている。 | リンク切れやリンクトラッキングの問題が発生した場合に備えて、クエリパラメーターを含む翻訳タグが識別されます。 |
| 翻訳タグにHTML属性または構造が含まれている。 | スタイルやフォーマットの問題が発生した場合に備えて、HTML属性または構造を含む翻訳タグが識別されます。 |
