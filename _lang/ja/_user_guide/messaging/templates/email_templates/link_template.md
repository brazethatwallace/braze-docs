...
> リンクテンプレートを使用すると、パラメーターの追加やURLの前置によって、メールCampaign用のダイナミックで再利用可能なリンクを作成できます。これにより、CampaignやメッセージのURL全体で一貫性を確保できます。
...
例えば、新製品の発売に向けたプロモーションメールCampaignを実施しているとします。リンクテンプレートを使用して、ユーザーを製品ページに誘導し、ユーザーの名前や特定のプロモーションコードを含むようにリンクをパーソナライズできます。これにより、リンクをクリックしたユーザー数や購入に至ったユーザー数を追跡できます。このように、リンク全体の一貫性を確保し、分析をより効果的に追跡できます。
...
5. （オプション）トグルを選択して、メールCampaignやCanvasesのリンクにリンクテンプレートを自動的に追加します。これは、新規または既存のメールに新しいリンクを追加する際に適用されます。
...
## メールCampaignでのリンクテンプレートの使用 {#using-link-templates-in-email-campaigns}
