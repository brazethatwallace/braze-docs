...
- エントリスケジュールはAPIトリガーです。ユーザーがカートを放棄した際にエントリさせるには[`/canvas/trigger/send`エンドポイント]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_triggered_canvases/)を使用するか、**カスタムイベントを実行**や**カート更新イベントを実行**などのトリガーを使用したアクションベースのスケジュールに切り替えてください。
...
ステップバイステップのウォークスルーについては、[放棄カート]({{site.baseurl}}/user_guide/messaging/templates/canvas_templates/braze_templates/abandoned_cart/)を参照してください。
