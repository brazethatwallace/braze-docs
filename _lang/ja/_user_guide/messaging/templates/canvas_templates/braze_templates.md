...
> Brazeには、一般的なユースケースのベストプラクティスとして参照・使用できるキャンバステンプレートが用意されています。これらのテンプレートは編集できませんが、**コンテンツ** > **キャンバス** > **Brazeテンプレート**で確認したり、キャンバスで使用したりできます。
...
- **購入した？**アクションパスステップで購入を行うと、ユーザーはキャンバスを退出します。このテンプレートは、購入後のジャーニーが別途あることを前提としています。
- キャンバスには**アイテム別リマインダー**のメール、遅延ステップ、メールとSMSのインテリジェントチャネル分割、Content Cards付きのチャネルメッセージ（メール、SMS、アプリ内メッセージ）、およびオーディエンス同期ステップが含まれています。パートナーとオーディエンスで**広告リターゲティング**を設定してください。
