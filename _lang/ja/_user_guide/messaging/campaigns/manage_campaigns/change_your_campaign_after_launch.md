...
[配信速度レート制限]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/frequency_capping/#delivery-speed-rate-limiting)を使用しているキャンペーンを一時停止すると、Brazeは分単位のスロットに送信を分散します。**Resume**を選択しても、キャンペーンが一時停止されている間にキャンセルされたスロットのメッセージは再送信されず、キャンペーンの再開時にすべてのメッセージが必ずしも送信されるわけではありません。

キャンペーンが一時停止されていたためにメッセージを受信しなかったユーザーがいる場合は、**Resume**で未送信メッセージを配信しようとするのではなく、キャンペーンを複製してそれらのユーザーのみをターゲットにしてください。
