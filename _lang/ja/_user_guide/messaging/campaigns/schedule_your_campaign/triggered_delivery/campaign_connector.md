...
{% alert important %}
Campaign Connectorのトリガーは、アプリ内メッセージキャンペーンのトリガーには使用できません。アプリ内メッセージは、カスタムイベントやセッション開始などのSDKイベントによってのみトリガーできます。詳細については、[アプリ内メッセージの作成]({{site.baseurl}}/user_guide/channels/in_app_messages/traditional/)を参照してください。
{% endalert %}
