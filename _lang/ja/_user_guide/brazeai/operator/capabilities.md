...
page_type: reference
toc_headers: h2
description: "このリファレンス記事では、BrazeAI Operator™がダッシュボード全体でできることについて説明します。キャンペーン、キャンバス、セグメント、レポート、ダッシュボード、エージェントの構築、コピー・メッセージ・Liquid・画像の生成、データ変換、コンテンツ品質のレビュー、情報の検索などが含まれます。"
---
...
オペレーターは、ジャーニーのアイデアから[キャンバス]({{site.baseurl}}/user_guide/messaging/canvas/create_a_canvas)の下書き作成、さらに既存のキャンバスの改善までをサポートします。オペレーターが提案する変更はすべてアクションカードとして表示され、保存前に確認できます。

ジャーニーを自然言語で記述してください。オペレーターがエントリ条件、ステップ、遅延、メッセージを含む下書きを組み立てます。また、ステップの追加やメッセージコンテンツの更新など、既存のキャンバスの編集をオペレーターに依頼することもできます。キャンバスビルダーで下書きを確認し、フォローアッププロンプトで調整してから公開してください。

たとえば、カート放棄から1時間後にメールリマインダーを送信し、ユーザーがまだ購入していない場合は24時間後にプッシュを送信するカート放棄ジャーニーの構築をオペレーターに依頼できます。

これはどのダッシュボードページからでも開始できます。まだキャンバスのページにいない場合、オペレーターがリクエストを完了するためにそのページに[ナビゲート](#navigate-the-dashboard)します。
...
- 画像生成は、他のオペレーターアクションとともに、会社全体の1日あたりのオペレーター使用制限にカウントされます。詳細については、[制限事項](#limitations)を参照してください。
...
- **不快な言語：** 不快または不適切な言語がないかスキャンし、メッセージングを敬意あるものに保つために修正できるようにします。
...
- **フラグされた不快な言語を再確認する：** 不快な言語の検出は堅牢に設計されていますが、誤検出が発生する場合があります。フラグされたセクションを慎重にレビューし、必要に応じて適切な変更を行ってください。
...
{% alert note %}
オペレーターの対応範囲は頻繁に変更されます。特定の画面やワークフローがサポートされているかどうかわからない場合は、オペレーターに直接聞いてください。
{% endalert %}

オペレーターのダッシュボードサポートは広範ですが、制限があります。

- **キャンバス：** オペレーターは現在のキャンバスエディターで[キャンバスを作成・編集](#canvases)できます。[オリジナルのキャンバスエディター]({{site.baseurl}}/user_guide/messaging/canvas/create_a_canvas)、テンプレート選択ページからのキャンバス開始、キャンバス構築中の**ユーザーとしてプレビュー**の使用はサポートされていません。オペレーターは、既存のキャンバスの設定（ターゲティングや配信設定など）を参照して、質問に回答したり出力の根拠とすることができます。
- **キャンペーンの複製：** オペレーターはキャンペーン一覧ビューから既存のキャンペーンを複製することはできません。類似のキャンペーンを作成するには、オペレーターに新規作成を依頼するか、一覧ビューの**その他のアクション**メニューから手動でキャンペーンを複製してください。
- **ドラッグ＆ドロップエディター：** オペレーターは、[メール]({{site.baseurl}}/user_guide/channels/email/drag_and_drop)、[バナー]({{site.baseurl}}/user_guide/channels/banners/create_a_banner#compose-a-banner)、[アプリ内メッセージ]({{site.baseurl}}/user_guide/channels/in_app_messages/drag_and_drop)のドラッグ＆ドロップエディターでメッセージデザインを直接生成・挿入することはできません。対応するHTMLエディターに切り替えてオペレーターを使用するか、コピーなどのコンテンツを生成して手動で貼り付けるようオペレーターに依頼してください。対応するチャネルとエディターについては、[メッセージの生成](#generate-messages)を参照してください。
- **画面の可視性：** オペレーターは、対応するプレビューやエディター内のコンテンツを含め、表示中のページを理解するためにページ対応コンテキストを使用します。オペレーターが読み取れない部分がある場合、推測する代わりにその旨を通知するため、そのコンテンツを自分で説明する必要があることがわかります。
- **使用制限：** オペレーターには、24時間ごとにリセットされる会社全体の1日の使用制限があります。すべてのオペレーターアクションがこの制限にカウントされ、使用量はオペレーターが読み取り・生成する量に応じて変動します。質問、情報の検索、[サポートチケットの提出]({{site.baseurl}}/user_guide/brazeai/operator/support_tickets)は比較的軽い使用量です。キャンペーンやセグメントなどのオブジェクトの作成や編集は、より多くの使用量を消費します。[画像生成](#generate-images)もこの制限にカウントされます。制限に達すると、「1日の使用制限に達しました」というメッセージが表示され、リセットされるまでオペレーターはリクエストを処理しません。トラブルシューティングの手順については、[トラブルシューティング]({{site.baseurl}}/user_guide/brazeai/operator/troubleshooting)を参照してください。
