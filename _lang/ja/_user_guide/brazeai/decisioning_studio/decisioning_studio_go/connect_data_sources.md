...
- **オーディエンスデータ**は、CEP（BrazeまたはSalesforce Marketing Cloud）で定義されたSegmentsまたはリストから直接取得され、特定の事前定義された属性のみを含めることができます（1Pデータは含まれません）
...
{% alert important %}
比較対象とするCampaigns（通常運用のCampaignsを含む）のデータが、Braze Currentsのエクスポートに含まれていることを確認してください。
{% endalert %}
