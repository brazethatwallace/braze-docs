...
## AI項目レコメンデーションの作成 {#creating-an-ai-item-recommendation}
...
![「レコメンデーションを定義する」ステップ。タイプに「AI Personalized」を選択し、「Do not recommend items users have previously interacted with」オプションを有効にしています。]({% image_buster /assets/img/item_recs_2-3.png %})

この設定により、レコメンデーションが最近更新されている場合、ユーザーがすでに購入または操作した項目がメッセージで再利用されるのを防ぎます。ただし、レコメンデーション更新の間に購入または操作された項目は引き続き表示される場合があります。無料版の項目レコメンデーションでは、更新は毎週行われます。Pro版のAI項目レコメンデーションでは、更新が24時間ごとに行われます。
...
{% alert tip %}
AI項目レコメンデーションを設定する際、イベントの選択は重要です。トリガーイベントによって、AIが生成するレコメンデーションを受け取るユーザーが決まります。AI項目レコメンデーションは、設定したイベントを完了したユーザーに対して生成されるため、この選択がレコメンデーションを受け取るユーザーを直接決定します。リーチしたいオーディエンスセグメント全体をカバーするイベントを選択してください。<br><br>同時に、カバレッジと関連性のバランスを取ることも重要です。ファネル上部のイベント（商品閲覧など）はより広いオーディエンスを捕捉する傾向がありますが、ビジネス成果との結びつきは弱くなります。一方、ファネル下部のイベント（購入など）は、よりターゲットを絞った、ビジネスに関連性の高いレコメンデーションを生成する傾向があります。最適なイベントとは、カバレッジと収益への影響のバランスが取れたものです。
{% endalert %}
...
![イベントの現在の追跡方法として選択されている「purchase」カスタムイベント。]({% image_buster /assets/img/item_recs_3.png %})
...
- ネストされたフィールドは、ドット表記を用いて`event_property.nested_property`の形式で**Property Name**ドロップダウンに入力する必要があります。たとえば、イベントプロパティ`location`内のネストされたプロパティ`district_name`を選択する場合は、`location.district_name`と入力します。
...
##### 製品の配列を持つカスタムイベントオブジェクトの例 {#example-custom-event-object-with-an-array-of-products}
...
##### 製品ID配列を含むネストされたオブジェクトを持つカスタムイベントオブジェクトの例 {#example-custom-event-object-with-a-nested-object-containing-a-product-id-array}
...
購入が行われると、APIを通じて購入オブジェクトが渡されます。
...
##### 製品IDにマッピングされた購入オブジェクトの例 {#example-purchase-object-mapped-to-product-id}
...
##### 製品IDにマッピングされた注文確定オブジェクトの例 {#example-order-placed-object-mapped-to-product-id}
