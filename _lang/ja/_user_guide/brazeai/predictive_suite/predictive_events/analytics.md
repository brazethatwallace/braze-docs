...
予測対象ユーザー全体のスコアの分布は、ページ上部のチャートに表示されます。右側のバケットにいるユーザーはスコアが高く、イベントを実行する可能性が高くなります。左側のバケットにいるユーザーは、イベントを実行する可能性が低くなります。チャートの下にあるスライダーを使用すると、ユーザーの範囲を選択し、選択した範囲のユーザーをターゲットにした場合の結果を推定できます。

スライダーを動かすと、下側のパネルの左半分のバーに、予測対象ユーザー全体のうちターゲットになるユーザーの数が表示されます。

![スライダーを動かすと、下側のパネルの左半分のバーに、予測対象ユーザー全体のうち選択した母集団を使用してターゲットになるユーザーの数が表示されます。]({% image_buster /assets/img/purchasePrediction/purchaseTargeting.png %}){: style="max-width:90%"}
...
### 過去のイベント行動が不足している {#not-enough-past-event-behavior}

イベント予測を作成する際に、「過去のイベント行動の数が非常に少ないため、この予測の構築に失敗します」またはそれに類似する過去のイベント行動に関するメッセージが表示されることがあります。

Brazeがモデルを構築するには、トレーニングウィンドウ内でターゲットイベントを実行した十分な数のユーザーが必要です。モデルには通常、そのウィンドウ内で**過去のイベント行動**としてラベル付けされたユーザーが少なくとも3,500人必要です。ユーザー群全体での使用率が非常に低いイベントは、この閾値を満たさない場合があります。

対象となるユーザーが少ない場合は、予測対象ユーザーを広げるか、予測を構築する前にイベントウィンドウを延長してください。設定手順については、[イベント予測の作成]({{site.baseurl}}/user_guide/brazeai/predictive_suite/predictive_events/creating_an_event_prediction)を参照してください。