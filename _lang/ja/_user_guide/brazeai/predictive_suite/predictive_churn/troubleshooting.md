...
| 症状 | 参照先 |
| --- | --- |
| 「トレーニングに十分なデータがありません」エラー | [トレーニングに十分なデータがない](#not-enough-data-to-train) |
| 「過去の非解約ユーザーが十分にいません」警告 | [予測対象ユーザーが少なすぎる](#problems-with-prediction-audience-size) |
| 予測対象ユーザーがサイズ上限を超えている | [予測対象ユーザーが大きすぎる](#prediction-audience-size-is-too-big) |
| 予測品質が40%未満 | [予測品質が低い](#prediction-has-poor-quality) |
| データがモデルに適しているかわからない | [データに関する考慮事項](#data-considerations) |
...
1. 解約予測が会社で有効になっていること、およびワークスペースに十分な月間アクティブユーザー（MAU）があることを確認します。通常、単一のワークスペースで300,000 MAUが必要です。
2. 解約の定義を確認します。フィルターが厳しすぎると、トレーニングに使用できる解約ユーザーの数が減少します。
3. 予測対象ユーザーの定義を確認します。過去の非解約ユーザーが少なすぎると、モデルのトレーニングがブロックされます。
4. カスタムイベント（カスタム属性のみではなく）が、解約リスクを示す高価値アクションをキャプチャしていることを確認します。
5. 定義を広げてもエラーが解消しない場合は、[Brazeサポート]({{site.baseurl}}/braze_support)にお問い合わせください。
