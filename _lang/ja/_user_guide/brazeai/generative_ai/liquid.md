---
nav_title: Liquidコード
article_title: Liquidコードを生成する
description: "この記事では、オペレーターを使用してメッセージング用のLiquidスニペットを生成する方法について、サポートされているチャネルやベストプラクティスを含めて説明します。"
page_type: reference
page_order: 5
---

# Liquidコードを生成する {#generate-liquid-code}

> オペレーターを使用して、任意のメッセージ作成画面から直接パーソナライゼーション用のLiquidを生成できます。自然言語でやりたいことを記述すると、オペレーターがコードを生成してメッセージに挿入します。

{% multi_lang_include brazeai/generative_ai/unification_note.md %}

## アクセス方法 {#how-to-access}

{% multi_lang_include brazeai/generative_ai/access_liquid.md %}

サポートされているチャネル、Liquid属性、ベストプラクティス、プロンプトの例などの詳細については、[オペレーター機能でのLiquid生成]({{site.baseurl}}/user_guide/brazeai/operator/capabilities/#generate-liquid)を参照してください。