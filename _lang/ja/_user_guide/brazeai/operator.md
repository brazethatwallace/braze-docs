...
自然言語を使って、達成したいことを説明します。明確で具体的なプロンプトにすることで、より有益な回答が得られます。プロンプトは、シンプルな質問から本格的な構築リクエストまで対応できます。

- **質問する：** Liquidが正しくレンダリングされないのはなぜですか？
- **何かを構築する：** 過去7日間にカートを放棄したユーザーのセグメントを下書きしてください。

オペレーターは、ステップバイステップの手順、Brazeドキュメントへのリンク、わかりやすい説明、キャンペーン・セグメント・コンテンツの下書きを提供でき、それらを確認して作業に直接挿入できます。オペレーターが変更を提案・適用する方法については、[オペレーターでアクションを実行する](#take-action-with-operator)を参照してください。

オペレーターは[GPT-5.6 Terra](https://developers.openai.com/api/docs/models/gpt-5.6-terra)を使用しており、複雑なマルチステップのタスクに適しています。オペレーターで構築できる内容の全範囲については、[Operatorでできること]({{site.baseurl}}/user_guide/brazeai/operator/capabilities)を参照してください。すぐに使えるプロンプト例については、[プロンプトライブラリー]({{site.baseurl}}/user_guide/brazeai/operator/prompt_library)を参照してください。
...
- **フォローアップの質問をする：** 最初の回答がニーズに合わない場合は、明確化や追加の詳細を求めましょう。オペレーターは、チャット履歴をクリアするまで会話内の以前のメッセージを記憶しています。
...
- [Operatorでできること]({{site.baseurl}}/user_guide/brazeai/operator/capabilities)：ダッシュボード全体でのオペレーターの機能を参照できます
