...
CampaignsやCanvasesに最適なメッセージチャネルを決定する際は、メッセージのコンテンツと緊急性を常に考慮してください。
...
![モバイル/Webプッシュはシンプルなコンテンツで高い緊急性、メールはリッチなコンテンツで高い緊急性、アプリ内/ブラウザメッセージはシンプルなコンテンツで低い緊急性、コンテンツカードは低い緊急性でリッチなコンテンツ]({% image_buster /assets/img_archive/messaging_matrix.png %})
...
Brazeを使用して、各チャネルでアクセシブルなメッセージングCampaignsを作成できます。エンジニアと協力して、実装においてアクセシビリティ基準を満たすようにしてください。追加のガイダンスが必要な場合は、以下をお勧めします。
