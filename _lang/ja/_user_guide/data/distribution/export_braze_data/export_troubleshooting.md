...
ダッシュボードのSegmentからの**ユーザーデータを CSV 形式でエクスポート**は、固定のカラムセットを使用します（[セグメントデータをCSVにエクスポート]({{site.baseurl}}/user_guide/data/distribution/export_braze_data/segment_data_to_csv#data-included-in-export)を参照）。`fields_to_export`カラムやパラメーターは含まれていません。
