...
## キャンバストリガーの同期 {#syncing-canvas-triggers}
...
始めるには、新しい同期を作成する際に **キャンバス Triggers** データタイプを選択します。

### キャンバストリガーの使用 {#using-canvas-triggers}

#### ステップ 1: キャンバストリガー用のデータソースを設定する {#step-1-set-up-data-source-for-canvas-triggers}
...
* キャンバストリガーでゼロコピー CDI を使用する場合、Braze はデータウェアハウスからのクエリ結果のバックアップを保存せず、データはユーザープロファイルにコピーされません。
* キャンバスコンテキストプロパティは、内部システムに最大 30 日間ログとして記録される場合があります。