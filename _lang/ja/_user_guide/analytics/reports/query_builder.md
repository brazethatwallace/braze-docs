...
Brazeは、これらのビューをクエリビルダーおよびSQLセグメントエクステンションから除外しています。ワークスペース規模でのクエリが遅く、タイムアウトすることが多いためです。クエリビルダーでカスタム属性のスナップショットを取得するには、`USER_CUSTOM_ATTRIBUTES_VIEW_SHARED` を使用してください。履歴データやほぼリアルタイムのカスタム属性データが必要な場合は、代わりにSnowflakeデータシェアリングを通じて除外されたビューをクエリしてください。

## クエリビルダーでのレポートの実行 {#running-reports-in-the-query-builder}

クエリビルダーでレポートを実行するには、以下の手順に従ってください。
...
4. エディターでSQLクエリを記述するか、**AIクエリビルダー**タブから[AIのサポートを受けます](#ai-query-builder)。独自のSQLを記述する場合は、要件とリソースについて[カスタムSQLクエリの記述](#custom-sql)を参照してください。
5. **クエリを実行**を選択します。
6. クエリを保存します。
7. レポートのCSVをダウンロードするには、**エクスポート**を選択します。

![「過去30日間のチャネルエンゲージメントと収益」テンプレートクエリの結果を表示するクエリビルダー。]({% image_buster /assets/img_archive/query_builder.png %})
...
SnowflakeデータベースへのクエリのデフォルトタイムゾーンはUTCです。そのため、**メールチャネルエンゲージメント**ページ（会社のタイムゾーンに従います）とクエリビルダーの結果の間でデータの不一致が生じる場合があります。
...
より長い期間のクエリ使用状況を監査したり、7日を超えて記録を維持したりする必要がある場合は、有効期限が切れる前に重要なクエリ結果をエクスポートまたは保存することをお勧めします。

### クエリビルダーと他のレポートソースの比較 {#comparing-query-builder-with-other-reporting-sources}

クエリビルダーの結果は、異なるデータソースと処理方法を使用しているため、他のレポートツールと異なる場合があります。

たとえば、クエリビルダーのソフトバウンス数は、SendGrid配信レポートよりも高くなる場合があります。クエリビルダーは重複排除なしでソフトバウンスのすべての発生をカウントします。ユーザーが最終的な配信前（または長期間のリトライ後）に複数回ソフトバウンスした場合、各ソフトバウンスの試行がカウントされます。SendGrid配信はBrazeが把握できない独自のデータとロジックを使用しているため、2つのレポート間でカウントが一致しない場合があります。

異なるレポートソース間でソフトバウンスがどのように追跡されるかの詳細については、メール分析用語集の[ソフトバウンス]({{site.baseurl}}/user_guide/channels/email/reporting/analytics_glossary#soft-bounce)を参照してください。

## AIクエリビルダーによるSQLの生成 {#generating-sql-with-the-ai-query-builder}

AIクエリビルダーは、OpenAIが提供する[GPT](https://openai.com/gpt-4)を活用して、クエリ用のSQLを推奨します。
...
3. 生成されたSQLを確認して正しいことを確認し、**Insert into Editor**を選択します。
...
変数を使用すると、定義済みの変数タイプをSQLで利用でき、値を手動でコピーする必要がなくなります。たとえば、キャンペーンのIDをSQLエディターに手動でコピーする代わりに、{% raw %}`{{campaign.${My campaign}}}`{% endraw %}を使用して、**変数**タブのドロップダウンからキャンペーンを直接選択できます。
...
- **置換値:** キャンペーンバリアントのAPI ID。カンマ区切りの文字列（`api-id1, api-id2`など）です。
...
- **置換値:** キャンバスバリアントのAPI ID。カンマ区切りの文字列（`api-id1, api-id2`など）です。
- **使用例:** {% raw %}`canvas_variation_api_id IN ({{canvas_variants.${some name}}})`{% endraw %}

##### キャンバスステップ {#canvas-step}

選択したキャンバスに属するキャンバスステップを選択するためのものです。キャンバスの変数と組み合わせて使用する必要があります。
...
選択したキャンバスに属するキャンバスステップを選択するためのものです。キャンバスまたは複数キャンバスの変数と組み合わせて使用する必要があります。

- **置換値:** キャンバスステップのAPI ID（複数）
