...
8. 次に、ステップ3での選択に応じて、キャンペーン、キャンバス、またはその両方を手動または自動でレポートに追加します。
    ...
    - **自動で追加：** レポートに含めるキャンペーンまたはキャンバスのルールを設定します。このページでは1つのフィールドのみ選択が必要です。
        ...
        - バナーは**チャネル**ドロップダウンのオプションにないため、チャネルルールを使用してバナーキャンペーンやキャンバスを自動的に追加することはできません。ただし、レポートの指標にバナーのKPIを含めることは可能です。<br><br>![「キャンペーンとキャンバスを自動で追加」セクション。レポートに追加するキャンペーンとキャンバスのルールを設定するフィールドが表示されています。]({% image_buster /assets/img/report_builder_2/automatically_add.png %}){: style="width:90%;"}<br><br>
