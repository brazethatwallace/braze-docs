...
アンインストール検出では、可視的な通知として表示されない低優先度のバックグラウンドプッシュを使用できます。これらは、標準的なメッセージング分析におけるキャンペーンの[**送信数**]({{site.baseurl}}/user_guide/analytics/reports/campaign_analytics)とは別のものです。アンインストールの傾向を分析する際は、アンインストールプッシュをマーケティング送信総数と直接比較するのではなく、アンインストールチャートをプッシュエンゲージメント指標と併せて確認してください。