...
トランザクションメールは、一部のBrazeパッケージでのみ利用可能です。詳細については、Brazeカスタマーサクセスマネージャーに連絡するか、[サポートチケット]({{site.baseurl}}/user_guide/administer/personal/braze_support)を開いてください。
...
| 配送通知 | 注文が出荷されたことを、トラッキング情報や配送予定日とともに顧客に通知します。 |
| アカウントアラート | 支払いの失敗、購読の変更、セキュリティアラートなど、アカウントに関する重要な通知を送信します。 |
...
トランザクションメールは、速度と信頼性に最適化された専用のBraze[トランザクション HTTP API]({{site.baseurl}}/user_guide/channels/transactional_email/create_a_transactional_email)を通じて送信されます。マーケティングメールとは異なり、トランザクションメールには以下の特徴があります。
