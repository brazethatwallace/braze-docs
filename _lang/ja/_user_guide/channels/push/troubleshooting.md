...
| プッシュ通知が遅延して届く | [プッシュ通知の遅延](#delayed-push-notifications) |
| プッシュの送信が予想より遅い | [プッシュ通知の送信が予想より遅い](#push-notifications-are-sending-slower-than-expected) |
...
| プッシュリンクがブラウザーではなくアプリ内で開く | [プッシュクリックが予期せずアプリ内で開く](#push-clicks-unexpectedly-open-in-app) |
| Web プッシュの許可または配信の問題 | [Web プッシュ通知が期待どおりに動作しない](#web-push-notifications-are-not-behaving-as-expected) |
| `.p12` から `.p8` への移行が必要（iOS） | [.p8 認証キーへの移行](#migrate-to-a-p8-authentication-key) |
| ログに特定のプッシュエラーコードがある | [プッシュエラーメッセージ](#push-error-messages) |
| アンインストール数がプラットフォームごとに一致しない | [アンインストール指標](#uninstall-metrics) |
| ユーザーやプッシュデータを別のワークスペースに移行する | [ワークスペースデータの移行](#workspace-data-migration) |
| セッションがプッシュ開封から開始されたかどうかを確認したい | [セッションとアトリビューション](#session-and-attribution) |
...
1. ユーザーがプッシュを購読またはオプトインしており、プロファイルの**エンゲージメント**タブに有効なプッシュトークンがあることを確認します。
...
4. デバイスに対して正しいプッシュタイプを使用していることを確認します（例：Android、iOS、Kindle）。
5. 内部テストの場合、テスターがデバイス上で正しいアプリにログインしていることを確認します。
6. それでも配信に失敗する場合は、[よくあるプッシュエラーメッセージ]({{site.baseurl}}/user_guide/channels/push/push_error_codes)を確認するか、キャンペーンまたはキャンバスID、ユーザーID、タイムゾーン付きのタイムスタンプを添えて[Brazeサポート]({{site.baseurl}}/user_guide/administer/personal/braze_support)にお問い合わせください。
...
プッシュ通知は以下の理由で遅延する場合があります。

- デバイスのデータ接続が弱い
- アプリ内のカスタムコードがBrazeのプッシュ通知を抑制している
- デバイスの設定におけるプッシュ通知に関するユーザーの設定
- キャンペーンまたはキャンバスで作成した際のプッシュのメッセージ優先度
- プッシュサービスプロバイダー（FCMおよびAPNs）のトラフィック遅延または問題

### プッシュ通知の送信が予想より遅い {#push-notifications-are-sending-slower-than-expected}

**症状：** キャンペーンまたはキャンバスのプッシュ送信が完了するまでに予想以上の時間がかかります。

プッシュ通知の設定が以下のベストプラクティスに従っていることを確認してください。

- プッシュ有効ステータスを考慮せずに大規模なオーディエンスに送信している場合、送信速度が遅くなる可能性があります。代わりに、プッシュが有効なユーザーのみに送信してオーディエンスのサイズを縮小することを検討してください。
- 可能であれば、即時送信ではなく、事前にキャンペーンをスケジュールしてみてください。
- キャンバスで多数のユーザーをプッシュ通知のターゲットにしている場合、キャンバス内の後続のメッセージステップは、ユーザーに即時送信するキャンペーンとは異なる処理時間を要することが予想されます。この場合、キャンバスの最初の「ステップ」はユーザーが特定のユーザージャーニーに適格かどうかを確認することであるため、通常キャンペーンの方がキャンバスよりも先に送信を完了します。
...
3. **iOSプッシュ登録を確認する:** iOSの場合、[APNsへのプッシュ通知の登録]({{site.baseurl}}/developer_guide/push_notifications?sdktab=swift)に関するプッシュ統合ガイドのステップ1を再確認してください。デリゲートオブジェクトがアプリの起動完了前に同期的に割り当てられていることを確認します。このステップは`application:didFinishLaunchingWithOptions:`メソッドで完了する必要があります。
...
Brazeは、ユーザーが通知をタップしてアプリがセッションを開始した場合に直接開封を記録します。アプリを開かずにリッチプッシュ通知を展開しても、直接開封は記録されません。
...
プッシュプロバイダーが登録トークンが無効であることを通知した場合（例：FCMからの`DEVICE_UNREGISTERED`や`NotRegistered`）、Brazeは影響を受けたプッシュトークンをユーザープロファイルから削除し、そのユーザーをアンインストール済みとしてカウントします。アンインストール追跡キャンペーンは、同じトークン削除ロジックを大規模に使用します。

その他のプッシュエラーはバウンスとして記録され、トークンは削除されません。例えば、[`MismatchSenderID`](#error-mismatch-sender-id)のような認証エラーは、BrazeがFCMとの認証に失敗したことを意味するため、アンインストールシグナルとして扱うのではなく、認証情報を修正してください。

Androidのアンインストール追跡について、Brazeはワークスペースの設定に応じて、アンインストール検出プッシュをドライラン（検証のみ）またはライブサイレントプッシュとして送信します。ドライランはメッセージを配信せずにリクエストを検証するため、その結果はライブ送信とは異なる場合があります。アンインストール数が少なく見える場合は、Androidの統合が[アンインストール追跡]({{site.baseurl}}/user_guide/analytics/tracking/uninstall_tracking)の前提条件を満たしていることを確認し、メッセージアクティビティログでバウンスエラーを確認してください。

## アンインストールの指標 {#uninstall-metrics}

### アンインストールの合計がAndroidとiOSの合計と一致しないのはなぜですか？ {#why-dont-total-uninstalls-match-android-plus-ios}

ワークスペースの**アンインストール合計**は、プラットフォーム固有のアンインストール指標の合計を超えることがあります。これは、Webプッシュトークンの無効化もアンインストール数にカウントされるためです。Webプッシュトークンが無効になった場合（たとえば、ユーザーがサイトデータを消去したり、許可を取り消したりした場合）、モバイルのアンインストール指標に変化がなくても、BrazeはそのWeb登録に対してアンインストールを記録することがあります。

### インポートされたiOSプッシュトークンはどの購読ステータスを表示しますか？ {#what-subscription-status-do-imported-ios-push-tokens-show}

インポートされたiOSプッシュトークンは、通常、ユーザーがそのワークスペースでBraze SDKを使用するアプリでセッションを記録するまで、**Subscribed**として表示されます。セッション開始時にSDKがトークンを登録した後、プッシュの承認が付与されていれば、プロファイルは通常**Opted-In**に移行します。購読状態とプロファイルフィールドについては、[プッシュの購読状態]({{site.baseurl}}/user_guide/channels/push/push_setup/push_subscription_states)を参照してください。

## ワークスペース間のデータ移行 {#workspace-data-migration}

### ワークスペース間でデータを移行できますか？ {#can-i-migrate-data-between-workspaces}

Brazeでは、ワークスペース間のワンクリック移行は提供していません。アプリまたはサイトを移行先ワークスペースのAPIキーに向け、[ユーザートラック]({{site.baseurl}}/api/endpoints/user_data/post_user_track)エンドポイントまたは[CSVインポート]({{site.baseurl}}/user_guide/audience/manage_audience/import_users/csv_import)でユーザーを再作成してください。移行元のプロファイルは、事前に[識別子によるユーザーエクスポート]({{site.baseurl}}/api/endpoints/export/user_data/post_users_identifier)または[セグメントによるユーザーエクスポート]({{site.baseurl}}/api/endpoints/export/user_data/post_users_segment)でエクスポートしてください。

コピーできるもの、再構築が必要なもの、プッシュトークンの制限については、[ワークスペース間のデータ移行]({{site.baseurl}}/user_guide/administer/global/create_and_manage_workspaces/migrate_workspace_data)を参照してください。大規模なワークスペース移行を計画する際は、Brazeアカウントチームと連携してください。

## セッションとアトリビューション {#session-and-attribution}

### セッション開始からユーザーがプッシュ通知でアプリを開いたかどうかわかりますか？ {#can-i-tell-from-session-start-whether-the-user-opened-the-app-from-a-push}

いいえ。セッション開始イベントには、そのセッションがプッシュ通知の開封から始まったかどうかを示すフラグは含まれていません。セッションとプッシュエンゲージメントを関連付けるには、プッシュの**直接開封**、**影響を受けた開封**、またはカスタムイベント（例：アプリ内のクリックハンドラーのログ記録）を使用してください。[プッシュ開封指標](#push-open-metrics)を参照してください。