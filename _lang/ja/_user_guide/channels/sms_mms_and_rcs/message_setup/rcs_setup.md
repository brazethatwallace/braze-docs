...
1. 現在のBraze契約にメッセージクレジットが含まれている必要があります。
...
Brazeでは、最初は少数のユーザーにRCSを送信してテストし、時間をかけてRCSサブスクリプショングループにより多くのユーザーを移行することをお勧めします。たとえば、SMSサブスクリプショングループに1,000,000人のユーザーが登録されている場合、まずすべてのユーザーを新しいサブスクリプショングループに移行し、次に50,000〜100,000人（5〜10%）の小規模なオーディエンスにSegmentを作成してRCSメッセージをテストするという方法が考えられます。
