...
![リンク短縮のトグルがあるメッセージ作成画面。]({% image_buster /assets/img/link_shortening/legacy/temp_shortening1.png %}){: width="1614" height="994"}
...
![「メッセージ」ボックスに長いURLがあり、プレビューに生成された短縮リンクが表示されたメッセージ作成画面。]({% image_buster /assets/img/link_shortening/legacy/temp_shortening3.png %}){: width="1569" height="516"}
...
「短縮SMSリンクをクリック」フィルターにキャンバスが表示されるようにするには、ショートリンクを含むキャンバスステップでも高度なトラッキングを有効にする必要があります。これにより、ユーザーレベルのクリックトラッキングが可能になります。ショートリンクが基本トラッキングで設定されている場合、SMSショートリンクのクリックイベントをフィルタリングするオプションは利用できません。クリックされた短縮SMSリンクに依存するキャンバスエントリやアクションパスを設定する場合も、同じ高度なトラッキング要件が適用されます。

{% alert important %}
アクティブなキャンバス内で下書きが作成された場合、短縮URLは生成されません。実際の短縮URLは、キャンバスの下書きがアクティブになったときに生成されます。
{% endalert %}

![テスト受信者を選択するフィールドがあるメッセージの「テスト」タブ。]({% image_buster /assets/img/link_shortening/legacy/temp_shortening2.png %}){: width="1569" height="947"}
...
![SMSおよびMMSのパフォーマンス指標テーブル。]({% image_buster /assets/img/link_shortening/shortening4.png %}){: width="1586" height="191"}
