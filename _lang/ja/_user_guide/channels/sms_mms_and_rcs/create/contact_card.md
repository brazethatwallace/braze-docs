...
{% alert note %}
連絡先カードの送信はMMSとして課金されます。連絡先カードを作成する際は、予想されるMMSの送信量とメッセージクレジットの使用量を確認し、Brazeの[請求ページ]({{site.baseurl}}/user_guide/administer/global/billing/)でコストを確認してください。
{% endalert %}
...
既存の連絡先カードを追加するには、CampaignまたはCanvasを作成し、希望のサブスクリプショングループを選択します。次に、メッセージ作成画面に**メディアを追加**オプションが表示されます。ここで、既存の連絡先カードファイルをアップロードするか、メディアライブラリから検索できます。