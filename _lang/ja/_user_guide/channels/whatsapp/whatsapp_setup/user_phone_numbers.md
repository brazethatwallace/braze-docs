...
電話番号がE.164形式に変換されてもバリデーションに合格しない場合、Brazeはその番号にWhatsAppメッセージを送信できません。フォーマットできない電話番号を持つユーザーは、WhatsAppを含むCanvasステップから自動的に退出します。
...
単一のCampaignまたはCanvasステップのSegment内で複数のユーザーが同じ電話番号を持っている場合、Brazeは送信の重複を排除し、その電話番号に1通のメッセージのみを送信します。