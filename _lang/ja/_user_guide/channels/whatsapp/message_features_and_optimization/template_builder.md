...
WhatsAppキャンペーンやキャンバスの作成中にテンプレートを作成することもできます。詳細については、[キャンペーンまたはキャンバスからテンプレートを作成する](#create-a-template-from-a-campaign-or-canvas)を参照してください。

### ステップ2:カテゴリとタイプを選択する {#step-2-choose-a-category-and-type}

テンプレートカテゴリとテンプレートタイプを選択し、準備ができたら**テンプレートに進む**を選択します。

{% alert note %}
Metaは[カテゴリガイドライン](https://developers.facebook.com/documentation/business-messaging/whatsapp/templates/template-categorization)とコンテンツに基づいてテンプレートを審査します。
{% endalert %}

#### マーケティング {#marketing}

マーケティングテンプレートは、プロモーションやエンゲージメントメッセージ（例：ウェルカムメッセージ、プロモーション、オファー、クーポン、ニュースレター、お知らせ）に使用します。

| タイプ | 説明 |
| --- | --- |
| **カスタム** | ゼロから作成する標準的なWhatsAppメッセージです。このレイアウトは[テンプレートを作成する](#step-4-build-your-template)で説明しています。 |
...
| **フロー** | WhatsAppフロー（例：アンケートや予約）を開くテンプレートです。MetaのWhatsApp Managerでフローを作成・管理し、テンプレート作成時に選択します。詳細については、[WhatsAppフロー]({{site.baseurl}}/user_guide/channels/whatsapp/message_features_and_optimization/whatsapp_flows)を参照してください。 |
{: .reset-td-br-1 .reset-td-br-2 aria-label="マーケティングテンプレートタイプ" }

#### ユーティリティ {#utility}

ユーティリティテンプレートは、非プロモーションメッセージ（例：注文確認、アカウント更新、領収書、予約リマインダー、請求）に使用します。Metaはプロモーションコンテンツをマーケティングに再分類します。

| タイプ | 説明 |
| --- | --- |
| **カスタム** | ゼロから作成する標準的なユーティリティメッセージです。[テンプレートを作成する](#step-4-build-your-template)と同じ手順に従ってください。 |
| **フロー** | ユーティリティフローテンプレート（例：リマインダー、フィードバック、注文管理）です。MetaのWhatsApp Managerでフローを作成・管理し、テンプレート作成時に選択します。詳細については、[WhatsAppフロー]({{site.baseurl}}/user_guide/channels/whatsapp/message_features_and_optimization/whatsapp_flows)を参照してください。 |
{: .reset-td-br-1 .reset-td-br-2 aria-label="ユーティリティテンプレートタイプ" }

{% alert note %}
カルーセルと期間限定オファーのレイアウトは、マーケティングテンプレートでのみ利用可能です。
{% endalert %}
...
- **テキスト：** 短いテキストヘッダーです。
- **メディア：** 画像、動画、またはドキュメント（URLのみ）です。Brazeはメディア参照を保存し、承認用にMetaにサンプルを送信します。
- **なし：** ヘッダーなし
...
{% raw %}
- Liquidタグ（例：`{{${first_name}}}`）を使用します。Brazeはお使いのLiquidを保存し、キャンペーンやキャンバスの作成画面でテンプレートを使用する際に表示します。
- 後でメッセージ作成時にパーソナライゼーションを追加する場合は、番号付きプレースホルダー（例：`{{1}}`）などの汎用変数を使用します。
{% endraw %}
...
Metaは、承認用に送信するテンプレート構造に文字数制限を適用します（例：本文は1,024文字、テキストヘッダーは60文字）。テンプレートビルダーでは、これらの制限はMetaに送信されるテンプレートに適用され、送信時の最終レンダリングメッセージには適用されません。

- **{% raw %}`{{ }}`{% endraw %} 変数：** BrazeはLiquid変数を番号付きプレースホルダー（{% raw %}`{{1}}`、`{{2}}`{% endraw %}）に変換してから長さをチェックします。{% raw %}`{{${first_name}}}`{% endraw %}のような長い式は、完全なLiquid構文ではなく短いプレースホルダーとしてカウントされます。
...
フローテンプレートの場合は、標準のコールトゥアクションボタンを追加する代わりに、フローボタンを構成し、Metaから既存のフローを選択します。
...
キャンペーンまたはキャンバスのメッセージステップを離れることなく、WhatsApp テンプレートを作成して送信できます。
...
3. カテゴリとタイプを選択し、WhatsApp テンプレートページと同じ方法でテンプレートを作成して送信します。
4. 送信後、Braze は保留中のテンプレートをメッセージにバインドします。テンプレートが保留中の間もパーソナライゼーションの作成を続け、Meta が承認した後に配信を開始します。

**ライブラリからテンプレートを選択**を選択すると、ビルダーを離れて既存のテンプレートを選択できます。

{% alert note %}
キャンペーンまたはキャンバスからテンプレートを作成する場合、Braze は作業内容を下書きとして保存できるため、メッセージステップを離れても内容が保持されます。
{% endalert %}
...
5. **テスト**タブを使用してメッセージをプレビューし、本文の変数を更新し、配信前にメッセージが期待どおりに表示されることを確認します。
