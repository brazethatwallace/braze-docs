...
[シードグループ]({{site.baseurl}}/user_guide/administer/global/user_management/internal_groups#seed-groups)は、品質保証を行うために、メールキャンペーンのコピーを内部ユーザーに自動的に送信します。シードメールには、件名の先頭に`[SEED]`が付加されるため、簡単に識別できます。
...
| ニュースレター | 月次または週次のニュースレターを手軽に送信し、ユーザーのエンゲージメントを維持します。 |
| トランザクション | 最近の購入についてユーザーに通知し、[トランザクションメール]({{site.baseurl}}/user_guide/channels/transactional_email/create_a_transactional_email)を使用して重要な商品情報や配送情報を届けます。
