...
Campaignsは、シンプルな単一メッセージング（1つのメッセージで製品についてユーザーに通知するなど）に使用します。Canvasesは、複数ステップのユーザージャーニー（時間の経過に伴うユーザーの動作に基づいてカスタマイズされた製品提案を送信するなど）に使用します。
...
## ステップ 2: メッセージタイプを指定する {#step-2-specify-your-message-types}
...
Campaignコンポーザーは、静的コンテンツが制限を超えた場合に警告を表示します。（Liquidを使用した動的コンテンツのサイズは予測しません。）**メッセージサイズが2 KBを超えた場合、送信時に中止されます。**これらの中止は、メッセージアクティビティログで理由 `Content card maximum size exceeded` として確認できます。
...
- **短い有効期限を使用する:** 時間に敏感なCampaigns（週末セールなど）の場合、特定の有効期限日を設定します。これにより、カードはフィードから自動的に削除され、関連性がなくなった後は制限にカウントされません。
...
- **長期実行Campaignsを監査する:** 定期的または継続的なCampaignsを確認して、時間の経過とともにフィードに多すぎるカードが蓄積され、ユーザーに悪い体験を与えていないことを確認します。
...
よくある混乱の原因は、Campaignの再適格性とすべてのContent Cardsの自動30日間有効期限の相互作用です。

すべてのContent Cardsは、送信または削除されてから30日後にBrazeのシステムから自動的にパージされます。再適格性を**オフ**にした長期実行の定期Campaignがある場合、ユーザーは30日後に同じカードを再度受信する可能性があります。元のカードがパージされると、システムはそのユーザーがCampaignを受信した記録を認識しなくなり、次のセッションで再び適格になります。

ユーザーが特定のCampaignからメッセージを1回だけ受信するようにするには、CampaignまたはCanvasステップに、このCampaignからメッセージを受信していないユーザー向けのオーディエンスフィルターを追加してください。このフィルターは、長期実行Campaignsからの重複送信を防ぐ最も信頼性の高い方法です。

### ライブContent Cardsの管理 {#managing-live-content-cards}

Content Cardsが送信された後、ユーザーに配信される準備ができた「受信トレイ」で待機します（メールの場合と同様）。コンテンツがContent Cardsに取り込まれた後（表示時）、その有効期間中は変更できません。これは、コネクテッドコンテンツを通じてAPIを呼び出し、エンドポイントからのデータが変更された場合でも適用されます。このデータは更新されません。新しいユーザーへの送信を停止し、ユーザーのフィードから削除することのみ可能です。Campaignを変更した場合、送信される将来のカードのみが更新されます。

#### 起動済みカードの更新 {#updating-launched-cards}

すでにカードを受信したユーザーのカードを変更するには、以下のいずれかの方法を使用する必要があります。

##### オプション 1: Campaignを複製する（即時変更に推奨） {#option-1-duplicate-the-campaign-recommended-for-immediate-changes}

{% alert tip %}
カードに最新のコンテンツを表示する場合、変更をすぐに反映する必要がある場合、または再適格性がオフの場合に、このオプションをお勧めします。
{% endalert %}

最初のアプローチは、Campaignをアーカイブし、新しい複製Campaignを起動することです。

1. 元のCampaignを停止し、プロンプトが表示されたら `Remove card after the next sync` を選択します。
2. Campaignを複製し、編集を行い、新しいバージョンを起動します。

Campaignを複製する場合、新しいバージョンのオーディエンスを定義する必要があります。セグメンテーションフィルターを使用して、更新されたカードを受信するユーザーを制御します。
...
たとえば、セッション開始時にトリガーされるCampaignを設定し、再適格性を30日に設定したとします。ユーザーが2日前にCampaignを受信し、コピーを変更したいとします。まず、Campaignをアーカイブしてフィードからカードを削除します。次に、Campaignを複製し、新しいコピーで再起動します。ユーザーが別のセッションを開始すると、すぐに新しいカードを受信します。

##### オプション 2: 同じCampaignを停止して再起動する {#option-2-stop-and-relaunch-the-same-campaign}
...
* **レポート:** 1つのCampaignに、起動されたカードバージョンのすべてのレポート分析が含まれます。Brazeは起動されたバージョン間を区別しません。

たとえば、セッション開始時にトリガーされ、再適格性が30日に設定されたCampaignがあるとします。ユーザーが2日前にCampaignを受信し、コピーを変更したいとします。まず、Campaignを停止してフィードからカードを削除します。次に、新しいコピーでCampaignを再公開します。ユーザーが別のセッションを開始すると、28日後に新しいカードを受信します。
...
CampaignまたはCanvasステップで、削除イベントを指定します。ユーザーがそのイベントを実行すると、Brazeがイベントを処理した後の次のフィード更新時にカードがフィードから削除されます。
...
{% alert tip %}
ユーザーのフィードからカードを削除するべき複数のカスタムイベントと購入を指定できます。ユーザーがそれらのアクションの**いずれか**を実行すると、Campaignのカードによって送信された既存のカードが削除されます。将来の適格なカードは、メッセージのスケジュールに従って引き続き送信されます。
{% endalert %}
...
カードが常に利用可能であるように見せたい場合は、30日ごとにカードを効果的に置き換える定期Campaignを作成できます。
