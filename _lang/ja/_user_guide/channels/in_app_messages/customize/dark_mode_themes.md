...
このオプションが有効になると、カラーピッカーを使用するか、既存の[カラープロファイル]({{site.baseurl}}/user_guide/messaging/templates/in_app_message_templates/in_app_message_template#color-profile)を選択して既存のダークまたはライトテーマを再利用することで、アプリ内メッセージのダークテーマカラーを選択できます。
...
2. **テンプレートを作成**を選択し、ドロップダウンから[カラープロファイル]({{site.baseurl}}/user_guide/messaging/templates/in_app_message_templates/in_app_message_template#color-profile)を選択します。
...
{% alert note %}
ダークモードアプリはAndroid 10およびiOS 13で導入されました。少なくともこれらのバージョンにスマートフォンをアップグレードしていないユーザーには、ライトテーマのみが表示されます。<br><br>キャンペーンは、ユーザーのダークモード設定やOSバージョンに関係なく、選択したオーディエンスの対象となるすべてのユーザーに配信されます。
{% endalert %}
