...
page_order: 2
description: "このリファレンス記事では、CampaignsまたはCanvasを使用してBrazeプラットフォームでアプリ内メッセージを作成する方法について説明します。"
channel:
  - in-app messages
tool:
  - Campaigns
...
この設定はメールキャプチャフォームメッセージタイプにのみ適用されます。メール入力フィールドのプレースホルダーテキストとして表示されるカスタムコピーを入力します。デフォルトは「メールアドレスを入力してください」です。
...
![トリガーアクションが「セッション開始」に設定されたアクションベースのCampaign。]({% image_buster /assets/img_archive/in_app_schedule.png %}){: style="max-width:80%"}
...
これらのバケット内で優先度を設定するには、**正確な優先度を設定**をクリックし、Campaignをドラッグ＆ドロップして正しい優先度に並べ替えることができます。
...
次に、[アプリ内メッセージレポート]({{site.baseurl}}/user_guide/channels/in_app_messages/reporting/)を確認して、メッセージングCampaignの結果にアクセスする方法を学びましょう。
...
2. デバイスは、ユーザーのトリガーイベントがCampaignの開始時間と終了時間（ユーザーのローカルタイムゾーンで定義）の範囲内で発生したかどうかを確認します。
