...
tool:
  - Media
description: "このリファレンス記事では、モーダルアプリ内メッセージのメッセージおよびデザイン要件について説明します。"

---

# モーダルアプリ内メッセージ {#modal-in-app-messages}
...
タブレットやデスクトップブラウザでは、モーダルアプリ内メッセージは以下のスクリーンショットのように、アプリ画面の中央に表示されます。

![大きな画面で表示されるモーダルアプリ内メッセージ。スマートフォンの画面と同様に、メッセージは画面の中央に表示されます。]({% image_buster /assets/img/modal-large-viewport.png %}){: style="max-width: 800px; border: none;"}