...
Brazeには、メールユーザー向けに3つのグローバルサブスクリプション状態があります。これらの状態は、ユーザーへのメッセージ配信を制御します。たとえば、`unsubscribed` 状態のユーザーは、`subscribed` や `opted-in` をターゲットとしたメッセージを受信しません。
...
{: .reset-td-br-1 .reset-td-br-2 aria-label="サブスクリプション状態" }

{% alert note %}
Brazeは、グローバルおよび購読グループにおけるサブスクリプション状態の変更をデータポイントとしてカウントしません。
{% endalert %}
...
メールアドレスがハードバウンスした場合、Brazeはユーザーのサブスクリプション状態を自動的に「配信停止」に設定しません。アドレスがハードバウンスした場合（無効または存在しない場合）、Brazeはそのアドレスを無効としてマークし、それ以上の送信を試みません。ユーザーがメールアドレスを変更した場合、Brazeは送信を再開します。Brazeはソフトバウンスを72時間再試行します。

### メールのサブスクリプション状態の更新 {#updating-email-subscription-states}

ユーザーのメールサブスクリプション状態を更新するには、4つの方法があります。

#### SDK統合 {#sdk-integration}

Braze SDKを使用して、ユーザーのサブスクリプション状態を更新します。
...
メールの下部に[ユーザー設定センター](#email-preference-center)のLiquidを含めて、ユーザーがオプトインまたはオプトアウトできるようにします。Brazeはユーザー設定センターからのサブスクリプション状態の更新を管理します。

### メールのサブスクリプション状態の確認 {#checking-email-subscription-state}

![John Doeのユーザープロファイル。メールのサブスクリプション状態が「購読中」に設定されています。]({% image_buster /assets/img/push_example.png %}){: style="float:right;max-width:35%;margin-left:15px;"}
...
サブスクリプション状態の変更をトラブルシューティングするには、ユーザープロファイルログの**メールサブスクリプション状態の変更**で履歴とソースを確認してください。以下のソースがメールサブスクリプション状態の変更をトリガーする可能性があります。
...
| キャンバスのユーザー更新ステップ | キャンバスの[ユーザー更新ステップ]({{site.baseurl}}/user_guide/engagement_tools/canvas/canvas_components/user_update)によって更新されたサブスクリプション状態 |
...
購読グループは、[グローバル購読ステート](#subscription-states)からオーディエンスをさらに絞り込むことができるセグメントフィルターです。これらのグループにより、エンドユーザーに対してより細かい購読オプションを提示できます。
...
セグメントを作成する際に、購読グループ名をフィルターとして設定し、グループにオプトインしたユーザーをターゲットにします。これは、月次ニュースレター、クーポン、メンバーシップティアなどに役立ちます。
...
アーカイブされた購読グループは編集できず、セグメントフィルターやユーザー設定センターにも表示されなくなります。メール、キャンペーン、またはキャンバスのセグメントフィルターとして使用されているグループをアーカイブしようとすると、そのグループのすべての使用箇所を削除するまでアーカイブできないことを示すエラーメッセージが表示されます。
...
時系列のカウントが**メール購読ステータスが購読解除**を使用したセグメントと大きく異なる場合は、グラフがその**購読グループ**のメンバーシップをカウントしているのに対し、そのフィルターは**グローバル**なメール購読ステートを反映していることを覚えておいてください。たとえば、ユーザーはグローバルでは購読済みでも、特定のグループからは購読解除されている場合があります。
...
メール設定センターでは、購読グループのニュースレターを受信するユーザーを管理できます。ダッシュボードの**購読グループ**から確認できます。作成した各購読グループは、設定センターのリストに追加されます。

設定センターの追加やカスタマイズの詳細については、[ユーザー設定センター]({{site.baseurl}}/user_guide/audience/subscription_preferences/preference_center)を参照してください。
