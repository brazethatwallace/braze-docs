...
このパターンは通常、現在のレピュテーションに対してメールボックスプロバイダーが受け入れる速度よりも速く送信していることを意味します。エンゲージメントとリスト品質の改善に加えて、[配信速度レート制限]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/frequency_capping#delivery-speed-rate-limiting)を使用して、キャンペーンやキャンバスからBrazeがメッセージを送信する速度にキャップを設定してください。これにより、配信到達性チームと長期的な修正に取り組む間、スロットリングを軽減できます。
