...
ドラッグ＆ドロップエディターでは、段落やその他のテキストブロック内に画像をネストすることはできません。テキストレイアウトの横や中に画像を配置するには、**行**の列を使用します。たとえば、デスクトップ用にはマルチカラム行で**モバイルで非表示**を設定し、モバイル専用の別の行（**デスクトップで非表示**と**モバイルでスタックしない**を必要に応じて設定）を使用することで、小さな画面でも画像とテキストがきれいに配置されます。
