...
digコマンドを実行して、リンクトラッキングがCDNを指していることを確認してください。ターミナルで `dig CNAME link_tracking_subdomain` を実行します。`ANSWER SECTION` にCNAMEの指し先が表示されます。CDNではなくメールサービスプロバイダー（ESP）（SendGridまたはSparkPost）を指している場合は、ドメインレジストリをCDNを指すように再設定してください。
