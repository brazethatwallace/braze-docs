...
- `email_open_tracking_disabled`:`true` または `false` を受け入れます。`false` に設定すると、このユーザーに送信されるすべての今後のメールに開封トラッキングピクセルが追加されます。SparkPost および SendGrid でのみ利用可能です。
- `email_click_tracking_disabled`:`true` または `false` を受け入れます。`false` に設定すると、このユーザーに送信される今後のメール内のすべてのリンクにクリックトラッキングが追加されます。SparkPost および SendGrid でのみ利用可能です。

参照情報として、この情報はユーザープロファイルの**エンゲージメント**タブにあるメールの**連絡先設定**に反映されます。
