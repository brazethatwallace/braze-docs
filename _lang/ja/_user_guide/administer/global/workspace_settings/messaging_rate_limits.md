...
![3つのCampaignのメッセージがどのようにディスパッチされるかの例。]({% image_buster /assets/img/workspace_messaging_rate_limits2.png %})
...
![2つのCampaignのメッセージが1分あたりどのようにディスパッチされるかの例。]({% image_buster /assets/img/workspace_messaging_rate_limits1.png %})
...
- [トランザクションメールCampaign]({{site.baseurl}}/user_guide/channels/transactional_email)を使用して送信されたメッセージは、ワークスペースメッセージングレート制限に含まれません。これは、それらのメッセージにはレート制限が適用されますが、設定されたワークスペースメッセージングレート制限のカウント対象にはならないことを意味します。
