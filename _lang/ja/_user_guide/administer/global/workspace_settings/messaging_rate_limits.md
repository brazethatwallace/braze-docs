...
### Webhookの送信キャパシティ {#webhook-sending-capacity}

配信速度レート制限が設定されていないWebhookキャンペーンの場合、Brazeはデフォルトのチャネルレート制限を適用しないため、高スループットで送信が進行する可能性があります。実際のボリュームは、サーバーレイテンシ、コネクテッドコンテンツの使用状況、webhookを受信する外部システムの送信速度など、いくつかの要因に依存します。Webhookキャンペーンにレート制限がない場合は、サーバーが大量のトラフィックに対応できるよう準備してください。それ以外の場合は、レート制限を適用してメッセージのフローを制御してください。
