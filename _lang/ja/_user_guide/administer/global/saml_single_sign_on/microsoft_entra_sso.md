...
セットアップ時に、Assertion Consumer Service（ACS）URL の提供を求められます。

| 要件 | 詳細 |
|---|---|
| Assertion Consumer Service（ACS）URL | `https://<SUBDOMAIN>.braze.com/auth/saml/callback` <br> IDプロバイダーによっては、Reply URL、Audience URL、または Audience URI と呼ばれることもあります。 |
| Entity ID | デフォルトは `braze_dashboard` です。 <br><br> このダッシュボードに一意の Entity ID を設定するには、カスタム Entity ID を有効にし、生成された値（`braze_dashboard_<COMPANY_ID>`）を使用してください。手順については、[カスタム Entity ID の使用]({{site.baseurl}}/user_guide/administer/global/saml_single_sign_on/saml_sso_setup#using-a-custom-entity-id)を参照してください。 |
| RelayState API キー | IDプロバイダーログインを有効にするには、**設定** > **API キー**に移動し、`sso.saml.login` 権限を持つ API キーを作成します。 |
{: .reset-td-br-1 .reset-td-br-2 aria-label="要件" }

## サービスプロバイダー（SP）によるMicrosoft Entra SSO内でのログイン開始 {#service-provider-sp-initiated-login-within-microsoft-entra-sso}
...
5. 同じ **Basic SAML Configuration** セクションで、[カスタムEntity ID]({{site.baseurl}}/user_guide/administer/global/saml_single_sign_on/saml_sso_setup#using-a-custom-entity-id)を使用しない限り、**Identifier (Entity ID)** は `braze_dashboard` のままにします。カスタムEntity IDを使用する場合は、Brazeのセキュリティ設定に表示される値と一致するように、ダッシュボードで生成された値（`braze_dashboard_<COMPANY_ID>`）を入力します。
6. **Relay State** フィールドに生成されたRelay State APIキーを入力して、RelayStateを設定します。

{% alert important %}
**Sign-On URL** フィールドは設定**しないでください**。このフィールドを空のままにして、IdP起点のSAML SSOの問題を防止してください。
{% endalert %}

{: start="7"}
7. Brazeが期待する特定の形式でSAMLアサーションをフォーマットします。ユーザー属性とユーザークレームについて、これらの属性と値をどのようにフォーマットする必要があるかを理解するために、以下のタブを参照してください。
...
{% alert important %}
メールフィールドは、Brazeのユーザーに設定されているものと一致する必要があります。ほとんどの場合、これは `user.userprincipalname` と同じですが、異なる構成の場合は、システム管理者と協力して、これらのフィールドが正確に一致することを確認してください。
{% endalert %}
...
Microsoft Entra管理センターでBrazeを設定すると、Microsoft Entraがターゲット URL（ログインURL）と **x.509** 証明書を提供するので、それをBrazeアカウントに入力します。
...
| 要件 | 詳細 |
|---|---|
| `SAML Name` | ログイン画面のボタンテキストとして表示されます。通常、これはIDプロバイダーの名前です（「Microsoft Entra」など）。 |
| `Target URL` | Microsoft Entraが提供するログインURLです。 |
| `Certificate` | `x.509` PEMエンコード証明書で、IDプロバイダーから提供されます。 |
{: .reset-td-br-1 .reset-td-br-2 aria-label="ステップ3: Braze内でMicrosoft Entra SSOを設定する" }

{% alert tip %}
BrazeアカウントユーザーがSAML SSOのみでサインインするようにしたい場合は、**セキュリティ設定**ページの**認証ルール**で[シングルサインオン認証を制限]({{site.baseurl}}/user_guide/administer/global/saml_single_sign_on/saml_sso_setup#restriction)できます。
{% endalert %}