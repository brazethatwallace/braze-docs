...
| **Segmentsとフィルター** | セグメントの定義はソースワークスペースに残ります。可能な限り同じロジックを使用して、送信先ワークスペースでSegmentsを再構築してください。 |
