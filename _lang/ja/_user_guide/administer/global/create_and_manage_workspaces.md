...
デフォルトでは、Segmentはワークスペース内のすべてのアプリとWebサイトをターゲットにします。1つのアプリまたはWebサイトのみをターゲットにするSegmentを設定するには:

1. 意味のある名前でSegmentを作成します。Brazeでは「All Users ({名前} {プラットフォーム})」という形式を使用しています。たとえば、「All Users (Upon Voyage iOS)」です。
