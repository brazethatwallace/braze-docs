...
> 管理者にとって、セキュリティは懸念事項のリストで優先順位の高い項目です。**セキュリティ設定**ページでは、認証ルール、IP 許可リスト、2 要素認証など、一般的な企業横断的セキュリティ設定の管理に役立ちます。
...
**Enforce complex passwords（複雑なパスワードを強制する）**を選択し、以下の各項目のうち少なくとも 1 つを含むパスワードを要求します：
- 大文字
- 小文字
- 数値
- 特殊文字
...
このフィールドを使用して、Braze がセッションをアクティブにしておく時間を定義します。Braze がセッションを非アクティブと判断した後（定義された分数の間、活動がない場合）、Braze はユーザーをログアウトさせます。2 要素認証が適用されている場合、入力可能な最大分数は 10,080 分（1 週間に相当）です。それ以外の場合は、最大セッション時間は 1,440 分（24 時間に相当）となります。

### シングルサインオン (SSO) 認証 {#single-sign-on-sso-authentication}

パスワードまたは SSO を使用して、ユーザーのログインを制限できます。

[SAML SSO]({{site.baseurl}}/user_guide/administer/global/saml_single_sign_on/) の場合、お客様は実施前に各自の SAML 設定を行う必要があります。お客様が Google SSO を使用する場合、セキュリティ設定ページを適用するだけですみ、その他の作業は不要です。

## ダッシュボードの IP 許可リスト {#dashboard-ip-allowlisting}

表示されているフィールドを使用して、ユーザーがあなたのアカウントにログインできる特定の IP アドレスとサブネット（会社のネットワークや VPN など）を許可リストに追加します。CIDR 範囲として IP アドレスとサブネットをコンマ区切りリストで指定します。指定がない場合、ユーザーはどの IP アドレスからでもログインできます。

## 2 要素認証（2FA） {#two-factor-authentication-2fa}

全社ユーザーに対して 2 要素認証が必須です。これにより、アカウントログに 2 段階目の ID 認証が追加され、ユーザー名とパスワードだけの場合よりも安全になります。ダッシュボードが 2 要素認証に対応できない場合は、カスタマーサクセスマネージャーにお問い合わせください。
...
## 2 要素認証（2FA）の設定 {#setting-up-two-factor-authentication-2fa}

### Authy での 2FA の設定 {#setting-up-2fa-with-authy}

1. デバイスのアプリストアから Authy アプリをダウンロードします。
2. Braze で電話番号を入力します。
3. デバイスに送信された通知をタップして、Authy アプリを開くよう求められます。
4. デバイスで Authy アプリを起動してコードを取得します。
5. Braze で、Authy から受け取った認証コードを入力します。

設定プロセス中に問題が発生し、Braze のホームページまたはログイン画面にリダイレクトされた場合は、以下を試してください：

- シークレットモードまたはプライベートブラウジングモードを使用する：シークレットモードまたはプライベートブラウジングウィンドウで再度設定を試みてください。これにより、ブラウザの拡張機能やプラグインによる問題を回避できます。
- 別のブラウザプロファイルを試す：問題が解決しない場合は、インストールされたプラグインとの競合を排除するために、別のブラウザプロファイルの使用を検討してください。

### 2FA が強制されていない場合の設定 {#setting-up-2fa-when-it-isnt-enforced}

Braze アカウントで 2 要素認証（2FA）が強制されていない場合に手動で有効にするには、以下の手順に従ってください：

1. App Store（iOS）、Google Play ストア（Android）、または Web から Authy、Google Authenticator、Okta Verify などの 2FA アプリをダウンロードします。メールまたは SMS で 2FA を設定する場合は、ステップ 2 に進んでください。
2. Braze で「アカウント管理」に移動し、**Two-Factor Authentication** セクションまでスクロールして、**Start Setup** を選択します。
3. ログインモーダルにパスワードを入力し、**Check Password** を選択します。
4. **Two-Factor Authentication Setup** モーダルで電話番号を入力し、**Enable** を選択します。
5. 2FA アプリ、メール、または SMS メッセージから生成された 7 桁のコードをコピーし、Braze に戻って **Two-Factor Authentication Setup** モーダルに貼り付けます。**Verify** を選択します。
6. （オプション）次の 30 日間 2FA の入力を省略するには、**Remember this account for 30 days** オプションを有効にします。
