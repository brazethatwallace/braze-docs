...
- **権限の割り当てボタン：** ユーザーを編集する際、そのユーザーが管理可能なすべてのワークスペースに対してワークスペースレベルの権限または権限セットを既に持っている場合、**権限を割り当て**ボタンが表示されなくなります。これは、ワークスペースレベルで割り当てるワークスペースが残っていないためです。
...
| ユーザー | ユーザープロファイルを表示（PII非表示） | PII準拠の方法でユーザープロファイルを表示します。この権限を持つユーザーは、「PIIとしてマークされたカスタム属性を表示」権限も持っていない限り、PIIとしてマークされたカスタム属性を参照するキャンペーンを保存または起動できません。<br><br>「ユーザープロファイルを表示（PII非表示）」権限は、使用前に有効化する必要があります。ワークスペースで有効化するには、カスタマーサクセスマネージャーにお問い合わせください。 |
...
| メディアライブラリアセット | メディアライブラリアセットを削除 | メディアライブラリアセットをUIから削除します。削除されたアセットは、それを参照するメッセージが壊れないよう、Brazeによって引き続きホストされます。アセットを完全に削除するには、Brazeサポートにお問い合わせください。 |
...
| オペレーター | BrazeAI<sup>TM</sup> Operatorを使用 | Braze Operatorにアクセスして、質問への回答、設定のナビゲーション、問題のトラブルシューティング、アイデアのブレインストーミングを行います |
