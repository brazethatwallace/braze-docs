...
### ステップ 1: グローバルコントロールグループ設定に移動する {#step-1-navigate-to-the-global-control-group-settings}

ダッシュボードから、**Audience** > **Global Control Group**に移動します。

### ステップ 2: 全ユーザーの一定割合をこのコントロールグループに割り当てる {#step-2-assign-a-percentage-of-all-users-to-this-control-group}

コントロールグループの割合を入力し、**Save**を選択します。入力すると、Brazeはグローバルコントロール、トリートメント、およびトリートメントサンプルに含まれるユーザー数の推定値を表示します。ワークスペース内のユーザー数が多いほど、この推定値はより正確になります。
...
### ステップ 3: 除外設定を割り当てる {#step-3-assign-exclusion-settings}
...
### ステップ 4: コントロールグループを保存する {#step-4-save-your-control-group}
...
グローバルコントロールグループは、**Global Control Group Settings**タブからいつでも無効にできますが、無効にするとこのグループのユーザーがすぐにキャンペーンやキャンバスの対象になることに注意してください。
...
個々のユーザーのプロファイルの**Engagement**タブにある**Miscellaneous**セクションに移動すると、グローバルコントロールグループのメンバーシップを確認できます。

![ユーザーのランダムバケット番号が6356で、グローバルコントロールグループに含まれていないことを報告する「Miscellaneous」セクション。]({% image_buster /assets/img/control_group/control_group1.png %}){: style="max-width:50%;"}
...
グローバルコントロールグループのレポートをダッシュボードから表示するには、**Analytics** > **Global Control Group Report**に移動します。
