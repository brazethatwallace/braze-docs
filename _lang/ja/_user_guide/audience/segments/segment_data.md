...
---
# Segmentデータ {#segment-data}

> このページでは、Brazeダッシュボードのセグメントセクションについて説明し、提供される統計の概要を紹介します。

## Segmentとメンバーシップに関するデータへのアクセス {#accessing-data-about-your-segments-and-membership}

Brazeダッシュボードの**Segments**ページには、すべてのSegmentの概要が表示され、各Segmentの詳細データを確認できます。このページで、Segment名を検索して選択すると、データの編集と表示が可能です。Segmentの作成方法については、[Segmentを作成する]({{site.baseurl}}/user_guide/audience/segments/creating_a_segment/#creating-a-segment)をご覧ください。
...
### Segment統計 {#segment-statistics}

フィルターの追加や削除に応じてリアルタイムで更新される、以下のSegment統計を確認できます。
...
Segmentのメンバーシップとサイズの監視について詳しくは、[Segmentサイズの測定]({{site.baseurl}}/user_guide/audience/segments/measuring_segment_size/)を参照してください。
...
[クエリビルダーのレポートテンプレート]({{site.baseurl}}/user_guide/analytics/reports/query_builder/data_by_segments/)を使用して、Campaigns、Canvas、バリアント、ステップのパフォーマンス指標をSegment別に分解できます。

## クエリビルダーを使用したSegment分解レポートの作成 {#creating-a-segment-breakdown-report-using-query-builder}
...
1. データを確認したいSegmentsに対して[Segment分析トラッキング]({{site.baseurl}}/user_guide/analytics/tracking/segment_analytics_tracking/)が有効になっている。
