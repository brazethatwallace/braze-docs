...
WNSのディープリンクサポートは、ユーザーの送信先に関するデータを含めてアプリケーションを起動するため、これとは異なります。WNSプッシュが作成されると、プッシュがクリックされアプリケーションが開かれたときに、アプリケーションの `OnLaunched` に渡される起動文字列を含めることができます。この起動文字列はすでにCampaignトラッキングに使用されており、アプリが起動したときに解析してユーザーをナビゲートするために使用できる独自のデータを追加する機能をユーザーに提供しています。
