...
## メッセージクレジット - Lambdaのクレジット比率表 {#credit-ratio-table-for-message-credits-lambda}

{% details クリックして展開 %}
<table class="credits-table" aria-label="メッセージクレジット - Lambdaのクレジット比率表">
    <colgroup>
        <col span="3">
...
Brazeは、Brazeプラットフォームから送信されたエージェントコンソールの呼び出し（Invocation）に対してメッセージクレジットを課金します。呼び出しは、エージェントがLLMへのコールを開始した時点で記録されます。デフォルトでは、契約にはサブスクリプション期間の各期間ごとに1万回の呼び出しが含まれています。
...
- RCSの課金は、2つの異なるメッセージタイプ（米国向けの区分あり）に基づいています。
    ...
    - **Rich Media RCS（米国のみ）:** メディアを含むもの、またはよりリッチなサジェスチョン/ボタン（webview、location、calendarなど）を含むテキスト、1メッセージとしてカウント
