...
- RCSの課金は、2つの異なるメッセージタイプに基づいています（米国向けには区別があります）。
    ...
    - **Rich RCS（米国のみ）:** テキストのみ、限定的なサジェスト/ボタン（quickReply、dialPhone、webviewなしのopenURL）を含む場合があり、160 UTF-8バイトごとにセグメント化されます
    - **Rich Media RCS（米国のみ）:** メディアを含むメッセージ、またはよりリッチなサジェスト/ボタン（webview、location、calendarなど）を含むテキスト。1メッセージとしてカウントされます
