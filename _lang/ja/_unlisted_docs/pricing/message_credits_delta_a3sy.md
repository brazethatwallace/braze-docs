...
## メッセージクレジット - Deltaのクレジット比率表 {#credit-ratio-table-for-message-credits-delta}

{% details クリックして展開 %}
<table class="credits-table" aria-label="メッセージクレジット - Deltaのクレジット比率表">
    <colgroup>
        <col span="3">
...
SMSメッセージセグメントは、SMS業界でメッセージを計測する方法です。メッセージセグメントとは、定義された文字数（GSM-7エンコーディングの場合は160文字、UCS-2エンコーディングの場合は67文字）までのグループであり、1回のSMS配信で送信されます。GSM-7エンコーディングで161文字のSMSを配信した場合、2つのメッセージセグメントが送信されたことになります。複数のメッセージセグメントを送信すると、追加料金が発生します。
...
RCSはSMSとMMSの次世代版です。SMSのような直接的で高エンゲージメントなチャネルのメリットを備えつつ、リッチコンテンツ（画像、動画、文書）、認証済みおよびブランド付き送信、候補返信やアクションなどのインタラクティブ機能など、現代の消費者が期待するよりリッチな機能を提供します。
...
Webhookは2024年12月9日にメッセージクレジットの対象となりました。Brazeは、Brazeプラットフォームから送信されたすべてのwebhookに対してメッセージクレジットを課金します。デフォルトでは、契約にはサブスクリプション期間の各期間ごとに10万件のwebhookが含まれています。追加のwebhookは、注文書に基づいて課金されます。
