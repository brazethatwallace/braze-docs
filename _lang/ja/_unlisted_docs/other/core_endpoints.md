...
BrazeのAPIコアエンドポイントは、BrazeのREST APIにおける特定のエンドポイントであり、Brazeアカウントのサービスレベルアグリーメント（SLA）に基づいて許容されるダウンタイムが定められています。詳細については、SLAドキュメントを参照するか、アカウントマネージャーにお問い合わせください。
