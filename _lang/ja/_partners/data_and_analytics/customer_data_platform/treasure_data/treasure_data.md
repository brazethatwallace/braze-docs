...
| Braze REST APIキー | `users.track`、`users.delete`、`users.alias.new`、`users.identify`の権限を持つBraze REST APIキー。<br><br>これはBrazeのダッシュボードで**設定** > **APIキー**から作成できます。 |
...
![REST APIキーとエンドポイントのフィールドがあるトレジャーデータのBraze認証フォーム。]({% image_buster /assets/img/treasure_data/braze_authentication.png %}){: style="max-width:80%;"}
...
![Export ResultsとBraze統合が選択されたトレジャーデータのクエリ結果ページ。]({% image_buster /assets/img/treasure_data/query_2.png %}){: style="max-width:80%;"}
...
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="カスタマイズ" }
