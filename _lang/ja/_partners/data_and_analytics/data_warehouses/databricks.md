...
description: "この記事では、BrazeとのDatabricks Delta Sharing（クローズドベータ）について説明します。これにより、Databricksアカウント内でBrazeのエンゲージメントデータやCampaignデータにアクセスできます。"
page_type: partner
search_tag: Partner
...
{% alert important %}
BrazeとのDatabricks Delta Sharingは**クローズドベータ**です。利用可能状況、対応リージョン、製品の動作は変更される場合があります。参加を希望される場合、またはこの機能がワークスペースで有効になっているかを確認する場合は、Brazeカスタマーサクセスマネージャーにお問い合わせください。
{% endalert %}
...
- Databricks SQLを使用してBrazeのイベントデータやCampaignデータをクエリする
- 複雑なレポートを作成し、アトリビューションモデリングを実行する
- BrazeデータをDatabricksアカウント内の他のデータと結合する
- チャネル、業界、デバイスプラットフォーム全体でエンゲージメントデータをベンチマークする

セットアップ手順については、[Databricks Delta Sharing]({{site.baseurl}}/partners/data_and_analytics/data_warehouses/databricks/delta_sharing/)を参照してください。
...
共有の設定と共有データのクエリの準備ができたら、[Databricks Delta Sharing]({{site.baseurl}}/partners/data_and_analytics/data_warehouses/databricks/delta_sharing/)に進んでください。