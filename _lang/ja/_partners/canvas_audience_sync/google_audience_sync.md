---
nav_title: Google
article_title: キャンバス オーディエンスを Google に同期
...
{% alert note %}
**「モバイル広告 ID が見つかりません。修正しましょう。」バナーについて:** iOS IDFA または Android GAID を一致させるフィールドとして使用してオーディエンスに同期した場合、ステップエディターにこのメッセージが表示されることがあります。これは**情報提供であり、エラーではありません**。マッチングするモバイル広告 ID フィールドがオーディエンスデータに存在すること（例えば、キャンバスパスのユーザーが対応する識別子を収集していること）を確認するようリマインドするものです。データを確認した後で、このメッセージを閉じることができます。
{% endalert %}
...
| *エラーが発生したユーザー数* | エラーのために Google と同期されなかったユーザーの数（約13時間の再試行後）。特定のエラー（Google 広告 API サービスの中断など）が発生すると、キャンバスは最大約13時間にわたって同期を再試行します。その時点で同期がまだ不可能な場合、*同期されていないユーザー*が入力されます。 |
