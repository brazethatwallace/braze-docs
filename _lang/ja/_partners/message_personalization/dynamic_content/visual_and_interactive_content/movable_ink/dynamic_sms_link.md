...
ユーザーが[連絡先カード]({{site.baseurl}}/user_guide/channels/sms_mms_and_rcs/create/contact_card/)または別の方法でブランドを連絡先として保存すると、**「タップしてプレビューを読み込む」**プロンプトとMovable Inkリンクが表示されるようになります。

![1]{: style="max-width:30%;"}

#### ステップ 2:Movable Inkリンクを送信する {#step-2-send-movable-ink-links}

1. Movable InkでSMSキャンペーンを作成し、クリックスルーURLを生成します。
...
ブランドを連絡先として保存しておらず、自動プレビューをオンにしているユーザーは、プレビュー画像を読み込むために**「タップしてプレビューを読み込む」**を選択する必要があります。
