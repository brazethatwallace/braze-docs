...
1. **設定** > **コネクテッドコンテンツ**に移動し、**認証情報を追加**をクリックして認証情報を追加します。
...
Brazeが添付ファイルの `url` からファイルを取得する際の注意点は以下のとおりです。

- **キャッシュ:** Brazeは最近取得したファイルを最大約24時間再利用する場合があります。送信のたびに最新バージョンのファイルを取得する必要がある場合は、バージョンごとに異なるURLを使用してください（たとえば、ファイルが変更されたときに変わるパスやクエリを使用します）。
