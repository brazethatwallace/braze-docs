...
> このエンドポイントを使用して、キャンペーンを複製します。このAPIエンドポイントは、[Brazeダッシュボードでキャンペーンを複製する]({{site.baseurl}}/user_guide/messaging/governance/duplicating)のと同様です。
...
| `tag_names` | オプション | 文字列の配列 | 作成されるキャンペーンのタグ。既存のタグである必要があります。リクエストに新しいタグを追加すると、元のキャンペーンにあったタグが上書きされます。 |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3  .reset-td-br-4 aria-label="リクエストパラメーター" }


## レスポンス {#response}

このエンドポイントは `202` ステータスコードを返し、キャンペーンの作成は非同期で行われます。[セキュリティイベントのダウンロード]({{site.baseurl}}/user_guide/administer/global/admin_settings/security_settings#security-event-report)を使用して、キャンペーンがいつ複製されたか、どのAPIキーによって複製されたかの記録を確認できます。
