...
description: "このランディングページでは、Brazeのメールリストとアドレスのエンドポイントについて説明し、一覧を掲載しています。"
page_type: landing

guide_top_header: "メールリストとアドレスのエンドポイント"
guide_top_text: "このエンドポイントセットを使用すると、ユーザーのメールサブスクリプションステータスを更新したり、Braze APIを使用してBrazeと他のメールシステムまたは独自のデータベースとの間で双方向同期を設定したりできます。"
