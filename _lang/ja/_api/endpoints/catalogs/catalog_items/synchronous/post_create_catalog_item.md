...
このエンドポイントを使用するには、`catalogs.create_item` 権限を持つ[APIキー]({{site.baseurl}}/api/basics#rest-api-key-permissions)が必要です。
...
このエンドポイントには、`201`、`400`、`404` の3つのステータスコードレスポンスがあります。
...
| `id-in-body` | リクエストボディのアイテムIDを削除してください。 |
