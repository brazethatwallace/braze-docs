...
このエンドポイントを使用するには、`catalogs.delete_selection` 権限を持つ [APIキー]({{site.baseurl}}/api/basics#rest-api-key-permissions)が必要です。
...
このエンドポイントには、`202` と `404` の2つのステータスコードレスポンスがあります。
...
ステータスコード `404` は、次のレスポンスボディを返す可能性があります。発生する可能性のあるエラーの詳細については、[トラブルシューティング](#troubleshooting)を参照してください。
