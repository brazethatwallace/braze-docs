...
このエンドポイントを使用するには、`custom_objects.object_relationships.delete` 権限を持つ[APIキー]({{site.baseurl}}/api/basics#rest-api-key-permissions)が必要です。
...
この例では、`acct-123` と `acct-456` の間の `subaccount` リレーションシップを削除します。両方のアカウントレコードはそのまま残ります。
...
| ステータス | 原因 | ガイダンス |
|---|---|---|
| `400` | バリデーションエラー | リクエストボディに有効な `rel_kind`、`related_type_name`、`related_external_id`、および `anchor` の値が含まれていることを確認してください。 |
| `404` | リレーションシップまたはエンドポイントオブジェクトが見つかりません | 両方のオブジェクトが存在し、リレーションシップのキー値が既存のエッジと一致していることを確認してください。 |
| `401` | REST APIキーが欠落しているか無効です | `Authorization` ヘッダーが `Bearer YOUR_REST_API_KEY` を使用しており、キーがアクティブであることを確認してください。 |
| `403` | APIキーに権限がないか、リクエストが許可リストによってブロックされています | キーに `custom_objects.object_relationships.delete` 権限があること、および設定されている場合はソースIPがキーの許可リストに含まれていることを確認してください。 |
