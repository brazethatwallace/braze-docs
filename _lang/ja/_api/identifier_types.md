...
[Canvas]({{site.baseurl}}/user_guide/messaging/canvas/)識別子またはCanvas IDは、ダッシュボード内の特定のCanvasに対してBrazeによって生成されるランダムキーです。Canvas IDは各Canvasに固有であり、APIを介してCanvasesを参照するために使用できます。
