...
description: "이 문서는 Braze와 Twilio 간의 파트너십에 대해 설명합니다."
page_type: update
channel:
  - SMS
  - Webhook
...
웹훅 URL은 Twilio 대시보드에서 제공됩니다. 이 URL은 Twilio 계정 ID(`TWILIO_ACCOUNT_SID`)를 포함하고 있기 때문에 사용자의 Twilio 계정에 고유합니다.

Twilio 예제에서 웹훅 URL은 `https://api.twilio.com/2010-04-01/Accounts/TWILIO_ACCOUNT_SID/Messages.json`입니다. Twilio 콘솔의 *Getting Started* 섹션에서 이 URL을 찾을 수 있습니다.
...
Twilio는 두 개의 요청 헤더, 즉 요청 Content-Type과 [HTTP 기본 인증](https://en.wikipedia.org/wiki/Basic_access_authentication#Client_side) 헤더를 요구합니다. 웹훅 작성기 옆에 있는 톱니바퀴 아이콘을 클릭한 다음 *Add New Pair*를 두 번 클릭하여 웹훅에 추가하세요.
...
`TWILIO_ACCOUNT_SID` 및 `TWILIO_AUTH_TOKEN`을 Twilio 대시보드의 값으로 바꾸세요. 마지막으로, Twilio의 API 엔드포인트는 HTTP POST 요청을 기대하므로 *HTTP Method* 드롭다운에서 해당 옵션을 선택하세요.
