...
## 캠페인 세부 정보 개선 {#campaign-details-improvement}

확장된 캠페인 세부 정보는 이제 **Campaign** 페이지의 ...잠깐만...**Campaign Details** 섹션에 표시됩니다!
...
## "내 항목만 보기" 설정이 유지됨 {#show-only-mine-setting-persists}

Campaigns 그리드의 "내 항목만 보기" 필터는 **Campaigns** 페이지를 방문할 때마다 그대로 유지됩니다.

## A/B 테스트 업데이트 {#ab-testing-updates}

최대 8개의 배리언트(및 선택 사항인 제어)가 포함된 일회성 [A/B 테스트]({{site.baseurl}}/user_guide/messaging/ab_testing/)를 사용자가 지정한 Campaign 오디언스의 비율로 전송한 다음, 미리 스케줄된 시간에 나머지 오디언스에게 최적의 배리언트를 전송할 수 있습니다.