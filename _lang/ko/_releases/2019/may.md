...
또한 Content Cards는 카드 고정, 카드 방출, API 기반 전달, 커스텀 카드 만료 시간, 카드 분석 등 더욱 개인화된 기능을 지원합니다.
...
## Roku 플랫폼 추가 {#roku-platform-addition}

Braze의 기능에 새로운 채널이 추가되었습니다! 새로운 채널로 확장함으로써 고객은 시청 행동을 이해하여 데이터를 강화하거나 모든 관련 채널에서 소비자에게 의미 있는 경험을 제공할 수 있습니다.

이제 데이터 강화 및 커스텀 이벤트 추적을 위해 [Roku 기기에서 데이터를 검색]({{site.baseurl}}/developer_guide/sdk_integration/?sdktab=roku)할 수 있습니다.
...
Campaign 제작 과정에서 이 단계를 강조하는 플랫폼 선택 기능을 통해 인앱 메시지가 전달될 위치와 플랫폼을 쉽게 선택할 수 있습니다.
...
Currents 기능을 지속적으로 개선하기 위해 모든 커넥터 유형에 걸쳐 Currents 이메일 이벤트에 `dispatch_id` 필드를 추가하고 있습니다.

`dispatch_id`는 Braze 플랫폼에서 전송되는 각 전송 또는 발송에 대해 생성된 고유 ID입니다.

스케줄된 메시지를 받는 모든 고객은 동일한 `dispatch_id`를 받지만, 동작 기반 또는 API 트리거된 메시지를 받는 고객은 메시지당 고유한 `dispatch_id`를 받습니다. `dispatch_id` 필드를 사용하면 어떤 반복 Campaign 인스턴스가 전환을 담당하고 있는지 식별할 수 있으므로 어떤 유형의 Campaign이 비즈니스 목표를 달성하는 데 도움이 되는지에 대한 더 많은 인사이트와 정보를 얻을 수 있습니다.

## '내 항목만 보기' Campaign 정렬 기능 {#only-show-mine-campaign-sorting-feature}

사용자가 Campaign 그리드에서 `Only Show Mine` 확인란을 선택하면 로그인한 사용자가 만든 Campaign만 표시되도록 결과가 필터링됩니다. 또한 사용자는 `created_by_me:true`를 입력하여 검색창을 사용할 수 있습니다.

또한 Campaign 그리드 사이드바의 크기를 조정할 수 있습니다!
...
`dispatch_id`를 사용하면 반복 메시지에 각 메시지의 실제 고유 열람 수 또는 고유 클릭 수를 반영할 수 있습니다. 이제 Currents에서 `dispatch_id`를 사용할 수 있으므로 고객은 이 데이터를 쉽게 매칭할 수 있습니다.
