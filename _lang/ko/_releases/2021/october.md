...
이 선택 사항인 고급 [구현 가이드]({{site.baseurl}}/developer_guide/push_notifications/?sdktab=android)에서는 커스텀 `FirebaseMessagingService` 서브클래스를 활용하여 푸시 메시지를 최대한 활용하는 방법을 설명합니다. 저희 팀이 구축한 커스텀 사용 사례와 함께 제공되는 코드 스니펫, 분석 로깅에 대한 안내가 포함되어 있습니다.
