...
사용자 데이터를 기반으로 BRAZE 캔버스에서 메시지(푸시, 이메일, SMS 또는 웹훅)를 트리거하는 데 일반적으로 사용하는 모든 기준을 이제 커스텀 오디언스를 사용하여 Facebook에서 해당 사용자에게 광고를 트리거하는 데 사용할 수 있습니다. 예를 들어 오디언스 동기화를 Facebook에 구성하면 이메일, 전화, 이름, 성 등 다양한 퍼스트파티 필드를 사용할 수 있습니다.
