...
예를 들어, Facebook에 Audience Sync를 구성할 때 이메일, 전화번호, 이름 및 성과 같은 다양한 퍼스트파티 필드를 사용할 수 있지만, Snapchat에서는 이메일, 전화번호 또는 모바일 광고주 ID 중 하나만 선택할 수 있습니다.
...
통합을 연결한 이전 사용자가 더 이상 비즈니스에 소속되어 있지 않은 경우, **Change Account**를 선택하여 새 사용자로 통합을 업데이트해야 합니다. 그런 다음 **확인**을 선택하고 새 사용자로 연결합니다. 이전 사용자에서 새 사용자로 전환하는 동안 동기화가 진행되면 활성 Canvases가 중단될 수 있으므로, Canvas에 사용자가 스케줄된 진입 전과 같이 활성 동기화가 진행되지 않는 시점에 사용자를 변경하는 것을 권장합니다.
