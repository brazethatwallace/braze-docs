...
Canvas에서 Google 오디언스 단계를 설정하기 전에 다음 항목이 생성되고 완료되었는지 확인하세요.
...
Braze SDK를 사용하여 동의 신호를 수집하는 경우 다음 최소 버전을 충족하는지 확인하세요:

{% sdk_min_versions swift:7.6.0 android:1.3.2 web:3.0.0 %}

### EEA, 영국 및 스위스 최종 사용자의 동의 수집 {#collecting-consent-for-eea-uk-and-switzerland-end-users}

Google의 EU 사용자 동의 정책에 따라 광고주는 EEA, 영국 및 스위스 최종 사용자에게 다음 사항을 공개하고 동의를 받아야 합니다:
...
EEA 최종 사용자가 오디언스 목록에 추가된 후 두 가지 동의(`$google_ad_user_data` 또는 `$google_ad_personalization`) 중 하나를 철회한 경우 오디언스 목록을 최신 상태로 유지하려면, 오디언스 동기화 단계를 사용하여 기존 오디언스 목록에서 사용자를 제거하는 Canvas를 설정해야 합니다.

{% alert note %}
EEA 사용자가 이전에 두 신호 모두에 동의한 경우, 해당 목록이 만료되거나 Google 오디언스 동기화를 통해 동의 상태가 명시적으로 업데이트되거나 또는 두 가지 모두가 발생할 때까지 해당 데이터는 Google의 고객 일치에 계속 사용됩니다.
{% endalert %}
...
필요한 동의 필드와 승인 상태 없이 EEA 사용자를 동기화하려고 하면 Google이 이를 거부하고 해당 사용자에게 광고를 게재하지 않습니다. 또한 명시적 동의 없이 EEA 사용자에게 광고가 게재되면 책임을 질 수 있으며 재정적 위험에 처할 수 있습니다. 이를 방지하려면 `true` Google 동의 속성을 가진 EEA, 영국 및 스위스 사용자만 포함하는 Segment 필터가 있는 Campaigns를 전송하는 것이 좋습니다. 고객 일치 업로드 파트너를 위한 EU 사용자 동의 정책에 대한 자세한 내용은 Google의 [FAQ](https://support.google.com/google-ads/answer/14310715)를 참조하세요.
...
Canvas에 컴포넌트를 추가한 다음 **Audience Sync**를 선택합니다.

![편집기에서 Canvas 컴포넌트를 선택하는 메뉴.]({% image_buster /assets/img/audience_sync/audience_sync3.png %}){: style="max-width:35%;"} ![사용자 여정에 추가된 Audience Sync 단계.]({% image_buster /assets/img/audience_sync/audience_sync5.png %}){: style="max-width:28%;"}

### 3단계: 동기화 설정 {#step-3-sync-setup}

1. **Custom Audience**를 선택하여 컴포넌트 편집기를 엽니다.
2. Audience Sync 파트너로 **Google**을 선택합니다.

![동기화를 시작할 파트너를 선택하는 옵션이 있는 Audience Sync 단계 설정.]({% image_buster /assets/img/audience_sync/audience_sync4.png %}){: style="max-width:80%;"}
...
- **Customer Contact Info**: 사용자의 이메일 또는 전화번호, 또는 Braze에 존재하는 경우 둘 다 포함합니다. Google은 별도의 식별자 대신 단일 필드로 동기화하도록 요구합니다. 식별자 중 하나만 있는 경우에도 이 단일 필드를 사용할 수 있습니다.
- **Mobile Advertiser ID**: iOS IDFA 또는 Android GAID 중 하나를 선택합니다. Google의 Customer Match 요구 사항으로 인해 동일한 고객 목록에 두 가지 모바일 광고 ID를 모두 포함할 수 없습니다.

{% alert note %}
**"Missing Mobile Ad IDs? Let's fix that." 배너에 대하여:** iOS IDFA 또는 Android GAID를 매칭 필드로 사용하여 오디언스에 동기화할 때 이 메시지가 단계 편집기에 나타날 수 있습니다. 이것은 **오류가 아닌 안내 메시지**입니다. 매칭에 사용하는 모바일 광고 ID 필드가 오디언스 데이터에 존재하는지(예: Canvas 경로의 사용자에게 해당 식별자가 수집되어 있는지) 확인하라는 알림입니다. 데이터를 확인한 후 닫을 수 있습니다.
{% endalert %}

{: start="4"}
4. 다음으로, 단계 편집기 하단의 **Create Audience** 버튼을 선택하여 오디언스를 저장합니다.

![커스텀 오디언스 Canvas 컴포넌트의 확장된 보기. 여기에서 원하는 광고 계정이 선택되고, 새 오디언스가 생성되며, "customer contact info" 체크박스가 선택되어 있습니다.]({% image_buster /assets/img/audience_sync/g_sync.png %})

오디언스가 성공적으로 생성되거나 이 과정에서 오류가 발생하면 단계 편집기 상단에 알림이 표시됩니다. 오디언스가 초안 모드로 생성되었으므로 나중에 Canvas 여정에서 사용자 제거를 위해 이 오디언스를 참조할 수 있습니다.
...
3. Braze는 사용자가 Google 오디언스 단계에 진입할 때 거의 실시간으로 사용자를 추가하거나 제거합니다.
4. Google 오디언스 단계를 구성한 후 **Done**을 선택합니다. Google 오디언스 단계에 새 오디언스에 대한 세부 정보가 표시됩니다.

![커스텀 오디언스 Canvas 컴포넌트의 확장된 보기. 여기에서 원하는 광고 계정과 기존 오디언스가 선택되어 있으며, "Add user to Audience" 라디오 버튼이 선택되어 있습니다.]({% image_buster /assets/img/audience_sync/g_sync2.png %})
...
오디언스가 Google에 동기화되는 데 6시간에서 12시간 정도 소요될 수 있습니다.

### 오디언스를 동기화했는데 Google에서 오디언스 크기가 0으로 표시되는 이유는 무엇인가요? {#ive-synced-an-audience-so-why-is-the-audience-size-in-google-zero}

개인정보 보호를 위해 사용자 목록 크기는 목록에 최소 1,000명의 멤버가 포함될 때까지 0으로 표시됩니다. 그 이후에는 크기가 가장 유효한 두 자릿수로 반올림됩니다.
...
일부 인증 오류는 자동으로 해결됩니다. Canvas **오디언스 동기화** 분석(예: *동기화된 사용자* 및 *오류 발생 사용자*)을 확인하여 사용자가 여전히 동기화되고 있는지 확인하세요. 문제가 계속되면 **파트너 통합** > **기술 파트너** > **Google Ads**로 이동하여 **Google 오디언스 동기화**를 찾고, **계정 변경**을 사용하여 필요한 액세스 권한과 완료된 설정을 갖춘 Google Ads 계정으로 다시 연결하세요.