...
description: "이 참고 문서에서는 Braze와 StackAdapt의 파트너십에 대해 설명합니다."
...
> [StackAdapt](https://www.stackadapt.com/)는 디지털 마케터가 타겟팅된 성과 중심 광고를 제공하는 데 사용하는 선도적인 AI 기반 마케팅 플랫폼입니다.

_이 통합은 StackAdapt에서 유지 관리합니다._

Braze와 StackAdapt 통합을 통해 Braze의 사용자 프로필 데이터를 StackAdapt 데이터 허브에 동기화할 수 있습니다. 두 플랫폼을 연결하면 고객에 대한 통합된 뷰를 생성하고 퍼스트파티 데이터를 활성화하여 광고 성과를 개선할 수 있습니다.
...
StackAdapt 데이터 허브는 Braze 계정에 직접 연결하여 사용자 프로필 속성을 가져옵니다. 이를 통해 StackAdapt 내에서 직접 Braze 고객 데이터를 활용하여 고급 오디언스 세분화 및 활성화를 진행할 수 있습니다.

### 데이터 흐름 {#data-flow}

1. StackAdapt는 제공된 API 자격 증명을 사용하여 Braze 인스턴스에 대한 보안 연결을 시작합니다.
2. StackAdapt는 사용자 프로필 데이터, 특히 사용자가 선택하고 매핑한 속성을 검색합니다.
3. 데이터는 정규화되어 StackAdapt 데이터 허브에 수집되며, 세분화하여 Campaign에 사용할 수 있게 됩니다.
4. 통합을 통해 예약된 데이터 동기화(예: 매일)가 가능하므로 StackAdapt 오디언스를 Braze의 최신 프로필 데이터로 최신 상태로 유지할 수 있습니다.

## 동기화되는 필드 {#fields-synced}
...
마케팅 커뮤니케이션에 대한 사용자 동의를 반영하는 필드(예: 이메일 구독 상태)를 Braze에서 정확하게 매핑하여 광고 활동이 사용자 선호도 및 개인정보 보호 규정을 준수하도록 하는 것이 중요합니다.
...
![StackAdapt의 Braze 연결 설정 화면.]({% image_buster /assets/img/stackadapt/stackadapt_braze_connection_settings.png %})

{: start="6"}
6. 연결을 선택하고 StackAdapt 광고주를 선택합니다.
7. **Property Mappings**를 구성합니다. StackAdapt가 제안하는 기본 매핑과 미리 선택된 속성을 검토하고 확인합니다.
8. (선택 사항) 추가 속성을 가져오려면 해당 확인란을 선택하고 PII 포함 여부와 데이터 유형을 지정합니다.

![StackAdapt의 Braze 속성 매핑 화면.]({% image_buster /assets/img/stackadapt/stackadapt_mappings.png %})

{: start="9"}
9. 프로필을 **목록**에 추가하거나 새 목록을 만들어 프로필을 그룹화하고 세분화할 수 있습니다.
10. **Activate Integration**을 선택하여 초기 데이터 동기화를 시작합니다.

## 고려 사항 {#considerations}

- **커스텀 이벤트 및 속성 가져오기:** 이 기능은 아직 지원되지 않습니다.
- **데이터 지연 시간:** 모든 사용자 프로필 데이터를 가져오는 데 최대 24시간이 걸릴 수 있습니다.
- **동의 관리:** Braze에서의 데이터 수집 관행이 개인정보 보호 규정을 준수하는지, 광고 목적으로 고객 데이터를 사용하는 데 필요한 동의를 받았는지 확인하세요. StackAdapt는 소스 시스템에서 전달된 동의 상태에 의존합니다.
- **속성 일관성:** 데이터의 효과를 극대화하려면 Braze에서 속성의 이름을 지정하고 채우는 방식에 일관성을 유지한 후 StackAdapt에 동기화하세요.