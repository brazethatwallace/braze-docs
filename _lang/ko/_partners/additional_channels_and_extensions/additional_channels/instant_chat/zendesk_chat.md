...
description: "Zendesk Chat을 Braze와 통합하고 양방향 단문 메시지 서비스 대화를 설정하는 방법을 알아보세요."
...
> [Zendesk Chat](https://www.zendesk.com/service/messaging/)은 각 플랫폼의 웹훅을 사용하여 양방향 단문 메시지 서비스 대화를 설정합니다. 사용자가 고객지원을 요청하면 Zendesk에 티켓이 생성됩니다. 상담원 응답은 API 트리거 단문 메시지 서비스 Campaign을 통해 Braze로 전달되고, 사용자 답장은 다시 Zendesk로 전송됩니다.
...
Braze 단문 메시지 서비스 기능과 Zendesk 실시간 상담원 응답을 결합하여 사용자 문의에 대해 사람이 직접 신속하게 지원함으로써 고객지원 효율성을 높이세요.
...
### 2단계: 아웃바운드 단문 메시지 서비스 Campaign 생성하기 {#step-2-create-an-outbound-sms-campaign}

다음으로, Zendesk의 웹훅을 수신하고 고객에게 커스텀 단문 메시지 서비스 응답을 보내는 단문 메시지 서비스 Campaign을 생성합니다.
...
2. 새 **트리거**를 생성합니다(예: **Respond via 단문 메시지 서비스 Braze**).
...
### 6단계: 인바운드 단문 메시지 서비스 전달 설정하기 {#step-6-set-up-inbound-sms-forwarding}

다음으로, Braze에서 두 개의 새 웹훅 Campaign을 생성하여 고객의 인바운드 단문 메시지 서비스를 Zendesk 받은편지함으로 전달할 수 있도록 합니다.

| Campaign | 목적 |
|--------------------|--------------------------------------------------------------------------------------|
| 웹훅 Campaign 1 | Zendesk에 새 티켓을 생성합니다. |
| 웹훅 Campaign 2 | 고객이 인바운드로 보낸 모든 대화형 단문 메시지 서비스 응답을 Zendesk로 전달합니다. |
{: .reset-td-br-1 .reset-td-br-2 aria-label="Step 6: Set up inbound-단문 메시지 서비스 forwarding" }

#### 6.1단계: 단문 메시지 서비스 키워드 카테고리 생성하기 {#step-61-create-an-sms-keyword-category}

Braze 대시보드에서 **Audience**로 이동하여 **단문 메시지 서비스 구독 그룹**을 선택한 다음 **Add Custom Keyword**를 선택합니다. 다음 필드를 작성하여 Zendesk 전용 단문 메시지 서비스 키워드 카테고리를 생성합니다.
...
{: .reset-td-br-1 .reset-td-br-2 aria-label="Step 6.1: Create an 단문 메시지 서비스 keyword category" }
...
**Schedule Delivery**에서 **Action-Based Delivery**를 선택한 다음 트리거 유형으로 **Send an 단문 메시지 서비스 Inbound Message**를 선택합니다. 또한 이전에 설정한 단문 메시지 서비스 구독 그룹과 키워드 카테고리를 추가합니다.
...
사용자의 나머지 단문 메시지 서비스 메시지를 Zendesk로 전달하는 웹훅 Campaign을 설정합니다:
