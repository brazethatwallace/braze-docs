...
* Braze 세분화 및 트리거 기능을 활용하여 개인화된 인앱 알림을 시작할 수 있습니다.
* Sendbird Notifications 플랫폼에서 맞춤형 인앱 알림을 생성하면, 앱 환경 내에서 전달되어 사용자 참여를 향상시킵니다.

Braze와 Sendbird Notifications의 결합된 기능을 활용하여 기업은 효과적인 인앱 알림 전략을 통해 고객 참여를 높이고 더 높은 전환율을 달성할 수 있습니다.
...
![Braze와 Sendbird Notifications 통합의 마케팅 및 트랜잭션 메시징 사용 사례를 요약한 다이어그램.]({% image_buster /assets/img/sendbird/use-cases.png %})

Braze와 Sendbird Notifications 통합은 고객 참여를 높이고 뛰어난 사용자 경험을 제공하기 위한 다양한 사용 사례를 제공합니다:

- **마케팅**: 검색 기록이나 과거 구매를 기반으로 한 독점 할인 등 사용자의 선호도에 맞춘 개인화된 프로모션 및 추천으로 타겟 캠페인을 강화합니다.
- **트랜잭션**: 주문 상태, 배송 세부 정보, 예상 배송 시간에 대한 알림을 포함하여 주문, 배송, 청구 및 결제에 대한 실시간 업데이트를 통해 고객 커뮤니케이션을 향상시킵니다.

## 통합 {#integration}

### 1단계: 알림 템플릿 생성 {#step-1-create-a-notification-template}

[Sendbird 템플릿](https://sendbird.com/docs/notifications/v1/templates)을 사용하면 각 채널에 대해 여러 템플릿을 구축하고 사용하여 개인화된 인앱 알림을 보낼 수 있습니다. 템플릿은 코드를 작성하지 않고도 Sendbird 대시보드에서 생성하고 커스터마이즈할 수 있습니다.

![알림 템플릿을 생성하기 위한 Sendbird 대시보드 템플릿 편집기.]({% image_buster /assets/img/sendbird/sendbird-dashboard-template.png %})

### 2단계: Sendbird 대시보드에서 Braze 통합 설정 {#step-2-set-up-the-braze-integration-on-sendbird-dashboard}
...
### 3단계: Sendbird Notification Builder 설치 {#step-3-install-sendbird-notification-builder}
...
#### 확장 프로그램에 Sendbird 자격 증명 추가 {#add-sendbird-credentials-to-the-extension}

확장 프로그램이 설치되면 브라우저 도구 모음에서 Sendbird 아이콘을 클릭하고 **Settings**를 선택합니다. 여기에서 **Sendbird Notification Builder**에 있는 앱 ID와 API 토큰을 입력합니다.

### 4단계: Sendbird 사용자 ID를 Braze 사용자 ID에 매핑 {#step-4-map-sendbird-user-id-to-braze-user-id}
...
3. **Receiver** 필드에서 <i class="fas fa-plus" aria-label="추가"></i> 아이콘을 클릭하고 Sendbird 사용자 ID에 매핑된 사용자 속성을 삽입합니다.
    - 커스텀 속성 `sendbird_id`를 Sendbird 사용자 ID로 사용하는 경우 `{{ '{{' }}custom_attribute.${sendbird_id}}}`.
    - Braze 사용자 ID를 Sendbird 사용자 ID로 사용하는 경우 `{{ '{{' }}${user_id}}}`.
...
1. 새 Canvas 또는 기존 Canvas에서 **메시지** 구성요소를 추가합니다.
2. 구성요소를 열고 **메시징 채널**에서 **Webhook**을 선택합니다.
3. 위에서 생성한 웹훅 템플릿을 선택합니다. Canvases에는 실시간 엔드포인트를 사용하는 것을 강력히 권장합니다.
4. **작성** 탭에서 변수를 편집하여 템플릿을 커스터마이즈합니다.

## 커스터마이제이션 {#customization}

### 전달 및 열람 상태 추적 {#track-delivery-and-open-status}

알림의 전달 및 열람 상태 이벤트를 Campaign의 전환 측정기준과 통합하려면 Braze 대시보드에서 커스텀 이벤트를 추가합니다.

1. Braze 대시보드에서 **설정** > **설정 관리** > **커스텀 이벤트**로 이동하고 **+ 커스텀 이벤트 추가**를 클릭합니다.
2. 커스텀 이벤트를 생성한 후 **속성정보 관리**를 클릭하고 "status"라는 이름의 속성정보를 추가한 다음 속성정보 유형으로 "String"을 선택합니다.
3. Campaigns 또는 Canvases에서 알림을 작성할 때 **Event Name** 필드에 커스텀 이벤트 이름을 입력합니다.

이 커스텀 이벤트는 각 알림에 대해 두 번 트리거됩니다. 메시지가 전송될 때와 사용자가 메시지를 열 때입니다.
