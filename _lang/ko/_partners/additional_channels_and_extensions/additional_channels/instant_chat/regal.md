...
Braze 데이터를 사용하여 AI 에이전트가 무엇을 말하고, 어떻게 응답하며, 언제 참여할지를 결정합니다. 대화 성과와 인사이트를 Braze로 다시 전송하여 타겟팅과 생애주기 마케팅을 개선합니다. 고객 여정의 핵심 순간에 AI 기반 전화와 SMS를 트리거하고, 각 대화에서 발생한 내용을 기반으로 Braze에서 후속 조치를 수행합니다.
...
유일한 필수 식별자는 `traits.phones` 내의 전화번호입니다. `traits.phones` 객체를 사용하여 하나 이상의 전화번호를 연락처에 연결할 수 있습니다. 각 전화번호에는 고유한 레이블, 기본 번호 지정, 음성 및 SMS 옵트인 상태를 저장할 수 있습니다. 이 구조는 연락처에 여러 전화번호가 있을 때 특히 유용합니다.
...
이 페이로드 예시는 나열된 전화번호에 현재 음성 및 SMS 동의 상태가 포함되어 있다고 가정합니다. 그렇지 않은 경우, 연락처를 생성할 때 `voiceOptIn`과 `smsOptIn`을 생략하고 옵트인이 수집될 때 해당 전화번호의 동의 상태를 업데이트하는 별도의 Canvas 또는 Campaign을 설정할 수 있습니다.
