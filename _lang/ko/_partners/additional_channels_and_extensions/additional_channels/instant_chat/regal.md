...
Braze 데이터를 사용하여 AI 에이전트가 무엇을 말하고, 어떻게 응답하며, 언제 참여할지를 결정합니다. 대화 성과와 인사이트를 Braze로 다시 전송하여 타겟팅과 생애주기 마케팅을 개선합니다. 고객 여정의 핵심 순간에 AI 기반 전화와 단문 메시지 서비스를 트리거하고, 각 대화에서 발생한 내용을 기반으로 Braze에서 후속 조치를 수행합니다.
...
| Regal API 키 | Regal API 키를 사용하면 Braze에서 Regal로 이벤트를 보낼 수 있습니다.<br><br>이 키를 받으려면 [support@regal.io](mailto:support@regal.io)로 이메일을 보내세요. |
| Braze 데이터 변환 | Regal에서 데이터를 수신하려면 [데이터 변환]({{site.baseurl}}/user_guide/data/unification/data_transformation)이 필요합니다. |
...
Regal에서 전화 및 문자 메시지에 사용할 수 있어야 하는 새 Braze 프로필을 생성할 때마다 Regal에 웹훅을 전송하는 Canvas 또는 Campaign을 만드세요.

1. "Create New Contact for Regal"이라는 제목의 Canvas 또는 Campaign을 만들고 진입 유형으로 **액션 기반**을 선택합니다.

2. 트리거 로직을 **커스텀 이벤트**로 설정한 다음, 전화번호가 포함된 프로필이 생성될 때 발생하는 이벤트를 선택합니다. Regal에서는 전화번호 필드가 설정되었는지 확인하는 필터를 추가하는 것도 권장합니다.
...
Regal에는 인증을 위한 HTTP 헤더와 HTTP 메서드도 필요합니다. 다음은 **설정** 탭에서 키-값 페어로 템플릿에 포함되어 있습니다:
{% raw %}
- **HTTP 메서드**: POST
- **요청 헤더**:
...
유일한 필수 식별자는 `traits.phones` 내의 전화번호입니다. `traits.phones` 객체를 사용하여 하나 이상의 전화번호를 연락처에 연결할 수 있습니다. 각 전화번호에는 고유한 레이블, 기본 번호 지정, 음성 및 단문 메시지 서비스 옵트인 상태를 저장할 수 있습니다. 이 구조는 연락처에 여러 전화번호가 있을 때 특히 유용합니다.
...
이 페이로드 예시는 나열된 전화번호에 현재 음성 및 단문 메시지 서비스 동의 상태가 포함되어 있다고 가정합니다. 그렇지 않은 경우, 연락처를 생성할 때 `voiceOptIn`과 `smsOptIn`을 생략하고 옵트인이 수집될 때 해당 전화번호의 동의 상태를 업데이트하는 별도의 Canvas 또는 Campaign을 설정할 수 있습니다.

### 2단계: 옵트인 정보 업데이트 {#step-2-update-opt-in-information}

앱에서 옵트인 및 옵트아웃이 서로 다른 시점에 발생할 수 있는 경우, 사용자가 구독 상태를 변경하면 Regal에 업데이트를 전송합니다.

Regal에서는 연락처 수준이 아닌 전화번호별로 옵트인 및 옵트아웃을 관리할 수 있도록 `traits.phones` 스키마를 사용하는 것을 권장합니다.

다음 Canvas 설정을 사용하여 최신 옵트인 정보를 Regal에 전송하세요.

1. "Send Opt In or Out to Regal"이라는 제목의 새 Canvas 또는 Campaign을 만듭니다.

2. 다음 트리거 옵션 중 하나를 선택한 다음, 사용자의 옵트인 상태를 나타내는 필드를 선택합니다:
    - **사용자 프로필 필드 업데이트됨**
    - **구독 그룹 상태 업데이트**
    - **구독 상태**
...
Regal에 전송하려는 각 주요 이벤트에 대해 Canvas 또는 Campaign을 설정합니다.

이러한 이벤트는 단순히 아웃리치를 트리거하는 것(예: 리드가 가입을 완료했을 때 확인 문자 전송) 이상의 역할을 합니다. Regal AI 에이전트가 대화하고, 의사결정을 내리고, 고객 여정 전반에 걸쳐 대화를 라우팅하는 방식을 구동하는 실시간 컨텍스트를 제공합니다. Braze에서 이벤트 데이터와 속성을 전송하면 AI 에이전트가 각 사용자의 행동, 선호도, 생애주기 단계에 따라 대화를 맞춤 조정할 수 있습니다.

예를 들어, Braze 이벤트와 속성을 Regal에서 다음과 같이 활용할 수 있습니다:

- **AI 에이전트 대화 개인화**: 최근 행동이나 제품 관심사를 대화에서 직접 참조합니다.
  - 예시: 사용자가 생명보험 옵션을 탐색한 경우, 에이전트가 대화에서 `contact.firstName`과 `contact.brazeProductInterest`를 참조할 수 있습니다.
- **동적 대화 로직 구동**: 에이전트가 실시간으로 우선순위를 조정합니다.
  - 예시: `contact.brazeAge`가 65 이상인 경우 Medicare 보장을 우선시하고, 그렇지 않으면 ACA 플랜과 현재 보험 상태에 집중합니다.
- **지능형 라우팅 및 에스컬레이션 구현**: 가치 또는 의도에 따라 대화를 라우팅합니다.
  - 예시: `contact.brazeLeadTier`가 "High Value"인 경우, 자격 확인 후 시니어 에이전트에게 전달하고, 그렇지 않으면 AI 에이전트가 계속 대화를 진행합니다.
- **메시지 및 오퍼 정렬**: Campaign 컨텍스트에 따라 에이전트가 제시하는 내용을 맞춤 조정합니다.
  - 예시: `contact.brazeCampaignName`이 "Spring Mortgage Promo"인 경우, 대화 중 프로모션 오퍼를 강조합니다.

"Send Product Interest Event to Regal"이라는 제목의 새 Canvas 또는 Campaign을 만드세요.
...
Regal에서는 주요 이벤트 발생 시 최신 연락처 속성을 보유할 수 있도록 이벤트 페이로드에 주요 사용자 프로필 속성을 함께 전송하는 것을 권장합니다.

{% alert note %}
Regal에 전송할 이벤트 또는 이러한 Canvases와 Campaigns의 설정 방법에 대해 질문이 있는 경우 [support@regal.io](mailto:support@regal.io)로 이메일을 보내주세요.
{% endalert %}

## 통합: Regal에서 Braze로 데이터 전송 {#integration-sending-data-from-regal-to-braze}

Regal 리포팅 웹훅과 Braze 데이터 변환을 사용하여 Regal 리포팅 이벤트(예: `SMS.sent` 및 `call.completed`)를 Braze로 전송합니다. 이러한 이벤트를 매핑하면 사용자 프로필에 표시되며 세분화, Canvas 및 Campaigns에 활용할 수 있습니다.
...
3. 변환 목록에서 <i class="fa-solid fa-ellipsis-vertical" title="작업 보기"></i> **작업 보기**를 선택한 다음 **웹훅 URL 복사**를 선택합니다.

### 2단계: Regal에서 리포팅 웹훅 활성화 {#step-2-enable-reporting-webhooks-in-regal}

리포팅 웹훅을 설정하려면 다음을 수행합니다.
1. Regal 앱으로 이동하여 **설정** 페이지를 엽니다.
...
현재 Regal은 이러한 이벤트를 재시도하지 않습니다. Braze가 5초 이내에 응답하지 않으면 Regal은 해당 이벤트를 삭제합니다. Regal은 향후 릴리스에서 재시도 기능을 추가할 예정입니다.

#### 이벤트 {#events}
리포팅 이벤트의 전체 목록, 속성정보 정의 및 샘플 페이로드에 대해서는 Regal의 [리포팅 웹훅 가이드](https://developer.regal.io/docs/reporting-webhooks#events)를 참조하세요.

### 3단계: Regal 이벤트를 Braze 이벤트로 변환 {#step-3-transform-regal-events-into-braze-events}

Braze [데이터 변환]({{site.baseurl}}/user_guide/data/unification/data_transformation) 기능을 사용하면 수신되는 Regal 이벤트를 Braze에서 속성, 이벤트 또는 구매로 추가하는 데 필요한 형식으로 매핑할 수 있습니다.

1. 데이터 변환에 이름을 지정합니다. 이벤트 웹훅별로 데이터 변환을 설정하는 것이 좋습니다.

2. 연결을 테스트하려면 Regal 에이전트 데스크톱에서 자신의 전화번호로 아웃바운드 통화를 생성하고 대화 요약 양식을 제출하여 `call.completed` 이벤트를 생성합니다.

3. Regal 연락처를 Braze 프로필에 매핑하는 데 사용할 식별자를 결정합니다. Regal 이벤트에서 사용 가능한 식별자는 다음과 같습니다.
   - `userId` - 이전에 해당 연락처에 대해 이 식별자를 전송한 경우에만 이벤트에 설정됩니다
   - `traits.phone`
   - `traits.email` - 이전에 해당 연락처에 대해 이 식별자를 전송한 경우에만 이벤트에 설정됩니다

Braze에서 Regal로의 이벤트 페이로드에서 Regal은 여러 전화번호와 전화번호 수준의 동의를 지원하기 위해 `traits.phones` 사용을 권장합니다. Braze로 다시 전송되는 Regal 리포팅 이벤트에서는 `traits.phone`이 이벤트 페이로드의 식별자로 여전히 표시될 수 있습니다.

#### Braze에서 지원하는 식별자 {#braze-supported-identifiers}
- Braze는 전화번호를 식별자로 지원하지 않습니다. 전화번호를 식별자로 사용하려면 Braze에서 [사용자 별칭]({{site.baseurl}}/user_guide/data_and_analytics/user_data_collection/user_profile_lifecycle#user-aliases)으로 설정할 수 있습니다.
- Braze 데이터 변환을 사용할 때 이메일 주소를 식별자로 사용할 수 있습니다. 해당 이메일 주소가 Braze에 프로필로 존재하면 기존 프로필이 업데이트됩니다. 해당 이메일 주소가 Braze에 아직 존재하지 않으면 이메일 전용 프로필이 생성됩니다.
...
**Regal의 통화 처리 결과를 기반으로 Braze에서 이메일 트리거하기**

다음 샘플 페이로드는 Regal의 `call.completed` 이벤트를 보여줍니다.
...
다음 샘플 데이터 변환은 이를 Braze의 커스텀 이벤트로 매핑합니다.
...
**Regal의 `contact.attribute.edited` 이벤트를 기반으로 Braze에서 프로필 속성 업데이트하기**

다음 샘플 페이로드는 Regal의 `contact.attribute.edited` 이벤트를 보여줍니다. Regal은 상담원이 대화 중 연락처 프로필의 속성을 업데이트할 때 이 이벤트를 전송합니다.
...
다음 샘플 데이터 변환은 새로운 커스텀 속성 값을 Braze 프로필의 관련 속성에 매핑합니다.
...
**`contact.experiment.assigned` 이벤트를 사용하여 Braze와 Regal의 실험을 동기화 유지하기**

다음 샘플 페이로드는 Regal의 `contact.experiment.assigned` 이벤트를 보여줍니다.
...
**Regal의 `contact.unsubscribed` 이벤트를 기반으로 Braze에서 연락처 구독 취소하기**

다음 샘플 페이로드는 Regal의 `contact.unsubscribed` 이벤트를 보여줍니다.
...
다음 샘플 데이터 변환은 Braze에서 해당 연락처의 구독을 취소합니다.
...
**Regal의 `call.analysis.available` 이벤트를 기반으로 Braze에서 맞춤형 후속 여정 트리거하기**

Regal의 `call.analysis.available` 이벤트를 사용하여 고객이 전환하지 않은 주요 이유를 식별하고 Braze에서 맞춤형 후속 여정을 트리거할 수 있습니다.

예를 들어:

- 주요 반대 이유가 가격인 경우, 가치 중심의 후속 이메일을 발송합니다.
- 주요 반대 이유가 타이밍인 경우, 사용자를 나중에 재고려할 수 있도록 육성 시퀀스에 배치합니다.
- 주요 반대 이유가 신뢰인 경우, 후기, 평점 또는 규정 준수 관련 안내를 발송합니다.
- `needs_human_agent`가 true인 경우, 영업 또는 지원 팀에 알리고 추가 자동화 메시징을 중단합니다.

다음 샘플 페이로드는 Regal의 `call.analysis.available` 이벤트를 보여줍니다.
...
데이터 변환을 사용하여 `call_analysis` 필드(예: `primary_objection` 및 `needs_human_agent`)를 Braze 커스텀 이벤트 또는 프로필 속성에 매핑할 수 있습니다. 그런 다음 Braze에서 해당 값에 따라 분기하는 Canvas 또는 Campaign 로직을 구축합니다.

{% endtab %}
{% tab 통화 기록 링크 저장 %}

**`call.transcript.available` 이벤트의 기록 링크로 프로필 속성 업데이트하기**

`call.transcript.available` 이벤트를 사용하여 전체 통화 기록 링크를 Braze에 전송합니다. 데이터 변환을 통해 기록 URL을 Braze 사용자 프로필 속성에 매핑하면, 팀에서 고객 프로필에서 대화를 확인하고 검토할 수 있습니다.

다음 샘플 페이로드는 Regal의 `call.transcript.available` 이벤트를 보여줍니다.
