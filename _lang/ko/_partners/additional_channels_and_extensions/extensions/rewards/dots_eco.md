...
> [DOTS.ECO](https://dots.eco)를 사용하면 추적 가능한 디지털 인증서를 통해 실제 환경에 미치는 영향에 대해 사용자에게 보상할 수 있습니다. 각 인증서에는 공유 가능한 인증서 URL 및 이미지 URL과 같은 메타데이터를 포함할 수 있으므로, 사용자는 자신의 영향력 증명을 확인하고 다시 방문할 수 있습니다.

_이 통합은 DOTS.ECO에서 유지 관리합니다._
...
### 1단계: Canvas를 생성하고 사용자 업데이트 단계 추가하기 {#step-1-create-a-canvas-and-add-a-user-update-step}
...
### 2단계: 고급 JSON 작성 - 연결된 콘텐츠를 사용하여 DOTS.ECO에 POST 요청하기 {#step-2-compose-advanced-json-make-a-post-request-to-dotseco-using-connected-content}

**사용자 업데이트** 단계에서 **고급 JSON 편집기**로 전환하고 연결된 콘텐츠를 사용하여 DOTS.ECO 인증서 API에 POST 요청을 합니다.
...
`https://impact.dots.eco/api/v1/certificate/add?format=sdk`로 요청을 보냅니다.
...
### 3단계: 메시지에 인증서 표시하기 {#step-3-display-the-certificate-in-messages}
...
![DOTS.ECO 메시지 작성 섹션.]({% image_buster /assets/img/dots_eco/dotseco_messages_compose.png %})

예를 들어:
- {% raw %}`{{custom_attribute.${certificate_image_url}}}`{% endraw %}를 사용하여 인앱 메시지에 인증서 이미지를 표시합니다.
- {% raw %}`{{custom_attribute.${certificate_url}}}`{% endraw %}를 사용하여 호스팅된 인증서에 연결합니다.

![DOTS.ECO 메시지 클릭 시 동작.]({% image_buster /assets/img/dots_eco/dotseco_messages_compose_onclickbehavior.png %})
...
Braze 대시보드의 **설정** > **메시지 활동 로그**에서 연결된 콘텐츠 오류를 검토합니다.

- **연결된 콘텐츠가 빈 값을 반환하는 경우**: `:save result`가 설정되어 있고 예상 응답 필드를 올바르게 참조하고 있는지 확인합니다.
- **속성이 메시지 단계에 표시되지 않는 경우**:
...
- **`422` 오류(처리할 수 없는 엔터티)**: 앱 토큰과 임팩트 수량이 유효한지 확인합니다.
- **`401` 오류**: 인증 토큰이 존재하고 올바른지 확인합니다.
- **메시지 단계에 이미지 미리보기가 표시되지 않는 경우**: 사용자 업데이트 단계에서 **사용자에게 테스트 보내기**를 선택한 다음 동일한 사용자를 사용하여 메시지를 미리 봅니다.