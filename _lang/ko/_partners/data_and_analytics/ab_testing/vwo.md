...
VWO를 Braze와 통합하면 VWO 실험 데이터를 활용하여 타겟 세그먼트를 생성하고 개인화된 캠페인을 전달할 수 있습니다.
...
1. VWO 계정에 로그인합니다.
2. VWO 대시보드에서 **Configurations > Integrations**로 이동합니다. 여기에서 워크스페이스 수준에서 통합을 활성화할 수 있으며, 이렇게 하면 기본적으로 향후 모든 테스트 캠페인에 통합이 적용됩니다.

   ![VWO 통합 구성]({% image_buster /assets/img/vwo/vwo1_settings.png %})

4. Braze 통합을 선택하여 활성화합니다.
5. 선택적으로, 기존 캠페인에 대해서도 Braze 통합을 활성화할 수 있습니다. 이렇게 하려면 캠페인을 선택한 다음 **Configuration > Integrations**로 이동하여 Braze를 활성화합니다.

   ![Braze 통합 활성화]({% image_buster /assets/img/vwo/vwo2_enable_braze.png %})

6. 통합을 활성화하면 VWO가 캠페인 수준에서 실험 데이터를 Braze로 전송하기 시작합니다.
...
VWO는 다음 형식을 사용하여 캠페인 실험 데이터를 커스텀 이벤트로 Braze에 전송합니다:
...
1. 테스트 캠페인이 실행 중인 페이지에서 마우스 오른쪽 버튼을 클릭하고 **Inspect Element**를 선택합니다.
...
6. 0: 및 data:를 확장하면 n: "VWO"(커스텀 이벤트 이름) 및 p: {vwo_campaign_name: "<VWO 캠페인 이름>", vwo_variation_name: "<변형 이름>"}을 확인할 수 있습니다. 이는 VWO에서 Braze로 값이 전송되고 있음을 나타냅니다.
