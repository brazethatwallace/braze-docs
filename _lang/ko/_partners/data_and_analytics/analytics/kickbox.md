...
이렇게 하려면 Kickbox에 설명된 대로 워크플로의 **Entry Settings** 설정을 변경해야 합니다. **실행 기반 전달**을 선택하는 대신 **스케줄**을 선택합니다. 그런 다음 목록을 한 번에 인증할 스케줄 시간을 선택합니다.

### 인증된 Segments 생성 {#create-verified-segments}
...
즉, 이메일 주소가 인증된 사용자로 오디언스 Segments를 생성하여 Campaigns와 Canvases의 전달 성공률을 높이고 이메일 서비스 공급자와의 평판을 보호할 수 있습니다.
...
사용 사례에 따라 고객 프로필에 Kickbox 커스텀 속성 "result"가 존재하는 Segment를 생성하거나 해당 값이 "deliverable"인 Segment를 생성하는 것이 적절할 수 있습니다. 이 필터를 단독으로 사용하여 Segment를 만들거나 향후 모든 Segments의 일부로 만들어 그 안에 있는 모든 사용자를 검증할 수 있습니다.