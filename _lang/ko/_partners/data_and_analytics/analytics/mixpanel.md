...
Mixpanel로 내보낼 수 있는 이벤트는 두 가지 유형이 있습니다: 메시지 전송과 직접 관련된 Braze 이벤트로 구성된 [메시지 인게이지먼트 이벤트](#supported-currents-events)와 세션, 커스텀 이벤트, 플랫폼을 통해 추적되는 구매 등 기타 앱 또는 웹사이트 활동을 포함하는 [고객 행동 이벤트](#supported-currents-events)입니다. 모든 커스텀 이벤트에는 `[Braze Custom Event]` 접두사가 붙습니다. 커스텀 이벤트 속성정보와 구매 이벤트 속성정보에는 각각 `[Custom event property]`와 `[Purchase property]` 접두사가 붙습니다.

추가 이벤트 권한에 대한 액세스가 필요한 경우 계정 매니저에게 문의하거나 [지원 티켓]({{site.baseurl}}/braze_support)을 열어 주세요.

### 1단계: Mixpanel 자격 증명 받기 {#step-1-get-mixpanel-credentials}

Mixpanel 대시보드에서 새 프로젝트 또는 기존 프로젝트의 **Project Settings**를 클릭합니다. 여기에서 Mixpanel API 시크릿과 Mixpanel 토큰을 확인할 수 있습니다. 이 자격 증명은 다음 단계에서 Currents 연결을 생성할 때 사용됩니다.

### 2단계: Braze Currents 생성하기 {#step-2-create-braze-current}

1. Braze에서 **Currents** > **+ Create Current** > **Create Mixpanel Export**로 이동합니다.
2. 나열된 필드에 통합 이름, 연락처 이메일, Mixpanel API 시크릿, Mixpanel 토큰을 입력합니다.
3. 추적할 이벤트를 선택합니다. 사용 가능한 이벤트 목록이 제공됩니다.
4. **Launch Current**을 선택합니다.

![Braze Mixpanel Currents 페이지입니다. 이 페이지에는 통합 이름, 연락처 이메일, API 시크릿, Mixpanel 내보내기 토큰 필드가 포함되어 있습니다. 페이지 하단에는 전송할 수 있는 사용 가능한 Currents 이벤트가 나열되어 있습니다.]({% image_buster /assets/img_archive/mixpanel4.png %}){: style="max-width:80%;"}
...
Mixpanel API 키와 `braze_external_id` 값이 Braze와 Mixpanel 양쪽에서 예상한 값과 일치하는지 확인합니다. 코호트 동기화 API는 제품 간에 사용자 그룹을 공유하며, Braze의 `external_id`와 Mixpanel이 전송하는 식별자가 일치하지 않으면 동기화가 올바르게 작동하지 않습니다. Mixpanel의 코호트 동기화는 Mixpanel의 스케줄에 따라 실행됩니다(예: 1회 또는 약 2시간마다). 따라서 확인 사이에 충분한 시간을 두세요.
