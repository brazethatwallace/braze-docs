...
| Currents | 데이터를 다시 Amplitude로 내보내려면 계정에 [Braze Currents]({{site.baseurl}}/user_guide/data_and_analytics/braze_currents#access-currents)를 설정해야 합니다. |
...
Braze는 `external_user_id`가 설정된 사용자 또는 `device_id`가 설정된 익명 사용자에 대해서만 이벤트 데이터를 전송합니다. 익명 사용자의 경우 SDK에서 Amplitude 기기 ID를 Braze 기기 ID와 동기화해야 합니다. 예를 들면 다음과 같습니다:
...
{% alert note %}
Braze Currents는 Amplitude로 이벤트를 내보낼 때 `[Appboy]` 접두사를 적용합니다. 이 레이블은 Braze의 이전 제품 이름을 참조합니다. 이는 예상되는 동작이며 SDK 또는 통합 문제를 나타내지 않습니다.
{% endalert %}

Braze로 명명 및 가져오기된 모든 코호트에는 `[Amplitude]` 접두사와 `cohort_id` 접미사가 붙습니다. 즉, `cohort_id`가 "abcd1234"인 "TEST_COHORT"라는 코호트는 Braze 필터에서 `[Amplitude] TEST_COHORT: abcd1234`로 표시됩니다.

추가 이벤트 자격이 필요한 경우 계정 매니저에게 문의하거나 [지원 티켓]({{site.baseurl}}/braze_support)을 열어 주세요.

### 1단계: Braze에서 Amplitude 통합 구성 {#step-1-configure-amplitude-integration-in-braze}

Amplitude에서 Amplitude 내보내기 API 키를 확인합니다.
...
Braze에서 **Currents > + Create Current > Create Amplitude Export**로 이동합니다. 나열된 필드에 통합 이름, 연락처 이메일, Amplitude 내보내기 API 키 및 Amplitude 리전을 입력합니다. 그런 다음 추적할 이벤트를 선택합니다. 사용 가능한 이벤트 목록이 제공됩니다. 마지막으로 **Launch Current**를 클릭합니다.

{% alert note %}
Braze Currents에서 Amplitude로 전송되는 이벤트는 Amplitude 이벤트 볼륨 할당량에 포함됩니다.
{% endalert %}

![Braze Amplitude Currents 페이지. 이 페이지에는 통합 이름, 연락처 이메일, API 키 및 US 리전 필드가 포함되어 있습니다. Currents 페이지 하단에는 전송할 수 있는 사용 가능한 Currents 이벤트가 나열되어 있습니다.]({% image_buster /assets/img/amplitude4.png %})

{% alert tip %}
Amplitude API 키를 붙여넣을 때 "Invalid API key" 오류가 발생하면 키를 직접 입력해 보세요. 일부 브라우저에서는 복사하여 붙여넣을 때 유효성 검사 오류를 일으킬 수 있는 숨겨진 문자가 추가될 수 있습니다.
{% endalert %}
...
Currents는 Amplitude의 HTTP API에 연결되며, 이 API에는 기기당 초당 30개 이벤트의 [사용량 제한](https://developers.amplitude.com/docs/http-api-v2#upload-limit)과 기기당 하루 50만 개 이벤트의 문서화되지 않은 제한이 있습니다. 이 임계값을 초과하면 Amplitude는 Currents를 통해 기록된 이벤트를 제한합니다. 통합 내 기기가 이 사용량 제한을 초과하면 모든 기기의 이벤트가 Amplitude에 표시되는 시점이 지연될 수 있습니다.

정상적인 상황에서 기기는 초당 30개 이벤트 또는 하루 50만 개 이벤트를 초과하여 보고하지 않아야 하며, 이러한 이벤트 패턴은 통합이 잘못 구성된 경우에만 발생합니다. 이러한 유형의 지연을 방지하려면 SDK 통합이 SDK 통합 지침에 명시된 대로 정상적인 속도로 이벤트를 보고하도록 하고, 단일 기기에서 대량의 이벤트를 생성하는 자동화된 테스트를 실행하지 마세요.

## 지원되는 Currents 이벤트 {#supported-currents-events}

Braze는 다음 이벤트를 Amplitude로 내보내기하는 것을 지원합니다:
