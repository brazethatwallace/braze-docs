...
이것이 mParticle의 기본 옵션이지만, 대부분의 회사 사용자는 Braze에서 Segment를 생성할 때 필터링 경험을 위해 [단일 배열 속성](#array)을 사용하는 것을 선호합니다.
...
| API 키 운영체제 | Braze API 키가 해당하는 운영체제를 선택합니다. 이 선택은 오디언스 업데이트 시 전달되는 푸시 토큰의 유형을 제한합니다.                          |
...
mParticle은 Braze에서 Segment를 직접 관리하지 않으므로, 해당 mParticle 오디언스 연결이 삭제되거나 비활성화되어도 Segment를 삭제하지 않습니다. 이 경우 mParticle은 Braze에서 오디언스 사용자 속성을 업데이트하여 각 사용자에서 오디언스를 제거하지 않습니다.
...
[Android](https://github.com/mparticle-integrations/mparticle-android-integration-appboy), [iOS](https://github.com/mparticle-integrations/mparticle-apple-integration-appboy), [Web](https://github.com/mparticle-integrations/mparticle-javascript-integration-braze)용 mParticle SDK의 이러한 매핑은 오픈 소스이며 [mParticle의 GitHub 페이지](https://github.com/mparticle-integrations)에서 확인할 수 있습니다.

임베디드 키트 SDK 통합을 통해 푸시, 인앱 메시지 및 모든 관련 메시지 분석 추적을 포함한 전체 기능 모음을 활용할 수 있습니다.
...
- [커스텀 이벤트 등록정보]({{site.baseurl}}/user_guide/data/activation/events/custom_events/)는 문자열, 숫자, 부울 또는 날짜 오브젝트를 지원합니다. 배열이나 중첩 오브젝트는 지원하지 않습니다.
...
6. **키트 초기화 타이밍:** `didFinishLaunchingWithOptions`에서 Braze 인스턴스에 접근하면, 푸시가 도착할 때 mParticle 키트가 아직 준비되지 않았을 수 있습니다. 사용자가 알림을 열 때 Braze 키트가 활성화되도록 [`userNotificationCenter(_:didReceive:withCompletionHandler:)`]({{site.baseurl}}/developer_guide/push_notifications/?sdktab=swift)(또는 동등한 알림 응답 델리게이트)에서 푸시 처리를 초기화하세요.
