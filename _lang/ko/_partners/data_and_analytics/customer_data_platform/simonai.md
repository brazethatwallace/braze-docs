...
Braze와 Simon AI 통합을 사용하여 고급 오디언스를 구축하고 Braze에 동기화하여 실시간, 노코드 오케스트레이션을 수행할 수 있습니다. 이 통합을 통해 Simon AI의 ID 확인, 고객 데이터 통합, AI 기반 세분화를 활용하여 다운스트림에서 더욱 개인화되고 효과적인 Braze Campaign(캠페인)을 구동할 수 있습니다.
...
- Segment 등록정보 전달 및 유지
- 특성 및 연락처 등록정보 동기화

{% alert note %}
Simon과 Braze 통합을 사용할 때 Simon은 각 동기화 시 Braze에 델타만 전송하여 불필요한 데이터에 대한 비용을 방지합니다. 자세한 내용은 [특성 및 연락처 등록정보 동기화](#sync-traits-and-contact-properties)를 참조하세요.
{% endalert %}
...
1. 왼쪽 내비게이션에서 **Integrations**를 클릭한 다음 Braze로 스크롤합니다.
...
### Simon AI의 플로우 또는 여정에 Braze 동작 추가 {#add-braze-actions-to-flows-or-journeys-in-simon-ai}

Simon AI에서 Braze 계정을 인증한 후 [플로우][4] 및 [여정][5]에 Braze 동작을 추가할 수 있습니다.
...
![Simon AI에서 사용 가능한 Braze 동작 목록을 보여주는 드롭다운.][9]{: style="max-width:60%"}

일부 동작은 특정 플로우 유형 또는 여정에서만 사용할 수 있습니다. 자세한 내용은 [docs.simondata.com][6]에서 확인하세요.

### 특성 및 연락처 등록정보 동기화 {#sync-traits-and-contact-properties}
...
2. **Braze**를 선택합니다. 연락처 등록정보가 데이터셋별로 중첩되어 표시됩니다.
3. Simon과 Braze 통합을 사용할 때 동기화할 필드를 선택합니다:
   ...
   3. Simon에서 Braze와 처음 통합하는 경우 **Backfill all contacts**를 클릭합니다. 백필은 플로우 또는 여정에서 동작을 처음 사용할 때 모든 데이터 포인트를 Braze에 전송하여 모든 데이터가 완전히 동기화되도록 합니다. 이후 동기화에서는 이 화면에서 선택한 특성만 Braze에 전송됩니다. 이를 통해 필요한 데이터에 대해서만 비용이 청구되도록 할 수 있습니다.
