...
description: "이 문서에서는 특별히 제작된 Braze 데이터 변환 템플릿과 Crowdtwist의 데이터 푸시 오브젝트를 통해 Braze와 Oracle Crowdtwist 간의 파트너십에 대해 설명합니다."
...
* [라이브 푸시 사용자 리뎀션](https://docs.oracle.com/en/cloud/saas/marketing/crowdtwist-develop/Developers/LivePushUserRedemption.html): 사용자 보상 사용 데이터를 포함합니다.

Braze 데이터 변환 템플릿을 사용하면 데이터 푸시의 요소 중 Braze와 관련이 없는 요소를 필터링하고, 사용 가능한 "대상"에서 활용할 수 있도록 Braze에서 필요한 값을 할당할 수 있습니다.

예를 들어, 데이터 푸시를 사용하여 사용자가 로열티 등급을 변경하거나 보상을 사용할 때와 같이 관련 커스텀 이벤트 및 속성을 Braze에 전달할 수 있습니다. 또한 회원의 포인트 잔액과 같은 데이터가 회원의 고객 프로필에 업데이트되는 즉시 Braze에 커스텀 속성을 기록하는 데 사용할 수도 있습니다.
...
| Braze 데이터 변환 엔드포인트 | 이 통합은 Braze의 [데이터 변환 도구]({{site.baseurl}}/user_guide/data/unification/data_transformation)를 기반으로 합니다. 데이터 변환을 생성하면 Braze가 고유 엔드포인트를 생성하며, 이를 Crowdtwist의 Data Push 대상으로 추가할 수 있습니다.|
{: .reset-td-br-1 .reset-td-br-2 aria-label="전제 조건" }

## 통합 {#integration}

Braze와 Oracle Crowdtwist는 고객이 고객 프로필, 사용자 리뎀션, 사용자 활동 이벤트를 활용하는 자체 데이터 변환을 개발할 수 있도록 [데이터 변환 템플릿]({{site.baseurl}}/user_guide/data/unification/data_transformation/creating_a_transformation?redirected=1#step-2-create-a-transformation)을 만들었습니다.

## 1단계: Oracle Crowdtwist 템플릿에서 데이터 변환 만들기 {#step-1-create-data-transformation-from-oracle-crowdtwist-template}

**데이터 설정 > 데이터 변환 > 변환 만들기 > 템플릿 사용**으로 이동한 후, 원하는 "BRAZE <> CROWDTWIST" 템플릿을 선택합니다.

네 가지 템플릿을 확인할 수 있습니다. 각각 사용자 프로필, 사용자 활동, 사용자 리뎀션 이벤트를 변환하는 템플릿이 하나씩 있으며, 조건 로직을 사용하여 다양한 데이터 푸시 이벤트에 적용하는 마스터 템플릿이 있습니다.

[Oracle Crowdtwist의 데이터 푸시 설명서](https://docs.oracle.com/en/cloud/saas/marketing/crowdtwist-develop/Developers/DataPush.html)에 나와 있듯이, 데이터 푸시 객체에는 서로 다른 메타데이터가 포함되어 있으므로 적절한 Braze 객체를 생성하려면 각각 고유한 변환 코드가 필요합니다. 마스터 템플릿은 세 가지 유형의 객체를 모두 수용하는 단일 데이터 변환을 설정하고 각 객체의 값을 사용하여 적절한 출력을 생성하는 방법을 보여줍니다.

## 2단계: 템플릿 업데이트 및 테스트 {#step-2-update-and-test-template}

이 섹션에서는 주석이 달린 템플릿을 확인할 수 있습니다. 이러한 템플릿의 본문은 `/users/track` 대상에 적용되도록 설계되었습니다. 주석은 `//` 줄 시작과 녹색 텍스트로 표시되며, 변환 코드의 작동에 영향을 주지 않으므로 삭제할 수 있습니다.

변환은 JavaScript를 사용하며, "brazecall"이라는 객체를 생성합니다. 이 객체는 Braze REST API 엔드포인트로 전송되는 요청 본문을 작성하는 곳입니다. 이러한 대상에 대한 요청의 필수 구조에 대한 안내는 "대상" 섹션의 링크를 참조하세요.

{% alert note %}
각 "키"의 "값"이 `payload.`로 시작하는 것을 확인하세요. payload는 Oracle Crowdtwist에서 수신한 데이터 객체를 나타냅니다. JavaScript 점 표기법을 사용하여 Braze 객체의 요소를 채울 데이터를 선택합니다. 예를 들어 `external_id: payload.thirdPartyId`가 있으면, Braze 외부 ID가 Oracle Crowdtwist에 저장된 `third_party_id` 값으로 설정된다는 의미입니다. Oracle Crowdtwist에서 전달되는 객체의 스키마 또는 구성에 대한 자세한 내용은 [Oracle 설명서](https://docs.oracle.com/en/cloud/saas/marketing/crowdtwist-develop/Developers/LivePushUserActivity.html)를 참조하세요.
{% endalert %}

{% alert important %}
Oracle Crowdtwist에서 전송된 객체를 사용하여 Braze에서 사용자를 생성합니다. `update_existing_only` 키에 `false` 값을 포함하면, 속성 또는 이벤트 객체에 Braze에 존재하지 않는 식별자가 포함된 경우 Braze가 이벤트 또는 속성 객체에 포함된 속성으로 고객 프로필을 생성합니다. Oracle Crowdtwist가 Braze에 이미 존재하는 프로필만 업데이트하도록 하려면, 각 속성 또는 이벤트 객체에서 이 속성을 `true`로 설정하세요.
{% endalert %}
...
이 가이드의 템플릿은 "Track Users" 대상으로 전송하도록 만들어졌지만, [Braze 데이터 변환 가이드]({{site.baseurl}}/user_guide/data/data_transformation/creating_a_transformation#step-2-create-a-transformation)에 나열된 엔드포인트와 관련 [REST API 설명서]({{site.baseurl}}/api/home)의 지원을 받아 원하는 엔드포인트로 전송하도록 템플릿을 설계할 수 있습니다.

### 테스트 {#testing}

템플릿을 원하는 대로 수정한 후에는 올바르게 작동하는지 검증해야 합니다. 변환 편집기에서 **Validate**를 선택하여 **Output** 섹션에서 미리보기를 생성하고, Braze가 선택한 대상에 대해 매핑된 요청을 수락하는지 확인합니다.

**Output** 필드에 표시된 객체에 만족하면 **Activate**를 선택하여 데이터 변환 엔드포인트가 데이터를 수신할 준비가 되도록 합니다.

변환 세부 정보 패널에서 데이터 변환의 웹훅 URL을 확인할 수 있습니다. 이 URL을 복사하여 Oracle Crowdtwist의 Integration Hub에서 구성에 사용하세요.

{% alert important %}
Braze 데이터 변환 엔드포인트에는 분당 1,000건의 요청으로 사용량 제한이 있습니다. 이 데이터를 Braze에서 얼마나 빠르게 사용할 수 있게 할지 고려하고, 더 높은 데이터 변환 사용량 제한이 필요한 경우 Braze 계정 매니저에게 문의하세요.
{% endalert %}

데이터 변환은 매우 동적인 도구이며, JavaScript에 대한 이해와 REST API 설명서의 안내를 통해 이 문서에서 설명하는 범위를 넘어서는 목적으로 설계할 수 있습니다. 데이터 변환 템플릿에 대한 복잡한 변경 사항의 지원 또는 문제 해결에 대해서는 고객 성공 매니저에게 문의하여 이용 가능한 안내에 대해 알아보세요.