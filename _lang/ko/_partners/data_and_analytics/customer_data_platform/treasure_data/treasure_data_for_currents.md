...
{% alert important %}
Braze Currents Streaming 커넥터는 베타 버전입니다. Treasure Data 지원팀에 문의하여 Treasure Data 계정에서 활성화하세요. 파트너 측 설정 세부 사항은 Treasure Data의 [Braze Currents Import Integration](https://docs.treasuredata.com/int/braze-currents-import-integration)을 참조하세요.
{% endalert %}
...
| Currents | 데이터를 Treasure Data로 내보내려면 계정에 [Braze Currents]({{site.baseurl}}/user_guide/data/distribution/braze_currents#how-to-access-currents)가 설정되어 있어야 합니다. |
| Braze Currents Streaming 커넥터 | Treasure Data 지원팀에 문의하여 Treasure Data 계정에서 Braze Currents Streaming 커넥터(베타)를 활성화하세요. |
| Treasure Data Write API 키 | Treasure Data Write API 키는 Braze에서 들어오는 인바운드 스트림을 인증합니다. |
{: .reset-td-br-1 .reset-td-br-2 aria-label="필수 조건" }

## 통합 {#integration}

### 1단계: Treasure Data에서 커넥터 구성 {#step-1-configure-the-connector-in-treasure-data}
...
4. **Source Settings**에서 다음을 구성합니다:

| 필드 | 설명 |
| ----- | ----------- |
| Source Name | 이 연결에 대한 설명적인 이름 |
...
5. 저장한 후 **Unique ID**(`task_id`)를 복사합니다. 다음 단계에서 이 값이 필요합니다.

### 2단계: Braze에서 Custom Currents Export 생성 {#step-2-create-a-custom-currents-export-in-braze}

Braze Currents UI의 **Treasure Data Export** 옵션은 레거시 Postback API 방식을 사용하며 더 이상 권장되지 않습니다. 대신 **Custom Currents Export**를 사용하세요.

1. Braze에서 **파트너 통합** > **Data Export**로 이동합니다.
2. **Create New Current** > **Custom Currents Export**를 선택합니다.
3. 통합 이름과 오류 알림을 위한 연락처 이메일을 입력합니다.
4. **Credentials**에서 Treasure Data 리전에 맞는 엔드포인트 URL을 입력합니다. Treasure Data Write API 키를 **Bearer Token**으로 입력합니다.
...
5. 내보낼 이벤트 유형을 선택합니다. Custom Currents 연결은 식별된 사용자와 `external_user_id`가 없는 사용자 모두에 대한 이벤트를 전송할 수 있습니다. Treasure Data는 두 가지 모두 수집합니다.
6. **Launch Current**을 선택합니다.

{% alert warning %}
Treasure Data Write API 키와 엔드포인트 URL을 최신 상태로 유지하세요. 엔드포인트에 **5일** 이상 연결할 수 없으면 Braze가 커넥터의 이벤트를 삭제하고 데이터가 영구적으로 손실됩니다.
{% endalert %}

## 데이터 쿼리 {#query-your-data}

이벤트가 흐르기 시작하면 SQL로 쿼리할 수 있습니다. Treasure Data는 페이로드를 플랫하게 변환하므로 JSON을 파싱할 필요가 없습니다.
...
{% alert note %}
Treasure Data의 `time` 필드는 Braze에서의 원래 이벤트 발생 시간이 아니라 Treasure Data가 이벤트를 수신하고 처리한 타임스탬프입니다.
{% endalert %}

**Multiple Tables**를 선택한 경우 각 이벤트 유형은 자체 테이블에 저장됩니다(예: `users_message_email_open` 또는 `users_behaviors_purchase`).

데이터가 도착하는지 확인하려면 Current을 시작한 후 몇 분 뒤에 카운트 쿼리를 실행합니다:
...
Treasure Data는 중첩된 JSON을 최대 두 단계까지 플랫하게 변환합니다:

| JSON 유형 | Treasure Data 컬럼 유형 |
| --------- | ------------------------- |
| string | string |
| number | long |
| boolean | string |
...
내보낸 데이터의 페이로드 구조는 커스텀 HTTP 커넥터의 페이로드 구조와 동일합니다. [커스텀 HTTP 커넥터 예시 리포지토리](https://github.com/Appboy/currents-examples/tree/master/sample-data/Custom%20HTTP/users/behaviors)에서 샘플 페이로드를 확인할 수 있습니다.

## 레거시 Postback 방식에서 마이그레이션 {#migrate-from-the-legacy-postback-method}

이전에 Braze에서 **Treasure Data Export**(Postback)를 사용한 경우:

1. 이 문서의 Custom Currents Export 설정을 완료합니다.
2. 새 테이블로 이벤트가 흐르고 있는지 확인합니다.
3. Braze에서 기존 Postback 기반 Current을 비활성화합니다.

원시 JSON 배열로 저장된 레거시 데이터는 `JSON_PARSE` 및 `UNNEST`로 계속 쿼리할 수 있습니다. 스트리밍 커넥터를 통해 수집된 새 데이터는 [데이터 스키마](#data-schema)에 설명된 플랫 스키마를 사용합니다.