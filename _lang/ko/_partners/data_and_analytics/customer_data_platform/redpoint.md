...
Braze와 Redpoint 통합을 통해 Redpoint CDP 데이터를 기반으로 Braze 세그먼트를 생성할 수 있습니다. Redpoint는 Braze로 데이터를 전달하는 두 가지 모드를 제공합니다:
...
RPI 오디언스 출력을 Braze에 동기화하려면 아웃바운드 전달 오퍼를 생성하고 **Braze Onboarding and Upsert** 또는 **Braze Append** 채널에 연결합니다. 이는 Braze에서 새 레코드를 생성하거나 병합할 것인지, 아니면 레코드가 이미 Braze에 존재하는 경우에만 Campaign 데이터를 추가할 것인지에 따라 달라집니다.

![]({% image_buster /assets/img/redpoint/rpi_to_braze_rpi_canvas.png %}){: style="max-width:80%;"}

RPI에서 워크플로가 성공적으로 실행되면, RPI에서 소싱된 오케스트레이션 및 CDP 데이터를 사용하여 Braze에서 세그먼트를 생성할 수 있습니다.
