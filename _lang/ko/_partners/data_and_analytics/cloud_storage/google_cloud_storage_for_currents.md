...
{% alert note %}
워크로드 아이덴티티 페더레이션(WIF)은 Currents의 인증 방법으로 지원되지 않습니다. JSON 비공개 키가 포함된 서비스 계정을 사용해야 합니다.
{% endalert %}
...
![Create Role 작업이 표시된 Google Cloud IAM 역할 페이지.]({% image_buster /assets/img/gcs1.png %})
...
{% alert note %}
`storage.objects.delete` 권한은 선택 사항입니다. 이 권한을 통해 Braze가 불완전한 파일을 정리할 수 있습니다.<br><br>드문 경우지만 Google Cloud가 연결을 조기에 종료하여 Braze가 Google Cloud Storage에 불완전한 파일을 기록할 수 있습니다. 대부분의 경우 Braze가 재시도하여 올바른 데이터로 새 파일을 생성하며, 이전 파일은 Google Cloud Storage에 남아 있게 됩니다.
{% endalert %}
...
![Google Cloud Platform의 서비스 생성 페이지에서 역할 이름을 'Select a Role' 필드에 입력합니다.]({% image_buster /assets/img/gcs4.png %})
