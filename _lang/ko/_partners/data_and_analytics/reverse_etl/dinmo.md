...
| Braze REST 엔드포인트 | REST 엔드포인트 URL. 엔드포인트는 Braze 인스턴스의 [API 엔드포인트]({{site.baseurl}}/developer_guide/rest_api/basics#endpoints)에 따라 다릅니다. |
...
| 데이터 웨어하우스 및 데이터 모델 | 통합을 시작하기 전에 DinMo에서 데이터 웨어하우스를 연결하고 Braze에 동기화할 데이터에 대한 모델 또는 세그먼트를 정의하세요. 자세한 내용은 [DinMo Braze 통합 가이드](https://docs.dinmo.io/integrations/destination-platforms/braze)를 참조하세요. |
...
* 데이터 웨어하우스의 커스텀 이벤트 및 구매 이벤트를 Braze로 전송하여 행동 기반 타겟팅에 활용할 수 있습니다.
* Braze 구독 그룹 멤버십을 DinMo에서 정의한 오디언스 Segments와 일치시킬 수 있습니다.
...
| `users.export.ids` | 일괄 작업을 위한 사용자 ID 내보내기 |
| `users.alias.update` | 사용자 별칭 업데이트 |
| `subscription.status.set` | 구독 상태 동기화 |
| `users.delete` | 미러 동기화 모드에만 해당 (기타 대상 서비스의 경우 선택 사항) |
...
* REST API URL이 올바르며 끝에 슬래시가 없는지 확인합니다.
* API 키가 유효하며 필요한 권한이 있는지 확인합니다.
* Braze 워크스페이스에서 IP 허용 목록을 사용하는 경우 DinMo의 IP 주소가 포함되어 있는지 확인합니다.

## 지원되는 대상 서비스 {#supported-destination-services}

DinMo의 각 대상 서비스는 동일한 일반적인 워크플로를 따릅니다: Braze 대상을 생성하고, DinMo 모델 또는 세그먼트를 빌드한 다음, 활성화를 생성하여 Braze로 데이터를 전송합니다. 단계별 활성화 안내는 [DinMo Braze 대상 서비스](https://docs.dinmo.io/integrations/destination-platforms/braze)를 참조하세요.
...
| [추적 이벤트 전송](https://docs.dinmo.io/integrations/destination-platforms/braze/send-track-events) | Braze로 커스텀 이벤트 및 구매 이벤트를 전송합니다. |
| [구독 상태 동기화](https://docs.dinmo.io/integrations/destination-platforms/braze/synchronize-subscription-statuses) | DinMo 세그먼트 멤버십을 기반으로 Braze 구독 그룹에서 사용자를 구독 또는 탈퇴 처리합니다. |
| [사용자 목록 내보내기](https://docs.dinmo.io/integrations/destination-platforms/braze/export-user-lists) | 세그먼트 멤버십을 Braze 사용자 속성으로 동기화하여 Braze 세분화에 활용합니다. |
...
* 삽입 모드를 활성화하면, 모델에 있는 새 사용자가 Braze에 생성됩니다(UPSERT 동작).
* 마지막 활성화 이후 변경된 속성 값이 Braze에서 업데이트됩니다.

삽입 모드를 활성화하지 않으면, DinMo는 Braze에 이미 존재하며 일치하는 외부 ID가 있는 사용자만 업데이트합니다.

활성화 설정 시, DinMo 모델에서 사용자의 [외부 ID]({{site.baseurl}}/developer_guide/analytics/setting_user_ids?tab=web) 또는 Braze ID에 해당하는 필드를 매핑합니다. 각 DinMo 필드를 Braze의 정확한 속성 이름에 매핑하세요. 해당 속성이 Braze에 존재하지 않으면 DinMo가 자동으로 생성합니다.
...
| MIRROR | Braze에서 레코드를 삽입, 업데이트, 삭제하여 소스를 미러링합니다. 삭제 작업을 위해 커넥터 지원이 필요합니다. |
{: .reset-td-br-1 .reset-td-br-2 aria-label="사용자 속성 동기화 모드" }

{% alert warning %}
미러 동기화 모드는 DinMo 소스에 더 이상 존재하지 않는 레코드를 Braze에서 영구적으로 삭제합니다. 데이터 웨어하우스가 단일 소스 오브 트루스이고 삭제가 의도적인 경우에만 미러 모드를 사용하세요. 프로덕션 환경에서 미러 동기화를 실행하기 전에 삭제 규칙을 검증하세요.
{% endalert %}

### 추적 이벤트 전송 {#send-track-events}

이 대상 서비스를 사용하여 DinMo 이벤트 모델 또는 세그먼트에서 Braze로 커스텀 이벤트 또는 구매 이벤트를 전송합니다. DinMo는 Braze가 각 유형에 대해 서로 다른 API를 사용하므로 커스텀 이벤트와 구매를 별도의 대상 서비스로 처리합니다.

모델의 각 레코드는 단일 이벤트 유형을 나타냅니다(예: `Purchase`). DinMo는 각 활성화 실행 시 새 이벤트만 전송하며, 이전에 전송된 이벤트를 업데이트하지 않습니다.

활성화 설정 시:

1. Braze에 표시될 정확한 이벤트 이름을 지정합니다. 이벤트가 존재하지 않으면 DinMo가 자동으로 생성합니다.
2. 필수 필드를 매핑합니다:
...
이 대상 서비스를 사용하여 Braze 구독 그룹을 DinMo 세그먼트 또는 모델과 일치하도록 유지합니다.

이 서비스를 활성화하기 전에:

1. Braze에서 대상 구독 그룹(단문 메시지 서비스 또는 이메일)을 생성합니다.
2. 해당 구독 그룹에 포함되어야 하는 사용자를 담은 DinMo 모델 또는 세그먼트를 빌드합니다.

활성화 설정 시 Braze의 정확한 구독 그룹 ID를 입력합니다. 여러 구독 그룹을 동기화하려면 그룹당 하나의 활성화를 생성하세요.

활성화가 실행되면:

* 사용자가 이미 Braze에 존재하는 경우, DinMo 세그먼트에 진입하는 사용자는 대상 구독 그룹에 구독됨으로 표시됩니다.
* DinMo 세그먼트를 떠나는 사용자는 구독 그룹에서 탈퇴 처리됩니다.

DinMo는 세그먼트에 포함된 적이 없는 사용자를 수정하지 않으며, 이 대상 서비스에서 새 Braze 사용자를 생성하지 않습니다.

### 사용자 목록 내보내기 {#export-user-lists}

이 대상 서비스를 사용하여 DinMo 세그먼트를 Braze 사용자 속성으로 표현합니다. Braze 제한 사항으로 인해 DinMo는 Braze 목록을 직접 생성하지 않습니다. 대신, 세그먼트에 포함된 사용자의 사용자 속성을 `true`로 설정하고, 세그먼트를 떠나는 사용자는 `false`로 설정합니다.

활성화 설정 시 오디언스 이름을 지정합니다. DinMo는 이 이름을 Braze 속성으로 사용합니다(공백은 밑줄로 대체됩니다). 동일한 이름의 속성이 Braze에 이미 존재하지 않는지 확인하세요. 사용자의 외부 ID에 해당하는 DinMo 필드를 매핑합니다.
