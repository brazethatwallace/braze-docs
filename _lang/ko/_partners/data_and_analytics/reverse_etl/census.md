...
Braze와 Census 통합을 사용하면 오디언스 또는 제품 데이터를 Braze로 동적으로 가져와 개인화된 캠페인을 보낼 수 있습니다. 예를 들어, Braze에서 "고객 생애주기 가치 > 1000인 뉴스레터 구독자" 코호트를 생성하여 고가치 고객을 타겟팅하거나, "최근 30일 이내 활성 사용자" 코호트를 생성하여 특정 사용자를 타겟팅하여 예정된 베타 기능을 테스트할 수 있습니다.
...
| Braze REST API 키 | 모든 사용자 데이터 권한(`users.delete` 제외) 및 `segments.list` 권한이 있는 Braze REST API 키가 필요합니다. Census가 더 많은 Braze 오브젝트를 지원함에 따라 권한 세트가 변경될 수 있으므로, 지금 더 많은 권한을 부여하거나 향후 권한을 업데이트할 계획을 세우는 것이 좋습니다. <br><br> 이 키는 Braze 대시보드의 **설정** > **API 키**에서 생성할 수 있습니다. |
| Braze REST 엔드포인트 | REST 엔드포인트 URL입니다. 엔드포인트는 [인스턴스의 Braze URL]({{site.baseurl}}/developer_guide/rest_api/basics#endpoints)에 따라 달라집니다. |
| 데이터 웨어하우스 및 데이터 모델 | 통합을 시작하기 전에 Census에서 데이터 웨어하우스를 설정하고 Braze에 동기화할 데이터의 하위 집합 모델을 정의해야 합니다. 사용 가능한 데이터 소스 목록과 모델 생성 가이드는 [Census 설명서](https://docs.getcensus.com/destinations/braze)를 참조하세요. |
...
4. 적용할 동기화 규칙을 선택합니다(**Update or Create**가 가장 일반적인 선택이지만, 예를 들어 데이터 삭제를 처리하기 위한 고급 규칙도 선택할 수 있습니다).<br><br>
5. 다음으로, 레코드 매칭을 위해 Braze 오브젝트를 모델 필드에 [매핑](#supported-objects)할 동기화 키를 선택합니다.<br>!['동기화 키 선택' 프롬프트에서 Braze의 'External User ID'가 소스의 'user_id'와 매칭됩니다.]({% image_buster /assets/img/census/census_1.png %}){: style="max-width:80%;"}<br><br>
...
7. 세부 사항을 확인하고 동기화를 생성합니다.

동기화가 실행되면 사용자 데이터가 Braze에 저장됩니다. Braze Segment를 생성하고 향후 Braze Campaigns 및 Canvases에 추가하여 이 사용자들을 타겟팅할 수 있습니다.

{% alert note %}
Census와 Braze 통합을 사용할 때, Census는 각 동기화 시 변경된 데이터(델타)만 Braze에 전송합니다.
{% endalert %}
...
또한 Census는 Braze로 [구조화된 데이터](https://docs.getcensus.com/destinations/braze#supported-objects)를 전송하는 것을 지원합니다. 사용자 푸시 토큰을 전송하려면 데이터가 2~3개의 값(`app_id`, `token`, 선택 사항인 `device_id`)을 포함하는 오브젝트 배열로 구조화되어야 합니다.