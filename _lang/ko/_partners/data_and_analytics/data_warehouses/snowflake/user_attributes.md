...
## 대시보드와의 데이터 일치 {#data-parity-with-the-dashboard}

드문 경우이지만, 이 페이지의 Snowflake 뷰에 있는 기본 속성 및 커스텀 속성 값이 Braze 대시보드의 사용자 프로필에 표시되는 내용과 일치하지 않을 수 있습니다.

예를 들어, Snowflake에서는 속성이 `NULL`로 표시되지만 대시보드에서는 해당 사용자에 대한 값이 표시될 수 있습니다.

광범위한 불일치가 발견되면 고객 성공 매니저 또는 Braze 지원팀에 문의하세요.
...
## 사용자 프로필 스냅샷 {#user-profile-snapshots}

이 뷰는 사용자 프로필 속성의 주기적인 스냅샷을 제공합니다. 데이터는 최대 12시간까지 지연될 수 있으므로, 실시간 업데이트가 필요하지 않은 쿼리에 유용합니다.
...
| `ARCHIVED` | BOOLEAN | 사용자 프로필이 보관 처리되었는지 여부 |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="USERDEFAULTATTRIBUTESVIEWSHARED 스키마" }
...
| `ARCHIVED` | BOOLEAN | 사용자 프로필이 보관 처리되었는지 여부 |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="USERCUSTOMATTRIBUTESVIEWSHARED 스키마" }

#### CUSTOM_ATTRIBUTES 활용하기 {#working-with-custom_attributes}

`CUSTOM_ATTRIBUTES` 열은 모든 커스텀 속성을 JSON 객체로 저장합니다. Snowflake의 JSON 함수를 사용하여 개별 속성에 접근할 수 있습니다.
...
## 실시간 사용자 프로필 뷰 {#real-time-user-profile-views}

이 뷰는 사용자 프로필 속성에 대한 거의 실시간 업데이트를 제공하며, Braze에서 업데이트가 발생한 후 최대 10분까지 데이터가 지연될 수 있습니다.
...
* 최소한의 지연(~10분)으로 최신 사용자 속성을 제공합니다.
* 실시간 분석 및 최근 데이터가 필요한 시나리오에 유용합니다.
* **성능 고려 사항:**
    * 개별 사용자에 대한 쿼리는 더 빠릅니다(대형 웨어하우스를 사용할 경우 1분 이내).
    * USER_ID 필터 없이 쿼리하면 모든 사용자에 대한 집계가 필요하므로 실행 시간이 크게 늘어납니다.
    * 대규모 데이터셋(예: 1억 명 이상의 사용자)에 대한 쿼리는 수 분이 걸릴 수 있습니다.
...
| `EXTERNAL_USER_ID` | VARCHAR | 사용자 고유 식별자(설정된 경우) |
| `FIRST_NAME` | VARCHAR | 사용자의 이름 |
| `LAST_NAME` | VARCHAR | 사용자의 성 |
| `EMAIL_ADDRESS` | VARCHAR | 사용자의 이메일 주소 |
| `GENDER` | VARCHAR | 사용자의 성별 |
...
| `LANGUAGE` | VARCHAR | 사용자의 언어 설정 |
| `TIME_ZONE` | VARCHAR | 사용자의 시간대 |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="USERLATESTSTATEDEFAULTATTRIBUTESVIEWSHARED 스키마" }
...
## 과거 변경 로그 {#historical-change-logs}

이 뷰는 사용자 속성의 과거 변경 로그를 저장하며, 12시간 단위로 변경 사항을 캡처합니다.
...
* 6개월 롤링 기간 동안의 사용자 속성 변경 이력을 제공합니다.
* 데이터는 12시간마다 스냅샷이 생성되므로, 이 기간 내의 여러 업데이트는 하나의 레코드로 결합됩니다. 이 기간 내의 개별 변경 사항은 별도로 보존되지 않습니다.
* `EFF_DT`와 `END_DT`는 사용자 속성 상태의 시작과 종료를 나타냅니다.
...
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="USERDEFAULTATTRIBUTESHISTORYVIEWSHARED 스키마" }
...
### 사용자 Segments 구축 {#building-user-segments}
...
| 사용 사례                                               | 권장 뷰                                   | 참고                                                                 |
|--------------------------------------------------------|----------------------------------------------------|-----------------------------------------------------------------------|
| 최신 업데이트가 필요하지 않은 **일반 쿼리** | `USER_DEFAULT_ATTRIBUTES_VIEW_SHARED` 및 `USER_CUSTOM_ATTRIBUTES_VIEW_SHARED`               | 빠른 실행이 가능하며, 최대 12시간 전 데이터를 제공합니다.                          |
| **최신 사용자 속성**이 필요한 쿼리       | `USER_LATEST_STATE_DEFAULT_ATTRIBUTES_VIEW_SHARED` 및 `USER_LATEST_STATE_CUSTOM_ATTRIBUTE_VIEW_SHARED` | 거의 실시간에 가까운 업데이트를 제공하지만, 대규모 데이터셋에서는 속도가 느려질 수 있습니다. |
...
* `USER_DEFAULT_ATTRIBUTES_VIEW_SHARED` 또는 `USER_CUSTOM_ATTRIBUTES_VIEW_SHARED`에 대한 쿼리는 대규모 웨어하우스에서 대용량 데이터셋(약 10억 명의 사용자)을 기준으로 10초 이내에 결과를 반환해야 합니다.
* `USER_LATEST_STATE_DEFAULT_ATTRIBUTES_VIEW_SHARED` 또는 `USER_LATEST_STATE_CUSTOM_ATTRIBUTE_VIEW_SHARED `에 대한 단일 사용자 쿼리는 1분 이내에 결과를 반환하지만, `USER_ID` 필터링 없이는 확장성이 떨어집니다.
