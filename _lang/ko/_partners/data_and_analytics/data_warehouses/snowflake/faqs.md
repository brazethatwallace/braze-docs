...
- Snowflake 리전이 AP-Northeast-1(AWS)에 있고 Braze 대시보드 리전이 일본에 있는 경우.
- Snowflake 리전이 AP-Southeast-2(AWS)에 있고 Braze 대시보드 리전이 호주에 있는 경우.
- Snowflake 리전이 AP-Southeast-3(AWS)에 있고 Braze 대시보드 리전이 인도네시아에 있는 경우.
...
- **뷰(권장):** `app_group_id`로 필터링된 각 워크스페이스에 대한 뷰를 생성합니다. 이렇게 하면 데이터를 복제하지 않으면서도 각 팀이나 사용 사례에 대해 깔끔하고 범위가 지정된 워크스페이스 데이터 뷰를 제공할 수 있습니다.
- **로컬 테이블 복사:** `app_group_id`로 필터링된 별도의 테이블에 데이터를 복사합니다. 이 방법은 데이터를 복제하므로 일반적으로 뷰 접근 방식이 선호됩니다.
