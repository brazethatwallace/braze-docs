...
> [Constructor](https://constructor.com/)는 인공지능과 머신 러닝을 활용하여 이커머스 및 소매 웹사이트에 개인화된 검색, 추천, 브라우징 경험을 제공하는 검색 및 제품 디스커버리 플랫폼입니다.
...
- **프로모션 Campaign(캠페인)**: 시즌 세일이나 특별 할인에 맞춰 사용자 선호도에 맞는 큐레이트된 제품 추천이 포함된 개인화된 프로모션 메시지를 전달합니다.
...
Constructor의 Offsite Discovery API URL을 사용하여 제품 이미지를 렌더링하고 사용자를 적절한 제품 상세 페이지로 안내할 수 있습니다. 다음은 엔드포인트 구조에 대한 설명과 사용 방법 예시입니다.
...
| `campaign_id` | 이메일 Campaign의 고유 ID입니다. |
{: .reset-td-br-1 .reset-td-br-2 aria-label="파라미터" }
...
{: .reset-td-br-1 .reset-td-br-2 aria-label="선택 입력값" }