...
### 3단계: 새 Braze 자산 검토 {#step-3-review-new-braze-assets}
...
### 5단계: Canvas 자격 검토 {#step-5-review-canvas-eligibility}

각 Signals Canvas에서 **타겟 오디언스** 설정으로 이동하여 Wunderkind의 기본 진입 오디언스 및 종료 기준을 검토합니다.
...
- Canvas 자격 및 우선순위에 대한 자세한 내용은 Wunderkind 도움말 센터의 [Canvas 자격 검토](https://support.wunderkind.co/hc/en-us/articles/47156586245787-Review-Canvas-Eligibility)를 참조하세요.

### 6단계: 테스트 및 출시 {#step-6-test-and-launch}

Wunderkind는 라이브 전에 포괄적인 QA를 수행합니다:

- 신호가 API 오류 없이 올바른 Canvas ID로 전달되는지 확인합니다.
- `context` 필드(제품명, 이미지, URL)가 렌더링된 이메일 템플릿에 올바르게 채워지는지 확인합니다.
- 모의 Wunderkind 제품으로 템플릿을 미리보기하는 방법은 Wunderkind 도움말 센터의 [Braze용 Signals 테스트 및 출시](https://support.wunderkind.co/hc/en-us/articles/47156667414171-Test-and-Launch-Signals-for-Braze)를 참조하세요.

QA가 통과되면 Wunderkind 구현 매니저가 팀과 함께 프로덕션 출시를 조율합니다.

## Canvas 컨텍스트 페이로드 {#canvas-context-payload}

Wunderkind는 6가지 신호 유형을 지원합니다. 각 유형은 `/canvas/trigger/send`에서 해당 수신자의 [`context`]({{site.baseurl}}/api/objects_filters/context_object) 오브젝트 내에 고유한 키와 값 세트를 전달합니다([API 트리거 전달을 사용하여 Canvas 메시지 보내기]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_triggered_canvases) 참조). `WkPurpose` 필드는 해당 페이로드 내에서 신호 유형을 식별합니다.
...
| `WkPurpose` | String | 신호 유형 식별자(이 섹션의 Canvas별 값 참조) |
| `WKCouponCode` | String | 쿠폰 코드(해당하는 경우, 사용하지 않으면 빈 문자열) |
| `WKCouponPurpose` | String | 쿠폰 제안 설명(사용하지 않으면 빈 문자열) |
| `Items` | Array | 제품 오브젝트 배열(이 섹션의 제품 필드 참조) |
| `WkOpen` | String | 보고 목적으로 사용 가능한 추적 픽셀 |
...
`user_alias` 대신 Braze의 `email` 필드로 Canvas를 트리거하는 경우, 수신자에는 `email`과 `prioritization`만 포함해야 합니다([API 트리거 전달을 사용하여 Canvas 메시지 보내기]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_triggered_canvases) 참조). `context` 오브젝트는 다른 예시와 동일합니다.
...
Wunderkind가 `/canvas/trigger/send`를 호출하면, 각 수신자의 `context` 오브젝트에 전달한 키와 값이 Canvas 진입 데이터가 됩니다. 메시지 단계에서는 `context` Liquid 네임스페이스로 참조합니다. 예를 들어 {% raw %}`{{context.${WkPurpose}}}`{% endraw %}와 같이 사용하며, 이는 [Canvas 컨텍스트 오브젝트]({{site.baseurl}}/api/objects_filters/context_object) 및 [메시지]({{site.baseurl}}/user_guide/engagement_tools/canvas/canvas_components/message_step)에 설명되어 있습니다. 올바른 Liquid 구문을 사용하는 것 외에 추가 구성은 필요하지 않습니다.
