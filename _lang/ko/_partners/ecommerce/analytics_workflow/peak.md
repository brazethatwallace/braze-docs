...
> [Peak](https://peak.ai/)은 의사결정 인텔리전스 플랫폼으로, 의사결정 인텔리전스가 비즈니스 의사결정을 향상시키고 매출과 이익을 성장시키기 위한 인공지능의 상업적 적용인 엔드투아웃컴 시스템입니다.
...
시작점으로, Peak 테넌트가 Peak과 Braze 간의 통합을 호스팅해야 합니다. 이는 일반적으로 Peak 고객의 온보딩 과정에서 생성됩니다. 또한 의사결정 인텔리전스 솔루션이 먼저 필요합니다. 이 솔루션이 이후 Braze에 통합될 인공지능 기반 출력을 생성하기 때문입니다.

| 요구 사항 | 설명 |
| ----------- | ----------- |
| Peak 테넌트 | 테넌트라고 하는 Peak 플랫폼의 인스턴스가 통합을 호스팅하고 오케스트레이션하는 데 필요합니다. |
| 의사결정 인텔리전스 솔루션 | Peak과 Braze 간의 통합은 인공지능 기반 출력을 기반으로 하므로 테넌트 내에 Peak 또는 고객이 배포한 솔루션이 필요합니다. |
...
통합은 인공지능 모델 실행과 예측 고객 속성의 재계산을 기반으로 트리거됩니다. 이러한 인공지능 출력은 속성이 새로운 상태 또는 값으로 업데이트된 시점을 포함하여 Peak 내에 저장됩니다.

속성이 업데이트된 시점을 기반으로, Peak과 Braze 간의 마지막 동기화 이후 업데이트된 예측 속성을 가진 모든 고객을 수집하기 위한 선택이 수행됩니다.

### 2단계: Braze 업데이트 {#step-2-update-braze}

업데이트된 고객 및 관련 속성을 사용하여 Peak은 [벌크]({{site.baseurl}}/api/endpoints/user_data/post_user_track#need-to-update-users-in-bulk) 헤더를 활용하여 [`/user/track` 엔드포인트]({{site.baseurl}}/api/endpoints/user_data/post_user_track)를 통해 Braze에 POST합니다.
