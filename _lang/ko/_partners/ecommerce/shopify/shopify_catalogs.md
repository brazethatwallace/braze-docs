...
이미 Shopify 스토어를 설치한 경우에도 이 섹션의 지침에 따라 제품을 동기화할 수 있습니다.
...
Braze 카탈로그 ID로 사용할 기본 제품 식별자를 선택합니다:

- **Shopify Variant ID**는 SKU가 누락되었거나, 배리언트 간에 중복되었거나, 슬래시, 마침표, 공백, 앰퍼샌드 등의 문자를 포함할 수 있는 경우 좋은 기본값입니다. Variant ID는 숫자이며 항상 이러한 요구 사항을 충족합니다.
- **SKU**는 모든 배리언트에 Shopify Variant ID와 동일한 문자 규칙을 따르는 고유한 SKU가 있고, 메시징이나 분석에서 소매 SKU를 카탈로그 키로 사용하려는 경우에 적합합니다.
  - 허용되지 않는 문자가 포함된 자유 텍스트 SKU는 SKU 대신 Shopify Variant ID를 사용하여 처리할 수 있습니다.

선택한 값은 카탈로그 `item_id`가 되며 문자, 숫자, 하이픈, 밑줄만 포함할 수 있습니다.

{% alert note %}
SKU를 카탈로그 ID로 사용하는 경우, 스토어의 모든 제품과 배리언트에 SKU가 설정되어 있고 고유한지 확인하세요.<br><br>
- 항목에 SKU가 누락된 경우 Braze는 해당 제품을 카탈로그에 동기화할 수 없습니다.
- 동일한 SKU를 가진 제품이 두 개 이상인 경우 예기치 않은 동작이 발생하거나 제품 정보가 의도치 않게 덮어쓰여질 수 있습니다.
{% endalert %}
...
{% alert tip %}
각 제품 데이터 유형의 사용 예시는 [Shopify 카탈로그 사용 사례]({{site.baseurl}}/partners/ecommerce/shopify/shopify_catalogs/?tab=shopify%20product%20metafields#shopify-catalog-use-cases)를 참조하세요.
{% endalert %}

### 4단계: 동기화 진행 상황 추적 {#step-4-track-your-sync-progress}

구성을 저장하면 Braze가 제품 동기화를 시작하고 Shopify 파트너 페이지에서 상태가 **진행 중**으로 업데이트됩니다. 동기화 시간은 스토어의 제품 및 배리언트 수에 따라 달라집니다.

동기화가 진행 중이면 페이지를 떠나도 됩니다. 동기화가 완료되면 Braze가 대시보드 알림을 보냅니다. 완료 후 상태가 **활성**으로 업데이트되며, Shopify 파트너 페이지에서 카탈로그 이름을 선택하여 제품을 확인할 수 있습니다.
...
이 사용 사례에서는 동기화된 Shopify 카탈로그 데이터를 사용하여 메시지를 개인화하는 방법을 보여줍니다.
