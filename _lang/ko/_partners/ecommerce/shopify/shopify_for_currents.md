...
| 요구 사항 | 설명 |
| ----------- | ----------- |
| Currents | 데이터를 Shopify로 내보내려면 계정에 [Braze Currents]({{site.baseurl}}/user_guide/data_and_analytics/braze_currents#access-currents)가 설정되어 있어야 합니다. |
| Shopify 스토어 | [Braze와 Shopify 스토어를 하나 이상 설정]({{site.baseurl}}/shopify_standard_integration)했는지 확인하세요. |
...
1. Braze에서 **파트너 통합** > **Currents** > **+ 새 Currents 생성** > **Shopify 내보내기**로 이동합니다.
...
| ----------- | ---------------------- | ----- |
| `first_name` | `firstName` | 그대로 매핑됩니다. 프로필 업데이트에 해당 값이 있을 때만 전송됩니다. |
| `last_name` | `lastName` | 그대로 매핑됩니다. 프로필 업데이트에 해당 값이 있을 때만 전송됩니다. |
...
현재 이 통합은 Shopify 메타필드를 기록하지 않으므로, 메타필드가 필요한 프로필 필드는 동기화되지 않습니다. 특히 커스텀 속성은 Shopify로 전송되지 않습니다. 그 외에 전송되지 않는 필드로는 `external_user_id`, `gender`, `dob`(생년월일), `timezone`, `home_city`, `country`, `archived`가 있습니다.

Braze는 스토어에 `braze` 네임스페이스 아래에 메타필드 정의를 생성할 수 있습니다(예: `braze.gender`). 이러한 정의는 향후 사용을 위해 예약되어 있으며, 현재 Braze는 해당 정의에 값을 기록하지 않습니다. 예외적으로 `braze.user_id`는 고객을 매칭하는 데 사용되는 식별자를 저장합니다.