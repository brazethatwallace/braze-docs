...
> [RevenueCat](https://www.revenuecat.com/)은 iOS, Android, 웹 전반에서 구독 상태에 대한 단일 정보 소스입니다. 새로운 앱을 구축하든 이미 수백만 명의 가입자를 보유하고 있든, RevenueCat을 사용하여 크로스 플랫폼 인앱 구매를 구축하고, 제품과 가입자를 관리하며, 데이터를 분석할 수 있습니다. 서버 코드가 필요하지 않습니다.
...
이벤트 데이터와 함께 [사용자 별칭 오브젝트]({{site.baseurl}}/api/objects_filters/user_alias_object/)를 전송하려면 두 속성이 모두 필요합니다. 이러한 등록정보는 다른 [RevenueCat 가입자 속성](https://docs.revenuecat.com/docs/subscriber-attributes)과 마찬가지로 수동으로 설정할 수 있습니다. 예제 코드 스니펫은 1단계에 나와 있습니다.
...
1. RevenueCat 대시보드에서 프로젝트로 이동하여 왼쪽 메뉴에서 **Integrations** 카드를 찾습니다. **+ New**를 선택합니다.
