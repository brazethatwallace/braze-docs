...
샘플 페이로드를 Movable Ink 클라이언트 경험 팀과 공유해야 합니다. 구성한 페이로드를 기반으로 **Test** 탭에서 이 페이로드를 생성할 수 있습니다.
...
3. 모든 것이 정상이면 **Send test**를 클릭하여 테스트 요청을 보냅니다.
...
{% alert note %}
데이터가 실시간으로 Movable Ink에 스트리밍되도록 하려면 **Send campaign to users in their local time zone**을 선택하지 마세요.
{% endalert %}
...
이렇게 하면 Movable Ink가 이미지를 생성할 때 참조하는 행동 이벤트가 해당 행동 이벤트를 수신한 동일한 고객과 연결됩니다. UUID 값이 Braze `external_id`와 동일하지 않은 경우, 이 식별자를 활용하려면 UUID를 캡처하여 Braze에 속성으로 전달하거나 Braze 이벤트의 이벤트 등록정보에 포함해야 합니다.
...
이벤트 등록정보를 사용하여 Braze 내에서 이벤트 페이로드를 생성할 수 있습니다. 랜덤 사용자에 대한 샘플 페이로드를 생성하거나 특정 사용자 ID를 검색하여 생성합니다. 자세한 내용은 위의 [1d단계](#step-1d)를 참조하세요.
