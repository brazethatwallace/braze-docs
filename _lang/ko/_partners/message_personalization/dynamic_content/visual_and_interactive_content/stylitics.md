...
![Braze 이메일 경험에 삽입된 Stylitics 번들 콘텐츠 예시.]({% image_buster /assets/img/stylitics.png %}){: style="max-width:60%;"}
...
1. 이메일에 번들을 통합합니다. ESP가 위치와 커스터마이징을 결정합니다.
2. ESP가 Stylitics 콘텐츠를 포함하도록 트리거 이메일 코드를 업데이트합니다.
3. ESP가 업데이트된 트리거 시리즈를 테스트, 미리보기 및 시작합니다.

Stylitics는 아이템에 대한 번들 데이터만 제공합니다. 귀하와 ESP 사이에서 사용자 데이터를 보유하고 Stylitics 번들 데이터를 연결하여 사용자에게 전송할 수 있습니다.
...
귀하 또는 ESP가 아이템별로 API 호출을 수행하여 이메일에 번들 데이터를 채울 수 있습니다. Stylitics는 즉시 사용할 수 있는 자체 API를 사용하여 API 호출을 수행할 것을 권장합니다.
...
귀하 또는 ESP가 플랫 파일에서 아이템의 번들 데이터를 참조하여 이메일에 번들 데이터를 채울 수 있습니다. Stylitics는 번들 데이터를 CSV, TXT 또는 XML 형식으로 변환하여 매일 전송할 수 있습니다. 또한 ESP의 요구 사항에 맞게 파일 형식을 조정하는 데 도움을 줄 수 있습니다. 이 파일을 생성하는 데 2~3주가 소요됩니다.
...
- **파일 키**: ESP가 데이터를 참조할 수 있도록 파일의 키로 사용할 아이템 데이터 문자열에 대해 귀하와 Stylitics가 합의해야 합니다. SKU, `item_group_id` 또는 `item_number`가 일반적으로 사용됩니다.
...
귀하와 ESP가 Stylitics 데이터와 번들을 삽입할 HTML 이메일 템플릿을 생성합니다. 다음은 몇 가지 모범 사례와 권장 사항입니다.
...
Stylitics는 이 유형의 이메일 프로그램에 대한 번들 데이터를 제공합니다. 따라서 귀하, ESP 및 Stylitics 간의 공개 데이터 공유를 요청합니다. 가능하다면 성과 향상을 이해하고 프로그램을 개선하기 위해 다음 측정기준을 제공해 주시기 바랍니다:
- 발송된 이메일 수
- 열람된 이메일 수
- 조회 및 인게이지먼트
- 클릭률
...
- 사용할 이메일 결정
- 데이터 교환을 논의하기 위해 Stylitics와 ESP를 연결하여 API 옵션 또는 플랫 파일 옵션 결정
- ESP와 목업 생성
- 분석 방법 조율
- 출시 일정 조율