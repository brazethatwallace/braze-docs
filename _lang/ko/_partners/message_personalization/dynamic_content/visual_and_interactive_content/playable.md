...
Braze와 Playable 통합을 사용하면 최고의 콘텐츠(고품질 비디오)를 최고의 오디언스(이메일)에게 전달할 수 있으며, 받은편지함에서 자동으로 재생되는 고품질 비디오 콘텐츠로 클릭률 및 클릭 후 측정기준을 높일 수 있습니다.
...
Playable 플랫폼에서 비디오 파일을 업로드하거나 Facebook, Instagram, YouTube, X(구 Twitter), TikTok 등에 있는 비디오 URL을 입력하여 비디오를 추가합니다.

### 2단계: Playable에서 임베드 코드 복사하기 {#step-2-copy-the-embed-code-from-playable}

비디오를 업로드하면 Playable이 코드를 생성합니다. 이 코드를 Braze Campaign에 삽입하면 이메일에 비디오가 임베드되어 이메일을 열 때 자동으로 재생됩니다. 이메일이 열리면 Playable 서버가 이메일 클라이언트, 기기, 화면 크기, 네트워크 상태에 따라 최적의 비디오 버전을 전달합니다.

{% alert tip %}
비디오는 iPhone Mail, Gmail, Apple Mail, iOS용 Outlook, Android용 Outlook, Mac용 Outlook, 최신 버전의 Windows용 Outlook 365를 포함하여 98% 이상의 받은편지함에서 자동 재생됩니다. 레거시 Windows용 Outlook 사용자에게는 정적 이미지가 대신 표시됩니다.
{% endalert %}

### 3단계: Braze에 임베드 코드 붙여넣기 {#step-3-paste-the-embed-code-into-braze}

Braze 이메일 Campaign에 코드를 붙여넣은 다음, 이메일 Campaign을 디자인하고 테스트한 후 게시합니다.