...
Stayfilm은 다음을 포함하여 고객 생애주기 전반에 걸쳐 개인화된 비디오 전달을 지원합니다:

- **온보딩 및 환영 여정:** 프로필이나 가입 맥락에 맞게 개인화된 비디오로 신규 사용자를 환영합니다
- **제품 및 마켓플레이스 콘텐츠:** 카탈로그 또는 사용자 제공 미디어에서 제품 중심 비디오를 생성합니다
- **전환 및 활성화:** 상황별 비디오 메시징으로 주요 행동을 강화합니다
- **로열티 및 업셀:** 개인화된 오퍼 또는 사용 마일스톤을 비디오 형식으로 강조합니다
- **윈백 및 고객이탈 방지:** 맞춤형 비디오 콘텐츠로 비활성 사용자를 다시 참여시킵니다
...
| Braze 데이터 변환 | [Braze 데이터 변환]({{site.baseurl}}/user_guide/data/unification/data_transformation)을 사용하여 Stayfilm 콜백을 수신하고 [`/users/track` 엔드포인트]({{site.baseurl}}/api/endpoints/user_data/post_user_track)를 통해 Braze 고객 프로필에 매핑합니다. |
| Braze 사용자 식별자 | 이 안내서에서는 `external_id`를 사용하여 Stayfilm 작업을 Braze 고객 프로필과 연결합니다. `CallbackRelayData`에 전달하는 값은 Braze에서 사용자의 `external_id`와 일치해야 합니다. |
...
1. **아웃바운드:** Braze [웹훅 Campaign]({{site.baseurl}}/user_guide/channels/webhooks)이 Stayfilm `POST /Job` 엔드포인트로 렌더 작업을 전송합니다. 요청에는 사용자 미디어, 템플릿 구성, Braze 사용자의 `external_id`로 설정된 `CallbackRelayData`가 포함됩니다.
...
3. **전달:** 저장된 `stayfilm_video_url` 속성을 커스텀 HTML이 포함된 [인앱 메시지]({{site.baseurl}}/user_guide/channels/in_app_messages) 등의 메시징 채널에서 사용합니다.

이 안내서의 데이터 변환은 다음 커스텀 속성을 기록합니다:
...
변환은 또한 `stayfilm_video_ready` 또는 `stayfilm_video_failed`라는 이름의 커스텀 이벤트를 기록합니다.

## 통합 {#integration}

다음 단계는 개념 증명을 안내합니다. 흐름을 검증한 후 작업 페이로드, 속성, 메시징을 사용 사례에 맞게 조정하세요.

### 1단계: 테스트 사용자 만들기 {#step-1-create-a-test-user}

통합을 구축하고 검증하는 동안 사용할 테스트 사용자 프로필을 만드세요. 자세한 내용은 [사용자 가져오기]({{site.baseurl}}/user_guide/audience/manage_audience/import_users)를 참조하세요.

1. **오디언스** > **사용자 가져오기**로 이동합니다.
2. **빠른 사용자 추가**를 선택합니다.
3. `external_id`와 기타 필수 필드를 입력한 다음 **새 사용자 만들기**를 선택합니다.

{% alert important %}
이메일, 전화번호, 성명, 정부 발급 ID, 주소, 주문 세부 정보 등 개인 데이터를 `external_id`로 사용하지 마세요. 이 통합 전체에서 `external_id`를 대소문자 구분으로 처리하세요.
{% endalert %}

이 안내서에서는 `stayfilm-poc-001`을 예시 `external_id`로 사용합니다. 이후 단계에서 사용하므로 선택한 값을 기록해 두세요.

### 2단계: 데이터 변환 만들기 {#step-2-create-a-data-transformation}

Stayfilm 콜백을 수신하고 고객 프로필을 업데이트하기 위한 데이터 변환을 만드세요.
...
{: start="8"}
8. **저장**을 선택한 다음 생성된 웹훅 URL을 복사합니다.
...
cURL, Postman 또는 유사한 도구로 요청을 보냅니다. 성공적인 응답은 `{"message": "success"}`와 함께 HTTP 상태 `201`을 반환합니다.

{: start="10"}
10. **데이터 설정** > **데이터 변환**으로 이동하고, 변환이 목록에 나타나지 않으면 페이지를 새로고침합니다.
11. 변환을 열고 **검증**을 선택합니다. **출력**에서 검증이 성공했는지 확인합니다.
12. **활성화**를 선택합니다.
13. 복사한 웹훅 URL을 콜백 URL로 Stayfilm에 제공합니다.

{% alert note %}
`CallbackRelayData`에 Braze `external_id` 외에 추가 데이터를 저장하는 경우, `RelayedData`를 적절히 파싱하도록 변환 코드를 업데이트하세요.
{% endalert %}

### 3단계: Stayfilm에 작업을 전송하는 웹훅 Campaign 만들기 {#step-3-create-a-webhook-campaign-to-send-jobs-to-stayfilm}

Stayfilm에 렌더 작업을 제출하는 [웹훅 Campaign]({{site.baseurl}}/user_guide/channels/webhooks)을 만드세요.
...
7. **요청 본문**에서 **Raw Text**를 선택한 다음 Stayfilm이 제공하는 작업 페이로드를 붙여넣습니다. [연결된 콘텐츠]({{site.baseurl}}/user_guide/personalization_and_dynamic_content/connected_content/making_an_api_call)를 사용하여 본문을 동적으로 만들 수 있습니다.

Braze 사용자의 `external_id`로 설정된 `CallbackRelayData`를 포함합니다. Stayfilm은 콜백에서 이 값을 `RelayedData`로 반환합니다.
...
| `Authorization` | 연결된 콘텐츠를 통해 가져온 OAuth 베어러 토큰(다음 예시 참조) |
{: .reset-td-br-1 .reset-td-br-2 aria-label="요청 헤더" }

다음 연결된 콘텐츠 블록에서 *`{TENANT_ID}`*, *`{CLIENT_ID}`*, *`{CLIENT_SECRET_URL_ENCODED}`*, *`{SCOPE_URL_ENCODED}`*를 Stayfilm이 제공하는 값으로 교체합니다. *`{CLIENT_SECRET_URL_ENCODED}`*와 *`{SCOPE_URL_ENCODED}`*는 블록에 붙여넣기 전에 URL 인코딩하세요. OAuth 요구 사항은 [Stayfilm API 설명서](https://apidoc.stayfilm.com)를 참조하세요.
...
{% alert note %}
Campaigns 페이지를 떠났다가 돌아오는 경우, **상태**를 **전체**로 설정하여 아직 **초안** 상태인 Campaign을 찾으세요.
{% endalert %}

### 4단계: 웹훅 Campaign 테스트 {#step-4-test-the-webhook-campaign}

1. 웹훅 작성기에서 **테스트** 탭을 선택합니다.
2. **사용자로 메시지 미리보기**에서 **기존 사용자 선택**을 선택한 다음 테스트 사용자(예: `stayfilm-poc-001`)를 검색합니다.
3. **테스트 전송**을 선택합니다.

성공적인 응답은 다음과 유사한 JSON 본문과 함께 HTTP 상태 `201`을 반환합니다:
...
Stayfilm은 비디오를 비동기적으로 렌더링하고 처리가 완료되면 데이터 변환으로 콜백을 전송합니다. [Stayfilm API 설명서](https://apidoc.stayfilm.com)에 설명된 Stayfilm API 엔드포인트를 통해 작업 상태를 모니터링하세요.
...
고객 프로필에 `stayfilm_video_url`이 채워진 후 Campaign 또는 Canvas에서 렌더링된 비디오를 표시합니다.

1. **메시징** > **Campaigns**로 이동합니다.
2. **Campaign 만들기** > **인앱 메시지**를 선택합니다.
3. `Stayfilm Video Show`와 같은 Campaign 이름을 입력합니다.
4. 메시지 작성기에서 **기존 편집기**를 선택합니다.
...
8. **초안 저장**을 선택합니다.
9. **테스트** 탭을 선택합니다.
10. **사용자로 메시지 미리보기**에서 **기존 사용자 선택**을 선택한 다음 테스트 사용자의 `external_id`를 검색합니다.

프로필에 `stayfilm_video_url`이 설정되어 있으면 렌더링된 비디오가 미리보기에 나타나고 재생됩니다.

## 통합 확장 {#extend-the-integration}

이 안내서는 Stayfilm API의 일부만 다룹니다. 작업 템플릿, 미디어 입력 또는 다운스트림 메시징을 조정하려면 [Stayfilm API 설명서](https://apidoc.stayfilm.com)를 참조하고 웹훅 페이로드, 데이터 변환 매핑, Campaign 로직을 적절히 업데이트하세요.

## 고려 사항 {#considerations}

- **비동기 렌더링:** 비디오 생성은 즉시 이루어지지 않습니다. 웹훅과 동일한 흐름에서 인앱 메시지를 보내는 대신 `stayfilm_video_ready` 커스텀 이벤트 또는 `stayfilm_video_status` 기반 Segment에서 후속 메시징을 트리거하세요.
- **식별자 일관성:** `CallbackRelayData`의 값은 Braze 사용자의 `external_id`와 정확히 일치해야 합니다.
- **OAuth 토큰 캐싱:** 연결된 콘텐츠 예시는 OAuth 토큰을 3000초 동안 캐싱합니다. Stayfilm이 토큰 수명 요구 사항을 변경하면 `cache_max_age`를 조정하세요.
- **샌드박스 테스트:** 프로덕션 출시 전에 Braze 샌드박스에서 전체 콜백 루프를 검증하세요.
- **커스텀 속성 용량:** 이 통합이 생성하는 Stayfilm 커스텀 속성 및 이벤트에 대한 워크스페이스 용량이 충분한지 확인하세요.
...
| 문제 | 해결 방법 |
| ----- | ---------- |
| 데이터 변환 검증 실패 | 테스트 페이로드의 `RelayedData`가 유효한 Braze `external_id`와 일치하는지 확인한 다음, **검증**을 선택하기 전에 **데이터 변환** 페이지를 새로고침하세요. |
| 웹훅 테스트가 201이 아닌 응답을 반환 | 요청 헤더의 Stayfilm 자격 증명을 확인하고, OAuth 연결된 콘텐츠 블록이 URL 인코딩된 값을 사용하는지 확인하며, `POST /Job` URL이 올바른지 점검하세요. |
| 변환 로그에 콜백이 나타나지 않음 | Stayfilm이 활성 데이터 변환 웹훅 URL을 가지고 있는지 확인하고 비디오 렌더링이 완료될 때까지 기다리세요. |
| 인앱 미리보기에 비디오가 표시되지 않음 | 테스트 사용자 프로필에 `stayfilm_video_url`이 설정되어 있는지, 인앱 메시지가 **커스텀 코드**로 **웹 브라우저**를 대상으로 하는지 확인하세요. |
