...
description: "이 참조 문서에서는 마케팅 캠페인을 개인화하는 데 사용할 수 있는 날씨 API인 Braze와 AccuWeather 간의 파트너십에 대해 설명합니다."
page_type: partner
search_tag: Partner
...
다음은 Braze Campaigns 및 Canvases에서 참조할 수 있는 AccuWeather API입니다.

| API | 설명 |
|---|---|
| [Locations](https://apidev.accuweather.com/developers/locationsAPIguide) | 원하는 위치에 대한 위치 키를 가져옵니다. 위치 키를 사용하여 Forecast 또는 Current Conditions API에서 날씨 데이터를 검색합니다. |
| [Forecast](https://apidev.accuweather.com/developers/forecastsAPIguide) | 특정 위치에 대한 예보 정보를 가져옵니다. |
| [Current Conditions](https://apidev.accuweather.com/developers/currentConditionsAPIGuide) | 특정 위치에 대한 현재 조건 데이터를 가져옵니다. |
| [Indices](https://apidev.accuweather.com/developers/indicesApiGuide) | 특정 위치에 대한 일일 지수 값을 가져옵니다. 지수 가용성은 위치에 따라 다릅니다. |
| [Weather Alarms](https://apidev.accuweather.com/developers/weatheralarmsAPIguide) | 특정 위치에 대한 날씨 경보를 가져옵니다. AccuWeather 날씨 경보는 해당 위치의 일일 예보를 사용하여 결정됩니다. 예보 날씨가 [특정 임계값](https://apidev.accuweather.com/developers/weatheralarms)을 충족하거나 초과하면 해당 위치에 대한 경보가 존재합니다. |
| [Alerts](https://apidev.accuweather.com/developers/alertsApiGuide) | 공식 정부 기상청 및 주요 글로벌 기상 경보 제공업체로부터 악천후 경고를 가져옵니다. |
...
다음 예시에서는 미국 내 사용자의 우편번호에 대한 현재 조건에 따라 두 가지 다른 유형의 메시지를 표시하는 연결된 콘텐츠 호출을 보여줍니다. AccuWeather Locations 및 Current Conditions API 엔드포인트가 사용됩니다.
{% raw %}
...
다음은 AccuWeather가 JSON 오브젝트로 반환하는 예시입니다:
...
"Key" ID는 두 번째 GET 요청에서 사용되므로 유용한 변수입니다.
...
[WeatherText](https://apidev.accuweather.com/developers/currentConditionsAPIGuide)의 출력 결과를 `{{local_weather[0].WeatherText}}`를 참조하여 테스트할 수 있습니다.

API 호출이 `{{local_weather[0].WeatherText}}`로 응답하여 `Rain`을 반환하면, 사용자는 해당 푸시를 받게 됩니다.
