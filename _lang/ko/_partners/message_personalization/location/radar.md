...
> [Radar](https://www.radar.com/)는 선도적인 지오펜싱 및 위치 추적 플랫폼입니다. Radar 플랫폼에는 세 가지 핵심 제품이 있습니다: [지오펜스](https://radar.com/product/geofencing), [여행 추적](https://radar.com/product/trip-tracking), [Geo API](https://radar.com/product/api). 업계를 선도하는 Braze 참여 플랫폼과 Radar의 업계 최고 수준의 지오펜싱 기능을 결합하면 다양한 위치 기반 제품 및 서비스 경험을 통해 매출과 로열티를 높일 수 있습니다. 여기에는 픽업 및 배송 추적, 위치 트리거 알림, 상황별 개인화, 위치 확인, 매장 찾기, 주소 자동 완성 등이 포함됩니다.

_이 통합은 Radar에서 유지 관리합니다._
...
| 요구 사항 | 설명 |
|---|---|
| Radar 계정 | 이 파트너십을 이용하려면 Radar 계정이 필요합니다. |
...
사용자가 도로변 픽업을 위해 매장에 도착하면 도착 안내와 함께 푸시 알림을 보냅니다.
...
!["New In Store Deals, Walmart and target near you"를 표시하는 연결된 콘텐츠 푸시 메시지의 Android 이미지.]({% image_buster /assets/img/radar_example.png %}){: style="float:right;max-width:30%;border:0;"}
...
다음 예제에서는 Target 및 Walmart 위치에 대해 Radar 체인 필터링이 적용되고 주변 위치의 검색 반경이 2km로 설정되어 있습니다.
...
사용 사례를 종합하면 Campaign의 구문은 다음과 같습니다. 다음 코드는 `nearbyplaces.places` 오브젝트를 반복하여 고유 값을 추출하고 사람이 읽을 수 있는 적절한 구분 기호를 사용하여 메시지를 연결합니다.
...
{% alert tip %}
연결된 콘텐츠에서 사용할 수 있는 모든 Radar API는 [Radar 설명서](https://radar.com/documentation/api)를 참조하세요.
{% endalert %}