...
Braze와 Extole 통합을 사용하면 Extole의 친구 추천 및 성장 프로그램에서 고객 이벤트와 속성을 Braze로 가져올 수 있습니다. 이를 통해 고객 확보, 참여, 로열티를 높이는 더욱 개인화된 마케팅 Campaign을 만들 수 있습니다. 또한 개인화된 공유 코드 및 링크와 같은 Extole 콘텐츠 속성을 Braze 커뮤니케이션에 동적으로 가져올 수도 있습니다.
...
다음 활용 사례는 Extole과 Braze의 통합을 사용하는 몇 가지 방법을 보여줍니다. Extole 구현 및 고객 성공 매니저와 협력하여 회사의 특정 요구에 맞는 옵션을 개발하세요.

- 추천 및 참여 프로그램의 커스텀 이벤트를 사용하여 Braze Campaign 또는 Canvas를 트리거합니다
...
5. Braze로 전송하려는 Extole 이벤트를 추가합니다. 기본 이벤트, 이벤트 등록정보 및 사용자 속성은 [Extole 이벤트 테이블](https://dev.extole.com/docs/braze#extole-program-events)에 설명되어 있습니다.
6. `FULFILLED` 상태 외에 Braze로 전송하려는 보상 상태를 추가합니다. 사용 가능한 보상 상태에 대한 설명은 [Extole 보상 테이블](https://dev.extole.com/docs/braze#extole-rewards)을 참조하세요.
...
아래는 Extole이 Braze로 전송하는 기본 이벤트, 이벤트 등록정보 및 사용자 속성입니다. Extole 구현 또는 고객 성공 매니저에게 문의하여 통합에 추가할 Extole 이벤트를 식별하고 추가하세요.
...
### Extole 보상 {#extole-rewards}

기본적으로 Extole은 `FULFILLED` 상태의 보상 이벤트를 Braze로 전송하여 Braze Campaign 또는 Canvas를 통해 보상 알림을 트리거할 수 있도록 합니다. 추가 보상 상태에 대해서는 다음 테이블을 참조하세요.

| 보상 상태 | 설명 | 이벤트 등록정보 | 사용자 속성 |
| ----------- | ----------- | ----------- | ----------- |
| `FULFILLED` | 기본 상태입니다. Extole 보상 공급자가 보상에 값(예: 쿠폰 또는 기프트 카드)을 할당했습니다. | 이메일 <br>액면가  <br>쿠폰 코드  <br>액면가 유형  | 이메일 <br>이름  <br>성 |
| `EARNED` | 보상이 생성되어 사용자에게 연결되었습니다. | 이메일 <br>액면가  <br>쿠폰 코드  <br>액면가 유형  | 이메일 <br>이름  <br>성 |
| `SENT` | 보상이 이행되어 이메일 또는 기기를 통해 수신자에게 전송되었습니다. | 이메일 <br>액면가  <br>쿠폰 코드  <br>액면가 유형  | 이메일 <br>이름  <br>성 |
| `REDEEMED` | Extole로 전송된 전환 또는 사용 이벤트에서 확인된 바와 같이 수신자가 보상을 사용했습니다. | 이메일 <br>액면가  <br>쿠폰 코드  <br>액면가 유형  | 이메일 <br>이름  <br>성 |
| `FAILED` | 문제가 발생하여 보상이 발급되거나 전송되지 못했으며 주의가 필요합니다. | 이메일 <br>액면가  <br>쿠폰 코드  <br>액면가 유형  | 이메일 <br>이름  <br>성 |
| `CANCELED` | 보상이 비활성화되어 재고로 반환됩니다. | 이메일 <br>액면가  <br>액면가 유형  | 이메일 <br>이름  <br>성 |
| `REVOKED` | 이행된 보상이 무효화되었습니다. 예를 들어, Extole이 공급자 기프트 카드를 요청한 후 해당 카드가 오류로 전송되었음을 확인한 경우입니다. 공급자가 보상 취소를 지원하는 경우 Extole은 자금 반환을 요청하며 해당 보상은 더 이상 유효하지 않게 됩니다. | 이메일 <br>액면가   <br>액면가 유형  | 이메일 <br>이름  <br>성 |
