...
| 이메일 템플릿용 Braze REST API 키 | `templates.email.create`, `templates.email.update`, `templates.email.list` 권한이 있는 API 키.<br><br>Braze 대시보드의 **설정** > **API 키**에서 키를 생성하세요. |
| Content Blocks용 Braze REST API 키 | `content_blocks.create`, `content_blocks.update`, `content_blocks.info`, `content_blocks.list` 권한이 있는 API 키.<br><br>Braze 대시보드의 **설정** > **API 키**에서 키를 생성하세요. |
...
* **일괄 가져오기(권장).** Braze에서 **Data Settings** > **Custom Attributes**로 이동하고 **Export**를 선택하세요. Building Blocks에서 CSV를 업로드하세요.
...
2. 액션 바에서 **Push to Braze**를 선택하세요.
3. 워크스페이스를 선택하고 확인하세요. Building Blocks가 렌더링된 Liquid와 함께 Braze에 이메일 템플릿을 생성합니다.

템플릿은 Braze의 **템플릿 및 미디어** > **이메일 템플릿**에 이메일 이름과 이메일 설정에서 선택한 날짜로 명명되어 표시됩니다.
...
3. 하나 이상의 콘텐츠 풀을 우선순위 순서로 추가하세요. 이것이 Waterfall입니다: 활성이고 날짜가 유효한 행이 있는 첫 번째 풀이 렌더링됩니다. 라이브 항목이 없으면 스마트 블록이 다음 풀로, 그 다음 풀로 넘어갑니다. 일반적인 패턴은 Flash Sale > Weekly Offers > Evergreen Favorites로, 항상 표시할 콘텐츠가 있습니다.
...
업로드된 Building Blocks 템플릿은 Braze의 **템플릿 및 미디어** > **이메일 템플릿**에서 찾을 수 있습니다. 동기화된 스타일시트와 블록은 **템플릿 및 미디어** > **Content Blocks**에서 확인할 수 있습니다.
