...
| Braze API 캠페인 ID | [Braze API 캠페인 ID]({{site.baseurl}}/api/api_campaigns/)는 SmarterSends를 통해 전송되는 모든 캠페인의 고유 식별자입니다. Braze 대시보드의 **Messaging** > **Campaigns**에서 생성할 수 있습니다. |
{: .reset-td-br-1 .reset-td-br-2 role="presentation" }

## 활용 사례 {#use-cases}

Braze와 SmarterSends 통합을 통해 여러 채널과 위치에서 마케팅 캠페인을 생성하고 실행하여 분산 마케팅을 활용할 수 있습니다. 이러한 이점은 다음과 같습니다.
...
1. Braze에서 **Settings** > **API Keys**로 이동하여 **Create New API Key**를 클릭합니다.
2. API 키의 이름을 입력합니다.
...
1. Braze 워크스페이스에서 **Settings** > **App Settings**로 이동합니다.
2. 새 앱을 설정하거나 워크스페이스 내 기존 애플리케이션의 애플리케이션 ID를 사용합니다. 애플리케이션 ID는 **API Key**로 표시됩니다.
3. 이 ID를 복사하여 SmarterSends의 **App ID** 필드에 붙여넣습니다.

### 3단계: API 캠페인 생성 {#step-3-create-an-api-campaign}

API 캠페인을 사용하면 Braze 내에서 모든 SmarterSends 메일의 측정기준을 추적할 수 있으며, SmarterSends가 이러한 API 기반 캠페인을 트리거할 수 있습니다.

1. Braze에서 [API 캠페인을 생성]({{site.baseurl}}/api/api_campaigns/#create-a-new-campaign)합니다.
2. **Select Message Channel** 아래의 **Email**을 클릭하여 메시징 채널을 추가하고 측정기준 추적을 시작합니다.
3. 다음으로, Braze의 캠페인 ID를 복사하여 SmarterSends의 **Campaign ID** 필드에 붙여넣습니다.
...
5. SmarterSends에서 생성하는 각 그룹에 대해 Braze의 API 캠페인에 메시지 배리언트를 추가합니다. 그런 다음 메시지 배리언트 ID를 SmarterSends의 해당 그룹 메시지 배리언트 ID에 복사합니다.
...
각 SmarterSends 인스턴스는 브랜드의 로고 색상과 커스텀 도메인 이름으로 완전히 커스터마이징할 수 있어 익숙한 환경을 제공합니다. 또한 추가적인 개인화를 위해 Braze 워크스페이스 내 Segments를 기반으로 캠페인에서 사용자를 타겟팅할 속성 및 커스텀 속성을 정의할 수 있습니다.