...
> [Airbridge](https://www.airbridge.io/)는 모바일 기여도, 증분 측정, 마케팅 믹스 모델링을 통해 성장의 원천을 발견할 수 있는 통합 모바일 측정 플랫폼입니다.
...
자세한 지침은 [Airbridge](https://help.airbridge.io/en/guides/braze)를 방문하세요.

### 4단계: 통합 확인 {#step-4-confirm-the-integration}

Braze가 Airbridge로부터 기여도 데이터를 수신하면, Braze의 Airbridge 기술 파트너 페이지에서 연결 상태 표시기가 "Not Connected"에서 "Connected"로 변경되고 마지막으로 성공한 요청의 타임스탬프가 포함됩니다.

이 상태는 Braze가 기여 설치에 대한 데이터를 수신한 후에만 변경됩니다. Braze는 오가닉 설치를 무시하고(Airbridge 포스트백에서 제외) 연결 성공 여부를 판단할 때 이를 계산하지 않습니다.

## 사용 가능한 데이터 필드 {#available-data-fields}

Airbridge는 다음 데이터 필드 차트에 나열된 네 가지 유형의 기여도 데이터를 Braze에 보낼 수 있습니다. 이 데이터는 Airbridge 대시보드에서 볼 수 있으며 사용자 설치 경로 및 필터링에 사용됩니다.

제안된 대로 통합을 구성하면, Braze는 설치 데이터를 Segment 필터에 매핑합니다.

| Airbridge 데이터 필드 | Braze Segment 필터 | 설명 |
| -------------------- | ---------------------| ---- |
| `Channel` | Install Attribution Source | 설치 또는 딥링크 열람이 기여된 채널 |
...
Meta 비즈니스 Campaign에 대한 기여도 데이터는 파트너를 통해 제공되지 않습니다. 이 미디어 소스는 파트너가 서드파티와 기여도 데이터를 공유하는 것을 허용하지 않으므로 파트너는 해당 데이터를 Braze로 전송할 수 없습니다.

## Braze에서 Airbridge 클릭 추적 URL(선택 사항) {#airbridge-click-tracking-urls-in-braze-optional}

Braze Campaign에서 클릭 추적 링크를 사용하면 어떤 Campaign이 앱 설치와 재참여를 유도하는지 확인할 수 있습니다. 결과를 사용하여 마케팅 성과를 측정하고 더 높은 ROI를 위해 리소스를 어디에 투자할지 결정하세요.

Airbridge 클릭 추적 링크를 시작하려면 [Airbridge](https://help.airbridge.io/en/guides/creating-a-new-tracking-link)를 방문하세요. 설정이 완료되면 Airbridge 클릭 추적 링크를 Braze Campaign에 직접 삽입할 수 있습니다. 그러면 Airbridge는 링크를 클릭한 사용자를 기여하기 위해 [확률적 기여도 방법론](https://help.airbridge.io/en/guides/identity-matching)을 사용합니다. Braze Campaign에서 기여도의 정확성을 개선하기 위해 Airbridge 추적 링크에 기기 식별자를 추가하는 것을 권장합니다. 이를 통해 링크를 클릭한 사용자를 결정론적으로 기여할 수 있습니다.
