...
<div>
    <div class="container-fluid" id="main-container">
      <div class="row main-form">
        ...
        <aside class="operator-callout" aria-label="팁: BrazeAI Operator">
...
<div class="form-container">
        <form id="supportForm">
            ...
            <div id="step1">
                ...
                <button type="button" class="submit-btn" id="toStep2" >추천 콘텐츠로 계속</button>
            </div>
...
<div class='gpt-text1'>답변이 생성되었습니다 </div>
                 </div>
    ...
    <linearGradient id="paint0_linear_45139_47693" x1="890.652" y1="4.5" x2="-0.00114137" y2="4.5" gradientUnits="userSpaceOnUse">
      ...
      <stop offset="0.9" stop-color="#FFA524"/>
                ...
                <div class= "legal-disclaimer">
                  <em>면책 조항: 이 답변은 AI에 의해 생성되었으며 정보 제공 목적으로만 사용됩니다. Braze 설명서에서 링크된 출처를 확인해 주세요.</em>
                </div>
                <div class="gpt-res-buttons">
                <button type="button" class="submit-btn" id="backToStep1" style="background-color:grey;">뒤로</button>
                <div class="btn-right">

                <lable class='lable-gpt'>문제가 해결되었나요?</lable>
                <button type="button" class="submit-btn" onclick="window.location.href='/docs/'" >네, 현재 창 닫기 </button>
                <button type="button" class="submit-btn" id="toStep3">아니요, 티켓 생성 계속하기</button>
                </div>
                </div>
                </div>
...
<div class="form-container1"  style="display:none;">
 <div id="step3" >
<div id="firefox_warning" style="display:none;">Firefox 사용자의 경우 이 사이트를 허용 목록에 추가하거나 <a href="https://developer.mozilla.org/en-US/docs/Mozilla/Firefox/Privacy/Tracking_Protection?utm_source=mozilla&utm_medium=firefox-console-errors&utm_campaign=default" target="_blank">추적 방지 설정</a> 을 확인하지 않으면 티켓이 제출되지 않을 수 있습니다.</div>

<div id="ticket_mainform" class="container">
    <form  id="ticket_form">
      <div id="step3">
        <h2>필요한 것을 찾지 못하셨나요? 고객지원 팀에 문의하세요.</h2>
        <!-- Row 1 -->
        <div class="row">
          <div class="col-md-6">
            <div class="form-group">
        ...
        <div class="row">
          <div class="col-md-6" id="subcategory_div" style="display:none;">
            <div class="form-group">
              <label for="ticket_subcategory" class="required">질문 내용... </label>
              <select id="ticket_subcategory" name="00N0V000009G0ML" class="form-control"></select>
            </div>
          </div>
          <!-- <div class="col-md-6">
            <div class="form-group">
              <label for="ticket_type">플랫폼 *</label>
        ...
        <div class="row">
          <div class="col-md-6">
            <div class="form-group">
              <label for="ticket_name" class="required">이름</label>
              <input type="text" id="ticket_name" name="Name" placeholder="이름을 입력하세요" class="form-control" required>
            </div>
          </div>
          <div class="col-md-6">
            <div class="form-group">
              <label for="ticket_email" class="required">이메일 주소 </label>
              <input type="email" id="ticket_email" name="Email" placeholder="이메일을 입력하세요" class="form-control" required>
            </div>
          </div>
        </div>
        ...
        <div class="row">
          <div class="col-md-6">
            <div class="form-group">
              <label for="ticket_ccemail">참조 이메일 주소</label>
              <input type="email" id="ticket_ccemail" name="CCEmail" placeholder="참조 이메일을 입력하세요" class="form-control">
            </div>
          </div>
          <div class="col-md-6">
            <div class="form-group">
              <label for="ticket_priority" class="required">문제 심각도 </label>
              <select id="ticket_priority" name="priority" class="form-control">
                <option value="Critical">심각: 시스템 다운 또는 심각한 데이터 무결성 문제</option>
                <option value="High">높음: 심각한 기능 손실 또는 캠페인 발송 불가</option>
                <option value="Medium">보통: 성능 저하 또는 비즈니스에 상당한 영향을 미치는 문제</option>
                <option value="Low" selected>낮음: Braze 기능 또는 분석에 대한 질문</option>
              </select>
            </div>
          </div>
        </div>
        ...
        <!-- Subject (Full width) -->
        <div class="form-group">
          <label for="ticket_subject" class="required">제목 </label>
          <input type="text" id="ticket_subject" name="Subject" placeholder="어떤 내용에 대한 질문인가요?" class="form-control" required>
        </div>

        <!-- Question (Full width) -->
        <div class="form-group">
          <label for="ticket_issue" class="required">질문 </label>
          <textarea id="ticket_issue" name="ticket_issue" rows="7" class="form-control" placeholder="플랫폼, SDK 버전, REST API 엔드포인트, 재현 단계 등의 세부 정보를 포함해 주세요..."></textarea>
        </div>

        <!-- Info text -->
        <div class="form-group small-text">
          <p>기술 지원을 제공하거나 서비스 또는 기술 문제를 해결하기 위해 Braze가 대시보드와 데이터에 접근해야 할 수 있습니다. Braze 기술 지원은 글로벌 고객 기반을 지원하기 위해 여러 시간대에 걸쳐 표준 업무 시간 동안 운영됩니다. 해당 지역의 구체적인 지원 시간이나 업무 시간 외에 접수된 문제에 대해서는 지원 핸드북을 참조해 주세요. 응답 시간은 요청 제출 시점, 지원 등급 및 문제의 심각도에 따라 달라질 수 있습니다.</p>
        </div>

        <!-- Submit button -->
        <div class="form-group text-right">
          <button type="submit" name="Submit" value="Submit" class="btn btn-black" id="ticket_submit_button" role="button"> 제출 </button>
        </div>
...
var ticket_lookuptable = {
  ...
  'SelectOption': {
    'Technical Issue': {
      ...
      'SelectOption' : {
        'SDK Integrations' : {
          ...
          'LinksTitle': ['Self-Service SDK Debugging Tool','iOS: Initial SDK Setup','Android: Initial SDK Setup','Web: Initial SDK Setup','Sending Test Messages','Braze 학습 코스: Technical Integration Checklist and Toolkits'],
          ...
          'SelectOption' : {
            'Push' : {
              ...
              'SelectOption' : {
                ...
                'Other' : {
                  'ShowSubmit': true,
                  'LinksTitle': ['Braze Developer Guide','SDK 체인지로그','Sending Test Messages','Braze 학습 코스: Technical Integration Checklist and Toolkits'],
            ...
            'User Data' :{
              ...
              'Links': ['{{site.baseurl}}/developer_guide/platform_wide/analytics_overview#automatically-collected-data','{{site.baseurl}}/user_guide/data_and_analytics/custom_data/event_naming_conventions/','{{site.baseurl}}/user_guide/data_and_analytics/user_data_collection/user_profile_lifecycle#user-profile-lifecycle'],
                ...
                '커스텀 속성' : {
                  'ShowSubmit': true,
                  'LinksTitle': ['커스텀 속성','Android: Setting 커스텀 속성','iOS: Setting 커스텀 속성','Web: Setting 커스텀 속성'],
        ...
        'Email' : {
          ...
          'SelectOption': {
            ...
            'Deliverability' :{
              'ShowSubmit': true,
              'LinksTitle': ['Deliverability Pitfalls and Spam Traps','IP Warming','Braze 학습 코스: Achieving High Email Deliverability'],
            ...
            'Liquid' : {
              'ShowSubmit': true,
              'LinksTitle': ['Liquid Templating in Messages','Liquid Frequently Asked Questions','Braze 학습 코스: Dynamic Personalization with Liquid'],
        ...
        'Campaigns and Canvas' :{
          ...
          'SelectOption': {
            ...
            'Targeting and Segmentation' : {
              'ShowSubmit': true,
              'LinksTitle': ['Segmentation','Segment Insights','Braze 학습 코스: Segmentation',''],
            ...
            'Message Composition by Channel' : {
              ...
              'SelectOption' : {
                ...
                'Push' : {
                  'ShowSubmit': true,
                  'LinksTitle': ['Create a Push Campaign','Braze 학습 코스: Push'],
                ...
                'In-App Messages' : {
                  'ShowSubmit': true,
                  'LinksTitle': ['In-App Message Drag-And-Drop Editor Campaign','In-App Message Traditional Editor Campaign','Braze 학습 코스: In-App and In-Browser Messages'],
                ...
                'Content Cards' : {
                  'ShowSubmit': true,
                  'LinksTitle': ['Create a Content Card Campaign','Braze 학습 코스: Content Cards'],
    ...
    'Strategy Assistance' : {
      'Label': '* Category',
      'SelectDefault': 'Select a category...',
      'LinksTitle': ['Campaign Ideas and Strategies','Canvas Ideas and Strategies','Building Accessible Messages in Braze','Braze 학습 코스: Create Customer Journeys with Canvas Flow'],
      ...
      'SelectOption' : {
        'Tools and Use Cases' : {
          'ShowSubmit': true,
          'LinksTitle': ['Campaign Ideas and Strategies', 'Canvas Ideas and Strategies','Braze 학습 코스: Customer Engagement Tools and Use Cases'],
    ...
    'Account Administration' : {
      ...
      'SelectDefault': 'Select a category...',
      'Links': [],
      'ReferenceText': 'Your account 매니저 is a great resource for billing and contract related questions. ',
      'SelectOption' : {
        'Data Points' : {
          'ShowSubmit': true,
...
$( document ).ready(function() {
 ...
 function category_change(e) {
  ...
  $('#ticket_form').submit(function(e){
    ...
    $('#ticket_thankyou_msg').html('<h3>제출해 주셔서 감사합니다!</h3>고객지원 팀원이 곧 티켓에 응답할 것입니다.<br />확인 이메일을 받지 못한 경우 브라우저의 애드온, 콘텐츠/개인정보 설정 및 이메일 스팸 폴더를 확인해 주세요.<br />그래도 해결되지 않으면 담당 매니저에게 문의하거나 (<a href="mailto:support@braze.com">support@braze.com</a> 으로 이메일을 보내) 티켓이 제출되었는지 확인해 주세요.');
...
function initCaseForm() {
    ...
    document.getElementById('toStep3').addEventListener('click', function () {
        ...
        document.getElementById('ticket_form').addEventListener('submit', function(e) {
            ...
            if (!sid) {
                sid = createNewSid();
                document.쿠키 = `_gz_sid=${sid}; path=/; SameSite=None; Secure`;
