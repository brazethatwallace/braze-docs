...
---
{% api %}
## 1 이메일 템플릿 생성 {#1-create-email-template}
{% apimethod post %}
...
이메일 템플릿 REST API를 사용하여 Braze 대시보드의 템플릿 및 미디어 페이지에 저장된 이메일 템플릿을 프로그래밍 방식으로 관리할 수 있습니다. Braze는 이메일 템플릿을 생성하고 업데이트하기 위한 두 가지 엔드포인트를 제공합니다.

이 엔드포인트의 응답에는 후속 API 호출에서 템플릿을 업데이트하는 데 사용할 수 있는 `email_template_id` 필드가 포함됩니다.
...
#### 매개변수 세부 정보 {#parameter-details}

| 매개변수 | 필수 | 데이터 유형 | 설명 |
|---|---|---|---|
| `modified_after`  | 아니요 | ISO 8601 형식의 문자열 | 지정된 시간 또는 그 이후에 업데이트된 템플릿만 검색합니다. |
...
{% endapi %}
{% api %}
## 2 사용 가능한 이메일 템플릿 목록 {#2-list-available-email-template}
{% apimethod get %}
...
다음 엔드포인트를 사용하여 사용 가능한 템플릿 목록을 가져올 수 있습니다.
...
{% api %}
## 3 Campaigns 트리거 전송 {#3-campaigns-trigger-send}
{% apimethod post %}campaigns/trigger/send{% endapimethod %}
{% apitags %}Post, Campaigns, Trigger,Send{% endapitags %}

API 트리거 전달을 사용하면 메시지 콘텐츠를 Braze 대시보드에 보관하면서 API를 통해 메시지 전송 시기와 수신자를 지정할 수 있습니다.
...
{% api %}
## 4 Campaigns 트리거 전송 {#4-campaigns-trigger-send}
{% apimethod put %}users/track{% endapimethod %}
{% apitags %}PUT, Campaigns, Trigger, Send{% endapitags %}

이 엔드포인트는 커스텀 이벤트, 사용자 속성 및 구매를 기록하는 데 사용할 수 있습니다. 요청당 최대 75개의 속성, 이벤트 및 구매 오브젝트를 포함할 수 있습니다. 즉, 한 번에 최대 75명의 사용자에 대한 속성만 게시할 수 있지만, 동일한 API 호출에서 최대 75개의 이벤트와 최대 75개의 구매도 제공할 수 있습니다.
...
| 고객 프로필 필드 | 데이터 유형 사양 |
| ---| --- |
| country | (문자열) 국가 코드는 [ISO-3166-1 alpha-2 표준][17]에 따라 Braze에 전달해야 합니다. |
| current_location | (오브젝트) {"longitude": -73.991443, "latitude": 40.753824} 형식입니다. |
...
| dob | (생년월일) "YYYY-MM-DD" 형식의 문자열입니다(예: 1980-12-21). |
| email | (문자열) |
| email_subscribe | (문자열) 사용 가능한 값은 "opted_in"(이메일 메시지 수신을 명시적으로 등록함), "unsubscribed"(이메일 메시지 수신을 명시적으로 거부함), "subscribed"(수신 동의도 거부도 아님)입니다. |
| external_id | (문자열) 고유 사용자 식별자입니다. |
| facebook | `id`(문자열), `likes`(문자열 배열), `num_friends`(정수) 중 하나를 포함하는 해시입니다. |
| first_name | (문자열) |
| gender | (문자열) "M", "F", "O"(기타), "N"(해당 없음), "P"(밝히고 싶지 않음) 또는 nil(알 수 없음)입니다. |
| home_city | (문자열) |
| image_url | (문자열) 고객 프로필과 연결할 이미지의 URL입니다. |
| language | (문자열) 언어는 [ISO-639-1 표준][24]에 따라 Braze에 전달해야 합니다. <br>[허용되는 언어 목록](/docs/user_guide/data_and_analytics/user_data_collection/language_codes/) |
| last_name | (문자열) |
| marked_email_as_spam_at | (문자열) 사용자의 이메일이 스팸으로 표시된 날짜입니다. ISO 8601 형식 또는 yyyy-MM-dd'T'HH:mm:ss:SSSZ 형식으로 표시됩니다. |
| phone | (문자열) |
| push_subscribe | (문자열) 사용 가능한 값은 "opted_in"(푸시 메시지 수신을 명시적으로 등록함), "unsubscribed"(푸시 메시지 수신을 명시적으로 거부함), "subscribed"(수신 동의도 거부도 아님)입니다. |
