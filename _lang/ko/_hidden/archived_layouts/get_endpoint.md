...
article_title: "레이아웃 예시: GET: [엔드포인트 이름]"
search_tag: Endpoint
page_order: 1
excerpt_separator: ""
layout: api_page
...
{% alert note %}
`start_date`와 `end_date` 또는 `phone_numbers` 중 하나를 반드시 제공해야 합니다. `start_date`, `end_date`, `phone_numbers` 세 가지를 모두 제공하면 주어진 전화번호를 우선시하고 날짜 범위는 무시합니다.
{% endalert %}
