...
이 섹션은 파트너와 통합하고 서비스를 사용하기 시작하는 데 필요한 모든 것에 관한 것입니다. 이 정보를 전달하는 가장 좋은 방법은 통합이 추가 보안 검사나 승인을 받아야 하는지 여부와 같은 비기술적 중요 세부 사항을 설명하는 간단한 지침 문단을 사용하는 것입니다. 그런 다음 차트를 사용하여 통합의 기술적 요구 사항을 설명해야 합니다.

{% alert important %}
다음 요구 사항은 Braze에서 일반적으로 필요한 요구 사항입니다. 다음 차트에 나열된 속성 제목, 출처, 링크 및 문구를 사용하는 것을 권장합니다. 이러한 각 요구 사항이 어떤 용도로 사용되는지 알 수 있도록 설명을 조정하세요.
{% endalert %}

| 요구 사항 | 출처 | 액세스 | 설명 |
|---|---|---|---|
| Braze 워크스페이스 REST API 키 | Braze 플랫폼 | **Settings** > **App Settings** 페이지 | 이 설명은 워크스페이스 REST API 키를 어떻게 사용해야 하는지 알려줍니다. |
...
여기에서 통합을 단계별로 세분화합니다. 끝없는 문단을 작성하지 마세요. 이 문서는 마케터와 개발자 모두가 통합을 시작하고 실행하는 데 사용할 기술 문서입니다. 이 섹션의 유일한 목표는 Braze 사용자가 작업을 완료하는 데 도움이 되는 설명서를 작성하는 것입니다. 섹션 제목의 '통합 유형'은 병렬 통합인지, 서버 간 통합인지, 기본 통합인지를 나타냅니다. 이 파트너와 통합할 수 있는 방법이 두 가지 이상인 경우 여러 개의 통합 섹션을 사용할 수 있습니다.

이것이 커런츠 통합인 경우, 이 페이지는 Currents 섹션에 위치해야 하며, 해당 위치로 리디렉션하는 Nav 페이지를 구축해야 합니다.

### 1단계: 1단계에 대한 간략한 설명입니다 {#step-1-this-is-a-short-description-of-step-one}

필요에 따라 코드를 포함하여 세분화하면 됩니다. 한 가지 통합 방법만 제공할 필요 없이 여러 가지 코드 세트를 제공할 수 있다는 점을 기억하세요.
...
| 변수 | 설명 |
| -------- | ----------- |
| Page Title | 페이지 제목을 원하는 대로 지을 수 있습니다. 반드시 포함해야 합니다. |
| My First Heading | 대문자로 작성하는 것을 권장합니다. 이것은 선택 사항입니다. |
...
이 섹션은 **선택** 사항입니다. 여기에서 두 파트너 간의 통합을 커스터마이징하는 구체적인 방법을 설명할 수 있습니다.
...
이는 설명서에서 중요한 부분이 될 수 있습니다. 선택 사항이지만 통합의 일반적이거나 새로운 활용 사례를 설명하기에 좋은 곳입니다. 이를 통해 관계를 판매하거나 업셀하는 방법으로 사용할 수 있으며, 맥락과 아이디어를 제공하고 가장 중요한 것은 통합의 기능을 시각화할 수 있는 방법을 제공합니다.