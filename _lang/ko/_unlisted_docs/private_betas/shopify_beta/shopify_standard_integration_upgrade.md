...
- **주요 변경 사항:** 레거시 커넥터에서 새 커넥터로의 모든 중요한 변경 사항을 [Shopify 업그레이드 개요]({{site.baseurl}}/shopify_upgrade_overview#subscriber-collection)에서 확인하세요.
- **업그레이드 사전 요구 사항:** 엔지니어링 및 마케팅 팀과 함께 필요한 모든 [업그레이드 사전 요구 사항]({{site.baseurl}}/shopify_upgrade_overview#upgrade-prerequisites)을 완료했는지 확인하세요.
...
{% tab 구독자 목록 %}
통합을 통해 Shopify에서 이메일 또는 단문 메시지 서비스 구독자를 수집하고 있다면, 활성 메시지에 Shopify 스토어에 해당하는 구독자 목록이 포함되어 있는지 확인하세요.
...
{% alert important %}
업그레이드를 시작하기 전에 반드시 [모든 중단 변경 사항을 수정](#fixing-breaking-changes)해야 합니다.
{% endalert %}
...
![업그레이드 시 중단 변경 사항이 발생할 수 있음을 확인하는 Modal.]({% image_buster /assets/unlisted_docs/img/shopify/confirm_upgrade.png %})

### 2단계: Braze SDK 설정 {#step-2-set-up-the-braze-sdks}

표준 통합은 자동으로 Braze SDK를 Shopify 사이트에 추가합니다. 이미 Braze SDK를 직접 통합했거나 서드파티 도구를 사용한 경우, 업그레이드 시 이전 SDK 구현을 제거할 수 있도록 개발자와 조율하세요.

![새 통합이 자동으로 Braze 및 JavaScript SDK를 스토어에 구현한다는 것을 확인하는 Modal.]({% image_buster /assets/unlisted_docs/img/shopify/confirm_integration.png %}){: style="max-width:70%;"}
...
{% alert note %}
재인증 절차는 몇 분 정도 소요될 수 있으며, 완료되면 Shopify 페이지에서 자동으로 업데이트됩니다.
{% endalert %}
...
선택하는 외부 ID 유형은 Shopify 계정이 생성되거나 주문이 완료될 때 새 Shopify 고객 프로필에 할당됩니다. 또한 이미 Shopify 고객 ID 별칭이 있지만 Braze에서 외부 ID가 할당되지 않은 기존 고객 프로필을 업데이트하는 데에도 사용됩니다.

외부 ID 유형을 선택하려면 Braze로 돌아간 다음 **Confirm external ID**를 선택합니다.
...
![외부 ID를 선택하는 드롭다운이 있는 Modal.]({% image_buster /assets/unlisted_docs/img/shopify/external_id_field.png %}){: style="max-width:70%;"}
...
- **악용 위험:** 악의적인 사용자가 웹 브라우저를 변조하여 다른 사람의 이메일 주소를 자신의 외부 ID로 전송하면 민감한 메시지 또는 계정 정보에 접근할 수 있습니다.
{% endalert %}

기본적으로 Braze는 Shopify에서 제공된 이메일을 외부 ID로 사용하기 전에 자동으로 소문자로 변환합니다. 이메일 또는 해시된 이메일을 외부 ID로 사용하는 경우, 외부 ID로 할당하기 전이나 다른 데이터 소스에서 해싱하기 전에 이메일 주소도 소문자로 변환되었는지 확인하세요. 이렇게 하면 외부 ID의 불일치를 방지하고 Braze에서 중복 고객 프로필이 생성되는 것을 방지할 수 있습니다.

커스텀 외부 ID 유형을 선택한 경우 4.1~4.3단계로 진행합니다. 그렇지 않으면 5단계로 계속합니다.
...
메타필드가 생성된 후 고객에게 이를 채워 넣습니다. 다음 접근 방식을 권장합니다:
...
| 파라미터 | 설명 |
| --- | --- |
| `shopify_customer_id` | Shopify 고객 ID입니다. |
| `email_address` | 로그인한 사용자의 이메일 주소입니다. |
| `shopify_storefront` | 요청의 스토어프론트입니다. |
...
{% alert important %}
`shopify_customer_id`와 `email_address`가 Shopify의 고객 값과 일치하는지 확인하는 것이 중요합니다. [Admin API](https://shopify.dev/docs/api/admin-graphql) 또는 [Customer API](https://shopify.dev/docs/api/admin-rest/2025-04/resources/customer)를 사용하여 이러한 파라미터를 검증하고 `braze.external_id` 메타필드를 검색할 수 있습니다.
{% endalert %}
...
스토어 테마 내에서 Braze 앱 임베드를 활성화하려면 Braze로 돌아간 다음 **Go to Shopify**를 선택합니다.
...
Braze로 돌아오면 Shopify 통합 설치가 완료되었을 때 알림이 표시됩니다.
...
질문이 있는 경우 [지원팀에 문의]({{site.baseurl}}/user_guide/administer/personal/braze_support)하세요.