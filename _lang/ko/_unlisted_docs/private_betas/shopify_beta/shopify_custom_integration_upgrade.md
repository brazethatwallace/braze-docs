...
- 제품 조회 및 장바구니 업데이트 이벤트에 대한 추적 추가
...
![업그레이드로 인해 호환성 문제가 발생할 수 있음을 이해했는지 확인하는 모달.]({% image_buster /assets/unlisted_docs/img/shopify/confirm_upgrade.png %}){: style="max-width:50%;"}

체크박스를 선택하여 커스텀 경로 업그레이드의 1단계를 완료했는지 개발자와 확인한 다음 **Confirm**을 선택합니다.
...
| `shopify_storefront` | 요청에 대한 스토어프론트입니다. |
{: .reset-td-br-1 .reset-td-br-2 role="presentation"}
...
[4단계](#step-4-choose-an-external-id-type)를 반복하고, 커스텀 외부 ID를 Braze 외부 ID 유형으로 선택한 후 엔드포인트 URL을 입력합니다.
