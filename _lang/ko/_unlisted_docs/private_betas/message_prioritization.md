...
Braze는 이메일, LINE, 푸시 알림, 단문 메시지 서비스, 웹훅, WhatsApp 등 지원되는 채널에 대해 Currents 및 데이터 공유에서 메시지 우선순위 지정 관련 이벤트를 제공합니다. 여기에는 `users.messages.<channel>.Abort` 이벤트로 기록되는 우선순위 해제 및 최대 게재빈도 제한 이벤트와 설정된 재시도 창 내에서 메시지가 나중에 재시도된 시점을 보여주는 재시도 이벤트(`users.messages.<channel>.Retry` 이벤트로 기록)가 포함됩니다.
