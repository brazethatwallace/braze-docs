...
> 이 문서에서는 보고서 빌더에서 Segment를 보고 차원으로 사용하는 방법을 설명합니다. Segment별 보고 방법, Segment별 세분화 방법, 지원되는 조합을 포함합니다.

{% multi_lang_include alerts/early_access_beta_alert.md feature='Segment reporting' contact='customer success manager' %}

보고서 빌더는 행 및 드릴다운 옵션에서 **Segments**를 지원하므로, Segment의 성능을 확인하고 Campaign 또는 Canvas 성능을 Segment 멤버십별로 세분화할 수 있습니다. **행** 또는 **드릴다운** 드롭다운에 **Segments**가 표시되지 않는 경우, 해당 기능이 계정에 활성화되지 않은 것입니다.

다음과 같은 질문에 답할 수 있습니다:

- 특정 Segment가 시간 경과에 따라 어떤 성능을 보이고 있는가?
- 특정 Segment를 타겟팅하는 Campaigns와 Canvases는 무엇이며, 각각의 성능은 어떠한가?
...
4. (선택 사항) **드릴다운 추가**를 선택하여 Segment 데이터를 더 세분화합니다:
   ...
   - **날짜:** Segment의 크기 또는 성능 추세를 시간 경과에 따라 확인합니다. 꺾은선형 차트와 함께 사용하면 추세를 시각화할 수 있습니다.
5. **보고서 콘텐츠**에서 **Segments** 드롭다운을 열고 보고서에 추가할 Segment를 선택합니다.
6. **열** > **측정기준 사용자 지정**에서 측정기준을 선택한 다음, **보고서 콘텐츠**에서 날짜 범위를 설정합니다.
...
4. **열** > **측정기준 사용자 지정**에서 측정기준을 선택한 다음, **보고서 콘텐츠**에서 날짜 범위를 설정합니다.
...
| 전환 수(전환 A–D) 및 전환 이벤트 이름 | Segment와 Campaign 또는 Canvas 차원이 함께 표시될 때 사용 가능합니다. **Segments**를 행에 배치하고 **Campaigns 및 Canvases** 드릴다운을 사용하거나, **Campaigns**, **Canvases** 또는 **Campaigns 및 Canvases**를 행에 배치하고 **Segment** 드릴다운을 사용합니다. |
| 매출 및 전환율 | Segment 차원 보고서에서는 사용할 수 없습니다. |
| Segment 구매 매출 및 수 | **Segments**가 행에 있을 때만 사용 가능합니다. |
{: .reset-td-br-1 .reset-td-br-2 aria-label="Segment 보고 측정기준 가용성"}

행 및 드릴다운 선택이 측정기준에 미치는 영향에 대한 자세한 내용은 보고서 빌더의 [측정기준 가용성]({{site.baseurl}}/user_guide/analytics/reports/report_builder#metrics-availability)을 참조하세요.