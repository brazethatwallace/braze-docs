...
CSV 파일을 사용한 사용자 데이터 가져오기는 이름, 이메일과 같은 사용자 속성과 신발 사이즈와 같은 커스텀 속성의 기록 및 업데이트를 지원합니다. 두 가지 고유 사용자 식별자 중 하나를 지정하여 CSV를 가져올 수 있습니다: `external_id` 또는 사용자 별칭.

{% alert important %}
사용자 가져오기는 사용자 커스텀 이벤트의 기록 및 업데이트도 지원합니다. 사용자 속성과 마찬가지로 `external_id`, `braze_id` 또는 `user_alias_name`과 `user_alias_label`을 사용하여 가져올 수 있습니다. 자세한 내용은 [커스텀 이벤트 가져오기](#importing-custom-events)를 참조하세요.
{% endalert %}

{% alert note %}
`external_id`가 있는 사용자와 없는 사용자를 함께 업로드하는 경우, 각 가져오기에 대해 별도의 CSV 파일을 만들어야 합니다. 하나의 CSV 파일에 `external_ids`와 사용자 별칭을 동시에 포함할 수 없습니다.
{% endalert %}

### 외부 ID로 가져오기 {#importing-with-external-id}

고객 데이터를 가져올 때 각 고객의 고유 식별자(일명 `external_id`)를 지정해야 합니다. CSV 가져오기를 시작하기 전에 엔지니어링 팀으로부터 Braze에서 사용자가 어떻게 식별되는지 이해하는 것이 중요합니다. 일반적으로 이는 내부 데이터베이스 ID입니다. 이는 모바일 및 웹에서 Braze SDK가 사용자를 식별하는 방식과 일치해야 하며, 각 고객이 모든 기기에서 Braze 내에 단일 사용자 프로필을 갖도록 설계되었습니다. Braze [사용자 프로필 수명주기]({{site.baseurl}}/user_guide/data/unification/user_data/user_profile_lifecycle)에 대해 자세히 알아보세요.

가져오기에서 `external_id`를 제공하면 Braze는 동일한 `external_id`를 가진 기존 사용자를 업데이트하거나, 해당 항목이 없으면 해당 `external_id`가 설정된 새로 식별된 사용자를 생성합니다.
...
`external_id`가 없는 사용자를 타겟팅하려면 사용자 별칭이 포함된 사용자 목록을 가져올 수 있습니다. 별칭은 대체 고유 사용자 식별자 역할을 하며, 가입하지 않았거나 앱에 계정을 만들지 않은 익명 사용자에게 마케팅하려는 경우 유용할 수 있습니다.
...
세분화 내 CSV 내보내기 옵션을 통해 Braze에서 사용자 데이터를 내보낸 후 기존 사용자에게 새 커스텀 속성을 추가하려는 경우 유용할 수 있습니다.

{% alert important %}
`braze_id`를 사용하여 CSV 가져오기로 새 사용자를 생성할 수 없습니다. 이 방법은 Braze 플랫폼 내의 기존 사용자를 업데이트하는 데에만 사용할 수 있습니다.
{% endalert %}
...
사용자의 기본 속성을 가져오려면 **사용자 가져오기** > **속성**으로 이동하세요. 기본 사용자 속성은 Braze의 예약된 키입니다. 예를 들어 `first_name` 또는 `email`이 있습니다. 커스텀 속성은 비즈니스에 맞춤화됩니다. 예를 들어, 여행 예약 앱에는 `last_destination_searched`라는 커스텀 속성이 있을 수 있습니다.

{% alert important %}
고객 데이터를 속성으로 가져올 때 사용하는 열 헤더는 기본 사용자 속성의 철자 및 대소문자와 정확히 일치해야 합니다. 그렇지 않으면 Braze가 해당 사용자 프로필에 커스텀 속성을 자동으로 생성합니다.
{% endalert %}
...
|---|---|---|---|
| `external_id` | 문자열 | 고객의 고유 사용자 식별자. | 예, [다음 참고 사항](#about-external-ids)을 확인하세요. |
| `user_alias_name` | 문자열 | 익명 사용자의 고유 사용자 식별자. `external_id`의 대안. | 아니요, [다음 참고 사항](#about-external-ids)을 확인하세요. |
| `user_alias_label` | 문자열 | 사용자 별칭을 그룹화하기 위한 공통 레이블. | 예, `user_alias_name`을 사용하는 경우. |
| `first_name` | 문자열 | 사용자가 표시한 이름(예: `Jane`). | 아니요 |
| `last_name` | 문자열 | 사용자가 표시한 성(예: `Doe`). | 아니요 |
| `email` | 문자열 | 사용자가 표시한 이메일(예: `jane.doe@braze.com`). | 아니요 |
| `country` | 문자열 | 국가 코드는 ISO-3166-1 alpha-2 표준으로 Braze에 전달되어야 합니다(예: `GB`). | 아니요 |
| `dob` | 문자열 | "YYYY-MM-DD" 형식으로 전달되어야 합니다(예: `1980-12-21`). 이를 통해 사용자의 생년월일을 가져오고 "오늘" 생일인 사용자를 타겟팅할 수 있습니다. | 아니요 |
| `gender` | 문자열 | "M", "F", "O"(기타), "N"(해당 없음), "P"(밝히고 싶지 않음) 또는 nil(알 수 없음). | 아니요 |
| `home_city` | 문자열 | 사용자가 표시한 거주 도시(예: `London`). | 아니요 |
| `language` | 문자열 | 언어는 ISO-639-1 표준으로 Braze에 전달되어야 합니다(예: `en`). <br>[지원되는 언어 목록]({{site.baseurl}}/user_guide/data/unification/user_data/language_codes)을 참조하세요. | 아니요 |
| `phone` | 문자열 | 사용자가 표시한 전화번호, `E.164` 형식(예: `+442071838750`). <br>형식 지침은 [사용자 전화번호]({{site.baseurl}}/user_guide/channels/sms_mms_and_rcs/message_setup/user_phone_numbers)를 참조하세요. | 아니요 |
| `email_open_tracking_disabled` | Boolean | true 또는 false를 사용할 수 있습니다. true로 설정하면 이 사용자에게 향후 전송되는 모든 이메일에 오픈 추적 픽셀이 추가되지 않습니다. | 아니요 |
| `email_click_tracking_disabled` | Boolean | true 또는 false를 사용할 수 있습니다. true로 설정하면 이 사용자에게 향후 전송되는 이메일의 모든 링크에 대한 클릭 추적이 비활성화됩니다. | 아니요 |
| `email_subscribe` | 문자열 | 사용 가능한 값은 `opted_in`(이메일 메시지 수신을 명시적으로 등록), `unsubscribed`(이메일 메시지 수신을 명시적으로 거부), `subscribed`(동의도 거부도 하지 않음)입니다. | 아니요 |
| `push_subscribe` | 문자열 | 사용 가능한 값은 `opted_in`(푸시 메시지 수신을 명시적으로 등록), `unsubscribed`(푸시 메시지 수신을 명시적으로 거부), `subscribed`(동의도 거부도 하지 않음)입니다. | 아니요 |
| `time_zone` | 문자열 | 시간대는 IANA 시간대 데이터베이스와 동일한 형식으로 Braze에 전달되어야 합니다(예: `America/New_York` 또는 `Eastern Time (US & Canada)`). | 아니요 |
...
| `subscription_state` | 문자열 | `subscription_group_id`로 지정된 구독 그룹의 구독 상태. 허용되는 값은 `unsubscribed`(구독 그룹에 없음) 또는 `subscribed`(구독 그룹에 있음)입니다. | 아니요, `subscription_group_id`를 사용하는 경우 강력히 권장됩니다. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3  .reset-td-br-4 role="presentation" }
...
**사용자 가져오기** > **속성**으로 이동하여 사용자의 커스텀 속성을 가져올 수 있습니다. 기본 속성과 정확히 일치하지 않는 모든 헤더는 Braze 내에서 커스텀 속성을 생성합니다.

사용자 가져오기에서 지원되는 데이터 유형은 다음과 같습니다:

| 데이터 유형 | 설명 |
|-----------|-------------|
| Datetime | ISO-8601 형식으로 저장되어야 합니다 |
| Boolean | TRUE 또는 FALSE |
| Number | 공백이나 쉼표 없는 정수 또는 플로트, 플로트는 소수점 구분자로 마침표(.)를 사용해야 합니다 |
| 문자열 | 열 값을 큰따옴표로 감싸면 쉼표를 포함할 수 있습니다 |
| 공백 | 공백 값은 사용자 프로필의 기존 값을 덮어쓰지 않으며, CSV 파일에 기존 사용자 속성을 모두 포함할 필요가 없습니다 |
{: .reset-td-br-1 .reset-td-br-2 role="presentation" }

{% alert important %}
배열과 푸시 토큰은 사용자 가져오기에서 지원되지 않습니다. 특히 배열의 경우, CSV 파일의 쉼표가 열 구분자로 해석되므로 값에 포함된 쉼표가 파일 파싱 오류를 유발합니다. <br>이러한 종류의 값을 업로드하려면 [`/users/track` 엔드포인트]({{site.baseurl}}/api/endpoints/user_data/post_user_track) 또는 [클라우드 데이터 수집]({{site.baseurl}}/user_guide/data/unification/cloud_ingestion)을 사용하세요.
{% endalert %}
...
{% alert important %}
사용자 가져오기에서 행당 하나의 `subscription_group_id`만 설정할 수 있습니다. 다른 행에는 다른 `subscription_group_id` 값을 사용할 수 있습니다. 그러나 동일한 사용자를 여러 구독 그룹에 등록해야 하는 경우 여러 번 가져오기를 수행해야 합니다.
{% endalert %}

### 커스텀 이벤트 가져오기(얼리 액세스) {#importing-custom-events}

{% alert important %}
커스텀 이벤트 가져오기는 현재 얼리 액세스 중입니다. 얼리 액세스에 참여하고 싶으시면 Braze 계정 매니저에게 문의하세요.
{% endalert %}

사용자의 커스텀 이벤트를 가져오려면 **사용자 가져오기** > **이벤트**로 이동하세요.

커스텀 이벤트는 비즈니스에 맞춤화됩니다. 예를 들어, 스트리밍 앱에는 rented_movie라는 커스텀 이벤트가 있을 수 있습니다. CSV에는 다음에 대한 열 헤더가 있어야 합니다:
...
커스텀 이벤트에는 이벤트 속성정보가 포함될 수 있습니다. 예를 들어, 커스텀 이벤트 rented_movie에는 title과 genre라는 속성정보가 있을 수 있습니다. 이러한 이벤트 속성정보의 열 헤더는 `<event_name>.properties.<property name>` 형식이어야 합니다. 예를 들어 `rented_movie.properties.title`입니다.

| 사용자 프로필 필드 | 데이터 유형 | 정보 | 필수 |
|-----------------------------------------|-----------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------|
| `external_id` | 문자열 | 사용자의 고유 사용자 식별자. | 예, `external_id`, `braze_id`, 또는 `user_alias_name`과 `user_alias_label` 중 하나가 필요합니다. |
| `braze_id` | 문자열 | 사용자에게 할당된 Braze 식별자. | 예, `external_id`, `braze_id`, 또는 `user_alias_name`과 `user_alias_label` 중 하나가 필요합니다. |
...
| `<event name>.properties.<property name>` | 다양 | 커스텀 이벤트와 연결된 이벤트 속성정보. 예를 들어 `rented_movie.properties.title` | 아니요 |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3  .reset-td-br-4 role="presentation" }
...
CSV를 통해 가져온 각 고객 데이터는 사용자 프로필의 기존 값을 덮어쓰며, 외부 ID와 공백 값을 제외하고 데이터 포인트로 계산됩니다.

- CSV 가져오기를 통해 업로드된 외부 ID는 데이터 포인트를 소모하지 않습니다. 외부 ID만 업로드하여 기존 Braze 사용자를 세분화하기 위한 CSV 파일을 업로드하는 경우, 데이터 포인트를 소모하지 않고 수행할 수 있습니다. 가져오기에 사용자의 이메일이나 전화번호와 같은 추가 데이터를 포함하면, 기존 사용자 데이터를 덮어쓰고 데이터 포인트를 소모합니다.
    - 세분화 목적의 CSV 가져오기(`external_id`, `braze_id`, 또는 `user_alias_name`만 유일한 필드로 하여 수행한 가져오기)는 데이터 포인트를 소모하지 않습니다.
- 공백 값은 사용자 프로필의 기존 값을 덮어쓰지 않으며, CSV 파일에 기존 사용자 속성이나 커스텀 이벤트를 모두 포함할 필요가 없습니다.
- `email_subscribe`, `push_subscribe`, `subscription_group_id`, 또는 `subscription_state`를 업데이트해도 데이터 포인트 소모에 포함되지 않습니다.

{% alert important %}
CSV 가져오기 또는 API를 통해 사용자의 언어나 국가를 설정하면 Braze가 SDK를 통해 이 정보를 자동으로 캡처하지 못하게 됩니다.
{% endalert %}
...
2. **파일 찾아보기**를 선택하고 원하는 파일을 선택한 다음 **가져오기 시작**을 선택합니다. Braze에서 파일을 업로드하고 각 열의 열 머리글과 데이터 유형을 확인합니다.

{% alert important %}
CSV 가져오기는 대소문자를 구분합니다. 즉, CSV 가져오기에서 대문자를 사용하면 해당 필드가 기본 속성이 아닌 커스텀 속성으로 기록됩니다. 예를 들어 "email"은 올바른 표기이지만, "Email"은 커스텀 속성으로 기록됩니다.
{% endalert %}

![가져올 사용자 정보 유형으로 '이벤트' 옵션이 선택되어 있습니다.][5]

업로드가 완료되면 파일 내용의 미리보기를 확인할 수 있습니다. 표에 표시되는 정보는 CSV 파일의 상위 행에 있는 값을 기반으로 합니다.

**사용자 가져오기** 페이지에서 진행 상황을 추적할 수 있으며, 이 페이지는 5초마다 새로고침되거나 **테이블 새로고침**을 선택하면 업데이트됩니다. 가져오기 중에도 Braze 대시보드의 나머지 기능을 계속 사용할 수 있으며, 가져오기가 시작되고 끝나면 알림을 받게 됩니다.

또한 최근 가져오기, 파일 이름, CSV 유형, 파일의 행 수, 성공적으로 가져온 행 수, 각 파일의 총 행 수 및 각 가져오기의 상태를 확인할 수 있습니다.

한 번에 여러 CSV 파일을 가져올 수 있습니다. CSV 가져오기는 동시에 실행되므로 업데이트 순서가 순차적으로 보장되지 않습니다. CSV 가져오기를 순차적으로 실행해야 하는 경우, 두 번째 파일을 업로드하기 전에 첫 번째 CSV 가져오기가 완료될 때까지 기다려야 합니다.

가져오기 프로세스 중 오류가 발생하면 파일의 총 행 수 옆에 경고 아이콘이 나타납니다. 아이콘 위에 마우스를 올리면 특정 행이 실패한 이유에 대한 세부 정보를 확인할 수 있습니다. 가져오기가 완료되면 모든 데이터가 기존 프로필에 추가되거나 새 프로필이 생성됩니다.

![단일 열에 혼합된 데이터 유형으로 인해 오류가 발생한 CSV 파일 업로드 완료 화면][4]{: style="max-width:70%"}

### 고려 사항 {#considerations}

업로드 중 Braze에서 파일 상위 행에 형식 오류가 감지되면 이러한 오류가 요약과 함께 표시됩니다. 예를 들어 파일에 형식이 잘못된 행이 포함되어 있으면, 파일을 가져올 때 미리보기에서 이 오류가 표시됩니다. 오류가 있는 파일도 가져올 수는 있지만, 가져오기를 계속하기 전에 파일의 오류를 수정하는 것이 좋습니다.

또한 Braze에서 미리보기 생성 시 입력 파일의 모든 행을 스캔하지는 않으므로, 업로드 전에 전체 CSV 파일을 확인하는 것이 중요합니다. 이는 Braze에서 미리보기를 생성하는 동안 포착하지 못하는 오류가 존재할 수 있음을 의미합니다.

형식이 잘못된 행과 외부 ID가 없는 행은 가져오지 않습니다. 다른 모든 오류는 가져올 수 있지만, Segment를 생성할 때 필터링에 영향을 줄 수 있습니다. 자세한 내용은 [문제 해결](#troubleshooting) 섹션을 참조하세요.

{% alert warning %}
오류는 데이터 유형과 파일 구조만을 기반으로 합니다. 예를 들어 형식이 잘못된 이메일 주소도 문자열로 파싱할 수 있기 때문에 가져오기가 가능합니다.
{% endalert %}

### Lambda 사용자 CSV 가져오기 {#lambda-user-csv-import}

서버리스 S3 Lambda CSV 가져오기 스크립트를 사용하여 사용자 속성을 플랫폼에 업로드할 수 있습니다. 이 솔루션은 CSV 업로더 역할을 하며, S3 버킷에 CSV를 넣으면 스크립트가 API를 통해 업로드합니다.

백만 행이 포함된 파일의 예상 실행 시간은 약 5분입니다. 자세한 내용은 [사용자 속성 CSV를 Braze로 가져오기]({{site.baseurl}}/user_guide/data/unification/cloud_ingestion)를 참조하세요.

## 세분화 {#segmenting}

사용자 가져오기는 사용자 프로필을 생성 및 업데이트하며, Segments를 만드는 데도 사용할 수 있습니다. Segment를 만들려면 가져오기를 시작하기 전에 **이 CSV에서 가져오는 사용자로 Segment를 자동 생성**을 선택합니다.

Segment 이름을 설정하거나, 파일 이름인 기본값을 그대로 사용할 수 있습니다. Segment를 만드는 데 사용된 파일에는 가져오기가 완료된 후 해당 Segment를 볼 수 있는 링크가 표시됩니다.

Segment를 만드는 데 사용되는 필터는 선택한 가져오기에서 생성되거나 업데이트된 사용자를 선택하며, Segment 편집 페이지에서 다른 모든 필터와 함께 사용할 수 있습니다.
