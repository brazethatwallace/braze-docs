...
> 이 문서에서는 시간 경과에 따른 집계된 앱 제거를 확인하여 추세와 이상 징후를 파악하고, Campaign 수준의 제거를 추적하여 특정 캠페인이 앱 설치를 유도하는지 또는 방해하는지 확인하는 방법을 설명합니다.
...
Braze는 정기적인 푸시 Campaign에서 기본 수준의 제거 정보를 자동으로 수집합니다. 그러나 사용자마다 푸시 Campaign을 수신하는 빈도가 다를 수 있으므로, 제거 추적 기능을 제공하여 사용자들의 제거 활동에 대한 보다 정확한 스냅샷을 제공합니다.
...
## 제거 추적 켜기 {#turning-on-uninstall-tracking}
...
3. **Breakdown Values** 드롭다운에서 그래프에 포함할 Segment를 선택합니다.

{% alert note %}
제거 추적이 활성화되지 않은 앱은 일부 사용자(푸시 알림 타겟으로 지정된 사용자)의 제거만 보고하므로, 일일 제거 총계가 표시된 것보다 높을 수 있습니다.
{% endalert %}

## Campaign의 제거 추적 {#uninstall-tracking-for-campaigns}

Campaign 제거 추적은 특정 Campaign을 수신한 후 선택한 기간 내에 앱을 제거한 사용자 수를 표시합니다. 이 도구는 Campaign이 의도하지 않은 부정적인 사용자 행동을 유발할 수 있는지에 대한 인사이트를 제공하고, 전반적인 Campaign 효과를 측정하는 데 도움이 됩니다.

Campaign의 제거 통계는 특정 Campaign의 **Campaign Analytics** 페이지에 있습니다. 멀티채널 및 다변량 Campaign의 경우, 제거를 각각 채널 및 배리언트별로 세분화할 수 있습니다.
...
* 지난 24시간 동안 사용자가 성공적으로 수신한 모든 Campaign의 제거 횟수를 각각 1 증가시킵니다.
* 사용자가 24시간 동안 세 개의 Campaign을 수신한 후 제거하면, 세 개의 Campaign 모두에 대한 "제거" 횟수가 증가합니다.
...
제거 급증이 발생한 시점과 비슷한 시기에 메시지를 보낸 Campaign의 분석을 확인하세요. 특정 메시지가 제거 증가와 상관관계가 있다면, 해당 메시지가 사용자의 제거에 영향을 미치고 있을 수 있습니다.
...
APNs의 경우, 고객 프로필에서 `BadDeviceToken` 푸시 오류를 확인하세요. 제거 급증과 같은 시간대에 이 오류가 대량으로 발생하면, 제거가 실제일 가능성이 높습니다. `BadDeviceToken`은 기기의 푸시 토큰이 더 이상 유효하지 않음을 나타내며, 이는 일반적으로 앱이 제거되었을 때 발생합니다.
