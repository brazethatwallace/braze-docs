...
article_title: 쿼리 빌더
page_order: 4
description: "이 참조 문서에서는 쿼리 빌더에서 Snowflake의 Braze 데이터를 사용하여 보고서를 작성하는 방법을 설명합니다."
tool: Reports
alias: /query_builder/
---
...
3. 보고서에는 현재 날짜와 시간이 자동으로 이름으로 지정됩니다. 이름 위에 마우스를 올리고 <i class="fas fa-pencil" alt="Edit"></i>을 선택하여 SQL 쿼리에 의미 있는 이름을 지정합니다.
4. 편집기에서 SQL 쿼리를 작성하거나 **AI Query Builder** 탭에서 [AI의 도움을 받습니다](#ai-query-builder). 직접 SQL을 작성하는 경우 요구 사항과 리소스에 대해 [커스텀 SQL 쿼리 작성하기](#custom-sql)를 참조하세요.
5. **Run Query**를 선택합니다.
6. 쿼리를 저장합니다.
7. 보고서의 CSV를 다운로드하려면 **Export**를 선택합니다.
...
보고서를 처음 생성할 때 **Create SQL Query** > **Query Template**을 선택하여 쿼리 템플릿에 액세스합니다.
...
Snowflake 데이터베이스를 쿼리하는 기본 시간대는 UTC입니다. 따라서 **Email Channel Engagement** 페이지(회사의 시간대를 따름)와 쿼리 빌더 결과 사이에 일부 데이터 불일치가 있을 수 있습니다.
...
쿼리 빌더의 **Query history** 섹션에는 이전에 실행한 쿼리가 표시되어 작업을 추적하고 재사용하는 데 도움이 됩니다. 쿼리 기록은 7일간 보존되며, 7일이 지난 쿼리는 자동으로 제거됩니다.
...
1. 쿼리 빌더에서 보고서를 생성한 후 **AI Query Builder** 탭을 선택합니다.
2. 프롬프트를 입력하거나 샘플 프롬프트를 선택하고 **Generate**를 선택하여 프롬프트를 SQL로 변환합니다.
3. 생성된 SQL이 올바른지 검토한 다음 **Insert into Editor**를 선택합니다.
...
1. **Query Builder** 페이지에서 **Reference** 패널을 열고 **Available Data Tables**를 선택하여 사용 가능한 데이터 테이블과 이름을 확인합니다.
3. <i class="fas fa-chevron-down" alt=""></i> **See Details**를 선택하여 테이블 설명과 데이터 유형 등 테이블 열에 대한 정보를 확인합니다.
4. SQL에 테이블 이름을 삽입하려면 <i class="fas fa-copy" title="Copy table name to SQL editor"></i>를 선택합니다.

Braze에서 제공하는 사전 작성된 쿼리를 사용하려면 쿼리 빌더에서 보고서를 처음 생성할 때 **Query Template**을 선택합니다.
...
변수를 사용하면 SQL에서 사전 정의된 변수 유형을 사용하여 값을 수동으로 복사할 필요 없이 값을 참조할 수 있습니다. 예를 들어, Campaign의 ID를 SQL 편집기에 수동으로 복사하는 대신 {% raw %}`{{campaign.${My campaign}}}`{% endraw %}을 사용하여 **Variables** 탭의 드롭다운에서 직접 Campaign을 선택할 수 있습니다.

변수가 생성되면 쿼리 빌더 보고서의 **Variables** 탭에 표시됩니다. SQL 변수를 사용하면 다음과 같은 이점이 있습니다:
...
하나의 Canvas를 선택하기 위한 변수입니다. Campaign과 동일한 이름을 공유하면 **Variables** 탭에 Canvas 또는 Campaign 중 하나를 선택하는 라디오 버튼이 표시됩니다.
...
여러 Canvases를 선택하기 위한 변수입니다. Campaign과 동일한 이름을 공유하면 **Variables** 탭에 Canvas 또는 Campaign 중 하나를 선택하는 라디오 버튼이 표시됩니다.
...
하나의 Campaign을 선택하기 위한 변수입니다. Canvas와 동일한 이름을 공유하면 **Variables** 탭에 Canvas 또는 Campaign 중 하나를 선택하는 라디오 버튼이 표시됩니다.
...
여러 Campaigns를 다중 선택하기 위한 변수입니다. Canvas와 동일한 이름을 공유하면 **Variables** 탭에 Canvas 또는 Campaign 중 하나를 선택하는 라디오 버튼이 표시됩니다.
