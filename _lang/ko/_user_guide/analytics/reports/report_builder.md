...
3. **행** 드롭다운에서 보고할 항목을 선택합니다:
    ...
    **행** 선택에 따라 [확인할 수 있는 측정기준](#metrics-availability)이 달라집니다. 예를 들어, 다변량 측정기준은 **Canvases** 또는 **배리언트** 드릴다운이 포함된 **Campaigns**에 대해 보고할 때만 확인할 수 있습니다. **Campaigns 및 Canvases**에 대해 보고할 때는 해당 Campaigns와 Canvases에 다변량 테스트가 있더라도 이러한 측정기준을 확인할 수 없습니다.
...
4. (선택 사항) **드릴다운 추가**를 선택하여 데이터를 더 세분화된 뷰로 분류합니다:
    - 채널
    - 날짜
        - 데이터를 더 작은 시간 범위로 분할할 때 사용합니다. 예를 들어, Campaigns의 일별 성과를 확인하려면 다음과 같이 구성합니다:
            - **행**: Campaigns
            - **그룹화:** 날짜
            - **간격:** 일
    - 배리언트
...
8. 그런 다음, 3단계에서의 선택에 따라 Campaigns, Canvases 또는 둘 다를 수동 또는 자동으로 보고서에 추가합니다.
    ...
    - **자동 추가:** 보고서에 포함할 Campaigns 또는 Canvases에 대한 규칙을 설정합니다. 이 페이지에서 하나의 필드만 선택하면 됩니다.
        - 이 화면에서 설정한 조건을 충족하는 추가 Campaigns 또는 Canvases가 있으면 향후 보고서 실행 시 자동으로 추가됩니다.
        - **채널** 드롭다운에 배너 옵션이 없으므로, 채널 규칙을 사용하여 배너 Campaigns 또는 Canvases를 자동으로 추가할 수 없습니다. 그래도 보고서 측정기준에 배너 핵심 성과 지표(KPI)를 포함할 수 있습니다.<br><br>![보고서에 추가할 Campaign 및 Canvas 규칙을 설정할 수 있는 필드가 있는 "자동으로 Campaign 및 Canvas 추가" 섹션.]({% image_buster /assets/img/report_builder_2/automatically_add.png %}){: style="width:90%;"}<br><br>
9. **저장 및 실행**을 선택하여 보고서를 실행합니다.

{% alert note %}
구성 단계에서 선택한 날짜 범위와 Campaigns 또는 Canvases의 수에 따라 보고서 실행에 최대 몇 분이 소요될 수 있습니다.
{% endalert %}
...
{% alert tip %}
캔버스 배리언트 또는 단계에 대해 보고하려면 행에 **Canvases**를 선택하고, 드릴다운 필드를 비워 두거나 **Date**를 선택하세요. 이렇게 하면 **Canvas View** 드롭다운이 생성되어 Canvas 전체의 측정기준을 확인하거나, 배리언트, 단계 또는 메시지별로 측정기준을 그룹화할 수 있습니다.<br><br> 단계별로 그룹화하면 보고서 구성 중 미리보기 테이블에 최대 50개 행이 표시됩니다. 모든 행을 보려면 보고서를 실행하거나 CSV로 내보내세요.
...
| 제목란 | 이메일 Campaigns(**배리언트** 드릴다운 포함), Canvases, Canvases(**배리언트** 드릴다운 포함)에서 사용할 수 있습니다. |
...
보고서를 실행한 후 보고서 페이지에서 테이블 형식으로 결과를 확인할 수 있습니다.
...
{% alert note %}
꺾은선형 차트를 생성하려면 보고서를 구성할 때 드릴다운 옵션으로 **날짜**를 선택하세요. 이렇게 하면 시간에 따른 추세가 표시됩니다.
{% endalert %}
...
2. **대시보드에 추가**를 선택합니다.
3. 새 대시보드를 생성할지 기존 대시보드에 추가할지 선택합니다.<br><br>![새 대시보드 또는 기존 대시보드에 보고서를 추가할지 선택하는 옵션이 있는 창]({% image_buster /assets/img/report_builder_2/add_to_dashboard.png %}){: style="width:90%;"}<br><br>
...
Campaign 또는 Canvas는 **마지막 발송** 날짜가 설정한 **마지막 발송** 기간 내에 해당할 때 보고서에 표시됩니다. **발송** 및 기타 측정기준은 **데이터 표시 기간** 날짜 범위 내의 활동에 대해서만 채워집니다. 메시지가 **데이터 표시 기간** 동안 발송되지 않은 경우에도 해당 행에는 발송 건수가 0인 상태로 Campaign 또는 Canvas가 표시될 수 있습니다.

예를 들어, **마지막 발송**이 2025년 1월 1일~2025년 4월 14일로 설정되어 Campaign이 포함되었지만, **데이터 표시 기간**이 2024년 12월 1일~2025년 1월 14일인 경우를 가정해 보겠습니다. 해당 Campaign이 12월이나 1월에 발송 건수가 없었다면, 발송 측정기준 없이 테이블에 그대로 표시됩니다.
...
**파트너 통합**에 [Amazon S3 버킷]({{site.baseurl}}/partners/data_and_infrastructure_agility/cloud_storage/amazon_s3)이 연결되어 있는 경우, S3 버킷을 직접 탐색하여 이전 보고서의 데이터를 가져올 수 있습니다.