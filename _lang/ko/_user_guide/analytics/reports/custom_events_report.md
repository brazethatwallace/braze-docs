...
> 사용자 지정 이벤트 보고서를 사용하면 하나 이상의 커스텀 이벤트가 시간에 따라 발생한 추이를 확인할 수 있습니다. Segment별로 결과를 분류하고, KPI or 핵심 성과 지표(KPI) 수식을 적용하며, 추가 분석을 위해 데이터를 내보낼 수 있습니다.
...
| KPI or 핵심 성과 지표(KPI) or KPI or 핵심 성과 지표(KPI) or KPI or 핵심 성과 지표(KPI) or KPI or 핵심 성과 지표(KPI) or 핵심 성과 지표(KPI or 핵심 성과 지표(KPI)) 공식 | 원시 이벤트 수를 분자(예: 커스텀 이벤트 수)와 분모(예: 일일 활성 사용자, MAU 또는 분석이 활성화된 Segment 크기)로 구성된 계산 지표로 대체합니다. 하나 이상의 공식을 선택하면 차트에 선택한 날짜 범위에 걸쳐 각 공식의 값이 표시되므로, 총 이벤트 볼륨 대신 정규화된 성능(예: "활성 사용자당 이벤트 수")을 비교할 수 있습니다. 선택한 시간 범위와 공식에 대해 사용 가능한 데이터가 없는 경우, Braze는 "데이터 없음" 메시지를 표시합니다. 시간 범위를 넓히거나 다른 공식을 선택하세요. **Manage KPI or 핵심 성과 지표(KPI) formulas**를 선택하여 공식을 생성하거나 편집할 수 있습니다. |
