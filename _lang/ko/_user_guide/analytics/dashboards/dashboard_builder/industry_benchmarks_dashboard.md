...
| 섹션 | 설명 |
|----------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------|
| 핵심 성과 지표(KPI) 카드 | 각 주요 측정기준에 대한 워크스페이스의 비율과 업종 비율 대비 차이를 표시합니다. 녹색 위쪽 화살표는 워크스페이스가 업종 비율보다 높음을, 빨간색 아래쪽 화살표는 낮음을 나타냅니다. |
| 월별 추세 차트 | 시간 경과에 따른 워크스페이스 비율과 업종 비율을 비교하여 계절성 및 장기 추세를 파악할 수 있습니다. |
| 지역별 분석 | 지역별로 워크스페이스 비율과 업종 비율을 비교하여 지역 성능이 업종과 어떻게 다른지 확인할 수 있습니다. |
{: .reset-td-br-1 .reset-td-br-2 aria-label="섹션" }

모든 차트에서 밝은 색상의 계열은 업종 벤치마크를, 어두운 색상의 계열(**Workspace** 접두사 포함)은 자체 성능을 나타냅니다.
...
| 측정기준 유형 | 설명 | 예시 |
|----------|---------------------------------------|------------------------------------------------------|
| _Total_ | 모든 인게이지먼트 이벤트를 집계합니다. | 사용자가 세 번 클릭하면 세 번의 클릭으로 집계됩니다. |
...
모든 데이터 포인트를 분석하는 대신, Braze는 대표성 있는 샘플을 선택합니다. 샘플링 방법은 적절한 대표성을 위해 소규모 사용자 그룹을 과대 샘플링하고, 소수의 대규모 기업이 전체 업종의 결과를 왜곡하지 않도록 기업 규모에 따라 조정합니다.

### 2단계: 이상값 제거 {#step-2-outlier-removal}

Braze는 통계적 이상값을 식별하고 제거합니다. 이를 통해 평균 성능 비율에 미치는 영향을 최소화하면서 데이터의 변동성을 크게 줄여, 기본 추세를 변경하지 않으면서 이상값을 제거합니다.
