...
- **총 중단 수:** 중단된 메시지의 총 수입니다. 여기에는 Canvas에 진입하지 않았거나, 단계 실패를 경험했거나, 종료 이벤트를 수행하면서 종료 기준을 충족하여 Canvas를 종료한 Canvas 오디언스 멤버가 포함됩니다.
- **메시지 발송 수:** Braze가 성공적으로 처리하고 발송한 메시지의 총 수입니다.
...
이 시계열 차트는 메시지가 중단되었거나 사용자가 Canvas에서 제외된 다양한 이유를 일별로 분석하여 보여줍니다. 이 차트에는 발송 수가 표시되지 않습니다.
...
| 웹 푸시, 앱 또는 기기에 대해 사용자가 활성화되지 않음 | "Android 푸시, 앱 또는 기기에 대해 사용자가 활성화되지 않음" 중단 결과와 동일합니다. |
| Content Cards에 대해 사용자가 활성화되지 않음 | 사용자가 이 콘텐츠 카드를 포함하는 앱을 사용한 적이 없습니다. |
| 이메일에 대해 사용자가 활성화되지 않음 | 이 사용자에게 이메일을 발송할 수 없습니다. 일반적인 이유: {::nomarkdown}<ul><li> 고객 프로필에 이메일 주소가 없는 경우. </li><li> 사용자의 구독 상태로 인해 이 이메일 수신에서 제외된 경우. </li><li> 사용자의 이메일 주소가 이전에 유효하지 않은 것으로 표시된 경우(하드바운스). </li><li> 이 이메일 주소로 발송된 메시지가 지속적으로 스팸으로 표시되어 발송이 취소된 경우.</li></ul>{:/} |
...
- **다중 채널 메시지:** Campaign이나 캔버스 단계가 여러 채널(예: SMS 및 이메일)로 발송하도록 구성되었습니다. 한 명의 사용자가 한 채널(예: 이메일)에서는 "발송됨" 결과를, 다른 채널(예: "SMS/MMS/RCS에 대해 사용자가 활성화되지 않음")에서는 "중단" 결과를 받을 수 있습니다. 이 경우 해당 사용자는 차트에서 두 번 계산됩니다: 한 번은 "발송됨"으로, 한 번은 "중단"으로.
  - **예시:** 100명의 사용자에게 iOS와 Android를 모두 타겟팅하는 푸시 Campaign을 발송합니다. 사용자가 iOS 기기만 가지고 있는 경우, iOS 푸시를 수신("발송됨")하지만 Android 푸시에 대해서도 중단이 트리거됩니다("Android 푸시, 앱 또는 기기에 대해 사용자가 활성화되지 않음").
