...
## 보고서 설정 {#setting-up-your-report}

전환 대시보드 보고서를 설정하려면 다음을 수행합니다.

1. **Analytics** > **Conversions**로 이동합니다.
2. 보고서의 **Date Range**를 최대 90일 기간으로 선택합니다.
3. 분석할 Campaigns 또는 Canvases(또는 둘 다)를 선택합니다.
   - (선택 사항) 태그를 선택하여 Campaigns와 Canvases를 필터링합니다.
4. 메시지에 대해 분석할 **Channel(s)**을 선택합니다.
5. 배리언트, 캔버스 단계, 국가 또는 언어별 등 다양한 데이터 차원을 보려면 **Breakdown by** 레이어를 선택합니다.
...
{% alert note %}
여러 채널의 전환을 분석하는 경우 **Attribution Method**가 기본적으로 **Last-Touch Attribution**으로 설정됩니다.
{% endalert %}

{:start="8"}
8. **Create**를 선택하여 보고서를 실행합니다.

페이지가 로드된 후 **Conversion Event**를 선택하여 전환 데이터에 대한 보고서를 필터링합니다. 사용 가능한 선택 항목에는 Canvases와 Campaigns에서 사전 구성된 이벤트가 포함됩니다. 보고서 설정 시(6단계) 커스텀 이벤트를 선택한 경우 이 옵션은 사용할 수 없습니다.

### 커스텀 이벤트 사용 {#using-custom-events}
...
1. 보고서를 설정할 때 **Use custom events**를 켭니다.
2. 전환 이벤트로 사용할 커스텀 이벤트를 선택합니다.
3. 해당 이벤트가 전환으로 집계되기 위해 발생해야 하는 전환 기간을 선택합니다.

{% alert note %}
커스텀 이벤트를 선택하면 페이지에 **Conversion Event** 드롭다운이 표시되지 않으며, 다른 커스텀 이벤트에 대한 전환을 보려면 보고서를 다시 실행해야 합니다.
{% endalert %}
...
이 사용자는 날짜 범위가 10월 1일부터 10월 7일인 보고서에 **표시되지 않습니다**. 전환 이벤트가 정의된 날짜 범위 내에서 발생했더라도 사용자가 보고 기간 이전에 Canvas에 진입했기 때문입니다. 이 사용자가 보고서에 표시되려면 날짜 범위에 9월 30일이 포함되어야 합니다.
...
전환 세부 정보 테이블에는 항상 *수신자*와 *전환*(비율 및 합계) 열이 표시됩니다. 나머지 두 테이블 열은 보고서 설정 시 선택한 옵션에 따라 달라집니다.

![3열과 4열에 터치를 기여도 방법으로 표시하는 전환 세부 정보 테이블.]({% image_buster /assets/img_archive/conversions2_details.png %}){: style="border:none"}
...
{% alert note %}
이 그래프는 한 번에 하나의 채널에 대한 데이터만 표시합니다. 차트의 **Channel** 드롭다운을 사용하여 단일 채널을 선택하세요.
{% endalert %}

![이메일 전달, 이메일 열람, 이메일 클릭 및 전환에 대해 유사한 결과를 보여주는 두 이메일 Campaigns의 전환 퍼널 막대 그래프.]({% image_buster /assets/img_archive/conversions2_funnel.png %})
...
| --- | --- | --- | --- |
| 수신 시 | 메시지 수신 후 발생한 총 전환 수 | (고유 수신 전환) / (고유 수신자)로 계산 | {::nomarkdown}<ul><li>이메일 전달 시</li><li>SMS 전달 시</li></ul>{:/} |
| 발송 시 | 메시지 발송 후 발생한 총 전환 수 | (고유 발송 전환) / (고유 수신자)로 계산 | {::nomarkdown}<ul><li>푸시 발송 시</li><li>콘텐츠 카드 발송 시</li><li>SMS 발송 시</li></ul>{:/} |
| 열람 시 | 메시지 열람 후 발생한 총 전환 수 | (고유 열람 전환) / (고유 수신자)로 계산 | {::nomarkdown}<ul><li>이메일 열람 시</li><li>푸시 열람 시</li></ul>{:/} |
