...
SDK 초기화에 필요한 모든 세부 정보가 포함된 QR 코드를 스캔하면 고객 프로필이 생성되고, Braze Pilot 내 특정 앱 시뮬레이션으로 딥링크됩니다. 데모 QR 코드는 무료 체험판의 특정 데모 캠페인에 대한 컴패니언 드로어에 표시됩니다.
...
이 연결은 워크스페이스별로 적용됩니다. 즉, 데모 워크스페이스에서 연결을 초기화한 후 무료 체험판 대시보드의 라이브 워크스페이스로 전환하면, 해당 워크스페이스에서 실행된 캠페인을 수신하려면 SDK를 다시 초기화해야 합니다.
...
## 5단계: Pilot에서 Braze 메시징 체험하기

이제 Braze Pilot 사용자로서 Braze 대시보드에서 캠페인과 캔버스를 수신할 준비가 되었습니다! 데모 워크스페이스에서 실행된 캠페인을 방문하여 Braze 활용 사례에 대한 간단한 데모를 확인한 후, 라이브 워크스페이스로 이동하여 직접 캠페인을 보내기 시작하세요.

Braze에서 캠페인과 캔버스를 설정하는 방법에 대한 자세한 내용은 [시작하기: 캠페인 및 캔버스]({{site.baseurl}}/user_guide/get_started/campaigns_and_canvases/)를 참조하세요.