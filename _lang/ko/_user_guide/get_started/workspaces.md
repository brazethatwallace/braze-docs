...
- **별개의 활동 분리:** 워크스페이스는 서로 다른 브랜드나 활동을 분리할 수 있는 수단을 제공합니다. 예를 들어 사용자 기반이 다른 여러 개의 하위 브랜드가 있는 경우 각각에 대해 별도의 워크스페이스를 만드는 것이 좋습니다.

{% alert tip %}
이 접근 방식은 각 게임에 대해 개별 워크스페이스를 관리할 수 있는 모바일 게임 회사나, 운영하는 각 지역에 대해 별도의 워크스페이스를 원하는 이커머스 사이트와 같은 기업에 특히 유용합니다.
{% endalert %}

## 워크스페이스 계획 {#planning-workspaces}

각 플랫폼에서 앱의 각 버전에 대해 별도의 앱 인스턴스를 생성해야 합니다. 워크스페이스에 포함할 앱 인스턴스를 결정할 때는 타겟팅하려는 사용자를 생각하고 그에 따라 그룹화하세요.

하나의 워크스페이스에서 여러 앱 인스턴스를 사용할 수 있다는 점은 전체 앱 포트폴리오에 대한 메시징 사용량 제한을 설정할 수 있다는 점에서 매력적일 수 있습니다. 하지만 모범 사례로서, 하나의 워크스페이스에는 동일한(또는 매우 유사한) 앱의 서로 다른 버전만 함께 배치하는 것을 권장합니다.

### 공유 워크스페이스 {#shared-workspaces}

동일한 워크스페이스에 여러 앱 인스턴스를 포함하려는 일반적인 예:

- 여러 플랫폼에 걸쳐 거의 동일한 앱이 여러 개 있는 경우
- 앱의 주요 개정 버전이 다르지만 업그레이드할 때 동일한 사용자를 계속 참여시키려는 경우
...
- 이메일 구독(이는 올 오어 논 상황이 되어 규정 준수 문제에 노출될 수 있습니다.)
...
다른 경우에는 여러 개의 별도 워크스페이스를 사용하고 싶을 수도 있습니다. 일반적인 예는 다음과 같습니다:

- 동일한 앱의 개발 환경과 프로덕션 환경을 위한 별도의 워크스페이스
- 여러 하위 브랜드, 예를 들어 여러 게임을 제공하는 모바일 게임 회사
- 동일한 앱 또는 웹사이트를 다른 국가에서 운영하거나 다른 언어를 대상으로 하는 다른 현지화

### 중요 고려 사항 {#important-considerations}

워크스페이스는 별도의 데이터 사일로 역할을 한다는 점을 기억하세요. 사용자 데이터든 마케팅 자산이든 모든 데이터는 워크스페이스 내에 저장됩니다. 이 데이터는 해당 워크스페이스 외부에서 쉽게 공유할 수 없습니다.
...
예를 들어 동일한 앱의 개발 환경과 프로덕션 환경을 위한 별도의 워크스페이스가 있는 경우 프로덕션 워크스페이스에서 Currents 커넥터를 활성화하세요. 두 워크스페이스 모두에서 Currents를 사용하려면 추가 Currents 커넥터를 구매해야 합니다.
...
Segment, Campaign 또는 Canvas는 다른 워크스페이스에 저장된 데이터를 참조하거나 액세스할 수 없습니다. 반대로 여러 앱이 동일한 워크스페이스에 있는 경우 모든 앱의 데이터가 집계됩니다. 이는 [Braze의 필터에 영향을](#impact-on-segmentation-filters) 미칩니다.

### 각 접근 방식에 대한 개요 {#overview-of-each-approach}

다음 표에서는 워크스페이스 계획에 대한 이 두 가지 접근 방식의 장점과 단점을 설명합니다:

- **별도의 워크스페이스와 고객 프로필:** 하나의 워크스페이스에는 하나의 앱 인스턴스가 있고 한 사람은 해당 앱 인스턴스에 대해 하나의 고객 프로필을 갖습니다.
- **공유 워크스페이스와 고객 프로필:** 하나의 워크스페이스에 여러 개의 앱 인스턴스가 있고 한 사람이 모든 앱 인스턴스에 대해 하나의 고객 프로필을 갖습니다.
...
프로덕션 워크스페이스(실제 사용자에게 메시지를 보낼 워크스페이스)를 설정하려는 경우에는 테스트 워크스페이스도 함께 설정하는 것이 좋습니다. 테스트 워크스페이스는 실제 사용자 데이터가 없는 프로덕션 워크스페이스의 복제본입니다.
...
- **교육:** 새로운 팀원들은 실수가 실제적인 결과를 초래하지 않는 안전한 환경에서 워크스페이스에 익숙해질 수 있습니다.
