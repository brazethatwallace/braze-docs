...
Braze 대시보드에서 **템플릿** > **이메일 템플릿**으로 이동합니다.
...
개요 화면 하단에서 **미리보기 및 테스트**를 선택합니다. 여기에서 고객의 받은편지함에 이메일이 어떻게 표시되는지 미리 볼 수 있습니다. **사용자로 미리보기**를 선택하면 랜덤 사용자로 이메일을 미리 보거나, 특정 사용자를 선택하거나, 커스텀 사용자를 만들 수 있습니다. 이를 통해 연결된 콘텐츠와 개인화 호출이 제대로 작동하는지 테스트할 수 있습니다.

그런 다음 **미리보기 링크 복사**를 선택하여 랜덤 사용자에게 이메일이 어떻게 보이는지 보여주는 공유 가능한 미리보기 링크를 생성하고 복사할 수 있습니다. 이 링크는 7일 후에 다시 생성해야 합니다.
...
최종 확인이 준비되면 **테스트 발송**을 선택하고 본인이나 콘텐츠 테스터 그룹에게 테스트 메시지를 보내 다양한 기기와 이메일 클라이언트에서 이메일이 올바르게 표시되는지 확인합니다.

![테스트를 위해 발송할 예시 이메일 미리보기.]({% image_buster /assets/img_archive/newEmailTest.png %})

템플릿에 문제가 있거나 변경하고 싶은 사항이 있으면 **이메일 편집**을 선택하여 편집기로 돌아갑니다. **클래식** 편집기에서 수정한 내용은 HTML 편집기나 이메일 미리보기에 반영되지 않을 수 있습니다.

## 6단계: 템플릿 저장하기 {#step-6-save-your-template}

**템플릿 저장**을 선택하여 템플릿을 저장하세요. 이제 원하는 Campaign이나 Canvas 구성요소에서 이 템플릿을 사용할 수 있습니다. 템플릿에 액세스하려면 템플릿을 만든 편집 환경을 선택한 다음 사용 가능한 템플릿 목록에서 선택합니다.
...
![이메일 템플릿 댓글에 "Looks good to me"라고 적혀 있는 모습.]({% image_buster /assets/img/email_templates/template_comment.png %})
