...
> Braze 대시보드에서는 자체 HTML 이메일 템플릿을 업로드하고 나중에 캠페인에서 사용할 수 있도록 저장할 수 있습니다. 에디터를 사용하여 [이메일 템플릿을 생성]({{site.baseurl}}/user_guide/messaging/templates/email_templates/email_template/)할 수도 있습니다.
...
{% alert note %}
기존 템플릿을 편집하면 해당 템플릿의 이전 버전을 사용하여 생성된 Campaigns에는 변경 사항이 반영되지 않습니다.
{% endalert %}
...
Windows 컴퓨터에서 HTML Campaigns, 이메일 메시지가 포함된 캔버스 단계 또는 템플릿의 파일을 다운로드할 때 `|`(파이프 문자)가 지원되지 않으므로 ZIP 파일에서 다운로드 콘텐츠를 추출하려면 다른 애플리케이션을 사용해야 할 수 있습니다.
