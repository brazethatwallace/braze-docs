...
6. "A/B Test"라는 [실험 경로]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/experiment_step) 구성요소의 기본값을 유지합니다. 이 구성요소는 세션이 2회 미만인 사용자에게 두 가지 프로모션(다음 단계에서 커스터마이즈) 중 하나를 무작위로 발송합니다.
7. 실험 경로의 일부로 사용자에게 발송할 두 가지 프로모션을 커스터마이즈합니다. 이 사용 사례에서는 하나를 3개월 가입 20% 할인 프로모션으로, 다른 하나를 1개월 가입 10% 할인 프로모션으로 만듭니다.
