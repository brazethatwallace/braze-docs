...
아니요, 미디어 라이브러리는 비디오 파일을 지원하지 않습니다. 비디오는 외부에서 호스팅하거나 YouTube와 같은 플랫폼을 이용하세요.
...
미디어 라이브러리에 업로드된 이미지의 URL을 복사하려면 **콘텐츠** > **미디어 라이브러리**로 이동하세요. 참조하려는 이미지 위에 마우스를 올린 다음 **이미지 URL 복사** 아이콘을 선택하면 이미지 URL이 클립보드에 복사됩니다.

### 이메일에서 SVG 이미지를 사용할 수 있나요? {#can-i-use-svg-images-in-email}

SVG 이미지는 이메일 클라이언트 간 지원이 제한적이므로 이메일에는 권장되지 않습니다. Gmail 및 기타 주요 이메일 제공업체에서는 SVG 이미지를 렌더링하지 않아 수신자에게 이미지가 깨지거나 누락될 수 있습니다. 안정적인 이메일 렌더링을 위해 PNG, JPEG 또는 GIF 형식을 사용하세요.

### 기존 이미지를 자르려면 어떻게 하나요? {#how-do-i-crop-an-existing-image}

미디어 라이브러리에서 이미지를 선택하고 **자르기 및 새 이미지로 저장**을 클릭하면 기존 이미지를 자를 수 있습니다.

![미디어 라이브러리 이미지 미리보기.]({% image_buster /assets/img_archive/media_library_crop1.png %}){: height="75%" width="75%"}

자르기 편집기가 열리면 비율 유형을 선택하고 새 이미지의 이름을 편집할 수 있습니다. **저장**을 선택하면 새 이미지를 사용할 수 있습니다.
...
또한 이미지가 Photoshop(또는 유사한 소프트웨어)에서 만들어져 레이어가 많은 경우, 레이어를 병합하고 줄이는 것도 도움이 될 수 있습니다.
...
### 인공지능을 사용하여 이미지 생성하기 {#generate-an-image-using-ai}

**콘텐츠** > **미디어 라이브러리**에서 **Operator로 생성**을 선택하여 이미지를 생성할 수 있습니다. "미디어 라이브러리 에셋 편집" 권한이 필요합니다. 이 옵션이 보이지 않으면 Braze 계정 팀에 문의하세요. 단계 및 정책 세부 사항은 [BrazeAI로 이미지 생성하기]({{site.baseurl}}/user_guide/brazeai/operator/capabilities#generate-images) 및 [BrazeAI로 이미지 생성하기]({{site.baseurl}}/user_guide/messaging/design_and_edit/media_library#generate-ai)를 참조하세요.

### 미디어 라이브러리에서 이미지를 삭제하면 어떻게 되나요? {#what-happens-when-i-delete-an-image-from-the-media-library}
...
기존 URL에서 [파일을 교체]({{site.baseurl}}/user_guide/messaging/design_and_edit/media_library#replace-a-file)하여 이미 발송된 이메일의 이미지를 업데이트할 수 있습니다. 에셋의 URL과 ID는 동일하게 유지되므로, 이미 발송된 이메일을 포함하여 해당 에셋을 참조하는 모든 메시지에 새 파일이 반영됩니다. 일부 수신자는 변경 전에 기기에 이미지가 캐시된 경우 이전 이미지를 계속 볼 수 있으므로, 모든 수신자가 즉시 업데이트를 확인하는 것은 보장되지 않습니다.

### Braze는 Content Cards 및 인앱 메시지에서 외부 URL을 통해 추가된 이미지를 캐시하나요? {#does-braze-cache-images-added-through-an-external-url-in-content-cards-and-in-app-messages}

채널에 따라 다릅니다.

- **Content Cards 및 기존 인앱 메시지**(Modal, 슬라이드업, 전체 화면): 예. 메시지를 설정하면 Braze가 이미지를 자체 CDN에 복사합니다. 메시지의 이미지는 해당 복사본에서 제공되므로, 원본 소스(예: S3 버킷에서 에셋 제거)를 변경하거나 삭제해도 이미 생성되거나 발송된 Content Cards에는 영향을 미치지 않습니다.
- **HTML 인앱 메시지 및 드래그 앤 드롭 인앱 메시지:** 아니요. Braze는 이미지를 캐시하지 않습니다. 메시지는 제공한 URL에서 직접 이미지를 로드하므로, 소스 URL을 변경하거나 제거하면 실시간 Campaigns에서 이미지가 깨집니다.
- **이메일:** 이미지가 추가된 방식에 따라 동작이 달라집니다. 자세한 내용은 [이미 발송된 이메일의 이미지 에셋을 변경할 수 있나요?](#can-i-change-image-assets-in-emails-that-have-already-been-sent)를 참조하세요.
