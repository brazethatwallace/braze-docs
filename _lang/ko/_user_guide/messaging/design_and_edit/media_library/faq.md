...
### 5 MB 미만이고 지원되는 형식인데도 이미지를 업로드할 때 "예기치 않은 오류"가 표시됩니다. 무엇이 문제인가요? {#i-see-an-unexpected-error-when-uploading-an-image-even-though-its-under-5-mb-and-in-a-supported-format-whats-wrong}
...
2. **파일 이름의 특수 문자:** `&` 또는 `%`와 같은 특수 문자가 포함된 파일 이름은 업로드 실패를 유발할 수 있습니다. 파일 이름을 문자, 숫자, 하이픈 또는 밑줄만 사용하도록 변경한 후 다시 업로드해 보세요.
...
### 인공지능을 사용하여 이미지 생성 {#generate-an-image-using-ai}
