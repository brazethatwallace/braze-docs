...
page_order: 4
description: "Braze 캠페인과 캔버스에서 재사용 가능한 콘텐츠 블록을 생성, 사용 및 관리하는 방법을 알아보세요."
page_type: reference
tool:
  - Templates
  - Media
...
> 콘텐츠 블록을 사용하면 재사용 가능한 크로스채널 콘텐츠를 하나의 중앙 위치에서 관리할 수 있습니다. 캠페인 전반에 걸쳐 일관된 디자인을 만들거나, 다양한 채널을 통해 동일한 오퍼 코드를 배포하거나, 대규모로 일관된 메시징을 위한 사전 정의된 자산을 구축하는 데 활용하세요. [API를 사용]({{site.baseurl}}/api/endpoints/templates/)하여 콘텐츠 블록을 생성하고 관리할 수도 있습니다.
...
1. 편집기에서 **행** 탭으로 이동하여 **콘텐츠 블록**을 선택합니다.
...
3. **개인화 유형** 드롭다운에서 **콘텐츠 블록**을 선택합니다.
4. **속성** 필드에서 콘텐츠 블록의 이름을 선택합니다.
...
- 캔버스 이벤트 등록정보는 캔버스에서만 지원됩니다. 캠페인에서 캔버스 진입 등록정보가 포함된 콘텐츠 블록을 참조하면 값이 채워지지 않습니다.

## 콘텐츠 블록 미리보기 {#preview-content-blocks}

활성 캠페인 또는 캔버스에 콘텐츠 블록을 추가한 후, 콘텐츠 블록 라이브러리에서 콘텐츠 블록 위에 마우스를 올리고 <i class="fa fa-eye preview-icon"></i> **미리보기** 아이콘을 선택하여 미리볼 수 있습니다.
...
콘텐츠 블록을 업데이트하면 Liquid를 통해 콘텐츠 블록이 삽입된 모든 메시지에서 업데이트됩니다. 드래그 앤 드롭 편집기의 **행** 아래 **콘텐츠 블록** 드롭다운을 사용하여 가져온 콘텐츠 블록은 모든 메시지에서 업데이트되지 않습니다.
...
콘텐츠 블록을 편집한 후 **콘텐츠 블록 시작**을 선택하여 업데이트된 콘텐츠 블록을 저장하고 시작할 수 있습니다. 또는 **더 보기** > **복제**를 선택하여 콘텐츠 블록의 복제본을 생성할 수 있습니다.
