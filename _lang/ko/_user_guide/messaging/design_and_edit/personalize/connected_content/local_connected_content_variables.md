...
>  저장된 변수는 `connected_content` 요청이 포함된 필드 내에서만 접근할 수 있습니다. 예를 들어, `localweather` 변수를 메시지와 제목 필드 모두에서 사용하려면 두 필드 모두에서 `connected_content` 요청을 해야 합니다. 요청이 동일한 경우 Braze는 대상 서버에 두 번째 요청을 보내는 대신 캐시된 결과를 사용합니다. 그러나 HTTP POST를 통한 연결된 콘텐츠 호출은 기본적으로 캐시되지 않으며 대상 서버에 두 번째 요청을 보냅니다. POST 호출에 캐싱을 추가하려면 [`cache_max_age`](#configurable-caching) 옵션을 참조하세요.
...
`{{localweather.consolidated_weather[0].weather_state_name}}`을 참조하여 비가 오는지 여부를 테스트할 수 있으며, 이 오브젝트에서 사용하면 `Clear`를 반환합니다. 결과 위치 이름으로 개인화하려면 `{{localweather.title}}`이 `New York`을 반환합니다.
{% endraw %}
