...
템플릿에 {% raw %}`{% connected_content %}`{% endraw %} 태그가 두 개 이상 포함된 경우, 디버거는 수행된 모든 호출을 목록으로 표시합니다. 하나의 템플릿에서 여러 메시지 본문을 렌더링하는 채널(예: HTML, 일반 텍스트, 가속 모바일 페이지 본문을 각각 렌더링하는 이메일, 또는 기기별 본문을 각각 렌더링하는 Quick Push)의 경우, 디버거는 현재 미리보고 있는 본문뿐만 아니라 모든 본문에서 이루어진 연결된 콘텐츠 호출을 전부 표시합니다.
