...
## 다양한 데이터 유형에 대한 기본값 설정 {#setting-default-values-for-different-data-types}

이전 섹션의 예시는 문자열에 대한 기본값을 설정하는 방법을 보여줍니다. `empty`, `nil`(정의되지 않음), 또는 `false` 값을 가지는 모든 Liquid 데이터 유형에 대해 기본값을 설정할 수 있으며, 여기에는 문자열, 부울, 배열, 오브젝트, 숫자가 포함됩니다.

### 사용 사례: 부울 {#use-case-booleans}
...
### 사용 사례: 오브젝트 {#use-case-objects}

`city`와 `state` 속성정보를 포함하는 `location`이라는 중첩 커스텀 속성 오브젝트가 있다고 가정해 보겠습니다. 이러한 속성정보 중 하나라도 설정되지 않은 경우, 사용자에게 해당 정보를 제공하도록 안내하려 합니다.
...
{: start="3"}
3. 사용자의 위치를 메시지에 삽입하고, 주소 속성정보가 설정되지 않은 경우를 위한 기본값을 할당합니다.
...
### 사용 사례: 배열 {#use-case-arrays}

`destination`과 `departure_date` 속성정보를 가진 여행 정보가 포함된 `upcoming_trips`라는 배열 커스텀 속성이 있다고 가정해 보겠습니다. 사용자에게 예정된 여행이 있는지 여부에 따라 개인화된 메시지를 보내려 합니다.
...
{: start="2"}
2. `upcoming_trips`에 콘텐츠가 있는 경우 보낼 메시지를 지정합니다:<br><br>**2a.** 사용자에게 인사하고, 이름이 없는 경우를 대비하여 기본값을 포함합니다. <br>**2b.** `for` 태그를 사용하여 `upcoming_trips`에 포함된 각 여행의 속성정보(또는 정보)를 가져오도록 지정합니다. <br>**2c.** 메시지에 속성정보를 나열하고, `departure_date`가 설정되지 않은 경우를 위한 기본값을 포함합니다. (여행이 생성되려면 `destination`이 필수이므로, 해당 항목에는 기본값을 설정할 필요가 없다고 가정합니다.)<br>**2d.** `for` 태그를 닫은 다음 조건 로직을 닫습니다.
