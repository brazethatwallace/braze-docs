...
테스트 발송이 도착하지 않는 경우, 중단 조건을 충족하는 사용자로 미리보기한 다음 **Test Send**에서 **Override recipients' attributes with current preview user's attributes**를 활성화하세요(또는 조건을 충족하는 콘텐츠 테스트 그룹 멤버를 추가하세요).
