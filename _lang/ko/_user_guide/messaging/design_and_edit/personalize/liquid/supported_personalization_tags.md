...
| <a href='/docs/user_guide/channels/whatsapp/message_processing/messaging_users'>WhatsApp 속성</a> | `{{whats_app.${inbound_message_body}}}` <br> `{{whats_app.${inbound_media_urls}}}` <br> `{{whats_app.${inbound_flow_response}}}` <br> `{{whats_app.${inbound_product_id}}}` <br> `{{whats_app.${inbound_catalog_id}}}` |
...
| 캔버스 컨텍스트 변수 | `{{context.${your_context_variable}}}` |
...
캠페인, 카드, 캔버스 속성은 해당하는 메시징 템플릿에서만 지원됩니다(예: `dispatch_id`는 인앱 메시지 캠페인에서 사용할 수 없습니다).
...
### 캔버스와 캠페인 태그 차이점 {#canvas-and-campaign-tag-differences}
...
### 사용 사례: 최근 로케일에 따라 현지화 {#localize-based-on-recent-locale}
...
### 사용 사례: 시간대별 사용자 타겟팅 {#target-users-by-time-zone}
...
### 사용 사례: 사용자에게 랜덤 배리언트 보내기 {#send-users-random-variants}
