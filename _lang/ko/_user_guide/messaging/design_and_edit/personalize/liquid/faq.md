...
Liquid에 대해 더 알아보려면 Braze 학습 경로 [Liquid를 활용한 동적 개인화](https://learning.braze.com/path/dynamic-personalization-with-liquid)를 확인하세요. 또한 [Liquid 사용 사례 라이브러리]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/liquid/liquid_use_cases)에서 영감을 얻고 Liquid를 활용한 다양한 개인화 예시를 참고할 수 있습니다.
...
Braze에는 메시지에서 사용할 수 있는 Segments용 Liquid 코드를 생성하는 내장 기능이 있습니다. 구체적으로, 객체 내 여러 기준과 일치하는 Segment를 생성할 수 있습니다.
...
단일 `assign` 문으로 처리하기에 값이 너무 복잡한 경우(예: 다른 Liquid 변수나 커스텀 속성을 매개변수로 사용하는 URL) `capture`를 사용하세요. 또한 연결된 콘텐츠 호출의 본문에서 Liquid 변수를 구현할 때도 `capture`가 선호됩니다.
...
Content Block을 Liquid로 템플릿화하면, 블록 내의 모바일 미디어 쿼리가 블록을 메시지에 직접 드래그할 때와 동일한 방식으로 미리보기에 적용되지 않을 수 있습니다. 블록을 드래그하면 레이아웃은 유지되지만 소스 블록과의 연결이 해제되므로, 이후 블록을 편집해도 메시지가 자동으로 업데이트되지 않습니다.
...
Liquid 코드가 구문적으로 올바르게 보이지만 작동하지 않는 경우, 곧은 따옴표(`' '` 또는 `" "`)와 하이픈(`-`) 대신 스마트 따옴표(둥근 따옴표 `' '` 또는 `" "`)와 스마트 대시(엠 대시 `—`)가 사용되었는지 확인하세요. Liquid는 곧은 ASCII 문자만 인식하므로, 스마트 따옴표와 대시는 구문 분석 오류를 유발합니다.

이 문제는 macOS 키보드 설정에서 **스마트 따옴표 및 대시 사용**이 활성화되어 있을 때 흔히 발생하며, Braze 대시보드에서 입력할 때 문자가 자동으로 변환됩니다.

macOS에서 이 설정을 비활성화하려면:

1. **시스템 설정** > **키보드** > **텍스트 입력** > **편집**으로 이동합니다.
2. **스마트 따옴표 및 대시 사용**을 선택 해제합니다.
...
이 오류는 일반적으로 중괄호가 추가되었거나 누락되었음을 나타냅니다. 다른 Liquid 태그 표현식 안에 {% raw %}`{{ }}`{% endraw %}를 중첩하지 마세요. 예를 들어, 속성 참조를 추가 중괄호 쌍으로 감싸는 대신 {% raw %}`{{custom_attribute.${date_of_birth} | date: '%s'}}`{% endraw %}를 사용하세요.
...
이는 데이터 유형이 시간인 커스텀 속성을 문자열(`blank`)과 비교할 수 없기 때문에 실패합니다.

이를 해결하려면 시간 속성을 변수에 할당하고 렌더링 시 속성이 비어 있을 때 `default` 필터를 사용하여 문자열로 변환하세요:
