...
page_order: 3
description: "이 참조 페이지에서는 정적 또는 동적 콘텐츠를 다시 포맷하는 데 사용할 수 있는 필터를 나열합니다."
...
{% raw %}
<br><br>예를 들어, `{{15 | divided_by: 2}}`는 `7`을 출력하지만, `{{15 | divided_by: 2.0}}`은 `7.5`를 출력합니다.
{% endraw %}
{% endalert %}
...
`date` 필터는 타임스탬프를 다른 날짜 형식으로 변환하는 데 사용할 수 있습니다. `date` 필터에 매개변수를 전달하여 타임스탬프를 다시 포맷할 수 있습니다. 이러한 매개변수의 예시는 [strfti.me](http://www.strfti.me/)를 참조하세요.
