...
`{% endif %}` 태그는 조건 로직이 끝났음을 나타냅니다. 조건 로직이 포함된 모든 메시지에 `{% endif %}` 태그를 포함해야 합니다. 조건 로직에 `{% endif %}` 태그를 포함하지 않으면, Braze가 메시지를 구문 분석할 수 없어 오류가 발생합니다.

{% alert note %}
조건 태그(`if`, `elsif`, `unless`)는 연산자를 지원하지만 필터는 지원하지 않습니다. 조건문에서 필터링된 값을 평가하려면, 먼저 필터 결과를 변수에 할당한 다음 해당 변수를 참조하세요. 자세한 내용은 [연산자와 필터를 사용하는 위치]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/liquid/using_liquid/#where-to-use-operators-and-filters)를 참조하세요.
{% endalert %}
