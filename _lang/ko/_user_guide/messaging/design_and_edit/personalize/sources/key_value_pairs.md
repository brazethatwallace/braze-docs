...
Braze에서는 `extras`라고 하는 커스텀 정의 문자열 키-값 페어를 전송할 수 있습니다. API 트리거 및 예약된 API 트리거 캠페인에서 extras에 액세스하려면 대시보드에서 키를 "example_key"로, 값을 {% raw %}`"$json:{"foo": 1, "bar": 1}"`{% endraw %}로 설정합니다. 그러면 개발자 콘솔 출력이 `"extras": { "test": { "foo": 1, "bar": 1 }`가 됩니다.
...
{% alert note %}
iOS 운영체제는 일부 기능(제거 추적, 지오펜스, 푸시 스토리)에 대해 [알림을 제한]({{site.baseurl}}/developer_guide/platform_integration_guides/swift/push_notifications/silent_push_notifications/#ios-silent-notifications-limitations)할 수 있습니다. 이러한 기능에 문제가 발생하는 경우 iOS의 무음 알림 제한이 원인일 수 있습니다.
{% endalert %}
...
콘텐츠 카드에 키-값 페어를 추가하려면 Braze 메시지 작성기의 **설정** 탭으로 이동하여 **새 페어 추가**를 선택합니다.
