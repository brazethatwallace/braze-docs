...
- **Campaign 및 캔버스 단계**: 본문이나 헤더에서 Liquid를 지원하는 채널(예: 이메일, 푸시, SMS, 인앱 메시지, Content Cards, 웹훅)에 해당합니다.
- **드래그 앤 드롭 편집기**: 컨트롤이 블록 또는 편집기 도구 모음에 있는 경우가 많습니다. 예를 들어, 드래그 앤 드롭 인앱 메시지에서 **개인화 추가**를 선택하고, 개인화 유형을 선택한 다음, 생성된 스니펫을 콘텐츠에 배치한 후 **미리보기 및 테스트**에서 미리 볼 수 있습니다. 채널별 참고 사항은 해당 채널의 드래그 앤 드롭 또는 작성기 문서를 참조하세요(예: [인앱 메시지 스타일 설정]({{site.baseurl}}/user_guide/channels/in_app_messages/customize/style_settings#add-liquid) 또는 [드래그 앤 드롭으로 이메일 만들기]({{site.baseurl}}/user_guide/channels/email/drag_and_drop)).
- **전문 작성기**: 개인화 선택기를 제공합니다. 예를 들어, [아이템 추천]({{site.baseurl}}/user_guide/brazeai/item_recommendations/using_recommendations)에서는 동일한 스타일의 창 내에서 **아이템 추천**과 같은 **개인화 유형** 옵션을 사용합니다.
...
## 사전 서식이 지정된 변수 및 기본값 삽입 {#insert-pre-formatted-variables-and-defaults}

**개인화 추가** 도구는 선택적 기본값이 포함된 Liquid를 삽입하여 빈 프로필 데이터가 메시지를 깨뜨리지 않도록 도와줍니다.
...
이 도구는 커서가 위치한 지점에 지정한 기본값이 포함된 Liquid를 삽입합니다. 삽입 지점은 앞뒤 텍스트가 표시되는 미리보기 상자에서도 확인할 수 있습니다. 텍스트 블록이 강조 표시된 경우, 강조 표시된 텍스트가 대체됩니다.

![개인화 추가 Modal의 GIF로, 사용자가 기본값으로 'fellow traveler'를 입력하고 Modal이 작성기에서 강조 표시된 텍스트 'name'을 Liquid 스니펫으로 대체하는 모습을 보여줍니다.]({% image_buster /assets/img_archive/insert_var_shot.gif %})
...
먼저 `assign` 태그를 사용하여 `current_rewards_balance` 커스텀 속성을 "balance"라는 용어로 대체합니다. 이렇게 하면 조작할 수 있는 `balance`라는 변수가 생깁니다.
...
이러한 대시보드 기능을 사용하면 메시지를 작성하는 동안 Liquid를 더 쉽게 사용할 수 있습니다.

### 색상 라벨 {#color-labels}

각 Liquid 요소는 색상에 대응하므로, Liquid 편집기에서 Liquid를 한눈에 구분할 수 있습니다.

![다양한 Liquid 요소에 대한 여러 색상 라벨 다이어그램.]({% image_buster /assets/img/liquid_color_code.png %})
...
- [기본값 설정하기]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/liquid/setting_default_values) — Modal 외의 Liquid 기본값
- [필터]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/liquid/filters) — 날짜 형식 지정, 수학 연산, 문자열 등