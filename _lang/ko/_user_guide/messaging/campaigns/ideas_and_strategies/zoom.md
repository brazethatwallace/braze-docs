...
page_order: 1
page_type: tutorial
description: "이 문서에서는 이메일, 푸시, 인앱 메시지 Campaign에서 Zoom 참석자 등록을 자동화하는 방법을 설명합니다."
channel:
  - email
  - push
  - in-app messages
...
# Zoom 등록 자동화 {#automate-zoom-registration}
...
메시지 본문에 실행 버튼이 포함된 이메일 Campaign을 생성합니다. 사용자가 버튼을 클릭하면 웨비나 랜딩 페이지로 리디렉션합니다(리디렉션 링크에 적절한 파라미터 포함).

URL의 파라미터를 사용하여 사용자 데이터를 전달하고, 페이지가 로드될 때 실행되는 API 호출을 구축하여 사용자를 웨비나에 추가합니다.

![이름, 성, 이메일 주소, 구/군/시를 포함하기 위해 Liquid 템플릿을 사용한 이메일 메시지.]({% image_buster /assets/img/zoom/zoom1.png %})
...
1. 푸시 Campaign을 생성합니다<br><br>
	...
	![Braze에서 등록한 후 표시되는 웨비나 확인 랜딩 페이지.]({% image_buster /assets/img/zoom/zoom4.png %})<br><br>


2. 인앱 메시지 또는 버튼 클릭에 의해 트리거되는 웹훅 Campaign을 생성합니다.<br><br>
 	Braze 프로필에 있는 기존 사용자 데이터를 사용하여 사용자를 웨비나에 등록합니다.<br>

	![특정 Campaign의 버튼을 클릭한 사용자에게 전송되는 액션 기반 Campaign.]({% image_buster /assets/img/zoom/zoom6.png %})<br><br>
...
1. 인앱 메시지 Campaign을 생성합니다<br><br>
...
2. 인앱 메시지 또는 버튼 클릭에 의해 트리거되는 웹훅 Campaign을 생성합니다.<br><br>
	...
	![특정 Campaign의 버튼을 클릭한 사용자에게 전송되는 액션 기반 Campaign.]({% image_buster /assets/img/zoom/zoom5.png %})<br><br>
