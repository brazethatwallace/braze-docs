...
링크에 다음 URL을 추가하고, 자리 표시자를 적절히 대체하세요. 이 두 URL의 유일한 차이점은 Google 캘린더에 `&format=gcal`이라는 추가 매개변수가 필요하다는 것입니다.
...
- `EVENT_SUBJECT`: 이벤트의 제목
- `EVENT_LOCATION`: 이벤트의 위치
- `START_TIME`: UTC 기준 ISO 8601 형식(YYYY-MM-DDTHH:MM:SSZ)의 이벤트 시작 시간
- `END_TIME`: UTC 기준 ISO 8601 형식(YYYY-MM-DDTHH:MM:SSZ)의 이벤트 종료 시간
- `EVENT_DESCRIPTION`: 이벤트에 대한 설명

공백은 HTML 이스케이프 코드 `%20`으로 대체합니다. 예를 들어, "Meet Braze"라는 제목은 "Meet%20Braze"가 됩니다.
...
- **지속 시간:** `duration=30M`, 이벤트 종료 시간(dtend) 대신 1H 또는 30M과 같은 지속 시간을 지정합니다
- **알림 시간(분 단위):** `&reminder=15`
- **종일 이벤트:** `&allday=1`
- **UID:** 이벤트의 고유 식별자를 하드코딩하기 위한 선택적 매개변수로, 일부 캘린더 앱에서 시간이 지남에 따라 이벤트를 업데이트할 수 있게 합니다. 문자열 @ics.agical.io가 값에 자동으로 추가됩니다.

반복 이벤트를 위한 추가 매개변수도 설정할 수 있습니다:
- **주간 이벤트:** `&recur=weekly`
- **월간 이벤트:** `&recur=monthly`
- **반복 종료:** `&recuruntil=END_DATE`, 여기서 `END_DATE`는 UTC 기준 ISO 8601 형식(YYYY-MM-DDTHH:MM:SSZ)의 반복 종료 날짜 및 시간입니다

## 링크 동작 {#link-behavior}

사용자가 링크를 클릭하면, 캘린더가 URL의 UTC 타임스탬프를 사용자의 캘린더에 설정된 시간대로 자동 변환합니다.

예를 들어, "Google 캘린더에 추가" 링크 예시를 열고 캘린더가 CST로 설정되어 있는 경우, 이벤트 시간은 UTC 오후 3시가 CST 기준으로 변환된 시간(오전 10시)으로 자동 입력됩니다.

### Google 캘린더 {#google-calendar}

클릭하면 Google 캘린더가 새 탭 또는 창에서 열리며, 이벤트 세부정보가 초대장에 미리 입력되어 사용자가 바로 저장할 수 있는 상태로 표시됩니다. 이 동작은 모바일과 데스크톱 모두에서 동일합니다.

![이벤트 세부정보가 추가되어 저장할 준비가 된 Google 캘린더 이벤트 추가 대화상자.]({% image_buster /assets/img_archive/calendar_2.png %}){: style="max-width:75%"}

### iCal 또는 Outlook {#ical-or-outlook}

데스크톱에서 클릭하면 ICS 파일이 브라우저의 기본 다운로드 위치(일반적으로 **Downloads** 폴더)에 다운로드됩니다. 사용자가 ICS 파일을 열면 iCal 또는 Outlook이 실행되며, 해당 이벤트를 캘린더에 추가하라는 안내가 표시됩니다.

![새 이벤트를 추가하기 위한 대화상자가 표시된 iCal 캘린더로, 사용자에게 캘린더를 선택하고 확인하도록 안내합니다.]({% image_buster /assets/img_archive/calendar_3.png %}){: style="max-width:75%"}

![이벤트가 추가된 iCal 캘린더.]({% image_buster /assets/img_archive/calendar_4.png %}){: style="max-width:81%"}

모바일에서는 기기와 이메일 앱에 따라 동작이 달라집니다.

{% alert note %}
iPhone에서 Mail 앱과 Microsoft Outlook은 사용자가 iCal 링크를 탭하면 ICS 파일을 기기에 다운로드하지만, 해당 링크에서 캘린더 앱을 직접 열지는 않습니다. 이벤트를 추가하려면 **파일**, **다운로드** 또는 첨부파일 보기(앱에 따라 다름)에서 다운로드된 파일을 열고, 캘린더에서 나머지 단계를 완료하세요. 구체적인 위치는 이메일 앱과 iOS 설정에 따라 다릅니다.
{% endalert %}
