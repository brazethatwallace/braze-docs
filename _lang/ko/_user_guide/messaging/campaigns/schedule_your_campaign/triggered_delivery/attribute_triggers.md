...
- 구독 상태 업데이트.
- 부울, 정수 또는 문자열 커스텀 속성 값이 임의의 값 또는 특정 값으로 변경.

{% alert important %}
날짜 유형 커스텀 속성은 대시보드에서 속성 트리거 옵션으로 사용할 수 없습니다. 날짜 변경에 대응하려면 다른 전달 트리거 또는 채널 워크플로를 사용하세요.
{% endalert %}
...
`Change Custom Attribute Value` 트리거와 `any new value` 옵션을 사용하여 부울, 정수 또는 문자열 값이 임의의 새 값으로 변경될 때 사용자를 타겟팅합니다.
...
{% alert important %}
- 특정 커스텀 속성 값에 대한 속성 트리거는 배열 및 날짜 커스텀 속성에는 사용할 수 없습니다.
