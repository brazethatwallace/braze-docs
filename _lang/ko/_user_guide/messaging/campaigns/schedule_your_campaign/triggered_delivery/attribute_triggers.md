...
`Update Subscription Group Status` 트리거를 사용하여 이메일, 단문 메시지 서비스 또는 WhatsApp의 구독 그룹 상태가 업데이트된 사용자를 타겟팅합니다.

예를 들어, 프로그램에 수신 동의한 사용자에게 환영 단문 메시지 서비스 메시지를 보낼 수 있습니다. 또한 업데이트 소스를 지정하여 메시지가 발송되는 시점을 더 세밀하게 제어할 수 있습니다.
...
- REST API
- SDK
- Shopify (이메일, 단문 메시지 서비스)
- 인바운드 메시지 (단문 메시지 서비스)

예를 들어, Braze가 특정 인바운드 단문 메시지 서비스에 이미 자동으로 응답하므로 인바운드 메시지가 아닌 REST API에서 업데이트가 올 때만 환영 단문 메시지 서비스를 보내고 싶을 수 있습니다.
