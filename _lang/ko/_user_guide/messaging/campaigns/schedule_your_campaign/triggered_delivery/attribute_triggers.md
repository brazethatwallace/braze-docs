...
- 구독 상태 업데이트.
- 부울, 숫자, 문자열 또는 시간 유형 커스텀 속성 값이 임의의 값으로 변경.
- 부울, 숫자 또는 문자열 커스텀 속성 값이 특정 값으로 변경.

{% alert important %}
대시보드에서 정수 속성은 **숫자** 유형을 사용하고, 날짜 또는 타임스탬프는 **시간** 유형을 사용합니다(UI에서 "integer" 또는 "date"로 표시되지 않습니다). **시간** 유형 속성은 **임의의 새 값** 옵션이 있는 **커스텀 속성 값 변경**만 지원하며, **특정 값** 옵션은 지원하지 않습니다.
{% endalert %}
...
`Change Custom Attribute Value` 트리거와 `any new value` 옵션을 사용하여 부울, 숫자, 문자열 또는 시간 유형 값이 임의의 새 값으로 변경될 때 사용자를 타겟팅합니다.
...
`Change Custom Attribute Value` 트리거와 `specific value` 옵션을 사용하여 부울, 숫자 또는 문자열 커스텀 속성이 특정 값으로 변경될 때 사용자를 타겟팅합니다.
