...
## 사용량 제한 소개 {#about-rate-limiting}

Braze에서는 Campaign의 사용량을 제한하여 플랫폼에서 나가는 트래픽 양을 조절함으로써 마케팅 압력을 제어할 수 있습니다. Campaign에 대해 두 가지 유형의 사용량 제한을 구현할 수 있습니다:
...
Braze는 초당 사용량 제한을 지원하지 않습니다. Braze는 분 단위로 메시지 발송을 균등하게 분배하려고 하지만, 이를 보장할 수는 없습니다. 예를 들어, 분당 5,000개의 메시지 사용량 제한이 있는 Campaign의 경우, 5,000개의 요청을 분 단위로 균등하게 분배하려고 시도하지만(초당 약 84개의 메시지), 초당 비율에 약간의 변동이 있을 수 있습니다.
...
"리타겟팅 필터 쇼케이스"라는 Segment를 만들고 "7일 이상 전에 마지막으로 앱을 사용한" 필터를 적용하여 사용자를 타겟팅한다고 가정해 보겠습니다. 이것은 표준 재참여 Segment입니다.

최근에 알림을 받은 다른 더 타겟팅된 Segment가 있는 경우, 이 Segment에 대한 보다 일반적인 Campaign으로 사용자가 타겟팅되는 것을 원하지 않을 수 있습니다. 이 Segment에 "마지막으로 푸시를 받은 시간" 필터를 추가하면, 지난 24시간 내에 다른 알림을 받은 경우 다음 24시간 동안 이 Segment에서 제외됩니다. 24시간 후에도 Segment의 다른 기준을 충족하고 추가 알림을 받지 않았다면, 다시 Segment에 포함됩니다.

![필터 그룹 "7일 이상 전에 마지막으로 앱을 사용한"이 있는 "리타겟팅 필터 쇼케이스"라는 Segment.]({% image_buster /assets/img_archive/rate_limit_daily.png %}){: style="max-width:80%;"}
...
Winning Variant 또는 Personalized Variant와 같은 최적화를 사용하는 경우, Campaign은 초기 실험과 최종 발송의 두 번의 발송으로 구성됩니다.
...
**Every time the campaign is scheduled**를 선택하면 두 단계가 설정된 숫자로 별도로 제한됩니다. 이는 일반적으로 바람직하지 않습니다.

#### Campaign의 최대 노출 횟수 제한 설정 {#setting-a-maximum-impression-cap-on-campaigns}
...
인앱 메시지 규칙은 세션 시작 시 앱에 전달되므로, Braze는 제한에 도달하기 전에 사용자에게 메시지를 보낼 수 있지만, 사용자가 메시지를 트리거할 때 제한에 이미 도달한 상태일 수 있습니다. 이 경우 기기는 여전히 메시지를 표시합니다.

예를 들어, 사용자가 레벨을 클리어할 때 트리거되는 인앱 메시지가 있는 게임이 있고, 이를 100회 노출로 제한했다고 가정해 보겠습니다. 지금까지 99회의 노출이 있었습니다. Alice와 Bob이 모두 게임을 열고, Braze는 그들의 기기에 레벨을 클리어하면 메시지를 받을 자격이 있다고 알립니다. Alice가 먼저 레벨을 클리어하고 메시지를 받습니다. Bob이 다음으로 레벨을 클리어하지만, 그의 기기가 세션 시작 이후 Braze 서버와 통신하지 않았기 때문에 메시지가 제한에 도달했다는 것을 알지 못하고, 그도 메시지를 받습니다. 그러나 노출 제한에 도달하면, 다음에 기기가 적격한 인앱 메시지 목록을 요청할 때 시스템은 해당 메시지를 보내지 않고 기기에서 메시지를 제거합니다.

### 사용량 제한과 A/B 테스트 {#rate-limiting-and-ab-testing}

A/B 테스트에서 사용량 제한을 사용할 때, 사용량 제한은 대조군에 테스트 그룹과 동일한 방식으로 적용되지 않으며, 이는 시간 편향의 잠재적 원인이 됩니다. 이 편향을 피하려면 적절한 전환 기간을 사용하세요.
...
75,000개의 메시지를 분당 10,000개의 사용량 제한으로 발송하려는 경우, 전달은 8분에 걸쳐 분산됩니다. Campaign은 처음 7분 동안 각각 10,000개 이하의 메시지를 전달하고, 마지막 1분에 5,000개를 전달합니다.

#### 발송 횟수 {#number-of-sends}

사용량 제한이 적용된 메시지는 각 분 동안 균등하게 발송되지 않을 수 있습니다. 분당 10,000개의 사용량 제한 예시를 사용하면, Braze는 분당 10,000개 이상의 메시지가 발송되지 않도록 합니다. 이는 10,000개 메시지 중 더 높은 비율이 후반 30초보다 전반 30초 내에 발송될 수 있음을 의미합니다.

사용량 제한은 메시지 발송 시도 시작 시 적용됩니다. 발송 완료에 걸리는 시간에 변동이 있을 때, 완료된 발송 수가 몇 분 동안 사용량 제한을 약간 초과할 수 있습니다. 시간이 지남에 따라 분당 발송 수는 사용량 제한 이하로 평균화됩니다.

{% alert important %}
Segment의 총 사용자 수와 관련하여 이 형태의 사용량 제한으로 시간에 민감한 메시지를 지연시키는 것에 주의하세요. 예를 들어, Segment에 3,000만 명의 사용자가 포함되어 있지만 사용량 제한을 분당 10,000개로 설정하면, 사용자 기반의 상당 부분이 다음 날까지 메시지를 받지 못할 수 있습니다.
{% endalert %}

#### 멀티채널 Campaign 및 Canvases {#multichannel-campaigns-and-canvases}

멀티채널 Campaign 또는 Canvas에 대한 전달 속도 사용량 제한을 설정할 때, 공유 사용량 제한 또는 채널 기반 제한을 설정할 수 있습니다.

멀티채널 Campaign 또는 Canvas가 공유 사용량 제한을 사용하는 경우, Campaign 또는 Canvas에서 분당 발송되는 총 메시지 수가 사용량 제한을 초과하지 않습니다. 예를 들어, Canvas의 사용량 제한이 분당 500,000개이고 이메일 및 SMS 메시지 단계를 포함하는 경우, Braze는 이메일과 SMS를 합쳐 분당 총 500,000개의 메시지를 발송합니다.

![Campaign이 발송하는 속도를 제한하는 옵션이 선택되어 있으며, 분당 500,000개의 메시지로 설정됨.]({% image_buster /assets/img_archive/multichannel_campaigns_rate_limit.png %}){: style="max-width:50%;"}

멀티채널 Campaign 또는 Canvas가 채널 기반 사용량 제한을 사용하는 경우, 사용량 제한은 선택한 각 채널에 적용됩니다. 예를 들어, Campaign 또는 Canvas에서 분당 최대 5,000개의 웹훅과 2,500개의 SMS 메시지를 발송하도록 설정할 수 있습니다.

![두 채널(웹훅 및 SMS/MMS/RCS)에 대한 별도의 사용량 제한으로, 각각 분당 5,000개 및 2,500개의 메시지.]({% image_buster /assets/img_archive/channel_rate_limits.png %}){: style="max-width:70%;"}
...
- **기존 Campaign 및 Canvases:** 모든 기존 Campaign 및 Canvases가 이 인터페이스로 마이그레이션되었습니다. 전달 동작은 동일하게 유지됩니다. 대시보드에는 Campaign이 공유 또는 채널별 로직을 사용하는지 표시됩니다.<br>
- **새 Campaign 및 Canvases:** 모든 새 Campaign 및 Canvases에는 선호하는 사용량 제한 로직을 선택할 수 있는 수동 토글이 있습니다. Campaign 또는 Canvas 사용량 제한을 설정하거나 업데이트할 때 의도한 동작에 맞는 사용량 제한 동작을 선택해야 합니다.
{% endalert %}
...
- SMS 발송은 구독 그룹당 50,000개의 사용량 제한이 적용됩니다. 일부 SMS 공급자는 다른 제한을 적용할 수 있습니다.
...
{% alert note %}
**사용량 제한은 속도 제한이며 정확한 발송 속도를 정의하지 않습니다.** 일반적으로 메시지는 주어진 분 내에 균등하게 분산되며, 대부분의 경우 설정된 제한에 근접하거나 매우 가까운 속도로 발송됩니다. 항상 그런 것은 아닙니다. 예를 들어, 메시지가 매우 큰 경우(많은 Content Blocks, 연결된 콘텐츠 태그 또는 카탈로그 항목 태그가 포함된 이메일 등) 또는 많은 Liquid 중단이 있는 경우(중단된 메시지도 슬롯을 소비하여 유효 발송 속도를 줄일 수 있음)에 해당합니다.<br><br>
...
- 특정 기간 내에 각 사용자가 채널에서 발송된 Campaign 또는 Canvas 구성요소를 받을 횟수.
- 특정 기간 내에 각 사용자가 [태그](#frequency-capping-by-tag)별로 발송된 Campaign 또는 Canvas 구성요소를 받을 횟수.

이 기간은 분, 일 또는 주(7일) 단위로 측정할 수 있으며, 최대 기간은 30일입니다.

각 최대 게재빈도 설정 라인은 `AND` 연산자로 연결되며, 워크스페이스당 최대 10개의 규칙을 추가할 수 있습니다. 동일한 메시지 유형에 대해 여러 제한을 포함할 수 있습니다. 예를 들어, 사용자를 하루에 1개 이하의 푸시, 주당 3개 이하의 푸시로 제한할 수 있습니다. 중단된 메시지는 최대 게재빈도 설정에 포함되지 않습니다.
...
특정 Campaign이 최대 게재빈도 설정 규칙을 무시하도록 하려면, 해당 Campaign의 전달을 스케줄할 때 Braze 대시보드에서 **Frequency Capping**을 **OFF**로 토글하여 설정할 수 있습니다.
...
{% alert note %}
이 동작은 Campaign 또는 Canvas에 대해 최대 게재빈도 설정을 끌 때의 기본 동작을 변경합니다. 변경 사항은 이전 버전과 호환되며 현재 라이브 상태인 메시지에는 영향을 미치지 않습니다.
{% endalert %}
...
- 이 사용자는 월요일과 수요일에 트리거된 Campaign 발송을 받습니다.
- 이 사용자는 해당 주에 이미 2개의 푸시 Campaign 발송을 받았으므로 목요일의 세 번째 Campaign 발송을 받지 않습니다.
...
태그별 최대 게재빈도 설정을 사용하면 메인 태그와 중첩 태그에 규칙을 설정할 수 있으므로, Braze는 모든 태그를 고려합니다. 예를 들어, 메인 태그 A를 최대 게재빈도 설정으로 사용하도록 선택한 경우, 제한을 결정할 때 모든 중첩 태그(예: 태그 B 및 C)의 정보도 포함합니다.
...
- 사용자는 `promotional` 태그가 있는 푸시 알림 Campaign을 주당 1개 이하로 받아야 합니다.<br><br>
...
태그별 최대 게재빈도 설정 규칙은 Campaign 또는 Canvas 구성요소에서 채널당 100개의 메시지와 같은 대규모에서는 제대로 적용되지 않을 수 있습니다.
