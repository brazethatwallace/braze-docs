...
번역하려는 텍스트를 Liquid 번역 태그 {% raw %}`{% translation your_id_here %}`와 `{% endtranslation %}`{% endraw %}로 감싸고 태그 ID를 지정합니다. 번역 태그 ID는 메시지 내에서 고유해야 합니다. {% raw %}`{% translation header %}`{% endraw %}와 같이 텍스트를 명확하게 설명하는 의미 있는 ID 이름을 사용하는 것이 좋습니다.

번역용으로 표시된 메시지 예시는 다음과 같습니다: {% raw %}`{% translation greeting %}Hello!{% endtranslation %}`{% endraw %}

{% alert tip %}
번역하려는 텍스트를 선택한 후 키보드 단축키 **Cmd + Alt + L**(macOS) 또는 **Ctrl + Alt + L**(Windows)을 사용하여 번역 태그로 감쌀 수 있습니다.<br><br> 이 단축키는 이메일 및 Content Blocks용 드래그 앤 드롭 편집기를 제외한 다국어 메시징을 지원하는 모든 채널에서 사용할 수 있습니다. 해당 편집기에서는 **개인화 추가** 버튼을 사용하여 번역 태그를 추가하세요.
{% endalert %}
...
![제목, 오퍼 텍스트, 오퍼 금액 및 CTA에 대한 번역 태그가 포함된 CSV.]({% image_buster /assets/img/multi-language_support/csv_template_example.png %}){: style="max-width:50%;"}

{% endtab %}
{% tab 번역 API 사용 %}

파트너 번역 API를 사용하여 Campaigns 및 Canvases에서 번역을 관리하고 업데이트합니다. 이 방법은 현지화를 위해 외부 시스템을 사용하거나 번역 파트너에 직접 연결하려는 경우에 유용합니다.
...
URL이 번역 태그로 감싸져 있으면 Braze가 추적 정보를 어디에 추가해야 하는지 판단하지 못할 수 있습니다. 이 기능이 올바르게 작동하려면 URL 끝에 추적이 추가될 위치를 나타내는 특수 문자를 포함해야 합니다.
...
| 번역 태그에 전체 URL 또는 Liquid로 생성된 URL이 포함되어 있습니다. | 링크 깨짐 또는 링크 추적 문제가 발생할 경우를 대비하여 URL이 포함된 번역 태그가 식별됩니다. |
| 번역 태그에 쿼리 파라미터가 포함되어 있습니다. | 링크 깨짐 또는 링크 추적 문제가 발생할 경우를 대비하여 쿼리 파라미터가 포함된 번역 태그가 식별됩니다. |
