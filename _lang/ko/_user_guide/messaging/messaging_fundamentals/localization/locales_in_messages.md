...
메시지에 번역을 추가하려면 먼저 [지원하려는 로캘을 생성]({{site.baseurl}}/user_guide/administer/global/workspace_settings/multi_language_settings)해야 합니다. 로캘은 메시징에 사용할 수 있는 언어(및 선택적으로 지역) 배리언트를 정의합니다.

### 2단계: 번역할 콘텐츠 표시 {#step-2-mark-content-for-translation}

번역하려는 텍스트를 Liquid 번역 태그 {% raw %}`{% translation your_id_here %}`와 `{% endtranslation %}`{% endraw %}로 감싸고 태그 ID를 지정합니다. 번역 태그 ID는 메시지 내에서 고유해야 합니다. 텍스트를 명확하게 설명하는 의미 있는 ID 이름을 사용하는 것이 좋습니다(예: {% raw %}`{% translation header %}`{% endraw %}).

번역용으로 표시된 메시지 예시: {% raw %}`{% translation greeting %}Hello!{% endtranslation %}`{% endraw %}

{% alert tip %}
번역하려는 텍스트를 선택한 후 키보드 단축키 **Cmd + Alt + L**(macOS) 또는 **Ctrl + Alt + L**(Windows)을 사용하여 번역 태그로 감쌀 수 있습니다.<br><br> 이 단축키는 이메일 및 Content Blocks용 드래그 앤 드롭 편집기를 제외한 다국어 메시징을 지원하는 모든 채널에서 작동합니다. 해당 편집기에서는 **개인화 추가** 버튼을 사용하여 번역 태그를 추가하세요.
{% endalert %}
...
| 권장 사항 | 이유 |
| --- | --- |
| Liquid 생성 URL은 현지화가 필요한 경우에만 번역 태그로 감싸세요. | Liquid 구문이 올바르게 렌더링되려면 정확하게 유지되어야 합니다. |
| 쿼리 파라미터(예: `?utm_source=promo`)를 번역 태그 안에 포함하지 마세요. | 번역자가 실수로 특수 문자를 변경하거나 제거하여 링크가 깨질 수 있습니다. |
...
{% alert important %}
[이메일 링크 추적](#email-link-tracking)(링크 별칭 지정 또는 링크 템플릿)을 사용하는 경우, URL이 번역 태그로 감싸져 있을 때 추가 설정이 필요합니다.
{% endalert %}
...
메시지에 번역 태그를 추가한 후, 편집기에서 **언어 관리**를 선택하고(이메일 및 Content Blocks용 드래그 앤 드롭 편집기에서는 **언어**) 번역을 추가할 로캘을 하나 이상 선택합니다.
...
메시지에 이미 번역이 저장된 Content Blocks가 포함되어 있는 경우, 해당 번역을 다시 업로드할 필요가 없습니다. 저장된 번역은 Content Blocks가 메시지에 추가될 때 자동으로 적용됩니다.

**언어 관리** 모달에서 저장된 번역이 있는 Content Blocks는 지원하는 로캘과 함께 목록에 표시됩니다. 이를 통해 새 번역을 추가하기 전에 메시지의 어떤 부분이 이미 현지화되어 있는지 확인할 수 있습니다.
...
**템플릿 다운로드**를 선택하여 선택한 번역 ID와 로캘의 매트릭스가 포함된 CSV를 다운로드합니다.

{% alert important %}
영어가 아닌 문자의 표시 문제를 방지하려면 번역 CSV에 Excel을 사용하지 마세요.
{% endalert %}

템플릿을 작성할 때 각 로캘에 대한 텍스트 콘텐츠만 번역하세요. 다운로드한 템플릿에 HTML 태그가 있는 경우, 태그는 변경하지 않고 태그 내의 텍스트만 번역하세요.

예를 들어, 템플릿에 다음이 포함되어 있는 경우:

```
<p style="margin:0;margin-bottom:0">A charming bakery dedicated to crafting artisanal breads.</p>
```

`A charming bakery dedicated to crafting artisanal breads.` 텍스트만 번역하고 HTML 태그 `<p style="margin:0;margin-bottom:0">`와 `</p>`는 그대로 유지하세요.
...
파트너 번역 API를 사용하여 Campaigns 및 Canvases에서 번역을 관리하고 업데이트합니다. 이 방법은 외부 시스템을 현지화에 사용하거나 번역 파트너에 직접 연결하려는 경우에 유용합니다.
...
메시지를 미리 보려면 **사용자로 미리보기** 드롭다운에서 **다국어 사용자** 옵션을 선택합니다. 이를 통해 다양한 로캘 정의 간에 전환하여 메시지의 모든 번역을 미리 볼 수 있습니다.
...
아랍어와 같이 오른쪽에서 왼쪽으로 쓰는 언어의 번역 파일을 작성할 때는 올바른 서식이 적용되도록 번역을 `span`으로 감싸세요.
...
이메일 Campaign에서 Braze는 각 URL에 추적 정보(쿼리 파라미터)를 추가하여 링크를 추적합니다. 이 동작은 [링크 별칭 지정]({{site.baseurl}}/user_guide/messaging/templates/email_templates/link_aliasing)과 [링크 템플릿]({{site.baseurl}}/user_guide/messaging/templates/email_templates/link_template)을 모두 지원합니다.

URL이 번역 태그로 감싸져 있으면 Braze가 추적 정보를 추가할 위치를 결정하지 못할 수 있습니다. 이 기능이 올바르게 작동하려면 추적이 추가되어야 할 위치를 나타내는 특수 문자를 URL 끝에 포함해야 합니다.

URL은 두 가지 특수 문자를 사용하여 이 동작을 제어합니다.
  - `?`는 아직 추적 정보가 없는 URL에 추적을 추가합니다.
  - `&`는 URL에 이미 `?`가 있는 경우 추가 추적을 추가합니다. URL에는 `?`가 하나만 포함될 수 있습니다.

| URL | `?`&nbsp;포함 여부 | 설명 | 예시 |
| --- | --- | --- | --- |
| 표준 URL | 아니요 | URL에 `?`가 아직 없는 경우 닫는 번역 태그 뒤에 `?`를 추가합니다. | {% raw %}```<a href="https://{% translation id_1 %}example.com{% endtranslation %}?">Shop Now</a>```{% endraw %} |
...
| Liquid 생성 | 아니요 | 생성된 URL에 `?`가 아직 없는 경우 닫는 번역 태그 뒤에 `?`를 사용합니다. | {% raw %}```<a href="{% translation id_1 %}{{ product_url }}{% endtranslation %}?">Shop Now</a>``` {% endraw %} |
...
다국어 메시지의 경우 **현지화 설정**에서 각 로케일의 접근성 언어를 설정합니다. **접근성** 섹션에서 {% raw %}`{{accessibility_language}}`{% endraw %}를 사용하면 문서 또는 카드 언어가 해당 로케일 값에 매핑됩니다.
...
다국어 이메일 템플릿을 다운로드할 때 오류가 발생하면, 번역 태그가 Braze의 이메일 본문 처리 방식과 충돌하는 HTML 속성이나 CSS 스타일을 감싸고 있을 수 있습니다.

Braze는 HTML 본문과 일반 텍스트 본문을 동일한 메시지의 별도 구성 요소로 처리합니다. 번역 태그에 `href` 참조와 CSS 스타일이 포함되면 태그 충돌이 발생하여 템플릿이 올바르게 다운로드되지 않을 수 있습니다.

이를 해결하려면:
- `href` 참조와 CSS 스타일을 번역 태그에서 제외하세요.
