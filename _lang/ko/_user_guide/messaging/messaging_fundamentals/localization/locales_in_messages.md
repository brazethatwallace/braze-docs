...
메시지에 번역을 추가하기 전에 먼저 [지원하려는 로캘을 생성]({{site.baseurl}}/user_guide/administer/global/workspace_settings/multi_language_settings)해야 합니다. 로캘은 메시징에 사용할 수 있는 언어(및 선택적으로 지역) 배리언트를 정의합니다.
...
| 권장 사항 | 이유 |
| --- | --- |
| 프로토콜(`https://`)은 번역 태그 밖에 유지합니다. 도메인과 경로만 감쌉니다(예: `example.com/en`). | 번역자가 실수로 특수 문자를 변경하거나 제거하여 링크가 깨질 수 있습니다. |
...
| 권장 사항 | 이유 |
| --- | --- |
| Liquid 생성 URL은 현지화가 필요한 경우에만 번역 태그로 감쌉니다. | Liquid 구문이 올바르게 렌더링되려면 주의 깊게 보존해야 합니다. |
| 쿼리 파라미터(예: `?utm_source=promo`)를 번역 태그 안에 포함하지 마세요. | 번역자가 실수로 특수 문자를 변경하거나 제거하여 링크가 깨질 수 있습니다. |
...
**언어 관리** 모달에서 저장된 번역이 있는 Content Blocks는 지원하는 로캘과 함께 목록에 표시됩니다. 이를 통해 새 번역을 추가하기 전에 메시지의 어떤 부분이 이미 현지화되었는지 확인할 수 있습니다.
...
파트너 번역 API를 사용하여 Campaigns 및 Canvases에서 번역을 관리하고 업데이트합니다. 외부 시스템을 현지화에 사용하거나 번역 파트너에 직접 연결하려는 경우 유용합니다.
...
#### HTML 메시지 전체를 번역 태그로 감쌀 수 있나요? {#can-i-wrap-entire-html-messages-in-a-translation-tag}
