...
page_order: 5.2
page_type: reference
description: "이 참조 문서에서는 Campaigns, Canvas, Liquid 및 Currents에 대한 디스패치 ID 동작에 대해 설명합니다."
---
...
예약된 메시지를 받은 모든 고객은 동일한 `dispatch_id`를 받지만, 액션 기반 또는 API 트리거된 메시지를 받은 고객은 메시지당 고유한 `dispatch_id`를 받습니다. `dispatch_id` 필드를 사용하면 반복 Campaign의 어떤 인스턴스가 전환에 기여했는지 식별할 수 있으므로, 어떤 유형의 Campaigns가 결과를 이끌어내는지 확인할 수 있습니다.
