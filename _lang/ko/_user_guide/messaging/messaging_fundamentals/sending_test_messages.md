...
메시징 캠페인을 테스트하기 전에 테스트 사용자를 식별하는 것이 중요합니다. 이러한 사용자는 기존 사용자 ID 또는 이메일 주소일 수도 있고, 메시징 캠페인 테스트 전용으로 사용되는 새로운 사용자일 수도 있습니다.

### 선택 사항: 콘텐츠 테스트 그룹 만들기 {#optional-create-a-content-test-group}

테스트 사용자를 구성하는 편리한 방법은 [콘텐츠 테스트 그룹]({{site.baseurl}}/user_guide/administer/global/user_management/internal_groups)을 만드는 것입니다. 이 그룹에는 Campaigns에서 테스트 메시지를 수신할 사용자 그룹이 포함됩니다. Campaign의 **테스트 수신자** 아래 **콘텐츠 테스트 그룹 추가** 필드에 이 테스트 그룹을 추가하면, 개별 테스트 사용자를 생성하거나 추가하지 않고도 테스트를 시작할 수 있습니다.

## 2단계: 채널별 테스트 메시지 보내기 {#step-2-send-channel-specific-test-messages}

테스트 메시지를 보내는 방법은 해당 채널에 대한 다음 섹션을 참조하세요.
...
최대 5분 동안 기기에서 테스트 메시지를 확인할 수 있습니다.

![배너 작성기의 미리보기 탭.]({% image_buster /assets/img/banners/preview_banner.png %})

{% alert note %}
미리보기는 하드웨어 차이로 인해 사용자 기기에서의 최종 렌더링과 동일하지 않을 수 있습니다.
{% endalert %}

### 테스트 체크리스트 {#test-checklist}

- 배너 Campaign이 배치에 할당되어 있습니까?
- 이미지와 미디어가 타겟팅된 기기 유형과 화면 크기에서 예상대로 표시되고 작동합니까?
- 링크와 버튼이 사용자를 올바른 위치로 안내합니까?
- Liquid가 예상대로 작동합니까? Liquid가 정보를 반환하지 않는 경우를 대비하여 기본 속성 값을 설정했습니까?
- 문구가 명확하고 간결하며 정확합니까?
...
{% alert important %}
[콘텐츠 테스트 그룹]({{site.baseurl}}/user_guide/administer/global/user_management/internal_groups#content-test-groups) 또는 개별 사용자에게 테스트를 보내려면, 테스트 메시지를 보내기 전에 테스트 기기에서 푸시가 활성화되어 있고 테스트 사용자에게 유효한 푸시 토큰이 등록되어 있어야 합니다. iOS 사용자의 경우, 테스트 Content Cards를 보려면 Braze가 보낸 푸시 알림을 탭해야 합니다. 이 동작은 테스트 Content Cards에만 적용됩니다.
{% endalert %}

테스트 Content Cards는 푸시 알림을 통해 전달됩니다. 카드는 푸시 페이로드에 패키징되며, 푸시가 수신되면 SDK가 이를 추출하여 로컬에 캐시합니다.

이 프로세스는 일반 카드 전달 시스템을 우회하므로, Content Cards를 테스트하더라도 푸시가 활성화되어 있어야 합니다.

테스트 Content Cards는 전송 후 약 5분 후에 만료됩니다.

Content Cards를 만든 후, 앱에 테스트 Content Cards를 보내어 실시간으로 어떻게 보이는지 확인할 수 있습니다.

1. Content Cards를 작성합니다.
2. **테스트** 탭을 선택하고 이 테스트 메시지를 받을 콘텐츠 테스트 그룹 또는 개별 사용자를 하나 이상 선택합니다.
3. **테스트 보내기**를 선택하여 앱에 Content Cards를 보냅니다.

![테스트 Content Cards]({% image_buster /assets/img/contentcard_test.png %})

### 미리보기 {#preview}

**미리보기** 탭에서 카드를 작성하면서 미리 볼 수 있습니다. 이를 통해 사용자의 관점에서 최종 메시지가 어떻게 보일지 시각화하는 데 도움이 됩니다.

{% alert note %}
작성기의 **미리보기** 탭에서 메시지의 뷰는 사용자 기기에서의 실제 렌더링과 동일하지 않을 수 있습니다. 미디어, 문구, 개인화 및 커스텀 속성이 올바르게 생성되는지 확인하려면 항상 기기에 테스트 메시지를 보내는 것이 좋습니다.
{% endalert %}

### 테스트 체크리스트

- 테스트 사용자가 유효한 푸시 토큰으로 푸시에 옵트인되어 있습니까?
- 이미지와 미디어가 예상대로 표시되고 작동합니까?
- Liquid가 예상대로 작동합니까? Liquid가 정보를 반환하지 않는 경우를 대비하여 [기본 속성 값]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/liquid/conditional_logic#accounting-for-null-nil-and-blank-attribute-values)을 설정했습니까?
- 문구가 명확하고 간결하며 정확합니까?
- 링크가 사용자를 올바른 위치로 안내합니까?
- 테스트 사용자가 유효한 푸시 토큰으로 푸시에 옵트인되어 있습니까?
...
- **URL이 올바르고 URL 인코딩되어 있는지 확인하세요:** URL의 특수 문자(공백이나 쿼리 매개변수 등)는 적절하게 인코딩되어야 합니다. 그렇지 않으면 이미지 요청이 실패합니다.
- **콘텐츠 보안 정책을 확인하세요:** 조직에 콘텐츠 보안 정책(CSP) 또는 내부 IT 보안 규칙이 있는 경우, 해당 정책이 이미지 도메인을 차단할 수 있습니다. 이미지 URL의 도메인이 CSP에서 허용되는지 확인하세요.
- **HTTPS를 사용하세요:** 브라우저와 앱에서 혼합 콘텐츠 차단을 방지하려면 이미지 URL은 `http://` 대신 `https://`를 사용해야 합니다.
- **브라우저에서 URL을 직접 열어보세요:** 이미지가 브라우저에서 로드되지 않는다면, 문제는 Braze가 아닌 이미지 URL 또는 호스팅에 있습니다.
...
일반적인 사용 사례는 사용자가 특정 Content Cards를 볼 수 없는 이유를 디버그하는 것입니다. 이를 위해 **이벤트 사용자 로그**에서 세션 시작 시 SDK에 전달된 Content Cards를 확인하고, 노출 전 해당 카드를 특정 Campaign으로 추적할 수 있습니다:
...
여기에서 메시지 설정과 콘텐츠를 검토하여 사용자가 특정 Content Cards를 볼 수 없는 이유를 파악할 수 있습니다.
...
이메일에 [환경설정 센터]({{site.baseurl}}/user_guide/audience/subscription_preferences/preference_center) 링크가 포함된 경우, 테스트 전송에서는 작동하는 링크가 생성되지 않으며 환경설정을 저장할 수 없습니다. 환경설정 센터를 테스트하려면 테스트 사용자 또는 소규모 내부 Segment에 메시지를 실행하세요. 자세한 내용은 [환경설정 센터 테스트]({{site.baseurl}}/user_guide/audience/subscription_preferences/preference_center/dnd_preference_center#testing-preference-centers)를 참조하세요.
...
앱 내에서 그리고 테스트 기기에서 푸시 알림이 설정되어 있다면, 앱에 테스트 인앱 메시지를 보내어 실시간으로 어떻게 보이는지 확인할 수 있습니다.

1. 인앱 메시지를 작성합니다.
2. **테스트** 탭을 선택하고 **개별 사용자 추가** 필드에 이메일 주소 또는 사용자 ID를 추가합니다.
3. **테스트 보내기**를 선택하여 기기에 푸시 메시지를 보냅니다.

기기 화면 상단에 테스트 푸시 메시지가 나타납니다.

![테스트 인앱 메시지]({% image_buster /assets/img_archive/test-in-app.png %})

{% alert important %}
테스트 전송 시 각 수신자에게 하나 이상의 인앱 메시지가 전송될 수 있습니다.
{% endalert %}

푸시 메시지를 직접 클릭하여 열면 앱으로 이동하여 인앱 메시지 테스트를 확인할 수 있습니다. 이 인앱 메시지 테스트 기능은 사용자가 테스트 푸시 알림을 클릭하여 인앱 메시지를 트리거하는 방식에 의존합니다. 따라서, 테스트 푸시 알림이 성공적으로 전달되려면 사용자가 해당 앱에서 푸시 알림을 수신할 수 있어야 합니다.

### 미리보기

**미리보기** 탭에서 인앱 메시지를 작성하면서 미리 볼 수 있습니다. 이를 통해 사용자의 관점에서 최종 메시지가 어떻게 보일지 시각화하는 데 도움이 됩니다. 랜덤 사용자, 특정 사용자 또는 커스텀 사용자에 대해 미리 볼 수 있습니다. 모바일 기기 또는 태블릿에 대한 메시지도 미리 볼 수 있습니다.

![인앱 메시지 작성 시 작성 탭에서 메시지가 어떻게 보일지 미리보기를 보여줍니다. 사용자가 선택되지 않았으므로, 본문 섹션에 추가된 Liquid가 그대로 표시됩니다.]({% image_buster /assets/img/in-app-message-preview.png %})

Braze에는 세 세대의 인앱 메시지가 있습니다. 각 세대에서 지원하는 기기를 기반으로 메시지를 전송할 기기를 세부 조정할 수 있습니다.

![인앱 메시지 미리보기 시 세대 간 전환.]({% image_buster /assets/img/iam-generations.gif %}){: height="50%" width="50%"}

{% alert warning %}
**미리보기**에서 메시지의 뷰는 사용자 기기에서의 실제 렌더링과 동일하지 않을 수 있습니다. 미디어, 문구, 개인화 및 커스텀 속성이 올바르게 생성되는지 확인하려면 항상 기기에 테스트 메시지를 보내는 것이 좋습니다.
{% endalert %}
...
- Liquid가 예상대로 작동합니까? Liquid가 정보를 반환하지 않는 경우를 대비하여 [기본 속성 값]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/liquid/conditional_logic#accounting-for-null-nil-and-blank-attribute-values)을 설정했습니까?
- 문구가 명확하고 간결하며 정확합니까?
- 버튼이 사용자를 올바른 위치로 안내합니까?

### 접근성 스캐너 {#accessibility-scanner}

접근성 모범 사례를 지원하기 위해, Braze는 기존 HTML 편집기를 사용하여 만든 인앱 메시지의 콘텐츠를 접근성 표준에 따라 자동으로 스캔합니다. 이 스캐너는 웹 콘텐츠 접근성 지침([WCAG](https://www.w3.org/WAI/standards-guidelines/wcag/)) 표준을 충족하지 않을 수 있는 콘텐츠를 식별하는 데 도움이 됩니다. WCAG는 장애가 있는 사람들이 웹 콘텐츠에 더 쉽게 접근할 수 있도록 World Wide Web Consortium(W3C)에서 개발한 국제적으로 인정받는 기술 표준입니다.
...
1. 모바일 푸시를 작성합니다.
2. **테스트** 탭을 선택하고 **개별 사용자 추가** 필드에 이메일 주소 또는 사용자 ID를 추가합니다.
3. **테스트 보내기**를 선택하여 작성한 메시지를 기기로 보냅니다.
...
Braze 대시보드에서 이미 푸시 메시지를 수락한 경우, 화면 모서리에 메시지가 표시됩니다. 그렇지 않은 경우, 메시지가 표시되면 **허용**을 선택하세요.
...
![테스트 Content Cards]({% image_buster /assets/img/sms_test.png %})

{% endtab %}
{% tab 웹훅 %}

웹훅을 만든 후, 테스트 전송을 통해 웹훅 응답을 확인할 수 있습니다. **테스트** 탭을 선택하고 **테스트 보내기**를 선택하여 제공된 웹훅 URL로 테스트를 보냅니다. 특정 사용자로서의 응답을 미리 보려면 개별 사용자를 선택할 수도 있습니다.
...
2. **테스트** 탭을 선택하고 이 테스트 메시지를 받을 콘텐츠 테스트 그룹 또는 개별 사용자를 하나 이상 선택합니다.
3. 이 메시지에 사용 중인 구독 그룹에 연결된 전화번호로 WhatsApp 메시지를 보내어 대화 창을 시작합니다. 연결된 전화번호는 **테스트** 탭의 알림에 표시됩니다.
...
메시지에 [개인화]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize)를 사용하는 경우, 캠페인을 올바르게 미리보기하고 사용자 데이터가 콘텐츠에 제대로 채워지는지 확인하기 위해 추가 단계가 필요합니다.

테스트 메시지를 보낼 때 **기존 사용자 선택** 옵션을 선택하거나 **커스텀 사용자**로 미리보기하세요.
...
기존 사용자를 선택하는 경우, 검색 필드에 특정 사용자 ID 또는 이메일을 입력합니다. 그런 다음 대시보드 미리보기를 사용하여 해당 사용자에게 메시지가 어떻게 표시되는지 확인하고, 해당 사용자가 보게 될 내용을 반영하는 테스트 메시지를 기기로 보냅니다.

![사용자 선택]({% image_buster /assets/img_archive/personalized_testing_select.png %})

#### 커스텀 사용자 선택 {#selecting-a-custom-user}

커스텀 사용자로 미리보기하는 경우, 사용자의 이름 및 커스텀 속성과 같이 개인화에 사용할 수 있는 다양한 필드에 텍스트를 입력합니다. 마찬가지로 자신의 이메일 주소를 입력하여 기기로 테스트 메시지를 보낼 수 있습니다.

![커스텀 사용자]({% image_buster /assets/img_archive/personalized_testing_custom.png %})

#### 기존 사용자 커스터마이징 {#customizing-an-existing-user}

무작위 또는 기존 사용자의 개별 필드를 편집하여 메시지 내 동적 콘텐츠를 테스트할 수 있습니다. **편집**을 선택하여 선택한 사용자를 수정 가능한 커스텀 사용자로 변환합니다.

!['사용자로 미리보기' 탭에 '편집' 버튼이 표시된 화면]({% image_buster /assets/img_archive/edit_user_preview.png %}){: style="max-width:50%;"}

### 커스텀 이벤트 속성정보로 개인화된 캠페인 테스트 {#testing-campaigns-personalized-with-custom-event-properties}

[커스텀 이벤트 속성정보]({{site.baseurl}}/user_guide/data/activation/events/custom_events/custom_event_properties)로 개인화된 캠페인을 테스트하는 방법은 위에서 설명한 다른 유형의 캠페인 테스트와 약간 다릅니다.
...
![속성정보를 포함한 테스트 메시지 작성]({% image_buster /assets/img_archive/testeventproperties-compose.png %})

{: start="2"}
2. [실행 기반 전달]({{site.baseurl}}/user_guide/messaging/campaigns/schedule_your_campaign/triggered_delivery)을 사용하여 이벤트 발생 시 캠페인을 전달합니다.
...
또는 커스텀 사용자 ID를 저장하고 있는 경우, 맞춤화된 테스트 메시지를 자신에게 보내서 캠페인을 테스트할 수도 있습니다.

1. 캠페인 문구를 작성합니다.
2. **테스트** 탭을 선택하고 **커스텀 사용자**를 선택합니다.
3. 페이지 하단에 커스텀 이벤트 속성정보를 추가하고, 상단 상자에 사용자 ID 또는 이메일 주소를 추가합니다.
4. **테스트 전송**을 선택하여 속성정보로 개인화된 메시지를 수신합니다.
...
1. 메시지 편집기에서 커스텀 이벤트 속성정보 값을 입력합니다.
...
테스트 메시지가 실제 사용자에게 발송되는 Campaign이나 Canvases와 동일하게 작동하지 않는 경우가 있습니다. 이러한 경우, Campaign 또는 Canvas를 제한된 수의 테스트 사용자에게 발송하여 이 동작을 검증하는 것을 고려하세요.
...
- 인앱 메시지 및 Content Cards를 테스트하려면 대상 사용자가 대상 기기에 대한 푸시 토큰을 보유하고 있어야 합니다.
- 이메일의 탈퇴 링크를 테스트하려면 테스트 사용자의 이메일 주소가 해당 워크스페이스에 있는지 확인하세요.
- `List-Unsubscribe` 헤더는 테스트 메시지 기능으로 전송된 이메일에 포함되지 않습니다.
- 시드 그룹 사용자에게 전송된 이메일은 고객 프로필의 Campaign 수신 목록을 업데이트하거나 대시보드 분석의 발송 수를 증가시키지 않습니다.

## 문제 해결 {#troubleshooting}

### 인앱 메시지 {#in-app-messages}

인앱 메시지 Campaign이 푸시 Campaign에 의해 트리거되지 않는 경우, 사용자가 푸시 메시지를 수신하기 **전에** 타겟 오디언스를 충족하는지 인앱 Campaign 세분화를 확인하세요.

Android 및 iOS에서의 테스트 전송 시, **푸시 권한 요청** 클릭 동작을 사용하는 인앱 메시지가 일부 기기에서 표시되지 않을 수 있습니다. 해결 방법은 다음과 같습니다:
- **Android:** 기기가 Android 13 이상이고 Android SDK 버전 21.0.0 이상이어야 합니다. 다른 원인으로는 인앱 메시지가 표시되는 기기에 이미 시스템 수준의 프롬프트가 있는 경우일 수 있습니다. **다시 묻지 않기**를 선택했을 수 있으므로, 다시 테스트하기 전에 앱을 재설치하여 알림 권한을 초기화해야 할 수 있습니다.
...
실행 기반 인앱 메시지 Campaign이 전달되려면, REST API가 아닌 Braze SDK를 통해 커스텀 이벤트를 기록해야 사용자가 기기에서 직접 적격한 인앱 메시지를 수신할 수 있습니다. 사용자는 세션 중에 해당 이벤트를 수행하면 인앱 메시지를 수신합니다.