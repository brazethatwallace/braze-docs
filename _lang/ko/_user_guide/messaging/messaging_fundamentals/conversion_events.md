...
- **과거 이벤트 보존:** Campaign 및 Canvas 대시보드의 전환 추적은 현재 사용자 프로필이 아닌 과거 행동을 측정합니다. 사용자가 지정된 기간 내에 전환 규칙을 충족하면, Braze는 분석에 하나의 전환을 기록하며, 이후 해당 사용자의 프로필이 일상적인 데이터 정리 또는 GDPR 준수 과정에서 삭제, 병합 또는 보관되더라도 이를 제거하지 않습니다. 실시간 세그먼트 수는 동적 세그먼트가 데이터베이스에 현재 존재하는 활성 프로필만 필터링하기 때문에 영구 대시보드 이벤트 로그보다 자연스럽게 낮을 수 있습니다.
