...
사용자가 **지연** 또는 기타 비메시지 단계에 있는 동안 해당 대기 시간 중에 발생한 전환은 사용자가 다음으로 이동할 때까지 이전 메시지 단계에 귀속됩니다. 사용자가 경로의 마지막 메시지 단계를 수신한 후에도, 추가 메시지 단계가 없더라도 Canvas 전환 기한(Canvas 진입 시점부터 계산)까지 전환이 기록될 수 있습니다.
