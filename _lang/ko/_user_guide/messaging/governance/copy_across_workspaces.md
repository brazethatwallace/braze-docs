...
1. 선택한 Campaign 옆의 <i class="fas fa-cog"></i> 기어 아이콘을 선택합니다.
...
1. **Messaging** > **Landing Pages**로 이동합니다.
...
{% alert note %}
이메일 Campaign, Canvases 또는 템플릿을 워크스페이스 간에 복사할 때 번역은 복사되지 않습니다. 복사 후 대상 워크스페이스에서 번역을 다시 입력하거나 다시 업로드하세요.
{% endalert %}
...
## 기능 플래그가 포함된 메시지 복사 {#copying-messages-with-feature-flags}
...
대상 워크스페이스에 존재하지 않는 기능 플래그 ID가 포함된 기능 플래그 단계가 있는 Campaign 또는 Canvas를 복사하면, 기능 플래그 단계는 복사되지만 그 내용은 복사되지 않습니다.
