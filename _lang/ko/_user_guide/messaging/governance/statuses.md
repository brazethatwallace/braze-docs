...
| 초안 | Campaigns 및 Canvases의 초안은 저장되었지만 아직 시작되지 않은 상태입니다. 편집을 계속하고 발송을 시작하려면 Braze 대시보드에서 **Messaging**으로 이동한 후 **Canvas** 또는 **Campaigns**를 선택하여 초안을 선택할 수 있습니다. |
...
Canvas를 재개하려면 Canvas 빌더의 **Summary** 단계로 이동하여 **Resume Canvas**를 선택하세요. 재활성화되면 이전에 중지된 메시지는 스케줄된 시간이 아직 지나지 않은 경우에 한해 스케줄대로 발송됩니다.
