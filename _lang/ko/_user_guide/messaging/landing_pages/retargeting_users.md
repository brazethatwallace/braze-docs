...
시작하기 전에 [랜딩 페이지]({{site.baseurl}}/user_guide/messaging/landing_pages/create_landing_pages/)를 생성해야 합니다.
...
{% tabs local %}
{% tab Segment 사용 %}
[Segment를 생성]({{site.baseurl}}/user_guide/audience/segments/creating_a_segment/)할 때, "리타겟팅" 그룹에서 **Submitted form on Landing Page**를 선택하세요.
...
여기에서 랜딩 페이지 양식을 제출했거나 제출하지 않은 사용자를 기준으로 세분화할 수 있습니다.
{% endtab %}

{% tab 메시지 트리거 사용 %}
[Campaign]({{site.baseurl}}/user_guide/messaging/campaigns/) 또는 [Canvas]({{site.baseurl}}/user_guide/messaging/canvas/)의 전달 옵션을 선택할 때, **Action Based Delivery**를 선택한 다음 **Submitted Landing Page form**을 선택하세요.

이 랜딩 페이지 양식을 통해 양식을 제출한 모든 사용자는 선택한 메시징 채널을 통해 메시지를 받거나 선택한 Canvas에 진입하게 됩니다.
