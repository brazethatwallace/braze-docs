...
![행과 양식 블록이 포함된 빌드 섹션.]({% image_buster /assets/img/landing_pages/dnd.png %}){: style="max-width:35%;"}
...
랜딩 페이지에 [양식](#form-blocks)이 포함되어 있는 경우, 확인 경험으로 사용할 두 번째 랜딩 페이지를 만드세요. 이 페이지는 양식 제출 후 사용자에게 감사를 표하거나 다음 단계를 안내해야 합니다.
...
에디터의 **미리보기** 탭에서 랜딩 페이지를 미리 볼 수 있습니다. 랜딩 페이지를 초안으로 저장한 후 **랜딩 페이지**로 이동하여 랜딩 페이지 옆의 **URL 복사**를 선택하면 URL을 방문할 수 있습니다. 공동 작업자와 URL을 공유할 수도 있습니다.

![URL 복사 옵션이 표시된 메뉴가 열린 랜딩 페이지.]({% image_buster /assets/img/landing_pages/copy-url.png %})

게시하기 전에 다음을 확인하세요:

- 플랜의 게시된 랜딩 페이지 한도를 초과하지 않았는지
- 각 양식 기반 페이지가 **웹 URL 열기** 작업을 사용하여 [확인 페이지](#step-4-create-a-confirmation-page)에 연결되어 있는지
- URL 경로 및 제목과 같은 모든 필수 페이지 필드가 완료되었는지

준비가 되면 **랜딩 페이지 게시**를 선택합니다.

{% alert note %}
iOS 및 Safari의 공격적인 팝업 차단기와 광고 차단기(Safari의 기본 제공 컨트롤 및 서드파티 확장 프로그램 포함)는 양식 **제출** 버튼이 다른 URL도 여는 경우(같은 탭이든 새 탭이든) 랜딩 페이지의 동작에 부정적인 영향을 줄 수 있습니다.
{% endalert %}
...
템플릿은 랜딩 페이지 편집기와 **랜딩 페이지 템플릿** 페이지(**콘텐츠** > **랜딩 페이지**)에서 모두 액세스하고 관리할 수 있습니다. 랜딩 페이지 템플릿에는 이름이 필수이며, 설명은 선택 사항입니다.
