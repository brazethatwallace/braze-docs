...
커스텀 코드 없이 텍스트 블록에 특정 스타일을 적용하려면 스타일을 지정할 텍스트를 강조 표시한 다음 **스타일을 위해 스팬으로 감싸기**를 선택합니다.

![다양한 글꼴 크기와 색상 등 서로 다른 스타일이 적용된 텍스트 섹션이 있는 텍스트 상자와 '스타일을 위해 스팬으로 감싸기' 옵션이 있는 도구 모음이 표시된 강조 표시 섹션.]({% image_buster /assets/img/landing_pages/wrap_with_span.png %}){: style="max-width:50%;"}
...
**페이지 컨테이너** 탭에서 랜딩 페이지의 모든 관련 구성요소 블록에 적용할 스타일을 설정할 수 있습니다. 이 스타일은 특정 블록으로 재정의하지 않는 한 페이지 전체에 적용됩니다.
...
랜딩 페이지 템플릿을 사용하여 다음 캠페인을 위한 템플릿을 만드세요. 이 템플릿은 랜딩 페이지 편집기와 **랜딩 페이지 템플릿** 페이지(**콘텐츠** > **랜딩 페이지**)에서 모두 접근하고 관리할 수 있습니다. 랜딩 페이지 템플릿에는 이름이 필수이며, 설명은 선택 사항입니다.
