...
랜딩 페이지 빌더에 액세스하려면 [특정 권한]({{site.baseurl}}/user_guide/messaging/landing_pages#prerequisites)이 필요합니다. 액세스 권한이 없는 경우 Braze 관리자에게 도움을 요청하세요.
...
메타 태그를 설정하여 브라우저 탭에서 페이지가 표시되는 방식을 커스터마이즈하고 검색 엔진 결과에 최적화합니다. 이 정보는 고객에게 표시됩니다.
...
| 라디오 버튼 | 사용자가 하나를 선택할 수 있는 옵션 목록을 추가합니다. 제출 시 고객 프로필에 연결된 커스텀 속성이 기록됩니다. |
...
{% alert important %}
양식이 포함된 랜딩 페이지를 만든 후 메시지에 [랜딩 페이지 Liquid 태그]({{site.baseurl}}/user_guide/messaging/landing_pages/tracking_users)를 삽입해야 합니다. 이 태그를 사용하면 Braze가 사용자가 양식을 제출할 때 기존 고객 프로필을 자동으로 식별하고 업데이트할 수 있습니다.
{% endalert %}
...
사용자 기기의 크기에 맞게 랜딩 페이지를 응답형으로 만들려면 작은 화면에서 열을 수직으로 쌓을 수 있습니다. 이를 활성화하려면 응답형으로 만들려는 행에 열을 추가한 다음 **열 커스터마이즈** 섹션에서 **작은 화면에서 수직으로 쌓기**를 토글합니다.

활성화하면 작은 화면에서 다중 열 콘텐츠의 수직 순서를 제어하기 위해 열 쌓기 순서를 반전할 수도 있습니다. 이를 통해 커스텀 코드 없이도 모바일에서 페이지가 더 보기 좋고 자연스럽게 표시됩니다.
...
양식 필드를 필수 또는 선택으로 설정할 수 있습니다. 필수 필드는 양식을 제출하기 전에 반드시 작성해야 합니다. 선택 필드는 사용자가 비워 두거나 선택하지 않을 수 있습니다.
...
- 랜딩 페이지에서 Liquid를 사용하고 Landing Pages Pro 자격이 있는 경우, 이 링크는 대신 실시간 페이지를 온디맨드로 렌더링하며 링크를 생성한 시점의 스냅샷이 아닌 현재 변경 사항을 반영합니다. 콘텐츠는 사용자별로 개인화됩니다.
