...
A/B 테스트에서 마케터는 Campaign 내 단일 변수(예: 이메일 제목란 또는 메시지 발송 시간)를 실험합니다. 오디언스의 하위 집합을 무작위로 두 개 이상의 그룹으로 나누고, 각 그룹에 서로 다른 변형을 제시한 후, 어떤 변형이 가장 높은 전환율을 보이는지 관찰합니다. 일반적으로 가장 성과가 좋은 변형이 나머지 오디언스에게 발송됩니다.
