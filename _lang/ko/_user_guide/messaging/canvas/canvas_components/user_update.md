...
page_order: 12
page_type: reference
description: "이 참조 문서에서는 사용자 업데이트 구성요소와 캔버스에서 이를 사용하는 방법을 다룹니다."
tool: Canvas
---
...
캔버스에서 이 구성요소를 사용하면 업데이트가 분당 `/users/track` 요청 사용량 제한에 포함되지 않습니다. 대신 이러한 업데이트는 일괄 처리되어 Braze가 Braze-to-Braze 웹훅보다 더 효율적으로 처리할 수 있습니다. 이 구성요소는 비과금 데이터 포인트(예: 구독 그룹)를 업데이트하는 데 사용될 때 [데이터 포인트]({{site.baseurl}}/user_guide/data/infrastructure/data_points/)를 기록하지 않습니다.
...
3. 이 커스텀 이벤트를 수행하는 사용자를 타겟으로 하는 캔버스를 생성합니다. 이제 사용자가 장바구니에 항목을 추가하면 이 캔버스가 트리거됩니다. 그런 다음 해당 사용자에게 직접 메시지를 보내 특정 금액에 도달했을 때 쿠폰 코드를 제공하거나, 일정 시간 동안 장바구니를 방치했을 때, 또는 사용 사례에 맞는 다른 상황에서 메시지를 보낼 수 있습니다.
...
사용자 업데이트 단계를 사용하여 `canvas_entry_property`를 유지할 수 있습니다. 장바구니에 항목이 추가될 때 트리거되는 이벤트가 있다고 가정해 보겠습니다. 장바구니에 가장 최근에 추가된 항목의 ID를 저장하고 리마케팅 캠페인에 사용할 수 있습니다. 개인화 기능을 사용하여 캔버스 진입 속성을 검색하고 속성에 저장합니다.
...
캔버스의 트리거 이벤트 등록정보를 속성으로 저장하려면 개인화 모달을 사용하여 캔버스 진입 속성을 추출하고 저장합니다. 사용자 업데이트는 다음 개인화 기능도 지원합니다:

* [연결된 콘텐츠]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/connected_content/)
* [콘텐츠 블록]({{site.baseurl}}/user_guide/messaging/design_and_edit/content_blocks/)
* [진입 속성]({{site.baseurl}}/user_guide/messaging/canvas/create_a_canvas/context_and_event_properties/canvas_persistent_entry_properties/)
...
{% alert warning %}
사용자 업데이트 단계에서 연결된 콘텐츠 Liquid 개인화를 사용할 때는 주의하는 것이 좋습니다. 이 단계 유형은 분당 200,000건의 요청 사용량 제한이 있습니다. 이 사용량 제한은 캔버스 사용량 제한보다 우선 적용됩니다.
{% endalert %}
...
{% alert important %}
캔버스 등록정보(예: `canvas_id`, `canvas_name`, `canvas_variant_name` Liquid 태그)는 사용자 업데이트 단계에서 지원되지 않습니다.
{% endalert %}
