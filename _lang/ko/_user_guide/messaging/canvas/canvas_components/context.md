...
{% alert note %}
편집기의 **Preview & Test Send** 섹션에서 Canvas를 미리보는 경우, 이 패널은 미리보기를 문자열로 생성하기 때문에 테스트 메시지 미리보기의 타임스탬프가 UTC로 표준화되지 **않습니다**. 즉, Canvas가 `time` 오브젝트를 수신하도록 설정된 경우, 메시지 미리보기는 Canvas가 실제로 활성화되었을 때 발생하는 상황을 정확하게 미리보기하지 않습니다. Canvas를 가장 정확하게 테스트하려면 사용자 경로 미리보기를 사용하는 것을 권장합니다.
{% endalert %}
...
Canvas 단계(컨텍스트 단계 포함)에서 Braze는 표준 연결된 콘텐츠 재시도 동작 대신 Canvas 전용 재시도 메커니즘을 사용합니다. 연결된 콘텐츠 호출이 실패하는 경우:
...
표준 연결된 콘텐츠에서 사용되는 `:retry` 태그는 Canvas 단계 내에서 수행되는 연결된 콘텐츠 호출에는 적용되지 않습니다. Canvas 단계에는 Canvas 워크플로에 최적화된 자체 재시도 로직이 있습니다.
...
Canvas 컨텍스트가 정식 출시됨에 따라, 액션 기반 Canvases의 모든 기본 타임스탬프 이벤트 등록정보는 UTC로 표시됩니다. 이 변경은 Canvas 단계와 메시지를 편집할 때 보다 예측 가능하고 일관된 경험을 보장하기 위한 광범위한 노력의 일환입니다. 이 변경은 특정 Canvas가 컨텍스트 단계를 사용하는지 여부에 관계없이 모든 액션 기반 Canvases에 영향을 미칩니다.
