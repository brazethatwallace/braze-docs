...
page_type: reference
toc_headers: "h2"
description: "이 참조 문서에서는 캔버스에서 컨텍스트 단계를 생성하고 사용하는 방법을 다룹니다."
tool: Canvas
...
> 컨텍스트 단계를 사용하면 사용자가 캔버스를 진행하는 동안 하나 이상의 변수를 생성하고 업데이트할 수 있습니다. 예를 들어, 시즌 할인을 관리하는 캔버스가 있는 경우, 컨텍스트 변수를 사용하여 사용자가 캔버스에 진입할 때마다 다른 할인 코드를 저장할 수 있습니다.

## 작동 방식

![캔버스의 첫 번째 단계로 사용된 컨텍스트 단계.]({% image_buster /assets/img/context_step3.png %}){: style="float:right;max-width:40%;margin-left:15px;"}

컨텍스트 단계를 사용하면 사용자가 특정 캔버스를 진행하는 동안 임시 데이터를 생성하고 사용할 수 있습니다. 이 데이터는 해당 캔버스 여정 내에서만 존재하며, 다른 캔버스나 세션 외부에서는 유지되지 않습니다.

컨텍스트 변수는 해당 특정 캔버스 여정에서만 존재합니다. 사용자의 프로필을 영구적으로 변경하지 않으며 다른 캔버스에 표시되지 않습니다. 따라서 특정 캠페인이나 워크플로에만 관련된 임시 정보에 적합합니다.
...
컨텍스트 단계 내에서 최대 10개의 컨텍스트 변수를 정의하거나 업데이트할 수 있습니다. 이러한 변수는 지연을 개인화하고, 사용자를 동적으로 세그먼트하고, 캔버스 전체에서 메시징을 풍부하게 하는 데 사용할 수 있습니다. 예를 들어, 사용자의 예약된 항공편 시간에 대한 컨텍스트 변수를 생성한 다음, 이를 사용하여 개인화된 지연을 설정하고 리마인더를 보낼 수 있습니다.

컨텍스트 변수를 설정하는 방법은 두 가지입니다:

- **캔버스 진입 시:** 커스텀 이벤트 또는 API 트리거의 등록정보가 자동으로 컨텍스트 변수로 채워집니다.
- **컨텍스트 단계에서:** 컨텍스트 단계를 추가하여 컨텍스트 변수를 수동으로 정의하거나 업데이트합니다.

각 컨텍스트 변수에는 이름, 데이터 유형, 값(Liquid 또는 개인화 추가 도구를 사용하여 설정)이 필요합니다. 정의된 후에는 {% raw %}`{{context.${flight_time}}}`{% endraw %}와 같은 Liquid를 사용하여 캔버스 전체에서 컨텍스트 변수를 참조할 수 있습니다. **컨텍스트 변수 이름** 필드에서 컨텍스트 변수 이름을 직접 입력하거나 단계 편집기의 드롭다운에서 선택할 수도 있습니다. 자세한 내용은 [컨텍스트 변수 참조]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/sources/context_variables/)를 확인하세요.

각 캔버스 진입은 최신 진입 데이터와 캔버스 설정을 기반으로 컨텍스트 변수를 재정의하므로, 사용자는 각각 고유한 컨텍스트를 가진 여러 활성 여정을 동시에 진행할 수 있습니다. 예를 들어, 고객에게 예정된 항공편이 두 개 있는 경우, 출발 시간과 목적지와 같은 항공편별 컨텍스트 변수를 각각 가진 두 개의 별도 여정 상태가 동시에 실행됩니다. 이를 통해 뉴욕행 오후 2시 항공편에 대한 개인화된 리마인더를 보내는 동시에, 내일 로스앤젤레스행 오전 8시 항공편에 대한 다른 업데이트를 보낼 수 있어 각 메시지가 특정 예약에 맞게 유지됩니다.
...
캔버스에 단계를 추가한 다음, 사이드바에서 구성요소를 드래그 앤 드롭하거나 <i class="fas fa-plus-circle"></i> 플러스 버튼을 선택하고 **컨텍스트**를 선택합니다.
...
3. Liquid 표현식을 직접 작성하거나 **개인화 추가**를 사용하여 기존 속성에서 Liquid 스니펫을 생성합니다.
4. **미리보기**를 선택하여 컨텍스트 변수의 값을 확인합니다.
5. (선택 사항) 추가 변수를 추가하려면 **컨텍스트 변수 추가**를 선택하고 1-4단계를 반복합니다.
6. 완료되면 **완료**를 선택합니다.

이제 메시지 및 사용자 업데이트 단계와 같이 Liquid를 사용하는 모든 곳에서 **개인화 추가**를 선택하여 컨텍스트 변수를 사용할 수 있습니다. **컨텍스트 변수 이름** 필드에서 컨텍스트 변수 이름을 직접 입력하거나 단계 편집기의 드롭다운에서 선택할 수도 있습니다. 전체 안내는 [컨텍스트 변수 참조]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/sources/context_variables/)를 확인하세요.
...
[오디언스 경로]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/audience_paths/) 및 [결정 분할]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/decision_split/) 단계에서 컨텍스트 변수를 사용하여 필터를 생성할 수 있습니다. 필터 설정, 비교 로직, 고급 예시는 [컨텍스트 변수 참조]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/sources/context_variables/#context-variable-filters)를 확인하세요.
...
{% alert note %}
편집기의 **미리보기 및 테스트 전송** 섹션에서 캔버스를 미리보는 경우, 이 패널은 미리보기를 문자열로 생성하기 때문에 테스트 메시지 미리보기의 타임스탬프가 UTC로 표준화되지 **않습니다**. 즉, 캔버스가 `time` 오브젝트를 수신하도록 설정된 경우, 메시지 미리보기는 캔버스가 실제로 활성화되었을 때 발생하는 상황을 정확하게 미리보기하지 않습니다. 캔버스를 가장 정확하게 테스트하려면 사용자 경로 미리보기를 사용하는 것을 권장합니다.
{% endalert %}
...
컨텍스트 변수가 유효한 경우, 캔버스 전체에서 해당 변수를 참조할 수 있습니다. 그러나 컨텍스트 변수가 올바르게 생성되지 않은 경우, 캔버스의 이후 단계도 올바르게 수행되지 않습니다. 예를 들어, 사용자에게 예약 시간을 할당하는 컨텍스트 단계를 생성하고 예약 시간의 값을 과거 날짜로 설정하면, 메시지 단계의 리마인더 이메일이 전송되지 않습니다.

## 연결된 콘텐츠 문자열을 JSON으로 변환
...
문제 해결 시, _업데이트되지 않음_ 측정기준을 모니터링하여 컨텍스트 변수가 올바르게 업데이트되고 있는지 확인하세요. 컨텍스트 변수가 유효하지 않은 경우, 사용자는 컨텍스트 단계를 지나 캔버스를 계속 진행할 수 있지만, 이후 단계에 적합하지 않을 수 있습니다.
...
캔버스 단계(컨텍스트 단계 포함)에서 Braze는 표준 연결된 콘텐츠 재시도 동작 대신 캔버스 전용 재시도 메커니즘을 사용합니다. 연결된 콘텐츠 호출이 실패하는 경우:

 - 메시지 단계의 경우, 연결된 콘텐츠 호출은 최대 5회까지 재시도할 수 있습니다.
 - 다른 모든 단계의 경우, Braze는 지수 백오프를 사용하여 약 13회 재시도합니다.

모든 재시도가 실패하면 사용자는 캔버스에서 나갑니다.

표준 연결된 콘텐츠에서 사용되는 `:retry` 태그는 캔버스 단계 내에서 수행되는 연결된 콘텐츠 호출에는 적용되지 않습니다. 캔버스 단계에는 캔버스 워크플로에 최적화된 자체 재시도 로직이 있습니다.
...
캔버스 컨텍스트가 정식 출시됨에 따라, 액션 기반 캔버스의 모든 기본 타임스탬프 이벤트 등록정보는 UTC로 표시됩니다. 이 변경은 캔버스 단계와 메시지를 편집할 때 보다 예측 가능하고 일관된 경험을 보장하기 위한 광범위한 노력의 일환입니다. 이 변경은 특정 캔버스가 컨텍스트 단계를 사용하는지 여부에 관계없이 모든 액션 기반 캔버스에 영향을 미칩니다.
...
### 캔버스 컨텍스트가 정식 출시된 이후 무엇이 변경되었나요?

캔버스 컨텍스트가 정식 출시됨에 따라 다음 사항이 적용됩니다:

- 액션 기반 캔버스의 [트리거 이벤트 등록정보]({{site.baseurl}}/user_guide/messaging/canvas/create_a_canvas/context_and_event_properties/)에서 [datetime 유형]({{site.baseurl}}/user_guide/data/activation/events/custom_events/custom_event_properties/)의 모든 타임스탬프는 [UTC](https://en.wikipedia.org/wiki/Coordinated_Universal_Time)로 표시됩니다.
- 이 변경은 특정 캔버스가 컨텍스트 단계를 사용하는지 여부에 관계없이 모든 액션 기반 캔버스에 영향을 미칩니다.

#### 이 변경의 이유는 무엇인가요?

이 변경은 캔버스 단계와 메시지를 편집할 때 보다 예측 가능하고 일관된 경험을 만들기 위한 광범위한 노력의 일환입니다.

#### API 트리거 또는 스케줄된 캔버스도 이 변경의 영향을 받나요?

아니요.

#### 이 변경이 캔버스 진입 등록정보에 영향을 미치나요?

예, `canvas_entry_property`가 액션 기반 캔버스에서 사용되고 등록정보 유형이 `time`인 경우 `canvas_entry_properties`에 영향을 미칩니다. 모든 상황에서 타임스탬프가 원하는 시간대로 표시되도록 Liquid `time_zone` 필터를 사용하는 것을 권장합니다.
...
메시지 단계에 다음 콘텐츠가 포함된 액션 기반 캔버스가 있다고 가정해 보겠습니다:
...
캔버스 진입 등록정보는 캔버스 컨텍스트 변수로 포함됩니다. 즉, Braze API를 사용하여 캔버스 진입 등록정보를 보내고 Liquid 스니펫으로 컨텍스트 변수를 사용하는 것과 유사하게 다른 단계에서 참조할 수 있습니다.
...
|`personalized_message`|  {% raw %}`"Enjoy a discount of" {{context.${promo_code}}} "on delivery from your favorite" {{context.${favorite_cuisine}}} restaurants!"`{% endraw %} | 이전 변수를 결합한 개인화된 메시지. 메시지 단계에서 Liquid 스니펫 {% raw %}`{{context.${personalized_message}}}`{% endraw %}를 사용하여 컨텍스트 변수를 참조하고 각 사용자에게 개인화된 메시지를 전달할 수 있습니다. 또한 컨텍스트 단계를 사용하여 [프로모션 코드]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/sources/promotion_codes#creating-a-promotion-code-list) 값을 저장하고 캔버스 전체의 다른 단계에서 템플릿으로 사용할 수 있습니다. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 role="presentation" }
...
컨텍스트 변수는 캔버스 전체에서 가장 최근 값을 사용하며, 각 업데이트는 해당 변수를 참조하는 모든 후속 단계에 영향을 미칩니다.