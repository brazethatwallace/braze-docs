...
page_order: 4
page_type: reference
description: "이 문서에서는 사용자 여정의 어느 지점에서든 여러 캔버스 경로를 서로 비교하고 대조군과 비교할 수 있는 구성요소인 실험 경로에 대해 설명합니다."
tool: Canvas
---

# 실험 경로 {#experiment-paths}

> 실험 경로를 사용하면 사용자 여정의 어느 지점에서든 여러 캔버스 경로를 서로 비교하고 대조군과 비교할 수 있습니다. 이 구성요소를 사용하면 경로 성과를 추적하여 캔버스 여정에 대해 정보에 기반한 결정을 내릴 수 있습니다.
...
실험 경로를 사용하려면 캔버스에 전환 이벤트가 포함되어야 합니다. 캔버스가 시작된 후에는 전환 이벤트를 추가할 수 없지만, 시작된 캔버스를 복제하고 전환 이벤트를 추가하여 실험 경로를 추가할 수 있습니다.
...
### 위닝 경로 및 개인화된 경로 성과 {#winning-path-and-personalized-paths-performance}
...
다운스트림 *발송 수*는 실험 경로의 비율 분할뿐만 아니라 각 경로의 단계, 지연, 채널 자격, 콘텐츠에 따라 달라집니다. 예를 들어, 서로 다른 지연, Intelligent Timing, 구독 상태 등이 경로 할당이 균등하더라도 메시지를 수신하는 사용자 수를 변경할 수 있습니다. 경로 결과를 비교하려면 공통 진입 지점에서 전환을 측정하는 [실험 경로 분석](#tracking-performance)을 사용하세요.
