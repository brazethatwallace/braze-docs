...
## 에이전트 단계 생성 {#creating-an-agent-step}
...
### 4단계: 추가 컨텍스트 추가(선택 사항) {#step-4-add-any-additional-context-optional}

에이전트 단계가 실행될 때 참조할 추가 컨텍스트 값을 포함할 수 있습니다. Canvas에서 일반적으로 사용하는 모든 Liquid 템플릿 값을 입력할 수 있습니다.

{% alert note %}
에이전트는 이미 **Instructions** 섹션에서 설정된 컨텍스트를 자동으로 수신하고 있습니다. 해당 섹션에서 이미 설정된 Liquid 변수는 여기에 다시 입력할 필요가 없습니다.
{% endalert %}
...
### 에이전트 단계는 언제 사용해야 하나요? {#when-should-i-use-an-agent-step}
