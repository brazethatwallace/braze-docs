...
{% alert important %}
스케줄 및 오디언스 평가는 예약된 진입과 즉시 발송 진입 간에 다를 수 있습니다. 예를 들어, 단계에서 발송 시점에 더 가까운 시점에 평가하는 옵션을 사용하는 경우가 이에 해당합니다. 예약된 진입 또는 발송 기간 몇 분 이내에 오디언스, 스케줄 또는 전달 설정을 편집하는 경우, 다음 단계에 이미 대기열에 추가된 사용자에게 변경 사항이 적용되는지 확인하세요([시작 후 Canvas 변경]({{site.baseurl}}/user_guide/messaging/canvas/managing_canvases/change_your_canvas_after_launch/) 참조). 확실하지 않은 경우 중지하고, 복제한 후 다시 시작하여 깨끗하게 재평가하세요.
{% endalert %}
