...
네. 시작 후에도 연결이 끊어진 단계가 있는 Canvases를 저장할 수 있습니다.

### 사용자가 연결이 끊어진 단계에 도달하면 어디로 이동하나요? {#where-do-users-go-when-theyve-reached-a-disconnected-step}

사용자가 Canvas 워크플로의 연결이 끊어진 단계에 있는 경우, 후속 단계가 있으면 해당 단계로 진행하며, 단계의 설정에 따라 사용자가 어떻게 진행할지 결정됩니다. 이는 사용자가 나머지 Canvas에 직접 연결하지 않고도 단계를 변경할 수 있도록 하기 위한 것입니다. 또한 즉시 라이브로 전환하기 전에 테스트할 수 있는 여유를 제공하여 초안 저장을 효과적으로 가능하게 합니다.
...
또한 해당 분배가 정확히 어떤 형태가 될지 보장할 수 없습니다. 균등한 분할을 원하는 경우 [무작위 버킷 번호]({{site.baseurl}}/user_guide/messaging/ab_testing/concepts/random_bucket_numbers) 필터를 추가하세요.
...
예외 이벤트는 사용자가 연결된 Canvas 구성요소를 수신하기 위해 대기하는 동안에만 트리거됩니다. 사용자가 미리 작업을 수행한 경우 예외 이벤트가 트리거되지 않습니다. 특정 이벤트를 미리 수행한 사용자를 제외하려면 대신 [필터]({{site.baseurl}}/user_guide/audience/segments/segmentation_filters)를 사용하세요.

### Canvas를 편집하면 이미 Canvas에 있는 사용자에게 어떤 영향을 미치나요? {#how-does-editing-a-canvas-affect-users-already-in-the-canvas}

다단계 Canvas의 일부 단계를 편집하면, 이미 오디언스에 포함되어 있지만 해당 단계를 아직 수신하지 않은 사용자는 업데이트된 버전의 메시지를 수신합니다. 이는 해당 단계에 대한 평가가 아직 이루어지지 않은 경우에만 발생합니다.
...
Canvas로 달성하고자 하는 목표에 따라 사용자 여정을 구축하는 방식이 달라질 수 있습니다. Canvas의 유연성을 통해 사용자 라이프사이클의 모든 단계에 맞는 사용자 여정을 매핑할 수 있습니다. 효과적인 사용자 여정을 만들기 위한 간소화된 접근 방식의 여러 예시는 [Braze Canvas 템플릿]({{site.baseurl}}/user_guide/messaging/templates/canvas_templates/braze_templates)을 확인하세요.

## 메시지 및 전달 {#messages-and-delivery}

### Canvas에서 인앱 메시지는 언제 전송되나요? {#when-are-in-app-messages-in-canvas-sent}

인앱 메시지는 다음 세션 시작 시 전송됩니다. 즉, 사용자가 Canvas가 중지되기 전에 캔버스 단계에 진입한 경우, 인앱 메시지가 아직 만료되지 않았다면 다음 세션 시작 시 해당 인앱 메시지를 수신합니다.

사용자가 Canvas가 중지되기 전에 세션을 시작했지만 인앱 메시지가 즉시 표시되지 않을 수 있습니다. 이는 인앱 메시지가 커스텀 이벤트에 의해 트리거되거나 지연된 경우에 발생할 수 있습니다. 따라서 Canvas가 중지된 후에도 사용자가 인앱 메시지 노출 횟수를 기록하고 인앱 메시지를 "수신"할 수 있습니다. 다만, 사용자는 Canvas가 중지되기 전에 세션을 시작했어야 하며, 캔버스 단계를 수신한 **이후**여야 합니다.

{% alert note %}
Canvas를 중지해도 메시지 수신을 기다리고 있는 사용자가 사용자 여정에서 이탈하지는 않습니다. Canvas를 다시 활성화하고 사용자가 여전히 메시지를 기다리고 있다면 해당 메시지를 수신합니다(메시지가 전송되었어야 할 시간이 이미 지난 경우에는 수신하지 않습니다).
{% endalert %}
...
인앱 메시지는 Braze에서 "푸시"되는 것이 아니라 SDK에 의해 "풀"됩니다. 적격한 사용자의 인앱 메시지는 세션 시작 시 자동으로 전달되며 트리거 이벤트가 발생할 때까지 "대기"합니다. 적격한 사용자가 세션을 시작할 때 메시지를 수신하므로 Braze는 이를 전송 이벤트로 보고하지 않습니다. 사용자가 트리거 이벤트를 수행하면 메시지가 표시되고 Braze는 노출 횟수를 기록하며 고객 프로필에서 캔버스 단계(또는 Campaign)를 수신됨으로 표시합니다. 따라서 인앱 메시지의 _전송_ 합계는 0입니다.

### 긴 지연 또는 분기 후에 사용자가 인앱 메시지를 수신하지 못한 이유는 무엇인가요? {#why-didnt-users-receive-my-in-app-message-after-a-long-delay-or-branch}

업스트림 [지연]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/delay_step) 단계와 오디언스 확인이 완료된 후, 사용자는 메시지 단계에 도달해야만 인앱 메시지를 수신할 수 있습니다. 메시지가 캘린더 날짜 또는 짧은 **단계가 사용 가능해진 후 기간** 내에 만료되는 경우, 느린 분기에 있는 사용자는 만료 후에 도착하여 메시지를 볼 수 없습니다. 만료 시간을 가장 긴 현실적인 경로 지연에 맞추세요. 자세한 내용과 예시는 [인앱 메시지 만료]({{site.baseurl}}/user_guide/messaging/canvas/create_a_canvas/canvas_by_channel/in-app_messages_in_canvas#in-app-message-expiration)를 참조하세요.
...
드래그 앤 드롭 인앱 메시지의 버튼 수준 측정기준은 상위 Canvas 요약이 아닌 **Canvas 세부 정보**의 **메시지** 단계 분석 카드에 표시됩니다. Canvas를 열고 메시지 단계를 선택한 후 인앱 인게이지먼트를 확인하세요. 보고 개념에 대한 자세한 내용은 [Canvas 분석으로 측정 및 테스트]({{site.baseurl}}/user_guide/messaging/canvas/testing_canvases/measuring_and_testing_with_canvas_analytics)를 참조하세요.
...
- **실행 기반 또는 API 트리거 진입:** 사용자는 진입 행동을 수행한 후에만 진입(및 단계 수신)하므로, 해당 행동이 발생할 때까지 실제 전송 수는 사전 추정치에 미치지 못합니다.
- **오디언스 경로:** 사용자는 자격을 충족하는 가장 높은 우선순위의 분기로 라우팅되므로, 하위 분기는 단순 Segment 수가 시사하는 것보다 적은 사용자를 수신할 수 있습니다.
- **오디언스 및 전송 시간 확인:** 전체 단계는 별도로 구성하지 않는 한 전송 시간에 필터를 재평가합니다. Canvas가 구축될 때 적격했던 사용자가 메시지 전송 전에 이탈할 수 있습니다.
- **대조군:** 글로벌 또는 Canvas 대조군은 진입자의 일부를 메시징에서 제외합니다.
- **방해금지 시간 및 지연:** 메시지가 보류되거나 재예약되어 보고 있는 보고 기간에서 전송이 이동할 수 있습니다.
- **최대 진입 또는 오디언스 상한:** 기본 Segment가 더 크더라도 진입 또는 전송 상한이 추가 사용자를 차단합니다.
...
**예상 오디언스**는 추정이 실행될 때 Segment 및 진입 필터와 일치하는 사용자를 반영합니다. 그 이후에 지연 또는 실행 기반 진입, 재적격, API 트리거 또는 분기 라우팅으로 인해 스냅샷과 비교하여 여정에 접촉하는 프로필 수가 증가할 수 있습니다. 전송 시간 필터가 실패하면 사용자가 이탈하여 실제 진입 또는 전송 수가 감소할 수도 있습니다. 타이밍, 상한 및 평가 설정을 [전송 수가 예상 오디언스 크기보다 낮은 이유는 무엇인가요?](#why-are-sends-lower-than-the-estimated-audience-size)와 함께 비교하세요.

### 고유 수신자 수가 타겟팅한 사용자 수보다 높은 이유는 무엇인가요? {#why-is-_unique-recipients_-higher-than-the-number-of-users-i-targeted}
...
예를 들어, 사용자가 월요일에 캔버스 단계를 수신하고 금요일에 다시 수신한 후 각 전송 후에 전환하면, Braze는 두 개의 수신자 행과 두 개의 범위 내 전환을 계산할 수 있습니다. 반복 진입 또는 재적격을 사용하면 동일한 소수의 프로필이 여러 날에 걸쳐 여러 _고유 수신자_를 생성할 수 있습니다.
...
세그먼터는 Canvas 또는 Campaign 통계보다 고유 사용자 데이터에 대해 더 정확한 통계입니다. Canvas 및 Campaign 통계는 어떤 일이 발생할 때 Braze가 증가시키는 숫자이기 때문에 세그먼터의 숫자와 다를 수 있는 변수가 있습니다. 예를 들어, 사용자는 Canvas 또는 Campaign에 대해 두 번 이상 전환할 수 있습니다.
...
익명 사용자는 Canvas에 진입하고 종료할 수 있지만, 식별되기 전까지 해당 행동이 특정 고객 프로필과 연결되지 않으므로 분석에서 상호작용이 완전히 추적되지 않을 수 있습니다. [쿼리 빌더]({{site.baseurl}}/user_guide/analytics/reports/query_builder)를 사용하여 이러한 측정기준에 대한 보고서를 생성할 수 있습니다.
...
사용자가 특정 Canvas의 배리언트 경로에 한 번도 진입하지 않은 경우입니다. Canvas에 진입했는지 여부와 관계없이 대조군에 속하지 않은 모든 사용자가 포함됩니다. 여기에는 다른 배리언트에 진입한 사용자와 어떤 배리언트에도 진입하지 않은 사용자가 모두 포함됩니다.
...
이전에는 기존 Canvas 편집기에서 사용자 여정의 단계를 생성할 때마다 기본적으로 전체 단계가 추가되었습니다. 이러한 전체 단계는 다양한 Canvas 구성요소로 대체되어 편집 경험에 대한 가시성과 커스텀 설정이 향상되었습니다. Canvas 단계 도구 모음에서 모든 Canvas 구성요소를 즉시 확인할 수 있습니다.

#### 단계 동작 {#step-behavior}

이전에는 각 전체 단계에 지연 및 스케줄 설정, 예외 이벤트, 오디언스 필터, 메시지 구성, 메시지 진행 옵션 등의 정보가 하나의 구성요소에 모두 포함되어 있었습니다. 현재 편집기에서는 이러한 설정이 분리되어 Canvas 구축 경험을 더 유연하게 커스텀할 수 있으며, 기능적으로도 일부 차이가 있습니다.

#### 메시지 구성요소 진행 {#message-component-advancement}

[메시지 구성요소]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/message_step)는 단계에 진입한 모든 사용자를 진행시킵니다. 메시지 진행 동작을 별도로 지정할 필요가 없어 전체 단계 구성이 더 간단해집니다. **메시지 전송 시 진행** 옵션을 구현하려면 별도의 오디언스 경로를 추가하여 이전 단계를 수신하지 못한 사용자를 필터링하세요.
...
Canvas를 편집하는 중에 "요청 시간 초과" 오류가 발생하여 [Braze 고객지원]({{site.baseurl}}/braze_support)에 문의해야 하는 경우, 해결 속도를 높이기 위해 다음 정보를 포함하세요:
...
이는 [Canvas 중지 시 발생하는 상황](#what-happens-when-you-stop-a-canvas)에서 설명된 것과 동일한 동작입니다. 전송 중인 이메일은 즉시 중단되지 않습니다.
...
Braze는 Campaigns 및 Canvases의 [웹훅]({{site.baseurl}}/user_guide/channels/webhooks) 단계에 대해 웹훅 **전송** 및 관련 전달 결과를 추적합니다. 단계 분석, [웹훅 리포팅]({{site.baseurl}}/user_guide/channels/webhooks/reporting) 또는 [Currents]({{site.baseurl}}/user_guide/data/distribution/braze_currents) 웹훅 이벤트를 사용하여 단계가 실행되었는지 확인하세요. 서버 측 수신 증거가 필요한 경우 엔드포인트의 요청 로그에서 추가 확인이 가능합니다.
...
동일한 초 내에 여러 진입을 허용하려면 트리거 이벤트 간격을 최소 1.1초로 설정하세요(예: 서버에서 이벤트 타이밍을 제어하는 경우). 동일한 초에 여러 트리거를 허용하는 Campaign 스타일 동작의 경우, 적절한 스케줄링 및 재적격 설정이 있는 [Campaigns]({{site.baseurl}}/user_guide/messaging/campaigns)와 사용 사례를 비교해 보세요.
...
앱별 타겟팅을 확인하려면 프로필 **테스트 푸시**에만 의존하지 말고, 좁은 오디언스(예: `external_id`로 필터)를 사용하여 Campaign 또는 Canvas를 통해 실시간 또는 테스트 메시지를 전송하세요.

여러 앱이 있는 **Canvas** 메시지 단계의 경우, 메시지 단계에서 **전송 시 오디언스 검증**을 켜서 전송 시점에 Segment 및 필터 검사가 실행되도록 하세요. 자세한 내용은 [메시지 단계]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/message_step)를 참조하세요.
...
### iOS 및 Android에서 Push Stories를 디버깅하려면 어떻게 하나요? {#how-do-i-debug-push-stories-on-ios-and-android}
...
이메일 상호작용 트리거(예: **이메일에서 별칭 클릭** 또는 **Campaign 또는 Canvas 단계에서 별칭 클릭**)를 사용하는 행동 그룹에는 해당 링크가 포함된 메시지를 이미 전송한 메시지 단계가 필요합니다. 행동 경로 단계가 클릭을 평가하기 전에 이메일이 전송되도록 단계를 추가하거나 순서를 변경하세요. 또는 사용자가 이 Canvas에서 이미 수신한 메시지와 일치하는 상호작용을 선택하세요. 상호작용 트리거의 전체 목록은 [실행 기반 전달]({{site.baseurl}}/user_guide/messaging/campaigns/schedule_your_campaign/triggered_delivery)을 참조하세요.
...
Braze는 적격 이벤트가 수집되고 사용자가 오디언스 규칙을 충족할 때 실행 기반 여정을 평가합니다. Canvas 또는 Campaign이 활성 상태였던 기간 외에 이벤트가 프로필에 도달하거나, 사용자가 오디언스와 일치하기 전에 이벤트가 도달한 경우, 진입 또는 후속 전송이 예상대로 이루어지지 않을 수 있습니다. 사용자 프로필 활동 로그와 [커스텀 이벤트 문제 해결]({{site.baseurl}}/user_guide/messaging/campaigns/schedule_your_campaign/triggered_delivery#troubleshooting-custom-events)의 문제 해결 단계를 사용하여 이벤트 타임스탬프를 활성화 시점 및 Segment 멤버십과 비교하세요. 동작이 여전히 예상과 일치하지 않으면 [Braze 고객지원]({{site.baseurl}}/braze_support)에 문의하세요.