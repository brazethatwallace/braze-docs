---
nav_title: 영구 진입 속성정보
article_title: 영구 진입 속성정보
alias: "/persistent_entry/"
page_type: reference
description: "이 참조 문서에서는 캔버스에서 영구 진입 속성정보를 사용하여 더 정교한 메시지를 보내고, 고도로 세분화된 최종 사용자 경험을 만드는 방법을 설명합니다."
tool: Canvas
page_order: 5
---
...
> 캔버스가 커스텀 이벤트, 구매 또는 API 호출에 의해 트리거되면, API 호출, 커스텀 이벤트 또는 구매 이벤트의 메타데이터를 캔버스 워크플로의 각 단계에서 개인화에 사용할 수 있습니다. 이러한 속성정보를 사용하여 더 정교한 메시지를 보낼 수 있습니다.

{% alert important %}
영구 진입 속성정보는 원래 캔버스 편집기의 산물이므로, 캔버스 진입 속성정보와 같은 용어에 대한 더 이상 사용되지 않는 참조가 역사적 참고를 위해 남아 있습니다. 현재 업데이트된 캔버스 편집기에 대해서는 [컨텍스트 및 이벤트 속성정보]({{site.baseurl}}/user_guide/messaging/canvas/create_a_canvas/context_and_event_properties/)를 참조하세요.
{% endalert %}

## 진입 속성정보 사용

진입 속성정보는 동작 기반 및 API 트리거 캔버스에서 사용할 수 있습니다. 이러한 진입 속성정보는 캔버스가 커스텀 이벤트, 구매 또는 API 호출에 의해 트리거될 때 정의됩니다. 자세한 내용은 다음 문서를 참조하세요:

- [캔버스 진입 속성정보 오브젝트]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/context/)
...
{% alert note %}
캔버스 진입 속성정보 오브젝트의 최대 크기 제한은 50KB입니다.
{% endalert %}

## 진입 속성정보를 사용하도록 캔버스 업데이트

이전에 `canvas_entry_properties`를 사용하는 메시지가 포함되지 않았던 활성 캔버스를 편집하여 `canvas_entry_properties`를 포함하도록 변경하면, `canvas_entry_properties`가 캔버스에 추가되기 전에 캔버스에 진입한 사용자에게는 해당 속성정보에 대응하는 값을 사용할 수 없습니다. 값은 변경이 이루어진 후 캔버스에 진입한 사용자에 대해서만 저장됩니다.

예를 들어, 11월 3일에 진입 속성정보를 사용하지 않는 캔버스를 처음 시작한 후 11월 11일에 캔버스에 새 속성정보 `product_name`을 추가한 경우, `product_name`의 값은 11월 11일 이후에 캔버스에 진입한 사용자에 대해서만 저장됩니다.

캔버스 진입 속성정보가 null이거나 비어 있는 경우, 조건문을 사용하여 메시지를 중단할 수 있습니다. 다음 코드 스니펫은 Liquid를 사용하여 메시지를 중단하는 방법의 예시입니다.
{%raw%}
```
