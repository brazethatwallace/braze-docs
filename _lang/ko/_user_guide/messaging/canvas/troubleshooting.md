...
> 이 페이지에서는 Canvas와 관련된 문제를 해결하는 방법을 안내합니다.
...
Canvas는 강력하고 복잡하며, 생성할 때 많은 시간과 노력을 기울이신다는 것을 알고 있습니다. Canvas가 원하는 대로 발송되지 않는 경우, Canvas 스케줄, 진입 오디언스, 진입 설정을 확인하고 [Canvas 생성]({{site.baseurl}}/user_guide/messaging/canvas/create_a_canvas/) 단계를 검토하는 것을 권장합니다.
...
[진입 설정]({{site.baseurl}}/user_guide/messaging/canvas/create_a_canvas/?tab=basics#selecting-entry-controls)은 Canvas가 어떻게 발송되는지 이해하는 데 중요합니다. Canvas에 진입할 수 있는 사용자 수를 제한했는지 확인하세요.
