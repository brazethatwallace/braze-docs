...
| 분석에서 진입 수보다 훨씬 많은 발송 수 또는 진입 수보다 많은 이탈 수가 표시됨 | [날짜 범위 필터링으로 예상치 못한 수치가 표시될 수 있음](#date-range-filtering-can-show-unexpected-numbers) |
...
{% details 일광 절약 시간제와 매일 스케줄된 Canvases %}

일광 절약 시간제(DST) 전환일에는 매일 스케줄된 Canvases가 평소보다 최대 1시간 일찍 또는 늦게 실행될 수 있습니다. 진입 기준이 스케줄된 진입 시간으로부터 1시간 이내의 타임스탬프를 가진 커스텀 속성이나 이벤트에 의존하는 경우, 속성이나 이벤트가 아직 기록되지 않아 DST 전환일에 사용자가 자격을 충족하지 못할 수 있습니다.
...
### 날짜 범위 필터링으로 예상치 못한 수치가 표시될 수 있음 {#date-range-filtering-can-show-unexpected-numbers}

**증상:** Canvas 또는 단계 분석에서 진입 수보다 훨씬 많은 발송 수, 또는 진입한 사용자보다 많은 이탈 사용자 등 예상치 못하거나 불가능해 보이는 수치가 표시됩니다.

이는 Canvas 분석 페이지 상단의 날짜 범위 캘린더 필터를 사용할 때 발생할 수 있습니다. 일부 사용자 동작을 제외하는 날짜 범위를 선택하면 표시되는 측정기준이 각 사용자 여정의 일부만 보여줄 수 있습니다.

예를 들어:
- 대부분의 사용자가 진입한 이후부터 시작하지만 메시지를 받은 시점을 포함하는 날짜 범위를 선택하면 100건의 진입에 8,000건의 발송이 표시될 수 있습니다.
- 이탈만 캡처하고 이전 진입은 캡처하지 않는 범위를 선택하면 이전 단계에 진입한 것보다 더 많은 사용자가 다음 단계로 이동하는 것처럼 보일 수 있습니다.

이를 해결하려면 Canvas가 시작된 시점부터 현재까지 모든 날짜를 포함하도록 날짜 범위를 조정하거나, 필요한 측정기준과 관련된 전체 기간을 포함하는 범위를 선택하세요.
