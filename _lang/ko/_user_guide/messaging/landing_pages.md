...
게시할 수 있는 랜딩 페이지 수, 커스텀 도메인 수, 사용 가능한 기능은 플랜 유형(무료 또는 프로(증분))에 따라 다릅니다.
...
네, 개인화되지 않은 랜딩 페이지는 높은 트래픽 시나리오를 효과적으로 처리할 수 있습니다. 개인화되지 않은 랜딩 페이지가 처음 요청되면 Braze는 Cloudflare를 통해 이를 캐시합니다. 이후 동일한 링크에 대한 모든 요청은 캐시에서 제공되므로 대량 요청 시에도 성능이 저하되지 않습니다. 이 캐시는 24시간 동안 유지되며, 캐시된 페이지 조회수는 사용량 제한에 포함되지 않습니다.
...
네. 드래그 앤 드롭 편집기에서 **커스텀 코드** 블록을 사용하여 HTML을 추가하거나 편집할 수 있습니다. 커스텀 코드에서 Braze SDK와 연동하려면 [랜딩 페이지용 JavaScript 브리지]({{site.baseurl}}/user_guide/messaging/landing_pages/javascript_bridge)를 참조하세요. 완전히 커스텀된 UI를 랜딩 페이지 양식에 연결하려면 [커스텀 양식 블록 만들기]({{site.baseurl}}/user_guide/messaging/landing_pages/custom_form_blocks)를 참조하세요.

### 랜딩 페이지에서 iframe을 사용할 수 있나요? {#can-i-use-iframes-on-landing-pages}

네. 드래그 앤 드롭 편집기에서 **커스텀 코드** 블록을 추가하고 삽입하려는 콘텐츠의 URL이 포함된 iframe 요소를 추가하세요.

삽입된 웹사이트가 콘텐츠 보안 정책(CSP)의 `frame-ancestors` 또는 `X-Frame-Options`를 통해 프레이밍을 제한하는 경우, iframe에서 페이지가 로드되지 않을 수 있습니다. Braze는 이러한 설정을 재정의할 수 없으며, 삽입된 사이트에서 랜딩 페이지 도메인을 허용하도록 구성해야 합니다.

### 랜딩 페이지 내에서 웹훅을 만들 수 있나요? {#can-i-create-a-webhook-inside-a-landing-page}

아니요. 하지만 **Submitted a Landing Page form** 이벤트를 Canvases 또는 웹훅 Campaigns의 트리거로 사용할 수 있습니다:
...
페이지가 Braze 채널을 통해 전송되지 않은 경우(예: 웹사이트나 광고를 통해), 해당 사용자가 이미 Braze에 존재하더라도 제출 시 새 고객 프로필이 생성될 수 있습니다. 이를 처리하려면 **Submitted a Landing Page form**으로 트리거되는 Canvas를 설정하고, [`/users/merge`]({{site.baseurl}}/api/endpoints/user_data/post_users_merge) 엔드포인트를 호출하는 Braze-to-Braze 웹훅 단계를 추가하여 새 프로필을 기존 프로필에 병합합니다.
