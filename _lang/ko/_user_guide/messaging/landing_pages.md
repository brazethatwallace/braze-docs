...
게시할 수 있는 랜딩 페이지, 커스텀 도메인의 수 및 사용할 수 있는 기능은 플랜 유형(무료 또는 프로(증분))에 따라 다릅니다.
...
삽입된 웹사이트가 콘텐츠 보안 정책(CSP)의 `frame-ancestors` 또는 `X-Frame-Options`를 통해 프레이밍을 제한하는 경우, 해당 페이지가 iframe에서 로드되지 않을 수 있습니다. Braze는 이러한 설정을 재정의할 수 없으며, 삽입된 사이트에서 랜딩 페이지 도메인을 허용하도록 구성해야 합니다.
