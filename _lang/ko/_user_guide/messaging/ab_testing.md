...
guide_top_text: "실험을 통해 메시징을 최적화하세요. A/B 테스트는 동일한 캠페인의 여러 버전에 대한 사용자 응답을 비교하며, 다변량 테스트는 이를 두 개 이상의 변수로 확장합니다. Braze에서는 설정 과정이 동일하기 때문에 두 용어를 같은 의미로 사용합니다. A/B 테스트와 <a href='/docs/user_guide/brazeai/intelligence_suite/intelligent_selection'>지능형 선택</a>을 함께 사용하여 결과를 자동으로 최적화하세요."
...
- **온보딩 캠페인 또는 반복 발송:** 트래픽이 높은 캠페인이 최대한 효과적인지 확인하세요.
...
- **시작 전에 테스트를 추가하세요.** 실행 중인 캠페인에 테스트를 추가하면 부정확한 결과가 나옵니다. 캠페인을 복제하고, 원본을 중지한 다음, 복제본에 테스트를 추가하세요.
