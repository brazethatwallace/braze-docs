...
- **온보딩 Campaigns(캠페인) 또는 반복 발송:** 트래픽이 높은 Campaigns가 최대한 효과적인지 확인하세요.
...
- **시작 전에 테스트를 추가하세요.** 실행 중인 Campaign에 테스트를 추가하면 부정확한 결과가 나옵니다. Campaign(캠페인)을 복제하고, 원본을 중지한 다음, 복제본에 테스트를 추가하세요.
