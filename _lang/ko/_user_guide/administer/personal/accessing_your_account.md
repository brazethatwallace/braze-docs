...
SSO를 사용하고 있으며 멀티 컴퍼니 개발자를 설정하려면, 커스텀 SAML SSO 통합을 설정하여 SAML 커스텀 엔티티 ID를 활성화해야 합니다. [서비스 공급자(SP) 시작 로그인]({{site.baseurl}}/user_guide/administer/global/saml_single_sign_on/saml_sso_setup)의 단계를 따르되, 다음 변경 사항을 적용하세요:
...
대부분의 회사 사용자에게는 드래그 앤 드롭 편집기가 로드됩니다. 그러나 VPN을 사용하거나 방화벽 뒤에 있는 경우 도메인을 허용 목록에 추가해야 할 수 있습니다. IT 관리자에게 `*.bz-rndr.com`이 허용 목록에 추가되었는지 확인하도록 문의하세요.

편집기는 다음과 같은 이유로 로드 문제가 발생할 수 있습니다.

- **일시적 오류:** 연결, 커뮤니케이션 또는 데이터 전송에 영향을 줄 수 있는 일시적인 장애입니다. 다행히 이러한 오류는 일반적으로 단기적인 조건에 의해 발생하며 체계적인 문제를 나타내지 않으므로, 별도의 조치 없이 자체적으로 해결되는 경우가 많습니다.
...
{% alert important %}
여전히 문제가 발생하는 경우 [지원 티켓을 생성]({{site.baseurl}}/user_guide/administer/personal/braze_support)하세요. 그 전에 IT 관리자가 `*.bz-rndr.com`이 허용 목록에 추가되었는지 확인했는지 점검하세요.
{% endalert %}
...
3. **회사 설정** > **보안 설정**으로 이동하여 SSO가 켜져 있는지 확인하세요.
...
1. **사용자 관리**로 이동합니다.
2. 2FA 문제가 있는 사용자에 대해 **사용자 편집**을 선택합니다.
3. 2FA 재설정 옵션을 선택합니다.
...
- [오류는 표시되지 않지만 Braze에 여전히 접근할 수 없습니다.](#instance-error)
...
이런 일은 자주 발생하지 않지만, Braze는 계정 정지 및 삭제를 매우 심각하게 다룹니다. 이 오류가 발생하면 회사의 Braze 관리자, Braze 계정 매니저 또는 [지원팀][support]에 문의하세요.
...
- **호환되는 플러그인 또는 서드파티 도구 사용:** 광고 차단기 또는 보안 소프트웨어가 Braze 대시보드 로드를 방해할 수 있습니다. 광고 차단기를 비활성화한 다음 Braze 대시보드에 로그인하여 테스트하세요.
...
**설정** > **회사 사용자**로 이동하여 사용자의 워크스페이스 수준 권한을 확인하세요. 필요한 워크스페이스를 **워크스페이스**에 추가하세요.
