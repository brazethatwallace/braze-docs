...
Braze에서는 동일한 클러스터 내에서 같은 이메일 주소를 여러 대시보드 사용자에 등록할 수 없습니다(예: US-01에 두 개의 대시보드가 있는 경우). 하지만 서로 다른 클러스터에서는 같은 이메일로 계정을 생성할 수 있습니다(예: US-01에 하나, US-05에 하나의 대시보드가 있는 경우). 동일한 클러스터 내에서 여러 Braze 대시보드에 접근해야 하는 경우, 다음 방법을 사용할 수 있습니다.
...
Google Chrome에서 "앗, 이런!" 오류가 표시되면 Chrome이 Braze 대시보드 페이지를 로드하는 데 문제가 있는 것입니다. 문제 해결 단계는 [Chrome에서 일반적인 오류 메시지 해결하기](https://support.google.com/chrome/answer/95669?co=GENIE.Platform%3DDesktop&hl=en)를 참조하세요.
...
대부분의 회사 사용자는 드래그 앤 드롭 편집기가 정상적으로 로드됩니다. 그러나 VPN을 사용하거나 방화벽 뒤에 있는 경우 도메인을 허용 목록에 추가해야 할 수 있습니다. IT 관리자에게 `*.bz-rndr.com`이 허용 목록에 추가되어 있는지 확인하도록 요청하세요.
...
3. **Company Settings** > **Security Settings**로 이동하여 SSO가 켜져 있는지 확인하세요.
4. 대시보드 고객 프로필에 이름과 성이 모두 포함되어 있는지 확인하세요. 성이 없으면 로그인 프로세스가 중단될 수 있습니다.
5. 대시보드에서 **Support** > **Braze Learning**으로 이동하여 Braze 학습에 접근하세요.
6. 문제가 계속되면 계정을 다시 생성하는 것을 고려하세요. 무료 평가판 기간에 Braze 학습에 접근했던 사용자는 현재 접근에 어려움을 겪을 수 있습니다.
...
관리자는 다음 단계를 수행하여 영향을 받는 사용자의 2FA를 재설정해야 합니다.

1. **Manage Users**로 이동합니다.
2. 2FA 문제가 있는 사용자의 **Edit User**를 선택합니다.
3. 2FA 재설정 옵션을 선택합니다.
...
- [오류는 표시되지 않지만 Braze에 로그인할 수 없습니다.](#instance-error)
...
관리자는 **설정** > **회사 사용자**로 이동하여 사용자의 워크스페이스 수준 권한을 확인하고 필요한 워크스페이스를 **워크스페이스**에 추가하여 이 문제를 해결할 수 있습니다.
