...
두 이메일 주소 모두 같은 받은편지함으로 이메일을 전달하지만, Braze에서는 로그인 시 별도의 계정으로 인식합니다.
...
멀티 컴퍼니 개발자 기능을 사용하면 하나의 사용자 계정을 여러 회사에서 공유할 수 있습니다. 대시보드 사용자는 프로필 메뉴에서 서로 다른 회사 대시보드 간에 토글할 수 있습니다.
...
대시보드 내에서 계정 간에 전환할 때는 2FA를 한 번만 완료하면 됩니다. 해당 세션에서 연결된 계정에 처음 로그인할 때만 인증이 필요합니다.
...
비밀번호를 재설정하려면 대시보드 로그인 페이지에서 **Forgot your password?** 링크를 선택하세요. 비밀번호 재설정 링크를 받을 이메일을 입력하라는 메시지가 표시됩니다.

![대시보드 로그인 화면에 "Forgot your password?" 프롬프트가 표시된 모습.]({% image_buster /assets/img_archive/enable_reset.png %}){: style="max-width:60%"}
...
Google Chrome에서 "앗, 이런!" 오류가 표시되면 Chrome이 Braze 대시보드 페이지를 로드하는 데 문제가 있는 것입니다. 문제 해결 단계는 [Chrome에서 일반적인 오류 메시지에 대한 도움말 보기](https://support.google.com/chrome/answer/95669?co=GENIE.Platform%3DDesktop&hl=en)를 참조하세요.

### 대시보드 탐색 중 "페이지를 새로고침하세요" 또는 "예기치 않은 오류" 발생 {#please-refresh-page-or-unexpected-error-while-navigating-the-dashboard}

이 오류는 회사 사용자가 어떤 워크스페이스에도 속하지 않을 때 나타날 수 있습니다. 문제를 해결하려면 다음을 수행하세요.
...
대부분의 회사 사용자에게는 드래그 앤 드롭 편집기가 정상적으로 로드됩니다. 그러나 VPN을 사용하거나 방화벽 뒤에 있는 경우 도메인을 허용 목록에 추가해야 할 수 있습니다. IT 관리자에게 `*.bz-rndr.com`이 허용 목록에 추가되었는지 확인하세요.

편집기가 다음과 같은 이유로 로드 문제를 겪을 수 있습니다.
...
{% alert important %}
여전히 문제가 발생하는 경우 [지원 티켓을 열어주세요]({{site.baseurl}}/user_guide/administer/personal/braze_support). 그 전에 IT 관리자가 `*.bz-rndr.com`이 허용 목록에 추가되었는지 확인했는지 점검하세요.
{% endalert %}
...
3. **Company Settings** > **Security Settings**로 이동하여 SSO가 켜져 있는지 확인합니다.
4. 대시보드 사용자 프로필에 이름과 성이 모두 포함되어 있는지 확인합니다. 성이 없으면 로그인 프로세스가 중단될 수 있습니다.
5. 대시보드에서 **Support** > **Braze Learning**으로 이동하여 Braze 학습에 접근합니다.
6. 문제가 계속되면 계정을 다시 만드는 것을 고려하세요. 무료 평가판 기간에 Braze 학습에 접근했던 사용자는 현재 접근에 어려움을 겪을 수 있습니다.

### 2단계 인증(2FA) 문제 {#two-factor-authentication-2fa-issues}

사용자가 2단계 인증(2FA)에 문제가 있어 Braze 대시보드에 접근할 수 없는 경우 여러 가지 원인이 있을 수 있습니다. 가장 일반적으로는 등록된 전화번호나 Authy 앱이 설치된 기기에 더 이상 접근할 수 없는 경우입니다.
...
수신한 오류 메시지를 통해 어떤 종류의 잠금인지 확인할 수 있습니다.
...
이런 경우는 자주 발생하지 않지만 Braze는 계정 정지 및 삭제를 매우 심각하게 다룹니다. 이 오류가 발생하면 회사의 Braze 관리자, Braze 계정 매니저 또는 [지원팀][support]에 문의하세요.

### Braze 대시보드가 로드되지 않거나 예상대로 작동하지 않음 {#braze-dashboard-wont-load-or-work-as-expected}

먼저 다른 브라우저에서 대시보드가 로드되는지 테스트하세요. 다른 브라우저에서 문제가 발생하지 않으면 다음을 시도하세요.
...
- **호환되는 플러그인 또는 서드파티 도구 사용:** 광고 차단기나 보안 소프트웨어가 Braze 대시보드 로드를 방해할 수 있습니다. 광고 차단기를 비활성화한 다음 Braze 대시보드에 로그인하여 테스트하세요.
...
위의 문제를 해결했는데도 대시보드가 여전히 로드되지 않거나 예상대로 작동하지 않으면 [지원팀]({{site.baseurl}}/braze_support)에 문의하세요.

### 사용자가 어떤 워크스페이스에도 속하지 않음 {#the-user-belongs-to-no-workspace}

관리자는 **Settings** > **Company Users**로 이동하여 사용자의 워크스페이스 수준 권한을 확인하고 필요한 워크스페이스를 **Workspaces**에 추가하여 이 문제를 해결할 수 있습니다.
...
2FA 설정 중 **Start setup**을 선택했지만 인증 코드(SMS 또는 이메일)를 받지 못하거나 인증 앱 설정을 완료할 수 없는 경우 브라우저 확장 프로그램, 쿠키 설정 또는 네트워크 제한이 방해하고 있을 수 있습니다. 다음을 시도하세요.
...
- 네트워크 전환: 회사 네트워크를 사용 중인 경우 방화벽 정책이 2FA 설정을 방해할 수 있습니다. 개인 연결이나 모바일 핫스팟으로 전환해 보세요.
- 브라우저 설정 전에 인증 앱 설치: 설정 중 **Authenticator app**을 선택하기 전에 모바일 기기에 인증 앱(Authy, Google Authenticator 또는 LastPass Authenticator 등)을 다운로드하여 설치하세요.
...
이러한 단계를 수행한 후에도 문제가 계속되면 [지원팀]({{site.baseurl}}/braze_support)에 문의하여 추가 지원을 받으세요.
