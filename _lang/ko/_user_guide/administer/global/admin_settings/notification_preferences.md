...
| 에이전트 콘솔 오류 | [에이전트 콘솔 에이전트]({{site.baseurl}}/user_guide/brazeai/agents/)가 실행 한도에 도달했거나, 사용할 수 없게 된 모델을 사용하거나, LLM 제공업체와의 청구 오류가 발생했을 때(자체 API 키 사용 시에만) 수신자에게 알립니다. | 이메일 |
