...
1. 사용자에 대한 권한을 추가하려면 **IAM**(Identity and Access Management) > **Policies** > **Create Policy**로 이동하십시오.
2. 제한된 권한을 부여하는 **Create Your Own Policy**를 선택하여 Braze가 지정된 버킷에만 접근할 수 있도록 합니다.
3. 원하는 정책 이름을 지정하십시오.
4. 다음 코드 스니펫을 **Policy Document** 섹션에 입력하십시오. "INSERTBUCKETNAME"을 버킷 이름으로 교체해야 합니다. 이러한 권한이 없으면 통합이 자격 증명 확인에 실패하여 생성되지 않습니다.
