...
이 필드를 사용하여 필요한 최소 비밀번호 길이를 변경할 수 있습니다. 기본 최솟값은 8자입니다.

### 비밀번호 복잡성 {#password-complexity}

**복잡한 비밀번호 적용**을 선택하면 비밀번호에 다음 각 항목 중 하나 이상을 포함하도록 요구할 수 있습니다:
- 대문자
- 소문자
- 숫자
...
이 필드를 사용하여 Braze가 세션을 활성 상태로 유지하는 기간을 정의할 수 있습니다. Braze가 세션이 비활성 상태(정의된 시간(분) 동안 활동 없음)라고 판단하면 사용자를 로그아웃시킵니다. 입력할 수 있는 최대 시간(분)은 회사에 2단계 인증이 적용된 경우 10,080분(1주일에 해당)이며, 그렇지 않은 경우 최대 세션 지속 시간은 1,440분(24시간에 해당)입니다.

### SSO 인증 {#single-sign-on-sso-authentication}

사용자가 비밀번호 또는 SSO를 사용하여 로그인하는 것을 제한할 수 있습니다.

[SAML SSO]({{site.baseurl}}/user_guide/administer/global/saml_single_sign_on)의 경우, 고객은 적용하기 전에 SAML 설정을 구성해야 합니다. Google SSO를 사용하는 고객은 추가 작업 없이 보안 설정 페이지만 적용하면 됩니다.

## 대시보드 IP 허용 목록 설정 {#dashboard-ip-allowlisting}
...
2단계 인증은 모든 회사 사용자에게 필수입니다. 계정 로그인에 두 번째 신원 확인 단계를 추가하여 사용자 이름과 비밀번호만 사용하는 것보다 더 안전하게 보호합니다. 대시보드에서 2단계 인증을 지원할 수 없는 경우 고객 성공 매니저에게 문의하세요.

2단계 인증이 켜져 있으면:

- 비밀번호를 입력하는 것 외에도 Braze 계정에 로그인할 때 인증 코드를 입력해야 합니다. 코드는 인증 앱, 이메일 또는 SMS를 통해 전송될 수 있습니다.
...
재설정을 통해 인증 앱 문제, 이메일 인증 미발송, SMS 장애 또는 사용자 오류로 인한 로그인 실패 등 일반적인 인증 문제를 해결할 수 있습니다.

### 회사 수준의 2FA 요구 사항 {#requirements-for-2fa-at-the-company-level}

먼저 **회사 설정** > **보안 설정** > **2단계 인증**으로 이동하여 대시보드에서 2FA가 활성화되어 있는지 확인합니다. 토글이 회색이면 회사에서 2FA가 켜지지 않은 것이며 모든 회사 사용자에게 필수가 아닙니다.

#### 2FA가 필수가 아닌 경우 사용자 옵션 {#user-options-when-2fa-isnt-mandatory}

회사 수준에서 2FA가 적용되지 않는 경우 개별 사용자는 계정 설정 페이지에서 직접 2FA를 설정할 수 있습니다. 이 경우 사용자가 설정하지 않아도 계정이 잠기지 않습니다. 사용자 관리 페이지를 확인하여 어떤 사용자가 2FA를 활성화했는지 확인할 수 있습니다.
...
## 상승된 접근 권한 {#elevated-access}

상승된 접근 권한은 Braze 대시보드에서 민감한 작업에 대해 추가적인 보안 계층을 제공합니다. 이 기능이 활성화되면, 사용자는 Segment를 내보내거나 API 키를 조회하기 전에 계정을 다시 인증해야 합니다. 상승된 접근 권한을 사용하려면 **설정** > **관리자 설정** > **보안 설정**으로 이동하여 토글을 켜세요.

사용자가 재인증에 실패하면, 이전 화면으로 리디렉션되며 해당 민감한 작업을 계속 진행할 수 없습니다. 재인증에 성공하면, 로그아웃하지 않는 한 이후 1시간 동안은 다시 인증할 필요가 없습니다.
...
워크스페이스에 적합한 기본 설정을 결정하는 것은 사용자의 책임입니다. 적절한 설정을 결정하는 가장 좋은 방법은 Braze로 이벤트 데이터를 전송하는 내부 팀 및 Braze에서 메시지 엑스트라를 사용하는 팀과 함께 해당 필드에 PII가 포함될 수 있는지 검토하는 것입니다.
...
2FA가 활성화되어 있지만 **활성화** 버튼을 선택해도 아무 반응이 없는 경우, 브라우저가 SMS를 통해 인증 코드를 전송하는 데 필요한 리디렉션을 차단하고 있기 때문일 수 있습니다. 이 문제를 해결하려면 다음 단계를 따르세요:
...
3. 여전히 SMS를 수신하지 못하는 경우, 가정 네트워크나 회사 외부 Wi-Fi 등 다른 네트워크 연결을 사용해 보세요. 회사 네트워크에는 SMS 전송을 방해하는 보안 정책이 있을 수 있습니다.
