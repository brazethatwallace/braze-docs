...
다음 각 항목 중 하나 이상을 포함하도록 비밀번호를 요구하려면 **복잡한 비밀번호 적용**을 선택합니다:
- 대문자
- 소문자
- 숫자
...
이 필드를 사용하여 Braze가 세션을 활성 상태로 유지하는 기간을 정의합니다. Braze가 세션이 비활성 상태라고 판단하면(정의된 시간(분) 동안 활동이 없는 경우) 사용자를 로그아웃합니다. 입력할 수 있는 최대 시간(분)은 회사에 2단계 인증이 적용된 경우 10,080분(1주일에 해당)이며, 그렇지 않으면 최대 세션 지속 시간은 1,440분(24시간에 해당)입니다.

### SSO 인증 {#single-sign-on-sso-authentication}

비밀번호 또는 SSO를 사용한 사용자 로그인을 제한할 수 있습니다.
...
2단계 인증은 모든 회사 사용자에게 필수입니다. 계정 로그인에 두 번째 신원 확인 단계를 추가하여 사용자 이름과 비밀번호만 사용하는 것보다 더 안전합니다. 대시보드에서 2단계 인증을 지원할 수 없는 경우 고객 성공 매니저에게 문의하세요.

2단계 인증이 켜져 있으면:

- 사용자는 비밀번호를 입력하는 것 외에도 Braze 계정에 로그인할 때 인증 코드를 입력해야 합니다. 코드는 인증 앱, 이메일 또는 SMS를 통해 전송될 수 있습니다.
...
대시보드 회사 아래에 여러 계정을 가진 고객은 쿠키가 특정 기기에 연결되어 있기 때문에 이 기능을 사용할 때 문제가 발생할 수 있습니다. 사용자가 동일한 기기를 사용하여 여러 계정에 로그인하면 해당 기기에서 이전에 인증된 계정의 쿠키가 대체됩니다. Braze는 하나의 기기가 하나의 계정에 연결되는 것을 기대하며, 하나의 기기로 여러 계정을 사용하는 것은 지원하지 않습니다.
...
재설정은 인증 앱 문제, 이메일 인증 미발송, SMS 장애 또는 사용자 오류로 인한 로그인 실패 등 일반적인 인증 문제를 해결할 수 있습니다.

### 회사 수준의 2FA 요구 사항 {#requirements-for-2fa-at-the-company-level}

먼저 **설정** > **회사 설정** > **관리자 설정** > **보안 설정** > **2단계 인증**으로 이동하여 대시보드에서 2FA가 활성화되어 있는지 확인합니다. 토글이 회색이면 회사에 2FA가 켜져 있지 않으며 모든 회사 사용자에게 필수가 아닙니다.

#### 2FA가 필수가 아닌 경우의 사용자 옵션 {#user-options-when-2fa-isnt-mandatory}

회사 수준에서 2FA가 적용되지 않는 경우 개별 사용자는 계정 설정 페이지에서 직접 2FA를 설정할 수 있습니다. 이 경우 사용자가 설정하지 않아도 계정이 잠기지 않습니다. **회사 사용자** 목록을 확인하여 어떤 사용자가 2FA를 활성화했는지 확인할 수 있습니다.

#### 2FA가 필수인 경우의 요구 사항 {#requirements-when-2fa-is-mandatory}

회사 수준에서 2FA가 적용되는 경우 로그인 시 자체 계정에 2FA를 설정하지 않은 사용자는 대시보드에서 잠기게 됩니다. 사용자는 접근 권한을 유지하려면 2FA 설정을 완료해야 합니다.
...
상승된 액세스는 Braze 대시보드에서 민감한 작업에 대해 추가적인 보안 계층을 제공합니다. 활성화되면 사용자는 Segment를 내보내거나 API 키를 조회하기 전에 계정을 다시 인증해야 합니다. 상승된 액세스를 사용하려면 **설정** > **회사 설정** > **관리자 설정** > **보안 설정**으로 이동하여 토글을 켜세요.

사용자가 재인증할 수 없는 경우, 이전 위치로 리디렉션되며 민감한 작업을 계속 진행할 수 없습니다. 재인증에 성공하면 로그아웃하지 않는 한 이후 1시간 동안은 다시 인증할 필요가 없습니다.
...
이 설정을 사용하여 사용자 삭제 프로세스 중 Braze가 이벤트의 특정 필드를 삭제할지 여부에 대한 기본 설정을 지정할 수 있습니다. 이 기본 설정은 Braze가 삭제한 사용자의 데이터에만 적용됩니다.

사용자가 삭제되면 Braze는 이벤트 데이터에서 모든 PII를 제거하지만, 분석 목적으로 익명화된 데이터는 유지합니다. 일부 사용자 정의 필드에는 최종 사용자 정보를 Braze로 전송하는 경우 PII가 포함될 수 있습니다. 이러한 필드에 PII가 포함되어 있다면, 삭제된 사용자의 이벤트 데이터를 Braze가 익명화할 때 해당 데이터를 삭제하도록 선택할 수 있습니다. 필드에 PII가 포함되어 있지 않다면 분석을 위해 유지할 수 있습니다.

워크스페이스에 적합한 기본 설정을 결정하는 것은 사용자의 책임입니다. 적절한 설정을 결정하는 가장 좋은 방법은 Braze로 이벤트 데이터를 전송하는 내부 팀 및 Braze에서 메시지 추가 정보를 사용하는 팀과 함께 해당 필드에 PII가 포함될 수 있는지 검토하는 것입니다.
...
워크스페이스별로 기본 설정을 커스터마이즈하려면 기본값과 다른 설정을 가진 기본 설정 그룹을 추가할 수 있습니다. 추가 기본 설정 그룹에 포함되지 않은 워크스페이스(향후 생성되는 워크스페이스 포함)에는 기본 설정이 적용됩니다.

![워크스페이스별 데이터 삭제 기본 설정 커스터마이즈 토글이 켜진 데이터 삭제 기본 설정 섹션.]({% image_buster /assets/img/deletion_preferences_1.png %})
...
2FA가 활성화되어 있지만 **Enable** 버튼을 선택해도 아무 반응이 없는 경우, 브라우저가 SMS를 통해 인증 코드를 전송하는 데 필요한 리디렉션을 차단하고 있기 때문일 수 있습니다. 이 문제를 해결하려면 다음 단계를 따르세요:
...
3. 여전히 SMS를 수신하지 못하는 경우, 홈 네트워크나 회사 외부 Wi-Fi 등 다른 네트워크 연결을 사용해 보세요. 회사 네트워크에는 SMS 전송을 방해하는 보안 정책이 있을 수 있습니다.
...
인증 및 접근 권한에 대한 자세한 내용은 다음을 참조하세요:

- [SAML 및 SSO]({{site.baseurl}}/user_guide/administer/global/saml_single_sign_on)에서 ID 공급자를 통한 SSO를 설정할 수 있습니다.
- [권한]({{site.baseurl}}/user_guide/administer/global/user_management/permissions)에서 사용자가 대시보드에서 수행할 수 있는 작업을 제어할 수 있습니다.