---
nav_title: 개인 정보 보호
article_title: 개인 정보 보호
page_order: 6
layout: dev_guide
description: "스팸 규정을 이해하고, 사용자 동의를 관리하며, 데이터 보호 법률 및 규정을 준수하여 Braze를 사용하는 방법을 알아보세요."

guide_top_header: "개인 정보 보호"
guide_top_text: "이 리소스를 활용하여 스팸 규정을 이해하고, 사용자 동의를 관리하며, 관련 데이터 보호 법률 및 규정을 준수하여 Braze를 사용할 수 있도록 하세요."
page_type: reference

guide_featured_title: "문서"
guide_featured_list:
- name: 스팸 규정
  link: /docs/user_guide/administer/global/privacy/spam_regulations
  image: /assets/img/braze_icons/message-check-circle.svg
- name: 동의 관리
  link: /docs/user_guide/administer/global/privacy/managing_consent
  image: /assets/img/braze_icons/user-square.svg
- name: 데이터 보호 기술 지원
  link: /docs/dp-technical-assistance
  image: /assets/img/braze_icons/database-01.svg
- name: SMS 법률 및 규정
  link: /docs/user_guide/channels/sms_mms_and_rcs/compliance_and_delivery/laws_and_regulations
  image: /assets/img/braze_icons/channel-sms.svg
---

## 법적 면책 조항

여기에 제공된 정보는 교육 목적으로만 제공되며 법률 자문으로 해석되어서는 안 됩니다. Braze는 모든 고객이 데이터 보호법 및 규정을 준수하여 Braze를 사용할 수 있도록 법률 자문을 구할 것을 권장합니다.

<br><br>