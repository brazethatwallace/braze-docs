...
| `Target URL` | Okta 관리자 대시보드에서 제공하는 로그인 URL입니다. **애플리케이션** > 내 애플리케이션 > **일반** 탭 > **앱 임베드 링크** > **임베드 링크**에서 찾을 수 있습니다. |
| `Certificate` | `x.509` PEM 인코딩 인증서는 ID 공급자가 제공합니다. 이 필드에 복사하여 붙여넣어야 합니다. Okta에서 **SAML 서명 인증서**로 이동하여 **작업** > **인증서 다운로드**를 선택하면 받을 수 있습니다. |
{: .reset-td-br-1 .reset-td-br-2 role="presentation" }
...
Okta에서 Braze SAML 앱의 **Sign On** 탭을 선택한 다음 **편집**을 클릭합니다.

그런 다음 `sso.saml.login` 권한이 있는 RelayState API 키를 **Default Relay State** 필드에 입력합니다.

![Okta Default RelayState가 Sign On 탭에 표시되어 있습니다.]({% image_buster /assets/img/Okta/okta2.png %}){: style="max-width:75%"}
