...
설정 시 로그인 URL과 어설션 소비자 서비스(ACS) URL을 제공해야 합니다.

| 요구 사항 | 세부 사항 |
|---|---|
| 어설션 소비자 서비스(ACS) URL | `https://<SUBDOMAIN>.braze.com/auth/saml/callback` <br><br> 유럽 연합 도메인의 경우 ACS URL은 `https://<SUBDOMAIN>.braze.eu/auth/saml/callback`입니다. <br><br> 일부 IdP에서는 이를 회신 URL, 로그인 URL, 오디언스 URL 또는 오디언스 URI라고도 합니다. |
| 엔티티 ID | 기본값은 `braze_dashboard`입니다. IdP에서 회사별 엔티티 ID가 필요한 경우 **보안 설정**에서 **커스텀 엔티티 ID**를 활성화하고 `braze_dashboard_<companyID>`를 사용하세요. |
| RelayState API 키 | **설정** > **설정 및 테스트** > **API 및 식별자**로 이동하여 **API 키** 탭을 열고 `sso.saml.login` 권한을 가진 API 키를 생성하세요. 생성된 API 키를 IdP 내에서 `RelayState` 매개변수로 입력합니다. 자세한 단계는 [RelayState 설정하기](#setting-up-your-relaystate)를 참조하세요. |
...
{% alert important %}
Okta를 ID 공급자로 사용할 계획이라면 [Okta 사이트](https://www.okta.com/integrations/braze/)에서 제공하는 사전 구축된 통합을 사용해야 합니다.
{% endalert %}
...
계정 매니저가 계정에서 SAML SSO를 활성화한 후 **설정** > **회사 설정** > **관리자 설정** > **보안 설정**으로 이동하여 SAML SSO 섹션을 **켜기**로 토글합니다.
...
| 대상 URL | IdP 내에서 Braze를 설정한 후 제공됩니다.<br> 일부 IdP에서는 이를 SSO URL 또는 SAML 2.0 엔드포인트로 참조합니다. |
| 인증서 | ID 공급자가 제공하는 `x.509` 인증서입니다.|
{: .reset-td-br-1 .reset-td-br-2 aria-label="2단계: Braze 구성" }

### 커스텀 엔티티 ID {#custom-entity-id}

기본적으로 Braze는 엔티티 ID(일부 IdP에서는 Audience 또는 Audience URI라고도 함)로 `braze_dashboard`를 사용합니다. IdP에서 회사별 엔티티 ID가 필요한 경우:
...
## 커스텀 Entity ID 사용하기 {#using-a-custom-entity-id}

기본적으로 모든 Braze 대시보드는 공유 Entity ID인 `braze_dashboard`를 사용합니다. 커스텀 Entity ID를 사용하면 대시보드에 고유 식별자를 부여하여, ID 제공자(identity provider)가 로그인 요청이 해당 대시보드에 대한 것인지 확인할 수 있습니다. 이 기능은 동일한 ID 제공자 내에서 여러 Braze 회사에 SAML SSO를 설정할 때 유용합니다.

커스텀 Entity ID 사용은 선택 사항입니다. 활성화하지 않으면 대시보드는 계속 `braze_dashboard`를 사용합니다.

{% alert warning %}
미리 구성된 [Braze Okta 마켓플레이스 앱](https://www.okta.com/integrations/braze/)은 공유 Entity ID `braze_dashboard`를 강제 적용하며, 커스텀 Entity ID와 호환되지 않습니다. Braze Okta 마켓플레이스 앱으로 SAML SSO를 이미 설정한 경우, 커스텀 SAML 앱을 통해 Okta의 Entity ID 필드를 업데이트하지 않고 커스텀 Entity ID를 활성화하면 로그인이 중단되어 사용자가 대시보드에 접근하지 못할 수 있습니다. Okta에서 커스텀 Entity ID를 사용하려면 커스텀 SAML 앱을 대신 설정하세요.
{% endalert %}

### 1단계: 커스텀 Entity ID 활성화 {#step-1-turn-on-the-custom-entity-id}

**설정** > **관리자 설정** > **보안 설정**으로 이동한 후 SAML 싱글 사인온 섹션을 엽니다. **커스텀 Entity ID** 토글을 활성화합니다. Braze가 `braze_dashboard_<COMPANY_ID>` 형식으로 대시보드의 고유 Entity ID를 생성합니다. **커스텀 Entity ID** 옵션이 보이지 않는 경우 Braze 계정 매니저에게 문의하세요.

### 2단계: ID 제공자 업데이트 {#step-2-update-your-identity-provider}

생성된 Entity ID를 복사하여 ID 제공자의 Braze 애플리케이션에 있는 Entity ID 필드에 붙여넣습니다. 제공자에 따라 이 필드는 **Entity ID**, **Audience**, 또는 **Audience URI**로 표시될 수 있습니다.

{% alert important %}
Entity ID는 Braze와 ID 제공자 양쪽에서 반드시 일치해야 합니다. 양쪽이 동일한 값을 사용할 때까지 사용자는 SAML SSO로 로그인할 수 없습니다. 사용자가 잠기는 것을 방지하려면 이 페이지를 저장하기 전에 ID 제공자를 먼저 업데이트하세요.
{% endalert %}

### 3단계: 저장 및 테스트 {#step-3-save-and-test}

보안 설정을 저장하고 로그아웃한 후, ID 제공자를 통해 다시 로그인하여 커스텀 Entity ID로 로그인이 정상적으로 작동하는지 확인합니다.
...
4. **권한** 아래의 **SSO** 드롭다운을 펼치고 **sso.saml.login**을 체크합니다.
5. **API 키 생성**을 선택합니다.
...
일부 ID 공급자는 IdP 시작 로그인을 지원하며, 이 경우 사용자는 Braze 로그인 페이지 대신 IdP 포털에서 시작합니다. IdP 시작 로그인에는 유효한 RelayState API 키와 올바른 ACS URL 구성이 필요합니다. 공급자별 설정 가이드:
...
![최소 비밀번호 길이 8자, 비밀번호 재사용 3회로 설정된 '인증 규칙' 섹션의 예시. 비밀번호는 180일 후 만료되며, 사용자는 1,440분간 비활성 상태 시 로그아웃됩니다.]({% image_buster /assets/img/sso3.png %})

제한을 설정하면 회사의 Braze 사용자는 이전에 비밀번호로 로그인한 적이 있더라도 더 이상 비밀번호로 로그인할 수 없습니다.

{% alert important %}
SSO가 적용된 후에는 SSO 인증에 실패할 경우 대체 로그인 옵션이 없습니다. SSO 적용을 활성화하기 전에 SSO 구성이 올바른지, 모든 인증서가 최신이고 갱신되었는지, 보안 설정이 제대로 관리되고 있는지 확인하여 로그인 문제를 방지하세요.
{% endalert %}

## SAML 트레이스 획득하기 {#obtaining-a-saml-trace}

SSO와 관련된 로그인 문제가 발생하는 경우, SAML 트레이스를 획득하면 SAML 요청에서 전송되는 내용을 확인하여 SSO 연결 문제를 해결하는 데 도움이 됩니다.
...
!["SAML 트레이스 내보내기 기본 설정" 메뉴에서 "None" 옵션이 선택된 모습.]({% image_buster /assets/img/export_saml_trace_preferences.png %})

## 문제 해결 {#troubleshooting}

### 사용자의 이메일 주소가 올바르게 설정되어 있나요? {#is-the-users-email-address-correctly-set-up}

`ERROR_CODE_SSO_INVALID_EMAIL` 오류가 발생하는 경우, 사용자의 이메일 주소가 유효하지 않은 것입니다. SAML 트레이스에서 `saml2:Attribute Name="email"` 필드가 사용자가 로그인에 사용하는 이메일 주소와 일치하는지 확인합니다. Microsoft Entra ID(이전 Azure 액티브 디렉토리)를 사용하는 경우, 속성 매핑은 `email = user.userprincipalname`입니다.

이메일 주소는 대소문자를 구분하며, ID 공급자(예: Okta, OneLogin, Microsoft Entra ID 등)에서 구성된 주소를 포함하여 Braze에 설정된 주소와 정확히 일치해야 합니다.
...
[SAML 유효성 검사 도구](https://www.samltool.com/validate_response.php)를 사용하여 SAML 인증서를 검증할 수 있습니다. 만료된 SAML 인증서도 유효하지 않은 SAML 인증서라는 점에 유의합니다.

### 올바른 SAML 인증서(x.509 인증서)를 업로드했나요? {#did-you-upload-a-correct-saml-certificate-x509-certificate}

SAML 트레이스의 `ds:X509Certificate` 섹션에 있는 인증서가 Braze에 업로드한 인증서와 일치하는지 확인합니다. 여기에는 `-----BEGIN CERTIFICATE-----` 헤더와 `-----END CERTIFICATE-----` 푸터가 포함되지 않습니다.

### SAML 인증서(x.509 인증서)를 잘못 입력하거나 형식을 잘못 지정했나요? {#did-you-mistype-or-misformat-your-saml-certificate-x509-certificate}

Braze 대시보드에 제출한 인증서에 공백이나 추가 문자가 없는지 확인합니다.

Braze에 인증서를 입력할 때는 PEM(Privacy Enhanced Mail) 인코딩이 되어 있어야 하며, `-----BEGIN CERTIFICATE-----` 헤더와 `-----END CERTIFICATE-----` 푸터를 포함하여 올바른 형식이어야 합니다.
...
영향을 받는 사용자에게 [브라우저의 캐시와 쿠키를 삭제](https://its.uiowa.edu/services/how-clear-cache-and-cookies-your-web-browser)한 후, SAML SSO로 다시 로그인을 시도하도록 안내합니다.

### RelayState를 설정했나요? {#did-you-set-your-relaystate}

`ERROR_CODE_SSO_INVALID_RELAY_STATE` 오류가 발생하는 경우, RelayState가 잘못 구성되었거나 존재하지 않을 수 있습니다. 아직 설정하지 않았다면, IdP 관리 시스템에서 RelayState를 설정해야 합니다. 단계별 지침은 [RelayState 설정하기](#setting-up-your-relaystate)를 참조합니다.

### SSO 로그인에 성공하면 Braze 로그인 페이지로 돌아오나요? {#does-successful-sso-sign-in-return-you-to-the-braze-login-page}

이 문제는 RelayState가 올바르게 구성되지 않은 경우 발생할 수 있습니다. **설정** > **설정 및 테스트** > **API 및 식별자**에서 IdP 로그인용 API 키를 생성하고, 해당 API 키를 IdP의 `RelayState` 매개변수로 설정했는지 확인합니다. RelayState는 로그인하려는 회사 계정을 식별합니다. 단계별 지침은 [RelayState 설정하기](#setting-up-your-relaystate)를 참조합니다.

여전히 로그인할 수 없는 경우, 가능하다면 SAML 트레이스를 첨부하여 [Braze 지원팀에 문의]({{site.baseurl}}/braze_support)합니다. 트레이스 캡처에 대한 도움말은 [SAML 트레이스 수집하기](#obtaining-a-saml-trace)를 참조합니다.

### 사용자가 Okta와 Braze 사이에서 로그인 루프에 갇혀 있나요? {#is-the-user-stuck-in-a-sign-in-loop-between-okta-and-braze}

사용자가 Okta SSO와 Braze 대시보드 사이를 반복적으로 순환하며 로그인할 수 없는 경우, Okta에서 SSO URL 대상을 해당 [Braze 인스턴스]({{site.baseurl}}/user_guide/administer/personal/sdk_endpoints)(예: `https://dashboard-07.braze.com`)로 설정해야 합니다.
...
- [SAML 적시 프로비저닝을 설정]({{site.baseurl}}/user_guide/administer/global/saml_single_sign_on/saml_just_in_time_provisioning)하면 새 사용자가 첫 SSO 로그인 시 자동으로 Braze 계정을 생성할 수 있습니다.