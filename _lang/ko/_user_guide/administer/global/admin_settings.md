...
guide_top_text: "관리자 설정은 회사 전체에 적용되며 모든 워크스페이스와 사용자에게 영향을 미칩니다. 이 설정을 사용하여 회사 연락처 정보를 관리하고, 알림 환경설정을 구성하고, 인증 규칙 및 IP 허용 목록과 같은 보안 정책을 적용하고, Braze MCP 서버의 OAuth 액세스를 제어할 수 있습니다.<br><br>이 설정을 관리하려면 <b>설정</b> > <b>관리자 설정</b>으로 이동하세요. 대부분의 기능은 Braze 계정 관리자만 사용할 수 있습니다. 워크스페이스별 구성은 <a href='/docs/user_guide/administer/global/workspace_settings'>워크스페이스 설정</a> 을 참조하세요."

page_type: landing
description: "연락처 정보, 알림 환경설정, 보안 정책, OAuth 액세스를 포함하여 모든 워크스페이스에 적용되는 회사 전체 관리자 설정을 관리합니다."
...
{% alert warning %}
**관리자 설정** 섹션에서 변경한 내용은 회사에 연결된 모든 Braze 사용자 계정에 적용됩니다.
{% endalert %}
