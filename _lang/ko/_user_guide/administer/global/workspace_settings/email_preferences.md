...
**이메일 환경설정**은 대시보드의 **설정** > **워크스페이스 설정**에서 찾을 수 있습니다.

## 발송 설정 {#sending-configuration}

**발송 설정** 섹션의 이메일 설정은 이메일 Campaign에 포함되는 세부 정보를 결정합니다. 특히 이 설정은 사용자가 Braze에서 보낸 이메일을 받을 때 표시되는 내용과 주로 관련이 있습니다.
...
{% alert note %}
Apple 메일 클라이언트는 커스텀 표시 이름에 `@` 기호가 사용될 경우 이를 인식하지 않습니다. 메일함 제공자마다 사용자에게 표시 이름 주소가 표시되는 방식이 다르므로, 이메일 클라이언트에 따라 표시 이름이 다르게 나타날 수 있습니다.
{% endalert %}

#### Liquid로 개인화하기 {#personalize-with-liquid}

[Liquid]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/liquid)를 **발신 표시 이름**, **로컬 파트**, **도메인** 필드에서 사용하여 커스텀 속성을 기반으로 발신자 이름과 이메일 주소를 동적으로 템플릿화할 수도 있습니다. **도메인** 필드에서 Liquid를 사용하려면 이메일 Campaign의 **발송 정보** 옵션으로 이동하여 **발신 표시 이름 + 주소 커스터마이즈** 확인란을 선택해야 합니다.

![발신 표시 이름, 주소, 도메인을 커스터마이즈할 수 있는 필드가 있는 발송 설정]({% image_buster /assets/img/email_settings/email_campaign_domain.png %})
...
이 섹션에서 이메일 주소를 추가하면 이메일 Campaign의 회신 주소로 선택할 수 있습니다. **기본값으로 설정**을 선택하여 이메일 주소를 기본 주소로 만들 수도 있습니다. 이러한 이메일 주소는 이메일 Campaign을 작성할 때 **발송 정보** 옵션에서 사용할 수 있습니다.

![여러 회신 주소를 입력할 수 있는 필드가 있는 회신 주소 섹션]({% image_buster /assets/img/email_settings/reply_to_address.png %}){: style="max-width:75%;" }

{% alert note %}
Braze 발송 도메인은 수신 이메일을 수락하지 않습니다. 수신자가 Braze 설정 발송 도메인에서 보낸 이메일에 답장하면 `550 5.7.1 relaying denied` 오류와 함께 해당 답장이 반송됩니다. 회신 주소는 발신 주소와 동일한 도메인을 공유할 필요가 없습니다. 답장을 수신해야 하는 경우(예: 캘린더 초대 확인을 수집하려는 경우) 발송용으로 구성되지 않고 메일 수신이 가능한 받은편지함이 설정된 하위 도메인을 사용하세요.
{% endalert %}

#### Liquid로 개인화하기

[Liquid]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/liquid)를 **회신 주소** 필드에서 사용하여 커스텀 속성을 기반으로 회신 주소를 동적으로 템플릿화할 수도 있습니다. 예를 들어, 조건 로직을 사용하여 다른 지역이나 부서로 답장을 보낼 수 있습니다:
...
{% alert tip %}
콘텐츠 블록을 사용하여 **회신 주소**를 채우는 경우, 최종 렌더링 값이 유효한 이메일 주소이고 `@`를 포함하는지 확인하세요. Braze는 최종 값이 발송 시점에야 결정되므로 설정 저장 시 이를 검증할 수 없습니다.

- 콘텐츠 블록에 로컬 파트(`@` 앞의 텍스트)와 도메인이 별도로 저장되어 있는 경우, 필드를 하나의 주소로 구성하세요(예: {% raw %}`{{content_blocks.${reply_to_local}}}@{{content_blocks.${reply_to_domain}}}`{% endraw %}).
{% endalert %}
...
이메일 열람 추적 픽셀은 보이지 않는 1 x 1&nbsp;px 크기의 이미지이며, 이메일 HTML에 자동으로 삽입됩니다. 이 픽셀은 Braze가 사용자가 이메일을 열람했는지 감지하는 데 도움을 줍니다. 사용자의 이메일 클라이언트가 추적 픽셀에 요청을 보내면, 해당 요청에는 IP 주소, 사용자 에이전트, 타임스탬프 등의 정보가 포함될 수 있습니다. 이메일 열람 정보는 해당 열람율을 파악하여 효과적인 마케팅 전략을 수립하는 데 매우 유용합니다.

### 배치 위치 {#placement}

Braze의 기본 동작은 이메일 하단, 일반적으로 `<body>` 태그 안에 추적 픽셀을 추가하는 것입니다. 대부분의 사용자에게 이 위치가 픽셀을 배치하기에 가장 이상적인 곳입니다.

픽셀은 이미 시각적 변화를 최소화하도록 스타일링되어 있지만, 의도하지 않은 시각적 변화가 발생하더라도 이메일 하단에서는 가장 눈에 띄지 않습니다. 이는 SendGrid 및 SparkPost와 같은 이메일 서비스 공급자에서도 기본 설정입니다.

예상치 못한 동작을 줄이려면 Liquid를 `<html>` 태그 내부에 유지하세요. 중첩되거나 중복된 문서 수준 태그는 이메일이 파싱되는 방식과 픽셀이 배치되는 위치를 변경하여 열람 추적 및 레이아웃에 영향을 줄 수 있습니다. 자세한 내용은 [Liquid 사용하기]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/liquid/using_liquid)를 참조하세요.

### 배치 위치 업데이트 {#update-the-placement}

Braze는 현재 ESP의 기본 열람 추적 픽셀 위치(이메일 `<body>`의 마지막 태그)를 재정의하여 `<body>`의 첫 번째 태그로 이동하는 기능을 지원합니다.

![SendGrid, SparkPost 또는 Amazon SES에 대해 이동 옵션이 있는 열람 추적 픽셀 섹션]({% image_buster /assets/img/open_pixel.png %}){: style="max-width:80%;" }

위치를 변경하려면:

1. Braze에서 **설정** > **워크스페이스 설정** > **이메일 환경설정**으로 이동합니다.
2. 다음 옵션 중 하나를 선택합니다: **Move for SendGrid**, **Move for SparkPost** 또는 **Move for Amazon SES**
3. **저장**을 선택합니다.

저장하면 Braze가 ESP에 특별 지침을 전송하여 모든 HTML 이메일의 상단에 열람 추적 픽셀을 배치합니다.
...
## 이메일 제목줄 추가 {#append-email-subject-lines}

토글을 사용하여 테스트 및 시드 이메일 제목줄에 "[TEST]" 및 "[SEED]"를 포함할 수 있습니다. 이를 통해 테스트로 발송된 이메일 Campaign을 쉽게 식별할 수 있습니다.

![테스트 및 시드 이메일 제목줄에 TEST 및 SEED 접두사를 추가하는 워크스페이스 이메일 환경설정 토글.]({% image_buster /assets/img/email_settings/test_and_seed_email_subject_line.png %}){: style="max-width:70%;"}
...
상업용 이메일의 경우, [CAN-SPAM법](https://en.wikipedia.org/wiki/CAN-SPAM_Act_of_2003)에 따라 모든 상업용 이메일에 탈퇴 옵션을 포함해야 합니다. 커스텀 푸터 설정을 사용하면 이메일 옵트아웃 푸터를 커스터마이즈하면서도 CAN-SPAM을 준수할 수 있습니다. 규정을 준수하려면 이 워크스페이스의 Campaign으로 발송되는 모든 이메일에 커스텀 푸터를 추가해야 합니다.

이메일 메시징용 커스텀 푸터를 만들 때 다음 요구 사항에 유의하세요:
- 탈퇴 URL과 실제 우편 주소를 포함해야 합니다.
- 100 KB 미만이어야 합니다.

![CAN-SPAM 규정 준수를 위한 탈퇴 링크 및 우편 주소 필드가 포함된 커스텀 이메일 푸터 편집기.]({% image_buster /assets/img/email_settings/custom_footer.png %})
...
Braze에서는 자체 HTML을 사용하여 **커스텀 탈퇴 페이지**를 설정할 수 있습니다. 이 페이지는 사용자가 이메일 하단에서 탈퇴를 선택한 후에 표시됩니다. 이 페이지는 750 KB 미만이어야 합니다.

![사용자가 이메일 탈퇴 후 표시되는 페이지의 커스텀 탈퇴 페이지 HTML 편집기 및 미리보기.]({% image_buster /assets/img/email_settings/custom_unsubscribe.png %})
...
자체 HTML을 사용하여 커스텀 옵트인 페이지를 만들 수 있습니다. 이메일에 이 페이지를 포함하면 사용자 라이프사이클 전반에 걸쳐 브랜딩과 메시지를 일관되게 유지할 수 있어 특히 유용합니다. 이 페이지는 750 KB 미만이어야 합니다.
...
{% alert tip %}
구독 페이지 또는 푸터의 **미리보기** 섹션에서 **미리보기 링크 복사**를 선택하면 임의의 사용자에게 이메일 푸터, 탈퇴 페이지 또는 옵트인 페이지가 어떻게 표시되는지 확인할 수 있는 공유 가능한 미리보기 링크를 생성하고 복사할 수 있습니다. 자세한 내용은 [공유 가능한 미리보기]({{site.baseurl}}/user_guide/messaging/governance/shareable_preview)를 참조하세요.
{% endalert %}
...
아니요, 이는 RFC 8058을 준수하지 않으므로 Yahoo와 Gmail의 원클릭 수신 거부 요구 사항에 대한 규정을 충족하지 못합니다.
{% enddetails %}

{% details 환경설정 센터를 작성할 때 "이메일 본문에 수신 거부 링크가 포함되어 있지 않습니다"라는 오류 메시지가 표시되는 이유는 무엇인가요? %}
환경설정 센터는 수신 거부 링크로 간주되지 않습니다. CAN-SPAM 규정을 준수하려면 이메일 수신자에게 모든 상업용 이메일의 수신을 거부할 수 있는 옵션이 제공되어야 합니다.
{% enddetails %}

{% details 원클릭 수신 거부 설정을 활성화한 후 이전 이메일 Campaigns 및 Canvases를 편집해야 하나요? %}
메시지 수준의 원클릭 list-unsubscribe 설정에 대한 사용 사례가 없는 경우, **이메일 환경설정**에서 설정이 켜져 있는 한 별도의 조치가 필요하지 않습니다. Braze는 모든 발신 마케팅 및 프로모션 메시지에 원클릭 수신 거부 헤더를 자동으로 추가합니다. 그러나 메시지별로 원클릭 수신 거부 동작을 구성해야 하는 경우, 이전 이메일 Campaigns 및 캔버스 단계를 적절히 업데이트해야 합니다.
{% enddetails %}

{% details 원본 메시지 또는 원시 데이터에서 list-unsubscribe 및 원클릭 수신 거부 헤더를 확인할 수 있는데, Gmail이나 Yahoo에서 수신 거부 버튼이 표시되지 않는 이유는 무엇인가요? %}
Gmail과 Yahoo는 list-unsubscribe 또는 원클릭 수신 거부 헤더의 표시 여부를 최종적으로 결정합니다. 신규 발신자이거나 발송자 평판이 낮은 발신자의 경우, 수신 거부 버튼이 표시되지 않을 수 있습니다.
{% enddetails %}

{% details 커스텀 원클릭 수신 거부 헤더는 Liquid를 지원하나요? %}
네, Liquid와 조건 로직이 지원되므로 헤더에 동적 원클릭 수신 거부 URL을 사용할 수 있습니다.
{% enddetails %}

{% alert tip %}
조건 로직을 추가하는 경우, URL에 공백을 추가하는 출력 값이 생기지 않도록 주의하세요. Braze는 이러한 공백을 제거하지 않습니다.
{% endalert %}
...
A/B 테스트가 포함된 Campaign의 경우, Braze는 사용자에게 배리언트 중 하나를 무작위로 전송합니다. 동일한 Campaign에 서로 다른 두 개의 구독 그룹이 설정되어 있는 경우(배리언트 A는 구독 그룹 A로 설정, 배리언트 B는 구독 그룹 B로 설정), 구독 그룹 B에만 가입한 사용자가 배리언트 B를 수신한다고 보장할 수 없습니다. 이미 옵트아웃한 구독 그룹에서 사용자가 수신 거부하는 시나리오가 발생할 수 있습니다.
{% enddetails %}

{% details 이메일 환경설정에서 이메일 수신 거부 헤더 설정이 꺼져 있지만, Campaign의 전송 정보에서 원클릭 list-unsubscribe 설정이 "워크스페이스 기본값 사용"으로 되어 있습니다. 버그인가요? %}
아니요. 워크스페이스 설정이 꺼져 있고 메시지 설정이 **워크스페이스 기본값 사용**으로 설정되어 있는 경우, Braze는 **이메일 환경설정**에서 구성된 사항을 따릅니다. 즉, 해당 Campaign에 원클릭 수신 거부 헤더를 추가하지 않습니다.
{% enddetails %}

{% details 구독 그룹이 보관 처리되면 어떻게 되나요? 전송된 이메일의 원클릭 수신 거부가 중단되나요? %}
원클릭에 대한 **전송 정보**에서 참조된 구독 그룹이 보관 처리되더라도 Braze는 여전히 원클릭 수신 거부를 처리합니다. 해당 구독 그룹은 대시보드(Segment 필터, 고객 프로필 및 유사한 영역)에 더 이상 표시되지 않습니다.
{% enddetails %}

{% details 원클릭 수신 거부 설정을 이메일 템플릿에서도 사용할 수 있나요? %}
아니요, 현재 이메일 템플릿에 대해 이 기능을 추가할 계획은 없습니다. 이러한 템플릿은 발송 도메인에 할당되지 않기 때문입니다. {% multi_lang_include product_feedback_cta.md context="gap" feature="per-domain sending for email templates" %}
{% enddetails %}

{% details 이 기능은 커스텀 옵션에 추가된 원클릭 수신 거부 URL이 유효한지 확인하나요? %}
아니요, Braze 대시보드에서 링크를 확인하거나 검증하지 않습니다. 발송 전에 URL을 충분히 테스트하세요.
{% enddetails %}