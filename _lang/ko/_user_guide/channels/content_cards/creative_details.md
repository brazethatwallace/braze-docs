...
article_title: 콘텐츠 카드 크리에이티브 세부 정보
page_order: 2
description: "이 문서에서는 세 가지 표준 콘텐츠 카드 유형에 대한 이미지 크기 권장 사항 및 해제 동작과 같은 크리에이티브 세부 정보를 다룹니다."
channel:
  - content cards
tool: Media
...
> 콘텐츠 카드와 카드가 위치한 피드의 커스터마이징은 캠페인 생성 과정에서 수행할 수 없습니다. 엔지니어 및 개발자와 협력하여 카드를 구축하고 커스터마이징해야 합니다. 기술적인 세부 정보는 [개발자 설명서]({{site.baseurl}}/developer_guide/getting_started/customization_overview)를 참조하세요.
...
![권장 세부 정보가 포함된 이미지 전용 콘텐츠 카드 이미지와 이미지 전용 예시]({% image_buster /assets/img/content_card_banner.png %}){: style="max-width:45%;border:0;"}
...
콘텐츠 카드는 기본적으로 텍스트와 이미지(GIF 포함)를 지원합니다. 현재 다른 글꼴 색상이나 여러 이미지와 같은 카드의 커스텀 스타일은 대시보드에서 설정할 수 없습니다. 통합 과정에서 콘텐츠 카드와 피드에 커스텀 스타일을 적용할 수 있습니다. 자세한 내용은 Braze SDK의 [카드 커스터마이징]({{site.baseurl}}/developer_guide/content_cards/customizing_cards/)을 참조하세요.
...
{% alert tip %}
사용자가 관련 행동을 취할 때 해제되도록 설정하여 콘텐츠 카드의 관련성을 유지하세요. 예를 들어, 프로모션 콘텐츠 카드를 사용자가 구매하는 즉시 해제되도록 설정하면 이미 구매한 항목에 대한 제안을 계속 보지 않게 됩니다.
{% endalert %}
