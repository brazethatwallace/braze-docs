...
Campaign은 단순한 단일 메시징(예: 하나의 메시지로 사용자에게 제품을 알리는 것)에 사용합니다. Canvases는 다단계 사용자 여정(예: 시간에 따른 사용자 행동을 기반으로 맞춤형 제품 추천을 보내는 것)에 사용합니다.
...
7. **전달 설정**에서 필요에 따라 이 단계의 오디언스 또는 수신자를 필터링합니다. Segments를 지정하고 추가 필터를 추가하여 오디언스를 더 세밀하게 조정할 수 있습니다. 오디언스 옵션은 지연 후 메시지가 전송되는 시점에 확인됩니다.
...
각 메시지 유형과 플랫폼에는 고유한 권장 비율과 요구 사항이 있을 수 있으므로, 이미지를 의뢰하거나 처음부터 만들기 전에 반드시 확인하세요. 콘텐츠 카드 메시지 필드는 총 크기가 2&nbsp;KB로 제한된다는 점을 유의하세요.
...
#### 콘텐츠 카드 크기 제한 {#size-limitations-for-content-cards}
...
각 사용자는 주어진 시점에 피드에 최대 250개의 만료되지 않은 Content Cards를 보유할 수 있습니다. 이 제한을 초과하면 Braze는 읽지 않은 카드라도 가장 오래된 카드부터 반환을 중지합니다. 닫은 카드도 이 제한에 포함되므로, 닫은 카드가 많으면 오래된 카드에 사용할 수 있는 공간이 줄어듭니다.

카드 제한 관련 문제를 방지하기 위해 다음 모범 사례를 권장합니다:

- **더 짧은 만료 날짜를 사용하세요:** 시간에 민감한 Campaign(예: 주말 세일)의 경우 특정 만료 날짜를 설정하세요. 이렇게 하면 카드가 피드에서 자동으로 제거되어 더 이상 관련이 없는 후에는 제한에 포함되지 않습니다.
- **실행 기반 제거를 활용하세요:** 트랜잭션 또는 목표 기반 카드에 대해 제거 이벤트를 설정하세요. 예를 들어, 사용자에게 프로필 완성을 요청하는 카드는 `profile_completed` 이벤트가 기록되는 즉시 제거되어야 합니다.
...
흔히 혼동되는 부분은 Campaign 재자격과 모든 Content Cards의 자동 30일 만료 간의 상호작용입니다.

모든 Content Cards는 전송되거나 제거된 후 30일이 지나면 Braze 시스템에서 자동으로 삭제됩니다. 재자격이 **비활성화**된 장기 실행 반복 Campaign이 있는 경우, 사용자가 30일 후에 동일한 카드를 다시 받을 수 있습니다. 원래 카드가 삭제되면 시스템은 해당 사용자가 Campaign을 받은 기록을 더 이상 볼 수 없어, 다음 세션에서 다시 자격이 부여됩니다.

사용자가 특정 Campaign의 메시지를 한 번만 받도록 하려면, 이 Campaign의 메시지를 받지 않은 사용자에 대한 오디언스 필터를 Campaign 또는 캔버스 단계에 추가하세요. 이 필터는 장기 실행 Campaign에서 중복 전송을 방지하는 가장 신뢰할 수 있는 방법입니다.

### 라이브 Content Cards 관리 {#managing-live-content-cards}

Content Cards가 전송된 후에는 사용자에게 전달될 준비가 된 "받은편지함"에 대기합니다(이메일과 유사한 방식). 콘텐츠가 콘텐츠 카드에 가져와진 후(표시 시점), 수명 동안 변경할 수 없습니다. 이는 연결된 콘텐츠를 통해 API를 호출하고 엔드포인트의 데이터가 변경되더라도 마찬가지입니다. 이 데이터는 업데이트되지 않습니다. 새 사용자에게 전송을 중지하고 사용자의 피드에서 제거하는 것만 가능합니다. Campaign을 수정하면 향후 전송되는 카드에만 업데이트가 적용됩니다.
...
첫 번째 방법은 Campaign을 아카이브하고 새로 복제된 Campaign을 시작하는 것입니다:
...
##### 옵션 2: 동일한 Campaign을 중지하고 다시 시작 {#option-2-stop-and-relaunch-the-same-campaign}

{% alert tip %}
알림 센터나 메시지 받은편지함의 고유한 메시지(예: 프로모션)에 대해, 분석을 통합하는 것이 중요하거나, 메시지의 적시성이 중요하지 않은 경우(예: 기존 수신자가 업데이트된 카드를 보기 전에 자격 기간을 기다릴 수 있는 경우) 이 옵션을 사용하는 것을 권장합니다.
{% endalert %}

이 방법은 모든 분석을 단일 Campaign에 통합합니다. 새로 자격이 부여된 사용자는 새 카드를 받지만, 기존 수신자에 대한 업데이트는 재자격이 될 때까지 지연됩니다:

1. Campaign을 중지하고, 메시지가 표시되면 **다음 동기화 후 카드 제거**를 선택합니다.
2. 필요에 따라 Campaign을 편집합니다.
3. Campaign을 다시 시작합니다.

###### 영향

* **기존 수신자:** 이미 카드를 받은 사용자는 재자격이 될 때까지 업데이트된 카드를 받지 못합니다. 재자격이 비활성화된 경우 새 카드를 받지 못합니다.
* **보고:** 하나의 Campaign에 시작된 카드 버전의 모든 보고 분석이 포함됩니다. Braze는 시작된 버전 간을 구분하지 않습니다.

예를 들어, 세션 시작 시 트리거되는 Campaign이 있고 재자격이 30일로 설정되어 있다고 가정합니다. 사용자가 2일 전에 Campaign을 받았고 문구를 변경하고 싶습니다. 먼저 Campaign을 중지하고 피드에서 카드를 제거합니다. 그런 다음 새 문구로 Campaign을 다시 게시합니다. 사용자가 다른 세션을 시작하면 28일 후에 새 카드를 받게 됩니다.
...
1. Content Cards Campaign을 열고 **캠페인 중지**를 선택합니다.
2. 메시지가 표시되면 **다음 동기화 후 카드 제거**를 선택합니다. 다음 피드 새로고침 시 카드가 제거됩니다.

##### 자동 카드 제거 {#action-based-card-removal}

구매 완료나 기능 활성화와 같은 특정 동작을 사용자가 수행할 때 카드를 자동으로 제거할 수 있습니다.

Campaign 또는 캔버스 단계에서 제거 이벤트를 지정합니다. 사용자가 해당 이벤트를 수행하면 Braze가 이벤트를 처리한 후 다음 새로고침 시 피드에서 카드가 제거됩니다.
...
{% alert tip %}
사용자의 피드에서 카드를 제거해야 하는 여러 커스텀 이벤트와 구매를 지정할 수 있습니다. 사용자가 이러한 동작 중 **하나라도** 수행하면 Campaign의 카드에서 전송된 기존 카드가 모두 제거됩니다. 향후 자격이 있는 카드는 메시지의 스케줄에 따라 계속 전송됩니다.
{% endalert %}
