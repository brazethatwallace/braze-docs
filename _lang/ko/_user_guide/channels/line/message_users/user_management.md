...
| 회사가 `/users/track` 엔드포인트를 사용하고 `native_line_id` 속성을 지정함 | 아니요 | 지정된 사용자에 대한 고객 프로필이 없는 경우([`external_id`, `user_alias`, `braze_id` 또는 `email`로 지정]({{site.baseurl}}/api/objects_filters/user_attributes_object#identifier-resolution)):<br>- `native_line_id`가 지정된 값으로 설정됩니다<br>- 요청에 지정된 다른 모든 속성이 고객 프로필에 설정됩니다 |
| 회사가 `/users/track` 엔드포인트를 사용하고 `native_line_id` 속성을 지정함 | 예 | 지정된 사용자에 대한 고객 프로필이 있는 경우([`external_id`, `user_alias`, `braze_id` 또는 `email`로 지정]({{site.baseurl}}/api/objects_filters/user_attributes_object#identifier-resolution)):<br>- `native_line_id`가 지정된 값으로 설정됩니다<br>- 요청에 지정된 다른 모든 속성이 고객 프로필에 설정됩니다<br>- 여러 프로필이 동일한 `native_line_id`를 가질 수 있습니다 |
