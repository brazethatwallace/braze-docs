...
- 프리미엄 또는 인증 계정 상태(기존 팔로워 동기화에 필요)
...
일관된 사용자 업데이트를 설정하고, 기존 사용자의 LINE ID를 가져오고, 이를 모두 LINE의 구독 상태와 동기화하려면:
...
{% alert note %}
단일 워크스페이스에는 하나의 LINE 계정만 사용할 수 있습니다. 여러 LINE 계정이 있는 경우 각각 다른 워크스페이스에서 사용하는 것을 권장합니다.
{% endalert %}
...
{% alert note %}
구독 그룹 상태는 지정할 필요가 없으며 무시됩니다. LINE이 사용자 구독 상태의 정보 소스이며, 구독 동기화 도구 또는 이벤트 업데이트를 통해 Braze에 동기화됩니다.
{% endalert %}

## 2단계: LINE 채널 통합 {#step-2-integrate-line-channel}

통합 프로세스가 완료되면 Braze가 해당 채널의 LINE 팔로워를 자동으로 Braze에 가져옵니다. 이미 Braze 고객 프로필과 연결된 LINE ID의 경우 각 프로필이 "subscribed" 상태로 업데이트되며, 나머지 LINE ID는 익명 사용자를 생성합니다. 또한 LINE 채널의 새 팔로워는 채널을 팔로우할 때 미식별 고객 프로필이 생성됩니다.
...
| 정보 유형 | 위치 |
| --- | --- |
| Provider ID | 공급자를 선택한 다음 **Settings** > **Basic information**으로 이동 |
...
{% alert note %}
이미 통합된 LINE 채널의 채널 시크릿과 채널 액세스 토큰을 업데이트하거나 교체하려면 **파트너 통합** > **기술 파트너** > **LINE**으로 이동하여 해당 통합을 선택하세요.
{% endalert %}

{: start="3"}
3. **Settings** 페이지 > **Response settings**로 이동하여 다음을 수행합니다:
   - **Greeting message**를 끕니다. 이는 팔로우 트리거를 통해 Braze에서 처리할 수 있습니다.
   - **Auto-response messages**를 끕니다. 모든 트리거 메시징은 Braze를 통해 이루어져야 합니다. 이렇게 해도 LINE 콘솔에서 직접 발송하는 것은 차단되지 않습니다.
   - **Webhooks**를 켭니다.

![계정이 채팅을 처리하는 방법에 대한 토글이 있는 응답 설정 페이지.]({% image_buster /assets/img/line/response_settings.png %}){: style="max-width:80%;"}

### 2.2단계: Braze에서 LINE 구독 그룹 생성 {#step-22-generate-line-subscription-groups-in-braze}

{% multi_lang_include alerts/note_alerts.md alert='subscription group limit' %}

1. LINE용 Braze 기술 파트너 페이지로 이동하여 LINE **Providers** 탭에서 기록한 정보를 입력합니다:
   - Provider ID
   - Channel ID
   - Channel secret
...
[사용자 ID 조정](#user-id-reconciliation)의 단계를 따라 사용자의 LINE ID를 기존 Braze 고객 프로필과 결합합니다.

## 4단계: 사용자 업데이트 방법 변경 {#step-4-change-your-user-update-methods}

이미 Braze에 사용자 업데이트를 제공하는 방법이 있다고 가정하면, 이후 Braze에 전송되는 사용자 업데이트에 해당 필드가 포함되도록 새 필드 `native_line_id`를 포함하도록 업데이트해야 합니다.
...
LINE은 사용자 구독 상태의 정보 소스입니다. 사용자의 LINE ID(`native_line_id`)가 있더라도 해당 사용자가 발송하려는 LINE 채널을 팔로우하지 않았다면 LINE은 해당 사용자에게 메시지를 전달하지 않습니다.
...
3. Braze가 팔로우 이벤트를 수신하고 익명 고객 프로필을 생성합니다(프로필 2).
4. [사용자 ID 조정](#user-id-reconciliation)을 통해 사용자가 해당 LINE ID를 가진 것으로 식별됩니다.
5. 프로필 1을 업데이트하여 `native_line_id` 속성을 설정합니다. 이 프로필은 프로필 2의 구독 상태를 상속합니다.
  - 이제 동일한 `native_line_id`를 가진 두 개의 고객 프로필이 있습니다. 이는 [5단계](#step-5-merge-profiles-optional)에 설명된 프로세스에서 `/users/merge` 엔드포인트를 사용하여 언제든지 병합할 수 있습니다.
...
이 방법은 소셜 미디어 로그인을 사용하여 조정합니다. 사용자가 앱에 로그인하면 [LINE 로그인](https://developers.line.biz/en/docs/line-login/overview/)을 사용하여 사용자 계정을 생성하거나 로그인할 수 있는 옵션이 제공됩니다.

{% alert note %}
각 사용자에 대해 올바른 LINE ID를 얻으려면 Braze와 통합된 LINE 공식 계정 또는 채널과 동일한 공급자 아래에 LINE 로그인을 설정하세요.
{% endalert %}

1. LINE 개발자 콘솔로 이동하여 LINE 로그인을 통해 앱에 로그인하는 [사용자의 이메일 주소를 얻을 수 있는 권한을 요청](https://developers.line.biz/en/docs/line-login/integrate-line-login/#applying-for-email-permission)합니다.
...
{: start="3"}
3. [ID 토큰 확인 호출](https://developers.line.biz/en/reference/line-login/#verify-id-token)을 사용하여 사용자의 이메일을 얻습니다.
...
**시나리오:** 초기 구독자 동기화 중 또는 통합 후 "팔로우" 이벤트를 통해 익명 사용자가 생성되었습니다.

1. 사용자가 LINE 로그인을 사용하여 앱에 로그인합니다.
2. LINE이 사용자의 이메일을 제공합니다.
3. 업데이트된 사용자를 Braze에 전송합니다(해당 이메일이 있는 기존 고객 프로필에 LINE ID를 추가하거나 익명 사용자를 이메일로 업데이트합니다).

##### 새 팔로워가 LINE 로그인을 사용하는 경우 {#new-follower-uses-line-login}

**시나리오:** 사용자의 LINE ID가 있는 고객 프로필이 Braze에 존재하지 않습니다.
...
이 방법을 사용하면 사용자가 LINE 계정을 앱의 사용자 계정에 연결할 수 있습니다. 그런 다음 Braze에서 {% raw %}`{{line_id}}`{% endraw %}와 같은 Liquid를 사용하여 사용자의 LINE ID를 웹사이트나 앱에 다시 전달하는 개인화된 URL을 생성할 수 있으며, 이를 알려진 사용자와 연결할 수 있습니다.

1. 구독 상태 변경에 기반한 액션 기반 Canvas를 생성하여 사용자가 LINE 채널을 구독할 때 트리거되도록 합니다.<br>![사용자가 LINE 채널을 구독할 때 트리거되는 Canvas.]({% image_buster /assets/img/line/account_link_1.png %})
...
4. (선택 사항) LINE 사용자가 식별될 때 트리거되는 액션 기반 Campaign 또는 Canvas를 생성하여 사용자에게 쿠폰 코드를 전송합니다. <br>![LINE 사용자가 식별될 때 트리거되는 액션 기반 Campaign.]({% image_buster /assets/img/line/account_link_2.png %})

#### 작동 방식 {#how-it-works}

사용자가 로그인한 후, 웹사이트나 앱에서 변경이 이루어져 사용자 ID가 URL의 일부로 전달된 LINE ID와 연결하기 위해 Braze로 다시 전송됩니다. 예시 코드는 다음과 같습니다:
...
[사용자 ID 조정](#user-id-reconciliation)을 설정하기 전에 "나는 누구인가" Canvas 또는 Campaign을 생성하여 LINE 채널을 테스트할 수 있습니다.
