...
- **미국 번호:** 모든 미국 번호는 유효한 지역 코드를 포함한 유효한 10자리 전화번호여야 합니다. 10자리 전화번호에 `+`와 국가 코드가 누락된 경우, Braze는 해당 번호를 미국 번호로 매핑합니다. 푸에르토리코 전화번호는 미국 스타일의 지역 코드를 사용하는 10자리 형식이지만, 여전히 `+`와 국가 코드가 필요합니다.
...
{% alert important %}
사용자 전화번호는 Braze에서 숫자 문자열로 표시됩니다. 선행 {% raw %}`+`{% endraw %} 이외에 숫자가 아닌 문자(예: `,`, `-`, `(`)가 포함된 번호를 가져오면, Braze에서 렌더링될 때 숫자가 아닌 문자는 제거됩니다. 예를 들어, `+1 (724) 123-4567`을 가져오면 `+17241234567`로 표시됩니다.
{% endalert %}

## 전화번호 유효성 검사 {#phone-number-validation}

Braze는 Google의 [libphonenumber](https://github.com/google/libphonenumber) 라이브러리를 사용하여 전화번호의 유효성을 검사합니다. 새로운 모바일 번호 접두사가 도입되면, 업스트림 라이브러리가 업데이트됨에 따라 지원이 추가됩니다. Braze는 유효한 접두사의 별도 목록을 관리하지 않습니다.

### 유효하지 않은 전화번호 처리 {#handling-invalid-phone-numbers}

전화번호가 유효하지 않은 것으로 판단되면, Braze는 해당 사용자의 전화번호를 유효하지 않음으로 표시하고 해당 전화번호로 추가 커뮤니케이션을 보내지 않습니다. 유효하지 않은 전화번호는 고객 프로필의 **인게이지먼트 탭**에 표시됩니다.
...
- **비활성화**: 모바일 가입자가 서비스를 해지하고 통신사에서 번호를 해제하여 전화번호가 비활성화되었습니다(이후 재활용되어 새 사용자에게 할당될 수 있습니다). 비활성화된 전화번호는 해당 전화번호로 SMS 또는 RCS 메시지를 보내지 않았더라도 유효하지 않음으로 표시될 수 있습니다.
...
## 세분화에서 거부된 SMS 발송 제외하기 {#exclude-rejected-sms-sends-from-segmentation}

{% alert important %}
SMS 거부는 Braze 계약 및 SMS 공급자에 따라 SMS 할당량에 포함될 수 있습니다. 청구 성과에 대해서는 [리포팅]({{site.baseurl}}/user_guide/channels/sms_mms_and_rcs/reporting)을 참조하세요.
{% endalert %}

거부된 SMS 발송이 있는 사용자를 Segment에서 제외하려면 [SQL 세그먼트 확장]({{site.baseurl}}/user_guide/audience/segments/segment_extension/sql_segments)을 사용하여 다음을 수행합니다:

1. **오디언스** > **세그먼트 확장**으로 이동합니다.
2. **새 확장 만들기** > **전체 새로고침** 또는 **증분 새로고침**을 선택합니다.
3. SMS 거부가 있는 사용자를 식별하는 SQL 쿼리를 작성합니다. 예를 들어, `USERS_MESSAGES_SMS_REJECTION_SHARED` 이벤트를 쿼리하여 SMS 거부를 받은 사용자를 찾을 수 있습니다.
4. 세그먼트 확장을 저장합니다.
...
사용자가 SMS 또는 RCS 메시지를 수신하려면 유효한 전화번호가 있어야 하며 구독 그룹에 옵트인되어 있어야 합니다. 구독 그룹은 운영 중인 SMS 또는 RCS 프로그램에 연결됩니다(반드시 [SMS, MMS 및 RCS 관련 법적 요건]({{site.baseurl}}/user_guide/channels/sms_mms_and_rcs/compliance_and_delivery/laws_and_regulations)을 준수하고 각 고객의 동의를 기록해야 합니다). 자세한 내용은 [SMS 및 RCS 구독 그룹]({{site.baseurl}}/sms_rcs_subscription_groups)을 참조하세요.

## 서드파티 소싱 및 검증 {#third-party-sourcing-and-verification}

Braze는 유효하지 않은 번호를 소싱하기 위해 서드파티 도구를 사용합니다. Braze는 이러한 서비스의 장애나 잘못된 정보에 대해 책임을 지지 않습니다. 따라서 이 도구를 유효하지 않은 번호를 검증하기 위한 유일한 규정 준수 방법으로 사용해서는 안 됩니다.
