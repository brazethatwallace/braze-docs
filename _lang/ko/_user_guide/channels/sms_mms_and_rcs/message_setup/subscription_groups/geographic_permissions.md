...
드롭다운을 사용하여 허용 목록에 국가를 추가합니다. 가장 일반적인 SMS, MMS, RCS 국가가 상단에 표시되며, 나머지 국가는 그 아래 섹션에 표시됩니다. 텍스트 필드에 입력하여 국가를 검색할 수도 있습니다.
...
변경 사항은 저장한 후에 적용됩니다. 허용 목록에서 국가를 제거하면 해당 국가의 다이얼링 코드를 가진 전화번호로 모든 SMS, MMS, RCS 메시지 전송이 차단됩니다.
...
특정 국가는 SMS, MMS, RCS 트래픽 펌핑의 위험이 더 높습니다. 이러한 국가는 국가 드롭다운에서 **High Fraud Risk** 태그로 표시됩니다.
...
{% alert note %}
허용 목록의 국가는 비즈니스 요구 사항을 지원하는 데 필요한 국가로만 제한하세요. 이렇게 하면 사기성 트래픽의 가능성을 최소화할 수 있습니다. SMS, MMS, RCS 트래픽 펌핑 방지에 대한 자세한 안내는 [SMS 트래픽 펌핑 사기 FAQ]({{site.baseurl}}/user_guide/channels/sms_mms_and_rcs/compliance_and_delivery/sms_traffic_pumping_fraud)를 참조하세요.
{% endalert %}

## 허용 목록 외 발신의 가시성 {#visibility-of-sends-outside-the-allowlist}

국가 허용 목록에 없는 국가로의 발신 시도는 중단됩니다. 중단된 메시지는 [메시지 활동 로그]({{site.baseurl}}/user_guide/administer/global/workspace_settings/logs_and_alerts/message_activity_log) 및 [SMS 중단 메시지 인게이지먼트 이벤트]({{site.baseurl}}/user_guide/data/distribution/braze_currents/event_glossary/message_engagement_events)에 기록됩니다.
...
### SMS, MMS, RCS 트래픽 펌핑이란 무엇인가요? {#what-is-sms-mms-and-rcs-traffic-pumping}

SMS, MMS, RCS 트래픽 펌핑(인위적으로 부풀린 트래픽이라고도 함)은 고객에게 심각한 재정적 노출을 초래할 수 있는 점점 증가하는 사기 수법입니다. 사기범은 보호되지 않은 공개 웹 양식, 인증 흐름 또는 API 엔드포인트를 악용하여 자신이 통제하거나 영향력을 행사하는 전화번호로 대량의 SMS, MMS, RCS 발송(옵트인 확인, 일회용 비밀번호, 알림 등)을 트리거할 수 있습니다. 그런 다음 공격자는 공모하거나 인지하지 못하는 모바일 네트워크로부터 해당 인위적 트래픽 생성에 대한 매출 분배금을 수취합니다. 이로 인한 후속 영향은 심각한 재정적 노출을 야기합니다.
...
고객은 SMS, MMS, RCS 트래픽 펌핑으로 인한 메시지를 포함하여, 서비스를 통해 고객을 대신하여 발송된 모든 모바일 메시지에 대한 책임이 있으며 해당 비용이 청구됩니다. 국가 허용 목록과 같은 플랫폼 안전장치는 신뢰할 수 있는 지역으로 전송을 제한하는 데 도움이 됩니다. 그러나 궁극적으로, 외부 노출 엔드포인트를 보호하고 치명적인 재정적 피해를 방지하는 것은 전적으로 고객의 책임입니다.

### 트래픽 펌핑을 방지하는 방법 {#how-to-prevent-traffic-pumping}

메시지 배포를 실제 고객이 거주하는 지리적 지역으로 엄격히 제한하지 않으면 사기 및 심각한 재정적 피해에 즉각적인 취약성이 생깁니다. 회사를 보호하려면 국가 허용 목록을 사용하여 배달 지역을 사전에 제한해야 합니다. 또한, 가장 중요한 것은, [SMS, MMS, RCS 트래픽 펌핑 사기 이해 및 방지]({{site.baseurl}}/user_guide/channels/sms_mms_and_rcs/compliance_and_delivery/sms_traffic_pumping_fraud)에 설명된 대로 업계 모범 사례에 따라 SMS, MMS, RCS 발송을 트리거하는 모든 온라인 전화번호 요청 양식 또는 API 엔드포인트를 보호해야 합니다.