...
예를 들어, 사용자 A가 여러 구독 그룹에 가입된 전화번호를 가지고 있고, 그 전화번호가 사용자 B에 추가되면, 사용자 B는 동일한 구독 그룹에 가입됩니다. 사용자가 기존 구독을 상속하는 것을 방지하려면, 사용자가 번호를 변경할 때마다 Braze REST API를 통해 이전 번호의 구독 그룹을 초기화할 수 있습니다. 여러 사용자가 이 전화번호를 공유하는 경우, 모든 사용자가 가입 취소됩니다.
...
### 사용자의 그룹 확인하기 {#check-a-users-group}
...
{% tab MMS %}
MMS 메시지를 보내려면 구독 그룹 내 최소 하나의 번호가 MMS 발송이 가능하도록 활성화되어 있어야 합니다. 이는 구독 그룹 옆에 있는 태그로 표시됩니다.
...
{% tab RCS %}
RCS 메시지를 보내기 전에 구독 그룹 내에 RCS 인증 발신자가 있어야 합니다.
...
포괄적인 구독 관리를 위해, 표준 또는 커스텀 키워드에 해당하지 않는 옵트아웃 의도(예: "문자 보내지 마세요")를 캡처할 수 있습니다. AI 에이전트를 생성하면 감정 분석을 사용하여 이러한 요청을 자동으로 식별하고 처리할 수 있습니다.
...
다음 방법 중 하나를 사용하여 오디언스를 정의합니다. 그 다음, **발송 설정** 단계로 이동하여 **가입됨 또는 옵트인된 사용자**를 선택합니다.

| 방법 | 설명 |
|------|------|
| **Segment 생성** | 구독 그룹의 모든 사용자 또는 세분화 필터를 사용한 하위 집합(예: 무작위 5~10%)을 포함하는 Segment를 만듭니다. Segments는 각 발송 전에 업데이트되어 현재 사용자 기반을 반영합니다. |
| **Campaign 또는 Canvas 필터 적용** | Campaign 또는 Canvas의 **타겟 오디언스** 단계에서 오디언스를 세분화합니다. 페이지를 떠나지 않고 타겟팅 옵션을 조정하여 유연하게 대응할 수 있습니다. |
{: .reset-td-br-1 .reset-td-br-2 aria-label="2단계: 오디언스 정의하기" }

### 3단계: 사용자 업데이트 단계 구성하기 {#step-3-configure-a-user-update-step}

Canvas에 사용자 업데이트 단계를 추가합니다. 해당 단계에서 **고급 JSON 에디터**를 열고 다음을 입력합니다(고유 사용자 식별자 필드로는 `braze_id` 필드를 사용하는 것을 권장합니다):
...
{% alert important %}
`use_double_opt_in_logic`를 사용하는 경우, 구독 상태가 업데이트되려면 고객 프로필이 이미 존재해야 합니다. 제공된 식별자에 연결된 고객 프로필이 없으면 구독 상태가 업데이트되지 않습니다.
{% endalert %}
...
Canvas 테스트가 성공적으로 완료되면, 사용자 하위 집합을 대상으로 Canvas를 시작하세요!

사용자가 성공적으로 마이그레이션되었는지 확인하려면, 업데이트된 개별 고객 프로필을 몇 개 확인하는 것을 권장합니다. **인게이지먼트** 탭에서 **연락처 설정**을 찾은 후 스크롤하여 사용자가 가입된 구독 그룹을 확인합니다. RCS 구독 그룹 토글이 켜져 있어야 합니다.
...
- **메시징 유형:** 트랜잭션 및 마케팅 등 각 메시징 유형에 대해 별도의 구독 그룹을 생성합니다.
