...
- **REST API:** [`/subscription/status/set` 엔드포인트]({{ site.baseurl}}/api/endpoints/subscription_groups/post_update_user_subscription_group_status/)를 사용하여 Braze REST API를 통해 사용자 프로필을 프로그래밍 방식으로 설정할 수 있습니다.
...
- **사용자 업데이트 단계:** [사용자 업데이트]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/user_update) 단계를 사용하여 Canvas에서 구독 상태를 업데이트합니다. 타이밍 고려 사항은 [Canvas에서 사용자 상태 업데이트하기](#update-a-users-state-in-a-canvas)를 참조하세요.
...
사용자가 Campaign 또는 Canvas에서 메시지를 수신하려면 선택한 구독 그룹에 가입되어 있어야 합니다. 유효한 사용자에 대해 발송이 실패하는 경우, [사용자 상태 설정하기](#set-a-users-state)의 방법 중 하나를 사용하여 해당 사용자가 가입 상태인지 확인하세요. 이중 옵트인 요구 사항에 대해서는 [구독 그룹 상태](#subscription-group-states)를 참조하세요.
...
![SMS 작성기에서 구독 그룹 드롭다운이 열려 있고 사용자가 "Messaging Service A for SMS"를 선택한 모습.]({% image_buster /assets/img/sms/sms_subgroup_select.png %})
...
![구독 그룹 드롭다운에서 "Messaging Service A for SMS"가 강조 표시되어 있습니다. 항목 앞에 "MMS" 태그가 표시되어 있습니다.]({% image_buster /assets/img/sms/mms_sub_group_tag.png %}){: style="max-width:40%"}
{% endtab %}
...
Braze는 처음에 소규모 사용자에게 RCS 전송을 테스트한 후, 시간이 지남에 따라 더 많은 사용자를 RCS 구독 그룹으로 마이그레이션하는 것을 권장합니다. 예를 들어, SMS 구독 그룹에 1,000,000명의 사용자가 가입되어 있는 경우, 먼저 모든 사용자를 새 구독 그룹으로 마이그레이션한 다음 50,000~100,000명(5~10%)의 소규모 오디언스를 세분화하여 RCS 메시지를 테스트하는 방식으로 진행할 수 있습니다.
...
Canvas를 성공적으로 테스트한 후, 사용자 하위 집합에 대해 Canvas를 시작하세요!
...
예를 들어, 브라질에서는 현지 시간 오전 9시에서 오후 9시 이외의 시간에 마케팅 메시지를 보내는 것이 금지되어 있으며, 브라질은 세 개의 시간대에 걸쳐 있습니다. 이러한 규정을 준수하기 위해 브라질과 미국에 메시지를 보내기 위한 별도의 그룹을 설정할 수 있습니다. 이렇게 하면 브라질의 사용자가 금지된 시간에 마케팅 메시지를 수신하는 것을 방지할 수 있습니다.