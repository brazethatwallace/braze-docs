...
4. [단계 스케줄]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/delivery_and_entry_types)을 선택하고 필요에 따라 지연을 지정합니다.
5. 필요에 따라 이 단계의 오디언스를 필터링합니다. Segment를 지정하고 추가 필터를 적용하여 이 단계의 수신자를 더 세밀하게 조정할 수 있습니다. 오디언스 옵션은 메시지가 발송되는 시점에 지연 후 확인됩니다.
...
{% alert tip %}
Braze는 RCS 발신자가 포함된 모든 구독 그룹에 대체용 SMS 코드를 하나 이상 포함할 것을 강력히 권장합니다. 이렇게 하면 RCS 메시지가 전달되지 않는 경우(예: 기기 비호환성 또는 불완전한 통신사 커버리지) SMS를 통해 사용자에게 메시지가 전달됩니다.
{% endalert %}
...
작성 환경은 선택한 메시지 유형에 따라 달라집니다. 메시지 유형에 해당하는 탭을 선택하세요.
...
MMS 메시지를 보내려면 구독 그룹에 MMS가 활성화된 전화번호가 하나 이상 있어야 합니다. 이는 작성기에서 구독 그룹 옆에 **MMS** 태그로 표시됩니다.

메시지 본문을 입력한 다음, [미디어 라이브러리]({{site.baseurl}}/user_guide/messaging/design_and_edit/media_library)에서 PNG, JPEG 또는 GIF 이미지를 업로드하거나 이미지 URL을 지정하세요. 메시지당 하나의 이미지만 지원됩니다.
...
### 이동통신사 동작 {#carrier-behavior}

MMS 메시지는 텍스트 전용 SMS와 다른 요금이 부과됩니다. 모든 이동통신사가 MMS를 수신할 수 있는 것은 아닙니다. 이러한 경우 MMS는 사용자가 선택할 수 있는 이미지 링크로 자동 변환됩니다.

{% alert note %}
Google Voice 번호로 MMS를 보내지 마세요. Google Voice는 MMS 지원이 제한적이어서 메시지 전달이 불안정합니다.
{% endalert %}

### 인바운드 MMS 및 개인화 {#inbound-mms-and-personalization}

고객이 미디어가 포함된 인바운드 메시지를 보내면, Braze는 [Currents SMS 인바운드 이벤트]({{site.baseurl}}/user_guide/data/distribution/braze_currents/event_glossary/message_engagement_events#sms-inbound-received-events)와 Liquid에서 {% raw %}`{{sms.${inbound_media_urls}}}`{% endraw %}로 해당 미디어를 노출합니다(예: 리타겟팅 또는 후속 메시지에서). Canvas에서 인바운드 SMS 속성을 사용하는 방법에 대한 자세한 내용은 [메시지 단계]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/message_step)를 참조하세요.
...
**텍스트** 또는 **미디어** 메시지 유형 중에서 선택하세요.

![텍스트 또는 미디어 메시지 유형을 선택하는 옵션.]({% image_buster /assets/img/rcs/rcs_text_media.png %}){: style="max-width:65%;"}
...
![트렌디한 패션 스타일을 홍보하는 RCS 메시지의 세 가지 추천 작업.]({% image_buster /assets/img/rcs/rcs_suggested_actions.gif %}){: style="max-width:70%;"}

**고려 사항:**

- Android와 iOS에서 잘림 방식이 다를 수 있습니다. Android는 전체 리치 메시지 텍스트를 표시하지만, iOS는 세 번째 줄 이후에 잘립니다.
- 메시지당 최대 5개의 버튼을 추가할 수 있습니다. 추천 작업 또는 추천 답장 중 하나일 수 있습니다.
- 긴 텍스트 블록과 많은 버튼은 수신자에게 부담을 줄 수 있으므로, 가능하면 간결하게 작성하세요.
- 경우에 따라 긴 텍스트 전용 메시지를 SMS보다 RCS로 보내는 것이 더 비용 효율적일 수 있습니다. 긴 SMS 메시지는 여러 개의 과금 세그먼트로 분할되지만, RCS 메시지는 메시지당 요금이 부과되기 때문입니다.
...
- 텍스트, 추천 답장, 추천 작업을 포함하여 텍스트 메시지 유형에서 사용 가능한 모든 기능을 지원합니다.
...
| 파일 유형 | 사양 |
| --- | --- |
| 전체 | 파일 크기는 100 MB로 제한됩니다. 파일 URL은 최대 2,048자까지 가능합니다. |
...
RCS 메시지 수신 경험은 이동통신사 커버리지, 모바일 기기 하드웨어, 운영 체제에 따라 달라질 수 있습니다. RCS는 Android 기기와 더 자연스럽게 통합되며, 기기에 따라 경험이 다른 속도와 품질로 렌더링될 수 있습니다.
...
{: start="4"}
4. 각 작업 그룹을 RCS 메시지 단계에 연결한 다음, 해당 추천 답장에 기반한 콘텐츠를 추가하세요.
5. 후속 메시지에 추천 답장을 추가하여 대화형 워크플로를 계속 이어가세요.
6. 워크플로가 완성될 때까지 2~4단계를 반복하세요.
...
Braze는 메시지를 보내기 전에 항상 미리보기와 테스트를 권장합니다. **테스트** 탭으로 전환하여 [콘텐츠 테스트 그룹]({{site.baseurl}}/user_guide/administer/global/user_management/internal_groups#content-test-groups) 또는 개별 사용자에게 테스트 SMS, MMS 또는 RCS 메시지를 보내거나, Braze에서 직접 사용자로서 메시지를 미리 볼 수 있습니다.

{% alert tip %}
SMS가 몇 개의 세그먼트로 분할될 수 있는지 테스트하려면 [SMS 세그먼트 계산기]({{site.baseurl}}/user_guide/channels/sms_mms_and_rcs/billing_calculator#segment-calculator)를 사용하여 문구 길이를 테스트하세요.
{% endalert %}

![작성기의 테스트 탭에서 SMS 문구를 미리보기합니다. 프로필 섹션에서 이름 필드가 "James"로 설정되어 있습니다. 미리보기 섹션에서 SMS는 "Hi James, we appreciate your support!"로 표시됩니다.]({% image_buster /assets/img/sms_campaign_test.png %})

{% alert note %}
MMS의 경우 에셋(이미지 및 메시지 본문)의 순서는 커스터마이징할 수 없습니다. 순서는 메시지를 수신하는 휴대폰에 따라 달라집니다.
{% endalert %}

{% alert note %}
RCS 렌더링은 사용자의 운영 체제, 기기 제조사, 통신사 및 메시징 앱(예: Google Messages 대 Apple Messages)에 의해 제어되므로 메시지 외관이 달라질 수 있습니다. Braze에 표시되는 미리보기가 최종사용자가 수신하는 것과 정확히 일치하지 않을 수 있습니다. 가능하면 실제 기기에서 최종 렌더링을 확인하세요. iOS 기기에서의 RCS 렌더링에 대한 자세한 내용은 [iOS 기기에서 RCS 메시지가 정확하게 렌더링되지 않는 이유는 무엇인가요?]({{site.baseurl}}/user_guide/channels/sms_mms_and_rcs/faqs#why-doesnt-my-rcs-message-render-accurately-on-ios-devices)를 참조하세요.
{% endalert %}
...
다음으로, Segments 또는 필터를 선택하여 오디언스를 좁혀 [사용자를 타겟팅]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/target_users)합니다. 이미 구독 그룹을 선택했을 것이며, 이를 통해 사용자가 원하는 커뮤니케이션 수준이나 카테고리에 따라 사용자를 좁힐 수 있습니다.

{% multi_lang_include audience/target_audiences.md %}

Segments에서 더 큰 오디언스를 선택하고, 선택 사항인 필터를 사용하여 해당 Segment를 더 좁힙니다. 대략적인 Segment 인구의 미리보기가 자동으로 표시됩니다. 정확한 Segment 멤버십은 항상 메시지가 발송되기 전에 계산된다는 점을 유의하세요.
...
Braze에서는 Campaign을 수신한 후 사용자가 특정 행동인 [전환 이벤트]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/conversion_events)를 얼마나 자주 수행하는지 추적할 수 있습니다. 사용자가 지정된 행동을 취하면 전환으로 집계되는 최대 30일의 기간을 설정할 수 있습니다.
...
특정 사용 사례에 맞는 커스텀 전환 이벤트를 설정할 수도 있습니다.
...
Campaign 또는 Canvas의 마지막 구성을 완료한 후, 세부 사항을 검토하고 테스트한 다음 발송하세요!

다음으로, [SMS, MMS, RCS 보고]({{site.baseurl}}/user_guide/channels/sms_mms_and_rcs/reporting)를 확인하여 캠페인 결과에 액세스하는 방법을 알아보세요.
