---
nav_title: "모범 사례"
article_title: 단문 메시지 서비스, MMS, RCS 모범 사례
page_order: 2
description: "이 참조 문서에서는 단문 메시지 서비스/MMS 모범 사례를 다룹니다."
...
# 단문 메시지 서비스, MMS, RCS 모범 사례 {#best-practices-for-sms-mms-and-rcs}

> 옵트아웃 모니터링 및 트래픽 펌핑에 대한 권장 사항을 포함하여 Braze에서의 단문 메시지 서비스, MMS, RCS 모범 사례에 대해 자세히 알아보세요.

## 수신 거부 모니터링 권장 사항 {#opt-out-monitoring-recommendations}

커뮤니케이션 수신 거부 요청을 준수하는 것은 법적 의무입니다. 단문 메시지 서비스 수신자의 채널 수신 거부 요청을 준수하지 않으면 벌금 등의 처벌을 받을 수 있으며 소송으로 이어질 수 있습니다. Braze는 단문 메시지 서비스 및 MMS 옵트인/수신 거부 관리를 효과적으로 지원하는 기능을 갖추고 있으며, 요청이 올바르게 처리되도록 돕는 메커니즘도 제공합니다.

당사와의 구독 계약에 따라, 고객은 당사 서비스 이용 시 관련 법률 준수에 대해 전적으로 책임을 집니다. 이에 따라 고객이 단문 메시지 서비스 설정을 올바르게 구성하는 데 각별히 주의를 기울이고, 해당 설정을 철저히 테스트하며, 수신 거부 준수 여부를 모니터링하기 위한 조치를 취하고, 수신 거부 요청 미준수 사례가 확인될 경우 신속하게 대응할 것을 강력히 권장합니다.

Braze에서 옵트인 및 수신 거부를 관리하기 위해 단문 메시지 서비스 및 MMS를 설정할 때, 다음 리소스 목록을 참고하세요:
* [단문 메시지 서비스 구독 그룹]({{site.baseurl}}/sms_rcs_subscription_groups): 구독 그룹과 옵트인/수신 거부 방법 및 상태에 대한 안내입니다.
...
* [단문 메시지 서비스 이중 옵트인]({{site.baseurl}}/user_guide/channels/sms_mms_and_rcs/message_features_and_optimization/keyword_processing/double_opt_in): 사용자가 단문 메시지 서비스 메시지를 수신하기 전에 옵트인 의사를 명시적으로 확인해야 합니다. 단문 메시지 서비스 이중 옵트인은 일부 국가에서 필수 요건이므로, Braze는 이를 구성할 것을 권장합니다.
* [단문 메시지 서비스 메시지 발송]({{site.baseurl}}/user_guide/channels/sms_mms_and_rcs/message_setup/sms_sending): Braze의 단문 메시지 서비스 발송 기본 사항으로, 구독 그룹의 중요성, 단문 메시지 서비스 세그먼트 및 메시지 본문 요구 사항 등을 다룹니다.

### 고려 사항 {#considerations}

단문 메시지 서비스 및 MMS가 여러 인스턴스에 걸쳐 설정된 경우, 잘못된 구성으로 인해 Campaign 또는 Canvas 수신 거부가 잘못된 워크스페이스로 전송될 수 있습니다.
...
Braze는 사용자 프로필(`user_id`) 수준과 전화번호(`channel_id`) 수준 모두에서 단문 메시지 서비스/MMS 구독을 관리합니다. 전화번호가 옵트인 또는 수신 거부되면 해당 번호를 공유하는 모든 프로필에 업데이트가 적용됩니다. 사용자가 특정 전화번호로 옵트인한 후 전화번호를 변경한 경우, 새 전화번호는 해당 사용자의 구독 그룹 상태를 상속받습니다. 따라서 사용자가 수신 거부한 후 새 전화번호로 앱이나 웹사이트에 다시 접속하더라도 원치 않는 메시지를 수신하지 않습니다.
...
트래픽 펌핑은 악의적인 행위자가 온라인 양식을 사용하여 대량의 단문 메시지 서비스 메시지(예: 옵트인 메시지 또는 일회용 비밀번호)가 발송되도록 트리거하는 사기 행위입니다. 악의적인 행위자는 이러한 메시지가 발송될 프리미엄 요금 전화번호를 설정하고, 해당 프리미엄 요금 번호가 설정된 이동통신사로부터 수익을 분배받아 불법 수익을 창출합니다.
...
고객은 발송하는 트래픽을 모니터링할 책임이 있으며, 계정을 통해 발송된 모든 단문 메시지 서비스에 대해 비용이 청구됩니다. Braze와 고객 사이에서 고객이 트래픽 펌핑을 감지하고 방지하기에 더 적합한 위치에 있는 당사자입니다.

## 다국가 단문 메시지 서비스 발송 {#multi-country-sms-sending}

일부 브랜드는 서로 다른 국가의 전화번호를 보유한 사용자 그룹에 메시지를 발송하고자 할 수 있습니다. 특정 국가의 전화번호로 단문 메시지 서비스 메시지를 발송하려면, 해당 국가에서 발급된 긴 코드 또는 짧은 코드를 사용하는 것이 모범 사례입니다. 실제로 짧은 코드는 해당 짧은 코드가 생성된 국가의 전화번호로만 단문 메시지 서비스를 발송할 수 있습니다.
...
1. **허가를 받으세요.** 비즈니스에서 단문 메시지 서비스를 사용할 때 가장 중요한 규칙 중 하나는 고객에게 연락하기 전에 먼저 허가를 받아야 한다는 것입니다. 이를 지키지 않으면 브랜드 이미지가 손상되고 막대한 법적 비용이 발생할 수 있습니다.
2. **사용 사례에 맞는 번호를 선택하세요.** 단문 메시지 서비스 메시지를 발송하고 수신할 수 있는 전화번호에는 긴 코드, 짧은 코드, 영숫자 발신자 ID의 세 가지 주요 유형이 있으며, 지역에 따라 기능과 가용성이 다릅니다. 비즈니스에 배니티 코드가 더 적합한지 미리 검토하세요.
...
4. **양방향 대화에 참여하세요.** 단문 메시지 서비스는 고객과 소통하는 데 매우 효과적인 채널이므로, 메시지에 대한 응답을 예상하고 효과적으로 처리하는 것이 중요합니다. 소비자의 85%는 정보를 수신하는 것뿐만 아니라 비즈니스에 답장하거나 대화에 참여할 수 있기를 원합니다.
...
모든 단문 메시지 서비스 Campaign 및 Canvases에서 지역 규정과 모범 사례를 준수할 수 있도록 [방해금지 시간]({{site.baseurl}}/user_guide/brazeai/intelligence_suite/intelligent_timing#quiet-hours)을 활성화할 것을 적극 권장합니다.
...
단문 메시지 서비스 메시지 본문 상단에 다음 스니펫을 포함하세요. 이 예시는 사용자의 [현지 시간대]({{site.baseurl}}/user_guide/messaging/campaigns/faq#what-does-local-time-zone-delivery-offer) 기준으로 오전 9시~오후 9시 범위를 벗어날 경우 발송을 중단합니다.
