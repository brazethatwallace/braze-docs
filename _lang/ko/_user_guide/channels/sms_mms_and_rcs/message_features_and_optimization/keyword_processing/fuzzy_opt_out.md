...
> Braze를 통해 SMS, MMS, RCS를 전송하는 사용자는 정의된 관련 법률, 규정 및 업계 표준을 준수해야 합니다. 수신 거부와 관련하여, 법률은 사용자가 "STOP"이라고 문자를 보내면 해당 메시징 프로그램과 관련된 모든 후속 메시지가 중단되어야 한다고 규정합니다. Braze는 이러한 메시지를 자동으로 처리하고 사용자의 구독을 해지합니다.<br><br>퍼지 수신 거부는 인바운드 메시지가 [수신 거부 키워드]({{site.baseurl}}/user_guide/channels/sms_mms_and_rcs/message_features_and_optimization/keyword_processing/optin_optout/)와 일치하지 않지만 수신 거부 의도를 나타내는 경우를 인식하려고 시도합니다. 퍼지 수신 거부가 활성화되어 있고 인바운드 키워드 응답이 "퍼지"로 판단되면, Braze가 자동으로 사용자의 구독을 해지하거나 수동으로 수신 거부하는 방법을 안내하는 메시지로 응답하도록 구성할 수 있습니다.
...
현재 영어를 [로컬 언어]({{site.baseurl}}/user_guide/channels/sms_mms_and_rcs/message_features_and_optimization/keyword_processing/keyword_handling/#multi-language-support)로 사용하여 생성된 수신 거부 키워드만 지원됩니다.

## 퍼지로 판단되는 기준 {#what-is-deemed-as-fuzzy}

인바운드 응답이 "퍼지"로 판단되는 기준은 다음과 같습니다:
