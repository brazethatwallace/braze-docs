...
고객이 라이프사이클 중 특정 시점(예: 초기 온보딩 중)에만 특정 키워드를 보내 응답을 받을 수 있도록 제한하려는 사용 사례가 있는 경우, Campaign 또는 Canvas에서 **구독 그룹 내 키워드 카테고리 OTHER에 인바운드 SMS 전송** 트리거를 사용하고 사용자가 특정 시점에 보낼 수 있는 키워드를 정의할 수 있습니다.
...
4. 트리거를 적절한 구독 그룹에 대한 **인바운드 SMS 전송**으로 설정하고 **키워드 카테고리 OTHER 범위** 내로 지정합니다.
