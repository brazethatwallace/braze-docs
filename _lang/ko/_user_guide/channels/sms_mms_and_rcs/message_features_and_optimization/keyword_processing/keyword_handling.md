...
page_order: 2
description: "이 참조 문서에서는 Braze가 양방향 단문 메시지 서비스, MMS, RCS 메시징 및 자동 응답을 처리하는 방법을 다룹니다. 키워드 트리거 작동 방식, 커스텀 키워드 카테고리, 다중 언어 지원에 대한 설명이 포함되어 있습니다."
page_type: reference
channel:
  - SMS
  - MMS
...
> 이 참조 문서에서는 Braze가 양방향 단문 메시지 서비스, MMS, RCS 메시징 및 자동 응답을 처리하는 방법을 다룹니다. 키워드 트리거 작동 방식, 커스텀 키워드 카테고리, 다중 언어 지원에 대한 설명이 포함되어 있습니다.
...
Braze의 단문 메시지 서비스, MMS, RCS를 사용하면 키워드 트리거를 생성하고, 커스텀 응답을 정의하며, 여러 언어에 대한 키워드 세트를 정의하고, 커스텀 키워드 카테고리를 설정할 수 있습니다.
...
1. Braze 대시보드에서 **오디언스** > **구독 그룹 관리**로 이동하여 **단문 메시지 서비스/MMS/RCS** 구독 그룹을 선택합니다.
...
1. Braze 대시보드에서 **오디언스** > **구독 그룹 관리**로 이동하여 **단문 메시지 서비스/MMS/RCS** 구독 그룹을 선택합니다. <br><br>
...
{% alert tip %}
실행 기반 Canvas가 인바운드 단문 메시지 서비스, MMS 또는 RCS 메시지에 의해 트리거되는 경우, Canvas의 첫 번째 [메시지 단계]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/message_step)에서 단문 메시지 서비스, MMS 또는 RCS 속성을 참조할 수 있습니다.
{% endalert %}
...
고객이 라이프사이클 중 특정 시점(예: 초기 온보딩 중)에만 특정 키워드를 보내 응답을 받을 수 있도록 제한하려는 사용 사례가 있는 경우, Campaign 또는 Canvas에서 **구독 그룹 내 키워드 카테고리 OTHER로 인바운드 단문 메시지 서비스 전송** 트리거를 사용하고 사용자가 특정 시점에 보낼 수 있는 키워드를 정의할 수 있습니다.
...
1. [단문 메시지 서비스 Campaign]({{site.baseurl}}/user_guide/channels/sms_mms_and_rcs/create)을 생성합니다.
2. **타겟 오디언스**에서 **모든 사용자**를 선택합니다(트리거가 메시지를 받는 대상을 제한합니다).
3. **스케줄**에서 **실행 기반 전달**을 선택합니다.
4. 트리거를 적절한 구독 그룹에 대한 **인바운드 단문 메시지 서비스 전송**, **키워드 카테고리 OTHER 내**로 설정합니다.
