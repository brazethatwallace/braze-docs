...
{% alert tip %}
옵트아웃 처리를 확장하고 싶으신가요? [퍼지 옵트아웃]({{site.baseurl}}/user_guide/channels/sms_mms_and_rcs/message_features_and_optimization/keyword_processing/fuzzy_opt_out)을 사용해 보세요. 이 기능은 수신 메시지가 옵트아웃 키워드와 정확히 일치하지 않지만 옵트아웃 의도를 나타내는 경우를 인식하려고 시도합니다.
{% endalert %}
...
**정확한 단일 단어 메시지**만 처리됩니다(대소문자 구분 없음). `STOP PLEASE`와 같은 키워드는 [퍼지 옵트아웃]({{site.baseurl}}/user_guide/channels/sms_mms_and_rcs/message_features_and_optimization/keyword_processing/fuzzy_opt_out)이 활성화되지 않은 경우 무시됩니다.
