...
> 이 페이지에서는 Braze 대시보드에서 자체 커스텀 도메인을 설정하는 방법을 다룹니다. 커스텀 도메인을 사용하면 일반적인 단축 링크나 Braze 도메인(`brz.ai`) 대신 브랜드 아이덴티티를 반영하는 브랜드 단축 링크를 사용할 수 있어, SMS 링크에 대한 사용자 신뢰도와 Campaign 인게이지먼트를 높일 수 있습니다.
...
{% alert important %}
45일 이내에 DNS 레코드가 업데이트되지 않으면 설정 토큰이 만료됩니다. **SMS/RCS and Messaging Apps Domains**에서 도메인 설정을 다시 시작하여 새 DNS 레코드를 생성하세요.
{% endalert %}
...
{% alert note %}
도메인 진행 상황은 자동으로 저장됩니다. 진행 중에 나가야 하는 경우, **SMS/RCS and Messaging Apps Domains** 페이지에서 보류 중인 도메인 항목을 선택하여 나중에 다시 시작할 수 있습니다.
{% endalert %}

### 지속적인 관리 및 사용 {#ongoing-management-and-usage}

도메인이 확인되면 커스텀 도메인이 **SMS/RCS and Messaging Apps Domains** 페이지의 테이블에 상태 표시와 함께 나타납니다. 연결된 도메인을 여러 구독 그룹, 워크스페이스 및 SMS, RCS, WhatsApp 채널에서 즉시 사용할 수 있습니다.
...
링크 단축이 켜진 상태로 전송된 Campaigns는 SMS, RCS 또는 WhatsApp 구독 그룹에 연결된 할당 도메인을 사용합니다.
...
**Pending** 또는 **Error** 상태의 도메인도 이 한도에 포함됩니다. 해당 도메인을 삭제하거나 **SMS/RCS and Messaging Apps Domains** 페이지에서 오류를 해결하세요.
