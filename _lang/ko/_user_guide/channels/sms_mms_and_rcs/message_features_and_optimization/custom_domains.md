...
{% alert important %}
DNS 레코드가 45일 이내에 업데이트되지 않으면 설정 토큰이 만료됩니다. **SMS/RCS 및 메시징 앱 도메인**에서 도메인 설정을 다시 시작하여 새 DNS 레코드를 생성하세요.
{% endalert %}

## 커스텀 도메인 추가하기 {#adding-a-custom-domain}

1. Braze에서 **회사 설정** > **SMS/RCS 및 메시징 앱 도메인**으로 이동합니다.
![여러 도메인이 나열된 "SMS/RCS 및 메시징 앱 도메인" 페이지.]({% image_buster /assets/img/main_page.png %})
...
도메인이 확인되면 커스텀 도메인이 **SMS/RCS 및 메시징 앱 도메인** 페이지의 테이블에 상태 표시와 함께 나타납니다. 연결된 도메인은 여러 구독 그룹, 워크스페이스, 그리고 SMS, RCS, WhatsApp 채널에서 즉시 사용할 수 있습니다.
