...
Braze는 봇 클릭이라고도 알려진 비인간 상호작용(NHI)으로 의심되는 클릭을 식별하기 위해 여러 입력을 사용하는 독자적인 탐지 시스템을 보유하고 있습니다. 봇 클릭은 클릭률을 부풀려 인게이지먼트 측정기준을 왜곡할 수 있습니다. Braze는 이를 필터링하여 의사결정에 필요한 신뢰할 수 있는 데이터 수집을 지원합니다.

이 시스템은 웹 크롤러, Android 및 iOS 링크 미리보기, 또는 CPaaS 보안 소프트웨어와 관련된 사용자 에이전트를 분석합니다. 필터링되는 사용자 에이전트의 예로는 `GoogleBot`, `GoogleMessages/20`, `python-requests/2.32.3`, `Barracuda Sentinel (EE)` 등이 있습니다.
