...
> [Gmail 모바일 프로모션 탭](https://developers.google.com/gmail/promotab/)을 사용하면 마케터가 제목란이나 프리헤더 정보만이 아닌 "카드" 내 주석을 통해 더 많은 정보를 전달할 수 있습니다. Braze에는 이메일 캠페인에서 카드를 구축하는 데 도움이 되는 기본 제공 도구가 있습니다.

## 필수 조건 {#prerequisites}

먼저 도메인과 하위 도메인을 Google의 프로모션 탭 지원팀 <a href="mailto:p-promo-outreach@google.com">p-promo-outreach@google.com</a>으로 전달하여 Gmail의 허용 목록에 추가하세요. 이렇게 하면 Gmail 프로모션 탭의 제품 캐러셀과 같이 풍부한 이미지를 표시하는 기능을 사용할 수 있습니다.

## Braze로 카드 구축하기 {#build-the-card-with-braze}

다음 단계에 따라 이메일 캠페인용 Gmail 프로모션 카드를 구축하세요. 편집기에서 **콘텐츠** 섹션을 벗어나면 **Gmail 프로모션** 탭의 필드와 정보가 초기화됩니다. 프로모션 카드 설정을 완료하고 생성된 HTML을 복사하여 HTML 코드를 잃지 않도록 하세요.

### 1단계: 이메일 캠페인 생성 {#step-1-create-an-email-campaign}

먼저 [이메일 캠페인을 생성]({{site.baseurl}}/user_guide/channels/email/html_editor/)하고 편집 환경으로 **HTML 코드 편집기**를 선택합니다.

### 2단계: Gmail 프로모션 카드에 세부 정보 추가 {#step-2-add-details-to-gmail-promotion-card}

다음으로 HTML 편집기의 **콘텐츠** 섹션으로 이동하여 **Gmail 프로모션** 탭을 선택합니다. **기본 정보** 아래의 필드를 채운 다음 **HTML 코드 생성**을 선택합니다. 그러면 **HTML 코드를 복사하여 `<Head>`에 붙여넣기** 섹션 아래에 Gmail 프로모션 탭 카드용 스크립트가 생성됩니다.
...
Gmail 프로모션 카드에 할인 오퍼, 딜 카드, 프로모션 카드 또는 모든 옵션을 포함할지 선택합니다.

{% tabs %}
{% tab 할인 오퍼 %}

할인 오퍼를 설정하면 할인의 유효 기간을 지정할 수 있습니다.

1. **할인 오퍼** 토글을 선택합니다.
2. **오퍼**에 할인에 대한 간단한 요약을 입력합니다. 예를 들어 "20% 할인"입니다.
3. **코드**에 사용자가 결제 시 적용해야 하는 프로모션 코드를 추가합니다.
4. 그런 다음 할인 오퍼의 시작 날짜와 시간을 선택합니다.
5. 할인 오퍼가 특정 시간에 종료되어야 하는지 또는 종료되지 않아야 하는지 결정합니다.

![할인 오퍼의 오퍼 값, 코드, 시작 날짜 및 시간을 지정하는 옵션.]({% image_buster /assets/img/gmail_promo_discount_details.png %}){: style="max-width:70%;"}

{% endtab %}
{% tab 딜 카드 %}

딜 카드를 사용하면 이메일 본문 상단에 주요 딜 정보를 직접 제공할 수 있습니다. 이를 통해 수신자가 오퍼 세부 정보를 빠르게 파악하고 조치를 취할 수 있습니다. 예를 들어, 딜 카드를 사용하여 한정 기간 오퍼를 홍보하고 사용자가 이메일 내에서 세부 정보를 찾아야 하는 번거로움을 줄일 수 있습니다.

1. **딜 카드** 토글을 선택합니다.
2. **오퍼**에 할인에 대한 간단한 요약을 입력합니다. 예를 들어 "모든 신발 20% 할인"입니다.
3. (선택 사항) **코드**에 사용자가 결제 시 적용해야 하는 프로모션 코드를 추가합니다.
4. 다음 URL 중 하나 이상을 입력합니다.
-  **오퍼 페이지 URL:** 특정 오퍼 랜딩 페이지의 URL입니다. "지금 쇼핑하기"(또는 유사한) 버튼이 생성됩니다. 딜 카드에 이 URL을 제공하는 것을 권장합니다.
- **판매자 홈페이지 URL:** 메인 홈페이지의 URL입니다. 특정 오퍼 페이지 URL을 사용할 수 없는 경우에만 이 필드를 사용하세요.
5. (선택 사항) 오퍼의 시작 날짜를 추가합니다.
6. 오퍼가 특정 시간에 종료되어야 하는지 또는 종료되지 않아야 하는지 결정합니다.

![딜 카드의 오퍼 값, 코드, 시작 날짜 및 시간을 지정하는 옵션.]({% image_buster /assets/img/gmail_promo_deal_cards.png %}){: style="max-width:70%;"}

{% endtab %}
{% tab 프로모션 카드 %}

제품 캐러셀의 프로모션 카드는 오퍼에 이미지를 제공하는 데 유용합니다. 제품 캐러셀의 변수를 커스터마이즈하고 최대 10개의 이미지 미리보기를 포함할 수 있으며, 각 이미지는 고유해야 합니다.

1. **프로모션 카드** 토글을 선택합니다.
2. **프로모션 카드 추가**를 선택합니다. 제품 캐러셀의 각 이미지는 고유한 URL을 가져야 하며 동일한 종횡비(4:5, 1:1, 1.91:1)를 사용해야 합니다.
3. 이미지 URL을 포함합니다.
4. **타겟 URL**에 프로모션 링크를 추가합니다.
...
{: start="5"}
5. 헤드라인, 통화, 가격 및 할인 값을 추가하여 프로모션 카드를 커스터마이즈합니다.
...
Gmail 프로모션 카드를 구축한 후 **HTML 코드 생성**을 선택합니다. 스크립트를 복사하여 이메일 HTML의 `<head>` 요소에 붙여넣습니다.

{% alert tip %}
드래그 앤 드롭 편집기의 경우, 생성된 HTML 코드를 **발송 설정** 아래의 [커스텀 헤드 태그]({{site.baseurl}}/user_guide/channels/email/drag_and_drop/#custom-head-tags) 섹션에 복사하여 붙여넣습니다.
{% endalert %}
...
### 이미지 포함 {#incorporate-images}

Gmail은 이메일 메시지와 관련된 강력한 이미지를 사용할 때 더 나은 결과를 보였습니다. Gmail은 이 공간이 이메일 마케팅에 필수적인 시각적 언어를 미리보기에 가져오도록 설계되었으므로 텍스트만 사용하는 디자인을 권장하지 않습니다. 텍스트가 잘린 이미지를 사용하거나 여러 캠페인에서 이미지를 반복 사용하지 마세요.

### 오퍼 설명 {#describe-offers}
...
[미리보기 도구](https://developers.google.com/workspace/gmail/promotab/preview)를 사용하여 주석을 미리보기하세요. 주석은 이메일이 상당수의 수신자에게 발송되어야만 렌더링되므로 테스트 이메일을 자신에게 보내는 것은 주석에 대해 작동하지 않습니다. 최종 이메일(이미지 URL 포함)을 최소 100명의 Gmail 수신자에게 발송하세요.
...
Google의 개인정보 보호 및 안전 규정으로 인해 주석이 포함된 이메일은 주석이 작동하려면 광범위하게 발송되어야 합니다. 캠페인을 시작하고 Google의 시스템이 "대량 발송"으로 감지할 수 있도록 최소 100명의 수신자에게 발송하는 것이 권장됩니다. 이미지 URL은 수신자마다 다를 수 없습니다.
