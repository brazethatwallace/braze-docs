...
- 각 IP 풀의 IP 주소 수
- 클릭 추적 도메인에 사용할 접미사(예: "clicks" 또는 "click", "links" 또는 "link")
...
| IP 풀 | IP 수 | 구성 세트 | 발송 도메인 | 클릭 추적 도메인 |
| --- | --- | --- | --- | --- |
...
### 각 발송 도메인에 대해 하나의 클릭 추적 도메인 사용

| 마케팅 IP 풀 | 구성 세트 | 발송 하위 도메인 | 클릭 추적 도메인 |
| --- | --- | --- | --- |
...
### 모든 발송 도메인에 대해 하나의 클릭 추적 도메인 사용

이 방식은 클릭 추적 도메인이 구성 세트의 발송 도메인 중 하나 이상과 일치해야 한다는 규칙에 기반합니다.
...
- Amazon SES의 IP 풀은 IP 주소 자체만 호스팅하며, 구성 세트가 발송 도메인과 클릭 추적 도메인을 호스팅합니다.
- 각 구성 세트에는 한 번에 하나의 IP 풀만 할당할 수 있지만, 동일한 IP 풀을 사용하면서 서로 다른 발송 도메인을 가진 여러 구성 세트를 생성할 수 있습니다.
...
- [SSL 설정]({{site.baseurl}}/user_guide/channels/email/email_setup/ssl)