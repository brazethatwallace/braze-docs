...
일반적으로 "이상적인 수치"는 반송률이 3%를 넘지 않으면서 메시지의 약 98%가 전달되는 것입니다. 메시지의 98% 미만이 전달되면 보통 우려할 만한 원인이 있습니다.
...
Outlook은 종종 표준 브라우저 렌더링 대신 Microsoft Word 렌더링을 사용하므로, 이미지가 잘못 렌더링되거나 이미지 주위에 테두리가 추가될 수 있습니다.
