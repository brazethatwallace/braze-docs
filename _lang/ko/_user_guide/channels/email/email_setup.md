...
| 요구 사항 | 설명 | 소스 |
|---|---|---|
| 전용 IP(인터넷 프로토콜) | 전용 IP는 단일 호스팅 계정에 독점적으로 제공되는 고유한 인터넷 주소입니다. | Braze는 이메일 발신자 평판을 제어할 수 있도록 전용 IP를 제공합니다. Braze 온보딩 팀이 이를 설정해 드립니다.|
| 화이트라벨 도메인 | 도메인과 서브도메인으로 구성됩니다. 화이트라벨링을 사용하면 DKIM 및 SPF에 대한 이메일 인증 검사를 통과할 수 있습니다. | Braze 온보딩 팀이 이러한 도메인을 생성해 드리지만, 이름은 직접 선택해야 합니다. |
| 서브도메인 | 이메일 주소 내 도메인의 하위 구분(예: "@news.company.com")입니다. 서브도메인을 사용하면 회사의 공식 이메일 평판에 손상을 줄 수 있는 오류를 방지할 수 있습니다. | 온보딩 팀이 이를 생성해 드리지만, 서브도메인 이름은 직접 결정해야 합니다. 현재 Braze 외부에서 사용 중인 서브도메인은 사용할 수 없습니다. |
| IP 풀 | 서로 다른 유형의 이메일(예: "프로모션"과 "트랜잭션")의 평판을 분리하여 한쪽의 평판이 다른 쪽에 영향을 미치지 않도록 하고 더 높은 전달 가능성을 지원하는 데 사용되는 선택적 구성입니다. | 온보딩 팀이 풀을 설정해 드립니다. 그런 다음 이메일을 작성할 때 **타겟 오디언스** 단계에서 이메일의 IP 풀을 확인할 수 있습니다.|
