...
스팸 필터는 메시지의 HTML 버전과 일반 텍스트 버전을 모두 확인하므로, 일반 텍스트 대체 요소를 활용하면 스팸 점수를 낮추는 데 효과적입니다. 또한 대체 텍스트(`alt=""`)는 사용자의 이메일 공급자에 의해 필터링되었을 수 있는 이메일 본문의 이미지를 보완하거나 경우에 따라 대체하는 역할을 합니다. 스크린 리더는 이미지를 설명하기 위해 대체 텍스트를 읽어주므로, 이미지에 대한 핵심 정보를 쉬운 언어로 제공할 수 있는 좋은 기회입니다.

{% alert note %}
대체 텍스트에 따옴표가 포함된 경우, 큰따옴표(`"`) 대신 작은따옴표(`'`)를 사용하세요. 큰따옴표는 HTML 속성이 조기에 닫혀 텍스트가 잘릴 수 있습니다. 예를 들어, `alt="Product 'Premium' Edition"`은 올바르게 작동하지만, `alt="Product "Premium" Edition"`은 잘립니다.
{% endalert %}
...
이메일 유효성 검사는 사용자의 이메일 주소가 업데이트되거나 API, CSV 업로드, SDK를 통해 Braze로 가져오거나 대시보드에서 수정될 때 수행됩니다. 이메일 주소에는 공백을 포함할 수 없으며, API를 사용하여 전송할 경우 공백이 있으면 `400` 오류가 발생할 수 있습니다.

Braze 서버를 통해 타겟팅되는 이메일 주소는 [RFC 2822](https://datatracker.ietf.org/doc/html/rfc2822) 표준에 따라 유효성이 검사되어야 합니다. Braze는 특정 문자를 허용하지 않으며 해당 문자를 유효하지 않은 것으로 인식합니다. 이메일이 반송되면 Braze는 해당 이메일을 유효하지 않은 것으로 표시하며 가입 상태는 변경되지 않습니다.
...
UTM 매개변수는 분석을 위해 URL에 태그를 지정합니다. Liquid 및 커스텀 속성을 사용하여 빌드할 수 있습니다.
...
사용자가 동일한 이메일을 두 번 받은 것이 아니라, 같은 제목란을 가진 두 개의 별도 이메일을 받았을 수 있습니다. Campaign 또는 Canvas를 복제할 때 이미지나 제목란과 같은 기본 이메일 구성 세부 사항을 놓치기 쉽습니다.

조사 방법:

1. 고객 프로필을 확인하고 사용자가 수신한 각 Canvas 및 Campaign을 검토합니다.
2. 변경 로그를 검토하여 Campaign 또는 Canvas가 출시 후 수정되었는지 확인합니다. 사용자가 이메일을 수신한 시점에 Campaign 또는 Canvas의 제목란이 원본과 동일했을 수 있습니다.
