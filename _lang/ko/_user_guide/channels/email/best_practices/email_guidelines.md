...
- 모바일과 데스크톱 버전 모두에 하나의 이메일 템플릿을 사용하려면 너비를 500픽셀 미만으로 유지하세요.
- 이미지는 5&nbsp;MB 미만이어야 합니다. 최대 호환성을 위해 PNG, JPEG 또는 GIF 사용을 권장합니다. 많은 주요 이메일 클라이언트가 아직 SVG와 WebP를 지원하지 않으므로 사용을 피하세요.
- 이미지에 높이와 너비를 설정하지 마세요. 저하된 이메일에서 불필요한 여백이 발생할 수 있습니다.
- 대부분의 이메일 클라이언트가 `div` 태그 사용을 지원하지 않으므로 사용하지 마세요. 대신 중첩 테이블을 사용하세요.
...
- 모바일에서는 이미지 열이 좁지만(각 약 100px), 여러 이미지 행은 여전히 맞습니다(예: 4개의 이미지 ≈ 4개의 사용 가능한 열).

## 대체 텍스트 {#alternative-text}

스팸 필터는 메시지의 HTML 버전과 일반 텍스트 버전을 모두 확인하므로, 일반 텍스트 대체 텍스트를 활용하면 스팸 점수를 낮추는 데 효과적입니다. 또한 대체 텍스트 `(alt="")`는 사용자의 이메일 공급자에 의해 필터링되었을 수 있는 이메일 본문의 이미지를 보완하거나 경우에 따라 대체하는 역할을 할 수 있습니다. 스크린 리더는 이미지를 설명하기 위해 대체 텍스트를 읽어주므로, 이미지에 대한 핵심 정보를 쉬운 언어로 제공할 수 있는 좋은 기회입니다.

대체 텍스트가 표시되는 방식은 Braze가 아닌 수신자의 이메일 클라이언트에 의해 결정됩니다. Gmail, Outlook, Apple Mail 등 클라이언트별 동작에 대한 자세한 내용은 [이메일 클라이언트의 대체 텍스트 표시 방식]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/accessibility#how-email-clients-display-alt-text)을 참조하세요.
...
- 먼저 커스텀 HTML/CSS를 제거합니다
- 미리보기에서 커스텀 폰트가 올바르게 로드되는지 확인합니다
- 행과 열의 패딩을 확인합니다
- 테이블 기반 레이아웃을 사용하고, 에디터의 너비 범위 내에서 작업합니다.
...
사용자가 중복 이메일을 수신했다고 보고하는 경우, 다음 시나리오를 통해 원인을 파악할 수 있습니다.

### Campaign 또는 Canvas 생성 시 구성 오류 {#configuration-error-at-campaign-or-canvas-creation}

사용자가 동일한 이메일을 두 번 받은 것이 아니라, 같은 제목란을 가진 두 개의 별도 이메일을 받았을 수 있습니다. Campaign 또는 Canvas를 복제할 때 이미지나 제목란과 같은 기본적인 이메일 구성 세부 사항을 놓치기 쉽습니다.

조사 방법:

1. 고객 프로필을 확인하고 사용자가 수신한 각 Canvas 및 Campaign을 검토합니다.
2. 변경 로그를 검토하여 Campaign 또는 Canvas가 출시 후 수정되었는지 확인합니다. 사용자가 수신한 시점에 Campaign 또는 Canvas의 제목란이 원본과 동일했을 수 있습니다.

### Campaign이 여러 번 발송된 경우 {#campaign-sent-multiple-times}

발송된 메시지 수가 오디언스의 사용자 수보다 현저히 많은 경우, Campaign이 여러 번 실행되었을 수 있습니다.
