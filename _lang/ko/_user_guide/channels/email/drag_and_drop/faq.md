...
일부 이메일 클라이언트는 다크 모드에서 배경 이미지를 대체하거나 저대비 텍스트를 반전시키므로, 본문 텍스트가 누락되거나 클라이언트 간에 다르게 렌더링될 수 있습니다(예: iOS의 Gmail과 Android의 Gmail). 밝은 배경을 위해 배경 이미지에만 의존하지 말고 외부 컨테이너와 주요 섹션에 `background-color`를 설정하세요.

## 드래그 앤 드롭 이메일 미리보기에서 커스텀 폰트가 표시되지 않는 이유는 무엇인가요? {#why-doesnt-my-custom-font-appear-in-drag-and-drop-email-preview}

커스텀 폰트는 메시지의 **텍스트** 블록이 해당 폰트를 참조할 때 편집기 미리보기에서 로드됩니다. **드래그 앤 드롭 이메일 편집기** 설정에서 커스텀 폰트를 구성한 후에도 미리보기에 대체 폰트가 표시되면, 해당 폰트를 사용하는 **텍스트** 블록을 추가하여 편집기가 미리보기용으로 폰트를 로드하도록 하세요. 폰트 파일에 교차 출처 리소스 공유(CORS)가 활성화되어 있는지 확인하세요. 발송 전에 **미리보기 및 테스트**와 대상 이메일 클라이언트를 다시 확인하세요. 설정 단계는 [커스텀 폰트]({{site.baseurl}}/user_guide/channels/email/customize/email_global_style_settings#custom-font)를 참조하세요.
...
## 이메일 미리보기에서 Content Block이 렌더링되지 않는 이유는 무엇인가요? {#why-doesnt-my-content-block-render-in-email-preview}

Content Block이 이메일 미리보기에서 렌더링되지 않는 경우, 닫히지 않은 앵커 태그가 있는지 확인하세요. 연결된 콘텐츠 URL의 경우, `replace` 필터를 사용하여 이중 인코딩된 앰퍼샌드(`&amp;amp;`)를 단일 인코딩된 앰퍼샌드(`&amp;`)로 변환하세요. Content Block 중첩은 두 단계까지로 제한하세요.

## 드래그 앤 드롭 Content Block이 커스텀 코드 블록 내에서 모바일 스타일을 잃는 이유는 무엇인가요? {#why-does-a-drag-and-drop-content-block-lose-mobile-styling-inside-a-custom-code-block}

드래그 앤 드롭 **Content Block**을 **커스텀 코드**(HTML) 블록 안에 배치하면, Content Block의 모바일 전용 스타일과 정렬이 발송된 메시지에 적용되지 않을 수 있습니다. Content Block과 템플릿 모두 드래그 앤 드롭 편집기를 사용하는 경우, 커스텀 코드 안에 중첩하는 대신 Content Block을 별도의 행으로 추가하세요.

여러 Content Block을 쌓을 때는 하나의 행에 여러 블록을 배치하는 대신 각 블록에 별도의 행을 사용하세요.
