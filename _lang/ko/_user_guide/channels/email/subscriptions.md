...
사용자의 글로벌 이메일 구독 상태가 변경되면 Braze는 동일한 이메일 주소를 공유하는 다른 프로필에 해당 상태를 전파하며, 변경당 최대 100개의 프로필까지 처리합니다. 동일한 이메일 주소를 공유하는 프로필이 100개를 초과하는 경우 Braze는 전파를 보장하지 않습니다. 동일한 이메일을 공유하는 사용자들의 구독 상태가 다르게 표시되면 Braze 고객지원에 문의하세요.
