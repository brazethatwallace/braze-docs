...
구독 그룹은 [글로벌 구독 상태](#subscription-states)에서 오디언스를 더 세분화할 수 있는 Segment 필터입니다. 이러한 그룹을 통해 최종 사용자에게 보다 세분화된 구독 옵션을 제공할 수 있습니다.

{% multi_lang_include alerts/note_alerts.md alert='subscription group limit' %}

예를 들어, 여러 카테고리의 이메일 Campaign(프로모션, 뉴스레터, 제품 업데이트)을 발송한다고 가정해 보겠습니다. 이 경우 구독 그룹을 사용하여 고객이 [이메일 환경설정 센터](#email-preference-center)를 통해 단일 페이지에서 구독하거나 탈퇴할 이메일 카테고리를 직접 선택할 수 있도록 할 수 있습니다. 또는 구독 그룹을 사용하여 일간, 주간, 월간 이메일 구독 그룹을 만들어 고객이 이메일 수신 빈도를 선택할 수 있도록 할 수도 있습니다.
...
3. 구독 그룹에 이름과 설명을 입력합니다. 워크스페이스 내 각 구독 그룹은 고유한 이름을 가져야 합니다. 이미 존재하는 이름을 선택하면 대시보드에 오류가 표시되며 변경 사항이 저장되지 않습니다.
4. **저장**을 선택합니다.
...
### 구독 그룹으로 세그먼팅하기 {#segmenting-with-a-subscription-group}

Segment를 만들 때 구독 그룹 이름을 필터로 설정하여 해당 그룹에 옵트인한 사용자를 타겟팅합니다. 이 기능은 월간 뉴스레터, 쿠폰, 멤버십 등급 등에 유용합니다.

![필터를 사용하여 '주간 이메일' 구독 그룹에 속한 사용자를 '이탈 사용자' Segment에서 타겟팅하는 예시.]({% image_buster /assets/img/segment_sub_group.png %}){: style="max-width:90%"}
...
**구독 그룹** 페이지의 **구독 그룹 시계열** 그래프를 참조하여 일정 기간 동안의 사용자 수를 기반으로 구독 그룹 크기를 확인할 수 있습니다. 이러한 구독 그룹 크기는 Segment 크기 계산 등 Braze의 다른 영역과도 일관됩니다. 매우 큰 워크스페이스의 경우 Braze는 정확한 수치 대신 추정 수치를 표시할 수 있습니다.
...
오늘의 구독 그룹 크기는 기본적으로 계산되지 않습니다. 날짜 범위에 오늘이 포함된 경우 **오늘의 통계 계산**을 선택하여 시계열에 오늘의 값을 추가합니다.

시계열 수치가 **이메일 구독 상태가 탈퇴됨**을 사용하는 Segment와 크게 다른 경우, 그래프는 해당 구독 그룹의 멤버십을 집계하는 반면 해당 필터는 글로벌 이메일 구독 상태를 반영한다는 점을 기억하세요(예: 사용자가 글로벌로는 구독 상태이지만 특정 그룹에서는 탈퇴한 경우).

#### 구독 그룹 수치와 Segment 수치가 다를 수 있는 이유 {#why-subscription-group-counts-can-differ-from-segment-counts}

구독 그룹 크기는 **구독 그룹** 필터만 사용하는 Segment와 일치합니다. 특정 그룹의 멤버십이 아닌 [글로벌 이메일 구독 상태](#subscription-states)를 반영하는 **이메일 구독 상태**를 사용하거나 여러 필터를 결합하는 Segment와는 차이가 날 수 있습니다. 예를 들어, 사용자가 글로벌로는 이메일에 구독되어 있지만 특정 구독 그룹에서는 탈퇴한 경우가 있을 수 있습니다.

사용자의 글로벌 구독 상태와 구독 그룹 멤버십을 비교하려면 해당 사용자의 프로필로 이동하여 **인게이지먼트** 탭을 선택합니다. 글로벌 상태별 필터링에 대해서는 [사용자 구독별 세그먼팅](#segmenting-by-user-subscriptions)을 참조하세요.

#### Campaign 분석에서 구독 그룹 확인하기 {#viewing-subscription-groups-in-campaign-analytics}

특정 이메일 Campaign에서 구독 상태를 변경(구독 또는 탈퇴)한 사용자 수를 해당 Campaign의 분석 페이지에서 확인할 수 있습니다.

1. Campaign의 **Campaign 분석** 페이지에서 **이메일 메시지 성능** 섹션까지 스크롤합니다.
2. **구독 그룹** 아래의 화살표를 선택하여 고객이 제출한 상태 변경의 집계 수치를 확인합니다.

![고객이 제출한 상태 변경의 집계 수치를 표시하는 '이메일 메시지 성능' 페이지.]({% image_buster /assets/img/campaign_analytics_sub_groups.png %})

### 사용자의 이메일 구독 그룹 확인하기 {#checking-a-users-email-subscription-group}

- **사용자 프로필:** Braze 대시보드의 [사용자 검색]({{site.baseurl}}/user_guide/audience/manage_audience/user_profiles#access-profiles) 페이지에서 개별 사용자 프로필에 접근할 수 있습니다. 여기에서 이메일 주소, 전화번호 또는 외부 사용자 ID로 사용자 프로필을 조회할 수 있습니다. **인게이지먼트** 탭에서 사용자의 이메일 구독 그룹도 확인할 수 있습니다.
