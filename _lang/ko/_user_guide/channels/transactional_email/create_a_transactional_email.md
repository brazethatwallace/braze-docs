...
{% alert important %}
트랜잭션 이메일은 추가 비용 없이 사용자를 타겟팅하는 데 사용할 수 있는 트랜잭션 캠페인과 다릅니다. 예를 들어, 트랜잭션 캠페인에는 사용자가 장바구니에 항목을 추가한 후 발송되는 메시지가 포함될 수 있습니다. 자세한 내용은 [오디언스 타겟팅 옵션]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/target_users/)을 확인하세요.
{% endalert %}
...
- **타겟 오디언스** 단계가 제거되었습니다. 트랜잭션 이메일은 전체 사용자 기반(탈퇴한 사용자 포함)을 대상으로 등록하므로 필터나 세그먼트를 지정할 필요가 없습니다. 따라서 이 메시지를 받을 대상에 적용할 로직이 있는 경우, 특정 사용자에게 메시지를 트리거하기 위해 Braze에 API 요청을 보내기 전에 해당 로직을 적용하는 것을 권장합니다.
