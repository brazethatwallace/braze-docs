...
이제 트랜잭션 이메일 Campaign 설정을 진행할 수 있습니다.

## 2단계: Campaign 구성하기 {#step-2-configure-your-campaign}

트랜잭션 이메일 Campaign의 생성 플로우는 [표준 이메일 Campaign]({{site.baseurl}}/user_guide/channels/email/html_editor)에 비해 간소화되어 있으므로, 비즈니스에 중요한 트랜잭션 이메일이 모든 사용자에게 도달할 수 있도록 보장합니다.

따라서 다른 Braze Campaign 유형에서 익숙할 수 있는 여러 설정이 이 Campaign 유형을 설정할 때 필요하지 않다는 것을 알 수 있습니다.

- **전송** 단계가 예약 옵션을 제거하여 간소화되었습니다. 트랜잭션 이메일은 항상 **전송** 페이지에 표시된 Campaign ID를 사용하여 Braze REST API를 통해 트리거됩니다. 재적격성 제어 및 최대 게재빈도 설정과 같은 추가 설정도 제거되어, 서비스가 발송 요청을 트리거할 때 모든 사용자가 이러한 중요한 트랜잭션 알림을 받을 수 있도록 합니다.
- **타겟 오디언스** 단계가 제거되었습니다. 트랜잭션 이메일은 탈퇴한 사용자를 포함하여 전체 사용자 기반을 적격 대상으로 등록하므로, 필터나 Segments를 지정할 필요가 없습니다. 따라서 이 메시지를 수신할 대상에 적용할 로직이 있는 경우, 특정 사용자에게 메시지를 트리거하기 위한 API 요청을 Braze에 보낼지 결정하기 전에 해당 로직을 적용하는 것을 권장합니다.
- **전환** 단계가 제거되었습니다. 트랜잭션 이메일은 현재 전환 이벤트 추적을 지원하지 않습니다.

![트랜잭션 이메일 Campaign을 생성하기 위한 작성, 전송, 확인 워크플로우.]({% image_buster /assets/img/transactional_campaign_compose.png %}){: width="1586" height="1112" style="max-width:80%;height:auto;"}

트랜잭션 이메일 Campaign을 구성하려면 다음 단계를 따르세요:

1. 메시지를 발송한 후 **Campaigns** 페이지에서 결과를 찾을 수 있도록 설명이 포함된 이름을 추가합니다.
2. 이메일을 작성하거나 템플릿에서 선택합니다.
3. `campaign_id`를 메모해 두세요. API Campaign을 저장한 후, [트랜잭션 이메일 엔드포인트]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_transactional_message) 문서에 명시된 대로 생성된 `campaign_id` 필드를 API 요청에 포함해야 합니다.
4. **Campaign 저장**을 클릭하면 API Campaign을 시작할 준비가 완료됩니다!

{% alert note %}
트랜잭션 이메일 Campaign의 원클릭 목록 탈퇴 설정은 다른 이메일 Campaign과 마찬가지로 기본값이 **워크스페이스 기본값 사용**으로 설정되어 있습니다. 이 설정은 트랜잭션 메시징을 위한 것이므로, Braze는 원클릭 탈퇴를 추가하지 않습니다. 이 Campaign 유형에 원클릭 탈퇴를 추가하려면, **발송 정보**에서 [이 설정을 편집]({{site.baseurl}}/user_guide/administer/global/workspace_settings/email_preferences#message-level-one-click-list-unsubscribe)하세요.
{% endalert %}
...
`Connected Content` 태그를 사용하면 발송 프로세스 중에 Braze가 외부 API 요청을 수행해야 하며, 요청하는 외부 서비스에 지연이 발생할 경우 메시지 발송 프로세스가 느려질 수 있습니다. 마찬가지로, `Promotion Code` 태그를 사용하면 발송 전에 프로모션 코드의 가용성을 평가하기 위해 Braze가 추가 처리를 수행해야 하며, 프로모션 코드가 사용 가능하지 않은 경우 발송 프로세스가 느려질 수 있습니다.

따라서 트랜잭션 이메일 Campaign의 어떤 필드에서도 `Connected Content` 또는 `Promotion Code` 태그를 포함하는 것을 지원하지 않습니다.