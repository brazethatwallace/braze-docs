...
2. 여러 채널을 타겟팅하는 Campaign의 경우 **멀티채널**을 선택합니다. 그렇지 않으면 **푸시 알림**을 선택합니다. 아직 확실하지 않다면 아래의 **일반 또는 멀티채널 푸시 캠페인 결정하기**를 참조하세요.
...
모바일, 웹, Kindle, iOS, Android 등 여러 기기와 플랫폼을 타겟팅하려는 경우, 이 단계에서의 선택이 이후 일부 기능 및 설정의 사용 가능 여부에 영향을 줄 수 있습니다.

멀티채널 또는 푸시 알림 캠페인을 만들기 전에 다음 의사결정 차트를 참조하세요:

!["캠페인 유형 선택을 위한 플로차트. 여러 기기와 플랫폼을 타겟팅하는지 결정하는 것으로 시작합니다. 아니오인 경우 '푸시 알림 선택'으로 이동합니다. 예인 경우 '어떤 유형의 푸시 메시지인가요?'를 묻고, 옵션은 '표준 푸시'로 '기기별 설정이 필요한가요?'라는 결정 지점으로 이어집니다. 아니오인 경우 '푸시 알림을 선택하고 빠른 푸시를 사용합니다'로 이동합니다. 예인 경우 '멀티채널 선택'으로 이동합니다. '어떤 유형의 푸시 메시지인가요?'로 돌아가서, 답이 'Push Stories 또는 인라인 이미지'인 경우 '멀티채널 선택'으로 안내합니다."]({% image_buster /assets/img_archive/flowchart_quickpush.png %})
...
4. 필요에 따라 이 단계의 오디언스를 필터링합니다. Segment를 지정하고 추가 필터를 추가하여 이 단계의 수신자를 더 세밀하게 조정할 수 있습니다. 오디언스 옵션은 메시지가 발송되는 시점에 지연 후 확인됩니다.
...
빠른 푸시 캠페인을 만드는 경우 알림 유형은 자동으로 **표준 푸시**로 설정되며 변경할 수 없습니다.
...
![푸시 알림 만들기의 작성 탭.]({% image_buster /assets/img_archive/push_compose.png %})
...
{% alert tip %}
멋진 문구를 작성하는 데 도움이 필요하신가요? [AI 카피라이팅 어시스턴트]({{site.baseurl}}/user_guide/brazeai/generative_ai/copywriting/)를 사용해 보세요. 제품 이름이나 설명을 입력하면 AI가 메시징에 사용할 수 있는 사람이 쓴 것 같은 마케팅 문구를 생성합니다.
...
다음으로, Segment 또는 필터를 선택하여 오디언스를 좁혀 [사용자를 타겟팅]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/target_users/)해야 합니다. 해당 대략적인 Segment 인구가 어떻게 보이는지 자동으로 미리보기가 제공됩니다. Campaign이 타겟팅하는 채널에 대한 상세한 오디언스 통계는 하단에서 확인할 수 있습니다. 사용자 기반의 몇 퍼센트가 타겟팅되고 있는지와 이 Segment의 생애주기 가치를 확인하려면 **추가 통계 표시**를 선택하세요.
