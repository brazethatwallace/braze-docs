...
# 일반적인 푸시 오류 메시지 {#common-push-error-messages}
...
### 푸시 반송: BadToken {#push-bounced-badtoken}
...
- 프로비저닝 환경 불일치:
	...
	- 이 보고는 프로덕션에서는 반송을 전송하지만 개발 환경에서는 전송하지 않습니다.<br><br>
- 프로비저닝 프로필 불일치:
...
### Bundle ID not allowed {#bundle-id-not-allowed}
...
이것은 Android [DEVICE_UNREGISTERED](#device-unregistered) 오류의 iOS 버전입니다. 일반적으로 다음 이유 중 하나로 발생합니다:
...
### 푸시 반송: APNS 피드백 서비스에서 제거됨 {#push-bounced-apns-feedback-service-removed}

이는 일반적으로 사용자가 앱을 삭제할 때 발생합니다. Braze는 매일 밤 APNS 피드백 서비스를 쿼리하여 무효한 토큰 목록을 가져옵니다. 자세한 내용은 Apple의 [APNs와 통신하기](https://developer.apple.com/library/archive/documentation/NetworkingInternet/Conceptual/RemoteNotificationsPG/CommunicatingwithAPNs.html)를 참조하세요.
