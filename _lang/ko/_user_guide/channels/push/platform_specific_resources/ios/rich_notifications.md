...
{% alert note %}
2020년 1월 기준으로 iOS 리치 푸시 알림은 10&nbsp;MB 미만의 1038x1038 이미지를 처리할 수 있지만, 가능한 한 작은 파일 크기를 사용하는 것을 권장합니다. 실제로 큰 파일을 전송하면 불필요한 네트워크 부하가 발생하고 다운로드 시간 초과가 더 자주 발생할 수 있습니다.
{% endalert %}

{% alert important %}
이미지 파일 크기가 너무 크거나, 종횡비가 올바르지 않거나, 텍스트가 최대 메시지 길이를 초과하거나, 제목 텍스트가 최대 제목 길이를 초과하면 푸시 알림 이미지가 예상대로 표시되지 않을 수 있습니다.
{% endalert %}

### 글자 수 {#character-count}

푸시에 포함할 정확한 글자 수에 대한 엄격한 규칙을 제공할 수는 없지만, iOS 메시지를 디자인할 때 고려할 [몇 가지 가이드라인]({{site.baseurl}}/user_guide/channels/push/create_a_push_message/message_and_image_formats)을 제공합니다. 이미지 유무, 사용자 기기의 알림 상태 및 표시 설정, 기기 크기에 따라 차이가 있을 수 있습니다. 확실하지 않을 때는 짧고 간결하게 작성하세요.

모범 사례로, Braze는 모바일 푸시 알림에서 선택적 제목과 메시지 본문 모두 각 줄을 약 30~40자로 유지하는 것을 권장합니다.
...
<tbody>
  <tr>
    <td width="33%">가장 일반적인 시나리오입니다.<br><br><b>제목:</b> 1줄의 텍스트<br><b>본문:</b> 4줄의 텍스트<br><b>이미지:</b> 정사각형 썸네일</td>
    <td width="33%">사용자가 메시지를 길게 누를 때입니다.<br><br><b>제목:</b> 1줄의 텍스트<br><b>본문:</b> 7줄의 텍스트<br><b>이미지:</b> 2:1 종횡비(권장, 아래 참고 사항 참조)</td>
    <td width="33%">사용자가 휴대폰이 잠금 해제되어 활성 상태일 때 푸시를 받는 경우입니다.<br><br><b>제목:</b> 1줄의 텍스트<br><b>본문:</b> 2줄의 텍스트</td>
  </tr>
</tbody>
</table>
...
{% alert note %}
확장된 푸시 알림에는 2:1 종횡비를 권장하지만, 거의 모든 종횡비가 지원됩니다. 이미지는 항상 알림의 전체 너비에 걸쳐 표시되며, 높이는 그에 맞게 조정됩니다.
{% endalert %}

#### 텍스트 잘림의 변수 {#variables-in-text-truncation}
...
이미지가 있으면 본문 텍스트가 줄당 약 10자 정도 줄어듭니다.
...
iOS 15에서는 시간 민감 및 긴급 표시가 제목을 타임스탬프 없이 새 줄로 밀어내어 약간 더 많은 공간을 제공합니다.
...
- **휴대폰 표시 설정:** 사용자는 일반적으로 접근성을 위해 휴대폰의 전체 UI 글꼴 크기를 늘리거나 줄일 수 있습니다.
- **기기 너비:** 메시지가 작은 휴대폰이나 넓은 iPad에 표시될 수 있습니다.
- **콘텐츠 유형:** 이모지와 "m", "w"와 같은 넓은 문자는 "i"나 "t"보다 더 많은 공간을 차지하며, "engagement"와 같은 긴 단어는 짧은 단어보다 더 갑작스럽게 줄바꿈될 수 있습니다.
...
이 메시지를 iOS 10을 실행하는 기기를 가진 사용자에게만 보내도록 제한할 수도 있습니다. iOS 10으로 업그레이드하지 않은 사용자의 경우, **Only send to devices with Rich Notification support**를 선택 해제하면 리치 콘텐츠 없이 텍스트 전용 알림으로 표시됩니다.
...
![사용자가 푸시 알림을 받고 메시지를 세게 눌러 "Hello!"라고 표시된 확장 이미지를 보여주는 모습.]({% image_buster /assets/img_archive/rich_notification_ios.gif %}){: style="max-width:50%;" }