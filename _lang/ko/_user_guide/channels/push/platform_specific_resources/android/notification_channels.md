...
## Android O로 전환 {#transitioning-to-android-o}
...
Android O부터 푸시 알림을 표시하려면 유효한 채널이 필요합니다. 앱이 Android O 이상을 대상으로 하는 경우 Braze SDK 버전 2.1.0 이상을 사용해야 합니다. 개발 팀은 사용할 채널과 각 채널에 대한 권장 알림 설정(예: 중요도, 소리, 조명)을 애플리케이션 코드에서 정의해야 합니다. Android 개발자 설명서는 [여기](https://developer.android.com/preview/features/notification-channels.html)에서, Braze 개발자 설명서는 [여기]({{site.baseurl}}/developer_guide/platform_integration_guides/android/push_notifications/integration/standard_integration/#step-5-define-notification-channels)에서 확인할 수 있습니다.
...
대시보드 대체 채널이 "Marketing"이고 채널을 선택한 적이 없는 Android 푸시 메시지가 10개 있습니다. 이 캠페인들은 "Marketing" 채널이 대시보드 대체 채널이기 때문에 "Marketing" 채널을 통해 발송됩니다.
...
이 상황에서 이전에 "Marketing" 채널을 통해 발송되던 채널 선택이 없는 10개의 캠페인은 이제 "Updates" 채널을 통해 발송됩니다. 이러한 메시지는 대체 채널을 통해 발송되기 때문입니다. "Social Notifications" 채널을 통해 발송되던 15개의 메시지는 계속 "Social Notifications" 채널을 통해 발송됩니다. "Marketing" 채널을 통해 발송되던 5개의 메시지는 계속 "Marketing" 채널을 통해 발송됩니다.
...
2. 드롭다운에서 사용하려는 채널을 선택합니다. 드롭다운이 없고 다음과 같은 화면이 표시되는 경우, 캠페인에서 채널을 선택하기 전에 먼저 채널을 추가해야 합니다.
