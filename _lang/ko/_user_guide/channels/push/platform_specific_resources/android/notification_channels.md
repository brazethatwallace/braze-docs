...
이러한 채널이 생성되면 엔지니어가 관련 채널 ID를 마케팅 팀에 전달해야 합니다. 팀은 Campaign 및 Canvases에서 사용할 수 있도록 채널 이름과 채널 ID를 Braze 대시보드에 입력해야 합니다.
...
대시보드 대체 채널이 "Marketing"이고 채널을 선택한 적이 없는 Android 푸시 메시지가 10개 있습니다. 이 Campaign들은 "Marketing" 채널이 대시보드 대체 채널이기 때문에 "Marketing" 채널을 통해 발송됩니다.
...
이 상황에서 이전에 "Marketing" 채널을 통해 발송되던 채널 선택이 없는 10개의 Campaign은 이제 "Updates" 채널을 통해 발송됩니다. 이러한 메시지는 대체 채널을 통해 발송되기 때문입니다. "Social Notifications" 채널을 통해 발송되던 15개의 메시지는 계속 "Social Notifications" 채널을 통해 발송됩니다. "Marketing" 채널을 통해 발송되던 5개의 메시지는 계속 "Marketing" 채널을 통해 발송됩니다.
...
3. **Manage Notification Channels**를 선택합니다. 여기에 추가된 채널은 모든 Campaign 및 Canvases에서 전역적으로 사용할 수 있습니다. 채널을 관리하려면 워크스페이스에 대한 "Manage Apps" [권한]({{site.baseurl}}/user_guide/administer/global/user_management/permissions/#limited-and-team-role-permissions)이 있어야 합니다.
...
5. 추가하려는 알림 채널의 이름과 ID를 입력합니다.<br><br>![]({% image_buster /assets/img_archive/push_notifications_channels_manage.png %})<br><br>
6. 추가하려는 각 알림 채널에 대해 4단계와 5단계를 반복합니다.
7. **Save**를 눌러 변경 사항을 저장합니다.

## 대체 채널 지정 {#specifying-your-fallback-channel}

대체 채널은 메시지에 대한 채널을 선택하지 않은 경우 Braze가 Android 메시지를 발송하려고 시도하는 채널입니다. 채널 선택이 없는 Android 메시지가 있는 Campaign 및 Canvases는 팀이 Braze 대시보드에 채널을 추가하기 전에 생성된 Campaign 및 Canvases뿐입니다. 대체 채널을 변경하면 명시적인 채널 선택이 없는 모든 Campaign 및 Canvases에 전역적으로 변경 사항이 적용됩니다.
...
2. 드롭다운에서 사용하려는 채널을 선택합니다. 드롭다운이 없고 다음과 같은 화면이 표시되는 경우, Campaign에서 채널을 선택하기 전에 먼저 채널을 추가해야 합니다.
