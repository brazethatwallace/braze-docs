...
page_order: 5
page_type: reference
description: "증상 색인과 표준 조사 경로를 사용하여 푸시 전달, 클릭 동작 및 자격 증명 문제를 진단합니다."
channel: push
---

# 푸시 문제 해결 {#troubleshoot-push}

> 이 페이지를 사용하여 푸시 전달, 클릭 동작 및 자격 증명 문제를 해결하세요. SDK별 설정에 대해서는 [Braze SDK 푸시 알림 문제 해결]({{site.baseurl}}/developer_guide/push_notifications/troubleshooting)을 참조하세요. 오류 코드에 대해서는 [일반적인 푸시 오류 메시지]({{site.baseurl}}/user_guide/channels/push/push_error_codes)를 참조하세요.
...
글로벌 최대 게재빈도 설정을 확인하세요. 워크스페이스에 글로벌 최대 게재빈도 설정이 적용되어 있고 지정된 기간 동안 푸시 알림 한도에 이미 도달한 경우 푸시 알림을 받지 못했을 수 있습니다.

Campaign **분석** 페이지에서 최대 게재빈도 설정 배너를 확인하여 지난 30일 동안 Campaign을 받지 못한 대략적인 사용자 수를 확인하세요. 개별 발송을 조사하려면 [메시징 진단 대시보드]({{site.baseurl}}/user_guide/analytics/dashboards/dashboard_builder/diagnostics_dashboard)를 사용하고 **최대 게재빈도 설정 적용됨**으로 필터링하세요. 규칙을 검토하거나 변경하려면 [글로벌 최대 게재빈도 설정]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/frequency_capping#freq-cap-feat-over)을 참조하세요.
...
## 오류: MismatchSenderID {#error-mismatch-sender-id}

**증상:** Android 푸시가 `MismatchSenderID` 오류와 함께 실패합니다.

MismatchSenderID는 Firebase Cloud Messaging(FCM)의 인증 실패를 나타냅니다. Firebase 발신자 ID와 FCM API 키가 올바른지 확인하세요.

올바른 Firebase 서버 키를 찾아 교체하려면 다음을 수행하세요.

1. 앱의 Firebase 콘솔로 이동합니다.
2. **Project Overview**에서 **Project Settings**를 선택합니다.
3. **Cloud Messaging** 탭에서 API 키와 함께 나열된 발신자 ID가 Braze의 발신자 ID(**설정** > **앱 설정** > **Cloud Messaging API Key**)와 일치하는지 확인합니다.

{% alert warning %}
Braze 대시보드에서 발신자 ID를 변경하지 마세요. 변경하면 기존 푸시 등록이 무효화됩니다. 발신자 ID가 일치하지 않는 경우, 일치하는 발신자 ID가 있는 Firebase 프로젝트를 찾아야 합니다.
{% endalert %}

4. **Project credentials** 아래에서 **Server Key**를 복사합니다.
5. Braze에서 **설정** > **앱 설정**으로 이동하여 앱을 선택한 다음, 서버 키를 **Cloud Messaging API Key** 필드에 붙여넣습니다(기존 키를 교체).
6. **저장**을 선택합니다.
7. 확인하려면 API 키를 변경하기 전과 후에 앱을 열지 않은 상태에서 기기로 테스트 푸시를 발송합니다. 이렇게 하면 새로운 푸시 등록 ID(푸시 토큰)를 생성하지 않아도 사용자가 계속 푸시 알림을 받을 수 있는지 확인할 수 있습니다.
...
**증상:** 푸시 알림을 탭해도 앱이 열리지 않거나 설정된 대로 이동하지 않습니다.

푸시 알림을 클릭해도 앱이 열리지 않는 경우, 플랫폼에 따라 다음 사항을 확인하세요.

### Android

1. **클릭 시 동작 확인:** Campaign이 클릭 시 앱을 열도록 설정되어 있는지 확인하세요.
2. **딥링크 처리 확인:** `braze.xml` 파일에서 `com_braze_handle_push_deep_links_automatically`가 `true`로 설정되어 있는지 `false`로 설정되어 있는지 확인하세요.
   ...
   - `false`로 설정된 경우, 앱에서 푸시 수신 및 열림 인텐트를 수신하고 처리할 브로드캐스트 리시버가 필요합니다. 이 리시버가 올바르게 구현되어 있는지 확인하세요.
3. **상세 로그 수집:** [상세 로깅을 활성화]({{site.baseurl}}/developer_guide/sdk_integration/verbose_logging)하고, 문제를 재현한 후, `braze.xml` 및 `AndroidManifest.xml`과 함께 로그를 Braze 고객지원에 제공하세요.
...
3. **상세 로그 수집:** [상세 로깅을 활성화]({{site.baseurl}}/developer_guide/sdk_integration/verbose_logging)하고, 문제를 재현한 후, 로그를 Braze 고객지원에 제공하세요.

## 푸시 클릭 시 예기치 않게 앱에서 열림 {#push-clicks-unexpectedly-open-in-app}

**증상:** 푸시 알림의 링크가 기기의 웹 브라우저 대신 앱 내에서 열립니다.
...
**증상:** iOS 푸시 자격 증명을 레거시 인증서에서 `.p8` 키로 마이그레이션해야 하거나, 자격 증명 변경 후 푸시 전달이 실패했습니다.
...
## 웹 푸시 알림이 예상대로 작동하지 않는 경우 {#web-push-notifications-are-not-behaving-as-expected}

**증상:** 브라우저 푸시 알림이 표시되지 않거나 사이트 권한이 멈춘 것처럼 보입니다.

브라우저에서 푸시 알림에 문제가 발생하는 경우, 사이트의 알림 권한을 재설정하고 사이트의 저장소를 지워야 할 수 있습니다. 다음 단계를 참고하세요.

{% tabs %}
{% tab Chrome %}

### 데스크톱에서 Chrome 재설정 {#reset-chrome-on-desktop}

1. Chrome 브라우저에서 URL 옆에 있는 **사이트 정보 보기** 슬라이더 아이콘을 선택합니다.
2. **알림** 아래에서 **권한 재설정**을 선택합니다.
3. Chrome DevTools를 엽니다. 운영 체제별 관련 단축키는 다음과 같습니다.
...
{: .reset-td-br-1 .reset-td-br-2 aria-label="데스크톱에서 Chrome 재설정" }

{:start="4"}
4. DevTools에서 **Application** 탭으로 이동합니다.
5. 사이드바에서 **Storage**를 선택합니다.
6. **Clear site data**를 선택합니다.
7. Chrome에서 업데이트된 설정을 적용하기 위해 페이지를 새로고침하라는 메시지가 표시됩니다. **Reload**를 선택합니다.

푸시 권한이 재설정되었습니다. 사이트에 새 탭을 열고 테스트해 보세요.

### Android에서 Chrome 재설정 {#reset-chrome-on-android}

사이트의 알림이 Android 알림 서랍에 표시되어 있는 경우:

1. 푸시 알림에서 <i class="fas fa-cog" title="설정"></i> **설정**을 선택한 다음 **사이트 설정**을 선택합니다.
...
4. 알림이 **보내기 전에 확인(권장)**으로 설정되어 있는지 확인합니다.
5. 목록에서 사이트를 찾습니다.
...
### 데스크톱에서 Firefox 재설정 {#reset-firefox-on-desktop}

1. 사이트 URL 옆에서 <i class="fa-solid fa-circle-info" alt="정보 아이콘"></i> 또는 <i class="fas fa-lock" alt="잠금 아이콘"></i>을 선택합니다.
2. **권한** 아래에서 **알림 수신** 옆에 있는 <i class="fa-solid fa-circle-xmark" title="이 권한을 지우고 다시 묻기"></i> **권한 지우기**를 선택하여 알림 권한을 지웁니다.
...
1. Safari를 엽니다.
2. [Mac 메뉴 막대](https://support.apple.com/guide/mac-help/whats-in-the-menu-bar-mchlp1446/mac)에서 **Safari** > **설정** > **웹사이트** > **알림**으로 이동합니다.
3. 목록에서 사이트를 선택합니다.
4. **제거**를 선택하여 해당 사이트의 알림 권한을 삭제합니다.
5. 그런 다음 **개인 정보 보호** > **웹사이트 데이터 관리**로 이동합니다.
6. 목록에서 사이트를 선택합니다.
...
**증상:** 특정 푸시 오류 코드(예: `DEVICE_UNREGISTERED`, `Unregistered` 또는 `NotRegistered`)가 표시됩니다.
