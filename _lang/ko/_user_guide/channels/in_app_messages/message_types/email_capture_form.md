...
> 이메일 캡처 메시지를 사용하면 사이트 사용자에게 이메일 주소를 제출하도록 쉽게 안내할 수 있으며, 제출된 이메일 주소는 고객 프로필에 저장되어 모든 메시징 캠페인에서 활용할 수 있습니다.

이 메시지 유형은 [기존 에디터]({{site.baseurl}}/user_guide/channels/in_app_messages/traditional/)에서 사용할 수 있습니다.

## 작동 방식 {#how-it-works}

최종 사용자가 이 양식에 이메일 주소를 입력하면, 해당 이메일 주소가 고객 프로필에 추가됩니다.
...
- 고객 프로필에 이메일 주소가 이미 존재하는 경우, 기존 이메일 주소가 새로 입력된 이메일 주소로 덮어쓰기됩니다.
- 알려진 사용자의 이메일 주소가 [하드바운스]({{site.baseurl}}/help/help_articles/email/email_bounces/#email-bounces)로 표시된 경우, 새로 입력된 이메일 주소가 Braze 프로필에 있는 주소와 다른지 확인합니다. 제공된 이메일 주소가 다르면 이메일 주소가 업데이트되고 하드바운스 상태가 제거됩니다.
...
일반적으로 이메일 캡처 양식의 로직은 간단합니다. 현재 활성 상태인 사용자의 Braze 고객 프로필에 이메일 주소를 설정합니다. 그러나 사용자가 식별되었는지(로그인 상태, `changeUser` 호출됨) 여부에 따라 동작이 달라집니다.
