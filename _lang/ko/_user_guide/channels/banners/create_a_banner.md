...
Banner를 시작하기 전에 개발팀에서 [앱 또는 웹사이트에 배치를 설정]({{site.baseurl}}/developer_guide/banners/placements)해야 합니다. 그동안 Banner Campaign 초안을 작성할 수는 있지만, 배치가 구성될 때까지 Campaign을 시작할 수는 없습니다.
...
메시지를 Campaign으로 보낼지 Canvas로 보낼지 잘 모르시겠나요? Campaign은 단일 타겟팅 메시징 캠페인에 적합하고, Canvas는 다단계 사용자 여정에 더 적합합니다.
...
7. 배너 Campaign의 시작 날짜와 시간을 선택합니다. 기본적으로 배너는 무기한 지속됩니다. **종료 시간**을 선택하고 종료 날짜와 시간을 지정하여 변경할 수 있습니다.
...
사용자가 배너의 링크를 클릭하면 앱 내부로 더 깊이 이동하거나 다른 웹페이지로 리디렉션할 수 있습니다. 또한 [커스텀 속성 또는 이벤트를 기록]({{site.baseurl}}/developer_guide/analytics)하도록 선택하여 사용자가 배너를 클릭할 때 커스텀 데이터로 사용자 프로필을 업데이트할 수 있습니다. 더 세분화된 클릭 추적을 위해 속성 패널의 **보고용 식별자** 필드를 사용하여 각 인터랙티브 요소에 커스텀 식별자를 할당합니다.
...
**닫기 동작** 섹션에서 **배너를 닫을 수 있음** 체크박스를 선택하여 사용자가 배너를 닫을 수 있도록 합니다. 이 기능은 광범위한 오디언스에게 기간 한정 프로모션을 홍보하면서도 관심 없는 사용자가 메시지를 숨길 수 있도록 하려는 경우에 유용합니다.
...
기본적으로 배너를 닫은 사용자는 해당 Campaign에 대해 다시 적격하지 않습니다. 닫은 사용자가 배너를 다시 볼 수 있도록 하려면 **전달 제어** 단계로 이동하여 **사용자가 Campaign을 다시 수신할 수 있도록 허용**을 선택합니다. 활성화되면 분, 시간, 일 또는 주 단위로 쿨다운 기간을 설정합니다.

카운트다운은 사용자가 배너를 닫은 시점부터 시작됩니다. 기간이 만료되면 사용자는 자동으로 재적격 상태가 되며, Campaign을 다시 시작할 필요가 없습니다. 재적격성은 사용자별, Campaign별로 추적됩니다.
...
{:start="2"}
2. **전환 할당**에서 최대 30일의 기간 내에 해당 행동을 전환으로 집계하는 전환 이벤트를 정의하여 사용자가 Campaign을 수신한 후 특정 행동을 수행하는 빈도를 추적합니다.

#### 전환 이벤트 선택 {#choose-conversion-events}

Braze에서는 사용자가 Campaign을 수신한 후 특정 행동을 수행하는 빈도인 [전환 이벤트]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/conversion_events)를 추적할 수 있습니다. 사용자가 지정된 행동을 취하면 전환으로 집계되는 최대 30일의 기간을 허용할 수 있습니다.
...
Canvas 배너 단계의 재적격성을 제어하려면 Canvas 재진입 설정을 사용합니다. 자세한 내용은 [Campaign 및 Canvas의 재적격성]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/re_eligibility)을 참조하세요.
