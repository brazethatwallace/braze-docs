...
3. **Business portfolio** 드롭다운에서 비즈니스 포트폴리오를 선택한 다음 **Next**를 선택합니다. 이것은 WhatsApp Business 계정에 연결되므로, 예상한 비즈니스 포트폴리오가 보이지 않으면 권한을 확인하세요.<br><br>![비즈니스 포트폴리오 이름을 포함한 비즈니스 정보를 입력하는 필드가 있는 창.]({% image_buster /assets/img/whatsapp/business_info.png %}){: style="max-width:50%;"}<br><br>
...
6. 전화번호를 입력하고 **Text message** 또는 **Phone call**을 선택합니다. 이 번호는 다른 WhatsApp 계정에 등록되어 있지 않아야 하는 것을 포함하여 WhatsApp 전화번호의 모든 요구 사항을 충족해야 합니다. <br><br>![전화번호를 추가하는 필드.]({% image_buster /assets/img/whatsapp/add_phone_number.png %}){: style="max-width:50%;"}<br><br>
