...
{% alert note %}
다른 비즈니스 솔루션 제공업체(BSP)에서 Braze 통합으로 마이그레이션하려면 [다른 비즈니스 솔루션 제공업체에서 마이그레이션]({{site.baseurl}}/user_guide/channels/whatsapp/whatsapp_setup/whatsapp_phone_numbers/migrate_a_phone_number#migrate-from-another-business-solution-provider)을 참조하세요.
{% endalert %}
...
{: start="3"}
3. [1단계](#step-1)에서 확인한 API 키와 기본 URL을 입력합니다.
4. **Connect**를 선택합니다.
5. 다음 사항을 고려하여 [임베디드 가입 워크플로]({{site.baseurl}}/user_guide/channels/whatsapp/whatsapp_setup/embedded_signup#whatsapp-embedded-signup-workflow)를 진행합니다.
- 다른 비즈니스 솔루션 제공업체에서 사용 중인 동일한 비즈니스 포트폴리오는 선택할 수 없습니다.
- 다른 비즈니스 솔루션 제공업체에서 사용 중인 전화번호는 선택할 수 없습니다.
- 기존 WABA를 선택하지 말고 새 WABA를 생성해야 합니다.

{% alert note %}
인증 코드를 받으려면 Infobip 대시보드 > **Analyze** > **Logs**로 이동하여 수신 SMS 메시지에서 코드를 확인하세요.
{% endalert %}

![인증 코드가 포함된 수신 SMS 메시지를 보여주는 메시지 로그.]({% image_buster /assets/img/whatsapp/byo_connector/verification_code.png %})

설정을 완료하면 전화번호가 WhatsApp 비즈니스 그룹 아래에 구독 그룹으로 표시됩니다. WhatsApp 비즈니스 그룹에는 연결된 Infobip 계정 이름과 API 기본 URL이 포함되어 있습니다. 네이티브 통합을 통해 연결된 계정에는 Infobip 계정 이름이 표시되지 않습니다.

{% alert note %}
각 WhatsApp 비즈니스 계정을 하나의 Infobip 계정에 연결하세요. 추가 전화번호 또는 구독 그룹을 연결할 때 해당 WhatsApp 비즈니스 계정이 이미 Infobip 계정에 연결되어 있는 경우, 기존 계정의 API 자격 증명을 다시 입력해야 합니다.
{% endalert %}

## 3단계: 메시지 보내기 {#step-3-sending-messages}

네이티브 통합 전송 프로세스를 따르세요. 여기에는 다음이 포함됩니다:
- [구독 그룹에 사용자 등록하기]({{site.baseurl}}/user_guide/channels/whatsapp/whatsapp_setup/subscription_groups)
...
| 사용 사례 | 이유 |
| --- | --- |
| Braze와 Infobip에서 인바운드 메시지 처리 | 이 경우 두 시스템 중 하나에 의해 트리거되는 로직 체인이 발생하여, 중복되거나 잠재적으로 상충하는 메시지 스레드가 생성될 수 있습니다. |
