...
### 3단계: 레이아웃 선택하기 {#step-3-choose-a-layout}

**레이아웃**에서 템플릿 유형을 선택합니다:

- **기본:** 표준 WhatsApp 메시지입니다. 이 문서에서 다루는 레이아웃입니다.
...
- **텍스트:** 짧은 텍스트 헤더.
- **미디어:** 이미지, 비디오 또는 문서(URL만 가능). Braze는 미디어 참조를 저장하고 승인을 위해 Meta에 샘플을 제출합니다.
- **없음:** 헤더 없음

#### 본문 {#body}

메시지의 주요 콘텐츠를 입력하고 Liquid 또는 일반 변수를 사용하여 본문을 필요에 따라 개인화합니다:

{% raw %}
- Liquid 태그를 사용합니다(예: `{{${first_name}}}`). Braze는 Liquid를 저장하고 Campaign 또는 Canvas 작성기에서 템플릿을 사용할 때 이를 표시합니다.
...
### 승인된 템플릿을 수정할 수 있나요? {#can-i-edit-a-template-after-its-been-approved}
