...
## Braze WhatsApp 채널 옵트인 설정 {#set-up-opt-ins-for-your-braze-whatsapp-channel}
...
인바운드 WhatsApp 메시지는 사용자를 WhatsApp 구독 그룹에 자동으로 구독시키지 않습니다. [사용자 업데이트 단계](#user-update-step), [웹훅](#webhook-campaign-to-trigger-a-second-whatsapp-campaign) 또는 API 호출을 통해 구독 상태를 명시적으로 업데이트해야 합니다.

Meta는 옵트인 문구에 다음 사항을 요구합니다:

- 해당 사용자가 비즈니스로부터 메시지를 수신하는 데 옵트인한다는 것을 명확히 명시
- 비즈니스 이름 포함(일반적인 표현인 "메시지를 보내드리겠습니다" 등이 아닌 구체적인 이름)
- 해당 지역 법률 준수

{% alert note %}
Braze는 구독 상태 업데이트를 허용하는 `/users/track` 엔드포인트 개선 사항을 출시했으며, 이에 대한 자세한 내용은 [구독 그룹]({{site.baseurl}}/user_guide/channels/whatsapp/whatsapp_setup/subscription_groups#update-subscription-status)에서 확인할 수 있습니다. 그러나 [`/v2/subscription/status/set` 엔드포인트]({{site.baseurl}}/api/endpoints/subscription_groups/post_update_user_subscription_group_status_v2)를 사용하여 이미 옵트인 프로토콜을 생성한 경우 해당 방식을 계속 사용할 수 있습니다.
{% endalert %}
...
이메일이나 문자 메시지에 대한 마케팅 동의를 이미 받고 있는 곳에 WhatsApp을 위한 추가 섹션을 포함하세요. 사용자가 옵트인한 후에는 `external_id`, [전화번호]({{site.baseurl}}/user_guide/channels/whatsapp/whatsapp_setup/user_phone_numbers), 업데이트된 구독 상태가 필요합니다. 이를 위해 Braze 설치 방식에 따라 [`/subscription/status/set` 엔드포인트]({{site.baseurl}}/api/endpoints/subscription_groups/post_update_user_subscription_group_status)를 활용하거나 [SDK](https://braze-inc.github.io/braze-swift-sdk/documentation/brazekit/braze/user-swift.class/addtosubscriptiongroup(id:fileid:line:)/)를 사용하세요.
...
고객 지원 채널에서 해결된 문제에 대해 마케팅 메시지 수신 옵트인 여부를 묻는 자동 메시지로 후속 조치를 취하세요. 이 기능은 선택한 고객 지원 도구의 기능 가용성과 사용자 정보를 보관하는 위치에 따라 달라집니다.

1. WhatsApp Business 전화번호에서 [메시지 링크](https://business.facebook.com/business/help/890732351439459?ref=search_new_0)를 제공합니다.
2. 고객이 옵트인을 나타내기 위해 "예"라고 응답하는 [빠른 답장 액션]({{site.baseurl}}/user_guide/channels/whatsapp/message_processing/messaging_users#quick-replies)을 제공합니다.
3. 커스텀 키워드 트리거를 설정합니다.
4. 위의 방법 중 하나를 사용하는 경우 다음과 같이 경로를 완료해야 할 수 있습니다:
...
사용자가 새 채널에서 확인 메시지를 받기를 원하는지 여부에 따라 Canvas 또는 Campaign으로 설정할 수 있습니다.

1. 인바운드 메시지의 실행 기반 전달 트리거로 Campaign을 생성합니다.
...
#### 인앱 메시지 또는 인브라우저 메시지 {#in-app-or-in-browser-message}

고객에게 WhatsApp 사용 옵트인을 요청하는 인앱 메시지 또는 브라우저 내 팝업을 생성합니다.

Braze SDK와 연동하기 위해 [JavaScript "bridge"]({{site.baseurl}}/user_guide/channels/in_app_messages/message_types/custom_html#javascript-bridge)가 포함된 [HTML 인앱 메시지](https://github.com/braze-inc/in-app-message-templates/tree/master/braze-templates/4-sms-capture-modal)를 사용하세요. WhatsApp 구독 그룹 ID를 사용해야 합니다.

#### 전화번호 수집 양식 {#phone-number-capture-form}

인앱 메시지용 드래그 앤 드롭 에디터에서 [전화번호 수집 양식]({{site.baseurl}}/user_guide/messaging/templates/in_app_message_templates/phone_number_capture) 템플릿을 사용하여 사용자 전화번호를 수집하고 WhatsApp 구독 그룹을 확장하세요.
...
WhatsApp은 앱 설정에서 사용자가 마케팅 메시지 수신을 거부할 수 있는 "혜택 및 공지" 토글을 제공합니다. 이 토글은 Braze 구독 그룹과 독립적으로 작동합니다:

- **Braze 구독 그룹**은 Braze 통합(API, 환경설정 센터 또는 SDK)을 통해 관리되며, 메시징 대상으로 지정할 사용자를 제어합니다.
- **WhatsApp 기본 토글**은 Meta에 의해 제어되며 Braze 외부의 플랫폼 수준에서 적용됩니다.

이 두 레이어는 설계상 자동으로 동기화되지 않습니다. 사용자가 WhatsApp에서 "혜택 및 공지" 토글을 끄면, 사용자의 Braze 구독 상태가 "Subscribed"로 표시되더라도 Meta가 플랫폼 수준에서 마케팅 메시지 전달을 차단합니다. 사용자의 환경설정은 전달 시점에서 적용됩니다.

{% alert note %}
Braze는 발송 시도가 이루어지고 Meta가 오류를 반환할 때까지 수신 거부 신호를 수신하지 않으므로, WhatsApp 토글을 통해 수신을 거부한 사용자는 메시지 발송이 시도될 때까지 Braze의 구독 수에 반영되지 않을 수 있습니다. 이는 해당 피드백 루프가 발생할 때까지 도달 추정치가 약간 과대 표시될 수 있음을 의미합니다.
{% endalert %}

### 일반 수신 거부 키워드 {#general-opt-out-keywords}

특정 단어를 메시지로 보내는 사용자가 향후 메시징 수신을 거부할 수 있도록 Campaign 또는 Canvas를 설정할 수 있습니다. Canvas는 성공적인 수신 거부를 확인하는 후속 메시지를 포함할 수 있어 특히 유용합니다.

#### 1단계: "인바운드 WhatsApp 메시지" 트리거로 Canvas 만들기 {#step-1-create-a-canvas-with-a-trigger-of-inbound-whatsapp-message}

![인바운드 WhatsApp 메시지를 보내는 사용자가 진입하는 실행 기반 Canvas 진입 단계.]({% image_buster /assets/img/whatsapp/whatsapp116.png %}){: style="max-width:85%;"}

키워드 트리거를 선택할 때 "Stop" 또는 "No Message"와 같은 단어를 포함하세요. 이 방법을 선택하는 경우, 고객이 수신 거부 단어를 알 수 있도록 해야 합니다. 예를 들어, 초기 옵트인을 수신한 후 "이 메시지 수신을 거부하려면 언제든지 'Stop'이라고 메시지를 보내세요."와 같은 후속 응답을 포함하세요.
...
WhatsApp 메시지 템플릿 생성기에서 "마케팅 수신 거부" 옵션을 포함할 수 있습니다. 이 옵션을 포함할 때마다 해당 템플릿이 구독 그룹 변경을 위한 후속 단계가 있는 Canvas에서 사용되도록 해야 합니다.

1. "마케팅 수신 거부" 빠른 답장이 포함된 메시지 템플릿을 만듭니다.<br>![푸터 옵션이 "마케팅 수신 거부"인 메시지 템플릿]({% image_buster /assets/img/whatsapp/whatsapp121.png %})<br><br>![마케팅 수신 거부 버튼을 구성하는 섹션.]({% image_buster /assets/img/whatsapp/whatsapp122.png %})<br><br>
2. 이 메시지 템플릿을 사용하는 Canvas를 만듭니다.<br><br>
...
WhatsApp에서 "START" 및 "STOP" 키워드 응답 워크플로를 다음 두 가지 방법으로 구성할 수 있습니다:
...
1. 실행 기반 단계 **WhatsApp 인바운드 메시지 보내기**로 Canvas를 생성합니다. **메시지 본문 위치**를 선택하고 **다음과 같음**에 "START"를 입력합니다.
...
{: start="2"}
2. Canvas에서 **사용자 업데이트 설정** 단계를 생성하고 **작업**에서 **고급 JSON 편집기**를 선택합니다. <br><br>![작업이 "고급 JSON 편집기"인 사용자 업데이트 단계.]({% image_buster /assets/img/whatsapp/user_update.png %})<br><br>
3. **사용자 업데이트 객체**에 다음 JSON 페이로드를 입력하고 `XXXXXXXXXXX`를 구독 그룹 ID로 교체합니다:
...
Braze가 [사용자 업데이트 단계]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/user_update) 요청을 일괄 처리하기 때문에 업데이트 완료 속도가 다를 수 있습니다. 구독 업데이트 직후 확인 메시지를 즉시 보내야 하는 시간에 민감한 옵트인 플로우의 경우, 사용자 업데이트 단계 대신 [웹훅 방법](#webhook-campaign-to-trigger-a-second-whatsapp-campaign)을 사용하세요.
...
1. 실행 기반 단계 **WhatsApp 인바운드 메시지 보내기**로 Campaign 또는 Canvas를 생성합니다. **메시지 본문 위치**를 선택하고 **다음과 같음**에 "START"를 입력합니다.

![메시지 본문이 "START"인 WhatsApp 메시지 단계.]({% image_buster /assets/img/whatsapp/whatsapp_inbound_message.png %}){: style="max-width:85%;"}

{: start="2"}
2. Campaign 또는 Canvas에서 웹훅 메시지 단계를 생성하고 **요청 본문**을 **원시 텍스트**로 변경합니다.

![웹훅 메시지 단계.]({% image_buster /assets/img/whatsapp/webhook_step.png %}){: style="max-width:85%;"}

{: start="3"}
3. **웹훅 URL**에 고객의 [엔드포인트 URL]({{site.baseurl}}/api/basics)을 입력하고 그 뒤에 엔드포인트 링크 `campaigns/trigger/send`를 추가합니다. 예를 들어, `https://dashboard-02.braze.eu/campaigns/trigger/send`입니다.
...
## "정규식"과 "is" 수정자의 차이점 이해하기 {#understanding-the-difference-between-regex-and-is-modifiers}

이 표에서 `STOP`은 수정자의 작동 방식을 보여주기 위한 예시 트리거 단어로 사용됩니다.

| 수정자 | 트리거 단어 | 동작 |
| --- | --- | --- |
| `Is` | `STOP` | 대소문자에 관계없이 "stop"이라는 단어가 단독으로 사용된 경우를 포착합니다. 예를 들어, "stop"은 포착하지만 "please stop"은 포착하지 않습니다. |
| `Matches regex` | `STOP` | 정확히 해당 대소문자로 "STOP"이 사용된 모든 경우를 포착합니다. 예를 들어, "STOP"과 "PLEASE STOP"은 포착하지만 "stop"은 포착하지 않습니다. |
| `Matches regex` | `(?i)STOP(?-i)` | 대소문자에 관계없이 "STOP"이 사용된 모든 경우를 포착합니다. 예를 들어, "stop", "please stop", "never stop sending me messages"를 모두 포착합니다. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="정규식과 is 수정자의 차이점 이해하기" }