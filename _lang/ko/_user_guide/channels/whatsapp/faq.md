...
메시징 한도는 WhatsApp 무결성 구축 개념입니다. 각 전화번호가 24시간 롤링 기간 동안 시작할 수 있는 비즈니스 시작 대화의 최대 수를 결정합니다. 네 가지 메시징 한도 수준이 있습니다: 1k, 10k, 100k, 무제한.
...
네, 전화번호 품질 등급이 너무 낮아지면 WhatsApp이 메시징 한도를 줄일 위험이 있습니다. Braze는 전화번호 상태 및 메시징 한도 수준 업데이트를 포함하여 WhatsApp의 품질 관련 업데이트에 대한 알림을 구독하고 받을 것을 권장합니다. WhatsApp Manager 대시보드에서 직접 알림을 구독할 수 있습니다.
...
품질 등급이 낮으면 전화번호 상태가 **Connected**에서 **Flagged**로 변경됩니다. 7일 동안 품질이 개선되지 않으면 상태는 **Connected**로 돌아갑니다. 그러나 메시징 한도는 다음 수준으로 감소합니다. 예를 들어, 이전에 100,000 메시징 한도를 가졌던 전화번호가 이제 10,000 메시징 한도를 갖게 됩니다.
...
원래 24시간 창 이후 사용자가 빠른 답장을 탭하는 Canvas 플로우의 경우, [24시간 창 외부의 빠른 답장 및 인바운드 메시지]({{site.baseurl}}/user_guide/channels/whatsapp/message_processing/messaging_users#quick-replies-and-inbound-messages-outside-the-24-hour-window)를 참조하세요.
...
#### Braze 대시보드에서 사용할 수 있는 측정기준은 무엇인가요? {#what-metrics-are-available-in-the-braze-dashboard}
Braze 대시보드에서 고유 수신자, 전송, 전달, 읽음 및 실패를 확인할 수 있습니다. Braze가 읽음을 추적하려면 사용자의 읽음 확인이 "켜짐"이어야 합니다. 다른 채널과 유사하게 Campaign 성능을 모니터링하기 위한 전환 이벤트도 설정할 수 있습니다.
