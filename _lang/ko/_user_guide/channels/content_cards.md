...
> 콘텐츠 카드를 사용하면 사용자가 즐겨 사용하는 앱 내에서 경험을 방해하지 않으면서 고도로 타겟팅된 풍부한 콘텐츠의 동적 스트림을 고객에게 전달할 수 있습니다. 콘텐츠 카드는 앱이나 웹사이트에 직접 삽입되어 메시지 받은편지함과 커스텀 인터페이스를 만들 수 있으며, 이메일이나 푸시 알림 같은 다른 채널의 도달 범위를 확장합니다.

## 필수 조건

콘텐츠 카드 사용 가능 여부는 Braze 패키지에 따라 다릅니다. 시작하려면 계정 매니저 또는 고객 성공 매니저에게 문의하세요.

콘텐츠 카드를 사용하려면 먼저 앱이나 웹사이트에 [Braze SDK]({{site.baseurl}}/developer_guide/content_cards/)를 연동해야 합니다. 추가 설정은 필요하지 않습니다. 자체 UI를 구축하려면 [콘텐츠 카드 커스터마이징 가이드]({{site.baseurl}}/developer_guide/content_cards/customizing_cards/)를 참조하세요.
...
신규 사용자가 앱과 웹사이트를 탐색할 때, 전략적으로 배치된 콘텐츠 카드를 통해 제공하는 서비스의 가치와 이점을 안내하세요. 홈페이지의 콘텐츠 카드로 사용자가 다른 커뮤니케이션 채널에 옵트인하도록 유도하고, 콘텐츠 카드로 구동되는 전용 온보딩 탭에 미완료 온보딩 작업을 저장하세요. 사용자가 원하는 작업을 완료한 후에는 카드를 제거하는 것을 잊지 마세요!
...
사용자 홈페이지 상단에 콘텐츠 카드를 표시하여 이벤트 참석을 유도하고, 위치 타겟팅을 사용하여 잠재 사용자가 있는 곳에서 도달하세요. 관련 오프라인 이벤트에 사용자를 초대하면 특별한 느낌을 줄 수 있으며, 특히 브랜드와의 이전 활동을 활용한 개인화된 메시징을 사용하면 더욱 효과적입니다.
...
사용자 동작과 선호도에 대한 데이터를 활용하여 홈페이지 또는 받은편지함 콘텐츠 카드에서 관련 콘텐츠를 실시간으로 표시하고 제품 서비스로 다시 유도하세요.
...
콘텐츠 카드를 활용하여 홈페이지나 전용 프로모션 받은편지함에서 프로모션 메시지와 미수령 혜택을 직접 강조하세요. 각 고객의 이전 구매를 기반으로 관련 콘텐츠를 가져와 시선을 사로잡는 개인화된 프로모션을 제공하세요.
...
이러한 주요 활용 사례 외에도 고객들은 콘텐츠 카드를 다양한 방식으로 활용하고 있습니다. 콘텐츠 카드의 강점은 유연성에 있습니다. 원하는 활용 사례가 여기에 표시되지 않은 경우, [키-값 페어]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/sources/key_value_pairs/)를 설정하고 앱이나 웹사이트에 페이로드를 전송할 수 있습니다.
