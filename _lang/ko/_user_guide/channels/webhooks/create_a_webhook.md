...
`Content-Type` [요청 헤더](#request-headers-optional)를 `application/x-www-form-url-encoded`로 설정하면 요청 본문은 URL 인코딩 문자열 형식이어야 합니다. 예를 들어:
...
다음으로, Segment 또는 필터를 선택하여 오디언스를 좁혀 [사용자를 타겟팅]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/target_users)해야 합니다. 이 단계에서는 Segment에서 더 넓은 오디언스를 선택하고, 원하는 경우 필터를 사용하여 해당 Segment를 더 세분화합니다. 대략적인 Segment 인구의 미리보기가 자동으로 표시됩니다. 정확한 Segment 멤버십은 항상 메시지가 전송되기 전에 계산된다는 점을 유의하세요.
