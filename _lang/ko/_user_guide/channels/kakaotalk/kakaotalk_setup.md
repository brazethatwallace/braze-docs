...
| KakaoTalk 비즈니스 채널 | Braze를 통해 KakaoTalk 메시지를 보내려면 KakaoTalk 계정이 KakaoTalk 비즈니스 채널이어야 합니다. 계정을 만들면 기본 상태는 일반 채널입니다. 계정을 비즈니스 채널로 전환하려면 사업자 인증과 관련 서류를 제출해야 합니다. |
| KakaoTalk 발신 키 | 유효한 KakaoTalk 발신 키가 필요합니다. |
| 연락처 전화번호 | KakaoTalk 채널 관리자의 연락처 전화번호가 필요합니다. |
| Braze 클러스터 IP 허용 목록 등록 | 모든 고객에게 IP 허용 목록 등록이 필요합니다. KakaoTalk을 Braze에 통합하기 전에 사용 중인 클러스터의 Braze IP 주소를 등록하세요. |
{: .reset-td-br-1 .reset-td-br-2 aria-label="사전 요구 사항" }

### Braze IP 주소 등록 {#register-braze-ip-addresses}

Comm.One 대시보드에서 사용 중인 클러스터의 Braze IP 주소를 등록하세요.

1. Comm.One 대시보드에서 **Account Management (계정 관리)**로 이동한 후 메뉴 아이콘을 선택하고 **View Details (자세히보기)**를 선택합니다.
2. **Center & Upload IP Allowlist (센터&업로드 IP 화이트리스트)**를 선택합니다.
3. 사용 중인 Braze 클러스터의 IP 주소를 추가합니다. 클러스터별 전체 IP 목록은 [IP 허용 목록]({{site.baseurl}}/user_guide/channels/webhooks/create_a_webhook#ip-allowlisting)을 참조하세요.
...
| 계정 유형 | 설명 |
| --- | --- |
| 일반 채널 | 모든 조직이 설정할 수 있는 표준 KakaoTalk 채널입니다. KakaoTalk을 통한 단체 메시지 발송과 1:1 채팅이 가능합니다. |
| [비즈니스 채널](https://www.kakaocorp.com/page/service/service/KakaoTalkChannel) | 신청 및 인증 절차가 필요한, 사업자 인증을 받은 KakaoTalk 채널입니다. 다음과 같은 향상된 기능을 제공합니다. {::nomarkdown}<ul><li>인증 배지</li><li>추천 채널 노출</li><li>비즈니스 메시징 지원</li></ul>{:/} |
...
{% alert important %}
KakaoTalk 채널 정보(채널 이름, 프로필 이미지 등)가 공식 제출 서류의 정보와 정확히 일치해야 합니다.
{% endalert %}
...
4. **Apply** 또는 **Request 버튼 (신청)**을 선택하여 절차를 시작합니다.
5. 필요한 정보를 입력합니다.
6. 심사 결과 알림을 기다립니다.
...
| **Credential (ID) & Password (비밀번호)** | **Sender number (사업자 등록번호)**와 같은 위치로 이동한 다음 **API** > **Brand Message (브랜드 메시지)**로 이동합니다. |
{: .reset-td-br-1 .reset-td-br-2 aria-label="CJ OliveNetworks" }

{% tabs local %}
{% tab Comm.One Login ID (로그인 아이디) %}

![가려진 로그인 ID가 표시된 Comm.One 대시보드.]({% image_buster /assets/img/kakaotalk/comm.one_login_id.png %})

{% endtab %}
{% tab Sender Key (발신프로필 키) %}

![가려진 발신프로필 키가 표시된 Comm.One 대시보드.]({% image_buster /assets/img/kakaotalk/sender_key.png %})

{% alert important %}
KakaoTalk 발신프로필 키는 한 번에 하나의 워크스페이스에만 통합할 수 있습니다. 동일한 발신프로필 키를 다른 워크스페이스에서 사용하려면 먼저 원래 워크스페이스에서 KakaoTalk 구독 그룹을 보관 처리한 다음, [Braze 지원팀]({{site.baseurl}}/user_guide/administer/personal/braze_support)에 연락하여 통합을 제거해야 합니다. Braze가 통합을 제거한 후 새 워크스페이스에서 통합을 설정할 수 있습니다.
{% endalert %}
...
![가려진 채널 이름이 표시된 Comm.One 대시보드.]({% image_buster /assets/img/kakaotalk/channel_profile_name.png %})

{% endtab %}
{% tab Credential (ID) & Password (비밀번호) %}

![가려진 자격 증명 ID와 비밀번호가 표시된 Comm.One 대시보드.]({% image_buster /assets/img/kakaotalk/id_and_password.png %})
...
4. **Name**을 입력하고, **Expiration date**를 선택한 다음, KakaoTalk에 필요한 API 범위를 선택합니다. 이러한 범위는 키가 수행할 수 있는 Infobip API 작업을 제어합니다.

![이름, 만료일 및 API 범위 필드가 표시된 Infobip API 키 생성 페이지.]({% image_buster /assets/img/kakaotalk/infobip_api_key_scopes.png %})

{: start="5"}
5. **CREATE**를 선택하여 키를 생성합니다.
6. 생성된 키를 복사합니다. 이 페이지로 돌아와 이름, 만료일 또는 API 범위를 업데이트할 수 있습니다.

##### 발신 프로필 UUID 및 채널 이름 {#sender-profile-uuid-and-channel-name}

1. [KakaoTalk 채널 관리자 센터](https://center-pf.kakao.com/)에서 **Channels**를 선택합니다.
2. **Channel Information** 창에서 **Channel name**과 **Search id**(발신 UUID)를 찾습니다.
3. **Customer center contact information**을 입력합니다. 이는 광고성 메시지를 보낼 때 필수입니다.
...
5. **내 채널** 목록에서 보고 싶은 채널을 선택한 다음 이전 단계를 반복합니다.

## 고객 프로필 설정 {#set-user-profiles}

KakaoTalk을 통해 메시지를 보내려면 고객 프로필에 E.164 형식의 전화번호가 있어야 합니다. 전화번호는 고객 프로필에 표시됩니다. KakaoTalk은 전화번호가 E.164 형식(예: `+821025749774`)이어야 합니다. 이는 여러 형식의 전화번호를 허용하는 다른 메시징 채널과 다를 수 있습니다.
