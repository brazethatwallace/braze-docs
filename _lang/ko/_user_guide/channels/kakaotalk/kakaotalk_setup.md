...
| KakaoTalk 비즈니스 채널 | Braze를 통해 KakaoTalk 메시지를 보내려면 KakaoTalk 계정이 KakaoTalk 비즈니스 채널이어야 합니다. 계정을 생성하면 기본 상태는 일반입니다. 계정을 비즈니스 채널로 전환하려면 사업자 인증을 완료하고 관련 서류를 제출해야 합니다. |
...
1. Comm.One 대시보드에서 **계정 관리**로 이동하여 메뉴 아이콘을 선택한 다음 **자세히보기**를 선택합니다.
...
| [비즈니스 채널](https://www.kakaocorp.com/page/service/service/KakaoTalkChannel) | 신청 및 인증 절차가 필요한 업그레이드된 사업자 인증 KakaoTalk 채널입니다. 다음과 같은 향상된 기능을 제공합니다. {::nomarkdown}<ul><li>인증 배지</li><li>추천 채널로 노출</li><li>비즈니스 메시징 지원</li></ul>{:/} |
...
## 카카오톡 통합 {#integrate-kakaotalk}

### 1단계: 카카오톡 채널을 Braze에 연결하기 {#step-1-connect-the-kakaotalk-channel-to-braze}

1. **파트너 통합** > **기술 파트너**로 이동하여 카카오톡 제공업체를 선택합니다.
2. 제공업체에 필요한 자격 증명을 수집한 다음(다음 섹션 참조), **기술 파트너** 페이지에 입력하고 저장합니다.
...
![Comm.One 대시보드에 표시된 마스킹 처리된 로그인 ID.]({% image_buster /assets/img/kakaotalk/comm.one_login_id.png %})

{% endtab %}
{% tab Sender Key (발신프로필 키) %}

![Comm.One 대시보드에 표시된 마스킹 처리된 발신 키.]({% image_buster /assets/img/kakaotalk/sender_key.png %})

{% alert important %}
카카오톡 발신 키는 한 번에 하나의 워크스페이스에만 통합할 수 있습니다. 동일한 발신 키를 다른 워크스페이스에서 사용하려면 먼저 원래 워크스페이스에서 카카오톡 구독 그룹을 보관 처리한 다음 [Braze 지원팀]({{site.baseurl}}/braze_support)에 연락하여 통합을 제거해야 합니다. Braze에서 통합을 제거한 후 새 워크스페이스에서 통합을 설정할 수 있습니다.
{% endalert %}

![Braze 카카오톡 채널의 자격 증명.]({% image_buster /assets/img/kakaotalk/cj_credentials.png %})

{% endtab %}
{% tab Channel name (카카오톡 채널 프로필명) %}

![Comm.One 대시보드에 표시된 마스킹 처리된 채널 이름.]({% image_buster /assets/img/kakaotalk/channel_profile_name.png %})

{% endtab %}
{% tab Credential (ID) & Password (비밀번호) %}

![Comm.One 대시보드에 표시된 마스킹 처리된 자격 증명 ID 및 비밀번호.]({% image_buster /assets/img/kakaotalk/id_and_password.png %})
...
| **API Key** | **Developer Tools** > **API Keys**를 선택합니다. |
| **Sender name / Sender key** | **Channels and Numbers** > **Channels**를 선택한 다음 **발송자** 탭을 선택합니다. |
...
[CSV를 업로드하거나 API를 사용]({{site.baseurl}}/user_guide/data/unification/user_data/import_users)하여 사용자를 생성함으로써 전화번호를 가져올 수 있습니다. 가져오기 전에 전화번호가 E.164 형식인지 확인하세요.