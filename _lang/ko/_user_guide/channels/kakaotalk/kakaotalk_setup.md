...
| KakaoTalk 비즈니스 채널 | Braze를 통해 KakaoTalk 메시지를 보내려면 KakaoTalk 계정이 KakaoTalk 비즈니스 채널이어야 합니다. 계정을 생성하면 기본 상태는 일반(Basic)입니다. 비즈니스 채널로 전환하려면 사업자 인증을 완료하고 관련 문서를 제출해야 합니다. |
| KakaoTalk 발신 키 | 유효한 KakaoTalk 발신 키가 필요합니다. |
| 연락처 전화번호 | KakaoTalk 채널 관리자의 연락처 전화번호입니다. |
| Braze 클러스터 IP 허용 목록 등록 | 모든 고객에게 IP 허용 목록 등록이 필요합니다. KakaoTalk을 Braze에 통합하기 전에 해당 클러스터의 Braze IP 주소를 등록하세요. |
...
1. Comm.One 대시보드에서 **계정 관리**로 이동한 후 메뉴 아이콘을 선택하고 **자세히보기**를 선택합니다.
2. **센터&업로드 IP 화이트리스트**를 선택합니다.
3. Braze 클러스터의 IP 주소를 추가합니다. 클러스터별 전체 IP 목록은 [IP 허용 목록]({{site.baseurl}}/user_guide/channels/webhooks/create_a_webhook#ip-allowlisting)을 참조하세요.
...
| 계정 유형 | 설명 |
| --- | --- |
| 일반 채널 | 모든 조직이 설정할 수 있는 표준 KakaoTalk 채널입니다. KakaoTalk을 통한 브로드캐스트 메시징 및 1:1 채팅이 가능합니다. |
...
신청을 시작하기 전에 다음 사업자 관련 서류를 준비하세요:
- 사업자등록증
- 대표자 신분증
- 재직증명서
- 업종별 인허가증
...
{% alert important %}
KakaoTalk 채널 정보(채널 이름, 프로필 이미지 등)는 공식 제출 서류의 정보와 정확히 일치해야 합니다.
{% endalert %}
...
3. **관리** 섹션에서 **비즈니스 채널 신청** 옵션을 선택합니다.
4. **신청** 버튼을 선택하여 절차를 시작합니다.
5. 필요한 정보를 입력합니다.
6. 심사 결과에 대한 알림을 기다립니다.
...
![Comm.One 대시보드에 마스킹 처리된 로그인 ID가 표시됩니다.]({% image_buster /assets/img/kakaotalk/comm.one_login_id.png %})

{% endtab %}
{% tab Sender Key (발신프로필 키) %}

![Comm.One 대시보드에 마스킹 처리된 Sender Key가 표시됩니다.]({% image_buster /assets/img/kakaotalk/sender_key.png %})

{% alert important %}
KakaoTalk Sender Key는 한 번에 하나의 워크스페이스에만 통합할 수 있습니다. 다른 워크스페이스에서 동일한 Sender Key를 사용하려면 먼저 원래 워크스페이스에서 KakaoTalk 구독 그룹을 보관 처리한 다음 [Braze 지원팀]({{site.baseurl}}/braze_support)에 연락하여 통합을 제거해야 합니다. Braze에서 통합을 제거한 후 새 워크스페이스에서 통합을 설정할 수 있습니다.
{% endalert %}
...
![Comm.One 대시보드에 마스킹 처리된 채널 이름이 표시됩니다.]({% image_buster /assets/img/kakaotalk/channel_profile_name.png %})

{% endtab %}
{% tab Credential (ID) & Password (비밀번호) %}

![Comm.One 대시보드에 마스킹 처리된 자격 증명 ID와 비밀번호가 표시됩니다.]({% image_buster /assets/img/kakaotalk/id_and_password.png %})
...
Infobip 대시보드와 [KakaoTalk Channel Admin Center](https://center-pf.kakao.com/)로 이동하여 다음 정보를 수집합니다.
...
| **Sender profile UUID** | KakaoTalk Channel Admin Center에서 **Channels**로 이동하여 채널 정보 창에서 **Search ID**를 찾습니다. |
| **Channel name** | KakaoTalk Channel Admin Center에서 동일한 채널 정보 창에서 **채널 이름**을 찾습니다. |
...
4. **Name**을 입력하고, **Expiration date**를 선택한 다음, KakaoTalk에 필요한 API 범위를 선택합니다. 이 범위는 키가 수행할 수 있는 Infobip API 작업을 제어합니다.

![이름, 만료 날짜, API 범위 필드가 표시된 Infobip Create API Key 페이지.]({% image_buster /assets/img/kakaotalk/infobip_api_key_scopes.png %})

{: start="5"}
5. **CREATE**를 선택하여 키를 생성합니다.
6. 생성된 키를 복사합니다. 이 페이지로 돌아가서 이름, 만료 날짜 또는 API 범위를 업데이트할 수 있습니다.

##### Sender profile UUID 및 채널 이름 {#sender-profile-uuid-and-channel-name}

1. [KakaoTalk Channel Admin Center](https://center-pf.kakao.com/)에서 **Channels**를 선택합니다.
2. **Channel Information** 창에서 **Channel name**과 **Search id**(sender UUID)를 찾습니다.
3. **Customer center contact information**을 입력합니다. 이는 광고 메시지를 발송할 때 필요합니다.

![고객 센터 연락처 정보 필드가 표시된 KakaoTalk 채널 정보 창.]({% image_buster /assets/img/kakaotalk/kakao_customer_center_contact.png %})

{: start="4"}
4. 다른 채널을 보려면 메뉴 상단의 채널 아이콘을 선택합니다.
5. **My channel** 목록에서 보려는 채널을 선택한 다음 이전 단계를 반복합니다.
...
사용자를 생성하려면 [CSV를 업로드하거나 API를 사용하여]({{site.baseurl}}/user_guide/audience/manage_audience/import_users) 전화번호를 가져오세요. 가져오기 전에 전화번호가 E.164 형식인지 확인하세요.