...
{% tabs local %}
  {% tab Scheduled Delivery %}
   ...
   월간 반복 스케줄을 선택하는 경우, 일부 달에는 선택한 날짜가 없을 수 있습니다. 예를 들어, 캔버스를 매월 31일에 전송하도록 설정했다고 가정해 보겠습니다. 이 경우 Braze는 해당 월의 마지막 날에 전송합니다. 예를 들어 4월 31일은 존재하지 않으므로 4월 30일에 전송됩니다.
    ...
    현지 시간대 전달을 사용할 때 Braze는 진입 자격을 두 번 평가합니다. 먼저 예약된 날의 사모아 시간(UTC+13 또는 서머타임 기간 중 UTC+14)에 평가하고, 다시 사용자의 현지 시간에 평가합니다. 사용자가 캔버스에 진입하려면 두 번의 확인을 모두 통과해야 합니다. 진입 필터가 상대적 시간 범위(예: "2일 이상 전")를 사용하는 경우, 첫 번째 확인 시점에 24시간이 아직 경과하지 않아 사용자가 하루 늦게 진입할 수 있습니다. 이를 방지하려면 최소 2일 이상과 같이 더 넓은 시간 범위를 사용하세요. 자세한 내용은 [Braze는 현지 시간대 전달을 위해 언제 사용자를 평가하나요?]({{site.baseurl}}/user_guide/engagement_tools/campaigns/faq/#when-does-braze-evaluate-users-for-local-time-zone-delivery)를 참조하세요.
  {% endtab %}
...
사용자가 캔버스에 다시 진입하여 이전에 진입했던 동일한 구성요소에 도달하고 각 진입에 대해 인앱 메시지를 받을 자격이 있는 경우, 사용자가 세션을 두 번 다시 열면 인앱 메시지 우선순위에 따라 메시지를 두 번 받게 됩니다.
{% enddetails %}
