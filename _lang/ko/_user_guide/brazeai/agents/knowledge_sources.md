...
> 지식 소스는 AI 에이전트가 카탈로그 데이터를 해석하고 목표를 달성하기 위해 적절한 정보를 검색하는 데 도움을 줍니다. Braze 에이전트에 대한 소개는 [Braze 에이전트]({{site.baseurl}}/user_guide/brazeai/agents)를 참조하세요. 에이전트에 지식을 추가하려면 [커스텀 에이전트 만들기]({{site.baseurl}}/user_guide/brazeai/agents/creating_agents#add-resources)를 참조하세요.
...
## 지식 소스 만들기 {#create-a-knowledge-source}
...
![카탈로그 "nyc_restaurants"를 참조하는 지식 소스 "nyc_restaurants".]({% image_buster /assets/img/ai_agent/knowledge_source_example.png %})

에이전트를 구축하면서 지식 소스를 만들 수도 있습니다. 에이전트의 **지침** 섹션으로 이동하여 **지식 추가** > **지식 소스 만들기**를 선택합니다.

## AI 에이전트에서 지식 소스 사용하기 {#use-a-knowledge-source-in-your-ai-agent}
...
카탈로그를 지식 소스로 변환하면 Braze 에이전트가 카탈로그의 단어와 구문 뒤에 숨겨진 실제 의미를 이해할 수 있으므로, 에이전트가 더 나은 출력을 생성하기 위해 의미 있는 데이터를 더 효과적으로 찾을 수 있습니다.

### 지식 소스는 언제 만들어야 하나요? {#when-should-i-create-a-knowledge-source}

카탈로그 데이터를 컨텍스트로 필요로 하는 커스텀 에이전트(Canvas 에이전트 또는 카탈로그 에이전트)를 설정할 때 지식 소스를 만드세요. 지식 소스는 에이전트의 지침에서 카탈로그를 직접 참조하는 것보다 더 정확하게 카탈로그 데이터를 검색하는 데 도움을 줍니다.

### 에이전트에 지식 소스를 컨텍스트로 제공한 경우, 원본 카탈로그도 컨텍스트로 할당해야 하나요? {#if-an-agent-has-been-given-a-knowledge-source-as-context-do-i-also-need-to-assign-the-original-catalog-as-context}

아니요. 지식 소스가 에이전트 컨텍스트로서 카탈로그를 대체하므로 둘 다 연결할 필요가 없습니다. 지식 소스를 만들 때 에이전트에 필요한 카탈로그 필드만 포함하세요.

### 지식 소스의 효과를 어떻게 평가해야 하나요? {#how-should-i-evaluate-the-effectiveness-of-a-knowledge-source}

일반 카탈로그를 참조하는 기존 에이전트를 복제하고, 해당 지식 소스를 참조하도록 전환하세요. 에이전트 콘솔에서 몇 가지 테스트 호출을 실행하여 정확성을 확인한 다음, 배포된 위치에서 기존 에이전트를 교체하거나, 실험 경로 단계를 사용하여 기존 에이전트와 새 에이전트를 A/B 테스트하여 성능 영향을 파악하는 것을 고려하세요.