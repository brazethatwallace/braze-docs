...
page_order: 1.6
description: "이 문서에서는 A/B Campaign이 최적의 인게이지먼트를 위해 자동으로 최적화할 수 있도록 하는 기능인 BrazeAI<sup>TM</sup> 배리언트 선택에 대해 설명합니다."
search_rank: 10
toc_headers: h2
---
...
> BrazeAI<sup>TM</sup> 배리언트 선택은 단일 발송 또는 반복 A/B 테스트에서 자동으로 실험을 실행하고 최적의 인게이지먼트 결과를 위해 최적화하는 기능입니다.
...
최적화 없는 열람율은 2.75%(.035\*.25 + .03\*.25 + 0.025\*.25 + 0.02\*.25)이며, 배리언트 선택은 배리언트 1(3.5%)을 선택하므로 향상은 27.3%입니다.
{% endtab %}
...
최적화 없는 열람율은 2.75%(.035\*.25 + .03\*.25 + 0.025\*.25 + 0.02\*.25)입니다. 배리언트 선택은 성과가 높은 배리언트에 더 큰 가중치를 부여합니다.
...
이 경우 선택된 열람율은 3.075%(.035\*.65 + .03\*.15 + 0.025\*.1 + 0.02\*.05)이며, 이는 11.8%의 향상입니다. 각 기간마다 이를 계산한 다음, 최적화 기간 전체에 걸쳐 평균을 산출합니다.
{% endtab %}
{% endtabs %}
