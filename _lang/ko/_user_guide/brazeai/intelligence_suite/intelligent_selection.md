...
일반적으로 재자격이 있는 Campaign은 사용자가 이전에 받은 것과 동일한 배리언트에 다시 진입하게 됩니다. 지능형 선택을 사용하면 이 기능의 최적 할당 측면으로 인해 배리언트 분포가 변경되었을 수 있으므로 Braze는 사용자가 동일한 Campaign 배리언트를 받을 것이라고 보장할 수 없습니다. 지능형 선택이 배리언트 성과를 재검토하기 전에 사용자가 다시 진입할 수 있도록 허용하면 재진입한 사용자로 인해 데이터가 왜곡될 수 있습니다.
