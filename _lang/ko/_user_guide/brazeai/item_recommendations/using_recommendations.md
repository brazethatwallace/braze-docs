...
4. **예측 항목 수**에 삽입하고 싶은 상위 제품의 수를 입력합니다. 예를 들어, 가장 많이 구매된 상위 세 개의 항목을 표시할 수 있습니다.
...
### 2단계: 이미지 참조(선택 사항) {#step-2-reference-an-image-optional}

추천 카탈로그에 이미지 링크가 포함되어 있는 경우 메시지에서 해당 이미지를 참조할 수 있습니다.

{% tabs %}
{% tab 드래그 앤 드롭%}
이메일 드래그 앤 드롭 편집기에서 이메일에 이미지 블록을 추가한 다음 이미지 블록을 선택하여 **이미지 속성**을 엽니다.

![드래그 앤 드롭 편집기의 이미지 속성 패널]({% image_buster /assets/img/image_with_liquid.png %}){: style="max-width:45%"}

**Liquid를 사용한 이미지**를 토글한 다음 **동적 URL** 필드에 다음을 추가합니다(URL 필드는 줄바꿈을 지원하지 않으므로 코드가 한 줄에 표시되는지 확인하세요):
...
{: .reset-td-br-1 .reset-td-br-2 aria-label="2단계: 이미지 참조(선택 사항)" }
