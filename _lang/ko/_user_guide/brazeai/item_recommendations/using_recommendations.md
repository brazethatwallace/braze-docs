...
2. **Personalization Type**에서 **Item Recommendation**을 선택합니다.
3. **Item Recommendation Name**에서 방금 생성한 추천을 선택합니다.
4. **Number of Predicted Items**에 삽입하고 싶은 상위 제품의 수를 입력합니다. 예를 들어, 가장 많이 구매된 상위 세 개의 항목을 표시할 수 있습니다.
5. **Information to Display**에서 각 항목에 포함할 카탈로그 필드를 선택합니다. 각 항목의 이 필드 값은 이 추천과 연결된 카탈로그에서 가져옵니다.
