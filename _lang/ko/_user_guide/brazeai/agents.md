...
실제로 에이전트는 제목란이나 제품 내 텍스트와 같은 메시지 카피를 자동으로 생성할 수 있어, 모든 고객이 자신에게 맞춤화된 커뮤니케이션을 받을 수 있습니다. 또한 실시간으로 적응하여 선호도, 동작 또는 기타 데이터를 기반으로 사용자를 다양한 캔버스 경로로 라우팅할 수 있습니다.

메시징을 넘어, 에이전트는 제품 및 프로필 필드 값을 계산하거나 생성하여 카탈로그를 풍부하게 하고 데이터를 최신 상태로 동적으로 유지할 수 있습니다. 반복적이거나 복잡한 작업을 맡아 팀이 수동 설정 대신 전략과 창의성에 집중할 수 있도록 합니다. Braze 에이전트는 백그라운드 프로세스가 아닌 협력자처럼 작동하여 문제를 해결하고 대규모로 영향을 전달하는 데 도움을 줍니다.

### Braze 에이전트와 다른 BrazeAI 기능의 사용 시점

사용자의 특정 컨텍스트를 활용하여 콘텐츠를 즉석에서 개인화할 때 에이전트를 사용하세요. 예를 들어, 에이전트가 특정 사용자의 좋아하는 아이스크림 맛이 초콜릿이고 좋아하는 토핑이 젤리곰이라는 것을 알고 있다면, 해당 사용자가 캔버스를 통과할 때 그 조합에 맞는 푸시 카피를 작성할 수 있습니다.
...
- [커스텀 에이전트 생성하기]({{site.baseurl}}/user_guide/brazeai/agents/creating_agents/)
