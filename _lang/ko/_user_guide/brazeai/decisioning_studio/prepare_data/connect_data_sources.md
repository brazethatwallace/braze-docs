...
| 데이터 유형 | 설명 | 예시 |
|-----------|-------------|----------|
| **고객 프로필** | 정적이거나 느리게 변하는 속성 | 고객 기간, 지역, 유입 채널, 만족도, LTV or 생애주기 가치 추정치 |
| **고객 행동** | 활동 및 인게이지먼트 패턴 | 계정 로그인, 기기 유형, 고객 서비스 상호작용, 제품 사용 |
| **트랜잭션 내역** | 구매 및 전환 데이터 | 구매한 제품, 거래 금액, 결제 수단, 구매 채널 |
| **마케팅 인게이지먼트** | 커뮤니케이션에 대한 반응 | 이메일 열람/클릭, 단문 메시지 서비스 인게이지먼트, 웹 및 모바일 활동, 설문조사 응답 |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="고객 데이터 유형" }

{% alert tip %}
에이전트가 고객에 대해 더 많은 정보를 가질수록 더 나은 성능을 발휘합니다. 비즈니스에 특히 중요한 인사이트에 대한 데이터를 포함하는 것을 고려하세요(예: 인공지능이 로열티 고객을 어떻게 다르게 대우하는지 확인하고 싶다면, 고객 데이터에 로열티 상태를 반드시 포함하세요).
{% endalert %}
...
사용자 프로필이나 커스텀 속성에 없는 고객 데이터의 경우, [Braze 클라우드 데이터 수집]({{site.baseurl}}/user_guide/data/unification/cloud_ingestion)을 통해 두 가지 방법으로 가져올 수 있습니다.

- Braze 데이터 플랫폼으로 수집합니다. 데이터 웨어하우스 데이터를 Braze 사용자 프로필, 커스텀 속성 또는 이벤트에 동기화합니다. 세분화 및 메시징을 위해 Braze에서도 데이터를 사용하려는 경우 이 방법을 선택하세요. Snowflake, Redshift, BigQuery, Databricks, Microsoft Fabric, AWS S3, Google Cloud Storage를 지원합니다.
- Decisioning Studio로 직접 전송합니다(얼리 액세스). 데이터 웨어하우스 데이터를 Braze 사용자 프로필이나 커스텀 속성에 추가하지 않고 Decisioning Studio로 직접 동기화합니다. Decisioning Studio에서 사용하되 Braze의 다른 곳에서는 필요하지 않은 데이터에 이 방법을 선택하세요. 이 옵션은 얼리 액세스 중이며, 설정 방법은 [클라우드 데이터 수집: Decisioning Studio 데이터 동기화]({{site.baseurl}}/user_guide/data/unification/cloud_ingestion/decisioning_studio)를 참조하세요.

Braze 데이터 플랫폼으로 전송하는 데이터에 만족하면, AI Decisioning 서비스 팀에 연락하여 사용자 프로필 또는 커스텀 속성의 어떤 필드를 AI Decisioning에 사용해야 하는지 논의하세요.

이 프로세스를 간소화하려면, Decisioning Studio에서 사용해야 할 고객 행동을 가장 잘 나타내는 Braze 사용자 프로필 속성 목록을 작성하세요([내보내기 가능한 필드 목록]({{site.baseurl}}/api/endpoints/export/user_data/post_users_segment#fields-to-export) 참조). 서비스 팀은 AI Decisioning에 가장 적합한 필드를 결정하기 위한 디스커버리 세션을 진행하는 데도 도움을 줄 수 있습니다.
...
이러한 패턴은 더 많은 엔지니어링 작업이 필요하지만, 현재 Braze 구성에 따라 더 적합할 수 있습니다. 자세한 내용은 AI Decisioning 서비스 팀에 문의하세요.
...
확장 ID와 API 키를 AI Decisioning 서비스 팀에 제공하세요. 팀에서 고객 데이터 수집의 다음 단계를 안내해 드립니다.
...
2. 프로필에 대한 전체 액세스 권한이 있는 비공개 API 키를 생성합니다.
...
이것이 가능한 경우, Decisioning Studio 전용으로 격리된 고객 데이터 내보내기용 GCS 버킷을 제공할 수 있습니다.
...
- **증분 업데이트:** 매일 전체 고객 이력의 스냅샷보다 증분 파일이 더 바람직합니다.
...
다른 옵션이나 완전히 커스텀 데이터 파이프라인도 가능합니다. 이러한 경우 팀에서 추가적인 서비스 작업이나 엔지니어링 작업이 필요할 수 있습니다. 무엇이 실현 가능하고 최적인지 판단하려면 AI Decisioning 서비스 팀과 협력하세요.

{% alert important %}
이 가이드에서는 가장 일반적인 통합 패턴을 설명합니다. 정보 보안팀은 여전히 모든 연결 지점을 검토해야 하며, 솔루션 컨설턴트가 구현에 대한 조언을 제공할 수 있습니다.
{% endalert %}
