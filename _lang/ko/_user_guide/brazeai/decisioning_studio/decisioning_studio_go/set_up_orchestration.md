...
1. **API Entry Event** 뒤에 **결정 분할**을 드래그 앤 드롭하세요.
2. **결정 분할** 세부 정보에서 첫 번째 경로에 대해 **Edit**를 선택하세요.

!["Edit" 버튼이 있는 결정 분할 세부 정보.]({% image_buster /assets/img/decisioning_studio_go/journey5.png %})

{: start="3"}
3. 추천 데이터 확장에서 전달된 템플릿 ID를 사용하도록 **결정 분할**을 업데이트하세요. **Journey Data** 아래에서 적절한 필드를 찾으세요.
...
8. **Done**을 선택하여 **결정 분할** 노드를 저장하세요.

![각 이메일 템플릿 ID에 대한 결정 분할의 두 가지 경로.]({% image_buster /assets/img/decisioning_studio_go/journey10.png %}){: style="max-width:65%;"}

**각 결정 분할에 이메일 추가:**

1. **Email** 노드를 **결정 분할**의 각 경로로 드래그하세요.
2. **Email**을 선택한 후, 각 경로에 적용될 적절한 템플릿을 선택하세요(즉, ID 값이 있는 템플릿이 결정 분할의 로직과 일치해야 합니다).
