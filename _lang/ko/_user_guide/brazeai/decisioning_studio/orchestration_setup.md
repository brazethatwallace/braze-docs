...
오케스트레이션은 Decisioning Studio와 고객 인게이지먼트 플랫폼(CEP) 간의 연결입니다. 의사 결정 에이전트가 각 고객에 대한 최적의 동작을 결정하면, 오케스트레이션은 CEP를 통해 개인화된 커뮤니케이션을 트리거하여 해당 결정을 실행합니다.
...
| **동적 콘텐츠 지원** | Campaigns이 추천 데이터를 기반으로 필드를 동적으로 채울 수 있어야 합니다. |
...
- **어떤 동적 요소?** 에이전트가 개인화할 메시지 부분(제목란, CTA, 오퍼, 타이밍 등)을 식별하세요. 이들은 API 트리거 속성정보 또는 동적 입력 안내가 됩니다.
...
Braze 통합의 경우, 에이전트가 최적화할 차원을 계획하세요. 이들은 Campaign에 동적 값을 전달하는 API 트리거 속성정보가 됩니다:
...
모든 최적화 차원에 대한 API 트리거 속성정보가 포함된 각 기본 템플릿에 대해 API 트리거 Campaign을 설정하세요.
...
5. **피드백 데이터를 계획하세요.** 오케스트레이션에는 메시지 발송과 에이전트가 학습하는 데 도움이 되는 인게이지먼트 및 전환 데이터 수집이 포함됩니다. 자세한 내용은 [데이터 준비]({{site.baseurl}}/user_guide/brazeai/decisioning_studio/prepare_data)를 참조하세요.
