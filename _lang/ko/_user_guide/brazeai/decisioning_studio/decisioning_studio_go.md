...
에이전트가 선택할 수 있는 배리언트(제목란, CTA, 이미지, 발송 요일, 발송 시간 등)를 정의합니다. Segment 내 각 사용자에 대해 에이전트는 설정한 제약 조건과 스케줄 내에서 인게이지먼트를 유도할 가능성이 가장 높은 옵션을 선택합니다.
...
| 설정 | 설명 |
|---|---|
| **오디언스** | 진입 오디언스로 사용할 단일 Braze Segment입니다. 에이전트가 자동으로 Segment를 의사 결정 그룹과 무작위 대조군으로 분할합니다. |
