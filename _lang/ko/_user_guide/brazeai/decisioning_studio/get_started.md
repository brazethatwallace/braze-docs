...
| 결정 사항 | 설명 | 예시 |
|----------|-------------|----------|
| **성공 측정기준** | 고객 참여를 개인화할 때 에이전트가 극대화하는 비즈니스 성과입니다. | 매출, LTV, ARPU, 전환, 유지 |
| **오디언스** | Decisioning Studio 에이전트가 인게이지먼트 결정을 내리는 대상 고객입니다. | 전체 고객, 로열티 회원, 이탈 위험 가입자 |
| **실험 그룹** | Decisioning Studio의 무작위 대조 시험을 어떻게 구성할 것인가? | Decisioning Studio, Random Control, BAU, Holdout |
| **차원** | 에이전트가 각 고객에 대해 개인화하는 인게이지먼트 결정 항목입니다. | 시간대, 제목란, 빈도, 오퍼, 채널 |
| **옵션** | 에이전트가 각 차원 내에서 선택할 수 있는 특정 배리언트입니다. | 특정 템플릿, 오퍼, 시간 기간 |
| **제약 조건** | 에이전트의 결정을 제한하는 비즈니스 규칙 및 한도입니다. | 지역 제한, 예산 한도, 자격 규칙 |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="주요 설계 결정 사항" }
...
{: .reset-td-br-1 .reset-td-br-2 aria-label="Decisioning Studio 기능" }
...
- **액션을 다양화하세요:** 에이전트가 취할 수 있는 액션의 집합이 다양할수록 각 사용자에 맞게 전략을 더 잘 개인화할 수 있습니다.
