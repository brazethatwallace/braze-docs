...
Braze에서 CSV 가져오기, 서버리스 S3 Lambda CSV 가져오기 스크립트, 직접 API 호출 또는 데이터 웨어하우스의 클라우드 데이터 수집을 통해 사용자 속성과 이벤트를 업로드할 수 있습니다.

### Braze CSV 가져오기 {#braze-csv-import}

CSV 가져오기를 사용하여 다음 사용자 속성 및 커스텀 이벤트를 기록하고 업데이트할 수 있습니다. 시작하려면 [CSV 가져오기]({{site.baseurl}}/user_guide/audience/manage_audience/import_users/csv_import)를 참조하세요.
...
Braze는 표준 CSV 형식의 사용자 데이터를 수신합니다. 기본 및 커스텀 속성 가져오기는 최대 500 MB 크기의 파일을 지원하며, 커스텀 이벤트 가져오기는 최대 50 MB 크기의 파일을 지원합니다. 식별자, 열 헤더, 유효성 검사 규칙 및 예시에 대한 자세한 내용은 [CSV 가져오기]({{site.baseurl}}/user_guide/audience/manage_audience/import_users/csv_import)를 참조하세요.

대시보드의 **사용자 가져오기**를 통해 대용량 CSV를 업로드하면, Braze가 파일을 수신하고 계산 단계를 실행하는 동안 페이지가 응답하지 않거나 느리게 반응할 수 있습니다. 업로드와 계산이 완료될 때까지 기다려 주세요. 총 소요 시간은 파일 크기에 따라 몇 분에서 몇 시간까지 걸릴 수 있으며, 파일이 클수록 계산 시간이 더 오래 걸립니다.
...
Braze는 가져오기 시 HTML 데이터를 삭제, 유효성 검사 또는 재포맷하지 않으므로, 웹 개인화에 사용하는 모든 가져오기 데이터에서 스크립트 태그를 제거해야 합니다.

웹 브라우저에서 개인화 용도로 사용할 데이터를 Braze로 가져올 때는 HTML, JavaScript 또는 웹 브라우저에서 렌더링될 때 악의적으로 활용될 수 있는 기타 스크립트 태그가 제거되었는지 확인합니다.

또는 HTML의 경우 Braze Liquid 필터(`strip_html`)를 사용하여 렌더링된 텍스트에서 HTML을 제거할 수 있습니다. 예를 들어:
