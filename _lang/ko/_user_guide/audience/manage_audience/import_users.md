...
| 커스텀 이벤트 | 사용자 동작을 나타내는 비즈니스에 고유한 이벤트입니다. | `trip_booked` | 50 MB |
...
대시보드에서 **Import Users**를 통해 대용량 CSV를 업로드하면, Braze가 파일을 수신하고 계산 단계를 실행하는 동안 페이지가 응답하지 않거나 느리게 응답할 수 있습니다. 업로드와 계산이 완료될 때까지 기다려 주세요. 총 소요 시간은 파일 크기에 따라 몇 분에서 몇 시간까지 걸릴 수 있으며, 파일이 클수록 계산 시간이 더 오래 걸립니다.
...
1,000,000행이 포함된 파일의 예상 실행 시간은 약 5분입니다. 자세한 내용은 [사용자 속성 CSV를 Braze로 가져오기](https://www.braze.com/docs/user_guide/data/unification/cloud_ingestion)를 참조하세요.
