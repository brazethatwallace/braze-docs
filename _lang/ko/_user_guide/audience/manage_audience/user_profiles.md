...
| 수신한 Campaigns | 사용자가 Campaign을 수신하거나 사용자에 대한 상호작용 데이터가 처음 감지되면 수신한 Campaign으로 표시됩니다.<br><br>메시지가 수신, 열림 또는 클릭되면 Braze는 상호작용을 기록한 프로필과 동일한 채널 식별자를 공유하는 모든 프로필의 데이터를 업데이트합니다(예: 이메일의 경우 동일한 이메일 주소, SMS 또는 WhatsApp의 경우 동일한 전화번호). 메시지를 수신, 열람 또는 클릭한 사람과 식별자를 공유하는 사용자는 원래 Campaign에 포함되지 않았거나 메시지를 직접 전송받지 않았더라도 이 필터에 일치할 수 있습니다.<br><br>목록에서 Campaign을 선택하여 확인합니다. |
...
| 수신한 Canvas 메시지 | 이 사용자가 수신한 Canvas 메시지와 수신 시점.<br><br>메시지가 수신, 열림 또는 클릭되면 Braze는 상호작용을 기록한 프로필과 동일한 채널 식별자를 공유하는 모든 프로필의 데이터를 업데이트합니다(예: 이메일의 경우 동일한 이메일 주소, SMS 또는 WhatsApp의 경우 동일한 전화번호). 메시지를 수신, 열람 또는 클릭한 사람과 식별자를 공유하는 사용자는 원래 Campaign에 포함되지 않았거나 메시지를 직접 전송받지 않았더라도 이 필터에 일치할 수 있습니다.<br><br>목록에서 메시지를 선택하여 확인합니다. |
