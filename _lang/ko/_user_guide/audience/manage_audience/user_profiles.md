...
[인게이지먼트 탭](#engagement-tab)을 사용하여 특정 사용자가 캠페인을 수신했는지 확인할 수 있습니다. 또한 해당 사용자가 캠페인을 수신한 경우, 언제 수신했는지도 확인할 수 있습니다. 사용자가 특정 Segment에 속해 있는지, 푸시, 이메일 또는 둘 다에 옵트인했는지도 확인할 수 있습니다. 이 정보는 문제 해결 목적으로 유용합니다. 예를 들어, 사용자가 수신할 것으로 예상했던 캠페인을 수신하지 못하거나, 수신할 것으로 예상하지 않았던 캠페인을 수신하는 경우 이 정보를 확인해야 합니다.

## 고객 프로필의 구성 요소 {#elements-of-user-profile}

고객 프로필에는 다섯 가지 주요 섹션이 있습니다.

- **개요:** 사용자에 대한 기본 정보, 세션 데이터, 커스텀 속성, 커스텀 이벤트, 구매 내역, 사용자가 마지막으로 로그인한 최신 기기 정보입니다.
...
| 개요 카테고리 | 포함 내용 |
| --- | --- |
| 프로필 | 성별, 연령대, 위치, 언어, 로캘, 시간대, 생년월일. |
| 세션 개요 | 세션 횟수, 첫 번째 및 마지막 세션 시점, 사용한 앱. |
...
| 수신한 Campaigns | **수신한 Campaigns**는 채널별 발송 및 조회 시점을 반영합니다. 대부분의 채널은 Braze가 메시지를 전달 공급자에게 전달할 때 발송을 기록하며, 메시지가 최종적으로 전달되지 않더라도 기록됩니다. **Content Cards**는 다릅니다. 사용자가 앱에서 카드를 조회한 후에만 여기에 표시됩니다. 채널별 분류는 [수신한 Campaigns에 표시되는 시점](#when-campaigns-appear-in-campaigns-received)을 참조하세요. <br><br>메시지가 수신, 열람 또는 클릭되면 Braze는 상호작용을 기록한 프로필과 동일한 채널 식별자를 공유하는 모든 프로필의 데이터를 업데이트합니다(예: 이메일의 경우 동일한 이메일 주소, SMS 또는 WhatsApp의 경우 동일한 전화번호). 메시지를 수신, 열람 또는 클릭한 사람과 식별자를 공유하는 사용자는 원래 해당 Campaign에 포함되지 않았거나 직접 메시지를 받지 않았더라도 이 필터에 일치할 수 있습니다.<br><br>이 목록은 리타겟팅 및 기록에 표시되는 내용을 결정할 때 [메시징 상호작용 데이터]({{site.baseurl}}/api/data_retention/messaging_interaction_data)(만료 규칙 포함)를 사용합니다.<br><br> 목록에서 Campaign을 선택하여 확인할 수 있습니다. |
...
| 수신한 Canvas 메시지 | 이 사용자가 수신한 Canvas 메시지와 수신 시점. 발송 시점은 **수신한 Campaigns**와 동일한 채널 규칙을 따릅니다. [수신한 Campaigns에 표시되는 시점](#when-campaigns-appear-in-campaigns-received)을 참조하세요.<br><br> 메시지가 수신, 열람 또는 클릭되면 Braze는 상호작용을 기록한 프로필과 동일한 채널 식별자를 공유하는 모든 프로필의 데이터를 업데이트합니다(예: 이메일의 경우 동일한 이메일 주소, SMS 또는 WhatsApp의 경우 동일한 전화번호). 메시지를 수신, 열람 또는 클릭한 사람과 식별자를 공유하는 사용자는 원래 해당 Campaign에 포함되지 않았거나 직접 메시지를 받지 않았더라도 이 필터에 일치할 수 있습니다.<br><br> 목록에서 메시지를 선택하여 확인할 수 있습니다. |
...
- **이메일:** Braze는 메시지가 이메일 서비스 공급자(ESP)에게 전달될 때 발송을 기록합니다. 전달 이후에는 Liquid 로직, 사용량 제한, 사용자가 도달 불가로 표시된 경우 등의 이유로 메시지가 중단되지 않습니다. 이후 이벤트는 보통 전달 또는 반송입니다.
- **푸시:** Braze는 메시지가 푸시 공급자(예: Apple Push Notification service(APNs) 또는 Firebase Cloud Messaging(FCM))에게 전달될 때 발송을 기록합니다. 공급자는 보통 즉시 전달을 시도하며, 기기를 사용할 수 없는 경우(예: 오프라인) 메시지가 만료될 때까지 재시도할 수 있습니다.
...
- **SMS, WhatsApp, 웹훅:** Braze는 메시지가 해당 채널의 전달 경로(예: SMS 또는 WhatsApp 공급자, 또는 웹훅 엔드포인트)에 진입할 때 발송을 기록합니다.

{% alert note %}
이 설명은 **수신한 Campaigns**에 대해 발송이 기록되는 시점을 다룹니다. 메시지가 공급자에 도달하기 전에 중단할 수 있는 [메시지 중단]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/liquid/aborting_messages)과는 별개입니다.
{% endalert %}
...
고객 프로필의 **메시지 기록** 탭에는 지난 30일간 개별 사용자에 대한 최근 메시징 관련 이벤트(약 40건)가 표시됩니다. 이 이벤트에는 사용자에게 발송된 메시지, 수신한 메시지, 상호작용한 메시지 등이 포함됩니다.
...
| --- | --- |
| 이메일 | 반송<br>클릭<br>지연 이벤트<br>전달<br>스팸 표시<br>열람([이메일 열람 이벤트에 대한 참고 사항](#note-on-email-open-event) 참조)<br>발송<br>소프트 반송<br>탈퇴 |
| SMS | 통신사 발송<br>전달<br>전달 실패<br>인바운드 수신<br>거부<br>발송 |
| 푸시 | 반송<br>영향받은 열람<br>iOS 포그라운드<br>열람<br>발송 |
| 인앱 메시지 | 클릭<br>노출 횟수 |
| Content Cards | 클릭<br>닫기<br>노출 횟수<br>발송 |
| 웹훅 | 발송 |
| WhatsApp | 중단<br>전달<br>실패<br>최대 게재빈도 설정 적용<br>인바운드 수신<br>읽음<br>발송 |
{: .reset-td-br-1 .reset-td-br-2 aria-label="메시지 인게이지먼트 이벤트" }

##### 메시지 중단 이벤트 {#message-abort-events}

메시지 중단 이벤트는 [Liquid]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/liquid/aborting_messages) 또는 [연결된 콘텐츠]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/connected_content/aborting_connected_content)의 조건 로직이나 Liquid 렌더링 시간 초과로 인해 사용자에게 보낸 메시지가 중단된 경우 발생합니다.
...
최대 게재빈도 설정 이벤트는 사용자가 메시지를 수신할 자격이 있지만 [최대 게재빈도 설정]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/frequency_capping#about-frequency-capping) 설정으로 인해 실제로 수신하지 못한 경우 발생합니다. **설정** > **최대 게재빈도 설정 규칙**에서 최대 게재빈도 설정을 커스터마이즈할 수 있습니다.
...
이메일 열람 통계는 집계 수준에서 유용할 수 있지만(예: 다른 제목란의 효과 비교), 개별 사용자에 대한 개별 열람 이벤트가 의미 있다고 가정해서는 안 됩니다.
...
- **메시지 발송** 데이터가 없는 이벤트는 해당 Campaign에 메시지 배리언트가 없음을 나타냅니다.
- **Campaign/Canvas** 및 **메시지 발송** 데이터가 없는 이벤트는 `campaign_id`와 `message_variation_id`를 지정하지 않은 API Campaign(API 트리거 Campaign이 아닌)에서 보낸 메시지임을 나타냅니다. 이 필드는 선택 사항이며 요청 본문에서 생략될 수 있습니다. 이 필드가 지정되면 해당 정보가 메시지 기록 로그에 채워집니다.
   - 특정 메시지가 메시징 기록에는 없지만 **수신한 Campaigns** 로그에 나타나는 경우, 사용자가 현재 사용자로 식별되기 전에 해당 Campaign을 수신했을 가능성이 높습니다. 기존 프로필이 분리된 경우 **수신한 Campaigns** 로그는 이전되지만 메시징 기록은 이전되지 않습니다.
- **Campaign/Canvas** 데이터가 없는 경우 수동 테스트가 발송되었을 수 있습니다. 수동 테스트는 **메시징 기록** 탭에 기록되지만, 발송된 Campaign 또는 Canvas는 기록되지 않습니다.
- 사용자가 시드 그룹이나 기타 내부 테스트 오디언스에 속한 경우, **메시징 기록**에는 프로덕션 발송에 비해 제한된 Campaign 또는 Canvas 메타데이터가 표시될 수 있습니다.
