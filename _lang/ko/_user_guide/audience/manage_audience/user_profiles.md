...
메시지 중단 이벤트는 사용자에게 전송된 메시지가 [Liquid]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/liquid/aborting_messages) 또는 [연결된 콘텐츠]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/connected_content/aborting_connected_content#aborting-connected-content)의 조건 로직이나 Liquid 렌더링 시간 초과로 인해 중단된 경우 발생합니다.
