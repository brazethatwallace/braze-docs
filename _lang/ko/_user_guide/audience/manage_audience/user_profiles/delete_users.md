...
사용자를 삭제하려면 관리자이거나 **Delete Users** 권한이 있어야 합니다. 사용자 삭제 기록을 보려면 관리자이거나 **View User Deletion Records** 권한이 있어야 합니다. 다음 권한은 사용자 삭제 및 삭제 기록을 제어합니다:
...
사용자 삭제를 사용하면 더 이상 필요하지 않거나, 실수로 생성되었거나, 규정 준수(예: GDPR 또는 CCPA)를 위해 삭제해야 하는 프로필을 제거하여 데이터베이스를 관리할 수 있습니다.

| 고려 사항 | 세부 정보 |
|---------------|---------|
| 최대 크기 | Segment를 삭제할 때 최대 1,000만 개의 고객 프로필을 삭제할 수 있습니다. |
| 대기 기간 | 모든 Segment 삭제에는 7일의 대기 기간과 삭제 처리에 소요되는 시간이 필요합니다. |
| 작업 제한 | 한 번에 하나의 Segment만 삭제할 수 있으며, 여기에는 7일의 대기 기간이 포함됩니다. |
{: .reset-td-br-1 .reset-td-br-2 aria-label="사용자 삭제에 대하여" }

## 사용자 삭제

Braze 대시보드를 통해 [개별 사용자](#delete-individual) 또는 [사용자 Segment](#delete-segment)를 삭제할 수 있습니다.

### 개별 사용자 삭제 {#delete-individual}

Braze에서 개별 사용자를 삭제하려면 **오디언스** > **사용자 검색**으로 이동한 다음 사용자를 검색하고 선택합니다. 중복된 고객 프로필을 삭제하는 경우 올바른 프로필을 선택했는지 확인하세요.
...
해당 사용자의 프로필 페이지에서 <i class="fa-solid fa-ellipsis-vertical" aria-label="옵션 표시"></i> **옵션 표시** > **사용자 삭제**를 선택합니다. 사용자가 Braze에서 완전히 삭제되기까지 몇 분이 걸릴 수 있습니다.


### Segment 삭제 {#delete-segment}

아직 생성하지 않았다면, 삭제하려는 고객 프로필이 포함된 [Segment를 생성]({{site.baseurl}}/user_guide/audience/segments/creating_a_segment)합니다. 중복 사용자를 삭제하는 경우 모든 고객 프로필을 포함해야 합니다.

Braze에서 **오디언스** > **오디언스 관리**로 이동한 다음 **사용자 삭제** 탭을 선택합니다.
...
**사용자 삭제**를 선택하고, 삭제할 Segment를 선택한 다음 **다음**을 선택합니다.
...
요청을 확인하기 위해 **DELETE**를 입력한 다음 **사용자 삭제**를 선택합니다.
...
이 Segment의 사용자는 즉시 삭제되지 않습니다. 대신 향후 7일 동안 삭제 대기 상태로 표시됩니다. 이 기간이 지나면 사용자가 삭제되며, 완료 시 이메일로 알림을 보내드립니다.

7일 대기 기간 동안 삭제 대기 중인 사용자는 명시적으로 제외하지 않는 한 여전히 Campaigns 및 Canvases를 수신할 수 있습니다. 대기 중인 사용자가 메시지를 수신하지 못하도록 하려면, **삭제 대기** 상태의 사용자를 Campaigns 및 Canvases에서 제외하는 Segment 필터를 추가하세요.

{% alert tip %}
Segment 변경과 관계없이 해당 사용자가 정확히 삭제되도록 **삭제 대기**라는 Segment 필터가 자동으로 생성됩니다. [이 필터를 사용]({{site.baseurl}}/user_guide/audience/segments/managing_segments#filters)하여 삭제 대기 상태를 확인할 수 있습니다.
{% endalert %}

Braze는 삭제 대기 중인 프로필 수가 포함된 확인 이메일을 발송합니다.

삭제를 계속 진행하려면 Braze에 로그인하여 삭제 요청을 확인합니다.
...
### 1,000만 명 이상의 사용자가 포함된 Segment를 삭제할 수 있나요?
...
### 한 번에 최대 1,000만 명의 사용자만 삭제할 수 있습니다. 이것은 버그인가요?

아니요, 이것은 버그가 아닙니다. 한 번의 Segment 삭제 실행에서 삭제할 수 있는 최대 고객 프로필 수는 1,000만 개입니다.

### 자동 사용자 병합이 사용자 삭제에 영향을 미치나요?

예약된 병합에 삭제 대기 중인 고객 프로필이 포함된 경우, Braze는 해당 프로필을 건너뛰고 병합하지 않습니다. 이러한 프로필을 병합하려면 먼저 삭제에서 제거해야 합니다.

### 삭제 대기 중인 사용자에게 전송된 데이터는 어떻게 되나요?

외부 시스템이나 SDK에서 전송된 데이터는 여전히 수신되지만, 해당 사용자는 활동과 관계없이 예정대로 삭제됩니다.

### 삭제 대기 중인 사용자에 대해 Canvases와 Campaigns가 트리거되나요?
...
### 대시보드 대신 API를 사용하여 사용자를 삭제할 수 있나요?
