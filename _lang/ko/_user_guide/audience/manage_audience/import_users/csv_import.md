...
CSV 가져오기를 사용하여 다음 사용자 속성 및 커스텀 이벤트를 기록하고 업데이트할 수 있습니다. Braze는 이 데이터를 다음 표에 나와 있는 최대 크기 이내의 표준 CSV 파일로 수신합니다.
...
{% alert note %}
`external_id`가 있는 사용자와 없는 사용자를 혼합하여 업로드하는 경우 각 가져오기에 대해 별도의 CSV를 생성해야 합니다. 하나의 CSV에 `external_id`와 사용자 별칭을 모두 포함할 수 없습니다.
{% endalert %}
{% endtab %}
...
- **커스텀 이벤트:** 비즈니스에 고유하며 사용자가 수행한 작업을 반영합니다. 예를 들어 여행 예약 앱의 경우 `trip_booked`가 있습니다.
...
| 숫자 | 공백이나 쉼표 없이 정수 또는 플로트여야 합니다. 플로트는 마침표(`.`)를 소수점 구분 기호로 사용해야 합니다. |
| 문자열 | 값이 큰따옴표(`""`)로 감싸져 있으면 쉼표를 포함할 수 있습니다. |
| 빈 값 | 빈 값은 고객 프로필의 기존 값을 덮어쓰지 않으며, CSV 파일에 모든 기존 사용자 속성을 포함할 필요가 없습니다. |
{: .reset-td-br-1 .reset-td-br-2 aria-label="커스텀 속성" }

{% alert important %}
배열, 푸시 토큰 및 커스텀 이벤트 데이터 유형은 사용자 가져오기에서 지원되지 않습니다. CSV 파일의 쉼표가 열 구분 기호로 해석되어 파일 파싱 시 오류가 발생할 수 있기 때문입니다.<br><br>이러한 종류의 값을 업로드하려면 [`/users/track` 엔드포인트]({{site.baseurl}}/api/endpoints/user_data/post_user_track) 또는 [Cloud Data Ingestion]({{site.baseurl}}/user_guide/data/unification/cloud_ingestion)을 대신 사용합니다.
{% endalert %}
...
| `subscription_state` | 문자열 | `subscription_group_id`로 지정된 구독 그룹의 구독 상태입니다. 허용되는 값은 `unsubscribed`(구독 그룹에 속하지 않음) 또는 `subscribed`(구독 그룹에 속함)입니다. | 아니요, 하지만 `subscription_group_id`를 사용하는 경우 강력히 권장됩니다 |
...
`external_id`는 필수가 아니지만 CSV 파일에는 다음 식별자 중 **하나**에 매핑되는 사용자 식별자가 포함되어야 합니다. 각 식별자에 대한 자세한 내용은 [식별자 선택](#choose-an-identifier)을 참조합니다.
...
CSV를 사용하여 커스텀 이벤트를 가져올 때 성공적인 데이터 가져오기를 위해 다음 요구 사항에 따라 파일을 포맷해야 합니다.

##### 커스텀 이벤트 포맷 이해 {#understanding-custom-event-formatting}

점 표기법을 사용하거나 해당 셀에 null이 아닌 값을 넣어 커스텀 이벤트 CSV를 올바르게 포맷하면 Braze가 각 속성정보를 최종 이벤트에 매핑합니다. 형식이 올바르지 않으면 속성정보가 삭제되거나 가져오기가 실패할 수 있으며, 특히 하나의 파일에 여러 이벤트 유형이 포함된 경우 더욱 그렇습니다.

##### 이벤트 속성정보에 점 표기법 사용 {#use-dot-notation-for-event-properties}

점 표기법을 사용하여 커스텀 이벤트와 해당 속성정보 간의 계층적 관계를 정의합니다. 이 포맷 규칙을 통해 각 이벤트에 대한 특정 속성을 포함하는 구조화된 이벤트 데이터를 가져올 수 있습니다.
...
다음 테이블은 속성정보가 있는 커스텀 이벤트를 가져오기 위한 올바른 포맷을 보여줍니다. 이 예시에서는 각각 다른 이벤트를 수행한 두 명의 사용자를 보여줍니다: 한 명은 영화를 대여했고 다른 한 명은 영화를 구매했습니다.
...
파일을 업로드하려면 **속성** 또는 **이벤트**를 선택하고 **파일 찾아보기**를 클릭한 다음 CSV를 업로드합니다. Braze는 처음 몇 행의 미리보기와 감지된 필드의 요약을 표시합니다.

대용량 파일(기본 속성 및 커스텀 속성의 경우 최대 500MB, 커스텀 이벤트의 경우 최대 50MB)의 경우 파일이 업로드되고 Braze가 가져오기를 계산하는 동안 대시보드가 일시적으로 응답하지 않을 수 있습니다. 이러한 업로드 및 계산은 작은 파일보다 완료하는 데 더 오래 걸릴 수 있습니다. 이 단계가 완료될 때까지 기다립니다. 파일 제한 및 타이밍에 대한 자세한 내용은 [CSV 구성]({{site.baseurl}}/user_guide/data/user_data_collection/user_import#constructing-your-csv)을 참조합니다.
...
미리보기 후 CSV 헤더를 Braze 속성, 이벤트 또는 이벤트 속성정보에 매핑할 수 있습니다. Braze는 CSV 파일의 필드를 동일한 이름의 속성, 이벤트 또는 이벤트 속성정보에 자동으로 매핑하고 필요한 경우 새 필드를 생성합니다. 또한 제안을 수동으로 조정하거나 다른 속성, 이벤트 또는 속성정보를 선택할 수 있는 유연성도 있습니다.

이벤트 속성정보의 경우 Braze는 CSV 셀에 null이 아닌 값이 포함되어 있는지 여부 또는 `<event name>.properties.<property name>` 형식의 점 표기법을 사용하는 헤더를 기반으로 속성정보를 감지하고 관련 이벤트와 연결합니다.
...
워크스페이스에 일치하는 속성, 이벤트 또는 이벤트 속성정보가 없는 경우 Braze는 CSV 필드의 이름과 감지된 데이터 유형을 사용하여 가져오기 시 새 속성, 이벤트 또는 속성정보를 생성하려고 시도합니다. 매핑 상태 옆의 **새 속성 편집**, **새 이벤트 편집** 또는 **새 속성정보 편집** 버튼을 선택하여 가져오기 전에 이 새 필드를 편집할 수 있습니다.
...
매핑 후 가져오기 설정 페이지에서 다음 타겟팅 기본 설정 중 하나를 선택할 수 있습니다. 가져오기에서 새 타겟팅 필터 또는 Segment를 생성할 필요가 없는 경우 **이 목록을 타겟팅 필터로 사용하지 않음**을 선택합니다.
...
![오류 및 경고가 있는 행 수를 보여주는 문제 발견 섹션이 있는 요약 페이지로, 뒤로 가기, 오류 보고서 다운로드 또는 가져오기 시작 옵션이 있습니다.]({% image_buster /assets/img/csv_import/summary_page_validation_results.png %})
...
[파일 유효성 검사](#file-validation)를 사용한 경우, 오류 보고서부터 확인하세요. 오류 보고서에는 플래그가 지정된 각 행의 구체적인 문제와 수정 방법에 대한 설명이 포함되어 있습니다. 유효성 검사가 아닌 가져오기 중에 실패한 행의 경우, **Import Users** 페이지에서 해당 행 위에 마우스를 올리고 <i class="fas fa-download" title="다운로드"></i> 버튼을 선택하여 오류 보고서를 다운로드하세요.
...
데이터를 올바르게 가져오려면 헤더 행이 있어야 합니다. 각 행은 헤더 행과 동일한 수의 셀을 가져야 합니다. 헤더 행보다 값이 많거나 적은 행은 가져오기에서 제외됩니다. 값 내의 쉼표는 구분자로 해석되어 이 오류를 유발할 수 있습니다.

또한 모든 데이터는 UTF-8로 인코딩되어야 합니다. 파일이 레거시 인코딩(예: 일부 Excel 기본값)으로 저장된 경우, 셀의 특수 문자와 URL이 손상되어 Braze 또는 발송된 메시지에서 물음표(`?`)로 표시될 수 있습니다.

CSV 파일에 빈 행이 있고 CSV 파일의 총 줄 수보다 적은 행이 가져와진 경우, 빈 행은 가져올 필요가 없으므로 가져오기에 문제가 있는 것이 아닐 수 있습니다. 올바르게 가져온 줄 수를 확인하고 가져오려는 사용자 수와 일치하는지 확인하세요.
...
일부 기본 속성은 사용자 업데이트에 유효한 특정 값만 허용할 수 있습니다. 자세한 내용은 [CSV 구성하기]({{site.baseurl}}/user_guide/audience/manage_audience/import_users)를 참조하세요.
