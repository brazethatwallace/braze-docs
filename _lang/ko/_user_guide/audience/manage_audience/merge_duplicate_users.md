...
- [POST: 사용자 병합]({{site.baseurl}}/api/endpoints/user_data/post_users_merge/) (`/users/merge`): 하나의 사용자 프로필을 다른 프로필에 병합합니다. 두 프로필 모두 이미 `external_id`가 있는 경우에도 가능합니다. 이 엔드포인트를 호출하기 전에 [필수 조건]({{site.baseurl}}/api/endpoints/user_data/post_users_merge/#prerequisites) 및 [병합 동작]({{site.baseurl}}/api/endpoints/user_data/post_users_merge/#merge-behavior)을 검토하세요.
