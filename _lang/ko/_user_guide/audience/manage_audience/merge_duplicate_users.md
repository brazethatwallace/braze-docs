...
중복 사용자를 일괄 병합하면 Braze가 일치하는 식별자(예: 이메일 주소)를 가진 프로필을 찾아 모든 데이터를 `external_id`가 있는 가장 최근에 업데이트된 프로필로 병합합니다. `external_id`가 있는 프로필이 없는 경우 `external_id`가 없는 가장 최근에 업데이트된 프로필이 대신 사용됩니다.
