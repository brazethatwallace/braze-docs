...
- Segment에 [태그]({{site.baseurl}}/user_guide/administer/global/workspace_settings/tags)를 추가하여 추가적으로 정리할 수 있습니다.

Segment는 **Segment 만들기**를 선택하는 즉시 저장됩니다. Segment 편집기에서 먼저 **저장**을 선택할 필요가 없습니다.

{% alert note %}
팀 수준의 "Segment 편집" 권한만 있는 경우(워크스페이스 수준이 아닌), Braze는 Segment가 생성될 때 팀을 할당합니다:
<br><br>
...
Segment에 최소 하나의 필터를 추가하세요. 원하는 만큼 많은 필터를 결합하여 세분화를 더 구체적으로 만들 수 있습니다.

{% multi_lang_include alerts/note_alerts.md alert='Segment profiles first app use' %}

### 필터 그룹 {#filter-groups}

필터는 필터 그룹으로 구성됩니다. 모든 필터는 최소 하나의 필터가 포함된 필터 그룹에 속해야 합니다. Segment에는 여러 필터 그룹이 있을 수 있습니다. 필터 그룹을 추가하려면 **필터 그룹 추가**를 선택하세요. 필터 그룹 이름 옆에 마우스를 올리면 나타나는 아이콘을 선택하여 이름을 편집할 수 있습니다.
...
그러나 "Foodies" Segment에 속하면서 "Non-foodies"와 "Candy-lovers" Segment 모두에 속하지 않는 사용자를 세분화하려면 `AND` 연산자를 사용하세요. 이렇게 하면 Campaign 또는 Canvas를 받는 사용자가 의도한 Segment("foodies")에 속하면서 동시에 다른 Segment("Non-foodies" 및 "Candy-lovers")에는 속하지 않게 됩니다.
...
#### 날짜 및 최근성 필터 표시 {#date-and-recency-filter-display}

일 단위를 사용하는 상대 시간 필터(예: 84일 이상 91일 미만 전에 수행된 이벤트)를 설정하면, Braze는 저장 후 일수가 7로 나누어 떨어지는 경우 값을 주 단위로 변환합니다. 예를 들어, 91일은 13주로 표시되지만, 121일은 7로 나누어 떨어지지 않으므로 일 단위로 유지됩니다. 이는 표시 변경일 뿐이며, 값은 여전히 일 단위로 저장되고 처리됩니다.

{% alert important %}
이미 **Segment 멤버십** 필터를 사용하는 Segments는 다른 Segments에 추가로 포함하거나 중첩할 수 없습니다. 이는 Segment A가 Segment B를 포함하고, Segment B가 다시 Segment A를 포함하려는 순환을 방지합니다. 이러한 상황이 발생하면 Segment가 자기 자신을 계속 참조하게 되어 누가 해당 Segment에 속하는지 계산할 수 없게 됩니다.
<br><br>
또한 이렇게 Segments를 중첩하면 복잡성이 증가하고 속도가 느려질 수 있습니다. 대신 포함하려는 Segment와 동일한 필터를 사용하여 Segment를 다시 만드세요.
{% endalert %}
...
**퍼널 통계 보기**를 선택하면 해당 필터 그룹의 통계를 표시하고 추가된 각 필터가 Segment 통계에 미치는 영향을 확인할 수 있습니다. 해당 시점까지의 모든 필터에 의해 타겟팅된 사용자의 예상 수와 비율을 볼 수 있습니다. 필터 그룹에 대한 통계가 표시되면 필터를 변경할 때마다 자동으로 업데이트됩니다. 이 통계는 추정치이며 생성하는 데 시간이 걸릴 수 있습니다.
...
사용자 흐름을 기록하는 필터를 추가하면 사용자가 이탈하는 지점을 확인할 수 있습니다. 예를 들어, 소셜 네트워킹 앱에서 온보딩 과정 중 사용자를 잃는 지점을 확인하고 싶다면, 가입, 친구 추가, 첫 메시지 전송에 대한 커스텀 데이터 필터를 추가할 수 있습니다. 85%의 사용자가 가입하고 친구를 추가하지만 45%만 첫 메시지를 보냈다면, 온보딩 및 마케팅 캠페인에서 더 많은 메시지 전송을 장려하는 데 집중해야 한다는 것을 알 수 있습니다.
...
![누락된 기준을 표시하며 "test1이 다음 타겟팅 기준에 부합하지 않습니다:"라는 알림이 있는 사용자 조회.]({% image_buster /assets/img_archive/user_lookup_nomatch.png %})
...
<br><br>
연결된 Segment를 보관할 때 어떤 Campaigns와 Canvases가 보관되는지 알려주는 경고 메시지가 표시됩니다.
{% endalert %}
...
각 사용자에게 푸시 알림을 하나만 보내도록 지정할 수 있습니다. [메시지를 작성]({{site.baseurl}}/user_guide/channels/push/create_a_push_message#step-4-compose-your-push-message)할 때, **추가 설정**에서 **사용자의 마지막 사용 기기에만 전송**을 선택합니다.
...
- **사용자의 Segment 멤버십이 예상과 다를 수 있습니다.**
    - 사용자가 다른 기기에 연결된 속성을 기반으로 현재 기기에서 타겟팅될 수 있습니다. 사용자가 예상치 못한 메시지를 수신한 경우, 고객 프로필에서 여러 기기를 확인해 보세요.
    - 사용자가 전송 시점에 타겟 Segment에 포함되어 있었지만, 보유한 기기 중 하나의 동작으로 인해 이후에는 해당 Segment에 포함되지 않을 수 있습니다. 이로 인해 사용자가 현재 필터 기준에 일치하지 않더라도 Campaign 또는 Canvas를 수신할 수 있습니다. <br><br>예를 들어, 사용자가 현재 OS 13.0을 사용하고 있더라도 최신 앱 버전이 OS 10.0인 사용자를 타겟팅하는 메시지를 수신할 수 있습니다. 이 경우, 메시지가 전송될 때 사용자의 OS가 10.0이었고 이후 OS 13.0으로 업그레이드한 것입니다.<br><br> 마찬가지로, 사용자가 나중에 다른 앱 버전의 기기를 사용하면 고객 프로필이 새로운 최신 앱 버전으로 업데이트됩니다. 이로 인해 사용자가 메시지 수신 자격이 없었던 것처럼 보일 수 있지만, 실제로는 전송 시점에 자격을 충족했던 것입니다.