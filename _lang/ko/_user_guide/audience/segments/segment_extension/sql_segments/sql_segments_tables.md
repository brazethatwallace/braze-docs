...
`global_control_group` | `null, boolean` | 이 변경으로 해당 버킷 번호가 전역 제어 그룹에 포함됨
`previous_global_control_group` | `null, boolean` | 이 변경 이전에 해당 버킷 번호가 전역 제어 그룹에 포함되어 있었으나 더 이상 포함되지 않음
`sf_created_at` | `timestamp`,&nbsp;`null` | 이 이벤트가 Snowpipe에 의해 수집된 시점
