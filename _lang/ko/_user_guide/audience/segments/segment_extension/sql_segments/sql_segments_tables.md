...
{% alert note %}
단일 하드바운스에 대해 동일한 사용자에 대한 여러 행이 표시될 수 있습니다. 이는 이벤트가 비동기적으로 처리되거나 관련 발송의 `dispatch_id` 값이 다를 때 발생할 수 있습니다. 중복 제거 또는 내보내기 분석 시 `dispatch_id`, `time`, `id`를 함께 고려하세요.
{% endalert %}
