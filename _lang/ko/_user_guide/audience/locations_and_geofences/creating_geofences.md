...
page_type: reference
toc_headers: h2
description: "위치 권한 설정, 위치 권한 프라이머 생성, 위치 기반 캠페인을 위한 지오펜스 구축 방법을 알아보세요."
tool:
  - Location
search_rank: 9
---
...
> 지오펜스는 위도와 경도에 반경을 결합하여 특정 글로벌 위치를 중심으로 원을 형성하는 가상 지리적 영역입니다. 지오펜스는 건물 크기부터 도시 전체 크기까지 다양할 수 있습니다. 지오펜스를 사용하면 사용자가 경계에 진입하거나 이탈할 때 실시간으로 캠페인을 트리거하거나, 몇 시간 또는 며칠 후에 후속 캠페인을 보낼 수 있습니다.

{% alert tip %}
안내 워크스루는 Braze 학습 과정 [지오펜스 생성](https://learning.braze.com/create-a-geofence)을 참조하세요.
{% endalert %}

## 작동 방식 {#how-it-works}

지오펜스는 지오펜스 세트로 구성됩니다. 지오펜스 세트는 플랫폼 전체에서 사용자를 세분화하거나 참여시키는 데 사용할 수 있는 지오펜스 그룹입니다. 각 지오펜스 세트는 최대 10,000개의 지오펜스를 포함할 수 있습니다. 지오펜스는 무제한으로 생성하거나 업로드할 수 있습니다.

지오펜스에 진입하거나 이탈하는 사용자는 세분화 및 리타겟팅에 사용할 수 있는 새로운 사용자 데이터 레이어를 추가합니다.

다음 기기 제한 사항을 유의하세요:

- Android 앱은 한 번에 최대 100개의 지오펜스를 로컬에 저장할 수 있습니다. Braze는 앱당 최대 20개의 지오펜스만 로컬에 저장하도록 구성되어 있습니다.
- iOS 기기는 앱당 한 번에 최대 20개의 지오펜스를 모니터링할 수 있습니다. Braze는 공간이 있는 경우 최대 20개의 위치를 모니터링합니다.
- 사용자가 20개 이상의 지오펜스를 수신할 자격이 있는 경우, Braze는 세션 시작 시 사용자와의 근접성을 기반으로 최대 수의 위치를 다운로드합니다.
...
### SDK 및 플랫폼 요구 사항 {#sdk-and-platform-requirements}
...
## 위치 권한 이해하기 {#understanding-location-permissions}
...
### 정밀 위치 vs 대략적 위치 {#precise-versus-approximate-location}
...
## 위치 권한 프라이머 설정 {#setting-up-a-location-permission-primer}

위치 권한 프라이머는 사용자가 네이티브 OS 권한 프롬프트를 보기 전에 위치 데이터 공유의 가치를 설명하는 인앱 메시지입니다. 네이티브 위치 프롬프트는 iOS에서 한 번만, Android에서는 제한된 횟수만 표시될 수 있으므로, 사전에 사용자를 준비시키면 옵트인 비율이 높아집니다.

### 1단계: 개발팀과 협력하기 {#step-1-work-with-your-development-team}
...
최상의 결과를 위해 사용자가 참여하고 있고 위치 공유에 가치를 느낄 가능성이 높을 때 위치 프라이머를 표시하세요.

- **아직 위치 접근 권한을 부여하지 않은 사용자를 타겟팅하세요.** 위치 권한 상태에 따라 사용자를 추적하고 세분화하는 최선의 방법을 결정하기 위해 개발팀과 협력하세요.
- **구매 완료, 매장 즐겨찾기 저장, 근처 이벤트 탐색 등 높은 가치의 행동 후에 프라이머를 표시하세요.** 사용자가 혜택을 이해할 때 옵트인할 가능성이 더 높습니다.
- **첫 실행 시 프라이머를 표시하지 마세요.** 사용자가 더 개인화된 경험을 원할 만큼 앱에서 충분한 가치를 경험할 때까지 기다리세요.

### 4단계: 권장 권한 수준 안내 {#step-4-encourage-the-recommended-permission-level}
...
{% alert tip %}
이 인앱 메시지는 사용자 여정의 어느 시점에서든 트리거할 수 있습니다—구매 후, 근처 콘텐츠를 탐색할 때, 또는 Canvas 플로우의 일부로. 다시 프롬프트를 표시할 때는 선택적으로 하세요: 옵트인 피로를 피하기 위해 이러한 캠페인을 충성도가 높거나 참여도가 높은 사용자로 제한하세요.
{% endalert %}

## 위치 프라이밍 전략 예시 {#example-location-priming-strategies}

### "앱 사용 중" 프라이머 {#while-using-the-app-primer}

소매 앱이 사용자가 매장을 즐겨찾기로 저장한 후 모달 인앱 메시지를 표시합니다:
...
### "항상 허용" 후속 조치 {#always-allow-follow-up}
...
## 수동으로 지오펜스 생성 {#manually-create-geofences}

### 1단계: 지오펜스 세트 생성 {#step-1-create-a-geofence-set}
...
![지도에 두 개의 원이 있는 "EastCoastGreaterNY"와 "WesternRegion" 두 개의 지오펜스가 포함된 지오펜스 세트.]({% image_buster /assets/img/geofence_example.png %})

## 지오펜스 대량 업로드 {#creating-geofence-sets-via-bulk-upload}
...
다음 예시는 두 개의 지오펜스를 지정하기 위한 올바른 GeoJSON 형식을 보여줍니다: 하나는 NYC의 Braze 본사이고, 다른 하나는 맨해튼 남쪽의 자유의 여신상입니다.
...
## 지오펜스 이벤트 사용 {#using-geofence-events}

지오펜스를 구성한 후에는 이를 사용하여 사용자와의 커뮤니케이션 방식을 향상하고 풍부하게 할 수 있습니다.

### Campaigns 및 Canvases 트리거 {#triggering-campaigns-and-canvases}

지오펜스 데이터를 캠페인 및 Canvas 트리거의 일부로 사용하려면 전달 방법으로 **실행 기반 전달**을 선택합니다. 다음으로 `Trigger a Geofence` 트리거 동작을 추가합니다. 마지막으로 메시지에 대한 지오펜스 세트와 지오펜스 전환 이벤트 유형을 선택합니다. 지오펜스 이벤트를 사용하여 Canvas를 통해 사용자를 진행시킬 수도 있습니다.

![사용자가 독일 공항에 진입할 때 트리거되는 지오펜스가 있는 실행 기반 캠페인.]({% image_buster /assets/img_archive/action_based_geofence_trigger.png %})

### 메시지 개인화 {#personalizing-messages}
...
## 지오펜스 세트 업데이트 {#updating-geofence-sets}

Braze SDK는 세션 시작 시 하루에 한 번만 지오펜스를 요청합니다. 세션 시작 후 지오펜스 세트를 변경하면 세트가 처음 다운로드된 시점부터 24시간을 기다려야 업데이트된 세트를 수신할 수 있습니다.

사용자가 백그라운드 푸시를 활성화한 경우, Braze는 지오펜스 세트가 업데이트될 때 24시간마다 사일런트 푸시를 보내 최신 위치를 기기로 다운로드합니다.

{% alert note %}
지오펜스가 기기에 로컬로 로드되지 않은 경우, 사용자가 해당 영역에 진입하더라도 지오펜스를 트리거할 수 없습니다.
{% endalert %}

## 지오펜스 모범 사례 {#geofence-best-practices}

### 지오펜스 구성 {#geofence-configuration}

- 안정적인 트리거를 위해 200미터 이상의 반경을 사용하세요.
- 서로 겹치거나 중첩된 지오펜스를 설정하지 마세요. 트리거에 문제가 발생할 수 있습니다.
- 지오펜스는 6시간마다 한 번만 진입 이벤트를 트리거할 수 있습니다. 이 쿨다운 기간은 로컬에서 적용됩니다. 사용자가 앱을 제거하거나 앱 데이터를 삭제하면 모든 쿨다운이 초기화됩니다.
- 기기에 총 20개 이하의 지오펜스만 저장할 수 있습니다. 사용자가 20개 이상의 자격이 있는 경우, Braze는 세션 시작 또는 사일런트 푸시 새로고침 시 근접성을 기반으로 가장 가까운 위치를 다운로드합니다.
- Braze는 사용자로부터 2,000킬로미터 반경 내의 지오펜스만 기기로 전송합니다.

### 기기 요구 사항 {#device-requirements}
...
Braze 지오펜스는 Wi-Fi, GPS 및 셀룰러 타워를 포함하여 기기에서 사용 가능한 모든 위치 제공자의 조합을 사용하여 사용자의 위치를 삼각측량합니다.

일반적인 정확도는 20미터에서 50미터 범위이며, 최상의 정확도는 5미터에서 10미터 범위입니다. 농촌 지역에서는 정확도가 크게 저하되어 수 킬로미터까지 갈 수 있습니다. 농촌 지역에서는 더 큰 반경의 지오펜스를 생성하세요.
...
### 지오펜스가 배터리 수명에 어떤 영향을 미치나요? {#how-do-geofences-affect-battery-life}

Braze 지오펜싱은 iOS 및 Android의 네이티브 지오펜스 시스템 서비스를 사용합니다. 정확도와 전력을 지능적으로 절충하여 배터리 수명을 절약하고 기본 서비스가 개선됨에 따라 성능을 향상시키도록 조정되어 있습니다.

### 지오펜스는 언제 활성화되나요? {#when-are-geofences-active}
...
지오펜스가 작동하려면 사용자가 기기에서 위치 서비스를 활성화하고 앱에 필요한 위치 권한 수준을 부여해야 합니다. 자세한 내용은 [위치 권한 이해하기](#understanding-location-permissions)를 참조하세요.

### 지오펜스 데이터가 고객 프로필에 저장되나요? {#is-geofence-data-stored-in-user-profiles}
...
### 지오펜스 안에 지오펜스를 설정할 수 있나요? {#can-i-set-up-a-geofence-within-a-geofence}

모범 사례로서 서로 겹치는 지오펜스를 설정하지 마세요. 알림 트리거에 문제가 발생할 수 있습니다.

### 사용자가 위치 접근을 거부하면 어떻게 되나요? {#what-if-a-user-denies-location-access}

개발팀은 OS의 앱 위치 설정 페이지를 여는 딥링크를 설정할 수 있으며, 사용자는 여기서 권한을 업데이트할 수 있습니다. 사용자 여정의 어느 시점에서든 커스텀 인앱 메시지 내에서 이 딥링크를 사용할 수 있습니다. 이 메시지를 표시할 시점을 선택적으로 결정하세요—옵트인 가능성을 높이기 위해 참여도가 높거나 높은 가치의 행동을 수행한 사용자를 타겟팅하세요. 자세한 내용은 [사용자를 OS 설정으로 리디렉션](#redirecting-users-to-os-settings)을 참조하세요.