...
### 익명 사용자 데이터는 어떻게 처리하나요? {#how-do-i-handle-anonymous-user-data}
...
### 이미 수집하고 식별한 사용자를 Braze 외부에서 가져오려면 어떻게 해야 하나요? {#how-can-i-import-users-i-have-already-collected-and-identified-outside-of-braze}
...
### 푸시 구독 상태의 차이점은 무엇인가요? {#whats-the-difference-between-the-push-subscription-statuses}
...
### 중복된 사용자를 식별한 경우 어떻게 하나요? {#what-if-ive-identified-duplicated-users}

{% apitags %}
Users
{% endapitags %}

중복 사용자를 식별한 경우 해당 고객 프로필을 정리해야 합니다. 다음 단계를 통해 진행할 수 있습니다:

1. `/users/export/ids` 엔드포인트를 사용하여 고객 프로필을 내보냅니다.
2. 올바른 고객 프로필을 식별하고(궁극적으로 팀이 올바른 정보를 결정해야 함), 둘 중 하나를 선택합니다:
    - `/user/track` 엔드포인트를 사용하여 유지하려는 실제 프로필과 관련된 모든 필드를 병합합니다.
    - users/delete 엔드포인트를 사용하여 데이터를 병합하지 않고 중복된 불필요한 프로필을 삭제합니다. 고객 프로필을 삭제한 후에는 **해당 정보를 복구할 수 있는 방법이 없습니다**.

{% alert important %}
먼저 올바른 `external_id` 및 해당 커스텀 속성과 이벤트가 포함된 새 고객 프로필을 가져오는 것이 좋습니다. 고객 프로필을 삭제한 후에는 다시 복구할 수 없으므로 삭제는 마지막 단계로 진행해야 합니다.
{% endalert %}
...
### 지난 365일 동안의 커스텀 이벤트 및 구매 행동을 기반으로 정확한 사용자 목록을 타겟팅하려면 어떻게 해야 하나요? {#how-can-i-target-precise-lists-of-users-based-on-their-custom-event-and-purchase-behavior-in-the-past-365-days}
...
#### 세그먼트 확장을 최신 상태로 유지 {#keeping-segment-extensions-up-to-date}

{% apitags %}
Segments
{% endapitags %}

이 확장을 특정 시점의 단일 스냅샷으로 표현할지, 아니면 매일 재생성할지를 지정할 수 있습니다. 초기 저장 후에 항상 확장 처리가 시작됩니다. 확장을 매일 재생성하려면 **매일 확장 재생성**을 선택하면 회사 시간대의 매일 자정 무렵에 재생성 처리가 시작됩니다.
...
### 특정 Campaign 또는 Canvas에 대해 고유 수신자 수와 전송 횟수에 차이가 있는 이유는 무엇인가요? {#why-is-there-a-difference-between-the-number-of-unique-recipients-and-the-number-of-sends-for-a-given-campaign-or-canvas}
...
현지 시간대 전달을 사용하면 사용자의 개별 시간대를 기준으로 Segment에 메시징 캠페인을 전달할 수 있습니다. 현지 시간대 전달을 사용하지 않는 캠페인은 Braze에서 회사의 시간대 설정에 따라 스케줄됩니다.
...
Braze는 모든 현지 시간대 캠페인을 24시간 전에 스케줄할 것을 적극 권장합니다. 이러한 캠페인은 하루 종일 전송해야 하므로 24시간 전에 스케줄하면 전체 Segment에 메시지가 도달할 수 있습니다. 그러나 필요한 경우 24시간 미만 전에도 이러한 캠페인을 스케줄할 수 있습니다. Braze는 전송 시간을 1시간 이상 놓친 사용자에게는 메시지를 보내지 않는다는 점에 유의하세요.
...
24시간 미만 전에 스케줄된 현지 시간대 캠페인을 수정해도 메시지의 스케줄은 변경되지 않습니다. 현지 시간대 캠페인을 편집하여 나중에 전송하기로 결정한 경우(예: 오후 6시 대신 오후 7시), 원래 전송 시간을 선택할 때 타겟 Segment에 있던 사용자는 여전히 원래 시간(오후 6시)에 메시지를 받게 됩니다. 현지 시간대를 편집하여 더 이른 시간(예: 오후 5시 대신 오후 4시)에 전송하는 경우 캠페인은 여전히 원래 시간(오후 5시)에 모든 Segment 멤버에게 전송됩니다.
...
현지 시간대 캠페인의 타겟 Segment에는 모든 시간 기반 필터에 최소 48시간의 기간이 포함되어야 전체 Segment에 전달이 보장됩니다. 예를 들어 다음 필터를 사용하여 둘째 날 사용자를 타겟팅하는 Segment를 생각해 보겠습니다:
...
### "안전 영역" 내에서 수정하면 어떻게 되나요? {#what-if-i-make-an-edit-within-the-safe-zone}
...
### "안전 영역"이 이미 지나간 경우 어떻게 해야 하나요? {#what-should-i-do-if-the-safe-zone-has-already-passed}
...
### Braze는 현지 시간대 전달을 위해 언제 사용자를 평가하나요? {#when-does-braze-evaluate-users-for-local-time-zone-delivery}

{% apitags %}
Campaigns
{% endapitags %}

Braze는 다음 두 가지 시점에서 사용자의 참가 자격을 평가합니다:

- 스케줄된 날의 사모아 시간(UTC+13) 기준
- 스케줄된 날의 사용자 현지 시간 기준

사용자가 참가 자격을 얻으려면 두 가지 검사를 모두 통과해야 합니다. 예를 들어 Canvas가 2021년 8월 7일 오후 2시(현지 시간 기준)에 출시될 예정인 경우 뉴욕에 위치한 사용자를 타겟팅하려면 다음과 같은 자격 요건을 확인해야 합니다:
...
### 하나의 배리언트가 있지만 여러 개의 브랜치가 있는 Canvas의 오디언스와 전송 시간이 동일하면 어떻게 되나요? {#what-happens-if-the-audience-and-send-time-are-identical-for-a-canvas-that-has-one-variant-but-multiple-branches}
...
- 사용자는 Canvas에 입장할 수 없게 됩니다.
- 사용자가 플로우의 어느 위치에 있더라도 더 이상의 메시지는 발송되지 않습니다.
    - **예외:** 이메일 Canvas는 즉시 중지되지 않습니다. 전송 요청이 SendGrid로 전송된 후에는 사용자에게 전달되는 것을 막을 수 있는 방법이 없습니다.

{% alert note %}
Canvas를 중지해도 단계에서 대기 중인 사용자는 종료되지 않습니다. Canvas를 다시 활성화해도 사용자가 여전히 대기 중인 경우 단계를 완료하고 다음 구성요소로 이동합니다. 그러나 사용자가 다음 구성요소로 진행해야 하는 시간이 지나면 대신 Canvas를 종료합니다.
{% endalert %}
...
### Canvas에서 사용자 전환은 어떻게 추적하나요? {#how-are-user-conversions-tracked-in-a-canvas}

{% apitags %}
Canvases
{% endapitags %}

사용자는 Canvas 항목당 한 번만 전환할 수 있습니다.
...
1. 사용자가 Canvas에 들어갑니다.
2. 첫 번째 단계는 지연이 없지만 방해금지 시간 내에 있으므로 메시지가 억제됩니다.
3. 사용자가 전환 이벤트를 수행합니다.

**결과:**
사용자는 전체 캔버스 배리언트에서 전환된 것으로 계산되지만 단계를 받지 않았으므로 단계에서는 계산되지 않습니다.
...
### 고유 사용자 수를 살펴볼 때 Canvas 분석과 세그먼터 중 어느 것이 더 정확할까요? {#when-looking-at-the-number-of-unique-users-is-canvas-analytics-or-the-segmenter-more-accurate}

{% apitags %}
Canvases
{% endapitags %}

세그먼터는 Canvas나 Campaign 통계에 비해 고유 사용자 데이터에 대해 더 정확한 통계를 제공합니다. 이는 Canvas와 Campaign 통계가 어떤 일이 발생하면 Braze가 증분하는 숫자이므로, 이 숫자가 세그먼터의 숫자와 다를 수 있는 변수가 존재하기 때문입니다. 예를 들어, 사용자는 Canvas 또는 Campaign에 대해 두 번 이상 전환할 수 있습니다.

{% endapi %}
{% api %}

### Canvas에 입장하는 사용자 수가 예상한 수와 일치하지 않는 이유는 무엇인가요? {#why-does-the-number-of-users-entering-a-canvas-not-match-the-expected-number}
...
### Braze는 어떤 측정기준을 측정하나요? {#what-metrics-does-braze-measure}
...
### Braze에서 매출은 어떻게 계산되나요? {#how-is-revenue-calculated-in-braze}
...
### Currents는 어떤 보고 기능을 지원하나요? {#what-reporting-capabilities-does-currents-enable}
...
### 반복 참여 보고서를 스케줄하려면 어떻게 해야 하나요? {#how-can-i-schedule-a-recurring-engagement-report}
...
### 참여 보고서와 보고서 빌더의 차이점은 무엇인가요? {#whats-the-difference-between-engagement-reports-and-the-report-builder}
