...
# 변환 만들기 {#create-a-transformation}

> Braze 데이터 변환을 사용하면 웹훅 통합을 구축 및 관리하여 외부 플랫폼에서 Braze로의 데이터 흐름을 자동화할 수 있습니다. 이러한 웹훅 통합을 통해 더욱 정교한 마케팅 사용 사례를 지원할 수 있습니다. 기본 코드에서 데이터 변환을 구축하거나, 특정 외부 플랫폼에서 시작하는 데 도움이 되는 전용 템플릿 라이브러리를 사용하여 구축할 수 있습니다.
...
이 단계는 선택 사항이지만, 소스 플랫폼에서 새로 만든 변환으로 테스트 웹훅을 전송하는 것을 권장합니다.

1. 변환의 URL을 복사합니다.
2. 소스 플랫폼에서 "테스트 보내기" 기능을 찾아 이 URL로 전송할 샘플 웹훅을 생성합니다.
- 소스 플랫폼에서 요청 유형을 묻는 경우 **POST**를 선택합니다.
- 소스 플랫폼에서 인증 옵션을 제공하는 경우 **No authentication**을 선택합니다.
- 소스 플랫폼에서 시크릿을 요청하는 경우 **No secrets**를 선택합니다.
3. Braze 대시보드에서 페이지를 새로고침하여 웹훅이 수신되었는지 확인합니다. 수신된 경우 **Most recent webhook** 아래에 웹훅 페이로드가 표시됩니다.
...
JavaScript 코드에 대한 경험이 거의 없거나 더 자세한 지침을 원하시면 **초급 - POST: Track users** 또는 **초급 - PUT: Update multiple catalog items** 탭을 참고하여 변환 코드를 작성하세요.
...
1. 새 변환에는 **Transformation Code** 섹션에 다음 기본 템플릿이 포함되어 있습니다:
...
{:start="2"}
2. 변환 호출에 커스텀 속성, 커스텀 이벤트 및 구매를 포함하려면 3단계로 건너뛰세요. 그렇지 않으면 필요하지 않은 섹션을 삭제하세요.<br><br>
3. 각 속성, 이벤트 및 구매 오브젝트에는 `external_id`, `user_alias`, `braze_id`, `email` 또는 `phone`과 같은 사용자 식별자가 필요합니다. 수신 웹훅의 페이로드에서 사용자 식별자를 찾은 다음, 페이로드 라인을 통해 변환 코드에서 해당 값을 템플릿화합니다. 점 표기법을 사용하여 페이로드 오브젝트 속성에 액세스합니다.<br><br>
4. 속성, 이벤트 또는 구매로 표현하려는 웹훅 값을 찾아 페이로드 라인을 통해 변환 코드에서 해당 값을 템플릿화합니다. 점 표기법을 사용하여 페이로드 오브젝트 속성에 액세스합니다.<br><br>
5. 각 속성, 이벤트 및 구매 오브젝트에 대해 `_update_existing_only` 값을 검토합니다. 존재하지 않을 수 있는 새 사용자를 생성하려면 `false`로 설정합니다. 기존 프로필만 업데이트하려면 `true`로 그대로 둡니다.<br><br>
6. **Validate**를 클릭하여 코드 출력의 미리보기를 반환하고 허용되는 `/users/track` 요청인지 확인합니다.<br><br>
7. 변환을 활성화합니다. 코드를 활성화하기 전에 추가 도움이 필요하면 Braze 계정 매니저에게 문의하세요.<br><br>
7. 소스 플랫폼에서 웹훅 전송을 시작하도록 합니다. 수신되는 각 웹훅에 대해 변환 코드가 실행되고 사용자 프로필이 업데이트되기 시작합니다.
...
1. 새 변환에는 **Transformation Code** 섹션에 다음 기본 템플릿이 포함됩니다:
...
## 5단계: 변환 모니터링 {#step-5-monitor-your-transformation}

변환을 활성화한 후 기본 **Transformations** 페이지의 분석에서 성과 요약을 확인하세요.

* **Incoming Requests:** 이 변환의 URL에서 수신된 웹훅의 수입니다. 수신 요청이 0이면 소스 플랫폼에서 웹훅을 전송하지 않았거나 연결할 수 없는 것입니다.
* **Deliveries:** 수신 요청을 받은 후 데이터 변환은 변환 코드를 적용하여 선택한 Braze 대상에 전송합니다.

수신 요청의 100%가 전달로 이어지는 것이 좋은 목표입니다. 전달 횟수는 수신 요청 횟수를 초과하지 않습니다.

### 문제 해결 {#troubleshooting}

자세한 모니터링 및 문제 해결을 위해 **Logs** 페이지에서 특정 로그를 참조하세요. 이 페이지에는 워크스페이스 전반의 모든 변환에 대한 마지막 1,000건의 수신 요청이 기록됩니다. 각 로그를 선택하여 수신 요청 본문, 변환 출력 및 변환 대상의 응답 본문을 확인할 수 있습니다.
