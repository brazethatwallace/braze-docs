...
Braze가 Redshift의 데이터에 접근할 수 있도록 보안 그룹을 변경해야 할 수도 있습니다. 아래 섹션의 IP 및 Redshift 클러스터를 쿼리하는 데 사용되는 포트(기본값 5439)에서 인바운드 트래픽을 명시적으로 허용해야 합니다. 인바운드 규칙이 "모두 허용"으로 설정되어 있더라도 이 포트에서 Redshift TCP 연결을 명시적으로 허용해야 합니다. 또한 Braze가 클러스터에 연결할 수 있도록 Redshift 클러스터의 엔드포인트가 공개적으로 접근 가능해야 합니다.
...
GCP에서 Braze가 테이블에서 데이터를 연결하고 읽을 수 있도록 서비스 계정을 생성합니다. 서비스 계정에는 다음 섹션의 권한이 있어야 합니다:
...
3단계의 자격 증명 설정을 위해 이 값을 보관해 두세요.
...
Braze 대시보드에서 연결된 소스를 생성합니다. **데이터 설정** > **클라우드 데이터 수집** > **연결된 소스**로 이동한 다음 **데이터 소스 추가**를 선택하고 **Snowflake**를 선택합니다.

**소스 설정**에서 다음을 입력합니다:
- **자격 증명:** **Account Locator**, **Username**, **Role**
- **구성:** **Warehouse**, **Database**, **Schema**

새 Snowflake 자격 증명을 생성하는 경우 연결을 테스트하기 전에 **자격 증명 저장 및 RSA 키 생성**을 선택합니다.

#### 3.2단계: 동기화 세부 정보 구성 {#step-32-configure-sync-details}

연결된 소스의 이름을 선택합니다. 이 이름은 새로운 CDI 세그먼트 확장을 생성할 때 사용 가능한 소스 목록에 표시됩니다.

이 소스에 대한 최대 실행 시간을 구성합니다. Braze는 최대 실행 시간을 초과하는 모든 쿼리를 자동으로 중단합니다. 허용되는 최대 실행 시간은 60분이며, 더 짧은 실행 시간은 Snowflake 계정에서 발생하는 비용을 줄여줍니다. 이 설정은 동기화 및 이를 사용하는 CDI 세그먼트 확장을 포함하여 이 소스를 통해 실행되는 쿼리에 적용됩니다.
...
Braze 대시보드에서 연결된 소스를 생성합니다. **데이터 설정** > **클라우드 데이터 수집** > **연결된 소스**로 이동한 다음 **데이터 소스 추가**를 선택하고 **Amazon Redshift**를 선택합니다.

**소스 설정**에서 다음을 입력합니다:
- **자격 증명:** **Redshift Host URL**, **Username**, **Password**, **Port**
- **구성:** **Database**, **Schema**

필요한 경우 **SSH 터널로 연결**을 활성화하고 **Tunnel Host**, **Tunnel Port**, **Tunnel Username**을 입력합니다.

#### 3.2단계: 동기화 세부 정보 구성

연결된 소스의 이름을 선택합니다. 이 이름은 새로운 CDI 세그먼트 확장을 생성할 때 사용 가능한 소스 목록에 표시됩니다.

이 소스에 대한 최대 실행 시간을 구성합니다. Braze는 최대 실행 시간을 초과하는 모든 쿼리를 자동으로 중단합니다. 허용되는 최대 실행 시간은 60분이며, 더 짧은 실행 시간은 Redshift 계정에서 발생하는 비용을 줄여줍니다.
...
자격 증명에 **SSH 터널로 연결**이 선택되어 있는 경우, **연결 테스트** 단계에서 RSA 공개 키를 기록해 두세요. Redshift에서 통합을 완료하는 데 필요합니다.

{% endtab %}
{% tab BigQuery %}
#### 3.1단계: BigQuery 연결 정보 및 소스 테이블 추가 {#step-31-add-bigquery-connection-information-and-source-table}

Braze 대시보드에서 연결된 소스를 생성합니다. **데이터 설정** > **클라우드 데이터 수집** > **연결된 소스**로 이동한 다음 **데이터 소스 추가**를 선택하고 **Google BigQuery**를 선택합니다.

**소스 설정**에서 다음을 입력합니다:
- **자격 증명:** **Credential name**을 입력하고 **JSON key**를 업로드합니다.
- **구성:** **Project**, **Dataset**
...
이 소스에 대한 최대 실행 시간을 구성합니다. Braze는 최대 실행 시간을 초과하는 모든 쿼리를 자동으로 중단합니다. 허용되는 최대 실행 시간은 60분이며, 더 짧은 실행 시간은 BigQuery 계정에서 발생하는 비용을 줄여줍니다. 이 설정은 동기화 및 이를 사용하는 CDI 세그먼트 확장을 포함하여 이 소스를 통해 실행되는 쿼리에 적용됩니다.
...
Braze 대시보드에서 연결된 소스를 생성합니다. **데이터 설정** > **클라우드 데이터 수집** > **연결된 소스**로 이동한 다음 **데이터 소스 추가**를 선택하고 **Databricks**를 선택합니다.

**소스 설정**에서 다음을 입력합니다:
- **자격 증명:** **Credential Name**, **Hostname**, **HTTP Path**, **Access Token**
- **구성:** **Catalog**, **Schema**
...
이 소스에 대한 최대 실행 시간을 구성합니다. Braze는 최대 실행 시간을 초과하는 모든 쿼리를 자동으로 중단합니다. 허용되는 최대 실행 시간은 60분이며, 더 짧은 실행 시간은 Databricks 계정에서 발생하는 비용을 줄여줍니다. 이 설정은 동기화 및 이를 사용하는 CDI 세그먼트 확장을 포함하여 이 소스를 통해 실행되는 쿼리에 적용됩니다.
...
Braze 대시보드에서 연결된 소스를 생성합니다. **데이터 설정** > **클라우드 데이터 수집** > **연결된 소스**로 이동한 다음 **데이터 소스 추가**를 선택하고 **Microsoft Fabric**을 선택합니다.

**소스 설정**에서 다음을 입력합니다:
- **자격 증명:** **Credentials Name**, **Tenant ID**, **Principal ID**, **Client Secret**, **Connection String**
- **구성:** **Database**, **Schema**

워크스페이스에서 **SSH 터널로 연결**을 사용할 수 있고 설정에 필요한 경우 **Tunnel Host**, **Tunnel Port**, **Tunnel Username**도 입력합니다.

#### 3.2단계: 동기화 세부 정보 구성

연결된 소스의 이름을 선택합니다. 이 이름은 새로운 CDI 세그먼트 확장을 생성할 때 사용 가능한 소스 목록에 표시됩니다.

이 소스에 대한 최대 실행 시간을 구성합니다. Braze는 최대 실행 시간을 초과하는 모든 쿼리를 자동으로 중단합니다. 허용되는 최대 실행 시간은 60분이며, 더 짧은 실행 시간은 Microsoft Fabric 계정에서 발생하는 비용을 줄여줍니다. 이 설정은 동기화 및 이를 사용하는 CDI 세그먼트 확장을 포함하여 이 소스를 통해 실행되는 쿼리에 적용됩니다.
