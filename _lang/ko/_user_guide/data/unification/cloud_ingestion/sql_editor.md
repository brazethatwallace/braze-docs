...
## 사전 요구 사항 및 제한 사항 {#prerequisites-and-limitations}

SQL 편집기에는 다음과 같은 제한 사항이 있습니다:

- 데이터 웨어하우스 소스에서만 사용할 수 있습니다: Snowflake, Redshift, BigQuery, Databricks, Fabric.
...
## 새로운 SQL 편집기 동기화 만들기 {#create-a-new-sql-editor-sync}

다음 단계에 따라 먼저 소스를 만든 다음 SQL 편집기로 동기화를 만드세요. CDI 소스를 이미 설정한 경우 3단계로 건너뛸 수 있습니다.

{% alert note %}
이 단계에서는 Snowflake 소스를 예시로 사용합니다. 다른 데이터 웨어하우스 소스의 설정 프로세스도 유사하며, [데이터 웨어하우스 통합 설정]({{site.baseurl}}/user_guide/data/unification/cloud_ingestion/integrations#setting-up-data-warehouse-integrations) 설명서의 [2단계: Braze 대시보드에서 새 소스 만들기]({{site.baseurl}}/user_guide/data/unification/cloud_ingestion/integrations#step-2-create-a-new-source-in-the-braze-dashboard)에서 확인할 수 있습니다.
{% endalert %}

### 1단계: Snowflake 역할, 권한, 웨어하우스 및 사용자 설정 {#step-1-set-up-your-snowflake-role-permissions-warehouse-and-user}

CDI에서 Snowflake 소스를 만들기 전에, Braze가 사용하는 Snowflake 사용자가 쿼리하려는 데이터에 접근할 수 있고 쿼리를 실행할 웨어하우스가 있는지 확인하세요.

#### 1.1단계: (선택 사항) 데이터베이스 및 스키마 만들기 {#step-11-optional-create-a-database-and-schema}

필요한 경우 CDI 데이터 전용 데이터베이스와 스키마를 만드세요:
...
사용 사례에 따라 여러 테이블이나 향후 테이블에 대한 접근 권한을 부여할 수도 있습니다. 예를 들어, 스키마의 모든 향후 테이블에 대한 접근 권한을 부여하려면 다음과 같이 합니다:

```sql
GRANT SELECT ON FUTURE TABLES IN SCHEMA BRAZE_CLOUD_PRODUCTION.INGESTION TO ROLE BRAZE_INGESTION_ROLE;
```

#### 1.3단계: 웨어하우스 설정 및 Braze 역할에 접근 권한 부여 {#step-13-set-up-the-warehouse-and-grant-access-to-the-braze-role}

Braze가 쿼리를 실행할 웨어하우스를 만드세요:
...
#### 1.4단계: Snowflake 사용자 만들기 {#step-14-create-a-snowflake-user}

Braze용 사용자를 만들고 역할을 할당하세요:
...
이 사용자는 Braze에서 Snowflake 소스를 구성할 때 사용합니다.

### 2단계: Braze 대시보드에서 새 소스 만들기 {#step-2-create-a-new-source-in-the-braze-dashboard}

이 단계에서는 Braze에서 Snowflake 소스를 만들고 연결을 검증합니다.
...
소스 이름을 선택하고 Snowflake 자격 증명 및 구성을 입력합니다.

{% alert note %}
**Snowflake 계정 로케이터** 필드에는 Snowflake [계정 식별자](https://docs.snowflake.com/en/user-guide/admin-account-identifier)를 입력하세요. 일반적으로 `xy12345.us-east-1.aws`와 같은 형식입니다. 이것은 데이터베이스 이름이나 웨어하우스 이름과 다릅니다.
{% endalert %}

#### 2.3단계: RSA 키 설정 완료 {#step-23-complete-rsa-key-setup}

자격 증명과 구성을 입력한 후 **자격 증명 저장**을 선택하고 RSA 키를 생성합니다. 그런 다음 Snowflake로 돌아가 설정을 완료합니다. 대시보드에 표시된 공개 키를 Braze가 Snowflake에 연결하기 위해 만든 사용자에게 추가합니다.

자세한 내용은 [Snowflake 키 페어 인증](https://docs.snowflake.com/en/user-guide/key-pair-auth)을 참조하세요. 키를 교체하려면 Braze에서 새 키 페어를 생성하고 새 공개 키를 제공할 수 있습니다.

```sql
ALTER USER BRAZE_INGESTION_USER SET RSA_PUBLIC_KEY='MIIBIjANBgkqhkiG9w0BA...';
```

Braze로 돌아가서 **연결 테스트**를 선택하여 소스 접근을 확인한 다음 소스를 만듭니다.

### 3단계: 새 동기화 만들기 및 SQL 쿼리 작성 {#step-3-create-a-new-sync-and-write-your-sql-query}

1. **데이터 설정** > **클라우드 데이터 수집** > **동기화**로 이동합니다.
2. **데이터 동기화 만들기**를 선택합니다.
3. **데이터 유형**에서 동기화를 선택합니다.
4. 2단계에서 만든 소스를 참조합니다.
5. **SQL**을 선택하고 웨어하우스에서 사용자 데이터를 반환하는 SQL 쿼리를 작성합니다. SQL 쿼리는 Braze에 동기화할 데이터를 정의합니다. 쿼리 결과가 동기화의 스키마가 됩니다.
...
검증을 성공적으로 완료하려면 SQL 쿼리가 다양한 필수 열을 반환해야 합니다:

| 동기화 데이터 유형 | 필수 열 |
|---|---|
| 속성 | - 사용자 식별자: `external_id`, `braze_id`, `alias_name`과 `alias_label`, 이메일 또는 전화번호 중 하나.<br>- `UPDATED_AT`.<br>- 동기화할 추가 열(속성) 최소 1개. |
| 사용자 삭제 | - 사용자 식별자: `external_id`, `braze_id`, `alias_name`과 `alias_label`, 이메일 또는 전화번호 중 하나.<br>- `UPDATED_AT`. |
| Canvas 트리거 | - 사용자 식별자: `external_id`, `braze_id`, `alias_name`과 `alias_label`, 이메일 또는 전화번호 중 하나.<br>- `UPDATED_AT`. |
| 커스텀 이벤트 | - 사용자 식별자: `external_id`, `braze_id`, `alias_name`과 `alias_label`, 이메일 또는 전화번호 중 하나.<br>- `UPDATED_AT`.<br>- 이벤트 이름을 나타내는 `NAME`.<br>- 이벤트 시간을 나타내는 `TIME`. 사용할 수 없는 경우 CDI는 `UPDATED_AT`를 대체로 사용합니다. |
| 구매 이벤트 | - 사용자 식별자: `external_id`, `braze_id`, `alias_name`과 `alias_label`, 이메일 또는 전화번호 중 하나.<br>- `UPDATED_AT`.<br>- `PRODUCT_ID`.<br>- `CURRENCY`.<br>- `PRICE`.<br>- 구매 이벤트 시간을 나타내는 `TIME`. 사용할 수 없는 경우 CDI는 `UPDATED_AT`를 대체로 사용합니다. |
| 카탈로그 | - 카탈로그 항목 식별자를 나타내는 `ID`.<br>- `UPDATED_AT`.<br>- 동기화할 추가 열(카탈로그 필드) 최소 1개. |
| 계정 | - 계정 식별자를 나타내는 `ID`.<br>- 계정 이름을 나타내는 `NAME`.<br>- `UPDATED_AT`.<br>- 동기화할 추가 열(계정 필드) 최소 1개. |
{: .reset-td-br-1 .reset-td-br-2 aria-label="4단계: 쿼리 미리보기 및 검증" }

필수 열 외의 추가 열은 각각 속성, Canvas 컨텍스트 속성정보, 이벤트 속성정보, 카탈로그 필드 및 계정 필드로 동기화됩니다. 미리보기 및 검증 오류와 해결 방법에 대한 유용한 팁은 [검증 동작](#validation-behavior) 및 [문제 해결](#troubleshooting)을 참조하세요.

### 5단계: 속성 매핑 검토 및 동기화 만들기 {#step-5-review-attribute-mapping-and-create-sync}

검증이 성공하면 **다음: 알림**으로 계속 진행하여 동기화를 만듭니다.

{% alert important %}
부정확한 SQL 구성은 데이터 포인트 과다 소비 및 광범위한 운영 리스크를 포함한 의도하지 않은 결과를 초래할 수 있습니다. 쿼리 로직이 올바른지 확인하는 것은 사용자의 책임이며, 동기화를 활성화하기 전에 모든 결과를 신중하게 미리보기해야 합니다.
{% endalert %}
...
동기화 실행이 이미 진행 중인 경우, 변경 사항은 다음 실행 시 적용됩니다.
