...
{% alert note %}
`UPDATED_AT`는 데이터 웨어하우스 통합에만 해당되며, S3 동기화에는 해당되지 않습니다.
{% endalert %}

동기화가 실행되면 Braze는 데이터 웨어하우스 인스턴스에 직접 연결하여 지정된 테이블에서 모든 새 데이터를 가져오고 Braze 대시보드의 해당 데이터를 업데이트합니다. 동기화가 실행될 때마다 Braze는 업데이트된 데이터를 반영합니다.

{% alert important %}
Braze CDI는 행 내용이 현재 Braze에 있는 것과 동일한지 여부에 관계없이 `UPDATED_AT` 값을 기준으로 행을 동기화합니다. 따라서 불필요한 데이터 포인트 사용량을 방지하기 위해 `UPDATED_AT`를 적절히 사용하여 새 데이터 또는 업데이트된 데이터만 동기화하는 것을 권장합니다.
{% endalert %}

### 예시: 반복 동기화 {#example-recurring-sync}

CDI 동기화에서 `UPDATED_AT`가 어떻게 사용되는지 설명하기 위해, 사용자 속성을 업데이트하는 다음 예시 반복 동기화를 살펴보겠습니다.
...
Cloud Data Ingestion은 다음 데이터 유형을 지원합니다:
- 사용자 속성(다음 항목 포함):
   - 중첩 커스텀 속성
   - 오브젝트 배열
   - 가입 상태
...
### 데이터 유형 문제 방지하기 {#avoiding-data-type-issues}

CDI를 사용하여 외부 소스(예: Databricks 또는 Snowflake)에서 데이터를 동기화할 때, 동기화 전에 소스 열이 올바른 데이터 유형을 사용하는지 확인하세요. 일반적인 문제는 다음과 같습니다:

- **문자열로 저장된 타임스탬프:** 소스 데이터베이스에서 날짜 열이 varchar 또는 문자열이 아닌 타임스탬프 또는 datetime 유형을 사용하는지 확인하세요.
- **문자열로 저장된 숫자:** 동기화 전에 소스 쿼리에서 숫자 열을 정수 또는 플로트 유형으로 캐스팅하세요.
- **동기화 간 일관되지 않은 유형:** 동기화 사이에 열 유형이 변경되면 Braze에서 새 데이터를 거부할 수 있습니다. 소스 스키마가 일관되게 유지되는지 확인하세요.

Braze 대시보드에서 커스텀 속성의 데이터 유형을 강제 적용하거나 변경하는 방법은 [커스텀 데이터 관리]({{site.baseurl}}/user_guide/data/activation/custom_data/managing_custom_data#forcing-data-type-comparisons)를 참조하세요.

외부 ID, 사용자 별칭, Braze ID, 이메일 또는 전화번호로 사용자 데이터를 업데이트할 수 있습니다. 외부 ID, 사용자 별칭 또는 Braze ID로 사용자를 삭제할 수 있습니다.

## 동기화되는 내용 {#what-gets-synced}

동기화가 실행될 때마다, Braze는 이전에 동기화되지 않은 행을 찾습니다. 이를 위해 테이블 또는 뷰의 `UPDATED_AT` 열을 확인합니다. Braze는 `UPDATED_AT`이 마지막으로 동기화된 `UPDATED_AT` 값보다 이후인 모든 행을 선택하여 가져옵니다. 실행 사이에 동일한 타임스탬프로 새 행이 추가된 경우, 정확한 경계 타임스탬프에 해당하는 행도 다시 동기화될 수 있습니다.
...
데이터 웨어하우스에서 다음 사용자와 속성을 테이블에 추가하고, `UPDATED_AT` 시간을 데이터를 추가하는 시간으로 설정합니다:
...
다음 예약된 동기화 시, Braze는 가장 최근에 동기화된 타임스탬프보다 이후의 `UPDATED_AT` 타임스탬프를 가진 모든 행을 동기화합니다. Braze는 필드를 업데이트하거나 추가하므로 매번 전체 고객 프로필을 동기화할 필요가 없습니다. 동기화 후 고객 프로필에 새 업데이트가 반영됩니다:
...
`customer_9012`에 대한 새 행이 추가되었지만, `UPDATED_AT` 값(`2022-07-16 00:25:30`)이 저장된 타임스탬프(`2022-07-19 09:07:23`)보다 이전이므로 동기화되지 않습니다. 그러나 기존 `customer_5678` 행은 `UPDATED_AT` 값이 저장된 타임스탬프와 같으므로 포함 경계로 인해 다시 동기화됩니다. 이 동작에 대한 자세한 내용은 [UPDATED_AT 시간이 동기화 시간과 같지 않도록 확인하기](#make-sure-the-updated_at-time-isnt-the-same-time-as-your-sync)를 참조하세요. 저장된 `UPDATED_AT`는 `2022-07-19 09:07:23`으로 유지됩니다.
...
이 세 번째 실행에서 `customer_1234`에 대한 새 행이 추가되었으며, `UPDATED_AT` 값(`2022-07-21 08:30:00`)이 저장된 타임스탬프보다 이후입니다. 이 새 행과 기존 `customer_5678` 행(저장된 타임스탬프와 동일한 `UPDATED_AT`를 가짐)이 모두 동기화됩니다. 저장된 `UPDATED_AT`는 이제 `2022-07-21 08:30:00`으로 설정됩니다.

{% alert note %}
`UPDATED_AT` 값은 특정 동기화의 실행 시작 시간보다 이후일 수도 있습니다. 그러나 마지막 `UPDATED_AT` 타임스탬프를 "미래"로 밀어내어 이후 동기화에서 더 이전 값을 동기화하지 않게 되므로 권장하지 않습니다.
{% endalert %}

## `UPDATED_AT` 열에 UTC 타임스탬프 사용하기 {#use-a-utc-timestamp-for-the-updated_at-column}

`UPDATED_AT` 열은 일광 절약 시간제 관련 문제를 방지하기 위해 UTC로 설정해야 합니다. 가능하면 `CURRENT_DATE()` 대신 `SYSDATE()`와 같은 UTC 전용 함수를 사용하세요.
...
### 새로 추가되거나 업데이트된 속성만 작성하여 소비 최소화하기 {#only-write-new-or-updated-attributes-to-minimize-consumption}

동기화가 실행될 때마다 Braze는 이전에 동기화되지 않은 행을 찾습니다. 이는 테이블 또는 뷰의 `UPDATED_AT` 열을 사용하여 확인합니다. Braze는 `UPDATED_AT` 값이 마지막으로 동기화된 `UPDATED_AT` 값보다 늦은 모든 행을 선택하여 가져오며, 현재 고객 프로필에 있는 것과 동일한지 여부는 관계없습니다. 경계 타임스탬프의 행은 새 행이 해당 타임스탬프를 공유하는 경우 다시 동기화될 수도 있습니다. 이러한 이유로, 추가하거나 업데이트하려는 속성만 동기화할 것을 권장합니다.

데이터 포인트 사용량은 CDI를 사용할 때나 REST API 또는 SDK와 같은 다른 수집 방법을 사용할 때 동일하므로, 소스 테이블에 새로 추가되거나 업데이트된 속성만 넣도록 관리하는 것이 중요합니다.

### `EXTERNAL_ID`와 `PAYLOAD` 열 분리하기 {#separate-external_id-from-payload-column}

`PAYLOAD` 객체에는 외부 ID 또는 기타 ID 유형이 포함되어서는 안 됩니다.

### 속성 제거하기 {#remove-an-attribute}

사용자 프로필에서 속성을 생략하려면 `null`로 설정할 수 있습니다. 속성을 변경하지 않으려면 업데이트될 때까지 Braze에 보내지 마세요. 속성을 완전히 제거하려면 `TO_JSON(OBJECT_CONSTRUCT_KEEP_NULL(...))`을 사용하세요.

### 증분 업데이트 수행하기 {#make-incremental-updates}

동시 업데이트가 이루어질 때 의도하지 않은 덮어쓰기를 방지하기 위해 데이터에 증분 업데이트를 수행하세요.

{% alert important %}
* **서로 다른 속성에 대한 업데이트:** 대부분의 경우 두 업데이트가 사용자의 동일한 속성에 영향을 미치지 않으면 완전히 독립적인 결과를 가집니다. 예를 들어 사용자의 `Color` 속성을 업데이트하고 별도로 `Size` 속성을 업데이트하면, 서로 몇 초 이내에 발생하더라도 두 업데이트 모두 올바르게 적용됩니다.
* **동일한 속성에 대한 업데이트:** 단일 동기화 실행 내에서 여러 업데이트가 동일한 속성을 대상으로 하면 경합 조건이 발생할 수 있습니다. 이러한 드문 경우에는 하나의 업데이트가 다른 업데이트를 덮어쓸 수 있습니다. 이를 방지하는 가장 좋은 방법은 CDI 동기화의 소스 데이터가 각 사용자의 최신 상태만 반영하도록 하거나, 특정 사용자 또는 사용자+속성 쌍에 대한 모든 업데이트가 단일 행에 포함되도록 하는 것입니다.
* **오브젝트 배열 연산자:** 독립적 업데이트의 유일한 예외는 오브젝트 배열에 대한 `$add`, `$remove`, `$update` 연산자로, 동일한 배열에 대한 업데이트가 서로 상호 작용할 수 있습니다.
* **이벤트:** 경합 조건은 이벤트에 영향을 미치지 않습니다. 각 이벤트는 고유하며 타임스탬프가 연결되어 있기 때문입니다.
{% endalert %}

이를 방지하는 가장 좋은 방법은 CDI 동기화의 소스 데이터가 각 사용자의 최신 상태만 반영하도록 하거나, 특정 사용자 또는 사용자+속성 쌍에 대한 모든 업데이트가 단일 행에 포함되도록 하는 것입니다.

### 다른 테이블에서 JSON 문자열 생성하기 {#create-a-json-string-from-another-table}

각 속성을 내부적으로 자체 열에 저장하려는 경우, 해당 열을 JSON 문자열로 변환하여 Braze와의 동기화를 채워야 합니다. 이를 위해 다음과 같은 쿼리를 사용할 수 있습니다:
...
Braze는 `UPDATED_AT` 타임스탬프를 사용하여 성공적으로 동기화된 데이터를 추적합니다. CDI는 또한 마지막으로 동기화된 타임스탬프의 행 수를 추적합니다. 실행 사이에 동일한 타임스탬프로 새 행이 추가되면, CDI는 해당 타임스탬프의 모든 행을 다시 동기화하여 중복 데이터가 발생할 수 있습니다. 자세한 내용과 팁은 [중복 타임스탬프로 행 재동기화 방지하기](#avoid-resyncing-rows-with-duplicate-timestamps)를 참조하세요.

### 테이블 구성 {#table-configuration}

고객이 모범 사례나 코드 스니펫을 공유할 수 있는 공개 [GitHub 리포지토리](https://github.com/braze-inc/braze-examples/tree/main/cloud-data-ingestion)가 있습니다. 자신의 스니펫을 기여하려면 풀 리퀘스트를 생성하세요!

### 데이터 포맷 {#data-formatting}

Cloud Data Ingestion 테이블 설정 요구 사항 및 페이로드 포맷 요구 사항은 [Cloud Data Ingestion 테이블 설정]({{site.baseurl}}/user_guide/data/unification/cloud_ingestion/table_setup)에 문서화되어 있습니다.

해당 페이지를 사용하여 다음을 구분하세요:

- 소스 테이블 요구 사항 (필수 열, 식별자 열 및 `UPDATED_AT` 동작)
- 페이로드 요구 사항 (각 데이터 유형에 대해 `/users/track` 객체 포맷과 일치해야 하는 필드)

### 데이터 웨어하우스 쿼리 시간 초과 방지하기 {#avoid-timeouts-for-data-warehouse-queries}

최적의 성능과 잠재적 오류를 방지하기 위해 쿼리가 1시간 이내에 완료되는 것을 권장합니다. 쿼리가 이 시간을 초과하면 데이터 웨어하우스 구성을 검토해 보세요. 웨어하우스에 할당된 리소스를 최적화하면 쿼리 실행 속도를 개선하는 데 도움이 될 수 있습니다.
...
| 행 수         | 기본적으로 각 실행은 최대 5억 개의 행을 동기화할 수 있습니다. Braze는 5억 개 이상의 새 행이 포함된 동기화를 중지합니다. 이보다 높은 한도가 필요한 경우 Braze 고객 성공 매니저 또는 Braze 지원팀에 문의하세요. |
| 행당 속성     | 각 행에는 단일 사용자 ID와 최대 250개의 속성이 포함된 JSON 오브젝트가 포함되어야 합니다. JSON 오브젝트의 각 키는 하나의 속성으로 계산됩니다(즉, 배열도 하나의 속성으로 계산됩니다). |
| 페이로드 크기           | 각 행에는 최대 1MB의 페이로드가 포함될 수 있습니다. Braze는 1&nbsp;MB를 초과하는 페이로드를 거부하고, 동기화 로그에 관련 외부 ID 및 잘린 페이로드와 함께 "Payload was greater than 1MB" 오류를 기록합니다. |
...
| 소스 리전       | Braze는 어떤 리전이나 클라우드 공급자에서든 데이터 웨어하우스 또는 클라우드 환경에 연결합니다.                                                                                        |
