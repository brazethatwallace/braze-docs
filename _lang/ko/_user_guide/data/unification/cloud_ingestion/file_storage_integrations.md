...
SQS 대기줄은 전역적으로 고유해야 합니다(예: CDI 동기화에는 하나만 사용할 수 있고 다른 워크스페이스에서 재사용할 수 없습니다).
...
2. 소스 이름을 선택하고 AWS 설정 프로세스에서 얻은 정보를 입력하여 새 소스를 생성합니다. 다음을 지정합니다:
...
Google Cloud 콘솔에서 **Pub/Sub** > **Topics** > **Create topic**으로 이동합니다. Google이 기본 구독을 생성하도록 하거나 별도로 생성할 수 있습니다. 그런 다음 해당 주제에 **풀** 구독을 생성합니다.
...
{% alert warning %}
이 구독에 데드 레터 대기줄을 구성하지 마세요. Braze는 클라우드 데이터 수집 구독에 대한 데드 레터 대기줄을 지원하지 않습니다. 자세한 내용은 Google Cloud 설명서의 [데드 레터 주제](https://cloud.google.com/pubsub/docs/dead-letter-topics)를 참조하세요.
{% endalert %}
...
2. 해당 폴더에 대한 전용 주제와 접두사 범위 알림을 생성한 다음 해당 주제에 구독을 생성합니다:
...
카탈로그 동기화의 경우, 소스 파일에는 다음 열이 포함되어야 합니다. 카탈로그 파일은 사용자 데이터 파일과 다른 식별자를 사용합니다.
