...
지정된 `external_id`가 없는 사용자를 [익명 사용자]({{site.baseurl}}/user_guide/data/unification/user_data/user_profile_lifecycle/anonymous_users/)라고 합니다. 예를 들어 웹사이트를 방문했지만 가입하지 않은 사용자 또는 모바일 앱을 다운로드했지만 프로필을 만들지 않은 사용자 등이 이에 해당할 수 있습니다.
...
- 세그먼터 내에서 "테스트" [필터]({{site.baseurl}}/user_guide/audience/segments/segmentation_filters/)를 사용하여 개별 사용자를 검색하고, [**사용자 검색**]({{site.baseurl}}/user_guide/audience/manage_audience/user_profiles/) 페이지에서 검색합니다.
...
### 익명 사용자를 식별하면 어떻게 되나요 {#what-happens-when-you-identify-anonymous-users}
...
### 익명 사용자 프로필에서 별칭의 동작 {#behavior-of-aliases-on-anonymous-user-profiles}
...
### 알려진 사용자 프로필에 별칭 설정하기 {#setting-aliases-on-known-user-profiles}
...
{% alert tip %}
고객의 고객 프로필 수명주기가 어떻게 적용될지 파악하기 어려우신가요? 사용자 데이터 수집 모범 사례를 보려면 [모범 사례]({{site.baseurl}}/user_guide/data/unification/user_data/best_practices/)를 방문하세요.
{% endalert %}
...
### 사용자 ID 관련 문제 해결 {#troubleshooting-with-user-ids}

모든 사용자 ID는 대시보드 내에서 테스트할 사용자를 찾고 식별하는 데 사용할 수 있습니다. Braze 대시보드에서 사용자를 찾으려면 [테스트 사용자 추가]({{site.baseurl}}/user_guide/administer/global/user_management/internal_groups/#adding-test-users)를 참조하세요.
