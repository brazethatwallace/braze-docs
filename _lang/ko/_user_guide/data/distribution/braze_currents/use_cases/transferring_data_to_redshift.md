...
> [Amazon Redshift](https://aws.amazon.com/redshift/)는 Amazon S3와 함께 Amazon Web Services에서 실행되는 인기 있는 데이터 웨어하우스입니다. Currents의 Braze 데이터는 Redshift로 직접 전송할 수 있도록 구조화되어 있습니다.
...
Redshift 클러스터가 VPC에 있는 경우, S3 로더를 실행하는 서버에서 연결을 허용하도록 VPC를 구성해야 합니다. Redshift 클러스터로 들어가서 로더가 연결할 VPC 보안 그룹 항목을 선택합니다. 그런 다음 새 인바운드 규칙을 추가합니다: **Type** = Redshift, **Protocol** = TCP, **Port** = 클러스터의 포트, **Source** = 로더를 실행하는 서버의 IP(또는 테스트용으로 "Anywhere").

### S3 전체 액세스 권한이 있는 Identity and Access Management(IAM) 사용자

S3 로더는 Currents 데이터가 포함된 파일에 대한 읽기 권한과 Redshift `COPY` 명령을 위해 생성하는 매니페스트 파일 위치에 대한 전체 액세스 권한이 필요합니다. [IAM 콘솔](https://console.aws.amazon.com/iam/home#/users)에서 `AmazonS3FullAccess` 권한을 가진 새 Identity and Access Management(IAM) 사용자를 생성하십시오. 자격 증명을 저장하십시오. 로더에 전달해야 합니다.

환경 변수, 공유 자격 증명 파일(`~/.aws/credentials`) 또는 [AWS 구성 파일](http://boto3.readthedocs.io/en/latest/guide/configuration.html#configuring-credentials)을 통해 액세스 자격 증명을 로더에 전달할 수 있습니다. 또는 `S3LoadJob` 오브젝트 내의 `aws_access_key_id` 및 `aws_secret_access_key` 필드에 직접 할당하여 로더에 포함할 수도 있지만, 소스 코드에 자격 증명을 하드코딩하는 것은 권장하지 않습니다.

## 사용법

### 샘플 사용법

다음 샘플 프로그램은 S3에서 `users.messages.contentcard.Impression` 이벤트의 데이터를 Redshift의 `content_card_impression` 테이블로 로드합니다.
...
로더를 실행하려면 먼저 Redshift 클러스터의 `host`, `port`, `database`와 `COPY` 쿼리를 실행할 수 있는 Redshift 사용자의 `user` 및 `password`를 제공해야 합니다. 또한 이전 섹션에서 생성한 S3 읽기 권한이 있는 Redshift 역할의 ARN을 제공해야 합니다.
...
이벤트 파일의 S3 버킷과 접두사, 그리고 `COPY`할 Redshift 테이블 이름을 제공해야 합니다.

또한 로더에서 요구하는 "auto" 옵션으로 Avro 파일을 `COPY`하려면, Redshift 테이블의 열 정의가 샘플 프로그램에 표시된 것처럼 Avro 스키마의 필드 이름과 일치해야 하며, 적절한 타입 매핑(예: `string`에서 `text`, `int`에서 `integer`)이 필요합니다.

모든 파일을 한 번에 복사하는 데 시간이 너무 오래 걸리는 경우 로더에 `batch_size` 옵션을 전달할 수도 있습니다. `batch_size`를 전달하면 로더가 모든 것을 동시에 복사할 필요 없이 한 번에 하나의 배치씩 점진적으로 복사하고 커밋할 수 있습니다. 하나의 배치를 로드하는 데 걸리는 시간은 `batch_size`뿐만 아니라 파일 크기와 Redshift 클러스터 크기에 따라 달라집니다.
