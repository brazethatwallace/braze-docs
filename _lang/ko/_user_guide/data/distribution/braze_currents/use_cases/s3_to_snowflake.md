...
# Amazon S3에서 Snowflake로 데이터 전송하기 {#transfer-data-from-amazon-s3-to-snowflake}

> 데이터가 현재 Amazon S3에 있는 경우, 추출, 로드, 변환(ELT) 프로세스를 사용하여 Snowflake 또는 다른 관계형 데이터 웨어하우스로 데이터를 전송할 수 있습니다. 이 페이지에서는 그 방법을 설명합니다.

{% alert note %}
보다 구체적인 사용 사례가 있고 Braze가 Currents 인스턴스를 서비스하기를 원하는 경우, Braze 계정 매니저에게 연락하여 Braze 데이터 전문가 서비스에 대해 문의하세요.
{% endalert %}

## 작동 방식 {#how-it-works}

추출, 로드, 변환(ELT) 프로세스는 데이터를 [Snowflake](https://www.snowflake.com/)로 이동하는 자동화된 프로세스이며, 이를 통해 [Braze Looker 블록](https://marketplace.looker.com/marketplace/directory)을 사용하여 Looker에서 해당 데이터를 시각화하여 Campaign(캠페인), Canvas 및 세그먼트에 대한 인사이트와 피드백을 도출할 수 있습니다.

Currents to S3 내보내기를 설정하고 라이브 이벤트 데이터를 수신한 후, 다음 구성요소를 구성하여 Snowflake에서 라이브 ELT 파이프라인을 구성할 수 있습니다:
...
### 1단계: 외부 S3 스테이지 구성 {#step-1-configure-the-external-s3-stage}

{% alert note %}
데이터베이스의 테이블은 이 단계에서 생성됩니다.
{% endalert %}

1. Braze에서 Currents를 설정할 때, Currents 파일이 S3 버킷으로 따라갈 폴더 경로를 지정하세요. 여기서는 기본 폴더 경로인 `currents`를 사용합니다.
...
### 2단계: 버킷 이벤트 생성 {#step-2-create-bucket-events}
