...
아니요. Currents에는 Campaign 또는 Canvas 태그나 메시지 수준의 키-값 페어가 포함되지 않습니다. 태그 데이터를 가져오려면 [내보내기 REST API]({{site.baseurl}}/api/endpoints/export/)를 사용하세요. 또 다른 해결 방법으로, Campaign에서 웹훅 채널을 사용하여 [Liquid]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/liquid/)로 값을 템플릿화하여 태그 또는 키-값 페어 데이터를 자체 엔드포인트로 전송할 수 있습니다.
...
## 커런츠 통합을 편집할 때 "고객 행동 이벤트 자격이 남아 있지 않습니다"라는 메시지가 표시되는 이유는 무엇인가요? {#why-do-i-see-you-do-not-have-any-remaining-customer-behavior-events-entitlements-when-editing-my-currents-integration}
...
Currents 이벤트의 `time` 필드는 32비트 정수로 저장되므로 초 단위 정밀도로 제한됩니다. 일부 이벤트에는 별도의 64비트 밀리초 정밀도 타임스탬프 필드도 포함되어 있습니다. 각 이벤트 유형에서 사용 가능한 필드는 [이벤트 용어집]({{site.baseurl}}/user_guide/data/distribution/braze_currents/event_glossary/)을 확인하세요.
...
## 참여 보고서가 S3로 전송되면 어떻게 되나요? {#what-happens-when-engagement-reports-are-sent-to-s3}
...
## 익명 사용자 데이터를 Braze 커런츠를 통해 Amplitude로 전송할 수 있나요? {#can-anonymous-user-data-be-sent-to-amplitude-through-braze-currents}

`device_id`로 식별되는 익명 사용자 데이터는 Currents를 통해 Amplitude로 전송할 수 있습니다. 이 기능을 사용하려면 Braze 계정 팀에서 기능을 활성화해야 합니다.

## Content Cards 및 인앱 메시지의 대조군 노출은 Currents에서 어떻게 기록되나요? {#how-are-control-group-impressions-for-content-cards-and-in-app-messages-logged-in-currents}
...
존재하지 않는 사용자를 타겟팅하면 API는 `200` 응답을 반환하지만, "Unknown external ID"라는 결과와 함께 발송이 취소됩니다. 해당 발송에 대한 Currents 이벤트는 생성되지 않습니다. `send_to_existing_only` 파라미터는 기본값이 `true`이므로, 명시적으로 `false`로 설정하지 않는 한 알 수 없는 사용자에 대한 발송은 자동으로 건너뜁니다.