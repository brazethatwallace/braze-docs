---
page_order: 2
nav_title: 카탈로그 트리거
article_title: 카탈로그 트리거
layout: dev_guide
noindex: true
guide_top_header: "카탈로그 트리거"
guide_top_text: "카탈로그 트리거는 카탈로그 데이터 변경에 따라 자동으로 발송되는 알림입니다. 수동으로 Campaign을 설정하지 않고도 고객이 관심 있는 항목이 다시 입고되거나 가격이 하락할 때 알림을 보낼 수 있습니다."

page_type: landing
description: "재입고 알림 및 가격 하락 알림과 같이 카탈로그 데이터 변경에 의해 트리거되는 자동 알림을 설정하세요."

guide_featured_title: "섹션 문서"
guide_featured_list:
- name: 재입고 알림
  link: /docs/user_guide/data/activation/catalogs/catalog_triggers/back_in_stock_notifications
  image: /assets/img/braze_icons/shopping-cart-03.svg
- name: 가격 하락 알림
  link: /docs/user_guide/data/activation/catalogs/catalog_triggers/price_drop_notifications
  image: /assets/img/braze_icons/sale-03.svg
---