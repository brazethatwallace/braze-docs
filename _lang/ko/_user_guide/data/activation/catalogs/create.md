...
| CSV 요구 사항 | 세부 정보 |
|-----------------|---------|
| 헤더 | CSV 파일의 첫 번째 열은 `id`로 명명해야 하며, 각 행은 고유한 `id` 값을 가져야 합니다. |
| 열 | CSV 파일은 최대 1,000개의 필드(열)를 가질 수 있으며, 각 열 이름은 최대 250자까지 가능합니다. |
| 파일 크기 | 무료 요금제의 경우, 회사 전체의 모든 CSV 파일의 총 크기는 500MB로 제한됩니다. 프로 요금제의 경우, 단일 CSV 파일의 최대 파일 크기는 2GB입니다. |
...
{% alert tip %}
카탈로그 이름에 [템플릿을 사용](#template-catalog-names)하여 언어 또는 Campaign과 같은 변수를 기반으로 카탈로그 이름을 동적으로 생성할 수도 있습니다.
{% endalert %}
...
다음으로 이 카탈로그의 이름을 "games_catalog"로 지정하고 **카탈로그 처리** 버튼을 선택합니다. 그러면 Braze가 카탈로그를 생성하기 전에 오류가 있는지 확인합니다.
...
### 2단계: 카탈로그 생성 {#step-2-create-your-catalog}
