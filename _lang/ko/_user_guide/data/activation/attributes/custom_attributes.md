...
page_order: 1
page_type: reference
description: "이 페이지에서는 커스텀 속성에 대해 설명하고 다양한 커스텀 속성 데이터 유형을 안내합니다."
search_rank: 1
---
...
Braze에 저장된 커스텀 속성은 오디언스 세그먼트를 구축하고 Liquid를 사용하여 메시징을 개인화하는 데 활용할 수 있습니다. Braze는 커스텀 속성에 대한 시계열 정보를 저장하지 않으므로, 커스텀 이벤트처럼 그래프를 기반으로 한 분석은 할 수 없다는 점을 유의하세요.
...
- 사용자가 기능을 볼 때마다 `feature_views_count` 속성을 증가시키는 것처럼 [숫자 속성]({{site.baseurl}}/user_guide/data/activation/custom_data/data_types/#numbers)으로 가치가 낮은 행동 횟수 기록
- `last_support_ticket_at` 또는 `last_password_reset_at`처럼 [시간 속성]({{site.baseurl}}/user_guide/data/activation/custom_data/data_types/#time)을 사용하여 가치가 낮은 행동이 마지막으로 발생한 시점 기록
...
![부울 유형의 커스텀 속성 4개.]({% image_buster /assets/img/export_custom_attributes.png %})

**마지막 업데이트** 열에는 커스텀 속성이 마지막으로 편집된 시점(예: 차단 목록에 추가되거나 활성 상태로 변경된 시점)이 표시됩니다.

{% alert important %}
메시지 타겟팅이 올바르게 작동하려면 커스텀 속성 데이터 유형이 실제 커스텀 속성과 일치하는지 확인하세요. <br><br>예를 들어, `newsletter_subscribed`가 문자열로 정의된 경우 Liquid 구문은 {% raw %}```{% if {{custom_attribute.${newsletter_subscribed}}} == 'true' %}```{% endraw %}와 같아야 합니다. `newsletter_subscribed`가 부울로 정의된 경우 Liquid 구문에 작은따옴표가 없어야 합니다: {% raw %}```{% if {{custom_attribute.${newsletter_subscribed}}} == true %}```{% endraw %}.
{% endalert %}
...
커스텀 속성이 현재 활성 캠페인, 캔버스 또는 세그먼트에서 사용 중이지 않아야 합니다. 속성이 아직 참조되고 있는 상태에서 데이터 유형을 변경하려고 하면 대시보드에 오류가 표시되고 변경이 차단됩니다.

### 데이터 유형 변경

1. 세그먼트 또는 필터에서 해당 속성을 사용하는 활성 캠페인이나 캔버스를 중지합니다.
2. 모든 세그먼트, 캠페인, 캔버스 필터에서 해당 속성을 제거합니다.
...
5. 관련 세그먼트, 캠페인, 캔버스에 속성을 다시 적용한 후 중지했던 캠페인이나 캔버스를 다시 활성화합니다.

### 알아두어야 할 사항

- **사용자 데이터는 소급 업데이트되지 않습니다.** 고객 프로필에 이전 데이터 유형의 속성 값이 있는 경우 해당 값은 변경되지 않습니다. 세분화 필터는 새 데이터 유형을 기준으로 조회하므로, 이전 값을 가진 사용자는 프로필이 업데이트될 때까지 일치하는 세그먼트에서 제외됩니다.
- **새 데이터는 새 데이터 유형과 일치해야 합니다.** 변경 후 이 속성에 대해 이전 데이터 유형을 전송하는 API 호출이나 SDK 이벤트는 수락되지 않습니다. 새 데이터 유형과 일치하는 값만 수집됩니다.
- **필터는 자동으로 업데이트되지 않습니다.** 변경된 속성을 참조하는 세그먼트 및 캠페인 필터는 소급 업데이트되지 않습니다. 변경 후 해당 필터를 제거하고 다시 추가해야 합니다.

## 사용 보고서 보기

사용 보고서에는 특정 커스텀 속성을 사용하는 모든 캔버스, 캠페인, 세그먼트가 나열됩니다. 이 목록에는 Liquid 사용은 포함되지 않습니다.
...
다음은 다양한 플랫폼에서 커스텀 속성을 설정하는 데 사용되는 방법 목록입니다.
