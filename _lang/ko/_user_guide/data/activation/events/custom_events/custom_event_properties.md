...
- **Currents:** 고객 행동 이벤트가 활성화되어 있는 경우, 이벤트 속성정보가 Currents 내보내기에 포함됩니다.
- **이벤트 사용자 로그:** 해당 사용자가 테스트 사용자이고 최근에 이벤트를 수행한 경우, 이벤트와 해당 속성정보가 **설정** > **이벤트 사용자 로그**에 표시됩니다.
- **세분화:** 해당 속성정보에 대해 커스텀 이벤트 속성정보 저장이 활성화되어 있는 경우, 이벤트 속성정보 필터를 사용하여 Segment를 생성하고 사용자가 해당 조건에 부합하는지 확인할 수 있습니다.
...
커스텀 이벤트 속성정보는 Campaign 트리거 조건 설정, 전환 추적, 메시징 개인화에 사용할 수 있습니다.

### 메시지 트리거 {#trigger-messages}

커스텀 이벤트 속성정보를 사용하면 특정 Campaign이나 Canvas의 오디언스를 더 세밀하게 좁힐 수 있습니다. 예를 들어, 이커머스 앱에서 사용자가 장바구니를 유기했을 때 메시지를 보내고 싶다면, `price`라는 커스텀 이벤트 속성정보를 추가하여 타겟 오디언스를 개선하고 Campaign 개인화를 강화할 수 있습니다.
...
![유기한 장바구니에 대한 커스텀 이벤트 속성정보 필터. 장바구니 내 항목 중 가격이 100달러 이상인 항목이 있는 경우 하나의 필터가 선택됩니다.]({% image_buster /assets/img_archive/customEventPropertiesNested.png %} "customEventPropertiesNested.png"){: style="max-width:70%;"}
...
- **API 호출:** API 호출 시 "is blank" 필터를 사용하면, 커스텀 이벤트 속성정보가 호출에서 제외되거나 값이 빈 문자열(`""`)인 경우 "blank"로 간주됩니다. 예를 들어, `"event_property": ""`를 포함하면 해당 사용자는 "blank"로 간주됩니다.
...
이 문제를 해결하려면 비교 전에 `plus: 0` 필터를 사용하여 속성정보를 숫자로 변환하세요:
...
예를 들어, 게임 앱에서 레벨을 완료한 사용자에게 메시지를 보내고 싶다면, 사용자가 해당 레벨을 완료하는 데 걸린 시간 속성정보로 메시지를 더욱 개인화할 수 있습니다.

다음 메시지는 [조건 로직]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/liquid/conditional_logic)을 사용하여 세 가지 다른 Segment에 맞게 개인화됩니다. `time_spent`라는 커스텀 이벤트 속성정보는 ``{% raw %} {{event_properties.${time_spent}}} {% endraw %}``를 호출하여 메시지에 포함할 수 있습니다.
...
이벤트 속성정보 세분화를 사용하면 수행된 커스텀 이벤트와 해당 이벤트에 연결된 속성정보를 기반으로 사용자를 타겟팅할 수 있습니다. 이를 통해 구매 및 커스텀 이벤트별 세분화 시 필터링 옵션이 확장됩니다.

커스텀 이벤트의 이벤트 속성정보는 이를 사용하는 모든 Segment에 대해 실시간으로 업데이트됩니다. **데이터 설정** > **커스텀 이벤트**로 이동하여 해당 커스텀 이벤트의 **속성정보 관리**를 선택하면 속성정보를 관리할 수 있습니다. 특정 Segment 필터에서 사용되는 커스텀 이벤트 속성정보는 최대 30일의 조회 기간을 가집니다.
...
![속성정보가 '항목 수'이고 값이 2인 '유기한 장바구니'가 포함된 필터 그룹으로, 지난 30 캘린더일 동안 1회 이상 발생한 경우입니다.]({% image_buster /assets/img/nested_object3.png %})

데이터는 해당 이벤트 속성정보를 활성화한 이후에만 기록되며, 이벤트 속성정보는 해당 날짜 이후부터만 사용할 수 있습니다.
...
중첩 객체(다른 객체 내부의 객체)를 사용하여 커스텀 이벤트 및 구매의 속성정보로 중첩된 JSON 데이터를 전송할 수 있습니다. 이 중첩 데이터는 메시지의 개인화된 정보 템플릿화, 메시지 전송 트리거, 사용자 세분화에 사용할 수 있습니다.
