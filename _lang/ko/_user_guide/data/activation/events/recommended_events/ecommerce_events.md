---
nav_title: eCommerce 권장 이벤트
article_title: eCommerce 권장 이벤트
page_type: reference
alias: /ecommerce_events/
toc_headers: h2
description: "이 참조 문서에서는 eCommerce 권장 이벤트와 등록정보, 사용법, 세분화, 관련 분석을 확인할 수 있는 위치 등을 설명합니다."
---

# eCommerce 권장 이벤트

> 이 페이지에서는 eCommerce 권장 이벤트와 등록정보를 다룹니다. 이러한 이벤트는 유기한 장바구니 타겟팅과 같이 마케터가 효과적인 메시징을 트리거하는 데 필요한 핵심 쇼핑 행동을 캡처하기 위해 만들어졌습니다.

{% alert important %}
eCommerce 권장 이벤트는 현재 얼리 액세스 중입니다. 이 얼리 액세스에 참여하고 싶으시다면 Braze 고객 성공 매니저에게 문의하세요. <br><br>새로운 [Shopify 커넥터]({{site.baseurl}}/partners/ecommerce/shopify/multiple_stores/?tab=shopify%20connector)를 사용하고 있다면, 이러한 권장 이벤트는 통합을 통해 자동으로 사용할 수 있습니다.
{% endalert %}

Braze는 데이터 계획에 시간이 걸린다는 것을 알고 있습니다. 개발 팀과 협력하여 지금부터 이러한 이벤트를 전송하기 시작하는 것을 권장합니다. eCommerce 권장 이벤트와 함께 일부 기능이 즉시 사용 가능하지 않을 수 있지만, 2025년 내내 eCommerce 역량을 강화할 새로운 제품이 도입될 예정입니다.

## eCommerce 권장 이벤트 유형

{% multi_lang_include alerts/important_alerts.md alert='Purchase event deprecation' %}

USD가 아닌 통화로 보고된 금액은 보고된 날짜의 환율을 기준으로 Braze에서 USD로 표시됩니다. 통화 변환을 방지하려면 통화를 USD로 하드코딩하세요.

{% tabs %}
{% tab ecommerce.product_viewed %}

고객이 제품 상세 페이지를 볼 때 트리거되는 제품 조회 이벤트를 사용할 수 있습니다.

#### 등록정보

| 등록정보 이름 | 필수 | 데이터 유형 | 설명 | 
|---|---|---|---|
| `product_id` | 예 | 문자열 | 조회된 제품의 고유 식별자입니다. <br> Shopify를 사용하지 않는 고객의 경우, SKU와 같은 카탈로그 항목 ID로 설정한 값이 됩니다. |
| `product_name` | 예 | 문자열 | 조회된 제품의 이름입니다. | 
| `variant_id` | 예 | 문자열 | 제품 배리언트의 고유 식별자입니다. 예를 들어 `shirt_medium_blue`가 있습니다. |
| `image_url` | 아니요 | 문자열 | 제품 이미지의 URL입니다. |
| `product_url` | 아니요 | 문자열 | 자세한 정보를 위한 제품 페이지 URL입니다. |
| `price` | 예 | 플로트 | 조회 시점의 제품 배리언트 단가입니다. |
| `currency` | 예 | 문자열 | 제품 가격이 표시되는 통화입니다("USD" 또는 "EUR" 등). [ISO 4217 형식](https://www.iso.org/iso-4217-currency-codes.html)을 사용합니다. |
...
두 개의 장바구니가 있는 경우, 병합된 사용자에 두 장바구니를 모두 추가합니다. 동일하거나 다른 장바구니인 경우 캔버스를 다시 대기줄에 넣어 가장 최근 장바구니 정보가 포함된 메시지를 발송합니다. `ecommerce.cart_updated` 이벤트에는 최신 장바구니 ID와 장바구니에 있는 최신 제품이 포함됩니다.
...
Braze는 eCommerce 권장 이벤트를 기반으로 하는 사전 구축된 캔버스 템플릿을 제공합니다. 예를 들어 결제 프로세스를 시작했지만 주문을 완료하기 전에 이탈한 고객을 타겟팅할 수 있습니다. 이러한 이벤트를 사용하여 메시징을 개인화하고 특정 오디언스를 타겟팅함으로써 사용자 여정을 향상시키는 데 도움이 되는 정보에 기반한 결정을 내릴 수 있습니다.
...
### 레거시 구매 이벤트와 eCommerce 권장 이벤트의 차이점은 무엇인가요?

레거시 구매 이벤트는 Braze [구매 오브젝트]({{site.baseurl}}/api/objects_filters/purchase_object/)를 사용하며 `product_id`와 `price`로 개별 제품 구매를 기록합니다. eCommerce 권장 이벤트(예: `ecommerce.order_placed`)는 커스텀 이벤트 등록정보를 사용하며 여러 제품, 할인 및 메타데이터를 포함한 전체 주문 컨텍스트를 단일 이벤트로 캡처합니다.

eCommerce 권장 이벤트의 출시와 함께, Braze는 향후 레거시 구매 이벤트를 단계적으로 폐지할 예정입니다. 현재 구매 이벤트를 사용하고 있다면 사전에 공지를 받게 됩니다. 그때까지는 공식 폐지 날짜까지 구매 이벤트를 계속 사용할 수 있습니다. 자세한 내용은 [권장 이벤트 개요]({{site.baseurl}}/user_guide/data/activation/events/recommended_events/)를 참조하세요.

### eCommerce 권장 이벤트에 커스텀 등록정보를 추가할 수 있나요?

eCommerce 권장 이벤트에는 필수 및 선택 필드가 포함된 정의된 스키마가 있습니다. 각 이벤트의 `metadata` 오브젝트 내에 추가 커스텀 데이터를 포함할 수 있습니다. 그러나 커스텀 주문 수준 태그나 독점 필드(예: 구매 채널 또는 소매점 정보)는 최상위 등록정보로 지원되지 않습니다. 세분화에 이러한 필드가 필요한 경우, eCommerce 이벤트와 함께 별도의 커스텀 이벤트로 계속 전송하세요.

### eCommerce 이벤트를 전송할 때 external_id를 포함해야 하나요?
...
### 인공지능 항목 추천 설정 드롭다운에 중첩된 제품 등록정보가 표시되지 않는 이유는 무엇인가요?
...
### 일부 eCommerce 이벤트가 Braze에 표시되지 않는 이유는 무엇인가요?
