...
이 템플릿은 시작점으로 설계되었습니다. 일반적인 맞춤 설정에는 다음이 포함됩니다.
  ...
  - **채널 추가:** 크로스채널 강화를 위해 이메일에 푸시, 단문 메시지 서비스 또는 인앱 메시지를 결합하세요.
...
| 라스트 터치 기여도 매출 대시보드     | 주문 전 사용자가 마지막으로 상호작용한 Campaign 또는 Canvas에 귀속된 매출입니다. 터치 이벤트에는 이메일 클릭, 푸시 열람, 콘텐츠 카드 클릭, 인앱 메시지 클릭, 단문 메시지 서비스 또는 WhatsApp 단축 링크 클릭이 포함됩니다. |
