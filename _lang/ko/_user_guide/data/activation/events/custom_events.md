...
또한, 차단된 커스텀 이벤트가 현재 Braze의 다른 영역에서 필터 또는 트리거에 의해 참조되고 있는 경우, 해당 이벤트를 참조하는 모든 필터 또는 트리거 인스턴스가 제거되고 보관될 것임을 설명하는 경고 모달이 나타납니다.
...
커스텀 이벤트 데이터로 생성된 Segments는 생성 이전의 과거 기록 데이터를 표시할 수 없습니다.
