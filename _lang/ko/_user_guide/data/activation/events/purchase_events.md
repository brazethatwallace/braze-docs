...
다음은 다양한 플랫폼에서 구매를 로깅하는 데 사용되는 메서드 목록입니다. 이 페이지에서는 구매 이벤트에 속성정보와 수량을 추가하는 방법에 대한 설명서도 확인할 수 있습니다. 이러한 속성정보를 기반으로 사용자를 더 세밀하게 타겟팅할 수 있습니다.
...
기록된 구매 이벤트를 기반으로 다양한 수와 유형의 후속 Campaigns를 트리거할 수 있습니다. 예를 들어, 최근 30일 이내에 구매한 사용자 Segment나 특정 금액 이상을 지출한 사용자 Segment를 생성할 수 있습니다.
...
이 예시에서는 `last_purchased_product` 속성정보를 기반으로 메시지가 개인화됩니다. 사용자가 마지막으로 구매한 제품이 "Running Shoes"인 경우, 러닝 반바지와 물병을 추천하는 메시지를 받습니다. 마지막 제품이 "Yoga Mat"인 경우, 요가 블록과 스트랩을 추천하는 메시지를 받습니다. `last_purchased_product`가 다른 제품인 경우, 일반적인 감사 메시지를 받습니다.

### 메시지 트리거 {#trigger-messages}

일반적인 사용 사례는 사용자가 구매할 때 이메일과 같은 메시지를 자동으로 보내는 것입니다. 예를 들어, 감사 메시지나 다음 구매를 위한 할인 코드를 보낼 수 있습니다.

이를 위해 실행 기반 Campaign 또는 Canvas를 생성한 다음 트리거 동작을 **구매하기**로 설정합니다. 구매한 제품이나 구매 금액 등 트리거에 대한 추가 조건을 지정할 수도 있습니다.
...
구매 이벤트가 USD가 아닌 통화로 기록되면, Braze는 [Open Exchange Rates](http://openexchangerates.org)의 환율을 사용하여 금액을 USD로 변환합니다. 이 환율은 24시간마다(약 동부 표준시 오전 4시) 갱신됩니다. 환율이 캐시되기 때문에, 특히 급격한 변동을 겪는 통화의 경우 실시간 시장 환율과 약간의 차이가 있을 수 있습니다.

#### 생애주기 매출 계산 {#lifetime-revenue-calculation}

Braze는 구매 이벤트를 사용하여 사용자의 생애주기 매출(생애주기 가치 또는 LTV라고도 함)을 계산합니다. 이는 고객과의 전체 미래 관계에 귀속되는 순이익에 대한 예측입니다. 이를 통해 고객 확보 및 유지 전략에 대해 정보에 기반한 의사결정을 내릴 수 있습니다.
...
- 특정 사용자의 생애주기 매출을 파악하려면 해당 사용자의 [고객 프로필]({{site.baseurl}}/user_guide/audience/manage_audience/user_profiles#overview-tab)을 참조하세요.

##### 환불이 생애주기 매출에 미치는 영향 {#impact-of-refunds-on-lifetime-revenue}

구매 이벤트를 사용하여 구매 데이터를 추적할 때, 음수 `price` 속성정보를 가진 Braze 구매 이벤트를 기록하여 환불을 추적해야 합니다. 이 방법은 생애주기 매출의 정확한 합계를 유지합니다.
...
가끔 데이터 포인트를 너무 많이 기록하거나, 마케팅 전략에 더 이상 유용하지 않거나, 실수로 기록된 구매 이벤트를 발견할 수 있습니다. 이 데이터가 Braze로 전송되지 않도록 하려면, 엔지니어링 팀이 앱이나 웹사이트의 백엔드에서 해당 항목을 제거하는 동안 커스텀 데이터 객체를 차단 목록에 추가할 수 있습니다.
