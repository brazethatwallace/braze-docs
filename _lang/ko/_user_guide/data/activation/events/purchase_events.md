...
## 구매 이벤트 기록하기 {#log-purchase-events}

[`/users/track` 엔드포인트]({{site.baseurl}}/api/endpoints/user_data/post_user_track)를 통해 [구매 오브젝트]({{site.baseurl}}/api/objects_filters/purchase_object)를 전달하거나, 다음 섹션에 나열된 SDK 라이브러리 중 하나를 사용하여 구매를 기록할 수 있습니다.

{% alert note %}
구매 이벤트 속성정보는 [커스텀 이벤트 속성정보]({{site.baseurl}}/user_guide/data/activation/events/custom_events/custom_event_properties#expected-format)와 동일한 데이터 타입을 사용합니다.
{% endalert %}

다음은 다양한 플랫폼에서 구매를 기록하는 데 사용되는 메서드 목록입니다. 이 페이지들에서는 구매 이벤트에 속성정보와 수량을 추가하는 방법에 대한 설명서도 확인할 수 있습니다. 이러한 속성정보를 기반으로 사용자를 더욱 세밀하게 타겟팅할 수 있습니다.
...
구매 이벤트를 설정하고 기록을 시작한 후에는 [개요 탭]({{site.baseurl}}/user_guide/audience/manage_audience/user_profiles#overview-tab)에서 사용자 프로필의 구매 데이터를 확인할 수 있습니다.

## 구매 데이터 활용하기 {#use-purchase-data}

Braze에서 구매 데이터를 활용하는 방법에는 여러 가지가 있습니다:

- **[세분화](#purchase-event-segmentation):** 구매 데이터를 사용하여 구매 행동을 기반으로 사용자 Segments를 만들 수 있습니다.
- **[개인화](#personalization):** 구매 데이터를 사용하여 사용자에게 보내는 메시지를 개인화할 수 있습니다.
- **[메시지 트리거](#trigger-messages):** 구매 이벤트를 기반으로 메시지가 트리거되도록 설정할 수 있습니다.
- **[분석](#analytics):** 구매 데이터를 분석하여 사용자 행동과 마케팅 Campaigns의 효과에 대한 인사이트를 얻을 수 있습니다.

### 세분화 {#purchase-event-segmentation}

기록된 구매 이벤트를 기반으로 다양한 수와 유형의 후속 Campaigns를 트리거할 수 있습니다. 예를 들어, 최근 30일 내에 구매한 사용자 Segment 또는 특정 금액 이상을 지출한 사용자 Segment를 만들 수 있습니다.
...
각 필터에 대한 자세한 내용은 [세분화 필터]({{site.baseurl}}/user_guide/audience/segments/segmentation_filters) 용어 사전에서 "Purchase behavior"로 필터링하여 확인하세요.

![정확히 3회 구매한 사용자를 필터링하는 화면]({% image_buster /assets/img/purchase_filter_example.gif %}){: style="max-width:80%;"}

{% alert tip %}
특정 구매가 발생한 횟수를 기준으로 세분화하려면, 해당 구매를 [증분 커스텀 속성]({{site.baseurl}}/developer_guide/platform_wide/analytics_overview#custom-attribute-storage)으로 개별 기록하세요.
{% endalert %}
...
이 예시에서는 `last_purchased_product` 속성정보를 기반으로 메시지가 개인화됩니다. 사용자가 마지막으로 구매한 제품이 "Running Shoes"인 경우, 러닝 반바지와 물병을 추천하는 메시지를 받습니다. 마지막 제품이 "Yoga Mat"인 경우, 요가 블록과 스트랩을 추천하는 메시지를 받습니다. `last_purchased_product`가 그 외의 값인 경우, 일반적인 감사 메시지를 받습니다.

### 메시지 트리거 {#trigger-messages}

일반적인 사용 사례 중 하나는 사용자가 구매할 때 이메일과 같은 메시지를 자동으로 보내는 것입니다. 예를 들어, 감사 메시지나 다음 구매에 사용할 할인 코드를 보낼 수 있습니다.

이렇게 하려면, 실행 기반 Campaign 또는 Canvas를 만든 다음 트리거 동작을 **구매하기**로 설정하세요. 구매한 제품이나 구매 금액 등 트리거에 대한 추가 조건을 지정할 수도 있습니다.

또한 Liquid를 사용하여 트리거된 메시지를 개인화할 수 있습니다. 다음 예시에서 `${purchase_product_name}`은 Braze 설정에서 구매한 제품의 이름을 저장하는 실제 속성 이름으로 대체해야 하는 커스텀 속성입니다.
...
세분화를 위한 구매 측정기준 추적 외에도, Braze는 각 제품의 구매 횟수와 시간 경과에 따른 매출도 기록합니다. 이를 통해 가장 인기 있는 제품을 파악하거나 프로모션 Campaign이 매출에 미치는 영향을 측정할 수 있습니다.
...
<table aria-label="매출 계산">
  <caption>매출 계산</caption>
    ...
    <tbody>
        ...
        <tr>
            <td class="no-split"><a href="/docs/user_guide/analytics/metrics_glossary#average-daily-revenue">평균 일일 매출</a></td>
            <td class="no-split">{% multi_lang_include analytics/metrics.md metric='Average Daily Revenue' %}</td>
        </tr>
        <tr>
            <td class="no-split"><a href="/docs/user_guide/analytics/metrics_glossary#daily-purchases">일일 구매 횟수</a></td>
...
구매 이벤트가 USD가 아닌 통화로 기록된 경우, Braze는 [Open Exchange Rates](http://openexchangerates.org)의 환율을 사용하여 해당 금액을 USD로 변환합니다. 환율은 24시간마다(미국 동부 시간 기준 오전 4시경) 갱신됩니다. 환율이 캐시되므로, 특히 급격한 변동을 보이는 통화의 경우 실시간 시장 환율과 약간의 차이가 있을 수 있습니다.

#### 생애주기 매출 계산 {#lifetime-revenue-calculation}

Braze는 구매 이벤트를 사용하여 사용자의 생애주기 매출(생애주기 가치 또는 LTV라고도 함)을 계산합니다. 이는 고객과의 전체 미래 관계에서 기대되는 순이익에 대한 예측입니다. 이를 통해 고객 획득 및 유지 전략에 대한 정보에 기반한 결정을 내릴 수 있습니다.

$$\text{Average purchase value} = \frac{\text{Total spend in dollars}}{\text{Total number of purchase events}}$$

Braze에서 사용자의 LTV를 파악할 수 있는 주요 위치가 두 곳 있습니다:

- 각 앱 및 사이트의 *생애주기 매출* 및 *사용자당 생애주기 가치*와 같은 전체 측정기준은 [매출 보고서]({{site.baseurl}}/user_guide/analytics/reports/revenue_report#exporting-revenue-data)를 참조하세요.
- 특정 사용자의 생애주기 매출을 확인하려면 해당 사용자의 [고객 프로필]({{site.baseurl}}/user_guide/audience/manage_audience/user_profiles#overview-tab)을 참조하세요.

##### 환불이 생애주기 매출에 미치는 영향 {#impact-of-refunds-on-lifetime-revenue}

구매 이벤트를 사용하여 구매 데이터를 추적하는 경우, 음수 `price` 속성정보를 가진 Braze 구매 이벤트를 기록하여 환불을 추적해야 합니다. 이 방법은 생애주기 매출의 정확한 합계를 유지합니다.

그러나 환불은 추가 구매 이벤트로 집계된다는 점을 유의하세요. 다음 예시를 살펴보겠습니다. Sam은 $12에 첫 구매를 했지만 구매의 일부를 반품하여 $5의 환불을 받았습니다. Sam의 프로필에는 다음과 같이 기록됩니다:

- $12의 구매 1건
- -$5의 구매 1건
- 생애주기 매출 $7

Sam의 프로필에는 두 건의 구매 이벤트가 있지만, 실제로는 한 건의 구매만 이루어졌습니다. 사용자의 구매 횟수를 기반으로 구축한 Segments나 사용 사례가 있는 경우 이 점을 고려하는 것이 중요합니다. 지속적인 환불은 사용자 프로필의 구매 횟수를 부풀릴 수 있습니다.
...
구매 이벤트 중 데이터 포인트를 지나치게 많이 기록하거나, 마케팅 전략에 더 이상 유용하지 않거나, 잘못 기록된 이벤트가 있을 수 있습니다. 이 데이터가 Braze로 전송되지 않도록 하려면, 엔지니어링 팀이 앱 또는 웹사이트의 백엔드에서 해당 데이터를 제거하는 동안 커스텀 데이터 오브젝트를 차단 목록에 추가할 수 있습니다.
