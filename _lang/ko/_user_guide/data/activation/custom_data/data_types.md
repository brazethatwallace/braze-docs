...
상대적 날짜를 사용하는 시간 필터(예: 1일 전 이상, 2일 전 미만)는 1일을 24시간으로 측정합니다. 이러한 필터를 사용하여 실행하는 모든 캠페인은 24시간 단위로 모든 사용자를 포함합니다. 예를 들어, `last used app more than 1 day ago`는 캠페인이 실행되는 정확한 시간으로부터 "24시간 이상 전에 마지막으로 앱을 사용한" 모든 사용자를 캡처합니다. 더 긴 날짜 범위로 설정된 캠페인에도 동일하게 적용됩니다. 따라서 활성화로부터 5일은 이전 120시간을 의미합니다.
...
- **필터가 자동으로 업데이트되지 않습니다.** 변경된 속성 또는 이벤트를 사용하는 세그먼트, 캠페인, 캔버스 또는 기타 위치는 업데이트되지 않습니다. 데이터 유형을 변경하기 전에 세그먼트 또는 필터에서 해당 속성을 사용하는 캠페인이나 캔버스를 중지하고, 해당 속성을 참조하는 필터에서 속성을 제거하세요.
- **기존 사용자 데이터는 소급 업데이트되지 않습니다.** 변경 전에 고객 프로필에 변경된 속성이 있었다면, 해당 값은 이전 데이터 유형으로 유지됩니다. 필터가 새 데이터 유형을 찾기 때문에 사용자가 변경된 속성을 포함하는 세그먼트에서 빠질 수 있습니다. 해당 고객 프로필을 업데이트하여(예: [`/users/track` 엔드포인트]({{site.baseurl}}/api/endpoints/user_data/post_user_track/) 사용) 새 유형과 일치하도록 하고 필요한 경우 세그먼트에 다시 포함되도록 하세요.
