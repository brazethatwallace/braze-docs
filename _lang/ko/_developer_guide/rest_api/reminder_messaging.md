...
page_order: 5
page_type: reference
description: "이 참조 문서에서는 Braze 랜딩 페이지, 커스텀 속성, Campaign을 사용하여 사용자가 다가오는 이벤트나 약속에 대한 개인화된 리마인더 메시지에 가입할 수 있도록 하는 방법을 안내합니다."
---

# 사용자 선택 리마인더 메시징 {#user-selected-reminder-messaging}
...
먼저 [Braze 랜딩 페이지를 생성]({{site.baseurl}}/user_guide/messaging/landing_pages/create_landing_pages/)합니다. 그런 다음 사용자를 랜딩 페이지로 연결하는 메시지(예: 이메일)를 생성합니다.
...
사용자가 이 링크를 클릭하면 Braze가 자동으로 사용자를 식별하므로, 제출한 환경설정이 기존 프로필에 기록됩니다—수동 URL 파라미터가 필요하지 않습니다. 전체 안내는 [양식을 통한 사용자 추적]({{site.baseurl}}/user_guide/messaging/landing_pages/tracking_users/)을 참조하세요.

## 2단계: 랜딩 페이지에서 환경설정 수집하기 {#step-2-capture-preferences-on-the-landing-page}
...
## 3단계: 리마인더 메시지 설정 및 트리거하기 {#step-3-set-up-and-trigger-reminder-messages}

랜딩 페이지를 통해 커스텀 속성을 수집한 후, 다가오는 이벤트에 대해 사용자에게 메시지를 보내는 Campaign을 생성합니다.

### 옵션 A: 공유 날짜 {#step-3-option-a-shared-dates}

부울 커스텀 속성을 사용한 경우([2단계](#option-a-shared-dates-dnd-form-blocks)의 옵션 A), 해당 속성을 Segment 필터로 사용하여 리마인더 메시지의 오디언스를 구축합니다. 그런 다음 이벤트 전에 스케줄된 새 Campaign을 생성하여 선택한 콘텐츠로 이 그룹을 타겟팅합니다.
...
지속적으로 리마인더를 보내려면 매일 반복되는 Campaign을 설정하여 매일 해당 기간 내에 다가오는 리마인더가 있는 사용자가 메시지를 받도록 합니다.
...
4. Campaign을 시작할 때 결과를 면밀히 모니터링합니다.

## 고려 사항 {#considerations}

- 날짜 기반 커스텀 속성을 사용하여 메시지를 보내는 방법에 대한 자세한 예제는 [REST API 메시징 가이드]({{site.baseurl}}/api/endpoints/messaging/)의 이메일 사용 사례를 참조하세요.
