...
기본적으로 Web SDK는 추적된 이벤트가 30분 동안 없으면 세션을 비활성으로 간주합니다. SDK를 초기화할 때 `sessionTimeoutInSeconds` 매개변수를 사용하여 이 임계값을 커스터마이즈할 수 있습니다. 코드 예제를 포함한 이 매개변수 구성에 대한 자세한 내용은 [기본 세션 타임아웃 변경하기](#changing-the-default-session-timeout)를 참조하세요.
