...
# Carthage 통합 {#carthage-integration}

## SDK 가져오기 {#import-the-sdk}

버전 `4.4.0`부터 Carthage를 통해 통합할 때 Braze SDK는 XCFrameworks를 지원합니다. 전체 SDK를 가져오려면 `Cartfile`에 다음 줄을 포함하세요:
```
...
플랫폼별 Carthage 사용법은 해당 [사용자 가이드](https://github.com/Carthage/Carthage#if-youre-building-for-ios-tvos-or-watchos)를 참조하세요.
{% endalert %}

### 이전 버전 {#previous-versions}

`3.24.0`~`4.3.4` 버전의 경우 `Cartfile`에 다음을 포함하세요:
```
binary "https://raw.githubusercontent.com/Appboy/appboy-ios-sdk/master/appboy_ios_sdk_full.json"
```

`3.24.0` 이전 버전을 가져오려면 `Cartfile`에 다음을 포함하세요:
```
github "Appboy/Appboy-iOS-SDK" "<BRAZE_IOS_SDK_VERSION>"
```

`<BRAZE_IOS_SDK_VERSION>`을 "x.y.z" 형식의 Braze iOS SDK [적절한 버전](https://github.com/Appboy/appboy-ios-sdk/releases)으로 바꿔야 합니다.

## 다음 단계 {#next-steps}

[통합 완료]({{site.baseurl}}/developer_guide/platforms/legacy_sdks/ios/initial_sdk_setup/completing_integration) 지침을 따르세요.

## 코어 전용 통합 {#core-only-integration}

UI 구성요소나 종속성 없이 코어 SDK를 사용하려면 `Cartfile`에 다음 줄을 포함하여 Braze Carthage 프레임워크의 코어 버전을 설치하세요:
