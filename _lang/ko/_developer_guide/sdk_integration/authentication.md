...
앱에서 이 기능을 활성화한 후, Braze 대시보드에서 유효하지 않거나 누락된 JSON 웹 토큰(JWT)이 포함된 요청을 거부하도록 설정할 수 있습니다. 여기에는 다음이 포함됩니다:

- 커스텀 이벤트, 속성, 구매 및 세션 데이터 전송
- Braze 워크스페이스에 새 사용자 생성
- 표준 고객 프로필 속성 업데이트
- 메시지 수신 또는 트리거

이제 인증되지 않은 로그인 사용자가 앱의 SDK API 키를 사용하여 다른 사용자를 사칭하는 등의 악의적인 작업을 수행하는 것을 방지할 수 있습니다.
...
RS256 JWT 알고리즘에 사용하기 위해 2048비트의 RSA 키를 권장합니다.

{% alert warning %}
비공개 키는 반드시 *비공개*로 유지하세요. 앱이나 웹사이트에 비공개 키를 노출하거나 하드코딩하지 마세요. 비공개 키를 아는 사람은 누구나 애플리케이션을 대신하여 사용자를 가장하거나 생성할 수 있습니다.
{% endalert %}

#### 1.2단계: 현재 사용자를 위한 JSON 웹 토큰 생성 {#create-jwt}

비공개 키를 확보했으면, 서버 측 애플리케이션에서 이를 사용하여 현재 로그인한 사용자에게 JWT를 반환해야 합니다.

일반적으로, 이 로직은 앱에서 현재 사용자의 프로필을 요청하는 곳(예: 로그인 엔드포인트 또는 현재 사용자의 프로필을 새로 고치는 곳)에 배치할 수 있습니다.
...
| 필드 | 필수 | 설명 |
| ----- | -------- | -------------------------------------------------------------------------------------- |
| `sub` | 예 | "subject"는 `changeUser`를 호출할 때 Braze SDK에 제공하는 사용자 ID와 동일해야 합니다. |
| `exp` | 예 | 이 토큰이 만료되기를 원하는 시점의 "expiration"으로, 초 단위의 Unix 타임스탬프입니다(예: 2030년 1월 1일의 경우 `1893456000`). |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="1.2단계: 현재 사용자를 위한 JSON 웹 토큰 생성" }

{% alert tip %}
JSON 웹 토큰에 대해 자세히 알아보거나, 이 서명 프로세스를 간소화하는 다양한 오픈 소스 라이브러리를 확인하려면 [https://jwt.io](https://jwt.io)를 방문하세요.
{% endalert %}

### 2단계: SDK 구성 {#sdk-integration}

이 기능은 다음 [SDK 버전]({{ site.baseurl }}/user_guide/engagement_tools/campaigns/ideas_and_strategies/new_features/#filtering-by-most-recent-app-versions)부터 사용할 수 있습니다:

{% sdk_min_versions swift:5.0.0 android:14.0.0 web:3.3.0 %}

{% alert note %}
iOS 통합의 경우, 이 페이지에서는 Braze Swift SDK에 대한 단계를 설명합니다. 레거시 AppboyKit iOS SDK의 샘플 사용법은 [이 파일](https://github.com/Appboy/appboy-ios-sdk/blob/master/Example/Stopwatch/Sources/AppDelegate.m)과 [이 파일](https://github.com/Appboy/appboy-ios-sdk/blob/master/Example/Stopwatch/Sources/Utils/SdkAuthDelegate.m)을 참조하세요.
{% endalert %}

#### 2.1단계: Braze SDK에서 인증 활성화 {#step-21-enable-authentication-in-the-braze-sdk}

이 기능을 활성화하면, Braze SDK는 Braze 서버에 대한 네트워크 요청에 현재 사용자의 마지막으로 알려진 JWT를 첨부합니다.

{% alert note %}
이 옵션만으로 초기화해도 Braze 대시보드에서 [인증을 적용](#braze-dashboard)하기 전까지는 데이터 수집에 아무런 영향을 미치지 않으니 걱정하지 마세요.
{% endalert %}

{% tabs %}
{% tab 웹 %}
`initialize`를 호출할 때, 선택적 `enableSdkAuthentication` 속성을 `true`로 설정합니다.
```javascript
...
{% endtab %}
{% tab React Native %}
SDK 인증은 네이티브 SDK 초기화 시 활성화해야 합니다. 네이티브 iOS 및 Android 코드에 다음 구성을 추가하세요:
...
또는 braze.xml에 `<bool name="com_braze_sdk_authentication_enabled">true</bool>`을 추가할 수 있습니다.
{% endtab %}
{% tab KOTLIN %}
Braze 인스턴스를 구성할 때, `setIsSdkAuthenticationEnabled`를 `true`로 호출합니다.
```kotlin
...
{% endtab %}
{% tab Objective-C %}
SDK 인증을 활성화하려면, Braze 인스턴스를 초기화하기 전에 `BRZConfiguration` 객체의 `configuration.api.sdkAuthentication` 속성을 `YES`로 설정합니다:
...
{% endtab %}
{% tab Swift %}
SDK 인증을 활성화하려면, SDK를 초기화할 때 `Braze.Configuration` 객체의 `configuration.api.sdkAuthentication` 속성을 `true`로 설정합니다:
...
{% endtab %}
{% tab Flutter %}
SDK 인증은 네이티브 iOS 및 Android 코드에서 SDK를 초기화하는 과정의 일부로 활성화해야 합니다. 네이티브 레이어에서 활성화하면, Flutter SDK 메서드를 사용하여 JWT 서명을 전달할 수 있습니다.

**iOS**

SDK 인증을 활성화하려면, 네이티브 iOS 코드에서 `configuration.api.sdkAuthentication` 속성을 `true`로 설정합니다:
...
SDK 인증을 활성화하려면, `config.xml`에서 `enableSDKAuthentication` 속성을 `true`로 설정합니다:
...
SDK 인증은 네이티브 SDK 초기화 시 활성화해야 합니다. iOS와 Android에 대해 SDK 인증을 별도로 구성합니다:

**iOS**

SDK 인증을 활성화하려면, SDK를 초기화할 때 `configuration.Api.SdkAuthentication` 속성을 `true`로 설정합니다:
...
SDK 인증을 활성화한 후, 다음 단계에 표시된 .NET MAUI 메서드를 사용할 수 있습니다.
{% endtab %}
{% tab Expo %}
Braze Expo 플러그인을 사용할 때, 앱 구성에서 `enableSdkAuthentication` 속성을 `true`로 설정합니다. 이렇게 하면 수동 네이티브 코드 변경 없이 네이티브 iOS 및 Android 레이어에서 SDK 인증이 자동으로 구성됩니다.
...
앱 구성에서 SDK 인증을 활성화한 후, 다음 단계에서 React Native 탭에 표시된 React Native JavaScript 메서드를 사용할 수 있습니다.
...
앱에서 Braze `changeUser` 메서드를 호출할 때마다, [서버 측에서 생성된](#braze-dashboard) JWT도 함께 제공합니다.

현재 사용자의 세션 중간에 토큰을 새로 고치도록 구성할 수도 있습니다.

{% alert note %}
`changeUser`는 사용자 ID가 _실제로 변경된_ 경우에만 호출해야 합니다. 사용자 ID가 변경되지 않은 경우 인증 토큰(JWT)을 업데이트하기 위한 방법으로 이 메서드를 사용해서는 안 됩니다.
{% endalert %}

{% tabs %}
{% tab 웹 %}
[`changeUser`](https://js.appboycdn.com/web-sdk/latest/doc/modules/braze.html#changeuser)를 호출할 때 JWT를 제공합니다:
...
[`changeUser`](https://braze-inc.github.io/braze-react-native-sdk/classes/Braze.Braze-1.html#changeUser)를 호출할 때 JWT를 제공합니다:
...
[`changeUser`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze/-i-braze/change-user.html)를 호출할 때 JWT를 제공합니다:
...
[`changeUser`](https://braze-inc.github.io/braze-swift-sdk/documentation/brazekit/braze/changeuser(userid:sdkauthsignature:fileid:line:))를 호출할 때 JWT를 제공합니다:
...
{% alert note %}
`changeUser`는 호출 스레드에서 즉시 반환됩니다. 여기서 제공된 SDK 인증 서명은 사용자 전환 작업이 완료된 후에 첨부됩니다.
{% endalert %}
...
Braze Expo 플러그인을 사용할 때, 동일한 React Native SDK 메서드를 사용합니다. `changeUser`를 호출할 때 JWT를 제공합니다:
...
#### 2.3단계: 유효하지 않은 토큰에 대한 콜백 함수 등록 {#sdk-callback}

이 기능이 [필수](#enforcement-options)로 설정된 경우, 다음 시나리오에서 SDK 요청이 Braze에 의해 거부됩니다:
- JWT가 Braze API에 수신될 때까지 만료된 경우
- JWT가 비어 있거나 누락된 경우
- Braze 대시보드에 업로드한 공개 키에 대해 JWT 검증에 실패한 경우

`subscribeToSdkAuthenticationFailures`를 사용하여 이러한 이유로 SDK 요청이 실패할 때 알림을 받도록 구독할 수 있습니다. 콜백 함수에는 관련 [`errorCode`](#error-codes), 오류 `reason`, 요청의 `userId`(사용자는 익명일 수 없음), 오류를 발생시킨 인증 토큰(JWT)이 포함된 객체가 전달됩니다.

실패한 요청은 앱이 새로운 유효한 JWT를 제공할 때까지 주기적으로 재시도됩니다. 해당 사용자가 여전히 로그인 상태라면, 이 콜백을 활용하여 서버에서 새 JWT를 요청하고 Braze SDK에 새로운 유효한 토큰을 제공할 수 있습니다.

인증 오류를 수신하면, 오류의 `userId`가 현재 로그인한 사용자와 일치하는지 확인한 다음, 서버에서 새 서명을 가져와 Braze SDK에 제공합니다. 이러한 오류를 모니터링 또는 오류 보고 서비스에 기록할 수도 있습니다.

{% alert tip %}
이러한 콜백 메서드는 Braze 요청이 얼마나 자주 거부되는지 추적하기 위해 자체 모니터링 또는 오류 로깅 서비스를 추가하기에 좋은 위치입니다.
{% endalert %}
...
네이티브 iOS 구현에서 SDK 인증 델리게이트를 설정합니다:
...
`Braze` 인스턴스에 SDK 인증 델리게이트를 설정합니다:
...
{% endtab %}
{% tab Expo %}
Braze Expo 플러그인을 사용할 때, 동일한 React Native SDK 메서드를 사용합니다:
...
SDK 요청은 앱의 SDK 인증 설정이 Braze 대시보드에서 **필수**로 설정되지 않는 한 인증 없이 평소와 같이 계속 흐릅니다.

통합에 문제가 발생한 경우(예: 앱이 SDK에 토큰을 잘못 전달하거나 서버가 유효하지 않은 토큰을 생성하는 경우), Braze 대시보드에서 이 기능을 비활성화하면 검증 없이 데이터가 평소와 같이 흐르게 됩니다.

#### 적용 옵션 {#enforcement-options}

대시보드의 **설정 관리** 페이지에서, 각 앱에는 Braze가 요청을 검증하는 방식을 제어하는 세 가지 SDK 인증 상태가 있습니다.

| 설정 | 설명 |
| ------ | ---------- |
| **비활성화** | Braze는 사용자에 대해 제공된 JWT를 검증하지 않습니다. (기본 설정) |
| **선택적** | Braze는 로그인한 사용자에 대한 요청을 검증하지만, 유효하지 않은 요청을 거부하지는 않습니다. |
| **필수** | Braze는 로그인한 사용자에 대한 요청을 검증하고 유효하지 않은 JWT를 거부합니다. |
{: .reset-td-br-1 .reset-td-br-2 aria-label="적용 옵션" }

![비활성화, 선택적, 필수 적용 옵션을 보여주는 Braze SDK 인증 설정.]({% image_buster /assets/img/sdk-auth-settings.png %})

**선택적** 설정은 이 기능이 앱의 SDK 트래픽에 미치는 잠재적 영향을 모니터링하는 데 유용한 방법입니다.

유효하지 않은 JWT는 **선택적** 및 **필수** 상태 모두에서 보고되지만, **필수** 상태에서만 SDK 요청을 거부하여 앱이 재시도하고 새 JWT를 요청하게 됩니다.
...
![인증 오류 발생 횟수를 보여주는 차트. 총 오류 수, 오류 유형 및 조정 가능한 날짜 범위도 표시됩니다.]({% image_buster /assets/img/sdk-auth-analytics.png %}){: style="max-width:80%"}
