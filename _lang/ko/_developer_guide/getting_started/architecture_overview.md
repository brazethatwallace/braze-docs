...
* [작업](#action): 마케팅 팀은 데이터를 기반으로 단문 메시지 서비스 및 이메일과 같은 다양한 메시징 채널을 통해 콘텐츠를 생성합니다.
...
Braze는 채널에 구애받지 않는 사용자 중심의 데이터 모델을 통해 진화하는 기술 환경을 처리하도록 설계되었습니다. 대시보드는 메시지 전달 및 트랜잭션 트리거를 관리합니다. 예를 들어, 마케터는 사용자가 특정 위치 근처에 설정된 지오펜스에 들어갈 때 새로 개장한 매장의 쿠폰을 제공하는 단문 메시지 서비스 메시지를 트리거하거나, 사용자가 좋아하는 프로그램의 새 시즌이 나왔음을 알리는 이메일을 보낼 수 있습니다.
