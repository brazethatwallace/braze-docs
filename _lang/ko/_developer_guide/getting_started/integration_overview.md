...
Monthly Active Users CY 24-25, Universal MAU, Web MAU, Mobile MAU를 구매한 고객의 경우, Braze는 SDK에서 세션, 사용자 속성, 이벤트 및 기타 고객 프로필 데이터를 업데이트하는 데 사용하는 API 요청에 대해 서버 측 사용량 제한을 적용합니다. 이는 플랫폼 안정성을 보장하고 빠르고 안정적인 서비스를 유지하기 위한 것입니다.

* 시간당 사용량 제한은 계정의 예상 SDK 트래픽에 따라 설정되며, 이는 구매한 월간 활성 사용자(MAU) 수, 업종, 시즌성 또는 기타 요인에 해당할 수 있습니다. 시간당 사용량 제한에 도달하면 Braze는 다음 시간까지 요청을 조절합니다.
...
* 버스트 사용량 제한은 매우 짧은 시간(즉, 수 초 이내) 내에 대량의 요청이 도착할 때 적용되는 단기 사용량 제한입니다. 버스트 제한이 발생해도 별도의 조치를 취할 필요가 없으며, SDK가 곧바로 재시도합니다.
* 정상 사용량 제한은 버스트 윈도우보다 긴 롤링 윈도우(예: 수 분) 동안의 지속적인 요청 볼륨을 제어하며, 버스트 제한과 시간당 사용량 제한 사이의 지속적인 트래픽을 원활하게 합니다.

### 사용량 제한 확인하기 {#finding-your-rate-limits}

예상 SDK 처리량에 기반한 현재 제한을 확인하려면 **설정** > **API 및 식별자** > **API 및 SDK 제한**으로 이동하세요.
...
* 일시적 증가가 필요한지 영구적 증가가 필요한지 여부.
* 증가가 필요한 이유.
* 영향을 받는 엔드포인트 및 환경.
* 시작 날짜, 기간, 피크 시간을 포함한 대략적인 트래픽 볼륨 및 타임라인.
* 호출을 일괄 처리하거나 트래픽을 시간에 걸쳐 분산할 수 있는지 여부.

요청을 제출하면 Braze가 이를 검토하고 결과를 업데이트해 드립니다.

### 변경 사항 및 지원 {#changes-and-support}

Braze는 시스템 안정성을 보호하거나 계정의 데이터 처리량 증가를 허용하기 위해 사용량 제한을 수정할 수 있습니다. 사용량 제한 및 비즈니스에 미치는 영향에 대한 질문이나 우려 사항이 있으면 Braze 지원팀 또는 고객 성공 매니저에게 문의하세요.