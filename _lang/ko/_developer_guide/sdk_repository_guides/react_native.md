...
다른 자격 증명으로 `Braze.initialize`를 다시 호출하면 현재 인스턴스가 해제되고 다시 생성되므로, 세션 중간에 재초기화가 가능합니다.
...
- `res/values`에 구성이 포함된 `braze.xml` 리소스 파일을 추가합니다. SDK가 JavaScript에서 `Braze.initialize()`를 호출할 때까지 대기하도록 지연 초기화를 활성화합니다. 기타 구성 값(푸시, 세션 타임아웃 등)은 여전히 이 파일에서 읽혀지며 초기화 시 적용됩니다.
...
선택 사항인 **SDK 인증**: 대시보드에서 활성화된 경우 `changeUser`의 두 번째 인수로 서명을 전달하거나 `Braze.setSdkAuthenticationSignature(signature)`를 호출합니다.
...
**`changeUser`**는 Braze에 **새로운** 활동을 어떤 사용자 ID에 귀속할지만 알려줍니다. 기기에 캐시된 SDK 데이터를 지우지는 **않습니다**. 별도의 "로그아웃" API는 없습니다. 기존 로그아웃(이전 사용자의 캐시된 프로필, 메시지, 토큰이 이 설치에서 사라지도록 로컬 Braze 상태를 지우는 것)이 필요한 경우 일반적으로 **`wipeData()`**를 사용합니다. 이는 전체 로컬 초기화입니다.
