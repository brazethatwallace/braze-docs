...
Braze React Native SDK는 iOS 및 Android 앱을 Braze에 연결합니다: 고객 프로필, 메시징 화면, 분석, 기능 플래그를 포함합니다. 네이티브 [Braze Swift SDK](https://github.com/braze-inc/braze-swift-sdk) 및 [Braze Android SDK](https://github.com/braze-inc/braze-android-sdk)를 JavaScript API로 래핑합니다.
...
- **Content Cards**: 기본 피드 UI 또는 카드를 가져와서 자체 UI 구축
- **배너**: `BrazeBannerView`를 포함한 배치 기반 HTML 배너
- **푸시 알림**: 권한 요청, 토큰 등록, 페이로드 리스너(**푸시 알림** 참조)
- **기능 플래그**: 새로고침, 속성정보 읽기, 노출 기록
...
- **Android**: Android Studio / Gradle, React Native 템플릿에 필요한 Kotlin Gradle 플러그인
- **푸시**(사용하는 경우): [푸시 설명서](https://www.braze.com/docs/developer_guide/platform_integration_guides/react_native/push_notifications/)에 따른 FCM(Android) 및 APNs(iOS) 설정
...
이 섹션에서는 Braze React Native SDK를 초기화하는 데 필요한 최소 설정을 안내합니다.
...
TypeScript 타입 정의는 패키지와 함께 제공됩니다(GitHub의 `src/index.d.ts`).

`Braze.initialize`를 다른 자격 증명으로 다시 호출하면 현재 인스턴스를 해체하고 다시 생성하므로, 세션 중 재초기화를 지원합니다.

---

## 네이티브 설정 {#native-setup}

> **기본 참조 자료:** 단계별 화면, Gradle/CocoaPods 변경 사항, Android XML 키의 전체 목록은 [Braze React Native 개발자 가이드](https://www.braze.com/docs/developer_guide/sdk_integration/?sdktab=react%20native)에서 확인할 수 있습니다. 이 섹션의 **Android** 및 **iOS** 스니펫은 최소한의 예시입니다.

### Android

- 템플릿에 아직 포함되어 있지 않은 경우 루트 `build.gradle`에 **Kotlin Gradle 플러그인**을 추가합니다(버전은 React Native 버전에 따라 다름).
- `res/values`에 구성이 포함된 `braze.xml` 리소스 파일을 추가합니다. SDK가 JavaScript에서 `Braze.initialize()`를 호출할 때까지 대기하도록 지연 초기화를 활성화합니다. 기타 구성 값(푸시, 세션 타임아웃 등)은 이 파일에서 읽혀지며 초기화 시 적용됩니다.
- `AndroidManifest.xml`에 `INTERNET` 및 `ACCESS_NETWORK_STATE`와 같은 기본 권한을 확인합니다.
- 푸시의 경우 FCM 통합 및 문서에 설명된 Braze 전용 발신자 ID / 등록 플래그를 완료합니다.
...
{% alert note %}
** API 키와 엔드포인트는 더 이상 `braze.xml`에서 설정하지 않으며, JavaScript에서 `Braze.initialize(apiKey, endpoint)`를 통해 전달됩니다.
{% endalert %}
### iOS
...
{% alert note %}
** `BrazeReactInitializer.configure`는 더 이상 사용되지 않는 `BrazeReactBridge.initBraze(_:)`를 대체하는 Swift 우선 API입니다. 또한 Objective-C 브릿지에서 `Braze.Configuration`의 Swift 타입 해석 문제를 해결합니다.
{% endalert %}
---

## 설정 레퍼런스 {#configuration-reference}

React Native에서 **설정은 네이티브 방식으로** 이루어집니다. Android는 `res/values/braze.xml`을 읽고, iOS는 **`BrazeReactInitializer.configure`**를 통해 등록된 클로저를 사용합니다. 두 가지 모두 JavaScript에서 `Braze.initialize(apiKey, endpoint)`가 호출될 때 적용됩니다.

### Android (`braze.xml`)

기본값은 XML에 정의되며, [`BrazeConfig.Builder`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.configuration/-braze-config/-builder/index.html)를 사용하여 시작 시 이를 재정의할 수 있습니다. 키와 유형에 대한 정식 목록은 [Android SDK 통합 가이드](https://www.braze.com/docs/developer_guide/platforms/android/sdk_integration/) 및 [`BrazeConfigurationProvider`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.configuration/-braze-configuration-provider/index.html)를 참조하세요(각 Kotlin 속성은 문서화된 `com_braze_*` 리소스에 대응합니다).
...
| `com_braze_api_key` | `string` | JavaScript에서 `Braze.initialize()`를 사용하는 경우 필요하지 않습니다(자격 증명은 JS에서 전달됩니다). 레거시 네이티브 우선 초기화에서만 필요합니다. |
| `com_braze_custom_endpoint` | `string` | JavaScript에서 `Braze.initialize()`를 사용하는 경우 필요하지 않습니다. 레거시 네이티브 우선 초기화에서만 필요합니다. |
| `com_braze_server_target` | `string` | 선택 사항인 클러스터/환경 선택기입니다(예: 일부 내부 또는 스테이징 빌드). Braze 통합에서 별도로 지정하지 않는 한 프로덕션에서는 `com_braze_custom_endpoint`를 사용하는 것이 좋습니다. |
| `com_braze_firebase_cloud_messaging_registration_enabled` | `bool` | `true`인 경우, Braze가 FCM에 등록합니다(일반적인 푸시 설정). |
...
| **기타** | *다양* | 여기에 표시되지 않은 추가 키(세션 타임아웃, 지오펜스, 위치, 알림 기본값, 기기 허용 목록, 지연 초기화, SDK 인증 등)가 있습니다. [`BrazeConfigurationProvider`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.configuration/-braze-configuration-provider/index.html) 및 [Android SDK 통합 가이드](https://www.braze.com/docs/developer_guide/platforms/android/sdk_integration/)를 참조하세요. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="Android (braze.xml)" }

### iOS (`Braze.Configuration`)

`BrazeReactInitializer.configure`에 전달되는 `configure` 클로저에서 네이티브 설정 속성을 지정합니다. 이 클로저는 `Braze.Configuration` 인스턴스를 수신하며, API 키와 엔드포인트는 JavaScript의 `Braze.initialize` 호출에서 자동으로 설정됩니다. 전체 세부 사항은 [`Braze.Configuration`](https://braze-inc.github.io/braze-swift-sdk/documentation/brazekit/braze/configuration-swift.class)과 중첩 유형 **`api`**, **`push`**, **`logger`**, **`location`**을 참조하세요.

| 영역 | 멤버(대표 예시) | 참고 |
|------|--------------------------|--------|
| **자격 증명** | `api.key`, `api.endpoint` | JavaScript의 `Braze.initialize(apiKey, endpoint)`에서 자동으로 설정됩니다. `configure` 클로저에서 이 값을 설정하지 마세요. |
| **로깅** | `logger.level` | 상세 로깅은 개발용입니다. 프로덕션에서는 로그를 줄이세요. |
| **푸시** | `push.automation`, `push.appGroup`, … | 자동화는 등록을 간소화하며, Push Stories/확장 기능 사용 시 `appGroup`이 필요합니다. |
| **인앱 메시지** | `triggerMinimumTimeInterval` | 기본값은 트리거 사이 **30**초입니다. |
| **세션** | `sessionTimeout` | 새 세션이 시작되기까지의 비활성 시간입니다(Braze 세션 설명서를 참조하세요). |
| **개인정보 보호/데이터** | `api.trackingPropertyAllowList`, `devicePropertyAllowList`, `api.sdkAuthentication` | [개인정보 보호 매니페스트](https://www.braze.com/docs/developer_guide/platform_integration_guides/swift/privacy_manifest/) 및 SDK 인증 제품 설정에 맞추어 구성하세요. |
| **네트워킹** | `api.requestPolicy`, `api.flushInterval` | 요청 재시도 정책 및 플러시 주기입니다. |
| **푸시 구독** | `optInWhenPushAuthorized` | `true`인 경우, 사용자가 알림을 허용한 후 구독 상태가 옵트인으로 변경될 수 있습니다. |
...
- **기본 Braze UI**를 사용하는 경우 [인앱 메시지 설명서](https://www.braze.com/docs/developer_guide/in_app_messages?sdktab=react%20native)를 참조하세요. 일반적으로 기본 UI만 표시하려면 `subscribeToInAppMessage`를 호출할 필요가 **없습니다**.
...
- **`getInitialPushPayload`**: 알림에서 앱이 열릴 때 RN `Linking` 경합 조건을 방지하기 위해 사용합니다. TypeScript 문서 주석 및 샘플 앱에 설명된 대로 네이티브 훅(iOS의 `BrazeReactUtils`, Android의 `BrazeReactUtils.populateInitialPushPayloadFromIntent`)이 필요합니다.
...
**`changeUser`**는 **새로운** 활동을 어떤 사용자 ID에 귀속시킬지 Braze에 알려주기만 합니다. 기기에 캐시된 SDK 데이터를 지우지는 **않습니다**. 별도의 "로그아웃" API는 없습니다. 전통적인 로그아웃(이 설치에서 이전 사용자의 캐시된 프로필, 메시지, 토큰 등 로컬 Braze 상태를 지움)이 필요한 경우 일반적으로 **`wipeData()`**를 사용합니다. 이 메서드는 완전한 로컬 초기화를 수행합니다.
...
**`wipeData()`** — 이 설치에 대한 Braze의 **로컬** 데이터를 지웁니다(캐시된 사용자/세션/카드 상태, 푸시 토큰 연결 등). **로그아웃 스타일** 동작(이전 사용자의 Braze 상태를 기기에 남기지 않아야 하는 경우), **"이 기기에서 내 데이터 삭제"**, 재설치 없이 **QA** 초기화, 엄격한 **개인정보 보호** 흐름에 사용합니다. **`changeUser`**만으로는 이러한 정리를 수행하지 않으며, **새로운** 이벤트를 수신할 사용자 ID만 설정합니다. **iOS**에서는 동작이 Android와 다를 수 있습니다(예: SDK 비활성화 상태와의 상호작용). 프로덕션에 배포하는 경우 Braze 네이티브 설명서를 참조하세요.

**`disableSDK()`** — SDK 작동을 중지합니다(구성된 대로 수집/전달 없음). **사용자 옵트아웃** 토글, **제한 모드**(컴플라이언스, 어린이 설정), 또는 종속성을 제거하지 않고 **디버깅**하는 경우에 사용합니다.

**`enableSDK()`** — **`disableSDK()`** 이후 SDK를 다시 켭니다. **iOS**에서는 다시 활성화가 **다음 앱 실행** 때까지 적용되지 않을 수 있습니다. 즉시 다시 활성화에 의존하기 전에 Braze Swift/iOS 설명서를 확인하세요.

---

## 이벤트 {#events}

`Braze.addListener(event, callback)`를 사용하여 구독합니다. 이 호출은 구독 객체를 반환하며, 수신 대기를 중단하려면 해당 객체에서 **`.remove()`**를 호출합니다.
...
React 컴포넌트에서 구독을 저장하고 정리 과정(예: `useEffect`의 반환 함수)에서 `.remove()`를 호출합니다:
...
- **Expo**: 가능하면 수동 네이티브 설정을 피하기 위해 [Braze Expo 플러그인](https://github.com/braze-inc/braze-expo-plugin)을 사용하세요.
...
{% endalert %}
다음 표는 Braze 플러그인 릴리스별로 지원되는 React Native 버전을 보여줍니다.
...
Expo 관리형 워크플로의 경우, [Braze Expo 플러그인 리포지토리](https://github.com/braze-inc/braze-expo-plugin)를 참조하세요.
...
개발 중에 **네이티브** 구성에서 Braze 로깅을 활성화하면 SDK가 시스템 콘솔(Xcode / Android Logcat)에 로그를 기록합니다. 이를 통해 초기화, 사용자 변경 및 이벤트 전달을 확인할 수 있습니다.

- **iOS** — `BrazeReactInitializer.configure`에 전달되는 `configure` 클로저에서 `config.logger.level = .debug`(또는 `.info`)를 설정합니다. 프로덕션 환경에서는 사용자에게 로그가 노출되지 않도록 줄이거나 비활성화하세요.
- **Android** — `braze.xml`에서 `com_braze_logger_initial_log_level` 리소스를 사용하거나 `BrazeConfig.Builder`에서 해당 값을 설정합니다([BrazeConfigurationProvider](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.configuration/-braze-configuration-provider/logger-initial-log-level.html) 참조). 릴리스 전에 상세하지 않은 레벨을 사용하거나 오버라이드를 제거하세요.

네트워크, 세션 또는 Campaign 동작에 대한 심층적인 문제 해결은 [Braze React Native 개발자 가이드](https://www.braze.com/docs/developer_guide/sdk_integration/?sdktab=react%20native) 및 네이티브 SDK 설명서([Swift](https://github.com/braze-inc/braze-swift-sdk) · [Android](https://github.com/braze-inc/braze-android-sdk))를 참조하세요.
...
질문이 있는 경우 Braze 기술 지원팀에 문의하여 도움을 받으세요.
