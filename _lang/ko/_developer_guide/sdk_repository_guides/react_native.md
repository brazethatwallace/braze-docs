...
- **푸시 알림**: 권한 요청, 토큰 등록, 페이로드 리스너([푸시 알림](#push-notifications) 참조)
...
- **푸시** (사용하는 경우): FCM(Android) 및 APNs(iOS) 설정은 [푸시 설명서](https://www.braze.com/docs/developer_guide/platform_integration_guides/react_native/push_notifications/)를 참조하세요
...
1. npm 패키지를 설치합니다([설치](#installation) 참조).
2. Android 및 iOS의 **네이티브 설정**을 완료합니다(구성, 권한, 필요한 경우 푸시 포함).
...
다른 자격 증명으로 `Braze.initialize`를 다시 호출하면 현재 인스턴스가 해제되고 다시 생성되므로, 세션 중 재초기화가 지원됩니다.

---

## 네이티브 설정 {#native-setup}

> **공식 참고 자료:** 단계별 화면, Gradle/CocoaPods 변경 사항, Android XML 키의 전체 목록은 [Braze React Native 개발자 가이드](https://www.braze.com/docs/developer_guide/sdk_integration/?sdktab=react%20native)에서 확인할 수 있습니다. 다음 스니펫은 최소한의 예시입니다.

### Android

- 템플릿에 아직 포함되어 있지 않다면 루트 `build.gradle`에 **Kotlin Gradle 플러그인**을 추가합니다(버전은 React Native 버전에 따라 다릅니다).
- `res/values`에 구성이 포함된 `braze.xml` 리소스 파일을 추가합니다. 지연 초기화를 활성화하여 SDK가 JavaScript에서 `Braze.initialize()`가 호출될 때까지 대기하도록 합니다. 다른 구성 값(푸시, 세션 타임아웃 등)은 여전히 이 파일에서 읽어 초기화 시점에 적용됩니다.
- `AndroidManifest.xml`에 `INTERNET` 및 `ACCESS_NETWORK_STATE`와 같은 기본 권한이 포함되어 있는지 확인합니다.
- 푸시의 경우, FCM 통합 및 설명서에 설명된 Braze 전용 발신자 ID/등록 플래그를 완료합니다.
...
- **`postInitialization` 클로저** *(선택 사항)*: 생성된 후 실시간 `Braze` 인스턴스를 수신하며, 인스턴스가 필요한 설정(예: 참조 저장, 델리게이트 설정)에 사용됩니다.
...
React Native에서 **구성은 네이티브 방식으로 이루어집니다**: Android는 `res/values/braze.xml`을 읽고, iOS는 **`BrazeReactInitializer.configure`**를 통해 등록된 클로저를 사용합니다. 두 설정 모두 JavaScript에서 `Braze.initialize(apiKey, endpoint)`를 호출할 때 적용됩니다.

### Android (`braze.xml`)

기본값은 XML에 정의되어 있으며, [`BrazeConfig.Builder`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.configuration/-braze-config/-builder/index.html)를 사용하여 시작 시 재정의할 수 있습니다. 키와 타입의 전체 목록은 [Android SDK 통합 가이드](https://www.braze.com/docs/developer_guide/platforms/android/sdk_integration/)와 [`BrazeConfigurationProvider`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.configuration/-braze-configuration-provider/index.html)에서 확인할 수 있습니다(각 Kotlin 속성은 문서화된 `com_braze_*` 리소스에 대응합니다).

자주 사용되는 항목:

| 키 | 리소스 타입 | 설명 |
|-----|---------------|-------------|
| `com_braze_enable_delayed_initialization` | `bool` | **필수입니다.** SDK가 JavaScript의 `Braze.initialize()` 호출을 기다리도록 `true`로 설정합니다. |
...
| `com_braze_server_target` | `string` | 선택적 클러스터/환경 선택기입니다(예: 일부 내부 또는 스테이징 빌드). Braze 통합에서 별도로 지정하지 않는 한 프로덕션에서는 `com_braze_custom_endpoint`를 사용하세요. |
| `com_braze_firebase_cloud_messaging_registration_enabled` | `bool` | `true`로 설정하면 Braze가 FCM에 등록합니다(일반적인 푸시 설정). |
| `com_braze_firebase_cloud_messaging_sender_id` | `string` | 자동 등록이 활성화된 경우 FCM 발신자 ID입니다. |
| `com_braze_handle_push_deep_links_automatically` | `bool` | Braze가 푸시 딥링크를 자동으로 열 수 있도록 합니다. |
| `com_braze_trigger_action_minimum_time_interval_seconds` | `integer` | 인앱 메시지 트리거 동작 사이의 최소 시간(초)입니다. |
| **기타** | *다양함* | 여기에 표시되지 않은 추가 키(세션 타임아웃, 지오펜스, 위치, 알림 기본값, 기기 허용 목록, 지연 초기화, SDK 인증 등)가 있습니다. [`BrazeConfigurationProvider`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.configuration/-braze-configuration-provider/index.html)와 [Android SDK 통합 가이드](https://www.braze.com/docs/developer_guide/platforms/android/sdk_integration/)를 참조하세요. |
...
| **푸시** | `push.automation`, `push.appGroup`, … | 자동화는 등록을 간소화합니다. `appGroup`은 Push Stories/확장 기능을 사용할 때 필요합니다. |
| **인앱 메시지** | `triggerMinimumTimeInterval` | 트리거 사이의 기본값은 **30**초입니다. |
| **세션** | `sessionTimeout` | 새 세션이 시작되기 전의 비활성 시간입니다(Braze 세션 설명서 참조). |
| **개인정보 보호/데이터** | `api.trackingPropertyAllowList`, `devicePropertyAllowList`, `api.sdkAuthentication` | [개인정보 보호 매니페스트](https://www.braze.com/docs/developer_guide/platform_integration_guides/swift/privacy_manifest/) 및 SDK 인증 제품 설정에 맞추어 설정하세요. |
| **네트워킹** | `api.requestPolicy`, `api.flushInterval` | 요청 재시도 정책 및 플러시 주기입니다. |
| **푸시 구독** | `optInWhenPushAuthorized` | `true`로 설정하면 사용자가 알림을 허용한 후 구독 상태가 옵트인으로 변경될 수 있습니다. |
| **인앱 메시지 + 사용자 변경** | `preventInAppMessageDisplayForDifferentUser` | 사용자 ID가 변경될 때 인앱 메시지 불일치를 줄여줍니다. |
...
React Native 브리지는 초기화 시 React 전용 **`api.sdkFlavor`** / SDK 메타데이터를 설정합니다. Braze 설명서에서 특별히 지시하지 않는 한 이 값을 재정의하지 마세요.
...
- **커스텀** 처리의 경우, `useBrazeUI: false`로 구독한 다음 필요에 따라 노출 횟수/클릭을 기록합니다:
...
- **`getInitialPushPayload`**: 알림에서 앱이 열릴 때 RN `Linking` 경합 조건을 방지하기 위해 사용합니다. iOS에서는 `BrazeReactUtils`, Android에서는 `BrazeReactUtils.populateInitialPushPayloadFromIntent` 네이티브 후크가 필요하며, TypeScript 문서 주석 및 샘플 앱에 설명되어 있습니다.
- **`Braze.addListener(Braze.Events.PUSH_NOTIFICATION_EVENT, ...)`**는 공개 타이핑에 따르면 **Android 전용**입니다.
...
**`changeUser`**는 Braze에 **새로운** 활동을 귀속시킬 사용자 ID만 알려줍니다. 기기에 캐시된 SDK 데이터를 지우지는 **않습니다**. 별도의 "로그아웃" API는 없습니다. 기존 로그아웃 방식(이전 사용자의 캐시된 프로필, 메시지, 토큰이 이 설치에 남지 않도록 로컬 Braze 상태를 지우기)이 필요한 경우, 일반적으로 **`wipeData()`**를 사용합니다. 이것은 전체 로컬 초기화입니다.
...
**`wipeData()`** — 이 설치에 대한 Braze의 **로컬** 데이터(캐시된 사용자/세션/카드 상태, 푸시 토큰 연결 등)를 지웁니다. 이전 사용자의 Braze 상태를 기기에 남겨서는 안 되는 **로그아웃 스타일** 동작, **"이 기기에서 내 데이터 삭제"**, 재설치 없이 **QA** 초기화, 또는 엄격한 **개인정보** 흐름에 사용합니다. **`changeUser`** 단독으로는 해당 정리를 수행하지 않으며, **새로운** 이벤트를 수신할 사용자 ID만 설정합니다. **iOS**에서는 Android와 동작이 다를 수 있습니다(예: SDK 비활성화 상태와의 상호작용). 프로덕션에서 이 기능을 배포하는 경우 Braze의 네이티브 설명서를 참조하세요.

**`disableSDK()`** — SDK의 작동을 중지합니다(구성에 따른 수집/전달 없음). **사용자 옵트아웃** 토글, **제한 모드**(규정 준수, 키즈 설정), 또는 종속성을 제거하지 않고 **디버깅**하는 데 사용합니다.

**`enableSDK()`** — **`disableSDK()`** 이후 SDK를 다시 활성화합니다. **iOS**에서는 재활성화가 **다음 앱 실행** 시까지 적용되지 않을 수 있습니다. 즉시 재활성화에 의존하기 전에 Braze Swift/iOS 설명서에서 확인하세요.

---

## 이벤트 {#events}

`Braze.addListener(event, callback)`를 사용하여 구독합니다. 이 호출은 구독 객체를 반환하며, 수신 대기를 중지하려면 해당 객체에서 **`.remove()`**를 호출합니다.
...
React 컴포넌트에서는 구독을 저장하고 정리 시 `.remove()`를 호출합니다(예: `useEffect`의 반환값).
...
- **Expo**: 수동 네이티브 설정을 최소화하려면 [Braze Expo 플러그인](https://github.com/braze-inc/braze-expo-plugin)을 사용하세요.
- **New Architecture / Turbo Modules**: 최신 플러그인 버전에서 지원됩니다. 마이그레이션하는 경우 개발자 가이드와 샘플 `AppDelegate` / Gradle 설정을 참조하세요.
- **프라이버시(iOS)**: `updateTrackingPropertyAllowList`와 같은 메서드는 프라이버시 매니페스트 관련 구성을 지원합니다. 자세한 내용은 [Swift 프라이버시 매니페스트](https://www.braze.com/docs/developer_guide/platform_integration_guides/swift/privacy_manifest/)를 참조하세요.
- **Jest**: `react-native` 네이티브 모듈 또는 Braze Turbo 모듈을 모킹하세요(패턴은 이 리포지토리의 `__tests__/jest.setup.js`를 참조하세요).
...
개발 중에 **네이티브** 설정에서 Braze 로깅을 활성화하면 SDK가 시스템 콘솔(Xcode / Android Logcat)에 기록합니다. 이를 통해 초기화, 사용자 변경, 이벤트 전달을 확인할 수 있습니다.

- **iOS** — `BrazeReactInitializer.configure`에 전달되는 `configure` 클로저에서 `config.logger.level = .debug`(또는 `.info`)를 설정합니다. 프로덕션에서는 사용자에게 로그가 노출되지 않도록 줄이거나 비활성화하세요.
- **Android** — `braze.xml`에서 `com_braze_logger_initial_log_level` 리소스를 사용하거나 `BrazeConfig.Builder`에서 동일한 값을 설정합니다([BrazeConfigurationProvider](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.configuration/-braze-configuration-provider/logger-initial-log-level.html) 참조). 출시 전에 상세하지 않은 수준으로 변경하거나 오버라이드를 제거하세요.
...
질문이 있으시면 Braze 기술 지원팀에 도움을 요청하세요.
