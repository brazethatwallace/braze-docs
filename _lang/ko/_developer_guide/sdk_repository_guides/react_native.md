...
- Braze 앱 API 키 및 SDK 엔드포인트가 포함된 **Braze 계정**
...
- **푸시** (사용하는 경우): FCM(Android) 및 APNs(iOS) 설정은 [푸시 알림 설명서](https://www.braze.com/docs/developer_guide/platform_integration_guides/react_native/push_notifications/)를 참조하세요
...
다른 자격 증명으로 `Braze.initialize`를 다시 호출하면 현재 인스턴스가 해제되고 다시 생성되므로, 세션 중 재초기화가 가능합니다.
...
- `res/values`에 구성이 포함된 `braze.xml` 리소스 파일을 추가합니다. SDK가 JavaScript에서 `Braze.initialize()`를 호출할 때까지 대기하도록 지연 초기화를 활성화합니다. 다른 구성 값(푸시, 세션 타임아웃 등)은 여전히 이 파일에서 읽혀지며 초기화 시 적용됩니다.
...
| **인앱 메시지 + 사용자 변경** | `preventInAppMessageDisplayForDifferentUser` | 사용자 ID가 변경될 때 일치하지 않는 인앱 메시지를 줄입니다. |
...
React Native 브리지는 초기화 시 React 전용 **`api.sdkFlavor`** / SDK 메타데이터를 설정합니다. Braze 설명서에서 별도로 지시하지 않는 한 이 값을 재정의하지 마세요.
...
**`changeUser`**는 Braze에 **새로운** 활동을 어떤 사용자 ID에 귀속할지만 알려줍니다. 기기에 캐시된 SDK 데이터를 지우지는 **않습니다**. 별도의 "로그아웃" API는 없습니다. 기존 로그아웃 방식(이전 사용자의 캐시된 프로필, 메시지, 토큰이 이 설치에 남지 않도록 로컬 Braze 상태를 지우는 것)이 필요한 경우, 일반적으로 **`wipeData()`**를 사용합니다. 이는 전체 로컬 초기화입니다.
...
**`wipeData()`** — 이 설치에 대한 Braze의 **로컬** 데이터(캐시된 사용자/세션/카드 상태, 푸시 토큰 연결 등)를 지웁니다. 이전 사용자의 Braze 상태를 기기에 남기지 않아야 하는 **로그아웃 방식** 동작, **"이 기기에서 내 데이터 삭제"**, 재설치 없이 **QA** 초기화, 또는 엄격한 **개인정보 보호** 흐름에 사용합니다. **`changeUser`**만으로는 이러한 정리를 수행하지 않으며, **새로운** 이벤트를 수신할 사용자 ID만 설정합니다. **iOS**에서는 동작이 Android와 다를 수 있습니다(예: SDK 비활성화 상태와의 상호작용). 프로덕션에서 이 기능을 사용하는 경우 Braze의 네이티브 설명서를 참조하세요.

**`disableSDK()`** — SDK 작동을 중지합니다(구성된 대로 수집/전달 없음). **사용자 옵트아웃** 토글, **제한 모드**(규정 준수, 어린이 설정), 또는 종속성을 제거하지 않고 **디버깅**하는 데 사용합니다.

**`enableSDK()`** — **`disableSDK()`** 이후 SDK를 다시 활성화합니다. **iOS**에서는 다시 활성화가 **다음 앱 실행** 전까지 적용되지 않을 수 있습니다. 즉시 재활성화에 의존하기 전에 Braze Swift/iOS 설명서에서 확인하세요.

---

## 이벤트 {#events}

`Braze.addListener(event, callback)`를 사용하여 구독합니다. 이 호출은 구독 객체를 반환하며, 수신을 중지하려면 해당 객체에서 **`.remove()`**를 호출합니다.
...
React 컴포넌트에서는 구독을 저장하고 정리 단계에서 `.remove()`를 호출합니다(예: `useEffect`의 반환 함수):
...
- **Jest**: `react-native` 네이티브 모듈 또는 Braze Turbo 모듈을 모킹합니다(패턴은 이 리포지토리의 `__tests__/jest.setup.js`를 참조하세요).
...
- **Android** — `braze.xml`에서 `com_braze_logger_initial_log_level` 리소스를 사용하거나 `BrazeConfig.Builder`에서 동일한 설정을 지정합니다([BrazeConfigurationProvider](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.configuration/-braze-configuration-provider/logger-initial-log-level.html) 참조). 릴리스 전에 상세하지 않은 로그 수준을 사용하거나 오버라이드를 제거하세요.
