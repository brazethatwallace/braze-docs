...
Braze React Native SDK는 iOS 및 Android 앱을 Braze에 연결합니다. 고객 프로필, 메시징 표면, 분석, 기능 플래그를 지원합니다. 네이티브 [Braze Swift SDK](https://github.com/braze-inc/braze-swift-sdk) 및 [Braze Android SDK](https://github.com/braze-inc/braze-android-sdk)를 JavaScript API로 래핑합니다.

**초기화는 JavaScript 기반입니다.** Android 리소스와 iOS `AppDelegate`에서 네이티브 설정(푸시, 로깅, 델리게이트)을 구성한 다음, JavaScript에서 `Braze.initialize(apiKey, endpoint)`를 호출하여 SDK를 시작합니다. 이를 통해 SDK 초기화 시점과 사용할 자격 증명을 완전히 제어할 수 있습니다. 초기화 후, 필요에 따라 다른 SDK 메서드(예: `changeUser`, `logCustomEvent`)를 호출합니다.
...
- **푸시** (사용하는 경우): FCM(Android) 및 APNs(iOS) 설정은 [푸시 설명서](https://www.braze.com/docs/developer_guide/push_notifications?sdktab=react%20native) 참조
...
2. Android 및 iOS용 **네이티브 설정**을 완료합니다(구성, 권한, 필요한 경우 푸시 등).
...
TypeScript 타이핑은 패키지에 포함되어 있습니다(GitHub의 `src/index.d.ts`).

다른 자격 증명으로 `Braze.initialize`를 다시 호출하면 현재 인스턴스를 해제하고 다시 생성하므로, 세션 중간에 재초기화할 수 있습니다.

---

## 네이티브 설정 {#native-setup}

> **정보 출처:** 단계별 화면, Gradle/CocoaPods 변경 사항, Android XML 키의 전체 목록은 [Braze React Native 개발자 가이드](https://www.braze.com/docs/developer_guide/sdk_integration/?sdktab=react%20native)에서 확인할 수 있습니다. 이 섹션의 **Android** 및 **iOS** 스니펫은 최소한의 예시입니다.

### Android

- 템플릿에 아직 포함되어 있지 않은 경우, 루트 `build.gradle`에 **Kotlin Gradle 플러그인**을 추가합니다(버전은 React Native 버전에 따라 다릅니다).
- `res/values`에 구성이 포함된 `braze.xml` 리소스 파일을 추가합니다. SDK가 JavaScript에서 `Braze.initialize()`를 호출할 때까지 대기하도록 지연 초기화를 활성화합니다. 그 밖의 구성 값(푸시, 세션 타임아웃 등)은 여전히 이 파일에서 읽혀지며 초기화 시점에 적용됩니다.
- `AndroidManifest.xml`에 `INTERNET` 및 `ACCESS_NETWORK_STATE`와 같은 기본 권한이 있는지 확인합니다.
- 푸시의 경우 FCM 통합과 설명서에 기술된 Braze 전용 발신자 ID/등록 플래그를 완료합니다.
...
`AppDelegate`에서 `BrazeReactInitializer.configure`를 사용하여 네이티브 구성을 등록합니다. 제공된 클로저는 저장되었다가 나중에 JavaScript에서 `Braze.initialize(apiKey, endpoint)`가 호출될 때 적용됩니다.
...
React Native에서 **구성은 네이티브 수준에서 이루어집니다**: Android는 `res/values/braze.xml`을 읽고, iOS는 **`BrazeReactInitializer.configure`**를 통해 등록된 클로저를 사용합니다. 두 방식 모두 JavaScript에서 `Braze.initialize(apiKey, endpoint)`를 호출할 때 적용됩니다.

### Android (`braze.xml`)

기본값은 XML에 정의되어 있으며, [`BrazeConfig.Builder`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.configuration/-braze-config/-builder/index.html)를 통해 시작 시 재정의할 수 있습니다. 키와 타입의 전체 목록은 [Android SDK 통합 가이드](https://www.braze.com/docs/developer_guide/platforms/android/sdk_integration/) 및 [`BrazeConfigurationProvider`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.configuration/-braze-configuration-provider/index.html)에서 확인할 수 있습니다(각 Kotlin 속성은 문서화된 `com_braze_*` 리소스에 대응합니다).

자주 사용되는 항목:

| 키 | 리소스 타입 | 설명 |
|-----|---------------|-------------|
| `com_braze_enable_delayed_initialization` | `bool` | **필수.** SDK가 JavaScript의 `Braze.initialize()` 호출을 기다리도록 `true`로 설정합니다. |
...
| `com_braze_server_target` | `string` | 선택적 클러스터/환경 선택기(예: 일부 내부 또는 스테이징 빌드). Braze 통합에서 별도로 지정하지 않는 한 프로덕션에서는 `com_braze_custom_endpoint` 사용을 권장합니다. |
| `com_braze_firebase_cloud_messaging_registration_enabled` | `bool` | `true`이면 Braze가 FCM에 등록합니다(일반적인 푸시 설정). |
| `com_braze_firebase_cloud_messaging_sender_id` | `string` | 자동 등록이 활성화된 경우 FCM 발신자 ID입니다. |
| `com_braze_handle_push_deep_links_automatically` | `bool` | Braze가 푸시 딥링크를 자동으로 열도록 합니다. |
| `com_braze_trigger_action_minimum_time_interval_seconds` | `integer` | 인앱 메시지 트리거 동작 간 최소 시간(초)입니다. |
...
| **푸시** | `push.automation`, `push.appGroup`, … | 자동화는 등록을 간소화합니다. `appGroup`은 Push Stories / 확장 프로그램 사용 시 필요합니다. |
| **인앱 메시지** | `triggerMinimumTimeInterval` | 트리거 간 기본값 **30**초입니다. |
| **세션** | `sessionTimeout` | 새 세션이 시작되기까지의 비활성 시간(Braze 세션 설명서 참조). |
...
| **푸시 구독** | `optInWhenPushAuthorized` | `true`이면 사용자가 알림을 허용한 후 구독 상태가 옵트인으로 전환될 수 있습니다. |
| **IAM + 사용자 변경** | `preventInAppMessageDisplayForDifferentUser` | 사용자 ID가 변경될 때 불일치하는 인앱 메시지를 줄여줍니다. |
...
React Native 브릿지는 초기화 시 React 전용 **`api.sdkFlavor`** / SDK 메타데이터를 설정합니다. Braze 설명서에서 별도로 지시하지 않는 한 이 값을 재정의하지 마세요.
...
- **기본 Braze UI**를 사용하는 경우, [인앱 메시지 설명서](https://www.braze.com/docs/developer_guide/in_app_messages?sdktab=react%20native)를 따르세요. 기본 UI를 표시하기 위해 `subscribeToInAppMessage`를 호출할 필요는 **없습니다**.
- **커스텀** 처리를 위해서는 `useBrazeUI: false`로 구독한 후 필요에 따라 노출 횟수/클릭을 기록합니다:
...
`Braze.addListener(Braze.Events.CONTENT_CARDS_UPDATED, ...)`를 사용하여 업데이트를 수신 대기할 수 있습니다.
...
- **`getInitialPushPayload`**: 알림에서 앱이 열릴 때 RN `Linking` 경합 조건을 방지하기 위해 사용합니다. TypeScript 문서 코멘트와 샘플 앱에 설명된 대로 네이티브 훅(iOS의 `BrazeReactUtils`, Android의 `BrazeReactUtils.populateInitialPushPayloadFromIntent`)이 필요합니다.
- **`Braze.addListener(Braze.Events.PUSH_NOTIFICATION_EVENT, ...)`**는 공개 타이핑 기준으로 **Android 전용**입니다.
...
**`changeUser`**는 Braze에 **새로운** 활동을 귀속시킬 사용자 ID만 알려줍니다. 기기에 캐시된 SDK 데이터를 삭제하지는 **않습니다**. 별도의 "로그아웃" API는 없습니다. 기존 로그아웃 방식(이전 사용자의 캐시된 프로필, 메시지, 토큰이 이 설치에 남지 않도록 로컬 Braze 상태를 지우는 것)이 필요한 경우, 일반적으로 **`wipeData()`**를 사용합니다. 이는 전체 로컬 초기화입니다.
...
**`wipeData()`** — 이 설치에 대한 Braze의 **로컬** 데이터(캐시된 사용자/세션/카드 상태, 푸시 토큰 연결 등)를 지웁니다. 기기에 이전 사용자의 Braze 상태를 남겨두면 안 되는 **로그아웃 스타일** 동작, **"이 기기에서 내 데이터 삭제"**, 재설치 없는 **QA** 초기화, 또는 엄격한 **개인정보 보호** 흐름에 사용합니다. **`changeUser`**만으로는 이 정리를 수행하지 않으며, **새로운** 이벤트를 수신할 사용자 ID만 설정합니다. **iOS**에서는 Android와 동작이 다를 수 있습니다(예: SDK 비활성화 상태와의 상호작용). 프로덕션에서 이를 적용하는 경우 Braze 네이티브 문서를 참조하세요.

**`disableSDK()`** — SDK 작동을 중지합니다(구성된 대로 수집/전달이 없음). **사용자 옵트아웃** 토글, **제한 모드**(규정 준수, 어린이 설정), 또는 종속성을 제거하지 않는 **디버깅**에 사용합니다.
...
`Braze.addListener(event, callback)`로 구독합니다. 호출 시 구독 객체가 반환되며, 수신을 중단하려면 해당 객체에서 **`.remove()`**를 호출합니다.
...
React 컴포넌트에서는 구독을 저장하고 정리 단계(예: `useEffect`의 반환부)에서 `.remove()`를 호출합니다.
...
- **Expo**: 가능한 경우 수동 네이티브 연결을 피하려면 [Braze Expo 플러그인](https://github.com/braze-inc/braze-expo-plugin)을 사용하세요.
...
| Braze 플러그인 | React Native | 새로운 아키텍처 |
|--------------|--------------|------------------|
| 9.0.0+       | ≥ 0.71       | 예              |
| 6.0.0+       | ≥ 0.68       | 예 (≥ 0.70.0)   |
| 2.0.0+       | ≥ 0.68       | 예              |
...
개발 중에 **네이티브** 설정에서 Braze 로깅을 활성화하면 SDK가 시스템 콘솔(Xcode / Android Logcat)에 로그를 기록합니다. 이를 통해 초기화, 사용자 변경, 이벤트 전달을 확인하는 데 도움이 됩니다.

- **iOS** — `BrazeReactInitializer.configure`에 전달되는 `configure` 클로저에서 `config.logger.level = .debug`(또는 `.info`)를 설정합니다. 프로덕션 환경에서는 사용자에게 로그가 노출되지 않도록 로그 수준을 줄이거나 비활성화하세요.
- **Android** — `braze.xml`에서 `com_braze_logger_initial_log_level` 리소스를 사용하거나 `BrazeConfig.Builder`에서 동일한 설정을 구성합니다([BrazeConfigurationProvider](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.configuration/-braze-configuration-provider/logger-initial-log-level.html) 참조). 릴리스 전에 상세하지 않은 로그 수준을 사용하거나 오버라이드를 제거하세요.

네트워크, 세션, Campaign 동작 등 보다 심층적인 문제 해결은 [Braze React Native 개발자 가이드](https://www.braze.com/docs/developer_guide/sdk_integration/?sdktab=react%20native) 및 네이티브 SDK 설명서([Swift](https://github.com/braze-inc/braze-swift-sdk) · [Android](https://github.com/braze-inc/braze-android-sdk))를 참조하세요.
...
- [푸시 알림 — React Native](https://www.braze.com/docs/developer_guide/push_notifications?sdktab=react%20native)
