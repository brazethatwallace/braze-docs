...
이 리포지토리의 형식은 Xamarin 구성 요소 형식입니다: `appboy-component` 아래에 `src`, `libs`, `component`, `nuget`, `samples` 디렉토리가 있습니다. `libs`, `src`, `samples`에는 각각 Android용과 iOS용 두 개의 디렉토리가 포함되어 있습니다. 각 디렉토리에는 다음이 포함됩니다:
...
- `nuget`: Xamarin NuGet 패키지를 위한 Nuspec 파일입니다.

## 버전 관리 {#versioning}

### 네이티브 바인딩 {#native-bindings}

다음 표에는 각 Xamarin 바인딩에 대해 지원되는 프레임워크 및 네이티브 Braze 프레임워크 버전이 나열되어 있습니다.
...
Braze SDK 버전 `4.0.0`부터 Xamarin 및 Xamarin.Forms에 대한 지원이 중단되었으며, [.NET MAUI](https://learn.microsoft.com/en-us/dotnet/maui/what-is-maui)에 대한 지원이 추가되었습니다.
