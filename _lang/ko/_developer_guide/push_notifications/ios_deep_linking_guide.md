...
> 이 가이드는 사용 중인 메시징 채널과 Branch or 브랜치 같은 타사 링크 제공업체 사용 여부에 따라 iOS 앱에 적합한 딥링킹 전략을 선택하는 데 도움을 줍니다.
...
| **유니버설 링크** | `https://myapp.com/products/123` | 이메일, 단문 메시지 서비스, 클릭 추적이 있는 채널 | 예 — 웹으로 폴백 |
...
- 단문 메시지 서비스 또는 링크가 래핑되거나 단축되는 다른 채널에서 전송할 때.
- 앱이 설치되지 않은 경우 웹 페이지로 폴백해야 할 때.
- Branch or 브랜치 또는 AppsFlyer와 같은 서드파티 링크 제공자를 사용할 때.
...
| BrazeDelegate (선택 사항) | 커스텀 라우팅(예: Branch or 브랜치)을 위해 `braze(_:shouldOpenURL:)`을 구현합니다 |
...
- 단문 메시지 서비스 Campaign에서 딥링크를 발송하는 경우(링크가 HTTPS URL로 단축될 수 있기 때문입니다).
- Branch or 브랜치, AppsFlyer 또는 다른 링크 제공업체를 사용하는 경우(해당 업체들이 자체 HTTPS 도메인을 사용하기 때문입니다).
...
| `BrazeDelegate.braze(_:shouldOpenURL:)` | SDK가 여는 모든 URL | 커스텀 라우팅 로직이 필요한 경우(예: Branch or 브랜치, 조건부 처리, 분석) |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="앱 코드에서 링크를 처리해야 하는 경우" }

{% alert tip %}
Branch or 브랜치 같은 타사 링크 제공업체를 사용하는 경우, URL을 가로채서 해당 제공업체의 SDK로 전달하도록 `BrazeDelegate.braze(_:shouldOpenURL:)`를 구현하세요. 전체 예제는 [딥링킹용 Branch or 브랜치]({{site.baseurl}}/partners/message_orchestration/deeplinking/branch_for_deeplinking)를 참조하세요.
{% endalert %}

## Braze에서 Branch or 브랜치 사용하기 {#branch}

[Branch or 브랜치]({{site.baseurl}}/partners/message_orchestration/deeplinking/branch_for_deeplinking)를 링크 제공업체로 사용하는 경우, 표준 유니버설 링크 구성 외에 몇 가지 추가 단계가 필요합니다:

1. **Branch or 브랜치 SDK**: [Branch or 브랜치 설명서](https://help.branch.io/developers-hub/docs/native-sdks-overview)에 따라 Branch or 브랜치 SDK를 통합하세요.
2. **Associated Domains**: Xcode의 **Signing & Capabilities** 아래에 Branch or 브랜치 도메인(예: `applinks:yourapp.app.link`)을 추가하세요.
3. **BrazeDelegate**: Branch or 브랜치 링크를 Braze가 직접 처리하지 않고 Branch or 브랜치 SDK로 라우팅하도록 `braze(_:shouldOpenURL:)`를 구현하세요.
4. **유니버설 링크 전달**: Braze SDK 구성에서 `configuration.forwardUniversalLinks = true`를 설정하세요.

구현 세부 사항 및 디버깅 안내는 [딥링킹용 Branch or 브랜치]({{site.baseurl}}/partners/message_orchestration/deeplinking/branch_for_deeplinking)를 참조하세요.