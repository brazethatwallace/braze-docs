...
layout: api_page
page_type: reference
description: "이 문서에서는 이메일 수신 거부 목록 검색 또는 조회 Braze 엔드포인트에 대한 자세한 내용을 설명합니다."

---
{% api %}
# 수신 거부된 이메일 주소 목록 조회하기 {#query-list-of-unsubscribed-email-addresses}
{% apimethod get %}
/email/unsubscribes
{% endapimethod %}
...
이 엔드포인트를 사용하려면 `email.unsubscribe` 권한이 있는 [API 키]({{site.baseurl}}/api/basics#rest-api-key-permissions)가 필요합니다.
...
| 매개변수 | 필수 | 데이터 유형 | 설명 |
| ----------|-----------| ---------|------ |
| `start_date` | 선택 사항 <br>(참고 참조) | YYYY-MM-DD 형식의 문자열 | 수신 거부를 검색할 범위의 시작 날짜로, end_date보다 이전이어야 합니다. API에서 UTC 시간 기준 자정으로 처리됩니다. |
| `end_date` | 선택 사항 <br>(참고 참조) | YYYY-MM-DD 형식의 문자열 | 수신 거부를 검색할 범위의 종료 날짜입니다. API에서 UTC 시간 기준 자정으로 처리됩니다. |
...
날짜 범위에 `limit` 이상의 수신 거부 건수가 있는 경우, 호출 결과가 `limit`보다 적거나 0이 될 때까지 매번 `offset`을 늘려가며 여러 번 API 호출을 수행해야 합니다.
