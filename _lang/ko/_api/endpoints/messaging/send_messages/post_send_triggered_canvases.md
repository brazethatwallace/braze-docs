...
- **이메일 및 SMS 주소 지정.** Braze에 아직 존재하지 않는 사용자에게 대부분의 이메일 또는 SMS API 트리거 전송을 하려면, `attributes` 내에 필요한 전달 필드(예: `email` 또는 워크스페이스에서 SMS에 사용하는 전화 속성)를 포함하세요. 동일한 호출에서 옵트인 상태를 변경해야 하는 경우 구독 그룹 멤버십 또는 가입 상태도 설정할 수 있습니다.
