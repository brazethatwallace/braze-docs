...
이 엔드포인트로 메시지를 보내려면 [Canvas]({{site.baseurl}}/api/identifier_types#canvas-identifier)를 구축할 때 생성된 Canvas ID가 있어야 합니다.
...
이 엔드포인트를 사용하려면 `canvas.trigger.schedule.update` 권한이 있는 [API 키]({{site.baseurl}}/api/basics#rest-api-key-permissions)가 필요합니다.
