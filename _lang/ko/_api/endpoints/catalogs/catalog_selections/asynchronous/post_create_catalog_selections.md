...
layout: api_page
page_type: reference
description: "이 문서에서는 카탈로그 선택 항목 만들기 Braze 엔드포인트에 대한 자세한 내용을 설명합니다."

---
{% api %}
# 카탈로그 선택 항목 만들기 {#create-catalog-selection}
{% apimethod post %}
...
{% alert note %}
`sort_field` 및 `sort_order` 매개변수는 함께 사용해야 합니다. 하나만 제공하고 다른 하나를 제공하지 않거나 두 매개변수를 모두 생략하면 선택 결과가 무작위 순서로 반환됩니다.
{% endalert %}
...
{% alert note %}
`geo` 필터를 적용하면 `sort_field` 및 `sort_order` 매개변수와 관계없이 시스템이 자동으로 가장 가까운 항목을 먼저 표시하도록 거리순으로 결과를 정렬합니다.
{% endalert %}
