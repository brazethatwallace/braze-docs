...
이 엔드포인트를 사용하려면 `catalogs.create_item` 권한이 있는 [API 키]({{site.baseurl}}/api/basics#rest-api-key-permissions)가 필요합니다.
...
{% alert note %}
`Location` 필드는 `geo` 데이터 유형을 사용하며, `[longitude, latitude]` 형식의 배열을 예상합니다.
{% endalert %}
