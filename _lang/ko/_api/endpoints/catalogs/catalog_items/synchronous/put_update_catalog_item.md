...
article_title: "PUT: 카탈로그 항목 교체"
search_tag: Endpoint
page_order: 6

layout: api_page
page_type: reference
description: "이 문서에서는 카탈로그 항목 교체 Braze 엔드포인트에 대한 자세한 내용을 설명합니다."

---
{% api %}
# 카탈로그 항목 교체 {#replace-catalog-item}
{% apimethod put %}
...
`item_id`를 찾을 수 없는 경우 이 엔드포인트가 카탈로그에 항목을 생성합니다. 이 엔드포인트는 동기식입니다.
...
이 엔드포인트를 사용하려면 `catalogs.replace_item` 권한이 있는 [API 키]({{site.baseurl}}/api/basics#rest-api-key-permissions)가 필요합니다.
