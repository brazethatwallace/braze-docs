...
---
{% api %}
# 여러 카탈로그 항목 편집 {#edit-multiple-catalog-items}
{% apimethod patch %}
...
이 엔드포인트를 사용하려면 `catalogs.update_items` 권한이 있는 [API 키]({{site.baseurl}}/api/basics#rest-api-key-permissions)가 필요합니다.
...
| 매개변수 | 필수 | 데이터 유형 | 설명 |
|---|---|---|---|
| `items` | 필수 | 배열 | 항목 오브젝트가 포함된 배열입니다. 항목 오브젝트에는 카탈로그에 존재하는 필드가 포함되어야 합니다. 요청당 최대 50개의 항목 오브젝트가 허용됩니다. |
