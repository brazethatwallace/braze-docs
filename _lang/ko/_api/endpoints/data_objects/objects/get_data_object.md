---
nav_title: "GET: 데이터 객체 가져오기"
article_title: "GET: 데이터 객체 가져오기"
search_tag: Endpoint
page_order: 2
layout: api_page
page_type: reference
description: "이 문서에서는 데이터 객체 가져오기 엔드포인트에 대해 자세히 설명합니다."
---
{% api %}
# 데이터 객체 가져오기 {#get-data-object}
{% apimethod get %}
/data_objects/objects/{type_name}/{external_id}
{% endapimethod %}

> 이 엔드포인트를 사용하여 하나의 데이터 객체를 반환합니다.

{% alert important %}
데이터 객체는 현재 얼리 액세스 단계입니다. 데이터 객체 API 키 권한이 **설정** > **API 키**에 표시되려면 워크스페이스가 활성화되어 있어야 합니다.
{% endalert %}
...
이 엔드포인트는 데이터 객체 읽기 버킷에 포함되며, 기본값으로 분당 50회 요청으로 제한됩니다.

## 경로 매개변수 {#path-parameters}

다음 표에는 `/data_objects/objects/{type_name}/{external_id}` 엔드포인트의 경로 매개변수가 나열되어 있으며 각각에 대한 설명이 포함되어 있습니다.

| 매개변수 | 필수 | 데이터 유형 | 설명 |
|---|---|---|---|
| `type_name` | 필수 | 문자열 | 데이터 객체 유형 머신 이름 |
| `external_id` | 필수 | 문자열 | 객체 식별자 |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 .reset-td-br-4 aria-label="데이터 객체 가져오기 경로 매개변수" }
...
이 예시는 `acct-123` 계정 레코드와 저장된 속성을 가져옵니다.
...
다음 표에는 성공 응답의 필드가 나열되어 있으며 각각에 대한 설명이 포함되어 있습니다.

| 매개변수 | 필수 | 데이터 유형 | 설명 |
|---|---|---|---|
| `data_object` | 필수 | 객체 | 반환된 데이터 객체 레코드 |
| `data_object.type_name` | 필수 | 문자열 | 데이터 객체 유형 머신 이름 |
| `data_object.external_id` | 필수 | 문자열 | 데이터 객체 식별자 |
| `data_object.attributes` | 필수 | 객체 | 필드 이름으로 키가 지정된 객체 속성 |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 .reset-td-br-4 aria-label="데이터 객체 가져오기 응답 매개변수" }
...
| 상태 | 원인 | 안내 |
|---|---|---|
| `404` | 유형을 찾을 수 없음(`data-object-type-not-found`) 또는 객체를 찾을 수 없음(`data-object-not-found`) | `type_name`과 `external_id`가 모두 워크스페이스에 존재하는지 확인하세요. |
...
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="데이터 객체 가져오기 오류" }
{% endapi %}