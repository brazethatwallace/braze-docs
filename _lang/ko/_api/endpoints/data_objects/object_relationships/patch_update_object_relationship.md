...
이 예시는 `acct-123`과 `acct-456` 사이의 기존 `subaccount` 관계에 속성을 병합하며, 생략한 속성은 변경되지 않습니다.
...
| `404` | 관계를 찾을 수 없음 (`data-object-relationship-not-found`) | 소스 오브젝트, 관련 오브젝트, 관계 키 값이 모두 존재하는지 확인하세요. |
| `401` | REST API 키가 누락되었거나 유효하지 않음 | `Authorization` 헤더에 `Bearer YOUR_REST_API_KEY`가 사용되고 있고 키가 활성 상태인지 확인하세요. |
| `403` | API 키에 권한이 없거나 허용 목록에 의해 요청이 차단됨 | 키에 `data_objects.object_relationships.update` 권한이 있고, 허용 목록이 구성된 경우 소스 IP가 키 허용 목록에 포함되어 있는지 확인하세요. |
