...
이 엔드포인트는 데이터 오브젝트 쓰기 버킷에 포함되어 있으며 기본 제한은 분당 50회 요청입니다.

## 경로 매개변수 {#path-parameters}

다음 표에는 `/data_objects/objects/{type_name}/{external_id}/users` 엔드포인트의 경로 매개변수가 나열되어 있습니다.
...
다음 표에는 `/data_objects/objects/{type_name}/{external_id}/users` 엔드포인트의 JSON 요청 본문 매개변수가 나열되어 있습니다.
...
이 예시는 사용자와 `acct-123` 사이의 `account_user` 관계에 대한 속성을 대체하여 이전에 저장된 모든 속성을 덮어씁니다.
...
| 매개변수 | 필수 | 데이터 유형 | 설명 |
|---|---|---|---|
| `user_relationship` | 필수 | 오브젝트 | 생성되거나 대체된 사용자 관계 레코드 |
...
| `404` | 관계 또는 오브젝트를 찾을 수 없음(`data-object-relationship-not-found`) | 오브젝트, 사용자, 관계 키 값이 모두 존재하는지 확인하세요. |
| `422` | 사용자당 오브젝트 제한 도달(`data-objects-per-user-limit-exceeded`) 또는 오브젝트당 사용자 제한 도달(`users-per-data-object-limit-exceeded`) | 사용자 또는 오브젝트의 관계 수를 줄이거나 워크스페이스 제한에 대해 Braze 지원팀에 문의하세요. |
| `401` | REST API 키가 누락되었거나 유효하지 않음 | `Authorization` 헤더가 `Bearer YOUR_REST_API_KEY`를 사용하고 있으며 해당 키가 활성 상태인지 확인하세요. |
| `403` | API 키에 권한이 없거나 허용 목록에 의해 요청이 차단됨 | 해당 키에 `data_objects.user_relationships.update` 권한이 있는지, 허용 목록이 구성된 경우 소스 IP가 키 허용 목록에 포함되어 있는지 확인하세요. |
