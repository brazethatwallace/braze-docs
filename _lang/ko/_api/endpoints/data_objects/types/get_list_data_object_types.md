---
nav_title: "GET: 데이터 오브젝트 유형 목록"
article_title: "GET: 데이터 오브젝트 유형 목록"
search_tag: Endpoint
page_order: 1
layout: api_page
page_type: reference
description: "이 문서에서는 데이터 오브젝트 유형 목록 엔드포인트에 대해 자세히 설명합니다."
---
{% api %}
# 데이터 오브젝트 유형 목록 {#list-data-object-types}
{% apimethod get %}
/data_objects/types
{% endapimethod %}

> 이 엔드포인트를 사용하여 워크스페이스의 데이터 오브젝트 유형을 나열할 수 있습니다.

{% alert important %}
데이터 오브젝트는 현재 얼리 액세스 단계에 있습니다. 데이터 오브젝트 API 키 권한이 **설정** > **API 키**에 표시되려면 워크스페이스가 사용 설정되어 있어야 합니다.
{% endalert %}
...
다음 표에서는 `/data_objects/types` 엔드포인트의 쿼리 매개변수를 나열하고 설명합니다.

| 매개변수 | 필수 | 데이터 유형 | 설명 |
|---|---|---|---|
| `search_term` | 선택 사항 | 문자열 | 유형 이름에 대한 대소문자를 구분하지 않는 접두사 필터 |
| `limit` | 선택 사항 | 정수 | 페이지 크기. 기본값 `100`. `1`에서 `250` 사이로 제한됨 |
| `offset` | 선택 사항 | 정수 | 오프셋. 기본값 `0`. 음수 값은 `0`으로 내림 처리됨 |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 .reset-td-br-4 aria-label="데이터 오브젝트 유형 목록 쿼리 매개변수" }

## 요청 예시 {#example-request}

이 섹션에는 샘플 쿼리 매개변수 페이로드와 샘플 cURL 요청이 포함되어 있습니다.

### 샘플 요청 페이로드 {#sample-request-payload}

이 요청의 쿼리 매개변수에 대한 참조로 다음 JSON 오브젝트를 사용하세요.
...
이 예시에서는 검색어 `acc`와 일치하는 데이터 오브젝트 유형을 나열하며, 페이지당 두 개의 결과를 반환합니다.
...
`metadata.display_name_source`는 해당 유형에 표시 이름 필드가 구성되어 있을 때 표시됩니다.
...
| 매개변수 | 필수 | 데이터 유형 | 설명 |
|---|---|---|---|
| `items` | 필수 | 배열 | 데이터 오브젝트 유형 레코드 목록 |
...
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 .reset-td-br-4 aria-label="데이터 오브젝트 유형 목록 응답 매개변수" }
...
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="데이터 오브젝트 유형 목록 오류" }
{% endapi %}