...
이 예시에서는 `acct-123`과 `acct-456` 간의 `subaccount` 관계를 교체하며, 이전에 저장된 모든 속성을 덮어씁니다.
...
| `object_relationship.to_custom_object` | 조건부 | 오브젝트 | `anchor=source`일 때의 관련 오브젝트 |
| `object_relationship.from_custom_object` | 조건부 | 오브젝트 | `anchor=target`일 때의 관련 오브젝트 |
...
| 상태 | 원인 | 안내 |
|---|---|---|
| `400` | 유효성 검사 오류 | `rel_kind`, `anchor`, `attributes`가 관계 유형에 유효한지 확인하세요. |
| `404` | 관계 또는 엔드포인트 오브젝트를 찾을 수 없음(`custom-object-relationship-not-found`) | 두 오브젝트와 관련 유형 이름이 워크스페이스에 존재하는지 확인하세요. |
| `422` | 오브젝트당 관계 제한 도달(`custom-object-relationship-limit-exceeded`) | 오브젝트의 관계 수를 줄이거나 워크스페이스 제한에 대해 Braze 지원팀에 문의하세요. |
| `401` | REST API 키 누락 또는 유효하지 않음 | `Authorization` 헤더에 `Bearer YOUR_REST_API_KEY`가 사용되었는지, 그리고 해당 키가 활성 상태인지 확인하세요. |
| `403` | API 키에 권한이 없거나 허용 목록에 의해 요청이 차단됨 | 키에 `custom_objects.object_relationships.update` 권한이 있는지, 그리고 허용 목록이 설정된 경우 소스 IP가 키 허용 목록에 포함되어 있는지 확인하세요. |
