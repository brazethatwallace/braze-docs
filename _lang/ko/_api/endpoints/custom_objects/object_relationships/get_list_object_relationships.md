...
> 이 엔드포인트를 사용하여 하나의 오브젝트 앵커에서 관련 커스텀 오브젝트를 조회합니다.
...
다음 표에는 `/custom_objects/objects/{type_name}/{external_id}/object_relationships` 엔드포인트의 경로 매개변수가 나열되어 있습니다.
...
다음 표에는 `/custom_objects/objects/{type_name}/{external_id}/object_relationships` 엔드포인트의 쿼리 매개변수가 나열되어 있습니다.
...
요청 매개변수의 참고용으로 이 JSON 오브젝트를 사용하세요.
...
이 예시에서는 `acct-123`이 연결하는 `subaccount` 레코드를 조회하여 첫 번째 페이지의 결과를 반환합니다.
...
`anchor=target`을 사용하면 관련 오브젝트가 `from_custom_object`로 반환됩니다.
...
| `items[].to_custom_object` | 조건부 | 오브젝트 | `anchor=source`일 때 관련 오브젝트 |
| `items[].from_custom_object` | 조건부 | 오브젝트 | `anchor=target`일 때 관련 오브젝트 |
| `items[].to_custom_object.type_name` | 조건부 | 문자열 | 관련 오브젝트 유형 이름 |
| `items[].to_custom_object.external_id` | 조건부 | 문자열 | 관련 오브젝트 외부 ID |
| `items[].to_custom_object.attributes` | 조건부 | 오브젝트 | 관련 오브젝트 속성 |
| `items[].from_custom_object.type_name` | 조건부 | 문자열 | 관련 오브젝트 유형 이름 |
| `items[].from_custom_object.external_id` | 조건부 | 문자열 | 관련 오브젝트 외부 ID |
| `items[].from_custom_object.attributes` | 조건부 | 오브젝트 | 관련 오브젝트 속성 |
...
| 상태 | 원인 | 안내 |
|---|---|---|
| `400` | 잘못된 `anchor` | `anchor`에는 `source` 또는 `target`을 사용하세요. |
...
| `403` | API 키에 권한이 없거나 허용 목록에 의해 요청이 차단됨 | 키에 `custom_objects.read` 권한이 있고, 허용 목록이 구성된 경우 소스 IP가 키 허용 목록에 포함되어 있는지 확인하세요. |
