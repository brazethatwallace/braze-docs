...
다음 표는 `/custom_objects/types/{type_name}/object_relationship_types` 엔드포인트의 경로 매개변수를 나열하고 설명합니다.
...
다음 표는 `/custom_objects/types/{type_name}/object_relationship_types` 엔드포인트의 쿼리 매개변수를 나열하고 설명합니다.
...
이 섹션에서는 샘플 매개변수 페이로드와 샘플 cURL 요청을 포함합니다.

### 샘플 요청 페이로드 {#sample-request-payload}

요청 매개변수에 대한 참고로 이 JSON 오브젝트를 사용하세요.
...
이 예시는 `account` 유형이 관계의 소스일 때 `account` 유형에 사용할 수 있는 오브젝트 관계 종류를 나열합니다.
...
이 섹션에서는 샘플 성공 응답과 응답 필드를 포함합니다.
...
`related_type_name`은 선택한 `anchor`에 대해 관계의 반대쪽에 있는 유형입니다.

### 응답 매개변수 {#response-parameters}

다음 표는 성공 응답에 포함된 필드를 나열하고 설명합니다.

| 매개변수 | 필수 | 데이터 유형 | 설명 |
|---|---|---|---|
| `items` | 필수 | 배열 | 사용 가능한 오브젝트 관계 유형 목록 |
| `items[].from_type_name` | 필수 | 문자열 | 소스 커스텀 오브젝트 유형 이름 |
| `items[].to_type_name` | 필수 | 문자열 | 타겟 커스텀 오브젝트 유형 이름 |
| `items[].rel_kind` | 필수 | 문자열 | 관계 종류 값 |
| `items[].display_name` | 필수 | 문자열 | 관계 종류의 표시 레이블 |
| `items[].related_type_name` | 필수 | 문자열 | 요청된 `anchor`에 대한 반대쪽 유형 |
| `total_count` | 필수 | 정수 | 일치하는 레코드의 총 개수 |
| `has_more` | 필수 | 불리언 | 결과의 다음 페이지가 있는지 여부 |
...
| `401` | REST API 키가 없거나 유효하지 않음 | `Authorization` 헤더가 `Bearer YOUR_REST_API_KEY`를 사용하고 있으며 키가 활성 상태인지 확인하세요. |
