...
Campaign을 구성하려면 다음 단계를 수행하세요:

1. 메시지를 보낸 후 Campaigns 페이지에서 결과를 찾을 수 있도록 설명이 포함된 제목을 추가하세요.
2. **Add Message**를 선택하고 API Campaign에 포함할 메시지 유형을 추가하세요. 이렇게 하면 `campaign_id`와 메시지 배리언트 ID가 생성되며, 이 ID는 포함하는 각 채널마다 다릅니다.
3. 선택적으로 전환 이벤트를 추가하여 특정 액션 또는 Campaign 목표에 대한 사용자 전환을 추적할 수 있습니다.
4. **Save Campaign**을 선택하여 API Campaign을 시작하세요.
...
{% raw %}
기본적으로 Braze는 API Campaigns에 원클릭 목록 수신 거부 헤더를 추가하지 않습니다. `{{${set_user_to_one_click_list_unsubscribe}}}` Liquid 태그를 API 요청의 이메일 헤더 필드에 포함하면 개별 API Campaign 발송에 이 헤더를 추가할 수 있습니다.
{% endraw %}
...
{% alert note %}
이러한 헤더를 포함한다고 해서 이메일 클라이언트가 수신 거부 버튼을 표시한다는 보장은 없습니다. 이메일 클라이언트는 발송자 평판 및 메시지 콘텐츠와 같은 요소를 기반으로 수신 거부 옵션 표시 여부를 결정합니다.
{% endalert %}
