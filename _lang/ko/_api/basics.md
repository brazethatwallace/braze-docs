...
| [내보내기]({{site.baseurl}}/api/endpoints/export)                          | Campaigns, Canvases, 핵심 성과 지표(KPI) 등의 다양한 세부 정보에 접근하고 내보냅니다.   |
| [미디어 라이브러리]({{site.baseurl}}/api/endpoints/media_library)           | Braze 내 에셋을 관리합니다.                                                            |
| [메시지]({{site.baseurl}}/api/endpoints/messaging)                         | Campaigns와 Canvases를 예약하고, 전송하고, 관리합니다.                                  |
...
| [템플릿]({{site.baseurl}}/api/endpoints/templates)                         | 이메일 메시징 및 Content Blocks 템플릿을 생성하고 업데이트합니다.                        |
| [사용자 데이터]({{site.baseurl}}/api/endpoints/user_data)                   | 사용자를 식별하고, 추적하고, 관리합니다.                                                |
...
Braze는 대시보드 및 REST 엔드포인트를 위해 여러 개의 인스턴스를 관리합니다. 계정이 프로비저닝되면 다음 URL 중 하나에 로그인하게 됩니다. 프로비저닝된 인스턴스에 따라 올바른 REST 엔드포인트를 사용하세요. 확실하지 않은 경우 [지원 티켓]({{site.baseurl}}/braze_support)을 열거나 다음 표를 사용하여 사용 중인 대시보드의 URL을 올바른 REST 엔드포인트와 매칭하세요.

Braze에서 REST 엔드포인트를 찾으려면 다음과 같이 합니다.
...
대부분의 API에 대해 Braze는 시간당 250,000건의 기본 사용량 제한을 적용합니다. 그러나 특정 요청 유형에는 고객 기반 전체에서 대량 데이터를 보다 효과적으로 처리하기 위해 자체 사용량 제한이 적용됩니다. 자세한 내용은 [API 사용량 제한]({{site.baseurl}}/api/api_limits)을 참조하세요.

### 사용자 ID {#user-ids}

- **외부 사용자 ID**: `external_id`는 데이터를 제출하는 대상 사용자의 고유 식별자입니다. 이 식별자는 동일한 사용자에 대해 여러 프로필이 생성되는 것을 방지하기 위해 Braze SDK에서 설정한 것과 동일해야 합니다.
- **Braze 사용자 ID**: `braze_id`는 Braze가 설정하는 고유 사용자 식별자입니다. 이 식별자를 사용하여 external_id 외에 REST API를 통해 사용자를 삭제할 수 있습니다.
...
## REST API 키에 대해 {#about-rest-api-keys}

REST API(Application Programming Interface) 키는 API에 전달하여 API 호출을 인증하고 호출하는 애플리케이션 또는 사용자를 식별하는 고유 코드입니다. 회사의 REST API 엔드포인트에 대한 HTTPS 웹 요청을 사용하여 API에 접근합니다. REST API 키는 앱 식별자 키와 함께 작동하여 데이터를 추적, 접근, 전송, 내보내기 및 분석하여 모든 것이 원활하게 운영되도록 합니다.

워크스페이스와 API 키는 Braze에서 밀접하게 연결되어 있습니다. 워크스페이스는 여러 플랫폼에 걸쳐 동일한 애플리케이션의 버전을 관리하도록 설계되어 있습니다. 많은 고객이 동일한 플랫폼에서 무료 및 프리미엄 버전의 앱을 포함하기 위해 워크스페이스를 사용하기도 합니다. 이러한 워크스페이스도 REST API를 활용하고 있으며 각자의 REST API 키를 가지고 있습니다. 이 키는 API의 특정 엔드포인트에 대한 접근을 포함하도록 개별적으로 범위를 지정할 수 있습니다. API에 대한 각 호출에는 호출하는 엔드포인트에 대한 접근 권한이 있는 키가 포함되어야 합니다.

REST API 키와 워크스페이스 API 키를 모두 `api_key`라고 합니다. `api_key`는 각 요청에 요청 헤더로 포함되며 REST API를 사용할 수 있도록 인증 키 역할을 합니다. 이러한 REST API는 사용자를 추적하고, 메시지를 보내고, 사용자 데이터를 내보내는 등의 작업에 사용됩니다. 새로운 REST API 키를 만들 때 특정 엔드포인트에 대한 접근 권한을 부여해야 합니다. API 키에 특정 권한을 할당하면 API 키가 인증할 수 있는 호출을 정확히 제한할 수 있습니다.

![API 키 탭의 REST API 키 패널.]({% image_buster /assets/img_archive/rest-api-key.png %})

{% alert tip %}
REST API 키 외에도 API에서 앱, 템플릿, Canvases, Campaigns, Content Cards, Segments와 같은 특정 항목을 참조하는 데 사용할 수 있는 식별자 키라는 유형의 키도 있습니다. 자세한 내용은 [API 식별자 유형]({{site.baseurl}}/api/identifier_types)을 참조하세요.
{% endalert %}

### REST API 키 만들기 {#creating-rest-api-keys}

새로운 REST API 키를 만들려면:

1. **설정** > **API 및 식별자**로 이동합니다.
2. **API 키 만들기**를 선택합니다.
3. 새 키를 쉽게 식별할 수 있는 이름을 지정합니다.
4. 새 키에 대해 [허용된 IP 주소](#api-ip-allowlisting) 및 서브넷을 지정합니다.
5. 새 키에 연결할 [권한](#rest-api-key-permissions)을 선택합니다.

{% alert important %}
새 API 키를 만든 후에는 권한 범위나 허용된 IP를 편집할 수 없다는 점에 유의하세요. 이 제한은 보안상의 이유로 적용됩니다. 키의 범위를 변경해야 하는 경우 업데이트된 권한으로 새 키를 만들고 기존 키 대신 해당 키를 구현하세요. 구현을 완료한 후에는 이전 키를 삭제할 수 있습니다.
{% endalert %}

### REST API 키 권한 {#rest-api-key-permissions}

API 키 권한은 사용자 또는 그룹에 할당하여 특정 API 호출에 대한 접근을 제한할 수 있는 권한입니다. API 키 권한 목록을 보려면 **설정** > **API 및 식별자**로 이동하고 API 키를 선택합니다.
...
| `users.alias.new` | [`/users/alias/new`]({{site.baseurl}}/api/endpoints/user_data/post_user_alias) | 기존 사용자에 대한 새 별칭을 만듭니다. |
...
| `users.export.segment` | [`/users/export/segment`]({{site.baseurl}}/api/endpoints/export/user_data/post_users_segment) | Segment별 고객 프로필 정보를 쿼리합니다. |
| `users.merge` | [`/users/merge`]({{site.baseurl}}/api/endpoints/user_data/post_users_merge) | 기존 두 사용자를 하나로 병합합니다. |
...
| `campaigns.trigger.schedule.create` | [`/campaigns/trigger/schedule/create`]({{site.baseurl}}/api/endpoints/messaging/schedule_messages/post_schedule_triggered_campaigns) | API 트리거 전달을 통해 Campaign 전송을 예약합니다. |
...
| `campaigns.list` | [`/campaigns/list`]({{site.baseurl}}/api/endpoints/export/campaigns/get_campaigns) | Campaigns 목록을 쿼리합니다. |
| `campaigns.data_series` | [`/campaigns/data_series`]({{site.baseurl}}/api/endpoints/export/campaigns/get_campaign_analytics) | 시간 범위에 걸친 Campaign 분석을 쿼리합니다. |
...
| `sends.data_series` | [`/sends/data_series`]({{site.baseurl}}/api/endpoints/export/campaigns/get_send_analytics) | 시간 범위에 걸친 메시지 전송 분석을 쿼리합니다. |
| `sends.id.create` | [`/sends/id/create`]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_create_send_ids) | 메시지 일괄 발송 추적을 위한 전송 ID를 생성합니다. |
| `campaigns.url_info.details` | [`/campaigns/url_info/details`]({{site.baseurl}}) | Campaign 내 특정 메시지 배리언트의 URL 세부 정보를 쿼리합니다. |
| `transactional.send` | [`/transactional/v1/campaigns/{campaign_id}/send`]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_transactional_message) | 트랜잭션 메시지 엔드포인트를 사용하여 트랜잭션 메시지를 보낼 수 있습니다. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="REST API 키 권한" }
...
| `canvas.trigger.schedule.create` | [`/canvas/trigger/schedule/create`]({{site.baseurl}}/api/endpoints/messaging/schedule_messages/post_schedule_triggered_canvases) | API 트리거 전달을 통해 Canvas 전송을 예약합니다. |
...
| `canvas.data_series` | [`/canvas/data_series`]({{site.baseurl}}/api/endpoints/export/canvas/get_canvas_analytics) | 시간 범위에 걸친 Canvas 분석을 쿼리합니다. |
| `canvas.details` | [`/canvas/details`]({{site.baseurl}}/api/endpoints/export/canvas/get_canvas_details) | 특정 Canvas의 세부 정보를 쿼리합니다. |
| `canvas.data_summary` | [`/canvas/data_summary`]({{site.baseurl}}/api/endpoints/export/canvas/get_canvas_analytics_summary) | 시간 범위에 걸친 Canvas 분석 요약을 쿼리합니다. |
...
| `segments.data_series` | [`/segments/data_series`]({{site.baseurl}}/api/endpoints/export/segments/get_segment_analytics) | 시간 범위에 걸친 Segment 분석을 쿼리합니다. |
...
| `purchases.revenue_series` | [`/purchases/revenue_series`]({{site.baseurl}}/api/endpoints/export/purchases/get_revenue_series) | 시간 범위에 걸쳐 앱에서 하루에 지출된 총 금액을 쿼리합니다. |
| `purchases.quantity_series` | [`/purchases/quantity_series`]({{site.baseurl}}/api/endpoints/export/purchases/get_number_of_purchases) | 시간 범위에 걸쳐 앱에서 하루 총 구매 수를 쿼리합니다. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="REST API 키 권한" }
...
| `events.data_series` | [`/events/data_series`]({{site.baseurl}}/api/endpoints/export/custom_events/get_custom_events_analytics) | 시간 범위에 걸친 커스텀 이벤트 발생 횟수를 쿼리합니다. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="REST API 키 권한" }
...
| 권한 | 엔드포인트 | 설명 |
|---|---|---|
| `sessions.data_series` | [`/sessions/data_series`]({{site.baseurl}}/api/endpoints/export/sessions/get_sessions_analytics) | 시간 범위에 걸쳐 일별 세션 수를 쿼리합니다. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="REST API 키 권한" }
...
| 권한 | 엔드포인트 | 설명 |
|---|---|---|
| `kpi.dau.data_series` | [`/kpi/dau/data_series`]({{site.baseurl}}/api/endpoints/export/kpi/get_kpi_dau_date) | 시간 범위에 걸쳐 일별 고유 활성 사용자 수를 쿼리합니다. |
| `kpi.mau.data_series` | [`/kpi/mau/data_series`]({{site.baseurl}}/api/endpoints/export/kpi/get_kpi_mau_30_days) | 시간 범위에 걸쳐 30일 롤링 윈도우 기준 총 고유 활성 사용자 수를 쿼리합니다. |
| `kpi.new_users.data_series` | [`/kpi/new_users/data_series`]({{site.baseurl}}/api/endpoints/export/kpi/get_kpi_daily_new_users_date) | 시간 범위에 걸쳐 일별 신규 사용자 수를 쿼리합니다. |
| `kpi.uninstalls.data_series` | [`/kpi/uninstalls/data_series`]({{site.baseurl}}/api/endpoints/export/kpi/get_kpi_uninstalls_date) | 시간 범위에 걸쳐 일별 앱 삭제 수를 쿼리합니다. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="REST API 키 권한" }
...
| 권한 | 설명 |
| --- | --- |
| `sso.saml.login` | ID 제공자가 시작하는 로그인을 설정합니다. 자세한 내용은 [서비스 제공자(SP) 시작 로그인]({{site.baseurl}}/user_guide/administer/global/saml_single_sign_on/saml_sso_setup)을 참조하세요. |
...
| `sms.invalid_phone_numbers.remove` | [`/sms/invalid_phone_numbers/remove`]({{site.baseurl}}/api/endpoints/sms/post_remove_invalid_numbers) | 사용자에서 유효하지 않은 전화번호 플래그를 제거합니다. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="REST API 키 권한" }
...
| `catalogs.update_item` | [`/catalogs/{catalog_name}/items/{item_id}`]({{site.baseurl}}/api/endpoints/catalogs/catalog_items/synchronous/put_update_catalog_item) | 기존 카탈로그에서 단일 항목을 업데이트합니다. |
...
| `sdk_authentication.delete` | [`/app_group/sdk_authentication/delete`]({{site.baseurl}}/api/endpoints/sdk_authentication/delete_sdk_authentication_key) | 앱에 대한 SDK 인증 키를 삭제합니다. |
...
기존 REST API 키의 세부 정보를 확인하거나 삭제하려면 **설정** > **API 및 식별자** > **API 키** 탭으로 이동합니다. REST API 키는 생성 후에는 편집할 수 없다는 점에 유의하세요.
...
| 마지막 확인 | 이 키가 마지막으로 사용된 날짜입니다. 한 번도 사용되지 않은 키의 경우 이 필드는 "N/A"로 표시됩니다. |
{: .reset-td-br-1 .reset-td-br-2 aria-label="REST API 키 관리" }

API 키의 세부 정보를 보려면 키 위로 마우스를 가져간 후 <i class="fa-solid fa-eye" alt="보기"></i> **보기**를 선택합니다. 여기에는 이 키가 가진 모든 권한, 허용된 IP(있는 경우), 그리고 이 키가 Braze IP 허용 목록에 옵트인되어 있는지 여부가 포함됩니다.

![Braze 대시보드의 API 키 권한 목록.]({% image_buster /assets/img_archive/view-api-key.png %})

[사용자를 삭제]({{site.baseurl}}/user_guide/administer/global/user_management/manage_company_users)할 때 Braze는 해당 사용자가 생성한 관련 API 키를 삭제하지 않는다는 점에 유의하세요. 키를 삭제하려면 키 위로 마우스를 가져간 후 <i class="fa-solid fa-trash-can" alt="삭제"></i> **삭제**를 선택합니다.

![휴지통 아이콘이 강조 표시되어 삭제를 보여주는 'Last Seen'이라는 이름의 API 키.]({% image_buster /assets/img_archive/api-key-options.png %}){: style="max-width:30%;"}

### REST API 키 보안 {#rest-api-key-security}

API 키는 API 호출을 인증하는 데 사용됩니다. 새 REST API 키를 만들 때 특정 엔드포인트에 대한 접근 권한을 부여해야 합니다. API 키에 특정 권한을 할당하면 API 키가 인증할 수 있는 호출을 정확히 제한할 수 있습니다.

REST API 키는 잠재적으로 민감한 REST API 엔드포인트에 대한 접근을 허용하므로 이 키를 안전하게 보관하고 신뢰할 수 있는 파트너에게만 공유하세요. 키를 공개적으로 노출해서는 안 됩니다. 예를 들어 이 키를 사용하여 웹사이트에서 AJAX 호출을 하거나 기타 공개적인 방식으로 노출하지 마세요.

좋은 보안 관행은 사용자에게 업무를 완료하는 데 필요한 만큼의 접근 권한만 부여하는 것입니다. 이 원칙은 각 키에 권한을 할당하여 API 키에도 적용할 수 있습니다. 이러한 권한은 계정의 다양한 영역에 대해 더 나은 보안과 제어를 제공합니다.

{% alert warning %}
REST API 키는 잠재적으로 민감한 REST API 엔드포인트에 대한 접근을 허용하므로 안전하게 저장하고 사용해야 합니다. 예를 들어 이 키를 사용하여 웹사이트에서 AJAX 호출을 하거나 기타 공개적인 방식으로 노출하지 마세요.
{% endalert %}

키를 실수로 노출한 경우 개발자 콘솔에서 삭제할 수 있습니다. 이 프로세스에 대한 도움이 필요하면 [지원 티켓]({{site.baseurl}}/braze_support)을 열어 주세요.
...
| 가시성 | **비공개로 유지해야 합니다**. 클라이언트 측 코드, 공개 리포지토리 또는 사용자 애플리케이션에 절대 노출하지 마세요. | 공개용으로 설계되었습니다. 앱 바이너리에 포함되거나 웹 브라우저 JavaScript에서 볼 수 있으며 Google Analytics 추적 ID와 유사합니다. |
| 노출 시 해결 방법 | 즉시 키를 취소하고 **설정** > **API 및 식별자** > **API 키**에서 대체 키를 만듭니다. 노출된 REST API 키는 메시지 전송, 사용자 데이터 내보내기 또는 계정 설정 수정에 사용될 수 있습니다. | 별도 조치가 필요 없습니다. SDK API 키는 데이터를 수집하고 클라이언트 측 메시지(인앱 메시지 및 Content Cards 등)를 가져오는 것만 가능합니다. 사용자 데이터를 내보내거나, 사용자 대신 메시지를 보내거나, Campaigns를 수정할 수 없습니다. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="REST API 키와 SDK API 키의 보안" }

### API IP 허용 목록 {#api-ip-allowlisting}

보안을 한층 강화하기 위해 특정 REST API 키에 대해 REST API 요청을 수행할 수 있는 IP 주소 및 서브넷 목록을 지정할 수 있습니다. 이를 허용 목록 또는 화이트리스트라고 합니다. 특정 IP 주소 또는 서브넷을 허용하려면 새 REST API 키를 만들 때 **IP 화이트리스트** 섹션에 추가합니다:

![API 키 생성 시 IP를 허용 목록에 추가하는 옵션.]({% image_buster /assets/img_archive/api-key-ip-whitelisting.png %})

아무것도 지정하지 않으면 모든 IP 주소에서 요청을 보낼 수 있습니다.

{% alert tip %}
Braze 간 웹훅을 만들면서 허용 목록을 사용하는 경우 [화이트리스트에 추가할 IP]({{site.baseurl}}/user_guide/message_building_by_channel/webhooks/creating_a_webhook#ip-whitelisting) 목록을 참조하세요.
{% endalert %}
...
1. **토큰 유효성:** REST API 키가 Braze에 존재하며 활성 상태인지 확인합니다(예: 취소되거나 비활성화되지 않았는지).
...
HTTP 헤더 이름은 대소문자를 구분하지 않으므로 `Authorization`과 `authorization`은 동일합니다. `Content-Type`과 같은 다른 표준 요청 헤더에도 동일하게 적용됩니다. HTTP 클라이언트가 생성하는 대소문자 형식 그대로 전송하면 됩니다.

Braze는 `Bearer` 스킴의 모든 대소문자 형식(`Bearer`, `bearer` 또는 `BEARER`)도 허용합니다. REST API 키 자체는 발급된 그대로 정확히 전송하세요.

### 네트워크 수준 보안 {#network-level-security}

Braze로 보내는 REST API 요청은 전체 요청 경로에 걸쳐 전송 계층 보안(TLS) 암호화로 보호됩니다. 다음 표는 서버에서 Braze까지의 API 요청 네트워크 흐름을 설명합니다:
...
TLS 암호화는 체인의 모든 구간을 다룹니다. 사용자 서버는 TLS를 통해 Cloudflare에 연결하고, Cloudflare는 NLB를 거쳐 NGINX 인그레스까지 별도의 TLS 연결을 설정하므로, API 키와 요청 데이터는 전송 중에 암호화된 상태를 유지합니다.

## 추가 리소스 {#additional-resources}

### Ruby 클라이언트 라이브러리 {#ruby-client-library}

Ruby를 사용하여 Braze를 구현하는 경우, [Ruby 클라이언트 라이브러리](https://github.com/braze-inc/braze-api-client-ruby)를 사용하여 데이터 가져오기 시간을 단축할 수 있습니다. 클라이언트 라이브러리는 특정 프로그래밍 언어(이 경우 Ruby)에 특화된 코드 모음으로, API를 보다 쉽게 사용할 수 있도록 해줍니다.
