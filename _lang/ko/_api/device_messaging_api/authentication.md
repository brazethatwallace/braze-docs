...
클라이언트 측 REST API 키는 하나의 워크스페이스로 범위가 제한되며 Device Messaging API 권한으로 제한됩니다. 이러한 키는 클라이언트 애플리케이션에 임베드할 수 있습니다.
...
Device Messaging API 요청을 수행하기 전에 애플리케이션의 인가 제어를 적용하세요.

## 인증 오류 {#authentication-errors}

인증 및 권한 실패는 엔드포인트에 따라 다를 수 있습니다. 각 엔드포인트의 상태 코드 표와 [Device Messaging API 오류 처리]({{site.baseurl}}/api/device_messaging_api/error_handling)를 참조하세요.