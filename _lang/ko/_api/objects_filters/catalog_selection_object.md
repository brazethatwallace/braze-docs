...
## 오브젝트 세부 정보 {#object-details}

| 키 | 필수 | 데이터 유형 | 설명 |
| --- | -------- | --------- | ----------- |
| `name` | 필수 | 문자열 | 카탈로그 선택의 이름입니다. |
| `description` | 선택 사항 | 문자열 | 카탈로그 선택에 대한 설명입니다. |
| `external_id` | 필수 | 문자열 | 선택에 대한 고유 식별자입니다. |
| `source` | 선택 사항 | 문자열 | 카탈로그 데이터의 소스입니다. Shopify 카탈로그의 경우 `"Shopify"`로 설정합니다. 허용되는 값은 `"Shopify"`와 `"Braze"`입니다. |
| `filters` | 선택 사항 | 오브젝트 배열 | 카탈로그 항목에 적용할 필터 오브젝트의 배열입니다. 요청당 최대 네 개의 필터를 지정할 수 있습니다. 필터가 제공되지 않으면 카탈로그의 모든 항목이 포함됩니다. |
