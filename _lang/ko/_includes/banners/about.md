...
- **동적 개인화:** 배너는 앱에서 명시적으로 `requestBannersRefresh()`를 호출할 때 활성 세션 중에 새로고침하거나, `subscribeToBannersUpdates()`가 구현되어 있고 관련 배치 ID에 대해 `requestBannersRefresh()`가 이전에 한 번 이상 호출된 경우 새 세션 시작 시 자동으로 새로고침할 수 있습니다.
