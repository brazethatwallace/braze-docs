...
Braze MCP 서버는 읽기 및 쓰기 도구를 모두 포함합니다. 이 도구들은 Braze 고객 프로필에서 데이터를 반환하지 않습니다. 에이전트는 Braze 대시보드 사용자 권한을 상속합니다. 사용 가능한 도구의 전체 목록은 [사용 가능한 API 기능]{% if include.section == "user" %}({{site.baseurl}}/user_guide/brazeai/mcp_server/available_api_functions/){% elsif include.section == "developer" %}({{site.baseurl}}/developer_guide/mcp_server/available_api_functions/){% endif %}을 참조하세요.

{% alert warning %}
사용자 수준 PII를 노출하는 도구는 사용할 수 없습니다.
{% endalert %}

MCP 서버를 사용하여 Campaign 및 Canvas 성능에 대해 질문하고, Segments 및 커스텀 속성을 탐색하고, 보고서를 생성하고, 자연어를 통해 이메일 템플릿, Content Blocks, 미디어 라이브러리 자산 등의 콘텐츠를 생성할 수 있습니다.

## 베타 MCP 서버는 더 이상 사용되지 않나요? {#is-the-beta-mcp-server-deprecated}

네. 2025년 8월에 출시된 로컬 호스팅 MCP 서버는 더 이상 사용되지 않으며 추가 업데이트를 받지 않습니다. 계속 사용할 수 있지만, Braze는 원격 호스팅 버전으로 마이그레이션할 것을 권장합니다.

### 원격 서버는 어떻게 다른가요? {#how-is-the-remote-server-different}

이전 Braze MCP 서버는 로컬 머신에서 실행되었습니다. 패키지를 설치하고, 구성 파일을 관리하고, 적절한 권한이 있는 Braze API 키를 생성해야 했습니다. 원격 MCP 서버는 이러한 로컬 설정을 제거합니다.

지원되는 MCP 클라이언트에서 1분 이내에 연결할 수 있습니다. 인증은 OAuth를 사용합니다. 액세스는 공유 API 키가 아닌 Braze 대시보드 사용자 계정에 연결되므로, 에이전트가 보고 수행할 수 있는 작업은 대시보드 권한을 반영합니다. 대시보드 사용자가 Braze에서 액세스를 잃으면 클라이언트도 액세스를 잃습니다.

주요 차이점은 다음과 같습니다:

- **설정:** 패키지를 설치하고 구성 파일을 편집하는 대신 Braze URL을 붙여넣습니다.
- **인증:** API 키를 생성하는 대신 Braze 계정으로 로그인합니다.
- **권한:** API 키 권한 대신 대시보드 사용자 계정을 기반으로 액세스가 결정됩니다.
...
네. 대화별 또는 요청별로 워크스페이스를 지정할 수 있습니다. 하나의 연결로 액세스 권한이 있는 모든 워크스페이스를 사용할 수 있습니다.
...
에이전트 액세스는 대시보드 사용자에 따라 변경됩니다. 권한 변경은 다음 요청 시 적용됩니다. 비활성화된 대시보드 사용자는 MCP 액세스를 잃습니다.
