...
자연어 도구를 통해 Braze MCP 서버를 사용할 때 다음 팁을 참고하세요.

- 특히 여러 워크스페이스에 액세스할 수 있는 경우, 프롬프트에서 워크스페이스를 확인하세요.
- 분석을 요청할 때 날짜 범위와 측정기준을 구체적으로 지정하세요.
- 출력을 검증할 때 에이전트에게 어떤 도구를 사용했는지 확인하도록 요청하세요.
- 영향력이 큰 권장 사항은 Braze 대시보드의 소스 데이터와 비교하세요.

## 사용 예시 {#usage-examples}

[Braze MCP 서버를 설정]{% if include.section == "user" %}({{site.baseurl}}/user_guide/brazeai/mcp_server/setup/){% elsif include.section == "developer" %}({{site.baseurl}}/developer_guide/mcp_server/setup/){% endif %}한 후, 자연어를 통해 Braze와 상호작용할 수 있습니다. 다음은 시작하는 데 도움이 되는 예시입니다.
...
`사용 가능한 Braze 워크스페이스를 보여주고, 프로덕션 캠페인 분석에 어떤 워크스페이스를 사용해야 하는지 알려주세요.`
{% endtab %}
{% tab 응답 예시 %}
...
프로덕션 캠페인 분석 프롬프트에는 `Marketing - Production`을 사용하세요.
{% endtab %}
{% endtabs %}
...
### 지난주 캠페인 성과를 보여주세요 {#show-me-campaign-performance-for-last-week}

{% tabs %}
{% tab 프롬프트 예시 %}

`Marketing - Production 워크스페이스에서 지난 7일간의 캠페인 성과를 발송, 열람, 클릭, 상위 성과 캠페인을 포함하여 보여주세요.`
{% endtab %}
{% tab 응답 예시 %}

요청하신 워크스페이스와 기간에 대해 `get_campaign_list`와 `get_campaign_dataseries`를 사용했습니다.
...
- 열람 기준 상위 캠페인: `YOUR-TEST-CAMPAIGN-NAME`

필요하시면 캠페인별, 채널별 또는 일별로 세분화할 수 있습니다.
{% endtab %}
{% endtabs %}
...
`Marketing - Production 워크스페이스에서 "YOUR-TEST-TEMPLATE-NAME"이라는 이름의 온보딩 이메일 템플릿을 만들어 주세요.`
{% endtab %}
{% tab 응답 예시 %}
...
## 워크플로 예시 {#example-workflows}

이 예시들은 여러 도구를 결합하여 작업을 처음부터 끝까지 완료하는 방법을 보여줍니다.

### 여러 워크스페이스에 걸친 리포트 생성 {#report-across-multiple-workspaces}
...
`get_workspaces`를 호출하여 접근 권한을 확인한 후, 각 워크스페이스에 대해 지난 7일간의 `get_campaign_dataseries`를 실행했습니다.

- `US Prod`: `YOUR-TEST-SENDS` 발송, `YOUR-TEST-OPEN-RATE` 열람율
- `EU Prod`: `YOUR-TEST-SENDS` 발송, `YOUR-TEST-OPEN-RATE` 열람율

이번 주에는 `US Prod`에서 더 많은 메시지를 발송했고, `EU Prod`의 열람율이 더 높았습니다. 각 워크스페이스를 Campaign 또는 채널별로 세분화할 수 있습니다.
{% endtab %}
{% endtabs %}

### 스테이징에서 프로덕션으로 템플릿 복사 {#copy-templates-from-staging-to-production}
...
`get_email_templates`와 `get_email_template_info`를 사용하여 `Staging`의 템플릿을 읽은 후, `create_email_template`을 사용하여 `Production`에 각 템플릿을 다시 생성했습니다.
...
`get_email_template_info`에서 지원하지 않는 드래그 앤 드롭 편집기 템플릿은 건너뛰었습니다. 복사된 템플릿을 검토하길 원하시면 알려주세요.
{% endtab %}
{% endtabs %}
...
`get_campaign_list`와 `get_campaign_dataseries`를 사용하여 `Production`의 지난 7일간 활동 데이터를 가져왔습니다.
...
발송 수가 전주 대비 증가했습니다. 채널별 세분화를 추가하거나 인게이지먼트가 감소하고 있는 Campaign을 표시할 수 있습니다.
{% endtab %}
{% endtabs %}
...
1. **클라이언트에 프롬프트를 입력합니다.** 지난주 Campaign 성과를 요청하는 것처럼 자연어로 요청을 입력합니다.
...
4. **Braze가 결과를 반환합니다.** 서버가 데이터를 클라이언트로 다시 전송하면, 클라이언트가 이를 포맷하여 사용자에게 표시합니다.

사용자의 접근 권한은 두 가지의 교집합으로 결정됩니다.

- **연결 인증 시 부여된 스코프**(예: `mcp:tools`).
- **사용자 본인의 대시보드 권한.** 대시보드에서 Campaigns를 조회할 수 없다면 에이전트도 조회할 수 없습니다. 이메일 템플릿을 생성할 수 있다면 에이전트도 생성할 수 있습니다. 에이전트는 사용자 본인의 접근 권한을 초과할 수 없습니다.
