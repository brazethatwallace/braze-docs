...
| 지원되는 MCP 클라이언트 | OAuth를 지원하는 원격 MCP 서버와 호환되는 모든 클라이언트를 사용할 수 있습니다. Braze는 Claude, ChatGPT, Cursor, OpenAI Codex, Claude Code, Visual Studio Code에서 검증을 완료했습니다. |
| Braze 대시보드 계정 | 회사에서 SSO 또는 SAML을 사용하는 경우를 포함하여, 일반 Braze 자격 증명으로 로그인합니다. 별도의 MCP 로그인은 없습니다. |
| 서버 엔드포인트 선택 | `https://mcp.braze.com/mcp`(US) 또는 `https://mcp.braze.eu/mcp`(EU)를 선택하세요. 두 엔드포인트 모두 모든 Braze 클러스터에 접근할 수 있습니다. |
| IP 허용 목록 미사용 | [IP 허용 목록](https://www.braze.com/docs/user_guide/administer/global/admin_settings/security_settings#dashboard-ip-allowlisting)을 사용하는 고객은 현재 얼리 액세스에 참여할 수 없습니다. |
{: .reset-td-br-1 .reset-td-br-2 aria-label="필수 조건" }

{% alert note %}
에이전트의 접근 권한은 대시보드 권한과 동일합니다. 대시보드 접근 권한이 전체 워크스페이스가 아닌 특정 팀으로 제한되어 있는 경우, 얼리 액세스 기간 동안 일부 도구가 작동하지 않을 수 있습니다.
{% endalert %}
...
관리자에게 이 권한이 표시되지 않는 경우, Braze 계정 매니저에게 연락하여 얼리 액세스 등록을 요청하세요.
...
### 1단계: 권한 및 워크스페이스 접근 권한 확인 {#step-1-confirm-permissions-and-workspace-access}
...
에이전트가 처음으로 Braze 도구를 호출하면, 클라이언트가 브라우저 창을 열어 Braze 로그인 페이지로 이동시킵니다.
...
![Claude Desktop이 Braze 계정 정보 및 Braze 데이터에 대한 광범위한 접근을 요청하고 있음을 보여주는 Braze 동의 화면. 개인정보 보호정책 동의 확인란과 Cancel 및 Continue 버튼이 표시되어 있습니다.]({% image_buster /assets/img/mcp_server/oauth_consent_screen.png %}){: style="max-width:65%;"}
...
설정이 완료되면 다음과 같은 간단한 검증 프롬프트를 전송하세요:
...
클라이언트를 연결하는 데는 몇 단계만 필요합니다. 다음 안내에서는 Claude를 사용하지만, 다른 지원 클라이언트에서도 흐름은 유사합니다.
...
5. SSO를 사용하는 경우를 포함하여, 평소 사용하는 자격 증명으로 Braze에 로그인합니다. 동일한 클러스터에서 둘 이상의 회사에 접근할 수 있는 로그인인 경우, 사용할 회사를 선택합니다.
6. 동의 화면에서 요청된 접근 권한을 검토하고, 확인 체크박스를 선택한 다음 **Continue**를 선택합니다. Claude가 채팅으로 돌아가며, 이제 에이전트가 Braze 도구를 사용할 수 있습니다.
...
얼리 액세스 기간 동안 Braze는 보안을 위해 지원되는 클라이언트 도메인의 허용 목록을 유지합니다. 허용 목록에 없는 클라이언트에서 연결하면 인증이 차단될 수 있습니다. 사용 중인 클라이언트가 지원되어야 한다고 생각되면 [mcp-product@braze.com](mailto:mcp-product@braze.com)으로 문의하세요.

Claude Code 및 OpenAI Codex와 같이 커스텀 스킴 없이 로컬 머신에서 실행되는 클라이언트도 작동합니다.

### 클라이언트에 도구가 표시되지 않는 경우 {#tools-dont-appear-in-your-client}

에이전트가 Braze 도구를 나열할 수 없는 경우, 몇 분 기다린 후 다시 시도하세요. 이러한 문제는 대부분 일시적이며 자동으로 해결됩니다.

문제가 계속되면 비디오를 녹화하여 [mcp-product@braze.com](mailto:mcp-product@braze.com)으로 보내 주세요.

### 에이전트가 예상 도구에 접근할 수 없는 경우 {#agent-cannot-access-expected-tools}

1. 대시보드 사용자에게 필요한 권한이 있는지 확인합니다. 에이전트는 사용자 본인의 대시보드 접근 권한과 일치하는 도구만 사용할 수 있습니다.
...
계정이 둘 이상의 워크스페이스에 접근할 수 있는 경우, Braze 대시보드에 표시된 정확한 이름을 사용하여 프롬프트에서 워크스페이스를 지정하세요. 워크스페이스를 지정하지 않으면 에이전트가 확인을 요청하거나 예상치 못한 워크스페이스를 사용할 수 있습니다.
...
회사는 처음 인증할 때 설정됩니다. 동일한 클러스터에서 다른 회사로 작업하려면 클라이언트에서 Braze 커넥터를 연결 해제하고, 다시 인증한 후 로그인 중에 다른 회사를 선택하세요.
