...
MCP 클라이언트는 이 도구들을 참조하여 Braze MCP 서버와 상호작용합니다.

### 워크스페이스 {#workspaces}

| 도구 | API 엔드포인트 | 접근 | 설명 |
| --- | --- | --- | --- |
| `get_workspaces` | N/A | read | 현재 OAuth 액세스 토큰이 접근할 수 있는 Braze 워크스페이스를 확인합니다. 이 도구를 먼저 호출하세요. 반환되는 각 워크스페이스 `id`가 다른 모든 도구에서 필요한 `app_group_id`입니다. |
...
| `get_campaign_dataseries` | [`/campaigns/data_series`]({{site.baseurl}}/api/endpoints/export/campaigns/get_campaign_analytics) | read | 시간 경과에 따른 Campaign 통계의 일별 시리즈를 조회합니다(채널별 전송, 열람, 클릭, 전환). |
...
| `launch_campaign`<sup>*</sup> | N/A | update | Campaign을 실행합니다. |
| `stop_campaign`<sup>*</sup> | N/A | update | 실행 중인 Campaign을 중지합니다. |
| `archive_campaign`<sup>*</sup> | N/A | update | Campaign을 보관합니다. |
| `unarchive_campaign`<sup>*</sup> | N/A | update | Campaign의 보관을 해제합니다. |
| `get_campaign_draft`<sup>*</sup> | N/A | read | Campaign 초안 세부 정보를 조회합니다. |
| `get_campaign_live_details`<sup>*</sup> | N/A | read | 실시간 Campaign 세부 정보를 조회합니다. |
| `create_campaign_message`<sup>*</sup> | N/A | create | Campaign 내 메시지를 생성합니다. |
...
| `create_campaign_message_variation`<sup>*</sup> | N/A | create | Campaign 내 메시지 배리언트를 생성합니다. |
| `update_campaign_message_variation`<sup>*</sup> | N/A | update | Campaign 메시지 배리언트를 업데이트합니다. |
| `delete_campaign_message_variation`<sup>*</sup> | N/A | delete | Campaign 메시지 배리언트를 삭제합니다. |
...
<sup>*</sup> 이 도구는 Campaign API 베타 프로그램에 참여 중인 고객만 사용할 수 있습니다. 계정에 이 기능이 활성화되어 있지 않으면 사용 시 오류가 발생할 수 있습니다. 베타 프로그램에 참여하려면 계정 매니저에게 문의하세요.
{: .reset-td-br-1 }
...
| `get_canvas_details` | [`/canvas/details`]({{site.baseurl}}/api/endpoints/export/canvas/get_canvas_details) | read | Canvas 메타데이터를 내보냅니다: 이름, 생성 시간, 현재 상태 등. |
...
| `get_canvas_data_summary` | [`/canvas/data_summary`]({{site.baseurl}}/api/endpoints/export/canvas/get_canvas_analytics_summary) | read | Canvas 시계열 데이터의 요약 롤업을 내보냅니다. |
...
| 도구 | API 엔드포인트 | 접근 | 설명 |
| --- | --- | --- | --- |
| `get_catalogs` | [`/catalogs`]({{site.baseurl}}/api/endpoints/catalogs/catalog_management/synchronous/get_list_catalogs) | read | 워크스페이스의 카탈로그 목록을 조회합니다. |
...
| `replace_catalog_items` | [`/catalogs/{catalog_name}/items`]({{site.baseurl}}/api/endpoints/catalogs/catalog_items/asynchronous/put_update_catalog_items) | update | 카탈로그의 여러 항목을 교체합니다. 항목이 존재하지 않으면 새로 생성합니다. 요청당 최대 50개 항목. |
...
| `create_catalog_selection` | [`/catalogs/{catalog_name}/selections`]({{site.baseurl}}/api/endpoints/catalogs/catalog_selections/asynchronous/post_create_catalog_selections) | create | 카탈로그에 셀렉션을 생성합니다. |
| `delete_catalog_selection` | [`/catalogs/{catalog_name}/selections/{selection_name}`]({{site.baseurl}}/api/endpoints/catalogs/catalog_selections/asynchronous/delete_catalog_selection) | delete | 카탈로그 셀렉션을 삭제합니다. |
...
| `get_events_data_series` | [`/events/data_series`]({{site.baseurl}}/api/endpoints/export/custom_events/get_custom_events_analytics) | read | 지정된 기간 동안 커스텀 이벤트의 발생 횟수를 조회합니다. |
...
| 도구 | API 엔드포인트 | 접근 | 설명 |
| --- | --- | --- | --- |
| `list_integrations` | [`/cdi/integrations`]({{site.baseurl}}/api/endpoints/cdi/get_integration_list) | read | 기존 클라우드 데이터 수집 통합 목록을 조회합니다. 호출당 10개. |
| `get_integration_job_sync_status` | [`/cdi/integrations/{integration_id}/job_sync_status`]({{site.baseurl}}/api/endpoints/cdi/get_job_sync_status) | read | 지정된 CDI 통합의 과거 동기화 상태를 조회합니다. 호출당 10개. |
...
| 도구 | API 엔드포인트 | 접근 | 설명 |
| --- | --- | --- | --- |
| `create_media_library_asset` | [`/media_library/create`]({{site.baseurl}}/api/endpoints/media_library/manage_assets/create) | create | 외부 URL 또는 base64 파일 콘텐츠를 통해 Braze 미디어 라이브러리에 에셋을 업로드합니다. 정확히 하나의 업로드 모드를 제공해야 합니다. |
...
| `get_segment_data_series` | [`/segments/data_series`]({{site.baseurl}}/api/endpoints/export/segments/get_segment_analytics) | read | 시간 경과에 따른 Segment 예상 규모의 일별 시리즈를 조회합니다. |
...
<sup>*</sup> 이 도구는 Segment API 베타 프로그램에 참여 중인 고객만 사용할 수 있습니다. 계정에 이 기능이 활성화되어 있지 않으면 사용 시 오류가 발생할 수 있습니다. 베타 프로그램에 참여하려면 계정 매니저에게 문의하세요.
{: .reset-td-br-1 }
...
| 도구 | API 엔드포인트 | 접근 | 설명 |
| --- | --- | --- | --- |
| `get_send_data_series` | [`/sends/data_series`]({{site.baseurl}}/api/endpoints/export/campaigns/get_send_analytics) | read | 추적된 `send_id`에 대한 일별 통계를 조회합니다(API Campaign). Braze는 전송 후 14일 동안 전송 분석 데이터를 보관합니다. |
...
| 도구 | API 엔드포인트 | 접근 | 설명 |
| --- | --- | --- | --- |
| `get_content_blocks` | [`/content_blocks/list`]({{site.baseurl}}/api/endpoints/templates/content_blocks_templates/get_list_email_content_blocks) | read | 기존 콘텐츠 블록 정보를 조회합니다. |
| `get_content_block_info` | [`/content_blocks/info`]({{site.baseurl}}/api/endpoints/templates/content_blocks_templates/get_see_email_content_blocks_information) | read | 기존 콘텐츠 블록의 정보를 가져옵니다. 선택적으로 Campaign 또는 Canvas 포함 데이터도 조회할 수 있습니다. |
...
| 도구 | API 엔드포인트 | 접근 | 설명 |
| --- | --- | --- | --- |
| `send_operator_prompt` | N/A | update | BrazeAI Operator에 자연어 프롬프트를 전송합니다. 백그라운드 작업을 제출하고 job_id를 반환합니다. |
| `get_operator_result` | N/A | read | job_id를 사용하여 제출된 Operator 작업의 결과를 폴링합니다. |
| `cancel_operator_job` | N/A | update | 실행 중인 Operator 작업을 취소합니다. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 .reset-td-br-4 aria-label="Operator" }

{% alert important %}
이 도구들은 Operator 베타 프로그램에 참여 중인 고객만 사용할 수 있습니다. 계정에 이 기능이 활성화되어 있지 않으면 사용 시 오류가 발생할 수 있습니다. 베타 프로그램에 참여하려면 계정 매니저에게 문의하세요.
{% endalert %}
