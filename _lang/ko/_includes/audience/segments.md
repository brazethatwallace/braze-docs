...
정기적으로 확장을 새로고침할 필요가 없는 경우 새로고침 설정을 사용하지 않고 저장할 수 있으며, Braze는 해당 시점의 사용자 멤버십을 기반으로 세그먼트 확장을 생성하는 기본값 동작을 적용합니다. 오디언스를 한 번만 생성한 다음 일회성 Campaign으로 타겟팅하려는 경우 기본값 동작을 사용하세요.
...
- 매주 월요일 오전 11시(회사 시간 기준)에 발송되는 이메일 Campaign이 있고, 발송 직전에 세그먼트를 새로고침하려면 매주 월요일 오전 10시로 새로고침 일정을 선택해야 합니다.
