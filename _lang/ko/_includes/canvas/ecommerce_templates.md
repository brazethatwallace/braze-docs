...
![적용된 "유기한 탐색" 캔버스 템플릿과 확장된 "진입 규칙".]({% image_buster /assets/img_archive/abandoned_browse.png %})

#### 설정

캔버스 페이지에서 **캔버스 템플릿 사용** > **Braze 템플릿**을 선택한 다음 **유기한 탐색** 템플릿을 적용합니다. 
...
- 진입 스케줄 
    ...
    - 시작 시간은 캔버스 템플릿을 생성하는 시점입니다<br><br>![캔버스의 "액션 기반 옵션".]({% image_buster /assets/img/ecommerce/abandoned_browse_entry.png %})<br><br> 
- 타겟 오디언스 
    - 진입 오디언스 
        - 이메일이 **비어 있지 않음**
        - 비즈니스 요구 사항에 맞게 진입 오디언스 기준을 수정할 수도 있습니다
    - 진입 제어
        - 캔버스의 전체 기간이 완료된 후 사용자가 이 캔버스에 다시 진입할 수 있습니다
    - 종료 기준 
        - `ecommerce.cart_updated`, `ecommerce.checkout_started` 또는 `ecommerce.order_placed` 수행<br><br>![캔버스의 진입 제어 및 종료 기준.]({% image_buster /assets/img/ecommerce/abandoned_browse_entry_exit.png %})<br><br> 
- 발송 설정 
    - 가입했거나 옵트인한 사용자 
- 지연 단계
    - 1시간 지연
...
![적용된 "유기한 장바구니" 캔버스 템플릿과 확장된 "진입 규칙".]({% image_buster /assets/img_archive/abandoned_cart.png %})

#### 설정

캔버스 페이지에서 **캔버스 템플릿 사용** > **Braze 템플릿**을 선택한 다음 **유기한 장바구니** 템플릿을 적용합니다. 
...
- 기본 사항 
    ...
    - 전환 이벤트: `ecommerce.order_placed`
        - 전환 마감일: 3일 
- 진입 스케줄 
    - 사용자가 **장바구니 업데이트 이벤트 수행**(드롭다운에 위치)을 트리거할 때 액션 기반 트리거
    - 시작 시간은 캔버스 템플릿을 생성하는 시점입니다<br><br>![캔버스의 "액션 기반 옵션".]({% image_buster /assets/img/ecommerce/abandoned_cart_entry.png %})<br><br> 
- 타겟 오디언스 
    - 진입 오디언스 
        - 이 앱을 **0회 이상** 사용한 적이 있음 
        - 이메일이 **비어 있지 않음**
    - 진입 제어
        - 사용자가 즉시 캔버스 진입 자격을 다시 얻음
    - 종료 기준 
        - `ecommerce.cart_updated`, `ecommerce.checkout_started` 또는 `ecommerce.order_placed` 수행<br><br>![캔버스의 진입 제어 및 종료 기준.]({% image_buster /assets/img/ecommerce/abandoned_cart_entry_exit.png %})<br><br> 
- 발송 설정 
    - 가입했거나 옵트인한 사용자 
- 지연 단계
     - 4시간 지연
...
1. 사용자가 장바구니에 항목을 추가하면 캔버스에 진입합니다.
2. 항목을 추가하거나 업데이트할 때마다 캔버스에 재진입합니다—이를 통해 장바구니 데이터와 메시징이 최신 상태로 유지됩니다.
3. 사용자가 결제 프로세스를 시작하면 장바구니에 `checkout_started` 태그가 붙고 캔버스를 종료합니다.
4. 동일한 장바구니 ID를 사용하는 이후 장바구니 업데이트는 재진입을 트리거하지 않습니다. 이 장바구니가 이미 결제 단계로 이동했기 때문입니다.

사용자가 결제 사용자 여정으로 이동하면 대신 [유기한 결제 캔버스](#abandoned-checkout)의 타겟이 됩니다. 이 캔버스는 구매 여정에서 더 진행된 사용자를 위해 설계되었습니다.
...
![적용된 "유기한 결제" 캔버스 템플릿과 확장된 "진입 규칙".]({% image_buster /assets/img_archive/abandoned_checkout.png %})

#### 설정

캔버스 페이지에서 **캔버스 템플릿 사용** > **Braze 템플릿**을 선택한 다음 **유기한 결제** 템플릿을 적용합니다. 
...
- 진입 스케줄 
    ...
    - 시작 시간은 캔버스 템플릿을 생성하는 시점입니다<br><br>![캔버스의 "액션 기반 옵션".]({% image_buster /assets/img/ecommerce/abandoned_checkout_entry.png %})
- 타겟 오디언스 
    - 진입 오디언스 
        - 이 앱을 **0회 이상** 사용한 적이 있음 
        - 이메일이 **비어 있지 않음**
    - 진입 제어
        - 사용자가 즉시 캔버스 진입 자격을 다시 얻음
        - 종료 기준 
            - `ecommerce.order_placed` 이벤트 수행<br><br>![캔버스의 진입 제어 및 종료 기준.]({% image_buster /assets/img/ecommerce/abandoned_checkout_entry_exit.png %})<br><br>
- 발송 설정 
    - 가입했거나 옵트인한 사용자 
- 지연 단계
    - 4시간 지연
...
![적용된 "주문 확인" 캔버스 템플릿과 확장된 "진입 규칙".]({% image_buster /assets/img_archive/order_confirmation_feedback.png %})

#### 설정

캔버스 페이지에서 **캔버스 템플릿 사용** > **Braze 템플릿**을 선택한 다음 **주문 확인 & 피드백 설문조사** 템플릿을 적용합니다. 
...
- 기본 사항 
    - 캔버스 이름: **피드백 설문조사가 포함된 주문 확인**
    - 전환 이벤트: `ecommerce.session_start`
        - 전환 마감일: 10일 
- 진입 스케줄 
    - 사용자가 `ecommerce.cart_updated` 이벤트를 수행할 때 액션 기반 트리거
    - 시작 시간은 캔버스 템플릿을 생성하는 시점입니다<br><br>![캔버스의 "액션 기반 옵션".]({% image_buster /assets/img/ecommerce/feedback_entry.png %})<br><br>
- 타겟 오디언스 
    - 진입 오디언스 
        - 이 앱을 **0회 이상** 사용한 적이 있음 
        - 이메일이 **비어 있지 않음**
    - 진입 제어
        - 사용자가 즉시 캔버스 진입 자격을 다시 얻음
    - 종료 기준 
        - 해당 없음<br><br>![캔버스의 추가 필터 및 진입 제어.]({% image_buster /assets/img/ecommerce/feedback_entry_exit.png %})<br><br>
- 발송 설정 
    - 가입했거나 옵트인한 사용자 
- 메시지 단계 
    - 이메일 템플릿과 HTML 블록을 Liquid 템플릿 예시와 함께 검토하여 미리 작성된 템플릿의 메시지에 제품을 추가하세요. 자체 이메일 템플릿을 사용하는 경우 다음 섹션에서 설명하는 것처럼 [Liquid 변수](#message-personalization)를 참조할 수도 있습니다.
