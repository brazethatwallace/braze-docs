...
#### 추정 오디언스와 현재 오디언스 {#estimated-audience-and-current-audience}

워크스페이스의 크기에 따라 **캠페인 세부 정보** 패널에서 오디언스 통계가 **추정 오디언스** 또는 **현재 오디언스**로 표시될 수 있습니다.

다음 표에서는 각 레이블이 사용되는 시점과 의미를 설명합니다.

| 하단 레이블 | 사용 시점 |
| --- | --- |
| **추정 오디언스** | Braze는 기본적으로 전체 데이터베이스 카운트를 실행하지 않습니다. 오디언스 크기는 샘플에서 추정되어 외삽되며, Segment 빌더의 **도달 가능 사용자** 범위와 유사합니다. 특히 대규모 워크스페이스나 워크스페이스 대비 작은 Segment의 경우 오차 범위가 예상됩니다. |
...
#### 마지막 확인 이후 변경 사항 {#changes-since-last-viewed}
...
-->
{% if include.channel == "Content Card" %}
### 콘텐츠 카드 성과 {#content-card-performance}
...
{% elsif include.channel == "in-app message" %}
### 인앱 메시지 성과 {#in-app-message-performance}
...
{% elsif include.channel == "SMS" %}
### SMS/MMS/RCS 성과 {#smsmmsrcs-performance}
...
{% elsif include.channel == "KakaoTalk" %}
### KakaoTalk 성과 {#kakaotalk-performance}
...
{% elsif include.channel == "whatsapp" %}
### WhatsApp 성과 {#whatsapp-performance}
...
#### 고유 수신자 대 고유 노출 수 {#unique-recipients-versus-unique-impressions}
...
_전송된 메시지 수_는 볼 수 있도록 제공된 Content Cards를 의미하고, _고유 수신자_는 실제로 본 Content Cards를 의미합니다.
...
추정 실제 열람율은 Campaigns에서만 사용할 수 있으며, Currents 이벤트에서는 보고되지 않습니다. 이 측정기준은 2023년 11월 14일 이전에 시작된 활성 캠페인에 대해서만 소급하여 계산됩니다.
...
![앱에 대해 "백그라운드 또는 포그라운드 푸시 활성화" 필터가 false로 설정되고 "제거됨" 필터가 선택된 Segment 빌더 섹션.]({% image_buster /assets/img/push_unsub_segment_example.png %})
