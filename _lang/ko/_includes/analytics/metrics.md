...
<i>하드바운스</i>는 영구적인 전달 오류로 인해 이메일이 수신자에게 전달되지 못하는 경우를 말합니다. 도메인 이름이 존재하지 않거나 수신자를 알 수 없기 때문에 하드바운스가 발생할 수 있습니다.
{% endif %}
...
<i>연기</i>란 이메일이 즉시 전달되지 않았지만, 특정 Campaign에 대한 시도가 중지되기 전에 전달 성공 가능성을 극대화하기 위해 일시적인 전달 실패 후 최대 72시간 동안 Braze가 이메일을 재시도하는 것을 말합니다.
{% endif %}
...
<i>본문 클릭</i>은 사용자가 버튼(버튼 1, 버튼 2)이 없고 기존 편집기로 작성된 메시지를 클릭하는 경우와, HTML 편집기 또는 드래그 앤 드롭 편집기를 사용하여 작성된 메시지에서 인수 없이 <code>brazeBridge.logClick()</code>을 사용하는 경우에 발생합니다.
{% endif %}
...
<i>메시지 닫기</i> 횟수는 메시지의 닫기 버튼을 클릭한 총 횟수입니다. 이는 기존 편집기가 아닌 드래그 앤 드롭 편집기에서 만든 인앱 메시지에만 존재합니다.
{% endif %}
...
<i>전환율</i>은 메시지의 모든 수신자 대비 정의된 이벤트가 발생한 횟수의 백분율입니다. 이 정의된 이벤트는 Campaign을 구축할 때 결정됩니다.
{% endif %}
...
<i>전환(B, C, D)</i>은 주요 전환 이벤트 이후에 추가된 추가 전환 이벤트입니다. Braze Campaign에서 수신한 메시지와 상호작용하거나 메시지를 본 후 정의된 이벤트가 발생한 횟수입니다.
{% endif %}

{% if include.metric == "Total Conversions" %}
<i>총 전환</i> 수는 사용자가 인앱 메시지 Campaign을 본 후 특정 전환 이벤트를 완료한 총 횟수입니다.
{% endif %}
...
{% if include.metric == "Deliveries %" %}
<i>전달 %</i>는 이메일 수신 가능 대상에게 성공적으로 전송된 총 메시지 수(전송)의 백분율입니다.
{% endif %}
...
<i>전송 실패율</i>은 메시지를 보낼 수 없어 실패한 전송의 백분율입니다. 이는 대기줄 오버플로, 계정 일시 정지, MMS의 경우 미디어 오류 등 다양한 이유로 발생할 수 있습니다.
{% endif %}
...
<i>이메일 수신 가능</i>은 이메일 주소가 기록되어 있고 명시적으로 옵트인 또는 구독을 신청한 총 사용자 수입니다.
{% endif %}
...
<i>일일 평균 매출</i>은 해당 일의 Campaign과 Canvas 매출의 합계를 평균한 값입니다.
{% endif %}
...
<i>읽음</i>은 사용자가 메시지를 읽은 경우입니다. Braze가 읽음을 추적하려면 사용자의 읽음 확인이 "켜짐" 상태여야 합니다.
{% endif %}

{% if include.metric == "Read Rate" %}
<i>읽음률</i>은 읽음으로 이어진 전송의 백분율입니다. 이 지표는 읽음 확인을 켠 사용자에게만 제공됩니다.
{% endif %}
...
<i>매출</i>은 설정된 <a href='/docs/user_guide/engagement_tools/campaigns/building_campaigns/conversion_events'>주요 전환 기간</a> 내에 Campaign 수신자로부터 발생한 총 수익(달러)입니다.
{% endif %}

{% if include.metric == "Messages Sent" %}
<i>보낸 메시지</i> 수는 Campaign에서 보낸 총 메시지 수입니다. 예약된 Campaign을 시작한 후, 이 측정기준에는 전송된 모든 메시지가 포함되며, 사용량 제한으로 인해 아직 전송되지 않은 메시지도 포함됩니다. 이는 메시지가 기기에 수신 또는 전달되었음을 의미하는 것이 아니라 메시지가 전송되었다는 의미입니다.
{% endif %}
...
<i>전송</i> 수는 Campaign에서 보낸 총 메시지 수입니다. 예약된 Campaign을 시작한 후, 이 측정기준에는 전송된 모든 메시지가 포함되며, 사용량 제한으로 인해 아직 전송되지 않은 메시지도 포함됩니다. 이는 메시지가 기기에 수신 또는 전달되었음을 의미하는 것이 아니라 메시지가 전송되었다는 의미입니다.
{% endif %}
...
<i>총 매출</i>은 설정된 주요 전환 기간 내에 Campaign 수신자로부터 발생한 총 매출(달러)입니다.
{% endif %}
...
<i>고유 닫기</i> 횟수는 Campaign에서 Content Cards를 닫은 고유 수신자 수입니다. 사용자가 Campaign에서 Content Cards를 여러 번 닫는 것은 하나의 고유 닫기로 계산됩니다.
{% endif %}
...
<i>수신 거부자</i> 또는 <i>수신 취소</i>는 수신 거부로 이어진 메시지 수입니다. 수신 거부는 Braze가 메시지 본문의 Braze 수신 거부 URL 또는 Braze가 처리하는 list-unsubscribe 헤더를 통해 수신 거부를 처리할 때 발생합니다.
{% endif %}

{% if include.metric == "Unsubscribes" %}
<i>구독 취소</i>는 메시지 본문의 Braze 수신 거부 URL 및 Braze가 요청을 처리하는 list-unsubscribe를 포함한 Braze 관리 수신 거부 경로를 통해 구독 상태가 수신 거부로 변경된 수신자 수입니다.
{% endif %}
