...
excerpt_separator: ""
page_type: glossary
description: "이 용어집은 Braze가 추적하고 커런츠를 사용하여 선택한 데이터 웨어하우스로 전송할 수 있는 고객 프로필 업데이트를 나열합니다."
tool: Currents
search_rank: 7
---

{% alert tip %}
이러한 이벤트는 [퀴리 빌더]({{site.baseurl}}/user_guide/analytics/query_builder/), [SQL 세그먼트 확장]({{site.baseurl}}/user_guide/engagement_tools/segments/sql_segments/), [Snowflake 데이터 공유]({{site.baseurl}}/partners/data_and_analytics/data_warehouses/snowflake/)에서 SQL 테이블로도 사용할 수 있습니다. SQL 테이블 스키마 및 열 세부 정보는 [SQL 테이블 참조]({{site.baseurl}}/user_guide/engagement_tools/segments/segment_extension/sql_segments/sql_segments_tables/)를 확인하세요.
{% endalert %}
...
이 고객 행동 및 사용자 이벤트 분석은 고객 프로필 업데이트 이벤트에 일반적으로 포함되는 정보 유형을 보여줍니다. 구성요소를 확실히 이해하면 개발자와 비즈니스 인텔리전스 전략 팀이 수신되는 커런츠 이벤트 데이터를 활용하여 데이터 중심 보고서와 차트를 만들고, 기타 유용한 데이터 측정기준을 활용할 수 있습니다.
...
커런츠는 페이로드가 900KB보다 큰 이벤트를 삭제합니다.
{% endalert %}