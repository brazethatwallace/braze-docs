...
**빌드** 섹션에서 블록을 드래그하여 드래그 앤 드롭 환경설정 센터 편집기의 행에 놓습니다. 각 블록에는 고유한 설정이 있으며, 오른쪽 패널은 선택한 요소의 속성 또는 스타일링으로 전환됩니다.

블록을 편집하기 전에 구독 그룹을 추가하고 구독 **스마트 블록**을 구성하세요(다음 섹션 참조). 전체 설정 흐름은 [드래그 앤 드롭으로 이메일 환경설정 센터 만들기]({{site.baseurl}}/user_guide/audience/subscription_preferences/preference_center/dnd_preference_center)를 참조하세요.
...
구독 그룹, 선택 사항인 **모두 구독** / **모두 탈퇴** 컨트롤, 설명을 나열하는 템플릿 블록을 추가합니다. 환경설정 센터 워크플로에서 그룹을 추가한 후 구성하세요.
...
- 그룹 추가 또는 제거
- 설명 추가 또는 제거
- 해당 블록의 그룹에 대해 **모두 구독** 및 **모두 탈퇴** 토글

기본 템플릿 하단의 **모두 탈퇴** 컨트롤은 필수이며, 이메일에서 [글로벌 탈퇴]({{site.baseurl}}/user_guide/channels/email/subscriptions#subscription-states)를 수행합니다.

## 알아두어야 할 사항 {#things-to-know}

- **공통 스타일:** 개별 블록을 조정하기 전에 **공통 스타일**에서 페이지 전체 기본값을 설정할 수 있습니다. 자세한 내용은 [드래그 앤 드롭 편집기를 사용하여 환경설정 센터 커스터마이즈하기]({{site.baseurl}}/user_guide/audience/subscription_preferences/preference_center/dnd_preference_center#step-4-customize-the-preference-center-using-the-drag-and-drop-editor)를 참조하세요.
