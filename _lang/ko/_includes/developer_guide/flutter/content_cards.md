## Flutter 콘텐츠 카드에 대하여 {#about-flutter-content-cards}
...
[플러그인 공개 인터페이스](https://github.com/braze-inc/braze-flutter-sdk/blob/master/lib/braze_plugin.dart)에서 사용할 수 있는 다음 메서드를 사용하여 앱 내에서 커스텀 Content Cards 피드를 구축할 수 있습니다:

| 메서드                                         | 설명                                                                                            |
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| `braze.requestContentCardsRefresh()`     | Braze SDK 서버에서 최신 Content Cards를 요청합니다.                                           |
| `braze.logContentCardClicked(contentCard)`    | 주어진 Content Card 오브젝트에 대해 클릭을 기록합니다.                                                            |
| `braze.logContentCardImpression(contentCard)` | 주어진 Content Card 오브젝트에 대해 노출 횟수를 기록합니다.                                                      |
| `braze.logContentCardDismissed(contentCard)`  | 주어진 Content Card 오브젝트에 대한 해제를 기록합니다.                                                        |
{: .reset-td-br-1 .reset-td-br-2 aria-label="카드 메서드" }

## 콘텐츠 카드 데이터 수신 {#receiving-content-card-data}
...
### Dart 레이어에서 콘텐츠 카드 데이터 수신 {#listen-for-content-card-data-in-the-dart-layer}
...
### 네이티브 iOS 레이어에서 콘텐츠 카드 데이터 전달 {#forward-content-card-data-from-the-native-ios-layer}
...
#### 콘텐츠 카드에 대한 콜백 재생 {#replaying-the-callback-for-content-cards}
