...
Roku SDK에는 `wipeData` 메서드가 포함되어 있지 않습니다. 다른 Braze SDK의 `wipeData()`와 기능적으로 동일한 초기화 상태를 만들려면, 네 개의 Braze 레지스트리 섹션을 지운 다음 SDK를 다시 초기화하세요.
...
| `braze.section.config` | 캐시된 SDK 구성 및 기능 플래그 데이터입니다. |
{: .reset-td-br-1 .reset-td-br-2 aria-label="이전에 저장된 데이터 삭제" }
...
### 2단계: Braze SDK 다시 초기화하기 {#step-2-re-initialize-the-braze-sdk}

[Braze SDK를 초기화]({{site.baseurl}}/developer_guide/sdk_integration/?sdktab=roku)하면, SDK는 누락된 레지스트리 데이터를 정상적으로 처리합니다:

- 기기 ID 섹션이 비어 있으므로 SDK가 새 UUID를 생성하고 기기를 익명으로 처리합니다.
- 사용자 ID 섹션이 비어 있으므로 SDK가 익명 사용자(빈 문자열 `""`)를 기본값으로 사용합니다.
- 세션 섹션이 비어 있으므로 SDK가 새 세션을 시작합니다.
- 구성 섹션이 비어 있으므로 SDK가 서버에서 구성을 다시 가져옵니다.

{% alert note %}
Roku SDK는 레지스트리를 지울 때 서버 측 삭제 요청을 생성하지 않습니다. Braze에서 사용자를 제거해야 하는 경우, 사용자의 `external_id` 또는 `braze_id`를 사용하여 [`/users/delete`]({{site.baseurl}}/api/endpoints/user_data/post_user_delete)로 요청을 보내세요.
{% endalert %}

## 로그아웃 및 푸시 등록 해제 {#logout-and-unregister-push}

이 기능은 아직 Roku SDK에서 지원되지 않습니다.