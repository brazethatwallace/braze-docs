...
## UI 대리자 설정(필수) {#setting-up-the-ui-delegate-required}

인앱 메시지의 표시를 사용자 지정하고 다양한 생명 주기 이벤트에 반응하려면 [`BrazeInAppMessageUIDelegate`](https://braze-inc.github.io/braze-swift-sdk/documentation/brazeui/brazeinappmessageuidelegate)를 설정해야 합니다. 이것은 트리거된 인앱 메시지 페이로드를 수신하고 처리하며, 표시 생명 주기 이벤트를 수신하고 표시 타이밍을 제어하는 데 사용되는 대리자 프로토콜입니다. `BrazeInAppMessageUIDelegate`를 사용하려면 다음이 필요합니다:
...
먼저 `BrazeInAppMessageUIDelegate` 프로토콜과 원하는 해당 메서드를 구현합니다. 아래 예제에서는 애플리케이션의 `AppDelegate` 클래스에서 이 프로토콜을 구현합니다.
...
### 2단계: `delegate` 오브젝트 할당 {#step-2-assign-the-delegate-object}

이 인앱 메시지 UI를 `inAppMessagePresenter`로 할당하기 전에 `BrazeInAppMessageUI` 인스턴스에서 `delegate` 오브젝트를 할당합니다.
...
{% alert important %}
일부 대리자 메서드는 매개변수가 언어 런타임과 호환되지 않아 Objective-C에서 사용할 수 없습니다.
{% endalert %}
{% endtab %}
{% endtabs %}

{% alert tip %}
인앱 메시지 UI 대리자의 단계별 구현에 대해서는 이 [튜토리얼](https://braze-inc.github.io/braze-swift-sdk/tutorials/braze/c1-inappmessageui)을 참조하세요.
{% endalert %}
...
### 클릭 동작 유형 {#click-action-types}

`Braze.InAppMessage`의 `clickAction` 속성은 기본값이 `.none`이지만 다음 값 중 하나로 설정할 수 있습니다:
...
### 클릭 동작 사용자 지정 {#customizing-on-click-behavior}

이 동작을 사용자 지정하려면 다음 샘플을 참조하여 `clickAction` 속성을 수정할 수 있습니다:
...
다음 [`BrazeInAppMessageUIDelegate`](https://braze-inc.github.io/braze-swift-sdk/documentation/brazeui/brazeinappmessageuidelegate) 대리자 메서드는 사용자가 인앱 메시지를 클릭할 때 호출됩니다. 이 콜백은 인앱 메시지 버튼 및 HTML 인앱 메시지 버튼(링크)에 대한 사용자 주도 클릭에 대해 트리거되며, 이러한 상호작용을 위한 선택적 매개변수로 버튼 ID가 제공됩니다. 이 콜백은 `brazeBridge.logClick()`을 통해 트리거된 프로그래밍 방식 클릭에 대해서는 호출되지 않습니다.
...
이 메서드는 Braze가 클릭 동작을 계속 실행할지 여부를 나타내는 부울 값을 반환합니다.
...
## 스와이프하여 슬라이드업 메시지 닫기 {#swiping-to-dismiss-slideup-messages}
...
인앱 메시지의 방향을 사용자 지정할 수 있습니다. 모든 메시지에 대한 새로운 기본 방향을 설정하거나 단일 메시지에 대한 커스텀 방향을 설정할 수 있습니다.

{% tabs local %}
{% tab 모든 메시지 %}
모든 인앱 메시지에 대한 기본 방향을 선택하려면 [`inAppMessage(_:prepareWith:)`](https://braze-inc.github.io/braze-swift-sdk/documentation/brazeui/brazeinappmessageuidelegate/inappmessage(_:preparewith:)-11fog) 메서드를 사용하여 `PresentationContext`에서 `preferredOrientation` 속성을 설정합니다.

예를 들어 세로를 기본 방향으로 설정하려면:
...
인앱 메시지가 표시된 후 메시지가 여전히 표시되는 동안 기기 방향이 변경되면 메시지가 기기와 함께 회전합니다(메시지의 `orientation` 구성에서 지원되는 경우).

메시지가 표시되려면 기기 방향이 인앱 메시지의 `orientation` 속성에서도 지원되어야 합니다. 또한 `preferredOrientation` 설정은 Xcode에서 타겟 설정의 **Deployment Info** 섹션에 있는 애플리케이션의 지원 인터페이스 방향에 포함된 경우에만 적용됩니다.
...
사용자 경험의 특정 시점에서 사용 가능한 인앱 메시지의 표시 여부를 제어할 수 있습니다. 전체 화면 게임 중이거나 로딩 화면에서 인앱 메시지가 나타나지 않도록 하려면 보류 중인 인앱 메시지를 지연시키거나 삭제할 수 있습니다. 인앱 메시지의 타이밍을 제어하려면 `inAppMessage(_:displayChoiceForMessage:)` [대리자 메서드](https://braze-inc.github.io/braze-swift-sdk/documentation/brazeui/brazeinappmessageuidelegate/inappmessage(_:displaychoiceformessage:)-9w1nb)를 사용하여 `BrazeInAppMessageUI.DisplayChoice` 속성을 설정합니다.
...
사용자 기기에 다크 모드가 활성화되어 있을 때 인앱 메시지가 다크 모드 스타일을 적용하지 않도록 하려면 `inAppMessage(_:prepareWith:)` [대리자 메서드](https://braze-inc.github.io/braze-swift-sdk/documentation/brazeui/brazeinappmessageuidelegate/inappmessage(_:preparewith:)-11fog)를 구현합니다. 메서드에 전달된 `PresentationContext`에는 표시할 `InAppMessage` 오브젝트에 대한 참조가 포함되어 있습니다. 각 `InAppMessage`에는 `dark` 및 `light` 모드 테마를 포함하는 `themes` 속성이 있습니다. `themes.dark` 속성을 `nil`로 설정하면 Braze가 인앱 메시지를 라이트 테마로 자동 표시합니다.

버튼이 있는 인앱 메시지 유형에는 `buttons` 속성에 추가 `themes` 오브젝트가 있습니다. 버튼이 다크 모드 스타일을 적용하지 않도록 하려면 [`map(_:)`](https://developer.apple.com/documentation/swift/array/map(_:)-87c4d)을 사용하여 `light` 테마가 적용되고 `dark` 테마가 없는 새 버튼 배열을 생성할 수 있습니다.
...
다음으로 기본 앱 스토어 리뷰 메시지를 비활성화하기 위해 `inAppMessage(_:displayChoiceForMessage:)` [대리자 메서드](https://braze-inc.github.io/braze-swift-sdk/documentation/brazeui/brazeinappmessageuidelegate/inappmessage(_:displaychoiceformessage:)-9w1nb)를 구현합니다.
...
### 4단계: 커스텀 클릭 시 동작 설정 {#step-4-set-custom-on-click-behavior}

다음으로 아래 내용을 포함하는 인앱 메시징 Campaign을 생성합니다:

- 키-값 페어 `"AppStore Review" : "true"`
- 클릭 시 동작을 "앱으로 딥링크"로 설정하고 딥링크 `{YOUR-APP-SCHEME}:app-store-review`를 사용합니다.
