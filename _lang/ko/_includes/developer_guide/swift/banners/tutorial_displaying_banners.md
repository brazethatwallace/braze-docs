...
### 3. 배너 초기화 및 콜백 제공 {#3-initialize-the-banner-and-provide-a-callback}
...
### 4. Auto Layout 제약 조건 활성화 {#4-enable-auto-layout-constraints}

기본적으로 배너 뷰를 숨기고, 자동 크기 조정 마스크 변환을 비활성화하여 Auto Layout 제약 조건을 활성화합니다.

!!단계
lines-BannerViewController.swift=43-58

### 5. 콘텐츠 앵커 및 높이 제약 조건 설정 {#5-anchor-content-and-set-height-constraints}

Auto Layout을 사용하여 주요 콘텐츠를 상단에 앵커하고, 배너 뷰를 그 뒤에 배치합니다. 배너의 선행, 후행 및 하단 가장자리를 안전 영역에 고정하고, 콘텐츠가 로드될 때 업데이트될 초기 높이 제약 조건을 `0`으로 설정합니다.
...
### 3. 뷰 컴포넌트 생성 {#3-create-a-view-component}

사용 가능한 배너를 표시하고 필요한 경우 주요 앱 콘텐츠를 포함하는 재사용 가능한 SwiftUI 뷰 컴포넌트를 만듭니다.

!!단계
lines-BannerSwiftUIView.swift=36-43

### 4. 사용 가능한 배너만 표시 {#4-only-display-available-banners}

SDK가 초기화되고 해당 사용자에 대한 배너 콘텐츠가 존재하는 경우에만 `BrazeBannerUI.BannerView`를 표시하세요. `.onAppear`에서 `getBanner(for:placementID)`를 호출하여 `hasBannerForPlacement`의 상태를 설정합니다.

!!단계
lines-BannerSwiftUIView.swift=17-32

### 5. 로드된 후에만 `BannerView` 표시 {#5-only-show-bannerview-after-it-loads}

UI에서 빈 공간을 피하기 위해 배너가 존재하고 SDK가 초기화된 경우에만 `BrazeBannerUI.BannerView`를 표시합니다.

!!단계
lines-BannerSwiftUIView.swift=23-32

### 6. 배너 높이를 동적으로 업데이트 {#6-dynamically-update-banner-height}
...
### 7. 배너 높이 제한 {#7-limit-the-banner-height}
