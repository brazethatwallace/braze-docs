## Braze API로 데이터 로깅(권장) {#logging-data-with-the-braze-api-recommended}

Braze API [`/users/track` 엔드포인트]({{site.baseurl}}/api/endpoints/user_data/post_user_track)를 사용하여 실시간으로 분석을 기록할 수 있습니다. 분석을 기록하려면 아래 스크린샷과 같이 키-값 페어 필드에 `braze_id` 값을 전송하여 업데이트할 고객 프로필을 식별합니다.

![세 세트의 키-값 페어가 포함된 푸시 메시지. 1. "Braze_id"를 Liquid 호출로 설정하여 Braze ID를 검색합니다. 2. "cert_title"을 "Braze Marketer Certification"으로 설정합니다. 3. "Cert_description"을 "Certified Braze marketers drive..."로 설정합니다.]({% image_buster /assets/img/push_implementation_guide/push18.png %}){: style="max-width:80%;"}

## 수동으로 데이터 로깅하기 {#logging-data-manually}

수동으로 로깅하려면 먼저 Xcode 내에서 워크스페이스를 구성한 다음, 분석을 생성, 저장 및 검색해야 합니다. 이를 위해서는 개발자 측에서 일부 커스텀 작업이 필요합니다. 아래에 표시된 코드 스니펫이 이 문제를 해결하는 데 도움이 됩니다.

모바일 애플리케이션이 이후에 실행될 때까지 분석 데이터가 Braze로 전송되지 않는다는 점에 유의해야 합니다. 즉, 해제 설정에 따라 푸시 알림이 해제되는 시점과 모바일 앱이 실행되어 분석이 검색되는 시점 사이에 불확정적인 시간이 존재하는 경우가 많습니다. 이 시간 버퍼가 모든 사용 사례에 영향을 미치는 것은 아니지만, 이 영향을 고려하여 필요한 경우 이 문제를 해결하기 위해 애플리케이션 열기를 포함하도록 사용자 여정을 조정해야 합니다.

![Braze에서 분석이 처리되는 방식을 설명하는 그래픽. 1. 분석 데이터가 생성됩니다. 2. 분석 데이터가 저장됩니다. 3. 푸시 알림이 해제됩니다. 4. 푸시 알림 해제와 모바일 앱 실행 사이의 불확정적인 시간. 5. 모바일 앱이 실행됩니다. 6. 분석 데이터가 수신됩니다. 7. 분석 데이터가 Braze로 전송됩니다.]({% image_buster /assets/img/push_implementation_guide/push13.png %})

### 1단계: Xcode 내에서 앱 그룹 구성 {#step-1-configure-app-groups-within-xcode}

Xcode에서 `App Groups` 기능을 추가합니다. 앱에 워크스페이스가 없는 경우 기본 앱 대상의 기능으로 이동하여 `App Groups`를 켜고 **+** 추가 버튼을 클릭합니다. 그런 다음 앱의 번들 ID를 사용하여 워크스페이스를 만듭니다. 예를 들어 앱의 번들 ID가 `com.company.appname`인 경우 워크스페이스 이름을 `group.com.company.appname.xyz`로 지정할 수 있습니다. 기본 앱 대상과 콘텐츠 확장 대상 모두에 대해 `App Groups`가 켜져 있는지 확인합니다.

![기본 앱과 확장 대상에 대해 App Groups가 활성화된 Xcode 서명 및 기능 화면]({% image_buster /assets/img/swift/push_story/add_app_groups.png %})
...
SDK가 초기화된 직후가 알림 콘텐츠 앱 확장에서 저장된 모든 분석을 기록하기에 가장 좋은 시점입니다. 보류 중인 이벤트를 반복하고 "Event Name" 키를 확인하며 Braze에서 적절한 값을 설정한 다음, 다음에 이 기능이 필요할 때를 대비해 스토리지를 지우면 됩니다.

1. 보류 중인 이벤트 배열 반복
2. `pendingEvents` 사전의 각 키-값 페어 반복
3. "Event Name"에 대한 키를 명시적으로 확인하여 적절히 값 설정
...
SDK가 초기화된 직후가 알림 콘텐츠 앱 확장에서 저장된 모든 분석을 기록하기에 가장 좋은 시점입니다. 보류 중인 속성을 반복하고 Braze에서 적절한 커스텀 속성을 설정한 다음, 다음에 이 기능이 필요할 때를 대비해 스토리지를 지우면 됩니다.

1. 보류 중인 속성 배열 반복
2. `pendingAttributes` 사전의 각 키-값 페어 반복
3. 해당 키와 값으로 개별 커스텀 속성 기록
...
사용자 속성을 저장할 때는 커스텀 오브젝트를 만들어 업데이트되는 속성 유형(`email`, `first_name`, `phone_number` 등)을 해독하는 것이 좋습니다. 오브젝트는 `UserDefaults`에서 저장/검색하는 것과 호환되어야 합니다. 이를 수행하는 방법에 대한 한 가지 예제는 `UserAttribute` 헬퍼 파일을 참조하세요.
...
2. 속성 데이터에서 인코딩된 `UserAttribute` 오브젝트 초기화
3. 사용자 속성 유형(이메일)에 따라 특정 사용자 필드 설정
