...
{% alert tip %}
전체 안내는 [개인정보 보호 추적 데이터 튜토리얼](https://braze-inc.github.io/braze-swift-sdk/tutorials/braze/e1-privacy-tracking/)을 참조하세요.
{% endalert %}
...
법무팀과 함께 Braze SDK의 현재 데이터 수집 정책을 검토하여 앱이 [Apple이 정의한](#what-is-tracking-data) 추적 데이터를 수집하는지 확인하세요. 추적 데이터를 수집하지 않는 경우, 현재 Braze SDK에 대한 개인정보 보호 매니페스트를 커스터마이징할 필요가 없습니다. Braze SDK의 데이터 수집 정책에 대한 자세한 내용은 [SDK 데이터 수집]({{site.baseurl}}/user_guide/data/user_data_collection/sdk_data_collection)을 참조하세요.
...
Xcode 프로젝트에서 앱의 `PrivacyInfo.xcprivacy` 파일을 열고, 테이블을 마우스 오른쪽 버튼으로 클릭한 후 **Raw Keys and Values**를 선택하세요.
...
**App Privacy Configuration** 아래에서 **NSPrivacyTracking**을 선택하고 값을 **YES**로 설정하세요.
...
**App Privacy Configuration** 아래에서 **NSPrivacyTrackingDomains**를 선택하세요. 도메인 배열에 새 요소를 추가하고 [이전에 `AppDelegate`에 추가한]({{site.baseurl}}/developer_guide/platform_integration_guides/swift/initial_sdk_setup/completing_integration#update-your-app-delegate) 엔드포인트 앞에 `sdk-tracking`을 붙인 값을 설정하세요.
...
다음으로, `AppDelegate.swift`를 열고 정적 또는 동적 추적 목록을 생성하여 선언하려는 각 [추적 속성정보](https://braze-inc.github.io/braze-swift-sdk/documentation/brazekit/braze/configuration-swift.class/trackingproperty/)를 나열하세요. Apple은 최종사용자가 ATT 프롬프트를 수락할 때까지 이러한 속성정보를 차단하므로, 본인과 법무팀이 추적으로 판단하는 속성정보만 나열하세요. 예를 들어:
...
{% tab 동적 예시 %}
다음 예시에서는 최종사용자가 [앱 추적 투명성(ATT) 프롬프트](https://developer.apple.com/documentation/apptrackingtransparency/attrackingmanager/requesttrackingauthorization(completionhandler:))를 수락한 후 추적 목록이 자동으로 업데이트됩니다. 앱 활성화 시 권한 요청은 씬(scene)별 이벤트이므로, 이 코드는 `AppDelegate.swift`의 `applicationDidBecomeActive(_:)`가 아닌 `SceneDelegate.swift` 파일의 `sceneDidBecomeActive(_:)` 메서드에 위치해야 합니다([`UIScene` 생명주기](https://developer.apple.com/documentation/technotes/tn3187-migrating-to-the-uikit-scene-based-life-cycle)를 채택한 앱에 필요). 1단계에서 구성한 `AppDelegate.braze` 정적 속성정보를 통해 `SceneDelegate`에서 Braze 인스턴스에 접근할 수 있습니다.
...
Braze Swift 버전 7.0.0 이상에서는 SDK와 `wipeData()` 메서드가 기기 ID로 UUID를 무작위로 생성합니다. 그러나 `useUUIDAsDeviceId`가 `false`로 설정되어 있거나 Swift SDK 버전 5.7.0 이하를 사용하는 경우, IDFV(Identifier for Vendors)가 해당 사용자의 기기 ID로 자동 사용되므로 [`/users/delete`]({{site.baseurl}}/api/endpoints/user_data/post_user_delete)에 대한 POST 요청도 수행해야 합니다.

수동 푸시 통합을 사용하는 경우, 앱에서 `wipeData()`를 호출한 후 동일한 앱 실행 내에서 SDK를 다시 활성화하면 `registerForRemoteNotifications()`를 다시 호출하여 Braze가 갱신된 기기 토큰을 수신할 수 있도록 해야 합니다. 자세한 내용은 [푸시 알림 설정]({{site.baseurl}}/developer_guide/push_notifications/?sdktab=swift)을 참조하세요.
...
Braze SDK는 사용자가 푸시 알림 등록을 해제하거나 로그아웃할 때 기기를 타겟팅 대상에서 제거하는 메서드를 제공합니다. 이러한 메서드는 Braze 서버와 SDK에서 현재 사용자의 푸시 등록 데이터를 삭제하므로, Braze는 더 이상 해당 사용자에게 푸시 알림 Campaign을 보내지 않습니다.

### 로그아웃 {#logout}

사용자가 애플리케이션에서 로그아웃하면, SDK의 `logout` 메서드를 호출하여 현재 사용자의 기기 푸시 등록을 제거하고 SDK에서 정리 작업을 자동으로 수행합니다. `logout` 메서드는 다음 작업을 수행합니다:

- Braze 서버에서 현재 사용자의 기기 푸시 토큰과 모든 Live Activities push-to-start 토큰을 등록 해제합니다.
- 등록 해제 호출이 성공하면, SDK는 로컬에 저장된 SDK 데이터를 삭제하고 SDK를 비활성화합니다.
- 실패 시, 오류와 `isRetriable` 플래그를 발생시켜 통합 개발자가 조치를 취할 수 있도록 합니다.

{% subtabs local %}
{% subtab Swift %}

다음 완료 핸들러 예시는 `logout` 성공 및 실패 처리를 보여줍니다. 콜백 기반 흐름에 사용하고, 로깅 부분을 앱의 재시도 또는 재인증 로직으로 교체하세요.
...
다음 async 예시는 일시 중단 `logout` API를 보여줍니다. async 워크플로에 사용하고 앱에 맞게 성공 및 실패 분기를 커스텀하세요.
...
이 Objective-C 예시는 완료 기반 `logout` 처리를 보여줍니다. Objective-C 통합에서 사용하고, 로깅 부분을 앱 흐름에 맞게 교체하세요.
...
`logout`이 성공한 후, [`enabled`](https://braze-inc.github.io/braze-swift-sdk/documentation/brazekit/braze/enabled/)를 다시 `true`로 설정한 다음, [Swift 푸시 설정]({{site.baseurl}}/developer_guide/push_notifications/?sdktab=swift)을 참고하여 운영 체제(OS) 또는 푸시 공급자에 알림을 다시 등록하세요.

#### 즉시 등록 해제 호출 방지 {#avoid-immediate-unregister-calls}

OS 또는 푸시 공급자에 푸시 알림을 등록한 직후에 `logout` 또는 `unregisterPush`를 호출하는 것은 피하세요. 비동기 서버 처리로 인해 드물게 푸시 토큰이 Braze 사용자에 다시 추가될 수 있습니다.
...
다음 완료 핸들러 예시는 `unregisterPush` 성공 및 실패 처리를 보여줍니다. 콜백 기반 흐름에 사용하고, 로깅 부분을 자체 재시도 로직으로 교체하세요.
...
다음 async 예시는 일시 중단 `unregisterPush` API를 보여줍니다. async 워크플로에 사용하고 앱에 맞게 성공 및 실패 분기를 커스텀하세요.
...
이 Objective-C 예시는 완료 기반 `unregisterPush` 처리를 보여줍니다. Objective-C 통합에서 사용하고, 로깅 부분을 앱 흐름에 맞게 교체하세요.
...
`unregisterPush`를 호출한 후, Braze 푸시 알림을 다시 보내기 전에 [Swift 푸시 설정]({{site.baseurl}}/developer_guide/push_notifications/?sdktab=swift)을 참고하여 OS 또는 푸시 공급자에 알림을 다시 등록하세요.
...
Live Activities는 push-to-start 토큰을 사용하여 원격으로 시작할 수 있습니다. Braze가 기기에서 Live Activities를 원격으로 시작하지 못하도록 하려면, `unregisterPushToStart` 메서드를 호출하여 현재 등록된 모든 유형(기본값) 또는 지정된 Activity 유형 목록을 등록 해제하세요.

현재 실행 중인 Live Activities는 계속해서 업데이트를 수신하며, 이 메서드는 원격으로 새 활동을 시작하는 기능만 제거합니다. Live Activities에 대한 자세한 내용은 [Live Activities]({{site.baseurl}}/developer_guide/live_notifications/live_activities)를 참조하세요.

#### 실행 중인 Live Activities 종료 {#end-any-running-live-activities}
...
{% alert note %}
Live Activity에 대해 `registerPushToStart`를 호출한 직후에 `logout` 또는 `unregisterPushToStart`를 호출하는 것은 피하세요. 비동기 서버 처리 특성으로 인해, 드문 경우에 push-to-start 토큰이 Braze 사용자에 다시 추가될 수 있습니다.
{% endalert %}

다음 예시는 모든 push-to-start 활동 유형을 등록 해제하는 방법을 보여줍니다. 로그아웃한 사용자가 더 이상 원격으로 시작된 새 Live Activities를 수신하지 않아야 할 때 사용하세요.
...
다음 예시는 특정 활동 유형을 등록 해제하는 방법을 보여줍니다. 선택한 Live Activities만 원격으로 시작되지 않도록 할 때 사용하세요.
...
{% alert note %}
`unregisterPushToStart`는 Live Activities가 Swift 전용 유형에 의존하므로 Objective-C API가 없습니다.
{% endalert %}

## IDFV 수집 {#idfv-collection}

이전 버전의 Braze iOS SDK에서는 IDFV(Identifier for Vendor) 필드가 사용자의 기기 ID로 자동 수집되었습니다. Swift SDK `v5.7.0`부터 IDFV 필드를 선택적으로 비활성화할 수 있게 되었으며, 대신 Braze가 랜덤 UUID를 기기 ID로 설정하도록 변경되었습니다. Swift SDK `v7.0.0`부터는 IDFV 필드가 기본적으로 수집되지 않으며, UUID가 기기 ID로 설정됩니다.

`useUUIDAsDeviceId` 기능은 [Swift SDK](https://github.com/braze-inc/braze-swift-sdk)에서 기기 ID를 UUID로 설정하도록 구성합니다. 기존에는 iOS SDK가 Apple에서 생성한 IDFV 값과 동일하게 기기 ID를 할당했습니다. iOS 앱에서 이 기능이 기본적으로 활성화되면, SDK를 통해 생성된 모든 신규 사용자에게 UUID와 동일한 기기 ID가 할당됩니다.
...
{% alert note %}
IDFV 생성은 Apple의 책임이며, IDFV는 Apple이 관리합니다. Braze는 IDFV를 변환하거나 변경하지 않으며, Apple은 대소문자나 형식에 대해 어떠한 보장도 하지 않습니다.
{% endalert %}
...
Swift SDK `v7.0.0+`에서 `useUUIDAsDeviceId`가 활성화된 경우(기본값), 모든 신규 사용자에게 랜덤 기기 ID가 할당됩니다. 기존에 존재하는 모든 사용자는 동일한 기기 ID 값을 유지하며, 이 값은 IDFV였을 수 있습니다.

이 기능이 활성화되지 않은 경우, 기기는 생성 시 계속 IDFV가 할당됩니다.

#### 다운스트림 {#downstream}

**기술 파트너**: 이 기능이 활성화되면 Braze 기기 ID에서 IDFV 값을 파생하는 기술 파트너는 더 이상 이 데이터에 접근할 수 없습니다. 파트너 통합에 기기에서 파생된 IDFV 값이 필요한 경우, 이 기능을 `false`로 설정하는 것이 좋습니다.

**Currents**: `useUUIDAsDeviceId`를 true로 설정하면 Currents에서 전송되는 기기 ID가 더 이상 IDFV 값과 동일하지 않게 됩니다.

### 자주 묻는 질문 {#frequently-asked-questions}

#### 이 변경으로 Braze의 기존 사용자에게 영향이 있나요? {#will-this-change-impact-my-existing-users-in-braze}

아니요. 이 기능이 활성화되어도 Braze의 기존 사용자 데이터를 덮어쓰지 않습니다. 새 UUID 기기 ID는 새로운 기기에 대해서만 생성되거나 `wipedata()`가 호출될 때만 생성됩니다.

#### 이 기능을 켠 후 끌 수 있나요? {#can-i-turn-this-feature-off-after-turning-it-on}

네, 이 기능은 필요에 따라 켜거나 끌 수 있습니다. 이전에 저장된 기기 ID는 절대 덮어쓰지 않습니다.

#### Braze의 다른 곳에서 IDFV 값을 여전히 캡처할 수 있나요? {#can-i-still-capture-the-idfv-value-through-braze-elsewhere}

네, Swift SDK를 통해 선택적으로 IDFV를 수집할 수 있습니다(기본적으로 수집은 비활성화되어 있습니다).