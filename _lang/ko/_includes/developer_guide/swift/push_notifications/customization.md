...
커스텀 푸시 알림 사운드는 앱의 메인 번들 내에서 로컬로 호스팅해야 합니다. 허용되는 오디오 데이터 형식은 다음과 같습니다:
...
{% alert tip %}
QuickTime Player에서 사운드를 열고 **동영상** 메뉴에서 **동영상 검사기 표시**를 선택하면 사운드를 검사하여 데이터 형식을 확인할 수 있습니다.
{% endalert %}

### 2단계: 사운드를 위한 프로토콜 URL 제공 {#step-2-provide-a-protocol-url-for-the-sound}
...
지정한 사운드 파일이 존재하지 않거나 "default" 키워드가 입력된 경우, Braze는 기기의 기본 알림 사운드를 사용합니다. 대시보드 외에도 사운드는 [메시징 API][12]를 통해 구성할 수 있습니다.

자세한 내용은 [커스텀 알림 사운드 준비](https://developer.apple.com/library/content/documentation/NetworkingInternet/Conceptual/RemoteNotificationsPG/SupportingNotificationsinYourApp.html)에 관한 Apple 개발자 문서를 참조하세요.

## 설정 {#settings}

대시보드를 통해 푸시 Campaign을 만들 때 **작성** 단계의 **설정** 탭을 클릭하면 사용 가능한 고급 설정을 볼 수 있습니다.

![고급 옵션이 포함된 Braze iOS 푸시 Campaign 작성 설정 탭.]({% image_buster /assets/img_archive/ios_advanced_settings.png %})

### 키-값 페어 {#key-value-pairs}

Braze에서는 커스텀 정의 문자열 키-값 페어(`extras`)를 푸시 알림과 함께 애플리케이션에 전송할 수 있습니다. 추가 항목은 대시보드 또는 API를 통해 정의할 수 있으며, 푸시 델리게이트 구현에 전달되는 `notification` 사전 내에서 키-값 페어로 사용할 수 있습니다.

### 알림 옵션 {#alert-options}

**알림 옵션** 확인란을 선택하면 알림이 기기에 표시되는 방식을 조정할 수 있는 키-값 드롭다운을 확인할 수 있습니다.

### 콘텐츠 가용 플래그 추가 {#adding-content-available-flag}

**콘텐츠 가용 플래그 추가** 확인란을 선택하여 기기가 백그라운드에서 새 콘텐츠를 다운로드하도록 지시합니다. 가장 일반적으로 [무음 알림]({{site.baseurl}}/developer_guide/push_notifications/silent/?sdktab=swift)을 보내고 싶은 경우 이 옵션을 선택할 수 있습니다.
...
축소 ID를 지정하여 유사한 알림을 결합합니다. 동일한 축소 ID로 여러 알림을 발송한 경우, 기기에는 가장 최근 수신된 알림만 표시됩니다. Apple의 [통합 알림](https://developer.apple.com/library/content/documentation/NetworkingInternet/Conceptual/RemoteNotificationsPG/APNSOverview.html#//apple_ref/doc/uid/TP40008194-CH8-SW1) 문서를 참조하세요.
