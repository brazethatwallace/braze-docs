{% multi_lang_include developer_guide/prerequisites/swift.md %} 또한 [푸시 알림을 설정]({{site.baseurl}}/developer_guide/push_notifications/?sdktab=swift)해야 하며, 여기에는 `UNNotification` 프레임워크 구현이 포함됩니다.

Push Stories를 수신하려면 다음 최소 SDK 버전이 필요합니다:

{% sdk_min_versions swift:5.0.0 %}

## Push Stories 설정 {#setting-up-push-stories}

### 1단계: 알림 콘텐츠 확장 타겟 추가 {#notification-content-extension}
...
Xcode에서 메인 앱 타겟의 **Signing & Capabilities** 패널을 사용하여 Background Modes 기능을 추가합니다. **Background fetch** 및 **Remote notifications** 체크박스를 모두 선택합니다.

![]({% image_buster /assets/img/swift/push_story/enable_background_mode.png %})

#### 앱 그룹 추가 {#adding-an-app-group}

또한 Xcode의 **Signing & Capabilities** 패널에서 메인 앱 타겟과 알림 콘텐츠 확장 타겟에 App Groups 기능을 추가합니다. 그런 다음 **+** 버튼을 클릭합니다. 앱의 번들 ID를 사용하여 앱 그룹을 만듭니다. 예를 들어 앱의 번들 ID가 `com.company.appname`인 경우 앱 그룹 이름을 `group.com.company.appname.xyz`로 지정할 수 있습니다.
...
`NotificationViewController.swift`에서 헤더 파일을 가져오기 위해 다음 줄을 추가합니다:
