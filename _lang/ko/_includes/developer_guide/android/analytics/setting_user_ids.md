...
또한 사용자가 로그아웃할 때 사용자 ID를 변경하지 **않는** 것이 좋습니다. 변경하면 이전에 로그인한 사용자를 재참여 캠페인으로 타겟팅할 수 없게 되기 때문입니다. 여러 사용자가 동일한 기기를 사용할 것으로 예상되지만 앱이 로그아웃 상태일 때 한 명만 타겟팅하려는 경우, 로그아웃 상태에서 타겟팅하려는 사용자 ID를 별도로 추적하고 앱의 로그아웃 프로세스의 일환으로 해당 사용자 ID로 전환하는 것을 권장합니다.
...
가입자 콜백은 `changeUser()`를 통해 사용자가 변경될 때 실행되며 `BrazeUserChangeEvent`를 수신합니다. `BrazeUserChangeEvent`는 현재 사용자가 변경되었거나 SDK가 방금 초기화되었을 때 발생합니다. SDK는 전환이 발생하지 않은 경우에도 동일한 사용자에 대해 여러 이벤트를 발생시킬 수 있습니다.
