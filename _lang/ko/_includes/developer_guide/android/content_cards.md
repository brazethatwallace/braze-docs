...
Android에서 콘텐츠 카드 피드는 Braze Android UI 프로젝트에서 사용할 수 있는 [프래그먼트](https://developer.android.com/guide/components/fragments.html)로 구현됩니다. [`ContentCardsFragment`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.ui.contentcards/-content-cards-fragment/index.html) 클래스는 자동으로 새로고침되고 Content Cards의 내용을 표시하며 사용 분석을 기록합니다. 사용자의 `ContentCards`에 나타날 수 있는 카드는 Braze 대시보드에서 생성됩니다.

활동에 프래그먼트를 추가하는 방법을 알아보려면 [Google의 프래그먼트 설명서](https://developer.android.com/guide/fragments#Adding)를 참조하세요.

## 카드 유형 및 등록정보 {#card-types-and-properties}

Content Cards 데이터 모델은 Android SDK에서 사용할 수 있으며 다음과 같은 고유한 Content Cards 유형을 제공합니다. 각 유형은 기본 모델을 공유하며, 이를 통해 고유한 등록정보를 가지는 것 외에도 기본 모델에서 공통 등록정보를 상속받을 수 있습니다. 전체 참조 설명서는 [`com.braze.models.cards`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.models.cards/index.html)를 참조하세요.
...
| `getViewed()` | 사용자가 카드를 읽었는지 읽지 않았는지를 반영하는 부울 값을 반환합니다.|
...
| `getOpenUriInWebView()` | 이 카드의 URI를 Braze WebView에서 열어야 하는지 여부를 반영하는 <br> 부울 값을 반환합니다.|
| `getExpiredAt()` | 카드의 만료일을 가져옵니다.|
| `isRemoved()` | 최종 사용자가 이 카드를 해제했는지 여부를 반영하는 부울 값을 반환합니다.|
| `isDismissibleByUser()` | 사용자가 카드를 닫을 수 있는지 여부를 반영하는 부울 값을 반환합니다.|
| `isClicked()` | 이 카드의 클릭 상태를 반영하는 부울 값을 반환합니다.|
| `isDismissed` | 카드가 닫혔는지 여부를 반영하는 부울 값을 반환합니다. 카드를 닫힌 것으로 표시하려면 `true`로 설정합니다. 카드가 이미 닫힌 상태로 표시된 경우 다시 닫힌 상태로 표시할 수 없습니다.|
| `isControl()` | 이 카드가 제어 카드이며 렌더링되지 않아야 하는지 여부를 나타내는 부울 값을 반환합니다.|
...
| `getDomain()` | 등록정보 URL의 링크 텍스트를 반환합니다.|
