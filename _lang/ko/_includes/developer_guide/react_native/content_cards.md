...
| `getContentCards()`                      | Braze SDK에서 Content Cards를 검색합니다. 서버에서 최신 카드 목록으로 해결되는 프로미스를 반환합니다. |
...
Content Cards 데이터 모델은 React Native SDK에서 사용할 수 있으며 다음과 같은 Content Cards 카드 유형을 제공합니다: [이미지 전용](#image-only), [캡션 이미지](#captioned-image), [클래식](#classic). 특정 카드의 대조군에 속하는 사용자에게 반환되는 특별한 [대조](#control) 카드 유형도 있습니다. 각 유형은 고유한 등록정보 외에도 기본 모델에서 공통 등록정보를 상속받습니다.
...
| `imageAspectRatio` | 카드 이미지의 종횡비입니다. 이미지 로드가 완료되기 전에 힌트 역할을 합니다. 특정 상황에서는 등록정보가 제공되지 않을 수 있습니다. |
...
대조 카드에는 몇 가지 중요한 차이점이 있는 모든 기본 등록정보가 포함됩니다. 특히 다음 사항이 중요합니다.

- `isControl` 등록정보는 `true`로 보장됩니다.
- `extras` 등록정보는 비어 있음이 보장됩니다.
