{% comment %}
  Note for GET export campaign/canvas data_series analytics endpoints.
  Use with multi_lang_include.

  Parameters:
  - type (required): "canvas" or "campaign" — selects the dashboard analytics label in the sentence.
{% endcomment %}
{% if include.type == 'canvas' %}
{% capture dashboard_match %}대시보드 Canvas 분석{% endcapture %}
{% elsif include.type == 'campaign' %}
{% capture dashboard_match %}대시보드 참여 분석{% endcapture %}
{% endif %}
{% alert note %}
이 엔드포인트의 수치는 {{ dashboard_match }} 또는 [Currents]({{site.baseurl}}/user_guide/data/distribution/braze_currents/)에서 구축한 집계와 항상 정확히 일치하지 않을 수 있습니다. 대시보드 측정기준과 API 시계열은 원시 Currents 이벤트와 다른 집계 기간 및 정의를 사용합니다.
{% endalert %}