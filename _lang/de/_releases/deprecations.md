...
## Protokoll der Verwerfungen {#deprecations-log}
...
### Angepasster Push-Empfänger für Android {#custom-push-broadcast-receiver-for-android}

**Unterstützung zurückgezogen**: Oktober 2022

Die Verwendung eines angepassten `BroadcastReceiver` für Push-Benachrichtigungen ist nicht mehr zeitgemäß. Verwenden Sie stattdessen [` subscribeToPushNotificationEvents()`]({{site.baseurl}}/developer_guide/push_notifications/customization/?sdktab=android#android_using-a-callback-for-push-events).

### Grouparoo Partnerschaft {#grouparoo-partnership}
...
**24. März 2022**: Die Braze Baidu Push-Integration ist veraltet, und es können keine neuen Baidu-Apps im Braze-Dashboard erstellt werden.<br>
**15. September 2022**: Es können keine neuen Baidu-Push-Nachrichten erstellt werden. Bestehende Nachrichten und Datenerfassung sind davon nicht betroffen.<br>
**11. Januar 2024**: Braze wird keine Nachrichten mehr versenden oder Daten von Baidu-Apps erfassen.

### appboyBridge globale Variable {#appboybridge-global-variable}

**Unterstützung zurückgezogen**: Mai 2021<br>
**Ersetzt durch**: `brazeBridge`

Die globale Variable `appboyBridge` ist veraltet und wird durch `brazeBridge` ersetzt. `appboyBridge` wird für bestehende Kund:innen weiterhin funktionieren, aber wir empfehlen Ihnen, auf `brazeBridge` zu migrieren, wenn Sie `appboyBridge` verwenden.

### Amazon Moments Partnerschaft {#amazon-moments-partnership}

**Unterstützung zurückgezogen**: Juni 2020

Die Unterstützung für Amazon Moments wurde im Juni 2020 eingestellt. Amazon Moments wird in Amazon Advertising aufgehen und hat seine APIs und unsere Integration eingestellt.

### Factual Partnerschaft {#factual-partnership}
...
### Originale In-App-Nachrichten {#original-in-app-messages}

**Unterstützung zurückgezogen:** Februar 2019<br>
**Ersetzt durch**: [In-App-Messaging]({{site.baseurl}}/user_guide/channels/in_app_messages/traditional/)
...
Vor diesen Versionen unterstützte Braze „originale In-App-Nachrichten“. Zuvor wurde die Unterstützung für originale In-App-Nachrichten für alle Kund:innen angeboten, die vor der neuen Version eine In-App-Kampagne durchgeführt hatten. Alle Kampagnenstatistiken waren von der Änderung nicht betroffen, und diejenigen, die ursprüngliche In-App-Nachrichten gesendet hatten, hatten die Möglichkeit, weitere Nachrichten über den Button **Kampagne erstellen** auf der **Kampagnen**-Seite zu senden.
...
Braze hat die Unterstützung für die Eclipse IDE eingestellt, da Google für das Eclipse Android Developer Tools (ADT) Plugin [den Support schrittweise beendet](http://android-developers.blogspot.com/2015/06/an-update-on-eclipse-android-developer.html) hat.
...
### Der Raw Event Stream (RES) {#the-raw-event-stream-res}

**Unterstützung zurückgezogen**: Juli 2018<br>
**Ersetzt durch**: [Currents]({{site.baseurl}}/user_guide/data/distribution/braze_currents/)

Der Raw Event Stream war der Vorgänger von [Currents]({{site.baseurl}}/user_guide/data/distribution/braze_currents/) und wurde eingestellt, um Platz für die Zukunft der Braze-Daten zu schaffen.

### Verzögerung im Leerlauf – GCM Feature {#delay-while-idle-gcm-feature}
...
### Angepasste Endpunkte {#custom-endpoints}

**Unterstützung zurückgezogen**: Dezember 2019

Entfernung von angepassten Endpunkten. Wenn Sie einen angepassten Endpunkt haben, können Sie diesen weiterhin verwenden, aber Braze gibt keine neuen mehr heraus.