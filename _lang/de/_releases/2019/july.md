...
Kund:innen können jetzt einen `null`-Wert für die „Antwort-an“-Adresse einer E-Mail-Nachricht über die Seite **E-Mail-Einstellungen** in Braze oder über die [API]({{site.baseurl}}/api/endpoints/messaging/#email-object-specification) festlegen. Wenn Sie diese Option verwenden, werden die Antworten an die angegebene Absenderadresse gesendet. Sie können das Adressfeld „Von“ jetzt als `dan@emailaddress.com` anpassen, und Ihre Kund:innen haben die Möglichkeit, direkt an Dan zu antworten.
