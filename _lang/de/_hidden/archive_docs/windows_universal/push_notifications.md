...
Eine Push-Benachrichtigung ist eine Benachrichtigung außerhalb der App, die auf dem Bildschirm der Nutzer:innen erscheint, wenn ein wichtiges Update or aktualisieren erfolgt. Push-Benachrichtigungen sind eine wertvolle Möglichkeit, Ihre Nutzer:innen mit zeitkritischen und relevanten Inhalten zu versorgen oder sie erneut für Ihre App zu begeistern.
...
## 3. Schritt: Update or aktualisieren für die Protokollierung von Öffnungen im Hintergrund {#step-3-update-for-background-open-logging}
...
Wenn Sie im Dashboard oder in der Representational State Transfer API einen zusätzlichen Launch String angeben, wird dieser am Ende des von uns erstellten Launch Strings nach dem Schlüssel „abextras=“ eingefügt. Ein Beispiel für einen Launch String könnte so aussehen: `ab_cn_id=_trackingid_abextras=page=settings`, wobei Sie `page=settings` im zusätzlichen Parameter für den Launch String angegeben haben, damit Sie ihn analysieren und die Nutzer:innen zur Einstellungsseite navigieren können.
...
### Teil 3: Deeplinking über die Representational State Transfer API {#part-3-deep-linking-through-the-rest-api}

Braze erlaubt es auch, Deeplinks über die Representational State Transfer API zu senden. [Windows Universal Push-Objekte]({{site.baseurl}}/api/objects_filters/) akzeptieren einen optionalen `extra_launch_string`-Parameter.