...
description: "Erfahren Sie, wie Sie die ausführliche Protokollierung für das Braze SDK or Software-Development-Kit aktivieren, Protokolle zur Fehlerbehebung erfassen und diese an den Braze-Support weiterleiten können."
---

# Ausführliche Protokollierung

> Die ausführliche Protokollierung liefert detaillierte Low-Level-Informationen aus dem Braze SDK or Software-Development-Kit und bietet Ihnen Einblick in die Initialisierung des SDK or Software-Development-Kit, die Kommunikation mit Servern und die Verarbeitung von Messaging-Kanälen wie Push-Benachrichtigungen, In-App-Nachrichten und Content-Cards.

Wenn etwas nicht wie erwartet funktioniert – beispielsweise eine Push-Benachrichtigung nicht ankommt, eine In-App-Nachricht nicht angezeigt wird oder Nutzerdaten nicht synchronisiert werden –, helfen Ihnen ausführliche Protokolle dabei, die Ursache zu identifizieren. Anstatt zu vermuten, können Sie genau sehen, was das SDK or Software-Development-Kit bei jedem Schritt ausführt.

{% alert tip %}
Wenn Sie debuggen möchten, ohne die ausführliche Protokollierung manuell zu aktivieren, können Sie den [SDK or Software-Development-Kit-Debugger]({{site.baseurl}}/developer_guide/sdk_integration/debugging) verwenden, um Debugging-Sitzungen direkt im Braze-Dashboard zu erstellen.
{% endalert %}
...
- **Überprüfen Sie die Initialisierung des SDK or Software-Development-Kit**: Bitte überprüfen Sie, ob das SDK or Software-Development-Kit mit dem korrekten SDK or Software-Development-Kit-API-Schlüssel und Endpunkt ordnungsgemäß startet.
...
- **Deeplinks debuggen**: Bitte überprüfen Sie, ob das SDK or Software-Development-Kit Deeplinks aus Push-Benachrichtigungen, In-App-Nachrichten oder Content-Cards empfängt und öffnet.
- **SitzungsTracking überprüfen**: Bitte bestätigen Sie, dass die Sitzungen wie erwartet beginnen und enden.
- **Diagnose von Verbindungsproblemen**: Bitte überprüfen Sie die Netzwerkanfragen und -antworten zwischen dem SDK or Software-Development-Kit und den Braze-Servern.
...
Bitte aktivieren Sie die ausführliche Protokollierung vor allen anderen SDK or Software-Development-Kit-Aufrufen in Ihrer`Application.onCreate()`Methode, um die vollständigsten Ergebnisse zu erfassen.
...
Ausführliche Informationen finden Sie unter [Android SDK or Software-Development-Kit-Protokollierung]({{site.baseurl}}/developer_guide/sdk_integration#android_enabling-logs).
...
Die`.debug`Stufe ist die ausführlichste und wird für die Fehlerbehebung empfohlen. Ausführliche Informationen finden Sie unter [SWIFT SDK or Software-Development-Kit-Protokollierung]({{site.baseurl}}/developer_guide/sdk_integration#swift_log-levels).

{% endtab %}
{% tab Web %}

Fügen Sie`?brazeLogging=true`als URL-Parameter hinzu oder aktivieren Sie die Protokollierung während der SDK or Software-Development-Kit-Initialisierung:
...
Die Protokolle werden auf dem Tab **„Konsole“** der Entwicklertools Ihres Browsers angezeigt. Ausführliche Informationen finden Sie unter [Internet-SDK or Software-Development-Kit-Protokollierung]({{site.baseurl}}/developer_guide/sdk_integration#web_logging).
...
3. Geben Sie im Feld **„SDK or Software-Development-Kit-Protokollstufe**“ bitte ein`0`.

{% endtab %}
{% tab React Native %}

Legen Sie die Protokollstufe während der SDK or Software-Development-Kit-Konfiguration fest:
...
3. Bitte verwenden Sie den Filter für`V/Braze`  oder , um die Braze`D/Braze` SDK or Software-Development-Kit-Ausgabe zu isolieren.
...
2. **Schritte zur Reproduktion**: Eine klare Beschreibung der Handlungen, die das Problem Trigger or triggern or triggern.
3. **Erwartetes vs. tatsächliches Verhalten**: Was Sie erwartet haben und was stattdessen eingetreten ist.
4. **SDK or Software-Development-Kit-Version**: Die Version des Braze SDK or Software-Development-Kit, die Sie verwenden.
