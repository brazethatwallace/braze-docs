...
| **Universal Link** | `https://myapp.com/products/123` | E-Mail, Kurzmitteilungsdienst or SMS, Kanäle mit Klick-Tracking | Ja — Fallback auf Web |
...
- Sie kein Klick-Tracking benötigen (Link-Wrapping durch den E-Mail-E-Mail-Anbieter or ESP)
...
- Sie E-Mails senden. Ihr E-Mail-Anbieter or ESP wrappt Links für das Klick-Tracking, daher müssen Links HTTPS sein.
- Sie Kurzmitteilungsdienst or SMS oder andere Kanäle nutzen, bei denen Links gewrappt oder gekürzt werden.
- Der Link auf eine Webseite zurückfallen soll, wenn die App nicht installiert ist.
...
Diese Option öffnet eine Webseite in einem modalen WebView innerhalb Ihrer App. Sie wird vollständig vom Braze SDK or Software-Development-Kit über `Braze.WebViewController` verarbeitet — Sie müssen keinen Code zur URL-Verarbeitung schreiben.
...
| `Info.plist` | Registrierung or registrieren Sie Ihr Schema unter `CFBundleURLTypes` und fügen Sie es zu `LSApplicationQueriesSchemes` hinzu |
| App-Delegate-Methode | Implementieren Sie `application(_:open:options:)`, um die URL zu analysieren und zu navigieren |
| Braze SDK or Software-Development-Kit-Konfiguration | Keine – das SDK or Software-Development-Kit öffnet URLs mit benutzerdefiniertem Schema standardmäßig |
...
| Braze SDK or Software-Development-Kit-Konfiguration | Setzen Sie `configuration.forwardUniversalLinks = true` |
...
|---|---|
| AASA-Datei | Nicht erforderlich |
| App-Delegate-Methode | Nicht erforderlich – das SDK or Software-Development-Kit übernimmt dies automatisch |
| Braze SDK or Software-Development-Kit-Konfiguration | Keine – wählen Sie **Open Web URL Inside App** im Campaign-Composer aus |
...
- Sie Deeplinks in Kurzmitteilungsdienst or SMS-Campaigns versenden (da Links möglicherweise zu HTTPS-URLs gekürzt werden).
...
1. **Branch SDK or Software-Development-Kit**: Integrieren Sie das Branch SDK or Software-Development-Kit gemäß der [Dokumentation von Branch](https://help.branch.io/developers-hub/docs/native-sdks-overview).
...
3. **BrazeDelegate**: Implementieren Sie `braze(_:shouldOpenURL:)`, um Branch-Links an das Branch SDK or Software-Development-Kit weiterzuleiten, anstatt sie direkt von Braze verarbeiten zu lassen.
4. **Universelle Links weiterleiten**: Setzen Sie `configuration.forwardUniversalLinks = true` in Ihrer Braze-SDK or Software-Development-Kit-Konfiguration.
