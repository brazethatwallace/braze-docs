...
Das Braze Web SDK ermöglicht es Ihnen, die Customer-Engagement-Plattform von Braze direkt in Ihre Webanwendungen zu integrieren. Entwickelt mit TypeScript und konzipiert für moderne Webentwicklung, bietet dieses SDK umfassende Tools für Nutzerverwaltung, Messaging, Analytics und Feature-Flags.
...
- **Content Cards**: Zeigen Sie personalisierte Content-Feeds und Aktionskarten an, die sich in Echtzeit aktualisieren
...
- **Analytics**: Verfolgen Sie angepasste Events, Nutzerinteraktionen und Konversionsmetriken
...
| `enableLogging` | `boolean` | `false` | Auf true setzen, um das Logging standardmäßig zu aktivieren. Beachten Sie, dass Braze dadurch in die JavaScript-Konsole loggt, die für alle Nutzer:innen sichtbar ist! Sie sollten diese Option entfernen oder mit setLogger einen alternativen Logger bereitstellen, bevor Sie Ihre Seite in die Produktion überführen. |
| `allowUserSuppliedJavascript` | `boolean` | `false` | Standardmäßig erlaubt das Braze Web SDK keine benutzerdefinierten JavaScript-Klickaktionen und aktiviert weder HTML-In-App-Nachrichten noch Banner, da diese es Braze-Dashboard-Nutzer:innen ermöglichen, JavaScript auf Ihrer Website auszuführen. Um anzugeben, dass Sie den Braze-Dashboard-Nutzer:innen vertrauen, nicht-schädliche JavaScript-Klickaktionen zu schreiben, setzen Sie diese Eigenschaft auf true. |
...
| `contentSecurityNonce` | `string` | `undefined` | Wenn Sie einen Wert für diese Option angeben, fügt das Braze SDK den Nonce zu allen vom SDK erstellten `<script>`- und `<style>`-Elementen hinzu. Dies kann verwendet werden, um das Braze SDK mit der Content Security Policy Ihrer Website kompatibel zu machen. Beachten Sie, dass Sie zusätzlich zum Setzen dieses Nonce möglicherweise auch das Laden von FontAwesome erlauben müssen, indem Sie entweder `use.fontawesome.com` zu Ihrer Content Security Policy Allowlist hinzufügen oder die Option `doNotLoadFontAwesome` verwenden und es manuell laden. |
| `noCookies` | `boolean` | `false` | Standardmäßig verwendet das Braze Web SDK Cookies. Um die Cookie-Nutzung zu deaktivieren, setzen Sie diese Option auf true. Beachten Sie, dass das Deaktivieren von Cookies die Fähigkeit des SDK beeinträchtigen kann, die Identität von Nutzer:innen zwischen Sitzungen zu speichern. |
| `allowCrawlerActivity` | `boolean` | `false` | Standardmäßig ignoriert das Braze Web SDK Aktivitäten von bekannten Spidern oder Web-Crawlern, wie z. B. Google, basierend auf dem User-Agent-String. Dies spart Datenpunkte, macht Analytics genauer und kann das Seitenranking verbessern. Wenn Sie jedoch möchten, dass Braze Aktivitäten dieser Crawler protokolliert, können Sie diese Option auf true setzen. |
| `disablePushTokenMaintenance` | `boolean` | `false` | Standardmäßig synchronisieren Nutzer:innen, die bereits die Web-Push-Berechtigung erteilt haben (z. B. über requestPushPermission oder von einem früheren Push-Anbieter), ihr Push-Token bei neuen Sitzungen automatisch mit dem Braze-Backend, um die Zustellbarkeit sicherzustellen. Um dieses Verhalten zu deaktivieren, setzen Sie diese Option auf true. |
...
| `requireExplicitInAppMessageDismissal` | `boolean` | `false` | Standardmäßig können In-App-Nachrichten durch Klicken außerhalb der Nachricht oder durch Drücken der Escape-Taste geschlossen werden. Setzen Sie diese Option auf true, damit Nutzer:innen explizit auf einen Schließen-Button oder Aktions-Button klicken müssen, um die Nachricht zu schließen. |
...
#### In-App-Nachrichteninteraktionen protokollieren {#log-in-app-message-interactions}
...
Wenn Sie `dismissBanner(banner)` aufrufen, verwaltet das SDK den Ausblendungsstatus des Banners, entfernt das Banner aus den aktiven Banner-Updates, benachrichtigt die Abonnent:innen des Ausblendungs-Events und synchronisiert die Ausblendung mit Braze. Benutzerdefinierte UIs sollten `subscribeToBannersUpdates` verwenden, um auf das Entfernen des ausgeblendeten Banners zu reagieren, anstatt `dismissBanner` nur als lokale UI-Änderung oder reine Analytics-Protokollierungsmethode zu behandeln.
...
#### Sofortigen Datenversand anfordern {#request-data-flush}
...
Alternativ können Sie, wenn Sie webpack zum Bündeln Ihrer App verwenden, dessen Magic Comments nutzen, um nur die benötigten Teile des SDK dynamisch zu importieren.
...
Bei der Verwendung von Jest kann ein Fehler wie `SyntaxError: Unexpected token 'export'` auftreten. Um dies zu beheben, passen Sie Ihre Konfiguration in `package.json` an, um das Braze SDK zu ignorieren:
...
2. **Abo-Widgets hinzufügen**: Fügen Sie Widgets hinzu, damit sich Nutzer:innen an- und abmelden können
...
5. **AMP-Web-Push-Element konfigurieren**: Fügen Sie das `amp-web-push`-Element mit Ihrem API-Schlüssel und der Basis-URL als Abfrageparameter hinzu
...
Electron unterstützt Web-Push-Benachrichtigungen nicht offiziell (siehe: dieses [GitHub-Issue](https://github.com/electron/electron/issues/6697)). Es gibt weitere [Open-Source-Workarounds](https://github.com/MatthieuLemoine/electron-push-receiver), die Sie ausprobieren können, die jedoch nicht von Braze getestet wurden.
...
- **Push-Berechtigungen**: Rufen Sie `braze.requestPushPermission()` als Reaktion auf Nutzerinteraktionen auf (z. B. Button-Klicks). Verwenden Sie Soft-Push-Prompts (angepasste UI), bevor Sie die Browser-Berechtigung anfordern
...
Das Web SDK kann über ein benutzerdefiniertes HTML-Tag in Ihrem Google Tag Manager-Container initialisiert und aufgerufen werden. Sehen Sie sich unsere [Google Tag Manager-Beispiel-App](https://github.com/braze-inc/braze-web-sdk/blob/master/sample-builds/google-tag-manager) an, um ein Beispiel für das Senden von Events an Braze über GTM zu erhalten, oder lesen Sie unsere [Integrationsdokumentation](https://www.braze.com/docs/developer_guide/sdk_integration/google_tag_manager) für weitere Details.

#### Andere Tag-Manager {#other-tag-managers}

Braze ist möglicherweise auch mit anderen Tag-Management-Lösungen kompatibel, indem Sie unsere Integrationsanweisungen innerhalb eines benutzerdefinierten HTML-Tags befolgen. Wenden Sie sich an eine Braze-Vertretung, wenn Sie Hilfe bei der Bewertung dieser Lösungen benötigen.
...
Übergeben Sie die Option `enableLogging: true` an die Initialisierungsfunktion (`braze.initialize('YOUR-API-KEY-HERE', { baseUrl: 'YOUR-SDK-ENDPOINT', enableLogging: true });`), damit Braze Protokolleinträge in die JavaScript-Konsole schreibt. Dies ist für die Entwicklung hilfreich, aber für alle Nutzer:innen sichtbar. Entfernen Sie diese Option daher oder [stellen Sie einen alternativen Logger bereit](https://js.appboycdn.com/web-sdk/6.10/doc/modules/braze.html#setlogger), bevor Sie Ihre Seite in die Produktion überführen.
