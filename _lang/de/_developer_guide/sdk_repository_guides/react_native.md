...
1. Installieren Sie das npm-Paket (siehe [Installation](#installation)).
2. Führen Sie das **native Setup** für Android und iOS durch (Konfiguration, Berechtigungen, Push bei Bedarf).
...
Ein erneuter Aufruf von `Braze.initialize` mit anderen Zugangsdaten beendet die aktuelle Instanz und erstellt sie neu. So wird eine Neuinitialisierung während einer laufenden Sitzung unterstützt.
...
- Fügen Sie das **Kotlin-Gradle-Plugin** in Ihrer Root-Datei `build.gradle` hinzu, falls Ihr Template dies nicht bereits enthält (die Versionen hängen von Ihrer React Native-Version ab).
- Fügen Sie eine `braze.xml`-Ressourcendatei in `res/values` mit Ihrer Konfiguration hinzu. Aktivieren Sie die verzögerte Initialisierung, damit das SDK auf den Aufruf von `Braze.initialize()` aus JavaScript wartet, bevor es startet. Andere Konfigurationswerte (Push, Sitzungs-Timeout usw.) werden weiterhin aus dieser Datei gelesen und zum Zeitpunkt der Initialisierung angewendet.
...
- Für Push führen Sie die FCM-Integration und alle Braze-spezifischen Sender-ID- / Registrierungs-Flags durch, die in der Dokumentation beschrieben sind.
...
- **`postInitialization`-Closure** *(optional)*: Empfängt die aktive `Braze`-Instanz nach der Erstellung, für Einrichtungen, die die Instanz erfordern (z. B. Speichern einer Referenz, Setzen von Delegates).

{% alert note %}
`BrazeReactInitializer.configure` ist eine Swift-first-API, die die veraltete Methode `BrazeReactBridge.initBraze(_:)` ersetzt. Sie behebt auch ein Swift-Typauflösungsproblem mit `Braze.Configuration` in der Objective-C-Bridge.
{% endalert %}
---
...
| `com_braze_server_target` | `string` | Optionale Cluster-/Umgebungsauswahl (z. B. für interne oder Staging-Builds). Bevorzugen Sie `com_braze_custom_endpoint` für die Produktion, es sei denn, Ihre Braze-Integration gibt etwas anderes vor. |
...
| `com_braze_firebase_cloud_messaging_sender_id` | `string` | FCM-Absender-ID, wenn die automatische Registrierung aktiviert ist. |
| `com_braze_handle_push_deep_links_automatically` | `bool` | Lässt Braze Push-Deeplinks automatisch öffnen. |
| `com_braze_trigger_action_minimum_time_interval_seconds` | `integer` | Mindestanzahl an Sekunden zwischen In-App-Nachricht-Triggeraktionen. |
| **Sonstige** | *verschiedene* | Zusätzliche Schlüssel, die hier nicht aufgeführt sind (Sitzungs-Timeout, Geofences, Standort, Benachrichtigungsstandardwerte, Geräte-Allowlists, verzögerte Initialisierung, SDK-Authentifizierung und mehr). Siehe [`BrazeConfigurationProvider`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.configuration/-braze-configuration-provider/index.html) und den [Android-SDK-Integrationsleitfaden](https://www.braze.com/docs/developer_guide/platforms/android/sdk_integration/). |
...
| **Protokollierung** | `logger.level` | Ausführliche Protokollierung ist für die Entwicklung gedacht; reduzieren Sie die Ausgabe in der Produktion. |
...
| **Sitzungen** | `sessionTimeout` | Inaktivitätsdauer vor einer neuen Sitzung (siehe Braze-Sitzungsdokumentation). |
...
| **Netzwerk** | `api.requestPolicy`, `api.flushInterval` | Anfrage-Wiederholungsrichtlinie und Flush-Intervall. |
| **Push-Abo** | `optInWhenPushAuthorized` | Wenn `true`, kann das Abo nach der Benachrichtigungsautorisierung durch die Nutzer:innen auf „Opted-in“ wechseln. |
| **IAM + Nutzer:innenwechsel** | `preventInAppMessageDisplayForDifferentUser` | Reduziert fehlerhafte IAM-Anzeigen, wenn sich die Nutzer-ID ändert. |
...
Die React Native Bridge setzt beim Initialisieren React-spezifische **`api.sdkFlavor`**- / SDK-Metadaten; überschreiben Sie diese nicht, es sei denn, die Braze-Dokumentation weist Sie ausdrücklich dazu an.
...
- **`getInitialPushPayload`**: Verwenden Sie diese Methode, wenn die App aus einer Benachrichtigung geöffnet wird, um Race-Conditions mit RN `Linking` zu vermeiden. Sie erfordert native Hooks (`BrazeReactUtils` auf iOS, `BrazeReactUtils.populateInitialPushPayloadFromIntent` auf Android), wie in den TypeScript-Dokumentationskommentaren und der Beispiel-App beschrieben.
...
**`wipeData()`** — Löscht die **lokalen** Braze-Daten für diese Installation (zwischengespeicherter Nutzer-/Sitzungs-/Kartenstatus, Push-Token-Zuordnung usw.). Verwenden Sie es für **Abmelde-Verhalten**, wenn der vorherige Braze-Status der Nutzer:in nicht auf dem Gerät verbleiben darf, sowie für **„Meine Daten auf diesem Gerät löschen“**, **QA**-Resets ohne Neuinstallation oder strikte **Datenschutz**-Flows. **`changeUser`** allein führt diese Bereinigung nicht durch – es legt nur fest, welche Nutzer-ID **neue** Events erhält. Auf **iOS** kann sich das Verhalten von Android unterscheiden (z. B. bei der Interaktion mit dem deaktivierten SDK-Status); konsultieren Sie die nativen Braze-Dokumente, wenn Sie dies in der Produktion verwenden.

**`disableSDK()`** — Stoppt den Betrieb des SDK (keine Erfassung/Weiterleitung wie konfiguriert). Verwenden Sie es für **Nutzer-Opt-out**-Umschalter, **eingeschränkte Modi** (Compliance, Einstellungen für Kinder) oder zum **Debuggen** ohne Entfernung der Abhängigkeit.
...
- **New Architecture / Turbo Modules**: Wird in neueren Plugin-Versionen unterstützt. Befolgen Sie den Entwicklerleitfaden und die Beispiel-Einstellungen für `AppDelegate` / Gradle, wenn Sie migrieren.
...
- **Jest**: Mocken Sie die nativen `react-native`-Module oder das Braze Turbo Module (siehe `__tests__/jest.setup.js` in diesem Repository für Beispiele).
...
## Braze Expo Plugin {#braze-expo-plugin}
...
`BrazeProject` in diesem Repository ist eine vollständige Beispiel-App (Nutzer:innenverwaltung, Content Cards, Feature-Flags, Banner usw.).
...
Details zum Repository und Beispielprojekte finden Sie unter [https://github.com/braze-inc/braze-react-native-sdk](https://github.com/braze-inc/braze-react-native-sdk).