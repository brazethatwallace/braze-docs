...
- **Push-Benachrichtigungen**: Berechtigungsabfragen, Token-Registrierung, Payload-Listener (siehe **Push-Benachrichtigungen**)
- **Feature-Flags**: Aktualisieren, Eigenschaften auslesen, Impressionen protokollieren
...
Dieser Abschnitt zeigt die minimale Konfiguration, die zur Initialisierung des Braze React Native SDK erforderlich ist.
...
Ein erneuter Aufruf von `Braze.initialize` mit anderen Zugangsdaten beendet die aktuelle Instanz und erstellt sie neu. Dadurch wird eine Neuinitialisierung während der laufenden Sitzung unterstützt.

---

## Native Einrichtung {#native-setup}

> **Maßgebliche Quelle:** Schritt-für-Schritt-Anleitungen, Gradle-/CocoaPods-Änderungen und die vollständige Liste der Android-XML-Schlüssel finden Sie im [Braze React Native Entwicklerhandbuch](https://www.braze.com/docs/developer_guide/sdk_integration/?sdktab=react%20native). Die **Android**- und **iOS**-Snippets in diesem Abschnitt sind minimale Beispiele.

### Android

- Fügen Sie das **Kotlin Gradle Plugin** in Ihrer Root-`build.gradle` hinzu, falls Ihr Template es noch nicht enthält (Versionen hängen von Ihrer React Native-Version ab).
- Erstellen Sie eine `braze.xml`-Ressourcendatei unter `res/values` mit Ihrer Konfiguration. Aktivieren Sie die verzögerte Initialisierung, damit das SDK auf `Braze.initialize()` aus JavaScript wartet, bevor es startet. Weitere Konfigurationswerte (Push, Session-Timeout usw.) werden weiterhin aus dieser Datei gelesen und zum Zeitpunkt der Initialisierung angewendet.
...
- **`configure`-Closure**: Empfängt eine `Braze.Configuration` und ermöglicht es Ihnen, native Konfigurationseigenschaften festzulegen (Logging, Push, Sessions usw.). Der API-Schlüssel und der Endpunkt werden aus JavaScript bereitgestellt – Sie legen sie hier nicht fest.
...
{% alert note %}
** `BrazeReactInitializer.configure` ist eine Swift-First-API, die das veraltete `BrazeReactBridge.initBraze(_:)` ersetzt. Sie behebt außerdem ein Problem mit der Swift-Typauflösung von `Braze.Configuration` in der Objective-C-Bridge.
{% endalert %}
---
...
| `com_braze_api_key` | `string` | Nicht erforderlich bei Verwendung von `Braze.initialize()` aus JavaScript (Zugangsdaten werden aus JS übergeben). Nur bei herkömmlicher nativer Initialisierung erforderlich. |
| `com_braze_custom_endpoint` | `string` | Nicht erforderlich bei Verwendung von `Braze.initialize()` aus JavaScript. Nur bei herkömmlicher nativer Initialisierung erforderlich. |
...
| `com_braze_trigger_action_minimum_time_interval_seconds` | `integer` | Mindestanzahl an Sekunden zwischen Trigger-Aktionen für In-App-Nachrichten. |
| **Sonstige** | *verschiedene* | Hier nicht aufgeführte zusätzliche Schlüssel (Session-Timeout, Geofences, Standort, Benachrichtigungsstandards, Geräte-Allowlists, verzögerte Initialisierung, SDK-Authentifizierung und mehr). Siehe [`BrazeConfigurationProvider`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.configuration/-braze-configuration-provider/index.html) und den [Android-SDK-Integrationsleitfaden](https://www.braze.com/docs/developer_guide/platforms/android/sdk_integration/). |
...
| **Protokollierung** | `logger.level` | Ausführliche Protokollierung ist für die Entwicklung gedacht; reduzieren Sie den Umfang in der Produktion. |
| **Push** | `push.automation`, `push.appGroup`, … | Automatisierung vereinfacht die Registrierung; `appGroup` wird für Push Stories / Erweiterungen benötigt, wenn diese verwendet werden. |
| **In-App-Nachrichten** | `triggerMinimumTimeInterval` | Standardmäßig **30** Sekunden zwischen Triggern. |
| **Sessions** | `sessionTimeout` | Inaktivitätsdauer bis zu einer neuen Session (siehe Braze-Session-Dokumentation). |
| **Datenschutz / Daten** | `api.trackingPropertyAllowList`, `devicePropertyAllowList`, `api.sdkAuthentication` | Abstimmung mit dem [Privacy Manifest](https://www.braze.com/docs/developer_guide/platform_integration_guides/swift/privacy_manifest/) und den SDK-Authentifizierungseinstellungen des Produkts. |
...
| **IAM + Nutzer:innenwechsel** | `preventInAppMessageDisplayForDifferentUser` | Reduziert falsch zugeordnete In-App-Nachrichten bei einem Wechsel der Nutzer-ID. |
...
- Mit der **Standard-Braze-UI** folgen Sie der [Dokumentation zu In-App-Nachrichten](https://www.braze.com/docs/developer_guide/in_app_messages?sdktab=react%20native); normalerweise müssen Sie `subscribeToInAppMessage` **nicht** aufrufen, um die Standard-UI anzuzeigen.
- Für **benutzerdefinierte** Verarbeitung abonnieren Sie mit `useBrazeUI: false` und loggen dann Impressionen/Klicks nach Bedarf:
...
- **`Braze.addListener(Braze.Events.PUSH_NOTIFICATION_EVENT, ...)`** ist laut den öffentlichen Typings **nur für Android** verfügbar.
...
Hinweis: `logPurchase` erwartet den **Preis als String** (siehe Typings).

### Datenverwaltung und SDK-Status

**`changeUser`** teilt Braze lediglich mit, welcher Nutzer-ID **neue** Aktivitäten zugeordnet werden sollen. Es löscht **keine** zwischengespeicherten SDK-Daten auf dem Gerät. Es gibt keine separate „Logout“-API: Wenn Sie eine herkömmliche Abmeldung benötigen (den lokalen Braze-Status leeren, damit das zwischengespeicherte Profil, die Nachrichten und Token des vorherigen Nutzers auf dieser Installation entfernt werden), verwenden Sie in der Regel **`wipeData()`**. Dabei handelt es sich um einen vollständigen lokalen Reset.
...
**`wipeData()`** — Löscht die **lokalen** Braze-Daten für diese Installation (zwischengespeicherter Nutzer-/Sitzungs-/Kartenstatus, Push-Token-Zuordnung usw.). Verwenden Sie dies für **Abmeldeverhalten**, wenn Sie den Braze-Status des vorherigen Nutzers nicht auf dem Gerät belassen dürfen, sowie für **„Meine Daten auf diesem Gerät löschen“**, **QA**-Resets ohne Neuinstallation oder strikte **Datenschutz**-Abläufe. **`changeUser`** allein führt diese Bereinigung nicht durch — es legt nur fest, welche Nutzer-ID **neue** Events empfängt. Auf **iOS** kann das Verhalten von Android abweichen (z. B. Zusammenspiel mit dem deaktivierten SDK-Status); konsultieren Sie die nativen Braze-Dokumentationen, wenn Sie dies in der Produktion einsetzen.

**`disableSDK()`** — Stoppt den Betrieb des SDK (keine Datenerhebung/-weiterleitung wie konfiguriert). Verwenden Sie dies für **Nutzer:innen-Opt-out**-Schalter, **eingeschränkte Modi** (Compliance, Kinderschutz-Einstellungen) oder **Debugging** ohne Entfernung der Abhängigkeit.

**`enableSDK()`** — Aktiviert das SDK nach **`disableSDK()`** wieder. Auf **iOS** wird die Reaktivierung möglicherweise **erst beim nächsten App-Start** wirksam; überprüfen Sie die Braze Swift/iOS-Dokumentation, bevor Sie sich auf eine sofortige Reaktivierung verlassen.
...
- **Expo**: Verwenden Sie nach Möglichkeit das [Braze Expo Plugin](https://github.com/braze-inc/braze-expo-plugin), um manuelles natives Einrichten zu vermeiden.
- **New Architecture / Turbo Modules**: Wird in neueren Plugin-Versionen unterstützt. Folgen Sie dem Entwicklerleitfaden und den Beispieleinstellungen für `AppDelegate` / Gradle, wenn Sie migrieren.
- **Datenschutz (iOS)**: Methoden wie `updateTrackingPropertyAllowList` unterstützen die Konfiguration im Zusammenhang mit dem Datenschutzmanifest. Weitere Informationen finden Sie unter [Swift-Datenschutzmanifest](https://www.braze.com/docs/developer_guide/platform_integration_guides/swift/privacy_manifest/).

- **Jest**: Mocken Sie die nativen Module von `react-native` oder das Braze-Turbo-Modul (siehe `__tests__/jest.setup.js` in diesem Repository für Beispiele).
...
Aktivieren Sie das Braze-Logging in der **nativen** Konfiguration während der Entwicklung, damit das SDK in die Systemkonsole schreibt (Xcode / Android Logcat). Dies hilft bei der Überprüfung der Initialisierung, von Nutzer:innenänderungen und der Event-Zustellung.
...
- **Android** — Verwenden Sie die Ressource `com_braze_logger_initial_log_level` in `braze.xml` oder setzen Sie den entsprechenden Wert auf `BrazeConfig.Builder` (siehe [BrazeConfigurationProvider](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.configuration/-braze-configuration-provider/logger-initial-log-level.html)). Verwenden Sie vor der Veröffentlichung ein weniger detailliertes Log-Level oder entfernen Sie die Überschreibung.
...
- [Braze-Entwicklerhandbuch — React Native](https://www.braze.com/docs/developer_guide/sdk_integration/?sdktab=react%20native)
...
Einzelheiten zum Repository und Beispielprojekte finden Sie unter [https://github.com/braze-inc/braze-react-native-sdk](https://github.com/braze-inc/braze-react-native-sdk).