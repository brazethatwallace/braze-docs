...
{% alert important %}
Damit das Standort-Tracking in iOS 14 auch für den ungefähren Standort zuverlässig funktioniert, müssen Sie Ihr SDK or Software-Development-Kit mindestens auf die Version `3.26.1` Update or aktualisieren or aktualisieren.
{% endalert %}

## Automatisches Standort-Tracking aktivieren

Ab Braze iOS SDK or Software-Development-Kit `v3.17.0` ist das Standort-Tracking standardmäßig deaktiviert. Sie können das automatische Standort-Tracking über die Datei `Info.plist` aktivieren. Fügen Sie das Wörterbuch `Braze` zu Ihrer Datei `Info.plist` hinzu. Fügen Sie im Wörterbuch `Braze` den booleschen Untereintrag `EnableAutomaticLocationCollection` hinzu und setzen Sie den Wert auf `YES`. Beachten Sie, dass vor Braze iOS SDK or Software-Development-Kit v4.0.2 der Wörterbuchschlüssel `Appboy` anstelle von `Braze` verwendet werden muss.
