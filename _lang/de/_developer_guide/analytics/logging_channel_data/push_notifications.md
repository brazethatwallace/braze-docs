...
Sie können Analytics in Realtime protokollieren, indem Sie Aufrufe an den [`/users/track`-Endpunkt]({{site.baseurl}}/api/endpoints/user_data/post_user_track) senden. Um Analytics zu protokollieren, senden Sie den `braze_id`-Wert aus dem Braze-Dashboard, um festzustellen, welches Nutzerprofil aktualisiert werden soll.
