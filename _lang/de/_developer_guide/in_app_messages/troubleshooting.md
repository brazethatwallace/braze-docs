...
article_title: Fehlerbehebung für In-App-Nachrichten im Braze SDK or Software-Development-Kit
page_order: 50
description: "Diagnostizieren Sie, warum In-App-Nachrichten nicht zugestellt oder angezeigt werden, mithilfe eines Symptomindex, eines standardisierten Untersuchungspfads, Canvas-IAM-Hinweisen und plattformspezifischen SDK or Software-Development-Kit-Prüfungen."
channel:
  - in-app messages
...
| Trigger or triggern wurden zurückgegeben, aber nichts wird auf dem Gerät angezeigt | [Plattformspezifische Fehlerbehebung bei der Anzeige](#platform-specific-display-troubleshooting) |
...
2. Öffnen Sie die [Event-Nutzerprotokolle]({{site.baseurl}}/user_guide/administer/global/workspace_settings/logs_and_alerts/event_user_log) und suchen Sie die SDK or Software-Development-Kit-Anfrage für diesen Sitzungsstart. In den **Antwortdaten**:
   ...
   - **Trigger or triggern In-App Message**-Zeilen listen jede In-App-Nachricht auf, die für diese Anfrage zurückgegeben wurde.
   - Wenn kein `triggers`-Schlüssel oder keine **Trigger or triggern In-App Message**-Zeilen vorhanden sind, gehen Sie zu [Fehlerbehebung: Nachrichten werden nicht angefordert](#troubleshoot-messages-not-being-requested).
   ...
   - Wenn **Trigger or triggern In-App Message**-Zeilen vorhanden sind, aber nichts angezeigt wird, gehen Sie zu [Plattformspezifische Fehlerbehebung bei der Anzeige](#platform-specific-display-troubleshooting).
   - Jeder Trigger or triggern-Payload enthält einen `type`: `inapp` (Standard) oder `templated_iam` (erfordert eine Template-Anfrage vor der Anzeige). Siehe [Typen von In-App-Nachrichten]({{site.baseurl}}/developer_guide/in_app_messages/triggering_messages#types-of-in-app-messages).
...
4. Bei geräteseitigen Anzeigeproblemen (Delegates, Rate-Limits, Ausrichtung, Sitzungs-Timeout) wählen Sie Ihren SDK or Software-Development-Kit-Tab unter [Plattformspezifische Fehlerbehebung bei der Anzeige](#platform-specific-display-troubleshooting).
...
{% alert important %}
In-App-Nachrichten in Canvas können nur durch Ereignisse getriggert werden, die über das SDK or Software-Development-Kit gesendet werden, nicht über die Representational State Transfer API.
{% endalert %}
...
- War der/die Nutzer:in beim **Sitzungsstart** im Segment, wenn das SDK or Software-Development-Kit neue In-App-Nachrichten anfordert?
...
- Wurde die Nachricht durch das standardmäßige Mindestintervall von 30 Sekunden zwischen Trigger or triggern or triggern unterdrückt? Siehe [Überschreiben des Standard-Rate-Limits]({{site.baseurl}}/developer_guide/in_app_messages/triggering_messages#overriding-the-default-rate-limit).
...
| Angepasste UI oder Handler unterdrückt die Anzeige | Überprüfen Sie Delegates (Mobilgerät) oder [`braze.subscribeToInAppMessage`](https://js.appboycdn.com/web-sdk/latest/doc/modules/braze.html#subscribetoinappmessage) (Web). Siehe [Anpassung]({{site.baseurl}}/developer_guide/in_app_messages/customization) und Ihren SDK or Software-Development-Kit-Tab für Ihre Plattform. |
...
| Trigger or triggern wurde auf dem Gerät nicht ausgelöst | Der Trigger or triggern muss lokal über das SDK or Software-Development-Kit erfolgen. Ein Representational State Transfer-API-Aufruf kann keine In-App-Nachricht im SDK or Software-Development-Kit Trigger or triggern or triggern. Siehe [Nachrichten Trigger or triggern or triggern]({{site.baseurl}}/developer_guide/in_app_messages/triggering_messages). |
...
- Ist die Trigger or triggern-Aktion im Dashboard und in der App-Integration korrekt konfiguriert?
...
- Verwenden Sie eine aktuelle SDK or Software-Development-Kit-Version? Einige In-App-Nachrichtentypen haben Mindestanforderungen an die SDK or Software-Development-Kit-Version.
...
- **Sitzungsstart-Prefetch der Campaign:** In-App-Nachrichten werden beim Sitzungsstart zwischengespeichert und angezeigt, wenn der Trigger or triggern ausgelöst wird. Ein Trigger or triggern, der vor dem nächsten Sitzungsstart auftritt, wird erst in dieser Sitzung angezeigt. Siehe [Nachrichten Trigger or triggern or triggern]({{site.baseurl}}/developer_guide/in_app_messages/triggering_messages).
...
- **Trigger or triggern-Synchronisierungs-Race-Condition:** Wenn Nutzer:innen ein Ereignis unmittelbar nach dem Sitzungsstart protokollieren, sind die Trigger or triggern möglicherweise noch nicht synchronisiert. Erwägen Sie, den Trigger or triggern auf den Sitzungsstart zu setzen und nach dem beabsichtigten Ereignis zu segmentieren, damit die Zustellung in der nächsten Sitzung nach dem Ereignis erfolgt.
...
- **Große Assets oder langsames CDN:** Optimieren Sie Bilder und Videos für HTML-In-App-Nachrichten. Auf Mobilgeräten können Bilder bei langsamen Netzwerken vor der Anzeige heruntergeladen werden – wählen Sie Ihren SDK or Software-Development-Kit-Tab für plattformspezifische Hinweise.

{% alert note %}
Wenn Ihre In-App-Nachricht durch den Sitzungsstart getriggert wird und Sie ein verlängertes Sitzungs-Timeout festgelegt haben, wird das Schließen und erneute Öffnen der App innerhalb dieses Zeitfensters die Sitzung nicht Update or aktualisieren or aktualisieren. Beispiel: Bei einem 300-Sekunden-Timeout wird eine Sitzungsstart-In-App-Nachricht erst angezeigt, wenn die Sitzung tatsächlich aktualisiert wird. Passen Sie das Sitzungs-Timeout oder den Trigger or triggern-Typ an, wenn dies Ihren Test beeinflusst.
{% endalert %}

## Fehlerbehebung bei der Zustellung {#delivery-troubleshooting}

Die meisten Probleme mit In-App-Nachrichten betreffen die **Zustellung** (das Gerät hat keine Trigger or triggern erhalten) oder die **Anzeige** (Trigger or triggern sind angekommen, wurden aber nicht angezeigt). Bestätigen Sie zuerst die [Zustellung](#troubleshooting-in-app-message-delivery) und prüfen Sie dann die [Anzeige](#platform-specific-display-troubleshooting).

### Fehlerbehebung bei der Zustellung {#troubleshooting-in-app-message-delivery}

Das SDK or Software-Development-Kit fordert In-App-Nachrichten beim Sitzungsstart von den Braze-Servern an. Bestätigen Sie, dass das SDK or Software-Development-Kit Trigger or triggern anfordert und Braze diese zurückgibt.
...
4. Suchen Sie in den [Event-Nutzerprotokollen]({{site.baseurl}}/user_guide/administer/global/workspace_settings/logs_and_alerts/event_user_log) die SDK or Software-Development-Kit-Anfrage für das Sitzungsstart-Ereignis. In den **Antwortdaten**:
   - Bestätigen Sie im Roh-JSON, dass `respond_with` `"triggers": true` enthält.
   ...
   Dann triagieren:
   - Wenn kein `triggers`-Schlüssel oder keine **Trigger or triggern In-App Message**-Zeilen vorhanden sind, siehe [Fehlerbehebung: Nachrichten werden nicht angefordert](#troubleshoot-messages-not-being-requested).
   ...
   - Wenn **Trigger or triggern In-App Message**-Zeilen vorhanden sind, aber nichts auf dem Gerät angezeigt wird, siehe [Plattformspezifische Fehlerbehebung bei der Anzeige](#platform-specific-display-troubleshooting).
   - Jeder Trigger or triggern-Payload enthält einen `type`: `inapp` (Standard) oder `templated_iam` (erfordert eine Template-Anfrage vor der Anzeige). Siehe [Typen von In-App-Nachrichten]({{site.baseurl}}/developer_guide/in_app_messages/triggering_messages#types-of-in-app-messages).
5. Bestätigen Sie, dass die richtigen In-App-Nachrichten in den Antwortdaten erscheinen.

![Event-Nutzerprotokoll mit SDK or Software-Development-Kit-Anfragen und Antwortdaten.]({% image_buster /assets/img_archive/event_user_log_iams.png %})
...
![Die SDK or Software-Development-Kit-Anfrage in den Event-Nutzerprotokollen, die ein erfolgreiches Sitzungsstart-Ereignis anzeigt.]({% image_buster /assets/img_archive/event_user_log_session_start.png %})
...
Informationen zu archivierten Campaigns, Trigger or triggern-Konfiguration und Ruhezeiten finden Sie in den [FAQ zu In-App-Nachrichten]({{site.baseurl}}/user_guide/channels/in_app_messages/faq).
...
Wenn Sie einen Delegate oder angepassten Handler verwenden, um In-App-Nachrichten manuell anzuzeigen, müssen Sie Impressionen und Klicks selbst protokollieren. Siehe Ihren SDK or Software-Development-Kit-Tab unter [Plattformspezifische Fehlerbehebung bei der Anzeige](#platform-specific-display-troubleshooting) für Swift- und Android-Details oder [In-App-Nachrichtendaten protokollieren]({{site.baseurl}}/developer_guide/in_app_messages/logging_message_data) für Web.
...
Wenn **Trigger or triggern In-App Message**-Zeilen in den Event-Nutzerprotokollen erscheinen, aber nichts auf dem Gerät angezeigt wird, wählen Sie Ihren SDK or Software-Development-Kit-Tab für Anzeige-Prüfungen (Delegates, Rate-Limits, Ausrichtung und angepasste Handler).
