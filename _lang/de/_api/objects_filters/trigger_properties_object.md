---
nav_title: "Trigger or triggern-Eigenschaften-Objekt"
article_title: API-Trigger or triggern-Eigenschaften-Objekt
page_order: 11
page_type: reference
description: "Dieser Referenzartikel erläutert die verschiedenen Komponenten des Trigger or triggern-Eigenschaften-Objekts."
tool: Campaigns

---

# Trigger or triggern-Eigenschaften-Objekt {#trigger-properties-object}
...
Beachten Sie, dass Trigger or triggern-Eigenschaften zwar als Template in Nachrichten eingefügt werden können, aber standardmäßig nicht automatisch im Kundenprofil or Nutzerprofil gespeichert werden.

{% alert note %}
Das Objekt `trigger_properties` und die Syntax {% raw %}`api_trigger_properties.${product_name}`{% endraw %} werden nur in Campaigns unterstützt. Um Nachrichten mit Schlüsseln und Werten aus einer API-Trigger or triggern-Anfrage für Canvas anzupassen, verwenden Sie das [Canvas-Eingangs-Eigenschaften-Objekt]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/context). Das Objekt `trigger_properties` hat eine maximale Größe von 50 KB.
{% endalert %}
...
Referenzieren Sie Trigger or triggern-Eigenschaften in Ihren Nachrichten-Templates mithilfe des `api_trigger_properties`-Namespace:
