...
Gehen Sie zu **Messaging** > **Campaigns** und wählen Sie **Create Campaign** aus. Wählen Sie dann **API Campaigns** aus. Nun können Sie mit der Konfiguration Ihrer API-Campaign fortfahren.
...
## Campaign konfigurieren {#configure-your-campaign}
...
2. Wählen Sie **Nachricht hinzufügen** aus und fügen Sie die Nachrichtentypen hinzu, die in Ihrer API-Campaign enthalten sind. So können Sie eine `campaign_id` und eine Nachrichtenvarianten-ID generieren, die sich für jeden enthaltenen Kanal unterscheidet.
...
{% raw %}
Standardmäßig fügt Braze den Header für die Ein-Klick-Abmeldung per Liste nicht zu API-Campaigns hinzu. Sie können diesen Header zu einzelnen API-Campaign-Sendungen hinzufügen, indem Sie den Liquid-Tag `{{${set_user_to_one_click_list_unsubscribe}}}` im Feld für E-Mail-Header Ihrer API-Anfrage einfügen.
{% endraw %}
