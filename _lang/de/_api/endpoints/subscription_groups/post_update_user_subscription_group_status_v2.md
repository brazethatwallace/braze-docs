...
{% api %}
# Abo-Gruppenstatus von Nutzer:innen aktualisieren (V2) {#update-users-subscription-group-status-v2}
{% apimethod post %}
...
{% alert note %}
Wenn Sie diesen Endpunkt mit [LINE-Abo-Gruppen]({{site.baseurl}}/user_guide/channels/line/message_users/subscription_groups) verwenden möchten, wenden Sie sich bitte an Ihren geschäftskunden-Success-Manager. <br><br>Für LINE-Abo-Gruppen empfehlen wir, ein angepasstes Attribut zu verwenden, um die Einwilligung über die Website oder App separat zu verfolgen, und dann Kampagnen mithilfe dieses angepassten Attributs in Kombination mit dem LINE-Abo-Status zu targeten. Dieser Ansatz stellt sicher, dass Ihr Abo-Status korrekt die Nutzer:innen widerspiegelt, die sich tatsächlich in der LINE-App angemeldet haben. Das manuelle Hinzufügen von Nutzer:innen zu LINE-Abo-Gruppen über die API kann zu nicht synchronisierten Zuständen und fehlgeschlagenen Sendungen führen, da Braze Nutzer:innen nicht erneut in der LINE-App abonnieren oder Nachrichten an Nutzer:innen senden kann, die ein Konto in LINE blockiert haben.
{% endalert %}
...
## SMS und WhatsApp {#sms-and-whatsapp}
