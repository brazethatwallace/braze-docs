...
> Verwenden Sie diesen Endpunkt, um verwandte benutzerdefinierte Objekte von einem Objektanker aus aufzulisten.
...
Die folgende Tabelle listet und beschreibt die Abfrageparameter für den Endpunkt `/custom_objects/objects/{type_name}/{external_id}/object_relationships`.

| Parameter | Erforderlich | Datentyp | Beschreibung |
|---|---|---|---|
| `anchor` | Optional | String | `source` (Standard) oder `target` |
| `rel_kind` | Optional | String | Nach einem Beziehungstyp filtern |
...
Der Statuscode `200` kann folgenden Antworttext zurückgeben.
...
Bei `anchor=target` werden verwandte Objekte als `from_custom_object` zurückgegeben.
...
| Parameter | Erforderlich | Datentyp | Beschreibung |
|---|---|---|---|
| `items` | Erforderlich | Array | Liste der Objektbeziehungs-Datensätze |
| `items[].rel_kind` | Erforderlich | String | Wert des Beziehungstyps |
| `items[].to_custom_object` | Bedingt | Object | Verwandtes Objekt bei `anchor=source` |
| `items[].from_custom_object` | Bedingt | Object | Verwandtes Objekt bei `anchor=target` |
| `items[].to_custom_object.type_name` | Bedingt | String | Typname des verwandten Objekts |
| `items[].to_custom_object.external_id` | Bedingt | String | Externe ID des verwandten Objekts |
| `items[].to_custom_object.attributes` | Bedingt | Object | Attribute des verwandten Objekts |
| `items[].from_custom_object.type_name` | Bedingt | String | Typname des verwandten Objekts |
| `items[].from_custom_object.external_id` | Bedingt | String | Externe ID des verwandten Objekts |
| `items[].from_custom_object.attributes` | Bedingt | Object | Attribute des verwandten Objekts |
...
| `403` | API-Schlüssel hat keine Berechtigung oder die Anfrage wird durch eine Zulassungsliste blockiert | Stellen Sie sicher, dass der Schlüssel die Berechtigung `custom_objects.read` besitzt und dass Ihre Quell-IP auf der Schlüssel-Zulassungsliste steht, sofern konfiguriert. |
