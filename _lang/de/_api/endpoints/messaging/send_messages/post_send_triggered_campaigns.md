...
article_title: "Campaign-Nachrichten mit API-getriggerter Zustellung versenden"
search_tag: Endpoint
page_order: 4
layout: api_page
page_type: reference
...
| `trigger_properties` | Optional | Objekt | Siehe [Trigger or triggern-Eigenschaften]({{site.baseurl}}/api/objects_filters/trigger_properties_object). Personalisierte Schlüssel-Wert-Paare gelten für alle Nutzer:innen in dieser Anfrage. |
...
| `recipients` | Optional | Array | Siehe [Empfänger:innen-Objekt]({{site.baseurl}}/api/objects_filters/recipient_object).<br><br>Wenn `send_to_existing_only` `false` ist, muss ein `attributes`-Objekt enthalten sein.<br><br>Sie können den Abo-Gruppenstatus einer Nutzer:in Update or aktualisieren or aktualisieren, indem Sie `subscription_groups` in das verschachtelte `attributes`-Objekt aufnehmen. Weitere Einzelheiten finden Sie unter [Nutzerattribute-Objekt]({{site.baseurl}}/api/objects_filters/user_attributes_object).<br><br>Wenn `recipients` nicht angegeben und `broadcast` auf true gesetzt ist, wird die Nachricht an das gesamte Segment gesendet, das im Braze-Dashboard als Zielgruppe der Campaign konfiguriert ist.<br><br>Wenn `email` der Bezeichner ist, müssen Sie [`prioritization`]({{site.baseurl}}/api/endpoints/user_data/post_user_identify#identifying-users-by-email-addresses-and-phone-numbers) in das Empfänger:innen-Objekt aufnehmen. |
| `attachments` | Optional | Array | Wenn `broadcast` auf true gesetzt ist, kann die Liste `attachments` nicht einbezogen werden. <br><br>Wenn eine Anhang-URL eine Anmeldung erfordert, fügen Sie `basic_auth_credential` zu diesem Anhang hinzu und setzen Sie den Wert auf den Namen einer gespeicherten Basic-Authentication-Anmeldeinformation. Informationen zum Einrichten einer Anmeldeinformation finden Sie unter [Authentifizierung für E-Mail-Dateianhänge]({{site.baseurl}}/api/objects_filters/messaging/email_object#authentication-for-email-file-attachments). |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3  .reset-td-br-4 aria-label="Anfrageparameter" }

### Verhalten bei der Empfänger:innen-Auflösung {#recipient-resolution-behavior}

In diesem Abschnitt wird erläutert, wie Braze ein Kundenprofil or Nutzerprofil für den Versand auswählt und was passiert, wenn kein Profil ausgewählt wird.
...
- **E-Mail- und Kurzmitteilungsdienst or SMS-Adressierung.** Für die meisten E-Mail- oder Kurzmitteilungsdienst or SMS-API-getriggerten Sendungen an Personen, die noch nicht in Braze vorhanden sind, fügen Sie die benötigten Zustellungsfelder in `attributes` ein (z. B. `email` oder die Telefon-Attribute, die Ihr Workspace für Kurzmitteilungsdienst or SMS verwendet). Sie können dort auch die Abo-Gruppenmitgliedschaft oder den Abo-Status festlegen, wenn sich der Opt-in-Status im selben Aufruf ändern muss.
...
- Wenn `prioritization` nicht genau ein Kundenprofil or Nutzerprofil zurückgibt, wiederholt Braze die Auflösung bis zu 40 Mal. Dieses Wiederholungsverhalten ist erwartungsgemäß.
...
Wenn Sie eine reine E-Mail-Campaign für eine Empfänger:in Trigger or triggern or triggern, die über `external_user_id` oder `user_alias` identifiziert wird, und dieses Kundenprofil or Nutzerprofil zum Zeitpunkt des Aufrufs keine E-Mail-Adresse hat, wiederholt Braze den Versand für bis zu ca. 2 Stunden. Dies deckt das gängige Muster ab, bei dem eine Nutzer:in erstellt und ihre E-Mail-Adresse kurz darauf festgelegt wird. Um ohne Verzögerung zu senden, fügen Sie das `email`-Attribut in `recipients[].attributes` ein, damit die Adresse im selben Aufruf wie der Trigger or triggern gesetzt wird.
...
Braze verfügt über ein Messaging-Objekt namens `attributes`, mit dem Sie Attribute und Werte für eine Nutzer:in hinzufügen, erstellen oder Update or aktualisieren or aktualisieren können, bevor Sie ihr eine API-getriggerte Campaign senden. Verwenden Sie den `campaign/trigger/send`-Endpunkt, da dieser API-Aufruf das Nutzerattribute-Objekt verarbeitet, bevor er die Campaign verarbeitet und versendet. Dadurch wird das Risiko von Problemen, die durch [Race-Conditions]({{site.baseurl}}/user_guide/messaging/ab_testing/concepts/race_conditions) verursacht werden, minimiert.

{% alert tip %}
Sie suchen die Canvas-Version dieses Endpunkts? Informieren Sie sich über das [Versenden von Canvas-Nachrichten mit API-getriggerter Zustellung]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_triggered_canvases).
{% endalert %}
