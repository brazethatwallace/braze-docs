...
## Manuelles Aufzeichnen von Daten {#logging-data-manually}

Für die manuelle Protokollierung müssen Sie zunächst Workspaces in Xcode konfigurieren und anschließend Analytics erstellen, speichern und abrufen. Dies erfordert einige angepasste Entwicklungsarbeit auf Ihrer Seite. Die folgenden Code-Snippets helfen Ihnen dabei.

Beachten Sie, dass Analytics erst dann an Braze gesendet werden, wenn die mobile Anwendung anschließend gestartet wird. Das bedeutet, dass je nach Ihren Einstellungen für das Schließen oft eine unbestimmte Zeitspanne zwischen dem Schließen einer Push-Benachrichtigung und dem Start der mobilen App sowie dem Abrufen der Analytics vergeht. Auch wenn dieser Zeitpuffer nicht alle Anwendungsfälle betrifft, sollten Sie diese Auswirkung berücksichtigen und Ihre User Journey entsprechend anpassen, sodass das Öffnen der Anwendung einbezogen wird, um dieses Problem zu lösen.

![Eine Grafik, die die Verarbeitung von Analytics in Braze beschreibt. 1. Analysedaten werden erstellt. 2. Analysedaten werden gespeichert. 3. Die Push-Benachrichtigung wird geschlossen. 4. Unbestimmte Zeitspanne zwischen dem Schließen der Push-Benachrichtigung und dem Start der mobilen App. 5. Die mobile App wird gestartet. 6. Analysedaten werden empfangen. 7. Analysedaten werden an Braze gesendet.]({% image_buster /assets/img/push_implementation_guide/push13.png %})

### Schritt 1: App-Gruppen in Xcode konfigurieren {#step-1-configure-app-groups-within-xcode}

Fügen Sie in Xcode die Funktion `App Groups` hinzu. Wenn Sie noch keine Workspaces in Ihrer App hatten, gehen Sie zu den Fähigkeiten des Haupt-Targets der App, aktivieren Sie `App Groups` und klicken Sie auf den Button **+** Hinzufügen. Verwenden Sie dann die Bundle-ID Ihrer App, um den Workspace zu erstellen. Wenn die Bundle-ID Ihrer App zum Beispiel `com.company.appname` lautet, können Sie Ihren Workspace `group.com.company.appname.xyz` nennen. Vergewissern Sie sich, dass `App Groups` sowohl für das Haupt-Target Ihrer App als auch für das Target der Inhaltserweiterung aktiviert ist.

![Xcode-Bildschirm „Signing & Capabilities“ mit aktivierten App-Gruppen für das Haupt-Target und das Erweiterungs-Target.]({% image_buster /assets/img/swift/push_story/add_app_groups.png %})
...
#### Angepasste Events speichern {#saving-custom-events}
...
3. Wenn ein bestehendes Array vorhanden ist, fügen Sie die neuen Daten an das bestehende Array an und speichern Sie
4. Wenn kein bestehendes Array vorhanden ist, speichern Sie das neue Array in `userDefaults`
...
4. Jedes andere Schlüssel-Wert-Paar wird dem Wörterbuch `properties` hinzugefügt
5. Einzelnes angepasstes Event protokollieren
...
3. Einzelne angepasste Attribute mit entsprechendem Schlüssel und Wert protokollieren
...
#### Nutzerattribute speichern {#saving-user-attributes}

Beim Speichern von Nutzerattributen empfehlen wir, ein angepasstes Objekt zu erstellen, um zu ermitteln, welche Art von Attribut aktualisiert wird (`email`, `first_name`, `phone_number` usw.). Das Objekt sollte mit der Speicherung und dem Abruf über `UserDefaults` kompatibel sein. In der Hilfsdatei `UserAttribute` finden Sie ein Beispiel dafür, wie Sie dies umsetzen können.
