...
| `get_campaign_dataseries` | [`/campaigns/data_series`]({{site.baseurl}}/api/endpoints/export/campaigns/get_campaign_analytics) | read | Tägliche Zeitreihe der Campaign-Statistiken über einen Zeitraum (Sends, Öffnungen, Klicks, Konversionen nach Kanal). |
...
| `get_campaign_draft`<sup>*</sup> | N/A | read | Rufen Sie die Details eines Campaign-Entwurfs ab. |
| `get_campaign_live_details`<sup>*</sup> | N/A | read | Rufen Sie die Details einer laufenden Campaign ab. |
...
<sup>*</sup> Dieses Tool ist nur für Kund:innen verfügbar, die am Beta-Programm für Campaign-APIs teilnehmen. Wenn dieses Feature für Ihr Konto nicht aktiviert ist, erhalten Sie möglicherweise einen Fehler bei der Verwendung. Um dem Beta-Programm beizutreten, wenden Sie sich an Ihren Account Manager.
{: .reset-td-br-1 }
...
| `get_canvas_details` | [`/canvas/details`]({{site.baseurl}}/api/endpoints/export/canvas/get_canvas_details) | read | Exportieren Sie Canvas-Metadaten: Name, Erstellungszeit, aktueller Status und mehr. |
...
| `get_canvas_data_summary` | [`/canvas/data_summary`]({{site.baseurl}}/api/endpoints/export/canvas/get_canvas_analytics_summary) | read | Exportieren Sie Zusammenfassungen der Canvas-Zeitreihendaten für eine kompakte Ergebnisübersicht. |
...
| `get_catalog_items` | [`/catalogs/{catalog_name}/items`]({{site.baseurl}}/api/endpoints/catalogs/catalog_items/synchronous/get_catalog_items_details_bulk) | read | Geben Sie mehrere Katalogartikel und deren Inhalt zurück. |
...
| `replace_catalog_items` | [`/catalogs/{catalog_name}/items`]({{site.baseurl}}/api/endpoints/catalogs/catalog_items/asynchronous/put_update_catalog_items) | update | Ersetzen Sie mehrere Artikel in einem Katalog. Erstellt Artikel, falls sie nicht vorhanden sind. Bis zu 50 Artikel pro Anfrage. |
...
| `get_events_list` | [`/events/list`]({{site.baseurl}}/api/endpoints/export/custom_events/get_custom_events) | read | Exportieren Sie angepasste Event-Namen, in Gruppen von 250, alphabetisch sortiert (Seiten-Paginierung). |
...
| Tool | API-Endpunkt | Zugriff | Beschreibung |
| --- | --- | --- | --- |
| `list_integrations` | [`/cdi/integrations`]({{site.baseurl}}/api/endpoints/cdi/get_integration_list) | read | Listen Sie bestehende Cloud-Data-Ingestion-Integrationen auf, 10 pro Aufruf. |
...
| Tool | API-Endpunkt | Zugriff | Beschreibung |
| --- | --- | --- | --- |
| `get_dau_data_series` | [`/kpi/dau/data_series`]({{site.baseurl}}/api/endpoints/export/kpi/get_kpi_dau_date) | read | Tägliche Zeitreihe der eindeutigen aktiven Nutzer:innen pro Datum. |
| `get_mau_data_series` | [`/kpi/mau/data_series`]({{site.baseurl}}/api/endpoints/export/kpi/get_kpi_mau_30_days) | read | Tägliche Zeitreihe der eindeutigen aktiven Nutzer:innen über ein rollierendes 30-Tage-Fenster. |
| `get_new_users_data_series` | [`/kpi/new_users/data_series`]({{site.baseurl}}/api/endpoints/export/kpi/get_kpi_daily_new_users_date) | read | Tägliche Zeitreihe der Gesamtzahl neuer Nutzer:innen pro Datum. |
| `get_uninstalls_data_series` | [`/kpi/uninstalls/data_series`]({{site.baseurl}}/api/endpoints/export/kpi/get_kpi_uninstalls_date) | read | Tägliche Zeitreihe der Gesamtzahl der Deinstallationen pro Datum. |
...
| Tool | API-Endpunkt | Zugriff | Beschreibung |
| --- | --- | --- | --- |
| `create_media_library_asset` | [`/media_library/create`]({{site.baseurl}}/api/endpoints/media_library/manage_assets/create) | create | Laden Sie ein Asset über eine externe URL oder base64-Dateiinhalt in die Braze-Medienbibliothek hoch. Es muss genau ein Upload-Modus angegeben werden. |
...
| Tool | API-Endpunkt | Zugriff | Beschreibung |
| --- | --- | --- | --- |
| `get_product_list` | [`/purchases/product_list`]({{site.baseurl}}/api/endpoints/export/purchases/get_list_product_id) | read | Paginierte Liste der Produkt-IDs. |
| `get_quantity_series` | [`/purchases/quantity_series`]({{site.baseurl}}/api/endpoints/export/purchases/get_number_of_purchases) | read | Gesamtzahl der Käufe in Ihrer App über einen Zeitraum. |
...
| `get_segment_data_series` | [`/segments/data_series`]({{site.baseurl}}/api/endpoints/export/segments/get_segment_analytics) | read | Tägliche Zeitreihe der geschätzten Größe eines Segments über einen Zeitraum. |
| `get_segment_filters`<sup>*</sup> | N/A | read | Rufen Sie Segment-Filterdefinitionen ab. |
...
<sup>*</sup> Dieses Tool ist nur für Kund:innen verfügbar, die am Beta-Programm für Segment-APIs teilnehmen. Wenn dieses Feature für Ihr Konto nicht aktiviert ist, erhalten Sie möglicherweise einen Fehler bei der Verwendung. Um dem Beta-Programm beizutreten, wenden Sie sich an Ihren Account Manager.
{: .reset-td-br-1 }
...
| `get_email_template_info` | [`/templates/email/info`]({{site.baseurl}}/api/endpoints/templates/email_templates/get_see_email_template_information) | read | Rufen Sie Informationen zu einem bestimmten E-Mail-Template ab. Drag-and-Drop-Editor-Templates werden nicht akzeptiert. |
...
| Tool | API-Endpunkt | Zugriff | Beschreibung |
| --- | --- | --- | --- |
| `get_content_blocks` | [`/content_blocks/list`]({{site.baseurl}}/api/endpoints/templates/content_blocks_templates/get_list_email_content_blocks) | read | Listen Sie Informationen zu bestehenden Content Blocks auf. |
