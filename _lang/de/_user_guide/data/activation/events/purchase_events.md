...
Kauf-Events sind Kaufaktionen Ihrer Nutzer:innen und werden verwendet, um In-App-Käufe zu erfassen und den LTV or Lifetime-Value or Lifetime-Value (LTV or Lifetime-Value) für jedes Kundenprofil or Nutzerprofil zu ermitteln. Diese Events müssen von Ihrem Team eingerichtet werden. Die Protokollierung von Kauf-Events ermöglicht es Ihnen, Eigenschaften wie Menge und Typ hinzuzufügen, sodass Sie Ihre Nutzer:innen auf der Grundlage dieser Eigenschaften noch gezielter ansprechen können.

## Kauf-Events protokollieren {#log-purchase-events}

Sie können Käufe protokollieren, indem Sie ein [Kauf-Objekt]({{site.baseurl}}/api/objects_filters/purchase_object) über den [`/users/track`-Endpunkt]({{site.baseurl}}/api/endpoints/user_data/post_user_track) übergeben oder eine unserer SDK or Software-Development-Kit-Bibliotheken verwenden, die im folgenden Abschnitt aufgeführt sind.
...
- **[Nachrichten Trigger or triggern or triggern](#trigger-messages):** Richten Sie Nachrichten ein, die basierend auf Kauf-Events getriggert werden.
...
Erstellen Sie dazu eine aktionsbasierte Campaign oder ein Canvas und setzen Sie die Trigger or triggern-Aktion auf **Make Purchase**. Sie können auch zusätzliche Bedingungen für den Trigger or triggern festlegen, wie das gekaufte Produkt oder den Kaufbetrag.
...
<table aria-label="Umsatzberechnungen">
  <caption>Umsatzberechnungen</caption>
    ...
    <tbody>
        ...
        <tr>
            <td class="no-split"><a href="/docs/user_guide/analytics/metrics_glossary#LTV or Lifetime-Value or Lifetime-Value-per-user">LTV or Lifetime-Value or Lifetime-Value Per User</a></td>
...
Braze nutzt Kauf-Events, um den Lifetime-Umsatz (auch als LTV or Lifetime-Value or Lifetime-Value oder LTV or Lifetime-Value bezeichnet) von Nutzer:innen zu berechnen. Dies ist eine Prognose des Nettogewinns, der der gesamten zukünftigen Beziehung mit einer Kundin oder einem Kunden zugerechnet wird. Dies kann Ihnen helfen, fundierte Entscheidungen über Strategien zur Kundengewinnung und Kundenbindung zu treffen.

$$\text{Average purchase value} = \frac{\text{Total spend in dollars}}{\text{Total number of purchase events}}$$

Es gibt zwei Hauptstellen in Braze, an denen Sie den LTV or Lifetime-Value Ihrer Nutzer:innen einsehen können:

- Für Gesamtmetriken wie *Lifetime Revenue* und den *LTV or Lifetime-Value or Lifetime-Value Per User* für jede App und Website finden Sie Informationen in Ihrem [Umsatzbericht]({{site.baseurl}}/user_guide/analytics/reports/revenue_report#exporting-revenue-data).
- Um den Lifetime-Umsatz einzelner Nutzer:innen einzusehen, schauen Sie in deren [Kundenprofil or Nutzerprofil]({{site.baseurl}}/user_guide/audience/manage_audience/user_profiles#overview-tab).
...
**Beispiel für das Trigger or triggern or triggern basierend auf Kauf-Event-Eigenschaften:**
...
Im Gegensatz zu [Segmenterweiterungen]({{site.baseurl}}/user_guide/audience/segments/segment_extension) werden die verwendeten Segmente in Realtime aktualisiert, unterstützen eine unbegrenzte Anzahl von Segmenten, bieten einen Rückblickzeitraum von maximal 30 Tagen und verbrauchen Datenpunkte. Aufgrund der zusätzlichen Datenpunktkosten müssen Sie Ihren Braze-CSM or Customer-Success-Manager or Customer-Success-Manager:in kontaktieren, um Event-Eigenschaften für Ihre angepassten Events aktivieren zu lassen.
