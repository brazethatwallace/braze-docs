...
## Einrichten von Data Warehouse-Integrationen {#setting-up-data-warehouse-integrations}
...
- **Nutzer:innen-Bezeichner-Spalten** – Ihre Tabelle kann eine oder mehrere Nutzer:innen-Bezeichner-Spalten enthalten. Jede Zeile sollte nur einen Bezeichner enthalten (entweder `external_id`, die Kombination aus `alias_name` und `alias_label`, `braze_id`, `email` oder `phone`). Eine Quelltabelle kann Spalten für einen, zwei, drei, vier oder alle fünf Bezeichner-Typen enthalten.
    ...
    - `BRAZE_ID` – Die Braze-Nutzer:innen-Kennung. Diese wird vom Braze SDK generiert, und neue Nutzer:innen können nicht mit einer Braze-ID über Cloud-Datenaufnahme erstellt werden. Um neue Nutzer:innen anzulegen, geben Sie eine externe Nutzer-ID oder einen Nutzer-Alias an.
...
#### Schritt 1.3: Warehouse einrichten und der Braze-Rolle Zugriff gewähren {#step-13-set-up-the-warehouse-and-give-access-to-braze-role}
...
#### Schritt 1.2: Nutzer:in anlegen und Berechtigungen erteilen {#step-12-create-user-and-grant-permissions}
...
#### Schritt 1.2: Zugriffs-Token erstellen {#step-12-create-an-access-token}
...
2. Wählen Sie auf dem Tab „Zugriffs-Token“ die Option **Generate new token**.
3. Geben Sie einen Kommentar ein, der Ihnen hilft, dieses Token zu identifizieren, z. B. „Braze CDI“, und ändern Sie die Lebensdauer des Tokens auf „keine Lebensdauer“, indem Sie das Feld „Lifetime (days)“ leer lassen.
...
5. Geben Sie eine Beschreibung für das Geheimnis ein und legen Sie einen Ablaufzeitraum fest. Wählen Sie dann **Add**.
...
#### Schritt 1.2: Zugang zu Fabric-Ressourcen gewähren {#step-12-grant-access-to-fabric-resources}
...
2. Geben Sie einen **Name** für den Workspace ein und wählen Sie **Apply**.

Nachdem Sie einen gemeinsamen Workspace haben, gewähren Sie dem Dienstprinzipal Zugriff:

1. Wählen Sie den Workspace aus und dann **Manage Access**.
2. Wählen Sie **+ Add people or groups**.
3. Suchen Sie nach dem Namen des Dienstprinzipals, den Sie in Schritt 1.1 erstellt haben, und wählen Sie ihn aus. Wenn er nicht angezeigt wird, überprüfen Sie, ob Sie die Einstellung **Service principals can use Fabric APIs** in Schritt 1.2 aktiviert haben.
4. Wählen Sie im Rollen-Dropdown **Contributor** aus.
...
### 2. Schritt: Neue Quelle im Braze-Dashboard erstellen {#step-2-create-a-new-source-in-the-braze-dashboard}
...
#### Schritt 2.1: Cloud-Datenaufnahme-Synchronisierung einrichten {#step-21-set-up-a-cloud-data-ingestion-sync}
...
### 3. Schritt: Neue Synchronisierung im Braze-Dashboard erstellen {#step-3-create-a-new-sync-in-the-braze-dashboard}
...
## Ausführen der Synchronisierung {#running-the-sync}
