...
| Identity and Access Management (IAM) | IAM ist ein Webdienst, mit dem Sie den Zugriff auf AWS-Ressourcen sicher steuern können. In diesem Tutorial erstellen Sie eine IAM-Richtlinie und weisen sie einer IAM-Rolle zu, um Ihren S3-Bucket mit der Cloud-Datenaufnahme von Braze zu integrieren. |
...
{% alert warning %}
Konfigurieren Sie keine Dead-Letter-Warteschlange für dieses Abo. Braze unterstützt keine Dead-Letter-Warteschlangen für Cloud-Datenaufnahme-Abos. Weitere Informationen finden Sie unter [Dead-letter topics](https://cloud.google.com/pubsub/docs/dead-letter-topics) in der Google Cloud-Dokumentation.
{% endalert %}
...
{: start="6"}
6. Wählen Sie **Preview and validate**, um zu bestätigen, dass Braze das Abo erreichen und die verfügbaren Dateien auflisten kann. Ein erfolgreicher Test listet vorhandene Dateien im Bucket auf, diese Dateien werden jedoch nicht automatisch synchronisiert.
