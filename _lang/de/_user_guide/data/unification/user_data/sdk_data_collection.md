...
Im Folgenden sind die strikt notwendigen Daten aufgeführt, die von Braze generiert und empfangen werden, wenn Sie das SDK initialisieren. Diese Daten sind nicht konfigurierbar und für die Kernfunktionen der Plattform unerlässlich. Mit Ausnahme von Sitzungsstart und Sitzungsende zählen alle anderen automatisch erfassten Daten nicht zur Datenpunkt-Nutzung.

| Attribut | Beschreibung | Warum es erfasst wird |
| --------- | ----------- | ------------------ |
| App-Versionsname /<br> App-Versionscode | Die aktuellste App-Version | Dieses Attribut wird verwendet, um Nachrichten zur App-Versionskompatibilität an die richtigen Geräte zu senden. Es kann genutzt werden, um Nutzer:innen über Dienstunterbrechungen oder Fehler zu informieren. |
| Land | Land, das durch IP-Adress-Geolocation identifiziert wird. Wenn die IP-Adress-Geolocation nicht verfügbar ist, wird es durch die [Gerätesprache](#optional-data-collected-by-default) identifiziert. Der Wert kann alternativ das sein, was die SDKs direkt mit `setCountry` festlegen, aber beachten Sie, dass die Übergabe eines Attributwerts über SDK oder API Datenpunkte protokolliert. **Nachdem das Land manuell festgelegt wurde (über die SDK-Methode, REST API oder CSV-Upload), aktualisiert das SDK diesen Wert nicht mehr automatisch.** | Dieses Attribut wird verwendet, um Nachrichten standortbasiert auszurichten. |
...
| Betriebssystem und Betriebssystemversion | Aktuell gemeldetes Gerät oder Browser und Geräte- oder Browserversion | Dieses Attribut wird verwendet, um Nachrichten nur an kompatible Geräte zu senden. Es kann auch innerhalb der Segmentierung verwendet werden, um Nutzer:innen zum Upgrade der App-Version aufzufordern. |
| Sitzungsstart und Sitzungsende | Wenn die Nutzer:innen beginnen, Ihre integrierte App oder Website zu verwenden | Das Braze SDK meldet Sitzungsdaten, die vom Braze-Dashboard zur Berechnung des Nutzer-Engagements und anderer Analytics verwendet werden, die für das Verständnis Ihrer Nutzer:innen wesentlich sind. Wann genau der Sitzungsstart und das Sitzungsende von Ihrer App oder Website aufgerufen wird, ist von Entwickler:innen konfigurierbar ([Android]({{site.baseurl}}/developer_guide/analytics/tracking_sessions?tab=android), [iOS]({{site.baseurl}}/developer_guide/analytics/tracking_sessions?tab=swift), [Web]({{site.baseurl}}/developer_guide/analytics/tracking_sessions?tab=web)). |
...
| Sitzungs-ID und Sitzungszeitstempel | Sitzungsbezeichner, ein zufällig generierter String und Sitzungszeitstempel | Wird verwendet, um festzustellen, ob Nutzer:innen eine neue oder bestehende Sitzung starten, und um die erneute Berechtigung für Nachrichten zu bestimmen, die für diese Nutzer:innen bestimmt sind.<br><br>Bestimmte Messaging-Kanäle wie In-App Messages und Content Cards werden beim Sitzungsstart mit dem Gerät synchronisiert. Unser Backend verwendet dann Daten darüber, wann es zuletzt die Braze-Server kontaktiert hat (die das Gerät speichert und zurücksendet), um festzustellen, ob die Nutzer:innen für neue Nachrichten berechtigt sind.|
...
Berechnete Metriken umfassen kanalbasierte Metriken (aufgeführt im [Glossar der Berichtsmetriken]({{site.baseurl}}/user_guide/analytics/metrics_glossary)) und die folgenden Attribute.
...
| Nachricht von Campaign erhalten                | Boolean. Dieser Filter richtet sich an Nutzer:innen basierend darauf, ob sie eine vorherige Campaign erhalten haben. |
...
Mindestintegration bedeutet, dass Sie nur die unter [Mindestintegration](#minimum-integration) aufgeführten erforderlichen Daten erfassen und die [standardmäßig erfassten optionalen Daten](#optional-data-collected-by-default) durch [Blockierung der optionalen SDK-Datenerfassung]({{site.baseurl}}/developer_guide/platform_integration_guides/sdk_primer#blocking-data-collection) deaktivieren.
...
Zusätzlich zu den Mindestintegrationsdaten werden die folgenden Attribute automatisch von Braze erfasst, wenn Sie die SDK-Integration initialisieren. Sie können die Erfassung dieser Attribute [deaktivieren]({{site.baseurl}}/developer_guide/platform_integration_guides/sdk_primer#blocking-data-collection), um eine Mindestintegration zu ermöglichen.
...
| Gerätemodell            | Android, iOS      | Die spezifische Hardware des Geräts                                                | Dieses Attribut wird verwendet, um Nachrichten nur an kompatible Geräte zu senden. Es kann auch für die Segmentierung verwendet werden.                                                 |
| Gerätemarke            | Android           | Die Marke des Geräts (zum Beispiel Samsung)                                         | Dieses Attribut wird verwendet, um Nachrichten nur an kompatible Geräte zu senden.                                                                                          |
| Mobilfunkanbieter des Geräts | Android, iOS      | Der Mobilfunkanbieter                                                                 | Dieses Attribut wird optional für das Nachrichten-Targeting verwendet.<br><br>**Hinweis:** Dieses Feld ist seit iOS 16 veraltet und wird in einer zukünftigen iOS-Version standardmäßig `--` zurückgeben. |
...
| Benachrichtigungseinstellungen   | Android, iOS, Web | Ob diese App Push-Benachrichtigungen aktiviert hat.                                   | Dieses Attribut wird verwendet, um Push-Benachrichtigungen zu aktivieren.                                                                                                                    |
...
| Zeitzone               | Android, iOS, Web | Geräte- oder Browserzeitzone                                                           | Dieses Attribut wird verwendet, um Nachrichten zur passenden Zeit entsprechend der Ortszeit der jeweiligen Nutzer:innen zu senden.                                                   |
| User-Agent              | Web               | [User-Agent](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/User-Agent) | Dieses Attribut wird verwendet, um Nachrichten nur an kompatible Geräte zu senden. Es kann auch für die Segmentierung verwendet werden.                                                 |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3  .reset-td-br-4 aria-label="Optionale Daten, die standardmäßig erfasst werden" }

Weitere Informationen zum Tracking von Eigenschaften auf Geräteebene (wie Mobilfunkanbieter, Zeitzone, Auflösung und andere) finden Sie in der plattformspezifischen Dokumentation: [Android]({{site.baseurl}}/developer_guide/storage?tab=android), [iOS]({{site.baseurl}}/developer_guide/storage?tab=swift), [Web]({{site.baseurl}}/developer_guide/storage#cookies).
...
|----------------------------|--------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Device Ad Tracking Enabled | Android, iOS | Auf iOS:<br>[`set(adTrackingEnabled:)`](https://braze-inc.github.io/braze-swift-sdk/documentation/brazekit/braze/set(adtrackingenabled:))<br><br>Auf Android:<br>[`Braze.setGoogleAdvertisingId()`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze/-i-braze/set-google-advertising-id.html) | Diese Eigenschaft erfordert zusätzliche Berechtigungen auf App-Ebene, die von der integrierenden Person gewährt werden müssen.                                                                                                                                                                                      |
| Device IDFA                | iOS          | Geräte-Identifier for Advertisers (IDFA)                                                                                                                                                                                                                                                                                         | Dies erfordert das Ad Tracking Transparency Framework, das eine zusätzliche Datenschutzprüfung durch den App Store auslöst. Weitere Informationen finden Sie unter [`set(identifierForAdvertiser:)`](https://braze-inc.github.io/braze-swift-sdk/documentation/brazekit/braze/set(identifierforadvertiser:)) |
...
| Most recent location | Android, iOS | Dies ist der zuletzt bekannte GPS-Standort des Geräts der Nutzer:innen. Dieser wird bei Sitzungsbeginn aktualisiert und im Nutzerprofil gespeichert. | Dies erfordert, dass die Nutzer:innen Ihrer App die Standortberechtigung erteilen. |
...
Um Braze optimal zu nutzen, implementieren unsere SDK-Integratoren häufig die Braze SDKs und protokollieren [angepasste Attribute]({{site.baseurl}}/user_guide/data/activation/attributes/custom_attributes#set-custom-attributes), [angepasste Events]({{site.baseurl}}/user_guide/data/activation/events/custom_events#logging-custom-events) und [Kauf-Events]({{site.baseurl}}/user_guide/data/activation/events/purchase_events#log-purchase-events), die für ihr Geschäft relevant sind – zusätzlich zu den automatisch erfassten Daten.
...
{% alert important %}
Braze blockiert Nutzerprofile („Dummy-Nutzer:innen“) mit mehr als 5.000.000 Sitzungen, mehr als 20.000 verschiedenen angepassten Event-Namen oder mehr als 20.000 verschiedenen Produktnamen bei Käufen und stoppt die Aufnahme aller eingehenden Daten für dieses Profil – sowohl über die SDKs als auch über die REST API. Weitere Informationen finden Sie unter [Spam-Blockierung]({{site.baseurl}}/user_archival).
{% endalert %}