...
{% alert note %}
Der Trigger **Interact with Canvas Step** ist nur für Campaigns verfügbar, nicht für aktionsbasierte Canvas-Eintritte. Wenn Sie ein Canvas auf Basis einer Nutzer:in triggern müssen, die einen bestimmten Schritt in einem anderen Canvas erreicht, verwenden Sie diesen Braze-zu-Braze-Webhook-Ansatz oder die Canvas-Komponente [Send to Destination]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/send_to_destination).
{% endalert %}
