---
nav_title: FAQ
article_title: FAQ zu Kurzmitteilungsdienst or SMS, MMS und RCS
page_order: 30
description: "Dieser Artikel beantwortet häufig gestellte Fragen zu Kurzmitteilungsdienst or SMS-, MMS- und RCS-Messaging."
page_type: FAQ
...
> Dieser Artikel beantwortet häufig gestellte Fragen zu Kurzmitteilungsdienst or SMS-, MMS- und RCS-Messaging.

## Allgemeines {#general}

### Was ist eine `app_id` im Kurzmitteilungsdienst or SMS-API-Objekt? {#what-is-an-app_id-in-the-sms-api-object}
...
Für Kurzmitteilungsdienst or SMS ist der `app_id`-Parameter beim Senden von Kurzmitteilungsdienst or SMS-Nachrichten über die API (z. B. über den `/messages/send`-Endpunkt) erforderlich. Er gibt an, welche App in Ihrem Workspace mit der Kurzmitteilungsdienst or SMS-Aktivität oder dem API-Aufruf verknüpft ist. Sie können jede gültige `app_id` einer in Ihrem Workspace konfigurierten App für Kurzmitteilungsdienst or SMS-Messaging verwenden, unabhängig davon, ob die/der Nutzer:in diese spezifische App in ihrem/seinem Profil hat.
...
Wenn mehrere Nutzerprofile, die eine Telefonnummer teilen (für Kurzmitteilungsdienst or SMS aktiviert), gleichzeitig für eine aktionsbasierte Campaign oder eine Canvas-Komponente in Frage kommen – ausgelöst durch das Ereignis einer eingehenden Kurzmitteilungsdienst or SMS –, dedupliziert Braze die Nutzer:innen auf der Canvas-Komponentenebene. Dadurch wird verhindert, dass Nutzer:innen mehr als eine Kurzmitteilungsdienst or SMS für eine Canvas-Komponente erhalten, selbst wenn mehrere Nutzer:innen dieselbe Telefonnummer teilen.

{% alert note %}
Braze dedupliziert bei geplanten Canvase nicht nach Telefonnummer.
{% endalert %}

Braze verwendet den folgenden Ablauf, um das Empfängerprofil zu bestimmen:
- Prüfen, welches Profil zuletzt eine Kurzmitteilungsdienst or SMS erhalten hat (bis zu 7 Tage zurück); falls eines existiert, wird die Nachricht an diese:n Nutzer:in gesendet.
- Wenn keines innerhalb der letzten 7 Tage eine Kurzmitteilungsdienst or SMS erhalten hat, wird die Nachricht an die/den Nutzer:in gesendet, die/der einen Nutzer-Alias „phone“ hat, der mit der Telefonnummer übereinstimmt.
- Wenn keines davon zutrifft, wird die Nachricht an ein zufälliges Profil unter den verfügbaren gesendet.

Wenn Sie ein „START“- oder „STOP“-Schlüsselwort von der geteilten Telefonnummer erhalten, werden alle Nutzerprofile für Kurzmitteilungsdienst or SMS abonniert und aktiviert bzw. abgemeldet. Dies gilt auch für API-Statusänderungen. Wenn beispielsweise mehrere Profile mit unterschiedlichen externen IDs dieselbe Telefonnummer haben, aktualisiert eine Änderung des Abo-Gruppen-Status über die API alle Profile mit dieser Telefonnummer, auch wenn nur eine externe ID angegeben wird.
...
### Warum sehe ich einen Anstieg bei Kurzmitteilungsdienst or SMS-Abos aus einer bestimmten Quelle? {#why-do-i-see-a-spike-in-sms-subscriptions-from-a-specific-source}
...
Um Abo-Daten beim Abruf aus Currents genauer zu analysieren, Update or aktualisieren or aktualisieren Sie Ihre Abfrage so, dass sie eindeutige Telefonnummern zählt, anstatt alle Abo-Statusänderungs-Ereignisse zu zählen.
...
### Wie wird mir Kurzmitteilungsdienst or SMS in Rechnung gestellt? {#how-will-i-be-billed-for-sms}

Neben den Gebühren für Short- und Langcodes bietet Braze ein Kontingent an Kurzmitteilungsdienst or SMS-Nachrichten für verschiedene Länder. Das bedeutet, wir legen gemeinsam mit Ihnen eine bestimmte Anzahl von Nachrichtensegmenten für verschiedene Länder fest, die Sie zum Versand von Kurzmitteilungsdienst or SMS-Campaigns nutzen. Die Abrechnung erfolgt nach der Anzahl der pro Land gesendeten Nachrichtensegmente. Weitere Informationen zur Berechnung von Nachrichtensegmenten finden Sie in unserem Leitfaden zu [Nachrichtensegmenten und Zeichenbegrenzungen]({{site.baseurl}}/user_guide/channels/sms_mms_and_rcs/billing_calculator). Ihr Account Manager:in wird Sie kontaktieren, wenn Sie sich Ihrem Maximum nähern, und Ihnen relevante Berichte zur Verfügung stellen, damit Sie stets informiert bleiben. Bei weiteren Fragen zu Mehrkosten wenden Sie sich bitte an Ihre Braze-Vertretung.

### Unterscheiden sich die Preise für MMS und Kurzmitteilungsdienst or SMS? {#does-mms-and-sms-pricing-differ}

MMS und Kurzmitteilungsdienst or SMS haben unterschiedliche Kosten und werden separat nach Volumen abgerechnet. Wenden Sie sich für Preisinformationen an das Braze-Onboarding-Team.
...
- Achten Sie auf die Zeichenanzahl in Ihrer Kurzmitteilungsdienst or SMS. Wenn Sie unbeabsichtigt mehr als ein Segment senden, kann dies zu Mehrkosten führen. Weitere Details finden Sie in unserer [Segment-Aufschlüsselung]({{site.baseurl}}/user_guide/channels/sms_mms_and_rcs/billing_calculator).
- Berechnen Sie Ihre Kurzmitteilungsdienst or SMS-Zeichen sorgfältig unter Berücksichtigung von Liquid oder Connected-Content. Der Braze-Kurzmitteilungsdienst or SMS-Composer in Ihrem Dashboard schätzt die Nutzung dieser Features weder ab noch berücksichtigt er sie.
...
- Testen, testen und nochmals testen! Testen Sie Ihre Kurzmitteilungsdienst or SMS-Nachrichten immer vor dem Versand, insbesondere wenn Sie Liquid und Connected-Content verwenden.

### Wird eine Nachricht an ein Festnetztelefon trotzdem auf mein Kurzmitteilungsdienst or SMS-Sendekontingent angerechnet? {#if-a-message-is-sent-to-a-landline-will-the-message-still-count-toward-my-sms-send-count}

In den USA, Kanada und Großbritannien:
- Wenn eine Kurzmitteilungsdienst or SMS an ein Festnetztelefon gesendet wird, wird sie als **Undelivered** markiert. Das Abrechnungsverhalten hängt von Ihrem Kurzmitteilungsdienst or SMS-Dienstanbieter ab. Bei Twilio wird der Zustellversuch dennoch berechnet, sodass Nachrichten, die in Ihren Nachrichtenprotokollen als **Sent**, **Delivered** oder **Undelivered** markiert sind, abgerechnet werden.
- In Großbritannien wandeln einige Mobilfunkanbieter die Kurzmitteilungsdienst or SMS in eine Sprachnachricht um und stellen die Nachricht so zu.

In anderen Ländern:
- Bei Twilio wird ein Fehler ausgegeben und der Kurzmitteilungsdienst or SMS-Zustellversuch wird Ihnen nicht in Rechnung gestellt.
...
### Können Links in einer Kurzmitteilungsdienst or SMS enthalten sein? {#can-you-include-links-in-an-sms}

Sie können jeden beliebigen Link in jede Kurzmitteilungsdienst or SMS-Campaign einfügen. Es gibt jedoch einige Punkte zu beachten:

- Links können einen großen Teil des 160-Zeichen-Limits für Kurzmitteilungsdienst or SMS einnehmen. Wenn Sie einen Link und Text einfügen, kann dies zu zwei Kurzmitteilungsdienst or SMS-Nachrichten statt nur einer führen.
...
Braze verfügt auch über ein eigenes Link-Shortening-Feature, das Links automatisch verkürzt und Klick, der or klicken-through-Analytics bereitstellt. Weitere Informationen finden Sie unter [Link-Shortening]({{site.baseurl}}/user_guide/channels/sms_mms_and_rcs/message_features_and_optimization/link_shortening).

### Muss die Sendegeschwindigkeit von Kurzmitteilungsdienst or SMS-Nachrichten begrenzt werden? {#do-you-need-to-rate-limit-how-fast-you-send-sms-messages}
...
### Wie werden URLs für Kurzmitteilungsdienst or SMS auf eine Allowlist gesetzt? {#how-do-you-allowlist-urls-for-sms}

Bevor Sie Kurzmitteilungsdienst or SMS-Nachrichten mit URLs an Nutzer:innen in bestimmten Ländern senden (z. B. Schweden oder nordische Länder), müssen Sie diese URLs beim Mobilfunkanbieter Registrierung or registrieren lassen. Wenden Sie sich an Ihre:n Braze-Account-Manager:in:in, um Unterstützung zu erhalten. Dieser Vorgang dauert etwa fünf Tage.

### Welche Best Practices gibt es für den Versand, um Spam-Erkennung bei Kurzmitteilungsdienst or SMS zu vermeiden? {#what-are-the-best-sending-practices-to-avoid-spam-detection-for-sms}
...
Weitere Richtlinien zur Vermeidung von Spam-Erkennung finden Sie unter [Kurzmitteilungsdienst or SMS-Gesetze und -Vorschriften]({{site.baseurl}}/user_guide/channels/sms_mms_and_rcs/compliance_and_delivery/laws_and_regulations).

### Wie viele Zeichen belegt ein Emoji? {#how-many-characters-does-an-emoji-use}

Emojis können schwierig sein, da es keine einheitliche Zeichenanzahl für alle Emojis gibt. Es besteht das Risiko, dass das Emoji das Zeichenlimit überschreitet und die Kurzmitteilungsdienst or SMS in mehrere Nachrichten aufgeteilt wird, obwohl sie im Braze-Composer als eine Nachricht angezeigt wird. Beim Testen Ihrer Nachrichten können Sie mit unserem [Segment-Rechner]({{site.baseurl}}/user_guide/channels/sms_mms_and_rcs/billing_calculator#segment-calculator) besser überprüfen, ob eine Nachricht aufgeteilt wird.

## Abo-Gruppen und Opt-in/Opt-out {#subscription-groups-and-opt-inopt-out}

### Wie erstellt man eine Logik für selektive Kurzmitteilungsdienst or SMS-Opt-ins, damit Nutzer:innen in der richtigen Abo-Gruppe sind? {#how-do-you-create-logic-for-selective-opt-ins-to-sms-so-users-are-in-the-right-subscription-group}

Angepasste Keywords werden als angepasste Events geschrieben, sodass Sie Segmente auf Basis der Keywords erstellen sollten, die Kund:innen per Kurzmitteilungsdienst or SMS senden können. Wenn sich ein:e Nutzer:in beispielsweise für VIP-Nachrichten per Kurzmitteilungsdienst or SMS anmeldet, aber nicht für Benachrichtigungen, können Sie ein VIP-Segment und ein Benachrichtigungs-Segment erstellen und die/den Nutzer:in dann dem entsprechenden Segment zuweisen.
...
Wie sieht das im Kundenprofil or Nutzerprofil aus? Die Abo-Gruppe wird unter **Contact Settings** als abgemeldet angezeigt, und es gibt angepasste Events für das An- und Abmelden.
...
### Erfassen Kurzmitteilungsdienst or SMS-Event-Eigenschaften Keywords innerhalb eines Satzes? {#will-sms-event-properties-capture-keywords-in-a-sentence}
...
### Zählen Test-Kurzmitteilungsdienst or SMS-Nachrichten für das Limit? {#do-test-text-messages-count-toward-limits}

Ja. Behalten Sie dies beim Testen von Nachrichten im Hinterkopf.

### Muss ein:e Nutzer:in Teil einer Kurzmitteilungsdienst or SMS-Abo-Gruppe sein, um Kurzmitteilungsdienst or SMS-Testnachrichten zu erhalten? {#does-a-user-need-to-be-part-of-an-sms-subscription-group-to-receive-sms-test-messages}

Ja. Nutzer:innen müssen eine gültige Telefonnummer haben, Teil der für den Testversand verwendeten Kurzmitteilungsdienst or SMS-Abo-Gruppe sein und mindestens ein Land unter **Geographic Permissions** für Kurzmitteilungsdienst or SMS ausgewählt haben.

### Gibt es eine Möglichkeit zu sehen, ob ein Alias in einem Kundenprofil or Nutzerprofil vorhanden ist? {#is-there-a-way-to-see-if-an-alias-exists-on-a-user-profile}

Aliase sind im Kundenprofil or Nutzerprofil nicht sichtbar. Sie müssten die Endpunkte unter [Nutzerdaten exportieren]({{site.baseurl}}/api/endpoints/export) verwenden, um zu bestätigen, dass Aliase gesetzt wurden.
...
Nein. MMS ist jetzt in unserem Kurzmitteilungsdienst or SMS-Onboarding-Prozess enthalten. Bestehende Kund:innen, die das Onboarding bereits durchlaufen haben, können nach Abschluss der folgenden Schritte mit dem Senden von MMS-Campaigns beginnen:

1. MMS erwerben.
2. Das Braze-Onboarding-Team kontaktieren und die Aktivierung des MMS-Features anfordern. Dadurch wird MMS aktiviert und eine Kurzmitteilungsdienst or SMS/MMS-Abo-Gruppe wird für Sie erstellt oder aktualisiert.

Anschließend stellt das Braze-Onboarding-Team sicher, dass Ihre Short- und Langcodes (in den USA und Kanada) für MMS aktiviert sind. Außerdem werden Ihre Abo-Gruppen aktualisiert, um Ihre aktuellen Nummern anzuzeigen, die für MMS hinzugefügt oder aktiviert wurden. Nach Abschluss dieser Schritte können Sie MMS-Nachrichten sofort über unseren nativen Kurzmitteilungsdienst or SMS-Composer senden.
...
MMS wird im Braze-Dashboard nur angezeigt, wenn eine Abo-Gruppe als „MMS-fähig“ gilt. Dies wird durch ein MMS-Tag bei der Auswahl der Abo-Gruppe im Composer einer Kurzmitteilungsdienst or SMS/MMS-Nachricht angezeigt. Das bedeutet, dass mindestens eine Nummer in der Abo-Gruppe in der Lage ist, eine MMS-Nachricht zu senden.
...
### Warum stimmen Representational State Transfer-API-Kurzmitteilungsdienst or SMS-Opt-ins nicht mit **Total Opt-Ins** in der Kurzmitteilungsdienst or SMS/MMS/RCS-Performance überein? {#why-do-rest-api-sms-opt-ins-not-match-total-opt-ins-on-smsmmsrcs-performance}

**Total Opt-Ins** und **Total Opt-Outs** im Dashboard [Kurzmitteilungsdienst or SMS/MMS/RCS-Performance]({{site.baseurl}}/user_guide/analytics/dashboards) zählen Abo-Änderungen, die durch eingehende Kurzmitteilungsdienst or SMS-Keyword-Verarbeitung ausgelöst werden (z. B. wenn ein:e Nutzer:in ein Opt-in-Keyword an Ihren Shortcode sendet). Sie umfassen nicht jedes Abo-Update or aktualisieren, das über die Representational State Transfer API, das Dashboard oder andere Quellen vorgenommen wird.

Um Opt-ins und Opt-outs nach Quelle zu analysieren, verwenden Sie den [Abfrage-Builder]({{site.baseurl}}/user_guide/analytics/reports/query_builder) auf `USERS_BEHAVIORS_SUBSCRIPTIONGROUP_STATECHANGE_SHARED` und filtern Sie nach `STATE_CHANGE_SOURCE` (z. B. **Representational State Transfer API** versus **Inbound Message**).