...
article_title: Eine Kurzmitteilungsdienst or SMS-, MMS- oder RCS-Nachricht erstellen
page_order: 1
description: "Erstellen Sie eine Kurzmitteilungsdienst or SMS-, MMS- oder RCS-Nachricht und konfigurieren Sie kanalspezifische Nachrichtentypen, Felder, Link-Shortening, Zustellungseinstellungen und Verhalten."
page_type: reference
...
# Eine Kurzmitteilungsdienst or SMS-, MMS- oder RCS-Nachricht erstellen {#create-an-sms-mms-or-rcs-message}

> Erstellen Sie personalisierte Kurzmitteilungsdienst or SMS-, MMS- und Rich Communication Services (RCS)-Nachrichten in Campaigns oder Canvas. Die ausgewählte Abo-Gruppe bestimmt, welche Nachrichtentypen und Absender verfügbar sind.
...
| Telefonnummern und Einwilligung der Nutzer:innen | Importieren Sie die Telefonnummern der Nutzer:innen und erfassen Sie die entsprechenden [Kurzmitteilungsdienst or SMS-, MMS- und RCS-Opt-ins]({{site.baseurl}}/user_guide/channels/sms_mms_and_rcs/compliance_and_delivery/collecting_user_opt_ins). |
...
| Nachrichten- oder Aktionsguthaben | Stellen Sie sicher, dass Ihr Konto über verfügbare Guthaben verfügt. Der Versand von Kurzmitteilungsdienst or SMS-, MMS- und RCS-Nachrichten über Braze verwendet diese Guthaben. |
{: .reset-td-br-1 .reset-td-br-2 aria-label="Voraussetzungen für Kurzmitteilungsdienst or SMS-, MMS- und RCS-Nachrichten" }
...
2. Wählen Sie **Kurzmitteilungsdienst or SMS/MMS/RCS** oder für Campaigns, die mehrere Kanäle ansprechen, **Multichannel Campaign**.
...
5. Fügen Sie Varianten für Ihre Campaign hinzu und benennen Sie diese. Sie können Kurzmitteilungsdienst or SMS/MMS- und RCS-Varianten in derselben Campaign einbinden. Weitere Informationen finden Sie unter [Multivariate und A/B-Tests]({{site.baseurl}}/user_guide/messaging/ab_testing).
...
| --- | --- |
| Nur Kurzmitteilungsdienst or SMS | Kurzmitteilungsdienst or SMS |
| Kurzmitteilungsdienst or SMS mit MMS-fähigen Nummern | Kurzmitteilungsdienst or SMS und MMS |
| RCS-fähig mit verifiziertem RCS-Absender | RCS und Kurzmitteilungsdienst or SMS, wenn die Gruppe auch einen Kurzmitteilungsdienst or SMS-Absender enthält. MMS ist ebenfalls verfügbar, wenn dieser Absender MMS-fähig ist. |
{: .reset-td-br-1 .reset-td-br-2 aria-label="Verfügbare Nachrichtentypen nach Abo-Gruppe" }

{% alert tip %}
Fügen Sie einer RCS-Abo-Gruppe mindestens einen Kurzmitteilungsdienst or SMS-Absender hinzu, damit Sie einen Kurzmitteilungsdienst or SMS-Fallback senden können, wenn die RCS-Zustellung fehlschlägt.
{% endalert %}

Wenn die Abo-Gruppe beide Protokolle unterstützt, wählen Sie **Kurzmitteilungsdienst or SMS/MMS** oder **RCS**. Für RCS wählen Sie **Text**, **Media** oder **Card**.
...
{% tabs local %}
{% tab Kurzmitteilungsdienst or SMS und MMS %}

#### Kurzmitteilungsdienst or SMS- und MMS-Felder und -Einstellungen {#sms-and-mms-fields-and-settings}

| Feld oder Einstellung | Beschreibung |
| --- | --- |
| **Sprache** | Sprachspezifische Inhalte in die Nachricht einfügen. |
| **Nachricht** | Geben Sie bis zu 1.600 Zeichen ein, einschließlich Liquid, Connected-Content und Emojis. Der Editor schätzt die Codierung, die Zeichenanzahl und die Anzahl der abrechenbaren Kurzmitteilungsdienst or SMS-Nachrichtensegmente. Eine MMS-Nachricht kann Medien ohne Nachrichtentext enthalten. |
...
{: .reset-td-br-1 .reset-td-br-2 aria-label="Kurzmitteilungsdienst or SMS- und MMS-Felder und -Einstellungen" }

Kurzmitteilungsdienst or SMS-Nachrichten verwenden GSM-7- oder UCS-2-Codierung und werden pro Nachrichtensegment abgerechnet. Ein einzelnes Zeichen kann die Codierung ändern und die Anzahl der abrechenbaren Segmente erhöhen. Informationen zu Codierungsregeln, Segmentgrößen und dem Segment-Rechner finden Sie unter [Kurzmitteilungsdienst or SMS- und RCS-Abrechnungsrechner]({{site.baseurl}}/user_guide/channels/sms_mms_and_rcs/billing_calculator).
...
Die MMS-Verfügbarkeit und -Darstellung hängen vom empfangenden Mobilfunkanbieter ab. Wenn ein Anbieter MMS nicht akzeptieren kann, werden die Medien über den Provider zu einem Link im Kurzmitteilungsdienst or SMS-Text. Vermeiden Sie das Senden von MMS an Google-Voice-Nummern, da die eingeschränkte MMS-Unterstützung zu unzuverlässiger Zustellung führen kann.

Wenn Nutzer:innen eingehende Medien senden, stellt Braze deren URLs in [Currents-Kurzmitteilungsdienst or SMS-Eingangs-Events]({{site.baseurl}}/user_guide/data/distribution/braze_currents/event_glossary/message_engagement_events#sms-inbound-received-events) und über {% raw %}`{{sms.${inbound_media_urls}}}`{% endraw %} in Liquid bereit.
...
| --- | --- | --- |
| **Text** | Erforderlicher Nachrichtentext, optionale vorgeschlagene Antworten oder Open-URL-Aktionen, optionaler Kurzmitteilungsdienst or SMS-Fallback und Linkverkürzung | Der Nachrichtentext kann bis zu 1.600 oder 3.072 Zeichen enthalten, abhängig vom Kurzmitteilungsdienst or SMS-Dienstanbieter. Fügen Sie bis zu fünf Vorschläge hinzu. |
| **Media** | Erforderliches Bild, Video, Dokument oder Audio; optionaler Nachrichtentext; optionale Vorschläge, Kurzmitteilungsdienst or SMS-Fallback und Linkverkürzung | Der Nachrichtentext kann bis zu 1.600 oder 3.072 Zeichen enthalten, abhängig vom Anbieter, und wird als zusätzliche RCS-Nachricht abgerechnet. Fügen Sie bis zu fünf Vorschläge hinzu. |
| **Card** | Medienkarte oder Nur-Text-Karte, Titel, Beschreibung, Buttons, optionale Vorschläge und optionaler Kurzmitteilungsdienst or SMS-Fallback | Der Titel kann bis zu 200 Zeichen enthalten. Die Beschreibung kann bis zu 1.600 oder 2.000 Zeichen enthalten, abhängig vom Anbieter. Fügen Sie zwischen einem und vier Buttons hinzu. Die Anbieterunterstützung bestimmt, ob Nur-Text-Karten und Vorschläge außerhalb der Karte verfügbar sind. |
...
Aktivieren Sie für jeden RCS-Nachrichtentyp **Kurzmitteilungsdienst or SMS senden, wenn RCS fehlschlägt**, um eine Fallback-Nachricht von bis zu 1.600 Zeichen hinzuzufügen. Die ausgewählte Abo-Gruppe muss einen Kurzmitteilungsdienst or SMS-Absender enthalten. Bei **Card**-Nachrichten sind Links in der Beschreibung nicht anklickbar; verwenden Sie stattdessen einen Open-URL-Button.

Einige Kurzmitteilungsdienst or SMS-Dienstanbieter unterstützen keine eigenständigen **Media**-Nachrichten oder Nur-Text-Karten. Der Editor zeigt nur die unterstützten RCS-Nachrichtentypen an. Bei **Card**-Nachrichten gilt die Linkverkürzung nur für Links im Kurzmitteilungsdienst or SMS-Fallback.
...
Der Editor akzeptiert eine Medien-URL mit bis zu 1.000 Zeichen. Verfügbare Formate und maximale Dateigröße hängen vom Kurzmitteilungsdienst or SMS-Dienstanbieter ab.
...
3. Für jede Aktionsgruppe:
   - Wählen Sie den Trigger or triggern **Eingehende Kurzmitteilungsdienst or SMS-Nachricht senden**.
...
Aktivieren Sie **Linkverkürzung**, um HTTP- und HTTPS-URLs zu verkürzen und Klicks für Kurzmitteilungsdienst or SMS-, MMS- und unterstützte RCS-Links zu tracken. Wählen Sie je nach der in Ihrem Workspace verfügbaren Version Basis- oder erweitertes Tracking oder verwenden Sie die einheitliche Linkverkürzung.

Erweitertes Tracking fügt Klickdaten auf Nutzer:innen-Ebene für Segmentierung und Retargeting hinzu. Die einheitliche Linkverkürzung fasst verkürzte Kurzmitteilungsdienst or SMS- und RCS-Links in einem personalisierten Format zusammen. Informationen zu unterstützten URLs, Liquid-Verhalten, Testanforderungen, angepassten Domains und Retargeting finden Sie unter [Linkverkürzung]({{site.baseurl}}/user_guide/channels/sms_mms_and_rcs/message_features_and_optimization/link_shortening).
...
Gehen Sie zum Tab **Test**, um die Nachricht als Nutzer:in in der Vorschau anzuzeigen oder eine Test-Kurzmitteilungsdienst or SMS, -MMS oder -RCS an eine [Content-Testgruppe]({{site.baseurl}}/user_guide/administer/global/user_management/internal_groups#content-test-groups) oder einzelne Nutzer:innen zu senden.

{% alert tip %}
Verwenden Sie den [Kurzmitteilungsdienst or SMS-Segment-Rechner]({{site.baseurl}}/user_guide/channels/sms_mms_and_rcs/billing_calculator#segment-calculator), um abzuschätzen, wie viele Segmente Ihre Nachricht enthält.
{% endalert %}
...
### 6. Schritt: Den Representational State Transfer Ihrer Campaign oder Ihres Canvas erstellen {#step-6-build-the-remainder-of-your-campaign-or-canvas}

{% tabs %}
{% tab Campaign %}

#### Zustellungszeitplan oder Trigger or triggern wählen {#choose-a-delivery-schedule-or-trigger}

Senden Sie Nachrichten zu einem geplanten Zeitpunkt oder als Reaktion auf eine Aktion oder einen API-Trigger or triggern. Informationen zu Zeitplan- und Trigger or triggern-Optionen finden Sie unter [Campaign planen]({{site.baseurl}}/user_guide/messaging/campaigns/schedule_your_campaign).
...
Die ausgewählte Abo-Gruppe filtert nach abonnierten Nutzer:innen. Kurzmitteilungsdienst or SMS- und MMS-Empfänger:innen benötigen außerdem eine gültige Telefonnummer. RCS-Empfänger:innen benötigen ein RCS-fähiges Gerät und eine Mobilfunkanbieterverbindung; verwenden Sie einen Kurzmitteilungsdienst or SMS-Fallback, um berechtigte Nutzer:innen zu erreichen, wenn die RCS-Zustellung fehlschlägt.
...
Verwenden Sie nach dem Start die [Kurzmitteilungsdienst or SMS-, MMS- und RCS-Berichterstattung]({{site.baseurl}}/user_guide/channels/sms_mms_and_rcs/reporting), um die Nachrichten-Performance zu überprüfen.

## Wissenswertes {#things-to-know}

- Kurzmitteilungsdienst or SMS wird pro Nachrichtensegment abgerechnet, MMS zu einem eigenen Tarif und RCS je nach Nachrichtentyp. Überprüfen Sie die [Kurzmitteilungsdienst or SMS- und RCS-Abrechnungsrechner]({{site.baseurl}}/user_guide/channels/sms_mms_and_rcs/billing_calculator), bevor Sie Nachrichten senden.
...
- RCS-Funktionen und -Beschränkungen variieren je nach Kurzmitteilungsdienst or SMS-Dienstanbieter. Der Editor zeigt nur die Optionen an, die für die ausgewählte Abo-Gruppe verfügbar sind.
