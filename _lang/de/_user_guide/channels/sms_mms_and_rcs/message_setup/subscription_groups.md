...
- **Telefonnummern-Erfassungsformular für In-App-Nachrichten:** Telefonnummern von Nutzer:innen können über das Telefonnummern-Erfassungs-Template im Drag-and-Drop-Editor für In-App-Nachrichten erfasst werden.
...
Bevor Nutzer:innen Nachrichten von einer Campaign oder einem Canvas erhalten können, müssen sie bei der ausgewählten Abo-Gruppe abonniert sein. Wenn Sendungen für ansonsten gültige Nutzer:innen fehlschlagen, bestätigen Sie deren Abo-Status mit einer der Methoden unter [Status von Nutzer:innen festlegen](#set-a-users-state). Informationen zu Double-Opt-in-Anforderungen finden Sie unter [Abo-Gruppenstatus](#subscription-group-states).
...
![SMS-Composer mit geöffnetem Dropdown für die Abo-Gruppe und hervorgehobenem „Messaging Service A for SMS“.]({% image_buster /assets/img/sms/sms_subgroup_select.png %})

## Best Practices für SMS-Abo-Gruppen {#sms-subscription-group-best-practices}

Erstellen Sie separate SMS-Abo-Gruppen für jeden Nachrichtenzweck (z. B. transaktional versus Marketing) und für jeden Workspace. Wenn Sie in mehreren Ländern tätig sind, sollten Sie separate Gruppen nach Region einrichten, um lokale Compliance-Regeln zu unterstützen – beispielsweise die Einschränkungen Brasiliens für Zeitfenster bei Werbenachrichten.
...
![Abo-Gruppen-Dropdown mit hervorgehobenem Eintrag „Messaging Service A for SMS“. Der Eintrag ist mit dem Tag „MMS“ gekennzeichnet.]({% image_buster /assets/img/sms/mms_sub_group_tag.png %}){: style="max-width:40%"}
{% endtab %}
...
Abhängig von Ihrer Integration kann Braze RCS-verifizierte Sender zu Ihren bestehenden SMS-Abo-Gruppen hinzufügen oder neue Abo-Gruppen für Sie einrichten. In beiden Fällen begleitet Sie Ihr Customer-Success-Manager durch ein reibungsloses und effizientes SMS-Traffic-Upgrade.
{% endtab %}
{% endtabs %}
...
Erstellen Sie ein Canvas und geben Sie ihm einen leicht erkennbaren Namen (z. B. „SMS-RCS Abo-Gruppen-Nutzertransfer“). Planen Sie die Campaign dann zu einem für Sie passenden Zeitpunkt.
