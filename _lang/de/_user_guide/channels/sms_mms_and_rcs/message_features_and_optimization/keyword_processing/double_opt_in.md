...
Double-Opt-in ermöglicht es Ihnen, eine ausdrückliche Zustimmung durch eingehende und ausgehende Opt-in-Campaigns einzuholen.

### Ausgehend {#outbound}

Wenn Nutzer:innen ihre Telefonnummer angeben, erhalten sie eine Nachricht, die um ihre Zustimmung bittet.

![Screenshot einer ausgehenden SMS-Nachricht, bei der die Marke schreibt: „Willkommen bei BRAND-Textupdates! 1 Nachricht pro Woche mit den neuesten Angeboten. Antworten Sie mit Y für das Opt-in.“, die Nutzer:innen mit „Y“ antworten und die Marke mit „Danke! Sie haben jetzt ein Opt-in für BRAND-Benachrichtigungen. Hier ist ein Promo-Code SMS10 für 10 % Rabatt auf Ihren ersten Einkauf!“ antwortet.]({% image_buster /assets/img/double_opt_in_outbound.png %}){:style="max-width:40%;"}

### Eingehend {#inbound}

Wenn Nutzer:innen eine Nachricht mit einem Opt-in-Keyword senden, erhalten sie eine Nachricht, die um ihre Zustimmung bittet.

![Screenshot einer eingehenden SMS-Nachricht, bei der Nutzer:innen „JOIN“ senden und die Antwort erhalten: „Antworten Sie mit Y, um zu bestätigen, dass Sie unserem SMS-Programm beitreten möchten. 3 Nachrichten/Woche, schreiben Sie jederzeit STOP zum Beenden“, und dann mit „Y“ antworten.]({% image_buster /assets/img/double_opt_in_inbound.png %}){:style="max-width:40%;"}

## Double-Opt-in aktivieren {#enabling-double-opt-in}

Um Double-Opt-in zu aktivieren, gehen Sie zur Tabelle **Global Keywords** in der entsprechenden Abo-Gruppe und klicken Sie auf **Edit** in der **Opt-In Keyword Category**. Wählen Sie dann Ihre Opt-in-Methode (**Opt-In** oder **Double Opt-In**). Die Auswahl von **Double Opt-In** erweitert die Seite und zeigt zusätzliche [konfigurierbare Felder](#configurable-fields) an.
...
| ----------- |----------- |----------------
| Opt-in-Anfrage | Keywords | Dies sind die Keywords, die Nutzer:innen senden können, um ihre Opt-in-Absicht anzuzeigen. `START` ist ein erforderliches Keyword. Diese Opt-in-Anfrage wird auch an Nutzer:innen gesendet, wenn ihr Abo-Status durch die im Abschnitt [Abo-Quellen](#subscription-sources) aufgeführten Quellen aktualisiert wird.
| | Antwortnachricht | Dies ist die erste Antwort, die Nutzer:innen erhalten, nachdem sie ein Opt-in-Keyword gesendet haben (zum Beispiel: „Antworten Sie mit Y, um zu bestätigen, dass Sie Nachrichten von dieser Nummer erhalten möchten. Es können Nachrichten- und Datengebühren anfallen.“)
| Double-Opt-in-Bestätigung | Keywords | Dies sind die Keywords, mit denen Nutzer:innen antworten können, um ihre Opt-in-Absicht zu bestätigen. Mindestens ein Keyword ist erforderlich. Diese Keywords sollten im Feld **Opt-In Prompt Reply Message** angegeben werden.
| | Antwortnachricht | Dies ist die Bestätigungsantwort, die Nutzer:innen erhalten, nachdem sie ihr Opt-in ausdrücklich bestätigt haben und nun Nachrichten empfangen können. Der Abo-Gruppenstatus der Nutzer:innen wird auf `Subscribed` gesetzt.
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="Configurable fields #configurable-fields" }

Wenn Nutzer:innen eine Opt-in-Anfrage erhalten, haben sie 30 Tage Zeit, ihre Opt-in-Absicht zu bestätigen. Wenn Nutzer:innen sich nach dem 30-Tage-Fenster anmelden möchten, müssen sie ein Opt-in-Keyword senden, um den Double-Opt-in-Workflow erneut zu starten.

![Die konfigurierbaren Felder haben zwei Abschnitte, „Opt-In Prompt“ und „Double Opt-In Confirmation“, jeweils mit den Feldern „Keywords“ und „Reply Message“.]({% image_buster /assets/img/double_opt_in_fields.png %})

## Abo-Gruppenstatus {#subscription-group-status}

Erst nachdem Nutzer:innen den Double-Opt-in-Workflow abgeschlossen haben, wird ihr [Abo-Gruppenstatus]({{site.baseurl}}/sms_rcs_subscription_groups/) auf `Subscribed` aktualisiert. Wenn Nutzer:innen den Workflow beginnen, aber nicht abschließen, bleiben sie `Unsubscribed` und können keine Nachrichten von dieser Abo-Gruppe erhalten.

Nutzer:innen können auch in den Double-Opt-in-Workflow eingebunden werden, wenn sie [über andere Quellen abonniert werden]({{site.baseurl}}/user_guide/channels/sms_mms_and_rcs/message_setup/subscription_groups/) (zum Beispiel REST API, SDK).
...
Für eingehende Nachrichten wird Double-Opt-in für alle in der Abo-Gruppe definierten Sprachen unterstützt. Das bedeutet, dass Sie Ihre automatischen Antworten in verschiedenen Sprachen definieren können und Braze die automatische Antwort sendet, die einer bestimmten Sprache zugeordnet ist, wenn ein passendes Keyword empfangen wird.

Nutzer:innen, die den Double-Opt-in-Workflow über Abo-Aktualisierungen betreten, die außerhalb eingehender Nachrichten erfolgen (zum Beispiel SDK, REST API, Shopify), erhalten nur die englischen Keywords.