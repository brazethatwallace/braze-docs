...
## Wie universelle Links und App Links funktionieren {#how-universal-links-and-app-links-work}

Universelle Links (iOS) und App Links (Android) sind standardmäßige Weblinks (`http://mydomain.com`), die sowohl auf eine Webseite als auch auf einen Inhalt innerhalb einer App verweisen.

Wenn ein universeller Link oder ein App Link geöffnet wird, prüft das Betriebssystem, ob eine installierte App für diese Domain registriert ist. Wird eine App gefunden, wird sie sofort gestartet, ohne die Webseite zu laden. Wird keine App gefunden, wird die Web-URL im Standard-Webbrowser der Nutzer:innen geladen, der auch so konfiguriert sein kann, dass er zum App Store bzw. Google Play Store weiterleitet.

Einfach ausgedrückt ermöglichen universelle Links einer Website, ihre Webseiten mit bestimmten App-Bildschirmen zu verknüpfen. Wenn Nutzer:innen auf einen Link zu einer Webseite klicken, die einem App-Bildschirm entspricht, kann die App direkt geöffnet werden (sofern sie aktuell installiert ist).

{% alert important %}
Firebase Dynamic Links ist veraltet. Braze hat keine direkte Integration mit Firebase, und Deeplinking wird außerhalb der Braze-Plattform verwaltet. Migrieren Sie zu plattformnativen Lösungen (universelle Apple-Links und Android App Links, wie in diesem Artikel beschrieben) oder zu alternativen Deeplinking-Dienstleistern. Informationen zur Migration finden Sie in den [Firebase-Migrations-FAQ](https://firebase.google.com/support/dynamic-links-faq).
{% endalert %}

Diese Tabelle zeigt die wichtigsten Unterschiede zwischen universellen Links und herkömmlichen Deeplinks:

|                        | Universelle Links und App Links                                | Deeplinks                   |
| ---------------------- | -------------------------------------------------------------- | ---------------------------- |
| Plattformkompatibilität | iOS (Version 9 und höher) und Android (Version 6.0 und höher)  | In verschiedenen mobilen Betriebssystemen verwendet    |
| Zweck                | Nahtlose Verknüpfung von Web- und App-Inhalten auf iOS- und Android-Geräten | Verlinkung zu bestimmten App-Inhalten |
| Funktion               | Leitet je nach Kontext zu Webseiten oder App-Inhalten weiter           | Öffnet bestimmte App-Bildschirme   |
| App-Installation       | Öffnet die App, wenn sie installiert ist, andernfalls Webinhalte | Setzt voraus, dass die App installiert ist |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="Wie universelle Links und App Links funktionieren" }
...
Damit Apps Universal Links oder App Links unterstützen können, müssen sowohl iOS als auch Android eine spezielle Berechtigungsdatei bereitstellen, die unter der Link-Domain gehostet wird. Diese Datei enthält Definitionen darüber, welche Apps Links von dieser Domain öffnen dürfen und – im Fall von iOS – welche Pfade diese Apps öffnen dürfen:

- **iOS:** Apple App Site Association (AASA)-Datei
- **Android:** Digital Asset Links-Datei

Zusätzlich zu dieser Berechtigungsdatei gibt es fest codierte Definitionen, welche Link-Domains die App öffnen darf, die innerhalb der App konfiguriert werden:

- **iOS:** Als „Associated Domains“ in Xcode festgelegt
- **Android:** In der `AndroidManifest.xml`-Datei der App definiert

Diese zweiteilige Domain-App-Verknüpfung ist erforderlich, damit ein Universal Link oder App Link funktioniert, und verhindert, dass eine beliebige App Links von einer bestimmten Domain übernimmt oder eine beliebige Domain eine bestimmte App öffnet.

{% tabs %}
<!--iOS instructions-->
{% tab iOS %}

Diese Schritte basieren auf der Apple-Entwicklerdokumentation. Weitere Informationen finden Sie unter [Allowing apps and websites to link to your content](https://developer.apple.com/documentation/xcode/allowing-apps-and-websites-to-link-to-your-content?language=objc).

### Schritt 1: App-Berechtigungen konfigurieren {#step-1-configure-your-app-entitlements}

{% alert note %}
[Ab Xcode 13](https://developer.apple.com/help/account/reference/provisioning-with-managed-capabilities/) kann Xcode die Berechtigungsprovisionierung automatisch für Sie übernehmen. Sie können wahrscheinlich direkt zu [Schritt&nbsp;1c](#step-1c) springen und bei Problemen auf diese Anleitung zurückgreifen.
{% endalert %}
...
4. Falls Sie noch keinen registrierten App Identifier haben, klicken Sie auf +, um einen zu erstellen.
...
#### Schritt 1b: Associated Domains im App Identifier aktivieren {#step-1b-turn-on-associated-domains-in-your-app-identifier}
...
Fügen Sie im Abschnitt „Domains“ den entsprechenden Domain-Tag hinzu. Sie müssen ihn mit `applinks:` als Präfix versehen. In diesem Fall sehen Sie, dass wir `applinks:yourdomain.com` hinzugefügt haben.

![Abschnitt „Associated Domains“]({% image_buster /assets/img_archive/universal_links_1d.png %})

#### Schritt 1e: Sicherstellen, dass die Berechtigungsdatei beim Build enthalten ist {#step-1e-confirm-that-the-entitlements-file-is-included-at-build}
...
### Schritt 2: Website für das Hosten der AASA-Datei konfigurieren {#step-2-configure-your-website-to-host-the-aasa-file}

Um Ihre Website-Domain mit Ihrer nativen App unter iOS zu verknüpfen, müssen Sie die Apple App Site Association (AASA)-Datei auf Ihrer Website hosten. Diese Datei dient als sichere Methode zur Verifizierung des Domain-Besitzes gegenüber iOS. Vor iOS 9 konnten Entwickler:innen jedes URI-Schema registrieren, um ihre Apps zu öffnen, ohne jegliche Verifizierung. Mit AASA ist dieser Prozess jedoch deutlich sicherer und zuverlässiger geworden.
...
- `appID`: Wird durch die Kombination der **Team-ID** Ihrer App (gehen Sie zu `https://developer.apple.com/account/#/membership/`, um die Team-ID zu erhalten) und des **Bundle Identifiers** erstellt. In diesem Beispiel ist „JHGFJHHYX“ die Team-ID und „com.facebook.ios“ die Bundle-ID.
- `paths`: String-Array, das angibt, welche Pfade in die Verknüpfung eingeschlossen oder davon ausgeschlossen werden. Sie können `NOT` vor einem Pfad verwenden, um Pfade zu deaktivieren. In diesem Beispiel werden alle Links auf diesem Pfad im Web geöffnet, anstatt die App zu öffnen. Sie können `*` als Platzhalter verwenden, um alle Pfade in einem Verzeichnis zu aktivieren, und `?`, um ein einzelnes Zeichen abzugleichen (z. B. /archives/201?/ für alle Nummern von 2010–2019).

{% alert note %}
Bei diesen Strings wird zwischen Groß- und Kleinschreibung unterschieden, und Query-Strings sowie Fragment-Bezeichner werden ignoriert.
{% endalert %}

### Schritt 3: AASA-Datei auf Ihrer Domain hosten {#step-3-host-the-aasa-file-on-your-domain}

Wenn Ihre AASA-Datei fertig ist, können Sie sie jetzt auf Ihrer Domain hosten, entweder unter `https://<<yourdomain>>/apple-app-site-association` oder unter `https://<<yourdomain>>/.well-known/apple-app-site-association`.
...
Stellen Sie beim Hosten der AASA-Datei sicher, dass die Datei die folgenden Richtlinien erfüllt:

- Wird über HTTPS bereitgestellt.
- Verwendet den MIME-Typ `application/json`.
- Überschreitet nicht 128 KB (Anforderung ab iOS 9.3.1).

### Schritt 4: App für die Verarbeitung von Universal Links vorbereiten {#step-4-prepare-your-app-to-handle-universal-links}
...
Fügen Sie den Universal Link zu einer E-Mail hinzu und senden Sie diese an ein Testgerät. Das Einfügen eines Universal Links direkt in das Safari-URL-Feld führt nicht dazu, dass die App automatisch geöffnet wird. Wenn Sie dies tun, müssen Sie die Website manuell nach unten ziehen, damit oben ein Hinweis erscheint, der Sie auffordert, die entsprechende App zu öffnen.
...
Diese Schritte basieren auf der Android-Entwicklerdokumentation. Weitere Informationen finden Sie unter [Add Android App Links](https://developer.android.com/training/app-links#add-app-links) und [Create Deep Links to App Content](https://developer.android.com/training/app-links/deep-linking).
...
In Ihrer Android-App müssen Sie die eingehenden Deeplinks verarbeiten. Sie können dies tun, indem Sie den Intent abrufen, der Ihre Activity gestartet hat, und die Daten daraus extrahieren.
...
{% alert note %}
Klick-Tracking-Links werden in der Regel im Rahmen Ihres Onboardings für E-Mail eingerichtet. Falls dies beim Kunden-Onboarding nicht abgeschlossen wurde, wenden Sie sich an Ihren Account Manager.
{% endalert %}

Unsere E-Mail-Versandpartner verwenden Klick-Tracking-Domains, um alle Links zu wrappen und URL-Parameter für das Klick-Tracking in Braze-E-Mails einzufügen.
...
Damit E-Mail-Links mit Klick-Tracking als Universal Links oder App Links funktionieren, müssen Sie eine zusätzliche Einrichtung vornehmen. Stellen Sie sicher, dass die Klick-Tracking-Domain (`links.email.example.com`) als Domain hinzugefügt wird, die von der App geöffnet werden darf. Darüber hinaus sollte die Klick-Tracking-Domain die AASA-Datei (iOS) oder Digital Asset Links-Datei (Android) bereitstellen. Dadurch wird sichergestellt, dass E-Mail-Links mit Klick-Tracking nahtlos funktionieren.

Wenn Sie nicht möchten, dass jeder Klick-Tracking-Link ein Universal Link oder App Link ist, können Sie je nach E-Mail-Versandpartner festlegen, welche Links als Universal Links behandelt werden sollen. Weitere Informationen finden Sie in den folgenden Tabs.
...
2. Fügen Sie das Attribut `universal="true"` zum Anchor-Tag (`<a>`) Ihres Links hinzu. Dadurch wird der URL-Pfad des gewrappten Links so geändert, dass er `/uni/` enthält.

{% alert note %}
Bei AMP-E-Mails sollte dieses Attribut data-universal="true" lauten.
{% endalert %}
...
Um einen SparkPost-Klick-Tracking-Link als Universal Link zu behandeln, fügen Sie das folgende Attribut im Bereich „Attribute“ des Drag-and-Drop-Editors für E-Mail hinzu, oder bearbeiten Sie den Link-HTML-Code manuell, um das folgende Attribut im Anchor-Tag Ihres Links einzufügen: `data-msys-sublink="custom_path"`.

Dieser benutzerdefinierte Pfad ermöglicht es Ihnen, URLs mit diesem Wert selektiv als Universal Link zu behandeln.
...
Stellen Sie dann sicher, dass Ihre App so eingerichtet ist, dass sie den benutzerdefinierten Pfad korrekt verarbeitet. Lesen Sie den Artikel von SparkPost zum Thema [Using SparkPost click tracking on deep links](https://support.sparkpost.com/docs/tech-resources/deep-links-self-serve#preferred-solution-using-sparkpost-click-tracking-on-deep-links). Dieser Artikel enthält Beispielcode für [iOS](https://support.sparkpost.com/docs/tech-resources/deep-links-self-serve#ios-swift-forwarding-clicks-to-sparkpost) und [Android](https://support.sparkpost.com/docs/tech-resources/deep-links-self-serve#forwarding-clicks-from-android-to-sparkpost).

{% endtab %}
{% tab Amazon SES %}

Verwenden Sie benutzerdefinierte Pfade, um Pfadsegmente zu E-Mail-Klick-Tracking-URLs hinzuzufügen. Dadurch entstehen vorhersagbare URL-Muster, die mobile Betriebssysteme für Universal Links und App Links erkennen können.

Wenn Nutzer:innen auf mobilen Geräten auf E-Mail-Links tippen, helfen benutzerdefinierte Pfade Ihnen zu steuern, ob Links in Ihrer Haupt-App, einer speziellen App oder dem mobilen Browser geöffnet werden (z. B. Produktseiten, Kundenbindungs-Programme, Abmelde-Links oder rechtliche Seiten).

So behandeln Sie einen Amazon SES-Klick-Tracking-Link als Universal Link oder App Link:

1. Fügen Sie `ses:custom-path`-Attribute zu Ihren Anchor-Tags im E-Mail-HTML hinzu, oder fügen Sie das Attribut im Bereich **Attribute** des Drag-and-Drop-Editors für E-Mail hinzu. Der benutzerdefinierte Pfad wird in die gewrappte Klick-Tracking-URL eingefügt.
...
Stellen Sie sicher, dass Ihre benutzerdefinierten Pfade diese Anforderungen erfüllen:
...
{:start="2"}
2. Überprüfen Sie, ob Ihre gewrappten Tracking-URLs das benutzerdefinierte Pfadsegment enthalten. Links folgen diesem Format: `track.yourstore.com/L1/{customPath}/...`
...
{:start="3"}
3. Konfigurieren Sie Ihre Site-Association-Dateien auf Ihrer Klick-Tracking-Domain, sodass die Pfade mit `/L1/{customPath}/` übereinstimmen.
...
Stellen Sie sicher, dass Ihre App so eingerichtet ist, dass sie diese gewrappten Links verarbeitet. Fügen Sie Ihre Klick-Tracking-Domain zu den zugehörigen Domains (iOS) oder Intent-Filtern (Android) Ihrer App hinzu und hosten Sie die AASA- oder Digital Asset Links-Datei auf dieser Domain, wie weiter oben in diesem Artikel beschrieben.
...
Sie können das Klick-Tracking für bestimmte Links deaktivieren, indem Sie HTML-Code zu Ihrer E-Mail-Nachricht für den HTML-Editor oder zu einem HTML-Block im Drag-and-Drop-Editor hinzufügen.
...
Wenn Sie den Drag-and-Drop-E-Mail-Editor verwenden, geben Sie Ihren HTML-Code als benutzerdefiniertes Attribut ein, wenn Ihr Link an Text, einen Button oder ein Bild angehängt ist.

##### Benutzerdefiniertes Attribut für einen Textlink {#custom-attribute-for-a-text-link}

#### SendGrid

Wählen Sie folgende Werte für das benutzerdefinierte Attribut:
...
![Ein benutzerdefiniertes Attribut für einen Textlink.]({% image_buster /assets/img/text_click_tracking_off.png %}){: style="max-width:60%;"}

##### Benutzerdefiniertes Attribut für einen Button oder ein Bild {#custom-attribute-for-a-button-or-image}
...
![Ein benutzerdefiniertes Attribut für einen Button.]({% image_buster /assets/img/button_click_tracking_off.png %}){: style="max-width:60%;"}

### Fehlerbehebung bei Universal Links mit Klick-Tracking {#troubleshooting-universal-links-with-click-tracking}

Wenn Ihre Universal Links in Ihren E-Mails nicht wie erwartet funktionieren – z. B. wenn die Empfänger:innen von ihrer E-Mail-App zum Webbrowser navigiert werden, bevor sie schließlich zur App weitergeleitet werden – beachten Sie diese Tipps zur Fehlerbehebung Ihres Universal-Link-Setups.

#### Outlook zeigt `[?it=` oder rohen URL-Text anstelle eines Buttons an {#outlook-shows-it-or-raw-url-text-instead-of-a-button}

Outlook zeigt möglicherweise Call-to-Action-Text wie `[?it=` an oder gibt Teile des `href` aus, wenn ein Link kein gültiges **`http://`- oder `https://`**-URL-Schema verwendet. Benutzerdefinierte Schemata, fehlende Schemata oder fehlerhafte URLs werden nicht als Hyperlinks behandelt, sodass der Client stattdessen den Attributtext anzeigt. Stellen Sie sicher, dass jeder Button, Bildlink und jede Tracking-URL eine vollständige `https://`- (oder `http://`-) Zieladresse verwendet. Dies gilt sowohl für Universal Links als auch für Standard-Web-Links.
...
- **iOS:** Überprüfen Sie die in Xcode für Ihre App eingerichteten zugehörigen Domains ([Schritt 1c: Zugehörige Domains in Ihrem Xcode-Projekt aktivieren]({{site.baseurl}}/user_guide/channels/email/customize/universal_links_and_app_links?tab=ios#step-1c)). Prüfen Sie, ob die Klick-Tracking-Domain in dieser Liste enthalten ist.
- **Android:** Öffnen Sie die App-Info-Seite (drücken Sie lange auf das App-Symbol und tippen Sie auf ⓘ). Suchen Sie im App-Info-Menü nach **Standardmäßig öffnen** und tippen Sie darauf. Dort sollte ein Bildschirm mit allen verifizierten Links angezeigt werden, die die App öffnen darf. Prüfen Sie, ob die Klick-Tracking-Domain in dieser Liste enthalten ist.

#### Jeder E-Mail-Link öffnet die App {#every-email-link-opens-the-app}

Wenn jeder Link in einer E-Mail Ihre App öffnet – einschließlich Links, von denen Sie erwarten, dass sie im Browser geöffnet werden – stimmen die AASA-`paths`-Werte (iOS) oder Android-`pathPrefix`-Werte auf Ihrer Klick-Tracking-Domain mit der gesamten Domain überein (z. B. `*` oder `/*`).

Beschränken Sie diese Muster auf die URLs, die die App öffnen sollen. Für SendGrid filtern Sie auf `/uni/` und fügen Sie `universal="true"` nur bei diesen Links hinzu. Siehe [Universal Links, App Links und Klick-Tracking](#universal-links-app-links-and-click-tracking).

#### Tracking-Domain kann keine .well-known-Dateien bereitstellen {#tracking-domain-cant-serve-well-known-files}

In manchen Fällen kann Ihre Klick-Tracking-Domain die erforderlichen `.well-known`-Dateien aufgrund von ESP-Einschränkungen oder Infrastrukturbeschränkungen nicht hosten. Wenn Sie die AASA- oder Digital Asset Links-Datei nicht auf Ihrer Tracking-Domain hosten können, ziehen Sie die folgenden Optionen in Betracht:
...
- **Tracking-Subdomain mit einem CDN vorschalten:** Wenn Sie vollständige Klick-Tracking-Abdeckung und Deeplinking benötigen, können Sie ein CDN (z. B. Cloudflare oder CloudFront) vor Ihre Tracking-Subdomain schalten. Konfigurieren Sie das CDN so, dass es die `.well-known`-Dateien lokal bereitstellt und den gesamten übrigen Traffic an Ihren ESP weiterleitet. Dieser Ansatz ist aufwendiger, gibt Ihnen aber die volle Kontrolle über Klick-Tracking und Universal Links.

#### Links funktionieren in einem Workspace, aber nicht in einem anderen {#links-working-in-one-workspace-but-not-another}

Wenn Universal Links oder App Links in Ihrem Produktions-Workspace korrekt funktionieren, aber in Ihrem Entwicklungs- oder Test-Workspace fehlschlagen, überprüfen Sie, ob die Domain der Absende-E-Mail-Adresse mit der in den E-Mail-Einstellungen jedes Workspaces konfigurierten Tracking-Domain übereinstimmt. Inkonsistente Konfigurationen zwischen Workspaces können dazu führen, dass sich Links unterschiedlich verhalten, selbst wenn dieselben E-Mail-Templates und AASA- oder Digital Asset Links-Dateien verwendet werden.
...
3. Stellen Sie sicher, dass Ihre Versanddomain und Tracking-Domain für den Workspace, in dem Links nicht funktionieren, korrekt aufeinander abgestimmt sind.
