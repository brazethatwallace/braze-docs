...
2. Wählen Sie **E-Mail** oder, für Campaigns, die auf mehrere Kanäle abzielen, **Multikanal**.
...
4. Fügen Sie bei Bedarf [Teams]({{site.baseurl}}/user_guide/administer/global/user_management/teams) und [Tags]({{site.baseurl}}/user_guide/administer/global/workspace_settings/tags) hinzu.
   * Tags erleichtern das Auffinden Ihrer Campaigns und das Erstellen von Berichten. Wenn Sie z. B. den [Berichts-Builder]({{site.baseurl}}/user_guide/analytics/reports/report_builder) verwenden, können Sie nach bestimmten Tags filtern.
...
{% alert tip %}
Wenn alle Nachrichten in Ihrer Campaign ähnlich sein oder denselben Inhalt haben werden, verfassen Sie Ihre Nachricht, bevor Sie zusätzliche Varianten hinzufügen. Sie können dann **Aus Variante kopieren** aus dem Dropdown **Variante hinzufügen** auswählen.
{% endalert %}
{% endtab %}
{% tab Canvas %}
...
{% alert tip %}
Wenn Sie benutzerdefiniertes HTML erstellen möchten und Hintergründe in der mobilen Gmail-App bei aktiviertem Dark Mode konsistent bleiben sollen, lesen Sie [Mobile Gmail-App und Hintergrundfarben im Dark Mode](#gmail-dark-mode).
{% endalert %}
...
Nachdem Sie Ihr Template ausgewählt haben, sehen Sie eine Übersicht Ihrer E-Mail, von der aus Sie direkt zum Vollbild-Editor wechseln können, um Ihre E-Mail zu verfassen, Ihre Versandinformationen zu ändern und Warnungen zur Zustellbarkeit oder zur Rechtskonformität einzusehen. Beim Verfassen können Sie zwischen den Tabs HTML, Klassisch, Nur-Text und [AMP]({{site.baseurl}}/user_guide/channels/email/customize/amp_for_email) wechseln.
...
Braze aktualisiert automatisch die Nur-Text-Version aus der HTML-Version, bis eine Bearbeitung der Nur-Text-Version erkannt wird. Nachdem Braze eine Bearbeitung erkennt, stoppt es die Aktualisierung der Nur-Text-Version, da davon ausgegangen wird, dass Sie absichtliche Änderungen vorgenommen haben. Um die automatische Synchronisierung wiederherzustellen, gehen Sie zu **Nur-Text** und wählen Sie **Aus HTML regenerieren** (nur sichtbar, wenn die Nur-Text-Version nicht synchronisiert wird).

{% alert tip %}
Um Bewegung in einer E-Mail mit einer exakten Vorschau hinzuzufügen, verwenden Sie GIFs anstelle von Elementen, die JavaScript erfordern, da die meisten Posteingänge JavaScript nicht unterstützen.
{% endalert %}


{% alert important %}
Braze entfernt automatisch HTML-Event-Handler, die als Attribute referenziert werden. Dadurch wird das HTML modifiziert, überprüfen Sie die E-Mail daher noch einmal, nachdem Sie fertig sind. Erfahren Sie mehr über [HTML-Handler](https://www.w3schools.com/tags/ref_eventattributes.asp).
{% endalert %}
...
Nachdem Sie Ihre E-Mail-Nachricht entworfen und erstellt haben, fügen Sie Ihre Versandinformationen in den **Versandeinstellungen** hinzu.

{% multi_lang_include email/sending_info_steps.md %}

{% multi_lang_include alerts/tip_alerts.md alert='Liquid email display name and reply-to address' %}

Eine Vorschau im rechten Bereich wird mit den von Ihnen hinzugefügten Versandinformationen befüllt. Diese Informationen können auch aktualisiert werden, indem Sie zu **Einstellungen** > **E-Mail-Präferenzen** > **Versandkonfiguration** gehen.

#### Erweitert {#advanced}

Aktivieren Sie unter **Versandeinstellungen** > **Erweitert** die Option **Inline-CSS** für die breiteste Client-Unterstützung. Wenn Nachrichten abgeschnitten werden oder Bilder sich auf die Zeilenhöhe strecken, versuchen Sie, Inline-CSS vorübergehend **auszuschalten**. Einige Templates funktionieren ohne Inlining besser.
...
Sie können auch E-Mail-Anhänge mit den folgenden Methoden hinzufügen:

{% multi_lang_include email/attachment_upload_options.md %}

Spezifische Best Practices finden Sie in den [E-Mail-Richtlinien]({{site.baseurl}}/user_guide/channels/email/best_practices/email_guidelines).

##### E-Mail-Header {#email-headers}

Um E-Mail-Header hinzuzufügen, wählen Sie **Neuen Header hinzufügen**. E-Mail-Header enthalten Informationen über die gesendete E-Mail. Diese [Schlüssel-Wert-Paare]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/sources/key_value_pairs) umfassen in der Regel Absender, Empfänger:in, Authentifizierungsprotokoll und Routing-Informationen. Braze fügt automatisch die gemäß RFC erforderlichen Header-Informationen hinzu, damit E-Mails die Posteingangsanbieter erreichen.

Braze gibt Ihnen die Flexibilität, bei Bedarf zusätzliche E-Mail-Header für erweiterte Anwendungsfälle hinzuzufügen. Es gibt einige reservierte Felder, die die Braze-Plattform beim Senden überschreibt.
...
{% alert warning %}
Die Gesamtzahl der hinzugefügten Schlüssel-Wert-Paare sollte 1 KB nicht überschreiten. Andernfalls werden die Nachrichten abgebrochen.
{% endalert %}
...
Nachdem Sie Ihre E-Mail fertiggestellt haben, testen Sie sie vor dem Senden. Wählen Sie am unteren Rand des Übersichtsbildschirms **Vorschau und Test**.

Hier können Sie eine Vorschau anzeigen, wie Ihre E-Mail im Posteingang einer Kund:in erscheinen wird. Mit der ausgewählten Option **Als Nutzer:in anzeigen** können Sie eine Vorschau Ihrer E-Mail als zufällige:r Nutzer:in anzeigen, eine:n bestimmte:n Nutzer:in auswählen oder eine:n angepasste:n Nutzer:in erstellen. So können Sie testen, ob Ihre Connected-Content- und Personalisierungsaufrufe wie erwartet funktionieren.

Anschließend können Sie **Vorschau-Link kopieren** auswählen, um einen teilbaren Vorschau-Link zu generieren und zu kopieren, der zeigt, wie die E-Mail für eine:n zufällige:n Nutzer:in aussehen wird. Weitere Informationen finden Sie unter [Teilbare Vorschau]({{site.baseurl}}/user_guide/messaging/governance/shareable_preview).

Sie können auch zwischen Desktop-, Mobil- und Nur-Text-Ansichten wechseln, um ein Gefühl dafür zu bekommen, wie Ihre Nachricht in verschiedenen Kontexten erscheint.

{% alert tip %}
Neugierig, wie Ihre E-Mail für Nutzer:innen im Dark Mode aussieht? Wählen Sie den Umschalter **Dark-Mode-Vorschau** im Bereich **Vorschau und Test** (nur Drag-and-Drop-Editor). Wenn Sie den HTML-Editor verwenden, können Sie das Dark-Mode-Rendering in Gmail für Mobilgeräte dennoch mit [Gmail-App für Mobilgeräte und Dark Mode](#gmail-dark-mode) adressieren.
{% endalert %}

Wenn Sie für eine abschließende Prüfung bereit sind, wählen Sie **Testnachricht senden** und senden Sie eine Testnachricht an sich selbst oder eine Tester-Gruppe, um zu bestätigen, dass die E-Mail auf verschiedenen Geräten und Clients korrekt angezeigt wird.

![Option „Testnachricht senden“ und Beispiel-E-Mail-Vorschau beim Verfassen Ihrer E-Mail.]({% image_buster /assets/img_archive/newEmailTest.png %})
...
<br><br>Um dies zu vermeiden, können Sie nach Ihrem gewünschten Vorschautext Leerraum erstellen, damit E-Mail-Clients keinen anderen störenden Text oder Zeichen in den Envelope-Inhalt ziehen. Im Bereich **Versandeinstellungen** können Sie das Kontrollkästchen **Leerraum nach Preheader hinzufügen** aktivieren, um automatisch Leerraum hinzuzufügen. <br><br>Alternativ können Sie, wenn Sie mehr Kontrolle benötigen, nach dem gewünschten Vorschautext manuell eine Kette von Zero-Width-Non-Joinern (‌`&zwnj;`) und geschützten Leerzeichen (`&nbsp;`) hinzufügen. <br><br>Wenn Sie den folgenden Code am Ende Ihres Vorschautexts im Preheader-Bereich des HTML-Editors einfügen, wird der gewünschte Leerraum erzeugt:<br><br>
...
Vor dem Senden kennzeichnet der Editor häufige Probleme:

- Anzeigename und Header des Absenders sind nicht gemeinsam festgelegt
...
- E-Mail-Body überschreitet 400&nbsp;KB
...
- Absender-Domain nicht auf der Allowlist (Zustellung stark gedrosselt)

## Schritt 4: Erstellen Sie den Rest Ihrer Campaign oder Ihres Canvas {#step-4-build-the-remainder-of-your-campaign-or-canvas}

{% tabs %}
{% tab Campaign %}
Erstellen Sie als Nächstes den Rest Ihrer Campaign. In den folgenden Abschnitten erfahren Sie, wie Sie die Tools von Braze für Ihre E-Mail-Campaign nutzen können.
...
{% alert note %}
Wenn bei API-getriggerten Campaigns die Aktion auf **Mit Campaign interagieren** eingestellt ist und eine **Empfangen**-Option als Interaktion ausgewählt wird, wird Ihre neue Campaign ausgelöst, sobald Braze die ausgewählte Campaign als gesendet markiert – auch wenn die Nachricht bounct oder nicht zugestellt werden kann.
{% endalert %}

Sie können auch die Dauer der Campaign festlegen, [Ruhezeiten]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/quiet_hours) konfigurieren und Regeln für das [Frequency-Capping]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/frequency_capping#about-frequency-capping) einrichten.

### Zielgruppe zusammenstellen {#choose-users-to-target}

Als Nächstes [stellen Sie Ihre Zielgruppe zusammen]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/target_users), indem Sie Segmente oder Filter auswählen. Braze zeigt eine Live-Vorschau der Segment-Population, einschließlich der Anzahl der Nutzer:innen, die per E-Mail erreichbar sind. Die genaue Segment-Zugehörigkeit wird kurz vor dem Versand berechnet.

{% multi_lang_include audience/target_audiences.md %}

Sie können Ihre Campaign auch nur an Nutzer:innen mit einem bestimmten [Abo-Status]({{site.baseurl}}/user_guide/channels/email/subscriptions) senden, z. B. an Nutzer:innen, die E-Mails abonniert haben und ein Opt-in erteilt haben.

Optional können Sie auch die Zustellung auf eine bestimmte Anzahl von Nutzer:innen innerhalb des Segments begrenzen oder Nutzer:innen erlauben, dieselbe Nachricht bei einer Wiederholung der Campaign erneut zu erhalten.
...
Bei Multichannel-Campaigns, die sowohl E-Mail- als auch Push-Kanäle ansprechen, möchten Sie Ihre Campaign möglicherweise so einschränken, dass nur Nutzer:innen mit explizitem Opt-in die Nachricht erhalten (abonnierte oder abgemeldete Nutzer:innen werden ausgeschlossen). Angenommen, Sie haben drei Nutzer:innen mit unterschiedlichem Opt-in-Status:

{% multi_lang_include messaging/intelligent_channel_user_examples.md %}

Wählen Sie dazu unter **Zielgruppenzusammenfassung** die Option, diese Campaign nur an „Nutzer:innen mit Opt-in“ zu senden. Dadurch wird sichergestellt, dass nur Nutzer:innen mit Opt-in Ihre E-Mail erhalten, und Braze sendet Push-Nachrichten standardmäßig nur an Push-aktivierte Nutzer:innen.

{% alert important %}
Fügen Sie bei dieser Konfiguration keine Filter im Schritt **Zielgruppen** hinzu, die die Zielgruppe auf einen einzelnen Kanal beschränken (z. B. `Foreground Push Enabled = True` oder `Email Subscription = Opted-In`).
{% endalert %}

### Konversions-Events wählen {#choose-conversion-events}

Braze ermöglicht es Ihnen, nachzuverfolgen, wie oft Nutzer:innen nach dem Erhalt einer Campaign bestimmte Aktionen ausführen – sogenannte [Konversions-Events]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/conversion_events). Sie können jede der folgenden Aktionen als Konversions-Event festlegen:

- App öffnen
- Kauf tätigen (Dies kann ein allgemeiner Kauf oder ein bestimmter Artikel sein)
- Ein bestimmtes angepasstes Event ausführen
- E-Mail öffnen

Sie können ein Zeitfenster von bis zu 30 Tagen festlegen, in dem Braze eine Konversion zählt, wenn Nutzer:innen die angegebene Aktion ausführen. Obwohl Braze Öffnungen und Klicks automatisch trackt, können Sie das Konversions-Event auf ein Öffnen oder einen Klick setzen, um [Optimierung mit BrazeAI<sup>TM</sup>]({{site.baseurl}}/user_guide/brazeai/intelligence_suite/variant_selection) zu nutzen.
{% endtab %}

{% tab Canvas %}
Falls noch nicht geschehen, vervollständigen Sie die übrigen Abschnitte Ihrer Canvas-Komponenten. Weitere Details zum Aufbau Ihres Canvas, einschließlich multivariater Tests und [Optimierung mit BrazeAI<sup>TM</sup>]({{site.baseurl}}/user_guide/messaging/canvas/create_a_canvas#optimize-canvas-variants-with-brazeai), finden Sie unter [Canvas erstellen]({{site.baseurl}}/user_guide/messaging/canvas/create_a_canvas#step-2-build-your-canvas).
{% endtab %}
{% endtabs %}

## Schritt 5: Überprüfen und starten {#step-5-review-and-deploy}

Der letzte Abschnitt fasst die von Ihnen erstellte Campaign zusammen. Bestätigen Sie alle relevanten Details und wählen Sie **Campaign starten**.

Informationen zum Zugriff auf die Ergebnisse Ihrer E-Mail-Campaigns finden Sie unter [E-Mail-Reporting]({{site.baseurl}}/user_guide/channels/email/reporting).