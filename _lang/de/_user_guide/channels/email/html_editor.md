...
article_title: Eine E-Mail mit benutzerdefiniertem HTML erstellen
page_order: 2
...
# Eine E-Mail mit benutzerdefiniertem HTML erstellen
...
Verwenden Sie Kampagnen für einfache, einzelne Nachrichten. Verwenden Sie Canvases für mehrstufige Nutzer-Journeys.

{% tabs %}
{% tab Campaign %}

1. Gehen Sie zu **Messaging** > **Campaigns** und wählen Sie **Create Campaign**.
2. Wählen Sie **Email** oder, für Kampagnen, die mehrere Kanäle ansprechen, **Multichannel**.
...
4. Fügen Sie nach Bedarf [Teams]({{site.baseurl}}/user_guide/administer/global/user_management/teams/) und [Tags]({{site.baseurl}}/user_guide/administer/global/workspace_settings/tags/) hinzu.
   * Tags erleichtern das Auffinden Ihrer Kampagnen und das Erstellen von Berichten. Wenn Sie beispielsweise den [Berichts-Builder]({{site.baseurl}}/user_guide/analytics/reports/report_builder/) verwenden, können Sie nach bestimmten Tags filtern.
5. Fügen Sie so viele Varianten hinzu und benennen Sie sie, wie Sie für Ihre Kampagne benötigen. Weitere Informationen zu diesem Thema finden Sie unter [Multivariate und A/B-Tests]({{site.baseurl}}/user_guide/messaging/ab_testing/).

{% alert tip %}
Wenn alle Nachrichten in Ihrer Kampagne ähnlich sein oder denselben Inhalt haben werden, verfassen Sie Ihre Nachricht, bevor Sie zusätzliche Varianten hinzufügen. Anschließend können Sie im Dropdown **Variante hinzufügen** die Option **Von Variante kopieren** wählen.
{% endalert %}
{% endtab %}
{% tab Canvas %}
...
{% alert tip %}
Wenn Sie benutzerdefiniertes HTML erstellen möchten und Hintergründe in der Gmail-Mobil-App bei aktiviertem Dark Mode des Geräts konsistent bleiben sollen, lesen Sie [Gmail-Mobil-App und Dark-Mode-Hintergrundfarben](#gmail-dark-mode).
{% endalert %}
...
## Schritt 3: Verfassen Sie Ihre E-Mail
...
### Schritt 3.1: Sendeinformationen hinzufügen
...
Hier können Sie eine Vorschau sehen, wie Ihre E-Mail im Postfach erscheint. Mit der ausgewählten Option **Preview as User** können Sie Ihre E-Mail als zufälliger Nutzer anzeigen, einen bestimmten Nutzer auswählen oder einen benutzerdefinierten Nutzer erstellen. So können Sie testen, ob Ihre Connected-Content- und Personalisierungsaufrufe wie erwartet funktionieren.
...
### Schritt 3.3: Auf E-Mail-Fehler prüfen
...
- Liquid-Syntaxfehler
- Content-Blöcke, die ein vollständiges `<!DOCTYPE html>` enthalten
...
{% tabs %}
{% tab Campaign %}
Erstellen Sie als Nächstes den Rest Ihrer Kampagne. In den folgenden Abschnitten finden Sie Details zur Verwendung der Braze-Tools zum Erstellen Ihrer E-Mail-Kampagne.

#### Zustellungszeitplan oder Trigger wählen

Liefern Sie E-Mails basierend auf einem geplanten Zeitpunkt, einer Aktion oder einem API-Trigger. Weitere Informationen finden Sie unter [Ihre Kampagne planen]({{site.baseurl}}/user_guide/messaging/campaigns/schedule_your_campaign/).

{% alert note %}
Bei API-getriggerten Kampagnen, bei denen die Trigger-Aktion auf **Mit Kampagne interagieren** eingestellt ist, führt die Auswahl einer **Empfangen**-Option als Interaktion dazu, dass Ihre neue Kampagne ausgelöst wird, sobald Braze die ausgewählte Kampagne als gesendet markiert, selbst wenn diese Nachricht bounct oder nicht zugestellt werden kann.
{% endalert %}

Sie können auch die Dauer der Kampagne festlegen, [Ruhezeiten]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/delivery_and_entry_types/#quiet-hours) angeben und [Frequency-Capping]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/frequency_capping/#frequency-capping)-Regeln einrichten.

#### Zielnutzer auswählen

Als Nächstes [stellen Sie Ihre Zielgruppe zusammen]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/target_users/), indem Sie Segmente oder Filter auswählen. Braze zeigt eine Live-Vorschau der Segmentpopulation an, einschließlich der Anzahl der per E-Mail erreichbaren Nutzer. Die genaue Segmentzugehörigkeit wird kurz vor dem Versand berechnet.

{% multi_lang_include target_audiences.md %}

Sie können auch wählen, Ihre Kampagne nur an Nutzer mit einem bestimmten [Abo-Status]({{site.baseurl}}/user_guide/channels/email/subscriptions/) zu senden, z. B. an diejenigen, die abonniert und für E-Mail angemeldet sind.

Optional können Sie die Zustellung auch auf eine bestimmte Anzahl von Nutzern innerhalb des Segments begrenzen oder Nutzern erlauben, dieselbe Nachricht bei einer Wiederholung der Kampagne zweimal zu erhalten.
...
Bei Multichannel-Kampagnen, die sowohl E-Mail- als auch Push-Kanäle ansprechen, möchten Sie Ihre Kampagne möglicherweise so einschränken, dass nur Nutzer die Nachricht erhalten, die ausdrücklich angemeldet sind (unter Ausschluss von abonnierten oder abgemeldeten Nutzern). Nehmen wir beispielsweise an, Sie haben drei Nutzer mit unterschiedlichem Opt-in-Status:

- **Nutzer A** ist für E-Mail abonniert und Push-aktiviert. Dieser Nutzer erhält die E-Mail nicht, wird aber den Push erhalten.
- **Nutzer B** ist für E-Mail angemeldet, aber nicht Push-aktiviert. Dieser Nutzer wird die E-Mail erhalten, erhält aber nicht den Push.
- **Nutzer C** ist für E-Mail angemeldet und Push-aktiviert. Dieser Nutzer wird sowohl die E-Mail als auch den Push erhalten.

Wählen Sie dazu unter **Audience Summary** aus, diese Kampagne nur an „nur angemeldete Nutzer" zu senden. Diese Option stellt sicher, dass nur angemeldete Nutzer Ihre E-Mail erhalten, und Braze sendet Ihren Push standardmäßig nur an Nutzer, die Push-aktiviert sind.
...
Braze ermöglicht es Ihnen zu verfolgen, wie oft Nutzer bestimmte Aktionen, [Konversions-Events]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/conversion_events/), nach Erhalt einer Kampagne ausführen. Sie können jede der folgenden Aktionen als Konversions-Event festlegen:
...
## Schritt 5: Überprüfen und bereitstellen

Der letzte Abschnitt fasst die von Ihnen entworfene Kampagne zusammen. Bestätigen Sie alle relevanten Details und wählen Sie **Launch Campaign**.
