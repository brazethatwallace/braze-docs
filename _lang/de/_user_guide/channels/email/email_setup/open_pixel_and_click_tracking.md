...
> [Öffnungspixel-Tracking]({{site.baseurl}}/user_guide/administer/global/workspace_settings/email_preferences#update-the-placement) und Klick-Tracking können für jedes Kundenprofil or Nutzerprofil ein- oder ausgeschaltet werden. Diese Flexibilität hilft Ihnen, regionale Datenschutzgesetze einzuhalten, wenn ein einzelnes Kundenprofil or Nutzerprofil angibt, dass es nicht mehr getrackt werden möchte.

## Aktivieren des Öffnungspixels oder Klick-Trackings {#turning-on-open-pixel-or-click-tracking}

Wenn Sie ein Kundenprofil or Nutzerprofil über die [API]({{site.baseurl}}/api/objects_filters/user_attributes_object#braze-user-profile-fields), per [CSV]({{site.baseurl}}/user_guide/audience/manage_audience/import_users#constructing-your-csv) oder über [Cloud Data Ingestion (CDI)]({{site.baseurl}}/user_guide/data/unification/cloud_ingestion) importieren oder Update or aktualisieren or aktualisieren, stehen Ihnen zwei Felder zur Bearbeitung zur Verfügung:
...
Diese Informationen werden zur Referenz im Kundenprofil or Nutzerprofil unter den E-Mail-**Kontakteinstellungen** im Tab **Engagement** angezeigt.
...
Wenn Ihr E-Mail-Anbieter (E-Mail-Anbieter or ESP) einen Link für das Klick-Tracking umschreibt, verwendet die resultierende URL Ihre Klick-Tracking-Domain und ein E-Mail-Anbieter or ESP-spezifisches Pfadpräfix. Die Muster, die jeder E-Mail-Anbieter or ESP generiert und die Sie für Firewall-Regeln und Sicherheits-Allowlists benötigen, finden Sie unter [Klick- und Öffnungs-Tracking-URL-Muster]({{site.baseurl}}/user_guide/channels/email/email_setup/ssl#click-and-open-tracking-url-patterns).