...
Das Startdatum muss morgen oder später in der Zeitzone Ihres Workspace liegen (oder in der Unternehmens-Zeitzone, wenn für den Workspace keine Überschreibung festgelegt ist).

Braze erstellt Campaigns um Mitternacht in dieser Zeitzone für den aktuellen Tag und den nächsten Tag (0 bis 1 Tage vor dem Versand). Beim Starten eines Plans werden bevorstehende Campaigns ebenfalls sofort erstellt.

## Wie viele Templates werden benötigt? {#how-many-templates-are-required}

Braze berechnet das Minimum aus Ihren geplanten Sendevolumen und den per E-Mail erreichbaren Nutzer:innen in den ausgewählten Segmenten (nicht der gesamten Segmentgröße). Stellen Sie mehr Templates als das Minimum bereit, damit das System bei Zustellbarkeitsproblemen Anpassungen vornehmen kann, ohne den Versand zu stoppen. Weitere Informationen finden Sie unter [Schritt 3: Zu sendende Nachrichten auswählen]({{site.baseurl}}/user_guide/channels/email/email_setup/ip_warming/automated_ip_warming#step-3-select-the-messages-to-send).
...
## Können Nutzer:innen am selben Tag mehr als ein E-Mail-Template erhalten? {#can-a-user-receive-more-than-one-email-template-on-the-same-day}

Ja. Braze schließt Nutzer:innen aus, die innerhalb des Plans bereits ein bestimmtes Template erhalten haben, aber Nutzer:innen, die ein anderes Template erhalten haben, bleiben berechtigt. Wenn die verfügbare Zielgruppe für den Zeitplan eines Tages aufgebraucht ist, durchläuft der Plan Ihre Templates erneut und einige Nutzer:innen erhalten an diesem Tag ein zweites Template.

Dies passiert, wenn die Gesamtzahl der per E-Mail erreichbaren Nutzer:innen in Ihren ausgewählten Segmenten kleiner ist als Ihr **Zielversandvolumen** – am häufigsten am letzten Tag oder an den letzten Tagen des Plans. Beispiel: Bei 400.000 per E-Mail erreichbaren Nutzer:innen und einem Zielversandvolumen von 600.000 erhalten am letzten Tag etwa 200.000 Nutzer:innen zwei Templates. Um dies zu vermeiden, halten Sie die Gesamtzahl der per E-Mail erreichbaren Nutzer:innen größer oder gleich Ihrem Zielversandvolumen. Weitere Informationen finden Sie unter [Zielgruppengröße und mehrere Sends pro Nutzer:in]({{site.baseurl}}/user_guide/channels/email/email_setup/ip_warming/automated_ip_warming#audience-size-and-multiple-sends-per-user).

## Kann ich das IP-Warming mitten im Zeitplan starten? {#can-i-start-ip-warming-mid-schedule}

Automatisiertes IP-Warming erstellt den Zeitplan immer ab dem Beginn der Aufwärmphase. Um einen Start mitten im Zeitplan zu simulieren, setzen Sie das **Aktuelles tägliches Sendevolumen** auf einen Wert größer als 0, der Ihrem aktuellen Volumen entspricht. Wenn das aktuelle Volumen größer als 0 ist, wendet Braze am ersten Tag keine IP-Anzahl-Skalierung an.

## Welche Zeitzone wird für den Versand verwendet? {#what-time-zone-is-used-for-sending}

Für den Versand wird die Workspace-Zeitzone verwendet, sofern eine festgelegt ist; andernfalls wird die Unternehmens-Zeitzone verwendet. Campaigns werden nicht in der jeweiligen Ortszeit der Nutzer:innen erstellt. Um in Ortszeit zu versenden, aktualisieren Sie die vom Plan erstellten Campaigns manuell.
...
Wenn das **aktuelle tägliche Sendevolumen** 0 beträgt, beginnt Tag 1 mit dem niedrigeren Wert von 50 Sendungen pro IP oder 500 insgesamt. Das Volumen wächst dann um etwa das 1,75-Fache pro Sendetag, vorbehaltlich der Ramp-Guardrails. Beispiel mit 10 IPs: 500 → 875 → 1.532 → 2.681.
...
Ja. Das Stoppen beendet den Plan dauerhaft: Braze deaktiviert verknüpfte Campaigns und erstellt keine zukünftigen mehr. Sie können einen gestoppten Plan nicht fortsetzen – erstellen Sie einen neuen Plan, um fortzufahren. Für Schritte zum Fortfahren nach einem Stopp siehe [Einen IP-Warming-Plan stoppen]({{site.baseurl}}/user_guide/channels/email/email_setup/ip_warming/automated_ip_warming#stop-an-ip-warmup-plan).

## Wann wird ein IP-Warming-Plan als abgeschlossen markiert? {#when-is-an-ip-warming-plan-marked-as-complete}

Der Plan wird als abgeschlossen markiert, nachdem der letzte geplante Versandtag endet – um Mitternacht in der geltenden Zeitzone (Workspace oder Unternehmen). Wenn zum Beispiel die letzte Campaign um 20 Uhr versendet wird, wird der Plan vier Stunden später um Mitternacht als abgeschlossen markiert.

## Welche Daten kann ich herunterladen? {#what-data-can-i-download}

Der CSV-Export enthält Zeilen pro Campaign mit täglichen Metriken: *Gesendet*, *Zugestellt*, *Bounces*, *Spam-Berichte*, *Gesamtöffnungen*, *Eindeutige Öffnungen*, *Geklickt* und *Abgemeldet*. Die Tracker-Tabelle fasst mehrere Campaigns desselben Tages in einer Tagesansicht zusammen. Weitere Informationen finden Sie unter [Wenn ein IP-Warming abgeschlossen ist]({{site.baseurl}}/user_guide/channels/email/email_setup/ip_warming/automated_ip_warming#when-an-ip-warming-completes).