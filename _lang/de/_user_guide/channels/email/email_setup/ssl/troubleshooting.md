...
6. Koordinieren Sie sich bei anhaltenden Problemen mit Ihrem CDN- oder IT-Team und kontaktieren Sie den [Braze-Support]({{site.baseurl}}/braze_support) mit Fehlercodes und allen Details von Ihrem CDN- oder Domain-Anbieter.
...
1. **Phase 1 (Ersteinrichtung):** Ihr Klick-Tracking-Domain-CNAME verweist direkt auf Ihren ESP-Endpunkt für die unverschlüsselte HTTP-Verifizierung.
2. **Phase 2 (SSL-Bereitstellung):** Sie aktualisieren den CNAME auf Ihren CDN- oder WAF-Edge, der Ihr benutzerdefiniertes SSL-Zertifikat hält und Anfragen mit den erforderlichen Headern an den ESP weiterleitet. Der ESP protokolliert den Klick und leitet die Empfänger:innen zum endgültigen Ziel weiter.

{% alert important %}
Braze aktiviert SSL-Klick-Tracking erst, nachdem die Verifizierung von Phase 1 abgeschlossen ist. Wenn SSL aktiviert ist, Ihr DNS aber noch auf den ESP verweist (Phase 1), können Empfänger:innen [SSL-Namenskonfliktfehler](#ssl-name-mismatch-errors) sehen.
{% endalert %}

## ESP Phase 1 und Phase 2 Routing {#esp-phase-1-and-phase-2-routing}

Prüfen Sie bei der Fehlerbehebung von Link-Tracking-Fehlern, ob Ihr DNS-Eintrag auf das unverschlüsselte ESP-Netzwerk (Phase 1) oder Ihr CDN (Phase 2) verweist.

| ESP | Phase 1 CNAME-Ziel (direkt zum ESP) | Phase 2 CNAME-Ziel | Erforderliche CDN-Konfiguration |
| --- | --- | --- | --- |
...
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 .reset-td-br-4 aria-label="ESP Phase 1 und Phase 2 Routing" }
...
### DNS verweist noch auf den ESP (Phase 1) {#dns-still-points-to-the-esp-phase-1}

Wenn Sie Braze anweisen, SSL-Klick-Tracking zu aktivieren, Ihren DNS-CNAME aber weiterhin direkt auf den ESP verweisen lassen (zum Beispiel auf SendGrids `sendgrid.net`), öffnet der Browser der Empfänger:innen Ihre Klick-Tracking-Domain und erreicht die Infrastruktur des ESP. Der ESP hat keinen Eintrag für Ihr benutzerdefiniertes Zertifikat und liefert sein eigenes Fallback-Zertifikat aus (zum Beispiel `*.sendgrid.net`). Die Namensabweichung lässt die Verbindung fehlschlagen und gibt eine Warnung zu einer privaten Verbindung zurück.
...
| --- | --- | --- |
| Wird zu einem ESP-Endpunkt aufgelöst (`sendgrid.net`, `spgo.io` oder `awstrack.me`) | DNS befindet sich noch in Phase 1 | Aktualisieren Sie Ihre Domain-Registrierung, um den Traffic über Ihr CDN zu leiten. Siehe [ESP Phase 1 und Phase 2 Routing](#esp-phase-1-and-phase-2-routing). |
...
- **Host-Header:** Amazon SES erfordert, dass Ihr CDN den korrekten Host-Header weiterleitet. Aktivieren Sie den `X-Forwarded-Host`-Header auf Ihrer Klick-Tracking-Domain. Informationen zu den Routing-Anforderungen für Phase 1 und Phase 2 finden Sie unter [ESP Phase 1 und Phase 2 Routing](#esp-phase-1-and-phase-2-routing).
