...
## Schritt 1: Neue Campaign erstellen {#step-1-create-a-new-campaign}
...
![Dropdown „Campaign erstellen“ mit hervorgehobener Option für Transaktions-E-Mails.]({% image_buster /assets/img/transactional_email_campaign.png %}){: width="534" height="800" style="float:right;max-width:35%;margin-left:15px;height:auto;"}

Nun können Sie mit der Konfiguration Ihrer Transaktions-E-Mail-Campaign fortfahren.

## Schritt 2: Ihre Campaign konfigurieren {#step-2-configure-your-campaign}

Der Erstellungsablauf für Transaktions-E-Mail-Campaigns ist im Vergleich zu dem einer [Standard-E-Mail-Campaign]({{site.baseurl}}/user_guide/channels/email/html_editor) vereinfacht, damit Ihre geschäftskritischen Transaktions-E-Mails alle Nutzer:innen erreichen können.

Daher werden Sie feststellen, dass einige Einstellungen, die Sie möglicherweise von anderen Braze-Campaign-Typen kennen, bei der Einrichtung dieses Campaign-Typs nicht erforderlich sind:

- Der Schritt **Zustellung** wurde vereinfacht und die Planungsoptionen wurden entfernt. Transaktions-E-Mails werden immer über die Braze REST API mit der auf der Seite **Zustellung** angezeigten Campaign-ID getriggert. Zusätzliche Einstellungen wie die Steuerung der erneuten Qualifizierung und Frequency-Capping-Einstellungen wurden ebenfalls entfernt, um sicherzustellen, dass alle Nutzer:innen für diese kritischen Transaktionsbenachrichtigungen erreichbar sind, wenn Ihr Dienst eine Sendeanfrage triggert.
- Der Schritt **Target Audiences** wurde entfernt. Da Transaktions-E-Mails Ihre gesamte Nutzerbasis als berechtigt einstufen (einschließlich abgemeldeter Nutzer:innen), müssen keine Filter oder Segments angegeben werden. Wenn Sie daher eine Logik anwenden möchten, wer diese Nachricht erhalten soll, empfehlen wir, diese Logik anzuwenden, bevor Sie entscheiden, ob die API-Anfrage an Braze gestellt werden soll, um die Nachricht an eine:n bestimmte:n Nutzer:in zu triggern.
- Der Schritt **Konversionen** wurde entfernt. Transaktions-E-Mails unterstützen derzeit kein Konversions-Event-Tracking.

![Erstellen-, Zustellungs- und Bestätigungs-Workflow zur Erstellung einer Transaktions-E-Mail-Campaign.]({% image_buster /assets/img/transactional_campaign_compose.png %}){: width="1586" height="1112" style="max-width:80%;height:auto;"}

Gehen Sie wie folgt vor, um Ihre Transaktions-E-Mail-Campaign zu konfigurieren:

1. Fügen Sie einen beschreibenden Namen hinzu, damit Sie die Ergebnisse auf Ihrer **Campaigns**-Seite finden können, nachdem Sie Ihre Nachrichten gesendet haben.
2. Verfassen Sie Ihre E-Mail oder wählen Sie ein Template aus.
3. Notieren Sie sich Ihre `campaign_id`. Nachdem Sie Ihre API-Campaign gespeichert haben, müssen Sie die generierte `campaign_id` in Ihre API-Anfrage einfügen, wie im Artikel zum [Transaktions-E-Mail-Endpunkt]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_transactional_message) angegeben.
4. Klicken Sie auf **Save Campaign**, und Sie können Ihre API-Campaign starten!

{% alert note %}
Die Ein-Klick-Listenabmeldungseinstellung für Transaktions-E-Mail-Campaigns ist standardmäßig auf **Use workspace default** eingestellt, ähnlich wie bei anderen E-Mail-Campaigns. Da dies für Transaktions-Messaging vorgesehen ist, fügt Braze keine Ein-Klick-Abmeldung hinzu. Um eine Ein-Klick-Abmeldung zu diesem Campaign-Typ hinzuzufügen, [bearbeiten Sie diese Einstellung]({{site.baseurl}}/user_guide/administer/global/workspace_settings/email_preferences#message-level-one-click-list-unsubscribe) unter **Sending Info**.
{% endalert %}

### Unzulässige Tags in Transaktions-E-Mails {#disallowed-tags-in-transactional-emails}

Die Liquid-Tags `Connected Content` und `Promotion Code` sind innerhalb von Transaktions-E-Mail-Campaigns nicht verfügbar.

Die Verwendung des `Connected Content`-Tags erfordert, dass Braze während des Sendevorgangs eine ausgehende API-Anfrage stellt, was den Nachrichtensendevorgang verlangsamen kann, wenn der externe Dienst, den wir anfragen, Latenzprobleme hat. Ebenso erfordert der `Promotion Code`-Tag, dass Braze eine zusätzliche Verarbeitung durchführt, um die Verfügbarkeit eines Aktionscodes vor dem Senden zu prüfen, was den Sendevorgang verlangsamen kann, sollte keiner verfügbar sein.

Aus diesem Grund unterstützen wir die Verwendung von `Connected Content`- oder `Promotion Code`-Tags in keinem Feld Ihrer Transaktions-E-Mail-Campaign.