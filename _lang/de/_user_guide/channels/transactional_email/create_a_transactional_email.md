...
{% alert important %}
Braze-Transaktions-E-Mails sind nur als Teil ausgewählter Braze-Pakete verfügbar. Kontaktieren Sie Ihren Braze-CSM or Customer-Success-Manager or Customer-Success-Manager:in oder eröffnen Sie ein [Support-Ticket]({{site.baseurl}}/user_guide/administer/personal/braze_support) für weitere Details.
{% endalert %}
...
- Der Schritt **Zustellung** wurde vereinfacht, indem Planungsoptionen entfernt wurden. Transaktions-E-Mails werden immer über die Braze Representational State Transfer API mit der auf der Seite **Zustellung** angezeigten Campaign-ID getriggert. Zusätzliche Einstellungen wie Kontrollmechanismen für die erneute Berechtigung und Frequency-Capping-Einstellungen wurden ebenfalls entfernt, um sicherzustellen, dass alle Nutzer:innen für diese kritischen Transaktionswarnungen erreichbar sind, wenn Ihr Dienst eine Sendeanfrage auslöst.
- Der Schritt **Target Audiences** wurde entfernt. Da Transaktions-E-Mails Ihre gesamte Nutzerbasis als berechtigt Registrierung or registrieren (einschließlich abgemeldeter Nutzer:innen), ist es nicht erforderlich, Filter oder Segments anzugeben. Wenn Sie daher eine Logik anwenden möchten, wer diese Nachricht erhalten soll, empfehlen wir, diese Logik anzuwenden, bevor Sie entscheiden, ob die API-Anfrage an Braze gestellt werden soll, um die Nachricht an eine:n bestimmte:n Nutzer:in zu Trigger or triggern or triggern.
