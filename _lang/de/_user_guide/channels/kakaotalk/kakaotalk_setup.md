...
| Kontakt-Telefonnummer | Eine Kontakt-Telefonnummer für die Verwaltung Ihres KakaoTalk-Kanals. |
| Braze-Cluster-IPs auf der Allowlist | Die Registrierung der IP-Allowlist ist für alle Kund:innen erforderlich. Registrieren Sie die Braze-IP-Adressen für Ihren Cluster, bevor Sie KakaoTalk in Braze integrieren. |
...
1. Gehen Sie in Ihrem Comm.One-Dashboard zu **Account Management (계정 관리)**, wählen Sie das Menü-Symbol und dann **View Details (자세히보기)** aus.
...
![Comm.One-Dashboard mit der Ansicht, in der Sie IP-Adressen hinzufügen können.]({% image_buster /assets/img/kakaotalk/register_braze_ip.png %})
...
- Ausweis der geschäftsführenden Vertretung
...
Nachdem Sie Ihre Dokumente zusammengestellt haben, führen Sie die folgenden Schritte aus:

1. Melden Sie sich beim [KakaoTalk Channel Admin Center](https://center-pf.kakao.com/) an.
...
2. Sammeln Sie die erforderlichen Zugangsdaten für Ihren Anbieter (siehe den folgenden Abschnitt), geben Sie diese auf der **Technologie-Partnerseite** ein und speichern Sie sie.
...
| **Credential (ID) & Password (비밀번호)** | Gehen Sie zum gleichen Ort wie für die **Sender number (사업자 등록번호)** und dann zu **API** > **Brand Message (브랜드 메시지)**. |
{: .reset-td-br-1 .reset-td-br-2 aria-label="CJ OliveNetworks" }

{% tabs local %}
{% tab Comm.One Login ID (로그인 아이디) %}

![Comm.One-Dashboard mit einer zensierten Anmelde-ID.]({% image_buster /assets/img/kakaotalk/comm.one_login_id.png %})
...
![Comm.One-Dashboard mit einer zensierten Zugangsdaten-ID und einem zensierten Passwort.]({% image_buster /assets/img/kakaotalk/id_and_password.png %})

{% endtab %}
{% endtabs %}

{% alert note %}
Es können nur die Kanäle registriert werden, die einer einzigen gemeinsamen ID zugeordnet sind.
{% endalert %}
...
Gehen Sie zu Ihrem Infobip-Dashboard und dem [KakaoTalk Channel Admin Center](https://center-pf.kakao.com/), um die folgenden Informationen zu sammeln.
...
4. Geben Sie den **Name** ein, wählen Sie das **Expiration date** und dann die für KakaoTalk erforderlichen API-Scopes. Diese Scopes steuern, welche Infobip-API-Aktionen Ihr Schlüssel ausführen kann.
...
1. Wählen Sie im [KakaoTalk Channel Admin Center](https://center-pf.kakao.com/) die Option **Channels**.
...
![KakaoTalk-Kanalinformationsfenster mit den Feldern für Kundenservice-Kontaktinformationen.]({% image_buster /assets/img/kakaotalk/kakao_customer_center_contact.png %})

{: start="4"}
4. Um einen anderen Kanal anzuzeigen, wählen Sie das Kanalsymbol oben im Menü.
5. Wählen Sie in der Liste **My channel** den Kanal aus, den Sie anzeigen möchten, und wiederholen Sie die vorherigen Schritte.

## Nutzerprofile einrichten {#set-user-profiles}

Nutzerprofile müssen Telefonnummern im E.164-Format enthalten, um Nachrichten über KakaoTalk zu senden. Telefonnummern werden im Nutzerprofil angezeigt. KakaoTalk erfordert Telefonnummern im E.164-Format (z. B. `+821025749774`). Dies unterscheidet sich von einigen anderen Messaging-Kanälen, die Telefonnummern in verschiedenen Formaten akzeptieren können.
