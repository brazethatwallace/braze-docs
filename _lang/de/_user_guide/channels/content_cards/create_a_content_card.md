...
{% alert tip %}
Wenn alle Nachrichten in Ihrer Campaign ähnlich sind oder denselben Inhalt haben, verfassen Sie Ihre Nachricht, bevor Sie weitere Varianten hinzufügen. Anschließend können Sie **Von Variante kopieren** aus dem **Variante hinzufügen**-Dropdown auswählen.
{% endalert %}
...
![Ein Fenster, in dem Englisch, Spanisch und Französisch als Sprachen ausgewählt sind, sowie Titel, Beschreibung und Linktext als zu internationalisierende Felder.]({% image_buster /assets/img/add_languages.png %}){: style="max-width:70%;"}
...
Schreiben Sie, was Sie möchten. Es gibt keine Beschränkungen, aber je schneller Sie Ihre Botschaft vermitteln und Ihre Kund:innen zum Klicken bringen, desto besser! Wir empfehlen klare und prägnante Titel und Nachrichteninhalte. Beachten Sie, dass diese Felder für reine Bildkarten nicht verfügbar sind.
...
Braze zeigt eine gepinnte Karte oben im Feed der Nutzer:innen an, und die Nutzer:innen können sie nicht verwerfen. Wenn der Feed einer Nutzer:in mehrere gepinnte Karten enthält, ordnet Braze diese chronologisch an. Wenn Braze eine Content-Card ausliefert, ist sie entweder gepinnt oder nicht gepinnt, und dieser Status ändert sich für die gesamte Lifetime der Karte nicht. Wenn Sie die Pinning-Einstellung einer Campaign ändern, gilt die Aktualisierung nur für Karten, die nach der Änderung gesendet werden. Sie ändert nicht den Pinning-Status von Karten, die sich bereits im Feed der Nutzer:innen befinden.

![Nebeneinander-Ansicht der Content-Card-Vorschau in Braze für Mobilgeräte und Web mit der ausgewählten Option „Diese Karte oben im Feed anpinnen“.]({% image_buster /assets/img/cc_pin_to_top.png %}){:style="border:none"}
...
Als Nächstes [stellen Sie Ihre Zielgruppe zusammen]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/target_users), indem Sie Segments oder Filter auswählen, um Ihre Zielgruppe einzugrenzen. Sie erhalten automatisch eine Vorschau der ungefähren Segment-Population. Beachten Sie, dass die genaue Segment-Zugehörigkeit immer vor dem Senden der Nachricht berechnet wird.
...
Zur Unterstützung der Performance haben Content Cards zwei wesentliche Einschränkungen: eine Begrenzung der Payload-Größe für jede einzelne Karte und eine maximale Anzahl von Karten, die in einem Feed angezeigt werden können.
...
* Verwenden Sie URL-Shortener für lange Links. URLs, insbesondere solche mit umfangreichen Tracking-Parametern, können zu Problemen mit dem Größenlimit führen. Die Verwendung eines URL-Shortening-Dienstes kann die Zeichenanzahl drastisch reduzieren und Platz in der Payload freigeben.
...
- **Verwenden Sie kürzere Ablaufdaten:** Legen Sie für zeitkritische Campaigns (z. B. einen Wochenendverkauf) ein bestimmtes Ablaufdatum fest. Auf diese Weise werden Karten automatisch aus dem Feed entfernt und zählen nicht mehr zum Limit, nachdem sie nicht mehr relevant sind.
...
Die erneute Berechtigung bestimmt, ob und wann Nutzer:innen eine Nachricht von derselben Campaign mehr als einmal erhalten können. Für Content Cards ist das Verständnis dieser Funktionsweise entscheidend für die Verwaltung wiederkehrender Campaigns und um sicherzustellen, dass Nutzer:innen keine doppelten oder veralteten Nachrichten erhalten.
...
#### Aktualisierung bereits gesendeter Karten {#updating-launched-cards}
...
Angenommen, Sie haben eine Campaign eingerichtet, die durch einen Sitzungsstart getriggert wird, und die erneute Berechtigung ist auf 30 Tage eingestellt. Ein:e Nutzer:in hat die Campaign vor zwei Tagen erhalten, und Sie möchten den Text ändern. Archivieren Sie zunächst die Campaign und entfernen Sie die Karten aus dem Feed. Duplizieren Sie dann die Campaign und starten Sie sie mit dem neuen Text erneut. Wenn der:die Nutzer:in eine weitere Sitzung hat, erhält er:sie sofort die neue Karte.
...
Dieser Ansatz hält alle Ihre Analytics in einer einzigen Campaign vereinheitlicht. Neu berechtigte Nutzer:innen erhalten die neue Karte, aber die Aktualisierung wird für bestehende Empfänger:innen verzögert, bis sie erneut berechtigt sind:
...
Angenommen, Sie haben eine Campaign, die durch einen Sitzungsstart getriggert wird und die erneute Berechtigung auf 30 Tage eingestellt hat. Ein:e Nutzer:in hat die Campaign vor zwei Tagen erhalten, und Sie möchten den Text ändern. Stoppen Sie zunächst die Campaign und entfernen Sie die Karte aus dem Feed. Veröffentlichen Sie dann die Campaign mit dem neuen Text erneut. Wenn der:die Nutzer:in eine weitere Sitzung hat, erhält er:sie die neue Karte in 28 Tagen.

{% alert note %}
Wenn Sie eine Campaign stoppen, die Entfernungs-Event-Einstellungen bearbeiten und die Campaign erneut starten, ohne die Karten aus dem Feed zu entfernen, verwenden alle bestehenden Karten in den Feeds der Nutzer:innen die aktualisierten Entfernungs-Event-Einstellungen. Die Karten behalten nicht die ursprüngliche Entfernungs-Event-Konfiguration von dem Zeitpunkt bei, als sie erstmals gesendet wurden.
{% endalert %}
...
Bei Campaigns mit aktionsbasierter Zustellung (z. B. Sitzungsstart) gibt es eine erwartete kurze Verzögerung zwischen dem Trigger-Event und dem Zeitpunkt, an dem die Karte verfügbar wird. Diese Verzögerung tritt auf, weil:
...
Dieser Fehler erscheint, wenn eine Content-Card-Kampagne mit mehreren Varianten unterschiedliche Ablaufeinstellungen für die einzelnen Varianten verwendet. Legen Sie für jede Variante dieselbe Ablaufzeit (Dauer oder bestimmter Zeitpunkt) fest, oder reduzieren Sie die Campaign auf eine einzelne Variante, und speichern Sie dann erneut. Informationen zum Festlegen der Ablaufzeit beim Erstellen einer Campaign finden Sie unter [Zustellungszeitplan oder Trigger auswählen](#choose-a-delivery-schedule-or-trigger).