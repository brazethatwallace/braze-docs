...
![LINE-Messaging-Integrationsseite mit dem Abschnitt zur LINE-Integration.]({% image_buster /assets/img/line/integration.png %}){: style="max-width:80%;"}
...
3. Wenn sich der Abo-Status ändert (z. B. wenn die Nutzer:in den Kanal blockiert, die Freundschaft aufhebt oder dem Kanal erneut folgt), erhält Braze die Aktualisierung von LINE und aktualisiert das Nutzerprofil mit der `native_line_id` entsprechend.
...
3. Wenn die Nutzer:in dem Kanal später folgt, erhält Braze die Aktualisierung von LINE und aktualisiert das Nutzerprofil mit dem Abo-Status `subscribed`.
...
2. Braze erstellt ein anonymes Nutzerprofil, bei dem das `native_line_id`-Attribut auf die LINE-ID der Follower:in und ein Nutzer-Alias `line_id` auf die LINE-ID der Follower:in gesetzt wird. Das Profil hat den Abo-Status `subscribed`.
...
5. Sie aktualisieren Profil 1, um das `native_line_id`-Attribut zu setzen. Dieses Profil übernimmt den Abo-Status von Profil 2.
  - Jetzt gibt es zwei Nutzerprofile mit derselben `native_line_id`. Diese können jederzeit über den `/users/merge`-Endpunkt im in [Schritt 5](#step-5-merge-profiles-optional) beschriebenen Prozess zusammengeführt werden.
...
**Szenario:** Eine anonyme Nutzer:in wurde während der initialen Abonnent:innen-Synchronisierung oder nach der Integration durch ein „Folgen“-Ereignis erstellt.
...
4. Wenn die Nutzer:in Ihrem offiziellen LINE-Konto folgt, empfängt Braze ein Folgen-Ereignis und aktualisiert den Abo-Status der Nutzer:in auf `subscribed`.

### Kontoverknüpfung {#user-account-linking}

Diese Methode ermöglicht es Nutzer:innen, ihr LINE-Konto mit dem Nutzerkonto Ihrer App zu verknüpfen. Sie können dann Liquid in Braze verwenden, wie z. B. {% raw %}`{{line_id}}`{% endraw %}, um eine personalisierte URL für die Nutzer:in zu erstellen, die die LINE-ID der Nutzer:in an Ihre Website oder App zurückgibt, wo sie dann einer bekannten Nutzer:in zugeordnet werden kann.
...
2. Erstellen Sie eine Nachricht, die Nutzer:innen dazu motiviert, sich auf Ihrer Website oder in Ihrer App anzumelden, wobei die LINE-ID der Nutzer:in als Abfrageparameter (über Liquid) übergeben wird, zum Beispiel:
...
4. (Optional) Erstellen Sie eine aktionsbasierte Campaign oder einen Canvas, der ausgelöst wird, wenn die LINE-Nutzer:in identifiziert wird, um der Nutzer:in ihren Gutscheincode zu senden. <br>![Aktionsbasierte Campaign, die ausgelöst wird, wenn die LINE-Nutzer:in identifiziert wird.]({% image_buster /assets/img/line/account_link_2.png %})

#### So funktioniert es {#how-it-works}

Nachdem sich die Nutzer:in angemeldet hat, wird auf Ihrer Website oder in Ihrer App eine Änderung vorgenommen, sodass die Nutzer-ID an Braze zurückgesendet wird, um sie mit der LINE-ID zu verknüpfen, die als Teil der URL übergeben wurde. Beispielcode:
...
3. Die Nutzer:in erhält eine LINE-Nachricht mit einem Link zu Ihrer Website und App und meldet sich an. Ihr Nutzerprofil ist jetzt bekannt.
