...
In einer App kann das Tracking für bis zu 25 Segments aktiviert werden. Braze empfiehlt das Tracking von Segments, die für Sie wichtig sind, um die Auswirkungen Ihrer Campaigns auf Sitzungen, Umsätze und Käufe zu analysieren.
...
Um die Daten der Segments für einen beliebigen angepassten Zeitraum visuell zu vergleichen, fügen Sie dem Diagramm Segments hinzu oder entfernen Sie sie. Wählen Sie in der Dropdown-Liste **Breakdown** die Option **By Segment** und wählen Sie dann Ihre Segments unter **Breakdown values** aus.
...
![Umsätze für mehrere Segments]({% image_buster /assets/img_archive/segment_revenue_multiple.png %})
...
Sehen Sie sich Daten über [angepasste Events im Zeitverlauf für Segments]({{site.baseurl}}/user_guide/data/activation/events/custom_events/#analytics) an, indem Sie zu **Analytics** > **Bericht zu angepassten Events** gehen.

## Verwendung von Abfrage-Builder-Templates {#using-query-builder-templates}

Wenn das Analytics Tracking aktiviert ist, können Sie mit den Berichts-Templates des Abfrage-Builders die Performance-Metriken für Campaigns, Canvas, Varianten und Schritte nach Segments aufschlüsseln. Mehr erfahren Sie unter [Segment-Daten]({{site.baseurl}}/user_guide/audience/segments/segment_data/#performance-data-by-segment).