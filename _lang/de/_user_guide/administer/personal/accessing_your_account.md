...
Wenn Ihr Braze-Dashboard einen unerwarteten Fehler anzeigt und das Konsolentool Ihres Browsers den Fehler `ReferenceError: structuredClone is not defined` ausgibt, ist Ihr Browser veraltet. Falls dieser Fehler weiterhin auftritt, deinstallieren Sie Ihren Browser und installieren Sie ihn erneut.
...
Wenn Sie innerhalb des Dashboards zwischen Konten wechseln, müssen Sie 2FA nur einmal abschließen – beim ersten Mal, wenn Sie sich während dieser Sitzung bei einem verknüpften Konto anmelden.
...
Wenn das Löschen des Browser-Caches und der Cookies Ihre Probleme nicht behebt, wenden Sie sich an den [Support]({{site.baseurl}}/support_contact).

### „Aw, Snap!“-Fehler in Google Chrome {#aw-snap-error-in-google-chrome}

Wenn Google Chrome den Fehler „Aw, Snap!“ anzeigt, hat Chrome Schwierigkeiten, die Braze-Dashboard-Seite zu laden. Informationen zur Fehlerbehebung finden Sie unter [Hilfe bei häufigen Fehlermeldungen in Chrome](https://support.google.com/chrome/answer/95669?co=GENIE.Platform%3DDesktop&hl=en).
...
2. Prüfen Sie, ob die/der Nutzer:in einem Workspace hinzugefügt wurde.
3. Falls sie/er keinem Workspace zugeordnet ist, fügen Sie sie/ihn hinzu und weisen Sie die entsprechenden Berechtigungen zu.
4. Bitten Sie die/den Nutzer:in, das Dashboard zu aktualisieren.
...
Wenn Sie Probleme bei der Anmeldung bei den Braze-Lernangeboten haben und in einer Schleife feststecken, die Sie zum Dashboard weiterleitet, führen Sie die folgenden Schritte aus:

1. Wenn Sie mehrere Braze-Konten haben, werden Sie nach zweimaliger Anmeldung mit dem falschen Konto zum Braze-Dashboard weitergeleitet. Bestätigen Sie, dass Sie sich beim richtigen Konto anmelden.
2. Wenn Sie einen Werbeblocker verwenden, stellen Sie sicher, dass dieser deaktiviert ist. Er kann Cookies blockieren, die für die Single-Sign-on-Funktionalität erforderlich sind.
...
Wenn ein:e Nutzer:in Probleme mit der Zwei-Faktor-Authentifizierung (2FA) hat und nicht auf das Braze-Dashboard zugreifen kann, kann dies verschiedene Ursachen haben. Am häufigsten hat die Person keinen Zugriff mehr auf die registrierte Telefonnummer oder das Gerät, auf dem die Authy-App installiert ist.

Ein:e Admin sollte die 2FA für die/den betroffene:n Nutzer:in wie folgt zurücksetzen:

1. Gehen Sie zu **Nutzer:innen verwalten**.
2. Wählen Sie **Nutzer:in bearbeiten** für die/den Nutzer:in mit 2FA-Problemen.
...
Braze kann die 2FA aus Sicherheitsgründen nicht im Auftrag von Nutzer:innen zurücksetzen. Wenn die/der Admin die 2FA nicht zurücksetzen kann, erstellen Sie ein Support-Ticket.

#### Hinweise {#considerations}

- Wenn 2FA auf Unternehmensebene erzwungen wird: Nach dem Zurücksetzen fordert Braze die/den Nutzer:in auf, die 2FA bei der nächsten Anmeldung erneut einzurichten.
- Wenn 2FA nicht auf Unternehmensebene erzwungen wird: Die/der Nutzer:in meldet sich am Dashboard an, ohne die 2FA erneut einrichten zu müssen. Wenn sie/er die 2FA aktivieren möchte, kann dies in den Kontoeinstellungen erfolgen.
...
- Überprüfen Sie, ob Sie sich bei der richtigen [Braze-Dashboard-Instanz]({{site.baseurl}}/user_guide/administer/personal/sdk_endpoints) anmelden. Wenden Sie sich zur Bestätigung an Ihre:n Kontoadministrator:in oder Ihren Braze Account Manager.
- Ihr Passwort ist möglicherweise abgelaufen, sodass Sie es [zurücksetzen](#resetting-your-password) müssen.
- Wenn Sie einen [Single-Sign-on]({{site.baseurl}}/user_guide/administer/global/saml_single_sign_on/saml_sso_setup)-Dienst verwenden, überprüfen Sie bei Ihrer/Ihrem Kontoadministrator:in, ob die Einrichtung korrekt abgeschlossen wurde.
...
Wenn Sie denselben Computer verwenden, mit dem Sie sich normalerweise anmelden, sollte Braze automatisch die richtige Instanz erkennen. Falls dies jedoch nicht der Fall ist oder Sie sich zum ersten Mal anmelden, beachten Sie Folgendes:
...
Dies kommt nicht sehr häufig vor, aber Braze nimmt Kontosperrungen und -löschungen sehr ernst. Wenn Sie auf diesen Fehler stoßen, wenden Sie sich an die/den Braze-Administrator:in Ihres Unternehmens, Ihren Braze Account Manager oder den [Support][support].
...
### Die/der Nutzer:in gehört keinem Workspace an {#the-user-belongs-to-no-workspace}

Admins können dies beheben, indem sie zu **Einstellungen** > **Unternehmensnutzer:innen** gehen, die Workspace-Berechtigungen der/des Nutzers:in überprüfen und die erforderlichen Workspaces unter **Workspaces** hinzufügen.

### Fehlerbehebung als neue:r Nutzer:in {#troubleshooting-as-a-new-user}

Wenn Sie ein:e neue:r Braze-Nutzer:in sind und Probleme bei der Anmeldung oder beim erstmaligen Zugriff auf Ihr Konto haben, befolgen Sie diese Schritte, um häufige Probleme zu beheben:
...
- E-Mail-Adresse überprüfen: Lassen Sie Ihre:n Admin die E-Mail-Adresse überprüfen, die mit Ihrem neuen Braze-Konto verknüpft ist, um sicherzustellen, dass sie korrekt ist.
...
- 2FA zurücksetzen: Wenn Sie Probleme bei der Einrichtung der 2FA haben, kann Ihr:e Admin die 2FA für Ihr Nutzerkonto in den Einstellungen zurücksetzen.
- Nutzer:in erneut hinzufügen: Wenn die Probleme weiterhin bestehen, kann die/der Admin Ihr Nutzerkonto aus dem Dashboard löschen und Sie erneut hinzufügen. Dies ermöglicht die Erstellung der/des Nutzers:in mit denselben Daten.
