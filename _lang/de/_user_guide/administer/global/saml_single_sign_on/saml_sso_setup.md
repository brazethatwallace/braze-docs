...
Mitglieder, die sich für die Nutzung von SSO entscheiden, können ihr Passwort nicht mehr verwenden. Nutzer:innen, die weiterhin ihr Passwort verwenden, können dies tun, sofern sie nicht durch die folgenden Einstellungen eingeschränkt werden.
...
{% alert important %}
Nachdem SSO erzwungen wurde, gibt es keine Fallback-Option für die Anmeldung, wenn die SSO-Authentifizierung fehlschlägt. Stellen Sie vor der Aktivierung der SSO-Erzwingung sicher, dass Ihre SSO-Konfiguration korrekt ist, alle Zertifikate aktuell und erneuert sind und Ihre Sicherheitseinstellungen ordnungsgemäß verwaltet werden, um Anmeldeprobleme zu vermeiden.
{% endalert %}

## SAML-Trace abrufen {#obtaining-a-saml-trace}

Wenn Sie bei der Anmeldung Probleme im Zusammenhang mit SSO feststellen, kann das Abrufen eines SAML-Traces Ihnen bei der Fehlerbehebung Ihrer SSO-Verbindung helfen, indem identifiziert wird, was in den SAML-Anfragen gesendet wird.
...
Lassen Sie die betroffene Nutzer:in [den Cache und die Cookies ihres Browsers löschen](https://its.uiowa.edu/services/how-clear-cache-and-cookies-your-web-browser) und versuchen Sie dann erneut, sich mit SAML SSO anzumelden.
...
Wenn sich eine Nutzer:in nicht anmelden kann, weil sie zwischen Okta-SSO und dem Braze-Dashboard in einer Schleife feststeckt, müssen Sie in Okta die SSO-URL-Zieladresse auf Ihre [Braze-Instanz]({{site.baseurl}}/user_guide/administer/personal/sdk_endpoints) setzen (z. B. `https://dashboard-07.braze.com`).
...
- [SAML Just-in-Time-Bereitstellung einrichten]({{site.baseurl}}/user_guide/administer/global/saml_single_sign_on/saml_just_in_time_provisioning), damit neue Nutzer:innen bei ihrer ersten SSO-Anmeldung automatisch ein Braze-Konto erstellen.