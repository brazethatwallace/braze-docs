...
### 1. Schritt: Erstellen Sie einen Nutzer für die Identitäts- und Zugriffsverwaltung (IAM)

Um Ihren geheimen Zugriffsschlüssel und Ihre Zugriffsschlüssel-ID abzurufen, müssen Sie einen IAM-Nutzer erstellen. Folgen Sie dazu den Anweisungen unter [Einrichten Ihres AWS-Kontos](https://docs.aws.amazon.com/IAM/latest/UserGuide/getting-started-account-iam.html#create-an-admin).

### 2. Schritt: Zugangsdaten abrufen

1. Nachdem Sie einen neuen Nutzer erstellt haben, generieren Sie den Zugriffsschlüssel und laden Sie Ihre Zugriffsschlüssel-ID und Ihren geheimen Zugriffsschlüssel herunter.
...
1. Gehen Sie zu **IAM** (Identitäts- und Zugriffsverwaltung) > **Policies** > **Create Policy**, um Berechtigungen für Ihren Nutzer hinzuzufügen.
...
1. Nachdem Sie eine neue Richtlinie erstellt haben, gehen Sie zu **Users** und wählen Sie Ihren spezifischen Nutzer aus.
...
Die AWS Rolle ARN Methode generiert einen Amazon Resource Name (ARN) für eine Rolle, der es dem Braze-Amazon-Konto ermöglicht, sich als Mitglied dieser Rolle zu authentifizieren.

### 1. Schritt: Richtlinie erstellen

1. Melden Sie sich als Kontoadministrator bei der AWS-Managementkonsole an.
2. Gehen Sie in der AWS-Konsole zum Abschnitt **IAM** (Identitäts- und Zugriffsverwaltung) > **Policies** und wählen Sie dann **Create Policy**.
...
4. Gehen Sie in der AWS-Konsole zum Abschnitt **IAM** (Identitäts- und Zugriffsverwaltung) > **Roles** > **Create Role**.
...
### 4. Schritt: Mit Braze AWS verknüpfen
