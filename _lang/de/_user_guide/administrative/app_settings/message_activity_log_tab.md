...
{% alert tip %}
Zusätzlich zu diesem Artikel empfehlen wir Ihnen auch unseren Braze-Lernkurs [Qualitätssicherung und Debugging-Tools](https://learning.braze.com/quality-assurance-and-debugging-tools-in-the-dashboard/), in dem Sie lernen, wie Sie das Nachrichten-Aktivitätsprotokoll für Ihre eigene Fehlerbehebung und Ihr Debugging nutzen können.
{% endalert %}
...
- Fehler bei fehlerhaftem Nutzer-Trigger
...
- Fehler bei nicht verfügbarem [Modell]({{site.baseurl}}/user_guide/brazeai/agents/reference/#models) von Braze Agents

Diese Nachrichten können von unserem eigenen System, Ihren Apps oder Plattformen oder von unseren Drittpartnern stammen. Dies kann zu einer unendlichen Anzahl von Meldungen führen, die in diesem Protokoll erscheinen können.

## Verstehen von Protokollmeldungen

Um herauszufinden, was Ihre Nachrichten bedeuten, achten Sie auf den Wortlaut jeder Nachricht und die dazugehörigen Spalten, denn das kann Ihnen bei der Fehlerbehebung mit Hilfe von Kontexthinweisen helfen. 

Wenn Sie beispielsweise einen Protokolleintrag mit der Nachricht „empty-cart_app" haben und sich nicht sicher sind, was dies bedeutet, schauen Sie links in die Spalte **Typ**. Wenn Sie „Fehler bei abgebrochener Nachricht" sehen, können Sie davon ausgehen, dass die Nachricht als [Abbruchnachricht]({{site.baseurl}}/user_guide/personalization_and_dynamic_content/liquid/aborting_messages/#aborting-messages) mit Liquid geschrieben wurde und dass die Nachricht abgebrochen wurde, weil die vorgesehene Empfängerin oder der vorgesehene Empfänger einen leeren Warenkorb in Ihrer App hatte.
...
Die Nachrichten im Nachrichten-Aktivitätsprotokoll können aus einer Vielzahl von Quellen stammen: Braze, Ihre Apps oder Plattformen oder unsere Drittpartner. Das bedeutet, dass es eine unendliche Anzahl von Meldungen gibt, die in diesem Protokoll erscheinen könnten – wie Sie sich vorstellen können, können wir sie nicht alle auflisten!
...
- Leider wurden die Nachrichten von [_IP_ADDRESS_] nicht versendet. Bitte wenden Sie sich an Ihren Internet-Provider, da ein Teil seines Netzwerks auf unserer Sperrliste steht.
- Nachricht aufgrund lokaler Richtlinie abgelehnt.
...
Die Fehler der letzten 60 Stunden finden Sie in den Nachrichten-Aktivitätsprotokollen. Protokolle, die mehr als 60 Stunden alt sind, werden gelöscht und sind nicht mehr zugänglich.
...
- Für dieselbe Kampagne oder denselben Canvas-Schritt werden innerhalb einer festgelegten Stunde bis zu 20 Fehlerprotokolle desselben Fehlertyps gespeichert. Dies gilt für die folgenden Fehlertypen:
    - Connected-Content-Fehler
    - Fehler bei Abbruchnachrichten
    - Webhook-Fehler
    - Fehler bei der SMS-Ablehnung
    - Fehler bei der SMS-Zustellung
    - WhatsApp-Fehler
    - A/B-Test-Fehler
- Für dieselbe Kampagne oder dieselbe Kombination aus Canvas-Schritt und App werden bis zu 20 Fehlerprotokolle bei Push-Benachrichtigungen desselben Fehlertyps für die folgenden Fehlertypen gespeichert:
...
- Für die folgenden Fehlertypen werden bis zu 100 Fehlerprotokolle desselben Fehlertyps für dieselbe App innerhalb einer festgelegten Stunde gespeichert:
    ...
    - APNs-Feedback: Fehler bei entfernten Token
- Für dieselbe Kampagne oder denselben Canvas-Schritt werden innerhalb einer festgelegten Stunde bis zu 100 Fehlerprotokolle desselben Fehlertyps gespeichert. Dies gilt für die folgenden Fehlertypen:
    ...
    - E-Mail-Block-Fehler
- Bis zu 100 User-Aliasing-Fehlerprotokolle werden für denselben Workspace innerhalb einer festgelegten Stunde gespeichert.

## Testsendungen

Das **Nachrichten-Aktivitätsprotokoll** zeigt Testprotokolle für die folgenden Messaging-Kanäle an:
...
Testversandprotokolle sind für die folgenden Kanäle nicht verfügbar: E-Mail, Content-Cards, In-App-Nachrichten und Push.

Testversandprotokolle sind mit „[TEST SEND]" gekennzeichnet, jedoch kann nicht garantiert werden, dass alle Testversandprotokolle diese Kennzeichnung aufweisen (beispielsweise weisen Connected-Content-Fehler diese Kennzeichnung nicht auf).