...
Wählen Sie einen Empfehlungstyp aus. Jeder Typ verwendet die Artikelinteraktionsdaten der letzten sechs Monate, wie z. B. Käufe, aufgegebene Bestellungen oder angepasste Event-Daten. Ausführlichere Informationen und Anwendungsfälle für jeden Typ finden Sie unter [Typen und Anwendungsfälle]({{site.baseurl}}/user_guide/brazeai/item_recommendations).

{% alert tip %}
Bei Verwendung von **Neueste** oder **KI-Personalisiert** erhalten Nutzer:innen mit unzureichenden Daten für individualisierte Empfehlungen als Fallback die **Beliebtesten** Artikel. Der Fallback **Beliebteste** gibt nur Artikel zurück, die im verknüpften Katalog vorhanden sind.<br><br>Für **KI-Personalisiert**-Empfehlungen können Sie auf der Seite **Analytics** die **Personalisierungsrate** einsehen, um zu erfahren, welcher Prozentsatz der Nutzer:innen, die das konfigurierte Event in den letzten 24 Monaten ausgeführt haben, personalisierte Empfehlungen in ihrem Profil gespeichert hat. Für **Neueste**-Empfehlungen zeigt die Seite **Analytics** den Anteil der Nutzer:innen, die **Neueste**-Empfehlungen erhalten, im Vergleich zum Fallback **Beliebteste**.
{% endalert %}
...
![Schritt „Empfehlung definieren“ mit „KI-Personalisiert“ als Typ und der ausgewählten Option „Keine Artikel empfehlen, mit denen Nutzer:innen bereits interagiert haben“.]({% image_buster /assets/img/item_recs_2-3.png %})
...
### Schritt 4: Interaktion für die Empfehlung auswählen {#step-4-select-the-interaction-to-drive-recommendations}
...
{% alert tip %}
Bei der Konfiguration von KI-Artikelempfehlungen ist die Wahl des Events wichtig. Ihr auslösendes Event bestimmt, wer eine KI-generierte Empfehlung erhält – KI-Artikelempfehlungen werden für Nutzer:innen generiert, die das von Ihnen konfigurierte Event abgeschlossen haben, sodass diese Wahl direkt bestimmt, wer Empfehlungen erhält. Wählen Sie ein Event, das das gesamte Zielgruppensegment abdeckt, das Sie erreichen möchten.<br><br>Gleichzeitig sollten Sie Abdeckung und Relevanz abwägen. Events am oberen Ende des Funnels (wie „Produkt angesehen“) erfassen tendenziell eine breitere Zielgruppe, sind aber weniger mit Geschäftsergebnissen verbunden, während Events am unteren Ende des Funnels (wie „Gekauft“) tendenziell gezieltere, geschäftsrelevantere Empfehlungen liefern. Das beste Event ist eines, das Abdeckung und Einfluss auf das Geschäftsergebnis in Einklang bringt.
{% endalert %}
...
##### Beispiel für ein angepasstes Event-Objekt mit einem Produkt-Array {#example-custom-event-object-with-an-array-of-products}
