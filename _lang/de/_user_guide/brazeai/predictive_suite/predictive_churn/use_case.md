...
description: "Dieses Beispiel veranschaulicht, wie eine fiktive Marke Predictive Abwanderung einsetzt, um proaktiv die Abwanderung von Nutzer:innen zu reduzieren."
page_type: tutorial
---

# Anwendungsfall: Abwanderung durch zeitnahe erneute Interaktion mit Inhalten verringern {#use-case-reduce-churn-with-timely-content-re-engagement}
...
## 1. Schritt: Erstellen Sie ein Modell zur Abwanderung-Prognose {#step-1-create-a-churn-prediction-model}

Camila beginnt damit, das Ergebnis zu modellieren, das sie vermeiden möchte: dass Nutzer:innen inaktiv werden. Für MovieCanon bedeutet Abwanderung, dass innerhalb von 14 Tagen kein Stream gestartet wird – dies ist das Verhalten, das sie vorhersagen möchte.

1. Im Braze-Dashboard navigiert Camila zu **Analytics** > **Predictive Abwanderung**.
2. Sie erstellt eine neue Abwanderung-Prognose und benennt sie „Abwanderung-Risiko in zwei Wochen“.
3. Um Abwanderung zu definieren, wählt sie `do not` und das angepasste Event `stream_started`, das aktives Engagement anzeigt.
...
## 2. Schritt: Segmentieren Sie Nutzer:innen nach Abwanderung-Risiko {#step-2-segment-users-by-churn-risk}

Nach Abschluss des Trainings weist Braze jeder in Frage kommenden Nutzer:in einen [Abwanderung-Risiko-Score]({{site.baseurl}}/user_guide/brazeai/predictive_suite/predictive_churn/analytics#churn_score) zwischen 0 und 100 zu.
...
2. Sie erstellt ein [Segment]({{site.baseurl}}/user_guide/audience/segments/creating_a_segment) mithilfe des [Filters „Abwanderung Risk Score“]({{site.baseurl}}/user_guide/audience/segments/segmentation_filters#churn-risk-score) und wählt die von ihr erstellte Abwanderung-Prognose aus:
...
Dank Predictive Abwanderung, intelligentem Timing und KI-gestützter Personalisierung reagiert Camilas Team nicht nur auf Abwanderung – sie sind ihm einen Schritt voraus. Und ihre Campaign läuft diskret im Hintergrund und erreicht die richtigen Personen zur richtigen Zeit mit Inhalten, die für sie tatsächlich von Interesse sind.