...
Braze berechnet den optimalen Sendezeitpunkt auf Grundlage einer statistischen Analyse der bisherigen Interaktionen Ihrer Nutzer:innen mit Ihrer App und deren Interaktionen mit den einzelnen Messaging-Kanälen. Dabei werden folgende Interaktionsdaten herangezogen:

- Sitzungszeiten
- Direkte Push-Öffnungen
...
Wenn für eine:n Nutzer:in keine relevanten Engagement-Daten vorliegen, anhand derer Braze den optimalen Sendezeitpunkt berechnen könnte, können Sie einen Fallback-Zeitpunkt festlegen.
...
- Beim Messaging an Ihre aktivsten Nutzer:innen (diese verfügen über die meisten Engagement-Daten)
...
2. Wählen Sie **Scheduled Delivery** als Zustellungstyp aus.
3. Wählen Sie unter **Time-Based Scheduling Options** die Option **Intelligent Timing** aus.
4. Legen Sie die Eintrittshäufigkeit fest. Für einmalige Sendungen wählen Sie **Once** und wählen ein Sendedatum aus. Für wiederkehrende Sendungen wählen Sie **Daily**, **Weekly** oder **Monthly** und konfigurieren die Wiederholungsoptionen. Weitere Hinweise finden Sie unter [Überlegungen](#considerations).
5. Konfigurieren Sie optional [Ruhezeiten](#quiet-hours).
...
![Campaign-Planungsbildschirm mit intelligentem Timing, Fallback-Zeit und Ruhezeiten-Einstellungen]({% image_buster /assets/img/intelligent_timing/campaign_scheduling.png %})

#### Ruhezeiten {#quiet-hours}

Verwenden Sie Ruhezeiten, um zu verhindern, dass Nachrichten zu bestimmten Uhrzeiten gesendet werden. Dies ist hilfreich, wenn Sie vermeiden möchten, dass Nachrichten in den frühen Morgenstunden oder über Nacht gesendet werden, während das intelligente Timing dennoch das beste Zustellfenster bestimmen kann.

{% alert note %}
Ruhezeiten haben die Einstellung **Only send within specific hours** ersetzt. Anstatt zu wählen, wann Nachrichten gesendet werden können, wählen Sie jetzt, wann sie nicht gesendet werden sollen. Um beispielsweise Nachrichten zwischen 16:00 und 18:00 Uhr zu senden, setzen Sie die Ruhezeiten von 18:00 Uhr bis 16:00 Uhr am nächsten Tag.
{% endalert %}

1. Wählen Sie **Enable Quiet Hours**.
2. Wählen Sie die Start- und Endzeit aus, zu der Nachrichten **nicht** gesendet werden sollen.

![Ruhezeiten-Schalter aktiviert mit Start- und Endzeit zur Blockierung der Nachrichtenzustellung über Nacht]({% image_buster /assets/img/intelligent_timing/quiet_hours.png %})

Wenn Ruhezeiten aktiviert sind, sendet Braze während der Ruhezeit keine Nachrichten – selbst wenn diese Zeit der optimalen Sendezeit einer Nutzerin oder eines Nutzers entspricht. Wenn die optimale Zeit einer Nutzerin oder eines Nutzers in das Ruhezeitfenster fällt, wird die Nachricht stattdessen am nächstgelegenen Rand des Fensters gesendet.

Wenn die Ruhezeiten beispielsweise von 22:00 bis 6:00 Uhr eingestellt sind und die optimale Zeit einer Nutzerin oder eines Nutzers 5:30 Uhr ist, hält Braze die Nachricht zurück und stellt sie um 6:00 Uhr zu – der nächstgelegenen Zeit außerhalb des Ruhezeitfensters.
...
Um eine Schätzung zu sehen, wie viele Nutzer:innen die Nachricht in jeder Stunde des Tages erhalten werden, verwenden Sie das Vorschau-Chart (nur für Campaigns).

1. Fügen Sie Segments oder Filter im Schritt „Zielgruppen“ hinzu.
2. Wählen Sie im Abschnitt **Preview Delivery Times for** (der sowohl im Schritt „Zielgruppen“ als auch im Schritt „Zustellung planen“ angezeigt wird) Ihren Kanal aus.
3. Klicken Sie auf **Refresh Data**.

![Zustellvorschau-Chart für Android-Push mit Spitzen-Engagement zwischen 12:00 und 14:00 Uhr sowie der beliebtesten App-Nutzungszeit um 14:00 Uhr.]({% image_buster /assets/img/intel-timing-preview.png %})

### Schritt 2: Sendedatum wählen {#step-2-choose-a-send-date}

Wählen Sie als Nächstes ein Sendedatum für Ihre Campaign aus. Beachten Sie Folgendes, wenn Sie Campaigns mit intelligentem Timing planen:

#### Campaign 48 Stunden im Voraus starten {#launch-campaign-48-hours-in-advance}

Starten Sie Ihre Campaign mindestens 48 Stunden vor dem geplanten Sendedatum. Dies liegt an den unterschiedlichen Zeitzonen. Braze berechnet die optimale Zeit um Mitternacht in Samoa-Zeit (UTC+13), einer der ersten Zeitzonen der Welt. Ein einzelner Tag umfasst weltweit etwa 48 Stunden, was bedeutet, dass die optimale Zeit einer Nutzerin oder eines Nutzers in ihrer Zeitzone bereits vergangen sein kann, wenn Sie eine Campaign innerhalb dieses 48-Stunden-Puffers starten, und die Nachricht nicht gesendet wird.

{% alert important %}
Wenn eine Campaign gestartet wird und die optimale Zeit einer Nutzerin oder eines Nutzers weniger als eine Stunde zurückliegt, wird die Nachricht sofort gesendet. Wenn die optimale Zeit mehr als eine Stunde zurückliegt, wird die Nachricht überhaupt nicht gesendet.
{% endalert %}

#### 3-Tage-Fenster für Segment-Filter {#3-day-window-for-segment-filters}

Wenn Sie eine Zielgruppe ansprechen, die eine Aktion innerhalb eines bestimmten Zeitraums durchgeführt hat, lassen Sie mindestens ein 3-Tage-Fenster in Ihren Segment-Filtern zu. Verwenden Sie beispielsweise statt `First used app more than 1 day ago` und `First used app less than 3 days ago` die Werte 1 Tag und 4 Tage.

![Filter für die Zielgruppe, bei denen die Campaign Nutzer:innen anspricht, die die App vor 1 bis 4 Tagen zum ersten Mal genutzt haben.]({% image_buster /assets/img/intelligent_timing/first_used_app.png %})

Dies liegt ebenfalls an den Zeitzonen – die Auswahl eines Zeitraums von weniger als 3 Tagen kann dazu führen, dass einige Nutzer:innen aus dem Segment herausfallen, bevor ihre optimale Sendezeit erreicht wird.
...
#### Optimierten Versand mindestens 2 Tage nach dem A/B-Test planen {#schedule-the-optimized-send-at-least-2-days-after-the-ab-test}

Wenn Sie [Mit BrazeAI<sup>TM</sup> optimieren]({{site.baseurl}}/user_guide/brazeai/intelligence_suite/variant_selection) für eine einzeln versendete Campaign verwenden, kann intelligentes Timing die Dauer und das Timing Ihrer Campaign beeinflussen.

Wenn Sie intelligentes Timing verwenden, legen Sie die Experimentdauer so fest, dass der optimierte Versand mindestens zwei Tage nach Beginn des A/B-Tests beginnt. Wenn Ihr Test beispielsweise am 16. April um 16:00 Uhr startet, konfigurieren Sie den optimierten Versand so, dass er frühestens am 18. April um 16:00 Uhr beginnt. So hat Braze genügend Zeit, das Nutzer:innenverhalten auszuwerten und Nachrichten zur optimalen Zeit zu senden.

### Schritt 3: Ruhezeiten konfigurieren (optional) {#step-3-configure-quiet-hours-optional}

Optional können Sie das Zustellfenster einschränken. Dies kann nützlich sein, wenn sich Ihre Campaign auf ein bestimmtes Ereignis, einen Sale oder eine Aktion bezieht, wird aber bei der Verwendung von intelligentem Timing generell nicht empfohlen. Weitere Informationen finden Sie unter [Überlegungen](#considerations).

Ruhezeiten fungieren als Nicht-Senden-Fenster. Intelligentes Timing bestimmt weiterhin die optimale Sendezeit für jede Nutzerin und jeden Nutzer, aber wenn diese Zeit in die Ruhezeiten fällt, verzögert Braze die Nachricht bis zur nächsten verfügbaren Zeit außerhalb der Ruhezeiten.

So konfigurieren Sie Ruhezeiten:

1. Wählen Sie bei der Konfiguration des intelligenten Timings **Enable Quiet Hours** aus.
2. Geben Sie die Start- und Endzeit des Ruhezeitenfensters ein.
...
![Campaign mit intelligentem Timing planen]({% image_buster /assets/img/intelligent_timing_1.png %})
...
Um eine Schätzung zu sehen, wie viele Nutzer:innen die Nachricht in jeder Stunde des Tages erhalten, verwenden Sie das Vorschau-Chart:

1. Fügen Sie Segments oder Filter im Schritt **Target Audiences** hinzu.
2. Wählen Sie im Abschnitt **Preview Delivery Times for** (der sowohl im Schritt **Target Audiences** als auch im Schritt **Schedule Delivery** angezeigt wird) Ihren Kanal aus.
3. Wählen Sie **Refresh Data** aus.

Das Vorschau-Chart zeigt jede Stunde des Tages in Ihrer Ortszeit an. Die Beschriftungen sind nicht auf eine globale Zeitzone eingestellt.
...
Nachrichten werden an Nutzer:innen gesendet, die an diesem Tag in den Schritt eingetreten sind, und zwar zu ihrer optimalen Ortszeit. Wenn ihre optimale Zeit an diesem Tag jedoch bereits vergangen ist, wird die Nachricht stattdessen am folgenden Tag zur optimalen Zeit zugestellt. Nachrichten-Schritte, die mehrere Kanäle ansprechen, können Nachrichten zu unterschiedlichen Zeiten für verschiedene Kanäle senden oder zu senden versuchen. Wenn die erste Nachricht in einem Nachrichten-Schritt zu senden versucht wird, werden alle Nutzer:innen automatisch weitergeführt.

### Schritt 2: Fallback-Zeit wählen {#step-2-choose-a-fallback-time}

Wählen Sie eine Fallback-Zeit, zu der die Nachricht an Nutzer:innen in Ihrer Zielgruppe gesendet wird, für die Braze keine relevanten Engagement-Daten hat, um eine optimale Sendezeit zu berechnen. {% multi_lang_include brazeai/intelligent_suite/fallback_time.md %}

### Schritt 4: Einen Verzögerungsschritt hinzufügen {#step-4-add-a-delay-step}

Im Gegensatz zu Campaigns müssen Sie Ihren Canvas nicht 48 Stunden vor dem Sendedatum starten, da intelligentes Timing auf Schrittebene und nicht auf Canvas-Ebene eingestellt wird.

Fügen Sie stattdessen einen [Verzögerungsschritt]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/delay_step) von mindestens zwei Kalendertagen zwischen dem Eintritt der Nutzerin oder des Nutzers in den Canvas und dem Erhalt des Schritts mit intelligentem Timing hinzu.

#### Kalendertage vs. 24-Stunden-Tage {#calendar-vs-24-hour-days}

Bei der Verwendung von intelligentem Timing nach einem Verzögerungsschritt kann das Zustelldatum je nach Berechnung der Verzögerung variieren. Dies gilt nur, wenn Ihre Verzögerung auf **After a duration** eingestellt ist, da es einen Unterschied zwischen der Berechnung von „Tagen“ und „Kalendertagen“ gibt.
...
- **Kalendertage:** 1 Tag ist der Zeitraum vom Eintritt der Nutzerin oder des Nutzers in den Verzögerungsschritt bis Mitternacht in ihrer bzw. seiner Zeitzone. Das bedeutet, dass 1 Kalendertag nur wenige Minuten lang sein kann.

Bei der Verwendung von intelligentem Timing empfehlen wir, Kalendertage statt 24-Stunden-Tage für Verzögerungen zu verwenden. Mit Kalendertagen wird die Nachricht am letzten Tag der Verzögerung zur optimalen Zeit gesendet. Bei einem 24-Stunden-Tag besteht die Möglichkeit, dass die optimale Zeit der Nutzerin oder des Nutzers vor dem Eintritt in den Schritt liegt, was bedeutet, dass ein zusätzlicher Tag zur Verzögerung hinzugefügt wird.

Nehmen wir beispielsweise an, Lukas optimale Zeit ist 14:00 Uhr. Er tritt am 1. März um 14:01 Uhr in den Verzögerungsschritt ein, und die Verzögerung ist auf 2 Tage eingestellt.

- Tag 1 endet am 2. März um 14:01 Uhr
- Tag 2 endet am 3. März um 14:01 Uhr

Intelligentes Timing ist jedoch auf Zustellung um 14:00 Uhr eingestellt, was bereits vergangen ist. Daher erhält Luka die Nachricht erst am folgenden Tag: am 4. März um 14:00 Uhr.

![Grafik, die den Unterschied zwischen Tagen und Kalendertagen zeigt: Wenn die optimale Zeit einer Nutzerin oder eines Nutzers 14:00 Uhr ist, sie aber um 14:01 Uhr in den Verzögerungsschritt eintreten und die Verzögerung auf 2 Tage eingestellt ist. Tage stellen die Nachricht 3 Tage später zu, da die Nutzerin oder der Nutzer nach ihrer optimalen Zeit eingetreten ist, während Kalendertage die Nachricht 2 Tage später am letzten Tag der Verzögerung zustellen.]({% image_buster /assets/img/intelligent_timing_daysvcalendardays.png %}){: style="border:none;"}
{% endtab %}
{% endtabs %}
...
- Intelligentes Timing sollte in den folgenden Szenarien nicht verwendet werden:
    - **Rate-Limiting:** Wenn sowohl Rate-Limiting als auch intelligentes Timing verwendet werden, gibt es keine Garantie dafür, wann die Nachricht zugestellt wird. Täglich wiederkehrende Campaigns mit intelligentem Timing unterstützen eine Obergrenze für den Gesamtversand von Nachrichten nicht zuverlässig.
    - **IP-Warming-Campaigns:** Einige Verhaltensweisen des intelligenten Timings können es erschweren, die täglichen Volumina zu erreichen, die beim erstmaligen IP-Warming benötigt werden. Das liegt daran, dass intelligentes Timing Segmente zweimal auswertet – einmal bei der Erstellung der Campaign oder des Canvas und ein weiteres Mal vor dem Senden an Nutzer:innen, um zu überprüfen, ob sie sich noch in diesem Segment befinden. Dies kann dazu führen, dass sich Segmente verschieben und verändern, was häufig dazu führt, dass einige Nutzer:innen bei der zweiten Auswertung aus dem Segment herausfallen. Diese Nutzer:innen werden nicht ersetzt, was sich darauf auswirkt, wie nah Sie an die maximale Nutzerobergrenze herankommen können.

## Fehlerbehebung {#troubleshooting}

### Vorschau-Chart zeigt wenige Nutzer:innen mit optimalen Zeiten {#preview-chart-showing-few-users-with-optimal-times}

Wenn für eine:n Nutzer:in keine relevanten Ereignisse vorliegen (z. B. bei neuen Nutzer:innen mit wenig oder keinem Engagement), verwendet Braze die konfigurierte Fallback-Einstellung – entweder Ihre benutzerdefinierte Fallback-Zeit oder die beliebteste Zeit zur Nutzung der App unter allen Nutzer:innen.

### Auswirkung der Zeitzone auf die Zustellung mit intelligentem Timing {#impact-of-time-zone-on-intelligent-timing-delivery}

Intelligentes Timing nutzt die lokale Zeitzone und Kalendertage jeder/jedes Nutzer:in, um den optimalen Zustellzeitpunkt zu bestimmen. Aus diesem Grund können Nutzer:innen in Zeitzonen, die der Referenzzeitzone Ihrer Campaign voraus oder hinterher sind, Nachrichten an einem anderen Kalendertag erhalten, als Sie erwarten würden.

Wenn beispielsweise eine Campaign für den 15. März geplant ist und die optimale Zeit einer/eines Nutzer:in für dieses Datum berechnet wird, kann eine Person in einer Zeitzone vor dem Referenzpunkt die Nachricht am späten Abend des 14. März in der Referenzzeitzone erhalten, während eine Person in einer Zeitzone hinter dem Referenzpunkt sie erst am 16. März erhält.

Falls Nutzer:innen Nachrichten nicht wie erwartet erhalten, überprüfen Sie, ob das Zeitzonen-Feld in ihrem Profil korrekt ausgefüllt ist. Wenn das Zeitzonen-Feld leer ist, erhalten die Nutzer:innen möglicherweise Nachrichten, die sich an der Zeitzone des Unternehmens statt an ihrer Ortszeit orientieren.

### Versand nach dem geplanten Datum {#sending-past-the-scheduled-date}

Ihre Campaign mit intelligentem Timing kann über das geplante Datum hinaus versenden, wenn Sie [Mit BrazeAI<sup>TM</sup> optimieren]({{site.baseurl}}/user_guide/brazeai/intelligence_suite/variant_selection) verwenden. Bei einer einmaligen Campaign sendet Braze die leistungsstärkste Variante nach dem ersten Test an die verbleibende Zielgruppe, was die Campaign-Dauer verlängert.

Wenn Sie intelligentes Timing verwenden, planen Sie genügend Zeit für den Abschluss des A/B-Tests ein und setzen Sie den optimierten Versand auf zwei Tage nach dem ersten Test an.
...
Intelligentes Timing konzentriert sich auf die Vorhersage, wann Nutzer:innen Ihre Nachrichten am ehesten öffnen oder anklicken, um sicherzustellen, dass Ihre Nachrichten die Nutzer:innen zu optimalen Engagement-Zeiten erreichen.
...
Nein, [maschinelle Öffnungen]({{site.baseurl}}/user_guide/analytics/metrics_glossary#machine-opens) sind von den Berechnungen für die optimale Zeit ausgeschlossen. Das bedeutet, dass die Sendezeiten ausschließlich auf echtem Nutzer:innen-Engagement basieren, was eine genauere Zeitplanung für Ihre Campaigns ermöglicht.
...
Ihre Campaign mit intelligentem Timing versendet möglicherweise über das geplante Datum hinaus, wenn **Mit BrazeAI<sup>TM</sup> optimieren** aktiviert ist. Bei einer Einmal-Campaign sendet Braze die Gewinnervariante an die verbleibende Zielgruppe, nachdem der A/B-Test beendet ist, wodurch sich die Campaign-Dauer verlängert.

Planen Sie genügend Zeit für den Abschluss des A/B-Tests ein und planen Sie den optimierten Versand für zwei Tage nach dem ersten Test.
...
Fügen Sie in Canvas einen [Nachrichten-Schritt]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/message_step) hinzu oder öffnen Sie einen vorhandenen, gehen Sie zu **Zustellungseinstellungen** und wählen Sie **Intelligentes Timing verwenden**. Gemäß der Canvas-Einrichtungsanleitung in diesem Artikel fügen Sie einen [Verzögerungsschritt]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/delay_step) von mindestens zwei Kalendertagen zwischen dem Canvas-Eintritt und diesem Nachrichten-Schritt ein, damit intelligentes Timing über ausreichend Engagement-Verlauf zur Auswertung verfügt.