...
Decisioning Studio Go ist ein KI-Entscheidungsagent für wiederkehrende E-Mail-Programme. Anstatt eine einzige gewinnende Betreffzeile, Sendezeit oder ein einzelnes Bild für die gesamte Zielgruppe auszuwählen, wählt der Agent die beste Kombination für jede Empfänger:in basierend auf deren bisherigem Engagement aus.

Sie definieren die Varianten, aus denen der Agent wählen kann – wie Betreffzeilen, CTAs, Bilder, Sendetage und Sendezeiten. Für jede Nutzer:in in Ihrem Segment wählt der Agent die Option aus, die am wahrscheinlichsten Engagement erzeugt, innerhalb der von Ihnen konfigurierten Einschränkungen und des Zeitplans.
...
Für jede Nutzer:in in der Decisioning-Studio-Gruppe wählt der Agent aus den von Ihnen bereitgestellten Optionen: welches Creative gesendet werden soll (einschließlich der spezifischen Betreffzeile, des CTA und des Bildes darin) und wann es gesendet werden soll (Wochentag und Tageszeit, unter Berücksichtigung von Ruhezeiten und der Ortszeit der Nutzer:in). [Ihren Decisioning Studio Go-Agenten einrichten]({{site.baseurl}}/user_guide/brazeai/decisioning_studio/decisioning_studio_go/setup) behandelt jeden dieser Punkte im Detail.
...
| **Zeitplan** | Sendefrequenz (z. B. eine einzelne Auswahl dreimal pro Woche), zulässige Wochentage, Ruhezeiten in der Ortszeit der Nutzer:in und Einhaltung der Frequency-Capping-Regeln auf Agentenebene. |
| **Creatives** | Ein oder mehrere Basis-Creatives, die im Braze-Composer erstellt werden. Innerhalb jedes Basis-Creatives können Sie eine Betreffzeile, einen CTA und ein Bild mithilfe von Liquid-Tags als Personalisierungspunkte markieren und dann eine Liste von Varianten für jeden bereitstellen. Der Agent entscheidet, welches Basis-Creative und welche Variante für jede Empfänger:in verwendet wird. |
| **Einschränkungen** | Limits, die verhindern, dass der Agent dasselbe Basis-Creative oder dieselbe Betreffzeile innerhalb eines von Ihnen definierten Zeitfensters mehr als einmal an eine Nutzer:in sendet. |
...
- [Ihren Decisioning Studio Go-Agenten einrichten]({{site.baseurl}}/user_guide/brazeai/decisioning_studio/decisioning_studio_go/setup) – Zielgruppe, Zeitplan, Creatives und Einschränkungen konfigurieren
