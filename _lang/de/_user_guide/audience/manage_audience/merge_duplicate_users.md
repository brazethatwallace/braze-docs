...
> Erfahren Sie, wie Sie doppelte Nutzer:innen finden und zusammenführen, um die Effektivität Ihrer Campaigns und Canvase zu maximieren.

## Representational State Transfer API: Nutzer:innen identifizieren und zusammenführen {#rest-api-identify-and-merge-users}
...
- [POST: Nutzer:innen zusammenführen]({{site.baseurl}}/api/endpoints/user_data/post_users_merge) (`/users/merge`): Führt ein Kundenprofil or Nutzerprofil mit einem anderen zusammen, auch wenn beide Profile bereits eine `external_id` haben. Lesen Sie [Voraussetzungen]({{site.baseurl}}/api/endpoints/user_data/post_users_merge#prerequisites) und [Zusammenführungsverhalten]({{site.baseurl}}/api/endpoints/user_data/post_users_merge#merge-behavior), bevor Sie diesen Endpunkt aufrufen.

Wenn ein anonymes Profil einem bestehenden identifizierten Profil zugeordnet wird (z. B. durch einen SDK or Software-Development-Kit-`changeUser()`-Aufruf oder `/users/identify`), verwaist Braze das anonyme Profil und kopiert nur bestimmte Felder auf das identifizierte Profil. Weitere Informationen finden Sie unter [Was passiert, wenn Sie anonyme Nutzer:innen identifizieren]({{site.baseurl}}/user_guide/data/unification/user_data/user_profile_lifecycle#what-happens-when-you-identify-anonymous-users).

Zusammenführungen von Nutzer:innen sind schwer rückgängig zu machen. Wenn Sie eine komplexe Zusammenführung über mehrere `external_id`-Werte oder große Profilmigrationen planen, wenden Sie sich an Ihren CSM or Customer-Success-Manager or Customer-Success-Manager:in, bevor Sie sich auf `/users/merge` verlassen.
...
Wenn eine Nutzersuche doppelte Profile zurückgibt, können Sie jedes Profil einzeln über das Kundenprofil or Nutzerprofil im Braze-Dashboard zusammenführen.
...
Wählen Sie aus, welches Kundenprofil or Nutzerprofil beibehalten und welches zusammengeführt werden soll, und wählen Sie dann **Merge profiles**. Wiederholen Sie diesen Vorgang, bis Sie alle doppelten Profile zusammengeführt haben.
...
Die CSV-Datei enthält eine Spalte **Created from**, die zeigt, wie jedes Profil ursprünglich erstellt wurde (z. B. über das [SDK or Software-Development-Kit]({{site.baseurl}}/developer_guide/sdk_integration), die [Representational State Transfer API]({{site.baseurl}}/api/basics) oder einen [CSV-Import]({{site.baseurl}}/user_guide/audience/manage_audience/import_users/csv_import)). Dies hilft Ihnen, die Quelle des Profils zu verstehen, bevor Sie Duplikate zusammenführen.

Vergleichen Sie beim Überprüfen doppelter Zeilen **Created from** mit Bezeichnern wie `external_id`, E-Mail-Adresse und Telefonnummer. Nutzen Sie diesen Kontext, um zu entscheiden, welches Profil als primäres Profil beibehalten werden soll, bevor Sie **Merge all duplicates** auswählen.

Das Feld **Created from** ist besonders nützlich, wenn doppelte Profile ähnliche Werte enthalten, aber aus unterschiedlichen Erfassungspfaden stammen. Es gibt Ihrem Team mehr Kontext für Zusammenführungsentscheidungen und hilft, versehentliche Zusammenführungen von Profilen zu vermeiden, die Sie bis zur weiteren Überprüfung lieber getrennt halten möchten.
...
| ---------------- | ----------- | -------------- | --------------------- | ------------------- | ------------ | --------------- | ---------------- |
| jane.doe@example.com   | 123-external-id | 555 123-4567 | example-id-12345 | email               | SDK or Software-Development-Kit          | TRUE            | FALSE            |
| john.doe@example.com   |                 | 555 123-4567 | example-id-12346 | email               | Representational State Transfer         | FALSE           | TRUE             |
...
Sie können Regeln verwenden, um zu steuern, wie doppelte Profile beim Zusammenführen aufgelöst werden, damit das relevanteste Kundenprofil or Nutzerprofil beibehalten wird. Wenn Regeln festgelegt sind, behält Braze Profile bei, die Ihren Kriterien entsprechen.
...
Bevor Sie Duplikate zusammenführen, verwenden Sie den [Endpunkt „Kundenprofil or Nutzerprofil nach Bezeichner exportieren“]({{site.baseurl}}/api/endpoints/export/user_data/post_users_identifier), um zu bestätigen, welche Profile für eine E-Mail-Adresse existieren und welche Felder jedes Profil enthält. Sie können auch in **Audience** > **User Search** nach E-Mail suchen, um Duplikate im Dashboard zu überprüfen.
