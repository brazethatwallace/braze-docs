...
Um Nutzer:innen zu löschen, müssen Sie ein:e Administrator:in sein oder über die Berechtigung **Delete Users** verfügen. Um Löschprotokolle von Nutzer:innen einzusehen, müssen Sie ein:e Administrator:in sein oder über die Berechtigung **View User Deletion Records** verfügen. Die folgenden Berechtigungen steuern das Löschen von Nutzer:innen und die Löschprotokolle:

| Berechtigung | Beschreibung |
|------------|-------------|
| Delete Users | Nutzer:innen einzeln oder in großen Mengen dauerhaft löschen. |
| View User Deletion Records | Löschprotokolle von Nutzer:innen einsehen. |
...
| Wartezeit | Alle Segment-Löschungen erfordern eine 7-tägige Wartezeit zuzüglich der Zeit, die für die Verarbeitung der Löschungen benötigt wird. |
| Auftragslimits | Es kann jeweils nur ein Segment gleichzeitig gelöscht werden, einschließlich der 7-tägigen Wartezeit. |
{: .reset-td-br-1 .reset-td-br-2 aria-label="Über das Löschen von Nutzer:innen" }

## Nutzer:innen löschen {#deleting-users}

Sie können einzelne Nutzer:innen ([Einzellöschung](#delete-individual)) oder ein [Segment von Nutzer:innen](#delete-segment) über das Braze-Dashboard löschen:
...
{% alert warning %}
Einzellöschungen von Nutzer:innen sind dauerhaft – Profile können nach dem Löschen nicht wiederhergestellt werden.
{% endalert %}

Wählen Sie auf der Profilseite <i class="fa-solid fa-ellipsis-vertical" aria-label="Optionen anzeigen"></i> **Show options** > **Delete User**. Beachten Sie, dass es einige Minuten dauern kann, bis die Nutzer:innen vollständig aus Braze gelöscht sind.
...
## Bestätigung von Segment-Löschungen {#confirming-segment-deletions}

Braze sendet eine Bestätigungs-E-Mail mit der Anzahl der Profile, die zur Löschung ausstehen.

Um mit der Löschung fortzufahren, melden Sie sich bei Braze an und bestätigen Sie die Löschanfrage.

Wenn Sie die Löschung nicht innerhalb des in der E-Mail angegebenen Zeitraums bestätigen, läuft die Löschanfrage ab und wird nicht ausgeführt.
