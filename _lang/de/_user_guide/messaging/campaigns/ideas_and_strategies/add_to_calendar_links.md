...
Fügen Sie Ihren Links die folgende URL hinzu und ersetzen Sie die Platzhalter. Der einzige Unterschied zwischen diesen beiden URLs besteht darin, dass Google Calendar einen zusätzlichen Parameter benötigt: `&format=gcal`.
...
Ersetzen Sie Leerzeichen durch den HTML-Escape-Code `%20`. Beispielsweise würde ein Betreff von „Meet Braze“ als „Meet%20Braze“ geschrieben werden.

Hier ein Beispiel für eine „Zu Google Calendar hinzufügen“-URL:
...
Wenn Sie beispielsweise den Beispiellink „Zu Google Kalender hinzufügen“ öffnen und Ihr Kalender auf CST eingestellt ist, wird die Uhrzeit des Ereignisses entsprechend der Umrechnung von 15:00 Uhr UTC in CST (10:00 Uhr) vorausgefüllt.

### Google Kalender {#google-calendar}

Beim Klicken öffnet sich Google Kalender in einem neuen Tab oder Fenster, wobei die Ereignisdetails in der Einladung vorausgefüllt sind und von Nutzer:innen gespeichert werden können. Dies funktioniert sowohl auf Mobilgeräten als auch auf dem Desktop.
...
Beim Klicken auf dem Desktop wird eine ICS-Datei in den Standard-Downloadordner Ihres Browsers heruntergeladen (in der Regel der Ordner **Downloads**). Nutzer:innen müssen dann die ICS-Datei öffnen, wodurch iCal oder Outlook gestartet wird und sie aufgefordert werden, das Ereignis zu ihrem Kalender hinzuzufügen.
...
{% alert note %}
Auf dem iPhone laden die Mail-App und Microsoft Outlook die ICS-Datei auf das Gerät herunter, wenn Nutzer:innen auf den iCal-Link tippen, aber diese Apps öffnen den Kalender nicht über den Link. Um das Ereignis hinzuzufügen, öffnen Sie die heruntergeladene Datei über **Dateien**, **Downloads** oder die Anhangsansicht (je nach App) und führen Sie dann die Schritte im Kalender aus. Der genaue Speicherort hängt von der E-Mail-App und den iOS-Einstellungen ab.
{% endalert %}
