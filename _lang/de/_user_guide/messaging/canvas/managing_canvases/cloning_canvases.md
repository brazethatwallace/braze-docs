---
nav_title: Canvases klonen
article_title: Canvases klonen
page_order: 3
...
# Canvases zu Canvas Flow klonen
...
> Wenn Sie ein bestehendes Canvas aus dem ursprünglichen Editor haben, können Sie dieses Canvas klonen, um eine Kopie in Canvas Flow zu erstellen. Durch den Wechsel zum aktuellen Canvas-Workflow erhalten Sie Zugang zu leichtgewichtigen [Canvas-Komponenten]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/), [persistenten Eingangs-Eigenschaften]({{site.baseurl}}/user_guide/messaging/canvas/create_a_canvas/#canvas-entry-properties) und [Bearbeitungen nach dem Start]({{site.baseurl}}/post-launch_edits). Ihr ursprüngliches Canvas wird dabei weder verändert noch gelöscht.

Um Ihr Canvas zu klonen, gehen Sie wie folgt vor:

1. Gehen Sie zum Canvas-Dashboard.
2. Identifizieren Sie das Canvas, von dem Sie eine Kopie im Canvas-Flow-Workflow erstellen möchten. Sie können Canvases mit dem Status **Entwurf**, **Aktiv** oder **Gestoppt** klonen.
3. Klicken Sie auf <i class="fas fa-ellipsis-vertical"></i> **Weitere Aktionen** und wählen Sie **Zu Canvas Flow klonen**.

![]({% image_buster /assets/img_archive/clone_to_v2_workflow.png %}){: style="max-width:25%;"}

{: start="4"}
4. Geben Sie den Namen Ihres neuen Canvas ein und klicken Sie auf **Zu Canvas Flow klonen**.

![]({% image_buster /assets/img_archive/clone_to_v2_modal.png %}){: style="max-width:70%;"}

Jetzt haben Sie zwei Versionen Ihres Canvas: das ursprüngliche Canvas und die Canvas-Flow-Version. Ihr ursprüngliches Canvas behält seinen ursprünglichen Status, und das geklonte Canvas hat den Status **Entwurf**. Sie können weiterhin auf das ursprüngliche Canvas zugreifen, aber Braze empfiehlt, den Canvas-Flow-Workflow zu verwenden, um Ihre Canvases weiterzubauen.

Zuvor konnten einige Canvases mit Verzweigungen nicht geklont werden. Jetzt können Sie Canvases mit Verzweigungen klonen. Beachten Sie, dass das Klonen von Canvases mit Verzweigungen zu nicht verbundenen Schritten führen kann. Beheben Sie diese nicht verbundenen Schritte (Schritte, die keinen vorhergehenden verbundenen Schritt haben), um sicherzustellen, dass Ihre Canvas-Journey korrekt abgebildet ist.

{% alert note %}
Wenn Sie ein aktives Canvas klonen, sendet Braze weiterhin Nutzer:innen durch das ursprüngliche Canvas. Wir empfehlen, ein Canvas vor dem Klonen zu stoppen, um zu vermeiden, dass doppelte Nachrichten von beiden Canvases an Nutzer:innen gesendet werden.
{% endalert %}

![Canvas-Dashboard mit zwei aufgelisteten Canvases: V2-Kopie von Canvas V1 und Canvas V1. Die V2-Kopie von Canvas V1 hat ein Symbol, das anzeigt, dass sie den Canvas-Flow-Workflow verwendet.]({% image_buster /assets/img_archive/clone_to_v2_dashboard.png %})

Sie haben das Klonen Ihres Canvas in den Canvas-Flow-Workflow abgeschlossen. Jetzt können Sie Ihre Canvases in dieser aktualisierten Erfahrung weiter aufbauen!
...
Weitere Informationen zu den Unterschieden zwischen dem ursprünglichen Canvas-Editor und der Canvas-Flow-Erfahrung finden Sie in den [Canvas-FAQ]({{site.baseurl}}/user_guide/messaging/canvas/faqs/#what-are-the-main-differences-between-canvas-flow-and-the-original-canvas-editor).