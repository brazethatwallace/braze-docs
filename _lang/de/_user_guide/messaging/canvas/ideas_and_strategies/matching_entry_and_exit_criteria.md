...
3. Wählen Sie **Add property filters** aus und fügen Sie einen Filter hinzu, bei dem der Vergleich der Basiseigenschaft `event_id` auf `equals` gesetzt ist.
4. Aktivieren Sie den **Personalize value**-Toggle, setzen Sie den **Personalization type** auf `Context Variables` und das **Attribut** auf `event_id`.
...
2. Fügen Sie für die erste Aktionsgruppe (ursprünglicher Kauf) einen Trigger für das abschließende angepasste Event hinzu (z. B. `Purchased_Ticket`). Wählen Sie **Add property filters** aus und fügen Sie einen Filter hinzu, bei dem der Vergleich der Basiseigenschaft `event_id` auf `equals` gesetzt ist. Aktivieren Sie **Personalize value**, setzen Sie den **Personalization type** auf `Context Variables` und das **Attribut** auf `event_id`.
