...
3. Überprüfen Sie den Messaging-Verlauf einer Nutzerin oder eines Nutzers, indem Sie zu **Audience** > **Nutzer:innen suchen** gehen, das Profil öffnen und **Messaging-Verlauf** (letzte 30 Tage) auswählen.
...
Kontaktieren Sie den [Braze-Support]({{site.baseurl}}/braze_support/) innerhalb von 30 Tagen nach dem Problem, wenn Sie den [standardmäßigen Untersuchungspfad](#standard-investigation-path) abgeschlossen haben und weiterhin Hilfe benötigen.

Geben Sie Folgendes an:

- Canvas-ID und betroffene Nutzer-IDs (externe ID oder Braze-ID)
- Zeitstempel mit Zeitzone
- Screenshots oder Exporte aus dem **Messaging-Verlauf** oder **Messaging-Diagnostics**
