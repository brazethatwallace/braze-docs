...
Beim Erstellen Ihrer Nutzer-Journeys möchten Sie möglicherweise auch den effektivsten Canvas-Pfad testen. Mit [Experimentpfaden]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/experiment_step) können Sie mehrere Canvas-Pfade an jedem Schritt testen. Sie können auch die Verbindungen zwischen den Schritten als allgemeine Vorschau nutzen. Orangefarbene Verbindungen zeigen an, dass der vorherige Schritt Nutzer:innen sofort zum nächsten Schritt voranbringt.
