...
![Wenn eine Nutzer:in einen Artikel auf Ihrer E-Commerce-Website ansieht, ihn aber nicht in den Warenkorb legt, könnte der erste Schritt des Canvas eine Push-Benachrichtigung sein, die fragt, ob Interesse am Kauf des Artikels besteht. Sie könnten den Produktnamen mit dem Liquid-Tag „canvas_entry_properties“ referenzieren.]({% image_buster /assets/img/persistent_entry_properties/PEP1.png %}){: style="border:0;margin-left:15px;"}

Der zweite Schritt könnte eine weitere Push-Benachrichtigung senden, die die Nutzer:in zum Checkout auffordert, wenn der Artikel in den Warenkorb gelegt, aber noch nicht gekauft wurde. Sie können weiterhin die Entry-Eigenschaft `product_name` referenzieren, indem Sie {% raw %}`{{canvas_entry_properties.${product_name}}}`{% endraw %} verwenden.
