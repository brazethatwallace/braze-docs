...
{% tabs local %}
  ...
  {% tab Zielgruppe %}
    ...
    - Erstellen Sie Ihre Zielgruppe, indem Sie Segmente und Filter hinzufügen
  ...
  {% tab Canvas erstellen %}
    ...
    Erfahren Sie, wie Sie [Ihr Canvas erstellen](#step-2-build-your-canvas), indem Sie den Canvas-Builder verwenden.
  {% endtab %}
  {% tab Zusammenfassung %}
...
Bei der geplanten Zustellung treten Nutzer:innen nach einem Zeitplan ein, ähnlich wie Sie eine Campaign planen würden. Sie können Nutzer:innen in ein Canvas aufnehmen, sobald es gestartet wird, sie zu einem zukünftigen Zeitpunkt in Ihre Journey eintreten lassen oder auf wiederkehrender Basis (täglich, wöchentlich oder monatlich).
...
Bei der Zustellung in der Ortszeit bewertet Braze die Eintritts-Berechtigung zweimal: zuerst zur Samoa-Zeit (UTC+13) am geplanten Tag und dann zur Ortszeit der Nutzer:innen. Ein:e Nutzer:in muss beide Prüfungen bestehen, um in das Canvas einzutreten. Wenn Ihre Eingangsfilter relative Zeitfenster verwenden (z. B. „vor mehr als 2 Tagen“), ist der 24-Stunden-Zeitraum zum Zeitpunkt der ersten Prüfung möglicherweise noch nicht abgelaufen, was dazu führt, dass Nutzer:innen einen Tag zu spät eintreten. Um dies zu vermeiden, verwenden Sie ein breiteres Zeitfenster, z. B. mindestens zwei Tage. Weitere Details finden Sie unter [Wann bewertet Braze Nutzer:innen für die Zustellung in der Ortszeit?]({{site.baseurl}}/user_guide/messaging/campaigns/faq#when-does-braze-evaluate-users-for-local-time-zone-delivery)
{% endtab %}
...
Sie können weitere Aspekte des Canvas-Verhaltens über das Fenster **Eingangs-Zielgruppe** steuern, einschließlich Regeln für die Wiederberechtigung und Frequency-Capping-Einstellungen. Beachten Sie, dass die aktionsbasierte Zustellung für Canvas-Schritte mit In-App-Nachrichten nicht verfügbar ist.
...
Sollte das Fenster für die Wiederberechtigung kürzer sein als die maximale Dauer des Canvas, kann ein:e Nutzer:in erneut eintreten und Nachrichten von mehr als einem Schritt erhalten. Im Grenzfall, dass der Wiedereintritt einer Nutzerin oder eines Nutzers denselben Schritt wie der vorherige Eintritt erreicht, dedupliziert Braze die Nachrichten dieses Schritts.

Wenn ein:e Nutzer:in erneut in das Canvas eintritt, denselben Schritt wie beim vorherigen Eintritt erreicht und für eine In-App-Nachricht bei jedem Eintritt berechtigt ist, erhält die Person die Nachricht zweimal (abhängig von der In-App-Nachrichten-Priorität), solange sie eine Sitzung zweimal erneut öffnet.
{% enddetails %}
...
{% alert important %}
In Workspaces mit mehreren Apps wird die Berechtigung der Canvas-Eingangs-Zielgruppe (einschließlich Segmente und Filter) nur beim Eintritt der Nutzer:innen in das Canvas bewertet, nicht bei einzelnen Nachrichten-Schritten. Wenn Ihr Workspace mehrere Apps hat und Sie sicherstellen müssen, dass Nachrichten-Schritte nur Nutzer:innen einer bestimmten App ansprechen, verwenden Sie einen der folgenden Ansätze in jedem Nachrichten-Schritt:
- Aktivieren Sie **Zielgruppe beim Nachrichtenversand validieren** in den [Zustellungsvalidierungen]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/message_step#delivery-validations) des Nachrichten-Schritts und fügen Sie app-spezifische Segmente oder Filter hinzu.
...
Das Festlegen der [Ausstiegskriterien]({{site.baseurl}}/user_guide/messaging/canvas/create_a_canvas/exit_criteria) bestimmt, welche Nutzer:innen ein Canvas verlassen sollen. Wenn ein:e Nutzer:in das Ausnahme-Event ausführt oder den Segmenten und Filtern entspricht, erhält sie keine weiteren Nachrichten.

#### Zielpopulation berechnen {#calculating-target-population}

Im Abschnitt **Zielpopulation** können Sie eine Zusammenfassung Ihrer Zielgruppe einsehen, z. B. Ihre ausgewählten Segmente und zusätzlichen Filter, sowie eine Aufschlüsselung, wie viele Nutzer:innen pro Messaging-Kanal erreichbar sind. Um die genaue Anzahl der erreichbaren Nutzer:innen in Ihrer Zielgruppe anstelle der Standard-Schätzung zu berechnen, wählen Sie [Exakte Statistiken berechnen]({{site.baseurl}}/user_guide/audience/segments/measuring_segment_size#calculating-exact-statistics).
...
Setzen Sie dazu die **Abo-Einstellungen** auf „Nur an Nutzer:innen mit Opt-in senden“. Diese Option stellt sicher, dass nur Nutzer:innen mit Opt-in Ihre E-Mail erhalten, und Braze sendet Ihre Push-Nachrichten standardmäßig nur an Nutzer:innen, die für Push aktiviert sind.

Diese Abo-Einstellungen werden pro Schritt angewendet, d. h. sie haben keinen Einfluss auf die Eingangs-Zielgruppe. Diese Einstellung wird verwendet, um die Berechtigung einer Nutzerin oder eines Nutzers für den Empfang jedes Canvas-Schritts zu bewerten.
...
{: start="4"}
4. Fügen Sie nach dem User-Update-Schritt einen [Zielgruppenpfade]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/audience_paths)-Schritt hinzu. Fügen Sie in jeder Zielgruppe Filter basierend auf Ihrem angepassten Attribut hinzu, anstatt Prozentsätze für die Variantenverteilung zu verwenden.<br><br>Wenn Sie beispielsweise {% raw %}`{% random 10 %}`{% endraw %} verwendet haben, könnte eine Gruppe `lottery_number` **ist kleiner als 4** verwenden, eine andere **ist größer als 3 und kleiner als 7**, und eine dritte **ist größer als 6 und kleiner als 10**.
...
Wenn Ihre Canvas-Journey an Schritten zunimmt, ist die maximale Dauer die längstmögliche Zeit, die Nutzer:innen benötigen können, um diesen Canvas abzuschließen. Diese wird berechnet, indem die Verzögerungen und Trigger-Fenster jedes Schritts für jede Variante für den längsten Pfad addiert werden. Wenn Ihr Canvas beispielsweise einen Delay-Schritt mit einer Verzögerung von 3 Tagen und einen Nachrichten-Schritt hat, beträgt die maximale Dauer Ihres Canvas 3 Tage.
...
Sie können jeden Schritt in Ihrem Canvas-Workflow bearbeiten, indem Sie eine der Komponenten auswählen. Angenommen, Sie möchten Ihren ersten Schritt bearbeiten, eine [Delay]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/delay_step)-Komponente, in Ihrem Workflow auf einen bestimmten Tag. Wählen Sie den Schritt aus, um seine Einstellungen anzuzeigen, und passen Sie Ihre Verzögerung auf den 1. März an. Das bedeutet, dass Ihre Nutzer:innen am 1. März zum nächsten Schritt in Ihrem Canvas weitergeleitet werden.
...
Wenn eine einzelne Variante mehrere Verzweigungen mit derselben Zielgruppe und Sendezeit hat, garantiert Braze keine gleichmäßige Aufteilung auf diese Verzweigungen. Die Verteilung kann die zuerst erstellte Verzweigung bevorzugen. Für eine gleichmäßige Aufteilung verwenden Sie [zufällige Bucket-Nummer]({{site.baseurl}}/user_guide/messaging/ab_testing/concepts/random_bucket_numbers)-Filter für jede Verzweigung. Weitere Informationen finden Sie unter [Was passiert, wenn Zielgruppe und Sendezeit für einen Canvas mit einer Variante, aber mehreren Verzweigungen identisch sind?]({{site.baseurl}}/user_guide/messaging/canvas/faqs#what-happens-if-the-audience-and-send-time-are-identical-for-a-canvas-that-has-one-variant-but-multiple-branches).
...
In diesem Beispiel ist unser Canvas in zwei Varianten aufgeteilt. Variante 1 umfasst 70 % der Nutzer:innen. Die zweite Variante ist eine Kontrollgruppe mit den verbleibenden 30 % der Nutzer:innen.

![Eine Beispiel-Variante in einem Braze-Canvas, bei der 70 % zu „Variante 1“ gehen, die im ersten Schritt eine Verzögerung von 1 Tag hat und im zweiten Schritt eine Nachricht sendet. Die anderen 30 % gehen zu einer „Kontrollgruppe“, die keine weiteren Schritte hat.]({% image_buster /assets/img_archive/Canvas_Multivariate_Flow.png %})
