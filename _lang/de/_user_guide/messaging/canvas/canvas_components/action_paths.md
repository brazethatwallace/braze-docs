...
## Einen Aktionspfad erstellen {#creating-an-action-path}

Um einen Aktionspfad zu erstellen, fügen Sie eine Komponente zu Ihrem Canvas hinzu. Ziehen Sie die Komponente per Drag-and-Drop aus der Seitenleiste, oder wählen Sie den <i class="fas fa-plus-circle"></i> Plus-Button am unteren Rand eines Schritts und wählen Sie **Action Paths**.

### Aktionseinstellungen {#action-settings}

Legen Sie in den **Action Settings** das **Evaluation Window** fest, um zu bestimmen, wie lange Nutzer:innen im Schritt gehalten werden. Standardmäßig werden Nutzer:innen innerhalb eines Tages ausgewertet, aber Sie können dieses Fenster je nach Ihrem Canvas in Sekunden, Minuten, Stunden, Tagen und Wochen anpassen. Das maximale Auswertungsfenster für einen Aktionspfad beträgt 31 Tage.
...
Standardmäßig ist **Ranking** deaktiviert. Wenn Nutzer:innen den Aktionspfad betreten und das Trigger or triggern-Event einer beliebigen Aktionsgruppe ausführen, werden sie sofort durch die entsprechende Aktionsgruppe weitergeleitet – basierend auf der **ersten qualifizierenden Aktion**, die sie nach dem Betreten des Schritts ausführen. Wenn Nutzer:innen eine zweite Aktion ausführen, die einer anderen Aktionsgruppe entspricht, wechseln sie nicht den Pfad – die erste Aktion bestimmt ihre Route. Wenn Nutzer:innen kein Trigger or triggern-Event ausführen, werden sie am Ende des Auswertungszeitraums durch die Standardgruppe **Alle anderen** weitergeleitet.
...
Beachten Sie, dass Sie einen Aktionspfad Trigger or triggern or triggern können, wenn sich ein verschachteltes angepasstes Attribut-Objekt ändert, jedoch nicht für Arrays von verschachtelten angepassten Attributen oder Änderungen an Objekt-Array-Datentypen.

#### In-App-Nachrichten {#in-app-messages}

Beachten Sie: Wenn der Trigger or triggern der Aktionsgruppe das Starten einer Sitzung ist und der nächste Schritt eine In-App-Nachricht ist, müssen Nutzer:innen zwei Sitzungsstarts durchführen, um die In-App-Nachricht zu erhalten. Der erste Sitzungsstart ordnet die Nutzer:innen der Aktionsgruppe innerhalb des Aktionspfads zu, und der zweite Sitzungsstart triggert die In-App-Nachricht.

#### Beispiel für den Rangfolge-Status {#ranking-status-example}

Nehmen wir an, Sie haben einen Aktionspfad mit einem Auswertungszeitraum von einem Tag und zwei Aktionsgruppen: Gruppe 1 und Gruppe 2. Gruppe 1 hat das Trigger or triggern-Event „Sitzung starten“ und Gruppe 2 hat „Kauf tätigen“. Wenn **Ranking** aktiviert ist, werden alle Nutzer:innen im Aktionspfad für einen Tag „gehalten“. Am Ende des Tages werden Nutzer:innen, die eine Sitzung gestartet und einen Kauf getätigt haben, zum Pfad mit dem höchsten Rang weitergeleitet. In diesem Fall würden die Nutzer:innen zu Gruppe 1 weitergeleitet.

Wenn im vorherigen Beispiel **Ranking** deaktiviert ist und Nutzer:innen eines der Trigger or triggern-Events ausführen („Sitzung starten“ oder „Kauf tätigen“), werden sie basierend auf der Trigger or triggern-Aktion in der entsprechenden Aktionsgruppe weitergeleitet.
...
Fügen Sie einen oder mehrere Trigger or triggern hinzu, um Ihre Aktionsgruppen zu definieren. Hier können Sie eine Reihe von Trigger or triggern or triggern auswählen, z. B. ob Nutzer:innen:
...
- Einen angepassten Attributwert ändern
  - Dies umfasst auch das erstmalige Hinzufügen eines neuen Attributs mit einem Wert zu einem Kundenprofil or Nutzerprofil (wenn das Attribut zuvor nicht vorhanden war).
  - Attribut-Trigger or triggern sind für Array-Attribute nicht verfügbar.
- Ihren Abo-Status oder Abo-Gruppenstatus Update or aktualisieren or aktualisieren
- Mit einer Campaign oder Content-Card interagieren
- Einen Standort betreten
- Einen Geofence Trigger or triggern or triggern
- Eine eingehende Kurzmitteilungsdienst or SMS- oder WhatsApp-Nachricht senden

#### Trigger or triggern „E-Mail-Adresse hinzufügen“ {#add-an-email-address-trigger}

Der Aktionsgruppen-Trigger or triggern **Add an Email Address** wird ausgelöst, wenn eine E-Mail-Adresse während des **Evaluation Window** des Aktionspfads zu einem Kundenprofil or Nutzerprofil hinzugefügt oder aktualisiert wird. Dieses Verhalten entspricht anderen Profilaktualisierungs-Trigger or triggern or triggern: Nutzer:innen werden durch die Aktionsgruppe weitergeleitet, wenn die Profiländerung unter Ihrer Konfiguration qualifiziert ist, einschließlich aller Filter auf dem Trigger or triggern.
...
### Canvase mit erneuter Berechtigung {#canvases-with-re-eligibility}

Wenn Nutzer:innen einen Aktionspfad mehrfach betreten und gleichzeitig mehrere Einträge im Aktionspfad haben, variiert das erwartete Verhalten je nach **Ranking**-Status.

| Ranking-Status | Verhalten des Aktionspfads |
|---|--------------|
| **Deaktiviert** | Nutzer:innen können einen Aktionspfad mehr als einmal betreten. Diese Einträge werden im Aktionspfad gehalten, bis eine Trigger or triggern-Aktion oder ein Event erfasst wird. Wenn das Trigger or triggern-Event die Eigenschaftsfilter eines Eintrags nicht erfüllt (z. B. wenn eine [Kontextvariable]({{site.baseurl}}/user_guide/messaging/canvas/create_a_canvas/context_variables) nicht mit den Eigenschaftsfiltern des Triggers übereinstimmt), verbleibt der Eintrag im Aktionspfad. <br><br>Wenn das Trigger or triggern-Event mehr als einen Eintrag erfüllt, dedupliziert Braze nur diese Einträge und leitet den frühesten übereinstimmenden Eintrag sofort durch die entsprechende Aktionsgruppe weiter. |
...
{: .reset-td-br-1 .reset-td-br-2 aria-label="Canvase mit erneuter Berechtigung" }
