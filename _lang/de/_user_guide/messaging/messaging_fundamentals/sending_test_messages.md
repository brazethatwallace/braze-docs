...
{% alert note %}
Beachten Sie, dass Ihre Vorschau aufgrund von Unterschieden bei der Hardware möglicherweise nicht identisch mit dem endgültigen Rendering auf dem Gerät der Nutzer:innen ist.
{% endalert %}

### Checkliste für Tests {#test-checklist}

- Ist Ihre Banner-Campaign einer Platzierung zugewiesen?
- Werden die Bilder und Medien wie erwartet auf Ihren Zielgerätetypen und Bildschirmgrößen angezeigt und verhalten sich entsprechend?
...
{% alert important %}
Um einen Test an [Content-Testgruppen]({{site.baseurl}}/user_guide/administer/global/user_management/internal_groups#content-test-groups) oder einzelne Nutzer:innen zu senden, muss Push auf Ihren Testgeräten aktiviert sein und gültige Push-Token für die Testnutzer:innen registriert sein, bevor Sie den Test senden. iOS-Nutzer:innen müssen die von Braze gesendete Push-Benachrichtigung antippen, um die Test-Content-Card anzuzeigen. Dieses Verhalten gilt nur für Test-Content-Cards.
{% endalert %}

Test-Content-Cards werden über eine Push-Benachrichtigung zugestellt. Die Card ist in der Push-Payload verpackt, und das SDK extrahiert und speichert sie lokal zwischen, sobald der Push empfangen wird.
...
Test-Content-Cards laufen etwa fünf Minuten nach dem Senden ab.
...
Sie können Ihre Card während der Erstellung im Tab **Vorschau** in der Vorschau anzeigen. Dies sollte Ihnen helfen, sich vorzustellen, wie Ihre endgültige Nachricht aus der Perspektive Ihrer Nutzer:innen aussieht.

{% alert note %}
Im Tab **Vorschau** Ihres Composers entspricht die Ansicht Ihrer Nachricht möglicherweise nicht dem tatsächlichen Rendering auf dem Gerät der Nutzer:innen. Wir empfehlen, immer eine Testnachricht an ein Gerät zu senden, um sicherzustellen, dass Ihre Medien, Texte, Personalisierung und angepassten Attribute korrekt generiert werden.
{% endalert %}
...
- **Prüfen Sie die Content-Sicherheitsrichtlinien:** Wenn Ihre Organisation eine Content-Sicherheitsrichtlinie (CSP) oder interne IT-Sicherheitsregeln hat, kann die Richtlinie die Bild-Domain blockieren. Bestätigen Sie, dass die Domain der Bild-URL von Ihrer CSP zugelassen wird.
- **Verwenden Sie HTTPS:** Bild-URLs sollten `https://` anstelle von `http://` verwenden, um das Blockieren gemischter Inhalte in Browsern und Apps zu vermeiden.
...
Von dort aus können Sie Ihre Nachrichteneinstellungen und Inhalte überprüfen, um im Detail zu ermitteln, warum Nutzer:innen eine bestimmte Content Card nicht sehen können.
...
Am oberen Rand Ihres Gerätebildschirms wird eine Test-Push-Nachricht angezeigt.
...
Wenn Sie direkt auf die Push-Nachricht klicken und sie öffnen, gelangen Sie zu Ihrer App, wo Sie Ihren In-App-Nachrichtentest anzeigen können. Beachten Sie, dass diese In-App-Nachrichtentestfunktion darauf basiert, dass Nutzer:innen auf eine Test-Push-Benachrichtigung klicken, um die In-App-Nachricht auszulösen. Daher müssen Nutzer:innen berechtigt sein, Push-Benachrichtigungen in der entsprechenden App zu erhalten, damit die Test-Push-Benachrichtigung erfolgreich zugestellt wird.

### Vorschau

Sie können Ihre In-App-Nachricht während der Erstellung im Tab **Vorschau** in der Vorschau anzeigen. Dies sollte Ihnen helfen, sich vorzustellen, wie Ihre endgültige Nachricht aus der Perspektive Ihrer Nutzer:innen aussieht. Sie können eine Vorschau anzeigen, wie Ihre Nachricht für zufällige Nutzer:innen, bestimmte Nutzer:innen oder angepasste Nutzer:innen aussieht. Sie können Nachrichten auch für Mobilgeräte oder Tablets in der Vorschau anzeigen.

![Composer-Tab beim Erstellen einer In-App-Nachricht mit der Vorschau, wie die Nachricht aussehen wird. Es sind keine Nutzer:innen ausgewählt, sodass das im Body-Bereich hinzugefügte Liquid unverändert angezeigt wird.]({% image_buster /assets/img/in-app-message-preview.png %})

Braze bietet drei Generationen von In-App-Nachrichten. Sie können genau festlegen, an welche Geräte Ihre Nachrichten gesendet werden sollen, basierend darauf, welche Generation sie unterstützen.
...
{% alert warning %}
In der **Vorschau** entspricht die Ansicht Ihrer Nachricht möglicherweise nicht dem tatsächlichen Rendering auf dem Gerät der Nutzer:innen. Wir empfehlen immer, eine Testnachricht an ein Gerät zu senden, um sicherzustellen, dass Ihre Medien, Texte, Personalisierung und angepassten Attribute korrekt generiert werden.
{% endalert %}
...
Um bewährte Barrierefreiheitspraktiken zu unterstützen, scannt Braze automatisch den Inhalt von In-App-Nachrichten, die mit dem traditionellen HTML-Editor erstellt wurden, anhand von Barrierefreiheitsstandards. Dieser Scanner hilft dabei, Inhalte zu identifizieren, die möglicherweise nicht den Web Content Accessibility Guidelines ([WCAG](https://www.w3.org/WAI/standards-guidelines/wcag/)) entsprechen. WCAG ist eine Reihe international anerkannter technischer Standards, die vom World Wide Web Consortium (W3C) entwickelt wurden, um Web-Inhalte für Menschen mit Behinderungen zugänglicher zu machen.

![Ergebnisse des Barrierefreiheits-Scans]({% image_buster /assets/img/Accessibilty_Scanner_IAM.png %})

{% alert note %}
Der In-App-Nachricht-Barrierefreiheits-Scanner wird nur bei Nachrichten ausgeführt, die mit benutzerdefiniertem HTML erstellt wurden.
{% endalert %}

#### Funktionsweise {#how-it-works}

Der Scanner wird automatisch bei benutzerdefinierten HTML-Nachrichten ausgeführt und bewertet Ihre gesamte HTML-Nachricht anhand des vollständigen [WCAG 2.1 AA-Regelwerks](https://www.w3.org/WAI/WCAG22/quickref/?versions=2.1&currentsidebar=%23col_customize&levels=aaa). Für jedes gekennzeichnete Problem zeigt er:
...
#### Automatisiertes Testen der Barrierefreiheit verstehen {#understanding-automated-accessibility-testing}
...
Wenn Sie einen Fehler sehen, dass keine der ausgewählten Nutzer:innen passende Push-Token für Web-Push haben, überprüfen Sie, ob die Testnutzer:innen ein gültiges Push-Token für die ausgewählte Plattform registriert haben. Um ein Push-Token zu erhalten, müssen Nutzer:innen für den Empfang von Push-Benachrichtigungen für die App auf ihrem Gerät konfiguriert sein. Weitere Details finden Sie unter [Push-Aktivierung und Push-Abo-Status]({{site.baseurl}}/user_guide/channels/push/push_setup/push_subscription_states).

{% endtab %}
{% tab SMS/MMS und RCS %}

Nachdem Sie Ihre SMS-, MMS- oder RCS-Nachricht erstellt haben, können Sie eine Testnachricht an Ihr Telefon senden, um zu sehen, wie sie in Echtzeit aussieht. Die Empfänger:innen müssen der SMS-Abo-Gruppe angehören, die Sie beim Senden des Tests auswählen, eine gültige Telefonnummer haben und mindestens ein Land unter **Geografische Berechtigungen** ausgewählt haben. Weitere Details finden Sie unter [SMS-FAQs]({{site.baseurl}}/user_guide/channels/sms_mms_and_rcs/faqs#does-a-user-need-to-be-part-of-an-sms-subscription-group-to-receive-sms-test-messages).
...
2. Wählen Sie den Tab **Test** und wählen Sie mindestens eine Content-Testgruppe oder einzelne Nutzer:innen aus, die diese Testnachricht erhalten sollen.
3. Initiieren Sie ein Konversationsfenster, indem Sie eine WhatsApp-Nachricht an die Telefonnummer senden, die der Abo-Gruppe zugeordnet ist, die Sie für diese Nachricht verwenden. Die zugeordnete Telefonnummer wird im Hinweis auf dem Tab **Test** aufgeführt.
...
Wenn Sie Campaigns testen, die Nutzerdaten befüllen oder angepasste Event-Eigenschaften verwenden, müssen Sie zusätzliche oder abweichende Schritte durchführen.

### Campaigns testen, die mit Nutzerattributen personalisiert sind {#testing-campaigns-personalized-with-user-attributes}

Wenn Sie [Personalisierung]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/overview) in Ihrer Nachricht verwenden, müssen Sie zusätzliche Schritte unternehmen, um Ihre Campaign ordnungsgemäß in der Vorschau anzuzeigen und zu überprüfen, ob die Nutzerdaten den Content korrekt befüllen.

Beim Senden einer Testnachricht wählen Sie entweder die Option **Select Existing User** oder zeigen die Vorschau als **Custom User** an.
...
#### Benutzerdefinierte:n Nutzer:in auswählen {#selecting-a-custom-user}

Wenn Sie die Vorschau als benutzerdefinierte:n Nutzer:in anzeigen, geben Sie Text für die verschiedenen verfügbaren Felder zur Personalisierung ein, wie z. B. den Vornamen und angepasste Attribute. Auch hier können Sie Ihre eigene E-Mail-Adresse eingeben, um einen Test an Ihr Gerät zu senden.

![Benutzerdefinierte:r Nutzer:in]({% image_buster /assets/img_archive/personalized_testing_custom.png %})

#### Bestehende:n Nutzer:in anpassen {#customizing-an-existing-user}

Sie können einzelne Felder einer:eines zufälligen oder bestehenden Nutzer:in bearbeiten, um dynamischen Content in Ihrer Nachricht zu testen. Wählen Sie **Edit**, um die:den ausgewählte:n Nutzer:in in eine:n benutzerdefinierte:n Nutzer:in umzuwandeln, die:den Sie ändern können.
...
Das Testen von Campaigns, die mit [angepassten Event-Eigenschaften]({{site.baseurl}}/user_guide/data/activation/events/custom_events/custom_event_properties) personalisiert sind, unterscheidet sich geringfügig vom Testen anderer oben beschriebener Campaign-Typen.
...
{: start="2"}
2. Verwenden Sie die [aktionsbasierte Zustellung]({{site.baseurl}}/user_guide/messaging/campaigns/schedule_your_campaign/triggered_delivery), um die Campaign beim Eintreten des Events zuzustellen.

{% alert note %}
Wenn Sie eine iOS-Push-Campaign testen, müssen Sie die Verzögerung auf eine Minute einstellen, um sich Zeit zu geben, die App zu verlassen, da iOS keine Push-Benachrichtigungen für die aktuell geöffnete App zustellt. Andere Campaign-Typen können so eingestellt werden, dass sie sofort zugestellt werden.
{% endalert %}

![Zustellung der Testnachricht]({% image_buster /assets/img_archive/testeventproperties-delivery.png %})

{: start="3"}
3. Richten Sie das Targeting auf Ihre Nutzer:innen wie beim Testen üblich ein, indem Sie einen Testfilter verwenden oder Ihre eigene E-Mail-Adresse als Ziel festlegen, und stellen Sie die Campaign fertig.
...
Alternativ können Sie die Campaign auch testen, indem Sie sich selbst eine personalisierte Testnachricht senden, sofern Sie benutzerdefinierte Nutzer-IDs gespeichert haben.

1. Verfassen Sie den Text für Ihre Campaign.
2. Wählen Sie den Tab **Test** und dann **Customized User**.
3. Fügen Sie die angepasste Event-Eigenschaft unten auf der Seite hinzu und geben Sie Ihre Nutzer-ID oder E-Mail-Adresse im oberen Feld ein.
4. Wählen Sie **Send Test**, um eine mit der Eigenschaft personalisierte Nachricht zu erhalten.

![Testen mit benutzerdefiniertem Nutzer]({% image_buster /assets/img_archive/testeventproperties-customuser.png %})
...
- Wenn Sie das Braze-[Einstellungscenter]({{site.baseurl}}/user_guide/audience/subscription_preferences/preference_center) aus Testnachrichten aufrufen, wird der Button **Einstellungen speichern** deaktiviert. Liquid-Tags des Einstellungscenters werden möglicherweise auch nicht zu gültigen Links aufgelöst. Dies ist erwartetes Verhalten. Informationen zum End-to-End-Test finden Sie unter [Einstellungscenter testen]({{site.baseurl}}/user_guide/audience/subscription_preferences/preference_center/dnd_preference_center#testing-preference-centers).
...
- Zum Testen von Abmeldelinks in E-Mails stellen Sie sicher, dass die E-Mail-Adresse Ihrer Testnutzer:in im entsprechenden Workspace vorhanden ist.
- Der `List-Unsubscribe`-Header ist nicht in E-Mails enthalten, die über die Testnachrichten-Funktion gesendet werden.
- E-Mails, die an Nutzer:innen der Seed-Gruppe gesendet werden, aktualisieren nicht die Liste der empfangenen Campaigns im Nutzerprofil und erhöhen nicht die Sendungen in den Dashboard-Analytics.

## Fehlerbehebung {#troubleshooting}

### In-App-Nachrichten {#in-app-messages}

Wenn Ihre In-App-Nachricht-Campaign nicht durch eine Push-Campaign ausgelöst wird, überprüfen Sie die Segmentierung der In-App-Campaign, um sicherzustellen, dass die Nutzer:innen die Zielgruppe erfüllen, **bevor** sie die Push-Nachricht erhalten.

Bei Testnachrichten auf Android und iOS werden In-App-Nachrichten, die das On-Click-Verhalten **Push-Berechtigung anfordern** verwenden, möglicherweise auf einigen Geräten nicht angezeigt. Als Workaround:
- **Android:** Die Geräte müssen Android 13 und unser Android SDK in Version 21.0.0 verwenden. Ein weiterer Grund kann sein, dass auf dem Gerät, auf dem die In-App-Nachricht angezeigt wird, bereits eine Aufforderung auf Systemebene vorhanden ist. Möglicherweise haben Sie **Nicht erneut fragen** ausgewählt, sodass Sie die App neu installieren müssen, um die Benachrichtigungsberechtigungen vor dem erneuten Testen zurückzusetzen.
