...
- **Einschränkungen:** Derzeit werden die folgenden Features nicht unterstützt: Canvas-Integration, API-getriggerte und aktionsbasierte Campaigns, Connected-Content, Aktionscodes, nutzergesteuerte Schließungen und `catalog_items` mit dem [`:rerender`-Tag]({{site.baseurl}}/user_guide/data/activation/catalogs/use/#using-liquid).
