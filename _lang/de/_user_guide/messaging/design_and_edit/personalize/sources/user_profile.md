...
Event-Eigenschaften sind in aktionsbasierten Kampagnen und im ersten Schritt eines aktionsbasierten Canvas verfügbar. Weitere Informationen finden Sie unter [Angepasste Events]({{site.baseurl}}/user_guide/data/activation/events/custom_events/).

## API-Trigger-Eigenschaften

{% raw %}
Für Kampagnen und Canvases, die über die API getriggert werden, können Sie zusätzliche Daten über das Trigger-Eigenschaften-Objekt übergeben. Referenzieren Sie diese Werte mit `{{api_trigger_properties.${property_name}}}`.
