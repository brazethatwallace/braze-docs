...
Diese Werte können im Tab **Einstellungen** beim Erstellen einer Push-Nachricht eingegeben werden. Wählen Sie **Alarmoptionen** und wählen Sie einen Alarm-Dictionary-Schlüssel, damit der Schlüssel automatisch in einem neuen Schlüssel-Wert-Eintrag ausgefüllt wird.
...
Nach dem Start der Kampagne sollten Sie überprüfen, dass Sie keine sichtbare Push-Benachrichtigung auf Ihrem Testgerät erhalten haben.

{% alert note %}
Das iOS-Betriebssystem kann [Benachrichtigungen drosseln]({{site.baseurl}}/developer_guide/platform_integration_guides/swift/push_notifications/silent_push_notifications/#ios-silent-notifications-limitations) für einige Features (Uninstall-Tracking, Geofences und Push-Storys). Beachten Sie, dass wenn Sie Schwierigkeiten mit diesen Features haben, die Drosselung stiller Benachrichtigungen durch iOS die Ursache sein könnte.
{% endalert %}

## In-App-Nachrichten

Sie können ein Schlüssel-Wert-Paar zu einer In-App-Nachricht im [traditionellen Editor]({{site.baseurl}}/user_guide/channels/in_app_messages/traditional/) hinzufügen, indem Sie den Tab **Einstellungen** auswählen, **Neues Paar hinzufügen** wählen und dann Ihre Schlüssel-Wert-Paare angeben.
