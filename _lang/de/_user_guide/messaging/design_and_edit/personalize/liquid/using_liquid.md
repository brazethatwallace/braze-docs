...
## Unterstützte Werte zum Ersetzen {#supported-values-to-substitute}
...
4. **Variablennamen müssen ASCII-Zeichen verwenden:** Liquid-Variablennamen (erstellt mit `assign` oder `capture`) unterstützen nur ASCII-Buchstaben, Ziffern und Unterstriche. Braze-Personalisierungsattributnamen (innerhalb von `custom_attribute.${...}` oder `event_properties.${...}`) können Nicht-ASCII-Zeichen enthalten.
...
1. Es über den HTML-Editor einzufügen, nicht über den klassischen Editor. Der klassische Editor kann Liquid als Klartext interpretieren. Beispielsweise würde Liquid als {% raw %}`Hi {{ ${first_name} }}, thanks for using our service!`{% endraw %} angezeigt, anstatt den Vornamen der Nutzerin oder des Nutzers einzusetzen.
2. Liquid-Code nur innerhalb des `<body>`-Tags zu platzieren. Eine Platzierung außerhalb dieses Tags kann bei der Zustellung zu inkonsistenter Darstellung führen.
