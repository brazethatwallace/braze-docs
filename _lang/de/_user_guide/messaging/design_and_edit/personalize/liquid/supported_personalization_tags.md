...
Da es eine große Bandbreite an Mobilfunkanbietern, Modellnamen und Betriebssystemen gibt, empfehlen wir, jede Logik, die bedingt von einem dieser Werte abhängt, gründlich zu testen. Diese Werte sind `null`, wenn sie auf einem bestimmten Gerät nicht verfügbar sind.
...
In diesem Fall gibt es zwei Optionen, die besser funktionieren könnten als das Setzen eines Standardwerts:

1. Die Nachricht für Kund:innen mit niedrigen, null- und leeren Kontoständen abbrechen.
...
2. Eine völlig andere Nachricht an diese Kund:innen senden, wie zum Beispiel:
...
Solange der Content-Block am Anfang Ihrer Nachricht steht, bezieht sich die Variable jedes Mal, wenn sie als Objekt in Ihre Nachricht eingefügt wird, auf Ihr gewähltes angepasstes Attribut.
{% endalert %}

### Anwendungsfall

Nehmen wir an, Sie ermöglichen es Ihren Kund:innen, ihre Rewards-Punkte gegen Preise einzulösen, nachdem sie 100 Rewards-Punkte gesammelt haben. Sie möchten also nur Kund:innen ansprechen, deren Punktestand größer oder gleich 100 wäre, wenn sie diesen zusätzlichen Kauf tätigen würden:
...
Nehmen wir an, Sie haben einen Sale auf Nike-Sneaker und möchten Kund:innen ansprechen, die Interesse an Nike gezeigt haben. Sie haben ein Array von Produktmarken, die auf dem Profil jeder Kund:in angesehen wurden. Dieses Array kann bis zu 25 Produktmarken enthalten, aber Sie möchten nur Kund:innen ansprechen, die ein Nike-Produkt als eine ihrer 5 letzten Produktansichten angesehen haben.
...
Mit [Whitespace-Kontrolle](https://shopify.github.io/liquid/basics/whitespace/) können Sie Leerzeichen um Ihre Tags entfernen, was Ihnen hilft, die Liquid-Ausgabe weiter zu kontrollieren.
