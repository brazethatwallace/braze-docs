---
nav_title: Nutzerprofildaten abrufen
...
# Nutzerprofildaten in Connected-Content-Aufrufen abrufen

> Diese Seite beschreibt, wie Sie Nutzerprofile in Ihre Connected-Content-Aufrufe einbinden und welche Best Practices für Liquid-Templating gelten.

## Voraussetzungen

Wenn eine Connected-Content-Antwort Nutzerprofilfelder enthält (innerhalb eines Liquid-Personalisierungs-Tags), müssen diese Werte zuvor in der Nachricht mit Liquid definiert werden – und zwar vor dem Connected-Content-Aufruf –, damit der Liquid-Passback korrekt gerendert wird. Ebenso muss das `:rerender`-Flag in der Anfrage enthalten sein. Beachten Sie, dass das `:rerender`-Flag nur eine Ebene tief wirkt, d. h. es wird nicht auf verschachtelte Connected-Content-Tags angewendet.

## Liquid-Templating in Connected-Content-Aufrufen
...
### `json_escape` mit Liquid-Tags verwenden, die das JSON-Format beschädigen könnten
...
Wenn beispielsweise die `message`-Event-Eigenschaft im folgenden Beispiel Zeichen enthält, die das JSON-Format beschädigen könnten, fügen Sie den `json_escape`-Filter wie in diesem Beispiel hinzu:
