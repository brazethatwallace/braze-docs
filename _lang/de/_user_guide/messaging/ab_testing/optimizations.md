...
{% alert note %}
Bei Gewinnervarianten und personalisierten Varianten führt Braze beim zweiten Versand erneut eine Berechtigungsprüfung durch. Nutzer:innen, die beim ersten Versand nicht im Zielsegment waren (oder anderweitig nicht erreichbar waren), können später hinzukommen; Nutzer:innen, die das Segment verlassen haben, erhalten die Folgenachricht möglicherweise nicht mehr. Planen Sie Ihr Segment und Ihren Zeitplan so, dass die gewünschte Zielgruppe bei beiden Versendungen berechtigt ist.
{% endalert %}
