...
# A/B-Tests mit Gewinnervariante oder personalisierten Varianten optimieren {#optimize-ab-tests-with-winning-variant-or-personalized-variants}

> Erfahren Sie, wie Sie die Gewinnervariante oder personalisierte Variante bei der Erstellung von multivariaten und A/B-Tests verwenden können.

Bei der [Erstellung eines A/B-Tests]({{site.baseurl}}/user_guide/messaging/ab_testing/create_tests/) für E-Mail-, Push-, Webhook-, SMS- und WhatsApp-Campaigns, die für einen einmaligen Versand geplant sind, können Sie eine Optimierung auswählen. Es gibt zwei Optimierungsmöglichkeiten: **Gewinnervariante** und **Personalisierte Variante**.
...
{% alert tip %}
Optimierungen finden Sie im Schritt **Target Audiences** bei der Kampagnenerstellung unter **A/B Testing**.
{% endalert %}
...
| Feld | Beschreibung |
| --- | --- |
| Determine Winning Variant | Die Metrik, für die optimiert werden soll. Wählen Sie zwischen *Unique Opens* oder *Clicks* für E-Mail, *Opens* für Push oder *Primary Conversion Rate* für alle Kanäle. Die Auswahl von *Opens* oder *Clicks* zur Bestimmung der Gewinnervariante hat keinen Einfluss auf die [Konversions-Events]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/conversion_events/) Ihrer Campaign. <br><br>Beachten Sie: Wenn Sie eine Kontrollgruppe verwenden, können Nutzer:innen in der Kontrollgruppe keine *Opens* oder *Clicks* ausführen, sodass die Performance der Kontrollgruppe garantiert `0` beträgt. Daher kann die Kontrollgruppe den A/B-Test nicht gewinnen. Dennoch kann es sinnvoll sein, eine Kontrollgruppe zu verwenden, um andere Metriken für Nutzer:innen zu verfolgen, die keine Nachricht erhalten. |
| Winning Variant Send Time | Datum und Uhrzeit, zu der die Gewinnervariante gesendet wird. |
| If No Winning Variant Can Be Determined | Was passiert, wenn keine Variante mit statistisch signifikantem Vorsprung gewinnt. Wählen Sie zwischen dem Senden der leistungsstärksten Variante oder dem Beenden des Tests ohne weitere Nachrichten. |
...
| Feld | Beschreibung |
| --- | --- |
| Determine Personalized Variant | Die Metrik, für die optimiert werden soll. Wählen Sie zwischen *Unique Opens* oder *Clicks* für E-Mail, *Opens* für Push oder *Primary Conversion Rate* für alle Kanäle. Die Auswahl von *Opens* oder *Clicks* zur Bestimmung der personalisierten Variante hat keinen Einfluss auf die [Konversions-Events]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/conversion_events/) Ihrer Campaign. <br><br>Beachten Sie: Wenn Sie eine Kontrollgruppe verwenden, können Nutzer:innen in der Kontrollgruppe keine *Opens* oder *Clicks* ausführen, sodass die Performance der Kontrollgruppe garantiert `0` beträgt. Daher kann die Kontrollgruppe den A/B-Test nicht gewinnen. Dennoch kann es sinnvoll sein, eine Kontrollgruppe zu verwenden, um andere Metriken für Nutzer:innen zu verfolgen, die keine Nachricht erhalten. |
| Personalized Variant Send Time | Datum und Uhrzeit, zu der die personalisierte Variante gesendet wird. |
| If No Personalized Variant Can Be Determined | Was passiert, wenn keine personalisierten Varianten gefunden werden. Wählen Sie zwischen dem Senden der Gewinnervariante oder dem Beenden des Tests ohne weitere Nachrichten. |
