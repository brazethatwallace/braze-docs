...
page_order: 2
page_type: reference
description: "Dieser Artikel behandelt das Konzept der zufälligen Bucket-Nummern und wie Sie diese nutzen können, um Varianten und Kontrollgruppen zu erstellen."
page_type: reference
tool:
  - Campaign
  - Canvas
...
> Eine zufällige Bucket-Nummer ist ein Nutzerattribut, mit dem sich gleichmäßig verteilte Segmente zufälliger Nutzer:innen erstellen lassen.

## Übersicht

Wenn ein Nutzerprofil in Braze erstellt wird, erhält diese Person automatisch eine zufällige Bucket-Nummer zwischen 0 und 9999 (einschließlich). Sie können diese Segmente nutzen, um die Wirksamkeit mehrerer Kampagnen oder Canvases auf Nutzergruppen über einen bestimmten Zeitraum zu testen.
...
{% alert note %}
Wenn eine Person gelöscht und neu erstellt wird, erhält sie eine andere zufällige Bucket-Nummer, da sie als neue:r Nutzer:in betrachtet wird.
{% endalert %}

Wenn Sie eine Globale Kontrollgruppe eingerichtet haben und zufällige Bucket-Nummern für andere Anwendungsfälle nutzen möchten, lesen Sie [Worauf Sie achten sollten]({{site.baseurl}}/user_guide/audience/global_control_group#things-to-watch-for).

### Wann Sie zufällige Bucket-Nummern verwenden sollten
...
## Segmente mit zufälligen Bucket-Nummern erstellen
...
3. Geben Sie im Abschnitt **Zielgruppe** Ihrer Kampagne oder Ihres Canvas die Einstellungen für zufällige Buckets an. So kann Braze Nutzer:innen automatisch den entsprechenden Buckets basierend auf den definierten Prozentsätzen zuweisen.
