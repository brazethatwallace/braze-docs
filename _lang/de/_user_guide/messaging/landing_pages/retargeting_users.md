...
- **Über einen Nachrichten-Trigger:** Sie können einen Nachrichten-Trigger einrichten, um Nutzer:innen nach dem Absenden des Formulars automatisch eine Nachricht zu senden oder sie in einen Canvas aufzunehmen.

{% tabs local %}
{% tab Über ein Segment %}
Wenn Sie [ein Segment erstellen]({{site.baseurl}}/user_guide/audience/segments/creating_a_segment), wählen Sie unter der Gruppe „Retargeting“ die Option **Submitted form on Landing Page** aus.
...
{% tab Über einen Nachrichten-Trigger %}
Wenn Sie die Zustellungsoption für Ihre [Campaign]({{site.baseurl}}/user_guide/messaging/campaigns) oder Ihren [Canvas]({{site.baseurl}}/user_guide/messaging/canvas) auswählen, wählen Sie **Action Based Delivery** und dann **Submitted Landing Page form** aus.
