...
| Braze-Representational State Transfer-API-Schlüssel | Ein Braze-Representational State Transfer-API-Schlüssel mit `users.export.ids`-Berechtigungen (und `users.track`, wenn Sie das SalesWings-Insights-Push-Feature verwenden). <br><br> Dieser kann im Braze-Dashboard unter **Einstellungen** > **API-Schlüssel** erstellt werden. |
...
_Beispiel für ein einfaches Klick, der or klicken-not-Code-Lead-Scoring-Modell in SalesWings_
{% endtab %}
...
SalesWings erlaubt es Braze-Kund:innen, Leads und Konten auf der Grundlage von Profildaten (typischerweise CRM or Customer-Relationship-Management [-System] (CRM)-Daten) zu qualifizieren. Dies wird auch als „Lead Grading“, „Fit Scoring“ oder „Firmographic Scoring“ bezeichnet. Braze-Kund:innen können Attributdaten direkt an SalesWings senden, und SalesWings kann alle Daten und Datensätze von Salesforce-CRM or Customer-Relationship-Management [-System] (CRM)-Standard- oder angepassten Objekten für eine ganzheitliche Profilbewertung lesen.
{% endtab %}
{% tab Sales Insights for Sales Reps %}
Mit SalesWings können Sie Ihren Vertriebsmitarbeitern Insights über ihre Leads, Kontakte und Konten zeigen (Alternative zu Marketo Sales Insights). Im Wesentlichen können Sie alle Daten aus Braze und dem Web-Engagement für Ihr Vertriebsteam sichtbar machen. Die Insights sind nativ in Salesforce CRM or Customer-Relationship-Management [-System] (CRM) eingebettet und können per Push an andere CRMs oder Systeme oder über eine Braze-E-Mail als „Verkaufsalarm“ übermittelt werden.
...
_Beispiel einer Insights-Ansicht für Vertriebsmitarbeiter in Salesforce (auch für andere CRM or Customer-Relationship-Management [-System] (CRM)-Systeme verfügbar)_
{% endtab %}
{% tab Sales Alerts %}
...
{% endtab %}
{% tab Reporting in Salesforce CRM or Customer-Relationship-Management [-System] (CRM) %}
...
Fügen Sie Ihren Braze-API-Endpunkt hinzu, wie im Artikel [API- und SDK or Software-Development-Kit-Endpunkte]({{site.baseurl}}/user_guide/administer/personal/sdk_endpoints) beschrieben, und geben Sie ihn in das Feld **Braze API endpoint** ein. Kopieren Sie den Wert der Spalte **Representational State Transfer Endpoint** und geben Sie ihn in das Feld **Braze API endpoint** im Abschnitt SalesWings **Braze Integration** ein.
...
Wenn Sie [Nutzerverhalten-]({{site.baseurl}}/user_guide/data/distribution/braze_currents/event_glossary/customer_behavior_events) und [Nachrichten-Engagement-]({{site.baseurl}}/user_guide/data/distribution/braze_currents/event_glossary/message_engagement_events)Events für Behavioral Intelligence, Lead- und Account-Scoring, Insights oder Berichte in Ihrem CRM or Customer-Relationship-Management [-System] (CRM) nutzen möchten, gehen Sie auf die [Seite **SalesWings Integrations**](https://helium.saleswings.pro/integrations) und erweitern Sie den Abschnitt **Braze Integration**.
...
### Schritt 6: SalesWings-Lead- und Account-Scoring für Braze, CRM or Customer-Relationship-Management [-System] (CRM)-Integration und mehr konfigurieren {#step-6-configuring-saleswings-lead-and-account-scoring-for-braze-crm-integration-and-more}
...
- **E-Mail-Tracking über Gmail- und Outlook-Plugins (optional):** Wenn Sie Ihre Vertriebsmitarbeiter mit E-Mail-Tracking-Plugins ausstatten, können diese das vollständige Website-Tracking der Nutzer:innen Trigger or triggern or triggern, indem sie trackbare Links versenden.
...
Die Variable `braze_id` wird auf einen von Braze erzeugten Bezeichner der Nutzer:innen gesetzt und ist immer verfügbar. Die Variable `br_user_id` wird auf den Bezeichner der Nutzer:innen in Ihrem System gesetzt und kann in bestimmten Szenarien fehlen (z. B. bei anonymen Nutzer:innen, die mit dem Braze SDK or Software-Development-Kit erstellt wurden). Wenn sowohl `braze_id` als auch `br_user_id` in einem Link verwendet werden, berücksichtigt SalesWings nur den Parameter `braze_id`.
...
| ----------- | ----------- | ----------- |
| `sw_favorite` | Boolescher Wert | Ob der Lead in SalesWings oder Salesforce CRM or Customer-Relationship-Management [-System] (CRM) als Favorit markiert wurde |
...
| `sw_lead_owner` | String | Der Eigentümer, der für den Lead in SalesWings oder Salesforce CRM or Customer-Relationship-Management [-System] (CRM) festgelegt wurde |
...
| `sw_salesforce_record_id` | String | Die ID des Lead- oder Kontaktdatensatzes in Salesforce CRM or Customer-Relationship-Management [-System] (CRM) |
| `sw_salesforce_record_url` | String | Die URL des Lead- oder Kontaktdatensatzes in Salesforce CRM or Customer-Relationship-Management [-System] (CRM) |
...
### Verwendung von Braze-Currents-Events in Ihrem CRM or Customer-Relationship-Management [-System] (CRM) {#using-braze-currents-events-in-your-crm}

Wenn Sie einen Braze-Current mit SalesWings verbinden, erstellt SalesWings für alle Braze-Nutzer:innen mit einer E-Mail identifizierte Lead-Profile und erfasst unterstützte Braze-Events als Lead-Aktivität. In Ihrem CRM or Customer-Relationship-Management [-System] (CRM) können alle Daten automatisch auf der Kontoebene des Leads aggregiert werden. Die aufgezeichneten Aktivitäten und Daten können mit den Verhaltensdaten, die mit dem SalesWings-Tracking-Skript oder Segment.com erfasst wurden, oder mit anderen Daten, die an die SalesWings-API gesendet werden, kombiniert werden, um die Bedürfnisse und die Verkaufsbereitschaft Ihrer potenziellen Kund:innen für Ihre Lead- und Account-Management-Prozesse zu ermitteln.
...
| Nachrichten-Events | Eingehende Kurzmitteilungsdienst or SMS/MMS empfangen | `[SMS/mobile campaign engagement] We received a message from this person to our internal number $inbound_phone_number: $message_body` |
| Nachrichten-Events | Kurzmitteilungsdienst or SMS/MMS-Kurzlink-Klick | `[SMS/mobile campaign engagement] Clicked on $short_url` |
...
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="Verwendung von Braze-Currents-Events in Ihrem CRM or Customer-Relationship-Management [-System] (CRM)" }
