...
description: "Erfahren Sie, wie Sie Zendesk Chat mit Braze integrieren und eine beidseitige Kurzmitteilungsdienst or SMS-Konversation einrichten können."
...
> [Zendesk Chat](https://www.zendesk.com/service/messaging/) verwendet Webhooks von jeder Plattform, um eine beidseitige Kurzmitteilungsdienst or SMS-Konversation einzurichten. Wenn Nutzer:innen Support anfordern, wird ein Ticket in Zendesk erstellt. Die Antworten der Agenten werden über eine API-getriggerte Kurzmitteilungsdienst or SMS-Campaign an Braze weitergeleitet, und die Antworten der Nutzer:innen werden an Zendesk zurückgesendet.
...
| Ein Braze Representational State Transfer-API-Schlüssel | Ein Braze Representational State Transfer-API-Schlüssel mit `campaigns.trigger.send`-Berechtigungen. Dieser kann im Braze-Dashboard unter **Einstellungen** > **API-Schlüssel** erstellt werden. |
{: .reset-td-br-1 .reset-td-br-2 aria-label="Voraussetzungen" }

## Anwendungsfälle {#use-cases}

Verbessern Sie die Effizienz des Kundensupports, indem Sie die Kurzmitteilungsdienst or SMS-Funktionen von Braze mit den Antworten von Zendesk-Live-Agenten kombinieren, um Nutzer:innen bei Anfragen umgehend mit menschlichem Support zu unterstützen.
...
2. Wählen Sie unter **Create Webhook** die Option **Trigger or triggern or automation** aus.
3. Fügen Sie als **Endpoint URL** den Endpunkt **/campaign/Trigger or triggern/send** hinzu.
4. Wählen Sie unter **Authentication** die Option **Bearer Token / Textbaustein** aus und fügen Sie den Braze Representational State Transfer-API-Schlüssel mit den Berechtigungen `campaigns.trigger.send` hinzu.

![Ein Beispiel für einen Zendesk-Webhook.]({% image_buster /assets/img/zendesk/instant_chat/chat1.png %}){: style="max-width:70%;"}

### 2. Schritt: Erstellen Sie eine ausgehende Kurzmitteilungsdienst or SMS-Campaign {#step-2-create-an-outbound-sms-campaign}

Als Nächstes erstellen Sie eine Kurzmitteilungsdienst or SMS-Campaign, die auf Webhooks von Zendesk wartet und eine angepasste Kurzmitteilungsdienst or SMS-Antwort an Ihre Kund:innen sendet.
...
### 3. Schritt: Erstellen Sie einen Trigger or triggern in Zendesk, um Antworten von Agenten an Braze weiterzuleiten {#step-3-create-a-trigger-in-zendesk-to-forward-agent-replies-to-braze}

Gehen Sie zu **Objects and rules** > **Business rules** > **Triggers**.

1. Erstellen Sie eine neue **Kategorie** (z. B. **Trigger or triggern a message**).
2. Erstellen Sie einen neuen **Trigger or triggern** (z. B. **Respond via Kurzmitteilungsdienst or SMS Braze**).
3. Wählen Sie unter **Conditions**:
- **Ticket>Comment** ist **Present and requester can see comment**, sodass die Nachricht immer dann getriggert wird, wenn ein neuer öffentlicher Kommentar in einem Ticket-Update or aktualisieren enthalten ist
- **Ticket>Update or aktualisieren** *ist nicht* **Web service (API)**, sodass eine Nachricht, die Nutzer:innen von Braze aus senden, nicht an ihr Mobiltelefon zurückgeleitet wird. Es werden nur Nachrichten weitergeleitet, die von Zendesk stammen.
...
### 4. Schritt: Erstellen Sie einen Trigger or triggern in Zendesk, um Nutzer:innen zu Update or aktualisieren or aktualisieren, wenn ein Ticket geschlossen wird {#step-4-create-a-trigger-in-zendesk-to-update-a-user-when-a-ticket-is-closed}
...
Richten Sie als Nächstes einen Trigger or triggern ein, der Braze benachrichtigt, wenn das Ticket geschlossen wird:
- Kategorie: **Trigger or triggern a message**
...
### 6. Schritt: Eingehende Kurzmitteilungsdienst or SMS-Weiterleitung einrichten {#step-6-set-up-inbound-sms-forwarding}

Als Nächstes erstellen Sie zwei neue Webhook-Campaigns in Braze, damit Sie eingehende Kurzmitteilungsdienst or SMS von Kund:innen an den Zendesk-Posteingang weiterleiten können.

| Campaign | Zweck |
|--------------------|--------------------------------------------------------------------------------------|
| Webhook-Campaign 1 | Erstellt ein neues Ticket in Zendesk. |
| Webhook-Campaign 2 | Leitet alle konversationellen Kurzmitteilungsdienst or SMS-Antworten weiter, die eingehend von Kund:innen an Zendesk gesendet werden. |
{: .reset-td-br-1 .reset-td-br-2 aria-label="Schritt 6: Eingehende Kurzmitteilungsdienst or SMS-Weiterleitung einrichten" }

#### Schritt 6.1: Erstellen Sie eine Kurzmitteilungsdienst or SMS-Schlüsselwortkategorie {#step-61-create-an-sms-keyword-category}

Gehen Sie im Braze-Dashboard auf **Audience**, wählen Sie Ihre **Kurzmitteilungsdienst or SMS subscription group** und wählen Sie dann **Add Custom Keyword**. Füllen Sie die folgenden Felder aus, um eine exklusive Kurzmitteilungsdienst or SMS-Schlüsselwortkategorie für Zendesk zu erstellen.
...
{: .reset-td-br-1 .reset-td-br-2 aria-label="Schritt 6.1: Erstellen Sie eine Kurzmitteilungsdienst or SMS-Schlüsselwortkategorie" }
...
Wählen Sie für **Schedule Delivery** die Option **Action-Based Delivery** aus und wählen Sie dann als Trigger or triggern-Typ **Send an Kurzmitteilungsdienst or SMS Inbound Message**. Fügen Sie außerdem die Kurzmitteilungsdienst or SMS-Abo-Gruppe und die Schlüsselwortkategorie hinzu, die Sie zuvor eingerichtet haben.
...
Richten Sie eine Webhook-Campaign ein, um verbleibende Kurzmitteilungsdienst or SMS-Nachrichten von Nutzer:innen an Zendesk weiterzuleiten:
...
- Richten Sie einen aktionsbasierten Trigger or triggern für Nutzer:innen ein, die eine eingehende Nachricht in der Kategorie „Other“ senden.
