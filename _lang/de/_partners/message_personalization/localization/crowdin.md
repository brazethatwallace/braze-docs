...
| **Braze-Mehrsprachigkeitseinstellungen** | Locales müssen in Ihrem Braze-Dashboard unter **Einstellungen** > **Einstellungen für die Lokalisierung** konfiguriert sein. |
{: .reset-td-br-1 .reset-td-br-2 role="presentation" }

## Integration von Braze Campaigns & Canvas {#braze-campaigns-canvas-integration}

Wenn Sie Inhalte in aktiven Nachrichten lokalisieren, verwenden Sie die [Braze Campaigns & Canvas App](https://store.crowdin.com/braze-content-translation), um übersetzbare Strings aus Ihren Campaign- und Canvas-Entwürfen mit der Braze-Mehrsprachigkeitsunterstützung zu synchronisieren.

Eine Video-Anleitung finden Sie unter [Braze Campaigns & Canvas Integration](https://youtu.be/ahG1ET4VRKA).

### 1. Schritt: Mehrsprachigkeitseinstellungen in Braze einrichten {#step-1-set-up-multi-language-settings-in-braze}

Bevor Sie Crowdin verbinden, fügen Sie Ihre Zielsprachen in Braze hinzu.

1. Gehen Sie in Braze zu **Einstellungen** > **Einstellungen für die Lokalisierung**.
...
### 2. Schritt: Das Braze-Projekt in Crowdin einrichten {#step-2-set-up-the-braze-project-in-crowdin}
...
4. Um die Integration zu öffnen, gehen Sie zu Ihrem Projekt **Integrations** > **Braze Campaigns & Canvas**.
...
- **Braze REST API key:** Erstellen Sie diesen in Braze unter **Settings** > **APIs and Identifiers** > **API Keys**. Erteilen Sie die Berechtigungen, die diese Integration benötigt (Campaigns, Canvas, Content Blocks und angepasste Attribute).
- **Braze REST endpoint:** Geben Sie die URL für Ihre Braze-Instanz ein (zum Beispiel `https://rest.iad-03.braze.com`). Weitere Informationen finden Sie unter [REST-API-Endpunkte]({{site.baseurl}}/api/basics/#endpoints).

![Braze-Seite „REST API Keys“ mit „Create API Key“ und dem Kopiersteuerelement für den REST-Endpunkt.]({% image_buster /assets/img/crowdin/braze_rest_api_keys.png %})

Wählen Sie **Log in with Braze Campaigns & Canvas**.

### 3. Schritt: Sprachzuordnung in Crowdin konfigurieren {#step-3-configure-language-mapping-in-crowdin}
...
![Integrationsbildschirm von Braze Campaigns & Canvas mit „Settings“ in der oberen Aktionsleiste.]({% image_buster /assets/img/crowdin/crowdin_campaigns_canvas_settings.png %})
...
3. Geben Sie die Locale-Schlüssel ein. Crowdin listet Ihre Projektsprachen auf (zum Beispiel Französisch, Italienisch). Geben Sie in jedem Feld den passenden **Braze-Locale-Schlüssel** ein.
   ...
   - Jeder Eintrag muss exakt mit dem **Locale-Schlüssel** für dieses Locale in den Braze-**Einstellungen für die Lokalisierung** übereinstimmen.

![Einstellungs-Modal auf dem Tab „General Settings“ mit Dateifilterfeldern und Sprachzuordnungszeilen (zum Beispiel Französisch zugeordnet zu fr).]({% image_buster /assets/img/crowdin/crowdin_language_mapping_settings.png %})

{: start="4"}
4. Wählen Sie **Save**, um die Zuordnung zu bestätigen.

### 4. Schritt: Übersetzungs-Tags zu Ihrer Braze-Nachricht hinzufügen {#step-4-add-translation-tags-to-your-braze-message}
...
### 5. Schritt: Übersetzungen in Crowdin verwalten {#step-5-manage-translations-in-crowdin}

Der Integrationsbildschirm hat zwei Seiten:

- **Rechte Seite (Braze):** Ihre Campaigns und Canvases.
...
1. Aktivieren Sie auf der **Braze-Seite (rechts)** das Kontrollkästchen für die Campaign oder das Canvas, das Sie übersetzen möchten.
...
### 6. Schritt: Nachricht als mehrsprachige:r Nutzer:in in Braze in der Vorschau anzeigen {#step-6-preview-the-message-as-a-multi-language-user-in-braze}
...
## Integration von Braze Email Templates {#braze-email-templates-integration}
...
### 1. Schritt: App installieren {#step-1-install-the-app}

1. Gehen Sie in Ihrem Crowdin-Projekt zum Tab **Store**.
2. Suchen Sie nach **Braze Email Templates** und wählen Sie **Install**.

![Crowdin Store mit ausgewähltem „Braze Email Templates“ und hervorgehobenem „Install“.]({% image_buster /assets/img/crowdin/crowdin_store_email_templates.png %})

{: start="3"}
3. Wählen Sie das Projekt (oder die Projekte) aus, in dem Sie diese Integration verwenden möchten.
4. Um die Integration zu öffnen, gehen Sie zu Ihrem Projekt **Integrations** > **Braze Email Templates**.

### 2. Schritt: Mit Braze verbinden {#step-2-connect-to-braze}

Autorisieren Sie die Verbindung mit Ihren Braze-API-Zugangsdaten:

![Crowdin-Verbindungsformular für Braze Email Templates mit REST-API-Schlüssel, REST-Endpunkt und „Log in with Braze Email Templates“.]({% image_buster /assets/img/crowdin/crowdin_email_templates_login.png %}){: style="max-width:85%;"}

1. **Braze REST API key:** Erteilen Sie `templates.email` und `content_blocks` (Lesen und Schreiben). Erstellen Sie den Schlüssel in Braze unter **Settings** > **APIs and Identifiers** > **API Keys**.

![Braze-Seite „REST API Keys“ mit „Create API Key“ und dem Kopiersteuerelement für den REST-Endpunkt.]({% image_buster /assets/img/crowdin/braze_rest_api_keys.png %})

{: start="2"}
2. Geben Sie für den **Braze REST endpoint** Ihre instanzspezifische URL ein (zum Beispiel `https://rest.iad-03.braze.com`).
3. Wählen Sie **Log in with Braze Email Templates**.

### 3. Schritt: Inhalte zur Übersetzung synchronisieren {#step-3-sync-content-for-translation}

Der Integrationsbildschirm zeigt Ihre Braze-Bibliothek:

- **Rechte Seite (Braze):** **E-Mail-Templates** und **Content Blocks**, die Sie synchronisieren können.
...
### 4. Schritt: Übersetzte Templates bereitstellen {#step-4-deliver-translated-templates}
...
![Crowdin- und Braze-Email-Templates-Panels mit E-Mail-Templates und Content Blocks, mit „Sync to Braze“ und „Sync to Crowdin“.]({% image_buster /assets/img/crowdin/crowdin_email_templates_sync_panels.png %})