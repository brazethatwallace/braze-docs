...
Verwenden Sie die Integration von Braze und CataBoom, um personalisierte Spiel-Links zu Ihren Nachrichten hinzuzufügen. Sie können Nutzer:innen-Bezeichner und Attribute in Echtzeit zwischen Catapult-Kampagnen und Braze übergeben. Anschließend können Sie mit diesen Daten personalisierte Kampagnen, Trigger und Folge-Journeys erstellen.
...
Wählen Sie im oberen Navigationsbereich **New Campaign** aus. Geben Sie einen Kampagnennamen ein, wählen Sie einen URL-Slug und wählen Sie Ihre Spielkategorie und Ihren Spieltyp aus.

![CataBoom-Dashboard-Formular „New Campaign“ mit Feldern für Kampagnenname, URL, Spielkategorie und Spieltyp.]({% image_buster /assets/img/cataboom/new_campaign.png %})
...
Sie haben nun genug konfiguriert, um einen Test in Braze durchzuführen. Die optionalen Schritte in diesem Abschnitt vervollständigen ein typisches vollständiges Spiel-Setup. Catapult bietet außerdem viele weitere Einstellungen, mit denen Sie das Gameplay anpassen können.
...
Die folgenden Screenshots zeigen **Prize Options** in der Kampagnenzusammenfassung und eine einfache Wahrscheinlichkeitskonfiguration mit einer 50-%-Gewinnwahrscheinlichkeit auf Level 1.
...
## Schritt 2: Erstellen Sie eine Nachricht in Braze {#step-2-create-a-message-in-braze}
...
Ersetzen Sie die Basis-URL sowie die Abfrageparameter `username` und `password` durch die Werte von der Seite **Link Configuration** Ihrer Kampagne in Catapult.
...
Verwenden Sie das gespeicherte `result` in Ihrer Card (z. B. als Link-URL oder im Nachrichtentext). Folgen Sie dem Antwortformat der CataBoom-API für Ihre Kampagne. Weitere Informationen zu Abfrageparametern und Liquid in URLs finden Sie unter [Einen API-Aufruf durchführen]({{site.baseurl}}/user_guide/personalization_and_dynamic_content/connected_content/making_an_api_call).
