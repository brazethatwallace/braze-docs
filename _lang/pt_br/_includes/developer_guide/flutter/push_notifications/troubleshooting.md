...
- O SDK nativo ainda envia um broadcast `BRAZE_PUSH_CLICKED` e o listener `push_opened` do Dart ainda é acionado conforme esperado.
- O SDK nativo não chama `startActivity()`, então o app não é trazido para o primeiro plano e o deep link não é seguido automaticamente.
