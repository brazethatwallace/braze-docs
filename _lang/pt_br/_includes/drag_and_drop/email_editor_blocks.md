...
| Alinhar | Define o alinhamento da imagem para a esquerda, centro ou direita dentro do bloco. |
...
{: .reset-td-br-1 .reset-td-br-2 aria-label="Redes sociais" }
...
Insere HTML bruto. Recomendado para [Liquid]({{site.baseurl}}/liquid), como Connected Content ou declarações condicionais.
...
| Quantidade | *(Dinâmico, somente Canvas, quando o gatilho de entrada não é um evento de visualização de produto)* Mostra ou oculta a quantidade do produto a partir do evento-gatilho. |
...
- **[Connected Content]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/connected_content):** Adicione um bloco **HTML** e insira sua chamada {% raw %}`{% connected_content %}`{% endraw %} nele.