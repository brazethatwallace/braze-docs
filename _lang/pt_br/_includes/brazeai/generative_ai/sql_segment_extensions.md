...
Como é possível acessar dados de IPI por meio desse recurso, você deve ter permissões de IPI para executar consultas de SQL de segmento.

## Criando uma extensão de segmento {#creating-a-segment-extension}

### Etapa 1: Escolha um editor {#step-1-choose-an-editor}

Existem dois tipos de editores SQL para escolher ao criar sua extensão de segmento SQL: o Editor SQL e o Editor SQL Incremental.

- **Atualização completa:** Cada vez que seu segmento é atualizado, a Braze consultará todos os dados disponíveis para atualizar seu segmento, o que consumirá mais créditos do que atualizações incrementais. Extensões de atualização completa podem regenerar automaticamente a associação diariamente, mas não podem ser atualizadas usando atualização incremental.
- **Atualização incremental:** A atualização incremental é uma forma mais eficiente em termos de custo para configurar sua consulta, embora a configuração envolva algumas [etapas](#step-2-write-your-sql) adicionais. Se você conseguir completar essas etapas adicionais ao construir seu segmento, vale a pena escolher essa opção porque sua consulta será executada usando menos créditos.
- **Gerador de SQL com IA:** O gerador de SQL com IA permite que você escreva um prompt em linguagem simples e o transforma em uma consulta SQL para o seu segmento. É uma maneira rápida de começar sem precisar escrever o SQL você mesmo.
...
4. Se desejar, selecione **Regenerar extensão diariamente**.<br><br>
   ...
   Quando selecionado, a Braze atualizará automaticamente a associação do segmento a cada dia. Isso significa que, a cada dia à meia-noite no fuso horário da sua empresa (com um possível atraso de uma hora), a Braze verificará novos usuários no seu segmento e os adicionará automaticamente. Se uma extensão de segmento não for utilizada por 7 dias, a Braze pausará automaticamente a regeneração diária. Uma extensão de segmento não utilizada é aquela que não faz parte de uma Campaign ou Canvas (a Campaign ou Canvas não precisa estar ativa para que a extensão seja considerada "em uso").<br><br>
...
O gerador de SQL com IA utiliza o [GPT](https://openai.com/gpt-4), desenvolvido pela OpenAI, para recomendar SQL para o seu segmento SQL.
...
1. Selecione **Iniciar gerador de SQL com IA** após criar um [segmento SQL]({{site.baseurl}}/user_guide/audience/segments/segment_extension/sql_segments) usando atualização completa ou incremental.
2. Digite seu prompt e selecione **Gerar** para transformar seu prompt em SQL.
3. Revise o SQL gerado para garantir que está correto e salve seu segmento.
...
Quando a extensão terminar o processamento, você pode [criar um segmento]({{site.baseurl}}/user_guide/engagement_tools/segments/segment_extension#step-5-use-your-extension-in-a-segment) usando sua extensão de segmento e direcionar esse novo segmento com suas Campaigns e Canvas.
...
{% alert important %}
Observe que as tabelas disponíveis para consulta contêm apenas dados de eventos. Se você deseja consultar atributos de usuário, deve combinar seu segmento SQL com filtros de atributos personalizados do [segmentador clássico]({{site.baseurl}}/user_guide/audience/segments/creating_a_segment).
{% endalert %}
...
- Não é possível consultar usuários com zero eventos, o que significa que qualquer consulta para usuários que realizaram um evento menos de X vezes precisaria seguir esta solução alternativa:
   ...
   2. Ao referenciar sua extensão de segmento no seu segmento, selecione `doesn't include` para inverter o resultado.
...
No exemplo a seguir, o segmento resultante conteria usuários que realizaram o evento `favorited` mais de 3 vezes durante os últimos 30 dias, após uma data especificada.
...
{% alert tip %}
Segmentos de atualização incremental consideram eventos atrasados, que são eventos ocorridos há mais de 2 dias (por exemplo, eventos do SDK que não foram enviados no momento em que foram capturados).
{% endalert %}
...
- Escreva uma única instrução SQL. Não inclua pontos e vírgulas.
- Seu segmento SQL incremental pode fazer referência a apenas um único evento. Seus menus suspensos de data e contagem são referentes ao evento escolhido.
...
Para extensões de segmento SQL incrementais, a prévia não incluirá os critérios adicionais do operador, número de vezes e campos de período de tempo.
...
{% alert tip %}
Se você criou um segmento em que espera que os usuários entrem e saiam regularmente, atualize manualmente a extensão de segmento que ele utiliza antes de direcionar esse segmento em uma Campaign ou Canvas.
{% endalert %}
...
Selecione uma extensão de segmento SQL para ver onde a extensão está sendo usada, arquivar a extensão ou [atualizar manualmente a associação ao segmento](#refreshing-segment-membership).
...
Para economizar créditos, faça uma prévia da sua consulta para garantir que ela esteja correta antes de salvar a extensão de segmento SQL.

Seus créditos serão redefinidos para 5 no primeiro dia de cada mês, às 12h UTC. Você pode monitorar o uso dos seus créditos durante o mês no painel de uso de créditos. Na página **Extensões de segmento**, clique em <i class="fa-solid fa-chart-column"></i> **Ver uso de créditos SQL**.
