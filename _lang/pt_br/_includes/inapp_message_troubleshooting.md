...
1. Sua campanha está configurada para direcionar a aplicativos móveis ou navegadores da web, conforme apropriado? Por exemplo, se sua campanha tiver como alvo apenas navegadores da web, ela não será enviada para dispositivos Android.
...
### Minha mensagem no app demorou muito para aparecer
...
### *Impressões* são maiores que *Impressões únicas*

Esse é um comportamento esperado e pode acontecer quando:

- Mesmo que a reelegibilidade esteja desativada, usuários que receberam a campanha podem ter mais de um dispositivo. O disparo da campanha é atualizado no próximo início de sessão, então um dispositivo não saberá se outro dispositivo já disparou a campanha até que o usuário inicie uma nova sessão.
...
Para saber mais sobre reelegibilidade, consulte [Reelegibilidade para campanhas e Canvas]({{site.baseurl}}/user_guide/engagement_tools/messaging_fundamentals/reeligibility/).

### As impressões são menores do que o esperado

1. Os disparos levam tempo para serem sincronizados com o dispositivo no início da sessão, portanto pode haver uma condição de corrida se os usuários registrarem um evento ou uma compra logo após iniciarem uma sessão. Uma possível solução alternativa seria alterar a campanha para disparar a partir do início da sessão e, em seguida, segmentar pelo evento ou compra pretendida. Note que isso entregaria a mensagem no app no próximo início de sessão após a ocorrência do evento.
...
1. Certifique-se de que ninguém alterou involuntariamente o segmento ou a campanha desde o lançamento. Nossos changelogs de segmento e campanha darão insights sobre as alterações feitas, quem fez a alteração e quando ela ocorreu.

![Link para visualizar o changelog na página Informações da campanha com sete alterações desde a última vez que o usuário visualizou a campanha]({% image_buster /assets/img_archive/trouble4.png %})
...
Se suas mensagens no app não estiverem sendo retornadas, é provável que haja um problema de direcionamento da campanha:

1. Seu segmento não contém seu usuário.
  - Verifique a guia [**Engajamento**]({{ site.baseurl }}/user_guide/engagement_tools/segments/using_user_search/#engagement-tab) do seu usuário para ver se o segmento correto aparece em **Segmentos**.
...
4. Se havia um grupo de controle na campanha, seu usuário pode ter caído no grupo de controle.
  ...
  - Ao criar campanhas para fins de teste de integração, certifique-se de não incluir um grupo de controle.
