...
Você pode tentar, mas a autenticação pode ser bloqueada. A Braze atualmente mantém uma lista de domínios permitidos por segurança. {% multi_lang_include product_feedback_cta.md context="pain_point" channel="feature" feature="support for your MCP client" %}
...
As listagens de diretório podem ainda não estar disponíveis em todos os clientes MCP. Você sempre pode se conectar manualmente usando a URL do MCP da Braze.
...
Não no momento. Se você usa [lista de IPs permitidos](https://www.braze.com/docs/user_guide/administer/global/admin_settings/security_settings#dashboard-ip-allowlisting), não é possível usar o servidor MCP remoto.
