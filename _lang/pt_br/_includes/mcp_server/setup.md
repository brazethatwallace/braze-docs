...
{% alert note %}
O acesso do seu agente reflete suas permissões no dashboard. Se o acesso ao seu dashboard estiver limitado a uma equipe em vez de um espaço de trabalho completo, algumas ferramentas podem não funcionar durante o Acesso Antecipado.
{% endalert %}
...
Se você não especificar um espaço de trabalho, seu agente poderá pedir que você esclareça.
...
1. Confirme que sua empresa está inscrita no Acesso Antecipado.
2. Confirme que seu usuário tem a permissão **Use MCP Server**.
...
1. Confirme que seu usuário do dashboard tem as permissões necessárias. Seu agente só pode usar ferramentas que correspondam ao seu próprio acesso no dashboard.
