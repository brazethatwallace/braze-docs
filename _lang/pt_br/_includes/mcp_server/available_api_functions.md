...
Antes de usar esse recurso, você precisará [configurar o servidor Braze MCP]{% if include.section == "user" %}({{site.baseurl}}/user_guide/brazeai/mcp_server/setup/){% elsif include.section == "developer" %}({{site.baseurl}}/developer_guide/mcp_server/setup/){% endif %}.
...
| `get_campaign_details` | [`/campaigns/details`]({{site.baseurl}}/api/endpoints/export/campaigns/get_campaign_details) | leitura | Recupera informações relevantes sobre uma Campaign específica pelo `campaign_id`. |
...
| `duplicate_campaign` | [`/campaigns/duplicate`]({{site.baseurl}}/api/endpoints/messaging/duplicate_messages/post_duplicate_campaigns) | criação | Duplica uma Campaign existente. |
| `create_campaign`<sup>*</sup> | N/A | criação | Cria uma nova Campaign. |
| `edit_campaign`<sup>*</sup> | N/A | atualização | Edita uma Campaign existente. |
| `launch_campaign`<sup>*</sup> | N/A | atualização | Lança uma Campaign. |
| `stop_campaign`<sup>*</sup> | N/A | atualização | Interrompe uma Campaign em execução. |
| `archive_campaign`<sup>*</sup> | N/A | atualização | Arquiva uma Campaign. |
| `unarchive_campaign`<sup>*</sup> | N/A | atualização | Desarquiva uma Campaign. |
| `get_campaign_draft`<sup>*</sup> | N/A | leitura | Recupera os detalhes do rascunho de uma Campaign. |
| `get_campaign_live_details`<sup>*</sup> | N/A | leitura | Recupera os detalhes de uma Campaign ativa. |
| `create_campaign_message`<sup>*</sup> | N/A | criação | Cria uma mensagem dentro de uma Campaign. |
| `update_campaign_message`<sup>*</sup> | N/A | atualização | Atualiza uma mensagem de Campaign. |
| `delete_campaign_message`<sup>*</sup> | N/A | exclusão | Exclui uma mensagem de Campaign. |
| `create_campaign_message_variation`<sup>*</sup> | N/A | criação | Cria uma variação de mensagem dentro de uma Campaign. |
| `update_campaign_message_variation`<sup>*</sup> | N/A | atualização | Atualiza uma variação de mensagem de Campaign. |
| `delete_campaign_message_variation`<sup>*</sup> | N/A | exclusão | Exclui uma variação de mensagem de Campaign. |
| `update_campaign_distribution`<sup>*</sup> | N/A | atualização | Atualiza as configurações de distribuição de uma Campaign. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 .reset-td-br-4 aria-label="Campaigns" }

<sup>*</sup> Esta ferramenta está disponível apenas para clientes que participam do programa beta de APIs de Campaign. Se a sua conta não tem este recurso ativado, você pode receber um erro ao tentar usá-la. Para participar do programa beta, entre em contato com o seu gerente de conta.
{: .reset-td-br-1 }
...
| `get_canvas_data_summary` | [`/canvas/data_summary`]({{site.baseurl}}/api/endpoints/export/canvas/get_canvas_analytics_summary) | leitura | Exporta resumos consolidados dos dados de série temporal de um Canvas para uma visão concisa dos resultados. |
...
| `get_catalog_items` | [`/catalogs/{catalog_name}/items`]({{site.baseurl}}/api/endpoints/catalogs/catalog_items/synchronous/get_catalog_items_details_bulk) | leitura | Retorna múltiplos itens de catálogo e seus conteúdos. |
...
| `create_catalog_fields` | [`/catalogs/{catalog_name}/fields`]({{site.baseurl}}/api/endpoints/catalogs/catalog_fields/asynchronous/post_create_catalog_fields) | criação | Cria múltiplos campos em um catálogo. |
| `delete_catalog_field` | [`/catalogs/{catalog_name}/fields/{field_name}`]({{site.baseurl}}/api/endpoints/catalogs/catalog_fields/asynchronous/delete_catalog_field) | exclusão | Exclui um campo de catálogo. |
| `create_catalog_items` | [`/catalogs/{catalog_name}/items`]({{site.baseurl}}/api/endpoints/catalogs/catalog_items/asynchronous/post_create_catalog_items_bulk) | criação | Cria múltiplos itens em um catálogo. Até 50 itens por requisição. |
| `edit_catalog_items` | [`/catalogs/{catalog_name}/items`]({{site.baseurl}}/api/endpoints/catalogs/catalog_items/asynchronous/patch_catalog_items_bulk) | atualização | Edita múltiplos itens existentes em um catálogo. Até 50 itens por requisição. |
| `replace_catalog_items` | [`/catalogs/{catalog_name}/items`]({{site.baseurl}}/api/endpoints/catalogs/catalog_items/asynchronous/put_update_catalog_items) | atualização | Substitui múltiplos itens em um catálogo. Cria itens caso não existam. Até 50 itens por requisição. |
| `delete_catalog_items` | [`/catalogs/{catalog_name}/items`]({{site.baseurl}}/api/endpoints/catalogs/catalog_items/asynchronous/delete_catalog_items_bulk) | exclusão | Exclui múltiplos itens em um catálogo. Até 50 itens por requisição. |
...
| `delete_catalog_selection` | [`/catalogs/{catalog_name}/selections/{selection_name}`]({{site.baseurl}}/api/endpoints/catalogs/catalog_selections/asynchronous/delete_catalog_selection) | exclusão | Exclui uma seleção de catálogo. |
...
| `get_events_data_series` | [`/events/data_series`]({{site.baseurl}}/api/endpoints/export/custom_events/get_custom_events_analytics) | leitura | Ocorrências de um evento personalizado em um período designado. |
...
| `get_integration_job_sync_status` | [`/cdi/integrations/{integration_id}/job_sync_status`]({{site.baseurl}}/api/endpoints/cdi/get_job_sync_status) | leitura | Status de sincronizações anteriores para uma integração CDI específica, 10 por chamada. |
| `trigger_integration_sync` | [`/cdi/integrations/{integration_id}/sync`]({{site.baseurl}}/api/endpoints/cdi/post_job_sync) | escrita | Dispara uma sincronização para uma integração CDI específica. |
...
| Ferramenta | Endpoint de API | Acesso | Descrição |
| --- | --- | --- | --- |
| `create_media_library_asset` | [`/media_library/create`]({{site.baseurl}}/api/endpoints/media_library/manage_assets/create) | criação | Faz upload de um ativo para a biblioteca de mídia da Braze por URL externo ou conteúdo de arquivo em base64. Exatamente um modo de upload deve ser fornecido. |
...
| Ferramenta | Endpoint de API | Acesso | Descrição |
| --- | --- | --- | --- |
| `get_segment_list` | [`/segments/list`]({{site.baseurl}}/api/endpoints/export/segments/get_segment) | leitura | Exporta Segments com nome, identificador de API do Segment e flag de rastreamento analítico. |
| `get_segment_details` | [`/segments/details`]({{site.baseurl}}/api/endpoints/export/segments/get_segment_details) | leitura | Recupera informações relevantes sobre um Segment pelo `segment_id`. |
...
<sup>*</sup> Esta ferramenta está disponível apenas para clientes que participam do programa beta de APIs de Segment. Se a sua conta não tem este recurso ativado, você pode receber um erro ao tentar usá-la. Para participar do programa beta, entre em contato com o seu gerente de conta.
{: .reset-td-br-1 }
...
| Ferramenta | Endpoint de API | Acesso | Descrição |
| --- | --- | --- | --- |
| `get_session_data_series` | [`/sessions/data_series`]({{site.baseurl}}/api/endpoints/export/sessions/get_sessions_analytics) | leitura | Número de sessões do seu app em um período designado. |
...
| `get_content_block_info` | [`/content_blocks/info`]({{site.baseurl}}/api/endpoints/templates/content_blocks_templates/get_see_email_content_blocks_information) | leitura | Obtém informações de um Content Block existente, opcionalmente com dados de inclusão em Campaigns ou Canvas. |
