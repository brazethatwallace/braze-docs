...
Encurtamos URLs que são renderizadas por Liquid, mesmo aquelas incluídas em propriedades de gatilho de API or interface de programação do aplicativo (API). Por exemplo, se {% raw %}`{{api_trigger_properties.${url_value}}}`{% endraw %} representa uma URL válida, encurtamos e rastreamos essa URL antes de enviar a mensagem.
