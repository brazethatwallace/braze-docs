...
A Braze removerá a capacidade de usar as seguintes cifras do dashboard web e das REST APIs da Braze:
...
A Braze desativará o suporte para TLS 1.0 e TLS 1.1 em todos os Serviços da Braze em 31 de maio de 2018 — incluindo o dashboard da Braze, REST APIs e APIs que se comunicam com nossos SDKs. Também removeremos o suporte para as cifras listadas na seção anterior em conexão com as APIs que recebem dados do SDK. Isso significa que toda comunicação TLS 1.0 e 1.1 de e para a Braze não será mais suportada pela nossa rede a partir dessa data.
...
Se a sua marca estiver utilizando as REST APIs da Braze, fale com sua equipe de engenharia para garantir que todas as chamadas de servidor para servidor para a Braze estejam usando TLS 1.2 até a data indicada, a fim de evitar uma interrupção do serviço. Saiba que algumas linguagens de programação — como o Java 7 — usam versões mais antigas do TLS por padrão, então sua equipe de engenharia pode precisar fazer algumas alterações no código para atender aos requisitos de criptografia atualizados.

Dispositivos Apple não serão afetados pela descontinuação planejada da Braze porque a Apple exige TLS 1.2 desde o final de 2016. O mesmo vale para os navegadores web modernos, então não prevemos que essas mudanças terão qualquer impacto no uso do Web SDK. No entanto, dispositivos Android executando Android 4.4 (KitKat) ou inferior podem não usar TLS 1.2 por padrão, então tome medidas para fazer upgrade de qualquer uma das suas integrações Android para pelo menos a versão 2.0.3 do SDK da Braze (que usa TLS 1.2 por padrão, se o dispositivo em questão puder suportá-lo) até 31 de maio de 2018.
