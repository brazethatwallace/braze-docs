...
Estas orientações se aplicam a clientes do Shopify Plus que adicionaram snippets de código SDK personalizados às páginas de informações, envio ou pagamento em `checkout.liquid`. Se você não fez essas personalizações, pode desconsiderar estas orientações.

Você não poderá mais adicionar snippets de código SDK personalizados às páginas de informações, envio ou pagamento em `checkout.liquid`. Em vez disso, será necessário adicionar snippets de código SDK personalizados às páginas de agradecimento ou de status de pedido. Isso permite que você reconcilie os usuários que concluíram o checkout.
1. Carregue o SDK da Braze para web nas páginas de agradecimento e status de pedido.
