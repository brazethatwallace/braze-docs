...
## Contagem de emblemas de push do iOS {#ios-push-badge-count}

Agora você pode [atualizar a contagem de emblemas]({{site.baseurl}}/help/best_practices/utilizing_badge_count#utilizing-badge-count) no criador de push da Braze.
Para cada mensagem push, você pode especificar a contagem de emblemas que a notificação dispara.

## Exportação de usuários via API usando endereços de e-mail {#exporting-users-via-api-using-email-addresses}
...
## Permissões de chaves da API REST {#rest-api-keys-permissions}

Agora você pode criar [várias chaves da API REST]({{site.baseurl}}/developer_guide/rest_api/basics#creating-rest-api-keys) e configurar permissões de acesso para cada uma delas. Cada chave pode ser configurada para conceder acesso a determinados endpoints.
