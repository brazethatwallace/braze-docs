...
### Upgrade de versão do SDK Shopify por autoatendimento {#shopify-self-serve-sdk-version-upgrade}
...
Agora você pode [substituir o arquivo de um ativo existente na biblioteca de mídia]({{site.baseurl}}/user_guide/messaging/design_and_edit/media_library#replace-a-file) mantendo a URL e o ID do ativo estáveis. Como a URL não muda, qualquer Campaign, Canvas, Content Block ou modelo que faça referência a esse ativo reflete automaticamente o arquivo atualizado, sem a necessidade de fazer upload novamente ou vincular novamente em todos os lugares onde é usado.

### Visualização em grade para a biblioteca de mídia {#grid-view-for-the-media-library}

{% multi_lang_include release_type.md release="General availability" %}

A biblioteca de mídia e bibliotecas de modelos selecionadas agora oferecem uma visualização em grade junto com a visualização em lista existente. A visualização em grade exibe os ativos como miniaturas com metadados principais (nome, tipo, última modificação), tornando mais rápido encontrar imagens e criativos visualmente em vez de pelo nome do arquivo. A filtragem e a pesquisa funcionam da mesma forma em ambas as visualizações.
...
O [Refiner](https://refiner.io) é uma plataforma de pesquisas in-app para apps SaaS e mobile. Ele permite que equipes de produto e de voz do cliente lancem pesquisas in-app direcionadas e coletem continuamente NPS, CSAT, CES, feedback de produto e dados zero-party dos usuários.
