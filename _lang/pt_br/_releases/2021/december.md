...
## Atualização no endpoint de exportação de usuários por Segment or segmento or segmento {#update-to-export-users-by-segment-endpoint}

A partir de dezembro de 2021, as seguintes alterações entrarão em vigor para o [endpoint Exportar usuários por Segment or segmento or segmento]({{site.baseurl}}/api/endpoints/export/user_data/post_users_segment/):

1. O campo `fields_to_export` nessa solicitação da API or interface de programação do aplicativo (API) será obrigatório. A opção de usar todos os campos como padrão será removida.
...
### Zeotap – CDP or plataforma de dados do cliente or CDP or plataforma de dados do cliente or plataforma de dados do cliente {#zeotap-customer-data-platform}
