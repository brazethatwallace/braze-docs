...
O [filtro `Live Activities Push to Start Registered for App`]({{site.baseurl}}/user_guide/audience/segments/segmentation_filters#live-activites-push-to-start-registered-for-app) segmenta seus usuários com base em estarem ou não registrados para iniciar uma Live Activity por meio de notificações por push do iOS para um app específico.
...
Você pode [adicionar uma imagem de fundo em uma linha]({{site.baseurl}}/user_guide/channels/in_app_messages/customize#background-image) de uma mensagem no app ou landing page no painel **Propriedades da linha**. Ative a opção **Imagem de fundo** e, em seguida, forneça uma URL de imagem ou selecione uma imagem da [biblioteca de mídia]({{site.baseurl}}/user_guide/messaging/design_and_edit/media_library). Por fim, configure seu texto alternativo, tamanho, posição e se a imagem se repete para criar padrões ao longo da linha.
