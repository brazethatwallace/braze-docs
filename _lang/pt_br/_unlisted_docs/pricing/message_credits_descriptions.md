...
As proporções de crédito de SMS e MMS são calculadas em incrementos de segmentos enviados a partir da plataforma Braze. As proporções de crédito de RCS são calculadas em incrementos de tipos Basic e Rich Media, ou tipos Single e Rich Media entregues a partir da plataforma Braze. Tanto os tipos de entrada quanto os de saída são cobrados.

{% alert note %}
Quando aplicável para esses canais, as taxas da operadora são cobradas separadamente (após o faturamento) e não são consideradas como parte dos Créditos de Ação.
{% endalert %}
...
### Resto da África {#rest-of-africa}
...
### Resto da Ásia-Pacífico {#rest-of-asia-pacific}
...
### Resto da Europa Central e Oriental {#rest-of-central-eastern-europe}
...
### Resto da América Latina {#rest-of-latin-america}
...
### Resto do Oriente Médio {#rest-of-middle-east}

Bahrein, Iraque, Jordânia, Kuwait, Líbano, Omã, Iêmen

### Resto da Europa Ocidental {#rest-of-western-europe}
...
As taxas de crédito do Agent Console são denominadas em incrementos de mil (1.000) invocações realizadas a partir da plataforma Braze. Uma invocação é registrada quando um agente inicia uma chamada a um LLM. Por padrão, seu contrato inclui uma quantidade de invocações conforme especificado pela sua edição da plataforma para cada período do seu prazo de inscrição. Invocações adicionais serão cobradas conforme seu formulário de pedido.
...
A Braze reserva-se o direito de cobrar créditos por Content Cards com base no número de Content Cards enviados caso o cliente não configure os Content Cards para registrar impressões únicas de acordo com as orientações da Braze. Isso será considerado aplicável se, dentro de seis (6) meses do primeiro envio de Content Cards, o cliente tiver:
...
{% alert note %}
Consulte nossa [documentação do Canvas]({{site.baseurl}}/partners/canvas_audience_sync) para saber mais sobre o Canvas Audience Sync e os parceiros disponíveis.
{% endalert %}
