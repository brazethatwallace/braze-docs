...
Use a opção **Okta - Braze app** se você configurou o app da Braze para SAML SSO no Okta. Se você configurou um app personalizado para SSO, siga as instruções na aba [Okta - Integração de app personalizado]({{site.baseurl}}/user_guide/administer/global/user_management/automated_user_provisioning/?tab=okta%20-%20custom%20app%20integration#step-1-set-up-scim-provisioning).
...
![Formulário de configurações de provisionamento SCIM com três campos: espaço de trabalho padrão, origin do serviço e lista de IPs permitidos (opcional). O botão "Gerar token SCIM" está desativado.]({% image_buster /assets/img/scim_unfilled.png %})
...
![Campos de endpoint da API SCIM e token SCIM exibidos com valores mascarados e botões de cópia. Abaixo do campo de token há um botão "Redefinir token".]({% image_buster /assets/img/scim.png %})
