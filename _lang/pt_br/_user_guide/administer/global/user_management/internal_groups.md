...
|--------------------|---------------------------------------------------------------------------------------------|
| **User Event Group**   | Use para verificar eventos ou registros do seu dispositivo de teste.<br><br>Para capturar registros do SDK e da REST API para membros do grupo, marque a caixa de seleção **User Events**. Sem essa configuração, os usuários adicionados ao grupo não exibirão registros no [Registro de usuários de eventos]({{site.baseurl}}/user_guide/administer/global/workspace_settings/logs_and_alerts/event_user_log). |
...
Seed Groups não estão disponíveis para Campaigns via API, mas você pode incluir Seed Groups usando uma entrada disparada por API na Campaign. Use isso para medir métricas de entregabilidade e manter um registro do conteúdo do seu e-mail para fins históricos e de arquivamento.
