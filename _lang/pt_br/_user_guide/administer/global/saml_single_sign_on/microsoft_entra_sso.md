...
Após a configuração, você deverá fornecer uma URL do Assertion Consumer Service (ACS).
...
| Chave de API do RelayState | Para ativar o login do provedor de identidade, acesse **Configurações** > **Configuração e teste** > **APIs e identificadores**, selecione a guia **API Keys** e crie uma chave de API com permissões `sso.saml.login`. |
...
2. Pesquise por **Braze** na caixa de pesquisa, selecione-a no painel de resultados e, em seguida, selecione **Add**.
...
5. Configure o RelayState inserindo sua chave de API gerada pelo Relay State no campo **Relay State**.

{% alert important %}
**Não** defina o campo **Sign-On URL**. Deixe este campo em branco para evitar problemas com o SSO SAML iniciado pelo IdP.
{% endalert %}
...
Depois que o gerente da sua conta tiver ativado o SSO SAML para a sua conta, faça o seguinte:
...
{% alert tip %}
Se você deseja que os usuários da sua conta da Braze façam login apenas com SSO SAML, é possível [restringir a autenticação de login único]({{site.baseurl}}/user_guide/administer/global/saml_single_sign_on/saml_sso_setup#restriction) na página **Configurações de segurança** em **Regras de autenticação**.
{% endalert %}