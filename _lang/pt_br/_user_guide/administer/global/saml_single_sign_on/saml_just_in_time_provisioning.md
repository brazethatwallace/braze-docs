...
O provisionamento just-in-time (JITP) via SAML requer que o SSO SAML esteja configurado e integrado. Não é compatível com o SSO do Google e só é suportado para fluxos de login iniciados pelo provedor de identidade (IdP-initiated).
...
| Login iniciado pelo IdP | Os usuários devem fazer login pelo portal do IdP no primeiro acesso. O login iniciado pelo SP (SP-initiated) sozinho não provisiona novos usuários. |
...
| Capacitação da empresa | A Braze precisa ativar o recurso `saml_jit_provisioning` para a sua empresa antes que o toggle **Automatic user provisioning** seja exibido. |
{: .reset-td-br-1 .reset-td-br-2 aria-label="Pré-requisitos do JITP" }

{% alert important %}
O provisionamento just-in-time via SAML precisa ser ativado para a sua empresa pela Braze. Entre em contato com o seu gerente de conta ou com o [suporte da Braze]({{site.baseurl}}/braze_support) se o toggle **Automatic user provisioning** não estiver disponível.
{% endalert %}
...
4. O usuário pode acessar a Braze imediatamente, sem a necessidade de um convite ou etapa de ativação separados.
...
{% alert note %}
Se sua empresa usa permissões granulares, revise o conjunto de permissões padrão após a migração para confirmar que os novos usuários JITP recebam o acesso pretendido.
{% endalert %}
...
7. Instrua os usuários a acessar a Braze pelo portal do IdP para o primeiro login. Depois disso, o botão de login único SAML será exibido para logins futuros.

## Perguntas frequentes {#frequently-asked-questions}

### Como desativo o JITP do SAML? {#how-do-i-disable-saml-jitp}
...
Não. Todos os usuários criados pelo JITP recebem o espaço de trabalho padrão e o conjunto de permissões configurado em **Security Settings**. Para atribuir acessos diferentes, crie os usuários manualmente ou use o [provisionamento automatizado de usuários via SCIM]({{site.baseurl}}/scim/automated_user_provisioning).

### O JITP funciona com login iniciado pelo SP? {#does-jitp-work-with-sp-initiated-login}

Não. O JITP só é executado durante o login iniciado pelo IdP, quando o usuário começa pelo portal do provedor de identidade.

## Solução de problemas {#troubleshooting}

### O usuário não foi provisionado no primeiro login via SSO {#user-was-not-provisioned-on-first-sso-sign-in}

Verifique o seguinte:

- O JITP está ativado e salvo em **Configurações de segurança**.
- O usuário fez login pelo portal do IdP (iniciado pelo IdP), e não apenas pela página de login da Braze.
...
O campo **Sign-On URL** no formulário **Basic SAML Configuration** do Microsoft Entra para a Braze pode fazer com que os usuários vejam apenas a opção de senha, em vez de um botão de SSO, ao fazer login iniciado pelo IdP. Para evitar esse problema, deixe o campo **Sign-On URL** em branco ao configurar a Braze no centro de administração do Microsoft Entra.