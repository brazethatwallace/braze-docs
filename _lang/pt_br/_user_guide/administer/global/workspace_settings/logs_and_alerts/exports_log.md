...
O log de exportações lista os trabalhos de exportação do espaço de trabalho atual. Cada linha representa uma tentativa de exportação e inclui o nome do Segment or segmento ou da lista de supressão, a origem da exportação, o status e os registros de data e hora.
...
| Nome do Segment or segmento | Nome do Segment or segmento ou da lista de supressão exportada. |
| Tipo de Segment or segmento | Se a exportação é de um **Segment or segmento** ou de uma **Lista de supressão**. |
| Origem | Onde a exportação foi disparada: **Dashboard** (exportação CSV pela interface) ou **API or interface de programação do aplicativo (API)** (API or interface de programação do aplicativo (API) de exportação de usuários). |
...
| Exportação de Segment or segmento via API or interface de programação do aplicativo (API) | `POST /users/export/segment` | [POST: Exportar perfil de usuário por Segment or segmento]({{site.baseurl}}/api/endpoints/export/user_data/post_users_segment) |
