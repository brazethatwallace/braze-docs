...
Essas configurações se aplicam globalmente a todas as campanhas de push, a menos que um TTL diferente seja definido para uma mensagem específica. Para ajustar o TTL de uma mensagem, consulte [Configurações avançadas de campanha]({{site.baseurl}}/user_guide/channels/push/platform_specific_resources/android/advanced_campaign_settings/#ttl).

Para definir um TTL de push padrão diferente:

1. Acesse **Configurações** > **Gerenciar configurações** > **Configurações de push**.
...
Você pode definir a prioridade padrão do Firebase Cloud Messaging (FCM) para todas as campanhas de push para Android. Essa prioridade determina como a notificação por push é entregue aos dispositivos dos usuários.
...
Essa configuração se aplica globalmente a todas as novas campanhas de push para Android, a menos que uma prioridade diferente seja selecionada ao criar uma campanha específica.
