...
A Braze calcula o horário ideal de envio com base em uma análise estatística das interações anteriores dos seus usuários com o seu app e com cada canal de envio de mensagens. Os seguintes dados de interação são usados:

- Horários de sessão
- Aberturas Diretas de push
- Aberturas por Influência de push
- Cliques em e-mail
- Aberturas de e-mail (excluindo [aberturas automáticas]({{site.baseurl}}/user_guide/analytics/metrics_glossary#machine-opens))
...
Por exemplo, Sam pode abrir seus e-mails regularmente pela manhã, mas abrir seu app e interagir com notificações à noite. Isso significa que Sam receberia uma Campaign de e-mail com Intelligent Timing pela manhã, enquanto receberia Campaigns com notificações por push à noite, quando é mais provável que ela interaja.
...
2. Selecione **Entrega agendada** como tipo de entrega.
3. Em **Opções de agendamento baseado em tempo**, selecione **Intelligent Timing**.
...
6. Especifique um [horário de fallback](#campaign-fallback). Esse é o horário em que a mensagem será enviada se o perfil do usuário não tiver eventos relevantes para calcular um horário ideal.
...
Use o horário de silêncio para impedir que mensagens sejam enviadas durante horários específicos. Isso é útil quando você quer evitar enviar mensagens durante a madrugada ou de noite, enquanto ainda permite que o Intelligent Timing determine a melhor janela de entrega.
...
![Alternância de horário de silêncio ativada com horário de início e término configurados para bloquear a entrega de mensagens durante a noite]({% image_buster /assets/img/intelligent_timing/quiet_hours.png %})

Quando o horário de silêncio está ativado, a Braze não enviará mensagens durante o período de silêncio, mesmo que esse horário coincida com o horário ideal de envio do usuário. Se o horário ideal do usuário cair dentro da janela de silêncio, a mensagem será enviada na borda mais próxima da janela.

Por exemplo, se o horário de silêncio estiver definido das 22h às 6h e o horário ideal de um usuário for 5h30, a Braze reterá a mensagem e a entregará às 6h — o horário mais próximo fora da janela de silêncio.
...
Para ver uma estimativa de quantos usuários receberão a mensagem em cada hora do dia, use o gráfico de prévia (apenas para Campaigns).

1. Adicione Segments ou filtros na etapa de públicos-alvo.
2. Na seção **Preview Delivery Times for** (que aparece tanto na etapa de públicos-alvo quanto na de entrega agendada), selecione seu canal.
...
Em seguida, selecione uma data de envio para sua Campaign. Lembre-se do seguinte ao agendar Campaigns com Intelligent Timing:

#### Lançar a Campaign 48 horas antes {#launch-campaign-48-hours-in-advance}

Lance sua Campaign pelo menos 48 horas antes da data de envio agendada. Isso se deve às variações de fuso horário. A Braze calcula o horário ideal à meia-noite no horário de Samoa (UTC+13), um dos primeiros fusos horários do mundo. Um único dia abrange cerca de 48 horas ao redor do globo, o que significa que, se você lançar uma Campaign dentro dessa janela de 48 horas, é possível que o horário ideal de um usuário já tenha passado em seu fuso horário, e a mensagem não será enviada.

{% alert important %}
Se uma Campaign for lançada e o horário ideal de um usuário estiver a menos de uma hora no passado, a mensagem será enviada imediatamente. Se o horário ideal estiver a mais de uma hora no passado, a mensagem não será enviada.
{% endalert %}

#### Janela de 3 dias para filtros de Segment {#3-day-window-for-segment-filters}

Se você estiver direcionando um público que realizou uma ação em um determinado período de tempo, permita pelo menos uma janela de 3 dias nos filtros de Segment. Por exemplo, em vez de `First used app more than 1 day ago` e `First used app less than 3 days ago`, use 1 dia e 4 dias.
...
Isso também se deve aos fusos horários — selecionar um período de menos de 3 dias pode fazer com que alguns usuários saiam do Segment antes que seu horário ideal de envio seja alcançado.
...
#### Agendar variantes vencedoras 2 dias após o teste A/B {#schedule-winning-variants-2-days-after-ab-test}

Se você estiver utilizando [testes A/B com otimização]({{site.baseurl}}/user_guide/messaging/ab_testing/optimizations), como enviar automaticamente a **variante vencedora** ou usar uma **variante personalizada**, o Intelligent Timing pode afetar a duração e o cronograma da sua Campaign.

Ao usar o Intelligent Timing, recomendamos agendar o envio da variante vencedora pelo menos **2 dias após** o início do teste A/B. Por exemplo, se seu teste A/B começar em 16 de abril às 16h, agende a variante vencedora para ser enviada não antes de 18 de abril às 16h. Isso dá tempo suficiente para a Braze avaliar o comportamento dos usuários e enviar mensagens no horário ideal.

![Seções de testes A/B mostrando teste A/B com variante vencedora selecionada, com critérios de vitória, data de envio e horário de envio local selecionados]({% image_buster /assets/img/intelligent_timing/ab_testing_intelligent_timing.png %})

### Etapa 3: Configurar o horário de silêncio (opcional) {#step-3-configure-quiet-hours-optional}

Opcionalmente, você pode limitar a janela de entrega. Isso pode ser útil se sua Campaign estiver relacionada a um evento, promoção ou venda específica, mas geralmente não é recomendado ao usar o Intelligent Timing. Para saber mais, consulte [Considerações](#considerations).

O horário de silêncio funciona como uma janela de não envio. O Intelligent Timing ainda determina o horário ideal de envio de cada usuário, mas se esse horário cair dentro do horário de silêncio, a Braze adiará a mensagem até o próximo horário disponível fora do período de silêncio.
...
Escolha um horário de fallback para usar caso o perfil de um usuário não tenha eventos relevantes para calcular um horário ideal de entrega.
...
1. Adicione Segments ou filtros na etapa **Públicos-alvo**.
2. Na seção **Preview Delivery Times for** (que aparece tanto na etapa **Públicos-alvo** quanto na de **Entrega agendada**), selecione seu canal.
3. Selecione **Refresh Data**.

O gráfico de prévia mostra cada hora do dia usando seu fuso local. Os rótulos não estão definidos para um único fuso horário global.
...
O gráfico mostra em azul os usuários que tinham eventos relevantes para calcular um horário ideal e em vermelho os usuários que usarão o horário de fallback. Use os filtros de cálculo para ajustar a visualização da prévia e ter uma visão mais detalhada de cada grupo de usuários.
{% endtab %}
...
No seu Canvas, adicione uma [etapa de Mensagem]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/message_step), depois vá em **Configurações de entrega** e selecione **Usando Intelligent Timing**.

As mensagens serão enviadas aos usuários que entraram na etapa naquele dia no horário local ideal. No entanto, se o horário ideal já tiver passado naquele dia, a mensagem será entregue no horário ideal do dia seguinte. Etapas de Mensagem que direcionam múltiplos canais podem enviar ou tentar enviar mensagens em horários diferentes para canais diferentes. Quando a primeira mensagem em uma etapa de Mensagem tenta ser enviada, todos os usuários avançam automaticamente.

### Etapa 2: Escolher um horário de fallback {#step-2-choose-a-fallback-time}

Escolha um horário de fallback para enviar a mensagem aos usuários do seu público que não possuem dados de engajamento relevantes para a Braze calcular um horário ideal de envio. {% multi_lang_include brazeai/intelligent_suite/fallback_time.md %}

### Etapa 4: Adicionar uma etapa de Postergação {#step-4-add-a-delay-step}

Diferente das Campaigns, você não precisa lançar seu Canvas 48 horas antes da data de envio, pois o Intelligent Timing é configurado no nível da etapa, não no nível do Canvas.

Em vez disso, adicione uma [etapa de Postergação]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/delay_step) de pelo menos dois dias corridos entre a entrada do usuário no Canvas e o momento em que ele receberá a etapa com Intelligent Timing.

#### Dias corridos vs. dias de 24 horas {#calendar-vs-24-hour-days}

Ao usar o Intelligent Timing após uma etapa de Postergação, a data de entrega pode variar dependendo de como você calcula a postergação. Isso se aplica apenas quando sua postergação está configurada como **Após uma duração**, pois há uma diferença entre como "dias" e "dias corridos" são calculados.

- **Dias:** 1 dia equivale a 24 horas, calculadas a partir do momento em que o usuário entra na etapa de Postergação.
- **Dias corridos:** 1 dia é o período desde quando o usuário entra na etapa de Postergação até a meia-noite em seu fuso horário. Isso significa que 1 dia corrido pode durar apenas alguns minutos.

Ao usar o Intelligent Timing, recomendamos usar dias corridos para postergações em vez de dias de 24 horas. Isso porque, com dias corridos, a mensagem será enviada no último dia da postergação, no horário ideal. Com um dia de 24 horas, há a chance de o horário ideal do usuário ser anterior ao momento em que ele entra na etapa, o que significa que um dia extra será adicionado à postergação.

Por exemplo, digamos que o horário ideal de Luka é 14h. Ele entra na etapa de Postergação às 14h01 em 1º de março, e a postergação está configurada para 2 dias.
...
![Gráfico mostrando a diferença entre dias e dias corridos: se o horário ideal de um usuário é 14h, mas ele entra na etapa de postergação às 14h01, e a postergação está configurada para 2 dias, a opção de dias entrega a mensagem 3 dias depois porque o usuário entrou na etapa após seu horário ideal, enquanto dias corridos entrega a mensagem 2 dias depois, no último dia da postergação.]({% image_buster /assets/img/intelligent_timing_daysvcalendardays.png %}){: style="border:none;"}
{% endtab %}
{% endtabs %}
...
O Intelligent Timing usa o fuso horário local de cada usuário e os dias do calendário para determinar o momento ideal de entrega. Por isso, usuários em fusos horários à frente ou atrás do fuso de referência da sua Campaign podem receber mensagens em um dia diferente do calendário do que você esperaria.

Por exemplo, se uma Campaign está agendada para 15 de março e o horário ideal de um usuário é calculado para essa data, um usuário em um fuso horário à frente do ponto de referência da Campaign pode receber a mensagem no final do dia 14 de março no fuso de referência, enquanto um usuário em um fuso horário atrás do ponto de referência pode recebê-la no dia 16 de março.

Se os usuários não estiverem recebendo mensagens como esperado, verifique se o campo de fuso horário no perfil deles está preenchido corretamente. Se o campo de fuso horário estiver vazio, o usuário pode receber mensagens que se alinham com o fuso horário da empresa em vez do fuso local.

### Envio após a data agendada {#sending-past-the-scheduled-date}

Sua Campaign com Intelligent Timing pode estar enviando após a data agendada se você estiver utilizando [testes A/B com otimização]({{site.baseurl}}/user_guide/messaging/ab_testing/optimizations). Campaigns que usam otimizações de testes A/B podem enviar automaticamente a variante vencedora após o término do teste inicial, aumentando a duração da Campaign. Por padrão, Campaigns com otimização enviam a variante vencedora para os usuários restantes no dia seguinte ao teste inicial, mas você pode alterar essa data de envio.

Se você usar o Intelligent Timing, recomendamos deixar mais tempo para o teste A/B terminar e agendar o envio da variante vencedora para 2 dias após o teste inicial, em vez de 1 dia.
