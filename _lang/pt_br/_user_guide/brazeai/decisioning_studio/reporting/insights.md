...
# Relatório de insights {#insights-report}

> Os insights mostram como as diversas opções de recomendação no seu banco de ações são geradas, como a seleção de blocos. Existem dois relatórios de insights diferentes: **Agent preferences** e **SHAPs**.

{% tabs local %}
{% tab agent preferences %}
O relatório **Agent preferences** ajuda você a identificar tendências sazonais e avaliar a relevância das escolhas no banco de ações, orientando decisões informadas para atualizações.
...
| Segment or segmento | O [Segment or segmento de público]({{site.baseurl}}/user_guide/audience/segments) que você criou na Braze. |
