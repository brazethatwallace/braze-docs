...
O Decisioning Studio Go é um agente de decisão por IA para programas recorrentes de e-mail. Em vez de escolher uma única linha de assunto, horário de envio ou imagem vencedora para todo o público, o agente seleciona a melhor combinação para cada destinatário com base no engajamento anterior.

Você define as variantes que o agente pode escolher — como linhas de assunto, CTAs, imagens, dias de envio e horários de envio. Para cada usuário no seu Segment, o agente escolhe a opção com maior probabilidade de gerar engajamento, dentro das restrições e do cronograma que você configurar.

Isso é diferente de testes A/B no nível da Campaign ou da [seleção inteligente]({{site.baseurl}}/user_guide/brazeai/intelligence_suite/intelligent_selection), que otimizam uma única variante para todo o público. O Decisioning Studio Go personaliza no nível individual em cada envio do programa.
...
| **Criativos** | Um ou mais criativos base criados no criador da Braze. Dentro de cada criativo base, você pode marcar uma linha de assunto, CTA e imagem como pontos de personalização usando Liquid tags e, em seguida, fornecer uma lista de variantes para cada um. O agente decide qual criativo base e variante usar para cada destinatário. |
...
| **Revisão e lançamento** | Uma tela de validação final exibe quaisquer alertas a serem resolvidos antes do lançamento. O agente passa de **Draft** para **Live** e começa a enviar. |
...
O melhor cenário são programas recorrentes de e-mail com públicos estáveis e conteúdo clicável, como calendários always-on (recompensas, lançamentos de conteúdo, nudges de ciclo de vida), programas evergreen (reconquista, reengajamento) e promoções com múltiplos e-mails. Esses programas oferecem ao agente volume e variedade suficientes para aprender de forma significativa.

Consulte [Exemplos para o Decisioning Studio Go]({{site.baseurl}}/user_guide/brazeai/decisioning_studio/decisioning_studio_go/examples) para orientações detalhadas de adequação por tipo de programa.
...
O Decisioning Studio Go é o nível de entrada do BrazeAI Decisioning Studio. Ele foi projetado para profissionais de marketing que desejam personalização de e-mail individual sem a complexidade de configuração de uma implementação completa do Decisioning Studio Pro.

O Decisioning Studio Pro adiciona:
- Otimização para qualquer métrica de negócios (não apenas cliques)
