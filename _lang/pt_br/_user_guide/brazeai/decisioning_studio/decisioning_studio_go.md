...
O Decisioning Studio Go é um agente de decisão com IA para programas de e-mail recorrentes. Em vez de escolher uma única linha de assunto vencedora, horário de envio ou imagem para todo o público, o agente seleciona a melhor combinação para cada destinatário com base no engajamento anterior.
...
Isso difere dos testes A/B no nível de Campaign ou da [seleção inteligente]({{site.baseurl}}/user_guide/brazeai/intelligence_suite/intelligent_selection), que otimizam uma única variante para todo o público. O Decisioning Studio Go personaliza no nível individual em cada envio do programa.

### Como funciona {#how-it-works}

O agente divide um Segment da Braze em dois grupos: um grupo do Decisioning Studio que recebe conteúdo de e-mail otimizado por IA e um grupo de controle aleatório (mínimo de 5%) que recebe combinações aleatórias das mesmas opções. O grupo de controle aleatório oferece uma medição contínua e comparável do aumento gerado pelo agente; você sempre pode ver como a experiência personalizada se sai em relação ao mesmo conteúdo enviado sem personalização.

Para cada usuário no grupo do Decisioning Studio, o agente escolhe entre as opções que você forneceu: qual criativo enviar (incluindo a linha de assunto, CTA e imagem específicos dentro dele) e quando enviá-lo (dia da semana e horário do dia, respeitando o horário de silêncio e o fuso local do usuário). [Configure seu agente do Decisioning Studio Go]({{site.baseurl}}/user_guide/brazeai/decisioning_studio/decisioning_studio_go/setup) cobre cada um desses pontos em detalhes.
...
| **Revisão e lançamento** | Uma tela de validação final exibe quaisquer alertas para serem resolvidos antes do lançamento. O agente passa de **Rascunho** para **Ativo** e começa a enviar. |
...
A melhor adequação são programas de e-mail recorrentes com públicos estáveis e conteúdo clicável, como calendários sempre ativos (recompensas, lançamentos de conteúdo, lembretes de ciclo de vida), programas perenes (reconquista, reengajamento) e promoções com múltiplos e-mails. Esses programas dão ao agente volume e variedade suficientes para aprender de forma significativa.
...
### Onde o Decisioning Studio Go se encaixa no pacote do Decisioning Studio {#where-decisioning-studio-go-sits-in-the-decisioning-studio-suite}
...
- [Analise exemplos para o Decisioning Studio Go]({{site.baseurl}}/user_guide/brazeai/decisioning_studio/decisioning_studio_go/examples) para confirmar se seu programa é uma boa opção
