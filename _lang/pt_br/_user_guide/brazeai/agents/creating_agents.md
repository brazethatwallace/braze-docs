...
- [Permissão]({{site.baseurl}}/user_guide/administer/global/user_management/permissions#list-of-permissions) para acessar o **Agent Console** no seu espaço de trabalho. Verifique com os administradores da Braze se você não encontrar essa opção.
- Permissão para criar e editar AI Agents personalizados.
...
Quando você cria um agente, define o propósito dele e estabelece diretrizes para como ele deve se comportar. Depois que está ativo, o agente pode ser implantado na Braze para gerar textos personalizados, tomar decisões em tempo real ou atualizar campos de catálogo. Enquanto constrói seu agente, você pode salvá-lo como rascunho e atualizá-lo a qualquer momento pelo dashboard. Cada salvamento cria uma nova versão que você pode revisar na guia [Histórico de versões]({{site.baseurl}}/user_guide/brazeai/agents/reference#version-history).

Os casos de uso a seguir mostram algumas formas de aproveitar agentes personalizados.

| Caso de uso | Descrição |
| --- | --- |
| Tratamento de feedback de clientes | Encaminhe o feedback do usuário a um agente para analisar o sentimento e gerar mensagens de acompanhamento empáticas. Para usuários de alto valor, o agente pode escalar a resposta ou incluir benefícios. |
| Localizar conteúdo | Traduza textos de catálogo para outro idioma em campanhas globais, ou ajuste o tom e o comprimento para canais específicos de cada região. Por exemplo, traduzir "Classic Clubmaster Sunglasses" para espanhol como "Gafas de sol Classic Clubmaster", ou encurtar descrições para campanhas de SMS. |
| Resumir avaliações ou feedback | Resuma sentimentos ou feedback em um novo campo, como atribuir pontuações de sentimento como Positivo, Neutro ou Negativo, ou criar um resumo curto como "A maioria dos clientes menciona ótimo caimento, mas nota envio lento." |
...
### Etapa 2: Escolher como criar um agente {#step-2-choose-how-to-build-an-agent}
...
Se você usar o Operator, revise e aprove as mudanças no chat antes de continuar para a próxima etapa.

### Etapa 3: Configurar os detalhes {#step-3-set-up-details}
...
5. Defina um limite diário de invocações. Por padrão, esse valor é definido como 250.000, mas pode ser aumentado para 1.000.000. Se você tiver interesse em aumentar o limite além de 1.000.000, entre em contato com seu gerente de sucesso do cliente para saber mais. Defina o limite alto o suficiente para o tamanho do público planejado após os testes. Um limite muito baixo causa falhas de limite diário (que não consomem créditos, mas aplicam valores de fallback ou deixam a saída como `null`).
...
Para gerenciar custos, reduza o limite diário de invocações. Para modelos [bring-your-own (BYO)]({{site.baseurl}}/user_guide/brazeai/agents/reference#option-2-bring-your-own-api-key), você também pode mudar para um modelo de menor custo ou reduzir o [nível de raciocínio]({{site.baseurl}}/user_guide/brazeai/agents/reference#thinking-levels). O **Braze Auto** não suporta ajuste do nível de raciocínio. Acompanhe o uso real em **Settings** > **Billing** > **Credits Usage** > **Agent Console**.
...
### Etapa 4: Escrever as instruções {#agent-instructions}
...
Consulte as [Instruções de escrita]({{site.baseurl}}/user_guide/brazeai/agents/reference#writing-instructions) para práticas recomendadas e os [Exemplos]({{site.baseurl}}/user_guide/brazeai/agents/reference#examples) para inspiração sobre como instruir seu agente.

#### Adicionar contexto {#add-resources}

{% alert important %}
Os agentes recebem apenas os dados que você passa explicitamente — eles não pesquisam perfis de usuário nem avisam quando dados necessários estão ausentes. Use Liquid nas suas instruções, selecione **+ Agent context**, adicione [etapas de contexto]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/context) a montante no Canvas ou passe contexto adicional na etapa do agente. Para uma lista completa de fontes de dados e orientações de design, consulte [Quais dados os agentes recebem]({{site.baseurl}}/user_guide/brazeai/agents/reference#what-data-agents-receive).
{% endalert %}
...
- [Fontes de conhecimento]({{site.baseurl}}/user_guide/brazeai/agents/knowledge_sources): dê ao agente acesso aos dados do catálogo por meio de uma fonte de conhecimento para uma recuperação mais precisa do que anexar um catálogo diretamente.
- [Pertencimento a Segments]({{site.baseurl}}/user_guide/brazeai/agents/reference#segment-membership-context): permita que o agente personalize respostas com base em quais Segments um usuário pertence. Você pode selecionar até cinco Segments.
- [Diretrizes da marca]({{site.baseurl}}/user_guide/administer/global/workspace_settings/brand_guidelines): referencie as diretrizes de voz e estilo da marca para o agente seguir. Por exemplo, se você quiser que seu agente gere textos de SMS incentivando usuários a se inscreverem em uma academia, use esse campo para referenciar sua diretriz predefinida de tom motivacional e ousado.
- [Todo o contexto do Canvas]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/sources/context_variables): analise todos os dados de contexto do Canvas para um usuário quando esse agente é invocado, incluindo quaisquer variáveis que não são referenciadas na seção **Instructions**.
...
{% alert tip %}
Para agentes de Canvas, você pode usar Liquid nas instruções para referenciar atributos do usuário, como nome e sobrenome, ou atributos personalizados. Qualquer variável Liquid nas instruções do agente é passada automaticamente para a etapa do agente quando um usuário entra na etapa. Consulte [Quais dados os agentes recebem]({{site.baseurl}}/user_guide/brazeai/agents/reference#what-data-agents-receive) para saber como passar contexto do Canvas e dados de perfil de forma intencional.
{% endalert %}

### Etapa 5: Selecionar a saída {#select-output}
...
### Etapa 6: Testar o agente {#step-6-test-the-agent}

O painel **Preview** é uma instância do agente que aparece como um painel lado a lado na experiência de configuração. Você pode usar essa seção para testar o agente enquanto o está criando ou fazendo atualizações, experimentando-o de forma semelhante aos usuários finais. Essa etapa ajuda a confirmar que ele está se comportando como esperado e oferece uma oportunidade de ajuste fino antes de colocá-lo em produção.

1. No campo **Test your agent**, insira dados de clientes de exemplo ou respostas de clientes — qualquer coisa que reflita cenários reais que seu agente lidará.
...
![Agent Console exibindo o painel de prévia para testar um agente personalizado. A interface mostra um campo de entradas de exemplo com dados de clientes, um botão de executar teste e uma área de resposta onde a saída do agente aparece.]({% image_buster /assets/img/ai_agent/custom_agent_test.png %})
...
Se algo parecer fora do esperado, atualize a configuração do agente e teste novamente. Execute algumas entradas diferentes para ver como o agente se adapta em diversos cenários, especialmente casos extremos como ausência de dados ou respostas inválidas.
...
### Etapa 7: Usar seu agente {#step-7-use-your-agent}
...
- [Referência de agentes]({{site.baseurl}}/user_guide/brazeai/agents/reference)
