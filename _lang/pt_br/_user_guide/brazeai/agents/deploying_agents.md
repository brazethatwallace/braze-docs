...
Você pode atualizar e editar os campos do catálogo que usam agentes. Para remover um agente de uma coluna, desmarque **Apply AI agent**. Isso reverte a coluna para uma coluna comum, e os campos mantêm os últimos valores que o agente aplicou na última execução no catálogo.
...
Você pode adicionar um campo chamado **product_description** a um catálogo e selecionar **description** como **Response Field** para preencher a coluna com as descrições do agente.
...
Você também pode substituir manualmente a célula gerada pelo agente selecionando **Edit Item** e atualizando a descrição com suas edições. Para reverter à descrição gerada pelo agente, selecione o símbolo de atualizar na célula.
...
![Registros para um agente AI Sentiment Score.]({% image_buster /assets/img/ai_agent/agent_logs.png %})

Selecione **View** em uma chamada de agente específica para ver a entrada, a saída e o ID do usuário.

![O painel de detalhes para um agente Random Sports Assignment que mostra o prompt de entrada, a resposta de saída e um ID do usuário associado.]({% image_buster /assets/img/ai_agent/agent_logs_view.png %})
