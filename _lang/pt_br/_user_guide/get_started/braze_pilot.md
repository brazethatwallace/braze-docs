...
Steppington é um app de fitness com treinos, metas de exercícios e um serviço premium Steppington+. Ele oferece vários locais para demonstrar [Cartões de conteúdo]({{site.baseurl}}/user_guide/channels/content_cards/), uma seção que pode ser revelada com [Feature Flags]({{site.baseurl}}/developer_guide/feature_flags) e uma robusta biblioteca de registro de eventos personalizados que possibilita ilustrar muitas jornadas de clientes para esse setor.
...
## Como o Pilot se conecta ao seu dashboard da Braze
...
- Alimentar notificações por push, mensagens no app e canais de envio de mensagens de Cartões de conteúdo que exigem integração de SDK para funcionar.
...
Campanhas e Canvas são a forma como você envia mensagens aos seus usuários. 

- As Campanhas são ideais para mensagens individuais enviadas a um segmento específico de público em vários canais. 
- Os Canvas são fluxos de trabalho avançados de Campanhas que permitem automatizar e orquestrar jornadas personalizadas de clientes em múltiplos canais. Em um Canvas, você pode configurar lógica de ramificação, postergações, pontos de decisão e eventos de conversão para guiar os clientes por uma série de interações. Os Canvas ajudam a garantir uma comunicação consistente e fluida em diferentes pontos de contato, aumentando as chances de engajamento e conversão do cliente.
...
![Uma mensagem no app no app MovieCanon "Curtindo o MovieCanon? Indique seus amigos!" com uma opção para inserir seu endereço de e-mail para enviar uma indicação.]({% image_buster /assets/img/braze_pilot/moviecanon_iam.png %}){:style="max-width:40%"}