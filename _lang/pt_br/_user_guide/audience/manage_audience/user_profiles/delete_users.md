...
page_order: 6
toc_headers: h2
description: "Saiba como excluir um usuário individual ou um Segment or segmento or segmento de usuários diretamente pelo dashboard da Braze."
alias: /delete_users/
---

# Excluir usuários {#delete-users}

> Saiba como excluir um usuário individual ou um Segment or segmento or segmento de usuários diretamente pelo dashboard da Braze.
...
|---------------|---------|
| Tamanho máximo | Você pode excluir até 10 milhões de perfis de usuário ao excluir um Segment or segmento. |
| Período de espera | Todas as exclusões de Segment or segmento exigem um período de espera de 7 dias, além do tempo necessário para processar as exclusões. |
| Limites de trabalho | Apenas um Segment or segmento pode ser excluído por vez, o que inclui o período de espera de 7 dias. |
{: .reset-td-br-1 .reset-td-br-2 aria-label="Sobre a exclusão de usuários" }

## Excluindo usuários {#deleting-users}

Você pode excluir um [usuário individual](#delete-individual) ou um [Segment or segmento de usuários](#delete-segment) pelo dashboard da Braze:
...
### Excluindo um Segment or segmento {#delete-segment}

Se ainda não tiver feito isso, [crie um Segment or segmento]({{site.baseurl}}/user_guide/audience/segments/creating_a_segment) contendo os perfis de usuário que você deseja excluir. Certifique-se de incluir todos os perfis de usuário se estiver excluindo usuários duplicados.
...
Selecione **Delete users**, escolha o Segment or segmento que deseja excluir e selecione **Next**.
...
Os usuários nesse Segment or segmento não serão excluídos imediatamente. Em vez disso, serão marcados como pendentes de exclusão pelos próximos 7 dias. Após esse período, eles serão excluídos e enviaremos um e-mail para informá-lo.

Durante o período de espera de 7 dias, os usuários pendentes de exclusão ainda podem receber Campaigns e Canvas, a menos que você os exclua explicitamente. Para evitar que usuários pendentes recebam mensagens, adicione um filtro de Segment or segmento para excluir usuários com o status **Pending Deletion** das suas Campaigns e Canvas.

{% alert tip %}
Para garantir que esses usuários exatos sejam excluídos independentemente de alterações no Segment or segmento, um filtro de Segment or segmento chamado **Pending Deletion** é criado automaticamente. Você pode [usar esse filtro]({{site.baseurl}}/user_guide/audience/segments/managing_segments#filters) para verificar o status das exclusões pendentes.
{% endalert %}

## Confirmando exclusões de Segment or segmento {#confirming-segment-deletions}
...
Ao lado de uma exclusão de Segment or segmento or segmento pendente, selecione <i class="fa-solid fa-eye"></i> **View details** para abrir os detalhes do registro de exclusão.
...
Você pode verificar o status de uma exclusão usando [filtros de Segment or segmento or segmento](#segment-filters), a página [Gerenciar público](#manage-audience) ou [relatórios de eventos de segurança](#security-event-report).

### Filtros de Segment or segmento or segmento {#segment-filters}

Quando você solicita a exclusão de um Segment or segmento or segmento de usuários, um [filtro de Segment or segmento or segmento]({{site.baseurl}}/user_guide/audience/segments/managing_segments#filters) chamado **Pending Deletion** é criado automaticamente. Você pode usá-lo para:
...
{% alert note %}
Para obter a lista exata de usuários que serão excluídos, use o [filtro de Segment or segmento or segmento Pending Deletion](#segment-filters).
{% endalert %}
...
| Nome do Segment or segmento or segmento | O nome do Segment or segmento or segmento usado para selecionar os usuários pendentes de exclusão. |
...
Não. Você não pode excluir segmentos com mais de 10 milhões de usuários. Se precisar de ajuda para excluir um Segment or segmento or segmento desse tamanho, entre em contato com o [suporte da Braze]({{site.baseurl}}/user_guide/administer/personal/braze_support).
...
Não, isso não é um bug. O número máximo de perfis de usuários que podem ser excluídos em uma única execução de exclusão de Segment or segmento or segmento é 10 milhões.
...
Sim. No entanto, você pode adicionar um filtro de inclusão de Segment or segmento or segmento para excluir todos os usuários com o [filtro de Segment or segmento or segmento](#segment-filters) **Pending Deletion**.
...
### Posso excluir usuários pela API or interface de programação do aplicativo (API) em vez do dashboard? {#can-i-delete-users-with-the-api-instead-of-the-dashboard}
