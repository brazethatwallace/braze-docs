---
nav_title: Medir o tamanho do Segment or segmento or segmento
article_title: Medir o tamanho do Segment or segmento
page_order: 5
page_type: reference
tool:
- Segments
description: "Esta página aborda como você pode monitorar a associação e o tamanho do seu Segment or segmento."
---

# Medir o tamanho do Segment or segmento {#measure-segment-size}

> Esta página aborda como você pode monitorar a associação e o tamanho do seu Segment or segmento.

## Cálculo de pertencimento a Segments {#segment-membership-calculation}

A Braze atualiza o pertencimento do usuário a Segments conforme os dados são enviados de volta aos nossos servidores e processados, normalmente de forma instantânea. O pertencimento de um usuário a um Segment or segmento não será alterado até que a sessão tenha sido processada. Por exemplo, um usuário que se enquadra em um Segment or segmento de usuários inativos quando a sessão começa será imediatamente removido desse Segment or segmento de usuários inativos quando a sessão for processada.

### Cálculo do total de usuários contatáveis {#total-reachable-users-calculation}

Cada Segment or segmento exibe o número total de usuários que são membros daquele Segment or segmento. Ao filtrar por **Usuários de todos os apps**, também são exibidos alguns dos canais de envio de mensagens mais utilizados (como web push ou e-mail) e o número de usuários contatáveis para esses canais específicos.
...
Um único usuário pode pertencer a diferentes grupos de usuários contatáveis. Por exemplo, um usuário pode ter tanto um endereço de e-mail válido quanto um token por push Android válido e ter feito opt-in para ambos, mas não ter nenhum token por push iOS associado. A diferença entre o total de usuários contatáveis e a soma dos diferentes canais representa o número de usuários que se qualificaram para o Segment or segmento, mas não são contatáveis por esses canais de comunicação.

{% alert note %}
O **total de usuários contatáveis** inclui todos que correspondem aos filtros do seu Segment or segmento, mesmo que não estejam mais inscritos em um canal. As linhas de canal, como **iOS**, contam os usuários que são contatáveis apenas naquele canal de acordo com as regras em [Usuários contatáveis por canal](#reachable-users-by-channel). Para alinhar os totais do Segment or segmento com os usuários inscritos, adicione filtros como **Push ativado para iOS** é verdadeiro (ou o equivalente para o seu canal).
{% endalert %}

## Estatísticas para tamanho do Segment or segmento or segmento {#statistics-for-segment-size}

As estatísticas estimadas são aproximadas por meio da amostragem de apenas uma parte do seu Segment or segmento or segmento, então você deve esperar que os tamanhos estimados sejam maiores ou menores do que o valor real, com espaços de trabalho maiores apresentando margens de erro potencialmente maiores. Para obter uma contagem precisa de usuários no seu Segment or segmento or segmento, selecione **Calcular estatísticas exatas**. A associação exata ao Segment or segmento or segmento sempre será calculada antes que um Segment or segmento or segmento seja afetado por uma mensagem enviada em uma Campaign ou Canvas.

A Braze fornece as seguintes estatísticas sobre o tamanho do Segment or segmento or segmento.
...
Você pode visualizar a estimativa de usuários contatáveis de um Segment or segmento inteiro, incluindo contagens estimadas de usuários para cada canal, no painel lateral **Usuários contatáveis**. Essa estimativa mostra um intervalo aproximado para o tamanho do seu Segment or segmento e uma estimativa de qual porcentagem da sua base de usuários total se enquadra nesse Segment or segmento. As estatísticas estimadas ficam em cache por 15 minutos, a menos que você faça edições no seu Segment or segmento. Nesse caso, as estatísticas estimadas serão atualizadas automaticamente. Você também pode visualizar uma contagem exata de usuários contatáveis (tanto para o Segment or segmento geral quanto por canal) selecionando **Calcular estatísticas exatas**.
...
Em casos em que o _Total_ é maior que a soma dos canais, a diferença representa o número de usuários que se qualificaram para o Segment or segmento, mas não são contatáveis por esses canais de comunicação.
...
Para visualizar uma contagem precisa do número de usuários no seu Segment or segmento, selecione **Calcular estatísticas exatas** no painel **Usuários contatáveis**.
...
## Visualizando o tamanho histórico de membros do Segment or segmento {#viewing-historical-segment-membership-size}

Para todos os Segments, você pode visualizar um gráfico histórico de membros que mostra a estimativa de membros do Segment or segmento para cada dia. Esse gráfico mostra como o tamanho do seu Segment or segmento mudou ao longo do tempo. Use o menu suspenso para filtrar os membros do Segment or segmento por intervalo de datas.

![Use o menu suspenso de membros históricos para filtrar os membros do Segment or segmento por intervalo de datas.]({% image_buster /assets/img_archive/historical_membership2.png %})

Como o objetivo desse gráfico é dar uma noção das tendências gerais de membros do Segment or segmento, a contagem diária é uma estimativa, semelhante a como o tamanho do Segment or segmento é uma estimativa antes de você selecionar **Calculate Exact Statistics**. E como esse gráfico mostra estimativas, é possível que o tamanho do seu Segment or segmento apareça como "0" nesse gráfico, mesmo que o tamanho real (que pode ser determinado após selecionar **Calculate Exact Stats**) não seja "0". É especialmente provável que o gráfico mostre uma estimativa de "0" se o seu Segment or segmento for muito pequeno em relação ao tamanho da população do seu espaço de trabalho.

Por exemplo, digamos que seu espaço de trabalho contém 100 milhões de usuários e seu Segment or segmento tem cerca de 700 usuários. É possível que, em alguns dias, nenhum usuário esteja no Segment or segmento e nenhum usuário caia na faixa de bucket aleatório usada para a estimativa histórica de membros, resultando em uma contagem de membros de 0 para aquele dia.

A Braze estima a contagem de membros do Segment or segmento consultando um subconjunto dos seus usuários e, em seguida, extrapolando esses resultados para todo o seu público. Isso significa que os resultados do gráfico fornecem apenas uma estimativa de quantos membros o Segment or segmento pode ter naquele dia, e é esperado que também haja flutuações diárias, pois uma amostra diferente de usuários pode ser consultada para essa estimativa a cada dia.
...
| Usuários são importados por CSV | Um arquivo CSV de usuários foi importado, aumentando significativamente os membros do Segment or segmento. |
| Critérios de público do Segment or segmento são modificados | As regras de público de um Segment or segmento existente (como filtros) foram alteradas, causando mudanças significativas nos membros do Segment or segmento. |
| Usuários são excluídos | Um número significativo de usuários foi excluído. |
| Uma integração com parceiros sincronizou com a Braze | Um terceiro enviou dados para a Braze que influenciaram significativamente os membros do Segment or segmento. |
