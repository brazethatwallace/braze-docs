...
Um Segment pode referenciar um filtro calculado junto com uma extensão de segmento SQL ou CDI — por exemplo, uma coorte definida no data warehouse a partir de uma extensão, combinada com regras de compra ou eventos personalizados que você mantém no criador de filtros calculados.

## Criar um filtro calculado {#create-a-calculated-filter}

Para criar um filtro calculado, defina critérios com base no comportamento do usuário, depois salve e ative o filtro antes de usá-lo em um Segment.
...
Ao escolher seu período de tempo, você pode especificar um intervalo de datas relativo (os últimos X dias), uma data de início, uma data de término ou um intervalo de datas exato.
...
Selecione **Salvar** para salvar seu filtro calculado. Você pode salvar um filtro sem ativá-lo, mas é necessário ativar o filtro antes que ele apareça como opção ao criar um Segment.

Depois que você ativa um filtro calculado, a Braze o avalia em tempo real quando um Segment, Campaign ou Canvas que o referencia é avaliado.
...
### Posso arquivar filtros calculados se eles existirem em uma Campaign ativa? {#can-i-archive-calculated-filters-if-they-exist-in-an-active-campaign}

Não. Antes de arquivar um filtro calculado, você precisa removê-lo de todos os envios de mensagens ativos.
