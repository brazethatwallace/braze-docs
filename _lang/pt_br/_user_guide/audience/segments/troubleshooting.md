...
Esse erro raro ocorre quando o público-alvo contém muitos valores de regex, valores de regex excessivamente longos, filtros muito detalhados (como "é qualquer um de 30.000 CEPs") ou filtros demais. Isso inclui todos os filtros no público de uma Campaign ou Canvas, sejam eles localizados nos Segments referenciados ou adicionados como filtros na etapa **Target Audience**.
...
![Erro para um filtro de atributo personalizado, `menu_item`, cujo valor de atributo excede 10.000 caracteres.]({% image_buster /assets/img/segment/segment_filter_error.png %})
...
Os usuários podem ter vários apps, então selecionar um app específico na seção **Apps usados** da página de segmentação retornará resultados de usuários que possuem pelo menos esse app. O filtro não retorna resultados apenas para os usuários que possuem exclusivamente esse app.

## Filtragem {#filtering}

### As opções de filtro mudaram {#filter-options-changed}

Suas opções de filtro estão relacionadas ao formato (tipo de dados) que você está enviando à Braze para seu atributo personalizado. Para revisar o tipo de dados que a Braze está reconhecendo para seus atributos personalizados, acesse **Data Settings** > **Custom Attributes**.

Se suas opções de filtro mudaram, isso indica que seus dados estão sendo enviados à Braze em um formato (tipo de dados) diferente do anterior. Para descrições detalhadas dos diferentes tipos de dados e suas opções de filtragem, consulte [tipos de dados de atributos personalizados]({{site.baseurl}}/user_guide/data/activation/custom_data/data_types#custom-attribute-data-types).

Lembre-se de que alterar o tipo de dados de um atributo personalizado no dashboard rejeita dados enviados à Braze em um formato diferente. Não é possível alterar o tipo de dados de um atributo personalizado enquanto esse atributo estiver referenciado em Campaigns, Canvas ou Segments ativos; o dashboard exibe um erro e bloqueia a alteração.
