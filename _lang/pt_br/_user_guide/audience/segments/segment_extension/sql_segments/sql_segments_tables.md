---
nav_title: "Referência de tabelas SQL"
article_title: "Referência de tabelas SQL"
page_order: 3
page_type: reference
toc_headers: h2
description: "Esta página é uma referência das tabelas e colunas SQL do Snowflake usadas no Query Builder, nas extensões de Segment SQL e no Compartilhamento de dados do Snowflake."
tool: Segments
---
...
A maioria das tabelas está disponível nas três ferramentas. As tabelas marcadas como **Somente Compartilhamento de dados do Snowflake** são exclusivas do Compartilhamento de dados do Snowflake e não estão acessíveis no Query Builder ou nas extensões de Segment SQL.
...
[AGENTCONSOLE_RAWLLMREQUEST_SHARED](#AGENTCONSOLE_RAWLLMREQUEST_SHARED) | Informações brutas de cada chamada LLM (**somente Snowflake Data Sharing**)
...
[USER_DEFAULT_ATTRIBUTES_HISTORY_VIEW_SHARED](#USER_DEFAULT_ATTRIBUTES_HISTORY_VIEW_SHARED) | Histórico de atributos padrão do perfil com intervalos de datas de vigência
...
[USER_CUSTOM_ATTRIBUTES_HISTORY_VIEW_SHARED](#USER_CUSTOM_ATTRIBUTES_HISTORY_VIEW_SHARED) | Histórico de atributos personalizados do perfil com intervalos de datas de vigência (**somente Snowflake Data Sharing**)
[USER_LATEST_STATE_CUSTOM_ATTRIBUTE_VIEW_SHARED](#USER_LATEST_STATE_CUSTOM_ATTRIBUTE_VIEW_SHARED) | Atributos personalizados do perfil por usuário em tempo quase real (**somente Snowflake Data Sharing**)
[USER_DEFAULT_ATTRIBUTES_HISTORY_RAW_VIEW_SHARED](#USER_DEFAULT_ATTRIBUTES_HISTORY_RAW_VIEW_SHARED) | Histórico bruto de atributos padrão do perfil sem data de encerramento (**somente Snowflake Data Sharing**)
[USER_CUSTOM_ATTRIBUTES_HISTORY_RAW_VIEW_SHARED](#USER_CUSTOM_ATTRIBUTES_HISTORY_RAW_VIEW_SHARED) | Histórico bruto de atributos personalizados do perfil sem data de encerramento (**somente Snowflake Data Sharing**)
[CATALOGS_ITEMS_SHARED](#CATALOGS_ITEMS_SHARED) | Itens de catálogo não excluídos
[CHANGELOGS_CAMPAIGN_SHARED](#CHANGELOGS_CAMPAIGN_SHARED) | Quando uma Campaign é alterada (**somente Snowflake Data Sharing**)
...
[USERS_BEHAVIORS_LOCATION_SHARED](#USERS_BEHAVIORS_LOCATION_SHARED) | Quando um usuário registra uma localização
[USERS_BEHAVIORS_PURCHASE_SHARED](#USERS_BEHAVIORS_PURCHASE_SHARED) | Quando um usuário realiza uma compra
...
[USERS_BEHAVIORS_GEOFENCE_DATAEVENT_SHARED](#USERS_BEHAVIORS_GEOFENCE_DATAEVENT_SHARED) | Quando um usuário aciona uma área com geofence — por exemplo, ao entrar ou sair de um geofence. Este evento é agrupado com outros eventos e recebido por meio do endpoint padrão de eventos, então pode não aparecer em tempo real.<br><br>Para registrar a atividade de geofence nesta tabela, selecione **Enable Analytics for Enter** e **Enable Analytics for Exit** nas configurações avançadas de cada geofence. Consulte a etapa 3 em [Criar geofences manualmente]({{site.baseurl}}/user_guide/audience/locations_and_geofences/creating_geofences#manually-create-geofences) para mais detalhes.
[USERS_BEHAVIORS_GEOFENCE_RECORDEVENT_SHARED](#USERS_BEHAVIORS_GEOFENCE_RECORDEVENT_SHARED) | Quando um usuário aciona uma área com geofence (por exemplo, ao entrar ou sair de um geofence). Este evento é recebido por meio do endpoint dedicado de geofence e, portanto, é recebido em tempo real assim que o dispositivo do usuário detecta que acionou um geofence. <br><br>Além disso, devido ao limite de frequência no endpoint de geofence, é possível que alguns eventos de geofence não sejam refletidos como RecordEvent. No entanto, todos os eventos de geofence são representados por DataEvent (mas potencialmente com algum atraso devido ao agrupamento).
...
[USERS_CAMPAIGNS_CONVERSION_SHARED](#USERS_CAMPAIGNS_CONVERSION_SHARED) | Quando um usuário converte em uma Campaign
[USERS_CAMPAIGNS_ENROLLINCONTROL_SHARED](#USERS_CAMPAIGNS_ENROLLINCONTROL_SHARED) | Quando um usuário é inscrito no grupo de controle de uma Campaign
[USERS_CAMPAIGNS_FREQUENCYCAP_SHARED](#USERS_CAMPAIGNS_FREQUENCYCAP_SHARED) | Quando um usuário atinge o limite de frequência de uma Campaign
...
[USERS_CANVAS_CONVERSION_SHARED](#USERS_CANVAS_CONVERSION_SHARED) | Quando um usuário converte em um evento de conversão de Canvas
[USERS_CANVAS_ENTRY_SHARED](#USERS_CANVAS_ENTRY_SHARED) | Quando um usuário entra em um Canvas
[USERS_CANVAS_EXIT_MATCHEDAUDIENCE_SHARED](#USERS_CANVAS_EXIT_MATCHEDAUDIENCE_SHARED) | Quando um usuário sai de um Canvas por corresponder aos critérios de saída de público
[USERS_CANVAS_EXIT_PERFORMEDEVENT_SHARED](#USERS_CANVAS_EXIT_PERFORMEDEVENT_SHARED) | Quando um usuário sai de um Canvas por ter realizado um evento de exceção
[USERS_CANVAS_EXPERIMENTSTEP_CONVERSION_SHARED](#USERS_CANVAS_EXPERIMENTSTEP_CONVERSION_SHARED) | Quando um usuário converte em uma etapa de experimento de Canvas
...
[USERS_CANVAS_FREQUENCYCAP_SHARED](#USERS_CANVAS_FREQUENCYCAP_SHARED) | Quando um usuário atinge o limite de frequência para uma etapa de Canvas
[USERS_CANVAS_REVENUE_SHARED](#USERS_CANVAS_REVENUE_SHARED) | Quando um usuário gera receita dentro do período de evento de conversão primária
[USERS_CANVAS_COSTEP_CONVERSION_SHARED](#USERS_CANVAS_COSTEP_CONVERSION_SHARED) | Eventos de conversão para a etapa do Canvas de Otimizador de Conteúdo
[USERS_CANVAS_COSTEP_SEND_SHARED](#USERS_CANVAS_COSTEP_SEND_SHARED) | Os envios do Canvas para a etapa do Canvas de otimização de conteúdo
...
[USERS_MESSAGES_EMAIL_BOUNCE_SHARED](#USERS_MESSAGES_EMAIL_BOUNCE_SHARED) | Um provedor de serviços de e-mail retornou um hard bounce. Um hard bounce indica uma falha permanente de entregabilidade.
...
[USERS_MESSAGES_LINE_ABORT_SHARED](#USERS_MESSAGES_LINE_ABORT_SHARED) | Quando uma mensagem LINE agendada não pode ser entregue, antes do envio para o LINE
...
[USERS_MESSAGES_NEWSFEEDCARD_ABORT_SHARED](#USERS_MESSAGES_NEWSFEEDCARD_ABORT_SHARED) | Um cartão do News Feed originalmente agendado foi abortado por algum motivo
...
[USERS_MESSAGES_PUSHNOTIFICATION_ABORT_SHARED](#USERS_MESSAGES_PUSHNOTIFICATION_ABORT_SHARED) | Uma notificação por push originalmente agendada foi abortada por algum motivo.
...
[USERS_MESSAGES_RCS_ABORT_SHARED](#USERS_MESSAGES_RCS_ABORT_SHARED) | Quando um envio RCS é interrompido devido a um erro detectado na Braze e a mensagem é descartada
...
[USERS_MESSAGES_RCS_READ_SHARED](#USERS_MESSAGES_RCS_READ_SHARED) | Quando o usuário final abre uma mensagem RCS no dispositivo
...
[USERS_MESSAGES_SMS_DELIVERYFAILURE_SHARED](#USERS_MESSAGES_SMS_DELIVERYFAILURE_SHARED) | Quando a Braze não consegue entregar a mensagem SMS ao provedor de serviços de SMS
...
[USERS_MESSAGES_WHATSAPP_ABORT_SHARED](#USERS_MESSAGES_WHATSAPP_ABORT_SHARED) | Uma mensagem WhatsApp originalmente agendada foi abortada por algum motivo
[USERS_MESSAGES_WHATSAPP_CLICK_SHARED](#USERS_MESSAGES_WHATSAPP_CLICK_SHARED) | Quando um usuário clica em um link ou botão em uma mensagem WhatsApp
[USERS_MESSAGES_WHATSAPP_DELIVERY_SHARED](#USERS_MESSAGES_WHATSAPP_DELIVERY_SHARED) | Quando uma mensagem WhatsApp é entregue
[USERS_MESSAGES_WHATSAPP_FAILURE_SHARED](#USERS_MESSAGES_WHATSAPP_FAILURE_SHARED) | Quando uma mensagem WhatsApp não é entregue a um usuário
[USERS_MESSAGES_WHATSAPP_INBOUNDRECEIVE_SHARED](#USERS_MESSAGES_WHATSAPP_INBOUNDRECEIVE_SHARED) | Quando uma mensagem WhatsApp é recebida de um usuário
[USERS_MESSAGES_WHATSAPP_READ_SHARED](#USERS_MESSAGES_WHATSAPP_READ_SHARED) | Quando um usuário abre uma mensagem WhatsApp
[USERS_MESSAGES_WHATSAPP_SEND_SHARED](#USERS_MESSAGES_WHATSAPP_SEND_SHARED) | Quando enviamos uma mensagem WhatsApp para um usuário
[USERS_MESSAGES_WHATSAPP_RETRY_SHARED](#USERS_MESSAGES_WHATSAPP_RETRY_SHARED) | Quando uma mensagem WhatsApp é reenviada após ser despriorizada ou ter atingido o limite de frequência (**somente Snowflake Data Sharing**)
[USERS_MESSAGES_BANNER_DISMISS_SHARED](#USERS_MESSAGES_BANNER_DISMISS_SHARED) | Quando um usuário dispensa um banner.
[USERS_MESSAGES_LANDINGPAGE_CLICK_SHARED](#USERS_MESSAGES_LANDINGPAGE_CLICK_SHARED) | Este evento ocorre quando um usuário final clica em determinados elementos e campos de formulário em uma landing page
[USERS_MESSAGES_LANDINGPAGE_FORMSUBMISSION_SHARED](#USERS_MESSAGES_LANDINGPAGE_FORMSUBMISSION_SHARED) | Este evento ocorre quando um usuário final preenche um formulário em uma landing page e clica no botão para enviar as informações
[USERS_MESSAGES_LANDINGPAGE_IMPRESSION_SHARED](#USERS_MESSAGES_LANDINGPAGE_IMPRESSION_SHARED) | Este evento ocorre quando o navegador de um usuário final carrega e exibe uma landing page
[USERS_MESSAGES_PUSHNOTIFICATION_RETRY_SHARED](#USERS_MESSAGES_PUSHNOTIFICATION_RETRY_SHARED) | Este evento ocorre quando uma mensagem é despriorizada ou atinge o limite de frequência e será reenviada posteriormente dentro da janela de reenvio configurada. Disponível apenas para clientes beta do recurso de priorização de mensagens
[USERS_MESSAGES_SURVEY_RESPONSE_SHARED](#USERS_MESSAGES_SURVEY_RESPONSE_SHARED) | Respostas de pesquisa enviadas por usuários finais
...
[USERS_PROFILE_UPDATE_SHARED](#USERS_PROFILE_UPDATE_SHARED) | Mensagem representando as atualizações de perfil de um usuário
[SNAPSHOTS_APP_SHARED](#SNAPSHOTS_APP_SHARED) | Snapshots de app (**somente Snowflake Data Sharing**)
[SNAPSHOTS_CAMPAIGN_MESSAGE_VARIATION_SHARED](#SNAPSHOTS_CAMPAIGN_MESSAGE_VARIATION_SHARED) | Snapshots de variação de mensagem de Campaign (**somente Snowflake Data Sharing**)
...
[CONTENTOPTIMIZER_COMPONENTSTORE_SHARED](#CONTENTOPTIMIZER_COMPONENTSTORE_SHARED) | Atualizações do armazenamento de componentes

## Console do agente {#agent-console}

{% alert note %}
As tabelas do Console do agente estão disponíveis somente no Compartilhamento de dados do Snowflake.
{% endalert %}
...
`invocation_id` | `string` | ID globalmente exclusivo para esta mensagem
`request_id` | `string` | ID exclusivo para esta solicitação geral de LLM e execução completa
`duration` | `int` | Duração da sessão em segundos
`prompt_tokens` | `int` | Quantos tokens de prompt esta solicitação utilizou
`completion_tokens` | `int` | Quantos tokens de conclusão esta solicitação utilizou
`total_tokens` | `int` | Quantos tokens no total esta solicitação utilizou
`cache_tokens` | `int` | Quantos tokens em cache esta solicitação utilizou
`reasoning_tokens` | `int` | Quantos tokens de raciocínio esta solicitação utilizou
`time` | `int` | Timestamp UNIX em que o evento ocorreu
`app_group_id` | `string` | ID BSON do grupo de apps ao qual este usuário pertence
...
`model_name` | `string` | Nome do modelo LLM utilizado nesta solicitação
`provider_request_id` | `string` | Qualquer ID de solicitação fornecido pelo provedor do modelo para a chamada de API
`cache_hit` | `boolean` | Se esta solicitação utilizou o cache para retornar a resposta
`llm_owned_by_customer` | `boolean` | Se verdadeiro, a chave de API do cliente foi utilizada; se falso, a chave da Braze foi utilizada
`is_error` | `boolean` | Se esta solicitação resultou em erro
...
`user_id` | `string` | [IPI] ID de usuário Braze do usuário que realizou este evento
...
`invocation_source` | `null,`&nbsp;`string` | Qual objeto Ruby invocou a solicitação de LLM
`sf_created_at` | `timestamp`,&nbsp;`null` | Quando este evento foi capturado pelo Snowpipe
`canvas_name` | `string` | Nome do Canvas
`canvas_variation_name` | `string` | Nome da variação do Canvas que este usuário recebeu
`canvas_step_name` | `string` | Nome da etapa do Canvas
`error` | `string` | Nome do erro
`thinking_level` | `string` | O nível de pensamento/raciocínio utilizado para a solicitação
...
`request_id` | `string` | ID exclusivo para esta solicitação geral de LLM e execução completa
`time` | `int` | Timestamp UNIX em que o evento ocorreu
`app_group_id` | `string` | ID BSON do grupo de apps ao qual este evento pertence
`agent_id` | `string` | ID BSON do CustomerDefinedAgent
`agent_name` | `string` | Nome do CustomerDefinedAgent
`model_provider` | `string` | Nome do provedor do modelo LLM
`model_name` | `string` | Nome do modelo LLM utilizado nesta solicitação
`duration` | `int`,&nbsp;`null` | Duração da sessão em segundos
`request` | `string` | [IPI] Prompt utilizado na solicitação
`http_status_code` | `int`,&nbsp;`null` | Código de status HTTP da resposta
`response_body` | `string`,&nbsp;`null` | [IPI] Resposta do LLM
`sf_created_at` | `timestamp`,&nbsp;`null` | Quando este evento foi capturado pelo Snowpipe
...
------|------|------------
`id` | `string` | ID globalmente exclusivo para este evento
`tool_call_id` | `string` | ID globalmente exclusivo para esta chamada de ferramenta
`duration` | `int` | Duração da sessão em segundos
`time` | `int` | Timestamp UNIX em que o evento ocorreu
`app_group_id` | `string` | ID BSON do grupo de apps ao qual este usuário pertence
`agent_id` | `string` | ID BSON do CustomerDefinedAgent
...
Campo | Tipo | Descrição
------|------|------------
`app_group_id` | `string` | ID BSON do espaço de trabalho
`app_id` | `string` | ID BSON do app
`user_id` | `string` | [IPI] ID de usuário Braze
`time` | `int` | Timestamp UNIX em segundos da atualização do perfil (para linhas preenchidas retroativamente, o horário do preenchimento)
`time_ms` | `int` | Timestamp UNIX em milissegundos da atualização do perfil (para linhas preenchidas retroativamente, o horário do preenchimento)
`update_source` | `string` | Origem da atualização do perfil
`sf_updated_at` | `timestamp` | Quando esta linha foi atualizada no Snowflake
`custom_attributes` | `variant` | [IPI] Atributos personalizados como um objeto JSON
`archived` | `boolean` | Se o perfil de usuário está arquivado
`external_user_id` | `string` | [IPI] ID externo do usuário
...
`sf_updated_at` | `timestamp` | Quando esta linha foi atualizada no Snowflake
`external_user_id` | `string` | [IPI] ID externo do usuário
`first_name` | `string` | [IPI] Nome
`last_name` | `string` | [IPI] Sobrenome
`email_address` | `string` | [IPI] Endereço de e-mail
`gender` | `string` | [IPI] Gênero
`phone_number` | `string` | [IPI] Número de telefone
`dob` | `string` | [IPI] Data de nascimento
`TIME_ZONE` | `string` | [IPI] Fuso horário
`home_city` | `string` | [IPI] Cidade de residência
`country` | `string` | [IPI] País
`language` | `string` | [IPI] Idioma
`archived` | `boolean` | Se o perfil de usuário está arquivado
...
`country` | `string` | [IPI] País
`language` | `string` | [IPI] Idioma
`eff_dt` | `timestamp` | Início do intervalo em que este estado de atributo estava vigente
`end_dt` | `timestamp` | Fim desse intervalo
`archived` | `boolean` | Se o perfil de usuário está arquivado
...
Para orientações de uso e consultas de exemplo, consulte [Atributos de usuário do Snowflake]({{site.baseurl}}/partners/data_and_analytics/data_warehouses/snowflake/user_attributes#historical-change-logs).
...
Para orientações de uso e consultas de exemplo, consulte [Atributos de usuário do Snowflake]({{site.baseurl}}/partners/data_and_analytics/data_warehouses/snowflake/user_attributes#real-time-user-profile-views).
...
`app_id` | `string` | ID BSON do app
`update_source` | `string` | Origem da atualização do perfil
`time` | `int` | Timestamp UNIX em segundos da atualização do perfil
`archived` | `boolean` | Se o perfil de usuário está arquivado
`sf_updated_at` | `timestamp` | Quando esta linha foi atualizada no Snowflake
`first_name` | `string` | [IPI] Nome
`last_name` | `string` | [IPI] Sobrenome
...
`app_group_id` | `null,`&nbsp;`string` | ID BSON do grupo de apps
`app_group_api_id` | `null,`&nbsp;`string` | ID de API do grupo de apps
...
`app_group_id` | `null,`&nbsp;`string` | ID BSON do grupo de apps ao qual este usuário pertence
`app_group_api_id` | `null,`&nbsp;`string` | ID de API do grupo de apps ao qual este usuário pertence
...
`global_control_group` | `null, boolean` | Com essa alteração, o número de bucket está incluído no grupo de controle global
`previous_global_control_group` | `null, boolean` | Antes dessa alteração, o número de bucket estava incluído no grupo de controle global, mas não está mais
...
`time` | `int` | Registro de data e hora UNIX em que o evento ocorreu
`app_group_id` | `string` | ID BSON do grupo de apps ao qual este usuário pertence
`api_id` | `string` | ID de API da Campaign
`name` | `null,`&nbsp;`string` | Nome da Campaign
`conversion_behaviors` | `null,`&nbsp;`string` | Comportamentos de conversão da Campaign
`actions` | `null,`&nbsp;`string` | Ações da Campaign
...
`time` | `int` | Registro de data e hora UNIX em que o evento ocorreu
`app_group_id` | `string` | ID BSON do grupo de apps ao qual este usuário pertence
`api_id` | `string` | ID de API do Canvas
...
------|------|----------
`id` | `string` | ID globalmente exclusivo para este evento
`user_id` | `string` | ID Braze do usuário que realizou o evento
`external_user_id` | `null,`&nbsp;`string` | [IPI] ID externo do usuário
`app_group_api_id` | `null,`&nbsp;`string` | ID de API do espaço de trabalho ao qual este usuário pertence
`app_api_id` | `null,`&nbsp;`string` | ID de API do app no qual esta ação ocorreu
`time` | `int` | Registro de data/hora Unix em que o usuário realizou o evento
...
`app_group_id` | `null,`&nbsp;`string` | BSON ID do grupo de apps ao qual este usuário pertence
`sf_created_at` | `timestamp`,&nbsp;`null` | Quando este evento foi capturado pelo Snowpipe
...
------|------|----------
`id` | `string` | ID globalmente exclusivo para este evento
`user_id` | `string` | ID Braze do usuário que realizou a instalação
`external_user_id` | `null,`&nbsp;`string` | [IPI] ID externo do usuário
`device_id` | `null,`&nbsp;`string` | `device_id` vinculado a este usuário caso o usuário seja anônimo
`app_group_api_id` | `null,`&nbsp;`string` | ID de API do espaço de trabalho ao qual este usuário pertence
`time` | `int` | Registro de data/hora Unix em que o usuário realizou a instalação
...
------|------|----------
`id` | `string` | ID globalmente exclusivo para este evento
`user_id` | `string` | ID Braze do usuário que registra a localização
`external_user_id` | `null,`&nbsp;`string` | [IPI] ID externo do usuário
`app_group_api_id` | `null,`&nbsp;`string` | ID de API do espaço de trabalho ao qual este usuário pertence
`app_api_id` | `null,`&nbsp;`string` | ID de API do app no qual esta localização foi registrada
`time` | `int` | Registro de data/hora Unix em que a localização foi registrada
...
`ll_accuracy` | `null, float` | Precisão de latitude e longitude da localização registrada
`alt_accuracy` | `null, float` | Precisão de altitude da localização registrada
`device_id` | `null,`&nbsp;`string` | ID do dispositivo no qual a localização foi registrada
`sdk_version` | `null,`&nbsp;`string` | Versão do SDK da Braze em uso quando a localização foi registrada
`platform` | `null,`&nbsp;`string` | Plataforma do dispositivo
`os_version` | `null,`&nbsp;`string` | Versão do sistema operacional do dispositivo
`device_model` | `null,`&nbsp;`string` | Modelo do dispositivo
`ad_id` | `null,`&nbsp;`string` | [IPI] Identificador de publicidade
...
------|------|----------
`id` | `string` | ID globalmente exclusivo para este evento
`user_id` | `string` | ID Braze do usuário que realizou a compra
`external_user_id` | `null,`&nbsp;`string` | [IPI] ID externo do usuário
`app_group_api_id` | `null,`&nbsp;`string` | ID de API do espaço de trabalho ao qual este usuário pertence
`app_api_id` | `null,`&nbsp;`string` | ID de API do app no qual a compra ocorreu
`time` | `int` | Registro de data/hora Unix em que o usuário realizou a compra
...
------|------|----------
`id` | `string` | ID globalmente exclusivo para este evento
`user_id` | `string` | ID Braze do usuário que desinstalou
`external_user_id` | `null,`&nbsp;`string` | [IPI] ID externo do usuário
`device_id` | `null,`&nbsp;`string` | `device_id` vinculado a este usuário caso o usuário seja anônimo
`app_group_api_id` | `null,`&nbsp;`string` | ID de API do espaço de trabalho ao qual este usuário pertence
`app_api_id` | `null,`&nbsp;`string` | ID de API do app que foi desinstalado
`time` | `int` | Registro de data/hora Unix em que o usuário desinstalou
`app_group_id` | `null,`&nbsp;`string` | BSON ID do grupo de apps ao qual este usuário pertence
`sf_created_at` | `timestamp`,&nbsp;`null` | Quando este evento foi capturado pelo Snowpipe
...
------|------|----------
`id` | `string` | ID globalmente exclusivo para este evento
`user_id` | `string` | ID Braze do usuário que atualizou o app
`external_user_id` | `null,`&nbsp;`string` | [IPI] ID externo do usuário
`app_group_api_id` | `null,`&nbsp;`string` | ID de API do espaço de trabalho ao qual este usuário pertence
`app_api_id` | `null,`&nbsp;`string` | ID de API do app que o usuário atualizou
`time` | `int` | Registro de data/hora Unix em que o usuário atualizou o app
`device_id` | `null,`&nbsp;`string` | ID do dispositivo no qual o usuário atualizou o app
...
------|------|----------
`id` | `string` | ID globalmente exclusivo para este evento
`user_id` | `string` | ID Braze do usuário que realiza esta ação
`external_user_id` | `null,`&nbsp;`string` | [IPI] ID externo do usuário
`app_group_api_id` | `null,`&nbsp;`string` | ID de API do espaço de trabalho ao qual este usuário pertence
`app_api_id` | `null,`&nbsp;`string` | ID de API do app no qual esta sessão ocorreu
`time` | `int` | Registro de data/hora Unix em que a sessão começou
...
`app_group_id` | `null,`&nbsp;`string` | BSON ID do grupo de apps ao qual este usuário pertence
`app_group_api_id` | `null,`&nbsp;`string` | ID de API do grupo de apps ao qual este usuário pertence
`app_api_id` | `null,`&nbsp;`string` | ID de API do app no qual este evento ocorreu
`time` | `int` | Registro de data/hora UNIX em que o evento aconteceu
...
`app_group_api_id` | `null,`&nbsp;`string` | ID de API do espaço de trabalho ao qual este usuário pertence
`app_api_id` | `null,`&nbsp;`string` | ID de API do app no qual esta sessão ocorreu
`time` | `int` | Registro de data/hora Unix em que a sessão terminou
...
`os_version` | `null,`&nbsp;`string` | Versão do sistema operacional do dispositivo
`device_model` | `null,`&nbsp;`string` | Modelo do dispositivo
`event_type` | `string` | Tipo de evento de geofence que foi disparado (por exemplo, 'enter' ou 'exit')
`location_set_id` | `string` | O ID do conjunto de localizações do geofence que foi disparado
`geofence_id` | `string` | O ID do geofence que foi disparado
`app_group_id` | `null,`&nbsp;`string` | BSON ID do grupo de apps ao qual este usuário pertence
`sf_created_at` | `timestamp`,&nbsp;`null` | Quando este evento foi capturado pelo Snowpipe
...
`id` | `string` | ID globalmente exclusivo para este evento
`time` | `int` | Registro de data/hora UNIX em que o evento aconteceu
`time_ms` | `int` | Hora em milissegundos em que o evento aconteceu
`user_id` | `string` | ID Braze do usuário que realizou este evento
`external_user_id` | `null,`&nbsp;`string` | [IPI] ID externo do usuário
`sdk_version` | `null,`&nbsp;`string` | Versão do SDK da Braze em uso durante o evento
`platform` | `null,`&nbsp;`string` | Plataforma do dispositivo
...
`push_token_created_at` | `null, int` | Registro de data/hora UNIX em que o token por push foi criado
`push_token_updated_at` | `null, int` | Registro de data/hora UNIX em que o token por push foi atualizado pela última vez
...
`ios_push_token_apns_gateway` | `null, int` | Gateway APNS do token por push; aplica-se apenas a tokens por push iOS: 1 para desenvolvimento, 2 para produção
`web_push_token_public_key` | `null,`&nbsp;`string` | Chave pública do token por push; aplica-se apenas a tokens web push
`web_push_token_user_auth` | `null,`&nbsp;`string` | Autenticação do usuário do token por push; aplica-se apenas a tokens web push
`web_push_token_vapid_public_key` | `null,`&nbsp;`string` | Chave pública VAPID do token por push; aplica-se apenas a tokens web push
`push_token_state_change_type` | `null,`&nbsp;`string` | Uma descrição do tipo de alteração de estado do token por push
`app_group_api_id` | `null,`&nbsp;`string` | ID de API do grupo de apps ao qual este usuário pertence
`app_api_id` | `null,`&nbsp;`string` | ID de API do app no qual este evento ocorreu
`sf_created_at` | `timestamp`,&nbsp;`null` | Quando este evento foi capturado pelo Snowpipe
...
------|------|----------
`id` | `string` | ID globalmente exclusivo para este evento
`user_id` | `string` | ID Braze do usuário afetado
`external_user_id` | `null,`&nbsp;`string` | [IPI] ID externo do usuário
...
`channel` | `null,`&nbsp;`string` | Canal do estado global de inscrição, como e-mail
`time` | `int` | Registro de data/hora Unix em que o estado de inscrição foi alterado
`timezone` | `null,`&nbsp;`string` | Fuso horário do usuário
`app_group_api_id` | `null,`&nbsp;`string` | ID de API do espaço de trabalho ao qual este usuário pertence
`app_api_id` | `null,`&nbsp;`string` | ID de API do app ao qual o evento pertence
`campaign_id` | `null,`&nbsp;`string` | ID Braze de uso interno da Campaign à qual este evento pertence
`campaign_api_id` | `null,`&nbsp;`string` | ID de API da Campaign à qual este evento pertence
`message_variation_api_id` | `null,`&nbsp;`string` | ID de API da variação de mensagem à qual este evento pertence
`canvas_id` | `null,`&nbsp;`string` | ID Braze de uso interno do Canvas ao qual este evento pertence
`canvas_api_id` | `null,`&nbsp;`string` | ID de API do Canvas ao qual este evento pertence
`canvas_variation_api_id` | `null,`&nbsp;`string` | ID de API da variação do Canvas à qual este evento pertence
`canvas_step_api_id` | `null,`&nbsp;`string` | ID de API da etapa do Canvas à qual este evento pertence
`send_id` | `null,`&nbsp;`string` | ID de envio da mensagem que originou esta ação de alteração de estado de inscrição
`app_group_id` | `null,`&nbsp;`string` | BSON ID do grupo de apps ao qual este usuário pertence
`channel_identifier` | `null,`&nbsp;`string` | [IPI] Identificador do usuário no canal referente ao evento
`sf_created_at` | `timestamp`,&nbsp;`null` | Quando este evento foi capturado pelo Snowpipe
`campaign_name` | `string` | Nome da Campaign
`message_variation_name` | `string` | Nome da variação de mensagem
`canvas_name` | `string` | Nome do Canvas
`canvas_variation_name` | `string` | Nome da variação do Canvas que este usuário recebeu
`canvas_step_name` | `string` | Nome da etapa do Canvas
...
`canvas_variation_api_id` | `null,`&nbsp;`string` | ID de API da variação do Canvas à qual este evento pertence
`canvas_step_api_id` | `null,`&nbsp;`string` | ID de API da etapa do Canvas à qual este evento pertence
`subscription_group_api_id` | `string` | ID de API do grupo de inscrições
...
`device_id` | `null,`&nbsp;`string` | `device_id` vinculado a este usuário, caso o usuário seja anônimo
`app_group_api_id` | `null,`&nbsp;`string` | ID de API do espaço de trabalho ao qual este usuário pertence
`time` | `int` | Carimbo de data/hora Unix em que o evento ocorreu
`app_api_id` | `null,`&nbsp;`string` | ID de API do app no qual este evento ocorreu
...
`campaign_id` | `string` | ID Braze de uso interno da campaign à qual este evento pertence
`campaign_api_id` | `null,`&nbsp;`string` | ID de API da campaign à qual este evento pertence
`message_variation_api_id` | `null,`&nbsp;`string` | ID de API da variação de mensagem que este usuário recebeu
...
`campaign_name` | `string` | Nome da campaign
`message_variation_name` | `string` | Nome da variação de mensagem
`conversion_behavior` | `string` | String codificada em JSON que descreve o comportamento de conversão
...
`timezone` | `null,`&nbsp;`string` | Fuso horário do usuário
`language` | `null,`&nbsp;`string` | [IPI] Idioma do usuário
`revenue` | `long` | O valor da receita em USD em centavos gerada
`app_group_id` | `null,`&nbsp;`string` | BSON ID do grupo de apps ao qual este usuário pertence
`sf_created_at` | `timestamp`,&nbsp;`null` | Quando este evento foi captado pelo Snowpipe
...
| `user_id`                              | `string`,&nbsp;`null`    | ID Braze do usuário que realizou este evento                                                                    |
...
| `app_group_id`                         | `string`,&nbsp;`null`    | ID Braze do espaço de trabalho ao qual este usuário pertence                                                    |
| `app_group_api_id`                     | `string`,&nbsp;`null`    | ID de API do espaço de trabalho ao qual este usuário pertence                                                   |
| `time`                                 | `int`,&nbsp;`null`       | Registro de data/hora Unix em que o evento ocorreu                                                              |
| `canvas_id`                            | `string`,&nbsp;`null`    | (Apenas para uso da Braze) ID do Canvas ao qual este evento pertence                                            |
| `canvas_api_id`                        | `string`,&nbsp;`null`    | ID de API do Canvas ao qual este evento pertence                                                                |
| `canvas_variation_api_id`              | `string`,&nbsp;`null`    | ID de API da variação do Canvas à qual este evento pertence                                                     |
| `canvas_step_api_id`                   | `string`,&nbsp;`null`    | ID de API da etapa do Canvas à qual este evento pertence                                                        |
...
| `exit_reason`                          | `string`,&nbsp;`null`    | Se esta é uma saída, o motivo pelo qual o usuário saiu do Canvas durante a etapa                                |
...
| `next_step_api_id`                     | `string`,&nbsp;`null`    | ID de API da próxima etapa no Canvas                                                                            |
| `sf_created_at`                        | `timestamp`,&nbsp;`null` | Quando este evento foi capturado pelo Snowpipe                                                                  |
| `canvas_name` | `string` | Nome do Canvas |
| `canvas_variation_name` | `string` | Nome da variação do Canvas que este usuário recebeu |
| `canvas_step_name` | `string` | Nome da etapa do Canvas |
...
| `app_group_api_id`                     | `string`,&nbsp;`null`    | ID de API do espaço de trabalho ao qual este usuário pertence                                                              |
| `time`                                 | `int`,&nbsp;`null`       | Registro de data/hora Unix em que o evento ocorreu                                                                         |
| `app_api_id`                           | `string`,&nbsp;`null`    | ID de API do app no qual este evento ocorreu                                                                               |
| `canvas_id`                            | `string`,&nbsp;`null`    | (Apenas para uso da Braze) ID do Canvas ao qual este evento pertence                                                       |
| `canvas_api_id`                        | `string`,&nbsp;`null`    | ID de API do Canvas ao qual este evento pertence                                                                           |
| `canvas_variation_api_id`              | `string`,&nbsp;`null`    | ID de API da variação do Canvas à qual este evento pertence                                                                |
| `canvas_step_api_id`                   | `string`,&nbsp;`null`    | ID de API da etapa do Canvas à qual este evento pertence                                                                   |
| `canvas_step_message_variation_api_id` | `string`,&nbsp;`null`    | ID de API da variação de mensagem da etapa do Canvas que este usuário recebeu                                              |
...
| `canvas_api_id`           | `string`,&nbsp;`null`    | ID de API do Canvas ao qual este evento pertence                     |
| `canvas_variation_api_id` | `string`,&nbsp;`null`    | ID de API da variação do Canvas à qual este evento pertence          |
| `canvas_step_api_id`      | `string`,&nbsp;`null`    | [Descontinuado] ID de API da etapa do Canvas à qual este evento pertence |
| `gender`                  | `string`,&nbsp;`null`    | [IPI] Gênero do usuário                                             |
| `country`                 | `string`,&nbsp;`null`    | [IPI] País do usuário                                               |
| `timezone`                | `string`,&nbsp;`null`    | Fuso horário do usuário                                              |
| `language`                | `string`,&nbsp;`null`    | [IPI] Idioma do usuário                                             |
...
| `canvas_variation_api_id`   | `string`,&nbsp;`null`    | ID de API da variação do Canvas à qual este evento pertence                                                                |
| `canvas_step_api_id`        | `string`,&nbsp;`null`    | ID de API da etapa do Canvas à qual este evento pertence                                                                   |
| `experiment_step_api_id`    | `string`,&nbsp;`null`    | ID de API da etapa de experimento à qual este evento pertence                                                              |
| `conversion_behavior_index` | `int`,&nbsp;`null`       | Tipo de evento de conversão que o usuário realizou, onde "0" é uma conversão primária e "1" é uma conversão secundária      |
| `sf_created_at`             | `timestamp`,&nbsp;`null` | Quando este evento foi capturado pelo Snowpipe                                                                             |
| `experiment_split_api_id` | `string`,&nbsp;`null` | ID de API da divisão do experimento na qual o usuário foi inscrito |
| `canvas_name` | `string` | Nome do Canvas |
| `canvas_variation_name` | `string` | Nome da variação do Canvas que este usuário recebeu |
| `canvas_step_name` | `string` | Nome da etapa do Canvas |
| `experiment_split_name` | `string` | Nome da divisão do experimento |
| `conversion_behavior` | `string` | String codificada em JSON que descreve o comportamento de conversão |
...
| `in_control_group`        | `boolean`,&nbsp;`null`   | Verdadeiro se o usuário foi inscrito no grupo de controle            |
| `sf_created_at`           | `timestamp`,&nbsp;`null` | Quando este evento foi capturado pelo Snowpipe                       |
| `experiment_split_api_id` | `string` | ID de API da divisão do experimento na qual o usuário foi inscrito |
| `canvas_name` | `string` | Nome do Canvas |
| `canvas_variation_name` | `string` | Nome da variação do Canvas que este usuário recebeu |
| `canvas_step_name` | `string` | Nome da etapa do Canvas |
| `experiment_split_name` | `string` | Nome da divisão do experimento |
...
Campo | Tipo | Descrição
------|------|------------
`app_group_api_id` | `string` | ID de API do grupo de apps ao qual este usuário pertence
`app_group_id` | `string` | ID BSON do grupo de apps ao qual este usuário pertence
`external_user_id` | `string` | [IPI] ID externo do usuário
`id` | `string` | ID globalmente exclusivo para este evento
`time` | `int` | Registro de data/hora UNIX em que o evento ocorreu
`user_id` | `string` | [IPI] ID de usuário Braze do usuário que realizou este evento
`dispatch_id` | `string` | ID do despacho ao qual esta mensagem pertence
`channel` | `string` | Canal ao qual este evento pertence
`conversion_type` | `string` | O tipo de conversão (abertura ou clique)
`combination_token` | `string` | Combinação de componentes atribuída
`sf_created_at` | `timestamp` | Quando este evento foi capturado pelo Snowpipe
...
`dispatch_id` | `string` | ID do despacho ao qual esta mensagem pertence
`channel` | `string` | Canal ao qual este evento pertence
`canvas_id` | `string` | ID BSON do Canvas ao qual este evento pertence
`canvas_variation_api_id` | `string` | ID de API da variação do Canvas à qual este evento pertence
`content_optimizer_step_id` | `string` | ID interno da etapa do Otimizador de Conteúdo
`combination_token` | `string` | Combinação de componentes atribuída
`sf_created_at` | `timestamp` | Quando este evento foi capturado pelo Snowpipe
...
`campaign_api_id` | `null,`&nbsp;`string` | ID de API da campanha à qual este evento pertence
`message_variation_api_id` | `null,`&nbsp;`string` | ID de API da variação de mensagem que este usuário recebeu
`gender` | `null,`&nbsp;`string` | [IPI] Gênero do usuário
`country` | `null,`&nbsp;`string` | [IPI] País do usuário
`timezone` | `null,`&nbsp;`string` | Fuso horário do usuário
...
`ad_tracking_enabled` | `null, boolean` | Se o rastreamento de publicidade está ativado para o dispositivo
`abort_type` | `null,`&nbsp;`string` | Tipo de interrupção. Para obter uma lista de valores, consulte [Tipos de interrupção](#abort-types).
`abort_log` | `null,`&nbsp;`string` | [IPI] Mensagem de log descrevendo os detalhes da interrupção (até 128 caracteres)
...
`canvas_step_name` | `string` | Nome da etapa do Canvas
`canvas_variation_name` | `string` | Nome da variação do Canvas que este usuário recebeu
`canvas_api_id` | `string` | ID de API do Canvas ao qual este evento pertence
`canvas_step_api_id` | `string` | ID de API da etapa do Canvas à qual este evento pertence
`canvas_step_message_variation_api_id` | `string` | ID de API da variação de mensagem da etapa do Canvas que este usuário recebeu
`canvas_variation_api_id` | `string` | ID de API da variação do Canvas à qual este evento pertence
...
`canvas_variation_api_id` | `string` | ID de API da variação do Canvas à qual este evento pertence
`canvas_variation_name` | `string` | Nome da variação do Canvas que este usuário recebeu
`is_unique` | `boolean` | Se este evento foi considerado exclusivo de 7 dias no momento do processamento
...
`dispatch_id` | `null,`&nbsp;`string` | ID do envio ao qual esta mensagem pertence
`send_id` | `null,`&nbsp;`string` | ID de envio da mensagem à qual esta mensagem pertence
`campaign_id` | `null,`&nbsp;`string` | ID Braze de uso interno da campanha à qual este evento pertence
`campaign_api_id` | `null,`&nbsp;`string` | ID de API da campanha à qual este evento pertence
`message_variation_api_id` | `null,`&nbsp;`string` | ID de API da variação de mensagem que este usuário recebeu
`canvas_id` | `null,`&nbsp;`string` | ID Braze de uso interno do Canvas ao qual este evento pertence
`canvas_api_id` | `null,`&nbsp;`string` | ID de API do Canvas ao qual este evento pertence
`canvas_variation_api_id` | `null,`&nbsp;`string` | ID de API da variação do Canvas à qual este evento pertence
`canvas_step_api_id` | `null,`&nbsp;`string` | ID de API da etapa do Canvas à qual este evento pertence
`canvas_step_message_variation_api_id` | `null,`&nbsp;`string` | ID de API da variação de mensagem da etapa do Canvas que este usuário recebeu
`gender` | `null,`&nbsp;`string` | [IPI] Gênero do usuário
`country` | `null,`&nbsp;`string` | [IPI] País do usuário
`timezone` | `null,`&nbsp;`string` | Fuso horário do usuário
`language` | `null,`&nbsp;`string` | [IPI] Idioma do usuário
`abort_type` | `null,`&nbsp;`string` | Tipo de interrupção. Para obter uma lista de valores, consulte [Tipos de interrupção](#abort-types).
`abort_log` | `null,`&nbsp;`string` | [IPI] Mensagem de log descrevendo os detalhes da interrupção (máximo de 2.000 caracteres)
`app_group_id` | `null,`&nbsp;`string` | ID BSON do grupo de apps ao qual este usuário pertence
`sf_created_at` | `timestamp`,&nbsp;`null` | Quando este evento foi captado pelo Snowpipe
`campaign_name` | `string` | Nome da campanha
`message_variation_name` | `string` | Nome da variação de mensagem
...
`content_card_id` | `string` | ID do cartão que gerou este evento
`app_group_id` | `null,`&nbsp;`string` | ID BSON do grupo de apps ao qual este usuário pertence
`message_extras` | `null,`&nbsp;`string` | [IPI] String JSON dos pares de chave-valor marcados durante a renderização do Liquid
`sf_created_at` | `timestamp`,&nbsp;`null` | Quando este evento foi captado pelo Snowpipe
`campaign_name` | `string` | Nome da campanha
`message_variation_name` | `string` | Nome da variação de mensagem
`canvas_name` | `string` | Nome do Canvas
...
`canvas_variation_name` | `string` | Nome da variação do Canvas que este usuário recebeu
`canvas_step_name` | `string` | Nome da etapa do Canvas
`message_extras` | `string` | [IPI] String JSON dos pares de chave-valor marcados durante a renderização do Liquid
...
`ip_pool` | `null,`&nbsp;`string` | Pool de IP a partir do qual o envio de e-mail foi feito
`bounce_reason` | `null,`&nbsp;`string` | [IPI] O código de razão SMTP e a mensagem amigável recebidos para este evento de bounce
...
`suspected_bot_click_reason` | `null, object` | Motivo pelo qual este evento foi classificado como bot
`sf_created_at` | `timestamp`,&nbsp;`null` | Quando este evento foi capturado pelo Snowpipe
`campaign_name` | `string` | Nome da Campaign
`message_variation_name` | `string` | Nome da variação de mensagem
`canvas_name` | `string` | Nome do Canvas
...
`canvas_step_name` | `string` | Nome da etapa do Canvas
`send_time` | `int` | Horário do evento de envio correspondente
`has_url_parameters` | `boolean` | Se a URL clicada continha parâmetros de consulta
`link_aliasing_enabled` | `boolean` | Se o link aliasing estava ativado para o espaço de trabalho quando este clique foi processado
...
`app_group_api_id` | `null,`&nbsp;`string` | ID de API do grupo de apps ao qual este usuário pertence
`send_id` | `null,`&nbsp;`string` | ID de envio da mensagem à qual esta mensagem pertence
`campaign_id` | `null,`&nbsp;`string` | BSON ID da Campaign à qual este evento pertence
`campaign_api_id` | `null,`&nbsp;`string` | ID de API da Campaign à qual este evento pertence
`message_variation_api_id` | `null,`&nbsp;`string` | ID de API da variação de mensagem que este usuário recebeu
...
`language` | `null,`&nbsp;`string` | [IPI] Idioma do usuário
`email_address` | `string` | [IPI] Endereço de e-mail do usuário
`user_agent` | `null,`&nbsp;`string` | User agent no qual a denúncia de spam ocorreu
`ip_pool` | `null,`&nbsp;`string` | Pool de IP a partir do qual o envio de e-mail foi feito
`esp` | `null,`&nbsp;`string` | ESP relacionado ao evento (SparkPost, SendGrid ou Amazon SES)
`from_domain` | `null,`&nbsp;`string` | Domínio de envio do e-mail
`app_group_id` | `null,`&nbsp;`string` | BSON ID do grupo de apps ao qual este usuário pertence
...
`ip_pool` | `null,`&nbsp;`string` | Pool de IP a partir do qual o envio de e-mail foi feito
`machine_open` | `null,`&nbsp;`string` | Preenchido com 'true' se o evento de abertura for disparado sem engajamento do usuário, por exemplo, por um dispositivo Apple com a Proteção de Privacidade de E-mail ativada. O valor pode mudar ao longo do tempo para oferecer mais granularidade.
`esp` | `null,`&nbsp;`string` | ESP relacionado ao evento (SparkPost, SendGrid ou Amazon SES)
`from_domain` | `null,`&nbsp;`string` | Domínio de envio do e-mail
`is_amp` | `null, boolean` | Indica que este é um evento AMP
`app_group_id` | `null,`&nbsp;`string` | BSON ID do grupo de apps ao qual este usuário pertence
...
`email_address` | `string` | [IPI] Endereço de e-mail do usuário
`ip_pool` | `null,`&nbsp;`string` | Pool de IP a partir do qual o envio de e-mail foi feito
`message_extras` | `null,`&nbsp;`string` | [IPI] Uma string JSON dos pares chave-valor tagueados durante a renderização Liquid
`esp` | `null,`&nbsp;`string` | ESP relacionado ao evento (SparkPost, SendGrid ou Amazon SES)
`from_domain` | `null,`&nbsp;`string` | Domínio de envio do e-mail
`sf_created_at` | `timestamp`,&nbsp;`null` | Quando este evento foi capturado pelo Snowpipe
`app_group_id` | `null,`&nbsp;`string` | BSON ID do grupo de apps ao qual este usuário pertence
...
Este evento ocorre quando uma mensagem é despriorizada ou limitada por frequência e é reenviada posteriormente dentro da janela de nova tentativa configurada.

Campo | Tipo | Descrição
------|------|------------
`id` | `string` | ID globalmente exclusivo para este evento
`user_id` | `string` | [IPI] ID Braze do usuário que realizou este evento
`external_user_id` | `null,`&nbsp;`string` | [IPI] ID externo do usuário
`app_group_id` | `null,`&nbsp;`string` | BSON ID do grupo de apps ao qual este usuário pertence
`app_group_api_id` | `null,`&nbsp;`string` | ID de API do grupo de apps ao qual este usuário pertence
`time` | `int` | Timestamp UNIX em que o evento ocorreu
`retry_type` | `null,`&nbsp;`string` | Tipo de nova tentativa
`retry_log` | `null,`&nbsp;`string` | Mensagem de log descrevendo os detalhes da nova tentativa
`send_id` | `null,`&nbsp;`string` | ID de envio da mensagem à qual esta mensagem pertence
`dispatch_id` | `null,`&nbsp;`string` | ID do despacho ao qual esta mensagem pertence
`campaign_id` | `null,`&nbsp;`string` | BSON ID da Campaign à qual este evento pertence
`campaign_api_id` | `null,`&nbsp;`string` | ID de API da Campaign à qual este evento pertence
...
`canvas_variation_name` | `string` | Nome da variação do Canvas que este usuário recebeu
`message_variation_name` | `string` | Nome da variação de mensagem
`is_unique` | `boolean` | Se este evento foi considerado exclusivo em 7 dias quando processado
...
`time` | `int` | Timestamp Unix em que o evento ocorreu
`app_api_id` | `null,`&nbsp;`string` | ID de API do app no qual este evento ocorreu
`card_api_id` | `null,`&nbsp;`string` | ID de API do cartão
`dispatch_id` | `null,`&nbsp;`string` | ID do despacho ao qual esta mensagem pertence
`send_id` | `null,`&nbsp;`string` | ID de envio da mensagem à qual esta mensagem pertence
`campaign_id` | `null,`&nbsp;`string` | ID Braze de uso interno da Campaign à qual este evento pertence
`campaign_api_id` | `null,`&nbsp;`string` | ID de API da Campaign à qual este evento pertence
...
`carrier` | `null,`&nbsp;`string` | Operadora do dispositivo
`browser` | `null,`&nbsp;`string` | Navegador do dispositivo
`version` | `string` | Qual versão de mensagem no app, legada ou disparada
`ad_id` | `null,`&nbsp;`string` | [IPI] Identificador de publicidade
`ad_id_type` | `null,`&nbsp;`string` | Um entre `ios_idfa`, `google_ad_id`, `windows_ad_id` OU `roku_ad_id`
`ad_tracking_enabled` | `null, boolean` | Se o rastreamento de publicidade está ativado para o dispositivo
`abort_type` | `null,`&nbsp;`string` | Tipo de interrupção. Para uma lista de valores, consulte [Tipos de interrupção](#abort-types).
`abort_log` | `null,`&nbsp;`string` | [IPI] Mensagem de log descrevendo os detalhes da interrupção (máximo de 2.000 caracteres)
`app_group_id` | `null,`&nbsp;`string` | BSON ID do grupo de apps ao qual este usuário pertence
`sf_created_at` | `timestamp`,&nbsp;`null` | Quando este evento foi capturado pelo Snowpipe
`campaign_name` | `string` | Nome da Campaign
...
`app_group_id` | `null,`&nbsp;`string` | BSON ID do grupo de apps ao qual este usuário pertence
`message_extras` | `null,`&nbsp;`string` | [IPI] Uma string JSON dos pares chave-valor tagueados durante a renderização Liquid
`locale_key` | `null,`&nbsp;`string` | [IPI] A chave correspondente às traduções (por exemplo, 'en-us') usada para compor esta mensagem (nulo para padrão).
`sf_created_at` | `timestamp`,&nbsp;`null` | Quando este evento foi capturado pelo Snowpipe
`dispatch_id` | `string` | ID do despacho ao qual esta mensagem pertence
`campaign_name` | `string` | Nome da Campaign
`message_variation_name` | `string` | Nome da variação de mensagem
...
`message_variation_api_id` | `null,`&nbsp;`string` | ID de API da variação de mensagem que este usuário recebeu
`native_line_id` | `null,`&nbsp;`string` | [IPI] O ID LINE do usuário do qual a mensagem foi enviada ou recebida
`send_id` | `null,`&nbsp;`string` | ID de envio da mensagem à qual esta mensagem pertence
`subscription_group_api_id` | `string` | ID de API do grupo de inscrições
`timezone` | `null,`&nbsp;`string` | Fuso horário do usuário
`campaign_name` | `null,`&nbsp;`string` | Nome da Campaign
`canvas_step_name` | `null,`&nbsp;`string` | Nome da etapa do Canvas
`canvas_variation_api_id` | `null,`&nbsp;`string` | ID de API da variação do Canvas à qual este evento pertence
`canvas_api_id` | `null,`&nbsp;`string` | ID de API do Canvas ao qual este evento pertence
`sf_created_at` | `timestamp`,&nbsp;`null` | Quando este evento foi capturado pelo Snowpipe
`message_extras` | `string` | [IPI] Uma string JSON dos pares chave-valor tagueados durante a renderização Liquid
...
Este evento ocorre quando uma mensagem é despriorizada ou tem sua frequência limitada e é reenviada posteriormente dentro da janela de nova tentativa configurada.

Campo | Tipo | Descrição
------|------|------------
`id` | `string` | ID globalmente exclusivo para este evento
`user_id` | `string` | [PII] ID de usuário da Braze do usuário que realizou este evento
...
`send_id` | `null,`&nbsp;`string` | ID de envio da mensagem à qual esta mensagem pertence
`dispatch_id` | `null,`&nbsp;`string` | ID do despacho ao qual esta mensagem pertence
`campaign_id` | `null,`&nbsp;`string` | ID BSON da Campaign à qual este evento pertence
`campaign_api_id` | `null,`&nbsp;`string` | ID de API da Campaign à qual este evento pertence
`message_variation_api_id` | `null,`&nbsp;`string` | ID de API da variante de mensagem que este usuário recebeu
`canvas_id` | `null,`&nbsp;`string` | ID BSON do Canvas ao qual este evento pertence
`canvas_api_id` | `null,`&nbsp;`string` | ID de API do Canvas ao qual este evento pertence
`canvas_variation_api_id` | `null,`&nbsp;`string` | ID de API da variação do Canvas à qual este evento pertence
`canvas_step_api_id` | `null,`&nbsp;`string` | ID de API da etapa do Canvas à qual este evento pertence
`canvas_step_message_variation_api_id` | `null,`&nbsp;`string` | ID de API da variante de mensagem da etapa do Canvas que este usuário recebeu
`device_id` | `null,`&nbsp;`string` | ID do dispositivo no qual o evento ocorreu
`line_channel_id` | `null,`&nbsp;`string` | O ID do canal LINE para o qual a mensagem foi enviada ou do qual foi recebida
`line_channel_name` | `null,`&nbsp;`string` | O nome do canal LINE para o qual a mensagem foi enviada ou do qual foi recebida
`native_line_id` | `null,`&nbsp;`string` | [PII] O ID do LINE do usuário a partir do qual a mensagem foi enviada ou recebida
`subscription_group_api_id` | `null,`&nbsp;`string` | ID de API do grupo de inscrições
`timezone` | `null,`&nbsp;`string` | Fuso horário do usuário
`sf_created_at` | `timestamp`,&nbsp;`null` | Quando este evento foi captado pelo Snowpipe
`campaign_name` | `string` | Nome da Campaign
`canvas_step_name` | `string` | Nome da etapa do Canvas
...
`push_to_start_token` | `null,`&nbsp;`string` | Token push to start da Live Activity
`update_token` | `null,`&nbsp;`string` | Token de atualização da Live Activity
`live_activity_event_type` | `null,`&nbsp;`string` | Tipo de evento da Live Activity. Um de ['start', 'update', 'end']
...
`browser` | `null,`&nbsp;`string` | Navegador do dispositivo — extraído do user_agent — no qual a abertura ocorreu
`abort_type` | `null,`&nbsp;`string` | Tipo de interrupção. Para uma lista de valores, consulte [Tipos de interrupção](#abort-types).
`abort_log` | `null,`&nbsp;`string` | [PII] Mensagem de log descrevendo os detalhes da interrupção (até 128 caracteres)
`sf_created_at` | `timestamp`,&nbsp;`null` | Quando este evento foi captado pelo Snowpipe
...
`device_id` | `null,`&nbsp;`string` | `device_id` para o qual tentamos realizar a entrega
`app_group_api_id` | `null,`&nbsp;`string` | ID de API do espaço de trabalho ao qual este usuário pertence
`time` | `int` | Timestamp Unix em que o evento ocorreu
`app_api_id` | `null,`&nbsp;`string` | ID de API do app no qual este evento ocorreu
`dispatch_id` | `null,`&nbsp;`string` | ID do despacho ao qual esta mensagem pertence
`send_id` | `null,`&nbsp;`string` | ID de envio da mensagem à qual esta mensagem pertence
`campaign_id` | `null,`&nbsp;`string` | ID interno da Braze da Campaign à qual este evento pertence
`campaign_api_id` | `null,`&nbsp;`string` | ID de API da Campaign à qual este evento pertence
`message_variation_api_id` | `null,`&nbsp;`string` | ID de API da variante de mensagem que este usuário recebeu
...
`abort_type` | `null,`&nbsp;`string` | Tipo de interrupção. Para uma lista de valores, consulte [Tipos de interrupção](#abort-types).
`abort_log` | `null,`&nbsp;`string` | [PII] Mensagem de log descrevendo os detalhes da interrupção (máximo de 2.000 caracteres)
`app_group_id` | `null,`&nbsp;`string` | ID BSON do grupo de apps ao qual este usuário pertence
`sf_created_at` | `timestamp`,&nbsp;`null` | Quando este evento foi captado pelo Snowpipe
`campaign_name` | `string` | Nome da Campaign
`message_variation_name` | `string` | Nome da variante de mensagem
`canvas_name` | `string` | Nome do Canvas
`canvas_variation_name` | `string` | Nome da variação do Canvas que este usuário recebeu
`canvas_step_name` | `string` | Nome da etapa do Canvas
`message_extras` | `string` | [PII] Uma string JSON dos pares chave-valor marcados durante a renderização Liquid
...
`device_id` | `null,`&nbsp;`string` | `device_id` para o qual tentamos realizar a entrega e que sofreu bounce
`app_group_api_id` | `null,`&nbsp;`string` | ID de API do espaço de trabalho ao qual este usuário pertence
`time` | `int` | Timestamp Unix em que o evento ocorreu
`app_api_id` | `null,`&nbsp;`string` | ID de API do app no qual este evento ocorreu
`dispatch_id` | `null,`&nbsp;`string` | ID do despacho ao qual esta mensagem pertence
...
`language` | `null,`&nbsp;`string` | [PII] Idioma do usuário
`platform` | `null,`&nbsp;`string` | Plataforma do dispositivo
`ad_id` | `null,`&nbsp;`string` | [PII] ID de publicidade do dispositivo para o qual tentamos realizar a entrega
`ad_id_type` | `null,`&nbsp;`string` | Tipo do ID de publicidade
`ad_tracking_enabled` | `null, boolean` | Se o rastreamento está ativado ou não para publicidade
`app_group_id` | `null,`&nbsp;`string` | ID BSON do grupo de apps ao qual este usuário pertence
`sf_created_at` | `timestamp`,&nbsp;`null` | Quando este evento foi captado pelo Snowpipe
`campaign_name` | `string` | Nome da Campaign
`message_variation_name` | `string` | Nome da variante de mensagem
...
`canvas_id` | `null,`&nbsp;`string` | ID Braze de uso interno do Canvas ao qual este evento pertence
`canvas_api_id` | `null,`&nbsp;`string` | ID de API do Canvas ao qual este evento pertence
`canvas_variation_api_id` | `null,`&nbsp;`string` | ID de API da variante do Canvas à qual este evento pertence
`canvas_step_api_id` | `null,`&nbsp;`string` | ID de API da etapa do Canvas à qual este evento pertence
`canvas_step_message_variation_api_id` | `null,`&nbsp;`string` | ID de API da variante de mensagem da etapa do Canvas que este usuário recebeu
`gender` | `null,`&nbsp;`string` | [PII] Gênero do usuário
`country` | `null,`&nbsp;`string` | [PII] País do usuário
...
`message_variation_name` | `string` | Nome da variante de mensagem
`canvas_name` | `string` | Nome do Canvas
`canvas_variation_name` | `string` | Nome da variante do Canvas que este usuário recebeu
`canvas_step_name` | `string` | Nome da etapa do Canvas
...
`button_action_type` | `null,`&nbsp;`string` | Tipo de ação do botão de notificação por push. Um dos seguintes: [URI, DEEP_LINK, NONE, CLOSE]. null se não for de um clique em botão
`slide_id` | `null,`&nbsp;`string` | Identificador do slide do carrossel de push no qual o usuário clicou
`slide_action_type` | `null,`&nbsp;`string` | Tipo de ação do slide do carrossel de push
`ad_id` | `null,`&nbsp;`string` | [PII] ID de publicidade do dispositivo para o qual tentamos fazer a entrega
`ad_id_type` | `null,`&nbsp;`string` | Tipo do ID de publicidade
`ad_tracking_enabled` | `null, boolean` | Se o rastreamento está ativado ou não para publicidade
`app_group_id` | `null,`&nbsp;`string` | ID BSON do grupo de apps ao qual este usuário pertence
...
`ad_tracking_enabled` | `null, boolean` | Se o rastreamento está ativado ou não para publicidade
`app_group_id` | `null,`&nbsp;`string` | ID BSON do grupo de apps ao qual este usuário pertence
`message_extras` | `null,`&nbsp;`string` | [PII] Uma string JSON dos pares de chave-valor taggeados durante a renderização Liquid
`is_sampled` | `null,`&nbsp;`string` | Indica se o envio de push foi amostrado e se era esperado um evento de entrega
`locale_key` | `null,`&nbsp;`string` | [PII] A chave correspondente às traduções (por exemplo, 'en-us') usada para compor esta mensagem (null para o padrão).
`sf_created_at` | `timestamp`,&nbsp;`null` | quando este evento foi capturado pelo Snowpipe
`campaign_name` | `string` | Nome da campanha
`message_variation_name` | `string` | Nome da variante de mensagem
`canvas_name` | `string` | Nome do Canvas
...
`user_id` | `string` | ID Braze do usuário que realizou este evento
`abort_log` | `null,`&nbsp;`string` | [PII] Mensagem de log descrevendo os detalhes da interrupção (até 128 caracteres)
`abort_type` | `null,`&nbsp;`string` | Tipo de interrupção. Para ver a lista de valores, consulte [Tipos de interrupção](#abort-types).
`campaign_name` | `null,`&nbsp;`string` | Nome da campanha
`canvas_id` | `null,`&nbsp;`string` | ID BSON do Canvas ao qual este evento pertence
`canvas_name` | `null,`&nbsp;`string` | Nome do Canvas
`canvas_step_name` | `null,`&nbsp;`string` | Nome da etapa do Canvas
`canvas_variation_name` | `null,`&nbsp;`string` | Nome da variante do Canvas que este usuário recebeu
`message_variation_name` | `null,`&nbsp;`string` | Nome da variante de mensagem
`subscription_group_api_id` | `string` | ID de API do grupo de inscrições
`message_variation_api_id` | `null,`&nbsp;`string` | ID de API da variante de mensagem que este usuário recebeu
`canvas_variation_api_id` | `null,`&nbsp;`string` | ID de API da variante do Canvas à qual este evento pertence
`canvas_step_api_id` | `null,`&nbsp;`string` | ID de API da etapa do Canvas à qual este evento pertence
...
`sf_created_at` | `timestamp`,&nbsp;`null` | quando este evento foi capturado pelo Snowpipe
`canvas_api_id` | `string` | ID de API do Canvas ao qual este evento pertence
`message_extras` | `string` | [PII] Uma string JSON dos pares de chave-valor taggeados durante a renderização Liquid
...
`user_phone_number` | `null,`&nbsp;`string` | [PII] Número de telefone do usuário a partir do qual a mensagem foi recebida
`message_variation_api_id` | `null,`&nbsp;`string` | ID de API da variante de mensagem que este usuário recebeu
`canvas_variation_api_id` | `null,`&nbsp;`string` | ID de API da variante do Canvas à qual este evento pertence
`canvas_step_api_id` | `null,`&nbsp;`string` | ID de API da etapa do Canvas à qual este evento pertence
`canvas_step_message_variation_api_id` | `null,`&nbsp;`string` | ID de API da variante de mensagem da etapa do Canvas que este usuário recebeu
`interaction_type` | `null,`&nbsp;`string` | O tipo de interação que gerou o clique. Exemplos de valores em string: Text URL, Reply, OpenURL
`element_label` | `null,`&nbsp;`string` | Detalhes opcionais sobre o elemento clicado, como o texto de uma resposta sugerida ou botão
`element_type` | `null,`&nbsp;`string` | Especifica se um interaction_type que é comum entre sugestões e botões veio de uma sugestão ou botão. Exemplos: Suggestion, Button
`campaign_api_id` | `null,`&nbsp;`string` | ID de API da campanha à qual este evento pertence
`url` | `null,`&nbsp;`string` | URL na qual o usuário clicou
`subscription_group_api_id` | `string` | ID de API do grupo de inscrições
`canvas_variation_name` | `null,`&nbsp;`string` | Nome da variante do Canvas que este usuário recebeu
`sf_created_at` | `timestamp`,&nbsp;`null` | quando este evento foi capturado pelo Snowpipe
`canvas_api_id` | `string` | ID de API do Canvas ao qual este evento pertence
...
`time` | `int` | Timestamp UNIX em que o evento ocorreu
`user_id` | `string` | ID Braze do usuário que realizou este evento
`action` | `null,`&nbsp;`string` | Ação realizada em resposta a esta mensagem. (por exemplo, Subscribed, Unsubscribed ou None).
`campaign_name` | `null,`&nbsp;`string` | Nome da campanha
`canvas_id` | `null,`&nbsp;`string` | ID BSON do Canvas ao qual este evento pertence
`canvas_name` | `null,`&nbsp;`string` | Nome do Canvas
`canvas_step_name` | `null,`&nbsp;`string` | Nome da etapa do Canvas
...
`canvas_step_name` | `null,`&nbsp;`string` | Nome da etapa do Canvas
`canvas_variation_name` | `null,`&nbsp;`string` | Nome da variante do Canvas que este usuário recebeu
`category` | `null,`&nbsp;`string` | Nome da categoria de palavras-chave, preenchido apenas para mensagens de resposta automática: 'opt-in', 'opt-out', 'help' ou valor personalizado
`device_id` | `null,`&nbsp;`string` | ID do dispositivo no qual o evento ocorreu
`dispatch_id` | `null,`&nbsp;`string` | ID do despacho ao qual esta mensagem pertence
`from_rcs_sender` | `null,`&nbsp;`string` | O ID do remetente RCS ou nome do agente usado para enviar a mensagem
`message_extras` | `null,`&nbsp;`string` | Uma string JSON dos pares de chave-valor taggeados durante a renderização Liquid
`message_variation_name` | `null,`&nbsp;`string` | Nome da variante de mensagem
`send_id` | `null,`&nbsp;`string` | ID de envio da mensagem à qual esta mensagem pertence
`subscription_group_api_id` | `string` | ID de API do grupo de inscrições
`to_phone_number` | `null,`&nbsp;`string` | [PII] Número de telefone do usuário que recebe a mensagem no formato e.164 (por exemplo, +14155552671)
...
`app_group_api_id` | `string` | ID de API do grupo de apps ao qual este usuário pertence
`app_group_id` | `string` | ID BSON do grupo de apps ao qual este usuário pertence
`external_user_id` | `string` | [PII] ID externo do usuário
`id` | `string` | ID globalmente exclusivo para este evento
`time` | `int` | Timestamp UNIX em que o evento ocorreu
`user_id` | `string` | [PII] ID Braze do usuário que realizou este evento
`app_api_id` | `string` | ID de API do app no qual este evento ocorreu
`campaign_id` | `string` | ID BSON da campanha à qual este evento pertence
`campaign_api_id` | `string` | ID de API da campanha à qual este evento pertence
`message_variation_api_id` | `string` | ID de API da variante de mensagem que este usuário recebeu
`gender` | `string` | [PII] Gênero do usuário
`country` | `string` | [PII] País do usuário
`TIME_ZONE` | `string` | Fuso horário do usuário
`language` | `string` | [PII] Idioma do usuário
`device_id` | `string` | ID do dispositivo no qual o evento ocorreu
`sdk_version` | `string` | Versão do SDK da Braze em uso durante o evento
`platform` | `string` | Plataforma do dispositivo
`os_version` | `string` | Versão do sistema operacional do dispositivo
`device_model` | `string` | Modelo do dispositivo
`resolution` | `string` | Resolução do dispositivo
`carrier` | `string` | Operadora do dispositivo
`browser` | `string` | Navegador do dispositivo — extraído do user_agent — no qual a abertura ocorreu
`button_id` | `string` | ID do botão clicado, se este clique representa um clique em um botão
`ad_id_type` | `string` | Um dos seguintes: ['ios_idfa', 'google_ad_id', 'windows_ad_id', 'roku_ad_id']
`ad_tracking_enabled` | `boolean` | Se o rastreamento de publicidade está ativado para o dispositivo
`banner_placement_id` | `string` | ID do posicionamento do banner especificado pelo cliente
`campaign_name` | `string` | Nome da campanha
`message_variation_name` | `string` | Nome da variante de mensagem
`canvas_api_id` | `string` | ID de API do Canvas ao qual este evento pertence
`canvas_step_api_id` | `string` | ID de API da etapa do Canvas à qual este evento pertence
...
`canvas_name` | `string` | Nome do Canvas
`canvas_step_name` | `string` | Nome da etapa do Canvas
`canvas_step_message_variation_api_id` | `string` | ID de API da variante de mensagem da etapa do Canvas que este usuário recebeu
`canvas_variation_api_id` | `string` | ID de API da variante do Canvas à qual este evento pertence
`canvas_variation_name` | `string` | Nome da variante do Canvas que este usuário recebeu
`ad_id` | `string` | [PII] Identificador de publicidade
`is_unique` | `boolean` | Se este evento foi considerado exclusivo em 7 dias quando processado
`sf_created_at` | `timestamp` | quando este evento foi capturado pelo Snowpipe
...
`time` | `int` | Timestamp UNIX em que o evento ocorreu
`user_id` | `string` | [PII] ID Braze do usuário que realizou este evento
`target` | `string` | ID de rastreamento configurado para o elemento que foi clicado
`landing_page_api_id` | `string` | ID de API da landing page à qual este evento pertence
`landing_page_name` | `string` | Nome da landing page
`sf_created_at` | `timestamp` | quando este evento foi capturado pelo Snowpipe
...
`user_id` | `string` | [PII] ID Braze do usuário que realizou este evento
`landing_page_api_id` | `string` | ID de API da landing page à qual este evento pertence
`liquid_enabled` | `boolean` | Um booleano que indica se a landing page contém Liquid e foi processada pelo pipeline de renderização Liquid.
`landing_page_name` | `string` | Nome da landing page
`sf_created_at` | `timestamp` | quando este evento foi capturado pelo Snowpipe
...
`app_api_id` | `string` | ID de API do app no qual este evento ocorreu
`dispatch_id` | `string` | ID do despacho ao qual esta mensagem pertence
`send_id` | `string` | ID de envio da mensagem à qual esta mensagem pertence
`campaign_id` | `string` | ID BSON da campanha à qual este evento pertence
`campaign_api_id` | `string` | ID de API da campanha à qual este evento pertence
`message_variation_api_id` | `string` | ID de API da variante de mensagem que este usuário recebeu
`canvas_id` | `string` | ID BSON do Canvas ao qual este evento pertence
...
`language` | `string` | [PII] Idioma do usuário
`platform` | `string` | Plataforma do dispositivo
`retry_type` | `string` | Tipo de nova tentativa
`retry_log` | `string` | Mensagem de log descrevendo os detalhes da nova tentativa
`campaign_name` | `string` | Nome da campanha
`message_variation_name` | `string` | Nome da variante de mensagem
`canvas_name` | `string` | Nome do Canvas
`canvas_variation_name` | `string` | Nome da variante do Canvas que este usuário recebeu
...
`time` | `int` | Timestamp UNIX em que o evento ocorreu
`user_id` | `string` | [PII] ID Braze do usuário que realizou este evento
`survey_id` | `string` | UUID da pesquisa à qual esta resposta pertence.
`question_id` | `string` | UUID da pergunta à qual esta resposta pertence.
`message_extras` | `string` | [PII] Uma string JSON dos pares de chave-valor taggeados durante a renderização Liquid
`gender` | `string` | [PII] Gênero do usuário
`country` | `string` | [PII] País do usuário
`TIME_ZONE` | `string` | Fuso horário do usuário
`device_id` | `string` | [PII] ID do dispositivo no qual o evento ocorreu
`sdk_version` | `string` | Versão do SDK da Braze em uso durante o evento
`platform` | `string` | Plataforma do dispositivo
`os_version` | `string` | Versão do sistema operacional do dispositivo
`device_model` | `string` | Modelo do dispositivo
...
`ad_id_type` | `string` | Um dos seguintes: ['ios_idfa', 'google_ad_id', 'windows_ad_id', 'roku_ad_id']
`ad_tracking_enabled` | `boolean` | Se o rastreamento de publicidade está ativado para o dispositivo
`answer_single_string` | `string` | [PII] A resposta bruta quando response_type é single_string
`answer_single_boolean` | `boolean` | [PII] A resposta bruta quando response_type é single_boolean
`answer_type` | `string` | Tipo de resposta do evento, um dos seguintes: ['single_int', 'single_string', 'single_boolean']
`answer_long_string` | `string` | [PII] A resposta bruta quando response_type é free_form_text
`survey_session_id` | `string` | Identificador exclusivo para agrupar todas as respostas de uma única sessão de pesquisa
`landing_page_api_id` | `string` | ID de API da landing page à qual este evento pertence
`campaign_api_id` | `string` | ID de API da campanha à qual este evento pertence
`message_variation_api_id` | `string` | ID de API da variante de mensagem que este usuário recebeu
`canvas_api_id` | `string` | ID de API do Canvas ao qual este evento pertence
...
`canvas_step_api_id` | `string` | ID de API da etapa do Canvas à qual este evento pertence
`canvas_step_message_variation_api_id` | `string` | ID de API da variante de mensagem da etapa do Canvas que este usuário recebeu
`answer_multiple_strings` | `string` | [PII] A resposta bruta quando answer_type é multiple_string.
`survey_completion_status` | `string` | Um dos seguintes: ['completed', 'incomplete']
`response_id` | `string` | Identificador exclusivo que representa a resposta
`campaign_name` | `string` | Nome da campanha
`canvas_name` | `string` | Nome do Canvas
`canvas_step_name` | `string` | Nome da etapa do Canvas
`canvas_variation_name` | `string` | Nome da variante do Canvas que este usuário recebeu
`message_variation_name` | `string` | Nome da variante de mensagem
`app_api_id` | `string` | ID de API do app no qual este evento ocorreu
`question_reporting_id` | `string` | O identificador de relatório para a pergunta da pesquisa
`landing_page_name` | `string` | Nome da landing page
`answer_single_number` | `float` | [PII] A resposta bruta quando answer_type é single_number
`sf_created_at` | `timestamp` | quando este evento foi capturado pelo Snowpipe
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="USERSMESSAGESSURVEYRESPONSESHARED #USERSMESSAGESSURVEYRESPONSESHARED" }

## Eventos de mensagem SMS e perfis de usuário excluídos {#sms-message-events-and-deleted-user-profiles}

{% alert note %}
Para as tabelas compartilhadas `USERS_MESSAGES_SMS_*` (incluindo [`USERS_MESSAGES_SMS_REJECTION_SHARED`](#USERS_MESSAGES_SMS_REJECTION_SHARED), [`USERS_MESSAGES_SMS_DELIVERY_SHARED`](#USERS_MESSAGES_SMS_DELIVERY_SHARED) e [`USERS_MESSAGES_SMS_DELIVERYFAILURE_SHARED`](#USERS_MESSAGES_SMS_DELIVERYFAILURE_SHARED)), a Braze grava uma linha somente quando o perfil de usuário da Braze ainda existe no espaço de trabalho no momento em que o evento é processado para o Snowflake Data Sharing e o Currents. Se o usuário foi excluído antes da conclusão do processamento, o evento não aparece no Snowflake ou na sua exportação do Currents, mesmo quando as métricas de espaço de trabalho de SMS no dashboard ainda refletem contagens agregadas do caminho de relatório da Braze. Para o comportamento correspondente no Currents, consulte [Eventos de rejeição de SMS]({{site.baseurl}}/user_guide/data/distribution/braze_currents/event_glossary/message_engagement_events#sms-rejection-events) e os tipos de evento de SMS relacionados no mesmo glossário.
{% endalert %}
...
`canvas_variation_name` | `string` | Nome da variação do Canvas que este usuário recebeu
`canvas_step_name` | `string` | Nome da etapa do Canvas
`message_extras` | `string` | [IPI] Uma string JSON dos pares chave-valor marcados durante a renderização do Liquid
...
`subscription_group_api_id` | `null,`&nbsp;`string` | ID externo do grupo de inscrições
`app_group_id` | `null,`&nbsp;`string` | ID BSON do grupo de apps ao qual este usuário pertence
`is_sms_fallback` | `null, boolean` | Indica se o fallback de SMS foi tentado para esta mensagem RCS rejeitada. Está vinculado/associado ao evento de entrega de SMS
`sf_created_at` | `timestamp`,&nbsp;`null` | Quando este evento foi capturado pelo Snowpipe
`campaign_name` | `string` | Nome da campanha
`message_variation_name` | `string` | Nome da variação de mensagem
`canvas_name` | `string` | Nome do Canvas
...
`external_user_id` | `null,`&nbsp;`string` | [IPI] ID externo do usuário
`app_group_api_id` | `null,`&nbsp;`string` | ID de API do espaço de trabalho associado ao número de telefone de entrada
`time` | `int` | Carimbo de data/hora Unix em que o evento ocorreu
`user_phone_number` | `string` | [IPI] Número de telefone do usuário a partir do qual a mensagem foi recebida
`subscription_group_id` | `null,`&nbsp;`string` | ID do grupo de inscrições direcionado para esta mensagem SMS
`subscription_group_api_id` | `null,`&nbsp;`string` | ID de API do grupo de inscrições direcionado para esta mensagem SMS
`inbound_phone_number` | `string` | Número de entrada para o qual a mensagem foi enviada
`action` | `string` | Ação realizada em resposta a esta mensagem. Por exemplo, `Subscribed`, `Unsubscribed` ou `None`.
...
`canvas_variation_api_id` | `null,`&nbsp;`string` | ID de API da variação do Canvas à qual este evento pertence
`canvas_step_api_id` | `null,`&nbsp;`string` | ID de API da etapa do Canvas à qual este evento pertence
`canvas_step_message_variation_api_id` | `null,`&nbsp;`string` | ID de API da variação de mensagem da etapa do Canvas à qual este evento pertence
`app_group_id` | `null,`&nbsp;`string` | ID BSON do grupo de apps ao qual este usuário pertence
`sf_created_at` | `timestamp`,&nbsp;`null` | Quando este evento foi capturado pelo Snowpipe
`campaign_name` | `string` | Nome da campanha
`message_variation_name` | `string` | Nome da variação de mensagem
...
------|------|----------
`id` | `string` | ID globalmente exclusivo para este evento
`user_id` | `null,`&nbsp;`string` | ID Braze do usuário direcionado pela short_url, nulo se a short_url não utilizou rastreamento de cliques de usuário
`external_user_id` | `null,`&nbsp;`string` | [IPI] ID externo do usuário direcionado pela short_url, se existir; nulo se a short_url não utilizou rastreamento de cliques de usuário
`app_group_api_id` | `null,`&nbsp;`string` | ID de API do espaço de trabalho usado para gerar a short_url
`time` | `int` | Carimbo de data/hora Unix em que a short_url foi clicada
`timezone` | `null,`&nbsp;`string` | Fuso horário do usuário
`campaign_id` | `null,`&nbsp;`string` | ID Braze da campanha para a qual a short_url foi gerada, nulo se não for de uma campanha
`campaign_api_id` | `null,`&nbsp;`string` | ID de API da campanha para a qual a short_url foi gerada, nulo se não for de uma campanha
`message_variation_api_id` | `null,`&nbsp;`string` | ID de API da variação de mensagem para a qual a short_url foi gerada, nulo se não for de uma campanha
`canvas_id` | `null,`&nbsp;`string` | ID Braze do Canvas para o qual a short_url foi gerada, nulo se não for de um Canvas
`canvas_api_id` | `null,`&nbsp;`string` | ID de API do Canvas para o qual a short_url foi gerada, nulo se não for de um Canvas
`canvas_variation_api_id` | `null,`&nbsp;`string` | ID de API da variação do Canvas para a qual a short_url foi gerada, nulo se não for de um Canvas
`canvas_step_api_id` | `null,`&nbsp;`string` | ID de API da etapa do Canvas para a qual a short_url foi gerada, nulo se não for de um Canvas
`canvas_step_message_variation_api_id` | `null,`&nbsp;`string` | ID de API da variação de mensagem da etapa do Canvas para a qual a short_url foi gerada, nulo se não for de um Canvas
...
`user_phone_number` | `string` | [IPI] Número de telefone do usuário
`device_id` | `null,`&nbsp;`string` | ID do dispositivo no qual o evento ocorreu
`app_group_id` | `null,`&nbsp;`string` | ID BSON do grupo de apps ao qual este usuário pertence
`is_suspected_bot_click` | `null, boolean` | Se este evento foi processado como um evento de bot
...
Este evento ocorre quando uma mensagem é despriorizada ou sofre limitação de frequência e é reenviada posteriormente dentro da janela de reenvio configurada.
...
`retry_log` | `null,`&nbsp;`string` | Mensagem de log descrevendo os detalhes do reenvio
`sf_created_at` | `timestamp`,&nbsp;`null` | Quando este evento foi capturado pelo Snowpipe
`campaign_name` | `string` | Nome da campanha
`message_variation_name` | `string` | Nome da variação de mensagem
`canvas_name` | `string` | Nome do Canvas
...
`endpoint_url` | `null,`&nbsp;`string` | URL do endpoint sendo solicitado
`app_group_api_id` | `null,`&nbsp;`string` | ID de API do grupo de apps ao qual este usuário pertence
`app_group_id` | `null,`&nbsp;`string` | ID BSON do grupo de apps ao qual este usuário pertence
`campaign_api_id` | `null,`&nbsp;`string` | ID de API da campanha à qual este evento pertence
`campaign_id` | `null,`&nbsp;`string` | ID BSON da campanha à qual este evento pertence
...
`dispatch_id` | `null,`&nbsp;`string` | ID do despacho ao qual esta mensagem pertence
`external_user_id` | `null,`&nbsp;`string` | [IPI] ID externo do usuário
`host` | `null,`&nbsp;`string` | Host da solicitação
`id` | `string` | ID globalmente exclusivo para este evento
`message_variation_api_id` | `null,`&nbsp;`string` | ID de API da variação de mensagem que este usuário recebeu
`raw_response` | `null,`&nbsp;`string` | Resposta bruta truncada do endpoint
`retry_count` | `null, int` | Número de tentativas de reenvio realizadas
`send_id` | `null,`&nbsp;`string` | ID de envio da mensagem à qual esta mensagem pertence
`time` | `int` | Carimbo de data/hora UNIX em que o evento ocorreu
`url_path` | `null,`&nbsp;`string` | Caminho da URL sendo solicitada
`user_id` | `string` | ID Braze do usuário que realizou este evento
`webhook_duration` | `null, int` | Duração total desta solicitação em milissegundos
`webhook_failure_source` | `null,`&nbsp;`string` | Indica se o erro foi criado pela Braze ou pelo próprio endpoint. O campo de origem pode ser External Endpoint, Treat no status code to host unreachable
`is_terminal` | `null, boolean` | Se este evento foi a tentativa final de um envio
`sf_created_at` | `timestamp`,&nbsp;`null` | Quando este evento foi capturado pelo Snowpipe
`campaign_name` | `string` | Nome da campanha
`canvas_name` | `string` | Nome do Canvas
`canvas_step_name` | `string` | Nome da etapa do Canvas
...
`app_group_api_id` | `null,`&nbsp;`string` | ID de API do espaço de trabalho ao qual este usuário pertence
`subscription_group_api_id` | `string` | ID de API do grupo de inscrições
`campaign_id` | `null,`&nbsp;`string` | ID da Braze de uso interno da campanha à qual este evento pertence
`campaign_api_id` | `null,`&nbsp;`string` | ID de API da campanha à qual este evento pertence
`message_variation_api_id` | `null,`&nbsp;`string` | ID de API da variante de mensagem que este usuário recebeu
`canvas_id` | `null,`&nbsp;`string` | ID da Braze de uso interno do Canvas ao qual este evento pertence
`canvas_api_id` | `null,`&nbsp;`string` | ID de API do Canvas ao qual este evento pertence
`canvas_variation_api_id` | `null,`&nbsp;`string` | ID de API da variante do Canvas à qual este evento pertence
`canvas_step_api_id` | `null,`&nbsp;`string` | ID de API da etapa do Canvas à qual este evento pertence
`canvas_step_message_variation_api_id` | `null,`&nbsp;`string` | ID de API da variante de mensagem da etapa do Canvas que este usuário recebeu
...
`canvas_variation_name` | `string` | Nome da variante do Canvas que este usuário recebeu
`canvas_step_name` | `string` | Nome da etapa do Canvas
`bsuid` | `string` | O ID de usuário com escopo de negócios do WhatsApp do usuário do qual a mensagem foi recebida.
`message_extras` | `string` | [PII] Uma string JSON dos pares chave-valor marcados durante a renderização do Liquid
...
`short_url` | `null,`&nbsp;`string` | URL encurtada que foi clicada
`user_agent` | `null,`&nbsp;`string` | User agent no qual o relatório de spam ocorreu
`user_phone_number` | `null,`&nbsp;`string` | [PII] O número de telefone do usuário do qual a mensagem foi recebida
`sf_created_at` | `timestamp`,&nbsp;`null` | Quando este evento foi capturado pelo Snowpipe
`campaign_name` | `string` | Nome da campanha
`message_variation_name` | `string` | Nome da variante de mensagem
`canvas_name` | `string` | Nome do Canvas
...
`template_name` | `null,`&nbsp;`string` | [PII] Nome do modelo no WhatsApp Manager. Presente se estiver enviando uma mensagem de modelo
...
`flow_id` | `null,`&nbsp;`string` | O ID exclusivo do Flow no WhatsApp Manager. Presente se o usuário estiver respondendo a um WhatsApp Flow.
`flow_response_json` | `null,`&nbsp;`string` | [PII] Os valores de formulário com os quais o usuário respondeu. Presente se o usuário estiver respondendo a um WhatsApp Flow.
`message_id` | `null,`&nbsp;`string` | O ID exclusivo gerado pela Meta para esta mensagem
...
`canvas_step_message_variation_api_id` | `null,`&nbsp;`string` | ID de API da variante de mensagem da etapa do Canvas que este usuário recebeu
`dispatch_id` | `null,`&nbsp;`string` | ID do despacho ao qual esta mensagem pertence
`message_extras` | `null,`&nbsp;`string` | [PII] Uma string JSON dos pares chave-valor marcados durante a renderização do Liquid
`sf_created_at` | `timestamp`,&nbsp;`null` | Quando este evento foi capturado pelo Snowpipe
`send_id` | `null,`&nbsp;`string` | ID de envio da mensagem à qual esta mensagem pertence
`flow_id` | `null,`&nbsp;`string` | O ID exclusivo do Flow no WhatsApp Manager. Presente se o usuário estiver respondendo a um WhatsApp Flow.
`template_name` | `null,`&nbsp;`string` | [PII] Nome do modelo no WhatsApp Manager. Presente se estiver enviando uma mensagem de modelo
...
`app_group_api_id` | `null,`&nbsp;`string` | ID de API do grupo de apps ao qual este usuário pertence
`time` | `int` | Timestamp UNIX em que o evento ocorreu
`to_phone_number` | `null,`&nbsp;`string` | [PII] Número de telefone do usuário que está recebendo a mensagem no formato e.164
`device_id` | `null,`&nbsp;`string` | ID do dispositivo no qual o evento ocorreu
`timezone` | `null,`&nbsp;`string` | Fuso horário do usuário
`subscription_group_api_id` | `null,`&nbsp;`string` | ID de API do grupo de inscrições
`campaign_id` | `null,`&nbsp;`string` | ID BSON da campanha à qual este evento pertence
...
| `app_group_id`     | `string`,&nbsp;`null`    | ID da Braze do espaço de trabalho ao qual este usuário pertence              |
| `app_group_api_id` | `string`,&nbsp;`null`    | ID de API do espaço de trabalho ao qual este usuário pertence                |
| `time`             | `int`,&nbsp;`null`       | Registro de data e hora Unix em que a solicitação de exclusão foi processada |
| `sf_created_at`    | `timestamp`,&nbsp;`null` | Quando este evento foi captado pelo Snowpipe                                 |
...
| `app_group_id`     | `string`,&nbsp;`null`    | ID da Braze do espaço de trabalho ao qual este usuário pertence                              |
| `app_group_api_id` | `string`,&nbsp;`null`    | ID de API do espaço de trabalho ao qual este usuário pertence                                |
| `app_api_id`       | `string`,&nbsp;`null`    | ID de API do app ao qual o usuário órfão pertencia                                           |
| `time`             | `int`,&nbsp;`null`       | Registro de data e hora Unix em que o usuário se tornou órfão                                |
...
`app_api_id` | `string` | ID de API do app no qual este evento ocorreu
`update_source` | `string` | A fonte desta atualização
`archived` | `boolean` | Quando definido como True, indica que este usuário foi arquivado na Braze
`first_name` | `string` | [IPI] Nome do usuário
`last_name` | `string` | [IPI] Sobrenome do usuário
`email_address` | `string` | [IPI] Endereço de e-mail do usuário
`gender` | `string` | [IPI] Gênero do usuário
`phone_number` | `string` | [IPI] Número de telefone do usuário no formato e.164 (por exemplo, +14155552671)
`dob` | `string` | [IPI] Data de nascimento do usuário no formato "AAAA-MM-DD"
`TIME_ZONE` | `string` | Fuso horário do usuário
`home_city` | `string` | [IPI] Cidade de residência do usuário
`country` | `string` | [IPI] País do usuário
`language` | `string` | [IPI] Idioma do usuário
`custom_attributes` | `string` | String JSON válida dos atributos personalizados atualizados
`sf_created_at` | `timestamp` | Quando este evento foi captado pelo Snowpipe
...
`id` | `string` | ID globalmente exclusivo para este evento
`time` | `int` | Timestamp UNIX em que o evento ocorreu
`app_group_id` | `string` | BSON ID do grupo de apps ao qual este usuário pertence
...
`time` | `int` | Timestamp UNIX em que o evento ocorreu
`app_group_id` | `string` | BSON ID do grupo de apps ao qual este usuário pertence
`api_id` | `string` | ID da API da variação de mensagem da Campaign
`name` | `null,`&nbsp;`string` | Nome da variação de mensagem da Campaign
...
`experiment_splits` | `null,`&nbsp;`string` | Divisões de experimento para a etapa
...
Campo | Tipo | Descrição
------|------|------------
`app_group_api_id` | `string` | ID da API do grupo de apps ao qual este usuário pertence
`app_group_id` | `string` | BSON ID do grupo de apps ao qual este usuário pertence
`external_user_id` | `string` | [IPI] ID externo do usuário
`id` | `string` | ID globalmente exclusivo para este evento
`time` | `int` | Timestamp UNIX em que o evento ocorreu
`content_optimizer_step_id` | `string` | ID interno da etapa do Otimizador de Conteúdo
`combination_token` | `string` | Combinação de componentes atribuída
`content` | `string` | Carga útil do conteúdo renderizado em formato JSON
`is_active` | `boolean` | Se o token de combinação está sendo usado ativamente para entrega
`sf_created_at` | `timestamp` | Momento em que este evento foi captado pelo Snowpipe
...
## Tipos de interrupção {#abort-types}
