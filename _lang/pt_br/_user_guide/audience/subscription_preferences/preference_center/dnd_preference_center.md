...
Crie uma Central de Preferências acessando **Público** > **Centrais de Preferências de E-mail**. Aqui, uma lista de centrais de preferências personalizadas é exibida. Selecione **Criar novo** para criar uma nova Central de Preferências, ou selecione o nome de uma existente para fazer alterações.

## Etapa 2: Nomeie a Central de Preferências de e-mail {#step-2-name-the-email-preference-center}

Os nomes da Central de Preferências podem conter apenas caracteres alfanuméricos, hifens ou underscores. O nome que você fornecer determina a sintaxe da Liquid tag gerada.

Essa Liquid tag pode ser incluída em qualquer Campaign de e-mail de saída ou etapa do Canvas e direciona os usuários para a Central de Preferências.
...
### Definir grupos de inscrições disponíveis {#define-available-subscription-groups}

Para determinar quais grupos de inscrições devem ser exibidos na Central de Preferências, selecione o botão **+ Add subscription groups** para abrir um modal onde os grupos de inscrições desejados podem ser selecionados. Após a seleção, clique no botão **Add Subscription Groups** para adicioná-los à Central de Preferências.
...
- Incluir descrições
- Adicionar ou remover uma caixa de seleção **Subscribe to all** que inscreve o usuário em todos os grupos de inscrições exibidos neste bloco
- Adicionar ou remover uma caixa de seleção **Unsubscribe from all** que cancela a inscrição do usuário de todos os grupos de inscrições exibidos neste bloco

O botão **Unsubscribe from all** na parte inferior do modelo não pode ser removido e [cancela globalmente a inscrição]({{site.baseurl}}/user_guide/channels/email/subscriptions#subscription-states) do usuário para o recebimento de qualquer mensagem de e-mail.

## Etapa 4: Personalize a Central de Preferências usando o editor de arrastar e soltar {#step-4-customize-the-preference-center-using-the-drag-and-drop-editor}

### Definir estilos comuns {#set-common-styles}

Você pode definir certos estilos para serem aplicados em todos os blocos relevantes da sua Central de Preferências a partir da guia **Common Styles**. Os estilos definidos nesta seção são usados em toda a sua mensagem, exceto quando você os substitui para um bloco específico. Para uma experiência de design mais fácil, recomendamos configurar os estilos no nível da página antes de personalizar os estilos no nível do bloco.
...
Como prática recomendada, formate as propriedades de linha e coluna antes de formatar qualquer bloco dentro das linhas. Você pode ajustar o espaçamento e o alinhamento em vários lugares, então começar pela base torna mais fácil editar conforme avança.

{% endtab %}
{% tab Blocos %}

Blocos representam diferentes tipos de conteúdo que você pode usar na sua mensagem. Arraste um para dentro de um Segment de linha existente, que se ajusta automaticamente à largura da célula.
...
Se você estiver usando o bloco de Código Personalizado na sua central de preferências, frames inline podem não ser gerados no código personalizado quando entregues aos seus usuários.
...
## Etapa 5: Personalize sua página de confirmação {#step-5-customize-your-confirmation-page}

Em seguida, personalize a página de confirmação selecionando **Confirmation Page**. Essa página é exibida aos usuários após atualizarem suas preferências usando a Central de Preferências. Os mesmos recursos de estilização de [Definir estilos comuns](#set-common-styles) e [Componentes de arrastar e soltar da Central de Preferências](#drag-and-drop-preference-center-components) se aplicam a essa página.
...
## Etapa 6: Visualize e lance sua Central de Preferências {#step-6-preview-and-launch-your-preference-center}

Você pode visualizar sua Central de Preferências selecionando a guia **Prévia** dentro do editor. A prévia mostra tanto a Central de Preferências quanto a página de confirmação.
...
## Usar a Central de Preferências {#use-the-preference-center}
...
Se ocorrer um erro quando um usuário selecionar **Salvar** em uma Central de Preferências, a seguinte mensagem de erro padrão será exibida, que não pode ser personalizada ou estilizada no editor. No entanto, a localização das mensagens de erro ainda é suportada nessas páginas.

![Um erro informando "There was a problem saving your preferences. Please try again."]({% image_buster /assets/img/preference_center/preference_center11.png %}){: style="max-width:55%;"}