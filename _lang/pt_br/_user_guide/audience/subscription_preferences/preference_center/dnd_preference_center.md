...
Crie uma Central de Preferências acessando **Público** > **Centrais de Preferências de e-mail**. Aqui, uma lista de centrais de preferências personalizadas é exibida. Selecione **Criar nova** para criar uma nova Central de Preferências, ou selecione o nome de uma existente para fazer alterações.
...
O botão **Unsubscribe from all** na parte inferior do modelo não pode ser removido e [cancela globalmente a inscrição]({{site.baseurl}}/user_guide/channels/email/subscriptions#subscription-states) do usuário, impedindo o recebimento de qualquer mensagem de e-mail.
...
Como prática recomendada, formate as propriedades de linha e coluna antes de formatar qualquer bloco dentro das linhas. Você pode ajustar o espaçamento e o alinhamento em vários lugares, então começar pela base facilita a edição ao longo do processo.
...
Se você estiver usando o bloco de código personalizado na sua Central de Preferências, os iframes podem não ser gerados no código personalizado quando entregue aos seus usuários.
...
Se ocorrer um erro quando um usuário selecionar **Salvar** em uma Central de Preferências, a seguinte mensagem de erro padrão será exibida, que não pode ser personalizada ou estilizada no editor. No entanto, a localização das mensagens de erro ainda é compatível nessas páginas.
