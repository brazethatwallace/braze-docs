...
3. Se as configurações globais de reposição de estoque não tiverem sido definidas, será solicitado que você configure os eventos e propriedades personalizados que serão usados para disparar notificações de reposição de estoque:
    ...
    - **Catálogo de fallback** Esse é o catálogo que será usado para a inscrição de reposição de estoque, caso não haja uma propriedade `catalog_name` presente no evento personalizado.
    - **Evento personalizado para inscrições** é o evento personalizado da Braze que será usado para inscrever um usuário nas notificações de reposição de estoque. Quando esse evento ocorrer, o usuário que o realizou será inscrito.
...
5. Defina sua regra de notificação. Existem duas opções:
    - **Notificar todos os usuários inscritos** notifica todos os clientes que estão aguardando quando o item estiver novamente em estoque. 
...
6. Defina o **campo de estoque no catálogo**. Esse campo do catálogo será usado para determinar se o item está fora de estoque. O campo deve ser do tipo numérico.
