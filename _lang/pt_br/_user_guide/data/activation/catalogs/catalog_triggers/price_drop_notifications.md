...
# Notificações de queda de preço {#price-drop-notifications}
...
Quando um usuário aciona um evento personalizado para um item, nós automaticamente o inscrevemos para receber notificações de queda de preço para esse item. Quando o preço do item atender à sua regra de inventário (como uma queda maior que 50%), todos os inscritos serão elegíveis para notificações por meio de uma Campaign ou Canvas. No entanto, apenas os usuários que optaram por receber notificações receberão as notificações.

## Configurando um evento personalizado para notificações de queda de preço {#setting-a-custom-event-for-price-drop-notifications}
...
## Configurando notificações de queda de preço {#setting-up-price-drop-notifications}
...
{: start="4"}
4. Selecione **Save** e continue para a próxima seção para configurar as regras de notificação.
...
## Usando notificações de queda de preço em um Canvas {#using-price-drop-notifications-in-a-canvas}
