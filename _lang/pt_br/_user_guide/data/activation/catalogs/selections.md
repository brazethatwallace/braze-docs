...
- As seleções são ótimas para refinar recomendações a partir dos dados de catálogo da Braze. Se você está buscando inspiração, confira [Sobre recomendação de itens]({{site.baseurl}}/user_guide/brazeai/recommendations) para ver exemplos de casos de uso.

## Filtros de geolocalização {#geolocation-filters}

Se o seu catálogo inclui um [tipo de campo de geolocalização]({{site.baseurl}}/user_guide/data/activation/catalogs/create#supported-data-types), você pode usar filtros baseados em geolocalização nas suas seleções para exibir itens do catálogo com base na proximidade de um ponto geográfico.
...
4. Em **Filter Field**, selecione a coluna do catálogo pela qual deseja filtrar. Campos de string com mais de 1.000 caracteres não podem ser selecionados para filtros.
...
Após criar uma seleção, você pode usar a seção **Preview for user** para visualizar o que uma seleção retornaria para um usuário aleatório ou um usuário específico. Para seleções que usam personalização, você só pode visualizar a prévia após selecionar um usuário.
...
Depois de criar sua seleção, personalize suas mensagens com Liquid para inserir os itens filtrados daquele catálogo. Você pode fazer com que a Braze gere o Liquid para você a partir da janela de personalização encontrada nos criadores de mensagem:
...
Digamos que você tem um serviço de entrega de refeições e quer enviar uma mensagem personalizada para seus usuários que têm preferências alimentares específicas com base na categoria de alimentos visualizada mais recentemente.
...
Para usar esse catálogo e essa seleção em uma campanha, use o modal **Adicionar personalização** na seção de composição de mensagem ao criar uma campanha. Neste exemplo, selecionamos o catálogo com as informações do seu serviço de entrega de refeições e a seleção para recomendações de refeições com base na categoria visualizada mais recentemente. Isso nos permite exibir o nome e o preço da refeição. Para aprimorar ainda mais sua mensagem, você pode usar a seleção para também adicionar uma imagem da primeira refeição recomendada.

![Um cartão de conteúdo com o cabeçalho "Você vai AMAR essas refeições bem avaliadas!" com a seleção "recommendations_be_recent_category" na seção de composição de mensagem.]({% image_buster /assets/img_archive/catalog_selections3.png %}){: style="max-width:90%;"}

Por exemplo, digamos que você tem um usuário cuja categoria visualizada mais recentemente é "Frango". Usando a personalização configurada e uma campanha de cartão de conteúdo, você pode enviar três recomendações de refeições que incluem frango para esse usuário.
