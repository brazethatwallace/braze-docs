...
- O [SDK or kit de desenvolvimento de software da Braze]({{site.baseurl}}/developer_guide/analytics/setting_user_attributes)
...
Os identificadores informam à Braze qual perfil de usuário deve ser atualizado ou criado. Toda solicitação de API or interface de programação do aplicativo (API) e linha de CSV deve incluir pelo menos um identificador. Para detalhes sobre como escolher o identificador correto, consulte [Resolução de identificadores]({{site.baseurl}}/api/objects_filters/user_attributes_object#identifier-resolution).
...
| `braze_id` | String | Um identificador atribuído pela Braze, criado quando o SDK or kit de desenvolvimento de software detecta um dispositivo pela primeira vez. Somente leitura. Não pode ser editado. |
...
| `country` | String | Um código de país no formato [ISO 3166-1 alpha-2](https://en.wikipedia.org/wiki/ISO_3166-1) (por exemplo, `US`, `GB`). Definir `country` por importação de CSV ou API or interface de programação do aplicativo (API) impede que o SDK or kit de desenvolvimento de software o capture automaticamente. |
| `home_city` | String | A cidade de residência do usuário (por exemplo, `London`). |
| `language` | String | Um código de idioma no formato [ISO 639-1](https://en.wikipedia.org/wiki/List_of_ISO_639-1_codes) (por exemplo, `en`). Consulte a [lista de idiomas aceitos]({{site.baseurl}}/user_guide/data/unification/user_data/language_codes). Definir `language` por importação de CSV ou API or interface de programação do aplicativo (API) impede que o SDK or kit de desenvolvimento de software o capture automaticamente. |
...
| Campo | Tipo de dados | Formato e notas |
|---|---|---|
| `email_subscribe` | String | Um dos seguintes: `opted_in` (registrado explicitamente para receber e-mail), `unsubscribed` (cancelou explicitamente a inscrição de e-mail) ou `subscribed` (nem optou por receber nem cancelou a inscrição). |
...
| `email_click_tracking_disabled` | Booleano | `true` ou `false`. Defina como `true` para desativar o rastreamento de cliques em e-mails para este usuário. |
...
Para saber mais sobre a configuração de grupos de inscrições, consulte [Grupos de inscrições]({{site.baseurl}}/user_guide/audience/subscription_preferences/subscription_groups).

## Sessões e engajamento {#sessions-and-engagement}

Esses campos capturam quando o usuário usou seu app pela primeira ou última vez. O SDK or kit de desenvolvimento de software os registra automaticamente; normalmente, você os define apenas por meio de API or interface de programação do aplicativo (API) ou CSV ao migrar de outra plataforma.
...
Use esses campos ao migrar tokens por push de outra plataforma. Após a integração do SDK or kit de desenvolvimento de software da Braze, os tokens por push são capturados automaticamente. Para orientações sobre migração, consulte [Migração de tokens por push]({{site.baseurl}}/api/objects_filters/user_attributes_object#migrate-push-tokens).
...
| Campo | Tipo de dados | Formato e notas |
|---|---|---|
| `facebook` | Objeto | Um objeto contendo qualquer um dos seguintes: `id` (string), `likes` (array de strings) ou `num_friends` (inteiro). |
| `twitter` | Objeto | Um objeto contendo qualquer um dos seguintes: `id` (inteiro), `screen_name` (string, identificador do X), `followers_count` (inteiro), `friends_count` (inteiro) ou `statuses_count` (inteiro). |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 role="presentation" }
...
Para consultar o contrato completo da API or interface de programação do aplicativo (API), acesse o [Objeto de atributos de usuário]({{site.baseurl}}/api/objects_filters/user_attributes_object).
...
Não é possível definir alguns atributos padrão por meio da importação de CSV. Você deve enviar arrays, tokens por push e objetos aninhados pela API or interface de programação do aplicativo (API) ou pela [Ingestão de Dados na Nuvem]({{site.baseurl}}/user_guide/data/unification/cloud_ingestion). Para a lista completa de campos compatíveis com CSV e as etapas de importação, consulte [Atributos padrão]({{site.baseurl}}/user_guide/audience/manage_audience/import_users/csv_import#default-attributes).

## Considerações {#considerations}

Leve em conta estes pontos ao trabalhar com atributos padrão:

- **Os nomes dos campos diferenciam maiúsculas de minúsculas.** Use sempre letras minúsculas. Um cabeçalho ou chave que não corresponda exatamente ao nome de um atributo padrão é tratado como um atributo personalizado.
- **A captura automática do SDK or kit de desenvolvimento de software é suprimida quando você define valores por API or interface de programação do aplicativo (API) ou CSV.** Quando você define `country` ou `language` por API or interface de programação do aplicativo (API) ou CSV, a Braze para de capturar automaticamente esses campos pelo SDK or kit de desenvolvimento de software para esse usuário.
- **`null` remove um valor.** Defina um atributo padrão como `null` para removê-lo do perfil. Alguns campos, incluindo `external_id` e `user_alias`, não podem ser removidos após serem definidos.
- **Valores em branco no CSV não sobrescrevem.** Uma célula em branco em uma importação de CSV mantém o valor existente no perfil. Para limpar um valor, use a API or interface de programação do aplicativo (API).
- **Os fusos horários usam UTC como padrão.** Strings de data sem deslocamento são interpretadas como meia-noite UTC e exibidas no fuso horário do seu espaço de trabalho. Para especificar um fuso horário, adicione um deslocamento UTC (por exemplo, `2024-11-10T18:00:00-05:00`).

## Páginas relacionadas {#related-pages}

- [Objeto de atributos de usuário]({{site.baseurl}}/api/objects_filters/user_attributes_object) — Contrato completo da API or interface de programação do aplicativo (API) para o objeto de atributos.
- [Endpoint `/users/track`]({{site.baseurl}}/api/endpoints/user_data/post_user_track) — Endpoint REST or transferir estado representacional para criar e atualizar perfis de usuário.
- [Definir atributos de usuário]({{site.baseurl}}/developer_guide/analytics/setting_user_attributes) — Métodos do SDK or kit de desenvolvimento de software para definir atributos padrão e personalizados.
- [Importação de CSV]({{site.baseurl}}/user_guide/audience/manage_audience/import_users/csv_import) — Fazer upload de atributos padrão por meio de um arquivo CSV.
