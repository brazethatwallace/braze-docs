...
{% alert note %}
Os eventos de eCommerce da Braze e suas propriedades de evento Segment or segmentoáveis não contam como [pontos de dados]({{site.baseurl}}/user_guide/data/infrastructure/data_points).
{% endalert %}
...
- **Filtros de eCommerce:** Use a categoria **eCommerce** no segmentador, que contém filtros alimentados por eventos recomendados de eCommerce (como **Last Order Placed**, **Total Revenue** e **Average Order Value**). Para ver a lista completa de filtros disponíveis, consulte [Filtros de Segment or segmento]({{site.baseurl}}/user_guide/audience/segments/segmentation_filters).
...
- **Extensões de Segment or segmento or segmento:** Para segmentar com base em propriedades de evento aninhadas, incluindo o array de produtos aninhados ou as propriedades de objetos de metadados, use [extensões de Segment or segmento or segmento]({{site.baseurl}}/user_guide/audience/segments/segment_extension) com filtragem de propriedades de evento aninhadas. Isso permite criar públicos como "usuários que compraram o produto SKU-123 nos últimos 90 dias" ou combinar critérios em diferentes propriedades do mesmo pedido.

{% alert important %}
As extensões de Segment or segmento or segmento para eventos recomendados de eCommerce são um recurso pago e estão em acesso antecipado. Se você tiver interesse em participar do acesso antecipado, entre em contato com seu gerente de sucesso do cliente. Confirme se seu plano inclui acesso antes de recomendar a segmentação por propriedades aninhadas à sua equipe.
{% endalert %}
...
| Insights de Segment or segmento or segmento                         | Comparações de receita entre segmentos no dashboard de insights de Segment or segmento or segmento.                                                               |
...
| [Exportar dados de Segment or segmento or segmento para CSV]({{site.baseurl}}/user_guide/data/distribution/export_braze_data/segment_data_to_csv)           | Exportação em CSV dos membros do Segment or segmento or segmento. Para incluir eventos de eCommerce, selecione-os pelo nome no dropdown de eventos personalizados.                                                                                |
| [Exportar perfil de usuário por Segment or segmento (API or interface de programação do aplicativo (API))]({{site.baseurl}}/api/endpoints/export/user_data/post_users_segment#prerequisites) | Dados de perfil de usuário para membros do Segment or segmento or segmento, retornados via API or interface de programação do aplicativo (API). Os eventos de eCommerce são incluídos como eventos personalizados.                                                                                        |
...
O segmentador permite filtrar pelo número de vezes que um usuário realizou um evento de eCommerce. Para filtrar por propriedades específicas de produto (como `product_id` ou `product_name`), use [extensões de Segment or segmento or segmento]({{site.baseurl}}/user_guide/audience/segments/segment_extension), que aceitam filtragem por propriedades de evento aninhadas. Por exemplo, você pode encontrar todos os usuários que compraram o produto "SKU-123" nos últimos 90 dias.