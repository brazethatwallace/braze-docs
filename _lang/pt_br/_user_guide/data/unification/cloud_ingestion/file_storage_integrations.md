...
description: "Esta página aborda a Ingestão de Dados na Nuvem da Braze e como sincronizar dados relevantes do Amazon S3 ou do Google Cloud Storage para a Braze."
page_order: 4
page_type: reference
...
> Esta página aborda como configurar a Ingestão de Dados na Nuvem para sincronizar dados do Amazon S3 ou do Google Cloud Storage para a Braze.
...
| Identity and Access Management (IAM) | O IAM é um serviço web que permite controlar com segurança o acesso aos recursos da AWS. Neste tutorial, crie uma política de IAM e atribua-a a uma função de IAM para integrar seu bucket S3 com a Ingestão de Dados na Nuvem da Braze. |
...
1. Primeiro, crie uma nova fonte no dashboard da Braze. Acesse **Configurações de dados** > **Ingestão de Dados na Nuvem** > **Sources**, selecione **Add data source** e, em seguida, selecione **Amazon S3**.
...
![A seção S3 Connection Details mostrando os campos de credenciais (configuração da AWS e configuração da Braze) e configuração.]({% image_buster /assets/img/cloud_ingestion/s3_ingestion_1.png %})
...
- URL do SQS (deve ser único para cada nova integração)
- Caminho da pasta (opcional, deve ser único entre sincronizações em um espaço de trabalho)
...
8. Adicione e-mail(s) de contato para notificações caso a sincronização seja interrompida por problemas de acesso ou permissões. Opcionalmente, ative notificações para erros em nível de usuário e sincronizações bem-sucedidas.
...
- Um bucket do Cloud Storage para armazenamento de dados
...
| Bucket do Cloud Storage | Um bucket é o contêiner que armazena os arquivos de dados que você deseja que a Braze ingira. |
...
### Etapa 1: Criar um bucket do Cloud Storage {#step-1-create-a-cloud-storage-bucket}
...
O conector precisa exatamente destas permissões: `storage.buckets.get`, `storage.objects.get` e `storage.objects.list` no bucket, e `pubsub.subscriptions.consume` na assinatura. Você pode concedê-las com uma função personalizada ou com funções predefinidas.
...
5. Na etapa **Data definition**, selecione sua fonte GCS e especifique o seguinte:
...
{% alert important %}
O caminho da pasta e a assinatura devem ser únicos entre sincronizações em um espaço de trabalho para múltiplas sincronizações que compartilham o mesmo bucket de origem.
{% endalert %}
...
2. Crie um tópico dedicado e uma notificação com escopo de prefixo para essa pasta e, em seguida, crie uma assinatura nesse tópico:
...
A Braze não impõe requisitos adicionais de nome de arquivo além do que é exigido pelo seu provedor de armazenamento de arquivos. Os nomes de arquivo devem ser únicos. Adicionar um timestamp ajuda a garantir a unicidade.
...
| `EMAIL` | O endereço de e-mail do usuário. Se existirem vários perfis com o mesmo endereço de e-mail, o perfil atualizado mais recentemente terá prioridade nas atualizações. Se você incluir tanto e-mail quanto telefone, a Braze usará o e-mail como identificador principal. |
...
3. Faça upload de arquivos no seu bucket de origem que contenham apenas colunas de identificadores de usuário. Não inclua uma coluna `PAYLOAD` — a sincronização falha se a carga útil estiver presente, para evitar exclusões acidentais.
...
{% alert important %}
A exclusão de usuários é permanente e não pode ser desfeita. Inclua apenas os usuários que você pretende remover. Para saber mais, consulte [Excluir usuários com ingestão de dados na nuvem]({{site.baseurl}}/user_guide/data/unification/cloud_ingestion/delete_users).
{% endalert %}
...
1. Use a mesma sincronização que você usa para [sincronizar dados de catálogo]({{site.baseurl}}/user_guide/data/unification/cloud_ingestion/sync_catalogs_data) (tipo de dados **Catalogs**).
...
Você pode usar arquivos existentes para validar se a Braze consegue acessar seu bucket e detectar arquivos para ingestão, mas eles não são sincronizados com a Braze. Para que o CDI os processe, você precisa fazer o re-upload dos arquivos existentes que deseja sincronizar para o bucket de origem.

### Tratamento de erros inesperados em arquivos (Amazon S3) {#handling-unexpected-file-errors-amazon-s3}

Se você está observando um número alto de erros ou arquivos com falha, pode ser que outro processo esteja adicionando arquivos ao bucket S3 em uma pasta diferente da pasta de destino do CDI.

Quando arquivos são enviados para o bucket de origem, mas não para a pasta de origem, o CDI processa a notificação do SQS, mas não executa nenhuma ação no arquivo. Isso pode aparecer como um erro.
...
Assim como o Amazon S3, o CDI só processa arquivos enviados após a criação da sincronização. Cada novo objeto dispara uma mensagem `OBJECT_FINALIZE` para o seu tópico Pub/Sub. Para ingerir arquivos que já existem no bucket, faça o re-upload deles.
