...
page_order: 7
page_type: reference
description: "Esta página fornece uma visão geral de como disparar Canvas da Braze usando CDI."
---

# Personalização sem cópia usando CDI {#zero-copy-personalization-using-cdi}
...
### Usando gatilhos do Canvas {#using-canvas-triggers}

#### Etapa 1: Configurar fonte de dados para gatilhos do Canvas {#step-1-set-up-data-source-for-canvas-triggers}

{% tabs %}
{% tab Snowflake %}

##### Etapa 1.1: Configure sua tabela de origem no Snowflake {#step-11-set-up-your-source-table-in-snowflake}
...
##### Etapa 1.2: Configurar credenciais {#step-12-set-up-credentials}
...
##### Etapa 1.1: Configure sua tabela de origem no Redshift {#step-11-set-up-your-source-table-in-redshift}
...
##### Etapa 1.1: Crie um novo projeto ou conjunto de dados para sua tabela de origem (opcional) {#step-11-create-a-new-project-or-dataset-for-your-source-table-optional}

```sql
CREATE SCHEMA BRAZE-CLOUD-PRODUCTION.INGESTION;
```

##### Etapa 1.2: Configure sua tabela de origem no BigQuery {#step-12-set-up-your-source-table-in-bigquery}
...
##### Etapa 1.3: Configurar credenciais {#step-13-set-up-credentials}
...
#### Etapa 1.2: Configure sua tabela de origem no Databricks {#step-12-set-up-your-source-table-in-databricks}
...
##### Etapa 1.1: Configure sua tabela de origem no Fabric {#step-11-set-up-your-source-table-in-fabric}
...
#### Etapa 3: Crie sua sincronização sem cópia {#step-3-create-your-zero-copy-sync}
...
7. Selecione **Test Connection** para confirmar se tudo funciona conforme o esperado. Se estiver conectando ao Snowflake, primeiro adicione a chave pública exibida no dashboard ao usuário criado para a Braze se conectar ao Snowflake. Para concluir esta etapa, você precisará de acesso **SECURITYADMIN** ou superior no Snowflake.
8. Salve a sincronização para começar a sincronizar os gatilhos do Canvas.

Quando a sincronização for executada, os usuários da sua tabela de origem começarão a entrar no Canvas. Use a análise de dados do Canvas e a página de registros de sincronização da Ingestão de dados na nuvem para monitorar o desempenho.
...
* Os resultados da tabela são armazenados temporariamente na Braze durante o processamento. Eles também são exportados para o Snowflake por 30 dias para que você possa visualizar exatamente o que foi sincronizado.
...
* Ao usar o CDI sem cópia com gatilhos do Canvas, a Braze não armazena um backup dos resultados da consulta do data warehouse e nenhum dado é copiado para o perfil do usuário.
