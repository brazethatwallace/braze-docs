...
Todos os dados persistentes associados a um usuário são armazenados em seu perfil de usuário. Depois que um perfil de usuário é criado, seja por meio da API or interface de programação do aplicativo (API) ou depois que um usuário é reconhecido pelo SDK or kit de desenvolvimento de software, é possível atribuir vários parâmetros a esse perfil para identificar e fazer referência a esse usuário.
...
Inicialmente, quando um usuário é reconhecido pelo SDK or kit de desenvolvimento de software, um perfil de usuário anônimo é criado com um `braze_id` associado: um identificador único que é atribuído automaticamente pela Braze, não pode ser editado e é específico do dispositivo. Esse identificador pode ser usado para atualizar o perfil de usuário por meio da [API or interface de programação do aplicativo (API)]({{site.baseurl}}/api/endpoints/user_data).
...
Quando você identifica perfis de usuário duplicados no seu espaço de trabalho, pode mesclá-los usando a REST or transferir estado representacional API or interface de programação do aplicativo (API). Para saber mais sobre a mesclagem de usuários e os métodos disponíveis, consulte [Mesclar usuários duplicados]({{site.baseurl}}/user_guide/audience/manage_audience/merge_duplicate_users).
...
Um alias pode ser atualizado com um novo nome para um determinado label após ser definido, seja usando nossos [endpoints de dados de usuário]({{site.baseurl}}/api/endpoints/user_data) ou passando um novo nome pelo SDK or kit de desenvolvimento de software. O alias de usuário ficará visível ao exportar os dados desse usuário.
...
Se você não souber essa informação, poderá chamar o [endpoint `Export user profile by identifier`]({{site.baseurl}}/api/endpoints/export/user_data/post_users_identifier) e encontrar o alias de usuário na resposta da API or interface de programação do aplicativo (API).
...
Você pode definir um novo alias de usuário para perfis de usuário identificados existentes por meio do nosso SDK or kit de desenvolvimento de software e da nossa API or interface de programação do aplicativo (API) usando os [endpoints de dados de usuário]({{site.baseurl}}/api/endpoints/user_data). No entanto, aliases de usuário não podem ser definidos pela API or interface de programação do aplicativo (API) para um perfil de usuário desconhecido já existente.
