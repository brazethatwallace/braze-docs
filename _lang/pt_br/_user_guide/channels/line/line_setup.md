...
- [Canal da API de envio de mensagens do LINE](https://developers.line.biz/en/docs/line-developers-console/overview/#channel)

O envio de mensagens do LINE pela Braze consome os créditos de mensagem ou ação da sua conta.

{% alert note %}
**Configurando `native_line_id`**: Você pode configurar `native_line_id` enviando atualizações de usuário para a Braze (por exemplo, com o [endpoint `/users/track`]({{site.baseurl}}/api/endpoints/user_data/post_user_track), [importação de CSV]({{site.baseurl}}/user_guide/audience/manage_audience/import_users#constructing-your-csv) ou [Cloud Data Ingestion]({{site.baseurl}}/user_guide/data/unification/cloud_ingestion)). Se o SDK do lado do cliente não tiver um campo dedicado para `native_line_id`, envie-o em atualizações de usuário do lado do servidor usando um desses métodos.
{% endalert %}
...
| --- | --- |
| Conta não verificada | Uma conta não revisada que pode ser obtida por qualquer pessoa (física ou jurídica). Essa conta é representada por um badge cinza e não aparecerá nos resultados de pesquisa dentro do app LINE. |
| Conta verificada | Uma conta que passou pela análise do LINE Yahoo. Essa conta é representada por um badge azul e aparecerá nos resultados de pesquisa dentro do app LINE.<br><br>Essa conta está disponível apenas para contas baseadas no Japão, Taiwan, Tailândia e Indonésia. |
| Conta premium | Uma conta que passou pela análise do LINE Yahoo. Essa conta é representada por um badge verde e aparecerá nos resultados de pesquisa dentro do app LINE. Esse tipo de conta é concedido automaticamente durante a análise, a critério do LINE. |
{: .reset-td-br-1 .reset-td-br-2 aria-label="Tipos de contas LINE" }

### Tipo de conta necessário {#required-account-type}

Para sincronizar seguidores com a Braze, sua conta LINE precisa ser verificada ou premium. Ao criar uma conta, o status padrão será não verificada. Você precisará solicitar a verificação da conta.
...
## Integração com o LINE {#integrating-line}
...
Esta etapa é necessária se você tem um usuário LINE existente e identificado, pois a Braze irá automaticamente buscar o status de inscrição e atualizar o perfil de usuário correto. Se você não reconciliou anteriormente os usuários com o LINE ID deles, pule esta etapa.

Você pode importar ou atualizar usuários usando qualquer um dos métodos que a Braze suporta, incluindo o endpoint [`/users/track`]({{site.baseurl}}/api/endpoints/user_data/post_user_track), [importação por CSV]({{site.baseurl}}/user_guide/audience/manage_audience/import_users#constructing-your-csv) ou [Cloud Data Ingestion]({{site.baseurl}}/user_guide/data/unification/cloud_ingestion).
...
{% alert note %}
O status do grupo de inscrições não deve ser especificado e será ignorado. O LINE é a fonte da verdade para o status de inscrição do usuário, que será sincronizado com a Braze por meio da ferramenta de sincronização de inscrições ou por atualizações de eventos.
{% endalert %}

## Etapa 2: Integrar o canal LINE {#step-2-integrate-line-channel}

Após a conclusão do processo de integração, a Braze automaticamente importará os seguidores desse canal LINE para a Braze. Para quaisquer IDs LINE que já estejam associados a um perfil de usuário da Braze, cada perfil será atualizado com o status "subscribed", e quaisquer IDs LINE restantes gerarão usuários anônimos. Além disso, novos seguidores do seu canal LINE terão perfis de usuário não identificados criados quando seguirem o canal.
...
1. No LINE, acesse a guia **Messaging API** e edite suas **Webhook settings**:
   - Defina a **Webhook URL** como `https://anna.braze.com/line/events`.
      - A Braze alterará automaticamente essa URL para uma diferente durante a integração, com base no cluster do seu dashboard.
...
{% alert note %}
Você pode atualizar ou rotacionar o channel secret e o channel access token de um canal LINE já integrado acessando **Integrações de parceiros** > **Parceiros de tecnologia** > **LINE** e selecionando sua integração.
{% endalert %}

{: start="3"}
3. Acesse a página **Settings** > **Response settings** e faça o seguinte:
   - Desative **Greeting message**. Isso pode ser gerenciado na Braze disparando com base no follow.
   - Desative **Auto-response messages**. Todo envio de mensagens disparado deve ser feito pela Braze. Isso não impedirá que você envie diretamente pelo console do LINE.
   - Ative **Webhooks**.

![Página de configurações de resposta com opções para gerenciar como sua conta lidará com chats.]({% image_buster /assets/img/line/response_settings.png %}){: style="max-width:80%;"}
...
{% alert important %}
Durante a integração, verifique se o channel secret está correto. Se estiver incorreto, pode haver inconsistências no status de inscrição.
{% endalert %}

![Página de integração de envio de mensagens LINE com a seção de integração LINE.]({% image_buster /assets/img/line/integration.png %}){: style="max-width:80%;"}
...
Combine os IDs do LINE dos seus usuários com os perfis de usuário existentes da Braze seguindo as etapas em [Reconciliação de ID de usuário](#user-id-reconciliation).
...
Se nenhum perfil de usuário existente for encontrado para o `external_id` fornecido, ele será adicionado ao perfil de usuário não identificado, tornando-o identificado. Se um perfil de usuário já existir para o `external_id`, todos os atributos exclusivos do perfil de usuário não identificado serão copiados para o perfil de usuário conhecido, incluindo `native_line_id` e o status de inscrição do usuário.

Você pode atualizar usuários do LINE conhecidos no seu aplicativo por meio do endpoint [`/users/track`]({{site.baseurl}}/api/endpoints/user_data/post_user_track), passando seus identificadores externos e `native_line_id`. Se um perfil de usuário não identificado já existir para um usuário e o mesmo `native_line_id` for adicionado a um perfil de usuário diferente por meio de `/users/track`, ele herdará todos os estados de inscrição do perfil de usuário não identificado. No entanto, perfis de usuários duplicados existirão com o mesmo `native_line_id`. Quaisquer atualizações de inscrição subsequentes provenientes de atualizações de eventos atualizarão todos os perfis correspondentes.

{% alert note %}
Os estados de inscrição do LINE são rastreados por `native_line_id`, não por `external_id`. Por exemplo, se o perfil de usuário do Usuário B for criado com o mesmo `native_line_id` do Usuário A, mas não com o mesmo `external_id`, o Usuário B herdará o status de inscrição do LINE do Usuário A.
{% endalert %}

Veja um exemplo de carga útil para `/users/track` que atualiza um perfil de usuário pelo ID externo de usuário para adicionar um `native_line_id`:
...
Conforme descrito anteriormente nesta seção, existe a possibilidade de vários perfis de usuário existirem com o mesmo `native_line_id`. Se seus métodos de atualização criarem perfis de usuário duplicados, você pode mesclar perfis de usuário não identificados com perfis de usuário identificados usando o endpoint `/user/merge`.

Veja um exemplo de carga útil para `/users/merge` que tem como alvo um perfil de usuário não identificado pelo alias de usuário `line_id`:
...
O LINE é a fonte da verdade para os estados de inscrição dos usuários. Mesmo que você tenha o LINE ID de um usuário (`native_line_id`), se esse usuário não tiver seguido o canal do LINE de onde você está enviando, o LINE não entregará mensagens a esse usuário.

Para ajudar a gerenciar isso, a Braze oferece ferramentas e lógica que dão suporte a uma base de usuários bem integrada, incluindo sincronização de inscrições e atualizações de eventos para seguidores e cancelamentos de seguimento no LINE.

### Sincronização de inscrições e lógica de eventos {#subscription-syncing-and-event-logic}

Para saber como a ferramenta de sincronização de inscrições e as atualizações de eventos de seguimento e cancelamento de seguimento mantêm o status de inscrição do LINE alinhado com a Braze, consulte [Status de inscrição]({{site.baseurl}}/user_guide/audience/subscription_preferences/subscription_status#line).
...
2. No espaço de trabalho de destino, integre o canal usando a [Etapa 2: Integrar canal LINE](#step-2-integrate-line-channel).

Confirme que você tem a permissão [Manage Subscription Groups]({{site.baseurl}}/user_guide/administer/global/user_management/permissions#list-of-permissions) em ambos os espaços de trabalho. Sem permissões em ambos os espaços de trabalho, a integração falhará com um erro indicando que o canal já está conectado.
...
### Perfil de usuário existente na Braze que já segue o canal LINE {#existing-braze-user-profile-already-follows-line-channel}

1. O perfil de usuário da Braze é atualizado com um atributo `native_line_id`. Seu status de inscrição padrão é `unsubscribed`.
2. A ferramenta de sincronização de inscrição é executada, identifica que o usuário está seguindo o canal LINE e então atualiza o perfil de usuário com o status de inscrição `subscribed`.
3. Se ocorrerem alterações no status de inscrição (como o usuário bloquear, remover ou voltar a seguir o canal), a Braze recebe a atualização do LINE e atualiza o perfil de usuário com o `native_line_id` adequadamente.

### Perfil de usuário existente que bloqueou, removeu ou deixou de seguir o canal LINE {#existing-user-profile-has-blocked-unfriended-or-unfollowed-line-channel}

1. O perfil de usuário da Braze é atualizado com um atributo `native_line_id`. Seu status de inscrição padrão é `unsubscribed`.
2. A ferramenta de sincronização de inscrição não identifica que o usuário está seguindo o canal LINE, e o status de inscrição do usuário permanece como `unsubscribed`.
...
### Criação do perfil de usuário ocorre após seguir o LINE {#user-profile-creation-occurs-after-line-follow}

1. O canal ganha um novo seguidor no LINE.
2. A Braze cria um perfil de usuário anônimo com o atributo `native_line_id` definido como o ID do LINE do seguidor e um alias de usuário `line_id` definido como o ID do LINE do seguidor. O perfil tem um status de inscrição `subscribed`.
3. O usuário é identificado como possuindo o ID do LINE por meio da [reconciliação de usuários](#user-id-reconciliation).
  - O perfil de usuário anônimo pode ser identificado usando o endpoint [`/users/identify`]({{site.baseurl}}/api/endpoints/user_data/post_user_identify). Atualizações subsequentes (por meio do endpoint [`/users/track`]({{site.baseurl}}/api/endpoints/user_data/post_user_track), [importação de CSV]({{site.baseurl}}/user_guide/audience/manage_audience/import_users#constructing-your-csv) ou [ingestão de dados na nuvem]({{site.baseurl}}/user_guide/data/unification/cloud_ingestion)) nesse perfil de usuário podem direcionar o usuário por esse `external_id` conhecido.
...
{% endraw %}
  ...
  - Um novo perfil de usuário pode ser criado (por meio do endpoint [`/users/track`]({{site.baseurl}}/api/endpoints/user_data/post_user_track), [importação de CSV]({{site.baseurl}}/user_guide/audience/manage_audience/import_users#constructing-your-csv) ou [ingestão de dados na nuvem]({{site.baseurl}}/user_guide/data/unification/cloud_ingestion)) definindo o `native_line_id`. Esse novo perfil herdará o estado do status de inscrição do perfil de usuário anônimo existente. Observe que isso resultará em vários perfis compartilhando o mesmo `native_line_id`. Eles podem ser mesclados a qualquer momento usando o endpoint `/users/merge` no processo descrito na [Etapa 5](#step-5-merge-profiles-optional).

### Criação do perfil de usuário ocorre antes de seguir o LINE {#user-profile-creation-occurs-before-line-follow}
...
4. O usuário é identificado como possuindo o ID do LINE por meio da [reconciliação de usuários](#user-id-reconciliation).
...
Os IDs do LINE são recebidos automaticamente pela Braze quando um usuário segue seu canal ou quando você usa o fluxo de trabalho único de "sincronização de seguidores". Os IDs do LINE também são específicos do canal que os usuários seguem, então é improvável que os usuários consigam fornecer seus IDs do LINE.

Existem duas formas de combinar um ID do LINE com um perfil de usuário existente na Braze:

- [Login pelo LINE](#line-login)
- [Vinculação de conta de usuário](#user-account-linking)

### Login pelo LINE {#line-login}

Esse método utiliza logins de redes sociais para reconciliação. Quando um usuário faz login no seu app, ele tem a opção de usar o [LINE Login](https://developers.line.biz/en/docs/line-login/overview/) para criar uma conta de usuário ou fazer login.

{% alert note %}
Para obter o ID correto do LINE para cada usuário, configure o LINE Login no mesmo provedor da sua conta oficial ou canal do LINE integrado com a Braze.
{% endalert %}

1. Acesse o Console de Desenvolvedores do LINE e [solicite permissão para obter os endereços de e-mail dos usuários](https://developers.line.biz/en/docs/line-login/integrate-line-login/#applying-for-email-permission) que fazem login no seu app pelo LINE Login.
...
{: start="3"}
3. Use a [chamada de verificação de token de ID](https://developers.line.biz/en/reference/line-login/#verify-id-token) para obter o e-mail do usuário.

4. Salve o ID do LINE do usuário (`native_line_id`) no perfil do usuário com um e-mail correspondente no seu banco de dados, ou crie um novo perfil de usuário com o e-mail e o ID do LINE.
...
##### Seguidor existente usa o LINE Login {#existing-follower-uses-line-login}
...
##### Novo seguidor usa o LINE Login {#new-follower-uses-line-login}

**Cenário:** Não existe um perfil de usuário na Braze com o ID do LINE do usuário.
...
Esse método permite que os usuários vinculem sua conta do LINE à conta de usuário do seu app. Você pode então usar Liquid na Braze, como {% raw %}`{{line_id}}`{% endraw %}, para criar uma URL personalizada para o usuário que envia o ID do LINE de volta para o seu website ou app, onde pode ser associado a um usuário conhecido.

1. Crie um Canvas baseado em ação que é acionado por uma mudança de estado de inscrição e dispara quando um usuário se inscreve no seu canal LINE.<br>![Canvas que dispara quando um usuário se inscreve no canal LINE.]({% image_buster /assets/img/line/account_link_1.png %})
...
4. (Opcional) Crie uma Campaign ou Canvas baseada em ação que dispara quando o usuário do LINE é identificado, para enviar o código do cupom ao usuário. <br>![Campaign baseada em ação que dispara quando o usuário do LINE é identificado.]({% image_buster /assets/img/line/account_link_2.png %})
...
2. A Braze cria um perfil de usuário anônimo com o ID do LINE, alias de usuário `line_id` e status do grupo de inscrições do LINE como `subscribed`.
...
4. O perfil de usuário anônimo que foi criado é identificado e mesclado por meio do [endpoint /users/identify]({{site.baseurl}}/api/endpoints/user_data/post_user_identify) ao perfil de usuário conhecido. O perfil de usuário conhecido agora contém o ID do LINE e tem status de inscrição `subscribed`.
...
## Criando usuários teste do LINE na Braze {#creating-line-test-users-in-braze}

Você pode testar seu canal do LINE antes de configurar a [reconciliação de usuários](#user-id-reconciliation) criando um Canvas ou uma Campaign do tipo "Quem sou eu".

1. Configure um Canvas que retorne o ID de usuário Braze de um usuário com base em uma palavra-gatilho específica. <br><br>Exemplo de gatilho <br><br>![Gatilho para enviar a Campaign para usuários que enviaram uma mensagem LINE de entrada para um grupo de inscrições específico.]({% image_buster /assets/img/line/trigger.png %}){: style="max-width:80%;"}<br><br>Exemplo de mensagem<br><br>![Mensagem do LINE informando o ID de usuário Braze.]({% image_buster /assets/img/line/message.png %}){: style="max-width:40%;"}<br><br>
