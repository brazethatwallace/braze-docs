...
> Os e-mails de transação da Braze são enviados para facilitar uma transação acordada entre um remetente e o destinatário. Este artigo de referência aborda como criar uma campanha de e-mail de transação no dashboard da Braze e gerar um `campaign_id` para incluir nas suas chamadas de API or interface de programação do aplicativo (API) para o nosso [endpoint `/transactional/v1/campaigns/{campaign_id}/send`]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_transactional_message).

{% alert important %}
O e-mail de transação da Braze está disponível apenas como parte de pacotes selecionados da Braze. Entre em contato com o seu gerente de sucesso do cliente da Braze ou abra um [ticket de suporte]({{site.baseurl}}/user_guide/administer/personal/braze_support) para mais detalhes.
{% endalert %}
...
3. Anote seu `campaign_id`. Depois de salvar sua campanha de API or interface de programação do aplicativo (API), você deve incluir os campos `campaign_id` gerados na sua solicitação de API or interface de programação do aplicativo (API), conforme indicado no artigo [Endpoint de e-mail de transação]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_transactional_message).
4. Clique em **Save Campaign** e pronto para iniciar sua campanha de API or interface de programação do aplicativo (API)!
