...
Você pode usar um webhook Braze-para-Braze quando precisar chamar a [REST API]({{site.baseurl}}/api/basics) da Braze de dentro da Braze para cenários diferentes de atualizações diretas de usuário a partir de etapas do Canvas. Exemplos comuns incluem:

- Disparar um [Canvas acionado por API]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_triggered_canvases) a partir de outro Canvas
...
Para atualizações de usuário dentro do Canvas, o método recomendado é usar a [Atualização de usuário]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/user_update).
...
3. Na guia **Redigir**, especifique a **Webhook URL** e o **Request Body** para o seu caso de uso da API.
4. Na guia **Configurações**, especifique o **HTTP Method** e os **Request Headers** conforme exigido pelo endpoint.
...
- Os webhooks Braze-para-Braze estão sujeitos aos [limites de taxa]({{site.baseurl}}/api/api_limits) dos endpoints.
