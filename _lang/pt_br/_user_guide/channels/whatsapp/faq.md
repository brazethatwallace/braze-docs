...
O gerenciamento de inscrições do canal WhatsApp funciona de forma semelhante a como funciona em outros canais da Braze. Consulte [Gerenciamento de inscrições de usuários]({{site.baseurl}}/user_guide/channels/whatsapp/whatsapp_setup/subscription_groups) para saber mais.
...
A Braze recomenda consultar as [diretrizes da Meta para métodos de aceitação](https://developers.facebook.com/docs/whatsapp/overview/getting-opt-in/) para manter a conformidade. Consulte [Aceitação e cancelamento]({{site.baseurl}}/user_guide/channels/whatsapp/message_processing/opt_ins_and_opt_outs) para métodos de configuração de Canvas e Campaigns.
...
#### Posso usar um grupo de inscrições do WhatsApp da Braze se envio mensagens do WhatsApp por meio de terceiros? {#can-i-use-a-braze-whatsapp-subscription-group-if-i-send-whatsapp-messages-through-a-third-party}
Não. Os grupos de inscrições do WhatsApp da Braze se aplicam a mensagens enviadas pelo canal WhatsApp da Braze. Se você envia mensagens do WhatsApp por meio de um provedor terceirizado ou integrações personalizadas fora de Campaigns e Canvas do WhatsApp da Braze, você pode armazenar o consentimento de aceitação em um atributo personalizado (ou seu próprio modelo de inscrição) e usar esse atributo para segmentação e elegibilidade. Para padrões relacionados quando a Braze é proprietária do número do WhatsApp, consulte [Como conecto suporte e marketing do WhatsApp na Braze?](#how-do-i-connect-whatsapp-support-and-marketing-in-braze).
...
A Meta tem seu próprio limite de throughput separado do limite de envio de mensagens da WABA. O limite padrão que a API em nuvem suporta é de 80 mensagens por segundo. Se você acha que suas campanhas excederão esse limite, pode [solicitar](https://developers.facebook.com/docs/whatsapp/cloud-api/overview/#throughput) que seu limite seja aumentado. A Meta recomenda que você envie essa solicitação com pelo menos três dias de antecedência dos envios da campanha.
...
Você pode criar e enviar modelos na Braze usando o [Construtor de Modelos do WhatsApp]({{site.baseurl}}/user_guide/channels/whatsapp/message_features_and_optimization/template_builder), ou no WhatsApp Manager da Meta. Modelos criados em qualquer um dos locais aparecem no dashboard da Braze com um indicador de status. Após o envio, campos bloqueados exigem reaprovação da Meta; consulte [limitações de edição no FAQ do Construtor de Modelos]({{site.baseurl}}/user_guide/channels/whatsapp/message_features_and_optimization/template_builder#can-i-edit-a-template-after-its-been-approved) para detalhes.
...
O WhatsApp permite que parâmetros variáveis sejam inseridos nos modelos de mensagem. As mensagens não podem começar ou terminar com um parâmetro variável. Os parâmetros variáveis podem ser preenchidos com lógica Liquid na plataforma da Braze. Consulte [compor uma mensagem do WhatsApp na Braze]({{site.baseurl}}/user_guide/channels/whatsapp/create_a_whatsapp_message#step-2-compose-your-whatsapp-message) para saber mais sobre parâmetros variáveis.
...
#### A mídia rica pode ser direcionada ou personalizada na Braze? {#can-rich-media-be-targeted-or-personalized-in-braze}
Sim. Você pode fazer upload de imagens estáticas da biblioteca de mídia, ou adicionar imagens por URL e personalizá-las com [Liquid]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/liquid) ou [Connected Content]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/connected_content). URLs de imagem suportam lógica Liquid completa em qualquer parte da URL. Isso se aplica a mensagens de modelo e mensagens de resposta (mensagens de mídia e layouts de resposta rápida). Para detalhes, consulte [Imagens dinâmicas]({{site.baseurl}}/user_guide/channels/whatsapp/create_a_whatsapp_message#dynamic-images).
...
Se você acredita que a Meta sinalizou incorretamente seu modelo, use o link de revisão no e-mail do WhatsApp para solicitar uma reanálise. A equipe do WhatsApp Business revisa a decisão e a reverte se apropriado.
...
#### Por que minha Campaign do WhatsApp não está enviando apesar da prévia do modelo funcionar? {#why-is-my-whatsapp-campaign-not-sending-despite-template-previewing}
Se seu modelo é exibido corretamente na prévia, mas o registro de processamento mostra **Abort** com detalhes "Param text cannot have new-line/tab characters or more than 4 consecutive spaces", verifique os valores dos parâmetros com Liquid na sua mensagem. O WhatsApp exige que os valores de texto dos parâmetros não contenham:
...
#### A Braze suporta fallback automático para SMS quando a entrega do WhatsApp falha? {#does-braze-support-automatic-sms-fallback-when-whatsapp-delivery-fails}

Não. A Braze não oferece um caminho nativo de fallback de WhatsApp para SMS. Para tentar novamente em outro canal, segmente usuários com envios de WhatsApp que falharam (por exemplo, por meio de eventos de falha do Currents) e direcione uma Campaign de SMS ou e-mail.

#### As mensagens de resposta do WhatsApp são gratuitas? {#are-whatsapp-response-messages-free}

Mensagens de resposta compostas no editor de Campaign ou Canvas da Braze (não modelos aprovados do WhatsApp) são tratadas como mensagens de serviço pela Meta. Mensagens de serviço enviadas pela integração nativa do WhatsApp da Braze não consomem Action Credits quando são enviadas como [mensagens de resposta]({{site.baseurl}}/user_guide/channels/whatsapp/create_a_whatsapp_message#response-messages) dentro de uma janela de atendimento ao cliente aberta.
...
Para enviar informações para a Braze, por exemplo, para indicar que um usuário está em uma conversa de suporte ativa, você pode registrar um atributo personalizado (como um booleano "has existing support chat = true/false") e usar isso como critério de segmentação em suas campanhas de marketing. Você também pode usar deep links entre dois threads de conversa para direcionar os usuários ao thread de suporte a partir do thread de marketing e vice-versa.
