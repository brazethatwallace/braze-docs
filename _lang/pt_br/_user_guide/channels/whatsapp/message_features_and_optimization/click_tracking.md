...
Para mensagens de modelo, o URL base deve ser enviado corretamente ao criar o modelo para ativar o rastreamento de cliques.

#### Etapa 1: Crie um modelo compatível com rastreamento de cliques no WhatsApp {#step-1-build-a-click-tracking-supported-template-in-whatsapp}

1. No seu WhatsApp Manager, crie um URL base que seja seu domínio personalizado ou `brz.ai`.
...
4. Para links de botão CTA, selecione **Dynamic** e forneça o URL base (`brz.ai` ou seu domínio personalizado).<br><br>![Seção para criar uma chamada para ação.]({% image_buster /assets/img/whatsapp/click_tracking/create_cta.png %})<br><br>
5. Para links no corpo do texto, ao escrever o modelo no seu WhatsApp Manager, remova quaisquer espaços inseridos nos links contidos no corpo que você deseja rastrear.<br><br>![Caixa de texto para inserir o corpo do conteúdo da chamada para ação.]({% image_buster /assets/img/whatsapp/click_tracking/cta_textbox.png %})

#### Etapa 2: Conclua seu modelo na Braze {#step-2-complete-your-template-in-braze}

Ao redigir, a Braze detectará automaticamente quais modelos possuem domínios de URL compatíveis, tanto no corpo do texto quanto para botões CTA. O status será exibido na parte inferior do modelo.
...
O URL de destino precisará ser fornecido para qualquer link com um URL base que corresponda a `brz.ai` ou ao seu domínio personalizado.
