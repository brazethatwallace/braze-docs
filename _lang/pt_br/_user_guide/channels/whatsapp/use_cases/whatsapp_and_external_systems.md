...
| Chave da API or interface de programação do aplicativo (API) REST or transferir estado representacional da Braze | Uma chave da API or interface de programação do aplicativo (API) REST or transferir estado representacional com permissões de `campaigns.trigger.send`. Ela pode ser criada no dashboard da Braze acessando **Configurações** > **Chaves de API or interface de programação do aplicativo (API)**. |
...
Para automatizar essa comunicação de forma eficiente, essa integração utiliza dois recursos principais da Braze: [Campaigns de webhook]({{site.baseurl}}/user_guide/channels/webhooks/create_a_webhook) e [Campaigns disparadas por API or interface de programação do aplicativo (API)]({{site.baseurl}}/user_guide/messaging/campaigns/schedule_your_campaign/api_triggered_delivery).
...
3. No campo **Webhook URL**, insira o endpoint de API or interface de programação do aplicativo (API) (URL) do sistema externo que receberá a mensagem.
...
### Etapa 2: Criar uma Campaign disparada por API or interface de programação do aplicativo (API) para mensagens de saída {#step-2}

Em seguida, crie uma Campaign disparada por API or interface de programação do aplicativo (API) para estabelecer uma forma de o seu sistema externo enviar mensagens de volta aos usuários pelo WhatsApp.
...
{: start="3"}
3. Adicione a propriedade de gatilho de API or interface de programação do aplicativo (API) ao corpo da mensagem, como {% raw %}`{{api_trigger_properties.${external_system_msg+body}}}`{% endraw %}. Isso permite que o seu sistema de IA preencha a mensagem que será enviada.
...
### Etapa 3: Conectar o sistema externo à Campaign disparada por API or interface de programação do aplicativo (API) {#step-3-connect-the-external-system-to-the-api-triggered-campaign}
...
3. Use a propriedade de gatilho de API or interface de programação do aplicativo (API) da [Etapa 2](#step-2) para inserir a resposta do sistema externo e não se esqueça de incluir sua chave de API or interface de programação do aplicativo (API) no cabeçalho da requisição para autenticação, como neste exemplo de cURL:
...
- Criar fluxos de conversa mais complexos com Campaigns disparadas por API or interface de programação do aplicativo (API) em múltiplas etapas.
