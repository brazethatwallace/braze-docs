...
- Um `external_id`, um [número de telefone]({{site.baseurl}}/user_guide/channels/whatsapp/whatsapp_setup/user_phone_numbers) e um status de inscrição atualizado para cada usuário. Isso pode ser feito usando o [SDK or kit de desenvolvimento de software](https://braze-inc.github.io/braze-swift-sdk/documentation/brazekit/braze/user-swift.class/addtosubscriptiongroup(id:fileid:line:)/) ou por meio do [endpoint `/users/track`]({{site.baseurl}}/api/endpoints/user_data/post_user_track) para atualizar o número de telefone e o status de inscrição.

Uma mensagem recebida pelo WhatsApp não inscreve automaticamente o usuário no seu grupo de inscrições do WhatsApp. Você precisa atualizar explicitamente o status de inscrição com uma [etapa de atualização de usuário](#user-update-step), [webhook](#webhook-campaign-to-trigger-a-second-whatsapp-campaign) ou chamada de API or interface de programação do aplicativo (API).
...
4. Para qualquer uma dessas opções, você provavelmente precisará concluir a jornada com o seguinte:
	...
	- Utilizar o [endpoint `/subscription/status/set`]({{site.baseurl}}/api/endpoints/subscription_groups/post_update_user_subscription_group_status) ou usar o [SDK or kit de desenvolvimento de software](https://braze-inc.github.io/braze-swift-sdk/documentation/brazekit/braze/user-swift.class/addtosubscriptiongroup(id:fileid:line:)/)
...
- Segment or segmento de clientes: grupo de marketing inscrito fora dos EUA
...
- **Os grupos de inscrições da Braze** são gerenciados pela sua integração com a Braze (API or interface de programação do aplicativo (API), Central de Preferências ou SDK or kit de desenvolvimento de software) e controlam quais usuários você direciona para o envio de mensagens.
...
{: start="5"}
5. Crie uma Campaign de WhatsApp (sua segunda Campaign) e defina o disparador como API or interface de programação do aplicativo (API). Certifique-se de copiar este `campaign_id` na carga útil JSON da sua primeira Campaign.

#### Considerações

- Atualizações de atributos dentro da carga útil JSON de disparo da API or interface de programação do aplicativo (API) do Canvas ainda não são suportadas, então você só pode disparar uma Campaign de WhatsApp para a mensagem de resposta do WhatsApp (como na etapa 2).
