...
Tanto Campaigns quanto Canvas podem iniciar, ramificar e ter alterações no meio da jornada a partir de uma mensagem de WhatsApp recebida (um usuário enviando uma mensagem para o seu WhatsApp), como uma palavra de disparo.

Certifique-se de que sua palavra de disparo corresponda ao que você espera dos usuários.

**Informações importantes:**
- Cada letra da sua palavra de disparo deve estar em maiúscula quando configurada. A Braze não exige que as palavras de disparo recebidas enviadas pelos usuários estejam em maiúscula. Por exemplo, enviar "jOin2023" ainda disparará o Canvas ou a Campaign.
- Se nenhuma palavra de disparo for especificada no disparador baseado em ação do cronograma de entrada, a Campaign ou o Canvas será executado para TODAS as mensagens de WhatsApp recebidas. Isso inclui mensagens que correspondem a frases em Campaigns e Canvas ativos, caso em que o usuário receberá duas mensagens de WhatsApp.

{% tabs %}
{% tab Campaign %}

![Opções de agendamento de Campaign baseado em ação.]({% image_buster /assets/img/whatsapp/whatsapp27.png %})
...
- Na etapa de ação, após criar os grupos de ação para as frases de filtro personalizadas, adicione um grupo de ação adicional para "Enviar mensagem do WhatsApp", mas **não marque Where the message body**. Isso capturará todas as respostas não reconhecidas dos usuários, de forma semelhante a uma cláusula "else".
- Recomendamos enviar uma mensagem do WhatsApp informando o usuário de que esse canal não é monitorado e orientando-o para um canal de suporte, se necessário.
...
![Criador de mensagem da etapa do WhatsApp com um modelo de resposta rápida preenchido.]({% image_buster /assets/img/whatsapp/whatsapp14.png %})

Crie uma etapa de ação que siga a etapa de mensagem. Crie um grupo por opção de resposta rápida nessa etapa de ação.

![Um Canvas em que a ação de avaliação é "enviar uma mensagem de entrada do WhatsApp".]({% image_buster /assets/img/whatsapp/whatsapp15.png %})

Para cada grupo de opção de resposta rápida, especifique o texto exato correspondente ao botão. As palavras-chave devem estar em letras maiúsculas.

![Uma etapa do Canvas em que a ação "enviar uma mensagem de entrada do WhatsApp" está configurada para enviar quando um corpo de mensagem específico é recebido.]({% image_buster /assets/img/whatsapp/whatsapp16.png %})
...
Provavelmente você vai querer uma mensagem de resposta para cada opção. Recomendamos ter uma opção genérica para respostas fora do escopo das respostas rápidas (como para clientes que respondem com uma mensagem geral em vez de uma opção predefinida). Por exemplo, "Desculpe, não reconhecemos sua resposta. Para questões de suporte, envie uma mensagem para <canal de suporte>."
...
Para respostas em menos de um minuto em fluxos bidirecionais do Canvas, minimize as etapas entre o disparo de entrada e o envio da mensagem de resposta. A arquitetura do Canvas, as idas e vindas de webhooks e o agrupamento de atualizações de usuários podem adicionar latência. Consulte [Minimizar a latência de resposta para fluxos bidirecionais]({{site.baseurl}}/user_guide/channels/whatsapp/best_practices#minimize-response-latency-for-two-way-flows).

Os seguintes eventos desbloqueiam mensagens de resposta:

- Mensagem de entrada
  - [Jornada de ação]({{site.baseurl}}/action_paths) ou [entrada baseada em ação]({{site.baseurl}}/user_guide/messaging/campaigns/schedule_your_campaign/triggered_delivery) com o disparo **Enviar uma mensagem de entrada do WhatsApp**.
...
- A etapa de mensagem de resposta ainda deve ocorrer dentro de 24 horas após a mensagem de entrada do usuário. Na maioria dos fluxos do Canvas, a resposta é enviada imediatamente após a avaliação da Jornada de ação, então isso não é um problema.
...
Se a sua Campaign ou o público do Canvas do WhatsApp baseado em ação depende de um atributo de tempo personalizado dentro de uma janela relativa (por exemplo, entre agora e as próximas 24 horas), combine dois filtros conforme descrito em [Tempo]({{site.baseurl}}/user_guide/data/activation/custom_data/custom_attributes#time).
