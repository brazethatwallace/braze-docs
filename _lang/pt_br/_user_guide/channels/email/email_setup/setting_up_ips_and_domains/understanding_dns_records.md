...
- Delegação CNAME (SendGrid e SparkPost): Crie um `CNAME` apontando seu subdomínio de volta para o provedor. O provedor hospeda e atualiza as políticas SPF em sua infraestrutura, passando na verificação SPF automaticamente.
...
O DKIM adiciona uma assinatura digital criptográfica ao cabeçalho do e-mail. O servidor receptor usa a chave pública do remetente (publicada no DNS) para verificar que o e-mail foi originado pelo proprietário do domínio e não foi alterado em trânsito.
...
## Arquitetura DNS específica por provedor {#esp-specific-dns-architecture}

Diferentes arquiteturas de provedores lidam com a delegação DNS de formas distintas. Ao provisionar seu ambiente, use os registros exatos mapeados para o cluster específico do seu provedor.

### Arquitetura SparkPost {#sparkpost-architecture}

O SparkPost usa uma configuração híbrida. Ele utiliza registros `CNAME` explícitos para apontar a infraestrutura de rastreamento e Return-Path de volta ao SparkPost, enquanto usa um registro `TXT` bruto para autenticação DKIM.
...
- Configuração DKIM: O SparkPost exige um registro `TXT` contendo a string exata da chave pública mapeada para um seletor específico.
...
| --- | --- | --- | --- |
| CNAME | mail.example.com | smtp.sparkpostmail.com | Return-Path / alinhamento SPF |
...
- Configuração DKIM: O SendGrid gera dois registros `CNAME` separados para DKIM (geralmente usando seletores como `s1` e `s2`). Eles apontam de volta para as chaves do SendGrid.
- O SendGrid fornece dois registros `CNAME` DKIM para que possa rotacionar as chaves criptográficas automaticamente sem exigir que você atualize seu DNS manualmente.
...
O Amazon SES usa Easy DKIM com registros `CNAME` junto com roteamento explícito por `MX` e `TXT` para rastreamento personalizado de bounces.

- Configuração DKIM: O Amazon SES usa Easy DKIM, fornecendo três registros `CNAME`. Eles apontam para subdomínios gerenciados pela AWS contendo as chaves públicas. O SES rotaciona essas chaves automaticamente de forma transparente para manter a conformidade de segurança.
- Configuração de SPF e MAIL FROM personalizado: O SendGrid e o SparkPost gerenciam o roteamento do domínio de bounce por meio de um `CNAME`. O Amazon SES exige um registro `MX` explícito e um registro `TXT` colocados diretamente no subdomínio MAIL FROM designado. O registro `MX` garante que os avisos de bounce retornem aos servidores da Amazon, e o registro `TXT` contém a string SPF autorizada e fixa.
...
### Divisão de strings em registros TXT DKIM {#txt-dkim-record-string-splitting}

Ao implantar o SparkPost ou configurações DKIM manuais, você pode encontrar chaves criptográficas longas (chaves DKIM de 2048 bits).

A especificação principal do DNS (RFC 1035) limita qualquer string de caracteres individual dentro de um registro `TXT` a um máximo de 255 caracteres. Uma chave pública de 2048 bits rotineiramente excede 400 caracteres, fazendo com que os registros de domínio rejeitem a string única ou a truncem, invalidando a assinatura.

A divisão de strings resolve esse problema. Divida a string de caracteres em blocos de menos de 255 caracteres. Coloque cada bloco entre aspas retas, separados por um espaço, dentro do mesmo registro `TXT`.
...
Se campanhas de marketing, recibos transacionais e e-mails internos de colaboradores compartilharem um espaço de domínio raiz idêntico, um pico repentino de reclamações de SPAM em marketing pode prejudicar a reputação do domínio principal. Isso arrisca direcionar comunicações corporativas críticas para pastas de SPAM. Usar um subdomínio distinto isola a reputação do seu alcance de marketing.

## Fluxo de trabalho de implementação {#implementation-workflow}

Para garantir uma transferência e implementação tranquilas, siga esta sequência:
...
2. Defina um valor baixo de Time-To-Live (TTL) (por exemplo, 300 segundos ou cinco minutos) para testes iniciais. Isso permite recuperação rápida caso ocorra um erro de digitação durante a inserção.
3. Execute uma consulta DNS (por exemplo, `dig CNAME mail.example.com`) ou use uma ferramenta de validação para confirmar que os registros resolvem corretamente antes de passar para a fase de aquecimento.

## Documentação de provedores DNS {#dns-provider-documentation}

Cada provedor DNS tem uma interface única. Compartilhe essas especificações com seu administrador de rede ou consulte a documentação específica do seu provedor para mapear as entradas corretamente no seu arquivo de zona.
...
Para recursos adicionais sobre provedores de domínio, consulte [Configurar IPs e domínios]({{site.baseurl}}/user_guide/channels/email/email_setup/setting_up_ips_and_domains#step-3-add-dns-records).