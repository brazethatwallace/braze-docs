...
**Grupos de teste:** Para Campaigns com [grupos de teste]({{site.baseurl}}/user_guide/administer/global/user_management/internal_groups#seed-groups), a Braze seleciona um perfil para entrega primária quando vários perfis compartilham um endereço. Esse destinatário primário pode não estar no seu grupo de teste, mesmo quando outro perfil com o mesmo endereço está.
...
Além disso, as mensagens podem estar sendo entregues e indo parar no SPAM, indicando problemas de reputação potencialmente sérios. É importante monitorar não apenas o número de mensagens sendo entregues, mas também as taxas de abertura e cliques para determinar se os usuários estão realmente vendo as mensagens em suas caixas de entrada. Como os provedores geralmente não reportam todas as instâncias de SPAM, uma taxa de SPAM de até 1% pode ser motivo de preocupação e análise adicional.
...
Esse é o comportamento esperado. Para precisão na taxa de abertura, crie e-mails como HTML e esteja ciente de que as aberturas não serão contabilizadas quando os destinatários visualizarem a versão em texto simples.
...
### Por que estou vendo mais _Cliques totais_ do que _Aberturas totais_ na minha análise de e-mail? {#why-am-i-seeing-more-_total-clicks_-than-_total-opens_-in-my-email-analytics}
...
- Os usuários clicam em alguns links do e-mail dentro do painel de prévia de seus telefones. Nesse caso, a Braze registra esse e-mail como clicado, mas não aberto.
...
A análise da Campaign mostra o número total de eventos de clique, enquanto os Segments retornam o número de usuários únicos que realizaram esses cliques. Como cada usuário pode clicar várias vezes, o total de cliques na análise é frequentemente maior do que a contagem de usuários que clicaram quando você cria um Segment.

Por exemplo, se 100 usuários clicam em um link 3 vezes cada, a análise da Campaign mostra 300 cliques totais, mas um Segment filtrado por "Clicou no e-mail" para essa Campaign retorna 100 usuários.
...
Quando um e-mail é cortado pelo provedor de e-mail do destinatário (como o Gmail cortando mensagens acima de aproximadamente 102 KB), o conteúdo na parte inferior do e-mail pode ser truncado. Como o pixel de rastreamento de abertura é tipicamente inserido na parte inferior do e-mail, o corte pode impedir que o rastreamento de abertura funcione.
...
Se o número total de cliques no link de cancelamento de inscrição no corpo for maior do que o número de _Cancelamentos de inscrição_, os usuários podem ter clicado no link mais de uma vez — por exemplo, se cancelaram a inscrição, se reinscreveram e cancelaram novamente, a análise de e-mail pode registrar múltiplos cliques no detalhamento de cliques.

Se um usuário clicar no link de cancelamento de inscrição duas vezes (por exemplo, se cancelou a inscrição, se inscreveu novamente e depois cancelou novamente), isso conta duas vezes na análise de e-mail.
...
| O destinatário tem filtragem de e-mail personalizada | O usuário ou seu administrador de TI pode ter configurado regras de caixa de entrada que filtram, redirecionam ou excluem mensagens recebidas. |
...
O [Relay de e-mail privado da Apple]({{site.baseurl}}/user_guide/channels/email/best_practices/apple_mail/email_private_relay_apple_SSO) exige que você registre seus domínios de envio no Portal de Desenvolvedores da Apple para evitar bounces. O Google Shielded Email não exige um processo manual de registro de domínio ou lista de permissões.
...
Se você envia um e-mail de teste da Braze para seu próprio endereço de e-mail e vê um aviso de SPAM ou alerta de phishing — como "o domínio de envio é semelhante ao domínio da sua empresa, mas não o reconhecemos" — esse é um recurso comum de segurança anti-phishing, não um erro na sua configuração da Braze.
...
3. Se todas as três passarem, seu envio de e-mail pela Braze está configurado corretamente.
