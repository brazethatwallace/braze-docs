...
page_order: 2
description: "Este artigo aborda detalhes criativos, como recomendações de tamanho de imagem e comportamento de descarte nos três tipos padrão de Cartões de conteúdo."
channel:
  - content cards
tool: Media
...
![Imagem de um feed de Cartões de conteúdo vazio]({% image_buster /assets/img/empty-cc.png %}){: style="max-width:45%"}

{% alert tip %}
Mantenha os Cartões de conteúdo relevantes configurando-os para serem descartados quando o usuário realizar ações relevantes. Por exemplo, configure Cartões de conteúdo promocionais para serem descartados assim que os usuários fizerem uma compra, para que eles não continuem vendo uma oferta de algo que já compraram.
{% endalert %}

### Usando GIFs em Cartões de conteúdo

| Cartões de conteúdo para Android | Cartões de conteúdo para iOS | Cartões de conteúdo para Web |
| --- | --- |---|
