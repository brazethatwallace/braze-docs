...
#### Filtrar pela última mensagem de SMS/MMS/RCS recebida {#filter-by-last-received-smsmmsrcs}
...
Filtra os usuários que receberam uma mensagem de uma campaign específica. Com esse filtro, você também pode optar por filtrar aqueles que não receberam mensagens de uma campaign.

![Filtro de segmentação Recebeu mensagem da campaign "SMS retargeting".]({% image_buster /assets/img/sms/filter1.png %})

### Disparar mensagens quando os usuários receberem SMS, MMS ou RCS {#trigger-messages}

Para disparar mensagens quando os usuários receberem mensagens de SMS, MMS ou RCS de uma campaign específica, selecione **Interagir com Campaign** como a ação-gatilho para uma campaign baseada em ação. Em seguida, selecione **Receber SMS** e a campaign que você deseja usar.

![Para disparar mensagens quando os usuários receberem mensagens de SMS, MMS ou RCS de uma campaign específica, selecione Interagir com Campaign como a ação-gatilho para uma campaign baseada em ação. Em seguida, selecione Receber SMS e a campaign de SMS, MMS ou RCS que você deseja usar.]({% image_buster /assets/img/sms/trigger.png %})

### Filtrar por links de rastreamento avançado {#filter-by-advanced-tracking-links}

Redirecione os usuários que clicaram em campaigns com [links de rastreamento avançado]({{site.baseurl}}/user_guide/channels/sms_mms_and_rcs/message_features_and_optimization/link_shortening).
Apenas as campaigns que possuem rastreamento avançado ativado aparecem nos menus suspensos a seguir:

#### Redirecionar usuários que clicaram em uma Campaign de SMS, MMS ou RCS específica {#retarget-users-who-have-clicked-a-specific-sms-mms-or-rcs-campaign}
...
Além das três categorias de palavras-chave padrão (Aceitação, Cancelamento e Ajuda), você também pode criar até 25 categorias de palavras-chave personalizadas, permitindo identificar palavras-chave e respostas arbitrárias. Essas categorias podem ser usadas para filtragem e redirecionamento. Para saber mais sobre as categorias de palavras-chave globais e como configurá-las, consulte [Processamento de palavras-chave]({{site.baseurl}}/user_guide/channels/sms_mms_and_rcs/message_features_and_optimization/keyword_processing).
...
![Filtro de segmentação "Último SMS enviado ao grupo de inscrições "Marketing SMS" com a palavra-chave "Opt-in" após 11 de agosto de 2020.]({% image_buster /assets/img/sms/retargeting1.png %})
...
{% alert note %}
Esses filtros usam [dados de interação de mensagens]({{site.baseurl}}/messaging_interaction_data). Para Campaigns e Canvas interrompidos, esses dados expiram após três meses, a menos que sejam usados em um filtro de redirecionamento ativo. Dados expirados podem ser recuperados. A janela de retenção do seu espaço de trabalho pode ser diferente do padrão.
{% endalert %}
...
![Campaign com o filtro "Respondeu SMS" para a campaign "SMS-283" "Promotion".]({% image_buster /assets/img/sms/clicked_opened_campaign.png %})
...
![Campaign com o filtro "Respondeu SMS" para campaign ou Canvas com a tag "Curbside Messaging Service C".]({% image_buster /assets/img/sms/clicked_opened_campaign_canvas_tag.png %})

#### Filtrar por resposta a uma etapa específica {#filter-by-replied-to-a-specific-step}

![Campaign com o filtro "Respondeu SMS" para a etapa "SMS Double Opt" "Step - Help".]({% image_buster /assets/img/sms/clicked_opened_step.png %})

### Disparar mensagens por palavra-chave {#trigger-messages-by-keyword}

Mensagens podem ser disparadas quando os usuários enviam mensagens de entrada com base em categorias de palavras-chave (o usuário enviou qualquer uma das palavras-chave) ou outras palavras-chave (o usuário enviou uma palavra-chave que não se encaixa em nenhuma das categorias existentes). Esses gatilhos são configurados na etapa de Entrega do criador de campaigns.

Ao avaliar se uma mensagem de entrada corresponde a um evento-gatilho definido, os espaços no início e no final são removidos antes do início da avaliação.

{% alert tip %}
Se um Canvas baseado em ação for disparado por uma mensagem SMS ou MMS de entrada, você pode referenciar [propriedades Liquid de SMS compatíveis]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/liquid/supported_personalization_tags) em qualquer etapa do Canvas até a próxima jornada de ação.
{% endalert %}

#### Disparar por categoria de palavra-chave de entrada {#trigger-by-inbound-keyword-category}

![Campaign SMS baseada em ação com o filtro de segmentação "Enviou palavra-chave "Opt-in" ao grupo de inscrições "Marketing SMS".]({% image_buster /assets/img/sms/retargeting2.png %}){: style="margin-top:10px;"}

#### Disparar por palavras-chave arbitrárias {#trigger-by-arbitrary-keywords}

Ao disparar uma mensagem com uma resposta de palavra-chave "Outra", você tem a oportunidade de avaliar o corpo da palavra-chave em uma correspondência exata de texto. Essa correspondência segue as mesmas regras mencionadas: apenas a **mensagem exata de uma única palavra** é processada (sem distinção entre _maiúsculas e minúsculas_). Uma palavra-chave enviada como `Hello Braze!` não corresponderia aos critérios mostrados no exemplo a seguir.

![Campaign SMS baseada em ação com a categoria de palavra-chave como "Other", onde o corpo da mensagem é exatamente "Hello" ou "Hey".]({% image_buster /assets/img/sms/retargeting3.png %}){: style="margin-top:10px;"}

#### Usar modelos de palavras-chave {#template-keywords}

Ao disparar uma Campaign ou componente do Canvas com um SMS ou MMS de entrada, você pode opcionalmente usar como modelo o texto ou os anexos de mídia que o usuário enviou no corpo da sua Campaign ou Canvas com Liquid. Isso permite acessar a resposta do usuário, que você pode incluir na sua resposta, aplicar lógica condicional ou qualquer outra coisa que possa fazer com Liquid.
