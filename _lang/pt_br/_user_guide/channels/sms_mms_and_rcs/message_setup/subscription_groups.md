...
## Estados dos grupos de inscrições {#subscription-group-states}
...
3. Adicione a [etapa Agent]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/agent_step) ao Canvas para identificar a intenção de cancelamento de inscrição.
4. Adicione uma [etapa de mensagem]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/message_step) SMS subsequente para confirmar a solicitação: "Parece que você está tentando cancelar a inscrição de SMS, então vamos cancelar sua inscrição. Se isso foi um engano, envie START para se inscrever novamente."
...
Escolha nomes descritivos e claros para os grupos de inscrições, de modo que o grupo correto seja selecionado ao criar Campaigns de SMS.

### Separe os grupos por país {#separate-groups-by-country}

As regulamentações de SMS variam por país. Sugerimos separar os grupos de inscrições de SMS por país. Isso ajuda você a atender aos padrões de conformidade em todas as regiões para as quais envia mensagens.
