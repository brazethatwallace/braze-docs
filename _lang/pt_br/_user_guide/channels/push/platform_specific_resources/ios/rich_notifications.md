...
- Para garantir que seu app possa enviar notificações Rich, siga as instruções de [integração de push para iOS]({{site.baseurl}}/developer_guide/push_notifications/rich?sdktab=swift), pois seu desenvolvedor precisará adicionar uma extensão de serviço ao seu app.
- Os tipos de arquivo que atualmente aceitamos para upload direto no nosso dashboard incluem JPEG, PNG ou GIF. Esses arquivos também podem ser inseridos no campo de URL modelável junto com estes tipos de arquivo adicionais: AIF, M4A, MP3, MP4 ou WAV.
...
- O iOS redimensiona as imagens para caber na tela e redimensiona as imagens Rich para a visualização ativa ou bloqueada.

{% alert note %}
Desde janeiro de 2020, as notificações Rich por push do iOS podem lidar com imagens de 1038x1038 com menos de 10&nbsp;MB, mas recomendamos usar o menor tamanho de arquivo possível. Na prática, enviar arquivos grandes pode causar sobrecarga desnecessária na rede e tornar os tempos limite de download mais comuns.
{% endalert %}
...
Embora não possamos fornecer uma regra rígida sobre o número preciso de caracteres a incluir em um push, nós [fornecemos algumas diretrizes]({{site.baseurl}}/user_guide/channels/push/create_a_push_message/message_and_image_formats) a considerar ao projetar mensagens iOS. Pode haver alguma variação dependendo da presença de uma imagem, do estado da notificação e da configuração de exibição do dispositivo do usuário, e do tamanho do dispositivo. Na dúvida, seja breve e direto.

Como prática recomendada, a Braze sugere manter cada linha de texto, tanto para o título opcional quanto para o corpo da mensagem, em aproximadamente 30 a 40 caracteres em uma notificação por push móvel.

#### Estados da notificação {#notification-states}

Seus usuários podem visualizar notificações por push em diversas situações diferentes e podem ver comprimentos de texto variados, conforme descrito a seguir.
...
<tbody>
  <tr>
    ...
    <td width="33%">Quando o usuário mantém pressionada uma mensagem.<br><br><b>Título:</b> 1 linha de texto<br><b>Corpo:</b> 7 linhas de texto<br><b>Imagem:</b> proporção 2:1 (recomendada, veja a nota a seguir)</td>
    <td width="33%">Quando o usuário recebe um push enquanto o celular está desbloqueado e ativo.<br><br><b>Título:</b> 1 linha de texto<br><b>Corpo:</b> 2 linhas de texto</td>
  </tr>
</tbody>
</table>
...
{% alert note %}
Embora recomendemos uma proporção 2:1 para notificações por push expandidas, praticamente qualquer proporção é compatível. As imagens sempre ocupam toda a largura da notificação, e a altura é ajustada proporcionalmente.
{% endalert %}

#### Variáveis na truncagem de texto {#variables-in-text-truncation}

Ao criar conteúdo, considere os seguintes cenários que podem impactar a quantidade de texto exibida.

{% tabs %}
{% tab Horário %}

Dependendo de quando o usuário interage com uma notificação por push, o carimbo de data/hora pode encurtar o texto do título.
...
O texto do corpo é reduzido em aproximadamente 10 caracteres por linha quando uma imagem está presente.
...
- **Configurações de exibição do celular:** o usuário pode aumentar ou diminuir o tamanho global da fonte da interface no celular, geralmente por motivos de acessibilidade.
- **Largura do dispositivo:** a mensagem pode ser exibida em um celular pequeno ou em um iPad amplo.
...
Siga as [etapas da Campaign]({{site.baseurl}}/user_guide/channels/push/create_a_push_message) para criar uma notificação por push para iOS. Você usará o mesmo criador utilizado para configurar notificações por push que não contêm conteúdo Rich.

### Etapa 2: Adicionar mídia {#step-2-add-media}

Adicione seu arquivo de imagem, GIF, áudio ou vídeo no campo **iOS Notification Image** no criador de mensagens. Consulte os [requisitos](#requirements) sobre como adicionar seus arquivos de conteúdo.
...
Você também pode limitar essa mensagem para enviar apenas a usuários que possuem um dispositivo com iOS 10. Para usuários que não fizeram upgrade para o iOS 10, a notificação aparecerá apenas como texto, sem o conteúdo Rich, se você deixar a opção **Only send to devices with Rich Notification support** desmarcada.
...
### Etapa 3: Continuar criando sua Campaign {#step-3-continue-creating-your-campaign}

Depois que o conteúdo da sua notificação Rich for enviado ao dashboard, você pode continuar [agendando sua Campaign]({{site.baseurl}}/user_guide/channels/push/create_a_push_message#choose-delivery-schedule-or-trigger).

Quando um usuário receber a notificação por push, ele pode pressionar com força a mensagem para expandir a imagem.

![Um usuário recebe uma notificação por push e pressiona com força a mensagem para exibir uma imagem expandida que diz "Hello!".]({% image_buster /assets/img_archive/rich_notification_ios.gif %}){: style="max-width:50%;" }