...
A partir do nível de API or interface de programação do aplicativo (API) 26 (Android O), as notificações por push exigem um canal válido para serem exibidas. Se o seu app tem como alvo o Android O ou posterior, você deve usar a versão 2.1.0 ou posterior do SDK or kit de desenvolvimento de software da Braze. Sua equipe de desenvolvimento deve definir os canais que você deseja usar, bem como as configurações de notificação sugeridas (por exemplo, importância, som, luzes) para cada canal no código do seu app. Para mais informações, consulte a [documentação para desenvolvedores do Android](https://developer.android.com/preview/features/notification-channels.html) e a [documentação para desenvolvedores da Braze]({{site.baseurl}}/developer_guide/push_notifications?sdktab=android).
...
## Canal padrão do SDK or kit de desenvolvimento de software {#sdk-default-channel}

O Android requer um canal válido para exibir notificações por push no nível de API or interface de programação do aplicativo (API) 26 (Android O) ou posterior. O SDK or kit de desenvolvimento de software para Android da Braze 2.1.0 inclui um canal padrão chamado "General", que será criado e usado caso você não especifique canais adicionais no dashboard ou tente enviar para um canal inválido. Você pode renomear esse rótulo no SDK or kit de desenvolvimento de software e fornecer uma descrição do canal. Recomendamos que considere isso para proporcionar uma melhor experiência do usuário.
...
Caso um ID de canal inválido seja fornecido à Braze (por exemplo, se você fornecer um ID de canal que seus desenvolvedores não criaram no SDK or kit de desenvolvimento de software), a notificação será entregue pelo canal padrão do SDK or kit de desenvolvimento de software. Por isso, recomendamos fortemente que você teste seus canais de notificação no dashboard da Braze durante o desenvolvimento.

Para entender melhor o comportamento esperado dos canais, consulte a tabela a seguir:

| Cenário | Resultado |
| ---|-------------
| **Empresa ABC** atualiza para um SDK or kit de desenvolvimento de software que suporta Android O<br>**Empresa ABC** não adiciona nenhum canal ao dashboard da Braze<br>**Empresa ABC** não renomeia o canal padrão do SDK or kit de desenvolvimento de software | As notificações por push enviadas para dispositivos Android O criarão um canal chamado "General" e as notificações serão enviadas pelo canal "General"
| **Empresa XYZ** atualiza para um SDK or kit de desenvolvimento de software que suporta Android O<br>**Empresa XYZ** não adiciona nenhum canal ao dashboard da Braze<br>**Empresa XYZ** renomeia o canal padrão do SDK or kit de desenvolvimento de software para "Marketing" | As notificações por push enviadas para dispositivos Android O criarão um canal chamado "Marketing" e as notificações serão enviadas pelo canal "Marketing"
| **Empresa LMN** atualiza para um SDK or kit de desenvolvimento de software que suporta Android O<br>**Empresa LMN** define dois canais no código da aplicação, "Promotions" e "Order Updates"<br>**Empresa LMN** adiciona os IDs de canal para "Promotions" e "Order Updates" ao dashboard da Braze<br>**Empresa LMN** designa "Promotions" como o canal de fallback do dashboard<br>**Empresa LMN** renomeia o canal padrão do SDK or kit de desenvolvimento de software para "Marketing" | As notificações por push enviadas para dispositivos Android O não criarão um canal<br><br>A menos que o profissional de marketing especifique explicitamente que as notificações devem ser enviadas pelo canal "Order Updates" ou "Marketing", todas as notificações criadas antes dos canais serem adicionados ao dashboard serão enviadas pelo canal "Promotions"<br><br>O canal padrão do SDK or kit de desenvolvimento de software, "Marketing", só será criado e utilizado se a empresa tentar enviar uma notificação por um ID de canal inválido ou se for explicitamente selecionado
| **Empresa HIJ** atualiza para Android O, mas não atualiza para o SDK or kit de desenvolvimento de software Android da Braze versão 2.1.0 ou posterior | As notificações enviadas para usuários com Android O ou posterior não aparecem |
