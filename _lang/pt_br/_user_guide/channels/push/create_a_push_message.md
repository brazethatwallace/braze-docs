...
Para saber mais sobre as opções de notificação específicas de cada plataforma, consulte [Opções de notificação para iOS]({{site.baseurl}}/user_guide/channels/push/platform_specific_resources/ios/notification_options) ou [Opções de notificação para Android]({{site.baseurl}}/user_guide/channels/push/platform_specific_resources/android/notification_options).
...
Quando compatível, o ícone do seu app é adicionado automaticamente como a imagem da sua notificação por push. Você também tem a opção de enviar notificações Rich, que permitem mais personalização nas suas notificações por push ao adicionar conteúdo adicional além do texto.
...
O dispositivo usado mais recentemente é determinado pelo dispositivo que possui o token por push atualizado mais recentemente, e não pelo dispositivo que teve a sessão mais recente.
...
Múltiplos envios ainda podem ocorrer se uma Campaign direcionar plataformas diferentes, como iOS e Android. Se um usuário tiver o app em ambas, ele poderá receber um push para ambas as plataformas.
...
Testar é, sem dúvida, uma das etapas mais importantes. Depois de finalizar sua mensagem push perfeita, teste-a antes de enviá-la. Selecione a guia **Teste** para escolher entre as opções de como testar sua mensagem push. Em **Destinatários de teste**, você pode selecionar um grupo de teste de conteúdo ou usuários individuais. Você também pode usar **Pré-visualizar mensagem como usuário** para ter uma ideia de como sua mensagem pode ser exibida no celular para um usuário aleatório, usuário existente, usuário personalizado ou usuário multilíngue.
...
Construa o restante da sua Campaign; consulte as seções a seguir para mais detalhes sobre como usar nossas ferramentas para criar notificações por push.

### Escolha o cronograma de entrega ou o disparo {#choose-delivery-schedule-or-trigger}

As mensagens push podem ser entregues com base em um horário agendado, uma ação ou um disparo de API. Para saber mais, consulte [Agendando sua Campaign]({{site.baseurl}}/user_guide/messaging/campaigns/schedule_your_campaign).
...
Em seguida, você deve [direcionar os usuários]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/target_users) escolhendo segmentos ou filtros para refinar seu público. Você recebe automaticamente uma prévia de como é a população aproximada desse segmento. Estatísticas detalhadas do público para os canais direcionados pela sua Campaign estão disponíveis no rodapé. Para ver qual porcentagem da sua base de usuários está sendo direcionada e o valor do tempo de vida para esse segmento, selecione **Show Additional Stats**.

{% multi_lang_include audience/target_audiences.md %}

{% details Por que minha métrica de Total de Usuários Contatáveis não corresponde à soma de todos os canais? %}

Ao visualizar o Total de Usuários Contatáveis para seu público filtrado, você pode notar que a soma das colunas individuais é menor do que o Total de Usuários Contatáveis. Essa diferença geralmente ocorre porque há vários usuários que se qualificam para o segmento ou filtros da Campaign, mas não são contatáveis por push (por exemplo, porque não possuem [tokens por push]({{site.baseurl}}/user_guide/channels/push/push_setup/push_token_lifecycle#push-tokens) válidos ou ativos).
...
Lembre-se de que a composição exata do segmento é sempre calculada antes do envio da mensagem.
...
Opcionalmente, você também pode limitar a entrega a um número específico de usuários dentro do segmento, ou permitir que os usuários recebam a mesma mensagem duas vezes em uma recorrência da Campaign.

#### Campaigns multicanal com e-mail e push {#multichannel-campaigns-with-email-and-push}

Para Campaigns multicanal direcionadas tanto para canais de e-mail quanto de push, você pode querer limitar sua Campaign para que apenas os usuários que aceitaram explicitamente recebam a mensagem (excluindo usuários inscritos ou que cancelaram a inscrição). Por exemplo, digamos que você tenha três usuários com diferentes status de aceitação:

{% multi_lang_include messaging/intelligent_channel_user_examples.md %}

Para fazer isso, em **Resumo do Público**, selecione enviar esta Campaign para "somente usuários que aceitaram". Essa opção garantirá que apenas os usuários que aceitaram receberão seu e-mail, e a Braze enviará push apenas para os usuários que estão habilitados para push por padrão.
