...
Quando um usuário final insere o endereço de e-mail neste formulário, a Braze adiciona o endereço de e-mail ao perfil de usuário.
...
- Se um endereço de e-mail já existir no perfil de usuário, o endereço de e-mail recém-inserido substituirá o endereço de e-mail existente.
- Se o usuário conhecido tiver um endereço de e-mail sinalizado como [hard bounce]({{site.baseurl}}/user_guide/channels/email/reporting/analytics_glossary#hard-bounce), a Braze verifica se o endereço de e-mail recém-inserido é diferente do endereço no perfil na Braze. Se o endereço de e-mail fornecido for diferente, a Braze atualiza o endereço de e-mail e remove o status de hard bounce.
- Se um usuário inserir um endereço de e-mail inválido, ele verá a mensagem de erro: "Please enter a valid email."
...
O formulário de captura de e-mail define o endereço de e-mail no perfil de usuário atualmente ativo na Braze. O comportamento varia conforme o usuário esteja identificado (logado, `changeUser` chamado) ou não.

Se um usuário anônimo inserir o e-mail no formulário e enviá-lo, a Braze adiciona o endereço de e-mail ao perfil dele. Se `changeUser` for chamado posteriormente durante a jornada web e um novo `external_id` for atribuído (por exemplo, quando um novo usuário se registra no serviço), todos os dados do perfil de usuário anônimo são mesclados, incluindo o endereço de e-mail.

Se `changeUser` for chamado com um `external_id` existente, o perfil de usuário anônimo é descartado e os [campos de dados específicos do perfil de usuário]({{site.baseurl}}/api/endpoints/user_data/post_users_merge#merge-behavior) que ainda não existem no usuário identificado são mesclados, mas os campos que já existem são perdidos, incluindo o endereço de e-mail.
...
## Etapa 1: Criar uma campanha de mensagem no app {#step-1-create-an-in-app-message-campaign}
...
Se você está usando uma mensagem no app para capturar e-mails de usuários, pode ser interessante limitar o público a usuários que ainda não forneceram essa informação.

- **Para direcionar usuários sem endereço de e-mail:** Use o filtro `Email Available` é `false`. Isso faz com que o formulário apareça apenas para usuários que não têm um e-mail cadastrado, ajudando a evitar solicitações redundantes para usuários já conhecidos.
...
Você também pode combinar os dois filtros usando a lógica `AND`, se desejar. Isso faz com que o formulário apareça apenas para usuários que não possuem endereço de e-mail nem ID de usuário externo — ideal para capturar novos leads ou solicitar a criação de conta.

## Etapa 4: Direcione os usuários que preencheram o formulário (opcional) {#step-4-target-users-who-filled-out-the-form-optional}
