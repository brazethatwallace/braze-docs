...
description: "Este artigo de referência aborda os requisitos de mensagem e design das mensagens no app em tela cheia."
page_type: reference
page_order: 1
channel:
  - in-app messages
...
# Mensagens no app em tela cheia {#fullscreen-in-app-messages}

> As mensagens em tela cheia ocupam toda a tela do dispositivo! Esse tipo de mensagem é ideal quando você realmente precisa da atenção do usuário, como para atualizações obrigatórias do app.
...
![Duas mensagens no app em tela cheia lado a lado na orientação retrato, detalhando as recomendações de imagem e texto. Consulte as seções a seguir para mais informações.]({% image_buster /assets/img/full-screen-spec.png %}){: style="max-width:801px;border:none;display:block;margin-left:auto;margin-right:auto"}

{% endtab %}
{% tab Paisagem %}

![Duas mensagens no app em tela cheia lado a lado na orientação paisagem, detalhando as recomendações de imagem e texto. Consulte as seções a seguir para mais informações.]({% image_buster /assets/img/full-screen-spec-landscape.png %}){: style="max-width:801px;border:none;display:block;margin-left:auto;margin-right:auto"}
...
As mensagens no app em tela cheia preenchem toda a altura do dispositivo e cortam horizontalmente (lados esquerdo e direito) conforme necessário. Mensagens em tela cheia com imagem e texto preenchem 50% da altura do dispositivo. Todas as mensagens no app em tela cheia preenchem a barra de status em dispositivos com "notch".
...
Em um tablet ou navegador de desktop, uma mensagem no app em tela cheia ficará centralizada na tela do app, conforme mostrado na captura de tela a seguir.

{% tabs %}
{% tab Retrato %}

![Mensagem no app em tela cheia como apareceria em uma tela grande na orientação retrato. A mensagem aparece como um modal grande centralizado na tela.]({% image_buster /assets/img/full-screen-large-viewport.png %}){: style="border:none;display:block;margin-left:auto;margin-right:auto"}

{% endtab %}
{% tab Paisagem %}

![Mensagem no app em tela cheia como apareceria em uma tela grande na orientação paisagem. A mensagem aparece como um modal grande centralizado na tela.]({% image_buster /assets/img/full-screen-large-viewport-landscape.png %}){: style="max-width:80%;border:none;display:block;margin-left:auto;margin-right:auto"}
