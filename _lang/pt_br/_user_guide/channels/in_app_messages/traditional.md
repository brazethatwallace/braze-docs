...
Comece escolhendo quais plataformas devem receber a mensagem. Use essa seleção para limitar a entrega de uma campanha a um conjunto específico de apps. Por exemplo, você pode escolher **Web Browsers** para uma mensagem no navegador incentivando os usuários a baixar seu app mobile, garantindo que eles não recebam a mensagem depois de já terem baixado o app. Como as seleções de plataforma são específicas para cada variante, você pode tentar testar o engajamento com a mensagem por plataforma.
...
Depois de selecionar uma plataforma de envio, navegue pelos tipos de mensagem, layouts e outras opções associadas a ela. Saiba mais sobre o comportamento esperado e a aparência de cada uma dessas mensagens na página [Tipos de mensagem]({{site.baseurl}}/user_guide/channels/in_app_messages/message_types), ou clicando nos tipos de mensagem vinculados nas tabelas a seguir.
...
- As mensagens **modal** ficam no meio-termo — visíveis o suficiente para chamar a atenção sem tomar conta da tela inteira.
- As mensagens em **tela cheia** são as que mais chamam a atenção e são ideais para anúncios críticos ou promoções.

Quanto mais complexo for o seu conteúdo, mais espaço você vai precisar — e mais provável é que sua mensagem interrompa o fluxo do usuário.
...
<tbody>
  <tr>
    ...
    <td>Grande e chamativa! Use quando quiser garantir que os usuários vejam seu conteúdo, como suas Campaigns mais importantes, notificações essenciais ou grandes promoções.<br><br>Note que, em dispositivos móveis, mensagens em retrato e paisagem não serão exibidas se a orientação do dispositivo não corresponder à orientação da mensagem.</td>
  </tr>
  <tr>
    <td><a href='/docs/user_guide/channels/in_app_messages/message_types/modal'>Modal</a></td>
    <td>Mensagens que cobrem a tela inteira com uma sobreposição de tela e um bloco de mensagem.</td>
    <td>
      <ul>
    ...
    <td>N/D</td>
    <td>Um bom meio-termo. Use quando precisar de uma forma evidente de captar a atenção do usuário, como incentivar os usuários a experimentar um novo recurso ou aproveitar uma promoção.</td>
  </tr>
  <tr>
...
<tbody>
  <tr>
    ...
    <td>Mensagens personalizadas que funcionam conforme definido no seu código customizado (HTML, CSS e/ou JavaScript).</td>
    <td>N/D</td>
  ...
  <tr>
    ...
    <td>N/D</td>
    <td>É necessário definir a opção de inicialização <span style="white-space: nowrap"><code>allowUserSuppliedJavascript</code></span> como <code>true</code> para que sua mensagem no app funcione.</td>
    <td>Quando você deseja solicitar que os usuários enviem seu endereço de e-mail.</td>
  </tr>
  <tr>
    ...
    <td>Quando você quer fazer upload ou escrever CSS personalizado para criar mensagens com estilo totalmente customizado e visualmente atraentes.</td>
  </tr>
</tbody>
</table>
...
A guia **Criação** permite editar todos os aspectos do conteúdo e do comportamento da sua mensagem.
...
O conteúdo da guia **Criação** varia de acordo com as opções de mensagem escolhidas na etapa anterior, mas pode incluir qualquer uma das seguintes opções:

### Idioma {#language}

Selecione **Adicionar idiomas** e escolha os idiomas desejados na lista disponível. Isso inserirá [Liquid]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/liquid/conditional_logic#conditional-logic) na sua mensagem. Recomendamos selecionar os idiomas antes de escrever o conteúdo para que você possa preencher o texto nos locais corretos do Liquid. Consulte nossa [lista completa de idiomas disponíveis]({{site.baseurl}}/developer_guide/localization?tab=android).

### Imagem {#image}

Dependendo do tipo de mensagem, você pode **Fazer upload de imagem**, **Escolher um badge** ou usar **Font Awesome**. Para fazer upload de uma imagem, selecione **Adicionar imagem** ou forneça um URL de imagem. Ao selecionar **Adicionar imagem**, a **Biblioteca de mídia** será aberta, onde você pode escolher uma imagem já carregada ou adicionar uma nova. Cada tipo de mensagem e plataforma pode ter suas próprias proporções e requisitos sugeridos — verifique quais são antes de criar ou produzir uma imagem do zero.
...
![Botão para iniciar o Assistente de Copywriting com IA, localizado no campo de mensagem do criador de mensagens no app.]({% image_buster /assets/img/ai_copywriter/ai_copywriter_iam.png %}){: style="max-width:60%"}
...
Quando disponível para o seu tipo de mensagem, você pode exibir até dois botões abaixo do corpo do texto. É possível criar e editar texto e cor personalizados para os botões. Você também pode adicionar um link de Termos de Serviço em formulários de captura de e-mail.

Se optar por usar apenas um botão, ele se ajustará automaticamente para ocupar o espaço disponível na parte inferior da mensagem, em vez de deixar espaço para um botão adicional.

#### Escolhendo um botão principal {#choosing-a-primary-button}

Se decidir formatar esses botões com suas próprias cores, recomendamos que use o Botão 2 para o resultado que você mais deseja.

Em outras palavras, se quiser que o usuário clique em um botão mais do que no outro, certifique-se de que ele seja o botão secundário. O botão secundário frequentemente demonstra maior potencial de receber cliques, especialmente se tiver uma cor que contraste ou se destaque do restante da mensagem. Isso fica ainda mais evidente quando o botão principal se mistura visualmente com a mensagem.
...
| Solicitar permissão de push | Exibe a solicitação nativa de permissão de push. Leia mais sobre [push priming]({{site.baseurl}}/user_guide/channels/push/best_practices/push_primer_messages), assim como [práticas recomendadas]({{site.baseurl}}/user_guide/channels/push/push_setup/push_subscription_states#best-practices) para preparar os usuários para push. |
{: .reset-td-br-1 .reset-td-br-2 aria-label="Comportamento ao clicar #button-actions" }

Nota: as opções __Solicitar permissão de push__, __Registrar evento personalizado__ e __Registrar atributo personalizado__ requerem as seguintes versões mínimas do SDK:

{% sdk_min_versions swift:5.4.0 android:21.0.0 web:4.0.3 %}

Para combinar várias ações ou executar ações adicionais do SDK não disponíveis no dashboard (como adicionar a um grupo de inscrições ou definir um tipo de inscrição de e-mail), você pode usar [deep links de Braze Actions]({{site.baseurl}}/developer_guide/braze_actions).

### Opções para dispositivos iOS {#ios-device-options}

Se desejar, você pode restringir sua mensagem no app para enviar apenas a dispositivos iOS. Para isso, clique em **Alterar** e selecione **Enviar apenas para dispositivos iOS**.
...
- **Aguardar deslizar ou toque do usuário:** Requer uma opção de dispensar ou fechar.
...
Essa configuração se aplica apenas ao tipo de mensagem com código personalizado. Copie e cole o HTML no espaço disponível e faça upload dos seus ativos usando um [arquivo ZIP]({{site.baseurl}}/user_guide/messaging/design_and_edit/media_library#zip-file-uploads).

### Texto de placeholder da captura de e-mail {#email-capture-input-placeholder}

Essa configuração se aplica apenas ao tipo de mensagem de formulário de captura de e-mail. Insira um texto personalizado que aparecerá como placeholder no campo de entrada de e-mail. O padrão é "Insira seu endereço de e-mail".
...
1. No criador de mensagem, selecione a guia **Configurações**.
2. Em **Pares chave-valor**, selecione **Adicionar novo par**.
3. Insira uma chave e um valor para cada par. Para adicionar outro par, selecione **Adicionar novo par** novamente.
...
### Escolha um disparo {#choose-a-trigger}
...
{% alert important %}
Se você pretende disparar sua mensagem no app com base em um evento personalizado, esse evento personalizado precisa ser enviado usando o SDK.
{% endalert %}
...
A entrega de mensagens no app é inteiramente baseada nos seguintes disparos de ação:
...
- As mensagens também podem ser configuradas para se repetir diariamente, semanalmente (opcionalmente em dias específicos) ou mensalmente.
...
Consulte nossa documentação para desenvolvedores sobre [disparo de eventos no servidor]({{site.baseurl}}/developer_guide/in_app_messages/triggering_messages?tab=web) e [entrega local de mensagens no app]({{site.baseurl}}/developer_guide/in_app_messages/triggering_messages?tab=web).

#### Disparo online versus offline {#online-versus-offline-triggering}

As mensagens no app funcionam enviando a mensagem e os disparos para o dispositivo do usuário. Depois que as mensagens no app estão no dispositivo, elas aguardam para serem exibidas até que a condição de disparo seja atendida. Se as mensagens no app já estiverem armazenadas em cache no dispositivo do usuário, você pode até disparar mensagens no app offline, sem conexão com a Braze (por exemplo, no modo avião).
...
As opções de alta, média e baixa prioridade para mensagens disparadas são agrupadas em categorias, e várias mensagens podem ter a mesma prioridade selecionada. Quando várias mensagens compartilham a mesma prioridade, a mensagem criada ou atribuída mais recentemente tem precedência e é exibida primeiro:

- **Categoria de prioridade padrão:** Quando duas campanhas compartilham o mesmo disparo e usam a prioridade padrão (média), a campanha criada por último recebe o disparo.
- **Categoria de prioridade específica:** Quando várias campanhas compartilham o mesmo disparo e são atribuídas a uma categoria de prioridade específica, a campanha atribuída mais recentemente a essa categoria recebe o disparo.
...
Em seguida, você deve [direcionar os usuários]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/target_users) escolhendo segmentos ou filtros para refinar seu público. Você recebe automaticamente um panorama de como é a população aproximada desse segmento. Tenha em mente que a participação exata no segmento é sempre calculada antes de a mensagem ser enviada.

{% alert note %}
Se houver uma postergação na etapa da mensagem no app, a participação no segmento será avaliada após a postergação. Se o usuário for elegível, a mensagem no app será sincronizada na próxima sessão disponível.
{% endalert %}

#### Reavaliar a elegibilidade da campanha e o Liquid {#re-evaluate-campaign-eligibility-and-liquid}

Em alguns cenários, você pode querer reavaliar a elegibilidade de um usuário quando ele dispara uma mensagem no app para exibição. Exemplos incluem campanhas que direcionam um atributo personalizado que muda com frequência ou mensagens que devem refletir quaisquer alterações de última hora no perfil.

![Caixa de seleção "Reavaliar a elegibilidade da campanha antes de exibir" selecionada.]({% image_buster /assets/img_archive/re-evaluate-iam-membership.png %}){:style="max-width:60%"}
...
Isso evita que mensagens no app sejam enviadas a usuários dentro de campanhas expiradas ou arquivadas. Se você não reavaliar a elegibilidade de um usuário, ele receberá a mensagem no app mesmo após a campanha ter expirado ou sido arquivada, porque a mensagem está no SDK e aguarda que os usuários a disparem.

{% alert note %}
Ativar essa opção resultará em uma pequena postergação (< 100ms) entre o momento em que um usuário dispara uma mensagem no app e o momento em que a mensagem é exibida, devido à solicitação adicional de elegibilidade e processamento de modelos.
<br><br>
...
Dados de usuário que o [endpoint `/users/track`]({{site.baseurl}}/api/endpoints/user_data/post_user_track) adiciona na mesma sessão podem, às vezes, ser usados na mensagem no app desse usuário. Por exemplo, se um usuário faz parte do público de uma mensagem no app que está aguardando um disparo, inicia uma sessão e, nessa mesma sessão, a REST API atualiza o perfil dele, esses novos dados podem aparecer na mensagem no app quando **Re-evaluate campaign eligibility before displaying** estiver selecionado. A Braze não processará o modelo da mensagem no app até que seja hora de renderizá-la.

Se um único disparo envia dados para a Braze e aciona a mensagem no app ao mesmo tempo, a mensagem não poderá usar esses dados de perfil recém-atualizados, mesmo com uma postergação agendada. Em vez disso, use dois disparos separados: um para enviar os dados e outro para disparar a mensagem no app.
...
Depois de terminar de criar sua Campaign ou Canvas, revise os detalhes, [teste]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/sending_test_messages?tab=in-app%20message) e envie!

Em seguida, confira o [Relatório de mensagens no app]({{site.baseurl}}/user_guide/channels/in_app_messages/reporting) para saber como acessar os resultados das suas campanhas de mensagens.

## Informações importantes {#things-to-know}

### Limites de campanhas de mensagem no app ativas {#active-in-app-message-campaign-limits}

A Braze valoriza a confiabilidade e a velocidade. Sugerimos que você envie apenas os dados necessários para a Braze e desative todas as campanhas que não agregam mais valor à sua marca.

O processamento de campanhas de mensagem no app baseadas em ação que ainda estão ativas, mas não estão mais enviando mensagens ou não são mais necessárias, reduz o desempenho geral dos serviços da Braze para você e para outros clientes. Esse tempo extra necessário para processar essas grandes quantidades de campanhas sem atividades significa que qualquer mensagem no app demorará mais para aparecer nos dispositivos do usuário final, o que impacta a experiência do usuário final.

{% alert important %}
Você pode ter até 200 campanhas de mensagem no app ativas e baseadas em ação por espaço de trabalho para otimizar a velocidade de entrega de mensagens e evitar timeouts. Isso não se aplica a Canvas.
{% endalert %}

A contagem de 200 inclui campanhas de mensagem no app ativas que ainda não atingiram o horário de término e aquelas que não têm horário de término. Campanhas de mensagem no app ativas que já passaram do horário de término não serão contabilizadas. O cliente médio da Braze tem um total de 26 campanhas ativas ao mesmo tempo, então é improvável que essa limitação afete você.

### Avaliação de entrega no horário local {#local-time-delivery-evaluation}

Quando uma campanha de mensagem no app é agendada usando o fuso local do usuário, a avaliação dos horários de início e término da campanha é feita no próprio dispositivo.

As campanhas de mensagem no app normalmente são enviadas ao dispositivo do usuário quando a sessão do app é iniciada ou atualizada. Nesse momento:

1. O SDK avalia se o usuário se qualifica para qualquer mensagem no app baseada em gatilho.
...
3. Se ambas as condições forem atendidas, a mensagem no app fica elegível para exibição.
...
- Assim como outros tipos de canal, as campanhas de mensagem no app devem idealmente ser lançadas com 24 a 48 horas de antecedência. Esse intervalo dá aos usuários tempo suficiente para atender aos critérios de elegibilidade e iniciar uma sessão para que a mensagem seja avaliada e exibida.