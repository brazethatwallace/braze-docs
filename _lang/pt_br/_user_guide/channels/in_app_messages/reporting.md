...
| Total de conversões | Quando um usuário visualiza uma Campaign de mensagem no app apenas uma vez, apenas uma conversão é contabilizada, mesmo que ele realize o evento de conversão várias vezes depois. No entanto, se a reelegibilidade estiver ativada e o usuário vir a Campaign de mensagem no app várias vezes, o *Total de conversões* pode aumentar uma vez para cada vez que o usuário registrar uma impressão de uma nova instância da Campaign de mensagem no app. <br><br> Por exemplo, se um usuário acionar uma mensagem no app duas vezes e converter após cada impressão (resultando em duas conversões), o *Total de conversões* aumenta em dois. No entanto, se houve apenas uma impressão seguida de dois eventos de conversão, apenas uma conversão é registrada e o *Total de conversões* aumenta em um. |
...
{% alert note %}
Em testes A/B, as *Impressões únicas* do grupo de controle podem exceder as *Impressões únicas* da variante, e o *Total de impressões* do controle pode exceder o *Total de impressões* da variante, quando as mensagens da variante exigem tempo de renderização (por exemplo, imagens grandes ou Conteúdo conectado com templates). Usuários que acionam a mensagem mas saem antes que a variante termine de renderizar podem não registrar impressões da variante, enquanto o grupo de controle registra impressões sem renderizar uma mensagem.
{% endalert %}
