...
| Compras e receita por Segment | Este relatório mostra métricas das mensagens enviadas para um Segment específico. <br><br> As métricas de compra são únicas ao longo do período do relatório. Um usuário pode gerar no máximo uma compra. A receita leva em conta todas as compras do período do relatório. |
| Compras e receita por variantes ou etapas, por Segment | Este relatório mostra métricas das variantes ou etapas do Canvas das mensagens enviadas para cada Segment. <br><br> As métricas de compra são únicas ao longo do período do relatório. Um usuário pode gerar no máximo uma compra. A receita leva em conta todas as compras do período do relatório. |
...
| Métricas de engajamento de e-mail por Segment | Este relatório mostra métricas das mensagens enviadas para cada Segment, como quantos e-mails foram enviados, entregues, tiveram soft bounce e hard bounce. <br><br> Todas as métricas são únicas ao longo do período do relatório. Por exemplo, se um e-mail de boas-vindas teve soft bounce uma vez em 21 de novembro, duas vezes em 22 de novembro e nunca foi entregue: {::nomarkdown} <ul><li> A métrica de <i>Soft Bounces</i> de 21 de novembro aumenta em um. </li><li> A métrica de <i>Soft Bounces</i> de 22 de novembro não é afetada.</li></ul>{:/} ![Captura de tela relacionada aos modelos de e-mail.]({% image_buster /assets/img_archive/email_engagement_segment.png %}) |
| Métricas de engajamento de e-mail por variantes ou etapas, por Segment | Este relatório mostra métricas das variantes ou etapas do Canvas das mensagens enviadas para cada Segment. Essas métricas incluem quantos e-mails foram enviados, entregues, tiveram soft bounce e hard bounce. <br><br> Todas as métricas são únicas ao longo do período do relatório. Por exemplo, se um e-mail de boas-vindas teve soft bounce uma vez em 21 de novembro, duas vezes em 22 de novembro e nunca foi entregue: {::nomarkdown} <ul><li> A métrica de <i>Soft Bounces</i> de 21 de novembro aumenta em um. </li> <li> A métrica de <i>Soft Bounces</i> de 22 de novembro não é afetada.</li></ul> {:/} |
...
## Detalhamento por Segment {#segment-breakdown}

| Nome da consulta | Descrição |
| -- | -- |
| Métricas de engajamento de e-mail por Segment | Este relatório mostra métricas de desempenho de e-mail detalhadas por Segment no nível de Campaign ou Canvas. |
| Compras e receita por Segment | Este relatório mostra métricas de compra e receita detalhadas por Segment para uma Campaign ou Canvas específico. |
...
| Desempenho de push por Segment | Este relatório mostra métricas de push detalhadas por Segment. |
{: .reset-td-br-1 .reset-td-br-2 aria-label="Detalhamento por Segment" }