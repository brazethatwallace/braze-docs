---
nav_title: Relatórios
article_title: Relatórios
page_order: 2
layout: dev_guide
guide_top_header: "Relatórios"
guide_top_text: "A Braze oferece diversas opções de relatórios para ajudar você a medir o desempenho de Campaigns, acompanhar o engajamento dos usuários e tomar decisões baseadas em dados. Não sabe por onde começar? Confira a <a href='#choosing-a-report'>tabela comparativa de relatórios</a> para encontrar o relatório ideal para seus objetivos."

page_type: landing
description: "Explore as opções de relatórios da Braze, incluindo análise de dados de Campaigns, o Criador de relatórios, o Criador de consultas, relatórios de engajamento e muito mais."
tool: Reports
search_rank: 2
guide_featured_title: "Artigos da seção"
guide_featured_list:
  - name: Análise de dados de Campaigns
    link: /docs/user_guide/analytics/reports/campaign_analytics
    image: /assets/img/braze_icons/bar-chart-01.svg
  - name: Criador de relatórios
    link: /docs/user_guide/analytics/reports/report_builder
    image: /assets/img/braze_icons/tool-01.svg
  - name: Criador de consultas
    link: /docs/user_guide/analytics/reports/query_builder
    image: /assets/img/braze_icons/code-02.svg
  - name: Relatórios de engajamento
    link: /docs/user_guide/analytics/reports/engagement_reports
    image: /assets/img/braze_icons/line-chart-up-01.svg
  - name: Relatório de receitas
    link: /docs/user_guide/analytics/reports/revenue_report
    image: /assets/img/braze_icons/piggy-bank-02.svg
  - name: Relatório de eventos personalizados
    link: /docs/user_guide/analytics/reports/custom_events_report
    image: /assets/img/braze_icons/calendar-check-02.svg

guide_menu_title: "Mais artigos"
guide_menu_list:
  - name: Análise de dados de Canvas
    link: /docs/user_guide/analytics/reports/canvas_analytics
    image: /assets/img/braze_icons/line-chart-down-01.svg
  - name: Relatórios de funil
    link: /docs/user_guide/analytics/reports/funnel_reports
    image: /assets/img/braze_icons/flag-02.svg
  - name: Relatórios de retenção
    link: /docs/user_guide/analytics/reports/retention_reports
    image: /assets/img/braze_icons/user-check-01.svg
  - name: Configurar relatórios
    link: /docs/user_guide/analytics/reports/configure_reporting
    image: /assets/img/braze_icons/settings-01.svg
---

## Escolhendo um relatório {#choosing-a-report}

| Relatório | Ideal para | Descrição |
| --- | --- | --- |
| [Análise de dados de Campaigns]({{site.baseurl}}/user_guide/analytics/reports/campaign_analytics/) | Resultados de Campaigns por canal | Visualize resultados em tempo real de cada Campaign, detalhados por canal de envio de mensagens. |
| [Análise de dados de Canvas]({{site.baseurl}}/user_guide/analytics/reports/canvas_analytics/) | Desempenho de Canvas | Visualize estatísticas gerais, desempenho de variantes e métricas por etapa de um Canvas. |
| [Criador de relatórios]({{site.baseurl}}/user_guide/analytics/reports/report_builder/) | Comparação entre Campaigns | Compare resultados de várias Campaigns ou Canvas em uma única visualização, com métricas personalizáveis e detalhamentos. |
| [Criador de consultas]({{site.baseurl}}/user_guide/analytics/reports/query_builder/) | Análise personalizada com SQL | Escreva consultas de SQL personalizadas nos seus dados da Braze no Snowflake ou use modelos prontos para análises comuns. |
| [Relatórios de engajamento]({{site.baseurl}}/user_guide/analytics/reports/engagement_reports/) | Exportações de e-mail agendadas | Configure exportações recorrentes em CSV com estatísticas de engajamento para Campaigns e Canvas selecionados. |
| [Relatório de eventos personalizados]({{site.baseurl}}/user_guide/analytics/reports/custom_events_report/) | Tendências de eventos personalizados | Monitore a frequência de eventos personalizados ao longo do tempo, detalhados por Segment ou normalizados por fórmulas de KPI. |
| [Relatório de receitas]({{site.baseurl}}/user_guide/analytics/reports/revenue_report/) | Receitas e compras | Acompanhe receitas, compras e detalhamentos por produto ao longo do tempo, com filtros opcionais por Segment. |
| [Relatórios de funil]({{site.baseurl}}/user_guide/analytics/reports/funnel_reports/) | Análise de funil de conversão | Analise as jornadas que seus clientes percorrem após receberem uma Campaign ou Canvas, incluindo pontos de abandono. |
| [Relatórios de retenção]({{site.baseurl}}/user_guide/analytics/reports/retention_reports/) | Impacto na retenção a longo prazo | Meça a eficácia de uma Campaign ou Canvas em reengajar usuários ao longo do tempo (até 30 dias). |
| [Configurar relatórios]({{site.baseurl}}/user_guide/analytics/reports/configure_reporting/) | Resumo semanal por e-mail | Assine um e-mail semanal de análise de dados e escolha quais eventos personalizados incluir. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 role="presentation" }