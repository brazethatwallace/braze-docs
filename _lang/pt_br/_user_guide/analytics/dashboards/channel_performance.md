...
| Taxa de cancelamento de inscrição | Taxa | (Número total de cancelamentos de inscrição únicos em cada dia do período) / (Número total de entregas no período)<br><br>Utiliza cancelamentos de inscrição únicos, que também são usados na análise de dados de Campaigns, visão geral e Report Builder. Esses cancelamentos de inscrição são registrados em todas as fontes (como a REST or transferir estado representacional API or interface de programação do aplicativo (API), importações de CSV, e-mails e cancelamentos de inscrição por lista). As taxas de cancelamento de inscrição na análise de dados de Campaigns e Canvas são cancelamentos que ocorrem como resultado de um clique em cancelar inscrição em um e-mail entregue pela Braze. |
