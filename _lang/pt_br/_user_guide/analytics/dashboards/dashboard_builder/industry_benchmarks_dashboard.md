...
| Gráfico de tendência mensal | Plota a taxa do seu espaço de trabalho em relação à taxa do setor ao longo do tempo, para que você possa identificar sazonalidades e tendências de longo prazo.                                   |
...
| _Distinto_ | Conta usuários únicos. | Se um usuário clica três vezes, isso é contado como um clique. |
...
<tr><td class="no-split"><i>Taxa de abertura única</i></td><td class="no-split">{% multi_lang_include analytics/metrics.md metric='Unique Opens' %} Essa taxa exclui aberturas por máquina.</td><td class="no-split"><i>Unique Opens</i> / <i>Unique Sends</i></td></tr>
<tr><td class="no-split"><i>Taxa de cliques únicos</i></td><td class="no-split">{% multi_lang_include analytics/metrics.md metric='Unique Clicks' %}</td><td class="no-split"><i>Unique Clicks</i> / <i>Unique Sends</i></td></tr>
<tr><td class="no-split"><i>Taxa de cliques por abertura única</i></td><td class="no-split">A porcentagem de usuários que clicaram em um e-mail após abri-lo.</td><td class="no-split"><i>Unique Clicks</i> / <i>Unique Opens</i></td></tr>
</tbody></table>
...
As métricas de push estão disponíveis para iOS, Android, web e em todas as plataformas combinadas.

<table aria-label="Métricas de push"><thead><tr><th>Métrica</th><th>Descrição</th><th>Fórmula</th></tr></thead><tbody>
<tr><td class="no-split"><i>Taxa de Aberturas Diretas</i></td><td class="no-split">{% multi_lang_include analytics/metrics.md metric='Direct Opens' %}</td><td class="no-split"><i>Direct Opens</i> / <i>Unique Sends</i></td></tr>
<tr><td class="no-split"><i>Taxa de Aberturas por Influência</i></td><td class="no-split">{% multi_lang_include analytics/metrics.md metric='Influenced Opens' %}</td><td class="no-split"><i>Influenced Opens</i> / <i>Unique Sends</i></td></tr>
<tr><td class="no-split"><i>Taxa de abertura total</i></td><td class="no-split">{% multi_lang_include analytics/metrics.md metric='Opens' %}</td><td class="no-split">(<i>Direct Opens</i> + <i>Influenced Opens</i>) / <i>Unique Sends</i></td></tr>
</tbody></table>
...
<tr><td class="no-split"><i>Taxa de entrega</i></td><td class="no-split">{% multi_lang_include analytics/metrics.md metric='Deliveries' %}</td><td class="no-split"><i>Deliveries</i> / <i>Unique Sends</i></td></tr>
<tr><td class="no-split"><i>Taxa de cliques em links curtos</i></td><td class="no-split">A porcentagem de usuários que clicaram em um link curto após receber um SMS.</td><td class="no-split"><i>Short Link Clicks</i> / <i>Unique Sends</i></td></tr>
</tbody></table>
...
<tr><td class="no-split"><i>Taxa de cliques</i></td><td class="no-split">A porcentagem de usuários que receberam um Content Card e clicaram em um link.</td><td class="no-split"><i>Unique Clicks</i> / <i>Unique Impressions</i></td></tr>
</tbody></table>
...
Em vez de analisar cada ponto de dados, a Braze seleciona uma amostra representativa. O método de amostragem sobre-representa grupos menores de usuários para garantir uma representação adequada e faz ajustes pelo tamanho da empresa, de modo que um pequeno número de empresas muito grandes não distorça os resultados de um setor inteiro.
...
A amostra é ponderada para refletir a população do mundo real. Pesos são aplicados a subgrupos para corrigir quaisquer desequilíbrios remanescentes da amostragem, resultando em benchmarks finais que são representativos e imparciais.

## Governança de dados {#data-governance}

- **Ciclo de atualização:** Os dados são atualizados mensalmente no dia 5 de cada mês e refletem até o último mês completo.
