...
Se o Canvas de destino usar um cronograma de entrada agendado, a etapa Enviar para Destino ignora esse cronograma de entrada. Ela também ignora o [**Limit entrance volume**]({{site.baseurl}}/user_guide/messaging/canvas/create_a_canvas#selecting-entry-controls) em **Entry Controls** no Canvas de destino quando estiver configurado como **Every time Canvas is agendar/cronograma**. Os usuários enviados por essa etapa não aguardam a próxima janela de avaliação agendada — eles são avaliados em relação aos critérios de público do Canvas de destino e inseridos imediatamente quando alcançam a etapa Enviar para Destino.
