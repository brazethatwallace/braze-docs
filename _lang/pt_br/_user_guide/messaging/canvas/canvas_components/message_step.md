...
### Etapa 2: Edite as configurações de entrega
...
As validações de entrega fornecem uma verificação adicional para confirmar que seu público atende aos critérios de entrega no momento do envio da mensagem. Essa configuração é recomendada se o horário de silêncio, o Intelligent Timing ou o limite de taxa estiverem ativados.

Selecione **Validate audience at message send** e adicione um segmento faturável ou filtros adicionais para validar quando a mensagem for enviada. Se um usuário não atender às validações de entrega definidas para uma etapa de Mensagem, escolha se ele sai do Canvas ou avança para a próxima etapa.
...
## Referenciar propriedades de contexto
...
As propriedades de evento referem-se às propriedades que você define para eventos personalizados e eventos de compra. Essas propriedades de evento podem ser usadas em campanhas com entrega baseada em ação, bem como em Canvas.
...
| _Evento de conversão primária_ | O número de vezes que um evento definido ocorreu após interagir com ou visualizar uma mensagem recebida de uma campanha da Braze. Você define esse evento ao criar a campanha. |
