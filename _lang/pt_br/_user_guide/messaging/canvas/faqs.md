...
## Criando e editando Canvas {#building-and-editing-canvas}
...
Esse erro aparece quando a combinação de ramificações de etapas e tamanho do público de entrada pode criar problemas de desempenho no cluster que impedem o envio de mensagens. Para etapas de resolução — incluindo o uso de [jornadas do público]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/audience_paths), redução de ramificações ou do tamanho do público e reconstrução no Canvas Flow — consulte [Erro "Too many Canvas branches"]({{site.baseurl}}/user_guide/messaging/canvas/troubleshooting#too-many-canvas-branches-error).

### Posso usar Optimize with BrazeAI<sup>TM</sup> com reelegibilidade em um Canvas? {#can-i-use-optimize-with-brazeai-with-re-eligibility-in-a-canvas}

Sim. Os Canvas podem usar [Optimize with BrazeAI<sup>TM</sup>]({{site.baseurl}}/user_guide/messaging/canvas/create_a_canvas#optimize-canvas-variants-with-brazeai) quando a reelegibilidade está ativada. A Braze não pode garantir a mesma variante na reentrada porque a alocação muda ao longo do tempo. As Campaigns exigem uma janela de reelegibilidade de 24 horas ou mais quando **Optimize with BrazeAI<sup>TM</sup>** está ativado.
...
### Para onde os usuários vão quando alcançam uma etapa desconectada? {#where-do-users-go-when-theyve-reached-a-disconnected-step}

Se um usuário estiver em uma etapa desconectada do fluxo do seu Canvas, ele avançará para a etapa seguinte, se houver uma, e a configuração da etapa determinará como o usuário deve avançar. Isso foi projetado para permitir que os usuários façam alterações nas etapas sem precisar conectá-las diretamente ao restante do Canvas. Isso também dá margem para testes antes de entrar em produção imediatamente, permitindo efetivamente salvar um rascunho.
...
Se você editar algumas das etapas de um Canvas com várias etapas, os usuários que já estavam no público, mas ainda não receberam as etapas, receberão a versão atualizada da mensagem. Isso só acontecerá se eles ainda não tiverem sido avaliados para a etapa.
...
### Devo criar um Canvas ou Canvas separados por ciclo de vida do usuário? {#should-i-build-one-canvas-or-separate-canvases-per-user-lifecycle}

Dependendo do que você deseja alcançar com seu Canvas, pode ser necessário adotar abordagens diferentes na construção da jornada do usuário. A flexibilidade do Canvas permite mapear jornadas de usuários para qualquer estágio do ciclo de vida do usuário. Confira nossos [modelos de Braze Canvas]({{site.baseurl}}/user_guide/messaging/templates/canvas_templates/braze_templates) para vários exemplos de abordagens simplificadas para criar jornadas de usuário eficazes.

## Mensagens e entrega {#messages-and-delivery}

### Quando as mensagens no app em Canvas são enviadas? {#when-are-in-app-messages-in-canvas-sent}
...
É possível que um usuário inicie uma sessão antes de o Canvas ser interrompido, mas não veja a mensagem no app imediatamente. Isso pode ocorrer se a mensagem no app for disparada por um evento personalizado ou estiver com postergação. Nesse caso, é possível que um usuário registre uma impressão de mensagem no app e "receba" a mensagem no app após o Canvas ser interrompido. No entanto, o usuário precisaria ter iniciado a sessão antes de o Canvas ser interrompido, mas **depois** de ter recebido a etapa do Canvas.

{% alert note %}
Interromper um Canvas não faz com que os usuários que estão aguardando o recebimento de mensagens saiam da jornada do usuário. Se você reativar o Canvas e os usuários ainda estiverem aguardando a mensagem, eles a receberão (a menos que o horário em que a mensagem deveria ter sido enviada já tenha passado — nesse caso, eles não a receberão).
{% endalert %}
...
Após a conclusão das etapas de [Postergação]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/delay_step) upstream e das verificações de público, os usuários se tornam elegíveis para uma mensagem no app somente quando chegam à etapa de Mensagem. Se a mensagem expirar em uma data do calendário ou em uma janela curta de **duração após a etapa estar disponível**, os usuários em ramificações mais lentas podem chegar após a expiração e nunca ver a mensagem. Alinhe a expiração com as postergações mais longas e realistas da sua jornada. Para saber mais e ver exemplos, consulte [Expiração de mensagens no app]({{site.baseurl}}/user_guide/messaging/canvas/create_a_canvas/canvas_by_channel/in-app_messages_in_canvas#in-app-message-expiration).
...
Essa mensagem aparece quando a personalização faz referência a campos que as mensagens no app não conseguem resolver no Canvas. Use o objeto `context` conforme descrito em [Propriedades de contexto e evento]({{site.baseurl}}/user_guide/messaging/canvas/create_a_canvas/context_and_event_properties) e [Etapa de mensagem]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/message_step). O namespace Liquid legado `canvas_entry_properties` possui restrições diferentes de `context`. Se você precisar que valores persistam em várias etapas, revise as [propriedades persistentes no editor original do Canvas]({{site.baseurl}}/user_guide/messaging/canvas/create_a_canvas/context_and_event_properties/canvas_persistent_entry_properties) com sua equipe da Braze. Os valores armazenados são apagados quando um usuário sai do Canvas antes de o dispositivo baixar o payload da mensagem no app.
...
Não. As variantes na mesma configuração multivariante ou etapa de Mensagem compartilham um único cronograma de entrega. Não é possível atribuir uma variante para envio às 18h e outra às 19h no mesmo envio agendado.
...
- Use ramificações ou uma etapa de [jornada experimental]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/experiment_step) para que os usuários sigam jornadas com tempos diferentes.
- Separe em Campaigns distintas se o caso de uso não precisar permanecer dentro de um único Canvas.
...
### O que acontece se um usuário tiver limite de frequência global em uma etapa de Mensagem do Canvas? {#what-happens-if-a-user-is-global-frequency-capped-at-a-canvas-message-step}

Ele não recebe o envio para o canal com limite, mas as etapas de Mensagem ainda avançam os usuários quando uma mensagem não é enviada por causa do limite de frequência global. Para os casos de avanço etapa a etapa, consulte [Como os usuários avançam]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/message_step#how-users-advance). O limite de frequência global por si só não remove os usuários de um Canvas; esse comportamento é separado das **Validações de entrega** em uma etapa de Mensagem. Para saber mais, consulte [Limite de frequência]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/frequency_capping).

### Por que os envios são menores do que o tamanho estimado do público? {#why-are-sends-lower-than-the-estimated-audience-size}

Os envios podem ser menores do que o **Público estimado** por muitos dos mesmos motivos das [Campaigns]({{site.baseurl}}/user_guide/messaging/campaigns/faq#why-are-sends-lower-than-the-estimated-audience-size), incluindo limites de frequência, filtros rigorosos de dispositivo ou navegador, janelas de reelegibilidade, limites de taxa e exclusões no nível do canal (por exemplo, alcançabilidade de push ou verificações de inscrição e entregabilidade de e-mail).

Fatores específicos do Canvas também se aplicam:

- **Entrada baseada em ação ou disparada por API:** Os usuários só entram (e recebem etapas) depois de realizarem o comportamento de entrada, então os envios realizados ficam abaixo da estimativa inicial até que essas ações ocorram.
- **Jornadas de público:** Os usuários são direcionados para a ramificação de maior prioridade para a qual se qualificam, então ramificações posteriores podem receber menos usuários do que uma contagem simples do Segment sugere.
- **Verificações de público e horário de envio:** Etapas completas reavaliam os filtros no horário do envio, a menos que você configure de outra forma. Usuários que se qualificaram quando o Canvas foi criado podem ser excluídos antes do envio da mensagem.
- **Grupos de controle:** Grupos de controle global ou do Canvas retêm uma parcela dos entrantes do envio de mensagens.
...
- **Limites máximos de entrada ou público:** Limites de entrada ou de envio impedem usuários adicionais mesmo quando o Segment subjacente é maior.
...
_Destinatários únicos_ pode ser maior do que o público esperado porque a Braze rastreia **destinatários únicos diários** para relatórios de Canvas e Campaign. Isso permite uma atribuição de conversão precisa cada vez que um usuário recebe uma mensagem na jornada.

Por exemplo, se um usuário recebe uma etapa do Canvas na segunda-feira e novamente na sexta-feira e converte após cada envio, a Braze pode contar duas linhas de destinatário e duas conversões no escopo. Com entradas recorrentes ou reelegibilidade, o mesmo pequeno conjunto de perfis pode produzir múltiplos _Destinatários únicos_ ao longo de vários dias.
...
Para [limites de taxa de velocidade de entrega]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/frequency_capping#delivery-speed-rate-limiting) e outros fatores que reduzem os envios em uma única ocorrência, consulte [Por que os envios são menores do que o tamanho estimado do público?](#why-are-sends-lower-than-the-estimated-audience-size).

### Por que um Segment pequeno de grupo de controle mostra mudanças na participação histórica? {#why-does-a-small-control-group-segment-show-changes-in-historical-membership}

Os gráficos de participação histórica usam amostras estimadas, então Segments pequenos — incluindo Segments do [grupo de controle global]({{site.baseurl}}/user_guide/audience/global_control_group) — podem mostrar variação diária mesmo quando o público subjacente é estável. Para entender como as estimativas funcionam e por que os gráficos podem variar, consulte [Visualizar o tamanho histórico de participação do Segment]({{site.baseurl}}/user_guide/audience/segments/measuring_segment_size#viewing-historical-segment-membership-size).
...
Existe uma jornada de Canvas com 10 notificações por push e o evento de conversão é "início de sessão" ("abre o app"):
...
- A taxa de conversão da variante ou o bloco de resumo no início de um Canvas reflete todas as conversões realizadas pelos usuários dentro daquela jornada, tenham ou não recebido uma mensagem, como um total agregado.
...
Mensagens arquivadas não são enviadas até que você as retorne a um estado editável. [Desarquive]({{site.baseurl}}/user_guide/messaging/governance/archiving#unarchiving) a Campaign ou o Canvas, defina o cronograma de entrada ou o horário de envio para uma janela futura (ou duplique a jornada se precisar de uma cópia limpa) e, em seguida, selecione **Resume** ou inicie conforme necessário. Consulte [Arquivar Campaigns e Canvas]({{site.baseurl}}/user_guide/messaging/governance/archiving).

### Por que meu Canvas não salva quando nenhum erro aparece? {#why-doesnt-my-canvas-save-when-no-error-appears}

Filtros de **Custom attribute** vazios nos filtros de público ou no nível da etapa podem bloquear o salvamento sem uma mensagem de validação detalhada. Abra cada cartão de filtro, remova regras de atributos personalizados incompletas ou insira o nome e o valor do atributo e, em seguida, selecione **Save** novamente.
...
Filtros como `Received Message from Canvas Step` são atualizados após a Braze registrar o evento de envio, recebimento ou engajamento correspondente para aquela etapa. In-App Messages podem registrar impressões separadamente das métricas de envio. Consulte [Por que um Canvas pode mostrar zero envios mesmo com impressões registradas?](#why-may-a-canvas-show-zero-sends-even-though-impressions-are-logged). Esses mesmos eventos aparecem nas métricas da etapa em **Canvas Details**.
...
O número de usuários que entram em um Canvas pode ser diferente do esperado por causa de como os públicos e os disparos são avaliados. Na Braze, o público é avaliado antes do disparo (a menos que se use um gatilho de [mudança de atributo]({{site.baseurl}}/user_guide/messaging/campaigns/schedule_your_campaign/triggered_delivery/attribute_triggers#change-custom-attribute-value)). Isso fará com que os usuários saiam do Canvas se não fizerem parte do público selecionado antes que quaisquer ações de disparo sejam avaliadas.
...
Observe que a atribuição de variante ocorre na entrada do Canvas. Se um usuário não tiver entrado em um Canvas, ele não será atribuído a nenhuma variante. Em outras palavras, ele não estará no grupo de controle nem em uma variante.
...
Anteriormente, cada etapa completa incluía informações como configurações de postergação e agendamento, eventos de exceção, filtros de público, configuração de mensagem e opções de avanço de mensagem, tudo em um único componente. Essas são configurações separadas no editor atual para tornar sua experiência de criação de Canvas mais personalizável, além de introduzir algumas diferenças de funcionalidade.

#### Avanço do componente de mensagem {#message-component-advancement}

Os [componentes de mensagem]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/message_step) avançam todos os usuários que entram na etapa. Não há necessidade de especificar o comportamento de avanço de mensagem, o que torna a configuração geral da etapa mais simples. Se você quiser implementar a opção **Advance when message sent**, adicione um componente separado de jornadas do público para filtrar os usuários que não receberam a etapa anterior.
...
Digamos que, em 12 de abril, temos um componente de postergação configurado para enviar o usuário para a próxima etapa em um dia às 14h. Um usuário entra no componente às 14h01 do dia 13 de abril.
...
- No editor atual, o usuário prosseguiria para a próxima etapa às 14h do dia 15 de abril. Note que esse é o mesmo horário, mas mais de um dia a partir do horário de entrada.
...
- **Fluxo de trabalho atual:** Levará 48 horas para que a postergação passe, então o usuário recebe a mensagem no terceiro dia às 14h.
...
O evento de exceção é aplicado usando jornadas de ação, que são separadas das etapas de mensagem. O horário de silêncio é aplicado no componente de mensagem. Isso significa que, se um usuário já passou pela jornada de ação (e não foi excluído pelo evento de exceção), encontra o horário de silêncio ao chegar ao componente de mensagem e tem seu Canvas configurado para reenviar a mensagem após o período de horário de silêncio, o evento de exceção não será mais aplicado. Note que esse caso de uso não é comum.

Para Segments e filtros, a etapa de mensagem possui validações de entrega que permitem que os usuários configurem Segments e filtros adicionais que são validados no momento do envio. Isso evita o caso extremo mencionado do horário de silêncio.
...
Se você encontrar um erro "Request Timed Out" ao editar um Canvas e precisar entrar em contato com o [suporte da Braze]({{site.baseurl}}/braze_support), inclua as seguintes informações para ajudar a agilizar a resolução:

{% multi_lang_include messaging/support_ticket_request_timed_out_details.md context='canvas' %}

## Entrega e solução de problemas no Canvas {#canvas-delivery-and-troubleshooting}

### Usuários órfãos são elegíveis para receber mensagens do Canvas? {#are-orphaned-users-eligible-to-receive-canvas-messages}

Não. [Usuários órfãos]({{site.baseurl}}/user_guide/data/unification/user_data/user_profile_lifecycle#what-happens-when-you-identify-anonymous-users) não são elegíveis para receber mensagens. Se um perfil é tornado órfão enquanto o usuário está em uma jornada do Canvas, ele sai silenciosamente do fluxo. A análise de dados nem sempre mostra um evento **Exited** para essa saída, e o resumo do fluxo pode incluir um `partial_update_token` sem `exited_date` ou `exit_reason`.
...
Sim. Depois que a Braze envia uma solicitação ao seu provedor de serviços de e-mail (ESP), a Braze não consegue recuperar esse envio. Parar um Canvas ou uma Campaign impede novas solicitações de envio, mas as mensagens já encaminhadas ao ESP ainda podem ser entregues e ainda podem incrementar as contagens de envio conforme o ESP as processa.
...
Esse erro de console significa que pelo menos uma etapa aponta para uma próxima etapa ausente ou inválida — por exemplo, após uma exclusão parcial, clonagem ou importação. Abra o Canvas no editor, reconecte as etapas órfãs ou remova etapas que não possuem mais um caminho downstream válido. Se o Canvas ainda não carregar, entre em contato com o [suporte da Braze]({{site.baseurl}}/braze_support) informando o ID do Canvas e uma captura de tela do erro de console.
...
Verifique o JSON na sua etapa de [atualização de usuário]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/user_update). Atualizações de array e atributos aninhados precisam de caminhos e valores válidos para o atributo que você está alterando. Não inclua campos que a etapa fornece automaticamente, como o ID de usuário externo. Use a guia **Prévia e teste** da etapa para confirmar a carga útil antes do lançamento.
...
Sim, se um perfil de usuário na Braze já existir. Usuários sem `external_id` são [usuários anônimos]({{site.baseurl}}/user_guide/data/unification/user_data/user_profile_lifecycle#anonymous-user-profiles) e podem ser referenciados com um `braze_id` ou [alias de usuário]({{site.baseurl}}/user_guide/data/unification/user_data/user_profile_lifecycle#user-aliases). Crie ou atualize o perfil com o [endpoint `/users/track`]({{site.baseurl}}/api/endpoints/user_data/post_user_track) ou seu SDK antes da entrada no Canvas e, em seguida, use [entrada baseada em ação ou disparada por API]({{site.baseurl}}/user_guide/messaging/canvas/create_a_canvas#step-12-determine-your-canvas-entry-schedule). O direcionamento padrão do Canvas ainda requer um perfil de usuário na Braze — não é possível enviar mensagens do Canvas apenas para um endereço de e-mail sem um perfil.

### Por que um usuário entrou em um Canvas menos vezes do que executou o evento-gatilho? {#why-did-a-user-enter-a-canvas-fewer-times-than-they-performed-the-trigger-event}

Para Canvas baseados em ação e disparados por API, a Braze deduplica eventos-gatilho para que um usuário possa entrar no máximo **uma vez por segundo** para o mesmo Canvas. Se um usuário realiza o mesmo gatilho várias vezes em um segundo, apenas uma entrada é processada.

Para permitir múltiplas entradas no mesmo segundo, espaçe os eventos-gatilho em pelo menos 1,1 segundo (por exemplo, quando você controla o timing dos eventos a partir do seu servidor). Para um comportamento no estilo de Campaign que permita múltiplos gatilhos no mesmo segundo, compare seu caso de uso com [Campaigns]({{site.baseurl}}/user_guide/messaging/campaigns) com agendamento e configurações de reelegibilidade apropriados.

### Quando os usuários são deduplicados em Canvas disparados por API? {#when-are-users-de-duplicated-in-api-triggered-canvases}

Se um usuário entra novamente em um Canvas disparado por API e alcança uma etapa de postergação onde já está na fila de uma entrada anterior para uma mensagem idêntica, a Braze deduplica o usuário para evitar envios duplicados. A segunda instância do Canvas sai, então o número de entradas pode exceder o número de envios.
...
Para confirmar o direcionamento específico de um app, envie uma mensagem ao vivo ou de teste por meio de uma Campaign ou Canvas com um público restrito (por exemplo, filtrando por `external_id`) em vez de confiar apenas no **push de teste** do perfil.

Para etapas de mensagem do **Canvas** com vários apps, ative **Validar público no envio da mensagem** na etapa de mensagem para que as verificações de Segment e filtro sejam executadas no momento do envio. Para saber mais, consulte [Etapa de mensagem]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/message_step).

Para o comportamento geral de push de teste, consulte [Envio de mensagens de teste]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/sending_test_messages) e [FAQ sobre push]({{site.baseurl}}/user_guide/channels/push/faqs).
...
A Braze envia essa notificação quando mensagens do Canvas são atrasadas por limite de frequência por 24 horas ou mais. O e-mail é enviado para os usuários do dashboard que fizeram alterações anteriormente no Canvas afetado (com base nos logs de alteração do Canvas). Se a Braze não conseguir determinar esses destinatários, o e-mail será enviado aos **administradores da empresa** do espaço de trabalho.
...
A Braze registra a saída assim que o evento de exceção ocorre, mas os usuários podem permanecer dentro de uma etapa até que os timers sejam concluídos — mais visivelmente em etapas de postergação. O comportamento também difere entre etapas agendadas e etapas disparadas por evento. Para cronogramas, exemplos e nuances de análise de dados, consulte [Critérios de saída]({{site.baseurl}}/user_guide/messaging/canvas/create_a_canvas/exit_criteria).
