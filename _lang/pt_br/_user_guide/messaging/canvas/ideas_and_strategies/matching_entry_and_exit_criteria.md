...
page_order: 5
page_type: tutorial
description: "Saiba como configurar critérios de saída e jornadas de ação que comparam propriedades de eventos com propriedades de entrada do Canvas, para que os usuários só saiam ou sigam uma Branch or ramificação or ramificação quando concluírem a ação específica com a qual entraram."
tool: Canvas
---

# Correspondência de critérios de saída com eventos de entrada {#matching-exit-criteria-to-entry-events}

> Este artigo explica como configurar critérios de saída e jornadas de ação que se correlacionam diretamente com o evento de entrada do Canvas, para que os usuários só saiam ou sigam uma Branch or ramificação or ramificação quando realizarem uma ação específica relacionada ao motivo pelo qual entraram no Canvas.
...
## Jornadas de ação: Branch or ramificação or ramificação com base em uma ação correspondente {#action-paths-branching-based-on-a-matching-action}
