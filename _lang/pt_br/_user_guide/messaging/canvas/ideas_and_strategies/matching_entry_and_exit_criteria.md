...
## Critérios de saída: saindo do Canvas quando uma ação correspondente ocorre
...
4. Ative o toggle **Personalize value**, defina o **Personalization type** como `Context Variables` e defina o **Atributo** como `event_id`.
...
## Jornadas de ação: ramificação com base em uma ação correspondente
...
2. Para o primeiro grupo de ação (compra original), adicione um gatilho para o evento personalizado de conclusão (como `Purchased_Ticket`). Selecione **Add property filters** e adicione um filtro onde a comparação da propriedade básica `event_id` esteja definida como `equals`. Ative **Personalize value**, defina o **Personalization type** como `Context Variables` e defina o **Atributo** como `event_id`.
