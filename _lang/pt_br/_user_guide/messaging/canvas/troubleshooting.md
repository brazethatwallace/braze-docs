...
| Ninguém ou menos usuários do que o esperado entraram | [Entradas baixas ou zero no Canvas](#low-or-zero-canvas-entries) |
| Os envios ou entregas são menores do que o público estimado | [Menos envios do que o esperado](#lower-sends-than-expected) |
| A análise de dados do Canvas parece errada (grupo de controle, conversões, zero envios) | [Divergências na análise de dados do Canvas](#canvas-analytics-mismatches) |
| A análise de dados mostra muito mais envios do que entradas ou mais saídas do que entradas | [A filtragem por intervalo de datas pode mostrar números inesperados](#date-range-filtering-can-show-unexpected-numbers) |
| O Canvas não salva ou o editor trava | [Problemas com o editor e ao salvar](#editor-and-save-issues) |
| Não consigo excluir uma variante do Canvas | [Não é possível excluir uma variante do Canvas por causa de um Segment arquivado](#cant-delete-a-canvas-variant-because-of-an-archived-segment) |
| Parei o Canvas, mas as mensagens ainda foram enviadas | [Comportamento do Canvas interrompido](#stopped-canvas-behavior) |
...
Use este fluxo de trabalho para investigar um usuário específico ou um problema de envio agregado. Comece na etapa 1 para cada incidente.
...
3. Verifique o registro de envio de mensagens de um usuário acessando **Audience** > **Search users**, abrindo o perfil e selecionando **Messaging History** (últimos 30 dias).
...
5. Verifique os resultados agregados na página de análise de dados do Canvas abrindo o [dashboard de Diagnóstico de Mensagens]({{site.baseurl}}/user_guide/analytics/dashboards/dashboard_builder/diagnostics_dashboard) e analisando os motivos de interrupção e descarte.
   - Se você vir um resultado que não reconhece, consulte [Resultados de interrupção]({{site.baseurl}}/user_guide/analytics/dashboards/dashboard_builder/diagnostics_dashboard#abort-outcomes) na documentação de diagnóstico.
...
6. Se você ainda estiver bloqueado, entre em contato com o [suporte da Braze]({{site.baseurl}}/user_guide/administer/personal/braze_support) em até 30 dias com o ID do Canvas, IDs dos usuários afetados, timestamps (com fuso horário) e capturas de tela do Messaging History ou do Diagnóstico de Mensagens.
...
**Sintoma:** Um usuário não entrou no Canvas quando você esperava, ou menos usuários entraram do que seus eventos-gatilho sugerem.

Os usuários devem corresponder ao **público-alvo** antes de a Braze avaliar o gatilho de entrada (exceto para gatilhos de [mudança em atributo]({{site.baseurl}}/user_guide/messaging/campaigns/schedule_your_campaign/triggered_delivery/attribute_triggers#change-custom-attribute-value)). Um gatilho sozinho não garante a entrada se o usuário não estava no público no momento da avaliação.
...
- **Reelegibilidade:** Determina se um usuário pode entrar no Canvas novamente após sair (janela de tempo e configuração **Allow users to re-enter Canvas**).
- **Reentrada:** Determina se um usuário que está atualmente dentro do Canvas pode entrar em uma jornada simultânea.

Um usuário pode ser reelegível, mas estar bloqueado porque ainda está dentro do Canvas, ou pode ter saído, mas ainda estar fora da janela de reelegibilidade. Verifique ambas as configurações quando um usuário não estiver reentrando em um Canvas.
...
- **Público-alvo no momento da avaliação:** Revise os changelogs de Segments e filtros. O [User Lookup]({{site.baseurl}}/user_guide/audience/segments/creating_a_segment) pode exibir um falso positivo para alguns tipos de filtro (por exemplo, atributos de data formatados como string).
...
A Braze deduplica múltiplas tentativas de entrada que ocorrem no mesmo instante, então você pode ver menos entradas no Canvas do que eventos-gatilho. Para testar múltiplas entradas, espaçe os eventos-gatilho com pelo menos um segundo de intervalo.

Se um usuário executa o mesmo gatilho várias vezes dentro de um segundo, a Braze processa apenas uma entrada. Verifique o Messaging Diagnostics para resultados como **User not re-eligible** quando as regras de reentrada ou reelegibilidade se aplicam.

{% details Horário de verão e Canvas agendados diariamente %}

Em dias de transição do horário de verão, Canvas agendados diariamente podem ser executados até uma hora antes ou depois do habitual. Se seus critérios de entrada dependem de atributos personalizados ou eventos com timestamps que ocorrem dentro de uma hora do horário programado de entrada, os usuários podem ainda não se qualificar no dia da mudança de horário porque o atributo ou evento ainda não foi registrado.

Por exemplo, suponha que os usuários normalmente recebem uma atualização de atributo personalizado às 15h no fuso horário do seu Canvas e seu Canvas é executado diariamente às 15h30 nesse mesmo fuso horário. Em um dia de adiantamento do horário de verão, o Canvas pode avaliar os usuários até uma hora antes do habitual em relação a essa atualização de atributo — antes que o atributo tenha sido registrado. Se a reelegibilidade estiver desativada, os usuários que entraram em dias anteriores não poderão reentrar, resultando em zero entradas para aquele dia.

Para evitar isso, garanta que as atualizações de atributos personalizados ou eventos ocorram mais de uma hora antes do horário programado de entrada do Canvas.
...
Verifique o [**histórico de mensagens**]({{site.baseurl}}/user_guide/audience/manage_audience/user_profiles#messaging-history-tab) do usuário para a etapa do Canvas e o registro de data e hora. Se não houver registro, volte para [O usuário não entrou no Canvas](#user-didnt-enter-the-canvas).

Em seguida, verifique os itens a seguir por tipo de disparo ou etapa:

- **Disparos de evento personalizado ou compra:** Confirme se o evento aparece em **Analytics** > **Custom Events Report** (ou **Revenue** para compras). Compare o registro de data e hora do evento com o momento em que o Canvas entrou em operação e com qualquer postergação agendada na etapa.
...
- **Horário de silêncio, Intelligent Timing, limites de frequência e limites de taxa:** Esses recursos podem adiar, suprimir ou interromper envios. Os usuários ainda podem permanecer no Canvas após uma interrupção por horário de silêncio.
- **Condições de corrida:** Se o usuário disparou várias ações ao mesmo tempo, consulte [Condições de corrida]({{site.baseurl}}/user_guide/messaging/ab_testing/concepts/race_conditions).

{% alert important %}
Quando uma etapa de mensagem do Canvas interrompe um envio, o usuário ainda avança para a próxima etapa. O Canvas avança na interrupção para que etapas posteriores de postergação e jornadas de ação não sejam bloqueadas permanentemente. Consulte [Como os usuários avançam]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/message_step#how-users-advance) e [Resultados de interrupção]({{site.baseurl}}/user_guide/analytics/dashboards/dashboard_builder/diagnostics_dashboard#abort-outcomes).
{% endalert %}
...
{% alert important %}
Se o seu Canvas baseado em ação envia mensagens antes do esperado, verifique se o registro de data e hora do evento personalizado usa a hora atual, e não uma hora retroativa. A Braze avalia as postergações a partir do registro de data e hora enviado com o evento. Consulte [Entrega baseada em ação]({{site.baseurl}}/user_guide/messaging/canvas/create_a_canvas#step-12-determine-your-canvas-entry-schedule).
{% endalert %}

## Entradas baixas ou zero no Canvas {#low-or-zero-canvas-entries}

**Sintoma:** Nenhum usuário ou menos usuários do que o esperado entraram no Canvas.
...
- Limites de frequência do espaço de trabalho ou filas de entrada podem adicionar atrasos entre o momento em que os usuários se qualificam e o momento em que entram ou avançam para uma etapa.
...
**Sintoma:** Os envios ou entregas são menores que o público estimado em uma etapa do Canvas.

As causas comuns incluem reavaliação do público no momento do envio, elegibilidade do canal, grupos de controle, horário de silêncio, Intelligent Timing, limites de frequência e comportamento de entrega de mensagens no app (zero _Envios_ com impressões é esperado para mensagens no app).

Se uma etapa de Mensagem mostrar muitos usuários que entraram, mas poucos envios, verifique se o Liquid `abort_message()` cancelou o envio. Para verificações no Message Activity Log, atributos ausentes e envios de teste, consulte [Solução de problemas de altas taxas de interrupção]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/liquid/aborting_messages#troubleshooting-high-abort-rates).
...
Use o [dashboard de diagnóstico de envio de mensagens]({{site.baseurl}}/user_guide/analytics/dashboards/dashboard_builder/diagnostics_dashboard) para ver os motivos de interrupção e descarte no nível da etapa.
...
Ao visualizar o Segment de usuários ativos, você verá que, embora ele contenha 29,8 mil usuários, apenas 64% deles têm push ativado:
...
Isso significa que, mesmo que você tenha especificado 90% dos usuários para entrar na variante, nem todos podem receber uma notificação por push. Usuários que não podem receber push ainda entram na variante — a contagem de envios reflete a elegibilidade do canal na etapa, não a atribuição de variante na entrada.

{% enddetails %}

### Filtrar por intervalo de datas pode exibir números inesperados {#date-range-filtering-can-show-unexpected-numbers}

**Sintoma:** A análise de dados do Canvas ou da etapa mostra números inesperados ou improváveis, como muito mais envios do que entradas, ou mais usuários saindo de uma etapa do que entraram.

Isso pode acontecer quando você usa o filtro de calendário de intervalo de datas no topo da página de análise de dados do Canvas. Se você selecionar um intervalo de datas que exclui algumas ações de usuários, as métricas exibidas podem mostrar apenas parte da jornada de cada usuário.

Por exemplo:
- Você pode ver 100 entradas com 8.000 envios se o intervalo de datas começar após a maioria dos usuários ter entrado, mas incluir o período em que receberam mensagens.
- Você pode ver mais usuários avançando para a etapa seguinte do que entraram na etapa anterior, se o intervalo capturar apenas as saídas, mas não as entradas anteriores.

Para resolver isso, ajuste o intervalo de datas para incluir todas as datas desde o lançamento do Canvas até o presente, ou selecione um intervalo que cubra todo o período relevante para as métricas de que você precisa.

Para definições de taxa de conversão e análise de dados por etapa, consulte [Análise de dados e conversões]({{site.baseurl}}/user_guide/messaging/canvas/faqs#analytics-and-conversions) nas Perguntas frequentes do Canvas.

## Problemas com o editor e salvamento {#editor-and-save-issues}

**Sintoma:** O editor do Canvas não carrega, congela ou não salva suas alterações.
...
| Não é possível salvar após arquivar uma variante | Uma variante arquivada ainda é referenciada em etapas posteriores; revise as conexões das etapas e restaure ou substitua a variante |
{: .reset-td-br-1 .reset-td-br-2 aria-label="Sintoma do editor" }

{% alert note %}
Se você tiver o mesmo Canvas aberto em várias guias do navegador, salvar em uma guia pode sobrescrever as alterações feitas em outra guia desatualizada. Essa condição de corrida pode fazer com que filtros de público sejam removidos ou alterações não intencionais sejam aplicadas. Para evitar perda de dados, feche quaisquer guias duplicadas antes de editar e salvar um Canvas.
{% endalert %}
...
Se o Canvas não carregar e não avançar, uma versão anterior não foi salva corretamente e pode conter etapas inválidas. Duplique o Canvas a partir do dashboard. Se o problema persistir, abra um [ticket de suporte]({{site.baseurl}}/user_guide/administer/personal/braze_support).

Para tickets de suporte de "Request Timed Out", inclua uma gravação de tela, carimbo de data/hora e fuso horário, navegador e versão, etapas para reproduzir e, opcionalmente, um log HAR das ferramentas de desenvolvedor do seu navegador. Consulte [O que devo incluir ao enviar um ticket de suporte para um erro "Request Timed Out"?]({{site.baseurl}}/user_guide/messaging/canvas/faqs#what-should-i-include-when-submitting-a-support-ticket-for-a-request-timed-out-error) nas Perguntas frequentes do Canvas.
...
Quando você interrompe um Canvas, os usuários não podem entrar e nenhuma mensagem adicional é enviada a partir do fluxo do Canvas. Envios de e-mail que já foram entregues ao seu provedor de serviços de e-mail não podem ser revertidos.

Usuários que estão aguardando em uma etapa de postergação ou jornada de ação não são automaticamente removidos da jornada quando você interrompe o Canvas. Se você reativar o Canvas antes que o horário agendado de envio expire, eles ainda poderão receber as etapas pendentes.
...
Esse erro aparece quando a combinação de ramificações de etapas e o tamanho do público de entrada pode causar problemas de desempenho no cluster, impedindo o envio de mensagens. A Braze exibe essa mensagem quando você lança um Canvas com entrada agendada — ela não aparece quando você salva um rascunho.
...
- Se o seu Canvas usa o editor original, [clone-o para o Canvas Flow]({{site.baseurl}}/user_guide/messaging/canvas/managing_canvases/cloning_canvases) e reconstrua com os componentes do Canvas.

Se ainda precisar lançar o Canvas sem alterações e não for possível migrar para o Canvas Flow, entre em contato com o [suporte da Braze]({{site.baseurl}}/support_contact).

## Quando entrar em contato com o suporte {#when-to-contact-support}

Entre em contato com o [suporte da Braze]({{site.baseurl}}/user_guide/administer/personal/braze_support) em até 30 dias após o problema, caso tenha concluído o [caminho de investigação padrão](#standard-investigation-path) e ainda precise de ajuda.

Inclua:

- ID do Canvas e IDs dos usuários afetados (ID externo ou ID da Braze)
- Carimbos de data/hora com fuso horário
- Capturas de tela ou exportações do **histórico de mensagens** ou do **Messaging Diagnostics**
