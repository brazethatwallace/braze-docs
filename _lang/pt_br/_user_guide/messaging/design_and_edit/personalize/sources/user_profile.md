...
{% raw %}
Quando uma campanha ou Canvas é disparada por um evento personalizado ou uma compra, as propriedades do evento ficam disponíveis para personalização. Referencie-as usando `{{event_properties.${property_name}}}`.
...
As propriedades de eventos estão disponíveis em campanhas baseadas em ação e na primeira etapa de um Canvas baseado em ação. Para saber mais, consulte [Eventos personalizados]({{site.baseurl}}/user_guide/data/activation/events/custom_events/).

## Propriedades de disparo via API

{% raw %}
Para campanhas e Canvas disparados pela API, você pode enviar dados adicionais usando o objeto de propriedades de disparo. Referencie esses valores com `{{api_trigger_properties.${property_name}}}`.
