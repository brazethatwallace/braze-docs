...
- Com Liquid condicional, todas as listas referenciadas têm códigos deduzidos, mesmo que apenas uma Branch or ramificação or ramificação seja exibida.
...
Por exemplo, no seguinte Liquid condicional, os códigos de ambas as listas (`vip-deal` e `regular-deal`) são deduzidos, mesmo que cada usuário veja apenas uma Branch or ramificação or ramificação:
