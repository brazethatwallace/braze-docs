...
Códigos de promoção permitem inserir valores únicos e com prazo de validade nas mensagens para impulsionar conversões. Cada lista pode conter até 20 milhões de códigos, e cada código pode durar até seis meses antes de expirar.
...
Se você colocar vários snippets da mesma lista em uma mensagem, a Braze aplica o mesmo código em todos os snippets. Para evitar ficar sem códigos, faça upload de mais códigos do que você espera usar.

{% tabs local %}
{% tab Exemplo %}
Pense nos códigos de promoção como cupons em uma agência dos correios. Quando o atendente retira um cupom da pilha para a sua carta, ele se foi — mesmo que a carta nunca chegue.
...
{% alert important %}
Códigos de promoção estão disponíveis em Campaigns de mensagem no app, mas não podem ser enviados em mensagens no app no Canvas.
{% endalert %}
...
Os códigos de promoção são compatíveis com e-mail, push móvel, web push, Content Cards, webhook, SMS e WhatsApp. Campaigns de mensagem no app oferecem suporte a códigos de promoção como recurso de acesso antecipado. Campaigns de e-mail de transação da Braze e mensagens no app em Canvas não oferecem suporte a códigos de promoção.

### Envios de teste e de grupo de teste contam para o uso? {#do-test-and-seed-sends-count-towards-usage}

Por padrão, envios de teste e envios de e-mail de grupo de teste usam códigos de promoção por usuário, por envio de teste. No entanto, você pode entrar em contato com o gerente da sua conta Braze para atualizar esse comportamento e não usar códigos de promoção durante os testes.
...
Se um determinado usuário for elegível para receber um código por vários canais, ele recebe o mesmo código em cada canal. Apenas um código de promoção é usado, independentemente dos canais recebidos.
...
Se a mensagem deveria conter um código de promoção de uma lista vazia ou expirada, a mensagem é cancelada.

Se a mensagem contém lógica Liquid que insere condicionalmente um código de promoção, a mensagem só é cancelada se deveria conter um código de promoção. Se a mensagem não deveria conter um código de promoção, a mensagem é enviada normalmente.
...
Se você fez upload de códigos incorretos, você tem duas opções para resolver isso:

- **Descontinuar a lista inteira:** Pare de usar a lista atual em quaisquer Campaigns, Canvas ou modelos. Em seguida, faça upload dos códigos corretos em uma nova lista e atualize todas as suas mensagens para usar a nova lista.
- **Esgotar os códigos incorretos:** Crie uma Campaign que envie códigos da lista incorreta para um usuário temporário até que todos os códigos errados sejam usados. Depois disso, faça um novo upload dos códigos corretos na mesma lista, excluindo os incorretos.
...
Quando uma mensagem usa um código de promoção, a Braze marca esse código como consumido para que não possa ser enviado novamente e atualiza a contagem restante da lista. A Braze não mantém um relatório de códigos enviados, não rastreia quais usuários específicos receberam cada código nem rastreia se os códigos foram resgatados.

Se você precisa associar códigos a usuários ou rastrear resgates por conta própria, você pode:

- Salvar códigos de promoção nos perfis de usuário por meio de uma etapa de User Update. Para saber mais, consulte [Salvar códigos de promoção em perfis de usuário]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/sources/promotion_codes/manage#save-to-profile).
...
Sim. Você pode salvar códigos de promoção no perfil de um usuário por meio de uma etapa de User Update. Para saber mais, consulte [Salvar códigos de promoção em perfis de usuário]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/sources/promotion_codes/manage#save-to-profile).