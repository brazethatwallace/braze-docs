...
A ferramenta **Adicionar personalização** aparece próxima a campos de texto com modelo em todo o dashboard, incluindo:
...
## Inserir variáveis pré-formatadas e padrões {#insert-pre-formatted-variables-and-defaults}

A ferramenta **Add Personalization** ajuda você a inserir Liquid com valores padrão opcionais para que dados de perfil vazios não quebrem seu texto.
...
A ferramenta insere Liquid com o valor padrão especificado no ponto onde seu cursor estava. O ponto de inserção também é indicado pela caixa de prévia, que mostra o texto antes e depois. Se um bloco de texto estiver destacado, o texto destacado será substituído.
...
{% raw %}
Algumas operações em Liquid exigem que você armazene o valor que deseja manipular como uma variável. Isso acontece frequentemente quando sua instrução Liquid inclui múltiplos atributos, propriedades de eventos ou filtros.
...
Em seguida, vamos usar o filtro `plus` para combinar o saldo do cartão-presente de cada usuário com o saldo de recompensas, representado por `{{balance}}`.
...
![A Braze recomendando diferentes atributos Liquid à medida que mais texto é inserido em um campo.]({% image_buster /assets/img/liquid_auto_complete.gif %}){: style="max-width:70%;"}
...
- [Filtros]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/liquid/filters) — formate datas, cálculos, strings e muito mais