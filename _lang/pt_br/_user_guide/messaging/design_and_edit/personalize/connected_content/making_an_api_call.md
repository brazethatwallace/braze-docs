...
Se a URL estiver indisponível e retornar uma página 404, a Braze renderiza uma string vazia em seu lugar. Se a URL retornar uma página HTTP 500 ou 502, a URL falha na lógica de nova tentativa.
...
- A Braze não cobra por chamadas de API e elas não contam para o uso de pontos de dados alocado.
...
A Braze enviará solicitações de Connected Content a partir das seguintes faixas de IP. As faixas listadas são adicionadas automática e dinamicamente a todas as chaves de API que foram habilitadas para a lista de permissões.
...
Para altas taxas de erro do seu host, consulte [Detecção de host com problemas de integridade]({{site.baseurl}}/help/help_articles/api/webhook_connected_content_errors#unhealthy-host-detection) e [Volume de chamadas de Connected Content](#understanding-connected-content-call-volume).
...
A Braze pode fazer a mesma chamada de API de Connected Content mais de uma vez por destinatário para renderizar a carga útil de uma mensagem. As cargas úteis de mensagens podem ser renderizadas várias vezes por destinatário para validação, lógica de nova tentativa ou outros fins internos. No entanto, apenas uma das chamadas de Connected Content preenche efetivamente uma mensagem.
