...
Verificamos as seguintes atribuições tratando-as como variáveis categóricas. Em outras palavras, um usuário possui ou não cada valor possível dessas atribuições, e testamos se elas afetam a taxa de conversão.
...
Em seguida, para cada uma das variáveis, também calculamos, com 95% de confiança, a taxa na qual os usuários com aquela atribuição ou comportamento específico converteram. Ao dividir essa taxa pela taxa base, conseguimos medir a razão. Se for muito maior que 1, os usuários com aquela atribuição ou comportamento têm mais probabilidade de converter. Se for muito menor, eles têm menos probabilidade. Exibimos o valor da razão na tabela. O valor só é exibido se estiver suficientemente distante de 1 para ser significativo no nível de 95% de confiança.