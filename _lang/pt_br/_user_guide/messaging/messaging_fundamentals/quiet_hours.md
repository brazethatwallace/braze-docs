...
- **O horário de silêncio não reavalia a associação ao Segment para um envio retido.** A Braze verifica a associação ao Segment quando a mensagem é disparada. Se o usuário for elegível nesse momento, a Braze retém a mensagem e a envia quando o horário de silêncio terminar. Isso é separado da [reavaliação da associação ao Segment no momento do envio]({{site.baseurl}}/user_guide/messaging/campaigns/schedule_your_campaign/triggered_delivery#audience-criteria-evaluation) ou das [validações de entrega]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/message_step#delivery-validations) do Canvas.
