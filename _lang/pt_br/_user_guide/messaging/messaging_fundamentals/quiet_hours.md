...
{% alert note %}
O horário de silêncio é aplicado no fuso local de cada usuário.
{% endalert %}
...
- **Sem Intelligent Timing:** o horário de silêncio funciona como uma janela de bloqueio de envio para o horário de envio agendado. Se o horário agendado cair dentro do horário de silêncio, a mensagem é retida e enviada quando a janela se encerrar.
...
## Considerações importantes {#things-to-consider}

- **As mensagens são enviadas ao mesmo tempo quando o horário de silêncio termina.** Se um grande público tiver mensagens retidas durante o horário de silêncio, todas essas mensagens serão enviadas de uma vez quando a janela se encerrar. Para Campaigns sensíveis ao tempo, considere como isso afeta o momento da entrega.
- **Horário de silêncio não é o mesmo que interrupção de uma mensagem.** A interrupção de uma mensagem a descarta completamente. O horário de silêncio retém a mensagem e a entrega posteriormente.
- **O horário de silêncio é independente do limite de frequência.** Cada um desses controles de entrega se aplica de forma independente. Uma mensagem que passa pelos limites de frequência ainda pode ser retida pelo horário de silêncio, e uma mensagem retida pelo horário de silêncio é avaliada em relação aos limites de frequência quando é finalmente enviada. Para saber mais, consulte [Limite de frequência]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/frequency_capping).
