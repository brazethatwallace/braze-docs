...
- [**Limites de frequência de API or interface de programação do aplicativo (API)**](https://braze.com/resources/articles/whats-rate-limiting): Revise os [limites de frequência]({{site.baseurl}}/api/api_limits) da API or interface de programação do aplicativo (API) da Braze para seus espaços de trabalho e evite erros. Se você deseja aumentar seus limites de frequência (e já está agrupando solicitações em lotes), entre em contato com seu gerente de sucesso do cliente. Lembre-se de que esse processo requer um prazo de antecedência, então planeje-se adequadamente.
...
- **Cliques e impressões:** Cliques e impressões para Banners são rastreados automaticamente com o SDK or kit de desenvolvimento de software.
- **Limitações:** Atualmente, os seguintes recursos não são compatíveis: integração com Canvas, Campaigns disparadas por API or interface de programação do aplicativo (API) e baseadas em ação, Connected Content, códigos de promoção e `catalog_items` usando a [tag `:rerender`]({{site.baseurl}}/user_guide/data/activation/catalogs/use#using-liquid).
...
- **Solicitação de posicionamentos:** Até 10 posicionamentos podem ser retornados ao SDK or kit de desenvolvimento de software em uma única solicitação de atualização. Cada posicionamento incluirá o Banner de maior prioridade para o qual o usuário é elegível.
...
- **Disparo de mensagens no app**: No início da sessão, o SDK or kit de desenvolvimento de software solicita que todas as mensagens no app elegíveis sejam enviadas ao dispositivo junto com seus disparadores, para que, se o usuário realizar o evento durante a sessão, ele possa receber a mensagem no app de forma rápida e confiável.
...
### Coisas para verificar
- [**Aceite/inscrito e push ativado**]({{site.baseurl}}/user_guide/channels/push/push_setup/push_subscription_states): Para que os usuários recebam uma mensagem por push da Braze, o status de inscrição deles precisa ser aceite (iOS) ou inscrito (Android), e `Push Enabled = True`. Note que o Android 13 introduz uma mudança importante na forma como os usuários gerenciam apps que enviam notificações por push. O [guia de upgrade do SDK or kit de desenvolvimento de software para Android 13]({{site.baseurl}}/developer_guide/platforms/android/android_13) da Braze continuará sendo atualizado conforme novas versões beta do Android 13 forem lançadas.

### Coisas para saber
- **Web push**: Se você já configurou o [SDK or kit de desenvolvimento de software Web]({{site.baseurl}}/user_guide/channels/push/platform_specific_resources/web) da Braze, considere utilizar o web push para engajar os usuários. O web push funciona da mesma forma que as notificações por push de apps no seu celular. Para saber mais sobre como compor um web push, confira [Criação de uma notificação por push]({{site.baseurl}}/user_guide/channels/push/create_a_push_message).
