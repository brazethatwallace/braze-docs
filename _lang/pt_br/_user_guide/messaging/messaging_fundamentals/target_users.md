...
{% tabs local %}
{% tab Segment único %}
Para direcionar membros de um Segment criado anteriormente, selecione um Segment no menu suspenso em **Direcionar por Segment de usuários**.
{% endtab %}

{% tab múltiplos segmentos %}
Para direcionar usuários que se enquadram em múltiplos segmentos criados anteriormente, adicione vários segmentos no menu suspenso em **Direcionar por Segment de usuários**. O público-alvo resultante será composto por usuários que estão no primeiro Segment, no segundo Segment, no terceiro Segment, e assim por diante.
{% endtab %}

{% tab múltiplos filtros %}
Para direcionar usuários sem adicionar um Segment, você pode usar uma série de filtros. Esse é um público improvisado durante a criação da mensagem e permite que você pule a criação de segmentos ao enviar para públicos pontuais.
...
{: start="2"}
2. Na etapa **Públicos-alvo**, confirme que seu Segment foi adicionado na seção **Direcionar por Segment de usuários**.
...
{% alert note %}
Isso não funcionará se você adicionar seu Segment na seção **Filtros adicionais** por meio de um filtro de associação a Segment. Você deve referenciar diretamente seu Segment em **Direcionar por Segment de usuários** para entregar sua mensagem apenas para aquele app.
{% endalert %}
...
- Para ver qual porcentagem da sua base de usuários está sendo direcionada ou o valor do tempo de vida (LTV) desse Segment, selecione **Mostrar estatísticas adicionais**.
