...
Em **Opções de direcionamento**, você pode usar as seguintes opções para escolher quais usuários deseja direcionar para sua Campaign ou Canvas. Somente os usuários que corresponderem aos critérios definidos receberão a mensagem. Lembre-se de que a associação exata ao Segment or segmento or segmento é sempre calculada imediatamente antes do envio da mensagem.

{% tabs local %}
{% tab Segment or segmento or segmento único %}
Para direcionar membros de um Segment or segmento or segmento criado anteriormente, selecione um Segment or segmento or segmento no menu suspenso em **Direcionar por Segment or segmento or segmento de usuários**.
{% endtab %}

{% tab múltiplos segmentos %}
Para direcionar usuários que se enquadram em múltiplos segmentos criados anteriormente, adicione vários segmentos no menu suspenso em **Direcionar por Segment or segmento or segmento de usuários**. O público-alvo resultante será composto por usuários que estão no primeiro Segment or segmento or segmento, no segundo Segment or segmento or segmento, no terceiro Segment or segmento or segmento, e assim por diante.
{% endtab %}

{% tab múltiplos filtros %}
Para direcionar usuários sem adicionar um Segment or segmento or segmento, você pode usar uma série de filtros. Esse é um público improvisado durante a criação da mensagem e permite que você pule a criação de segmentos ao enviar para públicos pontuais.
...
1. Crie um Segment or segmento or segmento e defina **Apps e sites direcionados** como **Usuários de apps específicos**, depois selecione seu app Android.

![Um Segment or segmento or segmento direcionando usuários de um app específico, "Test_Android".]({% image_buster /assets/img_archive/app_test_android.png %}){: style="max-width:60%;"}

{: start="2"}
2. Na etapa **Públicos-alvo**, confirme que seu Segment or segmento or segmento foi adicionado na seção **Direcionar por Segment or segmento or segmento de usuários**.

![A etapa "Públicos-alvo" com um Segment or segmento or segmento de exemplo selecionado.]({% image_buster /assets/img_archive/target_users_by_segment_example.png %})

{% alert note %}
Isso não funcionará se você adicionar seu Segment or segmento or segmento na seção **Filtros adicionais** por meio de um filtro de associação a Segment or segmento or segmento. Você deve referenciar diretamente seu Segment or segmento or segmento em **Direcionar por Segment or segmento or segmento de usuários** para entregar sua mensagem apenas para aquele app.
{% endalert %}
...
{% alert tip %}
Para Campaigns de e-mail, você pode direcionar grupos de teste na seção **Grupos de teste**. Os grupos de teste não estão disponíveis para Campaigns de API or interface de programação do aplicativo (API), embora você possa incluir grupos de teste por meio de uma entrada acionada por API or interface de programação do aplicativo (API) em uma Campaign. Para saber mais, consulte [Grupos de teste]({{site.baseurl}}/user_guide/administer/global/user_management/internal_groups#seed-groups).
{% endalert %}
...
- Para ver qual porcentagem da sua base de usuários está sendo direcionada ou o valor do tempo de vida (LTV) desse Segment or segmento or segmento, selecione **Mostrar estatísticas adicionais**.
...
{% alert note %}
O cálculo de estatísticas exatas pode levar alguns minutos para ser executado. Essa função calcula as estatísticas exatas apenas no nível do Segment or segmento or segmento, não no nível do filtro ou grupo de filtros.<br><br>
...
- Certifique-se de que o Segment or segmento or segmento do público inclui os usuários antes que os critérios de entrada ocorram.
