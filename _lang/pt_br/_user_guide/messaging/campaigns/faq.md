...
Por padrão, as campanhas verificam os filtros de público no momento da entrada. Para campanhas baseadas em ação com postergação, existe uma opção de reavaliar os critérios do Segment or segmento no momento do envio para garantir que os usuários ainda façam parte do público-alvo quando a mensagem for enviada.
...
Uma possível explicação é que a campanha ou o Canvas tem a reelegibilidade ativada, o que significa que os usuários que se qualificam para o Segment or segmento e as configurações de entrega poderão receber a mensagem mais de uma vez. Se a reelegibilidade não estiver ativada, a explicação provável para a diferença entre envios e destinatários únicos pode ser que os usuários possuem múltiplos dispositivos, em diferentes plataformas, associados aos seus perfis.
...
### Por que minha campanha tem uma base de usuários contatáveis menor do que o Segment or segmento que estou usando? {#why-does-my-campaign-have-a-smaller-reachable-user-base-than-the-segment-that-im-using-for-the-campaign}

Se você tiver um [grupo de controle global]({{site.baseurl}}/user_guide/audience/global_control_group) configurado, isso impedirá que uma porcentagem do seu público contatável receba campanhas. Isso significa que o número de usuários contatáveis do seu Segment or segmento pode ser maior do que o número de usuários contatáveis da sua campanha, mesmo que a campanha esteja usando o mesmo Segment or segmento.

### O que a entrega por fuso local oferece? {#what-does-local-time-zone-delivery-offer}

A entrega por fuso local permite enviar campanhas de mensagens para um Segment or segmento com base no fuso horário individual de cada usuário. Sem a entrega por fuso local, as campanhas serão agendadas com base nas configurações de fuso horário da sua empresa na Braze.
...
A Braze determinará automaticamente o fuso horário de um usuário a partir do dispositivo dele. Isso garante precisão no fuso horário e cobertura completa dos seus usuários. Usuários criados pela User API or interface de programação do aplicativo (API) ou de outra forma sem um fuso horário terão o fuso horário da sua empresa como fuso horário padrão até serem reconhecidos no seu app pelo SDK or kit de desenvolvimento de software.
...
A linha do tempo a seguir assume uma definição de Segment or segmento que inclui uma janela de associação limitada por tempo. Neste exemplo, os usuários saem do Segment or segmento 24 horas depois de ingressarem. Esse comportamento de filtro é uma razão pela qual um usuário pode passar na primeira verificação e falhar na segunda.
...
1. O Usuário A entra no Segment or segmento às 6h59 PST (4h59 no horário de Samoa).
2. A Braze verifica a associação ao Segment or segmento às 7h no horário de Samoa para determinar quais usuários são elegíveis para receber a campanha nas próximas 24 horas. O Usuário A está no Segment or segmento neste momento.
3. O Segment or segmento tem uma janela de 24 horas, então o Usuário A sai do Segment or segmento 24 horas após ingressar: 6h59 PST (4h59 no horário de Samoa).
4. A campanha por fuso local é enviada às 7h PST, mas o Usuário A já saiu do Segment or segmento.
...
A Braze recomenda fortemente que todas as campanhas por fuso local sejam agendadas com 24 horas de antecedência. Como essa campanha precisa ser enviada ao longo de um dia inteiro, agendá-la com 24 horas de antecedência garante que sua mensagem alcançará todo o seu Segment or segmento. No entanto, você pode agendar essas campanhas com menos de 24 horas de antecedência, se necessário. Tenha em mente que a Braze não enviará mensagens para usuários que perderam o horário de envio por mais de 1 hora.
...
Os Segments de destino para campanhas por fuso local devem incluir pelo menos uma janela de 48 horas para quaisquer filtros baseados em tempo para garantir a entrega a todo o Segment or segmento. Por exemplo, considere um Segment or segmento que direciona usuários no segundo dia com os seguintes filtros:

- Usou o app pela primeira vez há mais de 1 dia
- Usou o app pela primeira vez há menos de 2 dias

A entrega por fuso local pode perder usuários neste Segment or segmento com base no horário de entrega e no fuso local dos usuários. Isso acontece porque um usuário pode sair do Segment or segmento no momento em que seu fuso horário aciona a entrega.
...
Selecionar a opção **Exportar CSV de endereços de e-mail** baixa dados apenas de usuários com endereços de e-mail. Por exemplo, se você tem um Segment or segmento de 100.000 usuários, mas apenas 50.000 desses usuários têm endereços de e-mail, e você clica em **Exportar CSV de endereços de e-mail**, a exportação conterá apenas 50.000 linhas de dados. Em comparação, selecionar **Exportar CSV de dados de usuários** exporta todos os dados de usuários.

### Posso pesquisar uma campanha pelo seu identificador de API or interface de programação do aplicativo (API)? {#can-i-search-for-a-campaign-by-its-api-identifier}

Sim, use o filtro `api_id:YOUR_API_ID` na página de **Campaigns** para pesquisar uma campanha pelo seu identificador de API or interface de programação do aplicativo (API). Consulte [pesquisando campanhas]({{site.baseurl}}/user_guide/messaging/campaigns/manage_campaigns/search_campaigns) para saber mais.
...
### Qual é a diferença entre campanhas de API or interface de programação do aplicativo (API) e campanhas disparadas por API or interface de programação do aplicativo (API)? {#what-is-the-difference-between-api-campaigns-and-api-triggered-campaigns}

Campanhas disparadas por API or interface de programação do aplicativo (API) permitem que você gerencie o texto da campanha, testes multivariantes e regras de reelegibilidade dentro do dashboard da Braze enquanto dispara a entrega desse conteúdo a partir dos seus próprios servidores e sistemas. Essas mensagens também podem incluir dados adicionais para serem inseridos nos modelos das mensagens em tempo real.

Campanhas de API or interface de programação do aplicativo (API) são usadas para rastrear as mensagens enviadas usando a API or interface de programação do aplicativo (API). Diferentemente da maioria das campanhas, você não especifica a mensagem, os destinatários ou o cronograma, mas sim passa os identificadores nas suas chamadas de API or interface de programação do aplicativo (API).

### Como posso confirmar se meus usuários receberam uma campanha disparada por API or interface de programação do aplicativo (API)? {#how-can-i-confirm-if-my-users-received-an-api-triggered-campaign}

Você pode [criar um Segment or segmento]({{site.baseurl}}/user_guide/audience/segments/creating_a_segment) usando o filtro **Recebeu Campaign** e selecionar a campanha disparada por API or interface de programação do aplicativo (API) específica que deseja verificar. Depois de salvar o Segment or segmento, use o [endpoint `/users/export/segment`]({{site.baseurl}}/api/endpoints/export/user_data/post_users_segment) para exportar os usuários desse Segment or segmento.
...
### Qual é a diferença entre campanhas baseadas em ação e campanhas disparadas por API or interface de programação do aplicativo (API)? {#what-is-the-difference-between-action-based-and-api-triggered-campaigns}
...
#### Disparadas por API or interface de programação do aplicativo (API) {#api-triggered}

Campanhas disparadas por API or interface de programação do aplicativo (API) e disparadas por servidor são ideais para lidar com transações mais avançadas, permitindo que você dispare a entrega do conteúdo da campanha a partir dos seus próprios servidores e sistemas. A requisição de API or interface de programação do aplicativo (API) para disparar a mensagem também pode incluir dados adicionais para serem inseridos no modelo da mensagem em tempo real.

| Benefícios | Considerações |
| ---- | ---- |
| • Não registra pontos de dados<br><br>• Elementos de personalização são incluídos nas propriedades do payload JSON | • Não permite criar um Segment or segmento de usuários elegíveis para a mensagem nas propriedades do payload JSON<br><br>• Não é possível ver payloads JSON recebidos com o **Registro de atividade de mensagens** |
{: .reset-td-br-1 .reset-td-br-2 aria-label="Disparadas por API or interface de programação do aplicativo (API)" }
...
- **Reavaliação de Segment or segmento:** Para campanhas baseadas em ação ou agendadas que reavaliam no momento do envio, usuários que estavam no Segment or segmento quando a campanha foi enfileirada podem não mais se qualificar quando a mensagem é efetivamente enviada.
...
- **Filtros rigorosos de dispositivo ou navegador:** Filtros que correspondem apenas às versões mais recentes de apps ou navegadores reduzem o conjunto contatável no momento do envio em comparação com uma prévia ampla de Segment or segmento.
