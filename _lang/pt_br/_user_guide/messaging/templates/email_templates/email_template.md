...
Após selecionar sua experiência de editor, esta é a sua oportunidade de ser criativo ao personalizar seu modelo de e-mail. Você pode usar HTML para criar e reproduzir sua identidade visual no editor de HTML, ou incluir uma variedade de [detalhes criativos]({{site.baseurl}}/user_guide/channels/email/drag_and_drop) no editor de arrastar e soltar.

### Inclua um link de cancelamento de inscrição {#include-an-unsubscribe-link}

Ao criar seu modelo de e-mail, se você não incluir um link de cancelamento de inscrição, a Braze solicitará que você adicione um ao seu e-mail, pois ele é exigido por lei em todos os e-mails de marketing. Você pode adicionar esse link de cancelamento de inscrição como um rodapé na parte inferior dos seus e-mails usando a Liquid tag {% raw %}``${email_footer}``{% endraw %}, ou [personalizando o rodapé]({{site.baseurl}}/user_guide/channels/email/subscriptions#custom-footer) no seu modelo.

## Etapa 4: Verificar erros de e-mail {#step-4-check-for-email-errors}

Os erros de e-mail são apresentados na guia **Composição** do fluxo de trabalho da mensagem. Os erros impedem que você avance. Os "Alertas" indicam lembretes para ajudar você a seguir as práticas recomendadas. Dependendo do seu negócio, você pode optar por ignorá-los.

![Lista de erros e alertas de um exemplo de e-mail.]({% image_buster /assets/img/dnd_compose_error.png %}){: style="float:right;max-width:40%;margin-left:15px;"}

Veja a lista de erros que são verificados em nosso editor:

- Sintaxe Liquid incorreta
- [Corpos de e-mail maiores que 400 kb; é altamente recomendável que os corpos tenham menos de 102 kb]({{site.baseurl}}/user_guide/channels/email/best_practices)
...
Depois de terminar de compor seu modelo, você pode testá-lo antes de enviá-lo.
...
Em seguida, você pode usar **Copy preview link** para gerar e copiar um link de prévia compartilhável que mostra como o e-mail aparece para um usuário aleatório. Para saber mais, consulte [Prévia compartilhável]({{site.baseurl}}/user_guide/messaging/governance/shareable_preview).
...
Salve seu modelo selecionando **Save Template**. Agora você pode usar esse modelo em qualquer Campaign ou componente do Canvas. Para acessar seu modelo, selecione a experiência de edição com a qual você o criou e, em seguida, selecione-o na lista de modelos disponíveis.
...
Depois que seu modelo for salvo, os usuários poderão ver ícones sobre comentários não resolvidos. Selecione **Resolve** para resolver esses comentários.

![Um comentário em modelo de e-mail que diz "Looks good to me".]({% image_buster /assets/img/email_templates/template_comment.png %})

Para respostas a perguntas frequentes sobre modelos de e-mail, confira nossas [Perguntas frequentes sobre modelos]({{site.baseurl}}/user_guide/messaging/templates/email_templates/faq).