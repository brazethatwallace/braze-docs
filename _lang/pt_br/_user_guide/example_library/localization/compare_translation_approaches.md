...
- **Mensagens multilíngues:** Tags de tradução, uploads de CSV e a API or interface de programação do aplicativo (API) de tradução (acesso antecipado)
- **Parceiros de tradução:** Smartling, Phrase, Lokalise e outros
- **Connected Content:** Strings localizadas obtidas do seu CMS ou API or interface de programação do aplicativo (API) no momento do envio
...
- A Braze não gera traduções. Você fornece o copy por meio do dashboard, CSV, API or interface de programação do aplicativo (API), importação de catálogo, fluxo de trabalho com parceiros ou CMS externo.
- Liquid manual e Content Blocks com condicionais incorporados precisam de convenções de nomenclatura e processos de revisão à medida que os idiomas crescem, e fluxos multi-idioma e de parceiros centralizam atualizações, mas podem exigir manutenção via CSV ou API or interface de programação do aplicativo (API).
- Connected Content e alguns fluxos de parceiros dependem de sistemas externos. Se uma API or interface de programação do aplicativo (API) ou CMS estiver indisponível no momento do envio, o conteúdo localizado pode não ser carregado.
...
| Padrão (sem integração) | Sim | Sim | Parcial — configuração de catálogo necessária | Sim | Não — configuração do fornecedor | Não — API or interface de programação do aplicativo (API) ou CMS necessário |
...
| Esforço contínuo (BAU) | Alto — edições por mensagem | Médio — manutenção de blocos | Médio — atualizações via CSV ou API or interface de programação do aplicativo (API) | Médio — uploads de CSV | Médio — gerenciado na plataforma | Baixo — obtido no momento do envio |
...
6. **Connected Content:** Chame seu CMS ou API or interface de programação do aplicativo (API) de tradução no momento do envio. Teste com cuidado; a prévia pode não refletir as respostas da API or interface de programação do aplicativo (API) em tempo real. Consulte [Connected Content]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/connected_content).
