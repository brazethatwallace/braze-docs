---
nav_title: Vincular usuários do Web SDK entre domínios
article_title: Vincular usuários do Web SDK entre domínios por ID de dispositivo
page_order: 1
page_type: reference
description: "Passe o ID de dispositivo do Braze Web SDK do site de marketing da Kitchenerie para um domínio de loja separado, para que a atividade anônima compartilhe um único perfil de usuário."
---

# Vincular usuários do Web SDK entre domínios por ID de dispositivo {#link-cross-domain-web-sdk-users-through-device-id}
...
1. Lê o ID de dispositivo no domínio de origem com `getDeviceId` após a inicialização do SDK
...
- Recupere o ID de dispositivo somente após o Web SDK ser inicializado no domínio de origem. Chamar `getDeviceId` antes de `initialize` não retorna um valor.
- O Web SDK lê `deviceId` uma única vez em `initialize`. Não existe um `setDeviceId` pós-inicialização que altere o ID de dispositivo ativo. Leia o parâmetro da URL no domínio de destino antes de chamar `initialize`.
...
Consulte [Guia do repositório do Web SDK — Obter ID de dispositivo]({{site.baseurl}}/developer_guide/sdk_repository_guides/web#get-device-id) e [Opções de inicialização — `deviceId`]({{site.baseurl}}/developer_guide/sdk_repository_guides/web#initialization-options).

### Etapa 2: Ler o ID de dispositivo e inicializar o Web SDK no domínio de destino {#step-2-read-the-device-id-and-initialize-the-web-sdk-on-the-destination-domain}
...
- [Guia do repositório do Web SDK]({{site.baseurl}}/developer_guide/sdk_repository_guides/web)
- [Integração multidomínio para o Braze Web SDK]({{site.baseurl}}/developer_guide/platforms/web/multi_domain_integration)
- [Definir IDs de usuário por meio do Braze SDK]({{site.baseurl}}/developer_guide/analytics/setting_user_ids)
...
- [Armazenamento do Web SDK]({{site.baseurl}}/developer_guide/storage)