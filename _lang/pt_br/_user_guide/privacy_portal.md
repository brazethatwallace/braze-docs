---
nav_title: Portal de privacidade
article_title: Portal de privacidade
page_order: 9
layout: dev_guide
search_rank: 3
guide_top_header: "Portal de privacidade da Braze"
guide_top_text: "> Bem-vindo ao Portal de Privacidade da Braze. Aqui você encontrará todos os tipos de informações úteis sobre como a Braze pode ajudá-lo a ser um bom guardião dos dados de seus clientes e, o que é mais importante, ativar medidas para cumprir as regras de proteção de dados relevantes para a sua empresa. Reunimos informações e links para documentação que podem ajudá-lo a usar os Serviços da Braze em conformidade com as leis e os regulamentos de proteção de dados aplicáveis."
guide_top_text2: "<h4>O papel dos clientes</h4><br>Ao usar a Braze, os clientes da Braze são os controladores de dados que determinam quais dados são coletados e compartilhados com os Serviços da Braze. Em relação aos Serviços da Braze, a Braze é uma processadora de dados que processa dados pessoais de acordo com as instruções de nossos clientes. Nossos clientes são responsáveis por usar os Serviços da Braze em conformidade com as leis e regulamentos de proteção de dados que se aplicam a eles."
guide_top_text3: "<h4>Sua parceira em conformidade</h4><br>Como uma boa parceira, nos mantemos informados sobre os últimos desenvolvimentos e práticas em proteção de dados. Por exemplo, oferecemos um Adendo sobre Processamento de Dados que aborda a linguagem do Regulamento Geral de Proteção de Dados da UE e do Reino Unido (\"GDPR\"), a Lei de Privacidade do Consumidor da Califórnia, conforme alterada pela Lei de Direitos de Privacidade da Califórnia (\"CCPA\"), e outras leis estaduais de privacidade dos EUA. <br><br> Os Serviços da Braze incluem recursos que permitem que nossos clientes gerenciem o consentimento do usuário final, processem solicitações de \"não vender ou compartilhar\" e executem direitos do consumidor, como solicitações de exclusão."

alias: "/privacy-portal/"
alias: "/privacy_portal/"
description: ""

guide_featured_title: "Usando a Braze"
guide_featured_list:
- name: Assistência técnica de proteção de dados para solicitações de titulares de dados
  link: /docs/dp-technical-assistance
  image: /assets/img/braze_icons/settings-01.svg
- name: Pixel de abertura de e-mail e rastreamento de cliques
  link: /docs/user_guide/channels/email/email_setup/open_pixel_and_click_tracking
  image: /assets/img/braze_icons/mail-01.svg
- name: Inscrições de e-mail
  link: /docs/user_guide/channels/email/subscriptions
  image: /assets/img/braze_icons/mail-01.svg
- name: Gerenciamento de inscrições de e-mail
  link: /docs/user_guide/channels/email/subscriptions#unsubscribed-email-addresses
  image: /assets/img/braze_icons/users-01.svg
- name: Central de Preferências de e-mail
  link: /docs/user_guide/audience/subscription_preferences/preference_center
  image: /assets/img/braze_icons/list.svg
- name: Grupos de inscrições de SMS
  link: /docs/sms_rcs_subscription_groups
  image: /assets/img/braze_icons/users-01.svg
- name: Leis e regulamentos de SMS
  link: /docs/user_guide/channels/sms_mms_and_rcs/compliance_and_delivery/laws_and_regulations
  image: /assets/img/braze_icons/briefcase-02.svg
- name: Grupos de inscrições de WhatsApp
  link: /docs/user_guide/channels/whatsapp/whatsapp_setup/subscription_groups
  image: /assets/img/braze_icons/whatsapp.svg
- name: Inscrições por push
  link: /docs/user_guide/channels/push/push_setup/push_subscription_states
  image: /assets/img/braze_icons/users-01.svg
- name: Arquivamento de mensagem
  link: /docs/user_guide/data/distribution/export_braze_data/message_archiving
  image: /assets/img/braze_icons/clock-rewind.svg
- name: Coleta de dados padrão e mínima do SDK
  link: /docs/user_guide/data/unification/user_data/sdk_data_collection
  image: /assets/img/braze_icons/settings-01.svg
- name: Considerações sobre privacidade de dados do Canvas Audience Sync
  link: /docs/partners/canvas_audience_sync/overview#data-privacy-considerations
  image: /assets/img/braze_icons/fingerprint-03.svg
- name: Retenção de dados da Braze
  link: /docs/api/data_retention
  image: /assets/img/braze_icons/database-01.svg
- name: Arquivamento de usuários
  link: /docs/user_archival
  image: /assets/img/braze_icons/users-01.svg
- name: Acesso a IPI do usuário
  link: /docs/user_guide/administer/global/admin_settings/security_settings#view-pii
  image: /assets/img/braze_icons/eye.svg
- name: Recursos adicionais de privacidade
  link: https://www.braze.com/resources/search?section%5B%5D=articles&q=privacy
  image: /assets/img/braze_icons/book-closed.svg

guide_menu_title: "Jurídico"
guide_menu_list:
- name: Adendo sobre processamento de dados (PDF)
  link: https://cdn.sanity.io/files/b7pblshe/marketing-prod/278808c2579b16e3d478e52c341890a28d3216ab.pdf
  image: /assets/img/braze_icons/briefcase-02.svg
- name: Perguntas frequentes sobre o DPA
  link: https://www.braze.com/company/legal/dpa
  image: /assets/img/braze_icons/annotation-question.svg
- name: Cláusulas contratuais padrão
  link: https://www.braze.com/company/legal/scc
  image: /assets/img/braze_icons/briefcase-02.svg
- name: Subprocessadores da Braze
  link: https://www.braze.com/company/legal/subprocessors
  image: /assets/img/braze_icons/dataflow-04.svg
- name: Relatório de transparência
  link: https://www.braze.com/company/legal/transparency-report
  image: /assets/img/braze_icons/search-refraction.svg
- name: Perguntas frequentes sobre o GDPR
  link: https://www.braze.com/company/legal/gdpr-faq
  image: /assets/img/braze_icons/annotation-question.svg
- name: Política de uso aceitável
  link: https://www.braze.com/company/legal/aup
  image: /assets/img/braze_icons/thumbs-up.svg
- name: Declaração de inovação
  link: /docs/developer_guide/disclosures/innovation_statement
  image: /assets/img/braze_icons/lightbulb-05.svg
- name: Aviso da estrutura de privacidade de dados
  link: https://www.braze.com/company/legal/data-privacy-framework-notice
  image: /assets/img/braze_icons/book-closed.svg

guide_menu_title2: "Segurança"
guide_menu_list2:
- name: "Folha de dados de segurança, privacidade e arquitetura"
  link: https://www.braze.com/company/legal/security-schedule
  image: /assets/img/braze_icons/lock-01.svg
- name: Certificações de segurança da Braze
  link: /docs/developer_guide/disclosures/security_qualifications
  image: /assets/img/braze_icons/lock-01.svg

---

{% alert important %}
**Isenção de responsabilidade legal**<br>
As informações fornecidas aqui têm apenas fins educacionais e não devem ser interpretadas como aconselhamento jurídico. A Braze incentiva todos os nossos clientes a buscarem seu próprio aconselhamento jurídico sobre o uso da Braze em conformidade com as leis e regulamentos de proteção de dados.
{% endalert %}