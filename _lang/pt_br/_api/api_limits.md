...
## Limites de taxa por tipo de solicitação {#rate-limits-by-request-type}
...
### Solicitações com diferentes limites de taxa {#requests-with-different-rate-limits}
...
- [`/campaigns/trigger/schedule/create`]({{site.baseurl}}/api/endpoints/messaging/schedule_messages/post_schedule_triggered_campaigns) (apenas para chamadas não de transmissão)
...
- [`/canvas/trigger/schedule/create`]({{site.baseurl}}/api/endpoints/messaging/schedule_messages/post_schedule_triggered_canvases) (apenas para chamadas não de transmissão)
...
- A Campaign ou o Canvas sendo acionado (o `campaign_id` ou `canvas_id` na sua solicitação de API, se especificado)
- O público sendo direcionado (os segmentos ou filtros, ou para Campaigns de API, o `segment_id` na sua solicitação de API)
...
## Agrupamento de solicitações de API {#batching-api-requests}
...
### Agrupando solicitações de endpoints de envio de mensagens {#batching-messaging-endpoint-requests}
...
## Monitoramento dos seus limites de taxa {#monitoring-your-rate-limits}
...
### Postergação ideal entre endpoints {#optimal-delay-between-endpoints}
