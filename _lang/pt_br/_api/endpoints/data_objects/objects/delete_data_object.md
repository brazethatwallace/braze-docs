...
Para usar este endpoint, você precisa de uma [chave de API or interface de programação do aplicativo (API)]({{site.baseurl}}/api/basics#rest-api-key-permissions) com a permissão `data_objects.delete`.
...
| `403` | A chave de API or interface de programação do aplicativo (API) não tem permissão ou a solicitação foi bloqueada pela lista de permissões | Confirme que a chave tem a permissão `data_objects.delete` e que o IP de origem está na lista de permissões da chave, se configurada. |
