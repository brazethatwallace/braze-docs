...
> Use este endpoint para listar valores válidos de `rel_kind` para relacionamentos de usuário em um tipo de objeto de dados.

{% alert important %}
Data Objects está atualmente em acesso antecipado. Seu espaço de trabalho deve ser ativado antes que as permissões de chave de API or interface de programação do aplicativo (API) de Data Objects apareçam em **Configurações** > **Chaves de API or interface de programação do aplicativo (API)**.
{% endalert %}

## Pré-requisitos {#prerequisites}

Para usar este endpoint, você precisará de uma [chave de API or interface de programação do aplicativo (API)]({{site.baseurl}}/api/basics#rest-api-key-permissions) com a permissão `data_objects.read`.
...
A tabela a seguir lista e descreve os parâmetros de caminho para o endpoint `/data_objects/types/{type_name}/user_relationship_types`.
...
A tabela a seguir lista e descreve os parâmetros de consulta para o endpoint `/data_objects/types/{type_name}/user_relationship_types`.
...
`display_name` atualmente corresponde a `rel_kind`.
...
| `has_more` | Obrigatório | Booleano | Se outra página de resultados está disponível |
...
| `403` | A chave de API or interface de programação do aplicativo (API) não tem permissão ou a solicitação está bloqueada pela lista de permissões | Confirme se a chave possui `data_objects.read` e se o IP de origem está na lista de permissões da chave, se configurada. |
