---
nav_title: "GET: Listar tipos de objetos de dados"
article_title: "GET: Listar tipos de objetos de dados"
search_tag: Endpoint
page_order: 1
layout: api_page
page_type: reference
description: "Este artigo descreve os detalhes do endpoint Listar tipos de objetos de dados."
---
{% API or interface de programação do aplicativo (API) %}
# Listar tipos de objetos de dados {#list-data-object-types}
{% apimethod get %}
/data_objects/types
{% endapimethod %}

> Use este endpoint para listar os tipos de objetos de dados em um espaço de trabalho.
...
A tabela a seguir lista e descreve os parâmetros de consulta para o endpoint `/data_objects/types`.
...
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 .reset-td-br-4 aria-label="Parâmetros de consulta para listar tipos de objetos de dados" }
...
Use este objeto JSON como referência para os parâmetros de consulta nesta solicitação.
...
Este exemplo lista os tipos de objetos de dados que correspondem ao termo de busca `acc`, retornando dois resultados por página.
...
`metadata.display_name_source` está presente quando um campo de nome de exibição está configurado para o tipo.
...
| Parâmetro | Obrigatório | Tipo de dados | Descrição |
|---|---|---|---|
| `items` | Obrigatório | Array | Lista de registros de tipos de objetos de dados |
| `items[].type_name` | Obrigatório | String | Nome de máquina do tipo de objeto de dados |
...
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 .reset-td-br-4 aria-label="Parâmetros da resposta de listar tipos de objetos de dados" }
...
|---|---|---|
| `400` | Tipo ou valor inválido de parâmetro de consulta | Verifique se `limit` e `offset` são inteiros e se todos os valores dos parâmetros são válidos. |
...
| `403` | A chave de API or interface de programação do aplicativo (API) não tem permissão ou a solicitação está bloqueada pela lista de permissões | Confirme que a chave possui `data_objects.read` e que o IP de origem está na lista de permissões da chave, se configurada. |
| `429` | Limite de frequência excedido | Tente novamente após `X-RateLimit-Reset` e reduza a frequência das solicitações. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="Erros de listar tipos de objetos de dados" }
{% endapi %}