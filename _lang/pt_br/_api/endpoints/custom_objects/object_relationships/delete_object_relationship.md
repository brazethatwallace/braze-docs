...
Para usar este endpoint, você precisa de uma [chave de API]({{site.baseurl}}/api/basics#rest-api-key-permissions) com a permissão `custom_objects.object_relationships.delete`.
...
{% alert note %}
Este endpoint `DELETE` espera um corpo de solicitação JSON. Confirme se seu cliente HTTP envia corpos de solicitação em chamadas `DELETE`.
{% endalert %}
...
| `404` | Relacionamento ou objeto do endpoint não encontrado | Confirme se ambos os objetos existem e se os valores da chave de relacionamento correspondem a uma aresta existente. |
| `401` | Chave da REST API ausente ou inválida | Verifique se o cabeçalho `Authorization` utiliza `Bearer YOUR_REST_API_KEY` e se a chave está ativa. |
| `403` | Chave de API sem permissão ou solicitação bloqueada pela lista de permissões | Confirme se a chave possui `custom_objects.object_relationships.delete` e se o IP de origem está na lista de permissões da chave, caso configurada. |
| `429` | Limite de frequência excedido | Tente novamente após `X-RateLimit-Reset` e reduza a frequência de solicitações. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="Erros ao excluir relacionamento de objeto" }
{% endapi %}