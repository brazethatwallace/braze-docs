...
Para usar esse endpoint, você precisará de uma [chave de API or interface de programação do aplicativo (API)]({{site.baseurl}}/api/basics#rest-api-key-permissions) com a permissão `catalogs.create_fields`.
...
| Parâmetro | Obrigatório | Tipo de dados | Descrição                                                                                                  |
| --------- | -------- | --------- | ------------------------------------------------------------------------------------------------------------ |
| `fields`  | Obrigatório | Array     | Um array que contém objetos de campo. Os objetos de campos devem conter o nome e o tipo dos novos campos. |
...
{% alert note %}
Você deve fornecer os valores do campo de geolocalização como um array `[longitude, latitude]` — por exemplo, `[-73.988103, 40.779109]`. A latitude deve estar entre -90 e 90; a longitude deve estar entre -180 e 180.
{% endalert %}
