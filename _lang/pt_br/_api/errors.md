...
article_title: Erros e respostas da API or interface de programação do aplicativo (API)
description: "Este artigo de referência aborda os vários erros e respostas do servidor que podem surgir ao usar a API or interface de programação do aplicativo (API) da Braze e como solucioná-los."
page_type: reference
page_order: 2.3

---
# Erros e respostas da API or interface de programação do aplicativo (API) {#api-errors-and-responses}

> Este artigo de referência aborda os vários erros e respostas do servidor que podem surgir ao usar a API or interface de programação do aplicativo (API) da Braze e como solucioná-los.
...
| `400 No Recipients` | Não há IDs externos, IDs de Segment or segmento ou tokens por push na solicitação.|
| `400 Invalid Campaign ID` | Nenhuma Campaign de API or interface de programação do aplicativo (API) de envio de mensagens foi encontrada para o ID de Campaign fornecido.|
...
| `401 Unauthorized` | Chave de API or interface de programação do aplicativo (API) inválida. Causas comuns incluem:<br><br>- **Cabeçalho Authorization ausente ou malformado.** O valor do cabeçalho deve ser `Bearer` seguido de um espaço e sua chave de API or interface de programação do aplicativo (API): `Authorization: Bearer YOUR-API-KEY`. Erros comuns incluem omitir `Bearer`, omitir a chave após `Bearer` ou colocar o valor entre aspas.<br>- **Endpoint REST or transferir estado representacional incorreto.** Você está enviando a solicitação para a [instância]({{site.baseurl}}/user_guide/administer/personal/sdk_endpoints) incorreta. Por exemplo, se sua conta está na nossa instância da UE (`https://dashboard-01.braze.eu`), a solicitação deve ser enviada para `https://rest.fra-01.braze.eu`.<br>- **Permissões insuficientes.** Cada chave de API or interface de programação do aplicativo (API) é vinculada a um espaço de trabalho e a um conjunto de permissões específicos. Verifique as permissões da chave em **Configurações** > **Chaves de API or interface de programação do aplicativo (API)** no dashboard.<br>- **Chave de API or interface de programação do aplicativo (API) incorreta.** As chaves de API or interface de programação do aplicativo (API) são específicas do espaço de trabalho. Uma chave de um espaço de trabalho não pode ser usada para autenticar solicitações de um espaço de trabalho diferente. |
| `403 Forbidden` | O plano de tarifação não oferece suporte, ou a conta está inativada.|
| `403 Access Denied` | A chave da REST or transferir estado representacional API or interface de programação do aplicativo (API) que você está usando não tem permissões suficientes. Causas comuns incluem: {::nomarkdown}<ul><li><strong>A chave de API or interface de programação do aplicativo (API) é anterior ao recurso.</strong> Se a chave de API or interface de programação do aplicativo (API) foi criada antes do lançamento de um recurso (como grupos de inscrições ou catálogos), a chave não herda automaticamente essas permissões. Crie uma nova chave de API or interface de programação do aplicativo (API) com as permissões necessárias em <strong>Configurações</strong> &gt; <strong>Chaves de API or interface de programação do aplicativo (API)</strong>.</li><li><strong>Permissão específica do endpoint ausente.</strong> Cada endpoint de API or interface de programação do aplicativo (API) requer um escopo de permissão específico (por exemplo, <code>users.track</code> ou <code>email.status</code>). Verifique se as permissões da chave correspondem ao endpoint que você está chamando.</li><li><strong>Barra final ou erro de digitação na URL.</strong> Por exemplo, <code>/users/track/</code> (com uma barra final) em vez de <code>/users/track</code> pode gerar erros inesperados.</li></ul>{:/}|
...
A Braze retorna HTTP 400 quando o corpo da solicitação não é um JSON válido. Isso se aplica a endpoints REST or transferir estado representacional que aceitam um corpo JSON, como POST, PUT e PATCH.
