...
No início da sessão de um usuário, a Braze entrega todas as mensagens no app elegíveis ao dispositivo dele, enquanto simultaneamente pré-carrega os ativos para minimizar a latência de exibição. Se o evento-gatilho tiver mais de uma mensagem no app elegível, apenas a mensagem com a maior prioridade será entregue. Para saber mais, consulte [Ciclo de vida da sessão]({{site.baseurl}}/developer_guide/analytics/tracking_sessions).

{% alert note %}
As mensagens no app não podem ser disparadas pela API ou por eventos de API&#8212;apenas por eventos personalizados registrados pelo SDK. Para saber mais sobre registro, consulte [Registrando eventos personalizados]({{site.baseurl}}/developer_guide/analytics/logging_events).
{% endalert %}
...
{% tab swift %}
O exemplo a seguir usa lógica personalizada para definir a apresentação de uma mensagem no app com base nos pares de chave-valor em `extras`. Para um exemplo completo de personalização, confira [nosso app de exemplo](https://github.com/braze-inc/braze-swift-sdk/tree/main/Examples).
...
Para um controle mais avançado sobre o momento de exibição das mensagens, incluindo adiar e restaurar mensagens disparadas, consulte nosso [Tutorial: Adiando e restaurando mensagens disparadas]({{site.baseurl}}/developer_guide/in_app_messages/tutorials/deferring_triggered_messages).
{% endtab %}
...
Para um controle mais avançado sobre o momento de exibição das mensagens, incluindo exibir posteriormente e reenfileirar, consulte nossa página [Personalizando mensagens]({{site.baseurl}}/developer_guide/in_app_messages/customization?tab=global%20listener&subtab=kotlin#android_step-2-instruct-braze-to-use-the-custom-manager-listener).
{% endtab %}

{% tab swift %}
1. Implemente o delegate `BrazeInAppMessageUIDelegate` no seu app. Para um passo a passo completo, consulte [Tutorial: In-App Message UI](https://braze-inc.github.io/braze-swift-sdk/tutorials/braze/c1-inappmessageui).
...
#### Etapa 1: Criar um retorno de chamada de push para receber o push silencioso {#step-1-create-a-push-callback-to-receive-the-silent-push}

Registre seu retorno de chamada de push personalizado para escutar uma notificação por push silenciosa específica. Para saber mais, consulte [Configurando notificações por push]({{site.baseurl}}/developer_guide/push_notifications#android_setting-up-push-notifications).
...
Crie uma [Campaign de push silencioso]({{site.baseurl}}/developer_guide/push_notifications/silent?sdktab=android) disparada pelo evento enviado pelo servidor.

![Etapa de entrega de uma Campaign de push silencioso configurada para entrega baseada em ação com um evento personalizado server_event como gatilho.]({% image_buster /assets/img_archive/serverSentPush.png %})
...
#### Etapa 2: Criar uma Campaign de push silencioso {#step-2-create-a-silent-push-campaign}

Crie uma [Campaign de push silencioso]({{site.baseurl}}/developer_guide/push_notifications/silent?sdktab=swift) que seja disparada pelo evento enviado pelo servidor.

![Uma Campaign de mensagem no app com entrega baseada em ação que será entregue a usuários cujos perfis tenham o evento personalizado "server_event".]({% image_buster /assets/img_archive/iosServerSentPush.png %})

A Campaign de push deve incluir extras de pares chave-valor, que indiquem que essa Campaign de push é enviada para registrar um evento personalizado do SDK. Esse evento será usado para disparar a mensagem no app.

![Uma Campaign de mensagem no app com entrega baseada em ação que tem dois pares chave-valor. "CAMPAIGN_NAME" definido como "In-app message name example" e "IS_SERVER_EVENT" definido como "true".]({% image_buster /assets/img_archive/iOSServerPush.png %})
...
![Uma Campaign de mensagem no app com entrega baseada em ação que será entregue a usuários que realizarem o evento personalizado "In-app message trigger" onde "campaign_name" for igual a "IAM Campaign Name Example".]({% image_buster /assets/img_archive/iosIAMeventTrigger.png %})
...
{% tabs %}
{% tab web %}
Para o SDK web, use `braze.showInAppMessage(inAppMessage)` para exibir qualquer mensagem no app. Para detalhes e um exemplo, consulte [Exibindo uma mensagem em tempo real](#displaying-a-message-in-real-time).
{% endtab %}
