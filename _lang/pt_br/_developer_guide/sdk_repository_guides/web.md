---
nav_title: Web SDK or kit de desenvolvimento de software
article_title: Guia do repositório do Web SDK or kit de desenvolvimento de software
page_order: 1
description: "Referência do README do Braze Web SDK or kit de desenvolvimento de software espelhada do GitHub."
---

<!-- BEGIN GENERATED README CONTENT -->
# Guia do repositório do Web SDK or kit de desenvolvimento de software {#web-sdk-repository-guide}

## Sobre o Braze Web SDK or kit de desenvolvimento de software {#about-the-braze-web-sdk}

O Braze Web SDK or kit de desenvolvimento de software permite integrar a plataforma de engajamento do cliente da Braze diretamente nos seus aplicativos web. Desenvolvido com TypeScript e projetado para o desenvolvimento web moderno, este SDK or kit de desenvolvimento de software oferece ferramentas completas para gerenciamento de usuários, envio de mensagens, análise de dados e Feature Flags.
...
Seja para construir um aplicativo de página única, um site de e-commerce ou uma plataforma de conteúdo, o Braze Web SDK or kit de desenvolvimento de software oferece as ferramentas necessárias para criar experiências de usuário personalizadas e envolventes que impulsionam o crescimento e a retenção.

## Pré-requisitos {#prerequisites}

Antes de integrar o Web Braze SDK or kit de desenvolvimento de software, você precisará de:

- **Conta na Braze**: Uma conta na Braze com acesso à API or interface de programação do aplicativo (API)
- **Chave de API or interface de programação do aplicativo (API)**: A chave de API or interface de programação do aplicativo (API) do seu app, disponível no dashboard da Braze
...
2. **endpoint de SDK or kit de desenvolvimento de software or endpoint do SDK or kit de desenvolvimento de software**: Localizado em **Configurações** > **Autenticação do SDK or kit de desenvolvimento de software** > **Endpoints**
...
O snippet a seguir mostra a configuração mínima necessária para inicializar o Braze Web SDK or kit de desenvolvimento de software.
...
|--------|------|---------|-------------|
| `baseUrl` | `string` | **Obrigatório** | Esta opção é obrigatória para configurar o Braze Web SDK or kit de desenvolvimento de software para usar o endpoint apropriado para a sua integração — por exemplo: `braze.initialize('YOUR-API-KEY-HERE', { baseUrl: 'sdk.iad-03.braze.com' })` |
...
| `allowUserSuppliedJavascript` | `boolean` | `false` | Por padrão, o Braze Web SDK or kit de desenvolvimento de software não permite ações de clique em Javascript fornecidas pelo usuário, nem ativa mensagens no app em HTML e Banners, pois esses recursos permitem que os usuários do dashboard da Braze executem Javascript no seu site. Para indicar que você confia nos usuários do dashboard da Braze para escrever ações de clique em Javascript não maliciosas, defina esta propriedade como true. |
...
| `inAppMessageZIndex` | `number` | `999999` | Por padrão, o Braze SDK or kit de desenvolvimento de software exibirá In-App Messages com um z-index de 999999. Forneça um valor para esta opção para substituir esse padrão. |
...
| `contentSecurityNonce` | `string` | `undefined` | Se você fornecer um valor para esta opção, o Braze SDK or kit de desenvolvimento de software adicionará o nonce a quaisquer elementos `<script>` e `<style>` criados pelo SDK or kit de desenvolvimento de software. Isso pode ser usado para permitir que o Braze SDK or kit de desenvolvimento de software funcione com a Content Security Policy do seu site. Além de definir este nonce, pode ser necessário permitir o carregamento do FontAwesome, o que pode ser feito adicionando `use.fontawesome.com` à lista de permissões da sua Content Security Policy ou usando a opção `doNotLoadFontAwesome` e carregando-o manualmente. |
| `noCookies` | `boolean` | `false` | Por padrão, o Braze Web SDK or kit de desenvolvimento de software usa cookies. Para desativar o uso de cookies, defina esta opção como true. A desativação de cookies pode afetar a capacidade do SDK or kit de desenvolvimento de software de lembrar a identidade dos usuários entre sessões. |
| `allowCrawlerActivity` | `boolean` | `false` | Por padrão, o Braze Web SDK or kit de desenvolvimento de software ignora atividades de spiders ou web crawlers conhecidos, como o Google, com base na string de user agent. Isso economiza pontos de dados, torna a análise de dados mais precisa e pode melhorar o ranking da página. No entanto, se você quiser que a Braze registre a atividade desses crawlers, pode definir esta opção como true. |
...
| `enableSdkAuthentication` | `boolean` | `false` | Defina como true para ativar o recurso de autenticação do SDK or kit de desenvolvimento de software. Para saber mais sobre a autenticação do SDK or kit de desenvolvimento de software, consulte nossa documentação do produto. |
| `manageServiceWorkerExternally` | `boolean` | `false` | Por padrão, o Braze Web SDK or kit de desenvolvimento de software gerencia seu próprio service worker para notificações por push. Se você já gerencia um service worker na sua aplicação e deseja incorporar a funcionalidade do service worker da Braze a ele, defina esta opção como true e inclua o código do service worker da Braze no seu arquivo de service worker. |
...
| `serviceWorkerLocation` | `string` | `undefined` | Por padrão, o Braze Web SDK or kit de desenvolvimento de software procurará o arquivo do service worker na raiz do seu domínio. Forneça um valor para esta opção para substituir esse padrão e especificar um local personalizado para o arquivo do service worker. |
...
| `localization` | `string` | `undefined` | Se você fornecer um valor para esta opção, o Braze SDK or kit de desenvolvimento de software tentará exibir mensagens no app e Content Cards no idioma especificado. |
...
| `devicePropertyAllowlist` | `string[]` | `undefined` | Por padrão, o Braze SDK or kit de desenvolvimento de software detecta e coleta automaticamente todas as propriedades de dispositivo em DeviceProperties. Para substituir esse comportamento, forneça um array de DeviceProperties. Para desativar o envio de todas as propriedades aos servidores da Braze, forneça um array vazio. Sem algumas propriedades, nem todos os recursos funcionarão corretamente. Por exemplo, sem o fuso horário, a entrega com base no fuso local não funcionará. |
| `serviceWorkerScope` | `string` | `undefined` | Por padrão, o Braze Web SDK or kit de desenvolvimento de software registrará seu service worker com o escopo padrão (o diretório do service worker). Forneça um valor para esta opção para substituir esse padrão e especificar um escopo personalizado para o service worker. |
...
#### Verificar status do SDK or kit de desenvolvimento de software {#check-sdk-status}
...
#### Ativar/desativar SDK or kit de desenvolvimento de software {#enabledisable-sdk}
...
#### Destruir SDK or kit de desenvolvimento de software {#destroy-sdk}
...
#### Autenticação do SDK or kit de desenvolvimento de software {#sdk-authentication}
...
Se você usa um framework de renderização do lado do servidor (SSR), como o Next.js, pode encontrar erros porque o SDK or kit de desenvolvimento de software foi projetado para ser executado em um ambiente de navegador. Você pode resolver esses problemas importando o SDK or kit de desenvolvimento de software dinamicamente.

Você pode manter os benefícios do tree-shaking ao exportar as partes do SDK or kit de desenvolvimento de software de que precisa em um arquivo separado e, em seguida, importar esse arquivo dinamicamente no seu componente.
...
Como alternativa, se você usa o webpack para empacotar seu app, pode aproveitar os comentários mágicos para importar dinamicamente apenas as partes do SDK or kit de desenvolvimento de software de que precisa.
...
Se você usa o Vite e vê um aviso sobre dependências circulares ou `Uncaught TypeError: Class extends value undefined is not a constructor or null`, pode ser necessário excluir o SDK or kit de desenvolvimento de software da Braze da descoberta de dependências:
...
Ao usar o Jest, você pode ver um erro semelhante a `SyntaxError: Unexpected token 'export'`. Para corrigir isso, ajuste sua configuração no `package.json` para ignorar o SDK or kit de desenvolvimento de software da Braze:
...
5. **Configurar o elemento amp-web-push**: adicione o elemento `amp-web-push` com sua chave de API or interface de programação do aplicativo (API) e URL base como parâmetros de consulta.
...
- **Acesso global**: o SDK or kit de desenvolvimento de software fica disponível como `window.braze` quando carregado via CDN.
...
- **Registro padrão**: por padrão, o SDK or kit de desenvolvimento de software Web da Braze registra e gerencia seu service worker automaticamente quando `requestPushPermission()` é chamado, assim como no início de cada nova sessão para usuários que já concederam permissão de push. Você ainda precisa hospedar um arquivo de service worker no local esperado, contendo o código do service worker da Braze.
...
A tabela a seguir descreve as distribuições disponíveis do Braze Web SDK or kit de desenvolvimento de software.

| Nome | Descrição | npm | URL do CDN
| ---- | ----------- | --- | -------
| Full | SDK or kit de desenvolvimento de software completo com interface. Ao usar a versão npm, os bundlers JavaScript removem código não utilizado, incluindo código de interface. | `@braze/web-sdk` | https://js.appboycdn.com/web-sdk/6.11/braze.min.js
| Core | Contém o SDK or kit de desenvolvimento de software sem interface. Implemente sua própria interface para In-App Messages e Content Cards ao usar esta versão do SDK or kit de desenvolvimento de software. Use a biblioteca completa para a maioria das integrações, pois ela fornece elementos de interface personalizáveis por CSS. | N/A | https://js.appboycdn.com/web-sdk/6.11/braze.core.min.js
| No-AMD | Contém o SDK or kit de desenvolvimento de software completo sem suporte a AMD. Isso é útil se o seu site usa RequireJS ou outro carregador de módulos AMD, mas você prefere carregar o SDK or kit de desenvolvimento de software pelo CDN. | N/A | https://js.appboycdn.com/web-sdk/6.11/braze.no-amd.min.js
...
- [Documentação do SDK or kit de desenvolvimento de software](https://js.appboycdn.com/web-sdk/latest/doc/modules/braze.html)
