...
page_order: 21
page_type: reference
description: "Este artigo aborda os cabeçalhos de política de segurança de conteúdo necessários com o Braze Web SDK or kit de desenvolvimento de software."

---

# Cabeçalhos de política de segurança de conteúdo {#content-security-policy-headers}

> A Content-Security-Policy oferece segurança adicional ao restringir como e onde o conteúdo pode ser carregado em seu site. Este artigo de referência aborda quais cabeçalhos de política de segurança de conteúdo são necessários com o Web SDK or kit de desenvolvimento de software.
...
Se você usar um valor `nonce` nas diretivas `script-src` ou `style-src`, passe esse valor para a opção de inicialização `contentSecurityNonce` para propagá-lo para scripts e estilos recém-criados gerados pelo SDK or kit de desenvolvimento de software:
...
{% alert warning %}
Sua URL deve corresponder ao [endpoint de SDK or kit de desenvolvimento de software or endpoint do SDK or kit de desenvolvimento de software da API or interface de programação do aplicativo (API)]({{site.baseurl}}/user_guide/administer/personal/sdk_endpoints) da sua opção de inicialização `baseUrl` escolhida.
{% endalert %}
...
| `script-src 'unsafe-eval'` | Obrigatória ao usar o snippet de integração que contém referência a `appboyQueue`. Para evitar o uso dessa diretiva, [integre o SDK or kit de desenvolvimento de software usando o NPM]({{site.baseurl}}/developer_guide/platform_integration_guides/web/initial_sdk_setup?tab=package%20manager). |
