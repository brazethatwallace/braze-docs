...
article_title: Integração multidomínio para o SDK da Braze para web
platform: Web
page_order: 23
page_type: reference
description: "Saiba como implementar o SDK da Braze para web em vários domínios, incluindo estratégia de chave de API, configuração de push e comportamento de sessão."
---

# Integração multidomínio {#multi-domain-integration}

> Saiba como integrar o SDK da Braze para web em vários domínios web.

Quando sua implementação abrange vários domínios, os limites de origem do navegador afetam a forma como o SDK da Braze para web armazena e lê o estado do usuário.

## Escolha uma estratégia de app e chave de API {#choose-an-app-and-api-key-strategy}

Você pode usar uma única chave de API do SDK para web em vários domínios, mas, na maioria dos casos, usar chaves de API separadas mapeadas para apps separados no mesmo espaço de trabalho oferece mais controle.
...
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="Opções de estratégia de app e chave de API" }
...
Por padrão, cada domínio raiz armazena seu próprio estado do SDK. Para associar a atividade ao mesmo perfil de usuário da Braze entre domínios:
...
- Mantenha ambos os apps no mesmo espaço de trabalho se estiver usando chaves de API separadas.

Para orientações gerais sobre IDs de usuário, consulte [Definir IDs de usuário pelo SDK da Braze]({{site.baseurl}}/developer_guide/analytics/setting_user_ids?tab=web).
...
Por padrão, o tempo limite de sessão do SDK para web é de 30 minutos de inatividade. Para domínios raiz separados usando um único app/chave de API:
...
Para detalhes sobre o ciclo de vida básico de sessões, consulte [Rastrear sessões pelo SDK da Braze]({{site.baseurl}}/developer_guide/analytics/tracking_sessions?tab=web).