---
nav_title: Outras personalizações do SDK or kit de desenvolvimento de software
article_title: Outras personalizações do SDK or kit de desenvolvimento de software para iOS
platform: iOS
description: "Este artigo de referência aborda a personalização do SDK or kit de desenvolvimento de software, como nível de registro, coleta de IDFA e outras personalizações."
page_order: 3
...
# Outras personalizações do SDK or kit de desenvolvimento de software {#other-sdk-customizations}

## Nível de registro da Braze {#braze-log-level}

O nível de registro padrão para o Braze iOS SDK or kit de desenvolvimento de software é mínimo, ou `8` no gráfico a seguir. Esse nível suprime a maioria dos registros para que nenhuma informação sensível seja registrada em um aplicativo lançado em produção.
...
| 8        | Mínimo. O mínimo de informações será registrado no console do iOS. A configuração padrão do SDK or kit de desenvolvimento de software. |
...
{% alert note %}
Antes do Braze iOS SDK or kit de desenvolvimento de software v4.0.2, a chave do dicionário `Appboy` deve ser usada no lugar de `Braze`.
{% endalert %}
...
{% alert note %}
O nível de registro só pode ser definido em tempo de execução com o Braze iOS SDK or kit de desenvolvimento de software v4.4.0 ou mais recente. Se estiver usando uma versão anterior do SDK or kit de desenvolvimento de software, defina o nível de registro no momento da compilação.
{% endalert %}
...
## Tamanho aproximado do SDK or kit de desenvolvimento de software do iOS {#ios-sdk-size}

O tamanho aproximado do arquivo do framework do iOS SDK or kit de desenvolvimento de software é de 30&nbsp;MB, e o tamanho aproximado do .ipa (adição ao arquivo do app) está entre 1&nbsp;MB e 2&nbsp;MB.

A Braze mede o tamanho do nosso iOS SDK or kit de desenvolvimento de software observando o efeito do SDK or kit de desenvolvimento de software no tamanho do `.ipa`, de acordo com as [recomendações da Apple sobre tamanho de apps](https://developer.apple.com/library/content/qa/qa1795/_index.html). Se estiver calculando a adição de tamanho do iOS SDK or kit de desenvolvimento de software ao seu aplicativo, recomendamos seguir [Obter um relatório de tamanho do app](https://developer.apple.com/library/content/qa/qa1795/_index.html) para comparar a diferença de tamanho no seu `.ipa` antes e depois da integração do Braze iOS SDK or kit de desenvolvimento de software. Ao comparar os tamanhos do relatório de afinamento de apps, também recomendamos analisar os tamanhos de apps para arquivos `.ipa` afinados, pois os arquivos `.ipa` universais serão maiores do que os binários baixados da App Store e instalados nos dispositivos dos usuários.
