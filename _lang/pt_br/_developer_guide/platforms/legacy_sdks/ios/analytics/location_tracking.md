...
{% alert important %}
Para que o monitoramento de localização funcione de forma confiável no iOS 14 para usuários que dão permissão de localização aproximada, é necessário atualizar a versão do SDK or kit de desenvolvimento de software para, pelo menos, `3.26.1`.
{% endalert %}

## Ativação do monitoramento automático de localização

A partir do SDK or kit de desenvolvimento de software da Braze para iOS `v3.17.0`, o monitoramento de localização é desativado por padrão. É possível ativar o monitoramento automático de localização usando o arquivo `Info.plist`. Adicione o dicionário `Braze` ao seu arquivo `Info.plist`. No dicionário `Braze`, adicione a subentrada booleana `EnableAutomaticLocationCollection` e defina o valor como `YES`. Note que, antes do SDK or kit de desenvolvimento de software da Braze para iOS v4.0.2, a chave do dicionário `Appboy` deve ser usada no lugar de `Braze`.
