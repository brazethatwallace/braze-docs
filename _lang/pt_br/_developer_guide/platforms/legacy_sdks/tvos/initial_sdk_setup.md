...
article_title: Configuração inicial do SDK or kit de desenvolvimento de software para tvOS
platform: tvOS
page_order: 0
page_type: reference
description: "Esta página aborda as etapas de configuração inicial do SDK or kit de desenvolvimento de software da Braze para tvOS."
search_rank: 1
noindex: true
---
...
> Este artigo de referência aborda como instalar o SDK or kit de desenvolvimento de software da Braze para tvOS. A instalação do SDK or kit de desenvolvimento de software da Braze fornecerá a funcionalidade básica de análise de dados.

{% alert note %}
Nosso SDK or kit de desenvolvimento de software do tvOS atualmente oferece suporte à funcionalidade de análise de dados. Para adicionar um app para tvOS em seu dashboard, abra um [ticket de suporte]({{site.baseurl}}/user_guide/administer/personal/braze_support).
{% endalert %}

O SDK or kit de desenvolvimento de software da Braze para tvOS deve ser instalado ou atualizado usando o [CocoaPods](http://cocoapods.org/), um gerenciador de dependências para projetos Objective-C e Swift. O CocoaPods oferece mais simplicidade para integração e atualização.

## Integração do SDK or kit de desenvolvimento de software para tvOS com CocoaPods

### Etapa 1: Instale o CocoaPods

A instalação do SDK or kit de desenvolvimento de software pelo [CocoaPods](http://cocoapods.org/) para tvOS automatiza a maior parte do processo de instalação. Antes de iniciar esse processo, verifique se você está usando o [Ruby versão 2.0.0](https://www.ruby-lang.org/en/installation/) ou superior.
...
Sugerimos que você versione a Braze para que as atualizações do pod capturem automaticamente qualquer atualização menor que uma versão minor. A configuração fica assim: `pod 'Appboy-tvOS-SDK' ~> Major.Minor.Build`. Se quiser integrar automaticamente a versão mais recente do SDK or kit de desenvolvimento de software da Braze, mesmo com mudanças maiores, você pode usar `pod 'Appboy-tvOS-SDK'` no seu Podfile.

### Etapa 3: Instalação do SDK or kit de desenvolvimento de software da Braze

Para instalar o CocoaPods do SDK or kit de desenvolvimento de software da Braze, navegue até o diretório do seu projeto Xcode no terminal e execute o seguinte comando:
```
pod install
```
...
{% alert note %}
Desde dezembro de 2019, endpoints personalizados não são mais fornecidos. Se você tem um endpoint personalizado pré-existente, pode continuar a usá-lo. Para saber mais, consulte nossa <a href="{{site.baseurl}}/API or interface de programação do aplicativo (API)/basics#endpoints">lista de endpoints disponíveis</a>.
{% endalert %}
...
### Atualizando o SDK or kit de desenvolvimento de software da Braze via CocoaPods
...
- `YOUR-API-KEY`: A chave de API or interface de programação do aplicativo (API) do seu aplicativo pode ser encontrada em **Manage Settings** no dashboard da Braze.
...
Você também pode integrar nosso SDK or kit de desenvolvimento de software para tvOS manualmente — basta obter o Framework do nosso [repositório público](https://github.com/appboy/appboy-ios-sdk) e inicializar a Braze conforme descrito nas seções anteriores.
