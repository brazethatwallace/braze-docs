...
> Este guia ajuda você a escolher a estratégia de deep linking certa para seu app iOS, dependendo do canal de envio de mensagens que você está usando e se você usa um provedor de links de terceiros, como o Branch or ramificação.
...
- Você está usando um provedor de links de terceiros como Branch or ramificação ou Appsflyer.
...
Essa opção abre uma página web dentro de uma WebView modal no seu app. Ela é gerenciada inteiramente pelo SDK or kit de desenvolvimento de software da Braze usando `Braze.WebViewController` — você não precisa escrever nenhum código de tratamento de URL.
...
| Configuração do SDK or kit de desenvolvimento de software da Braze | Nenhuma — o SDK or kit de desenvolvimento de software abre URLs de esquema personalizado por padrão |
...
| Configuração do SDK or kit de desenvolvimento de software da Braze | Defina `configuration.forwardUniversalLinks = true` |
| BrazeDelegate (opcional) | Implemente `braze(_:shouldOpenURL:)` para roteamento personalizado (por exemplo, Branch or ramificação) |
...
|---|---|
| Arquivo AASA | Não é necessário |
| Método app delegate | Não é necessário — o SDK or kit de desenvolvimento de software gerencia isso automaticamente |
| Configuração do SDK or kit de desenvolvimento de software da Braze | Nenhuma — selecione **Open Web URL Inside App** no criador de Campaign |
...
- Você usa Branch or ramificação, AppsFlyer ou outro provedor de links (porque eles usam seus próprios domínios HTTPS).
...
| `BrazeDelegate.braze(_:shouldOpenURL:)` | Todas as URLs abertas pelo SDK or kit de desenvolvimento de software | Você precisa de lógica de roteamento personalizada (por exemplo, Branch or ramificação, tratamento condicional, análise de dados) |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="Quando você precisa de código no app para lidar com links" }

{% alert tip %}
Se você usar um provedor de links de terceiros, como o Branch or ramificação, implemente `BrazeDelegate.braze(_:shouldOpenURL:)` para interceptar URLs e encaminhá-las para o SDK or kit de desenvolvimento de software do provedor. Consulte [Branch or ramificação para deep linking]({{site.baseurl}}/partners/message_orchestration/deeplinking/branch_for_deeplinking) para um exemplo completo.
{% endalert %}

## Usando o Branch or ramificação com a Braze {#branch}

Se você usar o [Branch or ramificação]({{site.baseurl}}/partners/message_orchestration/deeplinking/branch_for_deeplinking) como seu provedor de links, sua configuração exigirá algumas etapas adicionais além da configuração padrão de link universal:

1. **SDK or kit de desenvolvimento de software do Branch or ramificação**: Integre o SDK or kit de desenvolvimento de software do Branch or ramificação seguindo a [documentação do Branch or ramificação](https://help.branch.io/developers-hub/docs/native-sdks-overview).
2. **Associated Domains**: Adicione seu domínio do Branch or ramificação (por exemplo, `applinks:yourapp.app.link`) no Xcode em **Signing & Capabilities**.
3. **BrazeDelegate**: Implemente `braze(_:shouldOpenURL:)` para encaminhar os links do Branch or ramificação para o SDK or kit de desenvolvimento de software do Branch or ramificação, em vez de permitir que a Braze os processe diretamente.
4. **Encaminhar links universais**: Defina `configuration.forwardUniversalLinks = true` na configuração do SDK or kit de desenvolvimento de software da Braze.

Para detalhes de implementação e orientações sobre depuração, consulte [Branch or ramificação para deep linking]({{site.baseurl}}/partners/message_orchestration/deeplinking/branch_for_deeplinking).