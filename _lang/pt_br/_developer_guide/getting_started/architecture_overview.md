...
Em um nível geral, a Braze trata de dados. A plataforma da Braze, com o SDK or kit de desenvolvimento de software, a REST or transferir estado representacional API or interface de programação do aplicativo (API) e as integrações com parceiros, permite que você agregue e atue em cima dos seus dados.
...
A Braze foi desenvolvida com base em uma arquitetura de fluxo de dados que utiliza Snowflake, Kafka, MongoDB e Redis. Dados de muitas fontes podem ser carregados na Braze por meio do SDK or kit de desenvolvimento de software e da API or interface de programação do aplicativo (API). A plataforma pode lidar com qualquer dado em tempo real, independentemente de como esteja aninhado ou estruturado. Os dados na Braze são armazenados no perfil do usuário.
...
- Eventos personalizados (rastreados pelo SDK or kit de desenvolvimento de software e pela API or interface de programação do aplicativo (API))
...
{% alert important %}
**Considerações sobre remoção de dados:** Eventos personalizados são armazenados no MongoDB e são separados dos dados do Snowflake. Se você precisar remover dados de eventos personalizados errôneos, deve tratá-los no MongoDB. Funcionalidades baseadas em Snowflake (como extensões de Segment or segmento or segmento SQL e outras funcionalidades baseadas em Snowflake) utilizam dados do Snowflake, que são tratados separadamente. Remover dados de um sistema não remove automaticamente do outro.
{% endalert %}

### Fontes de dados de backend por meio da API or interface de programação do aplicativo (API) da Braze {#backend-data-sources-through-the-braze-api}
A Braze pode extrair dados de bancos de dados de usuários, transações off-line e data warehouses por meio da nossa [REST or transferir estado representacional API or interface de programação do aplicativo (API)]({{site.baseurl}}/api/endpoints/user_data).

### Fontes de dados de frontend por meio do SDK or kit de desenvolvimento de software da Braze {#frontend-data-sources-through-braze-sdk}
A Braze captura automaticamente dados primários de fontes de dados de frontend, como dispositivos dos usuários, por meio do [SDK or kit de desenvolvimento de software da Braze]({{site.baseurl}}/user_guide/get_started/sdk_overview). O SDK or kit de desenvolvimento de software lida com novos usuários (anônimos) e gerencia os dados do perfil de usuário durante todo o ciclo de vida.
...
O [SDK or kit de desenvolvimento de software da Braze]({{site.baseurl}}/user_guide/get_started/sdk_overview) possibilita canais adicionais de envio de mensagens: push, mensagens no app e Content Cards. Você integra o SDK or kit de desenvolvimento de software ao seu app ou site para permitir que sua equipe de marketing use o dashboard da Braze para coordenar suas campanhas em todos os canais de envio de mensagens compatíveis.
...
A API or interface de programação do aplicativo (API) da Braze fornece [endpoints]({{site.baseurl}}/api/endpoints/export) que permitem exportar programaticamente análises de dados agregadas, bem como dados individuais de usuários. Esses dados podem ser exportados para públicos e Segments de qualquer tamanho.
...
O anúncio do seu produto foi enviado para todos os clientes que curtiram um item semelhante no seu app. Você definiu esse dado como um evento personalizado. O SDK or kit de desenvolvimento de software rastreou esse evento e segmentou a sua base de usuários de acordo. A Braze orquestrou o melhor horário do dia para enviar esse anúncio e personalizou a mensagem chamando a Mel pelo nome preferido dela.

Quando a Mel abre o anúncio, ela adiciona o novo produto à lista de desejos. A Braze rastreia automaticamente que ela clicou no e-mail. O SDK or kit de desenvolvimento de software rastreia que ela adicionou o novo produto à lista de desejos. Cada vez que interagem com a sua marca, você e seus usuários aprendem mais uns sobre os outros.
