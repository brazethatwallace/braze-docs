...
{% comment %}
Sobre este layout:
Esse layout é para guias de início rápido, guias passo a passo de produtos ou recursos e guias passo a passo de solução de problemas sem um vídeo. Este tutorial terá um sumário que mostra todos os níveis de cabeçalho.
Tutoriais:
Um passo a passo geral de um conceito de instrução. Deve conter CONHECIMENTO PRÁTICO. Concentra-se em um único tópico (por exemplo, como criar uma campanha, como criar um Canvas, etc.) Artigo orientado por metas ou tarefas que orienta PASSO A PASSO a solução de um problema ESPECÍFICO (como direcionar usuários específicos, como segmentar com base no local, etc.).
{% endcomment %}
# Como dar um título a este tutorial {#how-to-title-this-tutorial}
...
{% alert important %}
Certifique-se de informar os usuários sobre coisas importantes à medida que surgirem. Não espere até o final! Seus usuários estarão acompanhando o processo — as pessoas não leem as instruções _antes de_ iniciar o tutorial.
{% endalert %}
