...
page_order: 4
description: "Este artigo de referência aborda como registrar compras na plataforma Windows Universal."
hidden: true
---
...
As compras são registradas usando o `EventLogger`, que é uma propriedade exposta no IAppboy. Para obter uma referência do `EventLogger`, chame `Appboy.SharedInstance.EventLogger`.
...
Se quiser registrar as compras no nível do pedido em vez de no nível do produto, poderá usar o nome do pedido ou a categoria do pedido como `product_id`. Consulte nossa [especificação de objeto de compra]({{site.baseurl}}/api/objects_filters/purchase_object#naming-conventions) para saber mais.

## REST API

Também é possível usar nossa REST API para registrar compras. Consulte a documentação da [API de usuários]({{site.baseurl}}/developer_guide/rest_api/user_data#user-data) para obter detalhes.