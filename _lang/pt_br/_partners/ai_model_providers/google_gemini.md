...
2. Em **Authentication Method**, escolha **API Key** ou **Connect Google Account**.
3. Conclua a configuração do método escolhido:
   - **API Key:** Em **API Type**, selecione **Gemini API** ou **Gemini Enterprise Agent Platform (formerly Vertex AI)**. Insira sua chave de API. Se você selecionou Gemini Enterprise Agent Platform, insira também o **Project ID**. Selecione **Save**.
   - **Connect Google Account:** Selecione **Connect Google Account**, depois selecione **Connect Google** e faça login com sua conta do Google. Selecione seu **GCP Project** no menu suspenso. Se tanto a Gemini API quanto a Gemini Enterprise Agent Platform estiverem ativadas nesse projeto, escolha o **API Type** que a Braze deve usar. Selecione **Save**.
