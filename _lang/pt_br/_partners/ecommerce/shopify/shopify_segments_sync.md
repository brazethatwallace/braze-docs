...
{% alert important %}
A sincronização de segmentos do Shopify está atualmente em beta. Para solicitar acesso, entre em contato com seu CSM.
{% endalert %}
...
### Etapa 1: Selecione um Segment do Shopify para sincronizar {#step-1-select-a-shopify-segment-to-sync}

No Shopify, acesse **Customers** > **Segments** e selecione o Segment que deseja sincronizar com a Braze. Você pode sincronizar qualquer Segment criado usando a segmentação nativa do Shopify, incluindo segmentos baseados em histórico de pedidos, compras de produtos, tags de clientes, gasto vitalício e metafields.
...
1. Na página de detalhes do Segment no Shopify, abra o menu suspenso **Use segment** e selecione **Braze Segment Sync**.
...
{: start="2"}
2. O modal da extensão de ação da Braze é aberto, exibindo o nome do Segment e o tamanho do público. Selecione **Sync with Braze** para iniciar a importação.
...
Na Braze, acesse **Audience** > **Segments** e crie um novo Segment. Em **Add Filter**, selecione o filtro **Cohort Membership** e escolha o Segment sincronizado do Shopify no menu suspenso. Após salvar, você pode referenciar esse Segment da Braze ao direcionar usuários em uma Campaign ou Canvas.

![Criador de segmentos com o filtro "Shopify Cohorts".]({% image_buster /assets/img/shopify/segment_builder_cohort_import.png %})

## Ressincronização de um Segment {#re-syncing-a-segment}

Depois que um Segment é sincronizado, você pode atualizar a associação de coorte a qualquer momento usando a mesma extensão de ação.

1. No Shopify, abra o Segment sincronizado e selecione **Use segment** > **Braze Segment Sync**.
2. No modal, selecione **Sync now**.
3. Na caixa de diálogo de confirmação, selecione **Sync now** para iniciar a ressincronização.

A ressincronização é aditiva: usuários que correspondem ao Segment atual do Shopify são adicionados à coorte, mas os usuários que não correspondem mais permanecem na coorte.
...
Cada Segment sincronizado tem um status de sincronização.

| Status | Descrição |
| --- | --- |
| **Sincronizando** | A Braze está importando os membros do Segment. |
| **Na fila** | O Segment está aguardando um slot de sincronização disponível. |
| **Ativo** | O Segment está sincronizado e as atualizações de membros ocorrem quase em tempo real. |
| **Pausado** | As atualizações de membros para este Segment estão pausadas. |
...
- Segmentos que já estão sendo sincronizados ficam bloqueados. Para remover um Segment, exclua-o na Shopify.

Salve suas alterações para iniciar as sincronizações.

### Pausando um único Segment {#pausing-a-single-segment}

Para pausar a sincronização de um Segment, selecione **Pausar sincronização** na linha correspondente na tabela de segmentos e confirme. Quando a sincronização de um Segment é pausada:
...
- Renomear o Segment na Shopify ainda atualiza o nome de exibição da coorte.
- Excluir o Segment na Shopify ainda encerra o rastreamento.
- Solicitações de **Sincronizar agora** da extensão de ação da Shopify são rejeitadas.

Para retomar, selecione **Retomar sincronização** na linha correspondente. A Braze retoma as atualizações de membros e executa uma sincronização de recuperação. Usuários que saíram do Segment na Shopify enquanto a sincronização estava pausada permanecem na coorte da Braze.
...
- Solicitações de **Sincronizar agora** da extensão de ação são rejeitadas. A extensão continua exibindo o último status conhecido de cada Segment, e selecionar **Sincronizar agora** não inicia uma sincronização.
...
A Braze faz a correspondência dos usuários sincronizados com os perfis de usuário existentes na Braze, independentemente de como esses perfis foram criados, incluindo por meio do preenchimento histórico da Shopify, da sua própria plataforma de dados (como Snowflake ou outro data warehouse) ou de importações diretas pela API. Se a sua coorte for menor do que o seu Segment da Shopify, isso significa que alguns membros do Segment ainda não possuem um perfil correspondente na Braze. Para aumentar a cobertura de correspondência, preencha os perfis de usuário na Braze pelo método de sua preferência antes de realizar a sincronização.
