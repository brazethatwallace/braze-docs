...
1. **Preenchimento inicial:** Quando você sincroniza um Segment or segmento or segmento pela primeira vez, a Braze preenche todos os membros atuais e cria uma coorte correspondente na Braze. O preenchimento é executado de forma assíncrona e pode levar alguns instantes para ser concluído.
...
### Etapa 1: Selecione um Segment or segmento or segmento do Shopify para sincronizar {#step-1-select-a-shopify-segment-to-sync}

No Shopify, acesse **Customers** > **Segments** e selecione o Segment or segmento or segmento que deseja sincronizar com a Braze. Você pode sincronizar qualquer Segment or segmento or segmento criado usando a segmentação nativa do Shopify, incluindo segmentos baseados em histórico de pedidos, compras de produtos, tags de clientes, gasto vitalício e metafields.
...
1. Na página de detalhes do Segment or segmento no Shopify, abra o menu suspenso **Use segment** e selecione **Braze Segment Sync**.
...
{: start="2"}
2. O modal da extensão de ação da Braze é aberto, exibindo o nome do Segment or segmento or segmento e o tamanho do público. Selecione **Sync with Braze** para iniciar a importação.
...
Na Braze, acesse **Audience** > **Segments** e crie um novo Segment or segmento or segmento. Em **Add Filter**, selecione o filtro **Cohort Membership** e escolha o Segment or segmento or segmento sincronizado do Shopify no menu suspenso. Após salvar, você pode referenciar esse Segment or segmento da Braze ao direcionar usuários em uma Campaign ou Canvas.

![Criador de segmentos com o filtro "Shopify Cohorts".]({% image_buster /assets/img/shopify/segment_builder_cohort_import.png %})

## Ressincronização de um Segment or segmento or segmento {#re-syncing-a-segment}

Depois que um Segment or segmento or segmento é sincronizado, você pode atualizar a associação de coorte a qualquer momento usando a mesma extensão de ação.

1. No Shopify, abra o Segment or segmento sincronizado e selecione **Use segment** > **Braze Segment Sync**.
2. No modal, selecione **Sync now**.
3. Na caixa de diálogo de confirmação, selecione **Sync now** para iniciar a ressincronização.

A ressincronização é aditiva: usuários que correspondem ao Segment or segmento or segmento atual do Shopify são adicionados à coorte, mas os usuários que não correspondem mais permanecem na coorte.
...
Cada Segment or segmento or segmento sincronizado tem um status de sincronização.

| Status | Descrição |
| --- | --- |
| **Sincronizando** | A Braze está importando os membros do Segment or segmento or segmento. |
| **Na fila** | O Segment or segmento or segmento está aguardando um slot de sincronização disponível. |
| **Ativo** | O Segment or segmento or segmento está sincronizado e as atualizações de membros ocorrem quase em tempo real. |
| **Pausado** | As atualizações de membros para este Segment or segmento or segmento estão pausadas. |
...
- Segmentos que já estão sendo sincronizados ficam bloqueados. Para remover um Segment or segmento or segmento, exclua-o na Shopify.

Salve suas alterações para iniciar as sincronizações.

### Pausando um único Segment or segmento or segmento {#pausing-a-single-segment}

Para pausar a sincronização de um Segment or segmento or segmento, selecione **Pausar sincronização** na linha correspondente na tabela de segmentos e confirme. Quando a sincronização de um Segment or segmento or segmento é pausada:
...
- Renomear o Segment or segmento or segmento na Shopify ainda atualiza o nome de exibição da coorte.
- Excluir o Segment or segmento or segmento na Shopify ainda encerra o rastreamento.
- Solicitações de **Sincronizar agora** da extensão de ação da Shopify são rejeitadas.

Para retomar, selecione **Retomar sincronização** na linha correspondente. A Braze retoma as atualizações de membros e executa uma sincronização de recuperação. Usuários que saíram do Segment or segmento or segmento na Shopify enquanto a sincronização estava pausada permanecem na coorte da Braze.
...
- Solicitações de **Sincronizar agora** da extensão de ação são rejeitadas. A extensão continua exibindo o último status conhecido de cada Segment or segmento or segmento, e selecionar **Sincronizar agora** não inicia uma sincronização.
...
## Atualizações de Segment or segmento no Shopify {#segment-updates-in-shopify}

### Renomear um Segment or segmento {#renaming-a-segment}

Quando você renomeia um Segment or segmento do Shopify, a Braze atualiza automaticamente o nome de exibição da coorte correspondente. Nenhuma ressincronização é necessária. A Braze também atualiza o nome da coorte enquanto a sincronização de um Segment or segmento está pausada individualmente. Enquanto toda a sincronização de segments está pausada, a Braze atualiza os nomes das coortes quando você retoma.

### Alterar os critérios de um Segment or segmento {#changing-segment-criteria}

A Braze não atualiza automaticamente os membros da coorte quando você altera os critérios de um Segment or segmento do Shopify. Para incluir usuários que passaram a atender aos critérios, faça a ressincronização do Segment or segmento a partir da extensão de ação. Os usuários que não atendem mais aos critérios permanecem na coorte porque a ressincronização não remove membros. Para saber mais, consulte [Ressincronizar um Segment or segmento](#re-syncing-a-segment).
...
A Braze faz a correspondência dos usuários sincronizados com os perfis de usuário existentes na Braze, independentemente de como esses perfis foram criados, incluindo por meio do preenchimento histórico da Shopify, da sua própria plataforma de dados (como Snowflake ou outro data warehouse) ou de importações diretas pela API or interface de programação do aplicativo (API). Se a sua coorte for menor do que o seu Segment or segmento da Shopify, isso significa que alguns membros do Segment or segmento or segmento ainda não possuem um perfil correspondente na Braze. Para aumentar a cobertura de correspondência, preencha os perfis de usuário na Braze pelo método de sua preferência antes de realizar a sincronização.

## Limitações {#limitations}

- **Sincronização unidirecional.** A associação ao Segment or segmento flui apenas do Shopify para a Braze. Alterações na associação à coorte feitas diretamente na Braze não são enviadas de volta ao Shopify.
- **Sem criação de perfil.** Apenas clientes do Shopify que já possuem um perfil de usuário na Braze são adicionados à coorte.
- **Não é possível interromper a sincronização a partir da Braze.** Para interromper a sincronização de um Segment or segmento, exclua-o no Shopify. A coorte e seus membros permanecem na Braze e param de ser atualizados.
- **A ressincronização apenas adiciona membros.** Ressincronizar um Segment or segmento adiciona novos usuários correspondentes à coorte, mas não remove usuários que não fazem mais parte do Segment or segmento no Shopify.