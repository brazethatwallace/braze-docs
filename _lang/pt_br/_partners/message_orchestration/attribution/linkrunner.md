...
| SDK or kit de desenvolvimento de software do Linkrunner | Você deve instalar o [SDK or kit de desenvolvimento de software do Linkrunner](https://docs.linkrunner.io/introduction). |
| SDK or kit de desenvolvimento de software da Braze | Você deve integrar o [SDK or kit de desenvolvimento de software da Braze]({{site.baseurl}}/developer_guide/sdk_integration/). |
...
Se você usa a função `changeUser` do SDK or kit de desenvolvimento de software da Braze, passe o mesmo ID de usuário no parâmetro `userData` da função `signup` do SDK or kit de desenvolvimento de software do Linkrunner.

Se você não usa `changeUser`, passe o `brazeDeviceId` no parâmetro `userData` da função `signup` do SDK or kit de desenvolvimento de software do Linkrunner. Obtenha o `brazeDeviceId` a partir do SDK or kit de desenvolvimento de software da Braze.
...
### Etapa 2: Criar chave de API or interface de programação do aplicativo (API) na Braze {#step-2-create-api-key-in-braze}

No dashboard da Braze, acesse **Configurações** > **Configurações e teste** > **APIs e identificadores** > **Chaves de API or interface de programação do aplicativo (API)**.

1. Selecione **Criar chave de API or interface de programação do aplicativo (API)**.
2. Em **Dados de usuários**, selecione as seguintes permissões:
   - `users.track`
   - `users.export.ids`
3. Salve a chave de API or interface de programação do aplicativo (API).
4. Copie a chave de API or interface de programação do aplicativo (API) e o endpoint REST or transferir estado representacional. Cole esses valores no Linkrunner na próxima etapa. Trate a chave de API or interface de programação do aplicativo (API) como um segredo e não a compartilhe publicamente.
...
3. Insira a chave de API or interface de programação do aplicativo (API) e o endpoint REST or transferir estado representacional que você copiou na Etapa 2.
