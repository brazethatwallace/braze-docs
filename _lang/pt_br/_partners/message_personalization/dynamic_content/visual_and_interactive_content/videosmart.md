...
A VideoSmart fornecerá as credenciais para a autenticação de Conteúdo conectado usada pelo Content Block.

### Etapa 2: Configurar a autenticação de Conteúdo conectado {#step-2-set-up-connected-content-authentication}
...
{% alert note %}
- Sempre forneça valores padrão para os valores usados em `vs_record_data` para que a pré-visualização do e-mail da Braze seja exibida corretamente.
...
- O Conteúdo conectado é executado quando a mensagem é renderizada, então os valores podem diferir entre a pré-visualização e o envio se seus valores padrão ou atributos forem diferentes.
...
### A pré-visualização não está funcionando {#preview-not-working}

Se a pré-visualização da Braze falhar (por exemplo, tentativas repetidas ou erros de autenticação), verifique se:
...
- Quaisquer variáveis obrigatórias (por exemplo, `vs_campaign_id` ou campos obrigatórios em `vs_record_data`) têm valores padrão definidos para a pré-visualização.
