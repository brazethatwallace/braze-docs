...
## Direitos de acesso ao Data Distribution {#data-distribution-entitlements}

Seu direito de acesso ao Data Distribution determina quais tipos de evento estão disponíveis no seu compartilhamento de dados. A Braze organiza os eventos nas seguintes categorias:

| Direito de acesso | Categoria do evento | Descrição | Referência do glossário de eventos |
|------------|----------------|-------------|--------------------------|
| **Engagement Events** | Eventos de engajamento com mensagem | Eventos relacionados a envios, entregas, aberturas, cliques, bounces e outras interações com canais de envio de mensagens | [Eventos de engajamento com mensagem]({{site.baseurl}}/user_guide/data/distribution/braze_currents/event_glossary/message_engagement_events) |
| **Customer Behavior Events** | Eventos de engajamento com mensagem e eventos de comportamento do cliente | Inclui todos os eventos de engajamento com mensagem, além de eventos relacionados a compras, eventos personalizados, sessões, atribuição e ações do usuário no app | [Eventos de engajamento com mensagem]({{site.baseurl}}/user_guide/data/distribution/braze_currents/event_glossary/message_engagement_events), [Comportamento do cliente e eventos de usuário]({{site.baseurl}}/user_guide/data/distribution/braze_currents/event_glossary/customer_behavior_events) |
| **User Profiles and Attributes** | Eventos de engajamento com mensagem, eventos de comportamento do cliente e eventos de perfil de usuário | Inclui eventos de engajamento com mensagem e eventos de comportamento do cliente, além de eventos relacionados a alterações em perfis e atributos de usuário | [Eventos de engajamento com mensagem]({{site.baseurl}}/user_guide/data/distribution/braze_currents/event_glossary/message_engagement_events), [Comportamento do cliente e eventos de usuário]({{site.baseurl}}/user_guide/data/distribution/braze_currents/event_glossary/customer_behavior_events), [Eventos de perfil de usuário]({{site.baseurl}}/user_guide/data/distribution/braze_currents/event_glossary/user_profiles_events) |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 .reset-td-br-4 aria-label="Direitos de acesso ao Data Distribution" }

Para dúvidas sobre quais eventos estão incluídos no seu direito de acesso, entre em contato com seu gerente de conta ou gerente de sucesso do cliente da Braze.

## Sobre o Compartilhamento Seguro de Dados {#about-secure-data-sharing}

Com o compartilhamento de dados, nenhum dado real é copiado ou transferido entre contas. Todo o compartilhamento é feito por meio da camada de serviços e do armazenamento de metadados exclusivos do Snowflake. Esse é um conceito importante porque os dados compartilhados não ocupam espaço de armazenamento na sua conta e, portanto, não contribuem para os custos mensais de armazenamento de dados. Os **únicos** custos são pelos recursos computacionais (como warehouses virtuais) usados para consultar os dados compartilhados.

Além disso, usando os recursos integrados de funções e permissões do Snowflake, o acesso aos dados compartilhados pela Braze pode ser controlado e governado por meio dos controles de acesso já existentes para sua conta do Snowflake e os dados contidos nela. O acesso pode ser restrito e monitorado da mesma forma que seus próprios dados.

- **Reduza o tempo até os insights**<br>Diga adeus aos processos de ETL que levam semanas para serem implementados. As arquiteturas exclusivas da Braze e do Snowflake tornam todos os dados de engajamento do cliente e de Campaign imediatamente acessíveis e consultáveis a partir do momento em que chegam ao data lake. Nenhum dado é copiado ou movido, então você pode entregar experiências do cliente com base apenas nas informações mais relevantes e atualizadas.
- **Elimine os silos de dados**<br>Crie uma visão holística dos seus clientes em todos os canais e plataformas. O compartilhamento de dados facilita mais do que nunca a junção dos dados de engajamento do cliente da Braze com todos os outros dados do Snowflake, gerando insights mais ricos a partir de uma única fonte confiável de verdade.
...
| Permissões do espaço de trabalho da Braze | [Visualizar integrações do Currents]({{site.baseurl}}/user_guide/administer/global/user_management/permissions) para visualizar o compartilhamento de dados. [Editar integrações do Currents]({{site.baseurl}}/user_guide/administer/global/user_management/permissions) para criar, atualizar ou excluir um compartilhamento de dados. |
...
Se você tiver vários espaços de trabalho compartilhando dados para a mesma conta do Snowflake, consulte as [Perguntas frequentes sobre compartilhamento de dados do Snowflake]({{site.baseurl}}/partners/data_and_analytics/data_warehouses/snowflake/faqs) para obter orientações sobre como gerenciar configurações de múltiplos espaços de trabalho.
{% endalert %}

## Uso e visualização {#usage-and-visualization}

Após o compartilhamento de dados ser provisionado, crie um banco de dados a partir do compartilhamento de dados recebido, fazendo com que todas as tabelas compartilhadas apareçam na sua instância do Snowflake e possam ser consultadas como qualquer outro dado armazenado na sua instância. No entanto, lembre-se de que os dados compartilhados são somente leitura e só podem ser consultados, não podendo ser modificados ou excluídos de nenhuma forma.

Assim como o Currents, você pode usar o Snowflake Secure Data Sharing para:

{% multi_lang_include partners/data_sharing_use_cases.md %}

[Baixar os esquemas brutos das tabelas.](/docs/assets/download_file/data-sharing-raw-table-schemas.txt)

{% alert note %}
O download do esquema bruto não inclui visualizações de atributos de perfil de usuário. Para os esquemas completos e orientações de uso de `USER_DEFAULT_ATTRIBUTES_VIEW_SHARED`, `USER_LATEST_STATE_DEFAULT_ATTRIBUTES_VIEW_SHARED` e visualizações relacionadas de atributos de usuário, consulte [Atributos de perfil de usuário]({{site.baseurl}}/partners/data_and_analytics/data_warehouses/snowflake/user_attributes).
{% endalert %}
