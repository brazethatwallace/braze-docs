...
description: "Este artigo de referência descreve a parceria entre a Braze e o Treasure Data, uma plataforma de dados do cliente corporativo que permite escrever os resultados do trabalho diretamente na Braze."
...
* **Mapear IDs externos**: Mapeie os IDs para a conta de usuário da Braze a partir do seu sistema CRM.
...
| Chave da API REST da Braze | Uma chave da API REST da Braze com as permissões `users.track`, `users.delete`, `users.alias.new` e `users.identify`.<br><br>Ela pode ser criada no dashboard da Braze em **Settings** > **API Keys**. |
