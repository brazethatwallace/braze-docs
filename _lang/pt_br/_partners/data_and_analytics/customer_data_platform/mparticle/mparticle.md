...
| ------------------------ | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Chave de API                  | Encontrada no dashboard da Braze em **Configurações** > **Chaves de API**.<br><br>Se você estiver usando a navegação antiga, pode encontrar as chaves de API em **Console de desenvolvedor** > **Configurações da API**. |
...
| ------------ | ----------- |
| Chave do identificador do app Braze | A chave do identificador do seu app Braze pode ser encontrada no dashboard da Braze em **Configurações** > **Chaves de API**. Observe que as chaves de API serão diferentes para cada plataforma (iOS, Android e Web). |
| Tipo de identidade externa | O tipo de identidade de usuário da mParticle a ser encaminhado como ID externo para a Braze. Recomendamos manter o valor padrão, Customer ID |
