...
| Credenciais de API da Braze | As credenciais necessárias dependem do seu [modo de conexão](#connection-modes). O modo cloud usa uma chave da API REST da Braze. O modo dispositivo usa a chave de API do canal web para o SDK da Braze, que é separada da sua chave da API REST. |
...
O Chord oferece suporte ao modo cloud (chamadas de servidor para servidor por meio das REST APIs da Braze) e ao modo de dispositivo (o Chord inicializa o Braze Web SDK e encaminha as chamadas mapeadas). Escolha o modo adequado conforme sua necessidade: recursos completos do Web SDK (por exemplo, mensagens no app) ou apenas encaminhamento de eventos do lado do servidor.
...
Crie a chave da API REST no dashboard da Braze em **Configurações** > **Chaves de API**. Se você usa a navegação antiga, acesse **Console de desenvolvedor** > **API Settings**. A menos que o Chord documente requisitos diferentes para seu espaço de trabalho, a chave precisa das permissões `users.track` e `users.identify`. Para saber mais, consulte [Chaves de API]({{site.baseurl}}/api/basics).
...
{% alert important %}
Se outra ferramenta já envia os mesmos eventos para a Braze, coordene com os responsáveis por essa integração antes de conectar a Braze por meio da Chord CDP. Executar destinos em paralelo pode criar eventos duplicados downstream.
{% endalert %}
...
1. No Chord CDP, confirme que os eventos em tempo real estão chegando das suas fontes.
2. Verifique se o destino da Braze usa a chave de API correta, a versão do SDK (modo de dispositivo) e o endpoint REST ou SDK da sua instância.
3. Confirme que o destino está vinculado à fonte esperada no Chord.
4. No Chord, revise os logs do destino de API ou de funções para chamadas bem-sucedidas a `/users/track` e `/users/identify` e, em seguida, verifique novamente na Braze.
