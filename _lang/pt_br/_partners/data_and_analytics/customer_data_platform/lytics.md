...
> A [Lytics](https://www.lytics.com/) é a CDP or plataforma de dados do cliente or CDP or plataforma de dados do cliente or plataforma de dados do cliente (CDP or plataforma de dados do cliente) preferida para a próxima geração de empresas centradas no cliente. As soluções Lytics Decision Engine, Conductor e Cloud Connect oferecem aos profissionais de marketing e às equipes de dados oportunidades de realizar resolução de identidade, orquestração e otimização de campanhas em tempo real e em conformidade com a privacidade.
...
| Token da API or interface de programação do aplicativo (API) da Lytics | Um token da REST or transferir estado representacional API or interface de programação do aplicativo (API) da Lytics com permissões de gerenciamento de dados. <br><br> Isso pode ser criado no dashboard da Lytics em **Account Settings Console** > **Access Tokens** > **Create New Token**. |
| Chave da API or interface de programação do aplicativo (API) REST or transferir estado representacional da Braze | Uma chave da API or interface de programação do aplicativo (API) REST or transferir estado representacional da Braze com permissão `users.track`. <br><br> Isso pode ser criado no dashboard da Braze em **Configurações** > **Chaves de API or interface de programação do aplicativo (API)**. |
...
No prompt **Configure Authorization** exibido, forneça um rótulo e uma descrição e insira sua chave da API or interface de programação do aplicativo (API) REST or transferir estado representacional e a instância da Braze. Selecione **Complete** quando terminar.
...
#### Etapa 1: Criar um token da API or interface de programação do aplicativo (API) da Lytics {#step-1-create-a-lytics-api-token}

Navegue até o menu da conta da Lytics selecionando o nome da sua conta e selecione **Access Tokens** no menu suspenso. Em seguida, selecione **Create API or interface de programação do aplicativo (API) Token**.
...
Insira um nome, uma descrição opcional e um período de expiração do token. Em seguida, ative o escopo **Data Manager** para permissões de API or interface de programação do aplicativo (API) e selecione **Generate Token**. Copie o token e armazene-o em um local seguro.
...
A URL do webhook da Lytics é usada pela Braze para enviar uma mensagem para a API or interface de programação do aplicativo (API) da Lytics a partir da Braze. Essa mensagem pode ser usada para personalizar suas campanhas na Lytics ou para enriquecer seu perfil de cliente da Lytics. Os dois parâmetros a seguir devem ser adicionados à URL do webhook da Lytics:

- Número da conta da Lytics
- Token da API or interface de programação do aplicativo (API) da Lytics
...
Substitua `<ACCOUNT-NUMBER>` pelo número da sua conta e `<LYTICS-API-TOKEN>` pelo token da API or interface de programação do aplicativo (API) da Lytics.
...
Esta seção descreve como importar dados de usuários da Braze de um Segment or segmento or segmento para a Lytics.
...
#### Etapa 2: Exportar seus dados de Segment or segmento or segmento para CSV {#step-2-export-your-segment-data-to-csv}

Na Braze, navegue até **Público** > **Segments**. Localize o Segment or segmento or segmento que deseja exportar e selecione <i class="fas fa-gear" aria-label="Configurações"></i> e, em seguida, **Exportar dados de usuários em CSV**. É possível exportar até 500.000 usuários em um Segment or segmento or segmento. Para obter detalhes, consulte [Exportação de dados de Segment or segmento or segmento para CSV]({{site.baseurl}}/user_guide/data/distribution/export_braze_data/segment_data_to_csv).
