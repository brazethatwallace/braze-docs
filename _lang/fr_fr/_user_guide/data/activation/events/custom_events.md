...
### Résolution des problèmes d'attributs ou d'événements personnalisés en double {#troubleshooting-duplicate-custom-attributes-or-events}
...
- [.NET MAUI (anciennement Xamarin)]({{site.baseurl}}/developer_guide/analytics?sdktab=xamarin#custom-events)
...
- Résoudre les problèmes expliquant pourquoi un utilisateur est entré ou non dans une Campaign ou un Canvas déclenchés par un événement.
- Examiner un problème de support pour un utilisateur spécifique sans avoir à configurer une exportation de données.
...
Braze enregistre le nombre de fois où les événements personnalisés ont eu lieu et la dernière fois qu'ils ont été effectués par chaque utilisateur à des fins de segmentation. Pour la configuration des rapports, les filtres et les options d'exportation, consultez le [rapport des événements personnalisés]({{site.baseurl}}/user_guide/analytics/reports/custom_events_report).

Sur la page **Custom Events Report**, vous pouvez consulter de manière agrégée la fréquence de chaque événement personnalisé. Les lignes grises superposées sur la série temporelle indiquent la dernière fois qu'une Campaign a été envoyée, ce qui est utile pour visualiser l'impact de vos Campaigns sur l'activité des événements personnalisés.
...
Vous pouvez également utiliser les **filtres** pour ventiler vos événements personnalisés par heure, par utilisateurs actifs mensuels (MAU), par Segments ou par formules de KPI.
