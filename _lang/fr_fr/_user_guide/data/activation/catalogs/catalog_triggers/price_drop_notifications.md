...
# Notifications de baisse de prix {#price-drop-notifications}
...
Lorsqu'un utilisateur déclenche un événement personnalisé pour un article, nous l'abonnons automatiquement aux notifications de baisse de prix pour cet article. Lorsque le prix de l'article correspond à votre règle d'inventaire (par exemple, une baisse supérieure à 50 %), tous les abonnés deviennent éligibles aux notifications via une Campaign ou un Canvas. Cependant, seuls les utilisateurs ayant opté pour les notifications les recevront effectivement.

## Définition d'un événement personnalisé pour les notifications de baisse de prix {#setting-a-custom-event-for-price-drop-notifications}
...
## Mise en place des notifications de baisse de prix {#setting-up-price-drop-notifications}
...
2. Activez le basculeur **Price Drop**.
...
{: start="4"}
4. Sélectionnez **Save** et passez à la section suivante pour configurer les règles de notification.

### Mise en place des règles de notification {#setting-up-notification-rules}

1. Accédez à la page **Settings** de votre catalogue.
2. Pour les **Notification rules**, sélectionnez l'une des options suivantes :<br>

    - **Notify all subscribed users :** Informez tous les clients en attente lorsque le prix de l'article baisse.
    - **Set notification limits :** Notifiez un nombre déterminé de clients selon la période de notification que vous avez configurée. Braze informera le nombre de clients spécifié par incréments jusqu'à ce qu'il n'y ait plus de clients à notifier ou jusqu'à ce que le prix de l'article remonte. Votre taux de notification ne peut pas dépasser 10 000 utilisateurs par minute.<br>

2. Définissez le **Price field in catalog**. Il s'agit du champ du catalogue qui sera utilisé pour déterminer le prix de l'article. Il doit être de type numérique.
3. Définissez la **Price drop rule**. Il s'agit de la logique utilisée pour déterminer si une notification doit être envoyée. Une baisse de prix peut être configurée en pourcentage de variation de prix ou en fonction de la variation de la valeur du champ de prix.
...
## Utiliser les notifications de baisse de prix dans un Canvas {#using-price-drop-notifications-in-a-canvas}
