...
- Les sélections sont idéales pour affiner les recommandations à partir des données de catalogue Braze. Si vous cherchez de l'inspiration, consultez [À propos des recommandations d'articles]({{site.baseurl}}/user_guide/brazeai/item_recommendations) pour des exemples de cas d'usage.

## Filtres de géolocalisation {#geolocation-filters}

Si votre catalogue comprend un [type de champ Géolocalisation]({{site.baseurl}}/user_guide/data/activation/catalogs/create#supported-data-types), vous pouvez utiliser des filtres basés sur la géolocalisation dans vos sélections pour afficher les éléments du catalogue en fonction de leur proximité par rapport à un point géographique.
...
4. Pour **Filter Field**, sélectionnez la colonne du catalogue par laquelle vous souhaitez filtrer. Les champs de type chaîne de caractères contenant plus de 1 000 caractères ne peuvent pas être sélectionnés comme filtres.
5. Terminez la définition de vos critères de filtre en sélectionnant l'opérateur pertinent (par exemple, « égal à » ou « différent de ») et l'attribut.
6. Dans la section **Sort type**, déterminez comment les résultats sont triés. Par défaut, les résultats sont renvoyés sans ordre particulier. Pour spécifier un tri par un champ spécifique, désactivez **Randomize Sort Order** et spécifiez le **Sort Field** et le **Sort Order** (croissant ou décroissant).
...
L'utilisation de Liquid dans les catalogues, comme les attributs personnalisés et les événements personnalisés, peut entraîner des résultats différents pour chaque utilisateur dans votre sélection.
...
## Utiliser les sélections dans les communications {#using-selections-in-messaging}
...
2. Pour **Type de personnalisation**, sélectionnez **Éléments du catalogue**.
3. Sélectionnez le nom de votre catalogue.
4. Pour **Méthode de sélection des éléments**, sélectionnez **Utiliser une sélection**.
...
6. Sélectionnez l'icône **Copier** et collez le Liquid là où il doit apparaître dans votre message.

![La fenêtre modale Ajouter une personnalisation avec les sélections suivantes : « Éléments du catalogue » pour « Type de personnalisation », « Games » pour « Nom du catalogue », « Sélections » pour « Type de sélection », « game_selection » pour « Sélection », et « title » et « description_en » pour « Informations à afficher ».]({% image_buster /assets/img_archive/catalog_selections6.png %}){: style="max-width:70%;"}
...
Imaginons que vous possédez un service de livraison de repas et que vous souhaitez envoyer un message personnalisé à vos utilisateurs qui ont des préférences alimentaires spécifiques basées sur la catégorie de nourriture qu'ils ont consultée le plus récemment.

En utilisant un catalogue contenant les informations de votre service de livraison de repas — nom du repas, prix, image et catégorie du repas — vous pouvez créer une sélection pour recommander trois repas en fonction de la catégorie consultée le plus récemment par un utilisateur.

![Un exemple de sélection pour un service de livraison de repas avec deux filtres : l'un identifiant le type de produit comme un repas, et l'autre identifiant la catégorie comme la plus récemment consultée. La sélection est configurée pour randomiser l'ordre dans lequel les trois résultats sont renvoyés.]({% image_buster /assets/img_archive/catalog_selections2.png %}){: style="max-width:90%;"}

Pour utiliser ce catalogue et cette sélection dans une campagne, utilisez la fenêtre modale **Ajouter une personnalisation** dans la section de composition du message lors de la création d'une campagne. Dans cet exemple, nous avons sélectionné le catalogue contenant les informations de votre service de livraison de repas, ainsi que la sélection de recommandations de repas basées sur la catégorie consultée le plus récemment. Cela nous permet d'afficher le nom et le prix du repas. Pour enrichir davantage votre message, vous pouvez utiliser la sélection pour ajouter également une image du premier repas recommandé.

![Une Content Card avec l'en-tête « You will LOVE these highly rated meals! » et la sélection « recommendations_be_recent_category » dans la section de composition du message.]({% image_buster /assets/img_archive/catalog_selections3.png %}){: style="max-width:90%;"}

Par exemple, supposons que vous ayez un utilisateur dont la catégorie consultée le plus récemment est « Poulet ». En utilisant la personnalisation configurée et une campagne Content Card, vous pouvez envoyer trois recommandations de repas incluant du poulet pour cet utilisateur.

![Une Content Card avec une image de poulet grillé au citron, et une liste de trois recommandations de repas incluant du poulet basées sur la catégorie consultée le plus récemment par l'utilisateur.]({% image_buster /assets/img_archive/catalog_selections4.png %}){: style="max-width:90%;"}

En utilisant la même personnalisation, vous pouvez également envoyer trois recommandations de repas pour un utilisateur dont la catégorie consultée le plus récemment est « Bœuf ».

![Une Content Card avec une image de stroganoff de bœuf, et une liste de deux recommandations de repas incluant du bœuf basées sur la catégorie consultée le plus récemment par l'utilisateur.]({% image_buster /assets/img_archive/catalog_selections5.png %}){: style="max-width:90%;"}