...
4. Dans le champ **Filtre**, sélectionnez la colonne du catalogue sur laquelle vous souhaitez filtrer. Les champs de type chaîne de caractères de plus de 1 000 caractères ne peuvent pas être sélectionnés pour les filtres.
5. Terminez la définition de vos critères de filtrage en sélectionnant l'opérateur approprié (par exemple, « est égal à » ou « n'est pas égal à ») et l'attribut.
6. Dans la section **Type de tri**, déterminez comment les résultats sont triés. Par défaut, les résultats sont renvoyés sans ordre particulier. Pour spécifier un tri sur un champ spécifique, désactivez l'option **Ordre de tri aléatoire** et précisez le **champ de tri** et l'**ordre de tri** (croissant ou décroissant).
7. Dans la section **Limite des résultats**, saisissez le nombre de résultats (jusqu'à 50).
8. Sélectionnez **Créer une sélection**.

### Test et prévisualisation

Après avoir créé une sélection, vous pouvez utiliser la section **Prévisualisation pour l'utilisateur** pour voir ce qu'une sélection renverrait pour un utilisateur aléatoire ou un utilisateur spécifique. Pour les sélections qui utilisent la personnalisation, vous ne pouvez voir la prévisualisation qu'après avoir sélectionné un utilisateur.

### Liquid dans les résultats de la sélection
...
## Utiliser des sélections dans l'envoi de messages
...
Par exemple, imaginons un utilisateur dont la catégorie consultée le plus récemment est « Poulet ». En utilisant la personnalisation définie et une campagne de cartes de contenu, vous pouvez envoyer trois recommandations de repas incluant du poulet à cet utilisateur.
