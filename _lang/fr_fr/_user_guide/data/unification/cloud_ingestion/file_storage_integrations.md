...
{% alert important %}
La création d'une notification Cloud Storage vers Pub/Sub n'est pas disponible dans la console Google Cloud. Vous devez utiliser gcloud (illustré ici), Terraform ou l'API JSON. Pour en savoir plus, consultez [Configure Pub/Sub notifications for Cloud Storage](https://cloud.google.com/storage/docs/reporting-changes#enabling) dans la documentation Google Cloud.
{% endalert %}

Commencez par accorder à l'agent de service Cloud Storage l'autorisation de publier sur le sujet, puis créez la notification pour `OBJECT_FINALIZE`. L'événement `OBJECT_FINALIZE` se déclenche chaque fois qu'un nouvel objet est créé ou finalisé dans le compartiment.
