...
## Pourquoi ai-je reçu un e-mail : « Erreur dans la synchronisation CDI » ? {#why-was-i-emailed-error-in-cdi-sync}
...
Cela peut signifier que les identifiants dans CDI sont incorrects ou mal configurés dans l'entrepôt de données. Pour plus d'informations, consultez [Intégrations d'entrepôts de données]({{site.baseurl}}/user_guide/data/unification/cloud_ingestion/integrations).
...
## Pourquoi ai-je reçu un e-mail intitulé « Row errors in your CDI sync » ? {#why-was-i-emailed-row-errors-in-your-cdi-sync}
...
## Comment corriger l'erreur « Time must be string in ISO8601 Format » dans la configuration CDI ? {#how-do-i-fix-time-must-be-string-in-iso8601-format-in-cdi-setup}
...
Pour connaître l'ensemble des exigences relatives aux payloads, consultez [Configuration des tables pour l'ingestion de données cloud]({{site.baseurl}}/user_guide/data/unification/cloud_ingestion/table_setup).
...
Essayez d'ajouter les adresses IP officielles de Braze à votre liste d'autorisation IP. Pour plus d'informations, consultez [Intégrations d'entrepôts de données]({{site.baseurl}}/user_guide/data/unification/cloud_ingestion/integrations), ou autorisez les adresses IP pertinentes :
...
## Comment mettre à jour mes préférences d'alerte e-mail pour les intégrations CDI ? {#how-do-i-update-my-email-alert-preferences-for-cdi-integrations}
...
## Pourquoi une synchronisation CDI avec un petit nombre de lignes peut-elle encore prendre plusieurs minutes ? {#why-can-a-cdi-sync-with-a-small-number-of-rows-still-take-several-minutes}
