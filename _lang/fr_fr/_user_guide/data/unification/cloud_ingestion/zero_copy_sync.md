...
# Personnalisation sans copie à l'aide de CDI {#zero-copy-personalization-using-cdi}
...
### Utilisation des déclencheurs Canvas {#using-canvas-triggers}

#### Étape 1 : Configurer la source de données pour les déclencheurs Canvas {#step-1-set-up-data-source-for-canvas-triggers}

{% tabs %}
{% tab Snowflake %}

##### Étape 1.1 : Configurer votre table source dans Snowflake {#step-11-set-up-your-source-table-in-snowflake}
...
##### Étape 1.2 : Configurer les informations d'identification {#step-12-set-up-credentials}
...
##### Étape 1.1 : Configurer votre table source dans Redshift {#step-11-set-up-your-source-table-in-redshift}
...
##### Étape 1.1 : Créer un nouveau projet ou un nouvel ensemble de données pour votre table source (facultatif) {#step-11-create-a-new-project-or-dataset-for-your-source-table-optional}

```sql
CREATE SCHEMA BRAZE-CLOUD-PRODUCTION.INGESTION;
```

##### Étape 1.2 : Configurer votre table source dans BigQuery {#step-12-set-up-your-source-table-in-bigquery}
...
##### Étape 1.3 : Configurer les informations d'identification {#step-13-set-up-credentials}
...
##### Étape 1.1 : Créer un catalogue ou un schéma pour votre table source {#step-11-create-a-catalog-or-schema-for-your-source-table}

```sql
CREATE SCHEMA BRAZE-CLOUD-PRODUCTION.INGESTION;
```

#### Étape 1.2 : Configurer votre table source dans Databricks {#step-12-set-up-your-source-table-in-databricks}
...
##### Étape 1.1 : Configurer votre table source dans Fabric {#step-11-set-up-your-source-table-in-fabric}
...
#### Étape 3 : Créer votre synchronisation sans copie {#step-3-create-your-zero-copy-sync}
...
* Les résultats de la table sont temporairement stockés dans Braze pendant le traitement. Ils sont également exportés vers Snowflake pendant 30 jours afin que vous puissiez voir exactement ce qui a été synchronisé.
* Les messages archivés ne sont enregistrés nulle part dans Braze. Les copies sont envoyées pour être stockées exclusivement dans votre espace de stockage configuré.
* Lorsque vous utilisez le CDI sans copie avec les déclencheurs Canvas, Braze ne conserve pas de sauvegarde des résultats de requête provenant de l'entrepôt de données et aucune donnée n'est copiée dans le profil utilisateur.
* Les propriétés de contexte Canvas peuvent être enregistrées dans les systèmes internes pendant une durée maximale de 30 jours.