...
- Gérer des cas d'usage de données plus complexes avec SQL

## Prérequis et limitations {#prerequisites-and-limitations}
...
{% alert note %}
Braze n'exécute que des requêtes en lecture seule sur vos données et ne modifie pas vos tables sous-jacentes. Des objets temporaires peuvent être créés pendant l'exécution de la requête, mais ils ne sont pas conservés.
{% endalert %}

## Créer une nouvelle synchronisation avec SQL Editor {#create-a-new-sql-editor-sync}

Suivez ces étapes pour créer d'abord une source, puis une synchronisation avec SQL Editor. Si vous avez déjà configuré une source pour CDI, vous pouvez passer directement à l'étape 3.

{% alert note %}
Notez que ces étapes utilisent une source Snowflake comme exemple. Le processus de configuration pour d'autres sources d'entrepôt de données est similaire et se trouve dans [Étape 2 : Créer une nouvelle source dans le tableau de bord de Braze]({{site.baseurl}}/user_guide/data/unification/cloud_ingestion/integrations#step-2-create-a-new-source-in-the-braze-dashboard) de la documentation [Configuration des intégrations d'entrepôt de données]({{site.baseurl}}/user_guide/data/unification/cloud_ingestion/integrations#setting-up-data-warehouse-integrations).
{% endalert %}

### Étape 1 : Configurer votre rôle, vos permissions, votre entrepôt et votre utilisateur Snowflake {#step-1-set-up-your-snowflake-role-permissions-warehouse-and-user}
...
#### Étape 1.2 : Configurer le rôle et les permissions de base de données {#step-12-set-up-role-and-database-permissions}
...
Vous pouvez également accorder l'accès à plusieurs tables ou à des tables futures, selon votre cas d'usage. Par exemple, pour accorder l'accès à toutes les futures tables d'un schéma :
...
{% alert note %}
L'entrepôt doit avoir l'option de reprise automatique activée. Si ce n'est pas le cas, accordez à Braze des privilèges `OPERATE` supplémentaires sur l'entrepôt afin que Braze puisse l'activer lors de l'exécution de la requête.
{% endalert %}
...
Vous utiliserez cet utilisateur lors de la configuration de votre source Snowflake dans Braze.
...
{% alert note %}
Pour le champ **Snowflake Account Locator**, saisissez votre [identifiant de compte](https://docs.snowflake.com/en/user-guide/admin-account-identifier) Snowflake, qui suit généralement un format tel que `xy12345.us-east-1.aws`. Il ne s'agit pas d'un nom de base de données ni d'un nom d'entrepôt.
{% endalert %}
...
Pour plus d'informations, consultez [Authentification par paire de clés Snowflake](https://docs.snowflake.com/en/user-guide/key-pair-auth). Si vous souhaitez effectuer une rotation des clés à tout moment, Braze peut générer une nouvelle paire de clés et fournir la nouvelle clé publique.
...
3. Choisissez une synchronisation sous **Data Type**.
4. Référencez la source de l'étape 2.
5. Sélectionnez **SQL** et écrivez une requête SQL qui renvoie les données utilisateur depuis votre entrepôt. Votre requête SQL définit les données qui se synchronisent vers Braze. Le résultat de la requête devient le schéma de votre synchronisation.

Vous pouvez utiliser le Source Explorer pour parcourir les tables et vues disponibles à synchroniser, ou le générateur SQL par IA pour obtenir l'aide de Braze Operator sur votre requête SQL.
...
Pour que la validation réussisse, votre requête SQL doit renvoyer différentes colonnes obligatoires :

| Type de données de synchronisation | Colonnes obligatoires |
|---|---|
| Attributs | - Un identifiant utilisateur, parmi `external_id`, `braze_id`, `alias_name` et `alias_label`, e-mail ou numéro de téléphone.<br>- `UPDATED_AT`.<br>- Au moins une colonne supplémentaire (attribut) à synchroniser. |
| Suppression d'utilisateurs | - Un identifiant utilisateur, parmi `external_id`, `braze_id`, `alias_name` et `alias_label`, e-mail ou numéro de téléphone.<br>- `UPDATED_AT`. |
| Déclencheurs Canvas | - Un identifiant utilisateur, parmi `external_id`, `braze_id`, `alias_name` et `alias_label`, e-mail ou numéro de téléphone.<br>- `UPDATED_AT`. |
| Événements personnalisés | - Un identifiant utilisateur, parmi `external_id`, `braze_id`, `alias_name` et `alias_label`, e-mail ou numéro de téléphone.<br>- `UPDATED_AT`.<br>- `NAME` pour représenter le nom de l'événement.<br>- `TIME` pour représenter l'heure de l'événement. Si indisponible, CDI utilise `UPDATED_AT` comme substitut. |
| Événements d'achat | - Un identifiant utilisateur, parmi `external_id`, `braze_id`, `alias_name` et `alias_label`, e-mail ou numéro de téléphone.<br>- `UPDATED_AT`.<br>- `PRODUCT_ID`.<br>- `CURRENCY`.<br>- `PRICE`.<br>- `TIME` pour représenter l'heure de l'événement d'achat. Si indisponible, CDI utilise `UPDATED_AT` comme substitut. |
...
Les colonnes supplémentaires en dehors des colonnes obligatoires sont synchronisées respectivement en tant qu'attributs, propriétés de contexte Canvas, propriétés d'événement, champs de catalogue et champs de compte. Consultez [Comportement de validation](#validation-behavior) et [Résolution des problèmes](#troubleshooting) pour des conseils utiles sur les erreurs de prévisualisation et de validation et comment les corriger.
...
{% alert important %}
Une configuration SQL incorrecte peut entraîner des résultats non souhaités, y compris une surconsommation de points de donnée et des risques opérationnels plus larges. Vous êtes responsable de vous assurer que la logique de votre requête est correcte et devez prévisualiser attentivement tous les résultats avant d'activer une synchronisation.
{% endalert %}
...
## Prise en charge de `PAYLOAD` (ancien système) {#payload-support-legacy}

L'éditeur SQL prend en charge les [tables CDI héritées]({{site.baseurl}}/user_guide/data/unification/cloud_ingestion/integrations?tab=snowflake#step-1-set-up-tables-or-views) dans lesquelles une colonne `PAYLOAD` est présente.
...
- Vous ne pouvez pas enregistrer des modifications non valides
- Les modifications valides prennent effet après l'enregistrement

Si une exécution de synchronisation est déjà en cours, vos modifications prendront effet lors de la prochaine exécution.
