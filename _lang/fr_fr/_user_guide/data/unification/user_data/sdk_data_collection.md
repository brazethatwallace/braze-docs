...
Les indicateurs calculés comprennent des indicateurs par canal (répertoriés dans le [Glossaire des indicateurs de rapport]({{site.baseurl}}/user_guide/analytics/metrics_glossary)) et les attributs suivants.
...
| Received message from campaign with tag | Booléen. Ce filtre cible les utilisateurs selon qu'ils ont reçu ou non une Campaign portant actuellement une étiquette donnée. |
...
| Langue                | Android, iOS, Web | Langue de l'appareil ou du navigateur, déterminée à partir des paramètres régionaux de l'appareil.                                                           | Cet attribut est utilisé pour traduire les messages dans la langue préférée de l'utilisateur. Il est basé sur les paramètres régionaux de l'appareil.                                                                                            |
...
{% alert important %}
Braze bloque les profils utilisateur (« utilisateurs fictifs ») ayant plus de 5 000 000 de sessions, plus de 20 000 noms d'événements personnalisés distincts ou plus de 20 000 noms de produits distincts dans les achats, et cesse d'ingérer toutes les données entrantes pour ce profil, que ce soit depuis les SDK ou la REST API. Pour en savoir plus, consultez la section [Blocage du spam]({{site.baseurl}}/user_archival).
{% endalert %}