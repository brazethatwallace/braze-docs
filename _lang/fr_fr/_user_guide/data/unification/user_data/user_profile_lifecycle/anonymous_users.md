...
## Fusion des utilisateurs anonymes {#merging-anonymous-users}

Il arrive que des profils d'utilisateurs anonymes soient des doublons possédant le même numéro de téléphone ou la même adresse e-mail que d'autres profils utilisateur. L'un des doublons peut même être un profil utilisateur identifié. Ces doublons peuvent être fusionnés en un seul profil utilisateur en utilisant l'[endpoint POST : Fusionner les utilisateurs]({{site.baseurl}}/api/endpoints/user_data/post_users_merge) ou l'un des outils de fusion disponibles sur la plateforme Braze, comme la [fusion basée sur des règles]({{site.baseurl}}/user_guide/audience/manage_audience/merge_duplicate_users#rules-based-merging).

## Recherche d'un utilisateur anonyme {#looking-up-an-anonymous-user}

Comme les utilisateurs anonymes n'ont pas d'`external_id`, vous pouvez utiliser un ID d'appareil pour rechercher un profil spécifique. Les étapes suivantes montrent comment obtenir l'ID d'appareil de l'utilisateur actuel dans votre intégration du SDK Web :

1. Ouvrez les outils de développement de votre navigateur (par exemple, dans Chrome, appuyez sur **Command + Option + J** sur Mac ou **Ctrl + Shift + I** sur Windows).
...
{:start="3"}
3. Sur le tableau de bord de Braze, utilisez la [recherche d'utilisateurs]({{site.baseurl}}/user_guide/audience/manage_audience/user_profiles) pour rechercher l'ID d'appareil renvoyé.

## Cas d'usage {#use-cases}

### Cibler les utilisateurs anonymes dans votre segment {#target-anonymous-users-in-your-segment}

Étant donné que les utilisateurs anonymes n'ont pas d'`external_id`, vous pouvez les cibler en masse en utilisant le filtre de segmentation **L'ID externe utilisateur est vide**. Pour plus de précision, vous pouvez ajouter un attribut personnalisé aux utilisateurs anonymes que vous souhaitez cibler et filtrer en fonction de celui-ci.

Imaginons que vous attribuez l'attribut personnalisé « is_lead_profile » à chaque profil utilisateur anonyme. Vous pourriez cibler ces profils avec l'un ou les deux filtres suivants :

- **L'ID externe utilisateur est vide**
- « is_lead_profile » **est vrai**

![Filtres de segment pour un ID utilisateur externe vide et un attribut personnalisé « is_lead_profile » défini sur vrai.]({% image_buster /assets/img/getting_started/anonymous_users.png %})

### Capturer les données de paiement d'un utilisateur anonyme {#capture-checkout-data-from-an-anonymous-user}

Vous pouvez capturer les données de paiement d'un utilisateur anonyme (ou visiteur invité) en créant un profil utilisateur avec alias pendant le processus de paiement. Lorsqu'un utilisateur anonyme effectue un achat en utilisant un formulaire de capture web, un appel API peut se déclencher pour créer un profil utilisateur avec alias et enregistrer un événement d'achat. Vous pourrez ensuite mettre à jour le profil utilisateur créé via l'API de Braze.

Voici un exemple de payload qui sera généré lorsque le formulaire de capture web est soumis :
