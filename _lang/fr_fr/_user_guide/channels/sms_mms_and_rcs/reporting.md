...
Le profil utilisateur affiche l'état d'abonnement actuel, mais il est possible qu'il ne présente pas un champ unique « SMS désabonné le » à moins que vous ne définissiez un [attribut personnalisé]({{site.baseurl}}/user_guide/data/custom_data/custom_attributes) ou un mécanisme similaire lors du traitement des désabonnements.
...
|--------|------------|--------|
| Envoyé | Une Campaign ou une étape Canvas a été lancée ou déclenchée, et un payload SMS a été envoyé au fournisseur SMS. | Aucun frais |
...
| Rejeté | Le fournisseur SMS a reçu un accusé de rejet indiquant que le message n'a pas été distribué. Cela peut se produire pour plusieurs raisons, notamment le filtrage de contenu par l'opérateur ou la disponibilité de l'appareil de destination. | Facturé |
...
## Rapports sur le repli RCS et SMS {#rcs-and-sms-fallback-reporting}

Pour le comportement des événements de repli SMS pour le RCS (y compris `IS_SMS_FALLBACK=TRUE`), consultez [Comment le repli SMS fonctionne avec les événements et la segmentation]({{site.baseurl}}/user_guide/channels/sms_mms_and_rcs/message_setup/rcs_setup#how-sms-fallback-works-with-events-and-segmentation).

{% alert note %}
Les analyses de campagne du tableau de bord et les exports Snowflake peuvent différer légèrement en termes de timing et d'agrégation. Pour la réconciliation dans l'entrepôt de données, considérez les flux d'événements Snowflake ou Currents comme la source la plus granulaire lorsque les indicateurs ne correspondent pas exactement au tableau de bord.
{% endalert %}
