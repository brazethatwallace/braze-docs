...
{% alert note %}
Les indicateurs de clics du tableau de bord, tels que **Total des clics**, excluent l'activité suspectée de bots. Pour les indicateurs concernés, la segmentation, l'orchestration et les champs de réconciliation Currents (`is_suspected_bot_click`, `suspected_bot_click_reason`), consultez [Filtrage des clics de bots pour les liens SMS/RCS]({{site.baseurl}}/user_guide/channels/sms_mms_and_rcs/message_features_and_optimization/bot_click_filtering).
{% endalert %}
...
Cette requête renvoie les utilisateurs qui se sont désabonnés des communications SMS pour l'espace de travail et le groupe d'abonnement donnés, filtrés pour ne conserver que ceux associés à des Campaigns ou des Canvas.

### Horodatage du désabonnement {#opt-out-timing}

Les événements de mots-clés et de messages entrants dans Currents ou votre entrepôt de données, tels que les horodatages sur [`users.messages.sms.InboundReceive`]({{site.baseurl}}/user_guide/data/distribution/braze_currents/event_glossary/message_engagement_events#sms-inbound-received-events) ou les événements de changement d'état du groupe d'abonnement, constituent la source de référence pour déterminer quand Braze a enregistré le désabonnement.

{% alert note %}
Les horodatages des événements reflètent le moment où Braze a reçu ou traité le message entrant, et pas nécessairement le moment où l'utilisateur a envoyé le SMS ou le moment où un opérateur ou un fournisseur SMS l'a reçu. Si votre analyse considère les désabonnements comme le moment où Braze a traité le parcours de désabonnement entrant, ces horodatages correspondent à cette définition.
{% endalert %}

Le profil utilisateur affiche l'état d'abonnement actuel, mais peut ne pas exposer un champ unique « SMS désabonné le » à moins que vous ne définissiez un [attribut personnalisé]({{site.baseurl}}/user_guide/data/custom_data/custom_attributes) ou un mécanisme similaire lors du traitement des désabonnements.
...
|--------|------------|--------|
| Envoyé | Une Campaign ou une étape du Canvas a été lancée ou déclenchée, et un payload SMS a été envoyé au fournisseur de SMS. | Aucun frais |
| Échec de livraison | Le payload SMS n'a pas pu être envoyé au fournisseur de SMS. Cela peut se produire en raison de files d'attente saturées, de comptes suspendus ou d'erreurs de média (dans le cas des MMS). | Aucun frais |
| Livré | Le fournisseur de SMS a reçu une confirmation de livraison du message de la part de l'opérateur en amont (et, lorsque disponible, de l'appareil de destination). | Facturé |
| Rejeté | Le fournisseur de SMS a reçu un accusé de rejet indiquant que le message n'a pas été livré. Cela peut se produire pour plusieurs raisons, notamment le filtrage de contenu par l'opérateur ou l'indisponibilité de l'appareil de destination. | Facturé |
...
{% alert note %}
**Sends to Carrier** est obsolète pour les nouveaux tableaux de bord. Utilisez **Sent**, **Confirmed Delivery**, **Delivery Failed** et **Rejections** pour les rapports actuels. Consultez le [Glossaire des indicateurs de rapport]({{site.baseurl}}/user_guide/data/report_metrics) pour les définitions.
{% endalert %}

## Réconcilier les *rejets* avec Snowflake ou Currents {#reconcile-rejections-with-snowflake-or-currents}

L'indicateur *Rejets* dans le tableau de bord est un décompte agrégé au niveau de l'espace de travail. Il ne s'agit pas d'une exportation ligne par ligne, ce qui signifie que vous ne pouvez pas toujours faire correspondre chaque rejet à une ligne unique dans Snowflake ou à un seul événement `users.messages.sms.Rejection` dans Currents. Par exemple, si le profil utilisateur a été supprimé avant que Braze n'ait terminé le traitement du rejet pour l'exportation vers l'entrepôt de données, ce rejet n'apparaît pas dans votre table `USERS_MESSAGES_SMS_REJECTION_SHARED` ni dans le payload Currents, alors que les rapports agrégés SMS peuvent toujours refléter le résultat. Pour en savoir plus, consultez la [référence des tables SQL]({{site.baseurl}}/user_guide/audience/segments/segment_extension/sql_segments/sql_segments_tables#sms-message-events-and-deleted-user-profiles) et les [événements de rejet SMS]({{site.baseurl}}/user_guide/data/distribution/braze_currents/event_glossary/message_engagement_events#sms-rejection-events) dans le glossaire des événements Currents.