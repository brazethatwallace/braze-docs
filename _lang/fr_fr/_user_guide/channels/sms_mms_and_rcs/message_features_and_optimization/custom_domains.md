...
- **Transparence accrue :** Recevez des mises à jour en temps réel sur l'état de configuration de votre domaine grâce aux bannières dans le tableau de bord.
...
- Les domaines doivent être achetés, détenus et gérés par vous. Cela peut être fait par l'intermédiaire d'un registraire de domaines, tel que GoDaddy, Amazon Route 53 ou Cloudflare.
...
{% alert important %}
Si vos enregistrements DNS ne sont pas mis à jour dans les 45 jours, le jeton de configuration expire. Relancez la configuration du domaine depuis **Domaines SMS/RCS et applications de messagerie** pour générer de nouveaux enregistrements DNS.
{% endalert %}
...
3. Saisissez le domaine personnalisé que vous avez acheté dans le champ de saisie de l'application, qui utilise notre logique de validation existante pour un formatage correct, puis sélectionnez **Suivant** et **Soumettre**.
...
{: start="4"}
4. Demandez à votre équipe technique (par exemple, ingénierie ou informatique) de mettre à jour votre configuration DNS avec les détails de l'enregistrement DNS Cloudflare qui s'affichent. Votre équipe technique doit mettre à jour vos enregistrements DNS avec ces informations dans un délai de 45 jours.
...
La surveillance en direct vous alerte dans le tableau de bord de Braze si l'un de vos domaines actifs rencontre un problème, afin que vos liens personnalisés restent utilisables. Si vous rencontrez des problèmes, consultez les détails de l'erreur dans l'application ou contactez le [support]({{site.baseurl}}/braze_support) Braze pour obtenir de l'aide.
...
Pour des conseils plus détaillés sur la configuration et la gestion des domaines personnalisés, y compris les étapes de résolution des problèmes et les exigences techniques, [contactez le support]({{site.baseurl}}/braze_support).