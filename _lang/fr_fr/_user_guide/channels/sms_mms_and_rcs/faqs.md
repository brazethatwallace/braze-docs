...
Lorsque plusieurs profils utilisateur partageant le même numéro de téléphone (activé pour les SMS) sont éligibles à une Campaign ou un composant Canvas basé sur une action au même moment, déclenchés par l'événement d'un SMS entrant, Braze dédupliquera les utilisateurs au niveau du composant Canvas. Cela empêchera les utilisateurs de recevoir plus d'un SMS pour un composant Canvas, même si plusieurs utilisateurs partagent le même numéro de téléphone.
...
Pour éviter des mises à jour inutilement volumineuses, Braze mettra à jour un maximum de 100 profils utilisateur partageant un identifiant lorsqu'une mise à jour d'abonnement est effectuée. Si plus de 100 profils utilisateur partagent le même numéro de téléphone, tous les profils ne seront pas mis à jour.

### Pourquoi est-ce que je vois un pic d'abonnements SMS provenant d'une source spécifique ? {#why-do-i-see-a-spike-in-sms-subscriptions-from-a-specific-source}

Si vous observez une augmentation anormalement importante du nombre d'abonnements, notamment en examinant les données de l'endpoint [`/subscription/status/set`]({{site.baseurl}}/api/endpoints/subscription_groups/post_update_user_subscription_group_status) via Currents, cela peut être dû à des profils utilisateur en double.

Lorsqu'une requête est envoyée à l'endpoint `/subscription/status/set` avec uniquement un numéro de téléphone (sans `external_id` fourni), Braze met à jour tous les profils utilisateur partageant ce numéro de téléphone. Si votre espace de travail contient des profils en double, le nombre d'utilisateurs ayant mis à jour leur état d'abonnement est gonflé, même si un seul numéro de téléphone a changé.

Pour analyser les données d'abonnement de manière plus précise lors de l'extraction depuis Currents, mettez à jour votre requête pour compter les numéros de téléphone distincts plutôt que de compter tous les événements de changement d'état d'abonnement.
...
- Calculez soigneusement les caractères de vos SMS en tenant compte du Liquid ou du contenu connecté. Le composeur SMS de Braze dans votre tableau de bord n'estime pas et ne prend pas en compte l'utilisation de ces fonctionnalités.
...
### Si un message est envoyé à une ligne fixe, le message est-il quand même comptabilisé dans mon nombre d'envois SMS ? {#if-a-message-is-sent-to-a-landline-will-the-message-still-count-toward-my-sms-send-count}

Aux États-Unis, au Canada et au Royaume-Uni :
- Si un SMS est envoyé à une ligne fixe, il est marqué comme **Non distribué**. Le comportement de facturation dépend de votre fournisseur de services SMS. Avec Twilio, la tentative de distribution est tout de même facturée, donc les messages marqués comme **Envoyé**, **Distribué** ou **Non distribué** dans vos journaux de messages sont facturés.
- Au Royaume-Uni, certains opérateurs convertissent le SMS en message vocal, distribuant ainsi le message.

Dans les autres pays :
- Avec Twilio, une erreur est générée et vous n'êtes pas facturé pour la tentative d'envoi du SMS.
...
Vous pourriez être facturé pour des segments de message supplémentaires si votre message contient de la personnalisation Liquid. Le rendu des blocs de contenu ne se produit pas tant que le message n'est pas en cours de préparation pour l'envoi. Lorsque vous modifiez un SMS avec un bloc de contenu, Braze ne sait pas ce que le bloc de contenu contiendra, mais fournit une estimation approximative. Nous recommandons aux utilisateurs d'utiliser le panneau de test pour prévisualiser le message afin de mieux comprendre à quoi s'attendre.
...
- Les entreprises utilisent souvent des raccourcisseurs de liens pour limiter l'impact sur le nombre de caractères. Cependant, si vous envoyez un lien raccourci via un code long, les opérateurs peuvent bloquer ou refuser le message, car ils peuvent considérer la redirection du lien comme suspecte.
...
Les mots-clés personnalisés seraient enregistrés comme des événements personnalisés. Vous devriez donc créer des segments basés sur les mots-clés que les clients peuvent envoyer par SMS. Par exemple, si un utilisateur s'abonne aux SMS pour les messages VIP mais pas pour les alertes, vous pouvez créer un segment VIP et un segment d'alertes, puis attribuer l'utilisateur au segment approprié.
...
### Les SMS de test comptent-ils dans les limites ? {#do-test-text-messages-count-toward-limits}
...
Oui. Les utilisateurs doivent disposer d'un numéro de téléphone valide, faire partie du groupe d'abonnement SMS utilisé pour l'envoi de test et avoir au moins un pays sélectionné sous **Geographic Permissions** pour les SMS.
