...
- `myApp://deep-link?page=home&abButtonId=1` - Lien profond mobile avec clic sur le Bouton 2
...
### Ouvrir en tant que lien profond (mobile uniquement) {#open-as-deeplink-mobile-only}
...
## Téléchargement HTML avec prévisualisation {#html-upload-with-preview}
...
![Interaction avec la prévisualisation HTML en faisant défiler les pages.]({% image_buster /assets/img/iam-beta-javascript-preview.gif %})
...
2. Les ressources téléchargées sur Braze peuvent être réutilisées dans plusieurs campagnes.
...
1. La modification incompatible la plus notable avec ce nouveau type de message concerne les exigences du SDK. Les utilisateurs dont le SDK de l'application ne répond pas aux [exigences minimales de version du SDK](#supported-sdk-versions) ne verront pas le message.
2. Le lien profond `braze://close`, qui était auparavant pris en charge sur les applications mobiles, a été supprimé au profit de la méthode JavaScript `brazeBridge.closeMessage()`. Cela permet des messages HTML multiplateformes, car le web ne prend pas en charge les liens profonds.
3. Le suivi automatique des clics, qui utilisait `?abButtonId=0` pour les identifiants de bouton, et le suivi des « clics sur le corps » sur les boutons de fermeture ont été supprimés. Les exemples de code suivants montrent comment modifier votre HTML pour utiliser nos nouvelles méthodes JavaScript de suivi des clics :
