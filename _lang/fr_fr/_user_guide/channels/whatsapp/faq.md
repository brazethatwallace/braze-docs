...
#### Que se passe-t-il si je tente d'envoyer des messages alors que j'ai déjà atteint ma limite de communication ? {#what-happens-if-i-attempt-to-send-messages-when-i-have-already-reached-my-messaging-limit}
...
Si un utilisateur bloque votre entreprise, les messages suivants que vous tentez d'envoyer ne seront pas distribués, et vous ne serez pas facturé. Le statut d'abonnement de l'utilisateur ne sera pas mis à jour.
...
| Modèle utilitaire dans la fenêtre de service | Non facturés par Meta | Meta ne facture pas les modèles utilitaires envoyés dans les 24 heures suivant un message initié par l'utilisateur. La consommation de crédits d'action suit votre contrat. |
