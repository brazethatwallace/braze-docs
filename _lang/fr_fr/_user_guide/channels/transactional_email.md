...
- La [configuration de l'e-mail]({{site.baseurl}}/user_guide/channels/email/email_setup) finalisée, y compris la configuration de l'IP et du domaine, l'authentification et l'IP warming
- Une **clé REST API Braze** avec la permission `transactional.send`

## Cas d'usage {#use-cases}

L'e-mail transactionnel est conçu pour envoyer des messages non promotionnels déclenchés par un service. Les cas d'usage courants comprennent les suivants :

| Cas d'usage | Explication |
| --- | --- |
| Confirmations de commande | Confirmer qu'un achat client a bien été reçu et est en cours de traitement. |
| Réinitialisations de mot de passe | Distribuer des liens sécurisés et sensibles au temps pour permettre aux clients de réinitialiser leurs identifiants de compte. |
...
## En quoi les e-mails transactionnels diffèrent des e-mails marketing {#how-transactional-email-differs-from-marketing-email}
...
- Prennent en charge une distribution en quasi-temps réel pour les contenus urgents

## Étapes suivantes {#next-steps}

{% article_tiles %}
- name: Créer un e-mail transactionnel
