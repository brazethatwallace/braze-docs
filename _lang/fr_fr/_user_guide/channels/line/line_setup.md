...
L'envoi de messages LINE depuis Braze est prélevé sur les crédits de messages ou d'actions de votre compte.

{% alert note %}
**Définition de `native_line_id`** : vous pouvez définir `native_line_id` en envoyant des mises à jour utilisateur à Braze (par exemple, avec l'endpoint [`/users/track`]({{site.baseurl}}/api/endpoints/user_data/post_user_track), l'[importation CSV]({{site.baseurl}}/user_guide/audience/manage_audience/import_users#constructing-your-csv) ou l'[ingestion de données cloud]({{site.baseurl}}/user_guide/data/unification/cloud_ingestion)). Si votre SDK côté client ne dispose pas d'un champ dédié pour `native_line_id`, envoyez-le via des mises à jour utilisateur côté serveur en utilisant l'une de ces méthodes.
{% endalert %}
...
| Compte vérifié | Un compte ayant passé le processus de vérification de LINE Yahoo. Ce compte est représenté par un badge bleu et apparaîtra dans les résultats de recherche de l'application LINE.<br><br>Ce compte n'est disponible que pour les comptes basés au Japon, à Taïwan, en Thaïlande et en Indonésie. |
| Compte premium | Un compte ayant passé le processus de vérification de LINE Yahoo. Ce compte est représenté par un badge vert et apparaîtra dans les résultats de recherche de l'application LINE. Ce type de compte est automatiquement accordé lors de la vérification, à la discrétion de LINE. |
...
Pour configurer des mises à jour utilisateur cohérentes, importer les LINE ID des utilisateurs existants et les synchroniser avec les états d'abonnement LINE :
...
Cette étape est nécessaire si vous avez un utilisateur LINE existant et identifié, car Braze récupérera automatiquement son statut d'abonnement par la suite et mettra à jour le profil utilisateur correspondant. Si vous n'avez pas encore associé vos utilisateurs à leur ID LINE, ignorez cette étape.

Vous pouvez importer ou mettre à jour les utilisateurs en utilisant l'une des méthodes prises en charge par Braze, y compris l'endpoint [`/users/track`]({{site.baseurl}}/api/endpoints/user_data/post_user_track), l'[import CSV]({{site.baseurl}}/user_guide/audience/manage_audience/import_users#constructing-your-csv) ou l'[ingestion de données cloud]({{site.baseurl}}/user_guide/data/unification/cloud_ingestion).

Quelle que soit la méthode utilisée, mettez à jour le champ `native_line_id` pour fournir l'ID LINE de l'utilisateur. Pour en savoir plus sur `native_line_id`, consultez [Configuration de l'utilisateur](#user-setup).

{% alert note %}
Le statut du groupe d'abonnement ne doit pas être spécifié et sera ignoré. LINE est la source de vérité pour le statut d'abonnement de l'utilisateur, qui sera synchronisé avec Braze via l'outil de synchronisation des abonnements ou par les mises à jour d'événements.
{% endalert %}

## Étape 2 : Intégrer le canal LINE {#step-2-integrate-line-channel}

Une fois le processus d'intégration terminé, Braze récupérera automatiquement les abonnés LINE de ce canal dans Braze. Pour tout ID LINE déjà associé à un profil utilisateur Braze, chaque profil sera mis à jour avec le statut « subscribed », et les ID LINE restants généreront des utilisateurs anonymes. De plus, les nouveaux abonnés de votre canal LINE auront des profils utilisateur non identifiés créés lorsqu'ils suivront le canal.
...
1. Dans LINE, accédez à l'onglet **Messaging API** et modifiez vos **Webhook settings** :
   ...
   - Activez **Use webhook** et **Webhook redelivery**. <br><br> ![Page des paramètres de webhook permettant de vérifier ou modifier l'URL du webhook, d'activer ou de désactiver « Use webhook », « Webhook redelivery » et « Error statistics aggregation ».]({% image_buster /assets/img/line/webhook_settings.png %}){: style="max-width:70%;"}
...
{% alert note %}
Vous pouvez mettre à jour ou faire tourner le channel secret et le channel access token pour un canal LINE déjà intégré en accédant à **Partner Integrations** > **Technology Partners** > **LINE** et en sélectionnant votre intégration.
{% endalert %}

{: start="3"}
3. Accédez à votre page **Settings** > **Response settings** et effectuez les actions suivantes :
   - Désactivez **Greeting message**. Cela peut être géré dans Braze en déclenchant au moment de l'abonnement.
...
![Page des paramètres de réponse avec des bascules pour déterminer comment votre compte gère les conversations.]({% image_buster /assets/img/line/response_settings.png %}){: style="max-width:80%;"}

### Étape 2.2 : Générer des groupes d'abonnement LINE dans Braze {#step-22-generate-line-subscription-groups-in-braze}

Braze crée un [groupe d'abonnement]({{site.baseurl}}/user_guide/audience/subscription_preferences/subscription_groups#line-subscription-groups) pour chaque canal LINE que vous intégrez. Pour comprendre le fonctionnement des groupes d'abonnement LINE, consultez [Groupes d'abonnement LINE]({{site.baseurl}}/user_guide/channels/line/message_users/subscription_groups).
...
Si vous souhaitez ajouter une liste blanche d'IP dans votre compte LINE, ajoutez toutes les adresses IP répertoriées pour votre cluster dans [Liste d'autorisation d'IP]({{site.baseurl}}/user_guide/channels/webhooks/create_a_webhook#ip-allowlisting) à votre liste d'autorisation.

{% alert important %}
Lors de l'intégration, veillez à vérifier que votre channel secret est correct. S'il est incorrect, il peut y avoir des incohérences dans le statut d'abonnement.
{% endalert %}
...
{: start="2"}
2. Après la connexion, Braze générera automatiquement un groupe d'abonnement Braze pour chaque intégration LINE ajoutée avec succès à votre espace de travail. <br><br> Toute modification de votre liste d'abonnés (comme de nouveaux abonnés ou des désabonnements) sera automatiquement transmise dans Braze.
...
En supposant que vous disposez déjà d'une méthode pour fournir des mises à jour d'utilisateurs à Braze, vous devrez la modifier pour inclure le nouveau champ `native_line_id` afin que les mises à jour d'utilisateurs ultérieures envoyées à Braze incluent ce champ.

Des profils utilisateur non identifiés possédant un `native_line_id` peuvent exister dans Braze, créés dans le cadre du processus de synchronisation du statut d'abonnement, ou lorsqu'un nouvel abonné a suivi votre canal.

Lorsqu'un utilisateur LINE est identifié dans votre application via la [réconciliation des utilisateurs](#user-id-reconciliation) ou par d'autres moyens, vous pouvez cibler un profil utilisateur potentiellement non identifié dans Braze en utilisant l'endpoint [`/users/identify`]({{site.baseurl}}/api/endpoints/user_data/post_user_identify). Chaque profil utilisateur non identifié possédant un `native_line_id` dispose également d'un alias d'utilisateur `line_id` qui peut être utilisé pour cibler le profil utilisateur à identifier.
...
Voici un exemple de payload pour `/users/track` qui met à jour un profil utilisateur par l'ID externe de l'utilisateur pour ajouter un `native_line_id` :
...
## Configuration utilisateur {#user-setup}

LINE est la source de vérité pour les états d'abonnement des utilisateurs. Même si vous disposez de l'ID LINE d'un utilisateur (`native_line_id`), si cet utilisateur n'a pas suivi le canal LINE depuis lequel vous envoyez, LINE ne distribuera pas les messages à cet utilisateur.

Pour faciliter cette gestion, Braze propose des outils et une logique qui prennent en charge une base d'utilisateurs bien intégrée, notamment la synchronisation des abonnements et les mises à jour d'événements pour les abonnements et désabonnements LINE.

### Synchronisation des abonnements et logique d'événements {#subscription-syncing-and-event-logic}

Pour comprendre comment l'outil de synchronisation des abonnements et les mises à jour d'événements d'abonnement et de désabonnement maintiennent le statut d'abonnement LINE aligné avec Braze, consultez [Statut d'abonnement]({{site.baseurl}}/user_guide/audience/subscription_preferences/subscription_status#line).

## Réintégrer un canal LINE dans un autre espace de travail {#re-integrate-a-line-channel-in-another-workspace}

Pour utiliser un canal LINE dans un autre espace de travail Braze :

1. Dans l'espace de travail d'origine, archivez le groupe d'abonnement associé à ce canal.
...
Vérifiez que vous disposez de la permission [Gérer les groupes d'abonnement]({{site.baseurl}}/user_guide/administer/global/user_management/permissions#list-of-permissions) dans les deux espaces de travail. Sans les permissions nécessaires dans les deux espaces de travail, l'intégration échoue avec une erreur indiquant que le canal est déjà connecté.

Pour en savoir plus sur l'impact de l'archivage sur les groupes d'abonnement, consultez [Groupes d'abonnement LINE]({{site.baseurl}}/line/subscription_groups#archive-behavior).
...
3. Si un changement de statut d'abonnement survient (par exemple, l'utilisateur bloque, supprime ou suit à nouveau le canal), Braze reçoit la mise à jour de LINE et met à jour le profil utilisateur avec le `native_line_id` en conséquence.

### Un profil utilisateur existant a bloqué, supprimé ou cessé de suivre le canal LINE {#existing-user-profile-has-blocked-unfriended-or-unfollowed-line-channel}
...
5. Vous mettez à jour le profil 1 pour définir l'attribut `native_line_id`. Ce profil hérite du statut d'abonnement du profil 2.
  - Il y a désormais deux profils utilisateur avec le même `native_line_id`. Ceux-ci peuvent être fusionnés à tout moment à l'aide de l'endpoint `/users/merge` selon le processus décrit à l'[étape 5](#step-5-merge-profiles-optional).

## Réconciliation des ID utilisateur {#user-id-reconciliation}

Les ID LINE sont automatiquement reçus par Braze lorsqu'un utilisateur suit votre canal, ou lorsque vous utilisez le flux de travail ponctuel « synchroniser les abonnés ». Les ID LINE sont également spécifiques au canal que les utilisateurs suivent, il est donc peu probable que les utilisateurs puissent fournir leurs ID LINE.

Il existe deux façons de combiner un ID LINE avec un profil utilisateur Braze existant :

- [LINE Login](#line-login)
- [Association de comptes utilisateur](#user-account-linking)
...
4. Enregistrez l'ID LINE de l'utilisateur (`native_line_id`) dans le profil de l'utilisateur ayant un e-mail correspondant dans votre base de données, ou créez un nouveau profil utilisateur avec l'e-mail et l'ID LINE de l'utilisateur.

5. Envoyez les informations utilisateur nouvelles ou mises à jour à Braze en utilisant l'[endpoint `/user/track`]({{site.baseurl}}/api/endpoints/user_data/post_user_track), l'[importation CSV]({{site.baseurl}}/user_guide/audience/manage_audience/import_users#constructing-your-csv) ou l'[ingestion de données cloud]({{site.baseurl}}/user_guide/data/unification/cloud_ingestion).
...
1. L'utilisateur se connecte à votre application via LINE Login.
2. LINE vous fournit l'e-mail de l'utilisateur.
3. Vous envoyez à Braze l'utilisateur mis à jour (le profil utilisateur existant avec cet e-mail pour y ajouter l'ID LINE) ou vous mettez à jour l'utilisateur anonyme avec l'e-mail.
...
2. LINE vous fournit l'e-mail de l'utilisateur.
3. Vous pouvez :
  - Mettre à jour un profil utilisateur existant avec cet e-mail pour y ajouter également l'ID LINE de l'utilisateur.
  - Créer un nouveau profil utilisateur avec l'e-mail et l'ID LINE.
...
1. Créez un Canvas basé sur une action qui se déclenche lors d'un changement de statut d'abonnement et qui s'active lorsqu'un utilisateur s'abonne à votre canal LINE.<br>![Canvas qui se déclenche lorsqu'un utilisateur s'abonne au canal LINE.]({% image_buster /assets/img/line/account_link_1.png %})
...
1. LINE envoie un événement « follow » à Braze.
2. Braze crée un profil utilisateur anonyme avec l'ID LINE, l'alias utilisateur `line_id` et le statut du groupe d'abonnement LINE défini sur `subscribed`.
3. L'utilisateur reçoit un message LINE avec un lien vers votre site web et votre application, et se connecte. Son profil utilisateur est maintenant connu.
4. Le profil utilisateur anonyme qui a été créé est identifié et fusionné via l'[endpoint /users/identify]({{site.baseurl}}/api/endpoints/user_data/post_user_identify) avec le profil utilisateur connu. Le profil utilisateur connu contient désormais l'ID LINE et possède un statut d'abonnement `subscribed`.
