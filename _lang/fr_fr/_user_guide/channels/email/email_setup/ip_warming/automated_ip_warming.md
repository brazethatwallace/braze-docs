...
- « Access Campaigns »
- « Approve and Deny Campaigns » si le flux de travail d'approbation pour les campaigns est activé
    - Braze approuve automatiquement les campaigns créées à partir du réchauffement d'adresses IP automatisé en votre nom.

## Configurer un plan de réchauffement d'adresses IP automatisé
...
{% alert important %}
Les modifications apportées aux campaigns créées à partir de l'outil de réchauffement d'adresses IP (comme le changement de la date planifiée, du segment ou du volume) ne sont pas reflétées sur la page **Summary** du réchauffement d'adresses IP.
{% endalert %}
...
## Pendant le réchauffement d'adresses IP actif

Les campaigns de réchauffement d'adresses IP sont créées 1 à 2 jours à l'avance, sauf si vous lancez un réchauffement d'adresses IP le lendemain. Ces campaigns sont automatiquement nommées selon le format suivant : `IP Warming Day [X] - [Date] - [Template Name]`.
...
Si les statistiques sont en dessous de nos références, le système maintient le volume le jour suivant au lieu de l'augmenter, afin de limiter les risques pour votre réputation de l'expéditeur.

## Arrêter un plan de réchauffement d'adresses IP

Braze vous permet d'arrêter le réchauffement d'adresses IP et la création de futures campaigns, mais si une campaign est déjà active ou planifiée dans les prochaines 24 à 48 heures, vous devrez peut-être arrêter manuellement la campaign concernée. L'arrêt d'un plan de réchauffement d'adresses IP arrête également toutes les campaigns associées.
...
- Ajoutant un filtre à un segment si vous prévoyez d'utiliser le même segment que lors du dernier réchauffement d'adresses IP, en excluant les utilisateurs qui ont déjà reçu les campaigns précédentes

## Lorsqu'un réchauffement d'adresses IP se termine

Le réchauffement d'adresses IP est marqué comme terminé lorsque le dernier jour de réchauffement se termine à minuit dans le fuseau horaire de votre société. Par exemple, si la dernière campaign envoyée dans le plan de réchauffement d'adresses IP est envoyée à 20 h, alors le plan est marqué comme terminé après quatre heures.
