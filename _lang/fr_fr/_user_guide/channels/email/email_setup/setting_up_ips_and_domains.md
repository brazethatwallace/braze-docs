...
## Méthode 2 : Configuration e-mail en libre-service
...
Une fois la vérification de votre expéditeur terminée, nous recommandons le réchauffement d'adresses IP afin que vos messages atteignent les boîtes de réception de destination à un taux élevé et constant. Après avoir terminé cette configuration, assurez-vous de consulter également l'équipe d'onboarding de Braze pour confirmer que vos domaines et votre [adresse IP]({{site.baseurl}}/user_guide/message_building_by_channel/email/email_setup/ip_warming/) fonctionnent correctement.