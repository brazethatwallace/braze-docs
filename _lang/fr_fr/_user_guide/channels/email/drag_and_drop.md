...
{% alert note %}
Pour utiliser un modèle HTML personnalisé existant ou des modèles créés par un tiers, vous devez recréer le modèle en accédant à **Modèles** > **Modèles d'e-mail** et en sélectionnant **Éditeur par glisser-déposer** comme expérience d'édition.
{% endalert %}
...
![Bouton de l'assistant de rédaction, situé dans le panneau Contenu à côté des paramètres de style dans l'éditeur par glisser-déposer.]({% image_buster /assets/img/ai_copywriter/ai_copywriter_dnd.png %})
{% endalert %}
...
{% alert note %}
Les fonctionnalités avancées apparaîtront dans le compositeur de campagne ou de Canvas. Dans les fonctionnalités avancées, vous pouvez modifier votre paramètre CSS en ligne et saisir des paires clé-valeur d'en-tête ou supplémentaires (si configurées).
{% endalert %}
