...
En termes simples, les liens universels permettent à un site web d'associer ses pages web à des écrans spécifiques de l'application, de sorte que lorsqu'un utilisateur clique sur un lien vers une page web qui correspond à un écran de l'application, l'application peut être ouverte directement (si elle est actuellement installée).

{% alert important %}
Firebase Dynamic Links est obsolète. Braze n'a pas d'intégration directe avec Firebase, et la création de liens profonds est gérée en dehors de la plateforme Braze. Migrez vers des solutions natives de la plateforme (liens universels Apple et Android App Links, comme décrit dans cet article) ou vers des fournisseurs de services de deep linking alternatifs. Pour des conseils de migration, consultez la [FAQ de migration de Firebase](https://firebase.google.com/support/dynamic-links-faq).
{% endalert %}
...
| ---------------------- | -------------------------------------------------------------- | ---------------------------- |
| Compatibilité des plateformes | iOS (version 9 et ultérieure) et Android (version 6.0 et ultérieure)  | Utilisés sur divers OS mobiles    |
| Objectif                | Relier de façon fluide le contenu web et applicatif sur les appareils iOS et Android | Lier vers un contenu applicatif spécifique |
| Fonction               | Dirige vers des pages web ou du contenu applicatif en fonction du contexte           | Ouvre des écrans spécifiques de l'application   |
...
Les liens universels et les App Links sont le plus souvent utilisés pour les campagnes d'e-mail, car les e-mails peuvent être ouverts et cliqués depuis des appareils de bureau et mobiles.
...
- Votre application doit être disponible dans l'App Store (iOS) ou le Google Play Store (Android)

## Configuration des liens universels et des App Links {#setting-up-universal-links-and-app-links}

Pour que les applications prennent en charge les liens universels ou les App Links, iOS et Android nécessitent tous deux un fichier de permissions spécial hébergé au niveau du domaine du lien. Ce fichier contient les définitions des applications autorisées à ouvrir les liens provenant de ce domaine et, pour iOS, les chemins que ces applications sont autorisées à ouvrir :
...
Cette association bidirectionnelle entre le domaine et l'application est requise pour qu'un lien universel ou un App Link fonctionne et empêche toute application de détourner les liens d'un domaine particulier ou tout domaine d'ouvrir une application particulière.

{% tabs %}
<!--iOS instructions-->
{% tab iOS %}

Ces étapes sont adaptées de la documentation développeur Apple. Pour en savoir plus, consultez [Allowing apps and websites to link to your content](https://developer.apple.com/documentation/xcode/allowing-apps-and-websites-to-link-to-your-content?language=objc).
...
4. Si vous n'avez pas encore d'App Identifier enregistré, cliquez sur + pour en créer un.
   a. Saisissez un **Name**. Vous pouvez choisir n'importe quel nom.
...
#### Étape 1b : Activer les Associated Domains dans votre App Identifier {#step-1b-turn-on-associated-domains-in-your-app-identifier}

1. Dans votre App Identifier existant ou nouvellement créé, localisez la section **App Services**.
...
Avant de continuer, assurez-vous que votre projet Xcode utilise la même équipe que celle dans laquelle vous venez d'enregistrer votre App Identifier.
...
2. Vérifiez que le Bundle ID ([étape 1a](#step-1a)) de votre projet Xcode correspond à celui utilisé pour enregistrer l'App Identifier.
...
#### Étape 1e : Confirmer que le fichier de droits est inclus lors du build {#step-1e-confirm-that-the-entitlements-file-is-included-at-build}
...
Le fichier AASA contient un objet JSON avec une liste d'applications et les chemins d'URL sur le domaine qui doivent être inclus ou exclus en tant que liens universels. Voici un exemple de fichier AASA :
...
- `paths` : tableau de chaînes de caractères qui spécifie quels chemins sont inclus ou exclus de l'association. Vous pouvez utiliser `NOT` avant le chemin pour désactiver des chemins. Dans cet exemple, tous les liens sur ce chemin iront vers le web au lieu d'ouvrir l'application. Vous pouvez utiliser `*` comme caractère générique pour activer tous les chemins dans un répertoire et `?` pour correspondre à un seul caractère (par exemple /archives/201?/ pour correspondre à tous les nombres de 2010 à 2019).

{% alert note %}
Ces chaînes sont sensibles à la casse et les chaînes de requête et les identifiants de fragment sont ignorés.
{% endalert %}
...
Pour prendre en charge les liens universels dans votre application, suivez les étapes ci-dessous :
...
Ces étapes sont adaptées de la documentation développeur Android. Pour en savoir plus, consultez [Add Android App Links](https://developer.android.com/training/app-links#add-app-links) et [Create Deep Links to App Content](https://developer.android.com/training/app-links/deep-linking).

{% alert note %}
Les Android App Links nécessitent un `IBrazeDeeplinkHandler` personnalisé avec une logique pour gérer les liens provenant de leurs domaines séparément des autres URL web. Il peut être plus simple d'utiliser des deep links à la place et de maintenir des pratiques de liens uniformes pour les canaux autres que l'e-mail.
{% endalert %}

### Étape 1 : Créer des deep links {#step-1-create-deep-links}

Tout d'abord, vous devez créer des deep links pour votre application Android. Vous pouvez le faire en ajoutant des [filtres d'intention](https://developer.android.com/guide/components/intents-filters) dans votre fichier `AndroidManifest.xml`. Le filtre d'intention doit inclure l'action `VIEW` et la catégorie `BROWSABLE`, ainsi que l'URL de votre site web dans l'élément data.

### Étape 2 : Associer votre application à votre site web {#step-2-associate-your-app-with-your-website}

Vous devez associer votre application à votre site web. Cela peut être fait en créant un fichier Digital Asset Links. Ce fichier doit être au format JSON et inclure les détails des applications Android autorisées à ouvrir les liens vers votre site web. Il doit être placé dans le répertoire `.well-known` de votre site web.

### Étape 3 : Mettre à jour le fichier manifeste de votre application {#step-3-update-your-app-manifest-file}

Dans votre fichier `AndroidManifest.xml`, ajoutez un élément meta-data à l'intérieur de l'élément application. L'élément meta-data doit avoir un attribut `android:name` défini sur « asset_statements » et un attribut `android:resource` qui pointe vers un fichier de ressources contenant un tableau de chaînes de caractères incluant l'URL de votre site web.

### Étape 4 : Préparer votre application pour gérer les deep links {#step-4-prepare-your-app-to-handle-deep-links}

Dans votre application Android, vous devez gérer les deep links entrants. Vous pouvez le faire en récupérant l'intention qui a démarré votre activité et en extrayant les données de celle-ci.

### Étape 5 : Tester vos deep links {#step-5-testing-your-deep-links}

Enfin, vous pouvez tester vos deep links. Envoyez-vous un lien via une application de messagerie ou un e-mail et cliquez dessus. Si tout est correctement configuré, cela devrait ouvrir votre application.

{% endtab %}
{% endtabs %}

## Universal links, App Links et suivi des clics {#universal-links-app-links-and-click-tracking}
...
Pour permettre aux liens e-mail avec suivi des clics de fonctionner comme des universal links ou des App Links, vous devrez effectuer une configuration supplémentaire. Assurez-vous d'ajouter le domaine de suivi des clics (`links.email.example.com`) en tant que domaine que l'application est autorisée à ouvrir. De plus, le domaine de suivi des clics doit servir les fichiers AASA (iOS) ou Digital Asset Links (Android). Cela contribuera à garantir que les liens e-mail avec suivi des clics fonctionnent de façon fluide.

Si vous ne souhaitez pas que chaque lien de suivi des clics soit un universal link ou un App Link, vous pouvez spécifier quels liens doivent être des universal links en fonction du partenaire d'envoi d'e-mails. Consultez les onglets suivants pour plus de détails.

{% tabs %}
{% tab SendGrid %}

Pour traiter un lien de suivi des clics SendGrid comme un universal link :

1. Configurez vos valeurs AASA ou AndroidManifest pathPrefix pour traiter uniquement les liens contenant `/uni/` dans le chemin d'URL comme des universal links.
...
Avec cette configuration, les liens contenant `/uni/` dans le chemin d'URL fonctionneront comme des universal links, tandis que tous les autres liens fonctionneront comme des liens web.

{% endtab %}
{% tab SparkPost %}

Pour traiter un lien de suivi des clics SparkPost comme un universal link, ajoutez l'attribut suivant dans la section Attributs de l'éditeur par glisser-déposer pour l'e-mail, ou modifiez manuellement le HTML du lien pour inclure l'attribut suivant dans la balise d'ancrage de votre lien : `data-msys-sublink="custom_path"`.

Ce chemin personnalisé vous permet de traiter sélectivement les URL ayant cette valeur comme un universal link.
...
Utilisez des chemins personnalisés pour ajouter des segments de chemin aux URL de suivi des clics des e-mails. Cela crée des schémas d'URL prévisibles que les systèmes d'exploitation mobiles peuvent reconnaître pour les universal links et les App Links.

Lorsque les utilisateurs appuient sur des liens e-mail sur des appareils mobiles, les chemins personnalisés vous aident à contrôler si les liens s'ouvrent dans votre application mobile principale, une application spécialisée ou le navigateur mobile (par exemple, pages de produits, programmes de fidélité, liens de désabonnement ou pages juridiques).

Pour traiter un lien de suivi des clics Amazon SES comme un universal link ou un App Link :
...
- **Format :** Caractères alphanumériques, points, underscores et tirets uniquement
...
{:start="2"}
2. Confirmez que vos URL de suivi encapsulées incluent le segment de chemin personnalisé. Sans l'attribut, les liens suivis utilisent `track.yourstore.com/CL0/{encodedUrl}/...`. Avec l'attribut, ils suivent ce format : `track.yourstore.com/CL1/{customPath}/{encodedUrl}/...`
...
{:start="3"}
3. Configurez vos fichiers d'association de site sur votre domaine de suivi des clics afin que les chemins correspondent à `/CL1/{customPath}/`.
...
Android fait correspondre les chemins dans votre application plutôt que dans `assetlinks.json`. Définissez `android:pathPrefix="/CL1/{customPath}/"` sur le filtre d'intention dans votre `AndroidManifest.xml` pour chaque chemin personnalisé que votre application gère.
...
Si votre fournisseur de services d'e-mail marketing or e-mailing est SendGrid, utilisez le code HTML `clicktracking=off` comme ceci :
...
Si votre fournisseur de services d'e-mail marketing or e-mailing est SparkPost, utilisez le code HTML `data-msys-clicktrack="0"` comme ceci :
...
Si votre fournisseur de services d'e-mail marketing or e-mailing est Amazon SES, utilisez le code HTML `ses:no-track` comme ceci :
...
### Résolution des problèmes liés aux universal links avec suivi des clics {#troubleshooting-universal-links-with-click-tracking}

Si vos universal links ne fonctionnent pas comme prévu dans vos e-mails, par exemple en redirigeant le destinataire depuis son application de messagerie vers le navigateur web avant de finalement le rediriger vers l'application, consultez ces conseils pour résoudre les problèmes de configuration de vos universal links.

#### Outlook affiche `[?it=` ou du texte d'URL brut à la place d'un bouton {#outlook-shows-it-or-raw-url-text-instead-of-a-button}

Outlook peut afficher du texte d'appel à l'action comme `[?it=` ou imprimer une partie du `href` lorsqu'un lien n'utilise pas un schéma d'URL **`http://` ou `https://`** valide. Les schémas personnalisés, les schémas manquants ou les URL mal formées ne sont pas traités comme des liens hypertexte, de sorte que le client affiche le texte de l'attribut à la place. Confirmez que chaque bouton, lien image et URL suivie utilise une destination complète `https://` (ou `http://`). Cela s'applique aux universal links comme aux liens web standard.

#### Vérifier l'emplacement du fichier de liens {#verify-link-file-location}

Assurez-vous que le fichier AASA (iOS) ou le fichier Digital Asset Links (Android) est situé au bon endroit :
...
Il est important de s'assurer que ces fichiers sont toujours accessibles publiquement. Si vous ne pouvez pas y accéder, vous avez peut-être manqué une étape dans la configuration des universal links pour l'e-mail.

#### Vérifier les définitions de domaines {#verify-domain-definitions}

Assurez-vous que vous disposez des définitions correctes pour les domaines que votre application est autorisée à ouvrir.

- **iOS :** Examinez les domaines associés configurés dans Xcode pour votre application ([Étape 1c : Activer les domaines associés dans votre projet Xcode]({{site.baseurl}}/user_guide/channels/email/customize/universal_links_and_app_links?tab=ios#step-1c)). Vérifiez que le domaine de suivi des clics est inclus dans cette liste.
- **Android :** Ouvrez la page d'informations de l'application (appuyez longuement sur l'icône de l'application et cliquez sur ⓘ). Dans le menu d'informations de l'application, trouvez **Ouvrir par défaut** et appuyez dessus. Un écran devrait s'afficher avec tous les liens vérifiés que l'application est autorisée à ouvrir. Vérifiez que le domaine de suivi des clics est inclus dans cette liste.

#### Chaque lien e-mail ouvre l'application {#every-email-link-opens-the-app}

Si chaque lien d'un e-mail ouvre votre application, y compris les liens que vous vous attendez à voir s'ouvrir dans un navigateur, les valeurs `paths` AASA (iOS) ou `pathPrefix` Android sur votre domaine de suivi des clics correspondent à l'ensemble du domaine (par exemple `*` ou `/*`).

Limitez ces schémas aux URL qui doivent ouvrir l'application. Pour SendGrid, faites correspondre `/uni/` et ajoutez `universal="true"` uniquement sur ces liens. Voir [Universal links, App Links et suivi des clics](#universal-links-app-links-and-click-tracking).

#### Le domaine de suivi ne peut pas servir les fichiers .well-known {#tracking-domain-cant-serve-well-known-files}

Dans certains cas, votre domaine de suivi des clics peut ne pas être en mesure d'héberger les fichiers `.well-known` requis en raison des limitations du fournisseur de services d'e-mail marketing or e-mailing ou de contraintes d'infrastructure. Si vous ne pouvez pas héberger le fichier AASA ou Digital Asset Links sur votre domaine de suivi, envisagez les options suivantes :
...
- **Placer un CDN devant le sous-domaine de suivi :** Si vous avez besoin d'une couverture complète du suivi des clics et de la création de liens profonds, vous pouvez placer un CDN (tel que Cloudflare ou CloudFront) devant votre sous-domaine de suivi. Configurez le CDN pour servir les fichiers `.well-known` localement et acheminer tout le reste du trafic vers votre fournisseur de services d'e-mail marketing or e-mailing. Cette approche est plus complexe mais vous donne un contrôle total sur le suivi des clics et les universal links.

#### Les liens fonctionnent dans un espace de travail mais pas dans un autre {#links-working-in-one-workspace-but-not-another}

Si les universal links ou App Links fonctionnent correctement dans votre espace de travail de production mais échouent dans votre espace de travail de développement ou de test, vérifiez que le domaine de l'adresse e-mail d'envoi correspond au domaine de suivi configuré dans les paramètres e-mail de chaque espace de travail. Une configuration incohérente entre les espaces de travail peut entraîner un comportement différent des liens, même en utilisant les mêmes modèles d'e-mail et les mêmes fichiers AASA ou Digital Asset Links.

Pour vérifier votre configuration e-mail :

1. Accédez à **Paramètres** > **Préférences e-mail** dans le tableau de bord de Braze.
2. Examinez les **Paramètres d'e-mails sortants** sous **Configuration d'envoi**.
