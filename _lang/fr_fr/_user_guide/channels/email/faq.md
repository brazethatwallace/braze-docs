...
*Ouvertures totales* est le nombre de fois où l'e-mail a été ouvert par les utilisateurs, tandis que *Clics totaux* est le nombre de fois où les utilisateurs ont cliqué dans l'e-mail distribué, y compris tout type de clics tels que les clics sur les liens. Vous pouvez voir plus de clics que d'ouvertures pour l'une des raisons suivantes :
...
Le suivi d'ouverture repose sur le chargement de l'e-mail par le destinataire avec les images activées. Dans certains cas, les statistiques peuvent apparaître retardées ou des clics peuvent être enregistrés sans ouvertures correspondantes en raison de :
...
Si un utilisateur clique sur le lien de désabonnement deux fois (par exemple, s'il s'est désabonné, réabonné, puis désabonné à nouveau), cela compte deux fois dans les analyses d'e-mail.
...
- **Configurez un endpoint de redirection.** Hébergez un endpoint `https://` sur votre serveur qui redirige vers l'URL au schéma personnalisé de votre application. Les clients de messagerie préserveront le lien `https://`, et la redirection se chargera d'ouvrir l'application.
...
4. Après la pause, reprenez l'envoi à 10 à 20 % du volume précédent vers vos destinataires les plus engagés uniquement. Augmentez le volume lentement sur plusieurs semaines uniquement si aucune autre erreur 4xx ne se produit.
