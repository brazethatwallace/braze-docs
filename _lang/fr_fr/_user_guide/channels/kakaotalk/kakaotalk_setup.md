...
| --- | --- |
| Compte auprès d'un partenaire KakaoTalk pris en charge | Un compte auprès d'un partenaire KakaoTalk pris en charge, [CJ OliveNetworks](https://www.braze.com/partners/solutions-partners/cjolivenetworks/) ou [Infobip](https://marketplace.braze.com/partners/infobip), est nécessaire pour utiliser le canal de communication KakaoTalk. |
...
| IP du cluster Braze ajoutées à la liste d'autorisation | L'enregistrement sur la liste d'autorisation des IP est requis pour tous les clients. Enregistrez les adresses IP Braze de votre cluster avant d'intégrer KakaoTalk dans Braze. |
...
2. Rassemblez les identifiants requis pour votre fournisseur (consultez la section suivante), puis saisissez-les sur la page **Partenaires technologiques** et enregistrez.
...
![Champs de la page Partenaires technologiques pour CJ OliveNetworks.]({% image_buster /assets/img/kakaotalk/cj_olivenetworks.png %}){: style="max-width:30%;"}

#### Infobip

Accédez à votre tableau de bord Infobip et au [centre d'administration des canaux KakaoTalk](https://center-pf.kakao.com/) pour rassembler les informations suivantes.
...
| **Sender profile UUID** | Dans le centre d'administration des canaux KakaoTalk, accédez à **Channels** et trouvez le **Search ID** dans la fenêtre d'informations du canal. |
| **Channel name** | Dans le centre d'administration des canaux KakaoTalk, trouvez le **nom du canal** dans la même fenêtre d'informations du canal. |
...
4. Saisissez le **nom**, sélectionnez la **date d'expiration**, puis sélectionnez les portées API requises pour KakaoTalk. Ces portées contrôlent les actions de l'API Infobip que votre clé peut effectuer.

![Page de création de clé API Infobip affichant les champs de nom, de date d'expiration et de portées API.]({% image_buster /assets/img/kakaotalk/infobip_api_key_scopes.png %})

{: start="5"}
5. Sélectionnez **CREATE** pour générer la clé.
6. Copiez la clé générée. Vous pouvez revenir sur cette page pour mettre à jour le nom, la date d'expiration ou les portées API.

##### UUID du profil d'expéditeur et nom du canal {#sender-profile-uuid-and-channel-name}

1. Dans le [centre d'administration des canaux KakaoTalk](https://center-pf.kakao.com/), sélectionnez **Channels**.
2. Dans la fenêtre **Channel Information**, trouvez le **nom du canal** et le **Search id** (UUID de l'expéditeur).
3. Saisissez les **coordonnées du centre client**. Cette information est requise lors de l'envoi de messages publicitaires.
...
{: start="4"}
4. Pour afficher un canal différent, sélectionnez l'icône de canal en haut du menu.
5. Dans la liste **My channel**, sélectionnez le canal que vous souhaitez consulter, puis répétez les étapes précédentes.

## Configurer les profils utilisateur {#set-user-profiles}

Les profils utilisateur doivent contenir des numéros de téléphone au format E.164 pour leur envoyer des messages via KakaoTalk. Les numéros de téléphone sont affichés sur le profil utilisateur. KakaoTalk exige que les numéros de téléphone soient au format E.164 (par exemple, `+821025749774`). Cela diffère de certains autres canaux de communication qui peuvent accepter les numéros de téléphone dans plusieurs formats.

### Importer des numéros de téléphone {#import-phone-numbers}

Importez des numéros de téléphone en [chargeant un fichier CSV ou en utilisant l'API]({{site.baseurl}}/user_guide/audience/manage_audience/import_users) pour créer un utilisateur. Assurez-vous que les numéros de téléphone sont au format E.164 avant l'importation.