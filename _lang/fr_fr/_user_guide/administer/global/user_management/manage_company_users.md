...
3. Saisissez les informations demandées, notamment l'adresse e-mail, le département et le [rôle de l'utilisateur]({{site.baseurl}}/user_guide/administer/global/user_management/permissions#creating-a-role).
...
![Permissions au niveau de l'espace de travail avec une section pour les champs de permissions personnalisées.]({% image_buster /assets/img/add_new_user_3.png %})
...
La suspension d'un utilisateur met son compte dans un état inactif, où l'utilisateur ne peut plus se connecter, mais les données associées à son compte sont préservées. Seuls les administrateurs peuvent suspendre ou réactiver des utilisateurs de l'entreprise. Notez que les utilisateurs suspendus peuvent toujours recevoir des notifications de Braze.

Pour suspendre un utilisateur, accédez à **Paramètres** > **Paramètres de l'entreprise** > **Gestion des utilisateurs** > **Utilisateurs de l'entreprise**, recherchez son nom d'utilisateur, puis sélectionnez <i class="fa-solid fa-user-lock" aria-label="Suspendre l'utilisateur"></i> **Suspendre**.
...
- Les attributs associés à l'utilisateur
- L'adresse e-mail
...
### « L'e-mail est déjà utilisé » lors de l'ajout d'un utilisateur {#email-is-already-taken-when-trying-to-add-a-user}

Si vous essayez d'ajouter un nouvel utilisateur et recevez une erreur indiquant que l'e-mail est déjà utilisé, mais que vous ne le trouvez pas dans votre liste d'utilisateurs, cet utilisateur existe probablement dans une autre instance du même cluster du tableau de bord de Braze.

Pour créer ce nouvel utilisateur, vous pouvez effectuer l'une des actions suivantes :

1. Supprimer l'utilisateur de l'autre instance avant de pouvoir le créer dans la nouvelle, ou
2. Créer l'utilisateur avec une chaîne d'e-mail différente (telle que `testing+01@braze.com`) ou un autre alias d'e-mail.
...
- [Permissions]({{site.baseurl}}/user_guide/administer/global/user_management/permissions) pour configurer ce que chaque utilisateur peut faire dans le tableau de bord.
