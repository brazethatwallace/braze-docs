...
Utilisez ce champ pour modifier la longueur minimale requise pour le mot de passe. Le minimum par défaut est de huit caractères.

### Complexité du mot de passe {#password-complexity}

Sélectionnez **Exiger des mots de passe complexes** pour imposer que les mots de passe contiennent au moins un élément de chacune des catégories suivantes :
- Lettre majuscule
- Lettre minuscule
- Chiffre
...
Détermine le nombre minimal de nouveaux mots de passe devant être définis avant qu'un utilisateur puisse réutiliser un mot de passe. La valeur par défaut est trois.
...
L'authentification à deux facteurs est obligatoire pour tous les utilisateurs de l'entreprise. Elle ajoute un deuxième niveau de vérification d'identité à la connexion d'un compte, ce qui la rend plus sécurisée qu'un simple nom d'utilisateur et mot de passe. Si votre tableau de bord ne prend pas en charge l'authentification à deux facteurs, contactez votre gestionnaire de la satisfaction client.
...
Lorsque vous sélectionnez **Se souvenir de ce compte pendant 30 jours**, un cookie est enregistré sur votre appareil, ce qui vous permet de ne vous connecter avec l'authentification à deux facteurs qu'une seule fois sur une période de 30 jours.
...
Les clients disposant de plusieurs comptes au sein d'une entreprise sur le tableau de bord peuvent rencontrer des problèmes avec cette fonctionnalité, car le cookie est lié à un appareil spécifique. Si des utilisateurs utilisent le même appareil pour se connecter à plusieurs comptes, le cookie sera remplacé pour les comptes précédemment autorisés sur cet appareil. Braze s'attend à ce qu'un seul appareil soit associé à un compte, et non un appareil pour plusieurs comptes.

### Réinitialiser l'authentification d'un utilisateur {#resetting-user-authentication}

Si vous rencontrez des problèmes de connexion avec l'authentification à deux facteurs, contactez les administrateurs de votre entreprise pour réinitialiser votre authentification à deux facteurs. Les administrateurs peuvent effectuer les étapes suivantes :

1. Allez dans **Paramètres** > **Paramètres de l'entreprise** > **Gestion des utilisateurs** > **Utilisateurs de l'entreprise**.
2. Sélectionnez l'utilisateur dans la liste fournie.
3. Sélectionnez **Réinitialiser** sous **Authentification à deux facteurs**.

Une réinitialisation peut résoudre les problèmes d'authentification courants tels que des difficultés avec les applications d'authentification, la non-réception des e-mails de vérification, les échecs de connexion dus à des pannes de SMS ou à des erreurs de l'utilisateur, et bien plus encore.
...
Si la 2FA n'est pas imposée au niveau de l'entreprise, les utilisateurs individuels peuvent configurer la 2FA par eux-mêmes sur leur page Paramètres du compte. Dans ce cas, les utilisateurs ne seront pas bloqués s'ils ne la configurent pas. Vous pouvez identifier les utilisateurs qui ont choisi d'activer la 2FA en consultant la liste des **Utilisateurs de l'entreprise**.

#### Exigences lorsque la 2FA est obligatoire {#requirements-when-2fa-is-mandatory}

Si la 2FA est imposée au niveau de l'entreprise, les utilisateurs qui ne la configurent pas sur leur propre compte lors de la connexion seront bloqués sur le tableau de bord. Les utilisateurs doivent compléter la configuration de la 2FA pour conserver leur accès.

{% alert important %}
La 2FA est obligatoire pour tous les utilisateurs de l'entreprise uniquement si l'authentification unique (SSO) n'est pas activée. Si l'authentification unique est utilisée, la 2FA n'a pas besoin d'être imposée au niveau de l'entreprise.
{% endalert %}
...
1. Dans Braze, sélectionnez l'icône de votre profil dans l'en-tête global, puis sélectionnez **Manage your account**. Faites défiler jusqu'à la section **Two-Factor Authentication**, puis sélectionnez **Start Setup**.
...
## Accès renforcé {#elevated-access}

L'accès renforcé ajoute une couche de sécurité supplémentaire pour les actions sensibles dans votre tableau de bord de Braze. Lorsqu'il est activé, les utilisateurs doivent vérifier à nouveau leur compte avant d'exporter un Segment ou de consulter une clé API. Pour utiliser l'accès renforcé, accédez à **Paramètres** > **Paramètres de l'entreprise** > **Paramètres d'administration** > **Paramètres de sécurité** et activez-le.

Si un utilisateur ne parvient pas à se re-vérifier, il sera redirigé vers l'endroit où il s'était arrêté et ne pourra pas poursuivre l'action sensible. Une fois la re-vérification effectuée avec succès, il n'aura pas besoin de le refaire pendant l'heure suivante, sauf s'il se déconnecte entre-temps.
...
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="Champs pertinents" }
...
![Section Préférences de suppression des données avec un bouton activé pour personnaliser les préférences de suppression des données par espace de travail.]({% image_buster /assets/img/deletion_preferences_1.png %})
...
Si vous vous retrouvez dans une boucle après avoir saisi avec succès votre numéro de téléphone pour la 2FA et que vous êtes redirigé(e) vers la page de connexion, cela est probablement dû à un échec de vérification lors de la première tentative. Pour résoudre ce problème, suivez ces étapes :
...
Si le problème persiste après ces étapes, contactez le [support]({{site.baseurl}}/user_guide/administer/personal/braze_support) pour obtenir de l'aide.

### Impossible d'activer l'authentification à deux facteurs (2FA) {#cant-enable-two-factor-authentication-2fa}

Si la 2FA est activée mais que rien ne se passe lorsque vous sélectionnez le bouton **Activer**, cela peut être dû au fait que votre navigateur bloque la redirection nécessaire à l'envoi du code de vérification par SMS. Voici les étapes pour résoudre ce problème :
