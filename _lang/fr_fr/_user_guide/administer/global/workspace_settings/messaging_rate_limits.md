---
nav_title: Limites de débit d'envoi de messages
...
{% endalert %}
Le nombre total de messages envoyés par minute ne dépasse pas les limites de débit configurées pour l'espace de travail. Il n'y a pas d'ordre particulier déterminant quelles Campaigns sont distribuées dans les premières minutes par rapport aux minutes suivantes.
...
| Campaign   | Nombre de messages | Heure d'envoi |
|------------|--------------------|---------------|
...
Les messages sont traités en parallèle. Une fois traités, ils sont planifiés de manière à respecter la limite de débit d'envoi de messages de l'espace de travail selon le principe du premier arrivé, premier servi. Cela signifie que dans cet exemple, les messages envoyés chaque minute sont un mélange variable des Campaigns 1, 2 et 3 totalisant 100 000.

![Exemple de distribution des messages pour les trois Campaigns.]({% image_buster /assets/img/workspace_messaging_rate_limits2.png %})

Considérons l'exemple suivant avec une limite de débit d'envoi de messages de l'espace de travail de 100 000 messages par minute, avec les messages suivants configurés :
...
- Campaign 1 serait planifiée de 9 h à 9 h 10, avec 100 000 messages envoyés par minute.
- Campaign 2 serait planifiée pour être distribuée 5 minutes après son heure de distribution initiale, de 9 h 10 à 9 h 20, avec 100 000 messages envoyés par minute.

![Exemple de distribution des messages par minute pour les deux Campaigns.]({% image_buster /assets/img/workspace_messaging_rate_limits1.png %})
...
Lorsqu'une Campaign ou un Canvas possède sa propre limite de débit et qu'une limite de débit au niveau de l'espace de travail s'applique également, les deux sont appliquées. Par exemple, si une Campaign a une limite de débit de 500 000 mais que, en raison des limites de débit de l'espace de travail, elle ne peut envoyer que 100 000 messages par minute à ce moment-là, c'est la limite de débit de l'espace de travail qui prend effet.

Braze essaie de répartir uniformément la distribution des messages tout au long de la minute, mais ne peut pas le garantir. Par exemple, si vous avez une Campaign avec une limite de débit de 500 000 messages par minute, nous essaierons de répartir les 500 000 messages uniformément sur la minute (environ 8 400 messages par seconde), mais il peut y avoir des variations dans le débit par seconde.
...
- Les limites de débit d'envoi de messages de l'espace de travail ne sont pas prises en charge pour les messages in-app, les feature flags et les bannières.