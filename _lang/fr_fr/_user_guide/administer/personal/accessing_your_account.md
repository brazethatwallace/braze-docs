...
4. **Confirmez la bonne instance du tableau de bord :** Assurez-vous que vous demandez la réinitialisation depuis la bonne [instance du tableau de bord Braze]({{site.baseurl}}/user_guide/administer/personal/sdk_endpoints). Vérifiez auprès de l'administrateur de votre compte ou de votre gestionnaire de compte Braze en cas de doute.
...
Si vous rencontrez des problèmes de performance du tableau de bord, comme le non-chargement de votre tableau de bord ou de la liste de performance des Segment, essayez de vider le cache et les cookies de votre navigateur en suivant les étapes correspondant à votre navigateur.
...
Si vous rencontrez des problèmes de connexion à Braze Learning et que vous êtes bloqué dans une boucle qui vous redirige vers le tableau de bord, suivez les étapes suivantes :
...
{% alert note %}
Ce processus de réinitialisation s'applique également aux utilisateurs qui ont été verrouillés hors de leur compte en raison d'un trop grand nombre de demandes de jetons au cours de la dernière heure.
{% endalert %}
...
- Vérifiez que vous vous connectez à la bonne [instance du tableau de bord Braze]({{site.baseurl}}/user_guide/administer/personal/sdk_endpoints). Vérifiez auprès de l'administrateur de votre compte ou de votre gestionnaire de compte Braze pour en être sûr.
...
Cela n'arrive pas très souvent, mais Braze prend la suspension et la suppression de comptes très au sérieux. Si vous rencontrez une erreur « Account has been banned » en essayant de vous connecter, votre compte de tableau de bord est temporairement suspendu. Cela peut se produire pour plusieurs raisons.
...
| Violations des règles | Le compte peut avoir enfreint les conditions d'utilisation ou les politiques d'utilisation acceptable de Braze. |
...
- **Mettez à jour votre navigateur :** Mettez à jour votre navigateur vers la dernière version des [navigateurs pris en charge](#supported-browsers), puis essayez de vous connecter à votre tableau de bord.
...
Si vous êtes un nouvel utilisateur Braze et que vous avez des difficultés à vous connecter ou à accéder à votre compte pour la première fois, suivez ces étapes pour résoudre les problèmes courants :
