...
| E-mail | Braze transmet le message à un fournisseur de services d'e-mailing (ESP). L'ESP est ensuite responsable de la distribution finale. Cet ESP, par exemple, peut signaler un « rebond » si l'adresse e-mail est invalide ou si la boîte de réception est pleine. |
...
Ce graphique chronologique montre une ventilation horaire des raisons pour lesquelles un message a été abandonné ou un utilisateur a été retiré d'un Canvas. Les libellés de résultats dans ce graphique sont des libellés normalisés du tableau de bord, et non des valeurs brutes de la charge utile des événements. Ce graphique n'affiche pas le nombre d'envois.

### Journal granulaire des résultats de messages {#message-outcomes-granular-log}

Le tableau de bord affiche un tableau granulaire des résultats individuels de messages pour les filtres et la période sélectionnés. Utilisez ce tableau pour examiner des enregistrements spécifiques, notamment l'horodatage, l'ID utilisateur, l'étape du Canvas, le résultat, les détails et le canal.
...
Sélectionnez une ligne dans le tableau pour ouvrir le panneau de détails. Le panneau de détails fournit un contexte supplémentaire sur ce résultat, et Ask Operator propose des conseils de remédiation pour vous aider à résoudre le problème sous-jacent.

![Journal granulaire des résultats de Messaging Diagnostics avec une ligne sélectionnée et accès au panneau de détails.]({% image_buster /assets/img/messaging_diagnostics_dashboard_details_log.png %}){: style="max-width:45%;"} ![Panneau de détails de Messaging Diagnostics développé avec le contexte du résultat et les conseils de remédiation.]({% image_buster /assets/img/messaging_diagnostics_dashboard_drawer_expanded.png %}){: style="max-width:45%;"}
...
| Connected Content failed | Braze a tenté d'envoyer le message, mais le contenu connecté a échoué après le nombre maximal de tentatives (cinq par défaut). **Remarque :** Ce comptage représente le nombre de messages abandonnés en raison de l'atteinte du nombre maximal de tentatives, et non le nombre total de requêtes de contenu connecté échouées. |
...
| Media URL failure | Braze n'a pas pu traiter l'URL du média dans le message. Cela peut se produire lorsque l'URL est bloquée, invalide, expire, renvoie un statut HTTP invalide ou échoue à la validation SSL. |
...
| Exception or exit event | L'utilisateur était précédemment éligible pour recevoir le message, mais soit {::nomarkdown}<ul><li> a effectué un <a href="/docs/user_guide/messaging/campaigns/schedule_your_campaign/triggered_delivery#step-3-select-exception-events">événement d'exception</a> pour une Campaign basée sur une action, le message a donc été abandonné, soit </li><li> a rempli les <a href="/docs/user_guide/messaging/canvas/create_a_canvas#setting-exit-criteria">critères de sortie</a> du Canvas et a donc été retiré en cours de parcours.</li></ul>{:/} |
...
| Push credentials invalid | Les [identifiants push]({{site.baseurl}}/user_guide/channels/push/faqs#why-doesnt-an-opted-in-user-have-a-push-token) pour cette application sont manquants ou invalides, l'envoi a donc été annulé. Mettez à jour vos identifiants dans **Paramètres de l'application**. |
...
- **Étapes de message multiples (Canvas uniquement) :** Votre Canvas peut comporter plus d'une étape de message dans un parcours donné. Ce tableau de bord agrège tous les résultats, de sorte qu'un même utilisateur pourrait être compté plusieurs fois s'il passe par plusieurs étapes de message au cours de la période sélectionnée.
