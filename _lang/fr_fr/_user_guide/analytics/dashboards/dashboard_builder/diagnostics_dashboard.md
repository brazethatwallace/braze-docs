...
{% alert note %}
Le tableau de bord affiche uniquement les sept derniers jours de données au maximum. Tous les horodatages sont affichés dans le fuseau horaire de votre espace de travail.
{% endalert %}
...
| User failed pre-check for Message step | Braze effectue un premier ensemble de vérifications préliminaires sur l'éligibilité de l'audience, la rééligibilité et l'éligibilité du canal avant les validations complètes de distribution pour une étape Message du Canvas. Ce résultat signifie que l'utilisateur ou le message a échoué à l'une de ces vérifications, le message a donc été abandonné pour cette étape. |
| User failed pre-check for triggered message | Braze effectue un premier ensemble de vérifications préliminaires sur l'éligibilité de l'audience, la rééligibilité et l'éligibilité du canal avant de créer un message à envoyer à partir de ce déclencheur. Ce résultat signifie que l'utilisateur ou le message a échoué à l'une de ces vérifications, le message a donc été abandonné. |
| User no longer eligible | L'utilisateur faisait initialement partie de l'audience cible, mais ne correspondait plus aux critères d'audience avant que Braze n'envoie le message ou n'intègre l'utilisateur dans le Canvas. Le délai entre le moment où l'utilisateur a initialement rempli les critères d'audience et celui où il ne les remplit plus peut être dû à des retards liés à : {::nomarkdown}<ul><li>Le timing intelligent</li><li>Les heures calmes</li><li>L'heure locale</li><li>Les limites de débit de vitesse de distribution (non applicable pour l'entrée dans le Canvas)</li><li>Les retards du pipeline de communication</li></ul>{:/} |
...
| Subscription group failure | Le message n'a pas pu être envoyé en raison de problèmes de configuration du groupe d'abonnement ou du service de communication. Parmi les raisons courantes : des numéros d'envoi manquants pour SMS ou WhatsApp, ou un MMS non pris en charge sur le service de communication configuré. |
| User not eligible for channel | L'utilisateur n'est pas éligible pour recevoir ce message sur le canal sélectionné. Parmi les raisons courantes : des identifiants de canal manquants ou invalides, aucun jeton push éligible, des restrictions liées à l'état d'abonnement, une fonctionnalité de canal non prise en charge, ou des pays bloqués pour les canaux téléphoniques. |
