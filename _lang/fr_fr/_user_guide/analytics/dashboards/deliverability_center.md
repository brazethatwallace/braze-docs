...
Avant de vous connecter au Centre de livrabilité, vous devrez configurer un compte Google Postmaster Tools. Vous pouvez utiliser un compte Gmail professionnel ou personnel pour configurer votre Google Postmaster.
...
Microsoft SNDS fournit des données au niveau IP sur les plaintes pour spam et le volume d'envoi tels que rapportés par les fournisseurs de boîtes de réception Microsoft comme Outlook, Hotmail et en direct.
...
Pour les espaces de travail qui envoient des e-mails via Amazon SES, le centre de livrabilité affiche les indicateurs Microsoft SNDS pour vos adresses IP d'envoi dédiées. Braze rétroalimente jusqu'à 90 jours de données SNDS historiques lorsque cette fonctionnalité est activée pour votre espace de travail.

![Un exemple de résultats Microsoft SNDS, incluant des adresses IP, des destinataires, des commandes RCPT, des commandes DATA, des résultats de filtre et un taux de plainte.]({% image_buster /assets/img_archive/deliverability_center_msnds.png %})
...
#### Résultats de filtre {#filter-results}

Consultez ce tableau pour comprendre les résultats de filtre.

| Résultat | Définition |
| ----- | ---------- |
| Vert | Jugé comme spam par le filtre anti-spam de Microsoft jusqu'à 10 % de la période donnée. |
| Jaune | Jugé comme spam par le filtre anti-spam de Microsoft entre 10 % et 90 % de la période donnée. |
| Rouge | Jugé comme spam par le filtre anti-spam de Microsoft à plus de 90 % de la période donnée. |
{: .reset-td-br-1 .reset-td-br-2 aria-label="Résultats de filtre" }

#### Taux de plainte {#complaint-rate}

Il s'agit de la proportion de fois où un message reçu depuis l'IP fait l'objet d'une plainte de la part d'un utilisateur Hotmail ou Windows en direct pendant la période d'activité. Les utilisateurs ont la possibilité de signaler la quasi-totalité des messages comme indésirables via l'interface web.
...
| Supérieur à 0,3 % | Vérifiez votre processus d'inscription et assurez-vous que votre lien de désabonnement fonctionne. Envisagez également de mieux personnaliser vos e-mails pour votre audience. |
...
#### Détections de pièges à spam et période des messages piégés {#spam-trap-hits-and-trap-message-period}

{% alert important %}
Microsoft n'inclut plus les comptages de détections de pièges à spam ni les données de période des messages piégés dans les rapports SNDS. Pour plus d'informations, consultez l'[annonce SNDS de Microsoft](https://substrate.office.com/ip-domain-management-snds/snds).
{% endalert %}

Les détections de pièges à spam correspondaient au nombre de messages envoyés à des « comptes pièges », c'est-à-dire des comptes maintenus par Outlook.com qui ne sollicitent aucun e-mail.

Les colonnes de début et de fin de la période des messages piégés indiquaient quand les premiers et derniers messages envoyés aux comptes pièges ont été reçus depuis l'IP pendant la période d'activité.
...
Une boucle de rétroaction par e-mail (FBL) permet aux expéditeurs d'e-mails de recevoir des rapports lorsque les destinataires marquent des messages comme spam. Cependant, Gmail et iCloud ne proposent pas de boucles de rétroaction traditionnelles, ce qui signifie que Braze (via SparkPost ou SendGrid) ne reçoit pas de données de plaintes pour spam de la part de ces fournisseurs.
...
- Apple ne fournit pas d'outil Postmaster public équivalent à celui de Google. Concentrez-vous sur le maintien d'indicateurs d'engagement solides et le respect des bonnes pratiques en matière d'e-mail.

Pour maintenir une bonne livrabilité avec tous les fournisseurs, mettez en place une [politique de temporisation]({{site.baseurl}}/user_guide/channels/email/best_practices/sunset_policies) pour cesser automatiquement d'envoyer des e-mails aux utilisateurs non engagés. Cela permet d'éviter que vos e-mails soient marqués comme spam et protège votre réputation d'expéditeur.