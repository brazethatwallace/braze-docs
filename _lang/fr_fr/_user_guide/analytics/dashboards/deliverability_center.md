...
Avant de vous connecter au Centre de livrabilité, vous devez configurer un compte Google Postmaster Tools. Vous pouvez utiliser un compte Gmail professionnel ou personnel pour configurer votre compte Google Postmaster.

1. Accédez au [tableau de bord Google Postmaster Tools](https://postmaster.google.com/managedomains?pli=1).
2. En bas de la page, sélectionnez <i class="fas fa-plus-circle"></i> **Ajouter un domaine**.
3. Saisissez votre domaine racine (parent) pour authentifier votre e-mail. Assurez-vous que l'enregistrement TXT est lié à ce domaine racine (parent), et **non** au sous-domaine que vous utilisez via Braze. La vérification du domaine racine (parent) vous permet d'ajouter ultérieurement des sous-domaines dans Postmaster Tools sans créer d'enregistrements TXT supplémentaires. Par exemple, en vérifiant `braze.com`, vous pouvez ensuite ajouter `demo.braze.com` en tant que sous-domaine distinct dans Postmaster Tools pour consulter les indicateurs au niveau du sous-domaine.
4. Google génère un enregistrement TXT qui peut être ajouté directement au DNS de votre domaine. Celui-ci est généralement géré par la personne responsable de votre DNS. Pour obtenir des informations et des conseils sur la mise à jour de votre DNS spécifique, consultez [Vérifier votre domaine (étapes spécifiques à l'hébergeur)](https://support.google.com/a/topic/1409901).
...
{% alert note %}
Si vos sous-domaines ne sont pas inclus dans le Centre de livrabilité pour Google Postmaster, cela peut être dû au fait que seul le domaine racine (parent) a été ajouté à Google Postmaster. Une fois les domaines racines vérifiés dans Google Postmaster, vous pouvez ajouter vos sous-domaines, qui sont vérifiés automatiquement. Ce processus permet à Google de fournir des indicateurs au niveau du sous-domaine, qui peuvent ensuite être intégrés au Centre de livrabilité de Braze.
{% endalert %}
...
Cependant, Gmail et iCloud ne fonctionnent pas avec des boucles de rétroaction traditionnelles et ne signalent pas les plaintes pour spam à Braze. Cela signifie :<br>
...
Si Microsoft est votre principal fournisseur de boîtes de réception, vous pouvez consulter les données Microsoft SNDS dans le Deliverability Center. Cela inclut les adresses IP d'envoi dédiées pour les espaces de travail qui utilisent Amazon SES, SendGrid ou SparkPost. Utilisez ces données pour surveiller la santé de vos IP et comprendre comment les fournisseurs de boîtes de réception Microsoft évaluent vos envois.

Microsoft SNDS fournit des données au niveau IP sur les plaintes pour spam, les pièges à spam et le volume d'envoi tels que rapportés par les fournisseurs de boîtes de réception Microsoft comme Outlook, Hotmail et Live.

{% alert important %}
Si vous ne voyez pas vos données dans le Deliverability Center, contactez le [support]({{site.baseurl}}/user_guide/administer/personal/braze_support) avec la liste de vos adresses IP.
{% endalert %}

### Amazon SES

Pour les espaces de travail qui envoient des e-mails via Amazon SES, le Deliverability Center affiche les indicateurs Microsoft SNDS pour vos adresses IP d'envoi dédiées. Braze rétrocharge jusqu'à 90 jours de données SNDS historiques lorsque cette fonctionnalité est activée pour votre espace de travail.

{% alert note %}
Amazon SES ne fournit pas les indicateurs **Trap message period start** ni **Trap message period end**. Pour les adresses IP d'envoi SES, ces colonnes sont masquées dans le tableau Microsoft SNDS. Vous pouvez toujours consulter les autres indicateurs SNDS pour ces IP, y compris les pièges à spam.
{% endalert %}

![Un exemple de résultats Microsoft SNDS, incluant des adresses IP, des destinataires, des commandes RCPT, des commandes DATA, des résultats de filtre, un taux de plainte, le début et la fin de la période de messages piège, et les pièges à spam.]({% image_buster /assets/img_archive/deliverability_center_msnds.png %})
...
Cet indicateur suit le nombre de commandes DATA envoyées par l'IP. Les commandes DATA font partie du protocole SMTP utilisé pour envoyer des e-mails.

#### Résultats du filtre {#filter-results}

Consultez ce tableau pour comprendre les résultats du filtre.

| Résultat | Définition |
| ----- | ---------- |
| Vert | Considéré comme spam par le filtre anti-spam de Microsoft jusqu'à 10 % de la période donnée. |
| Jaune | Considéré comme spam par le filtre anti-spam de Microsoft entre 10 % et 90 % de la période donnée. |
| Rouge | Considéré comme spam par le filtre anti-spam de Microsoft pour plus de 90 % de la période donnée. |
{: .reset-td-br-1 .reset-td-br-2 aria-label="Résultats du filtre" }
...
| Résultat | Définition |
| ----- | ---------- |
| Inférieur à 0,3 % | Le taux de plainte idéal. |
| Supérieur à 0,3 % | Vérifiez votre processus d'inscription et assurez-vous que votre lien de désinscription fonctionne. Envisagez également de mieux personnaliser vos e-mails pour votre audience. |
| Supérieur à 100 % | Notez que SNDS affiche les plaintes pour le jour où elles ont été signalées, et non rétroactivement pour le jour où l'e-mail ayant fait l'objet de la plainte a été envoyé. |
{: .reset-td-br-1 .reset-td-br-2 aria-label="Taux de plainte" }

#### Pièges à spam {#spam-trap-hits}

Les pièges à spam correspondent au nombre de messages envoyés à des « comptes piège », c'est-à-dire des comptes gérés par Outlook.com qui ne sollicitent aucun e-mail. Il est probable que tout message envoyé à ces comptes piège soit considéré comme du spam. Il est donc important de surveiller cet indicateur pour s'assurer qu'il reste faible. Un faible nombre de pièges à spam signifie que les messages ne sont pas envoyés à ces comptes et sont bien envoyés à de véritables comptes.

#### Début et fin de la période de messages piège {#trap-message-period-start-and-end}

Ces colonnes indiquent quand les premiers et derniers messages envoyés à des comptes piège ont été reçus depuis l'IP pendant la période d'activité. Amazon SES ne fournit pas ces indicateurs, les colonnes sont donc masquées lorsque vous consultez uniquement les adresses IP d'envoi SES dans le tableau Microsoft SNDS.

{% alert tip %}
Si vous recherchez des enregistrements liés à l'un de vos domaines vérifiés dans Braze, notez que le Deliverability Center répertorie vos données provenant de Google Postmaster ou de Microsoft SNDS, ce qui signifie qu'il est possible que l'une ou l'autre plateforme n'ait aucune donnée à partager avec Braze. Vous pouvez également essayer de maintenir une distribution d'e-mails régulière, car cela peut contribuer à améliorer votre réputation.
{% endalert %}

## Plaintes pour spam et boucles de rétroaction {#spam-complaints-and-feedback-loops}

Une boucle de rétroaction par e-mail (FBL) permet aux expéditeurs d'e-mails de recevoir des rapports lorsque les destinataires signalent des messages comme spam. Cependant, Gmail et iCloud ne proposent pas de boucles de rétroaction traditionnelles, ce qui signifie que Braze (via SparkPost ou SendGrid) ne reçoit pas les données de plaintes pour spam de ces fournisseurs.

Étant donné que les données de plaintes pour spam ne sont pas disponibles auprès de Gmail et iCloud, il est important d'utiliser d'autres outils pour surveiller la santé et la réputation de vos e-mails auprès de ces fournisseurs majeurs :

- Utilisez [Google Postmaster Tools](https://www.gmail.com/postmaster/) pour surveiller la réputation de votre domaine et de votre IP, les taux de spam et l'engagement des utilisateurs. Vous pouvez intégrer Google Postmaster avec Braze comme décrit dans [Intégrer Google Postmaster](#integrating-google-postmaster).
- Apple ne fournit pas d'outil Postmaster public équivalent à celui de Google. Concentrez-vous sur le maintien de solides indicateurs d'engagement et le respect des bonnes pratiques en matière d'e-mail.

Pour maintenir une bonne livrabilité avec tous les fournisseurs, mettez en place une [politique de temporisation]({{site.baseurl}}/user_guide/channels/email/best_practices/sunset_policies) pour cesser automatiquement d'envoyer des e-mails aux utilisateurs non engagés. Cela permet d'éviter que vos e-mails soient signalés comme spam et protège votre réputation d'expéditeur.