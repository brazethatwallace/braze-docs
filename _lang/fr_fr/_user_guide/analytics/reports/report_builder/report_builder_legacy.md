...
page_order: 1
page_type: reference
description: "Cette page explique comment exécuter un rapport à l'aide de l'ancien générateur de rapports, y compris la création de rapports de comparaison de Campaigns et de Canvas, ainsi que la création de rapports et de graphiques."
tool:
  - Reports
...
> Le générateur de rapports vous permet de comparer les résultats de plusieurs Campaigns ou Canvas dans une seule vue, afin de déterminer facilement quelles stratégies d'engagement ont le plus impacté vos indicateurs clés. Pour les Campaigns comme pour les Canvas, vous pouvez exporter vos données et enregistrer votre rapport pour le consulter ultérieurement.<br><br>Pour une liste descriptive des indicateurs que vous trouverez dans vos rapports, consultez le [Glossaire des indicateurs de rapport]({{site.baseurl}}/user_guide/analytics/metrics_glossary/).

![Exemple de comparaison de Campaigns]({% image_buster /assets/img/campaign_comparison/campaign_main.png %}){: style="max-width:80%;"}

Utilisez ce rapport pour répondre à des questions clés sur l'engagement, par exemple :

- Quelles ont été les Campaigns ou les Canvas les plus performants pour une étiquette ou un canal spécifique ?
- Quelles variantes des Campaigns multivariantes ont eu le plus d'impact par rapport au groupe de contrôle ?
...
{% alert tip %}
Essayez d'utiliser les mêmes événements de conversion pour les conversions A, B, etc. dans les Campaigns et les Canvas que vous souhaitez comparer, afin de pouvoir aligner ces conversions dans vos rapports du générateur de rapports.
{% endalert %}
...
Si vous choisissez d'exécuter un rapport sur les Campaigns, vous pouvez sélectionner un rapport **Manuel** ou **Automatisé**. Les rapports peuvent contenir soit des Campaigns, soit des Canvas, mais pas les deux ensemble. Toutes les Campaigns et tous les Canvas dont les derniers messages ont été envoyés au cours des 12 derniers mois seront éligibles pour un rapport.

![Tableau de bord des Campaigns]({% image_buster /assets/img/campaign_comparison/create_report.png %}){: style="max-width:80%;"}
...
Les rapports Canvas fonctionnent de manière similaire à un rapport manuel de Campaigns, en ce sens que les sélections de Canvas et les mises à jour du rapport doivent également être effectuées manuellement. Vous pouvez inclure au maximum cinq Canvas dans un seul rapport.

### Étape 2 : Choisir vos indicateurs {#step-2-choose-your-metrics}

Après avoir créé votre rapport, vous trouverez un tableau vide contenant des Campaigns sur chaque ligne. Le tableau se remplira après avoir sélectionné **Modifier les colonnes** et choisi les indicateurs que vous souhaitez ajouter.

![Options de Campaigns]({% image_buster /assets/img/campaign_comparison/campaign_comparison_columns.png %}){: style="max-width:80%;"}

Votre tableau se remplira avec les indicateurs que vous avez choisis. Pour les définitions de ces indicateurs, consultez le [Glossaire des indicateurs de rapport]({{site.baseurl}}/user_guide/analytics/metrics_glossary/). Certains indicateurs ne sont disponibles que pour les rapports de comparaison de Campaigns.
...
Vous pouvez sélectionner une période spécifique pour consulter les données de votre rapport. Si une Campaign, un Canvas, une variante de Canvas ou un composant de Canvas particulier ne dispose d'aucune donnée pour la période sélectionnée, les résultats de cette ligne seront vides.

![Indicateur numérique de Campaign]({% image_buster /assets/img/campaign_comparison/metric.png %}){: style="max-width:60%;"}
...
![Note de Campaign]({% image_buster /assets/img/campaign_comparison/comparison_name.png %}){: style="max-width:60%;"}
...
## Rapport de comparaison de Campaigns avec des Campaigns multivariantes {#campaign-comparison-report-with-multivariate-campaigns}

Pour toute Campaign multivariante, vous pouvez consulter ces indicateurs ventilés par variantes et groupe de contrôle en cliquant sur la flèche à côté du nom de la Campaign. Les lignes contenant vos variantes incluront les résultats de performance pour cette variante, et la ligne contenant votre groupe de contrôle n'inclura que les résultats de vos événements de conversion.

![Note de Campaign]({% image_buster /assets/img/campaign_comparison/compare_note.png %}){: style="float:right;max-width:15%;margin-left:15px;"}

Les indicateurs remplissant la ligne de votre Campaign globale refléteront la performance de ses variantes, mais n'incluront pas la performance du groupe de contrôle. Par exemple, l'événement de conversion principal A pour votre Campaign globale sera la somme de l'événement de conversion principal A pour vos variantes, et n'inclura pas l'événement de conversion principal A pour votre groupe de contrôle.

{% alert important %}
Si vous supprimez une variante d'une Campaign multivariante, les données de cette variante ne seront pas disponibles pour une utilisation dans un futur rapport.
{% endalert %}
...
Lorsque vous accédez à un **rapport manuel** enregistré, vous pouvez consulter les mêmes Campaigns que vous aviez précédemment ajoutées, car elles correspondent toujours à votre filtre « Dernier envoi ».

Lorsque vous accédez à un **rapport automatisé** enregistré, le rapport se mettra automatiquement à jour pour inclure toutes les Campaigns correspondant actuellement à vos filtres. Par exemple, si votre rapport filtrait les Campaigns avec l'étiquette « Promotion », alors chaque fois que vous consulterez ce rapport, vous pourrez voir toutes les Campaigns avec l'étiquette « Promotion », même si ces Campaigns ont été créées après la création de ce rapport.

## Modifier les rapports {#editing-reports}

Dans un **rapport manuel**, vous pouvez modifier un rapport en sélectionnant **Modifier**. À partir de là, vous pouvez sélectionner ou désélectionner les Campaigns à inclure dans votre rapport.
...
Si votre rapport contient des Campaigns multivariantes, votre export inclura deux fichiers CSV :

- Un fichier contenant uniquement les indicateurs de haut niveau pour chaque Campaign
...
![Téléchargement de Campaign]({% image_buster /assets/img/campaign_comparison/download.png %}){: style="max-width:60%;"}
...
Utilisez les graphiques pour visualiser un indicateur sélectionné dans votre rapport. Les graphiques sont disponibles pour les rapports qui comportent des Campaigns et qui ont au moins un indicateur ajouté à leurs colonnes.

![Graphique de performance de Campaign avec l'indicateur Messages envoyés sélectionné]({% image_buster /assets/img/campaign_comparison/report_builder_charts.png %})
...
Si votre graphique ne contient qu'un seul indicateur, il affichera jusqu'à 30 Campaigns par ordre décroissant en fonction de l'indicateur sélectionné. Par exemple, si l'indicateur de votre graphique est les clics d'e-mail, alors votre graphique affichera les 30 Campaigns d'e-mail avec le plus de clics, classées du plus grand au plus petit nombre de clics. Si votre rapport contient plus de 30 Campaigns, seules les 30 premières seront affichées dans le graphique. Si vous sélectionnez plus d'un indicateur, votre graphique n'affichera que les cinq premières Campaigns en fonction du premier indicateur sélectionné.
