...
2. Le panneau de discussion Operator s'ouvre dans un panneau latéral.

![Le panneau de discussion Operator.]({% image_buster /assets/img/operator/operator_chat_panel.png %})
...
Décrivez ce que vous essayez d'accomplir en langage naturel. Des prompts clairs et précis produisent des réponses plus utiles. Vos prompts peuvent aller d'une simple question à une demande de création complète :
...
Operator peut fournir des instructions pas à pas, des liens vers la documentation Braze, des explications en langage courant, ainsi que des brouillons de Campaigns, de Canvas, de Segments et de contenu que vous pouvez examiner et insérer directement dans votre travail. Pour comprendre comment Operator propose et applique des modifications, consultez [Agir avec Operator](#take-action-with-operator).

Operator utilise [GPT-5.6 Terra](https://developers.openai.com/api/docs/models/gpt-5.6-terra), un modèle adapté aux tâches complexes et multi-étapes. Pour découvrir l'ensemble de ce qu'Operator peut vous aider à créer, consultez [Ce que vous pouvez faire avec Operator]({{site.baseurl}}/user_guide/brazeai/operator/capabilities). Pour des exemples prêts à l'emploi, consultez la [bibliothèque de prompts]({{site.baseurl}}/user_guide/brazeai/operator/prompt_library).
...
- **Soyez précis :** Au lieu de « Parle-moi de Canvas », essayez « Comment utiliser les parcours d'action dans Canvas ? ».
...
- **Tirez parti du contexte lié à la page :** Operator comprend votre emplacement dans Braze. Ouvrez Operator en consultant la page concernée pour obtenir les résultats les plus pertinents.
...
![Sélection des directives de marque dans le panneau de discussion Operator.]({% image_buster /assets/img/operator/operator_brand_guidelines.png %})
...
Cette prise en compte du contexte signifie que vous pouvez interagir avec Operator à l'aide de prompts courts et naturels, comme « Mets à jour les paramètres de mon éditeur pour correspondre à mes directives de marque. » Lorsque votre demande nécessite une autre partie du tableau de bord, Operator peut [vous y conduire]({{site.baseurl}}/user_guide/brazeai/operator/capabilities#navigate-the-dashboard) directement.
...
Tout d'abord, allez dans **Tools** > **Preferences** et sélectionnez **Automatically detect Markdown**. Ensuite, pour coller du Markdown, allez dans **Edit** > **Paste from Markdown**. Vous pouvez également faire un clic droit et sélectionner **Paste from Markdown**.

{% endtab %}
{% tab Microsoft Word et Outlook %}

Word et Outlook n'affichent pas le Markdown nativement. Collez la réponse dans un outil de prévisualisation Markdown en ligne, puis copiez le rendu et collez-le dans Word ou Outlook avec **Keep Source Formatting**. Vous pouvez également coller en texte brut et mettre en forme manuellement.
...
En bas de chaque réponse, utilisez les boutons pouce levé ou pouce baissé pour donner un retour rapide. Vos retours contribuent à améliorer les réponses d'Operator au fil du temps.

## Confidentialité et sécurité des données {#data-privacy-and-security}

BrazeAI Operator<sup>TM</sup> s'intègre à OpenAI, qui agit en tant que sous-traitant de Braze soumis à l'Addendum relatif au traitement des données (DPA) conclu entre vous et Braze. Les données envoyées à OpenAI via Braze ne sont pas utilisées pour entraîner ou améliorer les modèles d'OpenAI. Pour plus de détails sur la conformité HIPAA, la conservation des données, le traitement des données d'identification et la gouvernance, consultez [Confidentialité et sécurité des données]({{site.baseurl}}/user_guide/brazeai/operator/data_privacy_security).
