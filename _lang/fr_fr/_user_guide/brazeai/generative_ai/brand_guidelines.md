...
1. Dans votre compositeur de messages, recherchez et sélectionnez <i class="fa-solid fa-wand-magic-sparkles" title="Assistant de rédaction IA"></i> pour [ouvrir l'assistant de rédaction de l'intelligence artificielle]({{site.baseurl}}/user_guide/brazeai/generative_ai/copywriting/#access).
2. Sélectionnez **Apply brand guideline**, puis **Create a brand guideline**.

![Menu déroulant « Apply brand guidelines » ouvert avec le bouton « Create a brand guideline » en surbrillance.]({% image_buster /assets/img/ai_copywriter/create_brand_guideline_button.png %}){:style="max-width:75%"}

{: start="3"}

3. Saisissez un nom pour cette directive. Il s'agira de l'étiquette qui apparaît dans la sélection précédente.
4. Pour la rubrique **When will you use these brand guidelines?**, ajoutez des détails afin d'aider vos collègues (et vous à l'avenir) à comprendre le contexte d'utilisation de cette directive.
5. Si vous souhaitez qu'il s'agisse de la directive de marque par défaut pour l'espace de travail actuel, cochez la case **Use as default brand guideline**.
...
Testez vos directives pour en évaluer le résultat. Développez la section **Test your guidelines** pour générer des exemples de textes et les ajuster si nécessaire.
...
Lorsque vous êtes satisfait de vos directives, sélectionnez **Save brand guideline**. Vos nouvelles directives seront enregistrées dans votre espace de travail pour une utilisation ultérieure.
