...
Votre audience est l'ensemble des clients avec lesquels l'agent interagit. Dans Go, les audiences sont définies dans votre CEP :
...
2. Actualisez cette Data Extension quotidiennement avec les dernières données clients.
...
{% alert note %}
Les dimensions spécifiques disponibles dépendent de votre CEP et de la configuration de vos Campaigns. Utilisez les modèles et le contenu déjà configurés dans votre CEP.
{% endalert %}
...
| Groupe | Description |
|-------|-------------|
| **Decisioning Studio Go** | Les clients qui reçoivent des recommandations optimisées par l'IA |
...
{% alert important %}
Pour une comparaison précise, aucun client ne peut appartenir à plus d'un groupe d'expérimentation, et les clients sont répartis de manière aléatoire dans les groupes, sans biais.
{% endalert %}
...
- **Variez le contenu :** Utilisez des options réellement différentes. Tester des variations mineures peut ne pas fournir d'informations significatives.
...
Une fois votre agent conçu, configurez-le et lancez-le dans le tableau de bord de Braze :
