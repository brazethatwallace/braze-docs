...
{% alert tip %}
Les [recommandations personnalisées par l'IA](#recommendation-types) fonctionnent au mieux avec au moins quelques centaines d'articles de catalogue, au maximum 100 000 articles de catalogue, et généralement au moins 30 000 utilisateurs disposant de données d'achat ou d'interaction. Il ne s'agit que d'un guide approximatif et ces valeurs peuvent varier. Les autres types de recommandation peuvent fonctionner avec moins de données, notamment lorsque **Plus populaire** est utilisé comme solution de repli.
{% endalert %}
...
- Des données d'achat ou d'événement dans Braze (événements personnalisés, événement de commande passée ou objet d'achat) qui incluent une référence à l'article et doivent correspondre aux ID d'articles du catalogue.

### Étape 1 : Créer une nouvelle recommandation {#step-1-create-a-new-recommendation}

Vous pouvez créer une recommandation d'articles par IA depuis deux emplacements du tableau de bord :
...
Sélectionnez un type de recommandation. Chaque type utilise les six derniers mois de données d'interaction avec les articles, comme un achat, une commande passée ou des données d'événements personnalisés. Pour des informations plus détaillées et des cas d'usage pour chaque type, consultez [Types et cas d'usage]({{site.baseurl}}/user_guide/brazeai/item_recommendations).

{% alert tip %}
Lorsque vous utilisez **Plus récent** ou **Personnalisé par l'IA**, les utilisateurs ne disposant pas de suffisamment de données pour créer des recommandations individualisées reçoivent les articles **Plus populaires** en solution de secours. La solution de secours **Plus populaire** ne renvoie que les articles qui existent dans le catalogue associé.<br><br>Pour les recommandations **Personnalisé par l'IA**, consultez le **taux de personnalisation** sur la page **Analytics** pour voir quel pourcentage d'utilisateurs ayant effectué l'événement configuré au cours des 24 derniers mois dispose de recommandations personnalisées stockées sur leur profil. Pour les recommandations **Plus récent**, la page **Analytics** affiche la proportion d'utilisateurs recevant des recommandations **Plus récent** par rapport à la solution de secours **Plus populaire**.
{% endalert %}
...
Pour éviter de suggérer des articles qu'un utilisateur a déjà achetés ou avec lesquels il a déjà interagi, sélectionnez **Do not recommend items users have previously interacted with**. Cette option est disponible uniquement lorsque le **Type** de recommandation est défini sur **AI Personalized**.
...
Si vous souhaitez davantage de contrôle sur votre recommandation, choisissez une [sélection]({{site.baseurl}}/user_guide/data/activation/catalogs/selections) pour appliquer des filtres personnalisés. Les sélections filtrent les recommandations par colonnes spécifiques de votre catalogue, telles que la marque, la taille ou l'emplacement. Les sélections contenant du Liquid ne peuvent pas être utilisées dans votre recommandation.

![Un exemple de sélection « en stock » sélectionnée pour la recommandation.]({% image_buster /assets/img/item_recs_2-2.png %})
...
Sélectionnez l'événement pour lequel vous souhaitez que cette recommandation soit optimisée. Cet événement est généralement un achat, mais il peut aussi s'agir de toute interaction avec un article.

{% alert tip %}
Lors de la configuration des recommandations d'articles par IA, votre choix d'événement est important. Votre événement déclencheur détermine qui reçoit une recommandation générée par IA — les recommandations d'articles par IA sont générées pour les utilisateurs ayant effectué l'événement que vous configurez, ce qui détermine directement qui reçoit des recommandations. Sélectionnez un événement qui couvre l'ensemble du Segment d'audience que vous souhaitez atteindre.<br><br>En même temps, trouvez un équilibre entre la couverture et la pertinence. Les événements en haut de l'entonnoir (comme la consultation de produit) ont tendance à capturer une audience plus large mais sont moins liés aux résultats commerciaux, tandis que les événements en bas de l'entonnoir (comme les achats) tendent à produire des recommandations plus ciblées et pertinentes pour l'activité. Le meilleur événement est celui qui équilibre la couverture et l'impact sur les résultats.
{% endalert %}
...
- Les événements personnalisés représentant toute autre interaction avec un article (comme les consultations de produit, les clics ou les lectures de médias)
...
![L'événement personnalisé « purchase » sélectionné comme mode actuel de suivi des événements.]({% image_buster /assets/img/item_recs_3.png %})

{% alert note %}
Les événements personnalisés doivent disposer de suffisamment de données avant d'apparaître dans la liste des événements. Si votre événement personnalisé n'apparaît pas, il se peut que le backend Braze ne l'ait pas encore traité ou qu'il ne dispose pas de données suffisantes pour l'entraînement du modèle. Les recommandations par IA s'appuient sur des données historiques pour générer des informations, de sorte que les événements nouvellement créés ou rarement déclenchés ne seront pas disponibles tant que davantage de données n'auront pas été collectées.
{% endalert %}

### Étape 5 : Choisir le nom de la propriété correspondante {#property-name}

Pour créer une recommandation, vous devez indiquer à Braze quel champ de votre événement d'interaction (événement de commande passée, objet d'achat ou événement personnalisé) contient l'identifiant unique correspondant au champ `id` d'un article dans le catalogue. Vous hésitez ? [Consultez les exigences](#requirements).

Sélectionnez ce champ pour le **Property Name**.

Le champ **Property Name** est prérempli avec une liste de champs envoyés via le SDK à Braze. Si suffisamment de données sont fournies, ces propriétés sont également classées par ordre de probabilité d'être la propriété correcte. Sélectionnez celle qui correspond au champ `id` du catalogue.

![Le nom de propriété « purchase_item » sélectionné qui correspond aux ID d'articles dans le catalogue.]({% image_buster /assets/img/item_recs_4.png %})
...
- **Si vous avez sélectionné l'événement de commande passée ou si vous utilisez des [événements eCommerce]({{site.baseurl}}/user_guide/data/activation/events/recommended_events/ecommerce_events) pour entraîner les recommandations d'articles :** Saisissez `products.product_id` pour l'ID du produit.
...
- **Si vous avez sélectionné l'objet d'achat :** Il doit s'agir du `product_id` ou d'un champ de l'objet `properties` de votre événement d'interaction.
- **Si vous avez sélectionné un événement personnalisé :** Il doit s'agir d'un champ de l'objet `properties` de votre événement personnalisé.
- Les champs imbriqués doivent être saisis dans le menu déroulant **Property Name** en notation pointée au format `event_property.nested_property`. Par exemple, pour sélectionner la propriété imbriquée `district_name` dans la propriété d'événement `location`, vous saisiriez `location.district_name`. Pour en savoir plus sur les propriétés imbriquées dans les événements personnalisés, consultez [Objets imbriqués]({{site.baseurl}}/user_guide/data/activation/events/custom_events/nested_objects).

#### Exemples de mappages {#example-mappings}

Les exemples de mappages suivants font tous référence à ce catalogue type :
...
Supposons que vous souhaitez utiliser l'événement personnalisé `added_to_cart` pour recommander des produits similaires avant que le client ne passe en caisse. L'événement `added_to_cart` possède une propriété d'événement `product_sku`.

Alors la propriété `product_sku` doit inclure au moins l'une des valeurs de la colonne `id` dans le catalogue type : « ADI-BL-7 », « ADI-RD-8 », « ADI-WH-9 » ou « ADI-PP-10 ». Vous n'avez pas besoin d'événements pour chaque article du catalogue, mais vous en avez besoin de quelques-uns pour que le moteur de recommandation ait suffisamment de contenu pour fonctionner.

##### Exemple d'objet d'événement personnalisé {#example-custom-event-object}

Cet événement a `"product_sku": "ADI-BL-7"`, ce qui correspond au premier article du catalogue type.
...
Si les propriétés de votre événement contiennent plusieurs produits dans un tableau, chaque ID de produit sera traité comme un événement séparé et séquentiel. Cet événement peut utiliser la propriété `products.sku` pour correspondre aux premier et troisième articles du catalogue type.
...
En termes de mappage, une logique similaire s'applique pour les objets d'achat comme pour les événements personnalisés, sauf que vous pouvez choisir entre le `product_id` de l'objet d'achat ou un champ de l'objet `properties`.

N'oubliez pas que vous n'avez pas besoin d'événements pour chaque article du catalogue, mais vous en avez besoin de quelques-uns pour que le moteur de recommandation ait suffisamment de contenu pour fonctionner.

##### Exemple d'objet d'achat mappé à l'ID du produit {#example-purchase-object-mapped-to-product-id}

Cet événement a `"product_id": "ADI-BL-7"`, ce qui correspond au premier article du catalogue.
...
Cet événement a une propriété `"sku": "ADI-RD-8"`, ce qui correspond au deuxième article du catalogue.
...
##### Exemple d'objet de commande passée mappé à l'ID du produit {#example-order-placed-object-mapped-to-product-id}
...
Lorsque vous êtes prêt, sélectionnez **Create Recommendation**. Ce processus peut prendre entre 10 minutes et 36 heures. Vous recevrez une notification par e-mail lorsque la recommandation sera entraînée avec succès ou une explication indiquant pourquoi la création a pu échouer.

Vous trouverez la recommandation sur la page **Predictions**, où vous pourrez ensuite la modifier ou l'archiver selon vos besoins. Les recommandations sont automatiquement réentraînées une fois par semaine (version payante) ou par mois (version gratuite).