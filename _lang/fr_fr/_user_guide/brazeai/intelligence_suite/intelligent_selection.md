...
> La sélection intelligente est une fonctionnalité qui analyse deux fois par jour les performances d'une Campaign ou d'un Canvas récurrent et ajuste automatiquement le pourcentage d'utilisateurs qui reçoivent chaque variante de message.

## Conditions préalables {#prerequisites}

{% tabs %}
{% tab Campaign %}
Avant d'ajouter la sélection intelligente à votre Campaign, assurez-vous d'avoir correctement configuré les éléments suivants :

- Votre Campaign est envoyée selon une planification récurrente. Les Campaigns à envoi unique ne sont pas prises en charge.
...
![Section de test A/B d'une Campaign où la sélection intelligente est activée.]({% image_buster /assets/img/intelligent_selection1.png %})

La sélection intelligente va :
- Examiner à plusieurs reprises les données de performance et déplacer progressivement le trafic de la Campaign vers les variantes gagnantes.
...
La sélection intelligente fonctionne mieux pour les Campaigns envoyées plusieurs fois. Elle a besoin de données de performance précoces pour commencer à optimiser, de sorte que les Campaigns à envoi unique n'en bénéficieront pas. Pour ces Campaigns, nous vous recommandons plutôt d'utiliser un [test A/B]({{site.baseurl}}/user_guide/messaging/ab_testing/) traditionnel.


Vous pouvez ajouter la sélection intelligente à vos Campaigns et à vos Canvas.

{% tabs %}
{% tab Campaign %}
La sélection intelligente peut être ajoutée à n'importe quelle Campaign multi-envoi dans l'étape **Target Audiences** du compositeur de Campaign de Braze. Les Campaigns qui n'envoient qu'une seule fois ne peuvent pas bénéficier de cette fonctionnalité.

{% alert note %}
La sélection intelligente ne peut pas être utilisée dans les Campaigns dont la période de rééligibilité est inférieure à 24 heures, car elle affecterait l'intégrité de la variante de contrôle. Pour en savoir plus, consultez la [FAQ sur l'intelligence]({{site.baseurl}}/user_guide/brazeai/intelligence/faqs/#why-is-re-eligibility-in-less-than-24-hours-not-available-when-combined-with-intelligent-selection).
{% endalert %}
{% endtab %}
...
La sélection intelligente fonde la distribution des variantes sur l'état actuel des conversions de la Campaign. Elle détermine uniquement les distributions finales après la période de formation.

Cela signifie qu'au début de la Campaign, les sélections intelligentes à 99 % et à 1 % peuvent recevoir environ le même nombre d'envois, mais que les pourcentages finaux pour l'attribution des variantes peuvent être fixés à 99 %–1 %.

Si vous ne souhaitez pas que la sélection intelligente envoie à parts égales au début de la Campaign, nous vous recommandons d'utiliser un test A/B traditionnel avec des variantes fixes.
...
Nous ne permettons pas aux Campaigns avec sélection intelligente d'avoir une rééligibilité dans une fenêtre trop courte, car cela affecterait l'intégrité de la variante de contrôle. En créant un intervalle de 24 heures, nous contribuons à garantir que l'algorithme disposera d'un ensemble de données statistiquement valide.

Normalement, les Campaigns avec rééligibilité amèneront les utilisateurs à recevoir de nouveau la même variante qu'auparavant. Avec la sélection intelligente, Braze ne peut pas garantir qu'un utilisateur recevra la même variante de Campaign, car la distribution des variantes aurait changé en raison de l'aspect d'allocation optimale de cette fonctionnalité. Si l'utilisateur était autorisé à entrer de nouveau avant que la sélection intelligente ne réexamine les performances de la variante, les données pourraient être biaisées en raison des utilisateurs réentrés.

Par exemple, si une Campaign utilise ces variantes :
...
### Pourquoi mes variantes de sélection intelligente affichent-elles des envois égaux au début de ma Campaign ? {#why-are-my-intelligent-selection-variants-showing-equal-sends-during-the-early-stages-of-my-campaign}

La sélection intelligente alloue les variantes à envoyer en fonction de l'état actuel des conversions de la Campaign. Elle détermine uniquement les attributions de variantes finales après une période de formation durant laquelle les envois sont répartis de manière uniforme entre les variantes. Si vous ne souhaitez pas que la sélection intelligente envoie de manière uniforme au début de votre Campaign, utilisez des variantes fixes pour un test A/B traditionnel.
...
### Pourquoi ne puis-je pas activer la sélection intelligente dans mon Canvas ou ma Campaign (option grisée) ? {#why-cant-i-enable-intelligent-selection-in-my-canvas-or-campaign-grayed-out}
...
- Vous créez une Campaign à envoi unique
