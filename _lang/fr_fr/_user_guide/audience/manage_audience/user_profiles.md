...
- **E-mail :** Braze enregistre un envoi lorsque le message est transmis à votre fournisseur de services d'e-mail marketing or e-mailing (fournisseur de services d'e-mailing). Après cette transmission, le message n'est pas annulé en raison de la logique Liquid, de la limitation du débit ou du marquage de l'utilisateur comme injoignable. Les événements suivants sont généralement une distribution ou un rebond.
...
Pour chaque événement du tableau **Historique de communication**, vous pouvez voir le canal de communication, le type d'événement, l'horodatage de l'événement, la Campaign ou le message Canvas associé, ainsi que les données de l'appareil de l'utilisateur. Pour filtrer des événements spécifiques, cliquez sur **Filters** et sélectionnez les événements dans la liste.

##### Événements d'engagement liés aux messages {#message-engagement-events}

Les événements d'engagement suivants sont disponibles pour l'e-mail, le SMS, les notifications push, les messages in-app, les Content Cards et les webhooks. Pour en savoir plus sur le suivi de ces événements, consultez le [Glossaire des événements d'engagement liés aux messages]({{site.baseurl}}/user_guide/data/distribution/braze_currents/event_glossary/message_engagement_events).

| Canal | Événements d'engagement disponibles |
| --- | --- |
| E-mail | Rebond<br>Clic<br>Événements de report<br>Distribution<br>Signalement comme spam<br>Ouverture (voir la [note sur l'événement d'ouverture d'e-mail](#note-on-email-open-event))<br>Envoi<br>Échec provisoire d'envoi<br>Désabonnement |
...
- Lorsqu'un événement ne contient pas de données pour **Message Sent**, cela indique que la Campaign ne possède aucune variante de message.
- Lorsqu'un événement ne contient pas de données pour **Campaign/Canvas** et **Message Sent**, cela indique que ce message a été envoyé depuis une Campaign API (et non des Campaigns déclenchées par API) qui n'a pas spécifié les paramètres `campaign_id` et `message_variation_id`. Ces champs sont facultatifs et peuvent être omis du corps de la requête. Lorsque ces champs sont spécifiés, les informations correspondantes sont renseignées dans les journaux de l'historique de communication.
...
## Contraintes de taille des données {#data-size-constraints}

Braze n'impose pas de limite globale sur la taille totale d'un profil utilisateur. Cependant, certains types de données associés aux utilisateurs ont des capacités de taille définies.

### Tableaux d'attributs personnalisés {#custom-attribute-arrays}

Les tableaux d'attributs personnalisés (y compris les tableaux d'objets) ont une capacité de 100 Ko. Lorsque vous envoyez un tableau qui dépasse cette capacité, l'attribut personnalisé n'est pas traité. L'API renvoie une réponse de succès (201), mais le tableau n'apparaît pas dans le profil utilisateur et les données existantes pour cet attribut ne sont pas mises à jour.

Si vos tableaux approchent de cette capacité, envisagez de limiter le nombre d'objets que vous renseignez afin de maintenir la taille totale en dessous de 100 Ko.

Pour plus d'informations sur les attributs personnalisés, consultez [Types de données des attributs personnalisés]({{site.baseurl}}/user_guide/data/activation/custom_data/data_types#custom-attribute-data-types).
