...
{% alert important %}
Avant de pouvoir tester des messages de type bannière dans Braze, vous devez créer une campagne de bannière dans Braze. De plus, vérifiez que l'emplacement que vous souhaitez tester est déjà [intégré dans votre application ou votre site web]({{site.baseurl}}/developer_guide/banners/placements).
{% endalert %}
...
2. Sélectionnez **Aperçu** pour prévisualiser votre bannière ou envoyer un message de test.
...
{% alert note %}
Gardez à l'esprit que votre aperçu peut ne pas être identique au rendu final sur l'appareil d'un utilisateur en raison de différences matérielles.
{% endalert %}

### Liste de vérification du test {#test-checklist}

- Votre campagne de bannière est-elle assignée à un emplacement ?
...
3. Sélectionnez **Envoyer le test** pour envoyer votre Content Card à votre application.

![Content Card de test]({% image_buster /assets/img/contentcard_test.png %})

### Aperçu {#preview}

Vous pouvez prévisualiser votre carte pendant sa composition dans l'onglet **Aperçu**. Cela devrait vous aider à visualiser à quoi ressemblera votre message final du point de vue de l'utilisateur.

{% alert note %}
Dans l'onglet **Aperçu** de votre compositeur, l'affichage de votre message peut ne pas être identique à son rendu réel sur l'appareil de l'utilisateur. Nous recommandons de toujours envoyer un message de test à un appareil pour vous assurer que vos médias, votre texte, votre personnalisation et vos attributs personnalisés s'affichent correctement.
{% endalert %}
...
2. Sélectionnez **Aperçu et test**.
3. Sélectionnez l'onglet **Envoi de test** et ajoutez votre adresse e-mail ou votre ID utilisateur dans le champ **Ajouter des utilisateurs individuels**.
4. Sélectionnez **Envoyer le test** pour envoyer votre e-mail rédigé dans votre boîte de réception.
...
{% alert warning %}
Pour envoyer un test à des [groupes de test de contenu]({{site.baseurl}}/user_guide/administer/global/user_management/internal_groups#content-test-groups) ou à des utilisateurs individuels, les notifications push doivent être activées sur vos appareils de test avant l'envoi. Par exemple, vous devez avoir les notifications push activées sur votre appareil iOS pour appuyer sur la notification avant que le message de test ne s'affiche. {% endalert %}
...
3. Sélectionnez **Envoyer le test** pour envoyer votre notification push à votre appareil.

Une notification push de test apparaîtra en haut de l'écran de votre appareil.

![Test de message in-app]({% image_buster /assets/img_archive/test-in-app.png %})

{% alert important %}
Les envois de test peuvent entraîner l'envoi de plusieurs messages in-app à chaque destinataire.
{% endalert %}

En cliquant directement sur la notification push et en l'ouvrant, vous serez redirigé vers votre application, où vous pourrez consulter votre message in-app de test. Notez que cette fonctionnalité de test de message in-app repose sur le fait que l'utilisateur clique sur une notification push de test pour déclencher le message in-app. L'utilisateur doit donc être éligible pour recevoir des notifications push dans l'application concernée pour que la notification push de test soit correctement envoyée.

### Aperçu

Vous pouvez prévisualiser votre message in-app pendant sa composition dans l'onglet **Aperçu**. Cela devrait vous aider à visualiser à quoi ressemblera votre message final du point de vue de l'utilisateur. Vous pouvez prévisualiser l'apparence de votre message pour un utilisateur aléatoire, un utilisateur spécifique ou un utilisateur personnalisé. Vous pouvez également prévisualiser les messages pour les appareils mobiles ou les tablettes.

![Onglet Composition lors de la création d'un message in-app montrant l'aperçu du message. Aucun utilisateur n'est sélectionné, le Liquid ajouté dans la section du corps s'affiche donc tel quel.]({% image_buster /assets/img/in-app-message-preview.png %})
...
{% alert warning %}
Dans l'**Aperçu**, l'affichage de votre message peut ne pas être identique à son rendu réel sur l'appareil de l'utilisateur. Nous recommandons de toujours envoyer un message de test à un appareil pour vous assurer que vos médias, votre texte, votre personnalisation et vos attributs personnalisés s'affichent correctement.
{% endalert %}
...
Pour soutenir les bonnes pratiques d'accessibilité, Braze analyse automatiquement le contenu des messages in-app créés avec l'éditeur HTML traditionnel par rapport aux normes d'accessibilité. Ce scanner aide à identifier le contenu qui pourrait ne pas respecter les directives pour l'accessibilité des contenus web ([WCAG](https://www.w3.org/WAI/standards-guidelines/wcag/)). Les WCAG sont un ensemble de normes techniques reconnues internationalement, développées par le World Wide Web Consortium (W3C) pour rendre le contenu web plus accessible aux personnes en situation de handicap.
...
Le scanner s'exécute automatiquement sur les messages HTML personnalisés et évalue l'intégralité de votre message HTML par rapport à l'ensemble des [règles WCAG 2.1 AA](https://www.w3.org/WAI/WCAG22/quickref/?versions=2.1&currentsidebar=%23col_customize&levels=aaa). Pour chaque problème signalé, il affiche :

- L'élément HTML spécifique concerné
- Une description du problème d'accessibilité
- Un lien vers des informations complémentaires ou des conseils de correction
...
Si vous voyez une erreur indiquant qu'aucun des utilisateurs sélectionnés n'a de jeton push correspondant, l'utilisateur test ne dispose pas d'un jeton push valide pour la plateforme sélectionnée. L'utilisateur doit avoir démarré une session dans l'application et activé les notifications push pour cet appareil. Pour plus d'informations, consultez [Activation des notifications push et abonnement push]({{site.baseurl}}/user_guide/channels/push/push_setup/push_subscription_states).

#### Notification push Web {#web-push}

1. Créez votre notification push Web.
2. Sélectionnez l'onglet **Test**.
3. Sélectionnez **M'envoyer un test**.
4. Sélectionnez **Envoyer le test** pour envoyer votre notification push Web à votre navigateur web.

![Notification push Web de test]({% image_buster /assets/img_archive/testwebpush.png %})

Si vous avez déjà accepté les notifications push depuis le tableau de bord de Braze, le message s'affiche dans le coin de votre écran. Sinon, sélectionnez **Autoriser** lorsque vous y êtes invité, et le message s'affiche.

Si vous voyez une erreur indiquant qu'aucun des utilisateurs sélectionnés n'a de jeton push correspondant pour la notification push Web, vérifiez que l'utilisateur test dispose d'un jeton push valide enregistré pour la plateforme sélectionnée. Pour recevoir un jeton push, l'utilisateur doit être configuré pour recevoir des notifications push pour l'application sur son appareil. Pour plus de détails, consultez [Activation des notifications push et abonnement push]({{site.baseurl}}/user_guide/channels/push/push_setup/push_subscription_states).
...
Si vous prévisualisez en tant qu'utilisateur personnalisé, saisissez du texte pour les différents champs disponibles pour la personnalisation, tels que le prénom de l'utilisateur et les attributs personnalisés. Là encore, vous pouvez saisir votre propre adresse e-mail pour envoyer un test à votre appareil.
...
Vous pouvez modifier des champs individuels à partir d'un utilisateur aléatoire ou existant pour vous aider à tester le contenu dynamique de votre message. Sélectionnez **Edit** pour convertir l'utilisateur sélectionné en un utilisateur personnalisé que vous pouvez modifier.
...
Vous pouvez déclencher la campagne vous-même comme méthode robuste pour tester des campagnes personnalisées utilisant des propriétés d'événement personnalisées :

1. Rédigez le contenu impliquant la propriété d'événement.

![Rédaction d'un message de test avec des propriétés]({% image_buster /assets/img_archive/testeventproperties-compose.png %})

{: start="2"}
2. Utilisez la [livraison par événement]({{site.baseurl}}/user_guide/messaging/campaigns/schedule_your_campaign/triggered_delivery) pour distribuer la campagne lorsque l'événement se produit.
...
3. Ajoutez la propriété d'événement personnalisée en bas de la page, et ajoutez votre ID utilisateur ou votre adresse e-mail dans le champ supérieur.
...
- L'affichage du [centre de préférences]({{site.baseurl}}/user_guide/audience/subscription_preferences/preference_center) Braze à partir de messages de test entraîne la désactivation du bouton **Save Preferences**. Les étiquettes Liquid du centre de préférences peuvent également ne pas se résoudre en liens valides. Il s'agit d'un comportement attendu. Pour tester de bout en bout, consultez [Tester les centres de préférences]({{site.baseurl}}/user_guide/audience/subscription_preferences/preference_center/dnd_preference_center#testing-preference-centers).
...
- Les e-mails envoyés aux utilisateurs du groupe initiateur ne mettent pas à jour la liste des Campaigns reçues dans le profil utilisateur et n'incrémentent pas les envois dans les analyses du tableau de bord.
