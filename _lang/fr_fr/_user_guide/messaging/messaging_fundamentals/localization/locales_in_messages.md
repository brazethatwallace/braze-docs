...
{% alert important %}
Si vous utilisez le [suivi de liens par e-mail](#email-link-tracking) (aliasing de lien ou modèles de lien), une configuration supplémentaire est nécessaire lorsque les URL sont encadrées dans des balises de traduction.
{% endalert %}
...
Dans les Campaigns par e-mail, Braze suit les liens en ajoutant des informations de suivi (paramètres de requête) à chaque URL. Ce comportement prend en charge à la fois l'[aliasing de lien]({{site.baseurl}}/user_guide/messaging/templates/email_templates/link_aliasing) et le [templating de lien]({{site.baseurl}}/user_guide/messaging/templates/email_templates/link_template).
