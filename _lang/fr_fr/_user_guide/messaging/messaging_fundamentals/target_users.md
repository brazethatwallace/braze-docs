...
page_order: 12
page_type: reference
description: "Cet article de référence explique comment cibler votre audience dans les éditeurs de campagne et de Canvas."
tool:
    - Campaigns
    - Canvas
---
...
> Déterminer comment cibler vos utilisateurs est l'une des étapes les plus cruciales lors de la création d'une campagne ou d'un Canvas. En comprenant comment segmenter votre audience en fonction de ses comportements, préférences et données démographiques, vous pouvez adapter et personnaliser vos messages.
...
Vous pouvez envoyer un message de campagne ou une étape du Canvas à des applications spécifiques, par exemple en envoyant un message in-app ou une notification push uniquement aux applications Android ou iOS.
...
Lorsque vous créez une campagne ou un Canvas dans Braze, le ciblage s'effectue en deux parties :

1. **Audience cible :** qui est éligible
2. **Critères d'entrée :** ce qui déclenche la distribution

L'ordre est important : Braze vérifie si une personne fait partie de l'audience cible avant d'évaluer les critères d'entrée. Si un utilisateur n'est pas déjà éligible pour l'audience à ce moment-là, il n'entrera pas dans la campagne ou le Canvas, même s'il déclenche ultérieurement l'événement d'entrée. Considérez l'audience cible comme une salle d'attente : seuls les utilisateurs qui s'y trouvent déjà lorsque le déclencheur se produit peuvent avancer.
