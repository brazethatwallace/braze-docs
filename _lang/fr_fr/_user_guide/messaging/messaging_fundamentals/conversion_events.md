...
- **Conservation des événements historiques :** le suivi des conversions sur les tableaux de bord des Campaigns et des Canvas mesure les actions historiques, et non les profils utilisateur actuels. Lorsqu'un utilisateur remplit une règle de conversion dans la fenêtre définie, Braze enregistre une conversion dans les analyses et ne la supprime pas, même si le profil de cet utilisateur est ultérieurement supprimé, fusionné ou archivé lors d'opérations courantes d'hygiène des données ou de conformité RGPD. Les comptages de Segments en direct peuvent naturellement être inférieurs à vos journaux d'événements permanents du tableau de bord, car les segments dynamiques ne filtrent que les profils actifs existant actuellement dans la base de données.
...
Il est courant que le nombre total de conversions d'une variante Canvas soit supérieur à la somme des conversions de ses étapes individuelles. Cela s'explique par le fait que les conversions sont suivies différemment au niveau de la variante et au niveau de l'étape :
...
- **L'utilisateur quitte le Canvas avant de recevoir une étape.** Si un utilisateur entre dans le Canvas mais le quitte (par exemple, en raison d'un filtre ou d'une incompatibilité d'audience) avant l'envoi d'un message, une conversion qu'il effectue est toujours comptabilisée au niveau de la variante, mais pas au niveau d'une étape.
- **L'étape cible un sous-ensemble d'utilisateurs.** Si une étape est configurée pour n'envoyer qu'à une plateforme spécifique (comme le mobile), les utilisateurs sur d'autres plateformes (comme le web) peuvent toujours entrer dans le Canvas et convertir. Comme ces utilisateurs ne reçoivent jamais le message de l'étape, la conversion n'est pas comptabilisée au niveau de l'étape, mais uniquement au niveau de la variante.
