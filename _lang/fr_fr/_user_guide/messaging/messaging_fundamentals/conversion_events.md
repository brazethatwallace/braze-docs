...
- Un utilisateur est considéré comme ayant converti s'il a effectué l'événement de conversion spécifique dans la fenêtre, même s'il n'a pas ouvert ou cliqué sur le message.

### Événement de conversion principal {#primary-conversion-event}

L'événement de conversion principal est le premier événement que vous ajoutez lors de la création d'une campagne ou d'un Canvas. Cet événement a le plus d'impact sur votre engagement et vos rapports. Braze utilise votre événement de conversion principal pour :

- Calculer la variante de message gagnante dans les campagnes ou Canvas [multivariés]({{site.baseurl}}/user_guide/messaging/ab_testing/optimizations#winning-variant).
- Déterminer la fenêtre pendant laquelle le chiffre d'affaires est calculé pour la campagne ou le Canvas.
- Ajuster les distributions de messages pour les campagnes et les Canvas utilisant la [sélection intelligente]({{site.baseurl}}/user_guide/brazeai/intelligence_suite/intelligent_selection).

Le nombre d'événements de conversion principaux correspond au nombre d'événements de conversion qui se sont produits. Pour les campagnes multicanales, Braze compte les conversions par canal (comme décrit dans les [règles de suivi des conversions](#conversion-tracking-rules)), ce qui signifie que le nombre de conversions peut dépasser le nombre d'utilisateurs uniques et entraîner des taux de conversion supérieurs à 100 %. Braze calcule le taux d'événement de conversion principal en divisant ce nombre par le nombre de destinataires uniques. Braze considère un utilisateur comme destinataire lorsque le message est envoyé ou affiché, selon le canal. Par exemple, pour les notifications push ou les e-mails, un utilisateur devient destinataire après que Braze a envoyé le message. Pour les messages in-app ou les Content Cards, l'utilisateur doit voir le message pour être considéré comme destinataire.
...
Utilisez autant d'événements de conversion que nécessaire. L'ajout d'un deuxième ou troisième événement de conversion enrichit considérablement vos rapports. Par exemple, pour une campagne ciblant les utilisateurs inactifs, l'ajout d'un événement de conversion secondaire en plus de l'événement de conversion principal **Démarre une session** vous aide à comprendre l'efficacité de votre campagne pour ramener les utilisateurs dans votre application.
...
|-------------------------|----------------------------|
| **Démarre une session** | Un utilisateur est comptabilisé comme converti lorsqu'il ouvre l'une des applications que vous spécifiez (par défaut, toutes les applications de l'espace de travail). |
| **Effectue un achat** | Un utilisateur est comptabilisé comme converti lorsqu'il enregistre un [événement d'achat]({{site.baseurl}}/api/objects_filters/purchase_object). Cela suit tout achat par défaut, ou vous pouvez spécifier un produit particulier. |
| **Passe une commande** | Un utilisateur est comptabilisé comme converti lorsqu'il déclenche l'[événement eCommerce recommandé Order Placed]({{site.baseurl}}/user_guide/data/activation/events/recommended_events/ecommerce_events#ecommerce-recommended-events?tab=ecommerce.order_placed). Cela suit toute commande par défaut, ou vous pouvez filtrer par un produit spécifique.<br><br>L'événement « Passe une commande » est actuellement en accès anticipé. Contactez votre gestionnaire de compte Braze si vous souhaitez participer à cet accès anticipé. |
| **Effectue un événement personnalisé** | Un utilisateur est comptabilisé comme converti lorsqu'il effectue l'un de vos événements personnalisés existants (pas de valeur par défaut, vous devez spécifier l'événement). |
| **Met à jour l'application** | Un utilisateur est comptabilisé comme converti lorsqu'il met à jour la version de l'application sur l'une des applications que vous spécifiez (par défaut, toutes les applications de l'espace de travail). Braze effectue une comparaison numérique au mieux pour déterminer si le changement était une mise à jour. Les versions non numériques sont comptabilisées comme des conversions si la version change. |
| **Ouvre un e-mail** | Un utilisateur est comptabilisé comme converti lorsqu'il ouvre l'e-mail (uniquement pour les campagnes par e-mail). |
| **Clique sur un e-mail** | Un utilisateur est comptabilisé comme converti lorsqu'il clique sur un lien dans l'e-mail (uniquement pour les campagnes par e-mail). |
...
Définissez votre délai de conversion. Il s'agit du temps maximum pouvant s'écouler avant que Braze ne considère une conversion. Vous pouvez définir une fenêtre allant jusqu'à 30 jours pendant laquelle Braze comptabilise la conversion si l'utilisateur effectue l'action spécifiée.

![Le type d'événement de conversion « Effectue un achat » comme exemple pour enregistrer les conversions des utilisateurs qui effectuent un achat. Le délai de conversion est de 12 heures.]({% image_buster /assets/img_archive/conversion_event_selection.png %})
...
Accédez à la page **Détails** pour afficher les détails de chaque événement de conversion associé à la campagne que vous avez créée. Indépendamment des événements de conversion sélectionnés, vous pouvez également voir le chiffre d'affaires total attribué à cette campagne spécifique, ainsi qu'aux variantes spécifiques, pendant la fenêtre de l'événement de conversion principal.

{% alert note %}
Si vous ne sélectionnez aucun événement de conversion lors de la création de la campagne, le délai par défaut est de trois jours.
{% endalert %}
...
## Taux de conversion des étapes Canvas par rapport aux variantes {#canvas-step-versus-variant-conversion-rates}

Il est courant que le nombre total de conversions d'une variante du Canvas soit supérieur à la somme des conversions de ses étapes individuelles. Cela s'explique par le fait que les conversions sont suivies différemment au niveau de la variante et au niveau de l'étape :
...
- **L'étape cible un sous-ensemble d'utilisateurs.** Si une étape est configurée pour n'envoyer qu'à une plateforme spécifique (comme le mobile), les utilisateurs sur d'autres plateformes (comme le web) peuvent tout de même entrer dans le Canvas et convertir. Comme ces utilisateurs ne reçoivent jamais le message de l'étape, la conversion n'est pas comptabilisée au niveau de l'étape — uniquement au niveau de la variante.

Pour en savoir plus sur l'analyse Canvas, consultez [Mesurer et tester avec l'analyse Canvas]({{site.baseurl}}/user_guide/messaging/canvas/testing_canvases/measuring_and_testing_with_canvas_analytics).