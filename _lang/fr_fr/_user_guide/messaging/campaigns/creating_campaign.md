...
Pour créer et lancer une campagne, vous avez besoin des autorisations « Edit Campaigns » et « Launch Campaigns ». Pour obtenir la liste complète des autorisations de l'espace de travail et leur affichage dans le tableau de bord, consultez [Autorisations]({{site.baseurl}}/user_guide/administer/global/user_management/permissions/).
...
L'éditeur de campagne est l'endroit où vous définissez la distribution, les audiences, les conversions et les paramètres de lancement. Décidez si vous créez une campagne monocanale ou multicanale avant de continuer.

{% tabs %}
{% tab Monocanale %}

Une campagne monocanale atteint les utilisateurs via un seul canal de communication par lancement.

### Ce qui change {#whats-different}

#### Conversions et rapports {#single-channel-conversions}

Pour les campagnes monocanales, Braze suit les [événements de conversion]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/conversion_events/) que vous affectez à la campagne par rapport aux envois de ce canal. Pour les fenêtres d'attribution et les règles de comptage, consultez [Règles de suivi des conversions]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/conversion_events/#conversion-tracking-rules).
...
### Créer une campagne monocanale {#create-a-single-channel-campaign}
...
Une campagne multicanale atteint les utilisateurs via plusieurs canaux de communication en un seul lancement. Par exemple, envoyez un e-mail et une notification push simultanément.

{% alert note %}
Les [messages in-app]({{site.baseurl}}/user_guide/channels/in_app_messages/) ne sont pas disponibles dans les campagnes multicanales. Créez plutôt une campagne monocanale ou un Canvas.
{% endalert %}
...
Les groupes de contrôle de campagne comparent les variantes au sein d'un même canal (par exemple, e-mail A contre e-mail B). Ils ne servent pas à comparer des canaux entiers au sein d'une même campagne multicanale. Pour tester des canaux, des créations ou des timings ensemble au fil d'un parcours, utilisez [Canvas]({{site.baseurl}}/user_guide/messaging/canvas/).

#### Conversions et rapports {#multichannel-conversions}

Pour les campagnes multicanales, Braze suit les [événements de conversion]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/conversion_events/) par canal. Lorsqu'un utilisateur convertit après avoir reçu des messages sur plusieurs canaux, Braze peut attribuer cette conversion à ces différents canaux. Le nombre de conversions peut dépasser le nombre d'*utilisateurs uniques*, et les taux peuvent dépasser 100 %. Pour les règles complètes, consultez [Règles de suivi des conversions]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/conversion_events/#conversion-tracking-rules).

Les limites de débit pour les envois couvrant plusieurs canaux sont décrites dans [Campagnes multicanales et Canvas]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/frequency_capping/#multichannel-campaigns-and-canvases). Pour les règles à l'échelle de l'espace de travail (y compris la façon dont les envois multicanaux sont comptabilisés dans les plafonds), consultez [Limite de fréquence]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/frequency_capping/).

### Créer une campagne multicanale {#create-a-multichannel-campaign}

1. Accédez à **Messaging** > **Campaigns**.
2. Sélectionnez **Create campaign**.
3. Sélectionnez **Multichannel**.
4. À l'[étape Rédiger](#step-1-compose-messages), sélectionnez **Add channel** et choisissez chaque canal dont vous avez besoin. Sélectionnez les icônes de canal pour basculer entre les éditeurs pendant que vous rédigez le contenu de chaque canal.
...
- [Analyse des campagnes]({{site.baseurl}}/user_guide/analytics/reports/campaign_analytics/)