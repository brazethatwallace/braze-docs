...
# Optimiser les tests A/B avec la variante gagnante ou les variantes personnalisées
...
Lors de la [création d'un test A/B]({{site.baseurl}}/user_guide/messaging/ab_testing/create_tests/) pour des campagnes par e-mail, notification push, webhook, SMS et WhatsApp planifiées pour un envoi unique, vous pouvez sélectionner une optimisation. Deux options d'optimisation sont disponibles : **Variante gagnante** et **Variante personnalisée**.

![Options d'optimisation listées dans la section test A/B lors du choix de votre audience cible. Trois options sont listées : Pas d'optimisation, Variante gagnante et Variante personnalisée. Variante personnalisée est sélectionnée.]({% image_buster /assets/img_archive/ab_personalized_variant.png %})

Les deux options fonctionnent en envoyant un test initial à un pourcentage de votre segment cible. Une fois le test terminé, les utilisateurs restants de votre audience reçoivent soit la variante la plus performante (variante gagnante), soit la variante avec laquelle ils sont le plus susceptibles d'interagir (variante personnalisée).

{% alert tip %}
Les optimisations se trouvent dans l'étape **Audiences cibles** de la création de campagne, sous **Test A/B**.
{% endalert %}

## Variante gagnante

L'envoi de la variante gagnante fonctionne comme un test A/B standard. Les utilisateurs de ce groupe recevront la variante gagnante une fois le test initial terminé.
