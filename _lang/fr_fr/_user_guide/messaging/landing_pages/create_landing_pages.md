...
Si ce n'est pas encore fait, sélectionnez **Save as draft**. Pour commencer à personnaliser votre page, sélectionnez **Edit landing page**. L'éditeur par glisser-déposer préchargera un modèle par défaut que vous pouvez personnaliser selon votre cas d'usage.
...
| Paragraphe | Un bloc de texte pour des descriptions plus longues ou du contexte supplémentaire. Prend en charge la mise en forme de texte enrichi. |
...
| Lien | Un lien hypertexte sur lequel les utilisateurs peuvent cliquer pour accéder à une URL spécifiée. Peut être intégré dans le texte ou être autonome. |
...
| Code personnalisé | Un bloc qui vous permet d'insérer et d'exécuter du HTML, du CSS ou du JavaScript personnalisé pour une personnalisation avancée. Pour interfacer avec le SDK Braze depuis ce bloc, consultez [Pont JavaScript pour les pages de destination]({{site.baseurl}}/user_guide/messaging/landing_pages/javascript_bridge) et [Créer des blocs de formulaire personnalisés]({{site.baseurl}}/user_guide/messaging/landing_pages/custom_form_blocks). |
...
- Famille, graisse et taille de police
- Hauteur de ligne
...
Vous pouvez utiliser ces blocs pour créer un formulaire qui lie les données soumises par les utilisateurs à leur profil dans Braze. Gardez à l'esprit que si vous utilisez des blocs de formulaire, vous devrez également créer une page de destination supplémentaire pour l'état de confirmation.
...
| Groupe de cases à cocher | Les utilisateurs peuvent sélectionner parmi plusieurs choix proposés. Les valeurs sont définies ou ajoutées à un attribut personnalisé de type tableau. |
| Gérer les abonnements | Une liste de groupes d'abonnement e-mail, SMS ou WhatsApp. Les utilisateurs sélectionnent les groupes auxquels ils souhaitent s'inscrire lorsqu'ils soumettent le formulaire. Chaque bloc est destiné à un canal. Pour plus d'informations, consultez [Bloc Gérer les abonnements]({{site.baseurl}}/user_guide/messaging/landing_pages/manage_subscriptions). |
...
Vous pouvez définir des styles à appliquer à l'ensemble des blocs de composants pertinents de votre page de destination depuis l'onglet **Page container**. Ces styles s'appliquent partout sur votre page, sauf là où vous les remplacez par un bloc spécifique.
...
Lorsque cette option est activée, vous pouvez également inverser l'empilement des colonnes pour contrôler l'ordre vertical du contenu multi-colonnes sur les écrans plus petits. Cela permet aux pages d'avoir une meilleure apparence et une meilleure ergonomie sur mobile sans code personnalisé.

![Le basculeur « Vertically stack on smaller screens » dans la section « Customize columns ».]({% image_buster /assets/img/landing_pages/device_responsive_toggle.png %}){: style="max-width:50%;"}
...
![Un champ de formulaire de type case à cocher avec le basculeur « Required input field » sélectionné.]({% image_buster /assets/img/landing_pages/lp-optional-required.png %}){: style="max-width:50%;"}
...
Si votre page de destination inclut un [formulaire](#form-blocks), vous pouvez éventuellement créer une seconde page de destination pour servir d'expérience de confirmation. Cette page devrait remercier les utilisateurs ou proposer une étape suivante après la soumission du formulaire.
...
{% alert note %}
Si votre page de confirmation s'ouvre dans un nouvel onglet, un utilisateur qui retourne sur la page de destination d'origine et soumet à nouveau le formulaire avec des informations mises à jour peut écraser la soumission précédente, ce qui entraîne des données incohérentes.
{% endalert %}
...
- Chaque page basée sur un formulaire est liée à une [page de confirmation](#step-4-create-a-confirmation-page-optional) à l'aide de l'action **Open web URL**, ou utilise un [formulaire multi-étapes]({{site.baseurl}}/user_guide/messaging/landing_pages/create_landing_pages/multi_step_forms) avec son étape de confirmation intégrée
...
Pour analyser l'efficacité de votre page de destination, accédez à **Messaging** > **Pages de destination**, puis sélectionnez une page de destination que vous avez publiée. Vous pouvez y suivre le nombre de vues de page, de clics sur la page, de soumissions de formulaire et les taux de soumission de votre page de destination.
