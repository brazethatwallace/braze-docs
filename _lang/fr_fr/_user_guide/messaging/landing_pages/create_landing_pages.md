...
Une page de destination est une page web publiée et accessible en ligne, dotée d'une URL partageable que vos clients peuvent consulter.
...
Allez dans **Messaging** > **Landing Pages**, puis sélectionnez **Create landing page**. Vous pouvez également sélectionner le nom d'une page de destination existante pour la dupliquer ou y apporter des modifications.

### Étape 2 : Saisir les détails de la page {#step-2-enter-the-page-details}

Ajoutez des détails internes et publics qui vous aident à organiser, personnaliser votre image de marque et partager votre page de destination.
...
Configurez les balises méta pour personnaliser l'apparence de votre page dans l'onglet du navigateur et optimiser les résultats des moteurs de recherche. Celles-ci sont visibles par vos clients.
...
Si ce n'est pas déjà fait, sélectionnez **Save as draft**. Pour commencer à personnaliser votre page, sélectionnez **Edit landing page**. L'éditeur par glisser-déposer précharge un modèle par défaut que vous pouvez personnaliser selon votre cas d'usage.
...
| Paragraphe | Un bloc de texte pour les descriptions plus longues ou le contexte supplémentaire. Prend en charge la mise en forme de texte riche. |
...
| Lien | Un hyperlien sur lequel les utilisateurs peuvent cliquer pour naviguer vers une URL spécifiée. Peut être intégré dans du texte ou autonome. |
...
![Zone de texte avec différentes sections de texte stylisées, comme des tailles et couleurs de police différentes, et une section surlignée qui affiche une barre d'outils avec l'option « Wrap with span for style ».]({% image_buster /assets/img/landing_pages/wrap_with_span.png %}){: style="max-width:50%;"}

Ajustez les propriétés du span pour mettre à jour le style de votre texte, qui comprend :

- Famille de police, graisse, taille
- Hauteur de ligne
- Espacement des lettres
- Alignement et couleur du texte
- Espacement interne du bloc

![Panneau de propriétés du span avec différentes options à mettre à jour.]({% image_buster /assets/img/landing_pages/span_properties.png %}){: style="max-width:35%;"}


{% endtab %}
{% tab Blocs de formulaire %}

Vous pouvez utiliser ces blocs pour créer un formulaire qui lie les données soumises par l'utilisateur à son profil dans Braze. Gardez à l'esprit que si vous utilisez des blocs de formulaire, vous devrez également créer une page de destination supplémentaire pour l'état de confirmation.

![Un bloc de formulaire qui enregistre un nouveau client et envoie un code de réduction à son e-mail.]({% image_buster /assets/img/landing_pages/form.png %}){: style="max-width:70%;"}

{% alert tip %}
Vous pouvez diviser un long formulaire en plusieurs étapes, chacune avec ses propres champs et une étape de confirmation intégrée, en utilisant un [formulaire multi-étapes]({{site.baseurl}}/user_guide/messaging/landing_pages/create_landing_pages/multi_step_forms) au lieu de placer les blocs de formulaire directement dans une ligne.
{% endalert %}
...
| Case à cocher | Si un utilisateur coche la case, l'attribut du bloc est défini sur `true`. S'il la laisse décochée, son attribut est défini sur `false`. |
...
| Gérer les abonnements | Une liste de contrôle des groupes d'abonnement e-mail, SMS ou WhatsApp. Les utilisateurs sélectionnent les groupes auxquels ils souhaitent adhérer lorsqu'ils soumettent le formulaire. Chaque bloc correspond à un canal. Pour plus d'informations, consultez [Bloc Gérer les abonnements]({{site.baseurl}}/user_guide/messaging/landing_pages/manage_subscriptions). |
...
#### Adaptatif aux appareils des utilisateurs {#responsive-to-user-devices}
...
Lorsque cette option est activée, vous pouvez également inverser l'empilement des colonnes pour contrôler l'ordre vertical du contenu multi-colonnes sur les écrans plus petits. Cela améliore l'apparence et le ressenti de vos pages sur mobile sans code personnalisé.
...
#### Masquer des lignes et des blocs par appareil {#hide-rows-and-blocks-by-device}

Vous pouvez masquer une ligne ou un bloc sur ordinateur de bureau ou sur tablette et appareils mobiles. Sélectionnez la ligne ou le bloc sur le canvas. Dans le panneau des propriétés, sélectionnez **Hide on**, puis choisissez **Desktop** ou **Tablet and smaller devices**.

Une ligne ou un bloc masqué n'apparaît pas pour ce type d'appareil dans l'aperçu de l'éditeur par glisser-déposer ni sur la page de destination en ligne.

#### Champs optionnels et obligatoires {#optional-and-required-fields}

Vous pouvez choisir si certains champs de formulaire sont obligatoires ou optionnels. Les champs obligatoires doivent être remplis avant que le formulaire puisse être soumis. Les champs optionnels peuvent être laissés vides ou non sélectionnés par l'utilisateur.
...
### Étape 4 : Créer une page de confirmation (optionnel) {#step-4-create-a-confirmation-page-optional}

Si votre page de destination ne contient pas de formulaire, passez à l'étape suivante.

{% alert note %}
Si votre formulaire utilise un [formulaire multi-étapes]({{site.baseurl}}/user_guide/messaging/landing_pages/create_landing_pages/multi_step_forms), ignorez cette étape. Les formulaires multi-étapes incluent une étape de confirmation intégrée et verrouillée, vous n'avez donc pas besoin d'une page de confirmation séparée.
{% endalert %}

Si votre page de destination inclut un [formulaire](#form-blocks), vous pouvez éventuellement créer une deuxième page de destination pour servir d'expérience de confirmation. Cette page devrait remercier les utilisateurs ou fournir une prochaine étape après la soumission du formulaire.

1. Sélectionnez le bouton **Submit** sur votre formulaire
2. Choisissez si vous souhaitez inclure une page de confirmation lorsque les utilisateurs soumettent votre formulaire
    - Utilisez le comportement au clic **Open web URL** pour envoyer les utilisateurs vers une page de confirmation
    - Utilisez le comportement au clic **None** pour que les utilisateurs restent sur la page de destination
...
Vous pouvez prévisualiser votre page de destination dans l'onglet **Preview** de l'éditeur. Après avoir enregistré votre page de destination en tant que brouillon, vous pouvez visiter l'URL en allant dans **Landing Pages** et en sélectionnant **Copy URL** à côté de votre page de destination.

![Une page de destination avec le menu ouvert montrant l'option « Copy URL ».]({% image_buster /assets/img/landing_pages/copy-url.png %})

#### Partager un lien de prévisualisation {#sharing-a-preview-link}

Dans l'éditeur, vous pouvez également sélectionner **Copy preview link** pour partager la page avec des relecteurs qui n'ont pas accès au tableau de bord.
...
- Si votre page de destination utilise Liquid et que vous disposez de l'abonnement Landing Pages Pro, le lien affiche à la place la page en direct à la demande et reflète vos modifications actuelles plutôt qu'un instantané du moment où vous avez généré le lien. Le contenu est personnalisé par utilisateur. La prévisualisation affiche le favicon de Braze et ne peut pas être modifiée.
...
- Chaque page basée sur un formulaire renvoie vers une [page de confirmation](#step-4-create-a-confirmation-page-optional) en utilisant l'action **Open web URL**, ou utilise un [formulaire multi-étapes]({{site.baseurl}}/user_guide/messaging/landing_pages/create_landing_pages/multi_step_forms) avec son étape de confirmation intégrée
...
Pour analyser l'efficacité de votre page de destination, allez dans **Messaging** > **Landing Pages**, puis sélectionnez une page de destination que vous avez publiée. Ici, vous pouvez suivre le nombre de vues de page, de clics sur la page, de soumissions de page et les taux de soumission pour votre page de destination.
