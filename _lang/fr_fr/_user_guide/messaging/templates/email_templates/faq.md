...
**Solution de contournement :** Pour obtenir le même résultat, vous pouvez héberger le contenu de votre e-mail sur une page de destination externe (comme votre site web), vers laquelle vous pourrez ensuite créer un lien depuis la Campaign d'e-mail que vous êtes en train de créer, en utilisant l'outil **Lien** lors de la modification du corps de l'e-mail.
...
Vous pouvez modifier le lien de désabonnement dans le pied de page personnalisé en remplaçant {% raw %} `{{${set_user_to_unsubscribed_url}}}` {% endraw %} par un lien vers votre propre site web avec un paramètre de requête incluant l'identifiant de l'utilisateur. Par exemple :
{% raw %}
...
### Que se passe-t-il si je modifie un modèle d'e-mail actuellement utilisé dans une Campaign ? {#what-happens-if-i-edit-an-email-template-that-is-currently-being-used-in-a-campaign}

Les modifications apportées à un modèle existant ne seront pas répercutées dans les Campaigns créées à partir de versions précédentes de ce modèle. Pour les Campaigns API qui utilisent un modèle dans le corps de la REST API, Braze utilisera la dernière version du modèle au moment de l'envoi.

## Modèles de liens {#link-templates}

### Puis-je télécharger plusieurs modèles de liens dans mon e-mail ? {#can-i-upload-multiple-link-templates-to-my-email}
...
Les modèles de liens sont développés et ajoutés à chaque URL avant toute expansion Liquid. Si une partie de votre URL est générée à l'aide d'un extrait de code Liquid, nous recommandons que la base de l'URL et le point d'interrogation (?) soient codés en dur pour que les modèles de liens soient correctement développés.

Évitez d'ajouter le point d'interrogation (?) à votre Liquid, car cela amènerait les modèles de liens à d'abord ajouter un point d'interrogation (?), puis le processus d'expansion Liquid ajouterait un second point d'interrogation (?).
