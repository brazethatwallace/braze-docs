...
Sans un `?` ou `&` final (ou un autre point d'insertion pris en charge), l'aliasage de lien ne reconnaît pas l'URL, **Link Management** ne la liste pas et les modèles de lien ne s'appliquent pas.
...
Lorsque des balises Liquid conditionnelles sont utilisées à l'intérieur d'un `href` (par exemple, pour définir une URL avec {% raw %}`{% if %}`, `{% elsif %}` ou `{% unless %}`{% endraw %}), l'aliasage de lien ne s'applique pas à ces liens. Cela signifie que ces liens n'apparaissent pas dans **Link Management** et ne reçoivent pas de `lid` pour le suivi des clics.
