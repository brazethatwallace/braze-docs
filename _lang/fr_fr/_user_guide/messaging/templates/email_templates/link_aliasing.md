...
Les identifiants de lien seront également mis à jour si une Campaign par e-mail, un Canvas avec un message e-mail ou un Content Block est dupliqué.
...
1. Sélectionnez **Edit Email Body** pour ouvrir le compositeur glisser-déposer.
2. Dans la barre latérale du compositeur, sélectionnez **Content** (à côté de **Sending Settings** et **Preview & Test**). Pour en savoir plus sur cette disposition, consultez [Créer un e-mail avec le glisser-déposer]({{site.baseurl}}/user_guide/channels/email/drag_and_drop).
3. Dans le sous-menu **Content**, sélectionnez **Link Management** (il apparaît sous **Design and Build**). Si le sous-menu est réduit, développez-le à l'aide de la flèche de contrôle dans la barre latérale.
...
{% alert note %}
L'aliasing de lien est uniquement pris en charge dans les attributs `href` au sein des balises d'ancrage HTML où il est possible d'ajouter un paramètre de requête en toute sécurité. Il est recommandé d'inclure un point d'interrogation (?) à la fin de votre lien afin que Braze puisse facilement ajouter la valeur `lid`. Sans l'ajout de la valeur `lid`, Braze ne reconnaîtra pas l'URL pour l'aliasing de lien.
{% endalert %}
...
Lorsque l'aliasing de lien est activé, les messages, les Content Blocks et les modèles de lien ne sont pas modifiés. Tous les messages existants utilisant des modèles de lien ou des Content Blocks resteront identiques. Cependant, lorsque vous mettez à jour un message, le balisage d'alias de lien s'appliquera à tous les liens, vous devrez donc réappliquer les modèles de lien pour que les liens soient visibles.
...
Dans la barre latérale du compositeur, sélectionnez **Content** > **Link Management** (sous **Design and Build**), puis sélectionnez les alias que vous souhaitez marquer comme **suivis**. Les alias suivis sont disponibles dans les filtres de segmentation qui référencent les alias de lien (voir [Filtres de segmentation](#segmentation-filters)). Vous pouvez également envoyer des messages basés sur les actions ou faire avancer les utilisateurs dans un Canvas lorsqu'ils cliquent sur un alias de lien dans un e-mail — voir [Filtres basés sur les actions](#action-based-filters). Le paramètre **suivi** ne modifie pas la comptabilisation des clics sur ce lien dans les rapports de performances des e-mails.
...
Lorsque l'aliasing de lien est activé pour votre espace de travail, vous pouvez créer des messages basés sur les actions ciblant n'importe quel lien (suivi ou non suivi) ou recibler les utilisateurs selon qu'ils ont cliqué sur un alias dans n'importe quelle Campaign par e-mail ou composant Canvas.
...
- Si un lien a le suivi activé et a été cliqué dans une Campaign, vous pouvez trouver la Campaign comme option disponible dans le filtre de Segment, même si le suivi des liens a depuis été désactivé, tant qu'au moins un lien de ce message est encore suivi.
- Vous ne pouvez sélectionner un lien suivi comme filtre que s'il se trouve dans un Canvas actif (lancé), en utilisant le menu déroulant du filtre **Clicked Alias in Canvas Step**. Si le lien est suivi dans un brouillon de Canvas, vous ne pouvez pas sélectionner le lien suivi comme filtre.

Pour définir des liens comme non suivis, accédez à **Settings** > **Email Preferences** > **Link Aliasing Settings**.
...
Si vous exportez vos données d'engagement avec Currents, un événement de clic sur un e-mail sera légèrement différent si l'aliasing de lien est activé. Il comportera deux champs supplémentaires pour l'[événement de clic sur un e-mail]({{site.baseurl}}/user_guide/data/distribution/braze_currents/event_glossary/message_engagement_events#email-click-events) lorsque l'aliasing de lien est activé : `link_id` et `link_alias`.
...
Pour modifier les liens dans les Content Blocks existants qui ont été créés avant que Braze n'active l'aliasing de lien, dupliquez les Content Blocks existants, puis modifiez les liens dans les Content Blocks dupliqués.
...
{% alert tip %}
Pour les Content Blocks, Braze recommande de créer des copies des Content Blocks existants à utiliser dans les nouveaux messages. Cela peut être fait par duplication en masse pour éviter les scénarios où vous pourriez référencer un Content Block qui n'a pas été activé pour l'aliasing de lien dans un nouveau message.
{% endalert %}

## Aliasing de lien pour les URL générées par Liquid {#link-aliasing-for-urls-generated-by-liquid}

Pour les URL générées par Liquid (par exemple, `assign` dans le HTML, valeurs extraites d'un Content Block ou Liquid dans un attribut personnalisé), Braze a besoin d'un emplacement clair pour insérer le paramètre de requête `lid`. Dans la plupart des cas, lorsque du Liquid reste dans l'URL, Braze ne déduit pas s'il faut commencer une nouvelle chaîne de requête avec `?` ou joindre une requête existante avec `&`, sauf si vous ajoutez ce délimiteur vous-même.

Procédez comme suit :

- Si l'URL ne contient **pas** déjà de chaîne de requête, ajoutez `?` après le Liquid (par exemple, `{{my_url}}?`).
- Si l'URL contient **déjà** `?` et des paramètres de requête, ajoutez `&` après le Liquid (par exemple, `{{my_url}}&`).

{% alert note %}
Lorsque vous utilisez des [modèles de lien]({{site.baseurl}}/user_guide/messaging/templates/email_templates/link_template) avec des URL générées par Liquid, Braze peut normaliser de manière conservatrice l'URL rendue après l'exécution du Liquid lorsqu'elle contient exactement deux caractères `?` utilisés comme séparateurs de requête. Le second `?` peut être réécrit en `&` afin que Braze modifie le moins possible l'URL. <br><br>Braze n'essaie pas de corriger tous les cas de `?` en double, et le traitement des URL plus complexes reste intentionnellement limité. Ajoutez le bon `?` ou `&` dans votre balisage en premier lieu, et considérez toute normalisation comme une protection limitée — pas comme un substitut à des URL bien formées ou à la reconnaissance des liens dans **Link Management** lorsqu'aucun délimiteur n'est présent.
{% endalert %}

Sans un `?` ou `&` final (ou un autre point d'insertion pris en charge), l'aliasing de lien ne reconnaît pas l'URL, **Link Management** ne la liste pas et les modèles de lien ne s'appliquent pas.

### Fragments d'URL (`#`) et paramètres de suivi {#url-fragments-and-tracking-parameters}

Le fragment (`#` et tout ce qui suit) n'est pas envoyé au serveur lors d'une requête de lien normale. Braze insère `lid` dans la chaîne de requête, qui doit apparaître avant le `#`. Si votre `href` contient du Liquid et un fragment `#` mais pas de `?` ou `&` avant le `#`, Braze ne peut pas ajouter `lid` en toute sécurité, et le lien peut ne pas apparaître dans **Link Management** ni être suivi comme alias de lien.

Cela est particulièrement courant dans l'éditeur glisser-déposer lorsqu'une URL de bouton mélange du Liquid avec un modèle basé sur un hash (par exemple, un chemin statique, puis `#`, puis des paires clé-valeur supplémentaires). Dans ce cas, ajoutez `?` immédiatement avant le `#` afin que la chaîne de requête (y compris `lid`) soit analysée avant le fragment.
...
Sans identification de l'emplacement où ajouter les paramètres de requête, l'aliasing de lien ne reconnaît pas ces URL et les modèles de lien ne s'appliquent pas. Si vous voyez des erreurs telles que **Failed to be assigned an LID** pour une URL dynamique, vérifiez que le `href` utilise le modèle `?` ou `&` présenté dans les exemples de cette section.

### Considérations pour l'éditeur glisser-déposer {#drag-and-drop-editor-considerations}

Dans l'éditeur glisser-déposer, les champs contenant un lien (comme l'**URL** d'un bouton) valident le `href` sous-jacent avant l'exécution du Liquid. Les espaces, sauts de ligne et autres caractères non compatibles avec les URL peuvent provoquer un comportement inattendu lorsque Braze ajoute des modèles de lien ou des paramètres d'aliasing de lien. Lorsque vous avez besoin de Liquid conditionnel pour la destination, définissez l'URL dans un bloc HTML (voir la section suivante) et référencez une seule variable dans le champ **URL** du glisser-déposer au lieu de placer du Liquid complexe directement dans ce champ.
...
Si vous utilisez également des exports d'engagement en streaming, les événements de clic sur un e-mail incluent un champ **`url`** ; consultez [Événement de clic sur un e-mail](#email-clicks-event) sur cette page pour savoir comment ce payload est lié à l'aliasing de lien.

### Exemple {#example}

Utilisez ce modèle lorsque l'URL assignée n'a pas de paramètres de requête :
...
Lorsque des balises Liquid conditionnelles sont utilisées à l'intérieur d'un `href` (par exemple, pour définir une URL avec {% raw %}`{% if %}`, `{% elsif %}` ou `{% unless %}`{% endraw %}), l'aliasing de lien ne s'applique pas à ces liens. Cela signifie que ces liens n'apparaissent pas dans **Link Management** et ne reçoivent pas de `lid` pour le suivi des clics.

**Recommandé :** construisez l'URL finale dans un bloc HTML avec `assign` (ou {% raw %}`{% capture %}`{% endraw %}), puis référencez cette variable partout où vous avez besoin du lien. Dans l'éditeur glisser-déposer, collez la variable dans le champ **URL** du bouton avec un `?` ou `&` final selon le cas — par exemple, `{{url}}?`.
...
Dans le champ **URL** du bouton (glisser-déposer) ou en HTML, pointez le `href` vers la variable avec un délimiteur :
...
Sans la valeur `lid`, Braze ne traite pas l'URL comme un lien aliasé pour le suivi et la segmentation. Nous recommandons de mettre à jour votre backend ou votre site afin qu'il ignore le paramètre de requête `lid` lorsqu'il est présent. Cela préserve l'aliasing de lien, les rapports et les cas d'usage de segmentation décrits dans cet article.

Vous pouvez également désactiver l'aliasing de lien dans le tableau de bord pendant que vous planifiez une modification du backend. Accédez à **Settings** > **Email Preferences** > **Link Aliasing Settings**.

Si vous ne pouvez pas modifier vos systèmes de destination, contactez l'[assistance Braze]({{site.baseurl}}/braze_support) pour désactiver l'aliasing de lien pour votre espace de travail. Notez les considérations suivantes si l'aliasing de lien est désactivé pour votre espace de travail :
...
- Les messages existants créés lorsque l'aliasing de lien était activé peuvent encore contenir du balisage d'alias de lien dans le HTML. Vous devrez peut-être supprimer manuellement les paramètres `lid` restants là où vous ne les souhaitez plus.
- Si vous modifiez une Campaign existante, une étape e-mail Canvas ou un Content Block, vous devrez peut-être ajouter à nouveau les modèles de lien pour que les liens modélisés s'affichent correctement.
- Les rapports de clics pour les envois effectués lorsque l'aliasing de lien était activé peuvent ne pas correspondre proprement aux rapports après la désactivation de la fonctionnalité.
