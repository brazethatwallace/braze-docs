...
- Ajoutez une audience spécifique. Actuellement, les parcours d'audience se déclenchent en fonction de « A effectué un achat quelconque », mais vous pouvez adapter cela aux produits spécifiques que vous souhaitez cibler.
- Ce modèle suppose que vous disposez d'un parcours post-achat distinct, de sorte qu'un achat entraînera la sortie des utilisateurs du Canvas.
- Renseignez les détails dans l'étape Audience Sync.
...
- Configurez l'étape parcours d'action dans le modèle avec des événements personnalisés pour « Fonctionnalité activée » et « Visite guidée effectuée ».
...
- Dans l'éditeur de Canvas, ajoutez des applications spécifiques pour les étapes parcours d'action.
