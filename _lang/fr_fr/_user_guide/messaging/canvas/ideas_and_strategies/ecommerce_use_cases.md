...
# Comment utiliser les événements recommandés eCommerce
...
{% alert important %}
Les [événements recommandés eCommerce]({{site.baseurl}}/user_guide/data/activation/events/recommended_events/ecommerce_events/) sont actuellement en accès anticipé. Contactez votre gestionnaire de la satisfaction client Braze si vous souhaitez participer à cet accès anticipé. <br><br>Si vous utilisez le nouveau connecteur Shopify, les événements recommandés eCommerce seront automatiquement disponibles via l'intégration.
{% endalert %}

## Utiliser un modèle de Canvas

Pour utiliser un modèle de Canvas :
1. Accédez à **Envoi de messages** > **Canvas**.
...
### Fonctionnalités clés de Liquid
...
## Propriétés de l'événement imbriqué
...
Vous pouvez suivre le volume des événements recommandés eCommerce dans le [Rapport d'événements personnalisés]({{site.baseurl}}/user_guide/data/activation/events/custom_events#analytics). Filtrez par **Effectuer un événement personnalisé**, puis spécifiez le [nom de l'événement recommandé eCommerce]({{site.baseurl}}/user_guide/data/activation/events/recommended_events/ecommerce_events#types-of-ecommerce-recommended-events) pour visualiser ses performances au fil du temps.
...
#### Tableau de bord du chiffre d'affaires eCommerce
