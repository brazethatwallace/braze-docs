...
3. Sélectionnez **Add property filters**, puis ajoutez un filtre où la comparaison de la propriété de base `event_id` est définie sur `equals`.
4. Activez le basculeur **Personalize value**, définissez le **Personalization type** sur `Context Variables` et définissez l'**Attribut** sur `event_id`.
...
## Parcours d'actions : bifurquer en fonction d'une action correspondante
...
2. Pour le premier groupe d'actions (achat d'origine), ajoutez un déclencheur pour l'événement personnalisé d'achèvement (tel que `Purchased_Ticket`). Sélectionnez **Add property filters**, puis ajoutez un filtre où la comparaison de la propriété de base `event_id` est définie sur `equals`. Activez **Personalize value**, définissez le **Personalization type** sur `Context Variables` et définissez l'**Attribut** sur `event_id`.
