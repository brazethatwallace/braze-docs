...
# Étape d'agent Optimiseur de contenu
...
## Créer une étape Optimiseur de contenu
...
Glissez-déposez le composant **Optimiseur de contenu** depuis la barre latérale, ou sélectionnez le bouton <i class="fas fa-plus-circle"></i> plus en bas d'une étape et sélectionnez **Optimiseur de contenu**.

### Étape 2 : Créer votre message de base

Le message de base est le point de départ de votre étape. Les variantes de chaque composant de contenu sont insérées dynamiquement en fonction des combinaisons définies dans l'onglet **Paramètres de l'Optimiseur de contenu**.
...
Depuis l'onglet **Canaux de communication**, sélectionnez **E-mail** et créez votre message e-mail de base. Consultez notre section dédiée [E-mail]({{site.baseurl}}/user_guide/channels/email) pour obtenir de l'aide.
...
Depuis l'onglet **Canaux de communication**, sélectionnez **Notifications push** et créez votre notification push de base. Consultez notre section dédiée [Push]({{site.baseurl}}/user_guide/channels/push) pour obtenir de l'aide.
...
![Options pour ajouter et configurer des composants de contenu dans l'interface de l'Optimiseur de contenu. L'interface affiche des composants sélectionnables tels que Objet, En-tête du corps, Contenu du corps et CTA principal, chacun avec des champs pour saisir différentes variantes.]({% image_buster /assets/img/content_optimizer/add_content_components.png %})

#### Étape 4.1 : Configurer les composants de contenu

Pour configurer les composants, accédez à l'onglet **Paramètres de l'Optimiseur de contenu**.
...
![Interface des paramètres de l'Optimiseur de contenu montrant les options pour ajouter et configurer des composants de contenu pour l'optimisation des e-mails. Chaque composant dispose de champs de saisie pour entrer différentes variantes. Le texte visible inclut les noms des composants et les champs pour saisir le texte des variantes.]({% image_buster /assets/img/content_optimizer/content_optimizer_settings.png %})
...
![Options pour ajouter et configurer des composants de contenu tels que Objet, En-tête du corps, Contenu du corps et CTA principal. Chaque composant dispose de champs pour saisir différentes variantes.]({% image_buster /assets/img/content_optimizer/optimization_liquid_in_use.png %})

Si vous n'ajoutez pas d'étiquette Liquid pour un composant de contenu sélectionné, vous verrez un avertissement dans l'onglet **Paramètres de l'Optimiseur de contenu** et une erreur dans l'onglet **Canaux de communication**. Le Canvas ne peut pas être lancé tant que tous les composants sélectionnés ne sont pas correctement ajoutés à votre message de base.
...
| --- | --- | --- |
| E-mail | Objet | {% raw %}`{% message_component "Subject" %}`{% endraw %} |
| E-mail | En-tête du corps | {% raw %}`{% message_component "Body Header" %}`{% endraw %} |
| E-mail | Contenu du corps | {% raw %}`{% message_component "Body Content" %}`{% endraw %} |
| E-mail | CTA principal | {% raw %}`{% message_component "Primary CTA" %}`{% endraw %} |
| Push | Titre | {% raw %}`{% message_component "Title" %}`{% endraw %} |
...
### Pourquoi l'analytique de l'étape diffère de l'analytique générale

Voici les raisons pour lesquelles l'analytique de l'étape Optimiseur de contenu diffère de la section **Analytique** :
...
| --- | --- | --- |
| Étiquettes Liquid manquantes | Si vous ajoutez un composant de contenu (comme Objet ou CTA) mais n'insérez pas l'étiquette Liquid correspondante dans votre message de base, vous verrez : <br>- Un avertissement dans l'onglet **Paramètres de l'Optimiseur de contenu** <br>- Une erreur dans l'onglet **Canaux de communication** | Copiez l'extrait de code Liquid affiché sous chaque composant dans l'onglet **Paramètres de l'Optimiseur de contenu** et collez-le dans la partie appropriée de votre message. |
