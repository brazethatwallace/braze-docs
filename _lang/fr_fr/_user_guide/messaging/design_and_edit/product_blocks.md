...
# Blocs de produits à glisser-déposer {#drag-and-drop-product-blocks}
...
| Catalogue Braze | Vous devez créer un catalogue Braze qui inclut les champs suivants, que vous utilisez dans la configuration de votre bloc de produits :{::nomarkdown}<code><ul><li>product_title</li><li>product_url</li><li>variant_image_url</li></ul></code>{:/} |
| Sélection de catalogue | Pour les blocs de produits statiques, vous devez créer une [sélection de catalogue]({{site.baseurl}}/user_guide/data/activation/catalogs/selections/) pour spécifier quels produits inclure dans votre bloc de produits. |
{: .reset-td-br-1 .reset-td-br-2 role="presentation" }

## Types de blocs de produits à glisser-déposer {#types-of-drag-and-drop-product-blocks}
...
## Configuration du contenu des blocs de produits {#product-block-content-configuration}

Chaque type de bloc a des configurations de contenu différentes.

### Champs produit {#product-fields}

Dans la section **Product Fields**, sélectionnez votre type de bloc de produits, puis activez les champs que vous souhaitez inclure pour chaque produit. Chaque champ est extrait de sources différentes en fonction du type de bloc de produits que vous sélectionnez.
...
![Champs produit pour un bloc de produits dynamique, divisés en données de catalogue et données d'événement]({% image_buster /assets/img/product_blocks/dynamic_fields.png %}){: style="max-width:50%;"}
...
![Champs produit pour un bloc de produits statique, tous catégorisés comme données de catalogue.]({% image_buster /assets/img/product_blocks/static_fields.png %}){: style="max-width:50%;"}

### Options de disposition {#layout-options}

Utilisez les options de disposition pour personnaliser l'affichage de vos produits au sein de votre bloc de produits.
...
#### Fonctionnement des paramètres de style globaux des e-mails avec les blocs de produits {#how-global-email-style-settings-work-with-product-blocks}

Les styles existants pour les paragraphes et les boutons s'appliquent automatiquement aux éléments de texte et de bouton au sein du bloc de produits. Votre bloc de produits utilise donc systématiquement la mise en forme que vous avez définie pour les paragraphes et les boutons, ce qui garantit un aspect cohérent dans l'ensemble de votre e-mail.

## Configuration des blocs de produits {#setting-up-product-blocks}

### Configuration du catalogue {#catalog-setup}

{% alert important %}
Si vous utilisez l'intégration Braze et Shopify pour la [synchronisation des produits]({{site.baseurl}}/shopify_catalogs/), aucune étape supplémentaire n'est nécessaire pour utiliser les blocs de produits à glisser-déposer.<br><br> Si vous ne disposez pas d'informations sur les variantes de produit, vous devez dupliquer les informations de niveau supérieur du produit dans les champs produit et variante de produit au sein des payloads d'événements et des catalogues. Concrètement, vous devez fournir les mêmes détails de produit pour les deux identifiants afin de maintenir la cohérence nécessaire au bon fonctionnement du bloc de produits.
{% endalert %}

Pour utiliser les blocs de produits à glisser-déposer, vous devez configurer un catalogue Braze qui inclut des valeurs de champs spécifiques. Ces champs sont utilisés dans la configuration de votre bloc de produits. Assurez-vous que votre catalogue inclut les champs suivants :
...
{% alert note %}
Si vous n'utilisez pas Shopify, vous pouvez contacter votre gestionnaire de compte pour activer le mappage de champs, ce qui vous permet de connecter n'importe quel catalogue aux blocs de produits et de mapper ses champs vers `product_title`, `product_url` et `variant_image_url`.
{% endalert %}

## Création de blocs de produits {#creating-product-blocks}

Ce guide vous accompagne à travers les étapes de création, de test et de vérification du fonctionnement d'un bloc de produits dynamique ou statique à l'aide de notre éditeur d'e-mails par glisser-déposer.
...
### Étape 2 : Ajouter un bloc de produits {#step-2-add-a-product-block}

{% tabs %}
{% tab Bloc de produits dynamique %}
...
Faites glisser un bloc de produits dans votre message e-mail.
...
{% endtab %}
{% tab Bloc de produits statique %}

Faites glisser un bloc de produits dans votre message e-mail et sélectionnez le type de bloc statique.
Sélectionnez le catalogue que vous souhaitez utiliser pour votre bloc de produits. Vous devez sélectionner une sélection de catalogue pour spécifier quels produits s'affichent dans votre bloc de produits.

{% endtab %}
{% endtabs %}

![L'onglet « Contenu » contenant des blocs éditeur, tels que les blocs de produits.]({% image_buster /assets/img/product_blocks/product_block.png %}){: style="max-width:40%;"}

### Étape 3 : Configurer les champs produit {#step-3-configure-product-fields}

Sélectionnez les [champs produit](#product-fields) à afficher dans le bloc de produits. Sélectionnez **Apply Settings** après chaque modification pour voir les mises à jour dans l'éditeur.
...
![Bloc de produits avec un signe dollar ajouté devant le prix de l'article.]({% image_buster /assets/img/product_blocks/liquid.png %}){: style="max-width:45%;"}

### Étape 4 : Configurer les paramètres de disposition {#step-4-configure-layout-settings}

Modifiez les [options de disposition](#layout-options) pour mettre à jour l'affichage des produits au sein de votre bloc de produits, et assurez-vous de sélectionner **Apply Settings** après chaque modification.
...
Une prévisualisation sera générée dans le compositeur par glisser-déposer lorsque vous appliquez des modifications à votre bloc de produits.

![Compositeur d'e-mails par glisser-déposer affichant un bloc de produits généré avec différentes tuiles d'articles.]({% image_buster /assets/img/product_blocks/static_block_preview.png %})
