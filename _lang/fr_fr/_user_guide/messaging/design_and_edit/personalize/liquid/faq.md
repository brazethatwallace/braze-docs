...
Consultez la section [Tags de personnalisation pris en charge]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/liquid/supported_personalization_tags) pour obtenir la liste complète des étiquettes Liquid prises en charge.
...
Cette erreur se produit lorsqu'un attribut personnalisé de type heure ou une propriété d'événement est comparé directement à une valeur vide (une chaîne de caractères vide). Liquid ne prend pas en charge les comparaisons directes entre différents types de données, comme un objet de type heure et une chaîne de caractères.
...
Cela échoue car vous ne pouvez pas comparer un attribut personnalisé de type heure à une chaîne de caractères (`blank`).

Pour résoudre ce problème, convertissez l'attribut de type heure en chaîne de caractères en l'assignant à une variable et en utilisant le filtre `default` lorsque l'attribut est évalué comme vide au moment du rendu :
