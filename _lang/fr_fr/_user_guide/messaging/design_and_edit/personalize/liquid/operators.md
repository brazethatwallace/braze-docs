...
{% alert note %}
Les opérateurs peuvent être utilisés dans les instructions conditionnelles (`if`, `elsif`, `unless`) mais pas dans les instructions `assign`, les boucles `for`, les instructions `case`/`when` ou les crochets d'accès aux tableaux. Pour une explication complète, consultez [Où utiliser les opérateurs et les filtres]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/liquid/using_liquid/#where-to-use-operators-and-filters).
{% endalert %}
