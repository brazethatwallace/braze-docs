...
## Filtres d'encodage {#encoding-filters}
...
{% endraw %}
{% alert tip %}
L'étiquette `assign` peut être combinée avec du HTML pour vous faire gagner du temps et des efforts lors de la création de plusieurs liens hypertextes.
{% raw %}
```
...
## Filtre d'accès aux propriétés {#property-accessor-filter}
...
{% alert note %}
Il n'est pas possible d'instancier un hash en tant que variable (comme une expression) dans Liquid au sein de Braze.
{% endalert %}

{% raw %}

## Filtres de formatage des nombres {#number-formatting-filters}
...
## Filtre d'échappement JSON ou d'échappement de chaîne de caractères {#json-escape-or-string-escape-filter}
...
## Filtres de formatage JSON {#json-formatting-filters}
...
{% details Exemple d'entrée et de sortie pour json_parse %}
...
{% details Exemple d'entrée et de sortie pour as_json_string %}
