...
{% alert note %}
Les extras d'e-mail envoient des métadonnées aux fournisseurs de services d'e-mail marketing or e-mailing et ne sont pas publiés dans Currents ni dans Snowflake. Pour ajouter des métadonnées ou des valeurs dynamiques aux événements d'envoi Currents ou Snowflake, utilisez l'étiquette Liquid `message_extras`.
{% endalert %}
