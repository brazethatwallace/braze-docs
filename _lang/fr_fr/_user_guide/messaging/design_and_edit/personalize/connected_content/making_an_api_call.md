...
Le contenu connecté n'a pas sa propre limite de débit. La limite de débit est plutôt basée sur le taux d'envoi de messages. Nous recommandons de définir la limite de débit d'envoi de messages plus élevée que votre limite de débit prévue pour le contenu connecté s'il y a plus d'appels de contenu connecté que de messages envoyés.
