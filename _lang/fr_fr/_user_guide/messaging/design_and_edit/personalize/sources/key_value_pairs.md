...
{% alert note %}
Le système d'exploitation iOS peut [limiter les notifications]({{site.baseurl}}/developer_guide/platform_integration_guides/swift/push_notifications/silent_push_notifications/#ios-silent-notifications-limitations) pour certaines fonctionnalités (suivi des désinstallations, géorepérages et contenu push). Si vous rencontrez des difficultés avec ces fonctionnalités, la limitation des notifications silencieuses d'iOS pourrait en être la cause.
{% endalert %}

## Messages in-app

Vous pouvez ajouter une paire clé-valeur à un message in-app dans l'[éditeur traditionnel]({{site.baseurl}}/user_guide/channels/in_app_messages/traditional/) en sélectionnant l'onglet **Paramètres**, en sélectionnant **Ajouter une nouvelle paire**, puis en spécifiant vos paires clé-valeur.
...
![Ajouter une paire clé-valeur à une carte de contenu]({% image_buster /assets/img_archive/kvp_content_cards.png %}){: style="max-width:70%;"}