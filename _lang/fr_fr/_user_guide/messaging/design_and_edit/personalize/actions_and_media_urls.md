...
##### Attribuer les ouvertures push et les clics de messages in-app avec des balises UTM {#attribute-push-opens-and-in-app-message-clicks-with-utm-tags}
...
![Capture d'écran relative à l'attribution des ouvertures push et des clics de messages in-app avec des balises UTM.]({% image_buster /assets/img_archive/push_utm_tags.png %})
...
![Capture d'écran relative à l'attribution des ouvertures push et des clics de messages in-app avec des balises UTM.]({% image_buster /assets/img_archive/iam_utm_tags.png %})
...
Vous pouvez construire dynamiquement vos URL directement dans le compositeur Braze, ce qui vous permet d'ajouter des paramètres UTM dynamiques à vos URL ou d'envoyer aux utilisateurs des liens uniques (comme les diriger vers leur panier abandonné ou vers un produit spécifique de nouveau en stock).
...
Nous raccourcissons les URL générées par Liquid, y compris celles incluses dans les propriétés de déclenchement par API. Par exemple, si {% raw %}`{{api_trigger_properties.${url_value}}}`{% endraw %} représente une URL valide, nous raccourcissons et suivons cette URL avant d'envoyer le message.

### Raccourcir les URL dans l'endpoint `/messages/send` {#shorten-urls-in-messagessend-endpoint}

Le raccourcissement de liens est également activé pour les messages envoyés uniquement par API via l'[endpoint `/messages/send`]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_messages). Pour une liste complète des paramètres de requête, consultez les [paramètres de requête]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_messages#request-parameters).
