...
Dans Braze, les informations relatives à votre audience sont stockées dans les profils utilisateurs. Un [profil utilisateur]({{site.baseurl}}/user_guide/audience/manage_audience/user_profiles) est un ensemble complet d'informations et d'attributs qui décrivent un consommateur individuel. Il sert de référentiel central pour le stockage et la gestion des données relatives à son comportement, ses préférences et ses données démographiques.

### Composantes d'un profil utilisateur {#parts-of-a-user-profile}

En comprenant les profils utilisateurs, vous pouvez obtenir des informations sur votre audience et interagir avec elle de manière personnalisée et ciblée. Le profil d'un utilisateur contient de nombreuses informations, dont voici les principales :

- **Identifiant de l'utilisateur :** Chaque profil utilisateur est identifié de manière unique par un ID utilisateur, appelé `external_id`. Cet identifiant permet à Braze de suivre et d'associer les données des utilisateurs sur différents canaux et appareils, offrant ainsi une vue unifiée des interactions de chaque utilisateur avec votre marque. Les [profils utilisateurs anonymes]({{site.baseurl}}/user_guide/data/unification/user_data/user_profile_lifecycle/anonymous_users) (les utilisateurs qui visitent votre site web ou votre application sans se connecter) n'ont pas d'`external_id`, mais peuvent se voir attribuer des [alias d'utilisateur]({{site.baseurl}}/user_guide/data/unification/user_data/user_profile_lifecycle/anonymous_users#assigning-user-aliases) comme identifiant alternatif.
- [Attributs](#attributes) **:** Il s'agit d'informations spécifiques sur l'utilisateur, telles que son nom, son âge, sa localisation ou toute autre information démographique. Vous pouvez utiliser ces attributs pour segmenter votre audience et personnaliser vos messages.
...
Les attributs standard sont des attributs prédéfinis que vous pouvez suivre avec Braze après avoir intégré le SDK dans votre application. Il s'agit d'éléments communs d'information sur les utilisateurs que la plupart des applications trouveraient utiles, comme les données démographiques et les données sur les appareils. En voici quelques exemples :
...
Les [attributs personnalisés]({{site.baseurl}}/user_guide/data/activation/attributes/custom_attributes) sont des attributs que vous définissez en fonction des besoins spécifiques de votre entreprise. Ils vous permettent de suivre des informations qui sont uniques à votre application ou à votre entreprise.
...
Une application de vente au détail, en revanche, pourrait suivre des attributs personnalisés comme :
...
Les [événements personnalisés]({{site.baseurl}}/user_guide/data/activation/events/custom_events) sont des événements que vous définissez en fonction des actions spécifiques que vous souhaitez suivre au sein de votre application ou de votre site. Par exemple, une application de streaming musical peut suivre des événements personnalisés tels que :
...
Consultez notre article dédié pour en savoir plus sur les [points de données]({{site.baseurl}}/user_guide/data/infrastructure/data_points).
