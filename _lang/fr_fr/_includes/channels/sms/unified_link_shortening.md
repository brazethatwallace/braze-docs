...
Pour utiliser le raccourcissement de liens, assurez-vous que la case correspondante est cochée dans le composeur de messages.

{% tabs %}
{% tab Composeur SMS %}

![Composeur de messages SMS avec la case cochée pour le raccourcissement de liens.]({% image_buster /assets/img/link_shortening/shortening1.png %})

{% endtab %}
{% tab Composeur RCS %}

![Composeur de messages RCS avec la case cochée pour le raccourcissement de liens.]({% image_buster /assets/img/link_shortening/shortening1_rcs.png %})

{% endtab %}
{% endtabs %}

Braze ne reconnaît que les URL commençant par `http://` ou `https://`. Lorsqu'une URL est reconnue, la section **Aperçu** se met à jour avec une URL de marque substitutive. Braze estime la longueur du message après le raccourcissement, mais un avertissement vous invite à sélectionner un utilisateur test et à enregistrer le message en tant que brouillon pour obtenir une estimation plus précise.

![Composeur de messages avec une longue URL dans la zone « Message » et un lien raccourci généré dans l'aperçu.]({% image_buster /assets/img/link_shortening/shortening3.png %})
...
Pour savoir comment construire dynamiquement des URL directement dans le compositeur Braze, vous permettant d'ajouter des paramètres UTM dynamiques à vos URL ou d'envoyer aux utilisateurs des liens uniques, consultez [Utiliser la personnalisation Liquid dans les URL]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/actions_and_media_urls#use-liquid-personalization-in-urls).
...
Assurez-vous d'enregistrer la Campaign ou le Canvas avant d'envoyer un message test afin de recevoir une représentation de l'URL raccourcie qui est envoyée dans votre message. Si la Campaign ou le Canvas n'est pas enregistré avant un envoi test, l'envoi test inclut une URL de marque substitutive.
...
![Onglet « Test » du message avec des champs pour sélectionner les destinataires de test.]({% image_buster /assets/img/link_shortening/shortening2.png %})

{% alert note %}
La personnalisation Liquid et les URL raccourcies sont générées dans l'onglet **Test** après la sélection d'un utilisateur. Assurez-vous qu'un utilisateur est sélectionné pour obtenir un nombre de caractères précis.
{% endalert %}

## Suivi des clics {#click-tracking}

Lorsque le raccourcissement de liens est activé, le tableau **SMS/MMS/RCS Performance** comprend une colonne intitulée **Total Clicks** qui affiche le nombre d'événements de clics par variante ainsi que le taux de clics associé. Pour plus de détails sur les indicateurs, consultez [Performance des messages]({{site.baseurl}}/user_guide/channels/sms_mms_and_rcs/reporting).

![Tableau des indicateurs de performance SMS et MMS.]({% image_buster /assets/img/link_shortening/shortening4.png %})

Les tableaux **Historical Performance** et **SMS/MMS/RCS Performance** incluent également une option pour **Total Clicks** et affichent une série chronologique quotidienne des événements de clics. Les clics sont comptabilisés lors de la redirection (par exemple lorsqu'un utilisateur visite un lien) et peuvent être comptabilisés plusieurs fois par utilisateur.

## Reciblage des utilisateurs {#retargeting-users}

Pour obtenir des conseils sur le reciblage, consultez [Reciblage]({{site.baseurl}}/user_guide/channels/sms_mms_and_rcs/message_features_and_optimization/user_retargeting#filter-by-advanced-tracking-links).

{% multi_lang_include analytics/click_tracking.md section='Custom Domains' %}

{% multi_lang_include analytics/click_tracking.md section='Frequently Asked Questions' %}

### Est-ce que je peux savoir quels utilisateurs individuels cliquent sur une URL ? {#do-i-know-which-individual-users-are-clicking-on-a-url}
...
Non. Cependant, vous pouvez généralement attribuer des `send_ids` aux événements de clic en utilisant le [Query Builder]({{site.baseurl}}/query_builder) pour interroger les données Currents avec cette requête :
