...
- **Personnalisation dynamique :** Les bannières peuvent être actualisées soit au cours d'une session active lorsque votre application appelle explicitement `requestBannersRefresh()`, soit automatiquement au début d'une nouvelle session lorsque `subscribeToBannersUpdates()` a été implémenté et que `requestBannersRefresh()` a déjà été appelé au moins une fois pour les ID de placement concernés.
