...
Pour plus d'informations, consultez [Limites de débit de l'API]({{site.baseurl}}/api/api_limits). Contactez votre gestionnaire du succès des clients pour demander une augmentation.
...
{% alert important %}
Si vous incluez `canvases_received` ou `campaigns_received` dans le paramètre `fields_to_export`, votre requête ne sera pas éligible à la limite de débit plus élevée. Nous vous recommandons de ne les inclure dans votre requête que si vous avez un cas d'usage spécifique.
{% endalert %}
