...
Les administrateurs contrôlent l'accès au serveur MCP via l'autorisation **Use MCP Server**. Par défaut, les utilisateurs ne disposent pas de cette autorisation et celle-ci doit être explicitement accordée.

Si un administrateur ne voit pas cette autorisation, contactez votre gestionnaire de compte Braze pour demander une inscription à l'accès anticipé.

### Révoquer l'accès {#revoke-access}

Pour révoquer l'accès, supprimez l'autorisation **Use MCP Server** de l'utilisateur. La suppression des autorisations du tableau de bord d'un utilisateur supprime également ces capacités de tout agent connecté lors de la prochaine requête.
...
Dans votre client MCP, ajoutez un nouveau serveur distant ou un connecteur personnalisé et saisissez l'URL de votre serveur Braze MCP. Par exemple, dans Claude, vous pouvez accéder à **Settings** > **Connectors** > **Add custom connector** et coller l'URL.
...
3. Sur l'écran de consentement, examinez les accès demandés par l'application.
...
Votre session utilise des jetons d'accès à durée de vie courte qui se renouvellent automatiquement. Il se peut que vous deviez vous reconnecter occasionnellement.
...
Connecter un client ne prend que quelques étapes. La procédure suivante utilise Claude, mais le flux est similaire pour les autres clients pris en charge.

1. Dans Claude, accédez à **Settings** > **Connectors** > **Add custom connector**.
2. Saisissez un nom, tel que `Braze`, puis collez votre URL Braze MCP : `https://mcp.braze.com/mcp` pour les États-Unis ou `https://mcp.braze.eu/mcp` pour l'UE. Vous n'avez pas besoin d'un identifiant client, d'un secret client ni d'une clé API.
...
Pour confirmer la connexion, envoyez une invite de test telle que `Show my recent Canvases from the Production workspace`.
...
Après avoir confirmé que vos workflows et compétences fonctionnent sur le serveur distant, désactivez le serveur hébergé localement.
...
Pendant l'accès anticipé, Braze maintient une liste de domaines clients autorisés pour des raisons de sécurité. Si vous vous connectez depuis un client qui ne figure pas sur la liste autorisée, l'authentification peut être bloquée. Si vous pensez que votre client devrait être pris en charge, contactez [mcp-product@braze.com](mailto:mcp-product@braze.com).
...
Si votre agent ne parvient pas à lister les outils Braze, attendez quelques minutes et réessayez. Ces problèmes sont souvent temporaires et se résolvent d'eux-mêmes.
