## Intégration du SDK Cordova {#integrating-the-cordova-sdk}
...
{% alert warning %}
Ajoutez le SDK Cordova de Braze uniquement en suivant les méthodes ci-dessous. N'essayez pas de l'installer par d'autres moyens, car cela pourrait entraîner une faille de sécurité.
{% endalert %}
...
Si vous ne prévoyez pas d'utiliser la collecte de localisation ni les géorepérages, utilisez la branche `master` depuis GitHub.
...
{% tabs %}
{% tab ios %}
Les préférences de type entier sont lues comme des représentations sous forme de chaîne de caractères, comme dans l'exemple suivant :
...
{% tab android %}
En raison de la manière dont le framework Cordova 8.0.0+ gère les préférences, les préférences exclusivement entières (telles que les identifiants d'expéditeur) doivent être définies sous forme de chaînes de caractères précédées de `str_`, comme dans l'exemple suivant :
...
{% tabs %}
{% tab ios %}
Les préférences booléennes sont lues par le SDK à l'aide des mots-clés `YES` et `NO` comme représentation sous forme de chaîne de caractères, comme dans l'exemple suivant :
...
{% tab android %}
Les préférences booléennes sont lues par le SDK à l'aide des mots-clés `true` et `false` comme représentation sous forme de chaîne de caractères, comme dans l'exemple suivant :
...
## Configuration des canaux de notification pour les notifications en tête d'écran (Android uniquement) {#configuring-notification-channels-for-heads-up-notifications-android-only}

Sur Android 8.0 (niveau d'API 26) et versions ultérieures, le comportement des notifications est contrôlé par les canaux de notification. Pour afficher des notifications en tête d'écran — des alertes qui apparaissent brièvement en haut de l'écran lorsque l'utilisateur utilise son appareil — vous devez créer un canal de notification avec `NotificationManager.IMPORTANCE_HIGH` dans le code de votre application Android.
...
### Exemple : création d'un canal de notification à haute importance {#example-creating-a-high-importance-notification-channel}
...
Le SDK Braze pour Cordova 9.0.0 et les versions ultérieures utilisent le SDK Swift 9.0.0 ou une version ultérieure. À partir du SDK Swift 8.0.0, ce SDK natif est compilé avec **Xcode 15.2**. Si votre build iOS échoue après la mise à jour du plugin Cordova vers la version 9.0.0 ou ultérieure, mettez à jour Xcode vers la version 15.2 ou une version plus récente, et vérifiez qu'il correspond au [journal des modifications du SDK Swift]({{site.baseurl}}/developer_guide/changelogs/?sdktab=swift) pour la version iOS native utilisée par votre plugin.