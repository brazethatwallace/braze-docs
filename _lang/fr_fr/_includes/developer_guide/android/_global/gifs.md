...
Braze offre la possibilité d'utiliser une bibliothèque d'images personnalisée pour afficher des GIF animés. Bien que l'exemple suivant utilise [Glide](https://bumptech.github.io/glide/), toute bibliothèque d'images prenant en charge les GIF est compatible.

## Intégration d'une bibliothèque d'images personnalisée {#integrating-a-custom-image-library}

### Étape 1 : Créer le délégué du chargeur d'images {#step-1-creating-the-image-loader-delegate}

Le délégué du chargeur d'images doit implémenter les méthodes suivantes :
...
L'exemple d'intégration suivant est tiré de l'[application d'exemple d'intégration Glide](https://github.com/braze-inc/braze-android-sdk/tree/master/samples/glide-image-integration) incluse avec le SDK Android de Braze.
...
### Correctif pour le chargement d'images à partir du SDK Android 36.0.0 {#fixing-image-loading-for-android-sdk-3600-and-later}

À partir du SDK Android 36.0.0, `displayInAppMessage()` est une fonction `suspend`. Cela signifie que `renderUrlIntoInAppMessageView()` s'exécute sur un thread en arrière-plan plutôt que sur le thread principal.
...
1. Chargez l'image sur le thread en arrière-plan avec `submit().get()`.
2. Publiez la mise à jour de l'interface utilisateur sur le thread principal avec `imageView.post { ... }`.
3. Si le résultat chargé est un drawable GIF, lancez l'animation après l'avoir défini sur la vue.

Cela sépare le chargement d'images du rendu de l'interface utilisateur et garantit la compatibilité de votre chargeur d'images personnalisé avec le SDK Android 36.0.0 et les versions ultérieures.

Ces recommandations s'appliquent aux chargeurs d'images personnalisés Android. Les messages in-app web prennent en charge les GIF nativement.

L'exemple Kotlin suivant utilise des valeurs de substitution pour illustrer ce schéma :
...
### Étape 2 : Définir le délégué du chargeur d'images {#step-2-setting-the-image-loader-delegate}

Le SDK Braze utilisera tout chargeur d'images personnalisé défini avec [`IBrazeImageLoader`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.images/-i-braze-image-loader/index.html). Nous recommandons de définir le chargeur d'images personnalisé dans une sous-classe d'application personnalisée :
...
### Résolution des problèmes de chargement d'images Glide {#troubleshooting-glide-image-loads}

Si les images ne se chargent plus après avoir défini un [`IBrazeImageLoader`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.images/-i-braze-image-loader/index.html) personnalisé (par exemple avec Glide), vérifiez si un intercepteur OkHttp global ajoute des en-têtes d'authentification à chaque requête.

L'exemple Glide sur cette page utilise le même chemin de chargement pour les Content Cards, les messages in-app et les notifications push. Les images hébergées par Braze sont des URL CDN et n'utilisent pas l'authentification de votre REST API. Limitez la portée des intercepteurs à vos propres hôtes d'API, ou excluez les hôtes d'images Braze. Une image de Content Card qui échoue après une intégration Glide est un symptôme courant de ce schéma d'intercepteur.

## Chargement personnalisé d'images avec Jetpack Compose {#custom-image-loading-with-jetpack-compose}

Pour remplacer le chargement d'images avec Jetpack Compose, vous pouvez passer une valeur à [`imageComposable`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.jetpackcompose.contentcards.styling/-content-card-styling/index.html#-808910455%2FProperties%2F-1725759721). Cette fonction prend une `Card` et effectue le rendu de l'image ainsi que des modificateurs nécessaires. Vous pouvez également utiliser `customCardComposer` de `ContentCardsList` pour effectuer le rendu de la carte entière.

Dans l'exemple suivant, la bibliothèque Compose de Glide est utilisée pour les cartes répertoriées dans la fonction `imageComposable` :
