## Enregistrement des données avec l'API de Braze (recommandé) {#logging-data-with-the-braze-api-recommended}

Vous pouvez enregistrer des analyses en temps réel à l'aide de l'[endpoint `/users/track`]({{site.baseurl}}/api/endpoints/user_data/post_user_track) de l'API Braze. Pour enregistrer les analyses, envoyez la valeur `braze_id` dans le champ des paires clé-valeur (comme illustré dans la capture d'écran suivante) afin d'identifier le profil utilisateur à mettre à jour.
...
## Enregistrement manuel des données {#logging-data-manually}

L'enregistrement manuel nécessite de configurer au préalable des espaces de travail dans Xcode, puis de créer, enregistrer et récupérer les analyses. Cela implique un travail de développement personnalisé de votre côté. Les extraits de code suivants vous aideront à résoudre ce problème.

Il est important de noter que les analyses ne sont pas envoyées à Braze tant que l'application mobile n'a pas été relancée. Cela signifie que, selon vos paramètres de rejet, il existe souvent une période indéterminée entre le moment où une notification push est rejetée, le lancement de l'application mobile et la récupération des analyses. Même si cette période tampon n'affecte pas tous les cas d'usage, vous devez prendre en compte cet impact et ajuster votre parcours utilisateur si nécessaire afin d'inclure l'ouverture de l'application pour répondre à ce problème.

![Un graphique décrivant la manière dont les analyses sont traitées par Braze. 1. Les données d'analyse sont créées. 2. Les données d'analyse sont enregistrées. 3. La notification push est rejetée. 4. Période indéterminée entre le moment où la notification push est rejetée et le lancement de l'application mobile. 5. L'application mobile est lancée. 6. Les données d'analyse sont reçues. 7. Les données d'analyse sont envoyées à Braze.]({% image_buster /assets/img/push_implementation_guide/push13.png %})

### Étape 1 : Configurer les groupes d'applications dans Xcode {#step-1-configure-app-groups-within-xcode}

Dans Xcode, ajoutez la capacité `App Groups`. Si vous n'avez pas d'espace de travail dans votre application, accédez à la capacité de la cible d'application principale, activez `App Groups`, puis cliquez sur le bouton **+** Ajouter. Ensuite, utilisez l'ID du bundle de votre application pour créer l'espace de travail. Par exemple, si l'ID du bundle de votre application est `com.company.appname`, vous pouvez nommer votre espace de travail `group.com.company.appname.xyz`. Assurez-vous que les `App Groups` sont activés à la fois pour la cible de votre application principale et la cible de l'extension de contenu.

![Écran Signing and Capabilities de Xcode avec App Groups activé pour la cible de l'application principale et celle de l'extension.]({% image_buster /assets/img/swift/push_story/add_app_groups.png %})

### Étape 2 : Intégrer les extraits de code {#step-2-integrate-code-snippets}

Les extraits de code suivants constituent une référence utile sur la façon d'enregistrer et d'envoyer des événements personnalisés, des attributs personnalisés et des attributs utilisateur. Ce guide s'exprime en termes de `UserDefaults`, mais le code sera représenté sous la forme du fichier d'aide `RemoteStorage`. Il existe des fichiers d'aide supplémentaires, `UserAttributes` et `EventName Dictionary`, qui sont utilisés pour envoyer et enregistrer les attributs utilisateur.
...
Pour enregistrer des événements personnalisés, vous devez créer les analyses à partir de zéro. Pour ce faire, créez un dictionnaire, renseignez-le avec des métadonnées et enregistrez les données via un fichier d'aide.
...
3. Si un tableau existe déjà, ajouter les nouvelles données au tableau existant et enregistrer
4. S'il n'y a pas de tableau existant, enregistrer le nouveau tableau dans `userDefaults`
...
Le meilleur moment pour enregistrer les analyses sauvegardées à partir d'une extension d'application de contenu de notification est juste après l'initialisation du SDK. Pour ce faire, parcourez en boucle tous les événements en attente, vérifiez la présence de la clé « Event Name », définissez les valeurs appropriées dans Braze, puis effacez le stockage pour la prochaine utilisation de cette fonction.

1. Parcourir le tableau des événements en attente
...
5. Enregistrer chaque événement personnalisé individuellement
...
#### Enregistrement des attributs personnalisés {#saving-custom-attributes}

Pour enregistrer des attributs personnalisés, vous devez créer les analyses à partir de zéro. Pour ce faire, créez un dictionnaire, renseignez-le avec des métadonnées et enregistrez les données via un fichier d'aide.
...
#### Envoi d'attributs personnalisés à Braze {#sending-custom-attributes-to-braze}

Le meilleur moment pour enregistrer les analyses sauvegardées à partir d'une extension d'application de contenu de notification est juste après l'initialisation du SDK. Pour ce faire, parcourez les attributs en attente, définissez l'attribut personnalisé approprié dans Braze, puis effacez le stockage pour la prochaine utilisation de cette fonction.

1. Parcourir le tableau des attributs en attente
...
#### Enregistrement des attributs utilisateur {#saving-user-attributes}

Lorsque vous enregistrez des attributs utilisateur, nous vous recommandons de créer un objet personnalisé pour déterminer le type d'attribut mis à jour (`email`, `first_name`, `phone_number`, etc.). L'objet doit être compatible avec le stockage et la récupération depuis `UserDefaults`. Consultez le fichier d'aide `UserAttribute` pour un exemple de mise en œuvre.
...
#### Envoi d'attributs utilisateur à Braze {#sending-user-attributes-to-braze}
...
3. Définir le champ utilisateur spécifique en fonction du type d'attribut utilisateur (e-mail)
