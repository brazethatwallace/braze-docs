...
Le SDK Roku n'inclut pas de méthode `wipeData`. Pour obtenir un état vierge fonctionnellement équivalent à `wipeData()` sur les autres SDK Braze, effacez les quatre sections du registre Braze, puis réinitialisez le SDK.
...
| `braze.section.config` | La configuration du SDK mise en cache et les données des indicateurs de fonctionnalité. |
{: .reset-td-br-1 .reset-td-br-2 aria-label="Wiping previously-stored data" }

### Étape 1 : Effacer les sections du registre {#step-1-clear-the-registry-sections}
...
Lorsque vous [initialisez à nouveau le SDK Braze]({{site.baseurl}}/developer_guide/sdk_integration/?sdktab=roku), le SDK gère les données de registre manquantes de manière transparente :

- La section de l'ID de l'appareil est vide, le SDK génère donc un nouvel UUID et traite l'appareil comme anonyme.
