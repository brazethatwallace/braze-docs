...
Le SDK Web de Braze vous permet d'intégrer la plateforme d'engagement client de Braze directement dans vos applications Web. Conçu avec TypeScript et pensé pour le développement Web moderne, ce SDK fournit des outils complets pour la gestion des utilisateurs, la communication, l'analytique et les feature flags.

### Ce que vous pouvez faire {#what-you-can-do}

- **Gestion des utilisateurs** : suivez et gérez les identités, les attributs et le comportement des utilisateurs au sein de votre application Web.
- **Messages in-app** : affichez des messages et des notifications ciblés aux utilisateurs pendant qu'ils utilisent activement votre site.
...
- **Bannières** : affichez des bannières dans des emplacements spécifiques de votre site.
...
- **Analytique** : suivez les événements personnalisés, les interactions des utilisateurs et les indicateurs de conversion.
- **Gestion des sessions** : surveillez les sessions des utilisateurs et les tendances d'engagement.

Que vous développiez une application monopage, un site e-commerce ou une plateforme de contenu, le SDK Web de Braze vous fournit les outils nécessaires pour créer des expériences utilisateur personnalisées et engageantes qui favorisent la croissance et la fidélisation.
...
2. **Endpoint du SDK** : accessible dans **Paramètres** > **Authentification SDK** > **Endpoints**
3. **Service de traitement** : requis pour les notifications push (consultez la section Notifications push)
...
L'extrait de code suivant montre la configuration minimale requise pour initialiser le SDK Web de Braze.
...
| `enableLogging` | `boolean` | `false` | Définissez sur true pour activer la journalisation par défaut. Notez que cela entraînera l'affichage des logs de Braze dans la console JavaScript, visible par tous les utilisateurs ! Vous devriez probablement supprimer cette option ou fournir un logger alternatif avec setLogger avant de mettre votre page en production. |
| `allowUserSuppliedJavascript` | `boolean` | `false` | Par défaut, le SDK Web de Braze n'autorise pas les actions de clic JavaScript fournies par l'utilisateur, ni l'activation des messages in-app HTML et des Banners, car ils permettent aux utilisateurs du tableau de bord de Braze d'exécuter du JavaScript sur votre site. Pour indiquer que vous faites confiance aux utilisateurs du tableau de bord de Braze pour écrire des actions de clic JavaScript non malveillantes, définissez cette propriété sur true. |
...
| `disablePushTokenMaintenance` | `boolean` | `false` | Par défaut, les utilisateurs ayant déjà accordé l'autorisation de notification push Web (par exemple via requestPushPermission ou depuis un fournisseur de notifications push précédent) synchronisent automatiquement leur jeton push avec le backend de Braze lors d'une nouvelle session afin de garantir la livrabilité. Pour désactiver ce comportement, définissez cette option sur true. |
...
| `minimumIntervalBetweenTriggerActionsInSeconds` | `number` | `30` | Par défaut, les actions de déclenchement (par exemple, l'affichage d'un message in-app) ne peuvent être déclenchées qu'une seule fois toutes les 30 secondes par utilisateur. Fournissez une valeur pour cette option afin de remplacer cette valeur par défaut. |
...
| `localization` | `string` | `undefined` | Si vous fournissez une valeur pour cette option, le SDK de Braze tentera d'afficher les messages in-app et les Content Cards dans cette langue. |
...
#### Désinscrire les notifications push {#unregister-push}
...
Lorsque vous appelez `dismissBanner(banner)`, le SDK gère l'état de fermeture de la bannière, la retire des mises à jour actives des bannières, notifie les abonnés à l'événement de fermeture de la bannière et synchronise la fermeture avec Braze. Les interfaces personnalisées doivent utiliser `subscribeToBannersUpdates` pour réagir à la suppression de la bannière fermée, plutôt que de traiter `dismissBanner` uniquement comme un changement d'interface local ou une simple méthode d'enregistrement analytique.
...
#### Vérifier le statut du SDK {#check-sdk-status}
...
#### Obtenir l'identifiant de l'appareil {#get-device-id}
...
Si vous utilisez un framework de rendu côté serveur (SSR) tel que Next.js, vous pouvez rencontrer des erreurs car le SDK est conçu pour fonctionner dans un environnement navigateur. Vous pouvez résoudre ces problèmes en important le SDK de manière dynamique.
...
Sinon, si vous utilisez webpack pour empaqueter votre application, vous pouvez tirer parti de ses commentaires magiques pour importer dynamiquement uniquement les parties du SDK dont vous avez besoin.
...
Lorsque vous utilisez Jest, vous pouvez rencontrer une erreur similaire à `SyntaxError: Unexpected token 'export'`. Pour corriger cela, ajustez votre configuration dans `package.json` afin d'ignorer le SDK de Braze :
...
1. **Inclure le script de notification push Web AMP** : ajoutez la balise de script asynchrone à votre en-tête.
2. **Ajouter des widgets d'abonnement** : ajoutez des widgets pour permettre aux utilisateurs de s'abonner/se désabonner.
3. **Ajouter des fichiers d'aide** : incluez `helper-iframe.html` et `permission-dialog.html`.
4. **Créer un service de traitement** : ajoutez le fichier de service de traitement de Braze.
...
- **Obligatoire** : vous devez inclure le service de traitement de Braze pour que les notifications push fonctionnent.
- **Enregistrement par défaut** : par défaut, le SDK Web de Braze enregistre et gère automatiquement votre service de traitement lorsque `requestPushPermission()` est appelé, ainsi qu'au début de chaque nouvelle session pour les utilisateurs ayant déjà accordé l'autorisation de notification push. Vous devez tout de même héberger un fichier de service de traitement à l'emplacement attendu contenant le code du service de traitement de Braze.
...
- **Autorisations de notification push** : appelez `braze.requestPushPermission()` en réponse aux interactions des utilisateurs (par exemple, les clics sur les boutons). Utilisez des invites de notification push souples (interface personnalisée) avant de demander l'autorisation du navigateur.
...
| ---- | ----------- | --- | -------
| Full | SDK complet avec interface utilisateur. Lors de l'utilisation de la version npm, les bundlers JavaScript suppriment le code inutilisé, y compris le code d'interface utilisateur. | `@braze/web-sdk` | https://js.appboycdn.com/web-sdk/6.11/braze.min.js
| Core | Contient le SDK sans interface utilisateur. Implémentez votre propre interface utilisateur pour les In-App Messages et les Content Cards lorsque vous utilisez cette version du SDK. Utilisez la bibliothèque complète pour la plupart des intégrations, car elle fournit des éléments d'interface utilisateur personnalisables via CSS. | N/A | https://js.appboycdn.com/web-sdk/6.11/braze.core.min.js
| No-AMD | Contient le SDK complet sans prise en charge d'AMD. Cela est utile si votre site utilise RequireJS ou un autre chargeur de modules AMD, mais que vous préférez charger le SDK via le CDN. | N/A | https://js.appboycdn.com/web-sdk/6.11/braze.no-amd.min.js
...
Passez l'option `enableLogging: true` à la fonction d'initialisation (`braze.initialize('YOUR-API-KEY-HERE', { baseUrl: 'YOUR-SDK-ENDPOINT', enableLogging: true });`) pour que Braze enregistre les journaux dans la console JavaScript. Cela est utile pour le développement, mais est visible par tous les utilisateurs. Supprimez donc cette option ou [fournissez un logger alternatif](https://js.appboycdn.com/web-sdk/6.11/doc/modules/braze.html#setlogger) avant de mettre votre page en production.
