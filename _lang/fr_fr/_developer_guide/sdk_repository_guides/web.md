...
- **Messages in-app** : affichez des messages ciblés et des notifications aux utilisateurs lorsqu'ils utilisent activement votre site.
...
- **Notifications push** : envoyez des notifications push Web pour interagir avec les utilisateurs même lorsqu'ils ne sont pas sur votre site.
- **Feature flags** : contrôlez le déploiement de fonctionnalités et les tests A/B grâce à la gestion des feature flags côté serveur.
- **Analyse** : suivez les événements personnalisés, les interactions des utilisateurs et les indicateurs de conversion.
- **Gestion des sessions** : surveillez les sessions des utilisateurs et les tendances d'engagement.

Que vous développiez une application monopage, un site e-commerce ou une plateforme de contenu, le SDK Web de Braze fournit les outils dont vous avez besoin pour créer des expériences utilisateur personnalisées et engageantes qui favorisent la croissance et la fidélisation.

## Conditions préalables {#prerequisites}

Avant d'intégrer le SDK Web de Braze, vous aurez besoin des éléments suivants :
...
3. **Service de traitement** : requis pour les notifications push (voir la section Notifications push)
...
L'extrait de code suivant présente la configuration minimale requise pour initialiser le SDK Web de Braze.
...
|--------|------|--------|-------------|
| `baseUrl` | `string` | **Obligatoire** | Cette option est requise pour configurer le SDK Web de Braze afin d'utiliser l'endpoint approprié pour votre intégration. Par exemple : `braze.initialize('YOUR-API-KEY-HERE', { baseUrl: 'sdk.iad-03.braze.com' })` |
| `enableLogging` | `boolean` | `false` | Définissez sur true pour activer la journalisation par défaut. Notez que cela amènera Braze à écrire dans la console JavaScript, ce qui est visible par tous les utilisateurs ! Vous devriez probablement retirer cette option ou fournir un logger alternatif avec setLogger avant de mettre votre page en production. |
| `allowUserSuppliedJavascript` | `boolean` | `false` | Par défaut, le SDK Web de Braze n'autorise pas les actions de clic JavaScript fournies par l'utilisateur, ni les messages In-App Messages en HTML et les Banners, car ils permettent aux utilisateurs du tableau de bord de Braze d'exécuter du JavaScript sur votre site. Pour indiquer que vous faites confiance aux utilisateurs du tableau de bord de Braze pour écrire des actions de clic JavaScript non malveillantes, définissez cette propriété sur true. |
...
| `inAppMessageZIndex` | `number` | `999999` | Par défaut, le SDK de Braze affiche les In-App Messages avec un z-index de 999999. Fournissez une valeur pour cette option afin de remplacer cette valeur par défaut. |
...
| `appVersion` | `string` | `undefined` | Si vous fournissez une valeur pour cette option, les événements utilisateur envoyés à Braze seront associés à la version indiquée, qui peut être utilisée pour la segmentation des utilisateurs. |
| `appVersionNumber` | `string` | `undefined` | Une valeur numérique de version de l'application qui peut être utilisée pour la segmentation des utilisateurs. Cette valeur doit être envoyée avec quatre champs, par exemple « 1.2.3.4 », sinon elle sera ignorée. Remarque : `appVersion` doit également être défini, soit avec la même valeur, soit avec un nom unique pour cette version. |
| `contentSecurityNonce` | `string` | `undefined` | Si vous fournissez une valeur pour cette option, le SDK de Braze ajoutera le nonce à tous les éléments `<script>` et `<style>` créés par le SDK. Cela peut être utilisé pour permettre au SDK de Braze de fonctionner avec la politique de sécurité du contenu de votre site Web. Notez qu'en plus de définir ce nonce, vous devrez peut-être également autoriser le chargement de FontAwesome, ce que vous pouvez faire en ajoutant `use.fontawesome.com` à la liste d'autorisation de votre politique de sécurité du contenu ou en utilisant l'option `doNotLoadFontAwesome` et en le chargeant manuellement. |
| `noCookies` | `boolean` | `false` | Par défaut, le SDK Web de Braze utilise des cookies. Pour désactiver l'utilisation des cookies, définissez cette option sur true. Notez que la désactivation des cookies peut affecter la capacité du SDK à mémoriser l'identité des utilisateurs entre les sessions. |
| `allowCrawlerActivity` | `boolean` | `false` | Par défaut, le SDK Web de Braze ignore l'activité des robots d'indexation ou des robots d'exploration Web connus, tels que Google, en se basant sur la chaîne user agent. Cela économise des points de donnée, rend les analyses plus précises et peut améliorer le classement de la page. Cependant, si vous souhaitez que Braze enregistre l'activité de ces robots, vous pouvez définir cette option sur true. |
| `disablePushTokenMaintenance` | `boolean` | `false` | Par défaut, les utilisateurs qui ont déjà accordé l'autorisation de notification push Web (par exemple via requestPushPermission ou depuis un fournisseur push précédent) synchronisent automatiquement leur jeton push avec le backend de Braze lors de chaque nouvelle session pour assurer la livrabilité. Pour désactiver ce comportement, définissez cette option sur true. |
| `enableSdkAuthentication` | `boolean` | `false` | Définissez sur true pour activer la fonctionnalité d'authentification du SDK. Pour plus d'informations sur l'authentification du SDK, consultez notre documentation produit. |
| `manageServiceWorkerExternally` | `boolean` | `false` | Par défaut, le SDK Web de Braze gère son propre service de traitement pour les notifications push. Si vous gérez déjà un service de traitement dans votre application et souhaitez y intégrer la fonctionnalité du service de traitement de Braze, définissez cette option sur true et incluez le code du service de traitement de Braze dans votre fichier de service de traitement. |
| `minimumIntervalBetweenTriggerActionsInSeconds` | `number` | `30` | Par défaut, les actions de déclenchement (par exemple, l'affichage d'un message in-app) ne peuvent être déclenchées qu'une fois toutes les 30 secondes par utilisateur. Fournissez une valeur pour cette option afin de remplacer cette valeur par défaut. |
| `serviceWorkerLocation` | `string` | `undefined` | Par défaut, le SDK Web de Braze recherche son fichier de service de traitement à la racine de votre domaine. Fournissez une valeur pour cette option afin de remplacer cette valeur par défaut et de spécifier un emplacement personnalisé pour le fichier du service de traitement. |
...
| `localization` | `string` | `undefined` | Si vous fournissez une valeur pour cette option, le SDK de Braze tentera d'afficher les messages in-app et les Content Cards dans la langue correspondante. |
...
| `requireExplicitInAppMessageDismissal` | `boolean` | `false` | Par défaut, les messages in-app peuvent être fermés en cliquant à l'extérieur ou en appuyant sur la touche Échap. Définissez cette option sur true pour obliger les utilisateurs à cliquer explicitement sur un bouton de fermeture ou un bouton d'action pour fermer le message. |
| `devicePropertyAllowlist` | `string[]` | `undefined` | Par défaut, le SDK de Braze détecte et collecte automatiquement toutes les propriétés d'appareil dans DeviceProperties. Pour remplacer ce comportement, fournissez un tableau de DeviceProperties. Pour désactiver l'envoi de toutes les propriétés aux serveurs Braze, fournissez un tableau vide. Notez que sans certaines propriétés, toutes les fonctionnalités ne fonctionneront pas correctement. Par exemple, sans le fuseau horaire, la distribution selon le fuseau horaire local ne fonctionnera pas. |
| `serviceWorkerScope` | `string` | `undefined` | Par défaut, le SDK Web de Braze enregistre son service de traitement avec la portée par défaut (le répertoire du service de traitement). Fournissez une valeur pour cette option afin de remplacer cette valeur par défaut et de spécifier une portée personnalisée pour le service de traitement. |
...
#### Alias d'utilisateur et groupes d'abonnement {#user-aliases-and-subscription-groups}
...
#### Demander l'autorisation push {#request-push-permission}
...
#### Enregistrer les impressions de feature flags {#log-feature-flag-impressions}
...
#### Obtenir et afficher les bannières {#get-and-display-banners}
...
Lorsque vous appelez `dismissBanner(banner)`, le SDK gère l'état de fermeture de la bannière, la retire des mises à jour actives, notifie les abonnés à l'événement de fermeture de la bannière et synchronise la fermeture avec Braze. Les interfaces personnalisées doivent utiliser `subscribeToBannersUpdates` pour réagir au retrait de la bannière fermée, plutôt que de traiter `dismissBanner` comme un simple changement d'interface local ou une méthode d'enregistrement analytique uniquement.
...
### Analyse et événements {#analytics-events}
...
#### Demander un envoi immédiat des données {#request-data-flush}
...
#### Obtenir l'ID de l'appareil {#get-device-id}
...
#### Authentification du SDK {#sdk-authentication}
...
Vous pouvez également, si vous utilisez webpack pour empaqueter votre application, tirer parti de ses commentaires magiques pour n'importer dynamiquement que les parties du SDK dont vous avez besoin.
...
Si vous utilisez Vite et que vous voyez un avertissement concernant des dépendances circulaires ou `Uncaught TypeError: Class extends value undefined is not a constructor or null`, vous devrez peut-être exclure le SDK de Braze de sa découverte de dépendances :
...
Lorsque vous utilisez Jest, vous pouvez voir une erreur semblable à `SyntaxError: Unexpected token 'export'`. Pour corriger cela, ajustez votre configuration dans `package.json` pour ignorer le SDK de Braze :
...
1. **Inclure le script AMP web push** : ajouter la balise script asynchrone à votre en-tête
2. **Ajouter des widgets d'abonnement** : ajouter des widgets pour permettre aux utilisateurs de s'abonner/se désabonner
3. **Ajouter des fichiers d'aide** : inclure `helper-iframe.html` et `permission-dialog.html`
4. **Créer un service de traitement** : ajouter le fichier de service de traitement de Braze
5. **Configurer l'élément AMP web push** : ajouter l'élément `amp-web-push` avec votre clé API et l'URL de base en tant que paramètres de requête
...
- **Accès global** : le SDK est disponible en tant que `window.braze` lorsqu'il est chargé via CDN

### Service de traitement (notifications push) {#service-worker-push-notifications}

- **Requis** : vous devez inclure le service de traitement de Braze pour que les notifications push fonctionnent
- **Enregistrement par défaut** : par défaut, le SDK Web de Braze enregistre et gère automatiquement votre service de traitement lorsque `requestPushPermission()` est appelé, ainsi qu'au début de chaque nouvelle session pour les utilisateurs qui ont déjà accordé l'autorisation push. Vous devez tout de même héberger un fichier de service de traitement à l'emplacement attendu contenant le code du service de traitement de Braze.
- **Gérer votre propre service de traitement** : si vous gérez déjà un service de traitement dans votre application, définissez l'option d'initialisation `manageServiceWorkerExternally` sur `true`, ajoutez le code du service de traitement de Braze dans votre fichier de service de traitement et enregistrez-le vous-même en utilisant `navigator.serviceWorker.register()`
- **Autorisations push** : appelez `braze.requestPushPermission()` en réponse aux interactions utilisateur (par exemple, les clics sur des boutons). Utilisez des invites push souples (interface personnalisée) avant de demander l'autorisation du navigateur
...
| ---- | ----------- | --- | -------
| Full | SDK complet avec interface utilisateur. Lorsque vous utilisez la version npm, les bundlers JavaScript suppriment le code inutilisé, y compris le code de l'interface utilisateur. | `@braze/web-sdk` | https://js.appboycdn.com/web-sdk/6.12/braze.min.js
| Core | Contient le SDK sans interface utilisateur. Implémentez votre propre interface utilisateur pour les In-App Messages et les Content Cards lorsque vous utilisez cette version du SDK. Utilisez la bibliothèque complète pour la plupart des intégrations, car elle fournit des éléments d'interface utilisateur personnalisables via CSS. | N/A | https://js.appboycdn.com/web-sdk/6.12/braze.core.min.js
| No-AMD | Contient le SDK complet sans prise en charge d'AMD. Cela est utile si votre site utilise RequireJS ou un autre chargeur de modules AMD, mais que vous préférez charger le SDK via le CDN. | N/A | https://js.appboycdn.com/web-sdk/6.12/braze.no-amd.min.js
...
Passez l'option `enableLogging: true` à la fonction d'initialisation (`braze.initialize('YOUR-API-KEY-HERE', { baseUrl: 'YOUR-SDK-ENDPOINT', enableLogging: true });`) pour que Braze enregistre les événements dans la console JavaScript. C'est utile pour le développement, mais visible par tous les utilisateurs. Supprimez donc cette option ou [fournissez un autre logger](https://js.appboycdn.com/web-sdk/6.12/doc/modules/braze.html#setlogger) avant de mettre votre page en production.
...
- [Guide du développeur Braze](https://www.braze.com/docs/developer_guide/sdk_integration/?sdktab=web)
...
Pour les détails du dépôt et les exemples de projets, consultez [https://github.com/braze-inc/braze-web-sdk](https://github.com/braze-inc/braze-web-sdk).