...
Commencez par [créer une page d'accueil Braze]({{site.baseurl}}/user_guide/messaging/landing_pages/create_landing_pages/). Ensuite, créez un message (par exemple un e-mail) qui redirige les utilisateurs vers la page d'accueil.
...
Lorsqu'un utilisateur clique sur ce lien, Braze l'identifie automatiquement, de sorte que toutes les préférences soumises sont enregistrées dans son profil existant, sans paramètres d'URL manuels. Pour un guide complet, consultez [Suivre les utilisateurs via un formulaire]({{site.baseurl}}/user_guide/messaging/landing_pages/tracking_users/).

## Étape 2 : Recueillir les préférences sur la page d'accueil {#step-2-capture-preferences-on-the-landing-page}
...
- Pour un exemple détaillé d'envoi de messages basés sur des attributs personnalisés de type date, consultez le cas d'usage e-mail dans le [guide d'envoi de messages via la REST API]({{site.baseurl}}/api/endpoints/messaging/).
