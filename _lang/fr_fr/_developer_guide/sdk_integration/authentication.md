...
article_title: "Configurer l'authentification pour le SDK"
...
Après avoir activé cette fonctionnalité dans votre application, vous pouvez configurer le tableau de bord de Braze pour rejeter toute requête avec un jeton JSON Web Token (JWT) invalide ou manquant, ce qui inclut :
...
- La mise à jour des attributs standard du profil utilisateur
- La réception ou le déclenchement de messages

Vous pouvez désormais empêcher les utilisateurs connectés non authentifiés d'utiliser la clé API du SDK de votre application pour effectuer des actions malveillantes, comme usurper l'identité d'autres utilisateurs.

## Configuration de l'authentification {#setting-up-authentication}

### Étape 1 : Configurer votre serveur {#server-side-integration}

#### Étape 1.1 : Générer une paire de clés publique/privée {#generate-keys}

Générez une paire de clés publique/privée RSA256. La clé publique sera ajoutée au tableau de bord de Braze, tandis que la clé privée devra être stockée de manière sécurisée sur votre serveur.

Nous recommandons une clé RSA de 2048 bits pour une utilisation avec l'algorithme JWT RS256.

{% alert warning %}
N'oubliez pas de garder vos clés privées _privées_. N'exposez et ne codez jamais en dur votre clé privée dans votre application ou votre site web. Toute personne connaissant votre clé privée peut usurper l'identité d'utilisateurs ou créer des utilisateurs au nom de votre application.
{% endalert %}

#### Étape 1.2 : Créer un JSON Web Token pour l'utilisateur actuel {#create-jwt}

Une fois que vous disposez de votre clé privée, votre application côté serveur doit l'utiliser pour renvoyer un JWT à votre application ou votre site web pour l'utilisateur actuellement connecté.

En règle générale, cette logique peut être placée là où votre application demanderait normalement le profil de l'utilisateur actuel, par exemple un endpoint de connexion ou tout endroit où votre application actualise le profil de l'utilisateur actuel.
...
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="Étape 1.2 : Créer un JSON Web Token pour l'utilisateur actuel" }

**Payload JWT**

| Champ | Obligatoire | Description                                                                            |
| ----- | ----------- | -------------------------------------------------------------------------------------- |
| `sub` | Oui  | Le « sujet » doit correspondre à l'ID utilisateur que vous fournissez au SDK Braze lors de l'appel à `changeUser`  |
| `exp` | Oui | L'« expiration » correspond au moment où vous souhaitez que ce jeton expire, sous forme d'horodatage Unix en secondes (par exemple, `1893456000` pour le 1er janvier 2030).                                |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="Étape 1.2 : Créer un JSON Web Token pour l'utilisateur actuel" }

{% alert tip %}
Pour en savoir plus sur les JSON Web Tokens, ou pour parcourir les nombreuses bibliothèques open source qui simplifient ce processus de signature, consultez [https://jwt.io](https://jwt.io).
{% endalert %}

### Étape 2 : Configurer le SDK {#sdk-integration}

Cette fonctionnalité est disponible à partir des [versions de SDK]({{ site.baseurl }}/user_guide/engagement_tools/campaigns/ideas_and_strategies/new_features/#filtering-by-most-recent-app-versions) suivantes :

{% sdk_min_versions swift:5.0.0 android:14.0.0 web:3.3.0 %}

{% alert note %}
Pour les intégrations iOS, cette page détaille les étapes pour le SDK Swift de Braze. Pour un exemple d'utilisation avec l'ancien SDK iOS AppboyKit, consultez [ce fichier](https://github.com/Appboy/appboy-ios-sdk/blob/master/Example/Stopwatch/Sources/AppDelegate.m) et [ce fichier](https://github.com/Appboy/appboy-ios-sdk/blob/master/Example/Stopwatch/Sources/Utils/SdkAuthDelegate.m).
{% endalert %}
...
{% alert note %}
Ne vous inquiétez pas, l'initialisation avec cette seule option n'aura aucun impact sur la collecte de données, tant que vous n'aurez pas commencé à [appliquer l'authentification](#braze-dashboard) dans le tableau de bord de Braze.
{% endalert %}

{% tabs %}
{% tab Web %}
Lors de l'appel à `initialize`, définissez la propriété optionnelle `enableSdkAuthentication` sur `true`.
```javascript
...
Vous pouvez également ajouter `<bool name="com_braze_sdk_authentication_enabled">true</bool>` à votre fichier braze.xml.
{% endtab %}
{% tab KOTLIN %}
Lors de la configuration de l'instance Braze, appelez `setIsSdkAuthenticationEnabled` avec la valeur `true`.
```kotlin
...
{% endtab %}
{% tab Dart %}
Actuellement, l'authentification SDK doit être activée dans le cadre de l'initialisation du SDK dans le code natif iOS et Android. Pour activer l'authentification SDK dans le SDK Flutter, suivez les intégrations pour iOS et Android depuis les autres onglets. Une fois l'authentification SDK activée, le reste de la fonctionnalité peut être intégré en Dart.
{% endtab %}
{% tab Flutter %}
L'authentification SDK doit être activée dans le cadre de l'initialisation du SDK dans le code natif iOS et Android. Une fois activée dans la couche native, vous pouvez utiliser les méthodes du SDK Flutter pour transmettre la signature JWT.
...
Après avoir activé l'authentification SDK dans la couche native, vous pouvez utiliser les méthodes C# Unity présentées dans les étapes suivantes.
{% endtab %}
{% tab Cordova %}
L'authentification SDK doit être activée lors de l'initialisation native du SDK. Ajoutez la configuration suivante à votre code natif iOS et Android :
...
Pour activer l'authentification SDK, définissez la propriété `enableSDKAuthentication` sur `true` dans votre fichier `config.xml` :
...
Chaque fois que votre application appelle la méthode `changeUser` de Braze, fournissez également le JWT qui a été [généré côté serveur](#braze-dashboard).

Vous pouvez également configurer le jeton pour qu'il soit actualisé en cours de session pour l'utilisateur actuel.

{% alert note %}
Gardez à l'esprit que `changeUser` ne doit être appelée que lorsque l'ID utilisateur a _réellement changé_. Vous ne devez pas utiliser cette méthode pour mettre à jour le jeton d'authentification (JWT) si l'ID utilisateur n'a pas changé.
{% endalert %}
...
{% alert note %}
`changeUser` retourne immédiatement sur le thread appelant. La signature d'authentification SDK fournie ici est attachée une fois le changement d'utilisateur terminé.
{% endalert %}
...
Lorsque vous utilisez le plugin Braze Expo, utilisez les mêmes méthodes du SDK React Native. Fournissez le JWT lors de l'appel à `changeUser` :
...
Lorsque cette fonctionnalité est définie comme [Obligatoire](#enforcement-options), les scénarios suivants entraîneront le rejet des requêtes SDK par Braze :
- Le JWT était expiré au moment de sa réception par l'API Braze
- Le JWT était vide ou manquant
- Le JWT n'a pas pu être vérifié avec les clés publiques que vous avez téléchargées dans le tableau de bord de Braze

Vous pouvez utiliser `subscribeToSdkAuthenticationFailures` pour vous abonner aux notifications lorsque les requêtes SDK échouent pour l'une de ces raisons. Une fonction de rappel contient un objet avec le [`errorCode`](#error-codes) correspondant, la `reason` de l'erreur, le `userId` de la requête (l'utilisateur ne peut pas être anonyme) et le jeton d'authentification (JWT) qui a causé l'erreur.

Les requêtes échouées seront périodiquement réessayées jusqu'à ce que votre application fournisse un nouveau JWT valide. Si cet utilisateur est toujours connecté, vous pouvez utiliser ce rappel comme une opportunité pour demander un nouveau JWT à votre serveur et fournir au SDK Braze ce nouveau jeton valide.

Lorsque vous recevez une erreur d'authentification, vérifiez que le `userId` contenu dans l'erreur correspond à votre utilisateur actuellement connecté, puis récupérez une nouvelle signature auprès de votre serveur et fournissez-la au SDK Braze. Vous pouvez également consigner ces erreurs dans votre service de surveillance ou de signalement d'erreurs.

{% alert tip %}
Ces méthodes de rappel sont un excellent endroit pour ajouter votre propre service de surveillance ou de journalisation des erreurs afin de suivre la fréquence de rejet de vos requêtes Braze.
{% endalert %}
...
Gardez à l'esprit que les requêtes SDK continueront à circuler normalement sans authentification, sauf si le paramètre d'authentification SDK de l'application est défini sur **Obligatoire** dans le tableau de bord de Braze.

En cas de problème avec votre intégration (par exemple, votre application transmet incorrectement des jetons au SDK, ou votre serveur génère des jetons invalides), désactivez cette fonctionnalité dans le tableau de bord de Braze, et les données reprendront leur flux normal sans vérification.

#### Options d'application {#enforcement-options}

Dans la page **Gérer les paramètres** du tableau de bord, chaque application dispose de trois états d'authentification SDK qui contrôlent la façon dont Braze vérifie les requêtes.

| Paramètre | Description |
| --------- | ----------- |
| **Désactivé** | Braze ne vérifiera pas le JWT fourni pour un utilisateur. (Paramètre par défaut) |
| **Optionnel** | Braze vérifiera les requêtes des utilisateurs connectés, mais ne rejettera pas les requêtes invalides. |
| **Obligatoire** | Braze vérifiera les requêtes des utilisateurs connectés et rejettera les JWT invalides. |
{: .reset-td-br-1 .reset-td-br-2 aria-label="Options d'application" }

![Paramètres d'authentification du SDK Braze affichant les options d'application Désactivé, Optionnel et Obligatoire.]({% image_buster /assets/img/sdk-auth-settings.png %})

Le paramètre **Optionnel** est un moyen utile de surveiller l'impact potentiel de cette fonctionnalité sur le trafic SDK de votre application.

Un JWT invalide sera signalé dans les états **Optionnel** et **Obligatoire**, cependant seul l'état **Obligatoire** rejettera les requêtes SDK, ce qui amènera les applications à réessayer et à demander un nouveau JWT.
...
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 .reset-td-br-4 aria-label="Codes d'erreur" }
