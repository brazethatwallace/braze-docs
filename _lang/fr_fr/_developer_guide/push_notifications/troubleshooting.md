...
| Le deep link depuis une notification push ne s'ouvre pas correctement | [Résolution des problèmes de deep linking]({{site.baseurl}}/developer_guide/push_notifications/deep_linking_troubleshooting) |
...
5. Si le problème persiste, contactez l'[Assistance Braze]({{site.baseurl}}/braze_support) en fournissant les journaux détaillés, la plateforme, la version du SDK et l'identifiant de la Campaign ou du Canvas.

## Résolution des problèmes spécifiques à la plateforme {#platform-specific-troubleshooting}
...
- Si un utilisateur a refusé l'accès push à un site, il ne sera pas invité à nouveau à donner son autorisation, sauf s'il supprime le statut de refus dans les préférences de son navigateur.
...
- Si les utilisateurs réinstallent votre application, le SDK génère un nouveau jeton et le transmet à Braze. Cependant, les APNs et Braze peuvent toujours enregistrer le jeton d'origine comme valide.
- Si les utilisateurs désinstallent votre application, Braze ne reçoit pas immédiatement de notification, et le jeton apparaît toujours comme valide jusqu'à ce que les APNs le retire.
...
Lorsqu'une Campaign push est lancée, Braze envoie des requêtes aux APNs pour distribuer votre message. Plus précisément, les requêtes sont transmises aux APNs pour chaque jeton push valide actuel, sauf si **Envoyer à l'appareil le plus récent de l'utilisateur** est sélectionné. Après que Braze reçoit une réponse positive des APNs, Braze enregistre une distribution réussie sur le profil utilisateur, bien que l'utilisateur puisse ne pas avoir reçu le message réel pour des raisons telles que :
...
Braze utilise le certificat SSL push téléchargé dans le tableau de bord pour s'authentifier et vérifier qu'il est autorisé à envoyer des notifications push aux jetons push fournis. Si un appareil est en ligne, la notification devrait être reçue peu après l'envoi de la Campaign. Notez que Braze définit la [date d'expiration](https://developer.apple.com/documentation/usernotifications/setting_up_a_remote_notification_server/sending_notification_requests_to_apns#2947607) APNs par défaut des notifications à 30 jours.
...
{% alert note %}
Il est normal que les APNs renvoient initialement un statut de succès même si un jeton devient non enregistré, car les APNs ne signalent pas immédiatement les événements d'invalidation de jeton. Les APNs retardent intentionnellement le renvoi d'un statut `410` pour les jetons invalides selon un calendrier aléatoire, conçu pour protéger la vie privée des utilisateurs et empêcher le suivi des désinstallations d'applications. Vous pouvez continuer à envoyer des notifications en toute sécurité à un jeton non enregistré jusqu'à ce que les APNs renvoient un statut `410`.
{% endalert %}
...
Les APNs renvoient `DeviceTokenNotForTopic` (statut HTTP 400) lorsque le jeton push ne correspond pas au topic (bundle ID) configuré pour vos identifiants. Braze peut afficher cela dans le **journal d'activité des messages** ou les journaux de distribution push sous la forme `DeviceTokenNotForTopic`.
...
### Aucun utilisateur « push registered » affiché dans le tableau de bord (avant l'envoi de messages) {#no-push-registered-users-showing-in-the-dashboard-prior-to-sending-messages}
...
- Vérifiez que votre application vous invite à autoriser les notifications push. En général, cette invite apparaît lors de la première ouverture de l'application, mais elle peut être programmée pour apparaître ailleurs. Si elle n'apparaît pas là où elle devrait, le problème est probablement lié à la configuration de base des capacités push de votre application.
  - Vérifiez que les étapes d'[intégration push]({{site.baseurl}}/developer_guide/push_notifications/?sdktab=swift) ont été effectuées avec succès.
...
Si le certificat push que vous avez téléchargé dans le tableau de bord n'est pas le même que celui du profil de provisionnement avec lequel votre application a été compilée, les APNs rejetteront le jeton. Vérifiez que vous avez téléchargé le bon certificat et effectuez une autre session dans l'application avant de tenter une autre notification de test.
...
Ce qui suit indiquerait un problème avec l'enregistrement push ou que le jeton de l'utilisateur a été renvoyé à Braze comme invalide par les APNs après avoir été poussé :
...
La majeure partie du code qui gère les deep links gère également les ouvertures push. Tout d'abord, assurez-vous que les ouvertures push sont enregistrées. Si ce n'est pas le cas, corrigez ce problème (car la correction résout souvent également la gestion des liens).
