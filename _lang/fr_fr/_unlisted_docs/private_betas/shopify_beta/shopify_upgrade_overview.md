...
{% alert important %}
Si vous utilisez une boutique en ligne Shopify et que vos développeurs ont implémenté les SDK Braze directement sur votre site Shopify, ou via Google Tag gestionnaire ou une plateforme de données client, vous devez prévoir d'arrêter de les utiliser lors de la mise à niveau vers le nouveau connecteur Shopify.
{% endalert %}
