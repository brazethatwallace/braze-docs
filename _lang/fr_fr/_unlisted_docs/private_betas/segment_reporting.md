...
- Comment l'engagement se compare-t-il entre les Segments pour une même Campaign ou un même Canvas ?

{% alert note %}
Les rapports par Segment sont disponibles uniquement pour les Segments avec le [suivi analytique]({{site.baseurl}}/user_guide/analytics/tracking/segment_analytics_tracking) activé. Pour sélectionner **Segments** dans le menu déroulant **Lignes**, vous devez disposer de la [permission « View Dashboard Reports » au niveau de l'espace de travail]({{site.baseurl}}/user_guide/administer/global/user_management/permissions).
{% endalert %}

## Créer un rapport sur les Segments {#report-on-segments}

Pour créer un rapport directement sur les Segments :

1. Accédez à **Analytics** > **Report Builder (New)**.
2. Cliquez sur **Create New Report**.
3. Dans le menu déroulant **Lignes**, sélectionnez **Segments**.
...
Pour ventiler les rapports de Campaigns, de Canvas ou par canal par Segment :

1. Dans le menu déroulant **Lignes**, sélectionnez **Campaigns**, **Canvases** ou **Campaigns and Canvases**.
...
Les combinaisons de **Lignes** et de **Ventilation** suivantes sont prises en charge pour les rapports par Segment :
...
{: .reset-td-br-1 .reset-td-br-2 aria-label="Combinaisons de lignes et de ventilation prises en charge"}

{% alert note %}
Le générateur de rapports prend en charge une seule ventilation à la fois. Si vous sélectionnez **Campaigns** dans le menu déroulant **Lignes**, vous pouvez ventiler par **Variante** ou par **Segment**, mais pas les deux dans le même rapport.
{% endalert %}

## Disponibilité des indicateurs {#metrics-availability}

Tous les indicateurs du générateur de rapports ne sont pas disponibles lorsque vous créez un rapport sur les Segments. Les indicateurs que vous pouvez sélectionner dépendent également du fait que **Segments** se trouve dans **Lignes** ou dans **Ventilation**, et que le rapport inclut ou non une dimension Campaign ou Canvas.

| Indicateur | Disponibilité |
| ----- | ----- |
| Indicateurs de canal et de messagerie générale | Disponibles pour les combinaisons de lignes et de ventilation prises en charge. |
| Nombre de conversions (Conversions A–D) et noms des événements de conversion | Disponibles lorsque les dimensions Segment et Campaign ou Canvas apparaissent ensemble. Utilisez **Segments** dans les lignes avec une ventilation **Campaigns and Canvases** ; ou utilisez **Campaigns**, **Canvases** ou **Campaigns and Canvases** dans les lignes avec une ventilation **Segment**. |
...
Pour en savoir plus sur l'impact de vos sélections de lignes et de ventilation sur les indicateurs, consultez [Disponibilité des indicateurs]({{site.baseurl}}/user_guide/analytics/reports/report_builder#metrics-availability) dans le générateur de rapports.