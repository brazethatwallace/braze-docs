...
Les ratios de crédits SMS et MMS sont calculés par incréments de segments de message envoyés depuis la plateforme Braze. Les ratios de crédits RCS sont calculés par incréments de types Basic et Rich Media, ou de types Single et Rich Media délivrés depuis la plateforme Braze. Les messages entrants et sortants sont tous deux facturés.

{% alert note %}
Le cas échéant pour ces canaux, les frais opérateur sont facturés séparément (à terme échu) et ne sont pas comptabilisés dans les Action Credits.
{% endalert %}
...
{% alert note %}
Consultez notre [documentation Banner]({{site.baseurl}}/developer_guide/banner_cards) pour en savoir plus sur les Banners de Braze.
{% endalert %}
...
Les ratios de crédits d'archivage des messages sont calculés par incréments de mille messages archivés sur les canaux notification push, e-mail et SMS/MMS.
...
{% alert note %}
Consultez notre [documentation webhooks]({{site.baseurl}}/user_guide/message_building_by_channel/webhooks) pour en savoir plus sur les webhooks de Braze.
{% endalert %}