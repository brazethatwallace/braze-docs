...
- Vous pouvez avoir jusqu'à 30 Campaigns actives.
...
- La rétention des données d'interaction des Campaigns et des Canvas est de 2 mois sans restauration.
...
{% alert note %}
Les données d'interaction pour les Campaigns et les Canvas sont différentes des données Snowflake et n'ont aucun effet sur celles-ci.
{% endalert %}
...
Si l'une des fonctionnalités décrites dans cet article est mise à jour, cela sera reflété dans cet article et mentionné dans nos [notes de version]({{site.baseurl}}/help/release_notes#most-recent-braze-release-notes).