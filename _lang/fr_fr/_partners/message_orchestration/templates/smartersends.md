...
| ID de Campaign API Braze | L'[ID de Campaign API Braze]({{site.baseurl}}/api/api_campaigns/) est l'identifiant unique de toutes les campagnes envoyées par l'intermédiaire de SmarterSends. Celui-ci peut être créé dans le tableau de bord de Braze sous **Messaging** > **Campaigns**. |
...
1. Dans Braze, accédez à **Settings** > **API Keys** et cliquez sur **Create New API Key**.
...
1. Dans votre espace de travail Braze, accédez à **Settings** > **App Settings**.
2. Créez une nouvelle application ou utilisez l'ID d'une application existante dans votre espace de travail. Notez que l'ID de l'application est indiqué comme étant la **API Key**.
...
Une Campaign API permet de suivre les indicateurs de tous les envois SmarterSends dans Braze et permet à SmarterSends de déclencher ces campagnes basées sur l'API.
