...
Talkable intègre l'acquisition portée par les ambassadeurs dans le parcours client que Braze alimente. L'intégration transfère en temps réel chaque abonnement lié à une recommandation capturé par Talkable vers le profil Braze correspondant, de sorte que les flux de bienvenue, les parcours de recommandation, la segmentation et les messages de cycle de vie puissent s'appuyer sur un consentement vérifié et un contexte de recommandation — le tout sans exports manuels de listes ni synchronisations par lots.
...
* **Étape d'e-mail gating pour l'ami :** un ami complète l'étape d'e-mail gating de Talkable et s'abonne aux e-mails marketing.
...
Talkable envoie des données à Braze uniquement lors d'un événement d'abonnement explicite de la part d'une personne ayant donné son consentement dans Talkable — qu'il s'agisse d'un ambassadeur s'inscrivant à une Campaign ou d'un ami s'abonnant lors de l'e-mail gating. Talkable n'exécute pas de traitements par lots nocturnes, de synchronisations complètes ni de mises à jour implicites de profils. Talkable n'envoie jamais à Braze de profils n'ayant pas donné leur consentement.
...
- Acheminer les abonnements de recommandation vers un groupe d'abonnement Braze dédié pour un envoi de newsletter conforme à la réglementation.
...
| --- | --- |
| Un compte Talkable | Un site Talkable avec au moins une campagne configurée est requis pour tirer parti de ce partenariat. |
...
2. Recherchez **Braze** et sélectionnez **Install**.
...
2. Dans Braze, accédez à **Audience** > **Recherche d'utilisateurs** et effectuez une recherche par l'adresse e-mail de test.
...
Lorsque le profil de test semble correct dans Braze, retournez dans Talkable et activez l'action **Email opt-in**.
...
| Inscription au groupe d'abonnement | Non applicable | Talkable inscrit l'utilisateur comme abonné lorsqu'un groupe d'abonnement est configuré. |
...
1. Dans Braze, copiez un ID de groupe d'abonnement depuis **Audience** > **Subscription Group Management**. Pour en savoir plus, consultez [Gérer les abonnements des utilisateurs]({{site.baseurl}}/user_guide/message_building_by_channel/email/managing_user_subscriptions).
2. Dans l'action **Email opt-in** de Talkable, collez-le dans le champ **Subscription group identifier**.

Talkable inscrit chaque abonnement dans ce groupe d'abonnement avec le statut « subscribed », limitant les abonnements de recommandation à ce groupe plutôt qu'à un abonnement global. Talkable ajoute uniquement des abonnements ; il ne les supprime jamais.
...
Les valeurs sont formatées avec Liquid. Les variables suivantes sont disponibles :
...
{% alert note %}
Les deux champs d'alias sont obligatoires. Si seul `alias_name` ou `alias_label` est défini, Talkable n'envoie pas d'alias d'utilisateur et le profil est identifié uniquement par e-mail.
{% endalert %}
...
Accédez à **Audience** > **Recherche d'utilisateurs** et effectuez une recherche par e-mail pour afficher un profil créé ou mis à jour par Talkable.

Les champs standard (e-mail, téléphone, prénom ou nom) ainsi que les attributs personnalisés que vous avez configurés apparaissent sur le profil ; **Email Subscribe** affiche **Opted In**.

### Créer un Segment de recommandation {#build-a-referral-segment}

1. Créez un Segment filtré sur **Email Subscribe** est **Opted In**.
2. Affinez avec les attributs personnalisés envoyés par Talkable — par exemple, `talkable_is_advocate` égal à `true` pour cibler les ambassadeurs, ou `talkable_campaign_name` égal au nom de votre Campaign pour cibler un programme de recommandation spécifique.
...
* **Format de téléphone :** Braze attend les numéros de téléphone au format international (E.164).
* **Synchronisation en temps réel, déclenchée par événement :** Talkable envoie une requête par événement d'abonnement (un utilisateur par requête). Il n'y a pas de traitement par lots ni de synchronisation complète périodique ; le volume suit celui de vos abonnements par recommandation.
* **Distribution fiable :** si Braze renvoie temporairement une erreur, Talkable effectue automatiquement de nouvelles tentatives. Les échecs persistants déclenchent l'envoi d'une alerte par e-mail à l'administrateur du site.

## Résolution des problèmes {#troubleshooting}

| Erreur | Cause probable | Correction |
| --- | --- | --- |
| 401 Unauthorized | La clé API REST ne dispose pas des permissions `users.track`, ou l'endpoint pointe vers le mauvais cluster. | Réémettez la clé avec les permissions `users.track` et confirmez que l'endpoint REST correspond à votre cluster Braze. |
...
| Le profil n'apparaît pas | L'exemple de requête n'a pas encore été envoyé, ou l'action est désactivée. | Sélectionnez **Send sample payload** dans Talkable et assurez-vous que l'action **Email opt-in** est activée. |
| Les requêtes ne s'envoient plus après une rotation de clé | La clé API stockée a été révoquée ou remplacée dans Braze. | Dans l'**App Store** de Talkable, ouvrez l'application Braze, collez la nouvelle clé API REST, puis sélectionnez **Save** ; testez à nouveau avec **Send sample payload**. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="Résolution des problèmes" }

Pour plus d'informations sur l'intégration Talkable, consultez la [documentation d'intégration Braze de Talkable](https://docs.talkable.com/email_marketing_and_automation/braze/). Pour contacter le support Talkable, envoyez un e-mail à [support@talkable.com](mailto:support@talkable.com).