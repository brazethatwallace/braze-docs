...
Tous les clients Braze ont immédiatement accès à Audience Sync vers Google et Facebook, mais les clients disposant de crédits d'action peuvent accéder à tous les partenaires Audience Sync. Pour débloquer des destinations Audience Sync supplémentaires pour les clients qui ne disposent pas de crédits d'action, achetez Audience Sync Pro. Contactez votre gestionnaire de compte Braze pour plus de détails.

## Cas d'usage {#use-cases}

- Cibler les utilisateurs à forte valeur via des canaux propriétaires et payants pour générer des achats ou un engagement incrémental.
...
- Recibler avec des publicités les utilisateurs qui répondent moins aux autres canaux marketing.
...
| Destination | Délai pour que la destination associe les membres de l'audience | Limitation du débit | Audience similaire ou actalike | Conseils |
| --- | --- | --- | --- | --- |
| [Criteo]({{site.baseurl}}/partners/canvas_audience_sync/criteo_audience_sync) | Jusqu'à 24 heures | 250 000 requêtes par minute. Envoi par lots toutes les 5 secondes avec nouvelle tentative automatique. | Oui | {::nomarkdown}<ul><li>Criteo prend en charge jusqu'à 1 000 audiences publicitaires.</li><li>La taille minimale de l'audience est de 500, et la taille recommandée est supérieure à 20 000.</li></ul>{:/} |
...
| [Google Ads ou YouTube]({{site.baseurl}}/partners/canvas_audience_sync/google_audience_sync) | Entre 6 et 12 heures | Envoi par lots toutes les 5 secondes avec nouvelle tentative automatique basée sur le retour de Google | Non | {::nomarkdown}<ul><li><b>Customer match :</b> Utilisez l'identifiant publicitaire mobile, ou l'adresse e-mail ou le numéro de téléphone.</li><li>Les audiences Google nécessitent au moins 5 000 utilisateurs pour commencer à diffuser des publicités.</li><li>La taille de l'audience affichera zéro tant qu'il n'y aura pas au moins 1 000 utilisateurs.</li></ul>{:/} |
| [LinkedIn]({{site.baseurl}}/partners/canvas_audience_sync/linkedin_audience_sync) | Jusqu'à 48 heures | Braze regroupe jusqu'à 2 000 utilisateurs par requête avec une nouvelle tentative automatique pouvant aller jusqu'à environ 13 heures. | Audiences prédictives IA | {::nomarkdown}<ul><li>La taille minimale de l'audience est de 300 membres, en tenant compte du ciblage par emplacement.</li><li>LinkedIn affiche le taux de correspondance dans le tableau de bord de Braze.</li></ul>{:/} |
...
<sup>Lorsque la limitation du débit est atteinte, Braze relance les synchronisations pendant 13 heures.</sup>
...
Pour chaque partenaire, vous devrez configurer les éléments suivants dans votre étape Audience Sync :
...
Pour utiliser un partenaire Audience Sync Pro, notamment Criteo, LinkedIn, Pinterest, Snapchat ou TikTok, vous pouvez sélectionner vos partenaires en fonction de vos allocations d'achat Audience Sync Pro dans la section **Audience Sync Pro** de la page **Partenaires technologiques**.

![Audience Sync Pro sans partenaires sélectionnés pour le moment.]({% image_buster /assets/img/audience_sync/audience_sync_pro1.png %}){: style="max-width:75%;"}
...
Après avoir sélectionné vos destinations Audience Sync Pro, connectez le compte publicitaire du partenaire sélectionné en cliquant sur la vignette du partenaire.
...
### Traitement par lots et latence {#batching-and-latency}

Lorsque les utilisateurs entrent dans une étape Audience Sync dans Canvas, Braze les place dans un système de traitement par lots qui agrège les mises à jour des utilisateurs avant de les envoyer à l'API du partenaire. Un lot est envoyé lorsque l'une des conditions suivantes est remplie :
...
Si l'erreur est liée à des problèmes avec le composant Audience Sync (comme « L'audience n'existe pas ») dans Canvas, un e-mail est envoyé à l'utilisateur qui a configuré le Canvas. Si cet utilisateur n'existe plus, l'e-mail est redirigé vers l'administrateur de l'entreprise.

Pour configurer les destinataires de ces e-mails, contactez votre gestionnaire de la satisfaction client afin d'ajouter des destinataires dans les **Préférences de notification**. Ce paramètre couvre à la fois les erreurs d'intégration et les erreurs du composant Audience Sync. Les destinataires que vous ajoutez reçoivent ces e-mails en plus de l'utilisateur associé à l'erreur.

## Considérations relatives à la confidentialité des données {#data-privacy-considerations}

{% alert important %}
Cette documentation n'est pas destinée à fournir, et ne saurait être considérée comme fournissant, des conseils juridiques. L'utilisation d'Audience Sync est soumise à des exigences juridiques spécifiques. Pour vous assurer que vous l'utilisez en conformité avec toutes les lois applicables, vous devriez consulter votre conseiller juridique.
{% endalert %}

Lors de la création d'audiences pour le suivi publicitaire, vous pouvez souhaiter inclure ou exclure certains utilisateurs en fonction de leurs préférences, et vous conformer aux lois sur la confidentialité, telles que le droit « Ne pas vendre ou partager » en vertu du [CCPA](https://oag.ca.gov/privacy/ccpa). Les marketeurs doivent mettre en œuvre les filtres pertinents pour l'éligibilité des utilisateurs dans leurs critères d'entrée Canvas. Les options suivantes peuvent vous aider.

Si vous avez collecté l'[IDFA iOS via le SDK Braze]({{site.baseurl}}/developer_guide/platforms/legacy_sdks/ios/initial_sdk_setup/other_sdk_customizations), vous pourrez utiliser le filtre « Ads Tracking Enabled ». Sélectionnez la valeur `true` pour n'envoyer que les utilisateurs ayant consenti au suivi vers les destinations Audience Sync.

![Un Canvas avec une audience d'entrée définie sur « Ad Tracking Enabled is true ».]({% image_buster /assets/img/audience_sync/audience_sync2.png %})
...
![Un Canvas avec une audience d'entrée définie sur « opted_in_marketing equals true ».]({% image_buster /assets/img/audience_sync/audience_sync.png %})
...
En tant qu'annonceur, il est de votre responsabilité de gérer le consentement au suivi publicitaire ou au ciblage de vos utilisateurs.

Pour diffuser des publicités auprès de vos utilisateurs, vous devez respecter l'ensemble des lois et réglementations applicables, ainsi que les politiques et exigences de la plateforme publicitaire. Utilisez Braze uniquement pour cibler et synchroniser des utilisateurs pour lesquels vous avez obtenu leur consentement.

Pour maintenir vos listes d'audience à jour sur ces plateformes publicitaires et supprimer les utilisateurs ayant retiré leur consentement, configurez un Canvas pour retirer ces utilisateurs des listes d'audience existantes à l'aide d'une étape Audience Sync.