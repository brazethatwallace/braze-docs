...
| --- | --- |
| Compte DinMo | Un [compte DinMo](https://www.dinmo.com/) avec la permission de créer des destinations est requis pour tirer parti de ce partenariat. |
...
* Envoyer des événements personnalisés et des événements d'achat depuis les données de votre entrepôt vers Braze pour le ciblage comportemental.
...
## Permissions de la clé API {#api-key-permissions}

Accordez les permissions suivantes à votre clé API REST de Braze en fonction des services de destination que vous utilisez :

| Permission | Requise pour |
| --- | --- |
| `users.track` | Synchroniser les attributs utilisateur, envoyer des événements de suivi et valider la connexion de destination |
| `users.export.ids` | Exporter les ID utilisateur pour les opérations en masse |
| `users.alias.update` | Mettre à jour les alias d'utilisateur |
| `subscription.status.set` | Synchroniser les statuts d'abonnement |
| `users.delete` | Mode de synchronisation miroir uniquement (facultatif pour les autres services de destination) |
{: .reset-td-br-1 .reset-td-br-2 aria-label="Permissions de la clé API" }
...
* La clé API est valide et dispose des permissions requises.
...
Chaque service de destination dans DinMo suit le même workflow général : créer une destination Braze, construire un modèle ou un segment DinMo, puis créer une activation pour envoyer des données à Braze. Pour des instructions d'activation étape par étape, consultez [Services de destination Braze de DinMo](https://docs.dinmo.io/integrations/destination-platforms/braze).

Les services de destination suivants sont disponibles :

| Service de destination | Description |
| --- | --- |
| [Synchroniser les attributs utilisateur](https://docs.dinmo.io/integrations/destination-platforms/braze/synchronize-users-attributes) | Met à jour les attributs du profil utilisateur dans Braze et insère éventuellement de nouveaux utilisateurs. |
| [Envoyer des événements de suivi](https://docs.dinmo.io/integrations/destination-platforms/braze/send-track-events) | Envoie des événements personnalisés et des événements d'achat à Braze. |
| [Synchroniser les statuts d'abonnement](https://docs.dinmo.io/integrations/destination-platforms/braze/synchronize-subscription-statuses) | Abonne ou désabonne des utilisateurs dans un groupe d'abonnement Braze en fonction de l'appartenance à un segment DinMo. |
| [Exporter des listes d'utilisateurs](https://docs.dinmo.io/integrations/destination-platforms/braze/export-user-lists) | Synchronise l'appartenance à un segment vers un attribut utilisateur Braze pour l'utiliser dans la segmentation Braze. |
...
* Si vous activez le mode insertion, les nouveaux utilisateurs du modèle sont créés dans Braze (comportement UPSERT).
* Les valeurs d'attributs modifiées depuis la dernière activation sont mises à jour dans Braze.
...
Utilisez ce service de destination pour envoyer des événements personnalisés ou des événements d'achat depuis un modèle d'événements ou un segment DinMo vers Braze. DinMo traite les événements personnalisés et les achats comme des services de destination distincts, car Braze utilise des API différentes pour chaque type.
...
1. Spécifiez le nom de l'événement exactement tel qu'il doit apparaître dans Braze. Si l'événement n'existe pas, DinMo le crée.
...
3. Mappez les propriétés d'événement facultatives aux noms d'attributs Braze.
...
* Si les utilisateurs existent déjà dans Braze, les utilisateurs qui entrent dans le segment DinMo sont marqués comme abonnés au groupe d'abonnement cible.
...
Utilisez ce service de destination pour représenter un segment DinMo sous la forme d'un attribut utilisateur Braze. En raison d'une limitation de Braze, DinMo ne crée pas directement de liste Braze. À la place, il définit un attribut utilisateur sur `true` pour les utilisateurs présents dans le segment et sur `false` pour les utilisateurs qui quittent le segment.
...
Une fois l'activation exécutée, créez un Segment Braze qui filtre les utilisateurs dont l'attribut synchronisé est égal à `true`.
