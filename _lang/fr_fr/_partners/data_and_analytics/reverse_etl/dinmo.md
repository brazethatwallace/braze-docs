...
Chaque service de destination dans DinMo suit le même flux de travail général : créer une destination Braze, construire un modèle ou un segment DinMo, puis créer une activation pour envoyer les données vers Braze. Pour des instructions d'activation étape par étape, consultez [Services de destination DinMo Braze](https://docs.dinmo.io/integrations/destination-platforms/braze).
...
| [Envoyer des événements de suivi](https://docs.dinmo.io/integrations/destination-platforms/braze/send-track-events) | Envoyer des événements personnalisés et des événements d'achat vers Braze. |
| [Synchroniser les statuts d'abonnement](https://docs.dinmo.io/integrations/destination-platforms/braze/synchronize-subscription-statuses) | Abonner ou désabonner des utilisateurs dans un groupe d'abonnement Braze en fonction de l'appartenance au segment DinMo. |
| [Exporter des listes d'utilisateurs](https://docs.dinmo.io/integrations/destination-platforms/braze/export-user-lists) | Synchroniser l'appartenance à un segment vers un attribut utilisateur Braze pour l'utiliser dans la segmentation Braze. |
...
{% alert warning %}
Le mode de synchronisation miroir supprime définitivement les enregistrements de Braze lorsqu'ils ne sont plus présents dans la source DinMo. Utilisez le mode miroir uniquement lorsque votre entrepôt est la source unique de vérité et que les suppressions sont intentionnelles. Validez les règles de suppression avant d'exécuter des synchronisations miroir en production.
{% endalert %}
...
2. Mappez les champs requis :
   * **Event time** : horodatage du moment où l'événement s'est produit
   * **External ID** : ID externe de l'utilisateur associé à l'événement
3. Mappez les propriétés d'événement facultatives aux noms d'attribut Braze.
4. Définissez la planification de la fréquence d'envoi des nouveaux événements vers Braze.
