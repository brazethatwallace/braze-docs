...
# API de personnalisation Hightouch {#hightouch-personalization-api}
...
L'API de personnalisation fait office de cache pour les données sélectionnées dans votre entrepôt. Les données de recommandation devraient donc déjà y être stockées. Vous pouvez utiliser Hightouch pour les transformer selon un modèle si nécessaire. Ce type de données inclut :
- Les métadonnées de l'utilisateur, telles que la région géographique, l'âge ou d'autres informations démographiques
- Les actions ou événements des utilisateurs, y compris les achats précédents, les pages vues, les clics, etc.
...
### Étape 1 : Connecter la source de données à Hightouch {#step-1-connect-data-source-to-hightouch}
...
1. Dans Hightouch, accédez à **Sources Overview > Add Source**. Sélectionnez votre entrepôt de données comme source.<br><br>
...
### Étape 2 : Modéliser les données {#step-2-model-data}
...
1. Dans Hightouch, accédez à [**Models overview**](https://app.hightouch.com/models) > **Add model**, puis sélectionnez la source que vous venez de connecter. <br><br>
2. Choisissez ensuite une [méthode de modélisation](https://hightouch.com/docs/models/creating-models). Comme toutes vos informations doivent être regroupées dans un seul tableau, vous pouvez utiliser le sélecteur visuel de tableau pour le définir. Vous pouvez également écrire du code SQL pour n'inclure que les colonnes souhaitées ou vous appuyer sur vos modèles dbt, Looker Looks ou Sigma existants.<br><br>
3. Avant de continuer, prévisualisez votre modèle pour vous assurer qu'il interroge les données qui vous intéressent. Par défaut, Braze limite l'aperçu aux 100 premiers enregistrements. Après avoir validé vos données, cliquez sur **Continue**.<br><br>
4. Donnez un nom à votre modèle, par exemple « User recommendations ».<br><br>
5. Enfin, sélectionnez une clé primaire et cliquez sur **Finish**. Une clé primaire doit être une colonne contenant des identifiants uniques. C'est également le champ que vous utiliserez pour appeler l'API de personnalisation afin de récupérer les recommandations d'un utilisateur en particulier.

### Étape 3 : Configurer l'API de personnalisation {#step-3-configure-personalization-api}
...
2. Sélectionnez ensuite la région appropriée. La sélection de la région la plus proche de votre infrastructure réduira vos temps de réponse. Si aucune région n'est proche de votre infrastructure, contactez l'[assistance Hightouch](mailto:friends@hightouch.com).<br><br>
3. Accédez à la [page d'aperçu **Syncs**](https://app.hightouch.com/syncs) et cliquez sur le bouton **Add sync**. Sélectionnez ensuite le modèle approprié et la destination que vous avez précédemment configurée.<br><br>
...
### Étape 4 : Appeler l'API de personnalisation via le contenu connecté de Braze {#step-4-call-personalization-api-through-braze-connected-content}

Une fois que vous avez configuré votre instance d'API de personnalisation, vous pouvez l'utiliser comme endpoint de contenu connecté Braze.
...
Vous pouvez utiliser les modèles Liquid pour référencer les propriétés renvoyées dans le payload JSON et les utiliser dans vos messages.

Pour l'exemple de payload ci-dessous :
...
Les références Liquid suivantes renverraient ces exemples de données :
