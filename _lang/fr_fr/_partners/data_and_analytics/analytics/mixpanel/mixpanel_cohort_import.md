...
Une fois générée, vous pouvez créer une nouvelle clé ou invalider une clé existante. La clé d'importation des données et l'endpoint REST sont utilisés à l'étape suivante lors de la configuration d'un postback dans le tableau de bord de Mixpanel.<br><br>![Page du partenaire technologique Braze Mixpanel affichant la clé d'importation des données et l'endpoint.]({% image_buster /assets/img_archive/currents-mixpanel-edit.png %})
...
![Fenêtre modale de configuration de l'intégration Braze dans Mixpanel avec les champs de clé et d'endpoint.]({% image_buster /assets/img_archive/mixpanel2.png %}){: style="max-width:50%;"}
...
![Flux d'exportation de cohorte Mixpanel affichant les options de synchronisation Export to Braze.]({% image_buster /assets/img_archive/mixpanel3.png %}){: style="max-width:50%;"}
...
Dans Braze, pour créer un segment avec ces utilisateurs, accédez à **Audience** > **Segments**, nommez votre segment et sélectionnez **Mixpanel_Cohorts** comme filtre. Ensuite, utilisez l'option « includes » et choisissez la cohorte que vous avez créée dans Mixpanel.
