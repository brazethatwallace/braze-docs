...
Dans de rares cas, les valeurs des attributs par défaut et personnalisés dans les vues Snowflake de cette page peuvent ne pas correspondre à ce que vous voyez sur le profil d'un utilisateur dans le tableau de bord de Braze.
...
Si vous constatez des écarts généralisés, contactez votre gestionnaire du succès des clients ou le support Braze.
...
## Instantanés du profil utilisateur {#user-profile-snapshots}

Ces vues fournissent des instantanés périodiques des attributs du profil utilisateur. Les données sont retardées de 12 heures maximum, ce qui les rend utiles pour les requêtes qui ne nécessitent pas de mises à jour en temps réel.
...
* Performant pour les requêtes qui ne nécessitent pas une précision en temps réel.
* Exécution de requêtes plus rapide, en particulier lors du filtrage sur des attributs autres que `USER_ID`.
* **Limitation :** les données ne sont pas à jour en temps réel.
...
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="Schéma USERDEFAULTATTRIBUTESVIEWSHARED" }
...
| `ARCHIVED` | BOOLEAN | Indique si le profil utilisateur est archivé |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="Schéma USERCUSTOMATTRIBUTESVIEWSHARED" }

#### Travailler avec CUSTOM_ATTRIBUTES {#working-with-custom_attributes}
...
Ces vues fournissent des mises à jour en quasi temps réel des attributs de profil utilisateur, avec un délai pouvant aller jusqu'à 10 minutes après une mise à jour dans Braze.
...
* Fournit des attributs utilisateur à jour avec un délai minimal (~10 minutes).
* Utile pour l'analyse en temps réel et les scénarios nécessitant des données récentes.
* **Considérations de performance :**
    * Les requêtes sur des utilisateurs individuels sont plus rapides (moins d'une minute avec un entrepôt de grande taille).
    * Les requêtes sans filtre USER_ID nécessitent une agrégation sur l'ensemble des utilisateurs, ce qui entraîne des temps d'exécution nettement plus longs.
    * Les requêtes sur un jeu de données volumineux (par exemple plus de 100 millions d'utilisateurs) peuvent prendre plusieurs minutes.
...
| `LANGUAGE` | VARCHAR | Préférence linguistique de l'utilisateur |
| `TIME_ZONE` | VARCHAR | Fuseau horaire de l'utilisateur |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="Schéma USERLATESTSTATEDEFAULTATTRIBUTESVIEWSHARED" }
...
## Journaux de modifications historiques {#historical-change-logs}
...
* Les données sont capturées toutes les 12 heures, ce qui signifie que plusieurs mises à jour au cours de cette fenêtre sont combinées en un seul enregistrement. Les modifications individuelles au cours de cette période ne sont pas conservées séparément.
* `EFF_DT` et `END_DT` marquent le début et la fin de l'état d'un attribut utilisateur.

{% include partners/snowflake_user_attributes_date_fields_note.md %}

### Schéma de `USER_DEFAULT_ATTRIBUTES_HISTORY_VIEW_SHARED` {#user_default_attributes_history_view_shared-schema}
...
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="Schéma de USERDEFAULTATTRIBUTESHISTORYVIEWSHARED" }

### Schéma de `USER_CUSTOM_ATTRIBUTES_HISTORY_VIEW_SHARED` {#user_custom_attributes_history_view_shared-schema}
...
### Combiner les attributs par défaut et les attributs personnalisés {#combining-default-and-custom-attributes}
...
| Cas d'usage                                               | Vues recommandées                                   | Notes                                                                 |
|--------------------------------------------------------|----------------------------------------------------|-----------------------------------------------------------------------|
...
| Requêtes nécessitant les **derniers attributs utilisateur**       | `USER_LATEST_STATE_DEFAULT_ATTRIBUTES_VIEW_SHARED` et `USER_LATEST_STATE_CUSTOM_ATTRIBUTE_VIEW_SHARED` | Fournit des mises à jour en quasi temps réel, mais peut être plus lent pour les grands ensembles de données. |
| **Suivi historique** des modifications d'attributs           | `USER_DEFAULT_ATTRIBUTES_HISTORY_VIEW_SHARED` et `USER_CUSTOM_ATTRIBUTES_HISTORY_VIEW_SHARED`      | Stocke les modifications d'attributs avec une granularité de 12 heures.                     |
...
* Les requêtes sur `USER_DEFAULT_ATTRIBUTES_VIEW_SHARED` ou `USER_CUSTOM_ATTRIBUTES_VIEW_SHARED` devraient s'exécuter en moins de 10 secondes pour de grands ensembles de données (~1 milliard d'utilisateurs) sur un entrepôt de grande taille.
* Les requêtes sur `USER_LATEST_STATE_DEFAULT_ATTRIBUTES_VIEW_SHARED` ou `USER_LATEST_STATE_CUSTOM_ATTRIBUTE_VIEW_SHARED` pour un seul utilisateur s'exécutent en moins d'une minute, mais ne passent pas bien à l'échelle sans filtrage par `USER_ID`.
