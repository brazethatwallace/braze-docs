...
## Droits d'accès à la distribution de données {#data-distribution-entitlements}

Votre droit d'accès à la distribution de données détermine les types d'événements disponibles dans votre partage de données. Braze organise les événements dans les catégories suivantes :

| Droit d'accès | Catégorie d'événement | Description | Référence du glossaire des événements |
|------------|----------------|-------------|--------------------------|
| **Événements d'engagement** | Événements d'engagement lié aux messages | Événements liés aux envois, livraisons, ouvertures, clics, rebonds et autres interactions avec les canaux de communication | [Événements d'engagement lié aux messages]({{site.baseurl}}/user_guide/data/distribution/braze_currents/event_glossary/message_engagement_events) |
| **Événements de comportement client** | Événements d'engagement lié aux messages et événements de comportement client | Inclut tous les événements d'engagement lié aux messages, ainsi que les événements liés aux achats, événements personnalisés, sessions, attribution et actions des utilisateurs dans l'application | [Événements d'engagement lié aux messages]({{site.baseurl}}/user_guide/data/distribution/braze_currents/event_glossary/message_engagement_events), [Événements de comportement client et utilisateur]({{site.baseurl}}/user_guide/data/distribution/braze_currents/event_glossary/customer_behavior_events) |
| **Profils utilisateur et attributs** | Événements d'engagement lié aux messages, événements de comportement client et événements de profil utilisateur | Inclut les événements d'engagement lié aux messages et les événements de comportement client, ainsi que les événements liés aux modifications des profils utilisateur et des attributs | [Événements d'engagement lié aux messages]({{site.baseurl}}/user_guide/data/distribution/braze_currents/event_glossary/message_engagement_events), [Événements de comportement client et utilisateur]({{site.baseurl}}/user_guide/data/distribution/braze_currents/event_glossary/customer_behavior_events), [Événements de profil utilisateur]({{site.baseurl}}/user_guide/data/distribution/braze_currents/event_glossary/user_profiles_events) |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 .reset-td-br-4 aria-label="Droits d'accès à la distribution de données" }

Pour toute question concernant les événements inclus dans votre droit d'accès, contactez votre gestionnaire de compte ou gestionnaire de la satisfaction client Braze.

## À propos du partage sécurisé des données {#about-secure-data-sharing}

Avec le partage de données, aucune donnée réelle n'est copiée ni transférée entre les comptes. Tout le partage s'effectue via la couche de services unique de Snowflake et son magasin de métadonnées. Il s'agit d'un concept important, car les données partagées n'occupent aucun espace de stockage dans votre compte et ne contribuent donc pas à vos frais mensuels de stockage de données. Les **seuls** frais concernent les ressources de calcul (telles que les entrepôts virtuels) utilisées pour interroger les données partagées.
...
- **Réduisez le temps d'accès aux informations**<br>Dites adieu aux processus ETL qui nécessitent des semaines de mise en place. Les architectures uniques de Braze et de Snowflake rendent toutes les données d'engagement client et de Campaign immédiatement accessibles et interrogeables dès leur arrivée dans le lac de données. Aucune donnée n'est copiée ni déplacée, ce qui vous permet d'offrir des expériences client basées uniquement sur les informations les plus pertinentes et les plus récentes.
- **Éliminez les silos de données**<br>Créez une vue globale de vos clients à travers les canaux et les plateformes. Le partage de données facilite plus que jamais la jonction de vos données d'engagement client Braze avec toutes vos autres données Snowflake, générant ainsi des informations plus riches à partir d'une source unique et fiable.
- **Évaluez vos performances d'engagement**<br>Optimisez vos stratégies d'engagement client avec Braze Benchmarks. Cet outil interactif, propulsé par Braze et Snowflake, vous permet de comparer les données d'engagement de votre marque à des références sectorielles par canal, secteur d'activité et plateforme d'appareil.
...
| ----------- | ----------- |
| Accès à Braze | Contactez votre gestionnaire de compte Braze ou votre gestionnaire de la satisfaction client pour configurer le partage de données. |
...
Une fois le partage de données provisionné, créez une base de données à partir du partage de données entrant, ce qui fait apparaître toutes les tables partagées dans votre instance Snowflake et les rend interrogeables comme n'importe quelles autres données stockées dans votre instance. Cependant, gardez à l'esprit que les données partagées sont en lecture seule et ne peuvent être qu'interrogées, mais en aucun cas modifiées ou supprimées.

Comme pour Currents, vous pouvez utiliser le partage sécurisé de données Snowflake pour :

{% multi_lang_include partners/data_sharing_use_cases.md %}

[Télécharger les schémas bruts des tables.](/docs/assets/download_file/data-sharing-raw-table-schemas.txt)

{% alert note %}
Le téléchargement du schéma brut n'inclut pas les vues des attributs de profil utilisateur. Pour les schémas complets et les instructions d'utilisation de `USER_DEFAULT_ATTRIBUTES_VIEW_SHARED`, `USER_LATEST_STATE_DEFAULT_ATTRIBUTES_VIEW_SHARED` et des vues d'attributs utilisateur associées, consultez [Attributs de profil utilisateur]({{site.baseurl}}/partners/data_and_analytics/data_warehouses/snowflake/user_attributes).
{% endalert %}
...
Vous pourrez interroger les deux années les plus récentes de données pour chaque événement dans la vue `USERS_*_SHARED` correspondante. De plus, chaque événement disposera d'une vue `USERS_*_SHARED_ALL` qui peut être interrogée pour renvoyer à la fois les données anonymisées et non anonymisées.

#### Données historiques {#historical-data}

L'archive des données d'événements historiques dans Snowflake remonte à avril 2019. Durant les premiers mois où Braze stockait des données dans Snowflake, des modifications produit ont été effectuées qui ont pu entraîner des données légèrement différentes ou contenant des valeurs nulles (car nous ne transmettions pas les données dans tous les champs disponibles à cette époque). Il est préférable de considérer que tout résultat incluant des données antérieures à août 2019 peut être légèrement différent de ce qui est attendu.
