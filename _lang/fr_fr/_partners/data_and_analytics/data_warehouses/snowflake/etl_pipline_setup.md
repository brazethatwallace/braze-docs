...
# Configuration du pipeline d'événements ETL {#etl-event-pipeline-setup}
...
Vous pouvez utiliser cette requête de clics sur les e-mails pour analyser les interactions avec des e-mails spécifiques dans vos Campaigns et Canvas Braze.

## Configurer cette requête {#set-up-this-query}

Créez une base de données pour `BRAZE`, puis créez-en une si elle n'existe pas encore pour `BRAZE_CURRENTS;` :
...
Utilisez la commande suivante pour créer votre table :
...
Utilisez la commande suivante pour créer ou remplacer votre pipe :
...
## Aller plus loin avec cet exemple de requête {#do-more-with-this-query-example}

Copiez le paramètre `notification_channel` à partir de la sortie de la commande précédente et utilisez-le lors de la configuration des notifications du compartiment S3.

Synchronisez manuellement S3 vers Snowflake pour le nom de pipe indiqué :
```sql
ALTER PIPE
  pipe_users_messages_email_click
  refresh ;
...
Vérifiez l'état du pipe, qui indiquera quand le message a été transmis de S3 à Snowflake :
```sql
SELECT
  SYSTEM$PIPE_STATUS(
    'pipe_users_messages_email_click'
