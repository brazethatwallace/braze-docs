...
4. Complétez le mappage du catalogue vers la destination sous l'onglet **Data To Send**. Vous trouverez ci-dessous des détails sur la façon d'effectuer le mappage.
...
| Nom de l'événement personnalisé | Mappage statique | Ceci est mappé à `Event Name` dans le catalogue Zeotap.<br><br>Le nom de l'événement personnalisé et l'horodatage de l'événement personnalisé doivent tous deux être mappés pour capturer des événements personnalisés dans Braze. L'événement personnalisé ne peut pas être traité si l'un des deux n'est pas mappé. Pour plus d'informations, consultez la section sur l'[objet d'événement]({{site.baseurl}}/api/objects_filters/event_object/#what-is-the-event-object). |
