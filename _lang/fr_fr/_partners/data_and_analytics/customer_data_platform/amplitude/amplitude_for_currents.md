...
|---|---|
| Compte Amplitude | Un [compte Amplitude](https://amplitude.com/) est requis pour tirer parti de ce partenariat. |
...
Braze n'enverra les données d'événement que pour les utilisateurs dont l'`external_user_id` est défini ou les utilisateurs anonymes dont le `device_id` est défini. Pour les utilisateurs anonymes, vous devrez synchroniser votre ID d'appareil Amplitude avec l'ID d'appareil Braze dans le SDK. Par exemple :

```java
amplitude.setDeviceId(Appboy.getInstance(context).getDeviceId();)
```

Vous pouvez exporter deux types d'événements vers Amplitude : les [événements d'engagement lié aux messages](#supported-currents-events), composés des événements Braze directement liés à l'envoi de messages, et les [événements de comportement client](#supported-currents-events), incluant d'autres activités de l'application ou du site web telles que les sessions, les custom events et les achats suivis via la plateforme. Tous les événements standard sont préfixés par `[Appboy]`, et tous les custom events sont préfixés par `[Appboy] [Custom Event]`. Les propriétés de custom events et d'événements d'achat sont préfixées respectivement par `[Custom event property]` et `[Purchase property]`.

{% alert note %}
Braze Currents applique le préfixe `[Appboy]` lors de l'exportation des événements vers Amplitude. Ce libellé fait référence à l'ancien nom de produit de Braze. Il s'agit d'un comportement attendu et cela n'indique pas un problème de SDK ou d'intégration.
{% endalert %}
...
{% alert warning %}
Gardez votre clé API Amplitude à jour. Si les informations d'identification de votre connecteur expirent, le connecteur cessera d'envoyer des événements. Si cela persiste pendant plus de **48 heures**, les événements du connecteur seront supprimés et les données seront définitivement perdues.
{% endalert %}

### Étape 2 : Créer un Braze Current {#step-2-create-braze-current}

Dans Braze, accédez à **Currents > + Create Current > Create Amplitude Export**. Indiquez un nom d'intégration, une adresse e-mail de contact, la clé API d'exportation Amplitude et la région Amplitude dans les champs listés. Ensuite, sélectionnez les événements que vous souhaitez suivre ; une liste des événements disponibles est fournie. Enfin, cliquez sur **Launch Current**.
...
{% tab note %}
Pour en savoir plus, consultez la documentation d'Amplitude sur l'[intégration Appboy Amplitude](https://amplitude.zendesk.com/hc/en-us/articles/115000217351-Appboy-Amplitude-Integration#how-to-set-up-and-use-the-integration).
{% endtab %}
