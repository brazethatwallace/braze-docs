...
{% alert important %}
L'envoi d'attributs personnalisés supplémentaires augmentera votre consommation de points de données. Nous vous conseillons de vous adresser à votre gestionnaire du succès des clients pour mieux comprendre cette augmentation potentielle des points de données.
{% endalert %}
...
![Catalogue des destinations Adobe avec la tuile de destination Braze et l'action Configure.]({% image_buster /assets/img/adobe/braze-destination-configure.png %})
...
![Étape du compte de destination Adobe Braze avec la saisie de la clé API et l'action de connexion.]({% image_buster /assets/img/adobe/braze-destination-account.png %}){: style="max-width:60%"}
...
- **Marketing use case** : les cas d'usage marketing indiquent l'intention pour laquelle les données seront exportées vers la destination. Vous pouvez choisir parmi les cas d'usage marketing définis par Adobe ou créer votre propre cas d'usage marketing. Pour en savoir plus sur les cas d'usage marketing d'Adobe, consultez la page [Gouvernance des données dans Adobe Experience Platform](https://experienceleague.adobe.com/docs/experience-platform/rtcdp/privacy/data-governance-overview.html?lang=en#destinations).

![Étape d'authentification de la destination Adobe avec les champs nom, destination et endpoint.]({% image_buster /assets/img/adobe/braze-destination-authentication.png %}){: style="max-width:60%;"}
...
1. À l'étape du mappage, sélectionnez **Add new mapping**.<br>![Page de mappage des champs Adobe avec le bouton Add new mapping.]({% image_buster /assets/img/adobe/braze-destination-mapping.png %}){: style="max-width:50%;"}<br><br>
2. Dans la section du champ source, sélectionnez le bouton fléché à côté du champ vide pour ouvrir la fenêtre de sélection du champ source.<br>![Sélecteur de champ source Adobe pour le mappage de destination.]({% image_buster /assets/img/adobe/braze-destination-mapping-source.png %})<br><br>
3. Dans la fenêtre, sélectionnez les attributs Adobe à mapper à vos attributs Braze. <br>![Sélecteur d'attributs Adobe affichant les attributs source pour le mappage.]({% image_buster /assets/img/adobe/braze-destination-mapping-attributes.png %}){: style="max-width:70%;"}<br><br>Sélectionnez ensuite l'espace de noms de l'identité. Cette option permet de mapper un espace de noms d'identité de la plateforme à un espace de noms de Braze.<br>![Sélecteur d'espace de noms d'identité Adobe utilisé pour le mappage Braze.]({% image_buster /assets/img/adobe/braze-destination-mapping-namespaces.png %}){: style="max-width:80%;"}<br> Choisissez vos champs source, puis sélectionnez **Select**.<br><br>
4. Dans la section du champ cible, sélectionnez l'icône de mappage à côté du champ.<br>![Panneau de mappage du champ cible Adobe avec l'icône de mappage sélectionnée.]({% image_buster /assets/img/adobe/braze-destination-mapping-target.png %}){: style="max-width:90%;"} <br><br>
5. Dans la fenêtre de sélection du champ cible, vous pouvez choisir entre trois catégories de champs cibles :<br><br>• **Select identity namespace** : utilisez cette option pour mapper les espaces de noms d'identité de la plateforme aux espaces de noms d'identité de Braze.<br>• **Select custom attributes** : utilisez cette option pour mapper les attributs Adobe XDM aux attributs Braze personnalisés que vous avez définis dans votre compte Braze. <br><br>![Sélecteur de champ cible Adobe avec les options d'espace de noms d'identité et d'attributs personnalisés.]({% image_buster /assets/img/adobe/braze-destination-mapping-target-fields.png %}){: style="max-width:60%;"}<br><br>**Vous pouvez également utiliser cette option pour renommer des attributs XDM existants dans Braze.** Par exemple, le mappage d'un attribut XDM `lastname` à un attribut personnalisé `Last_Name` dans Braze créera l'attribut `Last_Name` dans Braze s'il n'existe pas déjà, et mappera l'attribut XDM `lastname` à celui-ci. <br><br> Choisissez vos champs cibles, puis sélectionnez **Select**.<br><br>
6. Votre mappage de champ devrait apparaître dans la liste.<br>![Mappages de champs Adobe vers Braze complétés, listés dans l'étape de mappage de destination.]({% image_buster /assets/img/adobe/braze-destination-mapping-complete.png %})<br><br>
