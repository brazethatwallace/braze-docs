...
Une intégration partenaire basée sur la fonctionnalité de Transformation des données pourrait être un modèle de code de transformation partagé avec les clients via la documentation publique.

Pour les clients communs, cela ressemblerait à ceci :

1. Ils se connectent à votre plateforme et configurent les webhooks.
2. Ils travaillent avec leur équipe Braze pour obtenir l'accès à la Transformation des données Braze et créent une nouvelle transformation dans leur tableau de bord de Braze.
...
7. Une fois activée, ils peuvent vérifier via l'outil de recherche d'utilisateurs de Braze que le profil utilisateur a été mis à jour en fonction du webhook, et modifier le code de transformation selon leurs besoins.
