...
Vous trouverez ci-après des explications pour la configuration d'un webhook Facebook Messenger dans Braze.
Pour ceux qui ont besoin d'aide supplémentaire pour configurer leur bot, un tutoriel complet sur le bot Messenger et un code d'exemple sont disponibles dans le [dépôt GitHub de Braze](https://github.com/Appboy/appboy-fb-messenger-bot) !

### Étape 1 : Collectez vos PSID {#step-1-collect-your-psids}
...
Chaque fois que vous envoyez un message, le PSID du destinataire sera inclus dans la propriété `recipient.id` de la requête afin d'identifier la personne qui doit recevoir le message.
...
#### Prévisualisation et test de votre webhook {#previewing-and-testing-your-webhook}
...
![Onglet Test dans le modèle de webhook Facebook Messenger montrant que vous pouvez prévisualiser le message en l'envoyant à un utilisateur existant.]({% image_buster /assets/img_archive/fbm-test.png %})
...
Si vous choisissez de ne pas créer de segment spécifique pour les abonnés Messenger, veillez à inclure un filtre pour les ID Messenger existants afin d'éviter les erreurs.
