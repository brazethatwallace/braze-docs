...
| ----------- | ----------- |
| Compte Typeform | Un compte Typeform avec accès aux webhooks est nécessaire pour tirer parti de ce partenariat. |
| Transformation de données Braze | Une [URL de transformation de données]({{site.baseurl}}/data_transformation) est nécessaire pour recevoir les données de Typeform. |
...
À l'étape 4, ajoutez l'URL du webhook de votre transformation de données comme **URL de destination**.
...
![Page View deliveries de Typeform avec Send test request sélectionné.]({% image_buster /assets/img/typeform/typeform_test_request.png %})
...
Au cours de cette étape, vous transformez le payload du webhook envoyé depuis Typeform en une valeur de retour sous forme d'objet JavaScript.

1. Actualisez votre transformation de données et assurez-vous que le payload de test Typeform est visible dans les **détails du webhook**.
...
Les modèles suivants sont des exemples créés à partir de notre [exemple de payload webhook Typeform](#example-typeform-webhook-payload). Ces modèles peuvent servir de point de départ. Vous pouvez partir de zéro ou supprimer des composants spécifiques selon vos besoins.

Dans ces modèles d'exemple, nous enregistrons un événement personnalisé dans le profil Braze. Le titre du Typeform est transmis comme nom de l'événement personnalisé, et les résultats du Typeform sont transmis en tant que propriétés d'événement. Ces modèles d'exemple ne prennent pas en charge les types de questions Calendly, File Upload ou Payment dans Typeform.

### Cas d'usage : l'e-mail comme identifiant {#use-case-email-as-identifier}

Dans ce modèle d'exemple, nous utilisons une adresse e-mail (capturée à partir d'une question de type adresse e-mail dans le typeform) comme identifiant.
...
### Cas d'usage : utiliser un identifiant transmis via les Hidden Fields {#use-case-using-identifier-passed-in-hidden-fields}

Vous pouvez utiliser les Hidden Fields de Typeform pour transmettre des données dans le payload du webhook Typeform, comme l'ID d'un utilisateur, sans avoir à inclure cette information dans la réponse du Typeform.

Dans ce modèle d'exemple, nous utilisons un Hidden Field « user_id » et nous le transmettons dans le payload de la requête `/users/track` en tant que `external_id`. Bien que nous utilisions « user_id », les champs peuvent être modifiés selon vos besoins.
...
Une fois votre Data Transformation rédigée, cliquez sur **Validate** pour vérifier que le code de votre Data Transformation est correctement formaté et fonctionnera comme prévu. Ensuite, enregistrez et activez votre Data Transformation.

Une fois activée, les données d'événement personnalisé sont enregistrées dans le profil d'un utilisateur lorsqu'il complète votre formulaire.
