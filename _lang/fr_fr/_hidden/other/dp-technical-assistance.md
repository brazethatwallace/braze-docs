...
{% alert important %}
Les recommandations de Braze figurant sur cette page ne s'appliquent pas à BrazeAI Decisioning Studio™. Pour gérer toute demande relative aux droits à la vie privée concernant des données personnelles dans BrazeAI Decisioning Studio™, contactez votre gestionnaire de compte.
{% endalert %}
...
De manière similaire au droit d'accès, vous pouvez utiliser l'[API REST](https://www.braze.com/docs/api/endpoints/export/#user-export) de Braze pour exporter les données personnelles d'un utilisateur final et les fournir à la personne concernée conformément à sa demande.
...
Pour plus d'informations sur la façon de créer des audiences à synchroniser avec des plateformes tierces, consultez la rubrique [Synchronisation d'audience](https://www.braze.com/docs/partners/canvas_steps).
