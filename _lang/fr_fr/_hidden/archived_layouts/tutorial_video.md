...
Tutoriels :
Une procédure pas-à-pas expliquant un concept instructif. Il doit contenir des INFORMATIONS PRATIQUES. Se concentre sur un seul sujet (par exemple, comment créer une Campaign, comment créer un Canvas, etc.) Un article abordant des objectifs ou des tâches spécifiques ÉTAPE PAR ÉTAPE pour résoudre un problème en particulier (par ex. : comment cibler un groupe d'utilisateurs, comment segmenter une audience en fonction de l'emplacement, etc.).
{% endcomment %}
# Comment choisir un titre pour ce tutoriel vidéo {#how-to-title-this-video-tutorial}

> Cet article vous guidera pour réaliser XYZ. À la fin de ce tutoriel, vous devriez être en mesure de faire A, B et C !

## Étape 1 : Commencez par faire ceci {#step-1-do-the-first-thing}

Voici ce que vous allez accomplir lors de cette étape. Voici comment reproduire l'exemple.

Veillez à [créer des liens vers d'autres références lorsque vous en avez besoin]({{site.baseurl}}/contributing/content_types/#references).
...
### Cela fait également partie de l'étape deux {#this-is-also-part-of-step-two}
...
## Étape 3 : Ajoutez votre conclusion {#step-3-do-the-last-thing}

La dernière étape du tutoriel doit donner au lecteur une idée du résultat attendu. N'hésitez pas à ajouter une image pour illustrer le résultat final.
...
2. Accompli une tâche, et
3. Être prêt à passer à la suite.
