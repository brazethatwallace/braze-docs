...
L'intégration de LiftIgniter et Braze utilise le contenu connecté pour vous permettre de recommander des sujets intéressants tels que des articles d'actualité, des vêtements, d'autres articles de vente au détail et des vidéos.
...
Une fois ce message placé dans le corps de l'éditeur, vous pouvez prévisualiser votre message. Vous pouvez même insérer des images, comme le montre l'exemple suivant :

![Une image de prévisualisation montrant à quoi ressemblera le message après son envoi.]({% image_buster /assets/img/liftigniter2.png %})