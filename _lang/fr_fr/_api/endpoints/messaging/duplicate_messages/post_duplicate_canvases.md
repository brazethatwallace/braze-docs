---
nav_title: "POST : Dupliquer les Canvas"
article_title: "POST : Dupliquer les Canvas"
search_tag: Endpoint
page_order: 5
layout: api_page
page_type: reference
description: "Cet article présente les détails de l'endpoint Dupliquer les Canvas."
---

{% api %}
# Dupliquer des Canvas à l'aide de l'API {#duplicate-canvases-using-the-api}
{% apimethod post core_endpoint|/docs/core_endpoints %}
/canvas/duplicate
{% endapimethod %}

> Utilisez cet endpoint pour dupliquer des Canvas. Cet endpoint API fonctionne de manière similaire à la [duplication de Canvas dans le tableau de bord de Braze]({{site.baseurl}}/user_guide/messaging/governance/duplicating).

## Conditions préalables {#prerequisites}

Pour utiliser cet endpoint, vous devez générer une clé API avec l'autorisation `canvas.duplicate`.

## Limite de débit {#rate-limit}

Cet endpoint est limité à 100 appels API par minute.

## Corps de la requête {#request-body}

```
Content-Type: application/json
Authorization: Bearer YOUR-REST-API-KEY
```

```json
{
  "canvas_id": (required, string) The Canvas identifier,
  "name": (required, string) The name of the resulting Canvas,
  "description": (optional, string) The description of the resulting Canvas,
  "tag_names": (optional, array of strings) The tags of the resulting Canvas,
}
```

## Paramètres de requête {#request-parameters}

| Paramètre | Requis | Type de données | Description |
| --------- | ---------| --------- | ----------- |
| `canvas_id` | Requis | Chaîne de caractères | Voir [Identifiant Canvas]({{site.baseurl}}/api/identifier_types). |
| `name` | Requis | Chaîne de caractères | Le nom du Canvas résultant. |
| `description` | Facultatif | Chaîne de caractères | Le champ de description du Canvas résultant. |
| `tag_names` | Facultatif | Tableau de chaînes de caractères | Les tags du Canvas résultant. Il doit s'agir de tags existants. Si vous ajoutez de nouveaux tags dans la requête, ils remplaceront tous les tags présents sur le Canvas d'origine. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3  .reset-td-br-4 aria-label="Paramètres de requête" }

## Réponse {#response}

Cet endpoint renvoie un code de statut `202` et la création du Canvas s'effectue de manière asynchrone. Vous pouvez utiliser le [téléchargement des événements de sécurité]({{site.baseurl}}/user_guide/administer/global/admin_settings/security_settings#security-event-report) pour consulter les enregistrements indiquant quand les Canvas ont été dupliqués et par quelle clé API.

{% endapi %}