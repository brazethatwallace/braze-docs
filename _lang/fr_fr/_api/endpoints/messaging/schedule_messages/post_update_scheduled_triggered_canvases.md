...
Notez que pour envoyer des messages avec cet endpoint, vous devez disposer d'un ID Canvas, créé lorsque vous créez un [Canvas]({{site.baseurl}}/api/identifier_types#canvas-identifier).
...
Pour utiliser cet endpoint, vous aurez besoin d'une [clé API]({{site.baseurl}}/api/basics#rest-api-key-permissions) avec l'autorisation `canvas.trigger.schedule.update`.
