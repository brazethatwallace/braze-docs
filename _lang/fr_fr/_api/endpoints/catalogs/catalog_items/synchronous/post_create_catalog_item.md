...
---
{% api %}
# Créer un produit du catalogue {#create-catalog-item}
{% apimethod post %}
...
Pour utiliser cet endpoint, vous aurez besoin d'une [clé API]({{site.baseurl}}/api/basics#rest-api-key-permissions) avec l'autorisation `catalogs.create_item`.
...
| `invalid-ids` | Les caractères pris en charge pour les noms d'ID de produit sont les lettres, les chiffres, les tirets et les traits de soulignement. |
...
| `invalid-keys-in-value-object` | Les clés d'objet produit ne peuvent pas inclure `.` ou `$`. |
