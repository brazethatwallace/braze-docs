---
nav_title: "GET : Exporter les sessions de l'application par période"
article_title: "GET : Exporter les sessions de l'application par période"
search_tag: Endpoint
page_order: 4
layout: api_page
page_type: reference
description: "Cet article présente en détail l'endpoint Braze Exporter les analyses de sessions d'application par période."

---
{% api %}
# Exporter les sessions de l'application par période {#export-app-session-by-time}
{% apimethod get %}
...
Pour utiliser cet endpoint, vous aurez besoin d'une [clé API]({{site.baseurl}}/api/basics#rest-api-key-permissions) avec l'autorisation `sessions.data_series`.
...
| `segment_id` | Facultatif | Chaîne de caractères | Voir [Identifiant API de Segment]({{site.baseurl}}/api/identifier_types). ID du Segment avec analyse activée pour lequel les sessions doivent être renvoyées. |
