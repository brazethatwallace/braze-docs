...
---
{% api %}
# Exporter le profil utilisateur par segment {#export-user-profile-by-segment}
{% apimethod post %}
...
Pour utiliser cet endpoint, vous aurez besoin d'une [clé API]({{site.baseurl}}/api/basics#rest-api-key-permissions) avec l'autorisation `users.export.segment`.
...
{% details Détail du chemin d'exportation pour ZIP %}
**Format ZIP :**
...
| `custom_attributes_to_export` | Facultatif  | Tableau de chaînes de caractères | Noms des attributs personnalisés spécifiques à exporter (jusqu'à 500). Omettez `custom_attributes` de `fields_to_export` lorsque vous utilisez ce paramètre, sinon Braze exporte tous les attributs personnalisés indépendamment de cette liste. Pour créer et gérer des attributs personnalisés dans le tableau de bord, allez dans **Data Settings** > **Custom Attributes**.                                                                                                                                                                                                          |
