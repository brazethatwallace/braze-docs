...
article_title: "GET : Exporter les analyses de Campaign"
search_tag: Endpoint
page_order: 4
layout: api_page
page_type: reference
description: "Cet article présente les détails de l'endpoint Braze d'exportation des analyses de Campaign."

---
{% api %}
# Exporter les analyses de Campaign {#export-campaign-analytics}
{% apimethod get %}
/campaigns/data_series
{% endapimethod %}

> Utilisez cet endpoint pour récupérer une série quotidienne de diverses statistiques pour une Campaign au fil du temps.

Les données renvoyées incluent le nombre de messages envoyés, ouverts, cliqués ou convertis par canal de communication.
...
Pour utiliser cet endpoint, vous aurez besoin d'une [clé API]({{site.baseurl}}/api/basics#rest-api-key-permissions) avec la permission `campaigns.data_series`.
...
| --------- | -------- | --------- | ----------- |
| `campaign_id` | Requis | String | Voir [identifiant API de Campaign]({{site.baseurl}}/api/identifier_types).<br><br> Le `campaign_id` pour les Campaigns API se trouve sur la page [Clés API]({{site.baseurl}}/user_guide/administer/global/workspace_settings/apis_and_identifiers) et sur la page **Détails de la Campaign** dans votre tableau de bord, ou vous pouvez utiliser l'[endpoint Lister les Campaigns]({{site.baseurl}}/api/endpoints/export/campaigns/get_campaigns). |
...
| `ending_at` | Optionnel | Datetime <br>([ISO-8601](https://en.wikipedia.org/wiki/ISO_8601) string) | Date à laquelle la série de données doit se terminer. Par défaut, correspond à l'heure de la requête. |
...
### Réponse multicanale {#multichannel-response}
...
Les types de messages possibles sont : `email`, `trigger_in_app_message`, `webhook`, `android_push`, `ios_push`, `kindle_push` et `web_push`. Tous les types de notifications push affichent les mêmes statistiques que celles présentées pour `android_push`.
...
### Consulter les échecs de distribution pour les Campaigns déclenchées par API {#viewing-delivery-failures-for-api-triggered-campaigns}
