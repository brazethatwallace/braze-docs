...
Pour utiliser cet endpoint, vous aurez besoin d'une [clé API]({{site.baseurl}}/api/basics#rest-api-key-permissions) avec la permission `custom_objects.object_relationships.update`.
...
| Paramètre | Obligatoire | Type de données | Description |
|---|---|---|---|
| `object_relationship` | Obligatoire | Objet | Enregistrement de relation créé ou remplacé |
| `object_relationship.rel_kind` | Obligatoire | String | Valeur du type de relation |
| `object_relationship.to_custom_object` | Conditionnel | Objet | Objet associé lorsque `anchor=source` |
| `object_relationship.from_custom_object` | Conditionnel | Objet | Objet associé lorsque `anchor=target` |
...
| `404` | Relation ou objets de l'endpoint introuvables (`custom-object-relationship-not-found`) | Vérifiez que les deux objets et les noms de types associés existent dans l'espace de travail. |
| `422` | Limite de relations par objet atteinte (`custom-object-relationship-limit-exceeded`) | Réduisez le nombre de relations pour l'objet, ou contactez le support Braze concernant les limites de l'espace de travail. |
...
| `403` | La clé API ne dispose pas de la permission ou la requête est bloquée par la liste d'autorisation | Vérifiez que la clé possède la permission `custom_objects.object_relationships.update` et que votre adresse IP source figure dans la liste d'autorisation de la clé, si elle est configurée. |
