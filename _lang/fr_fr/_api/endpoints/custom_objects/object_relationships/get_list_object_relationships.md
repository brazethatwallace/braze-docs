...
> Utilisez cet endpoint pour lister les objets personnalisés liés à partir d'un ancrage d'objet.
...
Le tableau suivant répertorie et décrit les paramètres de requête pour l'endpoint `/custom_objects/objects/{type_name}/{external_id}/object_relationships`.

| Paramètre | Obligatoire | Type de données | Description |
|---|---|---|---|
| `anchor` | Facultatif | String | `source` (par défaut) ou `target` |
| `rel_kind` | Facultatif | String | Filtrer par un type de relation |
| `limit` | Facultatif | Integer | Taille de page. Par défaut `100`. Limité entre `1` et `250` |
...
Cette section inclut un exemple de payload de paramètres et un exemple de requête cURL.
...
Avec `anchor=target`, les objets liés sont renvoyés sous la forme `from_custom_object`.
...
| `items[].to_custom_object` | Conditionnel | Object | Objet lié lorsque `anchor=source` |
| `items[].from_custom_object` | Conditionnel | Object | Objet lié lorsque `anchor=target` |
| `items[].to_custom_object.type_name` | Conditionnel | String | Nom du type de l'objet lié |
| `items[].to_custom_object.external_id` | Conditionnel | String | ID externe de l'objet lié |
| `items[].to_custom_object.attributes` | Conditionnel | Object | Attributs de l'objet lié |
| `items[].from_custom_object.type_name` | Conditionnel | String | Nom du type de l'objet lié |
| `items[].from_custom_object.external_id` | Conditionnel | String | ID externe de l'objet lié |
| `items[].from_custom_object.attributes` | Conditionnel | Object | Attributs de l'objet lié |
...
| `403` | La clé API n'a pas les permissions requises ou la requête est bloquée par la liste d'autorisation | Vérifiez que la clé dispose de la permission `custom_objects.read` et que votre adresse IP source figure dans la liste d'autorisation de la clé, si elle est configurée. |
