...
> Utilisez cet endpoint pour associer un utilisateur Braze à un objet personnalisé.
...
Pour utiliser cet endpoint, vous avez besoin d'une [clé API]({{site.baseurl}}/api/basics#rest-api-key-permissions) avec la permission `custom_objects.user_relationships.create`.
...
|---|---|---|
| `400` | `rel_kind` inconnu pour le type ou erreur de validation du schéma | Confirmez que `rel_kind` est valide pour le type d'objet et que les `attributes` correspondent au schéma de la relation. |
...
| `403` | La clé API ne dispose pas de la permission ou la requête est bloquée par la liste d'autorisation | Confirmez que la clé possède la permission `custom_objects.user_relationships.create` et que votre adresse IP source figure dans la liste d'autorisation de la clé, si celle-ci est configurée. |
