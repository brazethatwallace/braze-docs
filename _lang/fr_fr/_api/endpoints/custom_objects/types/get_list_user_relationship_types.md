...
> Utilisez cet endpoint pour lister les valeurs `rel_kind` valides pour les relations utilisateur sur un type d'objet personnalisé.
...
Le tableau suivant répertorie et décrit les paramètres de chemin pour l'endpoint `/custom_objects/types/{type_name}/user_relationship_types`.
...
Le tableau suivant répertorie et décrit les paramètres de requête pour l'endpoint `/custom_objects/types/{type_name}/user_relationship_types`.

| Paramètre | Obligatoire | Type de données | Description |
|---|---|---|---|
| `limit` | Facultatif | Integer | Taille de page. Par défaut `100`. Limité de `1` à `250` |
...
Cet exemple liste les types de relations utilisateur que vous pouvez utiliser pour associer des utilisateurs aux enregistrements `account`.
...
| `items[].display_name` | Obligatoire | String | Libellé d'affichage pour le type de relation |
