...
Le tableau suivant répertorie et décrit les paramètres de chemin pour l'endpoint `/custom_objects/types/{type_name}/object_relationship_types`.
...
Le tableau suivant répertorie et décrit les paramètres de requête pour l'endpoint `/custom_objects/types/{type_name}/object_relationship_types`.
...
Cette section inclut un exemple de payload et un exemple de requête cURL.
...
Cette section inclut un exemple de réponse réussie et les champs de réponse.
...
`related_type_name` correspond au type situé de l'autre côté de la relation pour l'`anchor` sélectionné.
...
| `items[].from_type_name` | Obligatoire | String | Nom du type d'objet personnalisé source |
| `items[].to_type_name` | Obligatoire | String | Nom du type d'objet personnalisé cible |
...
| `items[].related_type_name` | Obligatoire | String | Type du côté opposé pour l'`anchor` demandé |
...
|---|---|---|
| `400` | `anchor` invalide | Utilisez `source` ou `target` pour `anchor`. |
| `404` | Type non trouvé (`custom-object-type-not-found`) | Vérifiez que `type_name` existe dans l'espace de travail et correspond exactement au nom machine. |
...
| `403` | La clé API n'a pas la permission requise ou la requête est bloquée par la liste d'autorisation | Vérifiez que la clé dispose de la permission `custom_objects.read` et que votre adresse IP source figure dans la liste d'autorisation de la clé, si celle-ci est configurée. |
