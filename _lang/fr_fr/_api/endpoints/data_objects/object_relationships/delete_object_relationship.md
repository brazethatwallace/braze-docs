...
{% alert important %}
Les objets de données sont actuellement en accès anticipé. Votre espace de travail doit être activé avant que les autorisations de clé API pour les objets de données n'apparaissent dans **Paramètres** > **Clés API**.
{% endalert %}

## Prérequis {#prerequisites}

Pour utiliser cet endpoint, vous aurez besoin d'une [clé API]({{site.baseurl}}/api/basics#rest-api-key-permissions) avec l'autorisation `data_objects.object_relationships.delete`.
...
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 .reset-td-br-4 aria-label="Paramètres de chemin de suppression de relation d'objet" }
...
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 .reset-td-br-4 aria-label="Paramètres de requête de suppression de relation d'objet" }
...
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 .reset-td-br-4 aria-label="Paramètres de réponse de suppression de relation d'objet" }
...
| `404` | Relation ou objet de l'endpoint introuvable | Vérifiez que les deux objets existent et que les valeurs de la clé de relation correspondent à une arête existante. |
...
| `403` | La clé API ne dispose pas de l'autorisation ou la requête est bloquée par la liste d'autorisation | Vérifiez que la clé dispose de l'autorisation `data_objects.object_relationships.delete` et que votre adresse IP source figure dans la liste d'autorisation de la clé, si elle est configurée. |
