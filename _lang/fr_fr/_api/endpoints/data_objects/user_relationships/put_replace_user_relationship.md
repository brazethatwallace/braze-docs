...
Pour utiliser cet endpoint, vous aurez besoin d'une [clé API]({{site.baseurl}}/api/basics#rest-api-key-permissions) avec l'autorisation `data_objects.user_relationships.update`.
...
| Paramètre | Obligatoire | Type de données | Description |
|---|---|---|---|
| `user_relationship` | Obligatoire | Objet | Enregistrement de relation utilisateur créé ou remplacé |
...
| `404` | Relation ou objet introuvable (`data-object-relationship-not-found`) | Vérifiez que l'objet, l'utilisateur et les valeurs de clé de la relation existent tous. |
...
| `403` | La clé API n'a pas l'autorisation ou la requête est bloquée par la liste d'autorisation | Vérifiez que la clé dispose de l'autorisation `data_objects.user_relationships.update` et que votre adresse IP source figure dans la liste d'autorisation de la clé, si celle-ci est configurée. |
