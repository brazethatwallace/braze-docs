...
Pour utiliser cet endpoint, vous aurez besoin d'une [clé API]({{site.baseurl}}/api/basics#rest-api-key-permissions) avec la permission `data_objects.user_relationships.delete`.
...
Le tableau suivant répertorie et décrit les paramètres du corps de la requête JSON pour l'endpoint `/data_objects/objects/{type_name}/{external_id}/users`.
...
| `403` | La clé API ne dispose pas de la permission ou la requête est bloquée par la liste d'autorisation | Vérifiez que la clé dispose de la permission `data_objects.user_relationships.delete` et que votre adresse IP source figure dans la liste d'autorisation de la clé, si elle est configurée. |
