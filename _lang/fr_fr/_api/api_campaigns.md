...
1. Ajoutez un titre descriptif pour pouvoir retrouver les résultats sur la page Campaigns après avoir envoyé vos messages.
2. Cliquez sur **Add Message** et ajoutez les types de messages qui seront inclus dans votre campagne API. Cela vous permettra de générer un `campaign_id` et un ID de variante de message, qui diffère pour chaque canal que vous ajoutez.
