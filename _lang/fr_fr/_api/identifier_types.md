...
{% tabs local %}
{% tab App Identifiers %}
Allez dans **Paramètres** > **Clés API** > **App Identifiers**. Votre clé API pour chaque application est indiquée dans la colonne **Identifier**.
{% endtab %}

{% tab Paramètres des applications %}
Allez dans **Paramètres** > **Paramètres des applications**. Votre clé API est indiquée à côté du champ **API Key** dans la section des paramètres.
...
{% tabs local %}
{% tab Modèles %}
Allez dans **Modèles**, sélectionnez une page de modèle, puis choisissez un modèle préexistant. Si le modèle que vous voulez n'existe pas encore, créez-en un et enregistrez-le. Au bas de la page du modèle individuel, vous trouverez l'identifiant de votre modèle.
{% endtab %}

{% tab Clés API %}
Allez dans **Paramètres** > **Clés API**. Ici, Braze propose une recherche d'**Additional API Identifiers** qui vous permet de rechercher des identifiants spécifiques.
...
- Avec la distribution déclenchée par l'API pour collecter des statistiques sur les messages transactionnels
...
{% tabs local %}
{% tab Campaigns %}
Allez dans **Envoi de messages** > **Campaigns** et sélectionnez une Campaign préexistante. Si la Campaign que vous souhaitez n'existe pas encore, créez-en une et enregistrez-la. Au bas de la page de la Campaign individuelle, vous trouverez l'**identifiant API de Campaign**.
...
- Avec Currents pour apporter des données au niveau de l'utilisateur pour bénéficier d'une vue d'ensemble des Campaigns
- Avec la distribution déclenchée par l'API pour collecter des statistiques sur les messages transactionnels
