...
| [SCIM]({{site.baseurl}}/api/endpoints/scim)                               | Gérer les identités des utilisateurs dans les applications et services cloud.                       |
...
Pour la plupart des API, Braze applique une limitation du débit par défaut de 250 000 requêtes par heure. Cependant, certains types de requêtes ont leur propre limite de débit pour mieux gérer les volumes élevés de données à travers la base de clients. Pour plus de détails, consultez [Limites de débit de l'API]({{site.baseurl}}/api/api_limits)
...
{% alert tip %}
En plus des clés REST API, il existe également un type de clé appelé clés d'identifiant qui peut être utilisé pour référencer des éléments spécifiques tels que des applications, des modèles, des Canvas, des Campaigns, des Content Cards et des Segments depuis l'API. Pour plus d'informations, consultez les [types d'identifiants API]({{site.baseurl}}/api/identifier_types).
{% endalert %}
...
| `users.identify` | [`/users/identify`]({{site.baseurl}}/api/endpoints/user_data/post_user_identify) | Identifier un utilisateur uniquement alias avec un ID externe. |
...
| `users.merge` | [`/users/merge`]({{site.baseurl}}/api/endpoints/user_data/post_users_merge) | Fusionner deux utilisateurs existants entre eux. |
...
| `messages.schedule.create` | [`/messages/schedule/create`]({{site.baseurl}}/api/endpoints/messaging/schedule_messages/post_schedule_messages) | Planifier l'envoi d'un message à un moment spécifique. |
...
| `messages.schedule_broadcasts` | [`/messages/scheduled_broadcasts`]({{site.baseurl}}/api/endpoints/messaging/schedule_messages/get_messages_scheduled) | Interroger tous les messages diffusés planifiés. |
...
| `campaigns.url_info.details` | [`/campaigns/url_info/details`]({{site.baseurl}}) | Interroger les détails d'URL d'une variante de message spécifique au sein d'une Campaign. Cette permission est disponible uniquement pour les espaces de travail avec l'[aliasage de lien]({{site.baseurl}}/user_guide/messaging/templates/email_templates/link_aliasing) activé. Si cette permission n'est pas disponible dans votre espace de travail, contactez votre gestionnaire de compte Braze. |
...
| `canvas.data_summary` | [`/canvas/data_summary`]({{site.baseurl}}/api/endpoints/export/canvas/get_canvas_analytics_summary) | Interroger les résumés d'analyses d'un Canvas sur une période donnée. |
| `canvas.url_info.details` | [`/canvas/url_info/details`]({{site.baseurl}}/get_canvas_link_alias) | Interroger les détails d'URL d'une variante de message spécifique au sein d'une étape Canvas. Cette permission est disponible uniquement pour les espaces de travail avec l'[aliasage de lien]({{site.baseurl}}/user_guide/messaging/templates/email_templates/link_aliasing) activé. Si cette permission n'est pas disponible dans votre espace de travail, contactez votre gestionnaire de compte Braze. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="Permissions des clés REST API" }
...
| `purchases.revenue_series` | [`/purchases/revenue_series`]({{site.baseurl}}/api/endpoints/export/purchases/get_revenue_series) | Interroger les dépenses totales par jour dans votre application sur une période donnée. |
...
| `kpi.mau.data_series` | [`/kpi/mau/data_series`]({{site.baseurl}}/api/endpoints/export/kpi/get_kpi_mau_30_days) | Interroger le nombre total d'utilisateurs actifs uniques sur une fenêtre glissante de 30 jours sur une période donnée. |
...
| Permission | Endpoint | Description |
|---|---|---|
| `templates.email.create` | [`/templates/email/create`]({{site.baseurl}}/api/endpoints/templates/email_templates/post_create_email_template) | Créer un nouveau modèle d'e-mail dans le tableau de bord. |
...
| `templates.email.update` | [`/templates/email/update`]({{site.baseurl}}/api/endpoints/templates/email_templates/post_update_email_template) | Mettre à jour un modèle d'e-mail stocké dans le tableau de bord. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="Permissions des clés REST API" }
...
| `content_blocks.create` | [`/content_blocks/create`]({{site.baseurl}}/api/endpoints/templates/content_blocks_templates/post_create_email_content_block) | Créer un nouveau Content Block dans le tableau de bord. |
| `content_blocks.update` | [`/content_blocks_update`]({{site.baseurl}}/api/endpoints/templates/content_blocks_templates/post_update_content_block) | Mettre à jour un Content Block existant dans le tableau de bord. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="Permissions des clés REST API" }
...
Pour afficher les détails d'une clé API, survolez la clé et sélectionnez <i class="fa-solid fa-eye" alt="Voir"></i> **Voir**. Cela inclut toutes les permissions de cette clé, les adresses IP autorisées (le cas échéant), et si cette clé est inscrite au système d'autorisation IP de Braze.

![La liste des permissions de clés API dans le tableau de bord de Braze.]({% image_buster /assets/img_archive/view-api-key.png %})
...
![Une clé API nommée « Last Seen » avec l'icône de corbeille en surbrillance, affichant « Supprimer ».]({% image_buster /assets/img_archive/api-key-options.png %}){: style="max-width:30%;"}
...
{% alert warning %}
Étant donné que les clés REST API permettent l'accès à des endpoints REST API potentiellement sensibles, assurez-vous qu'elles sont stockées et utilisées de manière sécurisée. Par exemple, n'utilisez pas cette clé pour effectuer des appels AJAX depuis votre site web et ne l'exposez pas de toute autre manière publique.
{% endalert %}
...
### Sécurité des clés REST API et des clés SDK API {#security-of-rest-api-keys-and-sdk-api-keys}
...
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="Sécurité des clés REST API et des clés SDK API" }

### Mise en liste d'autorisation des adresses IP pour l'API {#api-ip-allowlisting}
...
![Option de mise en liste d'autorisation des adresses IP lors de la création d'une clé API.]({% image_buster /assets/img_archive/api-key-ip-whitelisting.png %})

Si vous n'en spécifiez aucune, les requêtes peuvent être envoyées depuis n'importe quelle adresse IP.

{% alert tip %}
Si vous effectuez un webhook Braze-à-Braze et utilisez la mise en liste d'autorisation, consultez la liste des [adresses IP à autoriser]({{site.baseurl}}/user_guide/channels/webhooks/create_a_webhook).
{% endalert %}
...
Si vous déployez Braze en utilisant Ruby, vous pouvez utiliser la [bibliothèque client Ruby](https://github.com/braze-inc/braze-api-client-ruby) pour réduire votre temps d'importation de données. Une bibliothèque client est une collection de code spécifique à un langage de programmation — dans ce cas, Ruby — qui facilite l'utilisation d'une API.
