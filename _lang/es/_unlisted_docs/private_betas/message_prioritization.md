...
Las reglas de priorización se basan en una clasificación de categorías, que es una etiqueta que puedes asignar a una campaña determinada (similar a una [etiqueta]({{site.baseurl}}/user_guide/administrative/app_settings/tags)). Puedes crear hasta 20 categorías en un momento dado.
...
Para reordenar las reglas, selecciona y arrastra el icono <i class="fa-solid fa-grip-vertical"></i> al inicio de una regla. Para eliminar una regla, selecciona el menú <i class="fas fa-ellipsis-vertical"></i> y luego **Eliminar regla**.

Asegúrate de seleccionar **Guardar** para que se apliquen tus actualizaciones.

## Configuración a nivel de campaña {#campaign-level-settings}

### Adhesión voluntaria {#opt-in}

{% alert important %}
Solo las campañas planificadas de un solo canal pueden adherirse a la priorización en este momento. Las campañas basadas en acciones, las desencadenadas por API y los Canvas no son compatibles.
{% endalert %}

Para adherir una campaña a la priorización, selecciona la casilla **Adhesión voluntaria a la priorización de mensajes** en la página **Planificación de entrega** de la campaña.
...
A continuación, asigna la campaña a una categoría seleccionando una del menú desplegable **Categoría**.
...
Puedes adherir hasta 25 campañas activas en un momento dado. Las campañas en borrador, detenidas o archivadas no cuentan para este límite.

### Ventana de reintento {#retry-window}

Una ventana de reintento permite que las campañas adheridas reintenten durante un máximo de tres días si el primer intento no tiene una prioridad lo suficientemente alta para enviarse. En cada día posterior, a la misma hora en que el mensaje fue planificado originalmente, se intenta enviar el mensaje de nuevo. Después del último día en la ventana de reintento, si el mensaje aún no se ha enviado, no se reintenta más y se desprioriza permanentemente.

La ventana de reintento debe ser más corta que el tiempo entre envíos de esa campaña. Si tienes una campaña que se envía cada lunes y miércoles, el intento de reintento ocurre el martes. Esto significa que la ventana de reintento debe configurarse en un día.
...
Para ser elegible para la priorización de mensajes, una campaña debe estar adherida a la limitación de frecuencia. Puedes confirmar que la campaña está adherida dentro de la sección **Controles de entrega** de la página **Planificación de entrega**.
...
Al priorizar dos mensajes de la misma categoría entre sí, daremos mayor prioridad al mensaje con la hora de envío más temprana. Para campañas recurrentes, la hora de envío se calcula como la próxima ocurrencia a partir de la medianoche de hoy en la hora de la empresa. Para campañas planificadas en hora local, asumiremos una hora de envío en la hora de la empresa.
...
- El mensaje de mayor prioridad entró en conflicto con otra campaña futura de prioridad aún mayor para una regla diferente.
...
### ¿Qué pasa con las campañas basadas en acciones o desencadenadas por API? {#what-about-action-based-or-api-triggered-campaigns}

En este momento, la priorización de mensajes no es compatible con las campañas basadas en acciones o desencadenadas por API.
...
En este momento, no hay funcionalidad de informes o análisis específica para esta característica. Te animamos a utilizar la [funcionalidad de informes de Braze]({{site.baseurl}}/user_guide/analytics/reporting) existente para monitorear la salud y el rendimiento de tus campañas priorizadas.