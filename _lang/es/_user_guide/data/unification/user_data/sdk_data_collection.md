...
| Datos de interacción del mensaje del SDK | Direct Opens de push, interacciones con mensajes dentro de la aplicación, interacciones con Content Cards | Este atributo se utiliza con fines de control de calidad, como comprobar que se ha recibido un mensaje y que el envío no está duplicado. |
...
| Operador inalámbrico del dispositivo | Android, iOS      | El operador de telefonía móvil                                                                 | Este atributo se utiliza opcionalmente para la segmentación de mensajes.<br><br>**Nota:** Este campo ha quedado obsoleto a partir de iOS 16 y pasará por defecto a `--` en una futura versión de iOS. |
...
| Resolución              | Android, iOS, Web | Resolución del dispositivo o navegador                                                          | Se utiliza opcionalmente para la segmentación de mensajes basada en dispositivos. El formato de este valor es "`<width>`x`<height>`".                                                                 |
...
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3  .reset-td-br-4 aria-label="Datos opcionales recopilados por defecto" }
...
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3  .reset-td-br-4 aria-label="Datos no recopilados por defecto" }
...
{% alert important %}
Braze prohibirá o bloqueará a los usuarios con más de 5.000.000 de sesiones ("usuarios ficticios") y dejará de ingerir sus eventos del SDK. Para más información, consulta <a href="/docs/user_archival#spam-blocking">Bloqueo de correo no deseado</a>.
{% endalert %}