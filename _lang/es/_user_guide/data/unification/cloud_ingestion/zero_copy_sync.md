...
page_order: 7
page_type: reference
description: "Esta página ofrece un resumen sobre cómo desencadenar Canvas de Braze utilizando CDI."
---

# Personalización sin copia utilizando CDI {#zero-copy-personalization-using-cdi}
...
### Uso de los desencadenantes de Canvas {#using-canvas-triggers}

#### Paso 1: Configurar el origen de datos para los desencadenantes de Canvas {#step-1-set-up-data-source-for-canvas-triggers}

{% tabs %}
{% tab Snowflake %}

##### Paso 1.1: Configura tu tabla de origen en Snowflake {#step-11-set-up-your-source-table-in-snowflake}
...
* `external_id` o `alias_name` y `alias_label` como columna identificadora del usuario. Estos identifican a los usuarios para los que deseas desencadenar la mensajería de Canvas.
  * `EXTERNAL_ID`: Identifica al usuario para que entre en el Canvas. Debe coincidir con el valor `external_id` utilizado en Braze.
...
##### Paso 1.2: Configurar credenciales {#step-12-set-up-credentials}
...
##### Paso 1.1: Configura tu tabla de origen en Redshift {#step-11-set-up-your-source-table-in-redshift}
...
##### Paso 1.1: Crea un nuevo proyecto o conjunto de datos para tu tabla de origen (opcional) {#step-11-create-a-new-project-or-dataset-for-your-source-table-optional}

```sql
CREATE SCHEMA BRAZE-CLOUD-PRODUCTION.INGESTION;
```

##### Paso 1.2: Configura tu tabla de origen en BigQuery {#step-12-set-up-your-source-table-in-bigquery}
...
##### Paso 1.3: Configurar credenciales {#step-13-set-up-credentials}
...
| Permiso | Propósito |
| :---- | :---- |
| BigQuery Connection User | Permite que Braze se conecte. |
...
##### Paso 1.1: Crea un catálogo o esquema para tu tabla de origen {#step-11-create-a-catalog-or-schema-for-your-source-table}

```sql
CREATE SCHEMA BRAZE-CLOUD-PRODUCTION.INGESTION;
```

#### Paso 1.2: Configura tu tabla de origen en Databricks {#step-12-set-up-your-source-table-in-databricks}
...
##### Paso 1.1: Configura tu tabla de origen en Fabric {#step-11-set-up-your-source-table-in-fabric}
...
#### Paso 3: Crea tu sincronización sin copia {#step-3-create-your-zero-copy-sync}

Una vez completada la configuración del origen de datos y lanzado el Canvas de destino, crea una nueva sincronización de datos:

1. En Braze, ve a **Configuración de datos** > **Ingesta de datos de Cloud**.
...
Cuando se ejecute la sincronización, los usuarios de tu tabla de origen comenzarán a entrar en el Canvas. Utiliza los análisis de Canvas y la página de registros de sincronización de Ingesta de datos de Cloud para supervisar el rendimiento.
...
* Los resultados de la tabla se almacenan temporalmente en Braze durante el procesamiento. También se exportan a Snowflake durante 30 días para que puedas ver exactamente lo que se sincronizó.
* Los mensajes archivados no se guardan en ningún lugar dentro de Braze. Las copias se envían para almacenarse exclusivamente en tu almacenamiento configurado.
* Cuando utilizas la CDI sin copia con desencadenantes de Canvas, Braze no almacena una copia de seguridad de los resultados de la consulta del almacén de datos y no se copian datos en el perfil de usuario.
* Las propiedades de contexto de Canvas pueden registrarse en sistemas internos durante un máximo de 30 días.