...
description: "Esta página trata sobre la ingesta de datos en el cloud de Braze y cómo sincronizar datos relevantes de Amazon S3 o Google Cloud Storage a Braze."
page_order: 4
page_type: reference
...
7. Añade correo(s) electrónico(s) de contacto para notificaciones de errores. Las sincronizaciones de Google Cloud Storage se basan en eventos, por lo que no se requiere una programación: Braze ingesta los archivos nuevos a medida que se suben. Revisa el resumen y selecciona **Create sync**.
...
{% alert important %}
La ruta de carpeta y la suscripción deben ser únicas entre las sincronizaciones de un espacio de trabajo cuando varias sincronizaciones comparten el mismo contenedor de origen.
{% endalert %}
...
Al igual que con Amazon S3, CDI solo procesa los archivos cargados después de crear la sincronización. Cada nuevo objeto desencadena un mensaje `OBJECT_FINALIZE` en tu tema de Pub/Sub. Para ingerir archivos que ya existen en el contenedor, vuelve a cargarlos.
