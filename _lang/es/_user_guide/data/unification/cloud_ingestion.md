...
{% alert note %}
La frecuencia de sincronización en el panel controla con qué frecuencia Braze ejecuta una sincronización (por ejemplo, opciones como ejecuciones cada hora o más frecuentes dentro de una hora). No establece un intervalo personalizado superior a una hora entre ejecuciones. Para ejecutar una sincronización fuera de la cadencia programada —por ejemplo, bajo demanda después de que se complete la carga de tu almacén de datos— utiliza el endpoint [Desencadenar una sincronización]({{site.baseurl}}/api/endpoints/cdi/post_job_sync) con tu ID de integración.
{% endalert %}
...
- Sincronizar de forma segura los datos de usuario, incluidos atributos, eventos y compras, desde tu almacén de datos a Braze.
...
### Objetos no relacionados con usuarios {#non-user-objects}
...
Cuando sincronizas datos de usuario a través de la Ingesta de datos de Cloud, puedes identificar a los usuarios utilizando uno o más de los siguientes tipos de identificadores. Cada fila en tu tabla de origen debe contener un valor para un solo tipo de identificador a la vez, pero tu tabla puede incluir columnas para uno, dos, tres, cuatro o los cinco tipos de identificadores.
...
Para instrucciones de configuración específicas de cada origen de datos y ejemplos de SQL, consulta [Integraciones de almacén de datos]({{site.baseurl}}/user_guide/data/unification/cloud_ingestion/integrations).

## Uso de puntos de datos {#data-point-usage}

Para los clientes con facturación basada en puntos de datos, la facturación de puntos de datos para la Ingesta de datos de Cloud es equivalente a la facturación de actualizaciones a través del [endpoint `/users/track`]({{site.baseurl}}/api/endpoints/user_data/post_user_track). Consulta [Puntos de datos]({{site.baseurl}}/user_guide/data/infrastructure/data_points) para más información.

{% alert important %}
La Ingesta de datos de Cloud de Braze cuenta para el límite de velocidad disponible, así que si envías datos usando otro método, el límite de velocidad se combina entre la API de Braze y la Ingesta de datos de Cloud.
{% endalert %}
