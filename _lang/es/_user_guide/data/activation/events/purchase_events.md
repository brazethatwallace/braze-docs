...
Puedes registrar compras pasando un [objeto de compra]({{site.baseurl}}/api/objects_filters/purchase_object) a través del [endpoint `/users/track`]({{site.baseurl}}/api/endpoints/user_data/post_user_track), o utilizando una de nuestras bibliotecas del SDK enumeradas en la siguiente sección.

{% alert note %}
Las propiedades de los eventos de compra utilizan los mismos tipos de datos que las [propiedades de eventos personalizados]({{site.baseurl}}/user_guide/data/activation/events/custom_events/custom_event_properties#expected-format).
{% endalert %}
...
## Ver datos de compras {#view-purchase-data}

Después de configurar y comenzar a registrar eventos de compra, puedes ver estos datos de compra en el perfil de un usuario en la [pestaña Resumen]({{site.baseurl}}/user_guide/audience/manage_audience/user_profiles#overview-tab).
...
- **[Análisis](#analytics):** Analiza tus datos de compra para obtener información sobre el comportamiento de los usuarios y la eficacia de tus Campaigns de marketing.

### Segmentación {#purchase-event-segmentation}

Puedes desencadenar cualquier número o tipo de Campaigns de seguimiento basándote en los eventos de compra registrados. Por ejemplo, puedes crear un segmento de usuarios que realizaron una compra en los últimos 30 días, o un segmento de usuarios que han gastado más de cierta cantidad.
...
{% alert tip %}
Para segmentar por el número de veces que se ha producido una compra específica, registra esa compra individualmente como un [atributo personalizado incremental]({{site.baseurl}}/developer_guide/platform_wide/analytics_overview#custom-attribute-storage).
{% endalert %}

### Personalización {#personalization}

Como cualquier otro tipo de datos que recopiles de tus usuarios, puedes usar los datos de compra para personalizar tu mensajería a través de Liquid. Por ejemplo, puedes enviar un correo electrónico personalizado a un usuario recomendando productos similares a los que acaba de comprar.

Supongamos que tienes una propiedad de evento de compra llamada `last_purchased_product` que almacena el nombre del último producto que un usuario compró. Puedes usar esta propiedad para personalizar un mensaje de correo electrónico de la siguiente manera:
...
Para hacerlo, crea una Campaign o un Canvas basado en acciones, y luego establece la acción desencadenante en **Realizar compra**. También puedes especificar condiciones adicionales para el desencadenador, como el producto comprado o el monto de la compra.
...
Además de hacer seguimiento de las métricas de compra para la segmentación, Braze también registra el número de compras de cada producto y los ingresos generados a lo largo del tiempo. Esto puede ser útil para identificar los productos más populares o medir el impacto de una Campaign promocional en las ventas.
...
#### Conversión de divisas {#currency-conversion}

Cuando los eventos de compra se registran en una divisa distinta al USD, Braze convierte el monto a USD utilizando las tasas de cambio de [Open Exchange Rates](http://openexchangerates.org). Estas tasas se actualizan una vez cada 24 horas (aproximadamente a las 4 am ET). Debido a que las tasas de cambio se almacenan en caché, puede haber pequeñas diferencias respecto a la tasa de mercado en tiempo real, particularmente para divisas que experimentan fluctuaciones rápidas.

#### Cálculo de ingresos del ciclo de vida {#lifetime-revenue-calculation}

Braze utiliza los eventos de compra para calcular los ingresos del ciclo de vida (también llamados valor de duración del ciclo de vida o LTV) de un usuario, que es una predicción de la ganancia neta atribuida a toda la relación futura con un cliente. Esto puede ayudarte a tomar decisiones informadas sobre las estrategias de adquisición y retención de clientes.

$$\text{Valor promedio de compra} = \frac{\text{Gasto total en dólares}}{\text{Número total de eventos de compra}}$$

Hay dos lugares principales en Braze donde puedes consultar para comprender el LTV de tus usuarios:

- Para métricas generales como *Ingresos del ciclo de vida* y el *Valor del ciclo de vida por usuario* para cada aplicación y sitio, consulta tu [Informe de ingresos]({{site.baseurl}}/user_guide/analytics/reports/revenue_report#exporting-revenue-data).
- Para comprender los ingresos del ciclo de vida de un usuario específico, consulta su [perfil de usuario]({{site.baseurl}}/user_guide/audience/manage_audience/user_profiles#overview-tab).

##### Impacto de los reembolsos en los ingresos del ciclo de vida {#impact-of-refunds-on-lifetime-revenue}

Cuando uses eventos de compra para hacer seguimiento de los datos de compra, debes registrar los reembolsos registrando un evento de compra de Braze con una propiedad `price` negativa. Este enfoque mantiene un total preciso de los ingresos del ciclo de vida.
...
Aunque Sam tendría dos eventos de compra en su perfil, en realidad solo realizó una compra. Esto es importante a considerar si tienes segmentos o casos de uso construidos en torno al número de compras que un usuario ha realizado. Los reembolsos constantes inflarán el recuento de compras en el perfil del usuario.
...
Es posible que en ocasiones identifiques eventos de compra que registren demasiados puntos de datos, que ya no sean útiles para tu estrategia de marketing o que se hayan registrado por error. Para evitar que estos datos se envíen a Braze, puedes incluir en la lista de bloqueo el objeto de datos personalizado mientras tu equipo de ingeniería trabaja en eliminarlo del backend de tu aplicación o sitio web.
