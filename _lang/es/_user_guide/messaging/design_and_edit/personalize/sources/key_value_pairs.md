...
Braze te permite enviar pares clave-valor de cadena definidos de forma personalizada, conocidos como `extras`. Para acceder a tus extras en campañas desencadenadas por API y campañas planificadas desencadenadas por API, en el dashboard establece una clave como "example_key" y un valor como {% raw %}`"$json:{"foo": 1, "bar": 1}"`{% endraw %}. Esto resultará en una salida de la consola para desarrolladores de `"extras": { "test": { "foo": 1, "bar": 1 }`
...
{% alert note %}
El sistema operativo iOS puede [limitar las notificaciones]({{site.baseurl}}/developer_guide/platform_integration_guides/swift/push_notifications/silent_push_notifications/#ios-silent-notifications-limitations) para algunas características (Uninstall Tracking, geovallas y Push Stories). Ten en cuenta que si estás experimentando dificultades con estas características, la limitación de notificaciones silenciosas de iOS podría ser la causa.
{% endalert %}
...
![Agregar par clave-valor a una tarjeta de contenido]({% image_buster /assets/img_archive/kvp_content_cards.png %}){: style="max-width:70%;"}