...
Cuando Braze envía un mensaje con un código promocional, el código se descuenta antes de que el mensaje se envíe. Para garantizar que los códigos sean consistentes, únicos y nunca se reutilicen:
...
Si colocas varios fragmentos de código de la misma lista en un mensaje, Braze aplica el mismo código en todos los fragmentos. Para evitar quedarte sin códigos, carga más códigos de los que esperas usar.
...
{% alert important %}
Los códigos promocionales están disponibles en campañas de mensajes dentro de la aplicación, pero no se pueden enviar en mensajes dentro de la aplicación en Canvas.
{% endalert %}
...
Si un usuario en particular es elegible para recibir un código a través de varios canales, recibe el mismo código a través de cada canal. Solo se usa un código promocional, independientemente de los canales recibidos.

### ¿Puedo usar varios fragmentos de código de Liquid para hacer referencia a la misma lista de códigos promocionales en un mensaje? {#can-i-use-multiple-liquid-snippets-to-reference-the-same-promotion-code-list-in-one-message}

Sí. Braze aplica el mismo código promocional en todas las instancias de ese fragmento en el mensaje, lo que garantiza que el usuario solo reciba un código único.
...
Si el mensaje debía contener un código promocional de una lista vacía o expirada, el mensaje se cancela.

Si el mensaje contiene lógica de Liquid que inserta un código promocional de forma condicional, el mensaje solo se cancela si debía contener un código promocional. Si el mensaje no debía contener un código promocional, el mensaje se envía normalmente.
...
- **Retirar la lista completa:** Deja de usar la lista actual en cualquier Campaign, Canvas o plantilla. Luego, carga los códigos correctos en una nueva lista y cambia todos tus mensajes para usar la nueva lista.
- **Agotar los códigos incorrectos:** Crea una Campaign que envíe códigos de la lista incorrecta a un usuario de marcador de posición hasta que se usen todos los códigos incorrectos. Después de eso, vuelve a cargar los códigos correctos en la misma lista, excluyendo los incorrectos.
