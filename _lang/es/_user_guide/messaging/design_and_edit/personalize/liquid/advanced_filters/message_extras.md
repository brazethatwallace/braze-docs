...
La etiqueta de Liquid `message_extras` añade pares clave-valor al evento de envío correspondiente en Currents y Snowflake Data Sharing.

Para enviar datos dinámicos o adicionales a tu evento de envío de Currents o Snowflake Data Sharing, inserta la etiqueta de Liquid adecuada en el cuerpo de tu mensaje.
...
Puedes añadir estas etiquetas según sea necesario para tus pares clave-valor en el cuerpo del mensaje. Sin embargo, la longitud de todas las claves y valores no debe superar los 1000 bytes (1&nbsp;KB). En Currents y Snowflake Data Sharing, verás un nuevo campo de evento llamado `message_extras` para tus eventos de envío. Esto genera una cadena serializada en JSON en un solo campo.

{% alert note %}
Los extras de correo electrónico envían metadatos a los proveedores de servicios de correo electrónico y no se publican en Currents ni en Snowflake. Para añadir metadatos o valores dinámicos a los eventos de envío de Currents o Snowflake, usa la etiqueta de Liquid `message_extras`.
{% endalert %}

## Cómo se envían los datos de extras de mensaje usando Currents {#how-message-extras-data-is-sent-using-currents}

Los **extras de mensaje** son pares clave-valor que se adjuntan en el momento del envío. La configuración depende del canal. Para correo electrónico, se añaden mediante encabezados. Para push en iOS, se incluyen en la carga útil del push. Todos los eventos de envío compatibles exponen el mismo campo `message_extras` en Currents (y Snowflake) una vez que se envía el mensaje.
...
1. En el cuerpo del mensaje del canal, introduce la etiqueta de Liquid `message_extras`. O bien, puedes usar el modal **Add Personalization** y seleccionar **Message Extras** como tipo de personalización.
...
3. Una vez que tu Campaign o Canvas se haya enviado, Braze adjunta los datos dinámicos en el momento del envío al campo `message_extras` en los eventos de envío de Currents o Snowflake Data Sharing.

## Comprobación de la sintaxis {#checking-syntax}

Cualquier otra entrada que no coincida con el estándar de etiquetas descrito anteriormente en esta sección puede no transmitirse correctamente a Currents o Snowflake. Comprueba que tu sintaxis o formato no incluya ninguno de los siguientes problemas:

- Delimitadores inexistentes, vacíos o mal escritos
- Claves duplicadas (Braze envía de forma predeterminada el par clave-valor que encuentra primero)
- Texto adicional antes de que se definan las claves o los valores
- Claves y valores en orden incorrecto
...
- Los pares clave-valor que superen los 1000 bytes (1&nbsp;KB) se truncan.
...
- Las salidas JSON resultantes solo generan valores de cadena.
- Puedes incluir variables de Liquid como clave o valor, pero no puedes anidar etiquetas de Liquid adicionales dentro de `message_extras`.
  - Por ejemplo, puedes usar el siguiente Liquid: {% raw %}`{% assign value = '123' %} {% assign key = 'test' %} {% message_extras :key {{key}} :value {{value}} %}`{% endraw %}

## Preguntas frecuentes {#frequently-asked-questions}

### ¿Cómo puedo asociar el campo message_extras en los eventos de envío a mis eventos de participación como aperturas y clics? {#how-can-i-associate-the-message_extras-field-in-the-send-events-to-my-engagement-events-like-opens-and-clicks}
