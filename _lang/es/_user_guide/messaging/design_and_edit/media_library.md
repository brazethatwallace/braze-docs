...
Usar la biblioteca de medios en lugar de una red de entrega de contenido (CDN) ofrece mejor almacenamiento en caché y rendimiento para los mensajes dentro de la aplicación. Todos los activos de la biblioteca de medios que se encuentran en un mensaje dentro de la aplicación se almacenan previamente en caché para una visualización más rápida y están disponibles para su visualización sin conexión. Además, la biblioteca de medios está integrada con los creadores de Braze, lo que te permite seleccionar o etiquetar imágenes en lugar de copiar y pegar las URL de las imágenes.
...
* Subir archivos de contacto virtual (.vcf)
* Subir archivos de video para usar en mensajes de WhatsApp
* Subir una carpeta con tus imágenes (hasta 50 imágenes)
...
* Arrastrar y soltar imágenes o carpetas para subirlas
* Eliminar imágenes

![Página de la biblioteca de medios que incluye una sección "Subir a la biblioteca" para arrastrar y soltar o subir archivos. También hay una lista del contenido subido en la biblioteca de medios.]({% image_buster /assets/img_archive/media_library_main.png %})
...
Algunos canales crean una copia optimizada de la imagen cuando se configura el mensaje, lo que genera una URL independiente. Esto aplica tanto si la imagen se añadió desde la biblioteca de medios como a través de una URL externa (por ejemplo, desde un contenedor de S3). Reemplazar el activo original de la biblioteca de medios no actualiza lo que ven los consumidores en los mensajes creados con esos canales, incluidos Content Cards, notificaciones push y banners.

Los mensajes dentro de la aplicación tradicionales (modal, deslizamiento hacia arriba y pantalla completa) también siguen este comportamiento. Sin embargo, los mensajes dentro de la aplicación HTML y los mensajes dentro de la aplicación de arrastrar y soltar no lo hacen. Para esos tipos, Braze no almacena en caché la imagen, por lo que cambiar o eliminar la URL de la imagen original rompe la imagen en las campañas en vivo.
...
Si no ves **Generar con Operator** en la página de la **Biblioteca de medios**, confirma que tienes el permiso "Edit Media Library Assets". Si la opción sigue sin aparecer, ponte en contacto con tu equipo de Braze para confirmar que tu espacio de trabajo tiene acceso a la generación de imágenes de BrazeAI. Si la generación falla, revisa la [política de contenido de OpenAI]({{site.baseurl}}/user_guide/brazeai/operator/capabilities#data-privacy-and-security).