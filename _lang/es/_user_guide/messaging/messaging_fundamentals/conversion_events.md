...
Para una Campaign personalizada de festividades dirigida a usuarios activos, un evento de conversión de **Iniciar una sesión** dentro de dos o tres días puede ser apropiado, ya que te permite tener una idea de la participación de los usuarios al recibir tu mensaje. También puedes seleccionar eventos adicionales como **Realiza un pedido**, **Actualiza la aplicación** o cualquiera de tus eventos personalizados como eventos de conversión.
...
El seguimiento de conversiones comienza cuando un usuario recibe la Campaign o entra en el grupo de control de la Campaign. Recibir un mensaje y la asignación a una variante generalmente ocurren al mismo tiempo. Para Campaigns de mensajes dentro de la aplicación, el seguimiento de conversiones comienza cuando Braze registra una impresión.
...
- **Retención de eventos históricos:** el seguimiento de conversiones en los paneles de Campaign y Canvas mide acciones históricas, no perfiles de usuario actuales. Cuando un usuario cumple una regla de conversión dentro de la ventana designada, Braze registra una conversión en los análisis y no la elimina, incluso si el perfil de ese usuario se elimina, fusiona o archiva posteriormente durante la higiene rutinaria de datos o los procesos de cumplimiento del RGPD. Los recuentos de segmentos en vivo pueden ser naturalmente menores que tus registros permanentes de eventos en el panel, ya que los segmentos dinámicos solo filtran perfiles activos que existen actualmente en la base de datos.
- Si un usuario realiza un evento de conversión dentro de los plazos de conversión de dos Campaigns o Canvas separados que recibió, la conversión se registra en ambos.
...
{% alert note %}
Si cancelas mensajes usando la etiqueta Liquid `abort`, Braze cancela los mensajes solo para los usuarios que pasan por las variantes. Los mensajes para los usuarios en el grupo de control no se cancelan, lo que puede llevar a porcentajes de conversión sesgados entre variantes y grupos de control. Como solución alternativa, usa la [segmentación]({{site.baseurl}}/user_guide/audience/segments/creating_a_segment) para dirigirte a tus usuarios en la entrada de la Campaign y Canvas.
{% endalert %}

## Crear una Campaign con seguimiento de conversiones {#creating-a-campaign-with-conversion-tracking}

### Paso 1: Configura tu Campaign {#step-1-set-up-your-campaign}

[Crea una Campaign]({{site.baseurl}}/user_guide/messaging/campaigns/creating_campaign) para el canal de mensajería que desees. Después de configurar los mensajes y la planificación de tu Campaign, puedes agregar hasta cuatro eventos de conversión para el seguimiento.

Usa tantos eventos de conversión como sea necesario. Agregar un segundo o tercer evento de conversión enriquece significativamente tus informes. Por ejemplo, para una Campaign dirigida a usuarios inactivos, agregar un evento de conversión secundario junto con el evento de conversión primaria **Inicia sesión** te ayuda a entender qué tan efectiva es tu Campaign para traer a los usuarios de vuelta a tu aplicación.
...
| **Abre el correo electrónico** | Un usuario se cuenta como convertido cuando abre el correo electrónico (solo para Campaigns de correo electrónico). |
| **Hace clic en el correo electrónico** | Un usuario se cuenta como convertido cuando hace clic en un enlace dentro del correo electrónico (solo para Campaigns de correo electrónico). |
...
## Tasas de conversión de pasos en Canvas frente a variantes {#canvas-step-versus-variant-conversion-rates}

Es común que el recuento total de conversiones de una variante en Canvas sea mayor que la suma de los recuentos de conversiones de sus pasos individuales. Esto sucede porque las conversiones se rastrean de manera diferente a nivel de variante y a nivel de paso:

- Las conversiones de variante se cuentan tan pronto como el usuario entra en la variante.
- Las conversiones de paso se cuentan solo después de que el mensaje del paso se envía al usuario.

Esto significa que cualquier usuario que entre al Canvas y realice el evento de conversión antes de recibir un paso cuenta para el total de la variante, pero no para ningún paso.

Los siguientes escenarios también pueden causar esta discrepancia:

- **El usuario sale del Canvas antes de recibir algún paso.** Si un usuario entra al Canvas pero sale (por ejemplo, debido a un filtro o una discrepancia de audiencia) antes de que se envíe algún mensaje, una conversión que realice aún cuenta a nivel de variante, pero no cuenta a nivel de ningún paso.
- **El paso se dirige a un subconjunto de usuarios.** Si un paso está configurado para enviarse solo a una plataforma específica (como móvil), los usuarios en otras plataformas (como web) aún pueden entrar al Canvas y convertir. Debido a que esos usuarios nunca reciben el mensaje del paso, la conversión no cuenta a nivel de paso, solo a nivel de variante.
