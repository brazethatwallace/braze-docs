...
En este enfoque, la localización se aplica a una sola plantilla en Braze utilizando [Liquid]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/liquid). Después del envío, el panel proporciona análisis agregados de la Campaign. La participación a nivel de usuario se puede medir mediante embudos de Segments personalizados, por ejemplo, combinando los filtros **País** y **Campaign recibida**.

| Ventajas | Consideraciones |
| --- | --- |
| - Enfoque centralizado<br>- Tiempo de creación de correo electrónico reducido, sin necesidad de crear un correo electrónico varias veces | - Creación manual de informes<br>- El informe de la Campaign muestra métricas agregadas en lugar de métricas por país<br>- Necesidad de probar exhaustivamente Liquid para asegurarte de que se rellena como se espera<br>- Dependiendo de cómo obtengas el valor del país o cuántos países tengas configurados, puede ser complicado probar cada país<br>- Más difícil programar envíos para horarios específicos en diferentes zonas horarias<br>- Más difícil de usar si deseas enviar contenido diferente por país. |
| --- | --- | --- |
{: .reset-td-br-1 .reset-td-br-2 aria-label="Una plantilla para todos" }

### Una plantilla por país {#one-template-per-country}

Este enfoque separa las plantillas en diferentes configuraciones regionales de envío. Después del envío, el panel informa los análisis de envío basados en cada país por separado, y cualquier evento de [Currents]({{site.baseurl}}/user_guide/data/distribution/braze_currents#how-to-access-currents) a nivel de usuario posterior también estará vinculado a una Campaign específica.
...
Después de enviar un Canvas, el panel proporciona [análisis de Canvas]({{site.baseurl}}/user_guide/messaging/canvas/testing_canvases/measuring_and_testing_with_canvas_analytics) agregados, mientras que la participación a nivel de usuario se puede medir mediante [embudos de Segments]({{site.baseurl}}/user_guide/audience/segments/measuring_segment_size) personalizados, como la combinación de los filtros [**País**]({{site.baseurl}}/user_guide/audience/segments/segmentation_filters#country) y [**Paso en Canvas recibido**]({{site.baseurl}}/user_guide/audience/segments/segmentation_filters#received-message-from-canvas-step).

| Ventajas | Consideraciones |
| --- | --- |
| - Enfoque centralizado<br>- Tiempo de creación de correo electrónico reducido, sin necesidad de crear un correo electrónico varias veces. | - Creación manual de informes<br>- El informe de Canvas muestra métricas agregadas en lugar de métricas por país<br>- Necesidad de probar exhaustivamente Liquid para asegurarte de que se rellena como se espera<br>- Dependiendo de cómo obtengas el valor del país o cuántos países tengas configurados, puede ser complicado probar cada país<br>- Más difícil programar envíos para horarios específicos en diferentes zonas horarias<br>- Más difícil de usar si deseas enviar contenido diferente por país. |
...
Una vez enviado, el panel proporciona análisis dinámicos por país y dentro de los eventos de [Currents]({{site.baseurl}}/user_guide/data/distribution/braze_currents#how-to-access-currents) a nivel de usuario basados en la ubicación actual del cliente.

| Ventajas | Consideraciones |
| --- | --- |
| - Informes de ingresos por país dentro de Braze (como por Canvas, variante o paso)<br>- Flexibilidad si hay contenido drásticamente diferente por país<br>- Se pueden añadir otros canales como parte del recorrido en el futuro | - Requiere una estructuración estratégica<br>- Se requiere más esfuerzo de creación (como pasos de mensaje separados para cada país)<br>- Canvas puede volverse grande y difícil de leer si tienes recorridos personalizados y complejos para cada país en un solo Canvas. |
...
Braze admite una etiqueta de Liquid {% raw %}`{% translation salutation %}Hello!{% endtranslation %}`{% endraw %} para dirigirte a usuarios en diferentes idiomas con un solo mensaje.

Para un recorrido completo, consulta la [guía sobre el uso de etiquetas de traducción]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/localization/locales_in_messages).

### Enfoques alternativos {#alternative-approaches}

{% tabs local %}
{% tab Liquid personalizado %}
Puedes pegar manualmente tu contenido en el cuerpo de tu mensaje y usar [Liquid]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize) para mostrar [condicionalmente]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/liquid/conditional_logic#conditional-logic) el idioma correcto al destinatario. Para hacerlo:
...
2. Puedes usar la siguiente plantilla de Liquid para ayudarte a construir tu mensaje. Para cada campo con plantilla, debes introducir las variaciones después del segmento entre corchetes de la plantilla. La variación debe corresponder al código de idioma referenciado entre corchetes antes de ella.
    {% raw %}
    ```liquid
    {% if ${language} == 'en' %}
    ...
    ```
    {% endraw %}
3. Prueba tu mensaje antes de enviarlo introduciendo el ID o correo electrónico de un usuario para verificar cómo aparecería un mensaje para una persona según su idioma.

{% alert tip %}
Siempre recomendamos incluir una sentencia {% raw %}`{% else %}`{% endraw %} en tu mensajería. Aunque la mayoría de los usuarios verán mensajes en su idioma específico, el texto será visible para aquellos que:
...
1. Crea manualmente un Content Block en el panel con la etiqueta "Needs Translation".
2. Tu servicio realiza una obtención nocturna de todos los Content Blocks usando el [endpoint `/content_blocks/list`]({{site.baseurl}}/api/endpoints/templates/content_blocks_templates/get_list_email_content_blocks).
3. Tu servicio obtiene detalles de cada Content Block a través del [endpoint `/content_blocks/info`]({{site.baseurl}}/api/endpoints/templates/content_blocks_templates/get_see_email_content_blocks_information) para ver qué bloques están etiquetados para traducción.
4. Tu servicio de traducción traduce el cuerpo de todos los Content Blocks con "Needs Translation".
5. Tu servicio llama al [endpoint `/content_block/update`]({{site.baseurl}}/api/endpoints/templates/content_blocks_templates/post_update_content_block) para actualizar el contenido traducido y actualizar la etiqueta a "Translation Complete".
{% endtab %}

{% tab Catálogos %}
Los [catálogos]({{site.baseurl}}/user_guide/data/activation/catalogs) te permiten acceder a datos de objetos JSON importados a través de API y archivos CSV para enriquecer tus mensajes, de forma similar a los atributos personalizados o las propiedades de eventos personalizados mediante Liquid. Por ejemplo:
...
Se puede hacer referencia a estos elementos del catálogo usando [personalización]({{site.baseurl}}/user_guide/data/activation/catalogs/create), como se muestra en el siguiente ejemplo, o [selecciones]({{site.baseurl}}/user_guide/data/activation/catalogs/selections) que te permiten crear grupos de datos.
...
{% subtab API JSON a través de SheetDB %}
Esta opción proporciona un método alternativo para transformar Google Sheets en objetos JSON consultados a través de contenido conectado. Al convertir una hoja de cálculo en una API JSON a través de SheetDB, puedes elegir entre [múltiples niveles de suscripción](https://sheetdb.io/pricing) según la cadencia de las llamadas a la API.
...
#### Paso 1: Dar formato a la hoja de Google {#step-1-format-the-google-sheet}
...
| pt | Oi | 3 | Oi2 | 7 |
| de | Hallo | 4 | Hallo2 | 8 |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 .reset-td-br-4 .reset-td-br-5 aria-label="Paso 1: Dar formato a la hoja de Google" }

#### Paso 2: Usar la etiqueta de Liquid de idioma en una llamada de contenido conectado {#step-2-use-the-language-liquid-tag-in-a-connected-content-call}
...
#### Paso 3: Crear plantillas para tus mensajes {#step-3-template-your-messages}
...
- El campo {% raw %}`{{${language}}}`{% endraw %} debe estar definido para todos los usuarios; de lo contrario, se debe incluir un bloque condicional de Liquid como controlador alternativo para usuarios sin un idioma.
- El modelado de datos dentro de Google Sheets debe seguir una estructura vertical orientada por idioma, en lugar de tener objetos de mensaje.
...
{% endsubtab %}
{% subtab API JSON a través de Sheetlabs %}

Esta opción convierte una hoja de Google en una API JSON que puedes consultar con contenido conectado. Sheetlabs admite grandes volúmenes de consultas y ofrece niveles gratuitos y de pago.

#### Paso 1: Preparar tu hoja de traducciones en Google Sheets {#step-1-prepare-your-translations-sheet-in-google-sheets}

Construye la hoja de Google de modo que cada fila sea un idioma. Por ejemplo:

| language | greeting | title1 | legal1 |
| ---- | ---- | ---- | ---- |
| en | Welcome! | Your exclusive offer is here | ... |
| fr | Bienvenue! | Votre offre exclusive est arrivée | ... |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 .reset-td-br-4 aria-label="Paso 1: Preparar tu hoja de traducciones en Google Sheets" }

#### Paso 2: Usar Sheetlabs para importar la hoja y crear una API {#step-2-use-sheetlabs-to-import-the-sheet-and-create-an-api}

1. Regístrate en [Sheetlabs](https://sheetlabs.com).
2. Sigue las instrucciones de Sheetlabs para importar datos desde Google Sheets.
3. Selecciona la hoja de cálculo que creaste en el paso 1.
4. Selecciona **Create a matching API**.

#### Paso 3: Añadir tu token de autenticación de Sheetlabs a Braze (opcional) {#step-3-add-your-sheetlabs-authentication-token-to-braze-optional}

Si tu API de Sheetlabs es pública, omite este paso. Si requiere autenticación:

1. Ve a la página **My Account** en Sheetlabs y copia tu token de API.
2. Sigue los pasos en [Autenticación de Braze con Basic Auth]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/connected_content/making_an_api_call#using-basic-authentication) para crear una credencial de autenticación básica en Braze. Usa tu nombre de usuario de Sheetlabs (dirección de correo electrónico) y el token de API que copiaste.
3. Guarda la credencial con un nombre como `sheetlabs_creds`.

#### Paso 4: Llamar a la API de Sheetlabs desde contenido conectado {#step-4-call-the-sheetlabs-api-from-connected-content}

Añade una llamada de contenido conectado a Sheetlabs. Reemplaza `/XXX/yourapi` con la ruta a la API que creaste en el paso 2.
...
#### Paso 5: Crear plantillas para tus mensajes {#step-5-template-your-messages}

Usa Liquid para hacer referencia a los campos devueltos. Por ejemplo:
...
- Define el campo {% raw %}`{{${language}}}`{% endraw %} para cada usuario que desees emparejar. Si un usuario no tiene un idioma configurado, incluye una alternativa de Liquid.
- Las llamadas de contenido conectado se pueden almacenar en caché. Mide la cadencia proyectada de tu API al elegir un plan de Sheetlabs.

Para más información, consulta [Uso de Sheetlabs con Braze](https://app.sheetlabs.com/docs/producers/braze/).
