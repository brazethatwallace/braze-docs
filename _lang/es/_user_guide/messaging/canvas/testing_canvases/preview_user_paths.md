...
Si la prueba coincide con un tipo de paso que no aparece en esta sección, el paso no compatible se omite y el usuario de prueba continúa al siguiente paso compatible.

### Pasos de agente {#agent-steps}

Cuando una ejecución de prueba alcanza un [paso de agente]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/agent_step), Braze hace una pausa y pregunta **¿Quieres ejecutar el agente "{agentName}"?** Elige cómo continuar:

- **Sí:** Opcionalmente, añade contexto en el campo de texto (además del perfil del usuario de prueba y cualquier contexto de Canvas que ya esté en el recorrido) y luego selecciona **Simular respuesta** para invocar al agente. Puedes introducir valores de ejemplo en lenguaje natural, por ejemplo, describiendo el contenido del carrito o el texto del mensaje entrante, para simular el contexto en tiempo de ejecución que el agente recibiría en producción.
- **No:** Braze no invoca al agente. El paso utiliza la **salida alternativa** configurada del agente en la sección **Salida** de la [Consola de agentes]({{site.baseurl}}/user_guide/brazeai/agents/creating_agents#configure-fallback-values).

Cuando seleccionas **Sí** y **Simular respuesta**, el agente se ejecuta para el usuario de vista previa, almacena su salida en la variable de salida del paso de agente y la prueba continúa por el recorrido. Las invocaciones de **Simular respuesta** cuentan para el límite de invocaciones diarias del agente y aparecen en **Consola de agentes** > **Registros**.
...
Si tu paso de agente depende de datos de un [paso de contexto]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/context) anterior, ejecuta **Probar Canvas** para que las variables de contexto se completen a lo largo de la ruta. Los grupos semilla no evalúan los pasos de contexto ni las variables de contexto para los destinatarios semilla.

### Detalles del paso en Canvas {#canvas-step-details}

Para ver más detalles sobre los criterios de entrada, selecciona **Ver más**. Los pasos con segmentación muestran los criterios cumplidos o no cumplidos. Los mensajes también muestran esto para las validaciones de entrega y la elegibilidad de canal. Los pasos de mensaje muestran qué canales se enviaron y cuáles no.

### Liquid

Braze procesa la lógica Liquid durante una ejecución de prueba, incluso si no estás enviando un mensaje de prueba real. Esto significa que la [lógica de cancelación de mensajes]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/liquid/aborting_messages) y otra lógica Liquid se reflejan y podrían afectar el recorrido del usuario en Canvas.

Si tu vista previa envía el último paso de tu recorrido de usuario en lugar de cancelarlo, la vista previa puede estar usando la hora actual como la hora que se evalúa para Liquid, en lugar de la hora real en la que el usuario estaría en el paso según la hora de entrada al Canvas.
...
Para los Canvas con un desencadenante de acción como "cambio en el valor de un atributo personalizado", Braze intenta simular el cambio estableciendo temporalmente el atributo del usuario en el desencadenante como vacío **solo para la ejecución de prueba del Canvas** (esto no afecta al perfil de usuario). Esto está destinado a probar que el atributo cambia desde su valor actual.

## Cuándo los usuarios entran y salen {#when-users-enter-and-exit}

Los usuarios de prueba entran en la vista previa incluso si no son elegibles en la vida real. Si no son elegibles, puedes ver por qué no han cumplido los criterios. Cuando un usuario de prueba entra en la vista previa, asumimos que ha cumplido los criterios del público objetivo y ha realizado los criterios de desencadenamiento de acción. Por ejemplo, para un Canvas que utiliza eventos personalizados en los criterios de entrada, se asume que el usuario de prueba ha realizado el evento personalizado como se espera en los criterios de entrada. Sin embargo, si el mismo evento personalizado se utiliza en otra parte del Canvas (como en los criterios de salida), considera cómo esto podría afectar la ruta de tu usuario.

Los eventos, desencadenadores de API, atributos personalizados y propiedades de entrada de Canvas que se asumen para permitir que un usuario de prueba entre en el Canvas no se actualizan en el perfil de usuario real y no persisten más allá de la ejecución de prueba. Por ejemplo, durante la prueba, cuando un atributo personalizado se utiliza como desencadenador de Canvas, los criterios de desencadenamiento se aplican a la vista previa del usuario **como si** hubiera desencadenado el cambio de atributo personalizado.

### Consideración {#consideration}

Si pruebas una ruta de acción con acciones que corresponden a criterios de salida (incluyendo propiedades del evento), los criterios de salida se desencadenan y la ejecución de prueba finaliza. Si pruebas un paso de mensaje que corresponde a criterios de salida, los criterios de salida se desencadenan y la ejecución de prueba finaliza.

En este punto, no puedes seleccionar un evento o propiedad específicos dentro de una ruta de acción para desencadenar criterios de salida (solo la ruta en su totalidad). Si un usuario pudiera cumplir potencialmente múltiples criterios de salida, el primero que se procesa y que cumple se muestra como resultado.
...
- Para recorridos de experimentos que usan Winning Path, la vista previa no incluye el periodo de espera cuando un usuario de prueba espera en un paso de mensaje. Braze asume que el usuario avanzó por la ruta seleccionada de inmediato.

## Envíos de prueba {#test-sends}

Puedes optar por enviar mensajes de prueba a un grupo de prueba interno o a un usuario individual a medida que se ejecuta la prueba. Esto significa que solo se envían los mensajes que el usuario encuentra a lo largo de la ruta de prueba. Los destinatarios reciben los mensajes con sus atributos de forma predeterminada, pero puedes anularlos con los atributos del usuario de prueba.
...
Al previsualizar las rutas de los usuarios, considera eliminar el contenido conectado que altera los perfiles de usuario o los datos referenciados en otros Canvas o Campaigns.

## Webhooks {#webhooks}

Los webhooks se ejecutan cuando se envían mensajes de prueba, pero no durante la ejecución de prueba. De forma similar al contenido conectado, considera eliminar los webhooks que alteran perfiles de usuario o datos a los que se hace referencia en otros Canvas o Campaigns.

## Variables de contexto y grupos semilla {#context-variables-and-seed-groups}

Para un paso de mensaje con correo electrónico como canal de mensajería, los grupos semilla envían copias semilla de los correos electrónicos cuando un usuario llega a este paso en el Canvas. Estas copias semilla no se envían como parte de los recorridos del Canvas de los propios destinatarios del grupo semilla, por lo que Braze no ejecuta los [pasos de contexto]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/context) ni evalúa las variables de contexto para esos destinatarios. Si el contenido de tu correo electrónico hace referencia a variables de contexto, los destinatarios del grupo semilla reciben una copia semilla sin esos datos completados. Para probar mensajes que dependen de datos de variables de contexto, usa la vista previa de **Test Canvas** con envíos de prueba en lugar de los grupos semilla.

## Ver los mensajes enviados a los usuarios {#view-messages-sent-to-users}

Vista previa de recorridos de usuarios simula un viaje; no sustituye la verificación de los envíos reales en un perfil de usuario. Para revisar los mensajes que Braze envió a un usuario específico, abre su perfil desde **Audiencia** > **Buscar usuarios** y luego usa las pestañas **Historial de mensajes** y **Participación**.

Para obtener información sobre los campos de búsqueda, los detalles de las pestañas y la ventana de 30 días del historial de mensajes, consulta [Perfiles de usuario]({{site.baseurl}}/user_guide/audience/manage_audience/user_profiles).

## Caso de uso {#use-case}

En este escenario, el Canvas está configurado para dirigirse a usuarios que no han tenido una sesión en una aplicación. Este recorrido incluye un paso de mensaje con un correo electrónico de bienvenida, un paso de demora configurado en un día y un paso de rutas de audiencia que se divide en dos rutas: usuarios con al menos una sesión y todos los demás. Dependiendo de la ruta de audiencia en la que caiga un usuario, se envía el paso de mensaje correspondiente.
...
![La sección "Resultados de la prueba" muestra que el usuario de prueba cumplió los criterios de entrada y proporciona un resumen de su recorrido, incluyendo qué pasos se le enviaron.]({% image_buster /assets/img/preview_user_path_results_example.png %})