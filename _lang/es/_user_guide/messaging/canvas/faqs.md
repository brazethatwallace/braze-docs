...
### ¿A dónde van los usuarios cuando llegan a un paso desconectado?
...
### ¿Debería crear un solo Canvas o Canvas separados por ciclo de vida del usuario?
...
### ¿Por qué un Canvas puede mostrar cero envíos aunque se registren impresiones?
...
Para conceptos de pruebas multivariante y A/B en campañas, consulta [Pruebas multivariante y A/B]({{site.baseurl}}/user_guide/messaging/ab_testing/).

## Análisis y conversiones

### ¿Cómo se rastrean las conversiones de usuarios en un Canvas?
...
### ¿Cuál es la diferencia entre los diferentes tipos de tasa de conversión?
...
### ¿Por qué la tasa de conversión de mi paso de Canvas no es igual a la tasa de conversión total de mi variante de Canvas?
...
Para ver los análisis de un componente de Canvas, ve a tu Canvas y desplázate hacia abajo en la página **Detalles de Canvas**. Aquí podrás ver los análisis de cada componente. Consulta [Análisis de Canvas]({{site.baseurl}}/user_guide/messaging/canvas/testing_canvases/measuring_and_testing_with_canvas_analytics/) para más detalles.

### Al observar el número de usuarios únicos, ¿qué es más preciso, los análisis de Canvas o el segmentador?

El segmentador es una estadística más precisa para datos de usuarios únicos en comparación con las estadísticas de Canvas o campañas. Esto se debe a que las estadísticas de Canvas y campañas son números que Braze incrementa cuando algo sucede, lo que significa que hay variables que podrían hacer que este número sea diferente al del segmentador. Por ejemplo, los usuarios pueden convertir más de una vez para un Canvas o campaña.
...
### ¿Qué ocurre con los usuarios anónimos durante su recorrido en Canvas?
...
### ¿Cuál es la diferencia entre "No ha entrado en variante de Canvas" y "No está en el grupo de control de Canvas"?
...
#### No ha entrado en variante de Canvas
...
#### Barra de herramientas de componentes de Canvas
...
#### Avance del componente de mensaje
...
Ten en cuenta que si Intelligent Timing está activado, el mensaje se enviará dentro de las 24 horas posteriores a que el usuario entre en el componente de mensaje a la hora inteligente identificada (incluso si no hay un componente de retraso involucrado).
...
##### Configuración de planificación "en" o "en el siguiente"
...
- **Registros de red (opcional):** Abre las herramientas de desarrollo de tu navegador (pestaña **Red**), reproduce el error y exporta el registro de red como un archivo de registro HTTP Archive (HAR). Esto ayuda al equipo de soporte a identificar qué llamada de API está agotando el tiempo.