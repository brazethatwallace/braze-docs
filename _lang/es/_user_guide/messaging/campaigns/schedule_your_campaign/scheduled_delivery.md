...
## Opción 1: Enviar tan pronto como se lance la Campaign {#option-1-send-as-soon-as-the-campaign-is-launched}
...
![La sección "Entrega" con "Programado" seleccionado y la opción de programación basada en tiempo de enviar tan pronto como se lance la Campaign.]({% image_buster /assets/img_archive/schedule_immediately.png %})

Este tipo de programación está diseñado para Campaigns puntuales que deseas enviar de inmediato, como mensajes sobre un evento actual. Una aplicación deportiva, por ejemplo, podría programar notificaciones push sobre actualizaciones de puntuación usando esta opción. Además, cuando envías mensajes de prueba dirigidos solo a ti o a tu equipo, esta opción te permite entregarlos de inmediato.
...
Para Campaigns que utilizan entrega programada, los criterios de audiencia siempre se evalúan en el momento del envío programado, no cuando se lanza la Campaign. Esto se aplica a cualquier retraso entre la programación y el envío, por ejemplo, límites de velocidad, zona horaria local, sincronización inteligente o un calendario de desencadenamiento.

### Momento de los cambios en Segments {#timing-of-segment-changes}

Si modificas un Segment que se utiliza como audiencia para una Campaign programada, los cambios realizados cerca de la hora de envío programada generalmente se incluyen cuando se evalúa la audiencia. El momento exacto de corte varía, pero los cambios generalmente se incluyen si terminan de procesarse antes de que Braze construya la audiencia para ese envío.

Por ejemplo, si actualizas un Segment a las 3:50 p. m. para una Campaign programada para enviarse a las 4 p. m., Braze utiliza los criterios actualizados del Segment al evaluar la audiencia, suponiendo que los cambios terminen de procesarse antes de que comience la ejecución de la Campaign.

#### Mejores prácticas {#best-practices}

Para dar tiempo a que los cambios en Segments terminen de procesarse antes de que se envíen tus Campaigns programadas:

- **Planifica con anticipación:** Realiza los cambios en Segments con suficiente antelación respecto a la hora de envío programada para que tengan tiempo de terminar de procesarse.
- **Prueba primero:** Cuando sea posible, prueba los cambios en una Campaign más pequeña antes de aplicarlos a Campaigns más grandes y críticas.

Para más información sobre las opciones de entrega programada, consulta [Tipos de entrega y entrada]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/delivery_and_entry_types#time-based-options).
