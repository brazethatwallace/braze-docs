...
## Configuración de una Campaign activada por API {#setting-up-an-api-triggered-campaign}

Configurar una Campaign activada por API requiere algunos pasos. Primero, crea una nueva Campaign multicanal o de un solo canal (con pruebas multivariante).

{% alert note %}
Una Campaign activada por API es diferente de una [Campaign de API]({{site.baseurl}}/developer_guide/rest_api/api_campaigns#api-campaigns).
{% endalert %}

A continuación, configura tu texto y notificaciones de la misma manera que lo harías normalmente para las notificaciones programadas y selecciona **API-Triggered Delivery**. Para obtener más información sobre cómo activar estas Campaigns desde tu servidor, consulta este artículo sobre el [envío de Campaigns activadas por API]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_triggered_campaigns).

![Configura tu texto y notificaciones de la misma manera que lo harías normalmente para las notificaciones programadas y selecciona API-Triggered Delivery.]({% image_buster /assets/img_archive/api_triggered_campaign_delivery.png %})

## Reducir el retraso entre el desencadenamiento por API y el envío {#reducing-delay-between-your-api-trigger-and-send}
...
![La propiedad de desencadenamiento mencionada incluida en el mensaje para completar automáticamente el nombre del usuario, seguida del texto: "liked your photo! Click here to see what they've been up to.".]({% image_buster /assets/img_archive/api_triggered_photo_social_example_1.png %}){: style="max-width:70%;"}
