...
Este filtro admite comparaciones numéricas para segmentar un rango de versiones de la aplicación. Por ejemplo, puedes segmentar usuarios cuya aplicación sea "menor que", "mayor que" e "igual a" la versión de la aplicación "1.2.3", lo cual puede ser útil para promocionar una nueva característica que requiera una actualización de la aplicación.

Este filtro puede reemplazar el filtro heredado "Nombre de versión de la aplicación", que requería listar explícitamente cada versión anterior o usar una expresión regular.

#### Cómo funciona {#how-it-works}

- Cada parte de la versión `major.minor.patch` enviada en la versión de tu aplicación se compara como números enteros.
- Si los números principales son iguales, Braze compara los números secundarios. Si los números secundarios son iguales, Braze compara los números de parche.
- Cuando se usan filtros "menor que" o "menor o igual que", si la versión de la aplicación no existe en el perfil de un usuario, el filtro devuelve `true` y el usuario se trata como si tuviera una versión anterior a la versión probada. Para evitar incluir usuarios sin datos de versión, usa filtros "mayor que" o "igual a" en su lugar.
...
### No ha usado la característica {#have-not-used-feature}

Cuando lanzas una nueva versión de la aplicación e introduces nuevas características, es posible que los usuarios no noten el nuevo contenido. Ejecutar una Campaign de reconocimiento de características es una excelente manera de enseñar a los usuarios sobre nuevas características o características que nunca han usado. Para hacerlo, debes crear un [atributo personalizado]({{site.baseurl}}/user_guide/audience/segments/segmentation_filters) que se asigne a los usuarios que nunca han completado una acción determinada dentro de tu aplicación o usar un [evento personalizado]({{site.baseurl}}/user_guide/audience/segments/segmentation_filters) para rastrear una acción particular. Puedes usar este atributo (o evento) para segmentar a los usuarios a los que deseas enviar la Campaign.

{% alert tip %}
¿Buscas reorientar una porción específica de tu audiencia? Consulta [Reorientación de Campaigns]({{site.baseurl}}/user_guide/messaging/campaigns/ideas_and_strategies/retargeting_campaigns) para aprender cómo reorientar Campaigns aprovechando las acciones previas de tus usuarios.
{% endalert %}