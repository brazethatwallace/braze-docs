...
La característica de desarrolladores multiempresa permite compartir una única cuenta de usuario entre varias empresas. Los usuarios del panel pueden alternar entre los paneles de diferentes empresas desde su menú de perfil de usuario.
...
2. **Revisa las carpetas de correo no deseado y spam:** Busca correos electrónicos de `@alerts.braze.com` en tu carpeta de correo no deseado o spam.
...
5. **Prueba con un navegador diferente:** Algunas extensiones o configuraciones del navegador pueden interferir con el proceso de restablecimiento de contraseña. Prueba usando un navegador diferente o una ventana de incógnito.
...
### Limpiar la caché y las cookies de tu navegador {#clearing-your-browser-cache-and-cookies}

Si tienes problemas con el rendimiento del panel, como que tu panel o la lista de rendimiento de Segment no se carguen, intenta limpiar la caché y las cookies de tu navegador siguiendo los pasos para tu navegador correspondiente.

{% alert important %}
Limpiar las cookies cierra tu sesión, por lo que se perderá el trabajo no guardado.
{% endalert %}

- [Limpiar caché y cookies en Chrome](https://support.google.com/accounts/answer/32050?hl=en&co=GENIE.Platform%3DDesktop)
- [Limpiar cookies en Safari en Mac](https://support.apple.com/en-gb/guide/safari/sfri11471/16.1/mac/13.0)
- [Limpiar cookies y datos de sitios en Firefox](https://support.mozilla.org/en-US/kb/clear-cookies-and-site-data-firefox)
...
Si limpiar la caché y las cookies de tu navegador no resuelve tus problemas, contacta a [Soporte]({{site.baseurl}}/support_contact).
...
2. Verifica si el usuario ha sido añadido a un espacio de trabajo.
3. Si no forma parte de ningún espacio de trabajo, añádelo y asigna los permisos apropiados.
...
6. Si continúas experimentando problemas, considera recrear tu cuenta. Los usuarios que accedieron a Braze Learning durante la fase de prueba gratuita pueden tener dificultades para acceder ahora.
...
5. Si el restablecimiento no resuelve el problema de inmediato, limpia tus cookies y caché.
...
- Si la 2FA se aplica a nivel de empresa: Después del restablecimiento, Braze solicita al usuario que configure su 2FA nuevamente la próxima vez que inicie sesión.
- Si la 2FA no se aplica a nivel de empresa: El usuario inicia sesión en el panel sin necesidad de configurar la 2FA nuevamente. Si desea habilitar la 2FA, puede hacerlo en la configuración de la cuenta.
...
- [Veo un error sobre suspensión de cuenta.](#account-suspension)
...
- Verifica que estás iniciando sesión en la [instancia correcta del panel de Braze]({{site.baseurl}}/user_guide/administer/personal/sdk_endpoints). Consulta con el administrador de tu cuenta o tu director de cuentas de Braze para estar seguro.
- Tu contraseña puede haber caducado, por lo que necesitas [restablecerla](#resetting-your-password).
...
Esto no sucede muy a menudo, pero Braze se toma muy en serio la suspensión y eliminación de cuentas. Si encuentras un error "Account has been banned" al intentar iniciar sesión, tu cuenta del panel está temporalmente suspendida. Esto puede suceder por varias razones.
...
| Preocupaciones de seguridad | Una actividad sospechosa puede haber desencadenado una suspensión automática por razones de seguridad. |
...
- **Actualiza tu navegador local:** [Limpia tus cookies y la caché del navegador](#clearing-your-browser-cache-and-cookies) y luego intenta iniciar sesión en tu panel nuevamente.
...
- **Confirma que estás accediendo al clúster correcto:** Asegúrate de que estás iniciando sesión en el clúster asignado a tu empresa. Por ejemplo, puedes estar asignado a US-03, pero estás iniciando sesión en US-01.
...
- **Verifica tu conexión de red:** Intenta desactivar tu VPN, si es posible, o deshabilita y vuelve a habilitar tu conexión de red.
- **Reinicia tu dispositivo:** Intenta iniciar sesión en tu panel de Braze después de reiniciar tu dispositivo.

Si has resuelto los problemas anteriores y tu panel aún no carga o no funciona como se espera, contacta a [Soporte]({{site.baseurl}}/braze_support).

### El usuario no pertenece a ningún espacio de trabajo {#the-user-belongs-to-no-workspace}

Los administradores pueden resolver esto yendo a **Configuración** > **Administración de usuarios**, verificando los permisos a nivel de espacio de trabajo del usuario y añadiendo los espacios de trabajo necesarios a **Espacios de trabajo**.
...
- Revisa tu carpeta de correo no deseado: Confirma que el correo electrónico de activación de la cuenta no fue filtrado a tu carpeta de correo no deseado o spam.
- Verifica tu dirección de correo electrónico: Pide a tu administrador que verifique la dirección de correo electrónico asociada a tu nueva cuenta de Braze para confirmar que sea correcta.
- Políticas de TI: Confirma con tu equipo de TI que no haya políticas vigentes que puedan impedir la recepción del correo electrónico de activación.
...
- Cambia de red: Si estás en una red corporativa, las políticas del firewall pueden interferir con la configuración de la 2FA. Intenta cambiar a una conexión personal o un punto de acceso móvil.
...
- Elimina perfiles de autenticación obsoletos: Si previamente iniciaste la configuración de la aplicación de autenticación pero no se completó, elimina cualquier perfil obsoleto en tu aplicación y vuelve a escanear el código QR.

Si continúas teniendo problemas después de probar estos pasos:
...
Si los problemas continúan después de estos pasos, contacta a [Soporte]({{site.baseurl}}/braze_support) para obtener asistencia adicional.
