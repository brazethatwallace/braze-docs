...
Ya sea tu primera vez iniciando sesión o la centésima, así es como accedes a tu panel. Si eres el primer usuario de tu empresa, sigue las indicaciones de la sección anterior. De lo contrario, puedes iniciar sesión después de que el administrador de Braze de tu empresa cree tu cuenta.
...
Si tu panel de Braze muestra un error inesperado y la herramienta de consola de tu navegador muestra el error `ReferenceError: structuredClone is not defined`, tu navegador está desactualizado. Si este error sigue ocurriendo, desinstala y reinstala tu navegador.

## Acceder a múltiples paneles de Braze {#accessing-multiple-braze-dashboards}

Braze no te permite registrar la misma dirección de correo electrónico para múltiples usuarios del panel en el mismo clúster (por ejemplo, si tienes dos paneles en US-01). Puedes usar el mismo correo electrónico para crear cuentas en diferentes clústeres (por ejemplo, si tienes un panel en US-01 y otro en US-05). Si necesitas acceder a múltiples paneles de Braze en el mismo clúster, puedes hacer lo siguiente:
...
La característica de desarrolladores multiempresa permite compartir una única cuenta de usuario entre múltiples empresas. Los usuarios del panel pueden alternar entre diferentes paneles de empresa desde su menú de perfil de usuario.
...
- Cambia el **Entity ID** a `braze_dashboard_<companyID>` para cada integración de panel.
- Ponte en contacto con tu administrador de éxito de cliente o director de cuentas para habilitar el feature flipper `saml_sso_custom_entity_id` para cada panel.
...
Para restablecer tu contraseña, selecciona el enlace **Forgot your password?** en la página de inicio de sesión del panel. Se te pedirá que introduzcas tu correo electrónico para recibir un enlace para restablecer tu contraseña.

![Inicio de sesión del panel con el mensaje "Forgot your password?".]({% image_buster /assets/img_archive/enable_reset.png %}){: style="max-width:60%"}

### Borrar la caché y las cookies de tu navegador {#clearing-your-browser-cache-and-cookies}

Si tienes problemas con el rendimiento del panel, como que tu panel o la lista de rendimiento de segmentos no se carguen, intenta borrar la caché y las cookies de tu navegador siguiendo los pasos para tu navegador correspondiente.
...
1. Si tienes múltiples cuentas de Braze, iniciar sesión con la cuenta incorrecta dos veces te envía al panel de Braze. Confirma que estás iniciando sesión en la cuenta correcta.
...
3. Ve a **Configuración de empresa** > **Configuración de seguridad** y verifica que el inicio de sesión único (SSO) esté activado.
...
- Si la 2FA no se aplica a nivel de empresa: el usuario inicia sesión en el panel sin necesidad de configurar la 2FA de nuevo. Si desea habilitar la 2FA, puede hacerlo en Configuración de cuenta.
...
- Verifica que estás iniciando sesión en la [instancia correcta del panel de Braze]({{site.baseurl}}/user_guide/administer/personal/sdk_endpoints). Consulta con tu administrador de cuenta o director de cuentas de Braze para asegurarte.
...
Si has resuelto los problemas anteriores y tu panel aún no carga o no funciona como se espera, ponte en contacto con [Soporte]({{site.baseurl}}/braze_support).
