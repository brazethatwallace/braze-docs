...
Ya sea la primera vez que inicias sesión o la centésima, así es como puedes acceder a tu panel. Si eres el primer usuario de tu empresa, sigue las indicaciones de la sección anterior. De lo contrario, puedes iniciar sesión después de que el administrador de Braze de tu empresa cree tu cuenta.
...
* [SSO de Microsoft Entra]({{site.baseurl}}/user_guide/administer/global/saml_single_sign_on/microsoft_entra_sso)
...
- **Correo electrónico del alias:** `rocky+1@gmail.com`
...
### Crear alias independientes con otros proveedores {#create-separate-aliases-with-other-providers}

Si tu proveedor de correo electrónico no admite alias con `+`, puedes crear alias independientes, como configurar `rocky@braze.com` para que reenvíe a `rocky.lotito@braze.com`. Esto permite que varias direcciones se redirijan al mismo buzón de entrada mientras Braze las reconoce como correos electrónicos diferentes.

### Usar desarrolladores multiempresa {#use-multi-company-developers}

La característica de desarrolladores multiempresa permite compartir una única cuenta de usuario entre varias empresas. Los usuarios del panel pueden alternar entre los paneles de distintas empresas desde el menú de su perfil de usuario.

Si tienes inicio de sesión único y quieres configurar desarrolladores multiempresa, necesitas habilitar un ID de entidad SAML personalizado configurando una integración SAML SSO personalizada. Sigue los pasos en [Inicio de sesión iniciado por el proveedor de servicios (SP)]({{site.baseurl}}/user_guide/administer/global/saml_single_sign_on/saml_sso_setup), pero aplica estos cambios:
- Cambia **Entity ID** a `braze_dashboard_<companyID>` para cada integración de panel.
- Contacta a tu administrador de éxito de cliente o director de cuentas para habilitar el interruptor de característica `saml_sso_custom_entity_id` en cada panel.
...
- **Contraseña de un solo uso basada en el tiempo (TOTP):** La configuración de TOTP no se sincroniza entre cuentas. Si usas una aplicación de autenticación, debes configurar un código independiente para cada panel en el que inicies sesión directamente.
...
Si usas inicio de sesión único (SSO), ten en cuenta que tener varias direcciones de correo electrónico diferentes podría generar complicaciones. Confirma que tu configuración de SSO esté correctamente definida para evitar problemas de acceso.

## Solución de problemas {#troubleshooting}

### Restablecer tu contraseña {#resetting-your-password}

Para restablecer tu contraseña, selecciona el enlace **¿Olvidaste tu contraseña?** en la página de inicio de sesión del panel. Se te pedirá que introduzcas tu correo electrónico para recibir un enlace y restablecer tu contraseña.


#### Correo electrónico de restablecimiento de contraseña no recibido {#password-reset-email-not-received}
...
2. **Revisa las carpetas de correo no deseado:** Busca correos electrónicos de `@alerts.braze.com` en tu carpeta de correo no deseado.
3. **Verifica los filtros de correo electrónico de TI:** Confirma con tu equipo de TI que los correos electrónicos de `@alerts.braze.com` no están siendo bloqueados o filtrados.
4. **Confirma la instancia correcta del panel:** Asegúrate de que estás solicitando el restablecimiento desde la [instancia correcta del panel de Braze]({{site.baseurl}}/user_guide/administer/personal/sdk_endpoints). Consulta con tu administrador de cuenta o director de cuentas de Braze si no estás seguro.
5. **Prueba un navegador diferente:** Algunas extensiones o configuraciones del navegador pueden interferir con el proceso de restablecimiento de contraseña. Prueba usar un navegador diferente o una ventana de incógnito.

Los enlaces de restablecimiento de contraseña caducan dos horas después de que se envía el correo electrónico. Si tu enlace ha caducado, solicita un nuevo restablecimiento desde la página de inicio de sesión.

Si ninguno de estos pasos funciona, un administrador puede eliminar y recrear tu cuenta de usuario como solución alternativa. Para más información, consulta [Administrar usuarios de la empresa]({{site.baseurl}}/user_guide/administer/global/user_management/manage_company_users).

{% alert note %}
Eliminar y recrear una cuenta de usuario restablece sus permisos y puede afectar la atribución de activos para Campaigns, Canvas y otro contenido que anteriormente pertenecía a ese usuario.
{% endalert %}

### Borrar la caché y las cookies del navegador {#clearing-your-browser-cache-and-cookies}

Si tienes problemas con el rendimiento del panel, como que el panel o la lista de rendimiento de Segment no se carguen, prueba borrar la caché y las cookies de tu navegador siguiendo los pasos correspondientes a tu navegador.

{% alert important %}
Borrar las cookies cierra tu sesión, por lo que se pierde el trabajo no guardado.
{% endalert %}
...
Si Google Chrome muestra un error "Aw, Snap!", Chrome tiene problemas para cargar la página del panel de Braze. Para ver los pasos de solución de problemas, consulta [Obtener ayuda con mensajes de error comunes en Chrome](https://support.google.com/chrome/answer/95669?co=GENIE.Platform%3DDesktop&hl=en).
...
3. Si no forma parte de ningún espacio de trabajo, añádelo y asigna los permisos adecuados.
...
Para la mayoría de los usuarios de la empresa, el editor de arrastrar y soltar debería cargarse. Sin embargo, si estás usando una VPN o estás detrás de un firewall, es posible que debas incluir un dominio en la lista de permitidos. Contacta a tu administrador de TI para verificar que `*.bz-rndr.com` esté en la lista de permitidos.

El editor puede experimentar problemas de carga debido a lo siguiente:

- **Error transitorio:** Son fallos temporales que pueden afectar la conectividad, la comunicación o la transferencia de datos. Afortunadamente, generalmente se resuelven por sí solos sin necesidad de intervención significativa, ya que suelen ser causados por condiciones de corta duración y no indican problemas sistémicos.
- **Error grave:** Puede involucrar un problema subyacente de infraestructura o producto. Puedes consultar nuestra [página de estado del sistema de Braze](https://braze.statuspage.io/), ya que probablemente somos conscientes de la situación y estamos trabajando activamente para resolverla.

{% alert important %}
Si sigues experimentando problemas, [abre un ticket de soporte]({{site.baseurl}}/user_guide/administer/personal/braze_support). Antes de hacerlo, confirma que tu administrador de TI ha verificado que `*.bz-rndr.com` esté en la lista de permitidos de tu lado.
{% endalert %}

### Acceder a Braze Learning {#accessing-braze-learning}

Si estás experimentando problemas para iniciar sesión en Braze Learning y te encuentras en un bucle que te redirige al panel, sigue estos pasos:
...
4. Confirma que tu perfil de usuario del panel incluya nombre y apellido. No tener un apellido puede interrumpir el proceso de inicio de sesión.
...
Si un usuario está experimentando problemas con la autenticación de dos factores (2FA) y no puede acceder al panel de Braze, puede deberse a varias razones. Lo más común es que ya no tenga acceso al número de teléfono registrado o al dispositivo donde está instalada la aplicación Authy.
...
{% alert note %}
Este proceso de restablecimiento también se aplica a usuarios que han sido bloqueados de su cuenta debido a solicitar demasiados tokens en la última hora.
{% endalert %}

### Cuenta bloqueada {#locked-out-of-account}

Si tu cuenta de Braze está bloqueada, puedes recuperar el acceso siguiendo estos pasos.
...
La seguridad de tu cuenta es importante para nosotros, por lo que las contraseñas son necesarias para iniciar sesión en tu cuenta de Braze.
- Comprueba que estás iniciando sesión en la [instancia correcta del panel de Braze]({{site.baseurl}}/user_guide/administer/personal/sdk_endpoints). Consulta con tu administrador de cuenta o director de cuentas de Braze para asegurarte.
- Es posible que tu contraseña haya caducado, por lo que necesitas [restablecerla](#resetting-your-password).
- Si utilizas un servicio de [inicio de sesión único]({{site.baseurl}}/user_guide/administer/global/saml_single_sign_on/saml_sso_setup), consulta con tu administrador de cuenta para verificar que la configuración se haya completado correctamente.
- Si tu empresa tiene varias instancias de Braze, puede que estés usando el correo electrónico incorrecto para iniciar sesión.
...
#### Suspensión de la cuenta {#account-suspension}

Esto no ocurre con mucha frecuencia, pero Braze se toma muy en serio la suspensión y eliminación de cuentas. Si encuentras un error de "La cuenta ha sido suspendida" al intentar iniciar sesión, tu cuenta del panel está temporalmente suspendida. Esto puede ocurrir por varias razones.
...
| Problemas de seguridad | Actividad sospechosa puede haber desencadenado una suspensión automática por razones de seguridad. |
{: .reset-td-br-1 .reset-td-br-2 aria-label="Razón de la suspensión de la cuenta" }

Para resolver este problema, contacta al administrador de Braze de tu empresa, a tu director de cuentas de Braze o a [Soporte]({{site.baseurl}}/support_contact).

### El panel de Braze no carga o no funciona como se esperaba {#braze-dashboard-wont-load-or-work-as-expected}
...
- **Usa complementos o herramientas de terceros compatibles:** Los bloqueadores de anuncios o el software de seguridad pueden impedir que el panel de Braze se cargue. Prueba deshabilitando un bloqueador de anuncios y luego iniciando sesión en tu panel de Braze.
        - También puedes consultar los registros de la consola de tu navegador. Los errores relacionados con `ERR_BLOCKED_BY_CLIENT` pueden indicar que el contenido está siendo bloqueado por un bloqueador de anuncios.
...
- **Actualiza tu navegador:** Actualiza tu navegador a la última versión del [navegador compatible](#supported-browsers) y luego intenta iniciar sesión en tu panel.
...
Si has resuelto los problemas anteriores y tu panel aún no carga o no funciona como se esperaba, contacta con [Soporte]({{site.baseurl}}/user_guide/administer/personal/braze_support).

### El usuario no pertenece a ningún espacio de trabajo {#the-user-belongs-to-no-workspace}

Los administradores pueden resolver esto yendo a **Configuración** > **Administración de usuarios**, comprobando los permisos a nivel de espacio de trabajo del usuario y añadiendo los espacios de trabajo necesarios en **Espacios de trabajo**.
...
- Políticas de TI: Confirma con tu equipo de TI que no existen políticas que puedan impedir que se reciba el correo electrónico de activación.
...
Si los problemas continúan después de estos pasos, contacta con [Soporte]({{site.baseurl}}/user_guide/administer/personal/braze_support) para obtener ayuda adicional.
