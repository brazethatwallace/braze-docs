...
> La página **API e identificadores** es tu centro centralizado para administrar todas tus claves de API REST or transferencia de estado representacional en un solo lugar. Aquí puedes acceder al conjunto de claves de API e identificadores de aplicación de cada espacio de trabajo.

Puedes encontrar la página **API e identificadores** en **Configuración**.

## Claves de API

Esta sección proporciona las claves de API REST or transferencia de estado representacional de tu espacio de trabajo, los identificadores únicos que te permiten acceder a los datos de un espacio de trabajo. Se requiere una clave de API REST or transferencia de estado representacional con cada solicitud a la API de Braze. Para más información sobre la creación y el uso de claves de API, consulta nuestro [resumen de claves de API REST or transferencia de estado representacional]({{site.baseurl}}/api/basics).

### Lista de IP permitidas de la API {#api-ip-allowlisting}

Para mayor seguridad, puedes especificar una lista de direcciones IP y subredes permitidas para realizar solicitudes de API REST or transferencia de estado representacional con una clave de API REST or transferencia de estado representacional determinada. Esto se denomina lista de permitidos, o lista blanca. Para permitir direcciones IP o subredes específicas, agrégalas en la sección **Whitelist IPs** al crear una nueva clave de API REST or transferencia de estado representacional:
...
- **Endpoints de API REST or transferencia de estado representacional:** Acciones como enviar mensajes, crear Campaigns o exportar datos.
- **Solicitudes de API del SDK or kit de desarrollo de software:** Eventos de tu experiencia del cliente, como desencadenar mensajes dentro de la aplicación o sincronizar perfiles de usuario. *Esta característica está disponible si has adquirido MAU or usuarios activos al mes or usuarios activos al mes (CY 24–25).*
