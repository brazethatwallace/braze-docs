...
| Requisito | Detalles |
|---|---|
| URL de Assertion Consumer Service (ACS) | `https://<SUBDOMAIN>.braze.com/auth/saml/callback` <br> Para algunos proveedores de identidad, también puede denominarse URL de respuesta, URL de audiencia o URI de audiencia. |
| ID de entidad | `braze_dashboard`|
| Clave de API de RelayState | Para habilitar el inicio de sesión del proveedor de identidad, ve a **Configuración** > **Configuración y pruebas** > **API e identificadores**, selecciona la pestaña **Claves de API** y crea una clave de API con permisos `sso.saml.login`. |
{: .reset-td-br-1 .reset-td-br-2 aria-label="Requisitos" }

## Inicio de sesión iniciado por el proveedor de servicios (SP) dentro de Microsoft Entra SSO {#service-provider-sp-initiated-login-within-microsoft-entra-sso}

### Paso 1: Añadir Braze desde la galería {#step-1-add-braze-from-the-gallery}

1. En tu centro de administración de Microsoft Entra, ve a **Identity** > **Applications** > **Enterprise Applications** y, a continuación, selecciona **New application**.
2. Busca **Braze** en el cuadro de búsqueda, selecciónalo en el panel de resultados y, a continuación, selecciona **Add**.
...
{: start="6"}
6. Formatea las aserciones SAML en el formato específico esperado por Braze. Consulta las siguientes pestañas sobre atributos de usuario y reclamaciones de usuario para comprender cómo deben formatearse estos atributos y valores.
...
{% alert important %}
El campo de correo electrónico debe coincidir con lo que está configurado para tus usuarios en Braze. En la mayoría de los casos, es lo mismo que `user.userprincipalname`; sin embargo, si tienes una configuración diferente, trabaja con tu administrador del sistema para asegurarte de que estos campos coincidan exactamente.
{% endalert %}

{% endtab %}
{% tab Reclamaciones de usuario %}

En la página **Set up Single Sign-On with SAML**, selecciona **Edit** para abrir el diálogo **User Attributes**. A continuación, edita las reclamaciones de usuario según el formato adecuado.
...
Puedes gestionar estas reclamaciones y valores de usuario desde la sección **Manage claim**.
...
### Paso 3: Configurar Microsoft Entra SSO dentro de Braze {#step-3}

Después de haber configurado Braze dentro del centro de administración de Microsoft Entra, Microsoft Entra proporciona una URL de destino (URL de inicio de sesión) y un certificado **x.509**, que introduces en tu cuenta de Braze.
...
{: .reset-td-br-1 .reset-td-br-2 aria-label="Paso 3: Configurar Microsoft Entra SSO dentro de Braze #step-3" }
