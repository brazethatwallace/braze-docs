---
nav_title: Vincular usuarios del SDK or kit de desarrollo de software Web entre dominios
article_title: Vincular usuarios del SDK or kit de desarrollo de software Web entre dominios mediante el ID de dispositivo
page_order: 1
page_type: reference
description: "Pasa el ID de dispositivo del SDK or kit de desarrollo de software Web de Braze desde el sitio de marketing de Kitchenerie a un dominio de tienda independiente para que la actividad anónima comparta un único perfil de usuario."
---

# Vincular usuarios del SDK or kit de desarrollo de software Web entre dominios mediante el ID de dispositivo {#link-cross-domain-web-sdk-users-through-device-id}

> Pasa el ID de dispositivo del SDK or kit de desarrollo de software Web de Braze a través de la URL de destino cuando dos dominios no pueden compartir cookies, de modo que las sesiones anónimas en ambos sitios se asignen al mismo perfil de usuario de Braze.

## Acerca de este ejemplo {#about-this-example}

Kitchenerie, un comercio minorista ficticio de artículos de cocina, aloja un sitio de marketing (`kitchenerie.com`) y una tienda (`kitchenerie.shop`). Cada dominio tiene su propia integración del SDK or kit de desarrollo de software Web de Braze. Las cookies del navegador no se comparten entre dominios, por lo que Braze asigna ID de dispositivo independientes —y perfiles anónimos separados— cuando el mismo usuario pasa del sitio de marketing a la tienda.

Este patrón:

1. Lee el ID de dispositivo en el dominio de origen con `getDeviceId` después de la inicialización del SDK or kit de desarrollo de software
...
Ambos dominios deben usar la misma clave de API del espacio de trabajo de Braze y el mismo punto final de SDK or kit de desarrollo de software para que los eventos se registren en un solo perfil.
...
- Recupera el ID de dispositivo solo después de que el SDK or kit de desarrollo de software Web se haya inicializado en el dominio de origen. Llamar a `getDeviceId` antes de `initialize` no devuelve un valor.
- El SDK or kit de desarrollo de software Web lee `deviceId` una sola vez en `initialize`. No existe un `setDeviceId` posterior a la inicialización que cambie el ID de dispositivo activo. Lee el parámetro de la URL en el dominio de destino antes de llamar a `initialize`.
...
En `kitchenerie.com` (dominio 1), inicializa el SDK or kit de desarrollo de software Web como de costumbre y luego añade el ID de dispositivo actual a los enlaces que apuntan a `kitchenerie.shop` (dominio 2).
...
Si tu versión del SDK or kit de desarrollo de software expone `getDeviceId` de forma síncrona (sin devolución de llamada), llámalo después de la inicialización:

```javascript
const deviceId = braze.getDeviceId();
```

Consulta [Guía del repositorio del SDK or kit de desarrollo de software Web — Obtener ID de dispositivo]({{site.baseurl}}/developer_guide/sdk_repository_guides/web#get-device-id) y [Opciones de inicialización — `deviceId`]({{site.baseurl}}/developer_guide/sdk_repository_guides/web#initialization-options).

### Paso 2: Leer el ID de dispositivo e inicializar el SDK or kit de desarrollo de software Web en el dominio de destino {#step-2-read-the-device-id-and-initialize-the-web-sdk-on-the-destination-domain}
...
- [Guía del repositorio del SDK or kit de desarrollo de software Web]({{site.baseurl}}/developer_guide/sdk_repository_guides/web)
- [Integración multidominio para el SDK or kit de desarrollo de software Web de Braze]({{site.baseurl}}/developer_guide/platforms/web/multi_domain_integration)
- [Establecer ID de usuario a través del SDK or kit de desarrollo de software de Braze]({{site.baseurl}}/developer_guide/analytics/setting_user_ids)
...
- [Almacenamiento del SDK or kit de desarrollo de software Web]({{site.baseurl}}/developer_guide/storage)