...
Los porcentajes se calculan comparando el rango de fechas actual con el rango de fechas anterior, con la excepción de *MAU or usuarios activos al mes or usuarios activos al mes* (MAU or usuarios activos al mes), que utiliza el último día del período anterior en lugar de un rango.
...
- LINE
- MAU or usuarios activos al mes or usuarios activos al mes (MAU or usuarios activos al mes)
- Nuevos usuarios
- Push
  - (Opcional) Desglose por segmento
- Sesiones
...
- Sesiones por hora
- Sesiones por MAU or usuarios activos al mes
...
*Usuarios* es el número total de usuarios creados en ese espacio de trabajo. Esto incluye a todos los usuarios registrados que han utilizado tu aplicación o sitio web en cualquier momento, y aquellos que podrían no estar asociados con una aplicación o sitio web específico. Este número es el porcentaje de cuántos de tus usuarios históricos están representados como *MAU or usuarios activos al mes or usuarios activos al mes* (MAU or usuarios activos al mes), lo cual es útil para ver la retención de usuarios durante un largo período de tiempo.

Una proporción baja de MAU or usuarios activos al mes respecto a usuarios puede indicar que necesitas diversificar tus canales de mensajería o aumentar tus esfuerzos para contactar a los usuarios inactivos. Consulta nuestra guía rápida sobre [captar usuarios inactivos]({{site.baseurl}}/user_guide/messaging/campaigns/ideas_and_strategies/capturing_lapsing_users) para más información. En general, la proporción de MAU or usuarios activos al mes respecto a usuarios históricos inevitablemente disminuirá con el tiempo debido a la cancelación de usuarios, pero las herramientas de Braze pueden ayudarte a minimizar este efecto manteniendo a los usuarios comprometidos durante más tiempo.
...
### MAU or usuarios activos al mes or usuarios activos al mes {#monthly-active-users}

*MAU or usuarios activos al mes or usuarios activos al mes* (MAU or usuarios activos al mes) es el número de usuarios que han registrado una sesión en tu aplicación o sitio en los últimos 30 días. Los MAU or usuarios activos al mes se calculan cada noche con una ventana móvil de 30 días. Los MAU or usuarios activos al mes te proporcionan una buena comprensión de la salud de una aplicación o sitio durante un período prolongado, ya que suavizan las inconsistencias entre días con intensidad de uso variable.

El porcentaje junto al recuento de MAU or usuarios activos al mes muestra el cambio en MAU or usuarios activos al mes para este período en comparación con el período anterior.

$$\text{Cambio en MAU or usuarios activos al mes} = \frac{\text{MAU or usuarios activos al mes del último día del rango} - \text{MAU or usuarios activos al mes del día anterior a la fecha de inicio}}{\text{MAU or usuarios activos al mes del día anterior a la fecha de inicio}}$$

#### Reglas de cálculo de MAU or usuarios activos al mes {#mau-calculation-rules}

Los cálculos de MAU or usuarios activos al mes siguen reglas específicas para garantizar una facturación precisa y consistente:
...
- **Cargas por CSV e importaciones por REST or transferencia de estado representacional API**: los usuarios cargados por CSV o la REST or transferencia de estado representacional API cuentan para el MAU or usuarios activos al mes cuando proporcionas `date_of_last_session` dentro de la ventana móvil de 30 días, o cuando posteriormente registran una sesión. Proporcionar solo `date_of_first_session` no afecta al MAU or usuarios activos al mes.
- **Eliminaciones por API**: eliminar un usuario a través de la API no actualiza el MAU or usuarios activos al mes de inmediato; el recuento se corrige automáticamente en el siguiente ciclo mensual.

{% alert note %}
Los usuarios anónimos también cuentan para tu MAU or usuarios activos al mes. Para dispositivos móviles, los usuarios anónimos dependen del dispositivo. Para usuarios web, los usuarios anónimos dependen de la caché del navegador. <br><br> Los recuentos de MAU or usuarios activos al mes en Braze pueden diferir de herramientas como Amplitude cuando cada producto utiliza una definición diferente de usuario activo. Compara la configuración en Amplitude (y tus [reglas de cálculo de MAU or usuarios activos al mes](#mau-calculation-rules)) antes de investigar una discrepancia como un problema del pipeline de datos.
{% endalert %}

#### Ejemplo de cálculo de MAU or usuarios activos al mes {#mau-calculation-example}

El siguiente ejemplo demuestra cómo funcionan los cálculos de MAU or usuarios activos al mes a través de diferentes acciones de usuario:

| Paso | Acción | Cambio inmediato en MAU or usuarios activos al mes | Total resultante |
|------|--------|----------------------|-----------------|
...
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 .reset-td-br-4 aria-label="Ejemplo de cálculo de MAU or usuarios activos al mes" }

Las instantáneas de MAU or usuarios activos al mes se calculan una vez al día y nunca cambian retroactivamente. En este ejemplo, el recuento de MAU or usuarios activos al mes del día posterior al paso 3 permanece permanentemente en 2, aunque el Usuario 2 posteriormente se convierta en huérfano. Sin embargo, el recuento de MAU or usuarios activos al mes de los días siguientes refleja solo al usuario no huérfano. Dentro de cualquier ventana de 30 días, este flujo consume en última instancia 1 MAU or usuarios activos al mes, ya que solo queda un usuario distinto y no huérfano.

##### Consideraciones sobre el recuento de MAU or usuarios activos al mes {#mau-count-considerations}

Los recuentos de MAU or usuarios activos al mes en Braze dependen de dónde los consultes. El MAU or usuarios activos al mes total se calcula a nivel de usuario, independientemente de las aplicaciones y plataformas, por lo que cada usuario se cuenta solo una vez. Sin embargo, cuando consultas los recuentos de MAU or usuarios activos al mes por aplicación, la suma de MAU or usuarios activos al mes de todas las aplicaciones puede superar tu MAU or usuarios activos al mes total; un usuario que utiliza varias aplicaciones en tu espacio de trabajo se cuenta en la métrica de MAU or usuarios activos al mes individual de cada aplicación.
...
El valor de *Adherencia* es una proporción entre los usuario activo diario y los MAU or usuarios activos al mes de un período determinado. En esencia, la adherencia mide el porcentaje de tus MAU or usuarios activos al mes que regresan diariamente.
...
{% alert important %}
El valor de MAU or usuarios activos al mes se calcula cada noche y no se actualizará hasta el día siguiente.
{% endalert %}
...
### Sesiones diarias por MAU or usuarios activos al mes {#daily-sessions-per-mau}

*Sesiones diarias por MAU or usuarios activos al mes* es la proporción de *Sesiones diarias* respecto a MAU or usuarios activos al mes en un día determinado. Esta estadística te indica cuántas sesiones por día puedes esperar que se registren por MAU or usuarios activos al mes. Cuando se agrega y promedia, esto puede darte una idea de la frecuencia relativa con la que tus usuarios utilizan tu aplicación o sitio. Es decir, si tus *Sesiones diarias por MAU or usuarios activos al mes* fueran en promedio 0,5, podrías esperar que cada MAU or usuarios activos al mes registre una sesión aproximadamente cada 2 días.