...
2. Selecciona un **Rango de fechas** para tu informe, con una ventana de hasta 90 días.
3. Selecciona las campañas o Canvas (o ambos) a analizar. 
   - (opcional) Filtra campañas y Canvas seleccionando una etiqueta.  
4. Selecciona los **Canales** a analizar para tus mensajes.
5. Selecciona una capa de **Desglosar por** para ver diferentes dimensiones de datos, como por variante, paso en Canvas, país o idioma.
...
{% alert note %}
Si estás analizando conversiones para múltiples canales, tu **Método de atribución** se establecerá de forma predeterminada en **Atribución de último punto de intervención**.
{% endalert %}
...
1. Al configurar tu informe, activa **Usar eventos personalizados**.
...
{% alert note %}
Si seleccionas un evento personalizado, no verás el desplegable **Evento de conversión** en la página y tendrás que volver a ejecutar el informe para ver conversiones de diferentes eventos personalizados.
{% endalert %}
...
{% alert note %}
Este gráfico solo muestra datos para un canal a la vez. Usa el desplegable **Canal** en el gráfico para seleccionar un solo canal.
{% endalert %}
...
### ¿Por qué tengo pocas conversiones en mi campaña o Canvas?
