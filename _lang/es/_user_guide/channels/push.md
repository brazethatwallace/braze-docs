...
| Ejemplo | Explicación |
| --- | --- |
| Incorporación inicial | Hasta que los usuarios completen los pasos iniciales para usar tu aplicación (como registrar una cuenta), su valor es extremadamente limitado. Usa notificaciones push para instar a los usuarios a completar estos pasos y así puedan comenzar a usar tu aplicación en su totalidad. |
...
| Ofertas con tiempo limitado | Si tienes una oferta con tiempo limitado, las notificaciones push son una excelente manera de informar a tus usuarios antes de que expire. Estos mensajes generalmente transmiten un alto sentido de urgencia y son ideales para recordar a los usuarios que se han alejado recientemente sobre tu aplicación. Por ejemplo, si tu aplicación es un juego y ofreces un bonus de moneda del juego por una racha de juego diaria, alertar a un usuario de que su racha está en riesgo puede ser una notificación push eficaz después de que haya alcanzado cierto número de días. |
{: .reset-td-br-1 .reset-td-br-2 aria-label="Ejemplos" }

## Regulaciones sobre mensajes push {#push-message-regulations}

Las notificaciones push llegan directamente al dispositivo de tu cliente, por lo que las políticas de aplicaciones y tiendas regulan cómo puedes usarlas.

{% alert important %}
Tus mensajes push deben cumplir con las [directrices de revisión del Apple App Store](https://developer.apple.com/app-store/review/guidelines/) y las [políticas de Google Play](https://support.google.com/googleplay/android-developer/answer/9888379). Esto incluye reglas sobre el uso de push para anuncios, correo no deseado, promociones y temas relacionados.
{% endalert %}
...
| Apple [4.5.4](https://developer.apple.com/app-store/review/guidelines/#apple-sites-and-services) | Las notificaciones push no deben ser necesarias para que la aplicación funcione y no deben contener información personal sensible o confidencial. No uses push para promociones o marketing directo a menos que los clientes hayan dado su adhesión voluntaria explícitamente mediante un lenguaje de consentimiento en la interfaz de tu aplicación y puedan cancelar la suscripción dentro de la aplicación. |
| Apple [4.10](https://developer.apple.com/app-store/review/guidelines/#monetizing-built-in-capabilities) | No puedes monetizar funcionalidades integradas como las notificaciones push, la cámara o el giroscopio, ni servicios de Apple como Apple Music o iCloud. |
...
{: .reset-td-br-1 .reset-td-br-2 aria-label="Regulaciones sobre mensajes push" }

## Preguntas frecuentes {#frequently-asked-questions}

### ¿Cuándo registra Braze un envío exitoso de push? {#when-does-braze-record-a-successful-send-for-push}

Braze normalmente registra un **envío** una vez que el mensaje se despacha desde Braze hacia Apple, Google o tu servicio de push web. Las métricas de **entregados**, aperturas, rebotes y señales de desinstalación se rastrean por separado y pueden llegar más tarde. Usa los análisis a nivel de paso y de Campaign junto con la [solución de problemas de push]({{site.baseurl}}/user_guide/channels/push/troubleshooting) cuando los **envíos** y las métricas posteriores parezcan desalineados.
...
- name: Configuración push
  ...
  description: Integra push y configura los ajustes de plataforma para iOS, Android y Web.
- name: Crear un mensaje push
  link: /docs/user_guide/channels/push/create_a_push_message
  description: Crea y envía Campaigns push y Canvas.
{% endarticle_tiles %}