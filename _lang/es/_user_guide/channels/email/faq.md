...
**Direcciones de correo electrónico únicas:** Braze no exige que las direcciones de correo electrónico sean únicas entre perfiles. Si dependes de una relación uno a uno entre una dirección de correo electrónico y un perfil, monitoriza los duplicados internamente al crear usuarios.

**Deduplicación antes de Liquid:** Para envíos en los que Braze deduplica por dirección de correo electrónico dentro de un mismo despacho (por ejemplo, Campaigns programadas en las que varios miembros del Segment con la misma dirección se procesan juntos), esa deduplicación ocurre antes de que Liquid se ejecute para el perfil elegido para representar esa dirección. Si Liquid aborta para ese perfil (por ejemplo, con [`abort_message()`]({{site.baseurl}}/user_guide/messaging/design_and_edit/personalize/liquid/aborting_messages)), esa dirección no recibe el mensaje en ese despacho, incluidos los perfiles que ya fueron omitidos por la deduplicación. Los envíos desencadenados no aplican esa misma deduplicación de direcciones dentro del despacho; varios perfiles que comparten una dirección pueden seguir siendo elegibles en un mismo lote, por lo que este comportamiento de aborto no se aplica de la misma manera (consulta el siguiente párrafo).
...
- **Se produjo un error durante la creación de la Campaign o el Canvas:** El usuario puede no recibir el mismo envío dos veces, pero puede recibir dos correos electrónicos separados con la misma línea del asunto. Cuando se duplica una Campaign o un Canvas, verifica los detalles de configuración del correo electrónico, como las imágenes o las líneas del asunto. También puedes consultar los registros de cambios para ver si la Campaign o el Canvas se modificó después del lanzamiento: un duplicado puede compartir la misma línea del asunto que el original cuando el usuario lo recibió.
...
### Excepción: Campaigns desencadenadas por API {#exception-api-triggered-campaigns}

Las Campaigns desencadenadas por API deduplicarán o enviarán duplicados dependiendo de dónde se defina la audiencia. Los correos electrónicos duplicados deben dirigirse por separado en la llamada a la API utilizando `user_ids` distintos para recibir múltiples entregas. Estos son tres posibles escenarios para las Campaigns desencadenadas por API:

- **Escenario 1: Correos electrónicos duplicados en el Segment objetivo:** Si el mismo correo electrónico aparece en varios perfiles de usuario que están agrupados en los filtros de audiencia del panel para una Campaign desencadenada por API, solo uno de los perfiles recibe el correo electrónico.
...
Los [píxeles de seguimiento de apertura]({{site.baseurl}}/user_guide/administer/global/workspace_settings/email_preferences#update-the-placement) aprovechan el dominio de seguimiento de clics del remitente para rastrear los eventos de apertura de correo electrónico. El píxel es una etiqueta de imagen añadida al HTML del correo electrónico. Generalmente es el último elemento HTML dentro de la etiqueta body. Cuando un usuario carga su correo electrónico, se realiza una solicitud para cargar la imagen desde el dominio de seguimiento de marca, lo que registra un evento de apertura.
...
Para las Campaigns de correo electrónico y los Canvas, el botón de detener no detiene inmediatamente el envío. Cuando las solicitudes de envío se envían, no se pueden detener para que no se entreguen al usuario, lo que puede ocurrir después de cierto retraso.

Aunque Braze no enviará más solicitudes una vez que la Campaign o el Canvas se detenga, los análisis aún pueden aumentar mientras el ESP termina de procesar las solicitudes que ya están en curso.
...
Puedes no ver aperturas ni clics de correo electrónico si hay una mala configuración en tu dominio de seguimiento. Esto puede deberse a cualquiera de las siguientes razones:
...
**Solución alternativa:** Para lograr este mismo resultado, puedes alojar el contenido de tu correo electrónico en una página de destino externa (como tu sitio web), que luego se puede enlazar desde la Campaign de correo electrónico que estás creando usando la herramienta **Enlace** al editar el cuerpo del correo electrónico.
...
- **Usa un proveedor de vinculación en profundidad.** Servicios como [Branch](https://www.branch.io/) generan vínculos profundos con formato HTTP que son compatibles con los clientes de correo electrónico, incluido Gmail.
...
Después de que se envía un correo electrónico, el buzón de entrada del destinatario decide cuándo se entrega. Los mensajes pueden ser diferidos durante horas o días debido a un buzón lleno, limitación del ESP desde una IP determinada y razones similares.

Cuando los mensajes diferidos se entregan en un día calendario diferente al día de envío, las _Entregas_ pueden superar los _Envíos_ para el mismo rango de fechas. Cuando muchos aplazamientos se acumulan en un día, los _Envíos_ pueden superar las _Entregas_ para ese rango.
...
Revisa el [registro de actividad de mensajes]({{site.baseurl}}/user_guide/administer/global/workspace_settings/logs_and_alerts/message_activity_log), los registros de cambios de la Campaign o el Canvas y la definición del Segment. Si aún no puedes conciliar el envío, contacta con soporte de Braze con los identificadores del usuario, el `dispatch_id` (si está disponible) y las marcas de tiempo.
...
|---|---|
| El usuario no era elegible para la Campaign o el Canvas | Verifica la configuración de **Públicos objetivo** (para Campaigns) o **Público objetivo** (para Canvas) en la [configuración]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/target_users) para confirmar que el usuario cumplió con todos los filtros de audiencia, criterios de Segment y reglas de entrega en el momento del envío. |
...
| Posible causa | Qué verificar |
|---|---|
| El proveedor de buzón (MBP) no estaba accesible | Un problema temporal impidió que el correo electrónico llegara al MBP del destinatario. Esto normalmente se resuelve solo con reintentos. Los proveedores de servicios de correo electrónico reintentan los rebotes blandos durante hasta 72 horas. |
...
| El destinatario tiene filtrado de correo personalizado | El usuario o su administrador de TI puede haber configurado reglas de buzón que filtran, redirigen o eliminan los mensajes entrantes. |
...
Outlook a menudo usa el renderizado de Microsoft Word en lugar del renderizado estándar del navegador, lo que puede causar que las imágenes se rendericen incorrectamente o añadir bordes alrededor de las imágenes.

Si las imágenes se muestran más grandes de su ancho esperado en Outlook, añade el siguiente CSS a la imagen:
...
- Si la dirección es correcta, contacta con el propietario del buzón o el equipo de TI de ese dominio.
...
2. Verifica que la autenticación SPF, DKIM y DMARC pase correctamente.
