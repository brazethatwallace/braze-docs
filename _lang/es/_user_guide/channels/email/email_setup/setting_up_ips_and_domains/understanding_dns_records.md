...
Dado que los proveedores de buzón de entrada receptores validan SPF contra este dominio Return-Path en lugar del dominio visible del encabezado `From:`, la configuración de SPF reside completamente a nivel de subdominio. Dependiendo del ESP subyacente, Braze gestiona esta validación de una de dos formas:
...
DKIM añade una firma digital criptográfica al encabezado del correo electrónico. El servidor receptor utiliza la clave pública del remitente (publicada en DNS) para verificar que el correo electrónico proviene del propietario del dominio y no fue alterado en tránsito.
...
Dado que esto requiere acceso al registro de dominio de tu empresa, tú o tu administrador de red deben añadir este registro a nivel de dominio raíz. Si estás comenzando, una política básica como `p=none` satisface los requisitos mínimos de los buzones de entrada. Para más información sobre DMARC, consulta [DMARC.org](https://dmarc.org/). Para orientación específica de DMARC en Braze, consulta [Autenticación de correo electrónico]({{site.baseurl}}/user_guide/channels/email/email_setup/authentication#dmarc).

## Arquitectura de DNS específica por ESP {#esp-specific-dns-architecture}
...
SparkPost utiliza una configuración híbrida. Usa registros `CNAME` explícitos para apuntar la infraestructura de seguimiento y Return-Path de vuelta a SparkPost, mientras utiliza un registro `TXT` sin procesar para la autenticación DKIM.
...
| --- | --- | --- | --- |
| CNAME | mail.example.com | smtp.sparkpostmail.com | Return-Path / alineación SPF |
| TXT | scph1226._domainkey.mail.example.com | v=DKIM1; k=rsa; p=... | Autenticación DKIM criptográfica |
...
SendGrid se basa en una infraestructura automatizada conocida como autenticación de dominio. En lugar de proporcionar claves `TXT` sin procesar, SendGrid proporciona una serie de registros `CNAME` que apuntan directamente a servidores gestionados por SendGrid.
...
Amazon SES utiliza Easy DKIM con registros `CNAME` junto con enrutamiento explícito `MX` y `TXT` para el seguimiento personalizado de rebotes.
...
- Configuración de SPF y MAIL FROM personalizado: SendGrid y SparkPost gestionan el enrutamiento del dominio de rebote a través de un `CNAME`. Amazon SES requiere un registro `MX` explícito y un registro `TXT` colocados directamente en el subdominio MAIL FROM designado. El registro `MX` asegura que los avisos de rebote regresen a los servidores de Amazon, y el registro `TXT` contiene la cadena SPF autorizada codificada.
...
Al implementar SparkPost o configuraciones DKIM manuales, puedes encontrar claves criptográficas largas (claves DKIM de 2048 bits).

La especificación principal de DNS (RFC 1035) limita cualquier cadena de caracteres individual dentro de un registro `TXT` a un máximo de 255 caracteres. Una clave pública de 2048 bits supera rutinariamente los 400 caracteres, lo que provoca que los registros de dominio rechacen la cadena única o la trunquen, invalidando la firma.
...
{% alert note %}
Cuando utilizas proveedores de DNS como Cloudflare o AWS Route 53, estas interfaces gestionan automáticamente la división cuando pegas una cadena larga. Los sistemas heredados (como GoDaddy o Network Solutions) requieren que formatees manualmente la división utilizando la técnica de comillas dobles.
{% endalert %}
...
Un dominio solo puede admitir un conjunto de registros `MX` de enrutamiento primario. Si mapeas tu dominio principal (`example.com`) a la infraestructura ESP de Braze, los registros `MX` personalizados requeridos para rebotes sobrescriben tus registros de correo electrónico corporativo. Esto puede interrumpir las plataformas de mensajería interna corporativa como Google Workspace o Microsoft 365.

#### Sobrecarga de includes SPF y el límite de 10 búsquedas {#spf-include-bloat-and-the-10-lookup-limit}
...
- Si un dominio principal añade los mecanismos ESP de Braze (`include:sparkpostmail.com` o `include:amazonses.com`), esto cuenta significativamente contra ese límite.
- Si se excede el límite, se desencadena un PermError SPF permanente, lo que provoca que todos los correos electrónicos corporativos fallen en la autenticación.

#### Aislamiento de reputación de IP y dominio {#ip-and-domain-reputation-isolation}

Si las campañas de marketing, los recibos transaccionales y los correos electrónicos internos de empleados comparten un espacio de dominio raíz idéntico, un aumento repentino en las quejas de correo no deseado de marketing puede dañar la reputación del dominio principal. Esto pone en riesgo que las comunicaciones corporativas críticas se enruten a carpetas de correo no deseado. Usar un subdominio distinto aísla la reputación de tu alcance de marketing.

## Flujo de trabajo de implementación {#implementation-workflow}

Para asegurar una transición e implementación fluidas, sigue esta secuencia:

1. Proporciona los registros estructurados a tu administrador de TI o de red para que los añada a tu plataforma de alojamiento (Cloudflare, Route 53, etc.).
...
3. Ejecuta una búsqueda de DNS (por ejemplo, `dig CNAME mail.example.com`) o utiliza una herramienta de validación para confirmar que los registros se resuelven correctamente antes de pasar a la fase de calentamiento.

## Documentación de proveedores de DNS {#dns-provider-documentation}

Cada proveedor de DNS tiene una interfaz única. Comparte estas especificaciones con tu administrador de red o consulta la documentación específica de tu proveedor para mapear las entradas correctamente en tu archivo de zona.
...
| Microsoft Azure DNS | [Gestionar registros de DNS con el portal de Azure](https://learn.microsoft.com/en-us/azure/dns/dns-operations-recordsets-portal) |
{: .reset-td-br-1 .reset-td-br-2 aria-label="Documentación de proveedores de DNS" }

Para recursos adicionales sobre proveedores de dominios, consulta [Configurar IPs y dominios]({{site.baseurl}}/user_guide/channels/email/email_setup/setting_up_ips_and_domains#step-3-add-dns-records).