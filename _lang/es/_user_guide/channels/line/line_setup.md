...
| --- | --- |
| Cuenta no verificada | Una cuenta no revisada que cualquier persona (particular o empresa) puede obtener. Esta cuenta se representa con una insignia gris y no aparecerá en los resultados de búsqueda dentro de la aplicación LINE. |
...
| Cuenta premium | Una cuenta que ha pasado la revisión de LINE Yahoo. Esta cuenta se representa con una insignia verde y aparecerá en los resultados de búsqueda dentro de la aplicación LINE. Este tipo de cuenta se concede automáticamente durante la revisión a discreción de LINE. |
...
| Channel access token | Selecciona tu proveedor y luego ve a **Channels** > tu canal > **Messaging API**. Si no hay un token de acceso de canal, selecciona **Issue**. |
...
![Página de configuración de respuestas con alternadores para definir cómo tu cuenta gestionará los chats.]({% image_buster /assets/img/line/response_settings.png %}){: style="max-width:80%;"}
...
Después de que el usuario inicia sesión, se realiza un cambio en tu sitio web o aplicación para que el ID de usuario se envíe de vuelta a Braze y se asocie con el ID de LINE que se pasó como parte de la URL, con código de ejemplo como:
