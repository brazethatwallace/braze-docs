...
3. Agrega las direcciones IP de tu clúster de Braze. Para obtener la lista completa de IP por clúster, consulta [Lista de IP permitidas]({{site.baseurl}}/user_guide/channels/webhooks/create_a_webhook#ip-allowlisting).
...
| --- | --- |
| Canal básico | Un canal de KakaoTalk estándar que cualquier organización puede configurar. Permite mensajería de difusión y chat 1:1 a través de KakaoTalk. |
| [Canal de negocios](https://www.kakaocorp.com/page/service/service/KakaoTalkChannel) | Un canal de KakaoTalk verificado y mejorado para empresas que requiere un proceso de solicitud y verificación. Ofrece características mejoradas, como {::nomarkdown}<ul><li>Señal de verificado</li><li>Aparición como canal recomendado</li><li>Soporte para mensajería empresarial</li></ul>{:/} |
...
Antes de comenzar la solicitud, reúne la siguiente documentación empresarial:
- Certificado de registro de empresa coreano
- ID del representante de la empresa
...
1. Inicia sesión en el [Centro de administración del canal de KakaoTalk](https://center-pf.kakao.com/).
2. Selecciona el canal de KakaoTalk existente que deseas actualizar.
3. En la sección **Management (관리)**, selecciona la opción de **Business Channel Application (비즈니스 채널 신청)**.
...
| **Credential (ID) & Password (비밀번호)** | Ve a la misma ubicación del **Sender number (사업자 등록번호)** y luego ve a **API** > **Brand Message (브랜드 메시지)**. |
...
Ve a tu panel de Infobip y al [Centro de administración de canales de KakaoTalk](https://center-pf.kakao.com/) para reunir la siguiente información.
...
| **Sender name / Sender key** | En el portal de Infobip, ve a **Channels and Numbers** > **Channels** y luego selecciona la pestaña **Senders**. |
| **Sender profile UUID** | En el Centro de administración de canales de KakaoTalk, ve a **Channels** y busca el **Search ID** en la ventana de información del canal. |
| **Channel name** | En el Centro de administración de canales de KakaoTalk, busca el **channel name** en la misma ventana de información del canal. |
...
4. Ingresa el **Name**, selecciona la **Expiration date** y luego selecciona los alcances de API necesarios para KakaoTalk. Estos alcances controlan qué acciones de la API de Infobip puede realizar tu clave.

![Página Create API Key de Infobip mostrando los campos de nombre, fecha de expiración y alcances de API.]({% image_buster /assets/img/kakaotalk/infobip_api_key_scopes.png %})
...
##### UUID del perfil del remitente y nombre del canal {#sender-profile-uuid-and-channel-name}

1. En el [Centro de administración de canales de KakaoTalk](https://center-pf.kakao.com/), selecciona **Channels**.
2. En la ventana **Channel Information**, busca el **Channel name** y el **Search id** (UUID del remitente).
3. Ingresa la **Customer center contact information**. Esto es obligatorio al enviar mensajes publicitarios.

![Ventana de información del canal de KakaoTalk mostrando los campos de información de contacto del centro de atención al cliente.]({% image_buster /assets/img/kakaotalk/kakao_customer_center_contact.png %})

{: start="4"}
4. Para ver un canal diferente, selecciona el ícono del canal en la parte superior del menú.
5. En la lista **My channel**, selecciona el canal que deseas ver y luego repite los pasos anteriores.

## Configurar perfiles de usuario {#set-user-profiles}

Los perfiles de usuario deben tener números de teléfono en formato E.164 para enviarles mensajes a través de KakaoTalk. Los números de teléfono se muestran en el perfil de usuario. KakaoTalk requiere que los números de teléfono estén en formato E.164 (por ejemplo, `+821025749774`). Esto difiere de algunos otros canales de mensajería que pueden aceptar números de teléfono en múltiples formatos.

### Importar números de teléfono {#import-phone-numbers}

Importa números de teléfono [subiendo un CSV o usando la API]({{site.baseurl}}/user_guide/audience/manage_audience/import_users) para crear un usuario. Asegúrate de que los números de teléfono estén en formato E.164 antes de importarlos.