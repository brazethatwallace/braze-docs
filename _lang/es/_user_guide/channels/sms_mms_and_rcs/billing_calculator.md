...
Los mensajes RCS se facturan en función de su contenido y del país en el que se entrega el mensaje. Para estimar los costos con precisión, es fundamental comprender los distintos tipos de mensajes y cómo se facturan.

### Tipos de facturación RCS {#rcs-billing-types}
...
Los mensajes RCS Single son mensajes de más de 160 caracteres O que incluyen elementos enriquecidos como botones o contenido multimedia. Se facturan como un único mensaje, independientemente de la longitud del mensaje.
...
Los mensajes Rich son mensajes de solo texto con o sin botones. Se facturan por segmento del mensaje, y cada segmento está limitado a 160 bytes UTF-8, lo que significa que **la cantidad de caracteres por segmento no es fija**. Un mensaje con solo 160 caracteres en inglés simple equivale a un segmento, pero un mensaje con texto más largo y emojis podría ser varios segmentos.

{% endtab %}
{% tab Rich media messages %}

Los mensajes Rich Media incluyen un archivo multimedia (imagen, video) o una Rich Card y se facturan como un único mensaje.
...
A medida que creas tu mensaje, el creador de mensajes mostrará el tipo de facturación en tiempo real a través de una etiqueta (Basic RCS, Single RCS, Rich o Rich Media), lo que te ayudará a controlar los costos antes de enviar.

Tu [panel de uso de créditos]({{site.baseurl}}/credits_usage_dashboard) refleja estos tipos de facturación y proporciona la cantidad de segmentos utilizados para los mensajes de EE. UU., brindándote una vista transparente del consumo de créditos de tus mensajes.