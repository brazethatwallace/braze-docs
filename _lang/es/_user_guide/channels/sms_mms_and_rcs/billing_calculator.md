...
- **Codificación GSM-7**
    - Los mensajes que superan el límite de 160 caracteres se segmentarán en segmentos de 153 caracteres y se enviarán individualmente, para luego reconstruirse en el dispositivo del destinatario. Por ejemplo, un mensaje de 161 caracteres se enviará como dos mensajes, uno con 153 caracteres y el segundo con 8 caracteres.
- **Codificación UCS-2**
    - Si incluyes caracteres no GSM como emojis, escritura china, coreana o japonesa en mensajes SMS, esos mensajes deben enviarse mediante codificación UCS-2. Los mensajes que superan el límite inicial del segmento de 70 caracteres harán que todo el mensaje se concatene en segmentos de 67 caracteres. Por ejemplo, un mensaje de 71 caracteres se enviará como dos mensajes, uno con 67 caracteres y el segundo con 4 caracteres.
...
### Cosas a tener en cuenta al crear tu texto {#things-to-keep-in-mind-as-you-create-your-copy}

- **Límite de caracteres por segmento**
    - [GSM-7](https://en.wikipedia.org/wiki/GSM_03.38) tiene un límite de 160 caracteres para un solo segmento SMS. Para mensajes con más de 160 caracteres, todos los mensajes se segmentarán con un límite de 153 caracteres.
...
- **Compatible con plantillas Liquid, contenido conectado, emojis y enlaces**
    - Las plantillas Liquid y el contenido conectado pueden poner tu mensaje en riesgo de superar el límite de caracteres para tu tipo de codificación. Puedes utilizar el [filtro truncate words](https://help.shopify.com/en/themes/liquid/filters/string-filters#truncatewords) para limitar el número de palabras que tu Liquid podría agregar al mensaje.
    - Los emojis no tienen un recuento de caracteres estándar en todos los emojis, así que asegúrate de probar que tus mensajes se segmenten y muestren correctamente.
...
- **Pruebas**
    - Siempre prueba tus mensajes SMS antes de lanzarlos, especialmente cuando uses Liquid y contenido conectado, ya que superar los límites del mensaje o del texto puede generar cargos adicionales. Ten en cuenta que los mensajes de prueba cuentan para tus límites de mensajes.<br><br>
- **Mensajes de respuesta automática**
    - Los mensajes de respuesta automática enviados por Braze, como las confirmaciones de doble adhesión voluntaria y las respuestas a palabras clave de HELP, son envíos SMS que cuentan como segmentos facturables. El número de segmentos facturables depende de la longitud del texto y la codificación de caracteres.
...
Los mensajes RCS se facturan en función de su contenido y del país en el que se entrega el mensaje. Para estimar los costos con precisión, es esencial entender los diferentes tipos de mensajes y cómo se facturan.
...
Los mensajes RCS Single son mensajes que superan los 160 caracteres O incluyen cualquier elemento enriquecido como botones o medios. Se facturan como un único mensaje, independientemente de la longitud del mensaje.
...
Los mensajes Rich son mensajes de solo texto con o sin botones. Se facturan por segmento del mensaje, y cada segmento está limitado a 160 bytes UTF-8, lo que significa que **el número de caracteres por segmento no es fijo**. Un mensaje con solo 160 caracteres en inglés simple es un segmento, pero un mensaje con texto más largo y emojis podría abarcar múltiples segmentos.

{% endtab %}
{% tab Rich media messages %}

Los mensajes Rich media incluyen mensajes de **medios** independientes y mensajes de **tarjeta**, y se facturan como un único mensaje. Para los diseños de tarjetas y los límites específicos de cada proveedor, consulta [Tipos de mensajes RCS]({{site.baseurl}}/user_guide/channels/sms_mms_and_rcs/create#rcs-message-types).
...
A medida que creas tu mensaje, el creador de mensajes mostrará el tipo de facturación en tiempo real mediante una etiqueta (Basic RCS, Single RCS, Rich o Rich Media), lo que te ayudará a controlar los costos antes de enviar.

Tu [panel de uso de créditos]({{site.baseurl}}/credits_usage_dashboard) refleja estos tipos de facturación y proporciona el número de segmentos utilizados para mensajes de EE. UU., brindándote una vista transparente del consumo de créditos de tus mensajes.