...
- **Orquestación:** Los clics de bots se filtran de los desencadenantes basados en acciones y las Rutas de Acción de Canvas que hacen referencia a interacciones con enlaces SMS, lo que permite que los desencadenantes reflejen el comportamiento humano.
- **Braze Intelligence:**
...
| --- | --- | --- |
| `is_suspected_bot_click` | Booleano | Indica si el clic es un clic sospechoso de bot. Para clics en enlaces cortos de SMS y RCS, Braze evalúa la detección de bots en cada clic y rellena este campo con `true` o `false`. |
| `suspected_bot_click_reason` | Cadena, Array | Indica el motivo de un clic sospechoso de bot (como `user_agent`). Se rellena cuando la detección de bots se ejecuta para clics en enlaces cortos de SMS y RCS. |
...
Para ayudarte a analizar tus datos, puedes usar la plantilla prediseñada para móviles **SMS click events by bots** en el [Query Builder]({{site.baseurl}}/user_guide/analytics/reports/query_builder/query_templates).
...
El filtrado de clics de bots se ejecuta automáticamente para los clics en enlaces acortados de SMS y RCS. Las tasas de clics del panel excluyen los clics sospechosos de bots, por lo que las tasas reportadas reflejan interacciones humanas en lugar de vistas previas de enlaces automatizadas o tráfico de rastreadores.

### ¿El filtrado de clics de bots evita que los bots hagan clic en los enlaces para cancelar suscripción? {#does-bot-click-filtering-prevent-bots-from-clicking-unsubscribe-links}
