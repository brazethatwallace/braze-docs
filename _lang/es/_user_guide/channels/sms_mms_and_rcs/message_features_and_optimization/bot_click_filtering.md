...
article_title: "Filtrado de clics de bots en servicio de mensajes cortos y RCS"
description: "Este artículo de referencia cubre el filtrado de clics de bots en servicio de mensajes cortos y RCS."
...
- **Filtros de segmentación:** Los filtros de Segment que hacen referencia a interacciones con enlaces servicio de mensajes cortos excluyen los clics de bots para una reorientación más precisa en Campaigns y Canvas.
- **Orquestación:** Los clics de bots se filtran de los desencadenadores basados en acciones y de las Rutas de Acción de Canvas que hacen referencia a interacciones con enlaces servicio de mensajes cortos, lo que permite que los desencadenadores reflejen el comportamiento humano.
- **Braze Intelligence:**
    - **Optimizar con BrazeAI<sup>TM</sup>:** Excluye los clics de bots al optimizar la selección de variantes.
    - **Canal inteligente:** Excluye los clics de bots cuando se selecciona servicio de mensajes cortos o RCS para una selección de canal precisa.
...
## Campos de Currents en eventos de clic de servicio de mensajes cortos {#currents-fields-in-sms-click-events}

Braze incluye los siguientes campos de Currents para eventos de clic de servicio de mensajes cortos:

| Campo | Tipo de datos | Descripción |
| --- | --- | --- |
| `is_suspected_bot_click` | Booleano | Indica si el clic es un clic sospechoso de bot. Para clics en enlaces cortos de servicio de mensajes cortos y RCS, Braze evalúa la detección de bots en cada clic y completa este campo con `true` o `false`. |
| `suspected_bot_click_reason` | Cadena, Array | Indica el motivo de un clic sospechoso de bot (como `user_agent`). Se completa cuando la detección de bots se ejecuta para clics en enlaces cortos de servicio de mensajes cortos y RCS. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="Campos de Currents en eventos de clic de servicio de mensajes cortos" }

## Plantilla del Query Builder {#query-builder-template}

Para obtener ayuda con el análisis de tus datos, puedes usar la plantilla prediseñada para móvil **Eventos de clics de servicio de mensajes cortos por bots** en el [Query Builder]({{site.baseurl}}/user_guide/analytics/reports/query_builder/query_templates).
...
El filtrado de clics de bots se ejecuta automáticamente para los clics en enlaces acortados de servicio de mensajes cortos y RCS. Las tasas de clics del panel excluyen los clics sospechosos de bots, de modo que las tasas reportadas reflejan interacciones humanas en lugar de vistas previas automatizadas de enlaces o tráfico de rastreadores.
