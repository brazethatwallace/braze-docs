...
Te recomendamos importar los números de teléfono en formato [`E.164`](https://en.wikipedia.org/wiki/e.164) para garantizar la precisión en caso de que envíes a varias regiones con diferentes códigos de país o de área&#8212;incluso para números de teléfono con sede en EE. UU.

- **Números de EE. UU.:** Todos los números de EE. UU. deben ser números de teléfono válidos de 10 dígitos con un código de área válido. Si a algún número de 10 dígitos le falta el `+` y el código de país, Braze lo mapea como número de EE. UU. Los números de teléfono de Puerto Rico aún requieren un `+` y el código de país aunque utilicen formato de 10 dígitos con códigos de área de estilo estadounidense.
...
{% alert important %}
Los números de teléfono de los usuarios aparecen en Braze como una cadena de dígitos. Si importas un número que contiene caracteres no numéricos (como `,`, `-` o `(`) distintos del {% raw %}`+`{% endraw %} inicial, los caracteres no numéricos se eliminan al mostrarse en Braze. Por ejemplo, al importar `+1 (724) 123-4567` aparece como `+17241234567`.
{% endalert %}

## Validación de números de teléfono {#phone-number-validation}

Braze utiliza la biblioteca [libphonenumber](https://github.com/google/libphonenumber) de Google para validar números de teléfono. Cuando se introducen nuevos prefijos de números móviles, se añade compatibilidad a medida que se actualiza la biblioteca de origen. Braze no mantiene una lista separada de prefijos válidos.
...
Braze depende de herramientas de terceros para obtener números no válidos. Braze no es responsable de interrupciones ni de información errónea de estos servicios. Por lo tanto, esta herramienta no debe utilizarse como tu único método de cumplimiento para verificar números no válidos.
