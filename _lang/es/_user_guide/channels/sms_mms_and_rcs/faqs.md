...
- Comprobar qué perfil recibió SMS más recientemente (hasta hace 7 días); si existe uno, enviarle a ese usuario.
...
- Calcula cuidadosamente los caracteres de tu SMS teniendo en cuenta Liquid o contenido conectado. El creador de SMS de Braze en tu panel no estima ni tiene en cuenta el uso de ninguna de estas características.
...
Es posible que se te cobren segmentos del mensaje adicionales si incluyes personalización con Liquid en tu mensaje. La creación de plantillas de bloques de contenido no ocurre hasta que el mensaje se está preparando para enviarse. Cuando editas un SMS con un bloque de contenido, Braze no sabe qué contendrá el bloque de contenido, pero proporciona una estimación aproximada. Recomendamos que uses el panel de prueba para previsualizar el mensaje y comprender mejor qué esperar.
...
Antes de enviar mensajes SMS con URLs a usuarios en ciertos países (por ejemplo, Suecia o países nórdicos), debes registrar estas URLs con el operador. Contacta a tu director de cuentas de Braze para obtener ayuda. Este proceso tarda aproximadamente cinco días.
...
Los emojis pueden ser complicados, ya que no existe un conteo de caracteres estándar para todos los emojis. Existe el riesgo de que el emoji supere el límite de caracteres y divida el SMS en varios mensajes, a pesar de mostrarse como un solo mensaje en el creador de Braze. Al probar tus mensajes, puedes verificar mejor si un mensaje se dividirá utilizando nuestra [calculadora de segmentos]({{site.baseurl}}/user_guide/channels/sms_mms_and_rcs/billing_calculator#segment-calculator).

## Grupos de suscripción y adhesión/cancelación voluntaria {#subscription-groups-and-opt-inopt-out}

### ¿Cómo se crea la lógica para adhesiones selectivas a SMS de modo que los usuarios estén en el grupo de suscripción correcto? {#how-do-you-create-logic-for-selective-opt-ins-to-sms-so-users-are-in-the-right-subscription-group}
...
### ¿Un usuario necesita ser parte de un grupo de suscripción de SMS para recibir mensajes de texto de prueba por SMS? {#does-a-user-need-to-be-part-of-an-sms-subscription-group-to-receive-sms-test-messages}

Sí. Los usuarios deben tener un número de teléfono válido, ser parte del grupo de suscripción de SMS utilizado para el envío de prueba y tener al menos un país seleccionado en **Geographic Permissions** para SMS.
