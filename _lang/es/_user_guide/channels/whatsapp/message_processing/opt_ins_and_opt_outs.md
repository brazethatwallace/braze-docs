...
- [Métodos de adhesión voluntaria externos a Braze](#external-to-braze-opt-in-methods)
  ...
  - [Mensaje saliente en el canal de WhatsApp de atención al cliente](#outbound-message-in-customer-support-whatsapp-channel)
  - [Mensaje entrante de WhatsApp](#inbound-whatsapp-message)
- [Métodos de adhesión voluntaria con tecnología de Braze](#braze-powered-opt-in-methods)
...
## Configura las adhesiones voluntarias para tu canal de WhatsApp en Braze {#set-up-opt-ins-for-your-braze-whatsapp-channel}
...
Meta requiere que el texto de adhesión voluntaria:

- Indique claramente que la persona está optando por recibir mensajes de tu empresa
...
{% alert note %}
Braze lanzó una mejora en el endpoint `/users/track` que permite actualizaciones del estado de suscripción. Puedes obtener más información en [Grupos de suscripción]({{site.baseurl}}/user_guide/channels/whatsapp/whatsapp_setup/subscription_groups#update-subscription-status). Sin embargo, si ya has creado protocolos de adhesión voluntaria usando el [endpoint `/v2/subscription/status/set`]({{site.baseurl}}/api/endpoints/subscription_groups/post_update_user_subscription_group_status_v2), puedes continuar haciéndolo allí.
{% endalert %}
...
Si has usado WhatsApp anteriormente, es posible que ya hayas creado una lista de usuarios con adhesiones voluntarias según los requisitos de WhatsApp. En este caso, sube un CSV o usa la API con la [siguiente información]({{site.baseurl}}/user_guide/audience/manage_audience/import_users#constructing-your-csv) en Braze.

#### Mensaje saliente en el canal de atención al cliente de WhatsApp {#outbound-message-in-customer-support-whatsapp-channel}

En tu canal de atención al cliente, haz un seguimiento de los problemas resueltos con un mensaje automático preguntando si desean adherirse a la mensajería de marketing. La funcionalidad aquí depende de la disponibilidad de características en tu herramienta de atención al cliente elegida y de dónde almacenas la información de los usuarios.
...
4. Para cualquiera de esas ideas, probablemente necesitarás completar la ruta con lo siguiente:
...
1. Crea una Campaign con el desencadenador de entrega basada en acciones de un mensaje entrante.
2. Crea una Campaign de webhook. Para ver un ejemplo de webhook, consulta [Grupos de suscripción]({{site.baseurl}}/user_guide/channels/whatsapp/message_processing/opt_ins_and_opt_outs#step-2-update-the-users-profile).

{% alert tip %}
Ten en cuenta que puedes crear una URL o un código QR para unirte a un canal de WhatsApp desde el [administrador de WhatsApp](https://business.facebook.com/wa/manage/phone-numbers/) en **Phone Number** > **Message Links**.<br>![Creador de códigos QR de WhatsApp.]({% image_buster /assets/img/whatsapp/whatsapp115.png %}){: style="max-width:55%;"}
{% endalert %}
...
En Canvas, configura una Campaign que pregunte a los clientes si desean adherirse a recibir mensajes de WhatsApp usando uno de los siguientes métodos:
...
## Configura las opciones de exclusión para tu canal de WhatsApp en Braze {#set-up-opt-outs-for-your-braze-whatsapp-channel}

### Interruptor de "Ofertas y anuncios" de WhatsApp {#whatsapp-offers-and-announcements-toggle}

WhatsApp ofrece un interruptor de "Ofertas y anuncios" en la configuración de la aplicación que permite a los usuarios excluirse de los mensajes de marketing. Este interruptor funciona de forma independiente de los grupos de suscripción de Braze:
...
- **El interruptor nativo de WhatsApp** está controlado por Meta y se aplica a nivel de plataforma, fuera de Braze.

Estas dos capas no se sincronizan automáticamente por diseño. Cuando un usuario desactiva el interruptor de "Ofertas y anuncios" en WhatsApp, Meta bloquea la entrega de mensajes de marketing a nivel de plataforma, incluso si el estado de suscripción del usuario en Braze aparece como "Subscribed". La preferencia del usuario se respeta en el momento de la entrega.

{% alert note %}
Dado que Braze no recibe una señal de exclusión hasta que se intenta un envío y Meta devuelve un error, los recuentos de suscripción en Braze pueden no reflejar a los usuarios que se han excluido a través del interruptor de WhatsApp hasta que se intente enviar un mensaje. Esto significa que las estimaciones de alcance pueden estar ligeramente sobreestimadas hasta que se produzca ese ciclo de retroalimentación.
{% endalert %}

### Palabras clave generales de exclusión {#general-opt-out-keywords}

Puedes configurar una Campaign o un Canvas que permita a los usuarios que envíen determinadas palabras excluirse de futuros mensajes. Los Canvas pueden ser especialmente beneficiosos, ya que te permiten incluir un mensaje de seguimiento que confirme la exclusión exitosa.

#### Paso 1: Crea un Canvas con un desencadenante de "Mensaje entrante de WhatsApp" {#step-1-create-a-canvas-with-a-trigger-of-inbound-whatsapp-message}
...
Al seleccionar los desencadenantes de palabras clave, incluye palabras como "Stop" o "No Message". Si eliges este método, asegúrate de que tus clientes conozcan tus palabras de exclusión. Por ejemplo, después de recibir la adhesión voluntaria inicial, incluye una respuesta de seguimiento como "Para excluirte de estos mensajes, envía "Stop" en cualquier momento."
...
### Selección de exclusión de marketing {#marketing-opt-out-selection}

Dentro del creador de plantillas de mensajes de WhatsApp, puedes incluir la opción de "exclusión de marketing". Cada vez que incluyas esta opción, asegúrate de que la plantilla se utilice en un Canvas con un paso posterior para un cambio de grupo de suscripción.

1. Crea una plantilla de mensaje con la respuesta rápida de "exclusión de marketing".<br>![Plantilla de mensaje con una opción de pie de página de "Exclusión de marketing"]({% image_buster /assets/img/whatsapp/whatsapp121.png %})<br><br>![Sección para configurar un botón de exclusión de marketing.]({% image_buster /assets/img/whatsapp/whatsapp122.png %})<br><br>
...
## Configura flujos de trabajo de adhesión voluntaria y cancelación {#set-up-opt-in-and-opt-out-workflows}
...
El [paso de actualización de usuario]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/user_update) puede añadir el número de teléfono del usuario al grupo de suscripción de WhatsApp cuando el usuario envía una palabra clave al número de teléfono del grupo de suscripción.

El paso de actualización de usuario evita condiciones de carrera porque el usuario no avanzará al siguiente paso en el Canvas antes de que su número de teléfono se añada al grupo de suscripción. También tiene menos pasos de configuración que los otros métodos, por lo que Braze generalmente recomienda este método.
...
{% alert important %}
Para los mensajes "STOP", invierte el paso del mensaje de confirmación de la cancelación y el paso de actualización de usuario. Si no lo haces, el usuario será dado de baja del grupo de suscripción primero y luego no será elegible para recibir el mensaje de confirmación.
{% endalert %}
...
{: start="2"}
2. En el Canvas, crea un paso **Set Up User Update** y en **Action** selecciona **Advanced JSON Editor**. <br><br>![Paso de actualización de usuario con una acción de "Advanced JSON Editor".]({% image_buster /assets/img/whatsapp/user_update.png %})<br><br>
3. Rellena el **User Update object** con la siguiente carga útil JSON, reemplazando `XXXXXXXXXXX` con tu ID de grupo de suscripción:
...
{: start="4"}
4. Añade un paso de mensaje de WhatsApp posterior. <br><br>![Paso de actualización de usuario en un Canvas.]({% image_buster /assets/img/whatsapp/message_step.png %}){: style="max-width:25%;"}

#### Consideraciones {#considerations}

La actualización puede completarse a velocidades variables porque Braze agrupa en lotes las solicitudes del [paso de actualización de usuario]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/user_update). Para flujos de adhesión voluntaria sensibles al tiempo en los que la confirmación debe enviarse inmediatamente después de la actualización de la suscripción, utiliza el [método de webhook](#webhook-campaign-to-trigger-a-second-whatsapp-campaign) en lugar de un paso de actualización de usuario.

### Campaign de webhook para desencadenar una segunda Campaign de WhatsApp {#webhook-campaign-to-trigger-a-second-whatsapp-campaign}

Una Campaign de webhook puede desencadenar la entrada en una segunda Campaign después de añadir el número de teléfono del usuario al grupo de suscripción de WhatsApp cuando el usuario envía una palabra clave al número de teléfono del grupo de suscripción.

{% alert important %}
No necesitas utilizar este método para los mensajes STOP. El mensaje de confirmación se enviará antes de que el usuario sea eliminado del grupo de suscripción, por lo que puedes utilizar uno de los otros dos pasos.
{% endalert %}
...
{: start="2"}
2. En la Campaign o el Canvas, crea un paso de mensaje de webhook y cambia el **Request Body** a **Raw Text**.

![Paso de mensaje para un webhook.]({% image_buster /assets/img/whatsapp/webhook_step.png %}){: style="max-width:85%;"}

{: start="3"}
3. Introduce la [URL del endpoint]({{site.baseurl}}/api/basics) del cliente en la **Webhook URL**, seguida del enlace del endpoint `campaigns/trigger/send`. Por ejemplo, `https://dashboard-02.braze.eu/campaigns/trigger/send`.
...
{: start="4"}
4. En el texto sin formato, introduce la siguiente carga útil JSON y reemplaza `XXXXXXXXXXX` con tu ID de grupo de suscripción. Necesitarás reemplazar el `campaign_id` después de crear tu segunda Campaign.
...
{: start="5"}
5. Crea una Campaign de WhatsApp (tu segunda Campaign) y configura el desencadenador como API. Asegúrate de copiar este `campaign_id` en la carga útil JSON de tu primera Campaign.

#### Consideraciones

- Las actualizaciones de atributos desde la carga útil JSON del desencadenador de la API de Canvas aún no son compatibles, por lo que solo puedes desencadenar una Campaign de WhatsApp para el mensaje de respuesta de WhatsApp (como en el paso 2).
- Una plantilla de WhatsApp debe estar aprobada para enviarla como mensaje de respuesta. Esto se debe a que una respuesta rápida requiere que el desencadenador del mensaje entrante esté dentro de la misma Campaign o Canvas. Si utilizas un [paso de actualización de usuario](#user-update-step), puedes enviar un mensaje de respuesta rápida sin la aprobación de Meta.

## Comprender la diferencia entre los modificadores "regex" e "is" {#understanding-the-difference-between-regex-and-is-modifiers}

En esta tabla, se utiliza `STOP` como ejemplo de palabra desencadenante para demostrar cómo funcionan los modificadores.
