...
Si la reproducción automática es necesaria y tus pruebas muestran que no funciona de forma predeterminada, puedes personalizar la `WKWebViewConfiguration` utilizada por los mensajes dentro de la aplicación HTML para ajustar el requisito de acción del usuario para la reproducción de medios, por ejemplo, estableciendo la propiedad `mediaTypesRequiringUserActionForPlayback`. Esto requiere personalización a nivel de SDK or kit de desarrollo de software. Para recursos de Swift, consulta [Personalizar mensajes dentro de la aplicación para el SDK or kit de desarrollo de software de Braze]({{site.baseurl}}/developer_guide/in_app_messages/customization?sdktab=swift) y [Agregar la interfaz JavaScript de Braze a WebViews para Swift]({{site.baseurl}}/developer_guide/in_app_messages/html_messages?sdktab=swift).
