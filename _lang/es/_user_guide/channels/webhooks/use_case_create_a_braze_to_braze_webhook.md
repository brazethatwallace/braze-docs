...
## Usa Enviar a destino para desencadenar otro Canvas {#use-send-to-destination-for-triggering-another-canvas}

Para desencadenar un segundo Canvas desde dentro de un Canvas, usa [Enviar a destino]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/send_to_destination) en lugar de un webhook de Braze a Braze. Este componente de Canvas está diseñado específicamente para conectar recorridos de Canvas y proporciona una forma más sencilla y eficiente de enviar usuarios de un Canvas a otro.

Enviar a destino evalúa a los usuarios según los criterios de entrada y audiencia del Canvas de destino cuando alcanzan el paso, sin necesidad de configurar webhooks ni claves de API. Los usuarios que cumplen los criterios entran en el Canvas de destino y pueden continuar su recorrido en el Canvas de origen si hay pasos adicionales a continuación.

{% alert tip %}
Añade [Enviar a destino]({{site.baseurl}}/user_guide/messaging/canvas/canvas_components/send_to_destination) a tu Canvas para enviar usuarios a otro recorrido de Canvas sin configurar webhooks ni llamadas a la API.
{% endalert %}
...
- Llamar a otros [endpoints de mensajería]({{site.baseurl}}/api/endpoints/messaging) para patrones de orquestación donde un flujo de trabajo en Braze necesita invocar una API que no tiene un componente de Canvas dedicado
...
Para crear un webhook de Braze a Braze, necesitas una [clave de API]({{site.baseurl}}/api/api_key) con permisos para el endpoint al que deseas acceder. Por ejemplo, para desencadenar un Canvas activado por API, necesitas una clave de API con el permiso `canvas.trigger.send`.
...
1. [Crea un webhook]({{site.baseurl}}/user_guide/channels/webhooks/create_a_webhook) como Campaign o componente de Canvas.
2. Elige **Plantilla en blanco**.
3. En la pestaña **Redactar**, especifica la **URL del webhook** y el **Cuerpo de la solicitud** para tu caso de uso de API.
4. En la pestaña **Configuración**, especifica tu **Método HTTP** y los **Encabezados de solicitud** según lo requiera el endpoint.
5. Configura cualquier ajuste de entrega adicional (por ejemplo, desencadenar desde un evento personalizado) y completa el resto de tu Campaign o Canvas.
...
- **URL del webhook:** Tu [URL del endpoint REST]({{site.baseurl}}/user_guide/administer/personal/sdk_endpoints) seguida de `/canvas/trigger/send`. Por ejemplo, para la instancia `US-06`, la URL sería `https://rest.iad-06.braze.com/canvas/trigger/send`.
...
Reemplaza `YOUR_API_KEY` con una clave de API de Braze que tenga permisos `canvas.trigger.send`. Puedes crear una clave de API en el panel de Braze yendo a **Configuración** > **Claves de API**.
...
Añade tu solicitud `/canvas/trigger/send` en el campo de texto. Para más detalles, consulta [Enviar mensajes de Canvas mediante entrega activada por API]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_triggered_canvases). El siguiente es un ejemplo del cuerpo de la solicitud para este endpoint, donde `your_canvas_id` es el Canvas ID de tu segundo Canvas:
...
- Las actualizaciones al perfil de usuario generan [puntos de datos]({{site.baseurl}}/user_guide/data/infrastructure/data_points) que cuentan para tu consumo total, mientras que desencadenar otro mensaje a través de los endpoints de mensajería no lo hace.
