...
> Predictive Churn (y cualquier modelo de aprendizaje automático) solo es tan bueno como los datos de que dispone el modelo. También depende en gran medida de disponer de ciertos volúmenes de datos con los que trabajar.

## Posibles errores {#potential-errors}
...
«No hay suficientes usuarios que no abandonan para elaborar una predicción fiable»

![Requisitos de datos de predicción que muestran 31 usuarios que han abandonado en el pasado (cumple el requisito) y 0 usuarios que no han abandonado en el pasado (por debajo del mínimo). Un mensaje de advertencia indica que no hay suficientes usuarios que no abandonan para crear la predicción.]({% image_buster /assets/img/churn/audience_size_error.png %})
