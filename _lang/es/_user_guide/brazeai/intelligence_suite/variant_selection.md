...
page_order: 1.6
description: "Este artículo cubre la selección de variante de BrazeAI<sup>TM</sup>, una característica que permite a tus campañas A/B optimizar automáticamente para obtener la mejor participación."
search_rank: 10
toc_headers: h2
---
...
> La selección de variante de BrazeAI<sup>TM</sup> es una característica que permite que tus pruebas A/B de envío único o recurrentes ejecuten automáticamente un experimento y optimicen para obtener los mejores resultados de participación.
...
Para usar la selección de variante de BrazeAI<sup>TM</sup>, necesitas lo siguiente en tu Campaign o Canvas:
...
Después de que la prueba se completa para envío único, y después de un breve retraso para envío recurrente, tenemos datos confiables para informar. Reportamos cualquier incremento que la selección de variante de BrazeAI<sup>TM</sup> pueda lograr en el panel.
...
### ¿Por qué la reelegibilidad en menos de 24 horas no está disponible cuando se combina con la selección de variante para Campaigns o Canvas recurrentes? {#why-is-re-eligibility-in-less-than-24-hours-not-available-when-combined-with-variant-selection-for-recurring-campaigns-or-canvases}
...
### ¿Por qué mis variantes muestran envíos iguales durante las primeras etapas de mi Campaign recurrente? {#why-are-my-variants-showing-equal-sends-during-the-early-stages-of-my-recurring-campaign}

La selección de variante solo determina las asignaciones finales de variantes después de un periodo de entrenamiento, donde los envíos se distribuyen uniformemente entre las variantes. Se ajusta con el tiempo a medida que nota tendencias de rendimiento. Si no quieres enviar de manera uniforme durante las primeras etapas de tu Campaign, usa variantes fijas para una prueba A/B tradicional.
...
### ¿Por qué no puedo habilitar la selección de variante en mi Canvas o Campaign? {#why-cant-i-enable-variant-selection-in-my-canvas-or-campaign}

Para envío único, no puedes habilitar la selección de variante si tu Canvas o Campaign está compuesto por una sola variante.
...
- Tu Canvas o Campaign está compuesto por una sola variante.