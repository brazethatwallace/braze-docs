---
nav_title: Recomendaciones de artículos
...
Como parte de la característica [de recomendaciones de artículos mediante IA]({{site.baseurl}}/user_guide/brazeai/item_recommendations/creating_recommendations/ai/), las recomendaciones personalizadas mediante IA aprovechan el aprendizaje profundo para predecir lo que más les puede interesar a tus usuarios en función de lo que les ha interesado en el pasado. Este método proporciona un sistema de recomendación dinámico y a medida que se adapta al comportamiento del usuario.
...
{% alert tip %}
Las recomendaciones personalizadas con IA funcionan mejor con cientos o miles de artículos y, normalmente, al menos 30.000 usuarios con datos de compra o interacción. Esto es solo una guía aproximada y puede variar. Los otros tipos de recomendación pueden funcionar con menos datos.
{% endalert %}
...
- Catálogo de artículos relevantes
...
4. (Opcional) Añade una selección para filtrar tu recomendación solo a los artículos relevantes.
...
El modelo de recomendación "Most popular" muestra los artículos con los que los usuarios tienen más interacciones.

#### Casos de uso

Basándose en los datos de interacción que se están rastreando, los casos de uso de este modelo podrían incluir la recomendación de:
...
3. Selecciona tu catálogo.
4. (Opcional) Añade una selección para filtrar tu recomendación solo a los artículos relevantes. Por ejemplo, el servicio de entrega de comida podría tener una selección para filtrar por ubicación del restaurante o tipo de plato.
...
El modelo de recomendación "Most recent" muestra los artículos con los que los usuarios han interactuado más recientemente. Utiliza este modelo para reducir el abandono animando a los usuarios inactivos a volver a interactuar con contenidos relevantes.
...
Recuerda a los usuarios su interés por artículos que han añadido recientemente a su carrito, pero que aún no han comprado. Por ejemplo, un comercio minorista online podría enviar recordatorios u ofrecer descuentos por tiempo limitado en los artículos de su carrito, animando a los usuarios a completar sus compras antes de que caduquen las ofertas.
{% details Requisitos %}
...
### Artículo en tendencia {#trending}

El modelo de recomendaciones "Trending" presenta los artículos que han mostrado un impulso más positivo en las interacciones recientes de los usuarios. Calculamos esto utilizando un análisis ponderado de aproximadamente 10 semanas de historial de eventos, aplicando la mayor ponderación a las últimas 2 semanas aproximadas. Para evitar que las pequeñas fluctuaciones afecten a la calidad de las recomendaciones, aplicamos un umbral de actividad y técnicas de suavizado estadístico.

A diferencia del modelo "Most popular", que presenta artículos con una interacción alta y constante, este modelo presenta artículos que han experimentado un repunte en las interacciones. Puedes utilizarlo para recomendar productos prometedores que actualmente están ganando tracción.
...
Destaca los artículos que han gustado recientemente a tus usuarios con mayor frecuencia. Por ejemplo, una aplicación de música podría presentar artistas prometedores que hayan experimentado un reciente aumento de los "Me gusta" de los usuarios.
...
{% alert note %}
Si utilizas selecciones, el campo de clasificación y cualquier límite no se utilizarán con las recomendaciones de artículos de IA. Esto significa que si creas una selección con un campo de clasificación específico y limitas el número de artículos devueltos, esas restricciones no se utilizarán cuando se procesen las recomendaciones de artículos de IA.
{% endalert %}
...
{% tabs local %}
{% tab Artículos nuevos %}
Este escenario no se basa directamente en las acciones del usuario, sino en los datos del catálogo. Puedes filtrar los artículos nuevos en función de su fecha de incorporación al catálogo y promocionarlos mediante Campaigns dirigidas o Canvas sin necesidad de entrenar un modelo de recomendación.
...
{% details Requisitos %}
- Catálogo de artículos relevantes con un campo para la fecha de adición
{% enddetails %}

{% details Configuración %}
1. Crea una selección basada en tu catálogo. Asegúrate de que tu catálogo tiene un campo de hora (campo con un **Tipo de datos** establecido en **Time**) que corresponde a la fecha en que se añadió el artículo.
...
Una herramienta de [recomendaciones basada en reglas]({{site.baseurl}}/rules_based_recommendations/) utiliza datos de usuario e información sobre productos para sugerir a los usuarios artículos relevantes dentro de los mensajes. Utiliza Liquid y los catálogos de Braze o contenido conectado para personalizar dinámicamente el contenido en función del comportamiento y los atributos del usuario.
...
- **Programas de fidelización:** Destacar los productos que maximizarían los puntos de fidelización o recompensas de un cliente en función de su saldo de puntos actual.
...
### ¿Qué hace que los artículos "Most popular" se mezclen con las recomendaciones de otros modelos? {#what-causes-most-popular-items-to-be-mixed-into-other-models-recommendations}

Cuando nuestra herramienta de recomendaciones selecciona una lista para ti, primero da prioridad a las selecciones personalizadas basadas en el modelo específico que has elegido, como "Most recent" o "AI Personalized". Si este modelo no puede completar la lista de 30 recomendaciones por cualquier motivo, se añaden algunos de tus artículos más populares entre todos los usuarios para asegurar que cada usuario tenga siempre un conjunto completo de recomendaciones.
...
- Los artículos no cumplen los criterios de selección actuales, quizás debido a un cambio en las existencias o en las preferencias del usuario.

Ten en cuenta que las recomendaciones funcionan de forma independiente y no tienen conocimiento de lo que recomiendan los demás modelos. Esto significa que cada sección puede tener artículos duplicados que ya se muestran en otras secciones de recomendaciones de IA en el mismo correo electrónico.

### ¿Cómo puedo evitar artículos duplicados en varias secciones de recomendaciones? {#how-do-i-prevent-duplicate-items-across-multiple-recommendation-sections}

Dado que cada recomendación funciona de forma independiente, el mismo artículo puede aparecer en más de una sección del mismo mensaje. Para eliminar duplicados, utiliza Liquid para hacer seguimiento de los ID de artículos que ya has mostrado y omitirlos en las secciones siguientes.
