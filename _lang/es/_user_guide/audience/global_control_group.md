...
Braze selecciona aleatoriamente múltiples rangos de [números de contenedor aleatorio]({{site.baseurl}}/user_guide/messaging/ab_testing/concepts/random_bucket_numbers#create-segments-using-random-bucket-numbers) e incluye a los usuarios de esos contenedores seleccionados. Si actualmente estás usando números de contenedor aleatorio para otros fines, consulta [Aspectos a tener en cuenta](#things-to-watch-for).
...
![Selecciona el parámetro con el que deseas ejecutar tu informe (sesiones o un evento personalizado en particular) y selecciona Run Report.]({% image_buster /assets/img/control_group/control_group6.png %})
