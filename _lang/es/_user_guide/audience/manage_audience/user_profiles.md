...
## Acceder a perfiles {#access-profiles}
...
Los perfiles de usuario son un gran recurso para la solución de problemas y las pruebas, ya que puedes acceder fácilmente a información sobre el historial de participación de un usuario, su pertenencia a Segments, dispositivo y sistema operativo.

Por ejemplo, si un usuario informa de un problema y no estás seguro de qué dispositivo y sistema operativo está utilizando, puedes usar la [pestaña Resumen](#overview-tab) para encontrar esta información (siempre que tengas su correo electrónico o ID de usuario). También puedes ver el idioma de un usuario, lo que podría ser útil si estás solucionando problemas con una [campaña multilingüe]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/localization) que no se comportó como esperabas.

Puedes usar la [pestaña Participación](#engagement-tab) para verificar si un usuario determinado recibió una Campaign. Además, si ese usuario en particular recibió la Campaign, puedes ver cuándo la recibió. También puedes verificar si un usuario está en un Segment determinado y si un usuario tiene la adhesión voluntaria a push, correo electrónico o ambos. Esta información es útil para la solución de problemas. Por ejemplo, deberías comprobar esta información si un usuario no recibe una Campaign que esperabas que recibiera o recibe una Campaign que no esperabas que recibiera.
...
- **Interacción:** Información sobre la configuración de contacto del usuario, Campaigns recibidas, Segments, estadísticas de comunicación, atribución de instalación y número de contenedor aleatorio.
...
| Atributos personalizados | Qué atributos personalizados están asociados a este usuario y su valor correspondiente, incluidos los atributos personalizados anidados. |
...
La pestaña **Interacción** contiene información sobre las interacciones de un usuario con los mensajes que le enviaste a través de Braze.
...
### Cuándo aparecen las Campaigns en Campaigns recibidas {#when-campaigns-appear-in-campaigns-received}
...
La pestaña **Historial de mensajes** del perfil de usuario muestra eventos recientes relacionados con mensajería (aproximadamente 40) para un usuario individual de los últimos 30 días. Estos eventos incluyen los mensajes que se enviaron al usuario, que recibió, con los que interactuó, y más.

Los datos en esta pestaña no se actualizan después de que se fusiona un usuario. Además, cualquier evento asociado con mensajes enviados a través de la API (por ejemplo, el [endpoint `/messages/send`]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_messages#creating-new-users-with-api-sends)) no aparece en esta pestaña si no se especifica un ID de Campaign en esos envíos.

{% alert important %}
Los eventos de RCS no son compatibles actualmente en la pestaña **Historial de mensajes**.
{% endalert %}
...
| Webhooks | Envío |
| WhatsApp | Cancelación<br>Entrega<br>Fallo<br>Limitación de frecuencia<br>Recepción entrante<br>Lectura<br>Envío |
...
## Restricciones de tamaño de datos {#data-size-constraints}

Braze no impone un límite general sobre el tamaño total de un perfil de usuario. Sin embargo, los tipos de datos específicos asociados a los usuarios tienen capacidades de tamaño definidas.

### Matrices de atributos personalizados {#custom-attribute-arrays}

Las matrices de atributos personalizados (incluidas las matrices de objetos) tienen una capacidad de 100 KB. Cuando envías una matriz que supera esta capacidad, el atributo personalizado no se procesa. La API devuelve una respuesta de éxito (201), pero la matriz no aparece en el perfil de usuario y los datos existentes para ese atributo no se actualizan.

Si tus matrices se acercan a esta capacidad, considera limitar el número de objetos que incluyes para mantener el tamaño total dentro de los 100 KB.

Para obtener más información sobre los atributos personalizados, consulta [Tipos de datos de atributos personalizados]({{site.baseurl}}/user_guide/data/activation/custom_data/data_types#custom-attribute-data-types).
