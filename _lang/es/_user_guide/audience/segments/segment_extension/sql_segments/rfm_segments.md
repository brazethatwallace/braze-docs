...
{% alert note %}
Los eventos de compra deben estar habilitados para usar Segments SQL RFM, ya que el valor monetario de tus usuarios se determina por los ingresos que han generado a través de los eventos de compra de Braze.
{% endalert %}

## Crear un segment RFM {#creating-an-rfm-segment}
...
![Modal con la opción de crear un segment de catálogo para eventos, compras o segments RFM.]({% image_buster /assets/img/segment/select_rfm_segment.png %}){: style="max-width:80%" }
...
{: start="4"}
4. Selecciona los [grupos RFM](#rfm-groups) generados para incluir en tu segment. Si seleccionas múltiples grupos, tu segment incluye a los usuarios que forman parte de cualquiera de los grupos seleccionados.
...
{: start="5"}
5. Ejecuta una vista previa y luego guarda tu segment.

{% alert note %}
No necesitas editar el código SQL en la plantilla para crear un segment RFM. Puedes usar exclusivamente el panel **Variables** para personalizar tu segment.
{% endalert %}

### Grupos RFM {#rfm-groups}

Los segments RFM se evalúan en un orden específico. Los usuarios se asignan al primer segment cuyos criterios cumplan, desde la parte superior de la lista de priorización hacia abajo. Por ejemplo, un usuario que califica tanto para "Champions" como para "Loyal Users" se asigna al segment "Champions" porque tiene una prioridad más alta.

| Grupo RFM | Descripción del segment | Rango de recencia (R) | Rango de frecuencia (F) | Rango monetario (M) |
|--------------------|-------------------------------------------------------------------------------------|------------------|--------------------|-------------------|
| Champions | El segment de usuarios más valioso con las puntuaciones más altas en todas las categorías. | 3 | 2-3 | 2-3 |
