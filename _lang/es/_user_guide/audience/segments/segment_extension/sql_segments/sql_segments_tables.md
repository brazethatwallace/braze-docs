...
page_type: reference
toc_headers: h2
description: "Esta página es una referencia de las tablas y columnas SQL de Snowflake utilizadas en el Generador de consultas, las Extensiones de segmento SQL y el Uso compartido de datos de Snowflake."
tool: Segments
---
...
- [Uso compartido de datos de Snowflake]({{site.baseurl}}/partners/data_and_analytics/data_warehouses/snowflake/)

La mayoría de las tablas están disponibles en las tres herramientas. Las tablas marcadas como **Solo Uso compartido de datos de Snowflake** son exclusivas del Uso compartido de datos de Snowflake y no son accesibles en el Generador de consultas ni en las Extensiones de segmento SQL.
...
Tabla | Descripción
------|------------
[AGENTCONSOLE_AGENTEXECUTED_SHARED](#AGENTCONSOLE_AGENTEXECUTED_SHARED) | Cuando se ejecuta un agente de la Consola de Agente (**solo Snowflake Data Sharing**)
...
[USER_CUSTOM_ATTRIBUTES_VIEW_SHARED](#USER_CUSTOM_ATTRIBUTES_VIEW_SHARED) | Instantánea periódica de atributos personalizados de perfil por usuario
...
[USER_DEFAULT_ATTRIBUTES_VIEW_SHARED](#USER_DEFAULT_ATTRIBUTES_VIEW_SHARED) | Instantánea periódica de atributos predeterminados de perfil por usuario
...
[USERS_BEHAVIORS_INSTALLATTRIBUTION_SHARED](#USERS_BEHAVIORS_INSTALLATTRIBUTION_SHARED) | Cuando un usuario instala una aplicación y se atribuye a un socio
...
[USERS_BEHAVIORS_GEOFENCE_DATAEVENT_SHARED](#USERS_BEHAVIORS_GEOFENCE_DATAEVENT_SHARED) | Cuando un usuario activa un área con geovalla, por ejemplo, al entrar o salir de una geovalla. Este evento se agrupa con otros eventos y se recibe a través del punto de conexión de eventos estándar, por lo que puede no aparecer en tiempo real.<br><br>Para registrar la actividad de geovallas en esta tabla, selecciona **Enable Analytics for Enter** y **Enable Analytics for Exit** en la configuración avanzada de cada geovalla. Consulta el paso 3 en [Crear geovallas manualmente]({{site.baseurl}}/user_guide/audience/locations_and_geofences/creating_geofences/#manually-create-geofences) para más detalles.
[USERS_BEHAVIORS_GEOFENCE_RECORDEVENT_SHARED](#USERS_BEHAVIORS_GEOFENCE_RECORDEVENT_SHARED) | Cuando un usuario activa un área con geovalla (por ejemplo, al entrar o salir de una geovalla). Este evento se recibió a través del punto de conexión dedicado de geovallas y, por lo tanto, se recibe en tiempo real tan pronto como el dispositivo del usuario detecta que ha activado una geovalla.<br><br>Además, debido al límite de velocidad en el punto de conexión de geovallas, es posible que algunos eventos de geovalla no se reflejen como RecordEvent. Sin embargo, todos los eventos de geovalla están representados por DataEvent (aunque potencialmente con cierto retraso debido al agrupamiento).
...
[USERS_MESSAGES_BANNER_ABORT_SHARED](#USERS_MESSAGES_BANNER_ABORT_SHARED) | Un mensaje de banner originalmente planificado fue cancelado por algún motivo
...
[USERS_MESSAGES_CONTENTCARD_ABORT_SHARED](#USERS_MESSAGES_CONTENTCARD_ABORT_SHARED) | Un mensaje de Content Card originalmente planificado fue cancelado por algún motivo.
...
[USERS_MESSAGES_EMAIL_ABORT_SHARED](#USERS_MESSAGES_EMAIL_ABORT_SHARED) | Un mensaje de correo electrónico originalmente planificado fue cancelado por algún motivo.
...
[USERS_MESSAGES_INAPPMESSAGE_ABORT_SHARED](#USERS_MESSAGES_INAPPMESSAGE_ABORT_SHARED) | Un mensaje dentro de la aplicación originalmente planificado fue cancelado por algún motivo.
...
[USERS_MESSAGES_LINE_ABORT_SHARED](#USERS_MESSAGES_LINE_ABORT_SHARED) | Cuando un mensaje LINE planificado no puede entregarse, antes de enviarlo a LINE
...
[USERS_MESSAGES_NEWSFEEDCARD_ABORT_SHARED](#USERS_MESSAGES_NEWSFEEDCARD_ABORT_SHARED) | Un mensaje de tarjeta de News Feed originalmente planificado fue cancelado por algún motivo
...
[USERS_MESSAGES_PUSHNOTIFICATION_ABORT_SHARED](#USERS_MESSAGES_PUSHNOTIFICATION_ABORT_SHARED) | Un mensaje de notificación push originalmente planificado fue cancelado por algún motivo.
...
[USERS_MESSAGES_SMS_ABORT_SHARED](#USERS_MESSAGES_SMS_ABORT_SHARED) | Un mensaje SMS originalmente planificado fue cancelado por algún motivo.
...
[USERS_MESSAGES_WEBHOOK_ABORT_SHARED](#USERS_MESSAGES_WEBHOOK_ABORT_SHARED) | Un mensaje webhook originalmente planificado fue cancelado por algún motivo
...
[USERS_MESSAGES_WHATSAPP_ABORT_SHARED](#USERS_MESSAGES_WHATSAPP_ABORT_SHARED) | Un mensaje de WhatsApp originalmente planificado fue cancelado por algún motivo
...
`time` | `int` | Marca de tiempo UNIX de la actualización del perfil (para filas rellenadas retroactivamente, la hora del relleno)
...
`update_source` | `string` | Origen de la actualización del perfil
`sf_updated_at` | `timestamp` | Cuándo se actualizó esta fila en Snowflake
`external_id` | `string` | [PII] ID externo del usuario
`first_name` | `string` | [PII] Nombre
`last_name` | `string` | [PII] Apellido
`email` | `string` | [PII] Dirección de correo electrónico
`gender` | `string` | [PII] Género
`phone` | `string` | [PII] Número de teléfono
...
`os_version` | `null,`&nbsp;`string` | Versión del sistema operativo del dispositivo
`device_model` | `null,`&nbsp;`string` | Modelo del dispositivo
`event_type` | `string` | Tipo de evento de geovalla que se desencadenó (por ejemplo, 'enter' o 'exit')
`location_set_id` | `string` | ID del conjunto de ubicaciones de la geovalla que se desencadenó
`geofence_id` | `string` | ID de la geovalla que se desencadenó
`app_group_id` | `null,`&nbsp;`string` | ID BSON del grupo de aplicaciones al que pertenece este usuario
`sf_created_at` | `timestamp`,&nbsp;`null` | Momento en que Snowpipe recogió este evento
...
`device_id` | `null,`&nbsp;`string` | ID del dispositivo en el que ocurrió el evento
`sdk_version` | `null,`&nbsp;`string` | Versión del SDK de Braze en uso durante el evento
`ios_push_token_apns_gateway` | `null, int` | Gateway APNS del token de notificaciones push; solo aplica a tokens push de iOS: 1 para desarrollo, 2 para producción
...
`push_token_provisionally_opted_in` | `null, boolean` | Indicador de aceptación provisional del token de notificaciones push
`ios_push_token_apns_gateway` | `null, int` | Gateway APNS del token de notificaciones push; solo aplica a tokens push de iOS: 1 para desarrollo, 2 para producción
`web_push_token_public_key` | `null,`&nbsp;`string` | Clave pública del token de notificaciones push; solo aplica a tokens push web
`web_push_token_user_auth` | `null,`&nbsp;`string` | Autenticación de usuario del token de notificaciones push; solo aplica a tokens push web
`web_push_token_vapid_public_key` | `null,`&nbsp;`string` | Clave pública VAPID del token de notificaciones push; solo aplica a tokens push web
`push_token_state_change_type` | `null,`&nbsp;`string` | Descripción del tipo de cambio de estado del token de notificaciones push
`app_group_api_id` | `null,`&nbsp;`string` | ID de API del grupo de aplicaciones al que pertenece este usuario
`app_api_id` | `null,`&nbsp;`string` | ID de API de la aplicación en la que ocurrió este evento
`sf_created_at` | `timestamp`,&nbsp;`null` | Momento en que Snowpipe recogió este evento
...
`state_change_source` | `null,`&nbsp;`string` | Origen del cambio de estado (REST, SDK, Dashboard, etc.)
...
`campaign_id` | `string` | ID de Braze de uso interno de la Campaign a la que pertenece este evento
`campaign_api_id` | `null,`&nbsp;`string` | ID de API de la Campaign a la que pertenece este evento
...
`timezone` | `null,`&nbsp;`string` | Zona horaria del usuario
`language` | `null,`&nbsp;`string` | [PII] Idioma del usuario
`revenue` | `long` | Cantidad de ingresos en USD generados, expresada en centavos
`app_group_id` | `null,`&nbsp;`string` | ID BSON del grupo de aplicaciones al que pertenece este usuario
`sf_created_at` | `timestamp`,&nbsp;`null` | Momento en que Snowpipe recogió este evento
...
| `canvas_id`                            | `string`,&nbsp;`null`    | (Solo para uso interno de Braze) ID del Canvas al que pertenece este evento                                                     |
...
`ip_pool` | `null,`&nbsp;`string` | Pool de IP desde el que se realizó el envío de correo electrónico
`bounce_reason` | `null,`&nbsp;`string` | [PII] El código de motivo SMTP y el mensaje descriptivo recibido para este evento de rebote
...
`user_agent` | `null,`&nbsp;`string` | User agent en el que ocurrió el clic
`ip_pool` | `null,`&nbsp;`string` | Pool de IP desde el que se realizó el envío de correo electrónico
...
`sending_ip` | `null,`&nbsp;`string` | Dirección IP desde la que se realizó el envío de correo electrónico
`timezone` | `null,`&nbsp;`string` | Zona horaria del usuario
`deferral_reason` | `null,`&nbsp;`string` | [PII] El código de motivo SMTP y el mensaje descriptivo recibido para este evento de aplazamiento
...
`language` | `null,`&nbsp;`string` | [PII] Idioma del usuario
`email_address` | `string` | [PII] Dirección de correo electrónico del usuario
`user_agent` | `null,`&nbsp;`string` | User agent en el que ocurrió la apertura
`ip_pool` | `null,`&nbsp;`string` | Pool de IP desde el que se realizó el envío de correo electrónico
...
`canvas_step_message_variation_api_id` | `null,`&nbsp;`string` | ID de API de la variación de mensaje del paso en Canvas que recibió este usuario
`canvas_variation_api_id` | `null,`&nbsp;`string` | ID de API de la variación de Canvas a la que pertenece este evento
`feature_flag_id_name` | `null,`&nbsp;`string` | Identificador de despliegue del conmutador de características
`message_variation_api_id` | `null,`&nbsp;`string` | ID de API de la variación de mensaje que recibió este usuario
`external_user_id` | `null,`&nbsp;`string` | [PII] ID externo del usuario
`device_id` | `null,`&nbsp;`string` | ID del dispositivo en el que ocurrió el evento
`time` | `int` | Marca de tiempo UNIX en la que ocurrió el evento
...
`carrier` | `null,`&nbsp;`string` | Operador del dispositivo
`browser` | `null,`&nbsp;`string` | Navegador del dispositivo
`button_string` | `null,`&nbsp;`string` | Identificador (button_string) del botón de notificación push en el que se hizo clic. null si no proviene de un clic en un botón
...
`canvas_step_message_variation_api_id` | `null,`&nbsp;`string` | ID de API de la variación de mensaje del paso en Canvas que recibió este usuario
`campaign_api_id` | `null,`&nbsp;`string` | ID de API de la Campaign a la que pertenece este evento
`from_rcs_sender` | `null,`&nbsp;`string` | El ID del remitente RCS o nombre del agente utilizado para enviar el mensaje
`sf_created_at` | `timestamp`,&nbsp;`null` | Momento en que este evento fue recogido por Snowpipe
...
{% alert note %}
Para las tablas compartidas `USERS_MESSAGES_SMS_*` (incluidas [`USERS_MESSAGES_SMS_REJECTION_SHARED`](#USERS_MESSAGES_SMS_REJECTION_SHARED), [`USERS_MESSAGES_SMS_DELIVERY_SHARED`](#USERS_MESSAGES_SMS_DELIVERY_SHARED) y [`USERS_MESSAGES_SMS_DELIVERYFAILURE_SHARED`](#USERS_MESSAGES_SMS_DELIVERYFAILURE_SHARED)), Braze escribe una fila solo cuando el perfil de usuario de Braze aún existe en el espacio de trabajo en el momento en que el evento se procesa para Snowflake Data Sharing y Currents. Si ese usuario fue eliminado antes de que se complete el procesamiento, el evento no aparece en Snowflake ni en tu exportación de Currents, incluso cuando las métricas del espacio de trabajo de SMS en el dashboard aún reflejan los recuentos agregados de la ruta de informes de Braze. Para el comportamiento correspondiente en Currents, consulta [Eventos de rechazo de SMS]({{site.baseurl}}/user_guide/data_and_analytics/braze_currents/event_glossary/message_engagement_events/#sms-rejection-events) y los tipos de eventos de SMS relacionados en el mismo glosario.
{% endalert %}
...
`to_phone_number` | `null,`&nbsp;`string` | [PII] Número de teléfono del destinatario
`subscription_group_api_id` | `null,`&nbsp;`string` | ID externo del grupo de suscripción
`category` | `null,`&nbsp;`string` | Nombre de la categoría de palabra clave, solo se completa para mensajes de respuesta automática: 'Opt-in', 'Opt-out', 'Help' o un valor personalizado
`app_group_id` | `null,`&nbsp;`string` | ID BSON del grupo de aplicaciones al que pertenece este usuario
...
------|------|------------
`id` | `string` | ID único global para este evento
`user_id` | `null,`&nbsp;`string` | ID de Braze del usuario al que se dirigió short_url, nulo si short_url no utilizó seguimiento de clics de usuario
`external_user_id` | `null,`&nbsp;`string` | [PII] ID externo del usuario al que se dirigió short_url si existe, nulo si short_url no utilizó seguimiento de clics de usuario
...
`message_variation_api_id` | `null,`&nbsp;`string` | ID de API de la variación de mensaje para la que se generó short_url, nulo si no proviene de una campaña
...
`canvas_variation_api_id` | `null,`&nbsp;`string` | ID de API de la variación de Canvas para la que se generó short_url, nulo si no proviene de un Canvas
...
`canvas_step_message_variation_api_id` | `null,`&nbsp;`string` | ID de API de la variación de mensaje del paso en Canvas para la que se generó short_url, nulo si no proviene de un Canvas
...
`webhook_failure_source` | `null,`&nbsp;`string` | Indica si el error fue creado por Braze o por el propio punto de conexión. El campo de origen puede ser External Endpoint, Treat no status code to host unreachable
...
`sf_created_at` | `timestamp`,&nbsp;`null` | Momento en que este evento fue recogido por Snowpipe
`send_id` | `null,`&nbsp;`string` | ID de envío del mensaje al que pertenece este mensaje
`flow_id` | `null,`&nbsp;`string` | ID único del flujo en WhatsApp Manager. Presente si el usuario está respondiendo a un flujo de WhatsApp.
...
`action` | `string` | Acción tomada en respuesta a este mensaje. Por ejemplo, `Subscribed`, `Unsubscribed` o `None`.
`sf_created_at` | `timestamp`,&nbsp;`null` | Momento en que este evento fue recogido por Snowpipe
`catalog_id` | `null,`&nbsp;`string` | ID de catálogo de un producto si se hace referencia a un producto en el mensaje de entrada. De lo contrario, vacío.
`product_id` | `null,`&nbsp;`string` | ID del producto comprado
`flow_id` | `null,`&nbsp;`string` | ID único del flujo en WhatsApp Manager. Presente si el usuario está respondiendo a un flujo de WhatsApp.
`flow_response_json` | `null,`&nbsp;`string` | [PII] Valores del formulario con los que respondió el usuario. Presente si el usuario está respondiendo a un flujo de WhatsApp.
`message_id` | `null,`&nbsp;`string` | ID único generado por Meta para este mensaje
...
| ------------------ | ------------------------ | -------------------------------------------------------------------------------------------- |
| `id`               | `string`,&nbsp;`null`    | ID único global para este evento                                                             |
| `user_id`          | `string`,&nbsp;`null`    | ID de Braze del usuario huérfano                                                             |
| `external_user_id` | `string`,&nbsp;`null`    | [PII] ID externo del usuario                                                                 |
| `device_id`        | `string`,&nbsp;`null`    | ID del dispositivo vinculado a este usuario, si el usuario es anónimo                        |
| `app_group_id`     | `string`,&nbsp;`null`    | ID de Braze del espacio de trabajo al que pertenece este usuario                             |
| `app_group_api_id` | `string`,&nbsp;`null`    | ID de API del espacio de trabajo al que pertenece este usuario                               |
