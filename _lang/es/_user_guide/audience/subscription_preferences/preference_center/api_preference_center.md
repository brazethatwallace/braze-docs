...
Al usar los [endpoints de centro de preferencias de Braze]({{site.baseurl}}/api/endpoints/preference_center), puedes crear un centro de preferencias, un sitio web alojado por Braze, que puede mostrar el estado de suscripción y los estados del grupo de suscripción de tu usuario. Usando HTML y CSS, tu equipo de desarrolladores puede crear el centro de preferencias para que el estilo de la página coincida con las directrices de tu marca.
...
El [endpoint Generar URL de centro de preferencias]({{site.baseurl}}/api/endpoints/preference_center/get_create_url_preference_center) te permite obtener la URL del centro de preferencias para un usuario específico fuera de un correo electrónico enviado a través de Braze.

{% alert note %}
Braze renderiza `confirmation_page_html` en un iframe que usa una URL `data:`. Los navegadores tratan las URL `data:` como orígenes opacos. Como resultado, los scripts en ese iframe no pueden cargar recursos externos adicionales, y la navegación de la ventana principal o la comunicación entre marcos desde esa página falla.<br><br>En su lugar, puedes enlazar a contenido externo, como una URL de cuestionario alojada, en lugar de incrustar scripts. Si debes incrustar una herramienta de terceros y el proveedor lo permite, usa un `<iframe title="Descripción del contenido incrustado" src="https://example.com/...">` que apunte a la URL HTTPS alojada de la herramienta.
{% endalert %}

### Paso 2: Incluir en tu Campaign de correo electrónico {#step-2-include-in-your-email-campaign}
...
Braze gestiona las actualizaciones del estado de suscripción desde el centro de preferencias, lo que mantiene el centro de preferencias sincronizado. Sin embargo, también puedes crear y alojar tu propio centro de preferencias utilizando las [API de grupos de suscripción]({{site.baseurl}}/api/endpoints/subscription_groups) con las siguientes opciones.

### Opción 1: Enlace con parámetros de cadena de consulta {#option-1-link-with-string-query-parameters}

Usa pares de campo-valor de cadena de consulta en el cuerpo de la URL para pasar el ID de usuario y la categoría de correo electrónico a la página, de modo que los usuarios solo necesiten confirmar su elección para cancelar la suscripción. Esta opción es adecuada para quienes almacenan un identificador de usuario en un formato hash y aún no tienen un centro de suscripción.
...
{% alert tip %}
También es posible aplicar un hash al ID externo del usuario en el momento del envío utilizando un filtro de Liquid. Esto convertirá el `user_id` en un valor hash MD5, por ejemplo:
{% raw %}
```liquid
...
Este enfoque no requiere pares de valor de cadena de consulta incrustados en la URL, ya que estos se pueden pasar en la carga útil del JSON web token, por ejemplo:
...
Los enlaces del centro de preferencias requieren un contexto de envío en vivo. Los envíos de prueba no generan URL válidas del centro de preferencias, y el botón **Guardar preferencias** está deshabilitado si la página se carga. Este es el comportamiento esperado. Para probar de extremo a extremo, lanza una Campaign o un paso en Canvas a un usuario de prueba o a un Segment interno pequeño, o usa el [endpoint Generar URL del centro de preferencias]({{site.baseurl}}/api/endpoints/preference_center/get_create_url_preference_center). Para más detalles, consulta [Probar centros de preferencias](#testing-preference-centers).
...
Se usa para renderizar el centro de preferencias cuando se utiliza el Liquid heredado {%raw%}`${preference_center_url}`{%endraw%}, lo que significa que los pasos en Canvas o las plantillas que hacen referencia a {%raw%}`${preference_center_url}` o `preference_center.${PreferenceCenterBrazeDefault}`{%endraw%} no funcionarán. Esto también aplica a mensajes enviados anteriormente que incluían el Liquid heredado o "PreferenceCenterBrazeDefault" como parte del mensaje.

Si haces referencia a {%raw%}`${preference_center_url}`{%endraw%} en un nuevo mensaje otra vez, se creará de nuevo un centro de preferencias llamado "PreferenceCenterBrazeDefault".

### ¿Los centros de preferencias admiten varios idiomas? {#do-preference-centers-support-multiple-languages}

No. Sin embargo, puedes aprovechar Liquid al escribir el HTML para páginas personalizadas de adhesión voluntaria y cancelación de suscripción. Si usas enlaces dinámicos para gestionar las cancelaciones de suscripción, se trata de un único enlace.

Por ejemplo, si estás haciendo seguimiento de la tasa de cancelaciones para usuarios hispanohablantes, necesitarías usar Campaigns separadas o aprovechar los análisis de Currents (como verificar cuándo un usuario cancela su suscripción y comprobar el idioma preferido de ese usuario).

Como otro ejemplo, para hacer seguimiento de las tasas de cancelación de suscripción de usuarios hispanohablantes, podrías agregar una cadena de parámetro de consulta como `?Spanish=true` a la URL de cancelación de suscripción si el idioma del usuario es español, y usar un enlace de cancelación de suscripción normal en caso contrario:
...
### ¿Se requieren tanto los enlaces de cancelación de suscripción como los centros de preferencias de correo electrónico para enviar? {#are-both-unsubscribe-links-and-email-preference-centers-required-for-sending}
...
De forma predeterminada, el icono junto al nombre de la pestaña del navegador (favicon) usa el logotipo de Braze. Para agregar un favicon personalizado, configúralo a través del atributo `links-tags` en tu llamada a la API de [centro de preferencias]({{site.baseurl}}/api/endpoints/preference_center) de creación o actualización. Braze entonces inyecta la etiqueta {% raw %}`<link rel="icon" ...>`{% endraw %} en la página alojada por ti.
