...
Antes de empezar a personalizar tu plantilla, puedes establecer estilos a nivel de mensaje para todo el mensaje utilizando el menú lateral. Por ejemplo, puedes personalizar el tipo de letra de todo el texto o el color de todos los enlaces incluidos en tu mensaje. También puedes hacer que el mensaje sea de tipo modal o de pantalla completa.
...
Te recomendamos incluir en tu mensaje un texto de adhesión voluntaria y enlaces a la política de privacidad y los términos y condiciones de tu marca. Asegúrate de trabajar con tu equipo jurídico para desarrollar un lenguaje adaptado a tu marca específica.

{% alert note %}
Las mejores prácticas de capacidad de entrega a menudo superan los requisitos legales, y nuestra recomendación es obtener siempre el consentimiento explícito para enviar correos electrónicos y permitir a los usuarios rechazarlos fácilmente.
{% endalert %}
...
### Doble verificación de adhesión voluntaria {#double-opt-in-verification}
...
1. Crea un Canvas basado en acciones y configúralo para que se active cuando un usuario añada una dirección de correo electrónico a Braze. Asegúrate de que permites dirigirte a usuarios que son nuevos en la plataforma (por ejemplo, utilizando un segmento sin filtros en el Canvas).
...
3. Añade un [paso de Rutas de Acción]({{site.baseurl}}/user_guide/engagement_tools/canvas/canvas_components/action_paths#action-paths).
4. Para la primera ruta, desencadena un correo electrónico cuando un usuario cambie su estado de suscripción de correo electrónico a `opted_in`. Este correo electrónico debe informar a los usuarios de que su correo electrónico ha sido confirmado.
5. Configura la otra ruta para salir del Canvas después de que expire la ventana.
