...
- El SDK or kit de desarrollo de software nativo sigue enviando una difusión `BRAZE_PUSH_CLICKED` y tu listener Dart `push_opened` sigue activándose como se espera.
- El SDK or kit de desarrollo de software nativo no llama a `startActivity()`, por lo que tu aplicación no se lleva al primer plano y el vínculo profundo no se sigue automáticamente.
