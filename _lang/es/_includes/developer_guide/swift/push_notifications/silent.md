...
{% alert warning %}
No se recomienda adjuntar un título y un cuerpo con `content-available=1` porque puede provocar un comportamiento indefinido. Para asegurarte de que una notificación es realmente silenciosa, excluye tanto el título como el cuerpo cuando configures la bandera `content-available` en `1.` Para más detalles, consulta la [documentación oficial de Apple sobre actualizaciones en segundo plano](https://developer.apple.com/documentation/usernotifications/setting_up_a_remote_notification_server/pushing_background_updates_to_your_app).
{% endalert %}
...
## Ignorar las notificaciones push internas {#ignoring-internal-push-notifications}

Braze utiliza notificaciones push silenciosas para gestionar internamente ciertas características avanzadas, como Uninstall Tracking. Si tu aplicación lleva a cabo acciones automáticas en el lanzamiento de aplicaciones o en notificaciones push en segundo plano, considera la posibilidad de controlar esa actividad para que no se desencadene con ninguna notificación push interna.

Por ejemplo, si tienes una lógica que llama a tus servidores para obtener nuevo contenido en cada push en segundo plano o lanzamiento de aplicación, puede que quieras evitar que los push internos de Braze la desencadenen para evitar tráfico de red innecesario. Dado que Braze envía ciertos tipos de push internos a todos los usuarios aproximadamente al mismo tiempo, puede producirse una carga significativa del servidor si no se controlan las llamadas de red durante el lanzamiento provocado por los push internos.

### Paso 1: Comprueba las acciones automáticas de tu aplicación {#step-1-check-your-app-for-automatic-actions}
...
1. **Receptores push.** Las notificaciones push en segundo plano llamarán a `application:didReceiveRemoteNotification:fetchCompletionHandler:` en `UIApplicationDelegate`.
...
### Paso 2: Utiliza el método interno de la utilidad push {#step-2-use-the-internal-push-utility-method}

Puedes utilizar el método de utilidad estática en `Braze.Notifications` para comprobar si tu aplicación ha recibido o ha sido lanzada por un push interno de Braze. [`Braze.Notifications.isInternalNotification(_:)`](https://braze-inc.github.io/braze-swift-sdk/documentation/brazekit/braze/notifications-swift.class/isinternalnotification(_:)) devuelve `true` para todas las notificaciones push internas de Braze, que incluyen Uninstall Tracking y notificaciones de sincronización de [conmutadores de características]({{site.baseurl}}/user_guide/messaging/feature_flags/).
