...
Para desactivar la actividad de seguimiento de datos en el SDK or kit de desarrollo de software Web, utiliza el método [`disableSDK()`](https://js.appboycdn.com/web-sdk/latest/doc/modules/braze.html#disablesdk). Esto sincronizará cualquier dato registrado antes de que se llamara a `disableSDK()`, y hará que todas las llamadas posteriores al SDK or kit de desarrollo de software Web de Braze para esta página y las cargas de páginas futuras se ignoren.
{% endtab %}
...
{% alert note %}
No es necesario inicializar el SDK or kit de desarrollo de software de Braze para llamar a `disableSDK()`, lo que te permite desactivar el seguimiento para usuarios completamente anónimos. Por el contrario, `enableSDK()` no inicializa el SDK or kit de desarrollo de software de Braze, por lo que también debes llamar a `initialize()` después para habilitar el seguimiento.
{% endalert %}
