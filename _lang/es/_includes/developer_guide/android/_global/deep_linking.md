...
2. **Comprueba que la gestión automática de vínculos profundos esté habilitada.** Verifica que `com_braze_handle_push_deep_links_automatically` esté establecido en `true` en `braze.xml`, o configura esta opción mediante la [configuración en tiempo de ejecución]({{site.baseurl}}/developer_guide/sdk_integration/?sdktab=android#runtime-configuration). Sin esta configuración, Braze no abre automáticamente tu aplicación ni el destino del vínculo profundo cuando alguien toca una notificación push.
