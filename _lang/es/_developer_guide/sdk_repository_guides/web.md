...
El SDK Web de Braze te permite integrar la plataforma de interacción con los clientes de Braze directamente en tus aplicaciones web. Construido con TypeScript y diseñado para el desarrollo web moderno, este SDK proporciona herramientas completas para la gestión de usuarios, mensajería, análisis y conmutadores de características.

### Qué puedes hacer {#what-you-can-do}

- **Gestión de usuarios**: Rastrea y gestiona identidades, atributos y comportamientos de los usuarios en tu aplicación web
- **In-App Messages**: Muestra mensajes y notificaciones dirigidos a los usuarios mientras utilizan activamente tu sitio
- **Content Cards**: Muestra fuentes de contenido personalizado y tarjetas promocionales que se actualizan en tiempo real
- **Banners**: Muestra mensajes de tipo banner en ubicaciones específicas dentro de tu sitio
...
- **Gestión de sesiones**: Monitoriza las sesiones de los usuarios y los patrones de participación
...
|--------|------|---------|-------------|
| `baseUrl` | `string` | **Obligatorio** | Esta opción es obligatoria para configurar el SDK de Web de Braze para que utilice el endpoint adecuado para tu integración, por ejemplo: `braze.initialize('YOUR-API-KEY-HERE', { baseUrl: 'sdk.iad-03.braze.com' })` |
| `enableLogging` | `boolean` | `false` | Establécelo como true para habilitar el registro de manera predeterminada. Ten en cuenta que esto hará que Braze registre información en la consola de JavaScript, que es visible para todos los usuarios. Probablemente debas eliminar esto o proporcionar un registrador alternativo con setLogger antes de publicar tu página en producción. |
| `allowUserSuppliedJavascript` | `boolean` | `false` | De forma predeterminada, el SDK de Web de Braze no permite acciones de clic en JavaScript proporcionadas por el usuario, ni habilita In-App Messages HTML ni Banners, ya que permiten a los usuarios del panel de Braze ejecutar JavaScript en tu sitio. Para indicar que confías en los usuarios del panel de Braze para escribir acciones de clic en JavaScript no maliciosas, establece esta propiedad en true. |
| `doNotLoadFontAwesome` | `boolean` | `false` | Braze utiliza Font Awesome para los iconos de los mensajes dentro de la aplicación. De forma predeterminada, Braze cargará automáticamente FontAwesome 4.7.0 desde la CDN de FontAwesome. Para desactivar este comportamiento (por ejemplo, porque tu sitio usa una versión personalizada de FontAwesome), establece esta opción en `true`. Ten en cuenta que si haces esto, eres responsable de asegurar que FontAwesome esté cargado en tu sitio; de lo contrario, los mensajes dentro de la aplicación podrían no mostrarse correctamente. |
| `inAppMessageZIndex` | `number` | `999999` | De forma predeterminada, el SDK de Braze mostrará los In-App Messages con un z-index de 999999. Proporciona un valor para esta opción para anular ese valor predeterminado. |
| `sessionTimeoutInSeconds` | `number` | `30` | De forma predeterminada, una sesión caduca tras 30 segundos de inactividad. Proporciona un valor para esta opción para anular ese valor predeterminado. |
...
| `appVersion` | `string` | `undefined` | Si proporcionas un valor para esta opción, los eventos de usuario enviados a Braze se asociarán con la versión indicada, que puede usarse para la segmentación de usuarios. |
| `appVersionNumber` | `string` | `undefined` | Un valor numérico de versión de la aplicación que puede usarse para la segmentación de usuarios. Este valor debe enviarse con cuatro campos, como "1.2.3.4"; de lo contrario, se ignorará. Nota: `appVersion` también debe establecerse, ya sea con el mismo valor o con un nombre único para esta versión. |
| `contentSecurityNonce` | `string` | `undefined` | Si proporcionas un valor para esta opción, el SDK de Braze añadirá el nonce a cualquier elemento `<script>` y `<style>` creado por el SDK. Esto puede usarse para permitir que el SDK de Braze funcione con la Política de seguridad de contenido de tu sitio web. Ten en cuenta que, además de establecer este nonce, es posible que también necesites permitir la carga de FontAwesome, lo que puedes hacer agregando `use.fontawesome.com` a la lista de permitidos de tu Política de seguridad de contenido o utilizando la opción `doNotLoadFontAwesome` y cargándolo manualmente. |
| `noCookies` | `boolean` | `false` | De forma predeterminada, el SDK de Web de Braze utiliza cookies. Para desactivar el uso de cookies, establece esta opción en true. Ten en cuenta que desactivar las cookies puede afectar la capacidad del SDK para recordar las identidades de los usuarios entre sesiones. |
| `allowCrawlerActivity` | `boolean` | `false` | De forma predeterminada, el SDK de Web de Braze ignora la actividad de arañas web o rastreadores conocidos, como Google, basándose en la cadena de agente de usuario. Esto ahorra puntos de datos, hace que los análisis sean más precisos y puede mejorar el posicionamiento en buscadores. Sin embargo, si deseas que Braze registre la actividad de estos rastreadores, puedes establecer esta opción en true. |
| `disablePushTokenMaintenance` | `boolean` | `false` | De forma predeterminada, los usuarios que ya han concedido permiso de push web (por ejemplo, a través de requestPushPermission o de un proveedor de push anterior) sincronizarán su token de notificaciones push con el backend de Braze automáticamente en cada nueva sesión para garantizar la capacidad de entrega. Para desactivar este comportamiento, establece esta opción en true. |
| `enableSdkAuthentication` | `boolean` | `false` | Establécelo en true para habilitar la característica de autenticación del SDK. Para más información sobre la autenticación del SDK, consulta nuestra documentación del producto. |
| `manageServiceWorkerExternally` | `boolean` | `false` | De forma predeterminada, el SDK de Web de Braze gestiona su propio service worker para las notificaciones push. Si ya gestionas un service worker en tu aplicación y deseas incorporar la funcionalidad del service worker de Braze, establece esta opción en true e incluye el código del service worker de Braze en tu archivo de service worker. |
...
| `serviceWorkerLocation` | `string` | `undefined` | De forma predeterminada, el SDK de Web de Braze buscará su archivo de service worker en la raíz de tu dominio. Proporciona un valor para esta opción para anular ese valor predeterminado y especificar una ubicación personalizada para el archivo de service worker. |
| `safariWebsitePushId` | `string` | `undefined` | Obligatorio para las notificaciones push de Safari. Este valor se puede encontrar en tu cuenta de desarrollador de Apple. Para más información sobre la configuración de las notificaciones push de Safari, consulta nuestra documentación del producto. |
...
| `requireExplicitInAppMessageDismissal` | `boolean` | `false` | De forma predeterminada, los mensajes dentro de la aplicación pueden descartarse haciendo clic fuera de ellos o presionando la tecla Escape. Establece esta opción en true para requerir que los usuarios hagan clic explícitamente en un botón de descarte o de acción para cerrar el mensaje. |
...
| `serviceWorkerScope` | `string` | `undefined` | De forma predeterminada, el SDK de Web de Braze registrará su service worker con el ámbito predeterminado (el directorio del service worker). Proporciona un valor para esta opción para anular ese valor predeterminado y especificar un ámbito personalizado para el service worker. |
...
#### Manejo personalizado de mensajes dentro de la aplicación {#custom-in-app-message-handling}
...
#### Registrar interacciones de mensajes dentro de la aplicación {#log-in-app-message-interactions}
...
Cuando llamas a `dismissBanner(banner)`, el SDK gestiona el estado de descarte del banner, elimina el banner de las actualizaciones activas de banners, notifica a los suscriptores del evento de descarte del banner y sincroniza el descarte con Braze. Las interfaces personalizadas deben usar `subscribeToBannersUpdates` para reaccionar a la eliminación del banner descartado, en lugar de tratar `dismissBanner` únicamente como un cambio local en la interfaz o como un método de registro de análisis.
...
Si utilizas un framework de renderizado del lado del servidor (SSR) como Next.js, es posible que encuentres errores porque el SDK está diseñado para ejecutarse en un entorno de navegador. Puedes resolver estos problemas importando dinámicamente el SDK.
...
Alternativamente, si usas webpack para empaquetar tu aplicación, puedes aprovechar sus comentarios mágicos para importar dinámicamente solo las partes del SDK que necesitas.
...
Si usas Vite y ves una advertencia sobre dependencias circulares o `Uncaught TypeError: Class extends value undefined is not a constructor or null`, puede que necesites excluir el SDK de Braze de su descubrimiento de dependencias:
...
1. **Incluir el script de notificaciones push web de AMP**: agrega la etiqueta de script asíncrona a tu encabezado
2. **Agregar widgets de suscripción**: agrega widgets para permitir a los usuarios suscribirse o cancelar su suscripción
3. **Agregar archivos auxiliares**: incluye `helper-iframe.html` y `permission-dialog.html`
4. **Crear un prestador de servicios**: agrega el archivo de prestador de servicios de Braze
5. **Configurar el elemento de notificaciones push web de AMP**: agrega el elemento `amp-web-push` con tu clave de API y la URL base como parámetros de consulta
...
Electron no es oficialmente compatible con las notificaciones push web (consulta: este [issue en GitHub](https://github.com/electron/electron/issues/6697)). Existen otras [soluciones alternativas de código abierto](https://github.com/MatthieuLemoine/electron-push-receiver) que puedes probar y que no han sido probadas por Braze.

### Integración con CDN {#cdn-integration}

- **Carga del script**: inicializa después de que se cargue la etiqueta del script colocando el código de inicialización después de la etiqueta del script, o usa el controlador de eventos `onload` de la etiqueta del script.
...
- **Registro predeterminado**: de forma predeterminada, el SDK Web de Braze registra y administra tu prestador de servicios automáticamente cuando se llama a `requestPushPermission()`, así como al inicio de cada nueva sesión para los usuarios que ya han concedido permiso de push. Aún necesitas alojar un archivo de prestador de servicios en la ubicación esperada que contenga el código del prestador de servicios de Braze.
- **Administrar tu propio prestador de servicios**: si ya administras un prestador de servicios en tu aplicación, establece la opción de inicialización `manageServiceWorkerExternally` en `true`, agrega el código del prestador de servicios de Braze a tu archivo de prestador de servicios y regístralo tú mismo usando `navigator.serviceWorker.register()`.
- **Permisos de push**: llama a `braze.requestPushPermission()` en respuesta a las interacciones del usuario (por ejemplo, clics en botones). Utiliza solicitudes push suaves (interfaz personalizada) antes de solicitar el permiso del navegador.

### Administradores de etiquetas {#tag-managers}

#### Tealium iQ

Tealium iQ ofrece una integración básica con Braze lista para usar. Para configurar la integración, busca Braze en la interfaz de administración de etiquetas de Tealium y proporciona la clave de API del SDK Web desde tu panel. Para más detalles o soporte de configuración avanzada de Tealium, consulta nuestra [documentación de integración](https://www.braze.com/docs/partners/data_and_infrastructure_agility/customer_data_platform/tealium/#about-tealium) o contacta con tu director de cuentas de Tealium.
...
Braze también puede ser compatible con otras soluciones de administración de etiquetas siguiendo nuestras instrucciones de integración dentro de una etiqueta HTML personalizada. Contacta con un representante de Braze si necesitas ayuda para evaluar estas soluciones.
...
| Nombre | Descripción | npm | URL CDN
| ---- | ----------- | --- | -------
| Full | SDK completo con interfaz de usuario. Cuando se usa la versión de npm, los empaquetadores de JavaScript eliminan el código no utilizado, incluido el código de la interfaz de usuario. | `@braze/web-sdk` | https://js.appboycdn.com/web-sdk/6.12/braze.min.js
| Core | Contiene el SDK sin interfaz de usuario. Implementa tu propia interfaz de usuario para In-App Messages y Content Cards cuando uses esta versión del SDK. Usa la biblioteca completa para la mayoría de las integraciones, ya que proporciona elementos de interfaz de usuario personalizables mediante CSS. | N/A | https://js.appboycdn.com/web-sdk/6.12/braze.core.min.js
| No-AMD | Contiene el SDK completo sin soporte para AMD. Esto es útil si tu sitio usa RequireJS u otro cargador de módulos AMD, pero prefieres cargar el SDK a través de la CDN. | N/A | https://js.appboycdn.com/web-sdk/6.12/braze.no-amd.min.js
...
Pasa la opción `enableLogging: true` a la función de inicialización (`braze.initialize('YOUR-API-KEY-HERE', { baseUrl: 'YOUR-SDK-ENDPOINT', enableLogging: true });`) para que Braze registre mensajes en la consola de JavaScript. Esto es útil durante el desarrollo, pero es visible para todos los usuarios, así que elimina esta opción o [proporciona un registrador alternativo](https://js.appboycdn.com/web-sdk/6.12/doc/modules/braze.html#setlogger) antes de pasar tu página a producción.
