...
## Acerca del SDK de Braze para React Native {#about-the-braze-react-native-sdk}
...
- **Content Cards**: fuente de tarjetas predeterminada, u obtén tarjetas y construye tu propia interfaz de usuario
...
1. Instala el paquete npm (consulta [Instalación](#installation)).
...
> **Fuente de referencia:** Las pantallas paso a paso, los cambios de Gradle/CocoaPods y la lista completa de claves XML de Android están en la [guía del desarrollador de Braze para React Native](https://www.braze.com/docs/developer_guide/sdk_integration/?sdktab=react%20native). Los siguientes fragmentos de código son ejemplos mínimos.

### Android

- Añade el **complemento Kotlin Gradle** en tu archivo raíz `build.gradle` si tu plantilla no lo incluye ya (las versiones dependen de tu versión de React Native).
- Añade un archivo de recurso `braze.xml` en `res/values` con tu configuración. Habilita la inicialización diferida para que el SDK espere a `Braze.initialize()` desde JavaScript antes de iniciarse. Otros valores de configuración (push, tiempo de espera de sesión, etc.) aún se leen de este archivo y se aplican en el momento de la inicialización.
- Asegúrate de que los permisos básicos como `INTERNET` y `ACCESS_NETWORK_STATE` estén en `AndroidManifest.xml`.
- Para push, completa la integración de FCM y cualquier indicador de ID de remitente o registro específico de Braze descrito en la documentación.
...
- **Closure `configure`**: recibe un `Braze.Configuration` y te permite configurar propiedades de configuración nativa (registro, push, sesiones, etc.). La clave de API y el endpoint se proporcionan desde JavaScript; no los configuras aquí.
- **Closure `postInitialization`** *(opcional)*: recibe la instancia activa de `Braze` después de su creación, para configuraciones que requieren la instancia (p. ej., almacenar una referencia, configurar delegados).

{% alert note %}
`BrazeReactInitializer.configure` es una API diseñada principalmente para Swift que reemplaza la función obsoleta `BrazeReactBridge.initBraze(_:)`. También resuelve un problema de resolución de tipos de Swift con `Braze.Configuration` en el puente de Objective-C.
{% endalert %}
---
...
| `com_braze_server_target` | `string` | Selector opcional de clúster/entorno (por ejemplo, algunas compilaciones internas o de pruebas). Prefiere `com_braze_custom_endpoint` para producción a menos que tu integración de Braze especifique lo contrario. |
| `com_braze_firebase_cloud_messaging_registration_enabled` | `bool` | Cuando es `true`, Braze se registra para FCM (configuración push habitual). |
| `com_braze_firebase_cloud_messaging_sender_id` | `string` | ID de remitente de FCM cuando el registro automático está habilitado. |
| `com_braze_handle_push_deep_links_automatically` | `bool` | Permite que Braze abra los vínculos profundos push automáticamente. |
...
| **Otros** | *varios* | Claves adicionales no mostradas aquí (tiempo de espera de sesión, geovallas, ubicación, valores predeterminados de notificación, listas de dispositivos permitidos, inicialización retrasada, autenticación del SDK y más). Consulta [`BrazeConfigurationProvider`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.configuration/-braze-configuration-provider/index.html) y la [guía de integración del SDK de Android](https://www.braze.com/docs/developer_guide/platforms/android/sdk_integration/). |
...
| **Push** | `push.automation`, `push.appGroup`, … | La automatización simplifica el registro; `appGroup` es necesario para Push Stories/extensiones cuando se utilicen. |
| **Mensajes dentro de la aplicación** | `triggerMinimumTimeInterval` | Predeterminado: **30** segundos entre desencadenantes. |
| **Sesiones** | `sessionTimeout` | Inactividad antes de una nueva sesión (consulta la documentación de sesiones de Braze). |
| **Privacidad/datos** | `api.trackingPropertyAllowList`, `devicePropertyAllowList`, `api.sdkAuthentication` | Alinéalo con el [manifiesto de privacidad](https://www.braze.com/docs/developer_guide/platform_integration_guides/swift/privacy_manifest/) y la configuración del producto de autenticación del SDK. |
| **Red** | `api.requestPolicy`, `api.flushInterval` | Política de reintentos de solicitudes y cadencia de vaciado. |
| **Suscripción push** | `optInWhenPushAuthorized` | Cuando es `true`, la suscripción puede pasar a optada cuando el usuario autoriza las notificaciones. |
| **IAM y cambios de usuario** | `preventInAppMessageDisplayForDifferentUser` | Reduce la aparición de IAM no coincidentes si cambia el ID de usuario. |
...
El puente de React Native establece **`api.sdkFlavor`** / metadatos del SDK específicos de React durante la inicialización; no los anules a menos que la documentación de Braze te lo indique.
...
**Autenticación del SDK** opcional: pasa una firma como segundo argumento de `changeUser`, o llama a `Braze.setSdkAuthenticationSignature(signature)` cuando esté habilitada en el panel.
...
**`changeUser`** solo le indica a Braze a qué ID de usuario atribuir la **nueva** actividad. **No** borra los datos del SDK almacenados en caché del dispositivo. No existe una API de "cierre de sesión" separada: si necesitas un cierre de sesión tradicional (borrar el estado local de Braze para que el perfil, los mensajes y los tokens en caché del usuario anterior desaparezcan de esta instalación), normalmente usas **`wipeData()`**. Esto realiza un reinicio local completo.
...
**`wipeData()`** — Borra los datos **locales** de Braze para esta instalación (estado de usuario/sesión/tarjetas en caché, asociación de token de notificaciones push, etc.). Úsalo para comportamiento de **cierre de sesión** cuando no debas dejar el estado de Braze del usuario anterior en el dispositivo, además de **"eliminar mis datos en este dispositivo"**, reinicios de **QA** sin reinstalar, o flujos estrictos de **privacidad**. **`changeUser`** por sí solo no realiza esa limpieza, solo establece qué ID de usuario recibe los **nuevos** eventos. En **iOS**, el comportamiento puede diferir de Android (por ejemplo, la interacción con el estado del SDK deshabilitado); consulta la documentación nativa de Braze si implementas esto en producción.

**`disableSDK()`** — Detiene la operación del SDK (sin recopilación/reenvío según la configuración). Úsalo para alternadores de **exclusión voluntaria del usuario**, **modos restringidos** (cumplimiento normativo, configuración para menores) o **depuración** sin eliminar la dependencia.
...
- **Nueva Arquitectura / Turbo Modules**: compatible con las versiones recientes del complemento; sigue la guía del desarrollador y la configuración de ejemplo de `AppDelegate` / Gradle si realizas la migración.
- **Privacidad (iOS)**: métodos como `updateTrackingPropertyAllowList` son compatibles con la configuración relacionada con el manifiesto de privacidad; consulta el [manifiesto de privacidad de Swift](https://www.braze.com/docs/developer_guide/platform_integration_guides/swift/privacy_manifest/).
- **Jest**: simula los módulos nativos de `react-native` o el Turbo module de Braze (consulta `__tests__/jest.setup.js` en este repositorio para ver los patrones).
...
Usa `RCT_NEW_ARCH_ENABLED=0 pod install` si necesitas la arquitectura legacy.
