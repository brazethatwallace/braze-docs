...
Por defecto, los mensajes dentro de la aplicación se desencadenan por tipos de eventos registrados por el SDK or kit de desarrollo de software. Si quieres desencadenar mensajes dentro de la aplicación mediante eventos enviados por el servidor, también puedes conseguirlo.

Para habilitar esta característica, enviarías un push silencioso al dispositivo, lo que permitiría al dispositivo registrar un evento basado en el SDK or kit de desarrollo de software. Este evento del SDK or kit de desarrollo de software desencadenaría posteriormente el mensaje dentro de la aplicación dirigido al usuario.
...
La Campaign push debe incluir extras de par clave-valor, que indiquen que esta Campaign push se envía para registrar un evento personalizado del SDK or kit de desarrollo de software. Este evento se utilizará para desencadenar el mensaje dentro de la aplicación:
...
El código del método `application(_:didReceiveRemoteNotification:fetchCompletionHandler:)` comprueba si existe la clave `IS_SERVER_EVENT` y registrará un evento personalizado del SDK or kit de desarrollo de software si está presente.
...
Debido a que se utiliza un mensaje push para registrar un evento personalizado del SDK or kit de desarrollo de software, Braze necesitará almacenar un token de notificaciones push para cada usuario para habilitar esta solución. Tanto para iOS como para Android, Braze solo almacenará un token a partir del momento en que el usuario haya recibido el aviso push del sistema operativo. Antes de esto, el usuario no será localizable mediante push, y la solución anterior no será posible.