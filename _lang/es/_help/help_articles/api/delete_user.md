...
page_type: reference
description: "Este artículo de ayuda describe las implicaciones de eliminar un perfil de usuario a través de la REST API de Braze."
tool: Dashboard
platform: API
---

# Eliminar usuarios mediante API {#remove-users-via-api}

Cuando [eliminas a un usuario a través de la REST API de Braze]({{site.baseurl}}/api/endpoints/user_data/#user-delete-endpoint/), se borran (anulan) los siguientes datos:
...
Cuando [eliminas a un usuario a través de la REST API de Braze]({{site.baseurl}}/api/endpoints/user_data/#user-delete-endpoint/), se producen los siguientes eventos:
- El perfil de usuario se borra (anula).
- El recuento de [usuarios de por vida]({{site.baseurl}}/user_guide/data_and_analytics/analytics/understanding_your_app_usage_data/#lifetime-users) se actualizará para tener en cuenta a los usuarios recién eliminados.
