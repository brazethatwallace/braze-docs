...
## Datos del catálogo de Shopify compatibles {#supported-shopify-catalog-data}
...
| `variant_image_url`  | cadena         | La misma URL de estilo CDN que la imagen del producto cuando no existe una imagen de variante; en caso contrario, una URL de imagen específica de la variante |
| `vendor`             | cadena         | "Flash and Thread", "PantsLabyrinth"                                            |
| `product_type`       | cadena         | "Outerwear", "T-Shirts" (del campo **Product type** del producto en Shopify)      |
...
| `product_tags`       | matriz          | `["Summer", "Sale", "New"]`<br><br>Requiere la sincronización de etiquetas de producto.                 |
| `collection_ids`     | matriz          | `[123456789012, 987654321098]` (ID de colecciones de Shopify)<br><br>Requiere la sincronización de colecciones de Shopify. |
| `Metafield columns`  | Varía según el tipo | Cada metafield sincronizado aparece como una columna separada con el nombre de su clave. Consulta [Metafields compatibles](#step-3) en la pestaña "Product metafields" del paso 3 para obtener más información. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="Datos del catálogo de Shopify compatibles" }

{% alert warning %}
Tu catálogo de Shopify está administrado por Shopify. Para actualizar tu catálogo, realiza los cambios directamente en tu tienda Shopify y se sincronizarán automáticamente con Braze. Para eliminar tu catálogo de Shopify, ve a la página del partner de Shopify en Braze y [desactiva la sincronización](#deactivate).
{% endalert %}

## Ejemplos de uso del catálogo de Shopify {#shopify-catalog-use-cases}
...
Utiliza las etiquetas de producto para personalizar mensajes en función de cómo están categorizados tus productos en Shopify. Por ejemplo, puedes enviar una promoción con todos los productos etiquetados como "Summer Sale" a través de una [selección de catálogo]({{site.baseurl}}/catalog_selections), o crear un segmento de usuarios que compraron productos etiquetados como "Premium".

Las etiquetas de producto se almacenan como un campo de matriz en cada elemento del catálogo. Para configurar la sincronización de etiquetas de producto, consulta [Etiquetas de producto de Shopify](#shopify-product-tags).
...
![Modal para seleccionar etiquetas de producto de Shopify, con 15 etiquetas relacionadas con ropa seleccionadas, incluyendo "Women's".]({% image_buster /assets/img/shopify/select_product_tags_womens.png %}){: style="max-width:80%;"}

### Personalización {#personalization}

{% alert note %}
Cuando hagas referencia a etiquetas de producto o colecciones en las selecciones de catálogo, utiliza solo el valor en sí, sin los corchetes de matriz `[]` ni las comillas `""` que aparecen en los datos del catálogo. Por ejemplo, si una etiqueta de producto se muestra como `["Women's"]` en tu catálogo, escribe `Women's` en tu filtro de selección.
{% endalert %}

1. Crea una selección de catálogo que filtre los productos que tengan la etiqueta de producto correspondiente, como "Women's". Solo puedes usar un campo de matriz único dentro de una selección de catálogo individual, y hasta 50 productos en tu selección de catálogo.

![Una selección de catálogo que filtra por etiquetas de producto que tengan el atributo "Women's".]({% image_buster /assets/img/shopify/edit_product_tags_selection.png %})

{: start="2"}
2. En el creador de mensajes, añade la selección donde quieras incluir mediante plantilla los productos de la selección de catálogo etiquetados con "Women's". Por ejemplo, podrías usar un bloque de producto HTML como este:
...
O, si quieres mencionar productos específicos etiquetados con "Women's" en una notificación push, puedes usar la herramienta **Añadir personalización** y especificar los elementos de tu catálogo.
...
![Creador de notificaciones push con una selección de catálogo que incorpora tres elementos con una etiqueta de producto.]({% image_buster /assets/img/shopify/add_personalization_product_tags.png %})

### Segmentación de catálogo (SQL) {#catalog-segmentation-sql}

Utiliza las [extensiones de segmento]({{site.baseurl}}/user_guide/audience/segments/segment_extension) para crear segmentos basados en usuarios que interactuaron con una etiqueta de producto. Por ejemplo, para encontrar usuarios que han interactuado con elementos del catálogo que contienen una etiqueta de producto específica, usa esta consulta:
...
{% endtab %}
{% tab Metafields de producto %}

Utiliza los metafields de producto para personalizar mensajes con detalles de producto personalizados que van más allá de los campos estándar de Shopify. Por ejemplo, incluye instrucciones de cuidado en una confirmación de pedido, muestra el país de origen en un correo electrónico de recomendación, o segmenta a los usuarios que compraron un material específico.

Cada metafield sincronizado se convierte en una columna separada en tu catálogo, con el tipo de datos determinado por el tipo de metafield. Para configurar la sincronización de metafields, consulta [Metafields de producto de Shopify](#shopify-product-metafields).

### Selección de catálogo

1. En Shopify, establece el metafield de producto `seasonal` en los productos relevantes con el valor `summer` (esto es un valor de metafield, no una etiqueta de producto).

![Modal para añadir metafields de producto, incluyendo el metafield seasonal con el valor summer.]({% image_buster /assets/img/shopify/summer_product_metafield.png %}){: style="max-width:80%;"}

{: start="2"}
2. En Braze, habilita la sincronización de metafields y selecciona `custom.seasonal` (o el espacio de nombres y la clave que coincidan con tu metafield de Shopify).

![Modal para seleccionar metafields de producto, con un desplegable expandido que tiene cuatro elementos seleccionados, incluyendo custom.seasonal.]({% image_buster /assets/img/shopify/select_metafields.png %}){: style="max-width:80%;"}

### Personalización

1. Crea una [selección de catálogo]({{site.baseurl}}/catalog_selections) que filtre por metafields que incluyan el valor correspondiente.

![Una selección de catálogo que filtra por metafields que tengan el atributo summer.]({% image_buster /assets/img/shopify/metafields_selection.png %})

{: start="2"}
2. En el creador de mensajes, añade la selección donde quieras incluir mediante plantilla los metafields de producto. Por ejemplo, podrías usar un bloque de producto HTML como este:
...
O, si quieres mencionar productos específicos con un valor de metafield concreto en una notificación push, puedes usar la herramienta **Añadir personalización** y especificar los elementos de tu catálogo.
...
![Creador de notificaciones push con una selección de catálogo que incorpora tres elementos usando una selección basada en metafields.]({% image_buster /assets/img/shopify/add_personalization_metafields.png %})

### Segmentación de catálogo (SQL)

Utiliza las [extensiones de segmento]({{site.baseurl}}/user_guide/audience/segments/segment_extension) para crear segmentos basados en usuarios que interactuaron con un metafield de producto. Por ejemplo, para encontrar usuarios que desencadenaron un evento de ecommerce con un producto cuya matriz de metafield contiene un valor específico, usa esta consulta:
...
Si quieres segmentar clientes que han realizado un pedido con metafields de producto específicos, utiliza una de las siguientes plantillas SQL de extensiones de segmento (todo el tiempo, período específico, primera o última vez que se desencadenó un evento).
...
Utiliza las colecciones de Shopify para incorporar agrupaciones de productos seleccionadas en tus mensajes, las cuales también se usan en tu sitio de Shopify y experiencias de la aplicación. Por ejemplo, puedes destacar "Novedades" en un correo electrónico promocional, realizar ventas cruzadas de "Más vendidos" en un Canvas de carrito abandonado, o dirigirte a usuarios que exploraron una colección de temporada.

### Selección de catálogo

1. En Shopify, crea una colección "New Women's Products - In Stock" con tus productos de mejor rendimiento.

![Lista de colecciones de Shopify, incluyendo "New Women's Products - In Stock".]({% image_buster /assets/img/shopify/shopify_collections.png %})

{: start="2"}
2. En Braze, habilita la sincronización de colecciones y selecciona "Women's Products - In Stock".

![Modal para seleccionar colecciones, con un desplegable expandido que selecciona cuatro colecciones.]({% image_buster /assets/img/shopify/select_collections_id.png %})

{% alert note %}
Para las colecciones de Shopify, debes usar el **ID de colección**, que se encuentra en la URL cuando visualizas la colección. Por ejemplo, una URL como `https://admin.shopify.com/store/se-team-ecommerce/collections/470645342446` tiene el ID de colección `470645342446`.
{% endalert %}

### Personalización

{% alert note %}
Cuando hagas referencia a los ID de colección en las selecciones de catálogo, utiliza solo el valor numérico del ID sin los corchetes de matriz `[]` que aparecen en los datos del catálogo. Por ejemplo, si los ID de colección se muestran como `[123456789012, 987654321098]` en tu catálogo, escribe solo el ID numérico (como `470645342446`) en tu filtro de selección.
{% endalert %}

1. Crea una selección de catálogo llamada "New Women's Products - In Stock" que se filtre con productos que tengan el ID de esa colección. Solo puedes usar un campo de matriz único dentro de una selección de catálogo individual, y hasta 50 productos en tu colección.
 - También puedes crear tus propias selecciones personalizadas filtrando con el campo **Colecciones**.

![Una selección de catálogo que filtra por colecciones que tengan el atributo de ID de colección "470645342446".]({% image_buster /assets/img/shopify/collections_selection.png %})

{: start="2"}
2. En tu mensaje, incluye tu colección mediante plantilla usando la selección creada o haciendo referencia directa a la colección. Por ejemplo, podrías usar un bloque de producto HTML como este:
...
O, si quieres mencionar nuevos productos específicos en una notificación push, puedes usar la herramienta **Añadir personalización** y especificar los elementos de tu catálogo.
...
![Creador de notificaciones push con una selección de catálogo que incorpora tres elementos con una etiqueta de producto.]({% image_buster /assets/img/shopify/add_personalization_collections.png %})

### Segmentación de catálogo (SQL)

Crea un segmento de usuarios que interactuaron con una colección. Utiliza las [extensiones de segmento]({{site.baseurl}}/user_guide/audience/segments/segment_extension) para crear segmentos basados en la pertenencia a una colección. Por ejemplo, para encontrar usuarios que compraron productos de una colección específica en el último año, usa esta consulta:
...
{% alert tip %}
También puedes configurar [notificaciones de caída de precio]({{site.baseurl}}/user_guide/data/activation/catalogs/catalog_triggers/price_drop_notifications) y [notificaciones de vuelta en existencia]({{site.baseurl}}/user_guide/data/activation/catalogs/catalog_triggers/back_in_stock_notifications).<br><br> Ten en cuenta que para cada ejemplo de uso, debes crear un evento personalizado que capture el estado de suscripción de un usuario en tu catálogo. El evento personalizado requiere una propiedad de evento que se asigne al <a href="/docs/partners/ecommerce/shopify/shopify_catalogs#step-2-select-your-product-identifier">SKU o ID de variante de Shopify</a> que hayas seleccionado como parte de la sincronización de productos de Shopify.
{% endalert %}
...
Si la sincronización de productos de Shopify presenta un error, podría deberse a alguno de los siguientes errores. Sigue las instrucciones sobre cómo corregir el problema y resolver la sincronización:

| Error | Motivo | Solución |
| --- | --- | --- |
| Error del servidor | Esto ocurre si hay un error del servidor en el lado de Shopify cuando intentamos sincronizar tus productos. | [Desactiva la sincronización](#deactivate) y vuelve a sincronizar todo tu inventario de productos. |
| SKU duplicado | Esto ocurre si usas el SKU como ID de artículo de catálogo y múltiples variantes comparten el mismo SKU. Cada `item_id` de catálogo debe ser único, por lo que los artículos afectados pueden no sincronizarse, acumular registros de errores o tener información de producto sobrescrita de forma no intencionada. | Audita tu lista completa de productos y variantes en Shopify para asegurarte de que no haya SKU duplicados. Si los hay, actualízalos para que sean SKU únicos solo en tu cuenta de la tienda Shopify. Después de corregir esto, [desactiva la sincronización](#deactivate) y vuelve a sincronizar todo tu inventario de productos. |
| Límite de catálogo excedido | Esto ocurre si excedes el límite de tu catálogo. Braze no podrá finalizar la sincronización ni mantenerla activa debido a la falta de espacio de almacenamiento disponible. | Hay dos soluciones para este problema:<br><br>1. Contacta a tu director de cuentas para mejorar tu nivel y aumentar el límite de tu catálogo. <br><br>2. Libera espacio de almacenamiento eliminando cualquiera de los siguientes elementos:<br>- Artículos de catálogo de otros catálogos<br>- Otros catálogos<br>- Selecciones creadas<br><br> Después de usar cualquiera de las soluciones, la sincronización debe desactivarse y luego volver a sincronizarse. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="Solución de problemas" }

Para obtener detalles sobre la validación de artículos de catálogo, consulta [Solución de problemas]({{site.baseurl}}/api/endpoints/catalogs/catalog_items/asynchronous/post_create_catalog_items_bulk#troubleshooting) en la documentación de la API de catálogos.