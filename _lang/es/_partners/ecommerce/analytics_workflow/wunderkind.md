...
La integración de Wunderkind Signals permite que señales de comportamiento de alta intención —como abandono del carrito de compras, abandono de producto y bajadas de precio— desencadenen recorridos de Canvas en tiempo real en Braze. Wunderkind identifica a los usuarios anónimos en tu sitio web, resuelve su identidad a una dirección de correo electrónico entregable y envía una carga útil de señal estructurada a Braze a través de la API de entrada de Canvas, iniciando automáticamente tus flujos de correo electrónico preconfigurados.
...
Cuando Wunderkind identifica a un usuario anónimo con alta intención y resuelve su identidad, envía una carga útil de señal a Braze usando el endpoint `/canvas/trigger/send`, lo que desencadena el recorrido de Canvas correspondiente para ese usuario en tiempo real.

Para obtener un resumen técnico completo, consulta el [Portal para desarrolladores de Wunderkind](https://developer.wunderkind.co/docs/integration-overview).

## Integración {#integration}

### Paso 1: Crear una clave de API de Braze para Wunderkind {#step-1-create-a-braze-api-key-for-wunderkind}
...
3. Otorga los permisos indicados en [Conceder a Wunderkind acceso a tu cuenta de Braze](https://support.wunderkind.co/hc/en-us/articles/47921719757339-Grant-Wunderkind-Access-to-Your-Braze-Account).
...
{: .reset-td-br-1 .reset-td-br-2 aria-label="Paso 3: Revisar los nuevos activos de Braze" }

### Paso 4: Completar la configuración del Canvas {#step-4-complete-canvas-setup}

Para cada Canvas de Signals, crea tus plantillas de correo electrónico con el editor de arrastrar y soltar de Braze o con HTML.

- Wunderkind rellena los datos de productos y sesiones en el objeto `context` de cada destinatario en `/canvas/trigger/send` en el momento del envío.
- Para obtener instrucciones detalladas sobre cómo utilizar Liquid con esa carga útil en tus plantillas, consulta [Complete Canvas setup](https://support.wunderkind.co/hc/en-us/articles/47155403143963-Complete-Canvas-Setup) en el Centro de ayuda de Wunderkind.

### Paso 5: Revisar la elegibilidad del Canvas {#step-5-review-canvas-eligibility}

Para cada Canvas de Signals, ve a la configuración del **público objetivo** para revisar el público de entrada predeterminado y los criterios de salida de Wunderkind.

- Para asegurarte de que no envías mensajes a tus usuarios con demasiada frecuencia, consulta [Limitación de velocidad centrada en el usuario]({{site.baseurl}}/user_guide/messaging/messaging_fundamentals/frequency_capping#user-centric-rate-limiting).
- Ajusta la configuración para evitar que los usuarios sigan recibiendo mensajes del Canvas después de realizar una compra. Por ejemplo, agrega la excepción **Make Purchase**.
- Algunos Canvas de Signals están preconfigurados con filtros de atributos personalizados para que los usuarios reciban el mensaje con la intención más alta posible.
- Consulta [Review Canvas eligibility](https://support.wunderkind.co/hc/en-us/articles/47156586245787-Review-Canvas-Eligibility) en el Centro de ayuda de Wunderkind para obtener detalles sobre la elegibilidad y prioridad del Canvas.

### Paso 6: Probar y lanzar {#step-6-test-and-launch}

Wunderkind realiza una prueba de calidad de extremo a extremo antes del lanzamiento:
...
- Consulta [Test and launch Signals for Braze](https://support.wunderkind.co/hc/en-us/articles/47156667414171-Test-and-Launch-Signals-for-Braze) en el Centro de ayuda de Wunderkind para obtener instrucciones sobre cómo previsualizar plantillas con productos de prueba de Wunderkind.

Cuando la prueba de calidad se aprueba, tu administrador de implementación de Wunderkind coordina el lanzamiento a producción con tu equipo.

## Carga útil de contexto de Canvas {#canvas-context-payload}

Wunderkind admite seis tipos de señal. Cada uno entrega un conjunto distinto de claves y valores dentro del objeto [`context`]({{site.baseurl}}/api/objects_filters/context_object) para ese destinatario en `/canvas/trigger/send` (consulta [Enviar mensajes de Canvas mediante entrega desencadenada por API]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_triggered_canvases)). El campo `WkPurpose` identifica el tipo de señal dentro de esa carga útil.

### Campos comunes (todos los tipos de Canvas) {#canvas-types-table}

| Propiedad | Tipo | Descripción |
| -------- | ---- | ----------- |
| `Origin` | Cadena | Siempre `"wunderkind"` |
| `DataOnly` | Cadena | Siempre `"Y"` — indica que Wunderkind actúa solo como capa de datos; Braze ejecuta el envío |
| `UserType` | Cadena | `"prospect"` o `"customer"` |
| `WkChannel` | Cadena | Siempre `"email"` para esta integración |
| `WkPurpose` | Cadena | Identificador del tipo de señal (consulta los valores por Canvas en esta sección) |
| `WKCouponCode` | Cadena | Código de cupón, si aplica (cadena vacía si no se usa) |
...
| Propiedad | Tipo | Descripción |
| -------- | ---- | ----------- |
| `WkCopy` | Cadena | Nombre del producto |
| `WkId` | Cadena | ID de producto |
| `WkImageUrl` | Cadena | URL de imagen del producto |
| `WkUrl` | Cadena | URL de la página de detalle del producto |
| `WkPrice` | Cadena | Precio original (solo Canvas de bajada de precio) |
| `WKSalePrice` | Cadena | Precio de oferta (solo Canvas de bajada de precio) |
| `WkQuantity` | Cadena | Unidades restantes (solo Canvas de bajo inventario) |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="Campos de artículo de producto" }

### Campos específicos de Canvas y valores de `WkPurpose` {#canvas-specific-fields-and-wkpurpose-values}
...
| De vuelta en inventario | `"back in stock"` | — |
| Bajada de precio | `"price drop"` | `WkPrice`, `WKSalePrice` en cada artículo |
| Bajo inventario | `"low stock"` | `WkQuantity` en cada artículo |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="Campos específicos de Canvas y valores de WkPurpose" }

### Cargas útiles de ejemplo {#example-payloads}

Cada objeto en `recipients` debe incluir exactamente uno de `external_user_id`, `user_alias`, `braze_id` o `email`. Para más información, consulta el [objeto Recipients]({{site.baseurl}}/api/objects_filters/recipient_object).

{% alert note %}
Cada ejemplo usa **un** identificador de destinatario de Braze. Los primeros seis usan solo `user_alias`; el último usa `email` con [`prioritization`]({{site.baseurl}}/api/endpoints/user_data/post_user_identify#identifying-users-by-email-addresses-and-phone-numbers) únicamente. El JSON de ejemplo omite la clave `WkChannel` dentro de `context` para que las herramientas de revisión no confundan su valor (`"email"`) con el campo `email` de destinatario de Braze. En producción, incluye `"WkChannel": "email"` en `context` como se documenta en la [tabla de campos comunes (todos los tipos de Canvas)](#canvas-types-table).
{% endalert %}
...
{% details Ejemplo de carga útil de abandono del carrito de compras %}
```json
{
...
{% details Ejemplo de carga útil de abandono de producto %}
```json
{
  "canvas_id": "<your_canvas_id>",
  "recipients": [
...
{% details Ejemplo de carga útil de resumen de categoría %}
```json
{
  "canvas_id": "<your_canvas_id>",
  "recipients": [
...
{% details Ejemplo de carga útil de vuelta en inventario %}
```json
{
  "canvas_id": "<your_canvas_id>",
  "recipients": [
...
{% details Ejemplo de carga útil de bajada de precio %}
```json
{
  "canvas_id": "<your_canvas_id>",
  "recipients": [
...
{% details Ejemplo de carga útil de bajo inventario %}
```json
{
  "canvas_id": "<your_canvas_id>",
  "recipients": [
...
{% details Ejemplo de identificador de correo electrónico (alternativo) %}
Si desencadenas el Canvas con el campo `email` de Braze en lugar de `user_alias`, el destinatario debe incluir solo `email` y `prioritization` (consulta [Enviar mensajes de Canvas mediante entrega desencadenada por API]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_triggered_canvases)). El objeto `context` coincide con los demás ejemplos.
...
Las siguientes métricas estarán disponibles próximamente en el panel de informes de Wunderkind:
...
{: .reset-td-br-1 .reset-td-br-2 aria-label="Informes" }
...
- [Centro de ayuda de Wunderkind — Descripción general de Signals para Braze](https://support.wunderkind.co/hc/en-us/articles/47156898436891-Signals-for-Braze-Overview)
- [Portal para desarrolladores de Wunderkind — Descripción general de la integración](https://developer.wunderkind.co/docs/integration-overview)
- [Enviar mensajes de Canvas mediante entrega activada por API]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_triggered_canvases)
