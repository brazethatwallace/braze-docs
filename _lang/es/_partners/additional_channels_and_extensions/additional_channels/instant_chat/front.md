...
> La integración de Front te permite aprovechar la Transformación de datos de Braze y los webhooks de cada plataforma para establecer un canal servicio de mensajes cortos conversacional bidireccional.

El webhook entrante de Front contendrá una carga útil que incluye el mensaje enviado por el agente en vivo. Será necesario reformatear la solicitud antes de que pueda ser aceptada por los endpoints de Braze. La plantilla de Transformación de datos de Front reformateará la carga útil y escribirá un evento personalizado en el perfil de usuario titulado **Outbound servicio de mensajes cortos Sent**, pasando el cuerpo del mensaje como una propiedad del evento.
...
| Una clave de API REST or transferencia de estado representacional de Front | Se utilizará una clave de API REST or transferencia de estado representacional de Front para realizar una solicitud de webhook saliente de Braze a Front. |
{: .reset-td-br-1 .reset-td-br-2 aria-label="Requisitos previos" }

## Ejemplos {#use-cases}

- Agiliza tu proceso de generación de clientes potenciales utilizando la mensajería servicio de mensajes cortos automatizada de Braze para identificar las preferencias de los usuarios y permitir a los agentes de ventas en vivo realizar el seguimiento y cerrar las ventas.
- Reactiva a los clientes que abandonaron sus carritos de la compra impulsando las conversiones de ventas mediante respuestas automatizadas por servicio de mensajes cortos y asistencia por chat en vivo.
...
### Paso 2: Crear una campaña de servicio de mensajes cortos salientes {#step-2-create-an-outbound-sms-campaign}

A continuación, crearás una campaña de servicio de mensajes cortos que escuchará los webhooks de Front y enviará una respuesta personalizada por servicio de mensajes cortos a tus clientes.
...
Para el tipo de entrega, selecciona **Entrega basada en acciones**; a continuación, para el desencadenante del evento personalizado, selecciona **Outbound servicio de mensajes cortos Sent**.
...
En el campo del endpoint de la API de salida, introduce la URL de webhook de Transformación de datos [que creaste anteriormente](#step-1-set-up-a-data-transformation-in-braze). Todos los mensajes salientes de los agentes en vivo de tu nuevo canal de Braze se enviarán aquí. Este canal también proporciona una URL de endpoint para que Braze reenvíe los mensajes servicio de mensajes cortos en el campo **Incoming URL**.
...
### Paso 5: Configurar el reenvío de servicio de mensajes cortos entrantes {#step-5-set-up-inbound-sms-forwarding}

A continuación, crearás dos nuevas campañas webhook en Braze para poder reenviar los servicio de mensajes cortos entrantes de los clientes al buzón de entrada de Front.

| Número | Propósito |
|---|---|
| Campaña webhook 1 | Señala a Front que se está solicitando una conversación de chat en vivo. |
| Campaña webhook 2 | Reenvía todas las respuestas servicio de mensajes cortos conversacionales enviadas por el cliente al buzón de entrada de Front. |
{: .reset-td-br-1 .reset-td-br-2 aria-label="Paso 5: Configurar el reenvío de servicio de mensajes cortos entrantes" }

#### Paso 5.1: Crear una categoría de palabras clave servicio de mensajes cortos {#step-51-create-an-sms-keyword-category}

En el panel de Braze, ve a **Audiencia**, elige tu **grupo de suscripción servicio de mensajes cortos** y, a continuación, selecciona **Añadir palabra clave personalizada**. Para crear una categoría de palabras clave servicio de mensajes cortos exclusiva para Front, rellena los siguientes campos.
...
{: .reset-td-br-1 .reset-td-br-2 aria-label="Paso 5.1: Crear una categoría de palabras clave servicio de mensajes cortos" }
...
Para **Programar entrega**, selecciona **Entrega basada en acciones** y, a continuación, elige **Enviar un mensaje servicio de mensajes cortos entrante** para tu tipo de desencadenante. Añade también el grupo de suscripción servicio de mensajes cortos y la categoría de palabras clave que [configuraste anteriormente](#step-51-create-an-sms-keyword-category).
...
Para **Programar entrega**, establece el **desencadenante basado en acciones** y el **grupo de suscripción servicio de mensajes cortos** igual que [en tu primera entrega](#step-53-schedule-the-first-delivery). Sin embargo, para la **categoría de palabras clave**, elige **Other**.
...
Tu campaña webhook ahora puede reenviar las respuestas servicio de mensajes cortos entrantes de tus clientes. Para filtrar las respuestas servicio de mensajes cortos de modo que solo se reenvíen los mensajes de los chats en vivo, añade el filtro de segmentación **Last Received Message From Specific Campaign** al paso **Público objetivo**.
...
1. En **Campaign**, selecciona la campaña de servicio de mensajes cortos [que creaste anteriormente](#step-2-create-an-outbound-sms-campaign).
...
Los archivos de imagen en Front no se mostrarán en los mensajes servicio de mensajes cortos enviados desde Braze.
