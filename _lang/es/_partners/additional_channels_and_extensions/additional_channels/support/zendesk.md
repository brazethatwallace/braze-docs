...
> [Zendesk Support Suite](https://www.zendesk.com/support-suite/) (ZSS) ofrece a las empresas la posibilidad de mantener conversaciones naturales con sus clientes a través del soporte omnicanal mediante correo electrónico, webchat, voz o aplicaciones de mensajería social. Zendesk ofrece un sistema de ticketing optimizado que valora el seguimiento y la priorización de las interacciones, lo que permite a las empresas tener una visión histórica unificada de sus clientes.
...
| Token de la API de Zendesk | Se necesita un [token de API](https://support.zendesk.com/hc/en-us/articles/226022787-Generating-a-new-API-token-\) de Zendesk para enviar solicitudes desde Braze al endpoint de tickets de Zendesk. |
| Identificador común (recomendado) | Se recomienda un [identificador común](#common-identifier) entre Braze y Zendesk. |
| Clave de API de Braze | Se necesita una clave de API de Braze para enviar solicitudes desde Zendesk a un endpoint de Braze. Asegúrate de que la clave de API que utilizas tiene los permisos correctos para el endpoint de Braze que utiliza tu webhook de Zendesk. |
...
- **Campaigns:** Ve a la página **Campaigns** en el panel de Braze. Haz clic en **Crear Campaign** y selecciona **Webhook**.
- **Canvas:** Desde un Canvas nuevo o existente, crea un paso completo o de mensaje en el constructor de Canvas. A continuación, haz clic en **Mensajes** y selecciona **Webhook** en las opciones de mensaje.
...
Otros casos de uso pueden gestionarse a través de las [API de soporte de Zendesk](https://developer.zendesk.com/rest_api/docs/support/introduction), que cambiarían en consecuencia el endpoint `/api/v2/` al final de la URL del webhook.
...
![Configuración del webhook de Braze con el encabezado de autorización de Zendesk y el método POST configurados.]({% image_buster /assets/img_archive/zendesk_step1.gif %}){: style="max-width:70%;"}
...
Previsualiza tu solicitud en el panel de **Vista previa** o navega hasta la pestaña **Prueba**, donde puedes seleccionar un usuario aleatorio o un usuario existente, o personalizar el tuyo propio para probar tu webhook.
...
3. Selecciona **Trigger** o **Automation** y haz clic en **Next**.<br>![Pantalla de creación de webhook de Zendesk con las opciones Trigger y Automation.]({% image_buster /assets/img_archive/zendesk2.png %}){: style="max-width:70%;"}<br><br>
4. Proporciona la siguiente información en tu webhook:
- Introduce un nombre y una descripción para el webhook.
- Introduce la URL del endpoint de Braze que utilizará tu webhook. {% raw %}En nuestro ejemplo utilizaremos `https://{{instance_url}}/users/track`.{% endraw %}
- Selecciona POST como método de solicitud del webhook y establece el formato de solicitud en JSON.
- Selecciona el método de autenticación de token de portador para el webhook y proporciona tu [clave de API de Braze]({{site.baseurl}}/api/basics#creating-rest-api-keys).
  - Asegúrate de que la clave de API que utilizas tiene los [permisos correctos]({{site.baseurl}}/api/basics#rest-api-key-permissions) para el endpoint de Braze que utiliza tu webhook.<br><br>
...
El siguiente ejemplo utiliza un desencadenador para invocar el webhook cuando el estado de un caso de soporte haya cambiado a "Resuelto" o "Cerrado".
...
4. Selecciona **Add condition** para configurar qué condiciones deben desencadenar el webhook. Por ejemplo, "Status category changed to closed" o "Status category changed to solved".![Constructor de condiciones de desencadenador de Zendesk mostrando condiciones de categoría de estado.]({% image_buster /assets/img_archive/zendesk1.png %}){: style="max-width:70%;"}<br><br>
5. Selecciona **Add action**, elige **Notify active webhook** y selecciona en el desplegable el webhook creado en el paso anterior.<br><br>
6. Define el cuerpo JSON para que se ajuste a tu endpoint de Braze, usando marcadores de posición de variables de Zendesk para rellenar dinámicamente los campos relevantes.<br>![Editor de carga útil de acción de webhook de Zendesk con variables de cuerpo JSON.]({% image_buster /assets/img_archive/zendesk3.png %}){: style="max-width:70%;"}<br><br>
