...
| Clave de API REST or transferencia de estado representacional de Braze | Se necesita una clave de API REST or transferencia de estado representacional de Braze con los permisos `messages.send`, `sends.id.create` y `campaigns.details`. Se puede crear en el panel de Braze desde **Settings** > **API Keys**. <br><br>Tu equipo de cuentas de Movable Ink te proporcionará directamente más instrucciones de configuración. Consulta la sección [Integración](#integration).|
