...
> [Foursquare](https://foursquare.com/) es una plataforma de datos de ubicación que proporciona segmentación por datos de ubicación en tus Campaigns de Braze. Utiliza el SDK Pilgrim de Foursquare en aplicaciones iOS y Android para desencadenar eventos en tiempo real basados en la ubicación, lo que te permitirá aprovechar las potentes capacidades de geosegmentación de Foursquare para enviar mensajes relevantes y personalizados con Braze.
...
Para integrar las dos plataformas, debes integrar los dos SDK y mapear los campos de usuario coincidentes. Después de integrar el SDK de Pilgrim, recibirás eventos de ubicación en el dispositivo o a través de un webhook.
...
Busca el espacio de trabajo y los ID de la aplicación en la consola para desarrolladores de Braze. A continuación, introduce tu clave de API REST de Braze y los ID de la aplicación en la consola de Foursquare Pilgrim.
...
Una vez configurada la integración, puedes configurar una Campaign o Canvas que responda a los eventos de ubicación generados por el SDK de Pilgrim. Esta ruta de integración es ideal para mensajería en tiempo real justo después de que los usuarios entren en un lugar de interés, o para comunicaciones de seguimiento diferidas después de que se hayan ido, como una nota de agradecimiento o un recordatorio.
...
Para reorientar a tus usuarios, utiliza el SDK de Pilgrim para establecer un atributo personalizado `last_location` en los perfiles de usuario de tus usuarios de Braze. A continuación, puedes usar la comparación `matches regex` para reorientar a los usuarios que visitaron un lugar concreto en el mundo real, por ejemplo, segmentando a todos los usuarios que estuvieron recientemente en una pizzería.
