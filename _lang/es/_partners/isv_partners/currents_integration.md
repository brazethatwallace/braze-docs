...
> Aprende a integrar un conector de Currents personalizado, para que puedas obtener datos de eventos de Braze en tiempo real, lo que permite análisis, informes y automatización más personalizados.

## Requisitos previos {#prerequisites}

Para integrar un conector de Currents personalizado en Braze, tendrás que proporcionar una URL de punto de conexión y un [token de autenticación opcional](#authentication).

Además, si tienes más de un grupo de aplicaciones en Braze, tendrás que configurar un conector de Currents personalizado para cada grupo. Sin embargo, puedes dirigir todos los grupos de aplicaciones al mismo punto de conexión, o a un punto de conexión con un parámetro adicional `GET`, como `your_app_group_key="Brand A"`.

## Evitar la pérdida de datos {#preventing-data-loss}

### Control de errores {#error-monitoring}

Para evitar la pérdida de datos y la interrupción del servicio, es esencial que supervises tus puntos de conexión en todo momento y que intentes solucionar los errores graves o el tiempo de inactividad en un plazo de 24 horas.

Para la mayoría de los tipos de error (como errores del servidor, errores de conexión a la red, etc.), Braze continuará poniendo en cola y reintentando las transmisiones de eventos durante un máximo de 24 horas. Pasado ese tiempo, se descartarán los eventos no transmitidos. Los conectores con tasas de error o tiempo de actividad constantemente deficientes se suspenderán automáticamente.

### Resiliencia ante cambios {#change-resilience}

Ocasionalmente, realizaremos cambios no disruptivos en los esquemas de Braze Currents. Los cambios no disruptivos son nuevas columnas anulables o tipos de eventos.
...
{% alert tip %}
Para consultar la lista completa de esquemas de eventos de Currents, consulta [Eventos de interacción con mensajes]({{site.baseurl}}/user_guide/data/distribution/braze_currents/event_glossary/message_engagement_events/).
{% endalert %}

## Procesamiento por lotes y serialización {#batching-and-serialization}

El formato de datos de destino es JSON sobre HTTPS. De forma predeterminada, los eventos se agrupan en lotes de 100 en función de lo siguiente:
...
- **Duración de un evento:** Normalmente, los eventos no se ponen en cola si un evento tiene más de 15 minutos de antigüedad. Cada tipo de evento tiene una cola independiente, por lo que la latencia puede variar según el tipo de evento.

A continuación, los eventos se envían al punto de conexión como una matriz JSON de todos los eventos con el siguiente formato:
...
Además, su estructura varía ligeramente de la estructura plana de [Eventos de interacción con mensajes]({{site.baseurl}}/user_guide/data/distribution/braze_currents/event_glossary/message_engagement_events/). En concreto, contienen dos subobjetos:

| Nombre | Descripción |
|----|-----------|
| `"user"` | Contiene propiedades de usuario como `user_id`, `external_user_id`, `device_id` y `timezone`. |
| `"properties"` | Contiene atributos de un evento, como el `app/campaign/canvas/platform` al que se aplica. |
...
### Eventos asociados a Canvas {#canvas-associated-events}
...
A continuación se muestran algunos ejemplos de cargas útiles para otros eventos que no están asociados ni a Campaigns ni a Canvas:
...
Los tokens de autenticación en tu carga útil son opcionales. Se pueden pasar a través de un encabezado HTTP `Authorization` utilizando el esquema de autorización `Bearer`, tal y como se especifica en [RFC 6750](https://tools.ietf.org/html/rfc6750#section-2.1). Aunque es opcional, si se pasa un token de autenticación, Braze siempre lo validará primero&#8212;incluso si no hay eventos en la carga útil.
...
Además, si el encabezado `Authorization` está presente, se construirá con el siguiente formato:
...
{% alert note %}
En el futuro, podemos utilizar los encabezados `Authorization` para implementar un esquema de autorización personalizado, de par clave-valor, que sea único para Braze. Esto se adheriría a la especificación [RFC 7235](https://tools.ietf.org/html/rfc7235), que es como algunas empresas implementan sus esquemas de autenticación, como Amazon Web Services (AWS).
{% endalert %}
...
La versión siempre será `1`, ya que no esperamos incrementar este número muy a menudo, si es que lo hacemos alguna vez.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Los datos de eventos se reenviarán siguiendo un patrón de retirada exponencial con jitter. Si los datos no se envían correctamente en 24 horas, se descartarán.</td>
    </tr>
    <tr>
      <td><code>400</code></td>
      <td>Error del lado del cliente</td>
      <td>El conector envió al menos un evento malformado. Los datos del evento se dividirán en lotes de tamaño 1 y se volverán a enviar. Cualquier evento de estos lotes de tamaño 1 que reciba otra respuesta <code>400</code> se eliminará de forma permanente. Debes informar de los casos repetidos.</td>
    </tr>
    <tr>
      <td><code>401</code></td>
      <td>No autorizado</td>
    ...
    <tr>
      ...
      <td>Indica un límite de velocidad. Los datos de eventos se reenviarán siguiendo un patrón de retirada exponencial con jitter. Si no se envían correctamente en 24 horas, se descartarán.</td>
    </tr>
  </tbody>
</table>
