...
* La URL de la REST API es correcta y no tiene barra diagonal al final.
...
Cada servicio de destino en DinMo sigue el mismo flujo de trabajo general: crear un destino de Braze, construir un modelo o segmento de DinMo y luego crear una activación para enviar datos a Braze. Para una guía paso a paso sobre activaciones, consulta [Servicios de destino de DinMo para Braze](https://docs.dinmo.io/integrations/destination-platforms/braze).
...
| [Sincronizar estados de suscripción](https://docs.dinmo.io/integrations/destination-platforms/braze/synchronize-subscription-statuses) | Suscribe o cancela la suscripción de usuarios en un grupo de suscripción de Braze según la pertenencia al segmento de DinMo. |
...
Utiliza este servicio de destino para actualizar atributos en perfiles de usuario existentes de Braze y, opcionalmente, insertar nuevos usuarios.
...
| MIRROR | Inserta, actualiza y elimina registros en Braze para reflejar la fuente. Requiere compatibilidad del conector con operaciones de eliminación. |
{: .reset-td-br-1 .reset-td-br-2 aria-label="Modos de sincronización de atributos de usuario" }

{% alert warning %}
El modo de sincronización espejo elimina permanentemente los registros de Braze cuando ya no están presentes en la fuente de DinMo. Utiliza el modo espejo solo cuando tu almacén de datos sea la única fuente de verdad y las eliminaciones sean intencionales. Valida las reglas de eliminación antes de ejecutar sincronizaciones espejo en producción.
{% endalert %}

### Enviar eventos de seguimiento {#send-track-events}

Utiliza este servicio de destino para enviar eventos personalizados o eventos de compra desde un modelo de eventos o segmento de DinMo a Braze. DinMo trata los eventos personalizados y las compras como servicios de destino separados porque Braze utiliza API diferentes para cada tipo.

Cada registro del modelo representa un único tipo de evento (por ejemplo, `Purchase`). DinMo envía solo eventos nuevos en cada ejecución de activación y no actualiza eventos enviados previamente.
...
2. Mapea los campos obligatorios:
   * **Event time**: Marca de tiempo de cuándo ocurrió el evento
   * **External ID**: ID externo del usuario asociado al evento
3. Mapea las propiedades opcionales del evento a nombres de atributos de Braze.
4. Configura la planificación de la frecuencia con la que se envían nuevos eventos a Braze.

### Sincronizar estados de suscripción {#synchronize-subscription-statuses}

Utiliza este servicio de destino para mantener un grupo de suscripción de Braze alineado con un segmento o modelo de DinMo.
...
Durante la configuración de la activación, introduce el ID exacto del grupo de suscripción de Braze. Para sincronizar múltiples grupos de suscripción, crea una activación por grupo.

Cuando se ejecuta la activación:

* Si los usuarios ya existen en Braze, los usuarios que entran al segmento de DinMo se marcan como suscritos al grupo de suscripción de destino.
* Los usuarios que salen del segmento de DinMo se marcan como dados de baja del grupo de suscripción.
