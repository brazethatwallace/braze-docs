...
| Clave de API REST de Braze | Una clave de API REST de Braze con los [permisos](#api-key-permissions) necesarios para los servicios de destino que planeas utilizar. Puedes crearla en el panel de Braze desde **Configuración** > **Claves de API**. |
...
| URL del panel de Braze | La URL del panel de Braze de tu instancia (por ejemplo, `https://dashboard.iad-01.braze.com`). Para más información, consulta [Endpoints de SDK disponibles]({{site.baseurl}}/user_guide/administer/personal/sdk_endpoints). |
| Almacén de datos y modelo de datos | Antes de comenzar la integración, conecta tu almacén de datos en DinMo y define un modelo o Segment para los datos que deseas sincronizar con Braze. Para más información, consulta la [guía de integración de DinMo con Braze](https://docs.dinmo.io/integrations/destination-platforms/braze). |
...
* Mantener la membresía de los grupos de suscripción de Braze alineada con los Segments de audiencia definidos en DinMo.
* Exportar Segments de DinMo como atributos de usuario de Braze y crear Segments de Braze a partir de esos atributos.

## Permisos de la clave de API {#api-key-permissions}

Otorga los siguientes permisos en tu clave de API REST de Braze según los servicios de destino que utilices:
...
### Paso 1: Configurar el destino de Braze en DinMo {#step-1-configure-the-braze-destination-in-dinmo}

1. En DinMo, ve a **Destinations** en la navegación lateral.
...
Después de guardar el destino, DinMo realiza una llamada de prueba (por ejemplo, `users.track`) para confirmar que tu clave de API y tu endpoint funcionan.

Si la validación falla, confirma lo siguiente:

* La URL de la REST API es correcta y no tiene una barra diagonal al final.
...
Cada servicio de destino en DinMo sigue el mismo flujo de trabajo general: crear un destino de Braze, construir un modelo o segmento de DinMo y luego crear una activación para enviar datos a Braze. Para obtener orientación paso a paso sobre la activación, consulta [Servicios de destino de DinMo para Braze](https://docs.dinmo.io/integrations/destination-platforms/braze).

Los siguientes servicios de destino están disponibles:

| Servicio de destino | Descripción |
| --- | --- |
| [Sincronizar atributos de usuario](https://docs.dinmo.io/integrations/destination-platforms/braze/synchronize-users-attributes) | Actualiza los atributos del perfil de usuario en Braze y, opcionalmente, inserta nuevos usuarios. |
...
| [Sincronizar estados de suscripción](https://docs.dinmo.io/integrations/destination-platforms/braze/synchronize-subscription-statuses) | Suscribe o cancela la suscripción de usuarios en un grupo de suscripción de Braze en función de la pertenencia a un segmento de DinMo. |
| [Exportar listas de usuarios](https://docs.dinmo.io/integrations/destination-platforms/braze/export-user-lists) | Sincroniza la pertenencia a un segmento con un atributo de usuario de Braze para su uso en la segmentación de Braze. |
...
Usa este servicio de destino para actualizar atributos en perfiles de usuario existentes de Braze y, opcionalmente, insertar nuevos usuarios.

Cuando ejecutas una activación:

* Si habilitas el modo de inserción, los nuevos usuarios del modelo se crean en Braze (comportamiento UPSERT).
* Los valores de atributos que cambiaron desde la última activación se actualizan en Braze.
...
Durante la configuración de la activación, mapea el campo de tu modelo de DinMo que corresponde al [ID externo]({{site.baseurl}}/developer_guide/platform_integration_guides/web/analytics/setting_user_ids) o ID de Braze del usuario. Mapea cada campo de DinMo al nombre exacto del atributo en Braze. Si un atributo no existe en Braze, DinMo lo crea.
...
| --- | --- |
| UPDATE | Actualiza los registros modificados de usuarios que ya existen en Braze. No inserta ni elimina registros. |
| UPSERT | Inserta nuevos registros y actualiza los registros modificados. No elimina registros. |
...
{% alert warning %}
El modo de sincronización MIRROR elimina permanentemente los registros de Braze cuando ya no están presentes en la fuente de DinMo. Usa el modo MIRROR solo cuando tu almacén de datos sea la única fuente de verdad y las eliminaciones sean intencionales. Valida las reglas de eliminación antes de ejecutar sincronizaciones MIRROR en producción.
{% endalert %}

### Enviar eventos de seguimiento {#send-track-events}

Usa este servicio de destino para enviar eventos personalizados o eventos de compra desde un modelo de eventos o segmento de DinMo a Braze. DinMo trata los eventos personalizados y las compras como servicios de destino separados porque Braze utiliza API diferentes para cada tipo.

Cada registro en el modelo representa un único tipo de evento (por ejemplo, `Purchase`). DinMo envía solo los eventos nuevos en cada ejecución de activación y no actualiza los eventos enviados previamente.
...
2. Mapea los campos obligatorios:
   * **Hora del evento**: Marca de tiempo de cuándo ocurrió el evento
   * **ID externo**: ID externo del usuario asociado al evento
3. Mapea las propiedades del evento opcionales a nombres de atributos de Braze.
4. Configura la programación de la frecuencia con la que se envían nuevos eventos a Braze.

### Sincronizar estados de suscripción {#synchronize-subscription-statuses}

Usa este servicio de destino para mantener un grupo de suscripción de Braze alineado con un segmento o modelo de DinMo.

Antes de activar este servicio:

1. Crea el grupo de suscripción de destino (SMS o correo electrónico) en Braze.
2. Construye un modelo o segmento de DinMo que contenga los usuarios que deben pertenecer a ese grupo de suscripción.
...
* Si los usuarios ya existen en Braze, los usuarios que entran en el segmento de DinMo se marcan como suscritos al grupo de suscripción de destino.
* Los usuarios que salen del segmento de DinMo se marcan como no suscritos del grupo de suscripción.

DinMo no modifica a los usuarios que nunca formaron parte del segmento y no crea nuevos usuarios de Braze en este servicio de destino.

### Exportar listas de usuarios {#export-user-lists}

Usa este servicio de destino para representar un segmento de DinMo como un atributo de usuario de Braze. Debido a una limitación de Braze, DinMo no crea una lista de Braze directamente. En su lugar, establece un atributo de usuario en `true` para los usuarios del segmento y en `false` para los usuarios que salen del segmento.

Durante la configuración de la activación, especifica el nombre de la audiencia. DinMo usa este nombre como el atributo de Braze (los espacios se reemplazan con guiones bajos). Confirma que no exista ya un atributo con el mismo nombre en Braze. Mapea el campo de DinMo que corresponde al ID externo del usuario.
