...
* **Celebración de ascenso de nivel:** Cuando un miembro sube de nivel en un programa de fidelización en Convercus, desencadena un Canvas personalizado en Braze con un mensaje de bienvenida, un beneficio exclusivo del nivel y el nuevo nivel y saldo de puntos del miembro.
...
* **Recuperación de miembros inactivos:** Para miembros inactivos, haz que Braze asigne un cupón personalizado en Convercus a través de un webhook y lo entregue por correo electrónico, push y mensajes dentro de la aplicación.
...
| --- | --- |
| Una cuenta de Convercus | Un programa Convercus activo. Ponte en contacto con tu director de cuentas de Convercus si aún no eres cliente. |
...
Necesitas un identificador de usuario coherente entre los sistemas: el valor utilizado como `external_id` (o el tipo de identificador elegido) en Braze debe coincidir con el identificador de miembro correspondiente en Convercus. De lo contrario, los eventos no se atribuyen al perfil correcto.
...
1. Configurar la conexión con Braze completando el formulario de integración:
   ...
   | Tipo de identificador | `external_id` o `user_alias`. Determina cómo se asocian los miembros de Convercus con los perfiles de usuario de Braze. |
...
2. Crear una clave de API para las llamadas entrantes. Crea una credencial `X-Convercus-Key` por programa. La clave sin procesar se muestra una sola vez al crearla, con el prefijo `cvc_` (formato: `cvc_<base64url>`). Almacénala en Braze cuando configures las Campaigns de webhook y los bloques de contenido conectado en el paso 2. Las claves se pueden revocar en cualquier momento desde la misma tarjeta; la revocación surte efecto de inmediato.

Después de guardar la conexión con Braze, Convercus comienza a transmitir de inmediato los eventos de fidelización de ese programa a Braze. No se requiere ninguna configuración de infraestructura adicional.

{% alert note %}
Cada programa de Convercus se configura de forma independiente. Un solo tenant de Convercus puede conectar diferentes programas a diferentes espacios de trabajo de Braze, cada uno con su propia clave de API.
{% endalert %}
...
| Registrar puntos de acumulación/canje | `POST /members/{accountId}/bookings` — crea un `EARNBOOKING` o `BURNBOOKING` en la cuenta de un miembro. Devuelve `{ "bookingId": "..." }`. |
| Sincronizar preferencias de suscripción de correo electrónico | `POST /subscriptions/email` — establece las adhesiones voluntarias del miembro como `allowed` o `declined`. Los canales de adhesión voluntaria se resuelven como solicitud `optins` > `defaultOptins`. Devuelve `200` (todo correcto), `207` (parcial — consulta `succeeded` / `failed`), o `400` (adhesiones voluntarias desconocidas o ninguna configurada). |
...
| --- | --- |
| `200` | Éxito. |
| `207` | Multi-Status — solo para el webhook de suscripción de correo electrónico, cuando algunas membresías se actualizaron y otras fallaron. |
...
{% alert warning %}
Las respuestas 5xx no son seguras para reintentar sin confirmar el éxito; estas operaciones no son idempotentes, y los reintentos pueden duplicar la asignación de cupones o la acreditación de puntos. Desactiva el reintento automático de Braze en 5xx para estos webhooks, o configura un número máximo de reintentos muy bajo.
{% endalert %}

### Paso 3: Verificar los datos en Braze {#step-3-verify-data-in-braze}

1. Desencadena un evento de fidelización en Convercus, por ejemplo, un cambio de nivel de estado, una transacción de puntos o un canje de cupón.
2. Abre el usuario correspondiente en Braze y confirma que el atributo personalizado, evento personalizado o compra esperados aparecen en el perfil. Los usuarios se asocian por `external_id` (o el tipo de identificador elegido en el paso 1).
...
## Usa Convercus con Braze {#use-convercus-with-braze}

### Paso 1: Personaliza mensajes con datos de fidelización sincronizados {#step-1-personalize-messages-with-synced-loyalty-data}

Una vez que la integración está en vivo, los eventos de Convercus llegan a cada perfil de usuario en Braze a través del endpoint [`/users/track`]({{site.baseurl}}/api/endpoints/user_data/post_user_track) y se pueden usar como cualquier otro dato nativo:
...
4. Usa eventos `purchase` transmitidos desde Convercus para impulsar recorridos basados en productos (reposición, venta cruzada por categoría, solicitudes de reseña post-compra).

#### Atributos personalizados {#custom-attributes}

| Atributo | Descripción |
| --- | --- |
| `convercus_account_id` | El ID de cuenta Convercus del miembro, único dentro de un programa Convercus / espacio de trabajo de Braze. |
| `convercus_user_id` | El ID de usuario Convercus que identifica a la persona subyacente en múltiples programas Convercus. |
...
| `convercus_status_level` | El nivel o estatus actual del miembro. |
...
{% alert note %}
Dentro de un espacio de trabajo de Braze, los miembros se identifican de forma única mediante `convercus_account_id`. `convercus_user_id` identifica a la persona subyacente en múltiples programas Convercus y se proporciona para análisis entre programas; para segmentación dentro de Braze, usa `convercus_account_id`.
{% endalert %}

**Mapeado de `email_subscribe`**

| Estado en Convercus | `email_subscribe` en Braze |
| --- | --- |
| Entrada `allowedOptins` para `email consent` o `newsletter` | `opted_in` |
| Entrada `declinedOptIns` para esos canales (y sin entrada permitida) | `unsubscribed` |
| Sin registro en ningún sentido | `subscribed` (valor neutral predeterminado de Braze) |
{: .reset-td-br-1 .reset-td-br-2 aria-label="Atributos personalizados" }
...
| --- | --- |
| `convercus_account_created` | Se crea una nueva cuenta en Convercus. |
...
| `convercus_membership_changed` | Cambian los datos de una membresía. |
| `convercus_membership_optins_changed` | Cambian las preferencias de adhesión voluntaria de un miembro. |
| `convercus_membership_terminated` | Finaliza una membresía. |
| `convercus_status_level_changed` | Cambia el nivel o estatus de un miembro. |
| `convercus_balance_changed` | Cambia el saldo de puntos de un miembro. |
...
| `convercus_user_logged_in` | El miembro inicia sesión en una superficie impulsada por Convercus. |
| `convercus_user_logged_out` | El miembro cierra sesión. |
| `convercus_user_created` | Se crea un nuevo usuario. |
| `convercus_user_changed` | Cambian los datos de perfil de un usuario. |
...
| `401` | `X-Convercus-Key` falta o no es válido (se maneja en el momento de la integración, no en Liquid). |
{: .reset-td-br-1 .reset-td-br-2 aria-label="Paso 2: Obtén datos de fidelización en vivo con contenido conectado" }

Ejemplo — renderizar el estado de fidelización de un miembro (nivel, puntos y ofertas activas):
...
- **Una configuración por programa:** Cada programa de fidelización se mapea a un único espacio de trabajo de Braze. Para conectar un segundo espacio de trabajo, configúralo en un programa separado.
...
- **Los eventos no aparecen en Braze:** Verifica que el valor utilizado como identificador (seleccionado en el paso 1) coincida con el `external_id` del usuario (o el tipo de identificador elegido) en Braze. Los identificadores que no coinciden provocan que los eventos se atribuyan al perfil incorrecto o se descarten.
...
- **El webhook devuelve `400`:** A la solicitud le falta `Content-Type: application/json`, o la carga útil no coincide con el esquema documentado. Para el webhook de suscripción de correo electrónico, un `400` también significa que las adhesiones voluntarias solicitadas son desconocidas para el programa o que no hay ninguna configurada.
