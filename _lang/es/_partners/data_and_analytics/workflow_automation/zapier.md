...
Siguiendo con nuestro ejemplo, queremos enviar una solicitud POST como JSON a un endpoint de Braze. Para ello, selecciona la opción **Webhooks** en **Apps**.

![Paso de Apps en Zapier con Webhooks seleccionado para la acción.]({% image_buster /assets/img_archive/zapier3.png %})
...
![Configuración del webhook en Zapier con el endpoint de Braze, encabezados y campos de carga útil.]({% image_buster /assets/img/zapier.png %}){: style="max-width:70%;"}
...
## Uso de Zapier con el endpoint `/users/track` {#using-zapier-with-the-userstrack-endpoint}

Para enviar datos al endpoint [`/users/track`]({{site.baseurl}}/api/endpoints/user_data/post_user_track) de Braze (por ejemplo, al usar un desencadenante como **New or Updated Spreadsheet Row** en Google Sheets), utiliza **Webhooks by Zapier** con una **Custom Request**; no uses la acción estándar **POST**. La acción POST estándar formatea la solicitud de una manera que no es compatible con el endpoint `/users/track`.
