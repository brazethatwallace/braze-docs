...
Puedes encontrar una lista completa de los eventos y las propiedades del evento que se pueden exportar desde Braze a Amplitude en las siguientes secciones. Todos los eventos enviados a Amplitude incluirán el `external_user_id` del usuario como el ID de Usuario de Amplitude. Las propiedades del evento específicas de Braze se enviarán bajo la clave `event_properties` en los datos enviados a Amplitude.
...
Contacta a tu director de cuentas o abre un [ticket de soporte]({{site.baseurl}}/user_guide/administer/personal/braze_support) si necesitas acceso a derechos de eventos adicionales.
...
En Braze, ve a **Currents > + Create Current > Create Amplitude Export**. Proporciona un nombre de integración, correo electrónico de contacto, clave de API de exportación de Amplitude y la región de Amplitude en los campos indicados. A continuación, selecciona los eventos que deseas rastrear; se proporciona una lista de eventos disponibles. Por último, haz clic en **Launch Current**.
...
{% alert tip %}
Si recibes un error de "Clave de API no válida" al pegar tu clave de API de Amplitude, intenta escribir la clave manualmente. Algunos navegadores pueden añadir caracteres ocultos al copiar y pegar que pueden provocar errores de validación.
{% endalert %}
