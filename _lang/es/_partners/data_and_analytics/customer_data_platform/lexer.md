...
La integración de Braze y Lexer te permite sincronizar datos entre las dos plataformas. Usa tus datos de Lexer para crear Segments valiosos en Braze o importa los existentes a Lexer para obtener información.
...
- **Braze export segment ID**: El ID del Segment que creaste en Braze que contiene todos los usuarios que deseas exportar a Lexer. Si hay usuarios que no deseas exportar a Lexer, puedes excluirlos del Segment que creaste en Braze. Para encontrar tu identificador de Segment, haz clic en el Segment deseado en Braze y localiza el **Segment API Identifier**.

![Pantalla de integración de Lexer que muestra los campos de integración de Braze para la URL de la API, la clave de API, los detalles del contenedor de AWS S3 y el ID de Segment de exportación de Braze.]({% image_buster /assets/img/lexer/braze_integrate_screen.png %})
...
## Envío de Segments a Braze {#sending-segments-to-braze}

### Paso 1: Crear activación {#step-1-create-activation}

Lexer Activate actualizará automáticamente tus perfiles de Braze, añadiendo o eliminando atributos a medida que los clientes entren y salgan de tu Segment.
...
5. Este es el atributo personalizado que se actualizará en Braze. Ponte en contacto con [el soporte de Lexer](support@lexer.io) para actualizarlo.
...
### Paso 3: Crea tu Segment en Braze {#step-3-create-your-braze-segment}
...
Para crear tu Segment, ve a **Segment > + Create Segment** y selecciona **Custom Attribute** como filtro. A continuación, selecciona `lexer_audience` como tu atributo y el nombre de la audiencia de Lexer que desees. Cuando hayas terminado, **guarda** tu audiencia.

Ahora puedes añadir este Segment recién creado a futuras Campaigns y Canvas de Braze para dirigirte a estos usuarios finales.