...
nav_title: Chord
article_title: Chord
description: "Conecta la CDP (CDP) de Chord a Braze para reenviar eventos de comercio electrónico y actualizaciones de identidad para mensajería, segmentación y journeys."
...
| Credenciales de API de Braze | Las credenciales que necesitas dependen de tu [modo de conexión](#connection-modes). El modo en la nube usa una clave de API REST de Braze. El modo de dispositivo usa la clave de API del canal Web para Braze SDK, que es diferente de tu clave de API REST. |
| Endpoint REST de Braze | Chord envía datos del lado del servidor a los endpoints [`/users/track`]({{site.baseurl}}/api/endpoints/user_data/post_user_track) y [`/users/identify`]({{site.baseurl}}/api/endpoints/user_data/post_user_identify). Tu URL base depende de tu instancia de Braze, por ejemplo, `https://rest.iad-01.braze.com`. Para más información, consulta [Endpoints de REST API de Braze]({{site.baseurl}}/api/basics#endpoints). |
{: .reset-td-br-1 .reset-td-br-2 aria-label="Requisitos" }

## Modos de conexión {#connection-modes}

Chord admite el modo en la nube (llamadas de servidor a servidor a través de las REST API de Braze) y el modo de dispositivo (Chord inicializa el SDK Web de Braze y reenvía las llamadas mapeadas). Elige el modo que mejor se ajuste a tus necesidades, ya sea que requieras todas las características del SDK Web (por ejemplo, mensajes dentro de la aplicación) o solo el reenvío de eventos del lado del servidor.

### Modo en la nube {#cloud-mode}

1. En la plataforma de datos de Chord, abre el CDP y ve a **Destinations**.
2. Selecciona **Add** junto a destinations, elige **Braze** del catálogo, luego introduce un nombre de destino y tu clave de API REST de Braze.
...
Usa la clave de API del canal Web desde **Configuración** > **Configuración de la aplicación** > **Web** > **API Key** en el panel de Braze. No utilices tu clave de API REST para el modo de dispositivo.
...
- **Opciones de eventos y SDK:** Por ejemplo, qué comportamientos de track o identify enviar, manejo de eventos de página, comportamiento de mensajes dentro de la aplicación, temporización de inicialización del SDK y configuración relacionada con el consentimiento.
...
{% alert important %}
Si otra herramienta ya envía los mismos eventos a Braze, coordínate con los responsables de esa integración antes de conectar Braze a través del CDP de Chord. Ejecutar destinos en paralelo puede crear eventos duplicados en sentido descendente.
{% endalert %}
...
2. Verifica que el destino de Braze utiliza la clave de API correcta, la versión del SDK (modo de dispositivo) y el endpoint REST o de SDK para tu instancia.
