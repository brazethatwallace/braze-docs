...
Para más información, consulta la sección [Getting started](https://help.octolis.com/) de Octolis.

### Paso 1: Conecta Octolis a tus orígenes de datos {#step-1-connect-octolis-to-your-data-sources}

Para enviar datos a Braze, debes asegurarte de que has creado al menos una [audiencia](https://help.octolis.com/audiences/create-a-no-code-audience). Una audiencia combina varios orígenes de datos, los aplica a los pasos de preparación y añade campos computados.

Estas audiencias deben construirse a partir de varios orígenes de datos. Un origen puede ser cualquiera de los siguientes:
- Un objeto de Salesforce (contactos, cuentas, etc.)
- Un objeto de Zendesk (tickets)
...
### Paso 2: Añadir Braze como destino {#step-2-add-braze-as-a-destination}
...
| Campo | Descripción |
| --- | --- |
| ¿Hacia dónde quieres sincronizar la audiencia? | La entidad de Braze en la que crearás o actualizarás registros. |
| ¿Qué campo se utiliza para identificar un registro? | El campo que utilizará Octolis para identificar un registro si ya existe en Braze. |
| ¿Con qué frecuencia quieres enviar cada registro? | Por defecto, la sincronización será incremental para todas las integraciones (API, base de datos, FTP). Esto significa que solo se actualizarán los valores nuevos desde la última actualización. Si es necesario, también puedes enviar tablas enteras a intervalos regulares. Al iniciarse, Octolis enviará la tabla completa. |
| ¿Qué campos deben sincronizarse? | Mapeado de campos de Octolis a Braze. La lista de todos los campos disponibles aparece en el menú desplegable. Para enviar un campo calculado a Braze, primero debes asegurarte de haber creado la columna correspondiente dentro de tu entidad de Braze. |
| ¿Cuándo quieres sincronizar la audiencia? | Cómo se enviarán los datos a Braze: manualmente, en tiempo real o programado.  |
| Sincronizar cuando el registro se... | Crea: para las adhesiones voluntarias, es importante que la tabla de Braze siga siendo maestra. No quieres que Octolis desencadene una sincronización cuando se actualice el campo.<br><br>Actualiza: en cambio, para un campo de nombre, por ejemplo, quieres poder actualizar el campo en tu tabla de Braze cada vez que un cliente te dé una nueva entrada. |
{: .reset-td-br-1 .reset-td-br-2 aria-label="Paso 4: Establece la configuración de salida" }

## Deduplicación multiclave {#multi-keys-deduplication}
