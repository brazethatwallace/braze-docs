...
- **Ejecución rápida de campañas:** Lanza campañas en minutos, no en semanas.
- **Voz de marca coherente:** Utiliza las plantillas de Jasper para asegurarte de que los textos generados se ajustan estrictamente a las directrices de la marca.
...
Hay dos métodos para generar contenido en Jasper y actualizar plantillas de Braze:

1. Utilizar directamente la API de Jasper
2. Utilizar Jasper Studio para crear una aplicación personalizada preparada para Braze

{% tabs %}
{% tab Jasper API %}

## Método: utilizar directamente la API de Jasper {#method-use-jasper-api-directly}
...
### Paso 1: Configurar Jasper {#step-1-set-up-jasper}
...
2. Utiliza la plantilla preconstruida de Jasper que está optimizada para generar plantillas de correo electrónico HTML de Braze, que tiene un ID de plantilla de `skl_BC53D8AC5B4B47E8BE557EBB706E9B47`.
...
### Salida de ejemplo {#sample-output}
```
{
...
## Método: crear una aplicación personalizada preparada para Braze con Jasper Studio {#method-build-a-braze-ready-custom-app-with-jasper-studio}

Jasper Studio es una plataforma sin código dentro de Jasper que te permite crear aplicaciones de IA a medida sin necesidad de soporte informático. Puedes diseñar una aplicación personalizada que genere estructuras JSON formateadas específicamente para la API de Braze, o generar contenido que pueda añadirse manualmente a tus mensajes de Braze.

1. En la pantalla de inicio de Jasper, selecciona **Create an App**.
2. Especifica la aplicación que quieres crear, como **Braze HTML Email Template** o **Content Block Template**.
...
5. Perfecciona las instrucciones proporcionadas al modelo de lenguaje grande (LLM) para la generación de contenidos.
6. Proporciona un ejemplo de la salida deseada, que puede incluir una salida JSON automatizada formateada para cargas útiles de Braze.
...
- **Salida JSON:** Genera una salida JSON. Esta carga útil puede utilizarse para llamar directamente al punto de conexión de Braze a través de `curl` o middleware, o integrarse en tu flujo de trabajo de operaciones de correo electrónico.

![Aplicación personalizada de Jasper para Braze.]({% image_buster /assets/img/jasper/jasper_custom_app.png %})

{% subtabs %}
{% subtab Sample JSON output (custom app) %}

## Ejemplo de salida JSON (aplicación personalizada) {#sample-json-output-custom-app}
...
## Ejemplo de solicitud de la API de Braze (utilizando la salida de la aplicación personalizada) {#sample-braze-api-request-using-custom-app-output}
...
Alternativamente, si eres especialista en marketing, puedes crear tu aplicación personalizada alineada con las directrices de la marca para generar contenido sin HTML ni copiar y pegar, y utilizar plantillas de Braze para el estilo.
