...
## Ingesta de datos de Cloud para Databricks {#cloud-data-ingestion-for-databricks}

La Ingesta de datos de Cloud de Braze para Databricks permite a los clientes sincronizar directamente los datos de usuarios (atributos, eventos, compras), así como las eliminaciones de usuarios, desde Databricks a Braze. Una vez sincronizados con Braze, estos datos pueden utilizarse como cualquier otro dato de la plataforma Braze. Esta característica es una ampliación de nuestro producto de [Ingesta de datos de Cloud]({{site.baseurl}}/user_guide/data/unification/cloud_ingestion/integrations/) y, actualmente, se encuentra en acceso anticipado.
...
- [Android SDK 26.0.0-26.1.0](https://github.com/braze-inc/braze-android-sdk/blob/master/CHANGELOG.md#2610)
	- {% raw %}Se ha añadido la posibilidad de configurar el comportamiento del enlace de destino para los mensajes HTML dentro de la aplicación a través de `BrazeConfig.setIsHtmlInAppMessageHtmlLinkTargetEnabled()` o mediante la adición de `<bool name="com_braze_html_in_app_message_enable_html_link_target">true</bool>` a tu `braze.xml`. De manera predeterminada, está habilitada.{% endraw %}
		- {% raw %}Cuando está habilitado, un enlace en un mensaje dentro de la aplicación que tiene establecido el destino del enlace (por ejemplo, `<a HREF="https://www.braze.com" target="_blank">Please Read</a>`) abrirá el enlace en un navegador, pero no cerrará el mensaje dentro de la aplicación.{% endraw %}
