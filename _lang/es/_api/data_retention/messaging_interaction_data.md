...
Para Campaigns y Canvas detenidos, sus datos de interacción expiran después de tres meses a menos que se utilicen en filtros de reorientación por Campaigns o Canvas activos. Los datos de interacción expirados se trasladan a almacenamiento a largo plazo y no están disponibles para su uso a menos que se restauren mediante el proceso descrito.
...
- Filtros de reorientación que reorientan en Campaigns o Canvas con una etiqueta determinada
...
- Exportaciones CSV de **User Data** en las páginas de resumen de Campaigns y Canvas

Estas características no incluyen datos de interacción expirados en sus resultados. Para incluir datos de interacción expirados en los resultados de estas características, restaura la Campaign o el Canvas con datos expirados.
...
- Configuración de Campaigns y Canvas
- Análisis de Campaigns y Canvas
...
1. Ve a la Campaign o el Canvas expirado.
2. En la parte superior de la página de destino de la Campaign o el Canvas, selecciona **Restore interaction data** en el banner.

También puedes restaurar datos de interacción para múltiples Campaigns desde la página **Campaigns** seleccionando las Campaigns y luego seleccionando **Restore interaction data**.
...
También puedes restaurar datos de interacción para Campaigns o Canvas expirados con una etiqueta determinada.

1. Ve a la página de **Campaigns** o **Canvas** y busca por la etiqueta correspondiente.
2. Selecciona tus Campaigns o Canvas.
3. Selecciona **Restore interaction data** para restaurar los datos de esas Campaigns o Canvas.

Después de otros tres meses de inactividad, estas Campaigns o Canvas expiran de nuevo.

### Reorientación por etiqueta {#retargeting-by-tag}

Las Campaigns que utilizan filtros de reorientación que reorientan por etiqueta no están exentas de la expiración. Los filtros de reorientación que reorientan por etiqueta incluyen:
...
Anteriormente, los datos de interacción de mensajería se eliminaban cuando una Campaign o un Canvas:

- No había enviado mensajes en 25 meses calendario, Y
- No se utilizaba para reorientación en ninguna Campaign, Canvas o Content Cards activos.

Las Campaigns y Canvas con datos de interacción de mensajería previamente eliminados no pueden utilizarse en filtros de reorientación para Campaigns, Canvas y Segments.
...
| --- | --- | --- |
| "Can't resume Canvases because at least one Canvas is using filters or segments that have expired data. Remove these and try again." | Cuando intentas reanudar uno o más Canvas (acción masiva) que utilizan filtros o segmentos con datos de interacción expirados | [Restaura los datos de interacción](#how-do-i-restore-messaging-interaction-data) para las Campaigns o Canvas referenciados en los filtros, o elimina los filtros afectados del Canvas |
| "Can't resume {name} because it is using filters or segments that have expired data. Remove these and try again." | Cuando intentas reanudar un solo Canvas que utiliza filtros o segmentos con datos de interacción expirados | [Restaura los datos de interacción](#how-do-i-restore-messaging-interaction-data) para las Campaigns o Canvas referenciados en los filtros, o elimina los filtros afectados del Canvas |
| "Resume is only available for stopped Canvases with available interaction data" | Cuando intentas reanudar un Canvas desde el menú de acción masiva, pero el Canvas tiene datos de interacción expirados | [Restaura los datos de interacción](#how-do-i-restore-messaging-interaction-data) para el Canvas |
| "You can't resume these Campaigns. One or more Campaigns include expired filters." | Cuando intentas reanudar una o más Campaigns que utilizan filtros con datos de interacción expirados | [Restaura los datos de interacción](#how-do-i-restore-messaging-interaction-data) para las Campaigns o Canvas referenciados en los filtros, o elimina los filtros afectados de la Campaign |
| "You can't unarchive these Cards. One or more Cards include expired filters." | Cuando intentas desarchivar una o más Content Cards que utilizan filtros con datos de interacción expirados | [Restaura los datos de interacción](#how-do-i-restore-messaging-interaction-data) para las Campaigns o Canvas referenciados en los filtros, o elimina los filtros afectados de la tarjeta |
