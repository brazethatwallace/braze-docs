...
layout: api_page
page_type: reference
description: "En este artículo se describen los detalles del endpoint de Braze Actualizar Canvas programados desencadenados por la API."
...
Ten en cuenta que para enviar mensajes con este endpoint, debes tener un ID de Canvas, creado cuando construyes un [Canvas]({{site.baseurl}}/api/identifier_types#canvas-identifier).
...
Para utilizar este endpoint, necesitarás una [clave de API]({{site.baseurl}}/api/basics#rest-api-key-permissions) con el permiso `canvas.trigger.schedule.update`.
