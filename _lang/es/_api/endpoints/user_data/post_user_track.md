...
layout: api_page
page_type: reference
description: "En este artículo se describen los detalles del endpoint Realizar un seguimiento de los usuarios de Braze."
toc_headers: h2
---
{% api %}
...
> Utiliza este endpoint para registrar eventos personalizados y compras, y actualizar los atributos del perfil de usuario.
...
Utiliza el [endpoint `/users/track/bulk`]({{site.baseurl}}/api/endpoints/user_data/post_user_track_bulk) para enviar lotes más grandes y reducir el volumen de solicitudes.

{% apiref postman %}https://documenter.getpostman.com/view/4689407/SVYrsdsG?version=latest#4cf57ea9-9b37-4e99-a02e-4373c9a4ee59 {% endapiref %}

## Requisitos previos {#prerequisites}

Para utilizar este endpoint, necesitarás una [clave de API]({{site.baseurl}}/api/basics) con el permiso `users.track`.
...
Puedes actualizar el perfil de un usuario por dirección de correo electrónico utilizando el endpoint `/users/track`.
...
Puedes actualizar un perfil de usuario por número de teléfono utilizando el endpoint `/users/track`. Este endpoint solo funciona si incluyes un número de teléfono válido.
...
La actualización del estado de suscripción con este endpoint actualiza al usuario especificado por su `external_id` (como Usuario1) y actualiza el estado de suscripción de cualquier usuario con el mismo correo electrónico que ese usuario (Usuario1).
...
### Errores específicos del endpoint {#endpoint-specific-errors}

Los siguientes errores son específicos del endpoint `/users/track` y se devuelven en la matriz `errors` de la respuesta. Utilízalos para solucionar problemas con objetos individuales en una solicitud.
...
{: .reset-td-br-1 .reset-td-br-2 aria-label="Errores específicos del endpoint" }
...
Braze crea un perfil y un usuario de solo correo electrónico, y establece el campo de correo electrónico en test@example.com, como se indica en el ejemplo de solicitud de actualización de un perfil de usuario por dirección de correo electrónico. Braze no crea un alias.
...
Puedes enviar datos a través de la API de Braze de un usuario que aún no haya utilizado tu aplicación móvil para generar un perfil de usuario. Si el usuario utiliza posteriormente la aplicación, toda la información posterior a su identificación mediante el SDK or kit de desarrollo de software se fusiona con el perfil de usuario existente que creaste mediante la llamada a la API. Cualquier comportamiento de usuario registrado de forma anónima por el SDK or kit de desarrollo de software antes de la identificación se pierde al fusionarse con el perfil de usuario existente generado por la API.
...
- Al hacer la transición de usuarios de perfiles de solo correo electrónico o solo teléfono a perfiles identificados, utiliza el [endpoint `/users/identify`]({{site.baseurl}}/api/endpoints/user_data/post_user_identify) para asignar un `external_id` al perfil existente, en lugar de enviar ambos a `/users/track`.
- Si ya existen duplicados, fusiónalos utilizando el [endpoint `/users/merge`]({{site.baseurl}}/api/endpoints/user_data/post_users_merge).
...
Las llamadas exitosas a `/users/track` generalmente se aceptan rápidamente, pero Braze sigue procesando las actualizaciones de atributos, eventos y compras de forma asíncrona. La latencia percibida puede aumentar cuando las cargas útiles son grandes o cuando el enrutamiento de red hacia tu [endpoint REST or transferencia de estado representacional]({{site.baseurl}}/api/basics#endpoints) es lento. Si necesitas un acuse de recibo síncrono por usuario o un orden más estricto entre llamadas, consulta [`/users/track/sync`]({{site.baseurl}}/api/endpoints/user_data/post_user_track_synchronous) (**beta limitada**).

### ¿Cómo afectan los límites de velocidad a `/users/track`? {#how-do-rate-limits-affect-userstrack}

Cuando te acercas a tu [límite de velocidad](#rate-limit), recibes respuestas `429`. Para las respuestas que no son `429` en contratos compatibles, puedes utilizar los encabezados de respuesta `X-RateLimit-*` descritos en [Encabezados de límite de velocidad para MAU or usuarios activos al mes or usuarios activos al mes CY 24-25, MAU or usuarios activos al mes universal, MAU or usuarios activos al mes web y MAU or usuarios activos al mes móvil](#rate-limit-headers-for-monthly-active-users-cy-24-25-universal-mau-web-mau-and-mobile-mau) para ver cuánto queda de tu ventana actual.
...
Para los clientes que hayan comprado MAU or usuarios activos al mes or usuarios activos al mes CY 24-25, MAU or usuarios activos al mes universal, MAU or usuarios activos al mes web o MAU or usuarios activos al mes móvil, Braze gestiona diferentes límites de velocidad en su endpoint `/users/track`:
- Los límites de velocidad por hora se establecen en función de la actividad prevista de ingesta de datos en tu cuenta, que puede corresponder al número de MAU or usuarios activos al mes or usuarios activos al mes que hayas comprado, al sector, a la estacionalidad o a otros factores.
...
Los límites actuales basados en la ingesta prevista se pueden encontrar en el panel en **Configuración** > **API e identificadores** > **Panel de uso de la API**. Podemos modificar los límites de velocidad para proteger la estabilidad del sistema o permitir un mayor caudal de datos en tu cuenta. Ponte en contacto con el soporte de Braze o con tu CSM or administrador de éxito de cliente or administrador de éxito de cliente si tienes preguntas o dudas sobre el límite de solicitudes por hora o por segundo y las necesidades de tu empresa.

### Encabezados de límite de velocidad para MAU or usuarios activos al mes or usuarios activos al mes CY 24-25, MAU or usuarios activos al mes universal, MAU or usuarios activos al mes web y MAU or usuarios activos al mes móvil {#rate-limit-headers-for-monthly-active-users-cy-24-25-universal-mau-web-mau-and-mobile-mau}
...
{: .reset-td-br-1 .reset-td-br-2 aria-label="Encabezados de límite de velocidad para MAU or usuarios activos al mes or usuarios activos al mes CY 24-25, MAU or usuarios activos al mes universal, MAU or usuarios activos al mes web y MAU or usuarios activos al mes móvil" }
