...
Para usar este endpoint, necesitas una [clave de API]({{site.baseurl}}/api/basics#rest-api-key-permissions) con el permiso `custom_objects.object_relationships.delete`.
...
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 .reset-td-br-4 aria-label="Parámetros de ruta para eliminar relación de objetos" }
...
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 .reset-td-br-4 aria-label="Parámetros de solicitud para eliminar relación de objetos" }

{% alert note %}
Este endpoint `DELETE` espera un cuerpo de solicitud JSON. Verifica que tu cliente HTTP envíe cuerpos de solicitud en las llamadas `DELETE`.
{% endalert %}
...
| Parámetro | Obligatorio | Tipo de datos | Descripción |
|---|---|---|---|
| `deleted` | Obligatorio | Boolean | Si la eliminación de la relación fue exitosa |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 .reset-td-br-4 aria-label="Parámetros de respuesta para eliminar relación de objetos" }
...
|---|---|---|
| `400` | Error de validación | Confirma que el cuerpo de la solicitud incluye valores válidos para `rel_kind`, `related_type_name`, `related_external_id` y `anchor`. |
| `404` | Relación u objeto del endpoint no encontrado | Confirma que ambos objetos existen y que los valores clave de la relación coinciden con una arista existente. |
...
| `403` | La clave de API no tiene permiso o la solicitud está bloqueada por la lista de permitidos | Confirma que la clave tiene el permiso `custom_objects.object_relationships.delete` y que tu IP de origen está en la lista de permitidos de la clave, si está configurada. |
