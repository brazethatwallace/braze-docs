...
> Utiliza este endpoint para listar objetos personalizados relacionados a partir de un objeto de anclaje.
...
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 .reset-td-br-4 aria-label="Parámetros de ruta para listar relaciones de objetos" }

## Parámetros de consulta {#query-parameters}

La siguiente tabla enumera y describe los parámetros de consulta del endpoint `/custom_objects/objects/{type_name}/{external_id}/object_relationships`.
...
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 .reset-td-br-4 aria-label="Parámetros de consulta para listar relaciones de objetos" }

## Solicitud de ejemplo {#example-request}

Esta sección incluye una carga útil de parámetros de muestra y una solicitud cURL de ejemplo.

### Carga útil de solicitud de muestra {#sample-request-payload}

Utiliza este objeto JSON como referencia para los parámetros de la solicitud.
...
Este ejemplo enumera los registros `subaccount` a los que `acct-123` enlaza, devolviendo la primera página de resultados.
...
Con `anchor=target`, los objetos relacionados se devuelven como `from_custom_object`.
...
| `items[].to_custom_object` | Condicional | Objeto | Objeto relacionado cuando `anchor=source` |
| `items[].from_custom_object` | Condicional | Objeto | Objeto relacionado cuando `anchor=target` |
| `items[].to_custom_object.type_name` | Condicional | Cadena | Nombre de tipo del objeto relacionado |
| `items[].to_custom_object.external_id` | Condicional | Cadena | ID externo del objeto relacionado |
| `items[].to_custom_object.attributes` | Condicional | Objeto | Atributos del objeto relacionado |
| `items[].from_custom_object.type_name` | Condicional | Cadena | Nombre de tipo del objeto relacionado |
| `items[].from_custom_object.external_id` | Condicional | Cadena | ID externo del objeto relacionado |
| `items[].from_custom_object.attributes` | Condicional | Objeto | Atributos del objeto relacionado |
...
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 .reset-td-br-4 aria-label="Parámetros de respuesta para listar relaciones de objetos" }
...
| `403` | La clave de API no tiene permiso o la solicitud está bloqueada por la lista de permitidos | Confirma que la clave tiene `custom_objects.read` y que tu IP de origen está en la lista de permitidos de la clave, si está configurada. |
