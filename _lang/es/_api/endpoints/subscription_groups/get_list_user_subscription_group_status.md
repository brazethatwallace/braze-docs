...
layout: api_page
page_type: reference
description: "En este artículo se describen los detalles del endpoint Listar el estado del grupo de suscripción del usuario de Braze."
...
> Utiliza este endpoint para obtener el estado de suscripción de un usuario en un grupo de suscripción.
...
Para utilizar este endpoint, necesitarás una [clave de API]({{site.baseurl}}/api/basics#rest-api-key-permissions) con el permiso `subscription.status.get`.
...
| `phone` | Obligatorio* | Cadena en formato [E.164](https://en.wikipedia.org/wiki/E.164) | El número de teléfono del usuario. Si no se incluye el correo electrónico, debes incluir al menos un número de teléfono (con un máximo de 50).<br><br> Si envías una dirección de correo electrónico y un número de teléfono (sin `external_id`), se producirá un error. |
