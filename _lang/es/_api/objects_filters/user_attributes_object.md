...
{% alert tip %}
Para una referencia de atributos estándar orientada al cliente, organizada por categoría e incluye orientación para SDK, API, CSV e ingesta de datos en la nube, consulta [Atributos estándar]({{site.baseurl}}/user_guide/data/activation/attributes/standard_attributes).
{% endalert %}
...
Después de [integrar el SDK de Braze]({{site.baseurl}}/developer_guide/sdk_integration), los tokens de notificaciones push de tus usuarios con adhesión voluntaria se migran automáticamente la próxima vez que abran tu aplicación. Hasta entonces, no puedes enviar notificaciones push a esos usuarios a través de Braze.
...
{% alert note %}
Los tokens registrados a través del SDK de Braze incluyen automáticamente el campo de gateway, ya que el SDK detecta el entorno a partir de los permisos de tu aplicación.
{% endalert %}
...
{% alert note %}
Para algunos proveedores de notificaciones push, Braze necesita aplanar los pares clave-valor para que puedan interpretarse correctamente. Para aplanar los pares clave-valor de una aplicación de Android específica, ponte en contacto con tu administrador de éxito de cliente.
{% endalert %}
