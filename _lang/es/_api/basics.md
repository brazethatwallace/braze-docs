...
## Colección REST API de Braze {#braze-rest-api-collection}
...
{: .reset-td-br-1 .reset-td-br-2 aria-label="Colección REST API de Braze" }

## Definiciones de la API {#api-definitions}

A continuación encontrarás un resumen de los términos que puedes ver en la documentación de la REST API de Braze.
...
### Permisos de la clave de REST API {#rest-api-key-permissions}
...
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="Permisos de la clave de REST API" }
...
{: .reset-td-br-1 .reset-td-br-2 aria-label="Permisos de la clave de REST API" }
...
### Gestión de claves de REST API {#managing-rest-api-keys}

Puedes ver los detalles de las claves de REST API existentes o eliminarlas desde **Configuración** > **API e identificadores** > pestaña **Claves de API**. Ten en cuenta que no puedes editar las claves de REST API después de crearlas.
...
{: .reset-td-br-1 .reset-td-br-2 aria-label="Gestión de claves de REST API" }

Para ver los detalles de una clave de API, pasa el cursor por encima de la clave y selecciona <i class="fa-solid fa-eye" alt="Ver"></i> **Ver**. Esto incluye todos los permisos que tiene esta clave, las IP de la lista blanca (si las hay) y si esta clave está incluida en la lista blanca de IP de Braze.
...
![Una clave de API llamada «Last Seen» con el icono de la papelera resaltado, mostrando «Eliminar».]({% image_buster /assets/img_archive/api-key-options.png %}){: style="max-width:30%;"}

### Seguridad de la clave de REST API {#rest-api-key-security}

Las claves de API se utilizan para autenticar una llamada a la API. Cuando creas una nueva clave de REST API, es necesario darle acceso a puntos finales específicos. Al asignar permisos específicos a una clave de API, puedes limitar exactamente qué llamadas puede autenticar dicha clave.

Dado que las claves de REST API permiten acceder a puntos finales de REST API potencialmente sensibles, protege estas claves y compártelas solo con socios de confianza. Nunca deben quedar expuestas públicamente. Por ejemplo, no utilices esta clave para hacer llamadas AJAX desde tu sitio web ni la expongas de ninguna otra forma pública.
...
{% alert warning %}
Dado que las claves de REST API permiten acceder a puntos finales de REST API potencialmente sensibles, asegúrate de que se almacenan y utilizan de forma segura. Por ejemplo, no utilices esta clave para hacer llamadas AJAX desde tu sitio web ni la expongas de ninguna otra forma pública.
{% endalert %}
...
Braze autentica las solicitudes de la REST API utilizando la clave de REST API pasada como token Bearer en el encabezado de solicitud `Authorization`. Cuando envíes una solicitud, incluye tu clave de API en el siguiente formato:
...
1. **Validez del token:** Verifica que la clave de REST API existe en Braze y está activa (por ejemplo, que no ha sido revocada ni desactivada).
