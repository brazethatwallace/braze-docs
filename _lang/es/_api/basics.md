...
## Colección de la REST API de Braze {#braze-rest-api-collection}

| Colección                                                                  | Propósito                                                                                          |
|----------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------|
| [Catálogos]({{site.baseurl}}/api/endpoints/catalogs)                       | Crea y gestiona catálogos y elementos de catálogo para referenciar en tus Campaigns de Braze.      |
...
| [Exportar]({{site.baseurl}}/api/endpoints/export)                          | Accede y exporta diversos detalles de tus Campaigns, Canvas, indicadores clave de rendimiento y más. |
| [Biblioteca multimedia]({{site.baseurl}}/api/endpoints/media_library)      | Gestiona activos dentro de Braze.                                                                  |
...
{: .reset-td-br-1 .reset-td-br-2 aria-label="Colección de la REST API de Braze" }

## Definiciones de la API {#api-definitions}

A continuación se presenta un resumen de los términos que puedes encontrar en la documentación de la API REST de Braze.

### Endpoints

Braze gestiona varias instancias diferentes para nuestro panel y endpoints REST. Cuando tu cuenta es aprovisionada, inicias sesión en una de las siguientes URL. Utiliza el endpoint REST correcto en función de la instancia a la que estés aprovisionado. Si no estás seguro, abre un [ticket de soporte]({{site.baseurl}}/user_guide/administer/personal/braze_support) o utiliza la siguiente tabla para hacer coincidir la URL del panel que utilizas con el endpoint REST correcto.

Para encontrar tu endpoint REST en Braze:

1. Inicia sesión en Braze y ve a **Configuración** > **APIs e identificadores** > **Claves de API**.
2. Selecciona una clave de API existente, o selecciona **Crear clave de API** para crear una nueva clave.
3. Copia el endpoint REST que se muestra en esta pestaña y utiliza ese endpoint para tus solicitudes de API.

{% alert important %}
Cuando utilices endpoints para llamadas a la API, usa el endpoint REST.

Para la integración de SDK, utiliza el [endpoint de SDK]({{site.baseurl}}/user_guide/administer/personal/sdk_endpoints), no el endpoint REST.
{% endalert %}
...
Para la mayoría de las API, Braze tiene un límite de velocidad predeterminado de 250.000 solicitudes por hora. Sin embargo, ciertos tipos de solicitudes tienen su propio límite de velocidad aplicado para gestionar mejor los grandes volúmenes de datos en toda la base de clientes. Para más detalles, consulta [Límites de velocidad de la API]({{site.baseurl}}/api/api_limits)

### ID de usuario {#user-ids}

- **ID externo de usuario**: El `external_id` sirve como identificador único del usuario para el cual estás enviando datos. Este identificador debe ser el mismo que configuraste en el SDK de Braze para evitar crear múltiples perfiles para el mismo usuario.
- **ID de usuario de Braze**: `braze_id` sirve como identificador único de usuario que Braze establece. Puedes usar este identificador para eliminar usuarios a través de la API REST, además de los external_ids.
...
Una clave de API REST (clave de API de transferencia de estado representacional) es un código único que pasas a una API para autenticar la llamada a la API e identificar la aplicación o el usuario que realiza la llamada. Accedes a la API usando solicitudes web HTTPS al endpoint de la REST API de tu empresa. Las claves de API REST funcionan en conjunto con las claves de identificador de aplicación para rastrear, acceder, enviar, exportar y analizar datos para ayudar a garantizar que todo funcione correctamente.

Los espacios de trabajo y las claves de API van de la mano en Braze. Los espacios de trabajo están diseñados para albergar versiones de la misma aplicación en múltiples plataformas. Muchos clientes también usan espacios de trabajo para contener versiones gratuitas y premium de sus aplicaciones en la misma plataforma. Como podrás notar, estos espacios de trabajo también hacen uso de la REST API y tienen sus propias claves de API REST. Estas claves pueden configurarse individualmente para incluir acceso a endpoints específicos de la API. Cada llamada a la API debe incluir una clave con acceso al endpoint solicitado.

Nos referimos tanto a la clave de API REST como a la clave de API del espacio de trabajo como `api_key`. La `api_key` se incluye en cada solicitud como un encabezado de solicitud y actúa como una clave de autenticación que te permite usar nuestras REST API. Estas REST API se utilizan para rastrear usuarios, enviar mensajes, exportar datos de usuario y más. Cuando creas una nueva clave de API REST, debes otorgarle acceso a endpoints específicos. Al asignar permisos específicos a una clave de API, puedes limitar exactamente qué llamadas puede autenticar dicha clave.

![Panel de claves de API REST en la pestaña Claves de API.]({% image_buster /assets/img_archive/rest-api-key.png %})

{% alert tip %}
Además de las claves de API REST, también existe un tipo de clave llamada clave de identificador que puede usarse para hacer referencia a elementos específicos como aplicaciones, plantillas, Canvas, Campaigns, Content Cards y Segments desde la API. Para más información, consulta [Tipos de identificadores de API]({{site.baseurl}}/api/identifier_types).
{% endalert %}
...
1. Ve a **Configuración** > **APIs e identificadores**.
2. Selecciona **Crear clave de API**.
3. Dale un nombre a tu nueva clave para identificarla de un vistazo.
4. Especifica las [direcciones IP permitidas](#api-ip-allowlisting) y subredes para la nueva clave.
5. Selecciona los [permisos](#rest-api-key-permissions) que deseas asociar con tu nueva clave.

{% alert important %}
Ten en cuenta que después de crear una nueva clave de API, no puedes editar el alcance de los permisos ni las IP permitidas. Esta limitación existe por razones de seguridad. Si necesitas cambiar el alcance de una clave, crea una nueva clave con los permisos actualizados e impleméntala en lugar de la antigua. Una vez completada tu implementación, puedes eliminar la clave antigua.
{% endalert %}

### Permisos de claves de API REST {#rest-api-key-permissions}

Los permisos de claves de API son permisos que puedes asignar a un usuario o grupo para limitar su acceso a ciertas llamadas de API. Para ver tu lista de permisos de claves de API, ve a **Configuración** > **APIs e identificadores** y selecciona tu clave de API.
...
| `users.export.ids` | [`/users/export/ids`]({{site.baseurl}}/api/endpoints/export/user_data/post_users_identifier) | Consultar información del perfil de usuario por ID de usuario. |
| `users.export.segment` | [`/users/export/segment`]({{site.baseurl}}/api/endpoints/export/user_data/post_users_segment) | Consultar información del perfil de usuario por Segment. |
...
| `users.alias.update` | [`/users/alias/update`]({{site.baseurl}}/api/endpoints/user_data/post_users_alias_update) | Actualizar un alias de un usuario existente. |
| `users.export.global_control_group` | [`/users/export/global_control_group`]({{site.baseurl}}/api/endpoints/export/user_data/post_users_global_control_group) | Consultar información del perfil de usuario en el grupo de control global. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="Permisos de claves de API REST" }
...
| Permiso | Endpoint | Descripción |
|---|---|---|
| `email.unsubscribe` | [`/email/unsubscribes`]({{site.baseurl}}/api/endpoints/email/get_query_unsubscribed_email_addresses) | Consultar direcciones de correo electrónico dadas de baja. |
| `email.status` | [`/email/status`]({{site.baseurl}}/api/endpoints/email/post_email_subscription_status) | Cambiar el estado de una dirección de correo electrónico. |
| `email.hard_bounces` | [`/email/hard_bounces`]({{site.baseurl}}/api/endpoints/email/get_list_hard_bounces) | Consultar direcciones de correo electrónico con rebote duro. |
...
| `email.blacklist` | [`/email/blacklist`]({{site.baseurl}}/api/endpoints/email/post_blacklist) | Añadir direcciones de correo electrónico a la lista de bloqueo. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="Permisos de claves de API REST" }
...
| `messages.schedule.create` | [`/messages/schedule/create`]({{site.baseurl}}/api/endpoints/messaging/schedule_messages/post_schedule_messages) | Programar el envío de un mensaje en un momento específico. |
...
| `messages.live_activity.update` | [`/messages/live_activity/update`]({{site.baseurl}}/api/endpoints/messaging/live_activity/update) | Actualizar una actividad en vivo de iOS. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="Permisos de claves de API REST" }
...
| `campaigns.trigger.schedule.create` | [`/campaigns/trigger/schedule/create`]({{site.baseurl}}/api/endpoints/messaging/schedule_messages/post_schedule_triggered_campaigns) | Programar el envío de una Campaign con entrega desencadenada por API. |
...
| `campaigns.url_info.details` | [`/campaigns/url_info/details`]({{site.baseurl}}) | Consultar detalles de URL de una variación de mensaje específica dentro de una Campaign. Este permiso solo está disponible para espacios de trabajo con [aliasing de enlaces]({{site.baseurl}}/user_guide/messaging/templates/email_templates/link_aliasing) habilitado. Si este permiso no está disponible en tu espacio de trabajo, comunícate con tu director de cuentas de Braze. |
| `transactional.send` | [`/transactional/v1/campaigns/{campaign_id}/send`]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_transactional_message) | Permite enviar mensajes transaccionales usando el endpoint de mensajería transaccional. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="Permisos de claves de API REST" }
...
| `canvas.url_info.details` | [`/canvas/url_info/details`]({{site.baseurl}}/get_canvas_link_alias) | Consultar detalles de URL de una variación de mensaje específica dentro de un paso en Canvas. Este permiso solo está disponible para espacios de trabajo con [aliasing de enlaces]({{site.baseurl}}/user_guide/messaging/templates/email_templates/link_aliasing) habilitado. Si este permiso no está disponible en tu espacio de trabajo, comunícate con tu director de cuentas de Braze. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="Permisos de claves de API REST" }
...
{: .reset-td-br-1 .reset-td-br-2 aria-label="Permisos de claves de API REST" }
...
| `preference_center.user.get` | [`/preference_center/v1/{preferenceCenterExternalId}/url/{userId}`]({{site.baseurl}}/api/endpoints/preference_center/get_create_url_preference_center) | Obtener un enlace del centro de preferencias para un usuario. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="Permisos de claves de API REST" }
...
| `subscription.groups.get` | [`/subscription/user/status`]({{site.baseurl}}/api/endpoints/subscription_groups/get_list_user_subscription_groups) | Obtener el estado de los grupos de suscripción a los que usuarios específicos están explícitamente suscritos o dados de baja. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="Permisos de claves de API REST" }
...
| Permiso | Endpoint | Descripción |
|---|---|---|
| `catalogs.add_items` | [`/catalogs/{catalog_name}/items`]({{site.baseurl}}/api/endpoints/catalogs/catalog_items/asynchronous/post_create_catalog_items_bulk) | Agregar múltiples elementos a un catálogo existente. |
...
| `catalogs.get_item` | [`/catalogs/{catalog_name}/items/{item_id}`]({{site.baseurl}}/api/endpoints/catalogs/catalog_items/synchronous/get_catalog_item_details) | Obtener un elemento individual de un catálogo existente. |
| `catalogs.update_item` | [`/catalogs/{catalog_name}/items/{item_id}`]({{site.baseurl}}/api/endpoints/catalogs/catalog_items/synchronous/put_update_catalog_item) | Actualizar un elemento individual en un catálogo existente. |
| `catalogs.create_item` | [`/catalogs/{catalog_name}/items/{item_id}`]({{site.baseurl}}/api/endpoints/catalogs/catalog_items/synchronous/post_create_catalog_item) | Crear un elemento individual en un catálogo existente. |
| `catalogs.delete_item` | [`/catalogs/{catalog_name}/items/{item_id}`]({{site.baseurl}}/api/endpoints/catalogs/catalog_items/synchronous/delete_catalog_item) | Eliminar un elemento individual de un catálogo existente. |
| `catalogs.replace_item` | [`/catalogs/{catalog_name}/items/{item_id}`]({{site.baseurl}}/api/endpoints/catalogs/catalog_items/synchronous/put_update_catalog_item) | Reemplazar un elemento individual de un catálogo existente. |
...
| `catalogs.get_items` | [`/catalogs/{catalog_name}/items`]({{site.baseurl}}/api/endpoints/catalogs/catalog_items/synchronous/get_catalog_items_details_bulk) | Obtener una vista previa de elementos de un catálogo existente. |
...
| `sdk_authentication.primary` | [`/app_group/sdk_authentication/primary`]({{site.baseurl}}/api/endpoints/sdk_authentication/put_primary_sdk_authentication_key) | Marcar una clave de autenticación de SDK como la clave principal de tu aplicación. |
| `sdk_authentication.delete` | [`/app_group/sdk_authentication/delete`]({{site.baseurl}}/api/endpoints/sdk_authentication/delete_sdk_authentication_key) | Eliminar una clave de autenticación de SDK de tu aplicación. |
| `sdk_authentication.keys` | [`/app_group/sdk_authentication/keys`]({{site.baseurl}}/api/endpoints/sdk_authentication/get_sdk_authentication_keys) | Obtener todas las claves de autenticación de SDK de tu aplicación. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="Permisos de claves de API REST" }

{% endtab %}
{% endtabs %}

### Administrar claves de API REST {#managing-rest-api-keys}

Puedes ver los detalles o eliminar las claves de API REST existentes desde **Configuración** > **APIs e identificadores** > pestaña **Claves de API**. Ten en cuenta que no puedes editar las claves de API REST después de crearlas.

La pestaña **Claves de API** incluye la siguiente información para cada clave:

| Campo | Descripción |
| ------------ | :------------------------------------------------------------------------------------------------------------------ |
| Nombre de la clave de API | El nombre dado a la clave en el momento de su creación. |
| Identificador | La clave de API. |
| Creada por | La dirección de correo electrónico del usuario que creó la clave. Este campo muestra "N/A" para claves creadas antes de junio de 2023. |
| Fecha de creación | La fecha en que se creó esta clave. |
| Última vez vista | La fecha en que se usó esta clave por última vez. Este campo muestra "N/A" para claves que nunca se han utilizado. |
{: .reset-td-br-1 .reset-td-br-2 aria-label="Administrar claves de API REST" }

Para ver los detalles de una clave de API, pasa el cursor sobre la clave y selecciona <i class="fa-solid fa-eye" alt="Ver"></i> **Ver**. Esto incluye todos los permisos que tiene esta clave, las IP permitidas (si las hay) y si esta clave está habilitada para la lista blanca de IP de Braze.

![La lista de permisos de claves de API en el panel de Braze.]({% image_buster /assets/img_archive/view-api-key.png %})

Ten en cuenta que al [eliminar un usuario]({{site.baseurl}}/user_guide/administer/global/user_management/manage_company_users), Braze no elimina las claves de API asociadas que ese usuario creó. Para eliminar una clave, pasa el cursor sobre la clave y selecciona <i class="fa-solid fa-trash-can" alt="Eliminar"></i> **Eliminar**.

![Una clave de API llamada "Última vez vista" con el icono de papelera resaltado, mostrando "Eliminar".]({% image_buster /assets/img_archive/api-key-options.png %}){: style="max-width:30%;"}

### Seguridad de claves de API REST {#rest-api-key-security}

Las claves de API se utilizan para autenticar una llamada de API. Cuando creas una nueva clave de API REST, necesitas otorgarle acceso a endpoints específicos. Al asignar permisos específicos a una clave de API, puedes limitar exactamente qué llamadas puede autenticar dicha clave.

Dado que las claves de API REST permiten el acceso a endpoints de REST API potencialmente sensibles, asegura estas claves y compártelas solo con partners de confianza. Nunca deben exponerse públicamente. Por ejemplo, no uses esta clave para hacer llamadas AJAX desde tu sitio web ni la expongas de ninguna otra manera pública.
...
{% alert warning %}
Dado que las claves de API REST permiten el acceso a endpoints de REST API potencialmente sensibles, asegúrate de que se almacenen y utilicen de forma segura. Por ejemplo, no uses esta clave para hacer llamadas AJAX desde tu sitio web ni la expongas de ninguna otra manera pública.
{% endalert %}
...
### Seguridad de claves de API REST y claves de API de SDK {#security-of-rest-api-keys-and-sdk-api-keys}
...
|---|---|---|
| Propósito | Autenticación del lado del servidor para la REST API (envío de mensajes, exportación de datos, administración de usuarios) | Identificación del lado del cliente para el SDK de Braze (ingesta de datos, mensajes dentro de la aplicación, Content Cards) |
| Visibilidad | **Deben permanecer privadas**. Nunca las expongas en código del lado del cliente, repositorios públicos o aplicaciones de usuario. | Diseñadas para ser públicas. Se incluyen dentro del binario de tu aplicación o son visibles en el JavaScript del navegador web, similar a un ID de seguimiento de Google Analytics. |
| Solución si se exponen | Revoca la clave inmediatamente y crea una de reemplazo en **Configuración** > **APIs e identificadores** > **Claves de API**. Una clave de API REST expuesta puede usarse para enviar mensajes, exportar datos de usuario o modificar la configuración de la cuenta. | No se requiere acción. Una clave de API de SDK solo puede ingerir datos y recuperar mensajería del lado del cliente (como mensajes dentro de la aplicación y Content Cards). No puede exportar datos de usuario, enviar mensajes en tu nombre ni modificar Campaigns. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="Seguridad de claves de API REST y claves de API de SDK" }

### Lista de IP permitidas para API {#api-ip-allowlisting}

Para mayor seguridad, puedes especificar una lista de direcciones IP y subredes que tienen permitido hacer solicitudes de REST API para una clave de API REST determinada. Esto se conoce como lista de permitidos, o lista blanca. Para permitir direcciones IP o subredes específicas, agrégalas a la sección **Whitelist IPs** al crear una nueva clave de API REST:

![Opción para agregar IP a la lista de permitidos al crear una clave de API.]({% image_buster /assets/img_archive/api-key-ip-whitelisting.png %})

Si no especificas ninguna, las solicitudes pueden enviarse desde cualquier dirección IP.

{% alert tip %}
Si estás haciendo un webhook de Braze a Braze y usas la lista de permitidos, consulta la lista de [IP a incluir en la lista blanca]({{site.baseurl}}/user_guide/channels/webhooks/create_a_webhook).
{% endalert %}
...
Braze autentica las solicitudes de la REST API utilizando la clave de API REST pasada como un token Bearer en el encabezado de solicitud `Authorization`. Cuando envías una solicitud, incluye tu clave de API en el siguiente formato:
...
1. **Validez del token:** Verifica que la clave de API REST existe en Braze y está activa (por ejemplo, que no ha sido revocada o deshabilitada).
2. **Autorización del token:** Confirma que la clave de API tiene los permisos necesarios para el endpoint solicitado.

Si la autenticación falla, la API devuelve una respuesta de error con un código de estado HTTP. Por ejemplo, `401 Unauthorized` indica una clave no válida o ausente, mientras que `403 Forbidden` indica que la clave no tiene permiso para el endpoint solicitado. Para más información, consulta [Errores de API]({{site.baseurl}}/api/errors).

### Formato de mayúsculas en los encabezados de solicitud {#header-casing}

Los nombres de los encabezados HTTP no distinguen entre mayúsculas y minúsculas, por lo que `Authorization` y `authorization` son equivalentes. Lo mismo aplica para otros encabezados de solicitud estándar, como `Content-Type`. Envía el formato de mayúsculas que tu cliente HTTP produzca.

Braze también acepta cualquier formato de mayúsculas del esquema `Bearer` (`Bearer`, `bearer` o `BEARER`). Envía la clave de API REST exactamente como fue emitida.

### Seguridad a nivel de red {#network-level-security}

Las solicitudes de la REST API a Braze están protegidas mediante cifrado Transport Layer Security (TLS) a lo largo de toda la ruta de la solicitud. La siguiente tabla describe el flujo de red para una solicitud de API desde tu servidor hasta Braze:
...
| 3 | Network Load Balancer (NLB) | Reenvía paquetes a la infraestructura de la aplicación. Los NLB operan en la Capa 4, lo que significa que no hay proxying en la Capa 7. Los paquetes se reenvían sin inspección ni modificación a nivel HTTP. |
| 4 | NGINX ingress | Termina la conexión TLS interna y enruta la solicitud. |
| 5 | Unicorn (servidor de aplicación) | Procesa la solicitud autenticada. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="Seguridad a nivel de red" }

El cifrado TLS cubre cada enlace de la cadena. Tu servidor se conecta a Cloudflare a través de TLS, y Cloudflare establece una conexión TLS separada a través del NLB hacia el NGINX ingress, por lo que tu clave de API y los datos de la solicitud permanecen cifrados en tránsito.

## Recursos adicionales {#additional-resources}

### Biblioteca de cliente Ruby {#ruby-client-library}

Si estás implementando Braze con Ruby, puedes usar la [biblioteca de cliente Ruby](https://github.com/braze-inc/braze-api-client-ruby) para reducir el tiempo de importación de datos. Una biblioteca de cliente es una colección de código específica de un lenguaje de programación (en este caso, Ruby) que facilita el uso de una API.

La biblioteca de cliente Ruby es compatible con los [endpoints de usuario]({{site.baseurl}}/api/endpoints/user_data).

{% alert important %}
Esta biblioteca de cliente está en fase beta. Para ayudar a mejorarla, envía tus comentarios a [smb-product@braze.com](mailto:smb-product@braze.com).
{% endalert %}