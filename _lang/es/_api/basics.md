...
page_order: 2.1
description: "Este artículo de referencia cubre los aspectos básicos de la API, incluyendo qué es una REST or transferencia de estado representacional API, la terminología y un resumen de las claves de API."
page_type: reference
alias: /api/api_key/
---
...
> Este artículo de referencia cubre los conceptos básicos de la API, incluida la terminología común y un resumen de las claves de la REST or transferencia de estado representacional API, los permisos y cómo mantenerlas seguras.

## Colección de Braze REST or transferencia de estado representacional API {#braze-rest-api-collection}
...
| [Exportar]({{site.baseurl}}/api/endpoints/export)                         | Accede y exporta diversos detalles de tus Campaigns, Canvas, indicador clave de rendimiento y más.                       |
...
| [servicio de mensajes cortos]({{site.baseurl}}/api/endpoints/sms)                                 | Gestiona los números de teléfono de tus usuarios en tus grupos de suscripción.                 |
| [Grupos de suscripción]({{site.baseurl}}/api/endpoints/subscription_groups) | Lista y actualiza los grupos de suscripción de servicio de mensajes cortos y correo electrónico almacenados en el panel de Braze. |
...
{: .reset-td-br-1 .reset-td-br-2 aria-label="Colección de Braze REST or transferencia de estado representacional API" }

## Definiciones de la API {#api-definitions}

A continuación se presenta un resumen de los términos que puedes encontrar en la documentación de la REST or transferencia de estado representacional API de Braze.

### Endpoints

Braze gestiona varias instancias diferentes para nuestro panel y endpoints REST or transferencia de estado representacional. Cuando tu cuenta es provisionada, inicias sesión en una de las siguientes URL. Usa el endpoint REST or transferencia de estado representacional correcto según la instancia a la que estés asignado. Si no estás seguro, abre un [ticket de soporte]({{site.baseurl}}/user_guide/administer/personal/braze_support) o usa la siguiente tabla para hacer coincidir la URL del panel que usas con el endpoint REST or transferencia de estado representacional correcto.

Para encontrar tu endpoint REST or transferencia de estado representacional en Braze:
...
3. Copia el endpoint REST or transferencia de estado representacional que se muestra en esta pestaña y usa ese endpoint para tus solicitudes de API.

{% alert important %}
Cuando uses endpoints para llamadas a la API, usa el endpoint REST or transferencia de estado representacional.

Para la integración de SDK or kit de desarrollo de software, usa el [punto final de SDK or kit de desarrollo de software]({{site.baseurl}}/user_guide/administer/personal/sdk_endpoints), no el endpoint REST or transferencia de estado representacional.
{% endalert %}
...
- **ID externo de usuario**: El `external_id` sirve como identificador único del usuario para el cual estás enviando datos. Este identificador debe ser el mismo que estableces en el SDK or kit de desarrollo de software de Braze para evitar crear múltiples perfiles para el mismo usuario.
- **ID de usuario de Braze**: El `braze_id` sirve como identificador único de usuario que Braze establece. Puedes usar este identificador para eliminar usuarios a través de la REST or transferencia de estado representacional API, además de los external_ids.
...
## Acerca de las claves de REST or transferencia de estado representacional API {#about-rest-api-keys}

Una clave de REST or transferencia de estado representacional API (clave de REST or transferencia de estado representacional API) es un código único que pasas a una API para autenticar la llamada a la API e identificar la aplicación o el usuario que realiza la llamada. Accedes a la API mediante solicitudes web HTTPS al endpoint de REST or transferencia de estado representacional API de tu empresa. Las claves de REST or transferencia de estado representacional API funcionan en conjunto con las claves de identificador de aplicación para rastrear, acceder, enviar, exportar y analizar datos, ayudando a garantizar que todo funcione sin problemas.

Los espacios de trabajo y las claves de API van de la mano en Braze. Los espacios de trabajo están diseñados para alojar versiones de la misma aplicación en múltiples plataformas. Muchos clientes también usan espacios de trabajo para contener versiones gratuitas y premium de sus aplicaciones en la misma plataforma. Como podrás notar, estos espacios de trabajo también utilizan la REST or transferencia de estado representacional API y tienen sus propias claves de REST or transferencia de estado representacional API. Estas claves pueden tener alcances individuales para incluir acceso a endpoints específicos de la API. Cada llamada a la API debe incluir una clave con acceso al endpoint invocado.

Nos referimos tanto a la clave de REST or transferencia de estado representacional API como a la clave de API del espacio de trabajo como `api_key`. La `api_key` se incluye en cada solicitud como un encabezado de solicitud y actúa como una clave de autenticación que te permite usar nuestras REST or transferencia de estado representacional API. Estas REST or transferencia de estado representacional API se utilizan para rastrear usuarios, enviar mensajes, exportar datos de usuario y más. Cuando creas una nueva clave de REST or transferencia de estado representacional API, debes darle acceso a endpoints específicos. Al asignar permisos específicos a una clave de API, puedes limitar exactamente qué llamadas puede autenticar una clave de API.

![Panel de claves de REST or transferencia de estado representacional API en la pestaña Claves de API.]({% image_buster /assets/img_archive/rest-api-key.png %})

{% alert tip %}
Además de las claves de REST or transferencia de estado representacional API, también existe un tipo de clave llamada claves de identificador que se pueden usar para hacer referencia a elementos específicos como aplicaciones, plantillas, Canvas, Campaigns, Content Cards y Segments desde la API. Para más información, consulta [Tipos de identificadores de API]({{site.baseurl}}/api/identifier_types).
{% endalert %}

### Crear claves de REST or transferencia de estado representacional API {#creating-rest-api-keys}

Para crear una nueva clave de REST or transferencia de estado representacional API:
...
### Permisos de claves de REST or transferencia de estado representacional API {#rest-api-key-permissions}
...
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="Permisos de claves de REST or transferencia de estado representacional API" }
...
{: .reset-td-br-1 .reset-td-br-2 aria-label="Permisos de claves de REST or transferencia de estado representacional API" }
...
{% endtab %}
{% tab Autenticación del SDK or kit de desarrollo de software %}

| Permiso | Endpoint | Descripción |
|---|---|---|
| `sdk_authentication.create` | [`/app_group/sdk_authentication/create`]({{site.baseurl}}/api/endpoints/sdk_authentication/post_create_sdk_authentication_key) | Crear una nueva clave de autenticación de SDK or kit de desarrollo de software para tu aplicación. |
| `sdk_authentication.primary` | [`/app_group/sdk_authentication/primary`]({{site.baseurl}}/api/endpoints/sdk_authentication/put_primary_sdk_authentication_key) | Marcar una clave de autenticación de SDK or kit de desarrollo de software como la clave principal para tu aplicación. |
| `sdk_authentication.delete` | [`/app_group/sdk_authentication/delete`]({{site.baseurl}}/api/endpoints/sdk_authentication/delete_sdk_authentication_key) | Eliminar una clave de autenticación de SDK or kit de desarrollo de software para tu aplicación. |
| `sdk_authentication.keys` | [`/app_group/sdk_authentication/keys`]({{site.baseurl}}/api/endpoints/sdk_authentication/get_sdk_authentication_keys) | Obtener todas las claves de autenticación de SDK or kit de desarrollo de software para tu aplicación. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="Permisos de claves de REST or transferencia de estado representacional API" }

{% endtab %}
{% endtabs %}

### Gestionar claves de REST or transferencia de estado representacional API {#managing-rest-api-keys}

Puedes ver detalles o eliminar claves de REST or transferencia de estado representacional API existentes desde **Configuración** > **API e identificadores** > pestaña **Claves de API**. Ten en cuenta que no puedes editar las claves de REST or transferencia de estado representacional API después de crearlas.
...
{: .reset-td-br-1 .reset-td-br-2 aria-label="Gestionar claves de REST or transferencia de estado representacional API" }
...
### Seguridad de las claves de REST or transferencia de estado representacional API {#rest-api-key-security}

Las claves de API se usan para autenticar una llamada a la API. Cuando creas una nueva clave de REST or transferencia de estado representacional API, necesitas darle acceso a endpoints específicos. Al asignar permisos específicos a una clave de API, puedes limitar exactamente qué llamadas puede autenticar una clave de API.

Dado que las claves de REST or transferencia de estado representacional API permiten el acceso a endpoints de REST or transferencia de estado representacional API potencialmente sensibles, protege estas claves y compártelas solo con partners de confianza. Nunca deben exponerse públicamente. Por ejemplo, no uses esta clave para hacer llamadas AJAX desde tu sitio web ni la expongas de ninguna otra manera pública.
...
{% alert warning %}
Dado que las claves de REST or transferencia de estado representacional API permiten el acceso a endpoints de REST or transferencia de estado representacional API potencialmente sensibles, asegúrate de que se almacenen y usen de forma segura. Por ejemplo, no uses esta clave para hacer llamadas AJAX desde tu sitio web ni la expongas de ninguna otra manera pública.
{% endalert %}
...
### Seguridad de las claves de REST or transferencia de estado representacional API y las claves de API de SDK or kit de desarrollo de software {#security-of-rest-api-keys-and-sdk-api-keys}

Las claves de REST or transferencia de estado representacional API y las claves de API de SDK or kit de desarrollo de software tienen perfiles de seguridad diferentes.

| | Claves de REST or transferencia de estado representacional API | Claves de API de SDK or kit de desarrollo de software |
|---|---|---|
| Propósito | Autenticación del lado del servidor para la REST or transferencia de estado representacional API (envío de mensajes, exportación de datos, gestión de usuarios) | Identificación del lado del cliente para el SDK or kit de desarrollo de software de Braze (ingesta de datos, mensajes dentro de la aplicación, Content Cards) |
...
| Solución si se exponen | Revoca la clave inmediatamente y crea un reemplazo en **Configuración** > **API e identificadores** > **Claves de API**. Una clave de REST or transferencia de estado representacional API expuesta puede usarse para enviar mensajes, exportar datos de usuario o modificar la configuración de la cuenta. | No se requiere ninguna acción. Una clave de API de SDK or kit de desarrollo de software solo puede ingestar datos y recuperar mensajería del lado del cliente (como mensajes dentro de la aplicación y Content Cards). No puede exportar datos de usuario, enviar mensajes en tu nombre ni modificar Campaigns. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 aria-label="Seguridad de las claves de REST or transferencia de estado representacional API y las claves de API de SDK or kit de desarrollo de software" }

### Lista de IP permitidas para la API {#api-ip-allowlisting}

Para mayor seguridad, puedes especificar una lista de direcciones IP y subredes que tienen permitido hacer solicitudes de REST or transferencia de estado representacional API para una clave de REST or transferencia de estado representacional API determinada. Esto se conoce como lista de permitidos o lista blanca. Para permitir direcciones IP o subredes específicas, agrégalas a la sección **IPs de la lista blanca** al crear una nueva clave de REST or transferencia de estado representacional API:
...
Braze autentica las solicitudes de la REST or transferencia de estado representacional API usando la clave de API REST or transferencia de estado representacional pasada como un token Bearer en el encabezado de solicitud `Authorization`. Cuando envías una solicitud, incluye tu clave de API en el siguiente formato:
...
1. **Validez del token:** Verifica que la clave de API REST or transferencia de estado representacional existe en Braze y está activa (por ejemplo, que no ha sido revocada ni deshabilitada).
...
Braze también acepta cualquier formato de mayúsculas y minúsculas del esquema `Bearer` (`Bearer`, `bearer` o `BEARER`). Envía la clave de API REST or transferencia de estado representacional exactamente como fue emitida.

### Seguridad a nivel de red {#network-level-security}

Las solicitudes de la REST or transferencia de estado representacional API a Braze están protegidas mediante cifrado Transport Layer Security (TLS) a lo largo de toda la ruta de la solicitud. La siguiente tabla describe el flujo de red de una solicitud de API desde tu servidor hasta Braze:
