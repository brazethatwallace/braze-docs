...
- **User-initiated dismissals:** Let users tap a built-in close button to dismiss a Banner. The Banner collapses immediately, and Braze syncs the dismissal so it stays hidden for that user across devices and sessions. See [Configure dismissal behavior]({{site.baseurl}}/user_guide/channels/banners/create_a_banner/#dismiss-behavior).
