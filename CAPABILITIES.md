...
| [`log-ux-debt`](.github/skills/log-ux-debt/SKILL.md) | Logs a UX Debt Jira ticket when a UI label, tooltip, or permission name in platform source does not match the documented copy — handles CODEOWNERS lookup, duplicate detection, and ticket creation with acceptance criteria | When `reference-repos` or `docs-discrepancies` surfaces a copy inconsistency and you want to file it with the owning team | `/log-ux-debt` | `Skill("anthropic-skills:log-ux-debt")` |
...
| Filing a UX Debt Jira ticket for a copy inconsistency found in platform source | `log-ux-debt` |
