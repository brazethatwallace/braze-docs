...
Customers can now set a `null` value for an email message's "reply-to" address from the **Email Settings** page in Braze or using the [API]({{site.baseurl}}/api/endpoints/messaging).  When used, replies will be sent to the listed "From" address.  You can now personalize the "From" address field as `dan@emailaddress.com`, and your customers will have the ability to reply directly back to Dan.
