...
4. Add [teams](https://braze.com/docs/user_guide/administrative/app_settings/manage_your_braze_users/teams/) and [tags](https://braze.com/docs/user_guide/administrative/app_settings/tags/) as needed.
   * Tags make your campaigns easier to find and build reports out of. For example, when using the [Report Builder](https://braze.com/docs/user_guide/analytics/reporting/report_builder/), you can filter by particular tags.

{: start="5"} 
5. Add and name as many variants as you need for your campaign. You can choose different platforms, message types, and layouts for each of your added variants. For more on this topic, refer to [Multivariate and A/B testing](https://braze.com/docs/user_guide/engagement_tools/testing/multivariant_testing/).
...
{: start="6"} 
6. Select an RCS-enabled [subscription group](https://braze.com/docs/sms_rcs_subscription_groups/). When selecting a subscription group, Braze will automatically add a segmenting filter, ensuring that only users subscribed will receive the campaign. Only long codes and short codes that belong to that subscription group will be used to send SMS to target users.
...
![Options to select from an RCS or SMS/MMS message type.]({% image_buster /assets/unlisted_docs/img/rcs/rcs_message_type.png %}){: style="max-width:65%;"}
...
{% endtab %}
{% tab Canvas %}
1. [Create your Canvas](https://braze.com/docs/user_guide/engagement_tools/canvas/create_a_canvas/create_a_canvas/) using the Canvas composer.
...
4. Select an RCS-enabled [subscription group](https://braze.com/docs/sms_rcs_subscription_groups/). When selecting a subscription group, Braze will automatically add a segmenting filter, ensuring that only users subscribed will receive the campaign. Only long codes and short codes that belong to that subscription group will be used to target users.
- **SMS fallback**: Braze strongly recommends that every subscription group that contains an RCS sender also includes at least one SMS code for fallback. This is important for deliverability in cases RCS messages fail to deliver. Some reasons for this may include user device incompatibility and incomplete carrier coverage in a given country or region. By enabling SMS fallback, your message will still deliver to your user and that you never miss that opportunity to connect with them.
...
During campaign and Canvas creation, choose from three RCS message types (Text, Media, Rich Card) to configure messages that best suit your goals.

![Options to select from a Text, Media, or Card message type.]({% image_buster /assets/unlisted_docs/img/rcs/rcs_text_media.png %}){: style="max-width:65%;"}
...
![A GIF of three Suggested Actions for an RCS message promoting trending fashion styles: "Fairytale royalty", "Edgy academia", and "Show me your other styles".]({% image_buster /assets/unlisted_docs/img/rcs/rcs_suggested_actions.gif %}){: style="max-width:70%;"}
...
![RCS composer with an option to upload a media file.]({% image_buster /assets/unlisted_docs/img/rcs/rcs_media_type.png %})
...
{% alert important %}
Rich Cards are in early access. Contact your Braze customer success manager if you're interested in participating in this early access.
{% endalert %}

A Rich card combines media, text, and buttons into a single message, creating a more intuitive and engaging experience for your customers. You can create two sub-types of Rich Cards: Text and Media.

{% subtabs %}
{% subtab Text %}
A Text Rich Card is a concise message focused on text. It must include the following elements:

- **Title:** Up to 200 characters. Can be personalized with Liquid.
- **Description:** Up to 2,000 characters. Can be personalized with Liquid.
- **Buttons:** At least one button is required. You can add up to four buttons with **Suggested reply** or **Open web URL** actions.

{% endsubtab %}
{% subtab Media %}

A Media Rich Card is a visual message containing an image or video. It must include the following elements:

- **Media:** An image, GIF, or video.
...
### Features
- **Card buttons** and **Suggestions:** You can add up to four buttons and five suggestions (up to 25 characters each) at the bottom of the Rich Card (buttons) or at the bottom of the message screen (suggestions). A user can select these click-tap options to send a specific response or take a specific action.
- **Personalization:** You can use Liquid to personalize all Rich Card elements, including the title, description, media, and buttons.
- **Billing:** Rich Cards are billed as a single rich (or “single”) RCS message.
- **URL guidance:** URLs entered as plain text in the title or description will not be clickable. You must use an **OpenURL button** to direct users to a website.

![Panel with options to select a Media or Text Rich card.]({% image_buster /assets/unlisted_docs/img/rcs/rcs_media_text.png %}){: style="max-width:65%;"}
...
Write your message using languages and personalization ([Liquid](https://braze.com/docs/user_guide/personalization_and_dynamic_content/liquid/), [Connected Content](https://braze.com/docs/user_guide/personalization_and_dynamic_content/connected_content/), and emojis) as needed. Be sure to adhere to our message copy limits to reduce your chances of overage charges.

{% alert important %}
Before proceeding, read our [guidelines for RCS message limits](#step-2-select-your-rcs-message-type). RCS messages are [charged per message](https://braze.com/docs/sms_rcs_billing_calculators/), so it’s a good idea to understand the nuances of what can be included in each type of RCS message.
{% endalert %}
...
RCS messages can be delivered based on a scheduled time, an action, or an API trigger. For more, refer to [Scheduling your campaign](https://braze.com/docs/user_guide/engagement_tools/campaigns/building_campaigns/delivery_types/).
...
Next, you'll select the larger audience from your segments and narrow that segment further with optional [filters](https://braze.com/docs/user_guide/engagement_tools/segments/segmentation_filters/). You'll automatically be given a preview of what that approximate segment population looks like right now. Keep in mind that exact segment membership is always calculated just before the message is sent.

{% alert tip %}
Interested in using RCS retargeting to target users based on their SMS and RCS interactions? Refer to [Retargeting](https://braze.com/docs/sms_mms_rcs_user_retargeting/).
{% endalert %}
...
Next, refer to [Reporting for SMS, MMS, and RCS](https://braze.com/docs/sms_mms_rcs_reporting/) to learn how you can access the results of your RCS campaigns.

## Analytics and reporting

Your campaign or Canvas analytics include:

- _Total Click_ stats that include all interactions with the Rich Card, such as button clicks and Suggested Replies or actions.
- A breakdown table that provides a more detailed view of these interactions.

{% alert note %}
The early access doesn't include user-level click tracking. _Total Clicks_ will increment each time a button is clicked. For example, if a user clicks the same button three times, the click count will increase by three.
{% endalert %}
...
Need help creating engaging copy? Try using the [AI copywriting assistant](https://braze.com/docs/user_guide/brazeai/operator/capabilities/#generate-copy). Input a product name or description, and the AI will generate human-like marketing copy for use in your messaging.

![Message composer with an icon to open the AI copywriting assistant.]({% image_buster /assets/unlisted_docs/img/rcs/rcs_ai_copywriter.png %}){: style="max-width:70%;"}
