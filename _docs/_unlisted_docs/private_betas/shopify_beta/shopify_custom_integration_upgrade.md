---
nav_title: Upgrading Shopify (Custom)
article_title: "Upgrading Your Custom Shopify Integration"
description: "Learn how to upgrade your custom Shopify integration for Braze."
page_type: partner
search_tag: Partner
...
# Upgrading your Shopify integration (custom)

> Learn how to upgrade your Shopify integration using the custom path for Braze. As part of our commitment to provide you with the best possible experience, we are requiring all Shopify integrations to [upgrade]({{site.baseurl}}/shopify/) to the latest version by August 28, 2025. This upgrade is essential because significant changes in Shopify's technology will impact how our integration functions.
...
| **Upgrade prerequisites** | Ensure you've completed all necessary [upgrade prerequisites]({{site.baseurl}}/shopify_upgrade_overview#upgrade-prerequisites) with your engineering and marketing teams. To upgrade your Shopify headless store with Braze, you need to complete two critical steps:<br><br>- Initialize and load the Braze Web SDK to enable onsite tracking<br>- Upgrade your existing store through the in-product upgrade experience |
...
![Panel with an option to start the upgrade.]({% image_buster /assets/unlisted_docs/img/shopify/start_shopify_custom_upgrade.png %}){: style="max-width:35%;"}
...
### Step 1: Initialize and load the Braze Web SDK to enable onsite tracking {#step-1}

If you haven't already, initialize and load the Braze Web SDK to enable onsite tracking. For a full walkthrough, refer to [Shopify custom integration setup](https://www.braze.com/docs/partners/ecommerce/shopify/shopify_custom_integration/#step-1): 
- Create a Braze web app 
- Add subdomain and environmental variables 
- Enable onsite tracking 
- Add a Shopify Account Login event 
- Add tracking for Product Viewed and Cart Update events 
...
Agree to the upgrade guidelines by checking the box, then select **Start the upgrade**.

![Modal to confirm that you understand upgrading may cause breaking changes.]({% image_buster /assets/unlisted_docs/img/shopify/confirm_upgrade.png %}){: style="max-width:50%;"}

Verify with your developers that you have completed Step 1 of the custom path upgrade by checking the box, then select **Confirm**. 

![Modal with a box to verify that you completed steps one to 5.]({% image_buster /assets/unlisted_docs/img/shopify/confirm_completed_steps.png %}){: style="max-width:50%;"}

{% alert important %}
For the integration to work correctly, make sure you complete [Step 1](#step-1) of the custom upgrade. If you skip this step, the integration may not function properly.
{% endalert %}
...
![Panel with an option to go to Shopify.]({% image_buster /assets/unlisted_docs/img/shopify/custom_go_to_shopify.png %}){: style="max-width:35%"}
...
![Shopify upgrade panel with a spinning icon next to "Reauthorize the Braze app".]({% image_buster /assets/unlisted_docs/img/shopify/reauthorize_app_loading.png %}){: style="max-width:35%;"}

### Step 4: Choose an external ID type

The external ID type you choose will be assigned to new Shopify customer profiles when either a Shopify account is created or an order is placed. It will also be used to update existing user profiles if they already have a Shopify customer ID alias but aren’t assigned an external ID in Braze.

To choose your external ID type, go back to Braze and select **Confirm external ID**.

![Shopify upgrade panel with a button to confirm the external ID.]({% image_buster /assets/unlisted_docs/img/shopify/custom_confirm_external_id.png %}){: style="max-width:35%;"}
...
![Modal with a dropdown to select the external ID.]({% image_buster /assets/unlisted_docs/img/shopify/external_id_custom.png %}){: style="max-width:50%;"}
...
To enable the Braze app embed within your store’s theme, back to Braze, select Go to Shopify.

![Shopify upgrade panel with a button to enable the Braze app embed.]({% image_buster /assets/unlisted_docs/img/shopify/custom_enable_app_embed.png %}){: style="max-width:35%;"}
