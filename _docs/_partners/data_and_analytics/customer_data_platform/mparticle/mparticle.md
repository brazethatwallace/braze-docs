...
1. **Connection output:** Confirm Braze is enabled as an output for the relevant connection and that the correct Braze instance, app identifier, and REST API key are configured.
2. **Identity mapping:** Server-to-server and audience syncs require an `external_id`. Anonymous users are not forwarded.
3. **Event mapping:** Verify events are routed to the Braze output and that unsupported data types (nested objects, arrays in event properties) are not being dropped.
