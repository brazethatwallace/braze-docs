...
With the updated customers and associated attributes, Peak will POST these to Braze using the [`/user/track` endpoint]({{site.baseurl}}/api/endpoints/user_data/post_user_track/), utilizing the [bulk]({{site.baseurl}}/api/endpoints/user_data/post_user_track#need-to-update-users-in-bulk) header.
