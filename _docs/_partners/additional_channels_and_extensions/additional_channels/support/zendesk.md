...
![Braze webhook settings showing HTTP method and Zendesk authorization headers.]({% image_buster /assets/img_archive/zendesk_step1.gif %}){: style="max-width:70%;"}
...
3. Select **Trigger** or **Automation** and click **Next**.<br>![Zendesk webhook creation flow with Trigger or Automation options.]({% image_buster /assets/img_archive/zendesk2.png %}){: style="max-width:70%;"}<br><br>
...
Our example uses a trigger to invoke the webhook when a support case status has been changed to "Solved" or "Closed". 
...
4. Select **Add condition** to set up which conditions should trigger the webhook. For example, "Status category changed to closed" or "Status category changed to solved".![Zendesk trigger conditions editor for status-based webhook firing.]({% image_buster /assets/img_archive/zendesk1.png %}){: style="max-width:70%;"}<br><br>
...
6. Define the JSON body to conform to your Braze endpoint, using Zendesk variable placeholders to dynamically populate the relevant fields.<br>![Zendesk webhook action body editor with JSON payload placeholders.]({% image_buster /assets/img_archive/zendesk3.png %}){: style="max-width:70%;"}<br><br>
