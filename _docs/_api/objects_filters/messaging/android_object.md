...
| Parameter | Details |
| --------- | ------- |
| `priority` | This parameter accepts values from `-2` to `2`, where `-2` represents "MIN" priority and `2` represents "MAX". `0` is the "DEFAULT" value. <br> <br> Any values sent outside of that range default to 0. For more information on which priority level to use, see [Android notification priority]({{site.baseurl}}/developer_guide/push_notifications/customization?sdktab=android#android_settings). |
...
| `notification_channel` | If this is not specified, Braze attempts to send the notification payload with the [dashboard fallback]({{site.baseurl}}/user_guide/channels/push/platform_specific_resources/android/notification_channels#dashboard-fallback-channel) channel ID. For more, see [Notification channels]({{site.baseurl}}/user_guide/channels/push/platform_specific_resources/android/notification_channels) and refer to the steps for [defining notification channels]({{site.baseurl}}/developer_guide/push_notifications?sdktab=android) during integration. |
| `send_to_sync` | For more information on `send_to_sync` messages, see [silent Android notifications]({{site.baseurl}}/developer_guide/push_notifications/silent?sdktab=android). |
