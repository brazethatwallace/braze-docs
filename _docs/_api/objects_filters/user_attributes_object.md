...
Braze does not provide a dedicated spam list in the dashboard. Braze blocks individual users with over 5 million sessions ("dummy users") and no longer ingests their SDK events. If an identifier is blocked, API requests may return `"provided external_id is blacklisted and disallowed"` (see [`/users/track`]({{site.baseurl}}/api/endpoints/user_data/post_user_track/)). To find affected profiles, create a [segment]({{site.baseurl}}/user_guide/audience/segments/creating_a_segment/) with the **Session Count** filter set to **more than 5,000,000**, export the segment via CSV, and cross-check profile fields in **Engagement** > **Search users** or with the [`/users/export/ids`]({{site.baseurl}}/api/endpoints/export/user_data/post_users_identifier/) endpoint.