...
| `department` | Optional | String | Valid department string from the [department string documentation]({{site.baseurl}}/api/objects_filters/scim_api_appendix/?sdktab=granular%20scim%20api#granularscimapi_department-strings). Omit or pass an empty string if unknown. |
| `permissions` | Optional | JSON object | Permissions object as described in the [permissions object documentation]({{site.baseurl}}/api/objects_filters/scim_api_appendix). When omitted or empty, Braze assigns default read-only workspace permissions on the default workspace configured in **SCIM Provisioning**. After granular permissions migration, this includes view-only workspace permissions. |
...
| `department` | String | Valid department string from the [department string documentation]({{site.baseurl}}/api/objects_filters/scim_api_appendix). |
| `permissions` | JSON object | Permissions object as described in the [permissions object documentation]({{site.baseurl}}/api/objects_filters/scim_api_appendix). |
| `id` | String | ID generated by Braze that is used for searching and managing user accounts. |
| `createdAt` | String | Date the user account was created in UTC. Returns `NA` when unset. |
| `lastSignInAt` | String | Date of last successful sign-on in UTC. Returns `NA` when the user has not signed in. Format: `YYYY Mon DD, H:MM AM/PM`. |
