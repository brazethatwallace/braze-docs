...
> Use this endpoint to link one Braze user to one custom object.
...
To use this endpoint, you need an [API key]({{site.baseurl}}/api/basics#rest-api-key-permissions) with `custom_objects.user_relationships.create`.
...
| `403` | API key lacks permission or request is blocked by allowlist | Confirm the key has `custom_objects.user_relationships.create` and that your source IP is on the key allowlist, if configured. |
