...
|`campaign_id`| Optional* | String | See [campaign identifier]({{site.baseurl}}/api/identifier_types#campaign-identifier) for more information. <br><br>*Required to track campaign metrics (such as _Sends_, _Clicks_, or _Bounces_) on the Braze dashboard, or to see events associated with this message in the user profile [Message History tab]({{site.baseurl}}/user_guide/engagement_tools/segments/user_profiles#messaging-history-tab). Without a `campaign_id`, Braze doesn't increment dashboard deliverability stats. Sends still appear in the [Message Activity Log]({{site.baseurl}}/user_guide/administer/global/workspace_settings/logs_and_alerts/message_activity_log), but not in email performance metrics in the dashboard. |
