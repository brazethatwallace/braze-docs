...
- If the known user has an email address that is flagged as having [hard bounced]({{site.baseurl}}/user_guide/channels/email/reporting/analytics_glossary/#hard-bounce), we'll check to see if the newly-entered email address differs to what's on their Braze profile. If the email address provided is different, the email address will be updated and the hard bounce status will be removed. 
