...
MMS availability and rendering depend on the receiving carrier. When a carrier doesn't accept MMS, Braze converts the media to a link in the message body. Avoid sending MMS to Google Voice numbers because its limited MMS support can cause unreliable delivery.
