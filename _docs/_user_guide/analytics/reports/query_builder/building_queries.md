---
nav_title: Build queries
article_title: Build queries in the Query Builder
page_order: 0
page_type: reference
description: "This reference article describes how to build reports using Braze data from Snowflake in the Query Builder."
tool: Reports
---

# Build queries in the Query Builder

> This reference article describes how to build reports using Braze data from Snowflake in the Query Builder.

{% multi_lang_include brazeai/generative_ai/building_queries.md %}