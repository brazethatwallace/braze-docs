...
By selecting the maximum user limit, you can limit the volume of messages sent on a per-channel basis or globally across all message types. Note that users assigned to Control Groups are not dispatched, so they do not count toward the limit.
