...
| Campaign or Canvas has no quiet hours window of its own, and a workspace quiet hours window exists for the channel it uses | The workspace quiet hours window applies automatically. |
| Campaign / Canvas is opted out | No quiet hours apply. |
...
To send during a workspace quiet hours window, include the optional `ignore_workspace_quiet_hours` parameter in your request.
