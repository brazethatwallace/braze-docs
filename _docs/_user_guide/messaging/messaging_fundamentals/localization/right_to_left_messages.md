...
For **push notifications**, title alignment generally follows the device language settings, while body alignment can follow the **first strong directional character on each line** (treat each line after a newline separately). That means a single push can mix alignment across lines—for example, a right-to-left body line followed by a left-to-right line. When you need predictable layout, keep each logical line directionally consistent and use line breaks between mixed-language segments.
