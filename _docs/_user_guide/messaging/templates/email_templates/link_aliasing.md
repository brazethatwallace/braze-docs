...
For URLs that are generated by Liquid, such as `assign` statements in the HTML or from a Content Block, you must append a question mark (`?`) to the Liquid tag. For Content Blocks, use question mark `?` if the URL has no existing query parameters, or ampersand `&` if it already has parameters. This allows Braze to append query parameters (`lid = somevalue`) so that link aliasing can work properly. Without identifying where to append query parameters, link aliasing will not recognize these URLs and link templates won't apply.
...
- Click reporting may look inaccurate for messages that were launched while the feature was on because clicks recorded with link alias tracking are aggregated differently than clicks without it.
