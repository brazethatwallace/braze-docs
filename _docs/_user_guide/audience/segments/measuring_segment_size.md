...
{% alert note %}
**Total reachable users** includes everyone who matches your segment filters, even if they are no longer subscribed to a channel (for example, they unsubscribed from push but still match the segment). Channel rows such as **iOS** only count users who are reachable on that channel under the rules in [Reachable users by channel](#reachable-users-by-channel). To line segment totals up with subscribed, push-eligible users, add filters such as **Push enabled for iOS** is true (or the equivalent for your channel).
{% endalert %}
