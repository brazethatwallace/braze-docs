...
- **Local time and Intelligent Timing campaigns:** Campaigns and Canvases that use a user's local time or [Intelligent Timing]({{site.baseurl}}/user_guide/messaging/campaigns/schedule_your_campaign/scheduled_delivery#option-3-intelligent-timing) for delivery continue to function as before and are not affected by workspace time zones.
