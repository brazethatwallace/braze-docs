...
function embedCodeToTarget(targetDiv, codeText, showBorder, showLineNumbers, showFileMeta, showCopy, isDarkStyle, fileURL, rawFileURL, lang, startLine, endLine, tabSize, serviceProvider) {
  ...
  if (showFileMeta) {
    ...
    meta.innerHTML = `<a target="_blank" href="${rawFileURL}" style="float:right">view raw<span class="sr-only"> (opens in new tab)</span></a>
<a target="_blank" href="${fileURL}">${rawFileURLSplit[rawFileURLSplit.length - 1]}<span class="sr-only"> (opens in new tab)</span></a>
delivered <span class="hide-in-phone">with <i class="fa-solid fa-heart"></i> </span>by <a target="_blank" href="${serviceProvider}">EmGithub<span class="sr-only"> (opens in new tab)</span></a>`;
