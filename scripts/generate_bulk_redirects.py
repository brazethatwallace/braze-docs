...
Client-side-only entries (source URLs with # fragments) are skipped because HTTP
requests do not include the fragment; those remain in broken_redirect_list.js.
...
def parse_validurls(js_text: str) -> dict[str, str]:
    ...
    for raw_source, raw_dest in VALIDURLS_RE.findall(js_text):
        source = unescape_js_string(raw_source)
        dest = unescape_js_string(raw_dest)
        entries[source] = dest
    return entries
