...
- **Liquid tags**: `{% ... %}` and `{{ ... }}` — copy exactly, including all parameters, whitespace, and hyphens. For **`{% image_buster /path/to.png %}`**, there must be a **space** between `image_buster` and the path — never fuse them as `{% image_buster/path... %}`; that form is invalid Liquid and breaks images (Copilot PR #13318).
...
- **One language for bold controls inside a single procedure**: Within **one** numbered or bulleted step list, every bold `**…**` string that names a **tab, button, breadcrumb, or composer control** must follow a **single** policy: **either** keep **all** of those tokens **verbatim** from the English source (same spelling and casing as the US UI in the English file), **or** localize **all** of them together per the appended style guide and the **existing translation** of the same snippet when one is provided. **Never** output a mix (for example localized **Zeilen** beside English **Done**, **Rows**, and **Launch Content Block** in the same list). Mixed lists confuse readers and fail review (PR #13335). When the locale already fully localized that snippet historically, stay consistent with that file; when starting from English-only bold UI with no locale precedent, default to verbatim English bold for every control in that list.
