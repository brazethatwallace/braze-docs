...
### Cross-section consistency for IA-moved files

- Braze's information architecture is in flux: English files frequently move between `_docs/_user_guide/engagement_tools/…`, `_docs/_user_guide/messaging/…`, `_docs/_user_guide/personalization/…`, etc. When the system prompt includes a **"Cross-section consistency (existing locale translations)"** section, the target locale already ships one or more pages with the **same file basename** as the one you are about to translate — they cover the same Braze concept at the old IA path (or at a sibling hub). In that case:
  - Reuse the sibling page's **`nav_title`**, **`article_title`**, **`title`**, **`description`**, and (when present) **`guide_top_header`** values **verbatim**. Copy the wording and quoting style exactly — do not paraphrase and do not invent a second translation for the same concept.
  - Align body **glossary terms** with the sibling so a reader jumping between sections sees the same labels. If the sibling calls the feature **Otimizador de Conteúdo**, **フィーチャーフラグ**, or **キャンバス**, your new translation must use the same string — do not leave bare English in `nav_title` / headings when the locale has already chosen a localized name.
  - This rule applies to **every** IA-moved page (for example `messaging/canvas.md` paired with `engagement_tools/canvas.md`, `messaging/feature_flags.md` paired with `engagement_tools/feature_flags.md`, channel landings, etc.), not just one specific file.
- If **no** sibling context is provided for this file but the target locale clearly already has an established translation for the concept (known Braze feature name), err on the side of reusing that established translation rather than introducing a new variant.
