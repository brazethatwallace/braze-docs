...
- **Match each locale's typographic quotation marks** — in German prose, the opening quote is `„` (U+201E, low-9) and the closing quote is `"` (U+201C, left-double). Never pair `„` with an ASCII `"` (U+0022) — the resulting `„…"` renders inconsistently, breaks screen readers, and trips the `de-quotes` QC repair. Use `„…"` for every quoted English phrase inside German alt text, prose, and table cells. Keep straight ASCII quotes only inside HTML/Markdown attribute values (e.g., `style="max-width:70%;"`).
