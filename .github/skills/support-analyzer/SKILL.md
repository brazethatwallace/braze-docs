...
|------|----------------|----------------|
| **CI — Support analyzer (Looker)** | Looker → CSV on `support-analyzer-data` → digest artifact → optional **Phase 2** draft PRs from [`.github/support_analyzer_phase2_rules.yml`](.github/support_analyzer_phase2_rules.yml) | Recurring themes; allowlisted inserts; no per-ticket judgment |
...
**Order of jobs:** `export` → `digest` → `phase2_doc_prs` → `notify` (Slack).

1. **Export** — Secrets `LOOKER_CLIENT_ID`, `LOOKER_CLIENT_SECRET`. Optional repository **variable** `LOOKER_SUPPORT_CASES_QUERY_ID` (saved Look id; date filters stay in Looker). Pushes `_data/support_cases_latest.csv` to branch **`support-analyzer-data`** (CSV only on that branch; the export script and `_data/pii_patterns.yml` are taken from the workflow ref, usually **`develop`**). Requires `SUPPORT_ANALYZER_EXPORT_ACKNOWLEDGE_SENSITIVE_DATA=1` in Actions (CSV may contain PII). The export script **redacts embedded credential-like strings** (patterns in `_data/pii_patterns.yml` — for example AWS access key IDs, SendGrid API keys, Twilio account/API key SIDs, GitHub/Slack/Stripe tokens) before write so GitHub **push protection** does not reject the data-branch push, and **fails** on zero rows or a sharp row-count drop vs. the prior export. Add or change redaction patterns in that YAML file only; no Python change required. **Local API export:** with `pip install requests pyyaml`, set the same env vars and run `python scripts/export_support_cases_from_looker.py`; optional `SUPPORT_ANALYZER_OUTPUT`, `LOOKER_BASE_URL`, `LOOKER_SUPPORT_CASES_QUERY_ID`.
2. **Digest** — Keyword digest to `.github/support_analyzer_weekly_digest.md`; workflow artifact **`support-analyzer-weekly-digest`** (30-day retention). No pull request—the digest is internal triage output only. **Schedule:** roughly twice weekly (Tuesday and Friday mornings, `America/New_York`) to align with the Looker export’s rolling ~3-day window; see workflow `cron` (see [Schedule events](https://docs.github.com/en/actions/using-workflows/events-that-trigger-workflows#schedule)).
3. **Phase 2** — Checks out the **triggering ref** (`github.ref`: default branch on schedule, selected branch on **`workflow_dispatch`**), then runs `scripts/support_analyzer_phase2.py` with **`--strict-anchors`** against the same CSV. May open **draft** PRs (label **`support analyzer`**, base **`develop`**, branch `support-analyzer/phase2-<rule_id>-<YYYY-MM-DD>-<run_id>`). Skips opening another draft for the same rule while an **open** PR already uses that branch prefix (dedup **fails closed** if `gh pr list` errors). Rule edits must target paths under **`_docs/`** only. Skips per-file edits when **`skip_if_contains`** or **`skip_if_contains_in_files`** already match on **`develop`** (each rule's `skip_if_contains` phrases should match merged prose so later runs do not duplicate); each PR body starts with a **stakeholder blurb** explaining automation. **Assignees:** paths touched by the rule are matched against [`.github/support_analyzer_doc_assignees.csv`](.github/support_analyzer_doc_assignees.csv) (export your spreadsheet to that shape: `page path`, `writer`, `team`, `github username`). Longest path prefix wins; multiple files can yield multiple assignees. The **GitHub Username** cell may contain **comma-separated** logins (for example `lydia-xie,zairro`) to assign more than one person for the same path. If `gh pr create --assignee` fails (invalid username or permissions), the script retries **without** assignees. Artifact **`support-analyzer-phase2-verification`**. Skipped when **`workflow_dispatch`** input **Skip Phase 2** is true (export + digest still run).
4. **Slack (`notify`)** — Posts to **#docs_request** via Docs PR Bot when configured (`SLACK_BOT_TOKEN`, `SLACK_DOCS_REQUEST_CHANNEL` channel ID; optional fallback `SLACK_DEPLOY_NOTIFY_CHANNEL`). **Failures** in export, digest, or Phase 2 post `:x:` with the workflow run link. **Success with Phase 2 doc PRs** lists each draft PR and **@mentions** assignees (Slack member lookup by GitHub username, with optional `SUPPORT_ANALYZER_GITHUB_TO_SLACK` JSON override). Runs that open no doc PRs stay quiet in Slack.
...
**Phase 2 rules CI:** Pull requests that change `.github/support_analyzer_phase2_rules.yml` or `scripts/support_analyzer_phase2.py` run the **Validate support analyzer Phase 2** workflow: static rules validation (structure, regex compile, `_docs/` paths, anchors) plus a `--dry-run` smoke test against `.github/fixtures/support_analyzer_phase2_smoke.csv`.
